...
Функция ВыбранныеЛентыНовостей()
	...
	ЛентыНовостейДляПоказа.Добавить("fedlaw54");
	ЛентыНовостейДляПоказа.Добавить("biz360");
	ЛентыНовостейДляПоказа.Добавить("RTNews");
...
&НаСервере
Функция КороткийТекстДляБаннера(ТекстНовости)
	
	НачалоТекста = СтрНайти(ТекстНовости,"<div style = ""display: none"" id=""UNF_BannerText"">");
	
	Если НачалоТекста = 0 Тогда
		Возврат "";
	КонецЕсли;
	
	КонецТекста = СтрНайти(ТекстНовости,"</div>",,НачалоТекста);
	СтрокаДляЗамены = Сред(ТекстНовости, НачалоТекста, КонецТекста - НачалоТекста);
	СтрокаДляЗамены = СтрЗаменить(СтрокаДляЗамены, "style = ""display: none""","");
	СтрокаДляЗамены = СтрЗаменить(СтрокаДляЗамены, "<br/>","");
	
	НачалоНовости = СтрНайти(ТекстНовости,"<div id=""newsText"">");
	КонецНовости = СтрНайти(ТекстНовости,"</div>",,НачалоТекста);
	ПодстрокаНовости = Сред(ТекстНовости, НачалоНовости, КонецНовости - НачалоНовости);

	ТекстНовости = СтрЗаменить(ТекстНовости, ПодстрокаНовости, СтрокаДляЗамены);
	
	СтрокаДата = "<div id=""importanceData""> </div>";
	НачалоДаты = СтрНайти(ТекстНовости,"<div id=""importanceData"">");
	КонецДаты  = СтрНайти(ТекстНовости,"</div>",,НачалоДаты);
	СтрокаДляЗамены = Сред(ТекстНовости, НачалоДаты, КонецДаты + 6 - НачалоДаты);
	
	ТекстНовости = СтрЗаменить(ТекстНовости, 
		СтрокаДляЗамены, 
		"");
		
	НачалоЛинк = СтрНайти(ТекстНовости,"<div id=""newsHyperlink"">");
	КонецЛинк  = СтрНайти(ТекстНовости,"</div>",,НачалоЛинк);
	СтрокаДляЗамены = Сред(ТекстНовости, НачалоЛинк, КонецЛинк + 6 - НачалоЛинк);
	
	ТекстНовости = СтрЗаменить(ТекстНовости, 
		СтрокаДляЗамены, 
		"");
		
	НачалоСтили = СтрНайти(ТекстНовости,"/* Здесь можно вставить свои стили");
	КонецСтили = СтрНайти(ТекстНовости,"*/",,НачалоСтили);
	СтрокаДляЗамены = Сред(ТекстНовости, НачалоСтили, КонецСтили + 2 - НачалоСтили);
