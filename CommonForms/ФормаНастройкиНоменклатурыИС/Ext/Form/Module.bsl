...
Процедура НастроитьЭлементыФормыНаСервере()
	...
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.НикотиносодержащаяПродукция Тогда
		...
		Тексты.Подвал                       = НСтр("ru = 'При продаже потребительской упаковки на ККТ тэг <1291> не заполняется, потребительская упаковка выбывает из оборота.'");
		...
		Если ВариантЧастичногоВыбытия = Перечисления.ВариантыУчетаЧастичногоВыбытияИС.НастроеннаяНоменклатура
			...
			ДанныеПодвала = Новый Массив();
			ДанныеПодвала.Добавить(НСтр("ru = 'При поштучной продаже единиц продукции из вскрытой маркируемой потребительской упаковки на ККТ в чеке заполняется Тэг <1291>, который в печатной форме чека отображается как: <количество единиц>/'"));
