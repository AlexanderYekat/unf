...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗаказВШапке И Контрагент.ВестиРасчетыПоЗаказам Тогда // заказ в шапке
		...
		НоваяСтрока.ВсегоРасч = ЦенообразованиеСервер.ПересчитатьИзВалютыВВалюту(
		   ...
		   И ТипЗнч(Параметры.Заказ) = Тип("Массив") Тогда
			ТаблицаЗаказов = СписокЗаказов.Выгрузить();
 			Для каждого ЭлементМассива Из Параметры.Заказ Цикл
				...
				СтрокаЗаказов.ВсегоРасч = ЦенообразованиеСервер.ПересчитатьИзВалютыВВалюту(
			...
			// Для учета авансов без заказа
			СтрокаЗаказов = ТаблицаЗаказов.Добавить();
			СтрокаЗаказов.Заказ = Заказ;
	...
	Если НЕ Параметры.Подбор Тогда
		...
		Заголовок = НСтр("ru = 'Восстановление предоплаты'");
	КонецЕсли;
	...
	Если Параметры.Свойство("ОтчетКомиссионера") Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ru = 'Зачет предоплаты (реализация)'")
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСписокАвансов
...
// Предоплата. Рассчитывает сумму платежа.
//
&НаКлиенте
...
Функция ПолучитьДанныеДокументПриИзменении(Документ)
	...
	Если ЭтоНакладныеИлиКорректировки Тогда
	...
	Иначе
		СуммаДокумента =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "РасшифровкаПлатежа").Выгрузить().Итог("СуммаРасчетов");
		СтруктураДанные.Вставить("СуммаРасчетов", СуммаДокумента);
	КонецЕсли;
...
Процедура ВыборАвансаФрагмент(СуммаРасчетов, Знач ТекущаяСтрока)
    ...
    Иначе
        ...
        Иначе
            НоваяСтрока.СуммаПлатежа = ВалютыУНФКлиентСервер.Пересчитать(
...
Процедура ЗаполнитьАвансы()
	...
	// Получим информацию о сумме аванса, если он был в валюте.
