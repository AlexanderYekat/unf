...
#КонецЕсли
	...
	ИначеЕсли ИмяСобытия = "ПрограммнаяПометкаУдаленияОтчета" Тогда
...
Процедура ВидЕГРЮЛНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	Элемент.СписокВыбора.ЗагрузитьЗначения(МассивОбъектов);
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Использование (отказ) организацией, зарегистрированной на Курильских острововах, права на освобождение от налога на прибыль"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.нал.на приб.Курилы", '20221015')
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Сведения о контролирующих лицах международной компании"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.уведомление КЛМГК", '20230101')
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Сведения о соблюдении международной компанией условий для применения пониженных налоговых ставок"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.собл.усл.пон.ставок", '20230101')
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Согласие налогоплательщика на информирование о наличии задолженности"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.согл.на инф.зад.5.02", '20230324')
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Информация об операциях по отчуждению ЦФА"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.опер.отчуждения ЦФА", '20230218')
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Заявление о выдаче справки о состоянии расчетов по налогам и сборам"
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Создание обособленных подразделений"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.ув.созд.об.подр.5.05")
				И ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЗначениеРеквизитаОбъекта(Ссылка, "ИмяФормы") =
				"Форма2023_1"
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Заявление о заявительном порядке возмещения НДС"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("заявительном порядке возм")
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Заявление об отзыве банковской гарантии"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("отзыве банковской гаранти")
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Заявление о замене банковской гарантии"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("замене банковской гаранти") Тогда
...
&НаСервере
Функция ДанныеПечатиУведомления(Ссылка)
	Результат = Новый Структура;
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "ВидУведомления,Организация,ИмяОтчета");
	Результат.Вставить("ЗаголовокФормы", "" + Реквизиты.ВидУведомления + " (" + Реквизиты.Организация + ")");
	СсылкаОбъект = Ссылка.ПолучитьОбъект();
	СписокПечатаемыхЛистов = ОбщегоНазначения.ОбщийМодуль("Отчеты." + Реквизиты.ИмяОтчета).СформироватьСписокЛистов(СсылкаОбъект);
	...
	КонтролирующийОрган = ?(УведомлениеОСпецрежимахНалогообложенияПовтИсп.ВидыУведомленийВФНС()[Реквизиты.ВидУведомления] = Истина,
	...
	ЭлектронныйДокументооборотСКонтролирующимиОрганами.ОпределитьСтатусИСостояниеСдачиОтчетности(
		Ссылка, Реквизиты.Организация, КонтролирующийОрган, СостояниеСдачиОтчетности, СтатусОтправки);
	СтатусОтправкиВЖурнале = УведомлениеОСпецрежимахНалогообложения.СохраненныйСтатусОтправкиУведомления(СсылкаОбъект);
	...
	Если МожноПечататьСЭЦП Тогда 
		ТекущееСостояние = РегламентированнаяОтчетность.ТекущееСостояниеОтправки(Ссылка, КонтролирующийОрган);
		...
		Если ТекущееСостояние <> Неопределено И ЗначениеЗаполнено(ЦиклОбмена) Тогда
			СтруктураРеквизитовФормы = Новый Структура("ПоследнийЦиклОбмена,мСохраненныйДок", ЦиклОбмена, Ссылка);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПечатьУведомлений(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТипЗнч(УведомлениеДляПечати) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
			...
			Попытка
				ДанныеПечатиУведомления = ДанныеПечатиУведомления(УведомлениеДляПечати);
				РегламентированнаяОтчетностьКлиент.ОткрытьФормуПредварительногоПросмотра(ЭтотОбъект, , Ложь, 
					ДанныеПечатиУведомления.СписокПечатаемыхЛистов, ДанныеПечатиУведомления);
		...
		ИначеЕсли ТипЗнч(УведомлениеДляПечати) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиРаспределенныйРеестр") Тогда
			...
			Если ТекущиеДанные.Наименование = НСтр("ru = 'Машиночитаемая доверенность (ФТС)'") Тогда
...
Процедура СообщитьПроНесоответствиеОтбору(Ссылка, ИмяРаздела)
	...
	ИначеЕсли Организация <> ЗначенияРеквизитов.Организация
			...
			И ИмяРаздела = НСтр("ru = 'ЕГРЮЛ'") Тогда
...
Процедура ОткрытьПрисоединенныеФайлы()
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ВозможноПрисоединениеФайловКОбъекту И ПараметрыПроверки.СтандартнаяОбработка Тогда
			...
			Если ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Отчеты") Тогда
				Если ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ФНС") Тогда
					ПоказатьПредупреждение(Оповещение,
						НСтр("ru = 'Обратите внимание, присоединенные файлы не отправляются при отправке отчета.
						|
			...
			Иначе // ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Уведомления")
				ПоказатьПредупреждение(Оповещение,
					НСтр("ru = 'Обратите внимание, присоединенные файлы не отправляются при отправке уведомления.
					|
...
Процедура ЗадатьСтатусОтветаНаТребование(СтрокаСписка, ПоддерживаетсяОплата, Ответы)
	...
	Если Данные.ЕстьОтвет Тогда
		...
		Если ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.ВходящийДокументСЭДОФСС") Тогда
			...
			КонецЕсли;
		Иначе
			Отправлено = КоличествоОтправленныхОтветовНаТребование(Данные.Ссылка, Ответы);
...
&НаКлиенте
Процедура ЗарегистироватьООО_ИП_ПослеВыбораОрганизации(ВыбраннаяОрганизация, ВидыУведомления) Экспорт
...
&НаКлиенте
Процедура ВыбратьОрганизациюИСоздатьУведомление(ВидыУведомления)
	
	Если ЗначениеЗаполнено(Организация) Тогда
		
		ЗарегистироватьООО_ИП_ПослеВыбораОрганизации(Организация, ВидыУведомления);
		
	Иначе
	
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ЗарегистироватьООО_ИП_ПослеВыбораОрганизации", 
			ЭтотОбъект,
			ВидыУведомления);
		
		ПоказатьВводЗначения(ОписаниеОповещения, , НСтр("ru = 'Выберите организацию'"), Тип("СправочникСсылка.Организации"));
