...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОтредактированаИстория" И ОрганизацияСсылка = Источник Тогда
		...
		//ИначеЕсли Параметр.ИмяРегистра = "НастройкиСистемыНалогообложения" Тогда
		//	
		//	Если НастройкиСистемыНалогообложенияНаборЗаписейПрочитан Тогда
		//		
		//		РедактированиеПериодическихСведенийКлиент.ОбработкаОповещения(
		//		ЭтотОбъект,
		//		ОрганизацияСсылка,
		...
		//		Источник);
		//		
		//		ОбновитьПолеУчетнаяПолитикаОрганизацийПериод(ЭтотОбъект);
		//		
		//	КонецЕсли;
...
Процедура СохранитьДанныеНаСервере(Отказ, ТекущаяСсылка)
	...
	Для Каждого Ресурс Из НастройкиЗарплатаКадрыРегламентированныйПрежние Цикл
		Если НастройкиЗарплатаКадры[Ресурс.Ключ] <> Ресурс.Значение Тогда
...
Процедура УправлениеФормой(Форма)
	...
	Иначе
		...
		Элементы.ИспользуетсяОтчетность.Видимость	= (Форма.КлючСистемыНалогообложения.ПлательщикЕНВД
			...
			ИЛИ Форма.КлючСистемыНалогообложения.ПрименяетсяАУСН) И Форма.ЭтоУНФ;
		...
		Элементы.ГруппаВыплатаЗарплатыИАванса.Видимость = Объект.ИспользуетсяОтчетность И Объект.ИПИспользуетТрудНаемныхРаботников;
		//БЗКБ
...
Процедура ЗаполнитьРеквизитыПоИНННаСервере(ИНН, ОписаниеОшибки = "")
	...
	Если ЭтоЮрЛицо Тогда
		...
		Если РеквизитыОрганизации.ЕГРЮЛ.ЮридическийАдрес <> Неопределено Тогда
			...
			СтруктураКИ.Представление = РеквизитыОрганизации.ЕГРЮЛ.ЮридическийАдрес.Представление;
			СтруктураКИ.Комментарий = РеквизитыОрганизации.ЕГРЮЛ.ЮридическийАдрес.Комментарий;
			СтруктураКИ.Значение = РеквизитыОрганизации.ЕГРЮЛ.ЮридическийАдрес.КонтактнаяИнформация;
			СтруктураКИ.Вид = Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации;
			ДобавляемаяКИ.Добавить(СтруктураКИ);
			
			СтруктураКИ = Новый Структура("Представление,Комментарий,Значение,Вид");
			СтруктураКИ.Представление = РеквизитыОрганизации.ЕГРЮЛ.ЮридическийАдрес.Представление;
			СтруктураКИ.Комментарий = РеквизитыОрганизации.ЕГРЮЛ.ЮридическийАдрес.Комментарий;
			СтруктураКИ.Значение = РеквизитыОрганизации.ЕГРЮЛ.ЮридическийАдрес.КонтактнаяИнформация;
			СтруктураКИ.Вид = Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации;
			ДобавляемаяКИ.Добавить(СтруктураКИ);
		КонецЕсли;
		
		Если РеквизитыОрганизации.ЕГРЮЛ.Телефон <> Неопределено Тогда
			СтруктураКИ.Представление = РеквизитыОрганизации.ЕГРЮЛ.Телефон.Представление;
			СтруктураКИ.Комментарий = РеквизитыОрганизации.ЕГРЮЛ.Телефон.Комментарий;
			СтруктураКИ.Значение = РеквизитыОрганизации.ЕГРЮЛ.Телефон.КонтактнаяИнформация;
			СтруктураКИ.Вид = Справочники.ВидыКонтактнойИнформации.ТелефонОрганизации;
			ДобавляемаяКИ.Добавить(СтруктураКИ);
		КонецЕсли;
		ДобавитьКонтактнуюИнформацию(ДобавляемаяКИ);
		...
		//	ДанныеЗаполненияКИ.Вид = Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации;
		//	
		//	СтрокаКИ = НоваяКонтактнаяИнформация.Добавить();
		//	ЗаполнитьЗначенияСвойств(СтрокаКИ, ДанныеЗаполненияКИ);
		//	
		//	ДанныеЗаполненияКИ.Вид = Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации;
		//	СтрокаКИ = НоваяКонтактнаяИнформация.Добавить();
		//	ЗаполнитьЗначенияСвойств(СтрокаКИ, ДанныеЗаполненияКИ);
		//КонецЕсли;
		//
		...
		//	ДанныеЗаполненияКИ.Вид = Справочники.ВидыКонтактнойИнформации.ТелефонОрганизации;
		//	СтрокаКИ = НоваяКонтактнаяИнформация.Добавить();
		//	ЗаполнитьЗначенияСвойств(СтрокаКИ, ДанныеЗаполненияКИ);
		//КонецЕсли;
		//
...
&НаСервереБезКонтекста
Процедура ОбновитьИнформациюВРегистреСведенийИсторияРегистрацийВОрганеПФР(ТекущаяОрганизацияСсылка, ТекущийРегистрационныйНомерПФР)
...
Процедура ПрочитатьДанные()
	...
	НастройкиЗарплатаКадрыРегламентированныйПрежние = Новый Структура;
	Для Каждого Ресурс Из Метаданные.РегистрыСведений.НастройкиЗарплатаКадры.Ресурсы Цикл
		НастройкиЗарплатаКадрыРегламентированныйПрежние.Вставить(Ресурс.Имя, Неопределено);
	КонецЦикла;
	ЗаполнитьЗначенияСвойств(НастройкиЗарплатаКадрыРегламентированныйПрежние, НастройкиЗарплатаКадрыРегЗначение);
...
&НаСервере
Процедура ДобавитьКонтактнуюИнформацию(ДобавляемаяКИ)
