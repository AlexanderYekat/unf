...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.КассаПоУмолчанию = Справочники.Кассы.ОсновнаяКассаНачальногоЗаполнения();
...
Функция РегистрацияВНалоговомОргане(Организация, Знач ДатаАктуальности = Неопределено) Экспорт
	...
	Возврат РегламентированнаяОтчетностьУСН.ПолучитьРегистрациюВИФНС(Организация);

КонецФункции

Функция НужноПоказыватьЭлементыБЗКБДляОрганизации(пОрганизация) Экспорт
	
	РеквизитыОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(пОрганизация, "ИспользуетсяОтчетность,ИПИспользуетТрудНаемныхРаботников");
	Возврат (ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплаты")
		И РеквизитыОрганизации.ИспользуетсяОтчетность И РеквизитыОрганизации.ИПИспользуетТрудНаемныхРаботников);
...
// Возвращает информацию о кассе по умолчанию для организации и валюту денежных средств.
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Структура - валюта и касса по-умолчанию, заполнена, когда не заполнена основная касса в настройках пользователя
Функция ВалютаИКассаПоУмолчанию(Объект, ЗначениеРеквизитаВалюта = Неопределено, ИмяРеквизитаВалюта = "") Экспорт
	
	Результат = Новый Структура;
	ДополнитьВалютуИКассуПриОтсутствииНастройкиПользователя(Результат, Объект, ЗначениеРеквизитаВалюта,
		ИмяРеквизитаВалюта);
	Возврат Результат;
...
Процедура ДополнитьВалютуИКассуПоУмолчанию(ЗначенияЗаполнения, Объект, ЗначениеРеквизитаВалюта = Неопределено,
	...
	ДополнитьВалютуИКассуПриОтсутствииНастройкиПользователя(ЗначенияЗаполнения, Объект, ЗначениеРеквизитаВалюта,
		ИмяРеквизитаВалюта);
...
Функция ПолноеНаименованиеНаДату(Организация, ДатаСведений) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаСведений) Тогда
		...
		|ИЗ
		|	Справочник.Организации.ИсторияНаименований КАК ИсторияКППОрганизаций
		|ГДЕ
		|	ИсторияКППОрганизаций.Ссылка = &Организация
		|	И ИсторияКППОрганизаций.Период <= &ДатаСведений
		|
...
Процедура ДополнитьВалютуИКассуПриОтсутствииНастройкиПользователя(ЗначенияЗаполнения, Объект, ЗначениеРеквизитаВалюта,
	ИмяРеквизитаВалюта)
	...
	Если УправлениеДоступом.ЧтениеРазрешено(ПользовательскаяНастройкаОсновнойКассы)
		И ЗначениеЗаполнено(ПользовательскаяНастройкаОсновнойКассы)
...
// Касса по умолчанию для указанной организации.
// 
// Параметры:
//  Организация - СправочникСсылка.Организации
// 
