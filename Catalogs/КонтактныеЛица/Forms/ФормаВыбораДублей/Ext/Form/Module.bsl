...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ИсключаяКонтрагентов", Контрагенты);
	
	Для Каждого Контрагент Из Контрагенты Цикл
		НовыйКонтакт = ИсключаяКонтрагентов.Добавить();
		НовыйКонтакт.Контрагент = Контрагент;
	КонецЦикла;
...
Процедура ПерейтиККонтакту(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Продолжить работу с выбранным контактом без сохранения текущего?'");
...
Процедура УстановитьСвязь(Команда)
	...
	Если ТипЗнч(Элементы.ТаблицаДублей.ТекущиеДанные.Ссылка) <> Тип("СправочникСсылка.Контрагенты") Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыДублей.Вставить("Контрагент", Элементы.ТаблицаДублей.ТекущиеДанные.Ссылка);
...
Процедура ОбновитьСписокДублейПоКИ()
	...
	|	И НЕ Контрагенты.Ссылка В (&Контрагенты)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 20
	...
	Контрагенты = ДанныеФормыВЗначение(ИсключаяКонтрагентов, Тип("ТаблицаЗначений")).ВыгрузитьКолонку("Контрагент");
...
Процедура УправлениеФормой()
	...
	Элементы.УстановитьСвязь.Видимость     = Контрагенты.Количество() > 0;
...
Процедура ОбработатьВопросПерейтиКОбъекту(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыЗакрытия.Вставить("ВыбранДубльКонтакта", Истина);
	ПараметрыЗакрытия.Вставить("ВыбранныйКонтакт",    Элементы.ТаблицаДублей.ТекущиеДанные.Ссылка);
