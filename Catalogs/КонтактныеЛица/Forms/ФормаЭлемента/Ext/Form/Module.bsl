...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ЗаполнитьОбъектПоДаннымКлассификатораКонтактов(КлассификаторДляЗаполненияКИ);
	КонецЕсли;
...
Процедура НаименованиеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.КлассификаторКонтактов") Тогда
		СтандартнаяОбработка = Ложь;
		ЗаполнитьОбъектПоДаннымКлассификатораКонтактов(ВыбранноеЗначение);
	КонецЕсли;
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", НаборСвойств_Справочник_КонтактныеЛица);
...
&НаСервере
Процедура ЗаполнитьОбъектПоДаннымКлассификатораКонтактов(Знач КлассификаторСсылка)
	...
	КонтактнаяИнформацияКлассификатора = Справочники.КлассификаторКонтактов.КонтактнаяИнформацияИзJSON(ДанныеКонтакта.Json);
	Если КонтактнаяИнформацияКлассификатора.Email.Количество() <> 0 Тогда
		Для каждого ОписаниеEmail Из КонтактнаяИнформацияКлассификатора.Email Цикл
			СтруктураКИ = Новый Структура("Представление,Комментарий,Значение,Вид");
			СтруктураКИ.Представление = ОписаниеEmail.Представление;
			СтруктураКИ.Вид = Справочники.ВидыКонтактнойИнформации.EmailКонтактногоЛица;
			ДобавляемаяКИ.Добавить(СтруктураКИ);
		КонецЦикла;
	КонецЕсли;
	
	УправлениеКонтактнойИнформациейУНФ.ДобавитьКонтактнуюИнформацию(ЭтотОбъект, Объект, ДобавляемаяКИ);
...
Процедура ЗаполнитьПоДаннымКонтактнойФормы(Параметры)
	...
	Если ДанныеКФ["НомераТелефонов"].Количество()>0 Тогда
		СтруктураКИ = Новый Структура("Представление,Комментарий,Значение,Вид");
		СтруктураКИ.Представление = ДанныеКФ["НомераТелефонов"][0];
		СтруктураКИ.Вид = Справочники.ВидыКонтактнойИнформации.ТелефонКонтактногоЛица;
		ДобавляемаяКИ.Добавить(СтруктураКИ);
	КонецЕсли;
	
	Для Каждого КИ Из ДанныеКФ["АдресаЭП"] Цикл
		СтруктураКИ = Новый Структура("Представление,Комментарий,Значение,Вид");
		СтруктураКИ.Представление = КИ;
		СтруктураКИ.Вид = Справочники.ВидыКонтактнойИнформации.EmailКонтактногоЛица;
		ДобавляемаяКИ.Добавить(СтруктураКИ);
	КонецЦикла;
	...
	Для Каждого КИ Из ДанныеКФ["Скайп"] Цикл
		СтруктураКИ = Новый Структура("Представление,Комментарий,Значение,Вид");
		СтруктураКИ.Представление = КИ;
		СтруктураКИ.Вид = Справочники.ВидыКонтактнойИнформации.SkypeКонтактногоЛица;
		ДобавляемаяКИ.Добавить(СтруктураКИ);
	КонецЦикла;
	...
	Для Каждого КИ Из ДанныеКФ["Другое"] Цикл
		СтруктураКИ = Новый Структура("Представление,Комментарий,Значение,Вид");
		СтруктураКИ.Представление = КИ;
		СтруктураКИ.Вид = Справочники.ВидыКонтактнойИнформации.МессенджерКонтактногоЛица;
		ДобавляемаяКИ.Добавить(СтруктураКИ);
	КонецЦикла;
...
&НаСервере
Процедура ПроверитьКИНаДубли(ДанныеКИ)
	...
	Если ДанныеКИ.Тип = ТипКИАдресЭП И НЕ ПроверятьАдресЭПНаДублиКонтакт Тогда
		Возврат;
	КонецЕсли;
	
	Если ДанныеКИ.Тип = ТипКИНомер И НЕ ПроверятьНомерТелефонаНаДублиКонтакт Тогда
		Возврат;
	КонецЕсли;
	...
	ПоказатьСкрытьНадписьОДубляхКИ(ДанныеКИ, ЕстьДубли);
	
КонецПроцедуры

&НаСервере
Процедура ПоказатьСкрытьНадписьОДубляхКИ(ДанныеКИ, ЕстьДубли)
	...
	ГруппаСоседнегоЗначенияКИ = СледующийЭлементВКоллекцииПочиненных(ГруппаЗначенияКИ.ПодчиненныеЭлементы, ГруппаТекущегоЗначенияКИ);
...
&НаСервере
Функция СледующийЭлементВКоллекцииПочиненных(ПодчиненныеЭлементы, ОтносительныйЭлемент)
