...
Процедура ПроверитьВыражениеПравилаЗагрузки(ВычисляемоеВыражение, МестоВыполненияВыражения, ТипОбъектаДО,
	...
	Если МестоВыполненияВыражения = Перечисления.МестаВыполненияВыраженийНаВстроенномЯзыке.НаСторонеИС Тогда
		...
		Если КонтекстВыражения.ОбъектXDTO = Неопределено Тогда
			КонтекстВыражения.ОбъектXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьНовыйОбъект(
				Прокси,
				ТипОбъектаДО,,,
		...
		Если КонтекстВыражения.ОбъектИС = Неопределено Тогда
			...
			Если ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъектаИС)
				...
				КонтекстВыражения.ОбъектИС = МенеджерОбъектаИС.СоздатьЭлемент();
			ИначеЕсли ОбщегоНазначения.ЭтоДокумент(МетаданныеОбъектаИС) Тогда
				КонтекстВыражения.ОбъектИС = МенеджерОбъектаИС.СоздатьДокумент();
			КонецЕсли;
			КонтекстВыражения.ОбъектИС.Заполнить(Неопределено);
		КонецЕсли;
...
Процедура ЗаполнитьТаблицуПоПравилу(ДанныеДляЗаполненияПоПравилу, Таблица, ПравилоРеквизита)
	...
	РеквизитыОбъектаИС = ДанныеДляЗаполненияПоПравилу.РеквизитыОбъектаИС; // см. ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПолучитьРеквизитыОбъектаИС
