...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Объект.ИспользуетсяРасширение = ВариантСпецификации;
		
	Если Объект.ИдентификаторСпецификации = "" Тогда
		Объект.ИдентификаторСпецификации = Объект.Ссылка.УникальныйИдентификатор();
	КонецЕсли;
КонецПроцедуры
...
Процедура СоставОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого ЗначениеВыбора Из ВыбранноеЗначение Цикл
		...
		НоваяСрока.ИспользоватьХарактеристики = СтруктураДанные.ИспользоватьХарактеристики;
		НоваяСрока.ПроверятьЗаполнениеХарактеристики = СтруктураДанные.ПроверятьЗаполнениеХарактеристики;
...
Процедура ЗаполнитьПризнакиИспользованияХарактеристик()
		НоменклатураВДокументахСервер.ЗаполнитьПризнакиИспользованияХарактеристик(Объект,,"Операции, СопоставлениеСостава");
КонецПроцедуры
...
Процедура СоставНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество)
	...
	СтруктураДанные.Вставить("ТипСтроки", СтрокаТабличнойЧасти.ТипСтрокиСостава);
	...
	СоставНоменклатураПриИзмененииНаСервере();
	

КонецПроцедуры

&НаСервере
Процедура СоставНоменклатураПриИзмененииНаСервере()
	НоменклатураВДокументахСервер.ЗаполнитьПризнакиИспользованияХарактеристик(Объект,,"Операции, СопоставлениеСостава");
КонецПроцедуры
...
Процедура ФормулаНоменклатураНачалоВыбора()
	...
	ПараметрыОткрытия.Вставить("ИдентификаторСпецификации",?(ТекущаяСтрока.ИдентификаторСпецификации="",Новый УникальныйИдентификатор(),ТекущаяСтрока.ИдентификаторСпецификации));
	ПараметрыОткрытия.Вставить("ИспользуетсяСопоставление", ТекущаяСтрока.ИспользуетсяСопоставление);
	ПараметрыОткрытия.Вставить("ИспользуетсяРасширение", ТекущаяСтрока.ИспользуетсяРасширение);
...
Процедура СоставФормулаНоменклатураЗавершениеВыбора(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") И ТекущаяСтрока <> Неопределено Тогда
		ВыбранноеЗначение.Свойство("ИспользуетсяРасширение", ТекущаяСтрока.ИспользуетсяРасширение);
		...
		ВыбранноеЗначение.Свойство("ИдентификаторСпецификации", ТекущаяСтрока.ИдентификаторСпецификации);
...
Процедура ОперацииПриАктивизацииЯчейки(Элемент)
	...
	ЭтоКолонкаКоличество = Элементы.Операции.ТекущийЭлемент = Элементы.ОперацииКоличество;
	Элементы.ОперацииКонтекстноеМенюНастройкаСбросаКоличества.Видимость = ЭтоКолонкаКоличество;
...
Процедура ОперацииОперацияПриИзменении(Элемент)
	...
	Если ЗапрашиватьСбросКоличества И СбрасыватьКоличество
		...
		ДопПараметр.Вставить("Элемент", Элемент);
		ДопПараметр.Вставить("Количество", СтрокаТабличнойЧасти.Количество);
		Оповещение = Новый ОписаниеОповещения("ОперацииПодтверждениеСбросаКоличестваЗавершение", ЭтотОбъект, ДопПараметр);
		ОткрытьФорму("ОбщаяФорма.ПодтверждениеСбросаКоличества", , , , , , Оповещение);
	...
	Иначе
		...
		Иначе
			НовоеКоличество = СтрокаТабличнойЧасти.Количество;
		КонецЕсли;
		ОперацииНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество);
...
&НаКлиенте
Процедура ОперацииПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ОперацииНоменклатураПриИзмененииПродолжить(ДополнительныеПараметры.Элемент, НовоеКоличество);
	
КонецПроцедуры

&НаКлиенте
Процедура ОперацииНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество)
...
Процедура ПодборСостав(Команда)
	...
	Для каждого ПараметрВыбора Из Элементы.СоставНоменклатура.ПараметрыВыбора Цикл
		...
		ИмяРеквизита = СтрЗаменить(ИмяРеквизита, "ДополнительныеПараметры.", "");
...
Процедура УстановитьУсловноеОформлениеФормы()
		...
		ВариантСпецификации = Объект.ИспользуетсяРасширение;
		Элементы.ГруппаРасширение.Видимость = Объект.ИспользуетсяРасширение;
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Состав.ИспользоватьХарактеристики", Ложь);
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, Элементы.СоставХарактеристика.Имя);
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", НСтр("ru = '<Не используется>'"));
...
&НаКлиенте
Процедура ВариантСпецификацииПриИзменении(Элемент)
	Элементы.ГруппаРасширение.Видимость = ВариантСпецификации;
	ИспользуетсяРасширение(ВариантСпецификации)
КонецПроцедуры

&НаСервере
Процедура ИспользуетсяРасширение(ВариантСпецификации)
	Объект.ИспользуетсяРасширение = ВариантСпецификации;
КонецПроцедуры
...
Функция ПараметрыСозданияДопРеквизита()
	...
	Если ЗначениеЗаполнено(КэшЗначений.КатегорияНоменклатуры) Тогда
		ТекущийНаборСвойств = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КэшЗначений.КатегорияНоменклатуры,
			"НаборСвойствСпецификации");
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ТекущийНаборСвойств) Тогда
		ТекущийНаборСвойств = УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Спецификации_Общие");
	КонецЕсли; 
...
Процедура ЗаполнитьСлужебныеДанныеПослеЧтенияОбъекта(Объект, КэшЗначений)
	...
	Для каждого СтрокаТабличнойЧасти Из Объект.Состав Цикл
		...
		СтрокаТабличнойЧасти.ИспользоватьХарактеристики = СтрокаТабличнойЧасти.Номенклатура.ИспользоватьХарактеристики;
	КонецЦикла; 
...
Процедура ДобавитьПараметрыСопоставленияВСтруктуруОткрытия(ПараметрыОткрытия, КлючСвязи, ИмяТабличнойЧасти)
	...
	Если ИмяТабличнойЧасти="СопоставлениеСостава" Тогда
		...
		СтруктураСтроки.Вставить("КлючСтрокиПравил");
		СтруктураСтроки.Вставить("Номенклатура");
		СтруктураСтроки.Вставить("Характеристика");
		СтруктураСтроки.Вставить("РеквизитСопоставления");
		СтруктураСтроки.Вставить("ЗначениеРеквизита");
	ИначеЕсли ИмяТабличнойЧасти="СопоставлениеОпераций" Тогда
		СтруктураСтроки = Новый Структура;
		СтруктураСтроки.Вставить("КлючСтрокиПравил");
		СтруктураСтроки.Вставить("Операция");
		СтруктураСтроки.Вставить("РеквизитСопоставления");
		СтруктураСтроки.Вставить("ЗначениеРеквизита");
	Иначе
		Возврат;
...
Процедура ОбновитьСопоставление(ТекущаяСтрока, ДанныеКонструктора, ИмяТабличнойЧасти)
	...
	Если ТекущаяСтрока.ИспользуетсяСопоставление Тогда
		...
		КонецЕсли; 
	ИначеЕсли ТекущаяСтрока.ИспользуетсяРасширение Тогда
		Если ИмяТабличнойЧасти="СопоставлениеОпераций" Тогда
			ИмяПоляФормула = "ФормулаОперация";
		Иначе
			ИмяПоляФормула = "ФормулаНоменклатура";
		КонецЕсли; 
		Если НЕ ПустаяСтрока(ДанныеКонструктора.ИдентификаторСпецификации) Тогда
			ТекущаяСтрока[ИмяПоляФормула] = СтрШаблон(НСтр("ru = '<Расширение: %1>'"), ДанныеКонструктора.ИдентификаторСпецификации);
		Иначе
			ТекущаяСтрока[ИмяПоляФормула] = НСтр("ru = '<Расширение>'");
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура БольшеВариантовНажатие(Элемент)
	Если ВариантСпецификации = 1 Тогда
		Если Объект.Ссылка.Пустая() Тогда
			ЗаполнитьИдентификаторПоказателяНаСервере()
		КонецЕсли;
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("ИдентификаторСпецификации", Объект.ИдентификаторСпецификации);
		ОткрытьФорму("ОбщаяФорма.ПомощникПодключенияПроцедурыРасширения", ПараметрыОткрытия,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьИдентификаторПоказателяНаСервере()
	
	СсылкаНовогоЭлемента = Справочники.Спецификации.ПолучитьСсылку();
	ЭтотОбъект.ИдентификаторСпецификации = СсылкаНовогоЭлемента.УникальныйИдентификатор();
	
	СправочникОбъект = РеквизитФормыВЗначение("Объект");
	СправочникОбъект.УстановитьСсылкуНового(СсылкаНовогоЭлемента);
