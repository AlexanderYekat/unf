...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ИдентификаторСпецификации",ИдентификаторСпецификации);
	Параметры.Свойство("ИспользуетсяРасширение",ИспользуетсяРасширение);
	...
	Если ЗначениеЗаполнено(КатегорияНоменклатуры) Тогда
		...
		Схема.Параметры.НаборСвойств_Справочник_Спецификации_Общие.Значение = УправлениеСвойствами.НаборСвойствПоИмени(
	...
	ИначеЕсли НЕ ПростыеТипы И Параметры.Свойство("ИспользуетсяРасширение") И Параметры.ИспользуетсяРасширение Тогда
		Режим = 2;
	Иначе
...
Процедура ОК(Команда)
	...
	РежимРасширения = (Режим=2);
	Результат = Новый Структура;
	Результат.Вставить("Результат", КодВозвратаДиалога.Да);
	Результат.Вставить("ИспользуетсяСопоставление", РежимСопоставления);
	Результат.Вставить("ИспользуетсяРасширение", РежимРасширения);
	Результат.Вставить("ИдентификаторСпецификации", ИдентификаторСпецификации);
...
Процедура БольшеВариантовНажатие(Элемент)
	Если Режим = 2 Тогда
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("ИдентификаторСпецификации", ИдентификаторСпецификации);
...
Процедура УправлениеФормой(Форма)
	...
	ИначеЕсли Форма.Режим = 2 Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаРасширение;
	Иначе
...
Функция ПроверитьЗаполнениеФормы()
	...
	Если (Режим=0) И ПустаяСтрока(Формула) Тогда
...
Функция ПередДобавлениемТекстаВФормулу(ЗНАЧ ТекстВставки)
	...
	Если СокрЛП(ПараметрыДобавленияТекста.ТекстВставки) = "ЕСЛИ" Тогда
		...
		ПараметрыДобавленияТекста.ТекстВставки = СтрШаблон("#ЕСЛИ <Условие>%1%2#ТОГДА %3%1%2#ИНАЧЕЕСЛИ <Условие>%1%2#ТОГДА %3%1%2#ИНАЧЕ <?>%1 #КОНЕЦЕСЛИ", Символы.ПС, Символы.Таб, ПервоеЗначениеУсловногоОператора);
...
Функция СвойствоПоНаименованию(ИмяОбъекта, Наименование)
	...
	ИначеЕсли ИмяОбъекта = "Спецификация" Тогда
		НаборСвойств_Справочник_Спецификации = УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Спецификации");
		МассивНаборов.Добавить(НаборСвойств_Справочник_Спецификации);
	ИначеЕсли ИмяОбъекта = "ЗаказНаПроизводство" Тогда
		НаборСвойств_Документ_ЗаказНаПроизводство = УправлениеСвойствами.НаборСвойствПоИмени(
			"Документ_ЗаказНаПроизводство");
		МассивНаборов.Добавить(НаборСвойств_Документ_ЗаказНаПроизводство);
