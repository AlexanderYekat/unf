...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВременныйНомерСчета = Объект.НомерСчета;
	...
	Если Параметры.Свойство("НеобходимаОтметкаНазначениеПлатежа") Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаНастройкаПечати;
		ТекущийЭлемент = Элементы.ТекстНазначения;
	КонецЕсли;
	
	Если Элементы.ОстатокДенежныхСредств.Видимость
		И Пользователи.РолиДоступны("ЧтениеОстатковДенежныхСредств") Тогда
		Элементы.ОстатокДенежныхСредств.Заголовок = ДвиженияДенежныхСредствВызовСервера.ЗаголовокНадписиОстатковДС(Объект.Ссылка, Объект.Владелец);
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Объект.НомерСчета = ВременныйНомерСчета;
...
Процедура НомерСчетаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВременныйНомерСчета = СтрЗаменить(Текст, " ", "");
...
#Если НЕ ВебКлиент Тогда
	...
	ВременныйНомерСчета = СтрЗаменить(Текст, " ", "");
	УстановитьПодсказкуНомераСчета();
...
&НаКлиенте
Процедура УстановитьПодсказкуНомераСчета()
	
	Если ЯвляетсяБанкомРФ И ВременныйНомерСчета <> Неопределено Тогда
		
		ДлинаСчетаРФ = 20;
		КоличествоЦифрВСчете = СтрДлина(ВременныйНомерСчета);
...
Процедура ОстатокДенежныхСредствНажатие(Элемент)
	...
	РаботаСФормойДокументаКлиент.ОткрытьОтчетДенежныеСредства(Объект.Ссылка, Объект.Владелец);
...
Функция ЭлементОсновной()
	...
	БанковскийСчетПоУмолчанию = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Владелец, "БанковскийСчетПоУмолчанию");
	Возврат БанковскийСчетПоУмолчанию = Объект.Ссылка;
...
Процедура УправлениеЭлементамиФормы()
	...
	ВладелецЭтоОрганизация = ТипЗнч(Объект.Владелец) = Тип("СправочникСсылка.Организации");
	
	// Настройка печати.
	Элементы.ГруппаРеквизитыСчетаОрганизации.Видимость = ВладелецЭтоОрганизация;
	Элементы.ГруппаРеквизитыСчетаКонтрагента.Видимость = НЕ ВладелецЭтоОрганизация;
	
	Если Пользователи.РолиДоступны("ЧтениеОстатковДенежныхСредств") Тогда
		Элементы.ОстатокДенежныхСредств.Видимость = ЗначениеЗаполнено(Объект.Ссылка) И ВладелецЭтоОрганизация;
	Иначе
...
Процедура ЗаполнитьСписокВариантовПредставленийСчета()
	...
	СтрокаНаименования = СокрЛп(ВременныйНомерСчета) + ?(ЗначениеЗаполнено(НаименованиеБанка), ", в " + Строка(НаименованиеБанка), "");
...
Процедура УстановитьНаименованиеБанка()
	...
	НаименованиеБанка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Банк, "Наименование");
	Объект.Наименование = Лев(СокрЛп(ВременныйНомерСчета) + ?(ЗначениеЗаполнено(НаименованиеБанка), ", в " + НаименованиеБанка, ""), 100);
...
Процедура ЗаписатьОсновнойСчет(СтруктураПараметров)
	...
	Попытка
		...
		ИмяСправочника = ОбщегоНазначения.ИмяТаблицыПоСсылке(СтруктураПараметров.Владелец); 
	...
	Исключение
		...
		МетаданныеОбъекта = СтруктураПараметров.Владелец.Метаданные();
		ПредставлениеОбъекта = ОбщегоНазначения.ПредставлениеОбъекта(МетаданныеОбъекта);
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось заблокировать %1: %2, для изменения основного банковского счета, по причине:
				 |%3'", ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеОбъекта, СтруктураПараметров.Владелец,
