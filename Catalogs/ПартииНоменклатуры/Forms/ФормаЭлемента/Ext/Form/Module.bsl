...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаборСвойств_Справочник_ПартииНоменклатуры = УправлениеСвойствами.НаборСвойствПоИмени(
		"Справочник_ПартииНоменклатуры");
	...
	Элементы.НалогообложениеНДС.Видимость = Объект.Статус = Перечисления.СтатусыПартий.ТоварыНаКомиссии;
	ВидСтавкиНДСВладельца = ?(ЗначениеЗаполнено(Объект.Владелец), Объект.Владелец.ВидСтавкиНДС, Неопределено);
	
	Если ЭтоНовый Тогда
		
		Если Параметры.ДополнительныеПараметры.Свойство("НалогообложениеНДС") Тогда
			Объект.НалогообложениеНДС = Параметры.ДополнительныеПараметры.НалогообложениеНДС;
		ИначеЕсли Параметры.ЗначенияЗаполнения.Свойство("НалогообложениеНДС") И ЗначениеЗаполнено(Параметры.ЗначенияЗаполнения.НалогообложениеНДС) Тогда
			Объект.НалогообложениеНДС = Параметры.ЗначенияЗаполнения.НалогообложениеНДС;
		Иначе
			
			Если Не ВидСтавкиНДСВладельца = Неопределено Тогда
				Если ВидСтавкиНДСВладельца = ПредопределенноеЗначение("Перечисление.ВидыСтавокНДС.БезНДС") Тогда
	...
	СозданиеИзПоляВвода = Параметры.Свойство("РежимВыбора") И Параметры.РежимВыбора;
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	Если ЗапретКомиссионныхПартийБезВладельца Тогда
		...
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Статус", Перечисления.СтатусыПартий.ТоварыНаКомиссии, ВидСравненияКомпоновкиДанных.Равно);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.ВладелецПартии", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
		РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "ВладелецПартии");
		РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ОтметкаНезаполненного", Истина);
		
		НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.Статус", Перечисления.СтатусыПартий.ТоварыНаКомиссии, ВидСравненияКомпоновкиДанных.НеРавно);
		РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "ВладелецПартии");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) И СозданиеИзПоляВвода
		И ЗначениеЗаполнено(Объект.Владелец) Тогда
		Если Не Объект.Владелец.ИспользоватьПартии Тогда
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	...
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", НаборСвойств_Справочник_ПартииНоменклатуры);
...
Функция ПараметрыСозданияДопРеквизита()
	...
	ТекущийНаборСвойств = НаборСвойств_Справочник_ПартииНоменклатуры;
...
Процедура ЗадатьВопросПоУдалениюПризнакаОсновнойПартии(ПараметрыОповещения)
	...
	ПоказатьВопрос(ОповещениеВопроса,
		НСтр("ru = 'Партия является <основной>. После записи признак <основная партия> будет удален. Продолжить?'"),
...
Процедура СтатусПриИзменении(Элемент)
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыПартий.ТоварыНаКомиссии") Тогда
