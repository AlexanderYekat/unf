...
Функция ИскатьСтрокуВТаблицеНаСервере(СтрокаПоиска, КБКИсходный = "")
	...
	Если НайденнаяОбласть = Неопределено Тогда
		...
		Если СтрДлина(СтрокаПоиска) = 17 И ТабДокумент.Область(ТекущаяОбласть.Верх, 1).Текст = СтрокаПоиска
			Или СтрНайти(ТабДокумент.Область(ТекущаяОбласть.Верх, 2).Текст, СтрокаПоиска) > 0 Или СтрДлина(
			СтрокаПоиска) < 17 И СтрНайти(ТабДокумент.Область(ТекущаяОбласть.Верх, 1).Текст, СтрокаПоиска) > 0 Тогда
			НайденнаяОбласть = ТекущаяОбласть;
		Иначе
...
Процедура ОпределитьНомерПриказа()
	...
	Период =  ОписаниеТипаЧисло.ПривестиЗначение(ГодНачалаДействияПриказа);
	
	Если Период > 2022 Тогда
		ИмяМакета      = "КлассификацияДоходовБюджетов2023";
		НадписьПриказа = НСтр("ru = 'по приказу Минфина от 17 мая 2022 № 75н
			|(в редакции приказа Минфина от 22.11.22 № 177н)'");
	Иначе
		ИмяМакета      = "КлассификацияДоходовБюджетов";
		НадписьПриказа = НСтр("ru = 'по приказу Минфина от 8 июня 2021 г. № 75н
			|(в редакции приказа Минфина от 20.10.2022 № 154н)'");
	КонецЕсли;
...
Процедура НастроитьСписокВыбора()
	...
	ДатаАктуальности = Макс(ДатаАктуальностиКлассификатора, Дата(ГодНачалаДействияПриказа, 1, 1));
	СтарыйПериод = НачалоГода(ДатаАктуальности) - 1;
	
	СписокВыбора.Добавить(Формат(Год(СтарыйПериод), "ЧГ="), Формат(СтарыйПериод, "ДФ='yyyy ""г.""'"));
	СписокВыбора.Добавить(Формат(Год(ДатаАктуальности), "ЧГ="), Формат(ДатаАктуальности, "ДФ='yyyy ""г.""'"));
