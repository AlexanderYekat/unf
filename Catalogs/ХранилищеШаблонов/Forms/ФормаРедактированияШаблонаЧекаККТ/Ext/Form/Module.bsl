...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Промокоды - определяет возможность вывода дополнительной таблицы "Промокоды" нефискального документа
	ИспользоватьПромокоды = ПолучитьФункциональнуюОпцию("ИспользоватьПромокоды");
	...
	Если Параметры.Свойство("ТаблицаУсловий") Тогда
		Для Каждого ОписаниеУсловия Из Параметры.ТаблицаУсловий Цикл
...
Процедура ОповещениеРедактированиеШаблонаЧека(ОтветФормы, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ОтветФормы) = Тип("Структура") Тогда
		...
		СтарыеСтрокиУсловий = ТаблицаУсловий.НайтиСтроки(Новый Структура("Идентификатор", ОтветФормы.Идентификатор));
		...
		Для Каждого АктуальноеУсловие Из ОтветФормы.ТаблицаУсловий Цикл
			...
			НовоеУсловие.Идентификатор = ОтветФормы.Идентификатор;
		КонецЦикла;
		//Если ОтветФормы.ТаблицаУсловий.Количество() > 0 Тогда
		//	ТекущаяСтрока.Элемент = ТекущаяСтрока.Элемент + НСТр("ru = '(по условию)'");
		//КонецЕсли;
		СформироватьМакетФискальногоЧека();
...
Процедура ЗагрузитьШаблонРасширенныйСервер()
	...
	СтрокаКодаВозвратаСБП = ДобавитьПараметрВДерево(СтрокаСоставЧека, "Текст", НСтр("ru = 'КодВозвратаСБП'"),
		СтруктураСтрокиШтрихкода);
	СтрокаКодаВозвратаСБП.ТипШтрихкода = 16;
	ТекстТипаПлатежнойСистемыККТ = Символы.ПС
		+ Строка(ПредопределенноеЗначение("Перечисление.ТипыПлатежнойСистемыККТ.СистемаБыстрыхПлатежей"));
	ПостфиксКодаВозврата = СтрШаблон(ШаблонПостфиксаКодаВозврата, ТекстТипаПлатежнойСистемыККТ, Символы.ПС);
	СтрокаКодаВозвратаСБП.Постфикс = ПостфиксКодаВозврата;
	
	СтрокаКодаВозвратаНСПК = ДобавитьПараметрВДерево(СтрокаСоставЧека, "Текст", НСтр("ru = 'КодВозвратаНСПК'"),
		СтруктураСтрокиШтрихкода);
	СтрокаКодаВозвратаНСПК.ТипШтрихкода = 16;
	ТекстТипаПлатежнойСистемыККТ = Символы.ПС
		+ Строка(ПредопределенноеЗначение("Перечисление.ТипыПлатежнойСистемыККТ.СертификатНСПК"));
	ПостфиксКодаВозврата = СтрШаблон(ШаблонПостфиксаКодаВозврата, ТекстТипаПлатежнойСистемыККТ, Символы.ПС);
	СтрокаКодаВозвратаНСПК.Постфикс = ПостфиксКодаВозврата;
...
Процедура ИмпортироватьШаблонИзФайлаСервер(АдресФайла)
	...
	Попытка
		...
		КолонкиШаблонаОбъекта = ШаблонЧекаОбъект.Колонки;
		
		ШаблонСФормы = РеквизитФормыВЗначение("ШаблонЧека");
		КолонкиШаблона = ШаблонСФормы.Колонки;
		
		Для Каждого КолонкаШаблона Из КолонкиШаблона Цикл
			КолонкаДерева = КолонкиШаблонаОбъекта.Найти(КолонкаШаблона.Имя);
			Если КолонкаДерева = Неопределено Тогда
				
				КолонкиШаблонаОбъекта.Добавить(
								КолонкаШаблона.Имя,
...
Функция ДобавитьПолеВШаблон(ИмяПараметра, ПредставлениеПараметра, СтруктураПараметра)
	...
	Если МожноДобавлять Тогда
		...
		Если ИмяПараметра = "НефискальныйДокумент" Тогда
			РезультатСоставЧека = ДобавитьПолеВШаблонСервер(Результат, "СоставЧека", НСтр("ru = 'Состав чека'"), СтруктураПараметра);
			Если ИспользоватьПромокоды Тогда
				РезультатПромокоды = ДобавитьПолеВШаблонСервер(Результат, "Промокоды", НСтр("ru = 'Промокоды к выдаче'"), СтруктураПараметра);
				...
				РезультатПромокодыКВыдаче = ДобавитьПолеВШаблонСервер(РезультатПромокоды, "Таблица", "ПромокодыКВыдаче", СтруктураТаблицыПромокодов);
				...
				РезультатПромокод = ДобавитьПолеВШаблонСервер(РезультатПромокодыКВыдаче, "Текст", НСтр("ru = 'ПромокодЗначениеПромокода'"), СтруктураПоляПромокод);
			КонецЕсли;
		КонецЕсли;
...
Функция ОбойтиПредопределенныеРазделы(Команда)
	...
	ТаблицаИдентификаторов = ТаблицаУсловий.Выгрузить(,"Идентификатор");
	ТаблицаИдентификаторов.Свернуть("Идентификатор");
	ИдентификаторыУсловий = ТаблицаИдентификаторов.ВыгрузитьКолонку("Идентификатор");
...
Процедура УдалитьСтрокиУсловийПоИдентификатору(Идентификаторы)
	...
	Для Каждого ИдентификаторУдалить Из ИдентификаторыУсловий Цикл
		СтрокиУдалить = ТаблицаУсловий.НайтиСтроки(Новый Структура("Идентификатор", ИдентификаторУдалить));
...
Функция ПолучитьИдентификаторыЭлементов(СтрокаДерева)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДерева, "Идентификатор") Тогда
		Если ЗначениеЗаполнено(СтрокаДерева.Идентификатор) Тогда
			Результат.Добавить(СтрокаДерева.Идентификатор);
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого СтрокаДерева Из ШаблонДерево.Строки Цикл
		...
		ИначеЕсли СтрокаДерева.ИмяКолонки = "НефискальныйДокумент" Тогда
			...
			Для Каждого СтрокаРазделНефискальногоДокумента Из СтрокаДерева.Строки Цикл
				...
				Если СтрокаРазделНефискальногоДокумента.ИмяКолонки = "СоставЧека"
					ИЛИ (СтрокаРазделНефискальногоДокумента.ИмяКолонки = "Промокоды" И ИспользоватьПромокоды) Тогда
...
Процедура ОбработатьТекстовуюСтроку(Область, ПоляСтроки, СтрокаШапкиЧека, ТаблицаДанных,
	...
	Если Не ПоляСтроки.Идентификатор = "" Тогда
		...
		Если Не ПечатьФискальныхЧековРМК.УсловияВыполнены(ПредставлениеШаблонаДляУсловий, ПоляСтроки.Идентификатор, ИмяРаздела, СтрокаШапкиЧека, СтрокаДанных) Тогда
...
Процедура ОткрытиеФормыРедактированияПараметров(ВыбраннаяСтрока)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		ВидимостьУсловий = Истина;
		СтруктураПараметров.Вставить("ВидимостьУсловий", ВидимостьУсловий);
		
		СуществующиеУсловия = ПолучитьПредставлениеТаблицыУсловийПоИдентификатору(ВыбраннаяСтрока);
		СтруктураПараметров.Вставить("ТаблицаУсловий", СуществующиеУсловия);
		Если ИспользоватьПромокоды И ЭтоСтрокаВыводаПромокода(ТекущаяСтрока) Тогда
	   		СтруктураПараметров.Вставить("ДоступенВыводПромокода", Истина);
		КонецЕсли;
...
&НаСервере 
Функция ПолучитьПредставлениеТаблицыУсловийПоИдентификатору(Идентификатор)
	СуществующиеУсловия = ТаблицаУсловий.НайтиСтроки(Новый Структура("Идентификатор", Формат(Идентификатор, "ЧДЦ=0; ЧН=0; ЧГ=")));
...
Функция ТекущееПолеЗапрещеноРедактировать()
	...
	Если ТекущаяСтрокаДерева <> Неопределено Тогда
		...
		Если ТекущаяСтрокаДерева.ИмяКолонки = "НефискальныйДокумент"
			...
			ИЛИ ТекущаяСтрокаДерева.ИмяКолонки = "Таблица" // Таблица промокодов
			ИЛИ ТекущаяСтрокаДерева.ИмяКолонки = "Отрезка"
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "ШаблонЧека.ЕстьУсловие", Истина, ВидСравненияКомпоновкиДанных.Равно);
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "ШаблонЧека.Элемент", "СоставнаяСтрока", ВидСравненияКомпоновкиДанных.Равно);
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "ШаблонЧекаЭлемент");
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", НСтр("ru = 'СоставнаяСтрока(Условию)'"));
