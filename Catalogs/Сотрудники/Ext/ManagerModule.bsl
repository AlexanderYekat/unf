...
Функция СоздатьНовогоСотрудника(Знач ДанныеНовогоСотрудника, Знач РегистрироватьНаУзлахПлановОбмена = Истина,
	...
	Попытка
		...
		ЗаполнитьЗначенияСвойств(Сотрудник, ДанныеНовогоСотрудника);
		...
		Если НЕ (ЗначениеЗаполнено(Сотрудник.Физлицо) И ОбщегоНазначения.СсылкаСуществует(Сотрудник.Физлицо)) Тогда 
			ФизическоеЛицо = Справочники.ФизическиеЛица.СоздатьЭлемент();
			ФизическоеЛицо.Наименование = Сотрудник.Наименование;
			ФизическоеЛицо.УстановитьНовыйКод();
			
			ФизическоеЛицо.ОбменДанными.Загрузка = ОтключитьБизнесЛогику;
			Если Не РегистрироватьНаУзлахПлановОбмена Тогда
				ФизическоеЛицо.ДополнительныеСвойства.Вставить("ОтключитьМеханизмРегистрацииОбъектов");
				ФизическоеЛицо.ОбменДанными.Получатели.АвтоЗаполнение = Ложь;
			КонецЕсли;		
		...
		Сотрудник.ОбменДанными.Загрузка = ОтключитьБизнесЛогику;
		Если Не РегистрироватьНаУзлахПлановОбмена Тогда
			Сотрудник.ДополнительныеСвойства.Вставить("ОтключитьМеханизмРегистрацииОбъектов");
			Сотрудник.ОбменДанными.Получатели.АвтоЗаполнение = Ложь;
		КонецЕсли;		
...
Функция ДанныеТрудовыхДоговоров(МассивОбъектов, ДистанционныеРаботы, ДоговорСНадомником)
	...
	УчетСтраховыхВзносовУНФ.СоздатьВТКадровыеДанныеСотрудников(Описатель, Истина, КадровыеДанныеПриема);
	...
	|ГДЕ
	|	ЕСТЬNULL(СотрудникиСДаннымиОПриеме.ПриказОПриеме, ЗНАЧЕНИЕ(Документ.ПриемНаРаботуУНФ.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Документ.ПриемНаРаботуУНФ.ПустаяСсылка)";
	...
	УчетСтраховыхВзносовУНФ.СоздатьВТФИООтветственныхЛиц(Запрос.МенеджерВременныхТаблиц, Истина,
...
Процедура СоздатьВТДанныеДокументовПриемНаРаботу(МенеджерВременныхТаблиц, МассивОбъектов)
	...
	|	ПриемНаРаботуУНФ.Период КАК Дата,
	|	ПриемНаРаботуУНФ.Сотрудник,
	|	ПриемНаРаботуУНФ.Период КАК Период,
	|	ПриемНаРаботуУНФ.Ссылка.Номер КАК НомерДок,
	|	ПриемНаРаботуУНФ.Ссылка.Дата КАК ДатаДок,
	|	"""" КАК Руководитель,
	...
	|ИЗ
	|	Документ.ПриемНаРаботуУНФ.Сотрудники КАК ПриемНаРаботуУНФ
	|ГДЕ
	|	ПриемНаРаботуУНФ.Ссылка.Проведен
	|	И ПриемНаРаботуУНФ.Ссылка В(&МассивОбъектов)";

	Если ПечатьПоЭлементуСправочника Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПриемНаРаботуУНФ.Ссылка В(&МассивОбъектов)",
			"ПриемНаРаботуУНФ.Сотрудник В(&МассивОбъектов)");
	КонецЕсли;
...
Процедура СоздатьВТКадровыеДанныеТрудовогоДоговора(МенеджерВременныхТаблиц, ИмяВременнойТаблицыОтборовСотрудников,
	...
	|ТарифнаяСтавка";
	УчетСтраховыхВзносовУНФ.СоздатьВТКадровыеДанныеСотрудников(ОписательВременныхТаблиц, Истина, КадровыеДанные);
