...
Процедура ДекорацияКартинкаКоличестваЦенНажатие(Элемент)
	...
	ОткрытьФорму("Обработка.УдалениеУстаревшихЦен.Форма", Неопределено, ЭтотОбъект, , , , ОписаниеОповещения);
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для каждого СтрокаСписка Из Строки Цикл
		...
		СтрокаСписка.Значение.Данные.ПредставлениеРасчета = 
			Справочники.ВидыЦен.ПолучитьПредставлениеСпособаРасчетаВидаЦены(СтрокаСписка.Ключ);
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Если ДанныеТекущейСтроки <> Неопределено Тогда 
		...
		Если ЗначениеЗаполнено(ДанныеТекущейСтроки.Комментарий) Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПодробноеОписание", "Заголовок", "Описание");
		Иначе
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПодробноеОписание", "Заголовок", "");
		КонецЕсли;
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеТекущейСтроки, "ПоказатьПредупреждение") Тогда

			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДекорацияПредупреждение",
				"Видимость", ДанныеТекущейСтроки.ПоказатьПредупреждение);
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыОтборПрайсЛисты
...
Процедура УстановитьУсловноеОформление()
	...
	Отбор.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("КоличествоПопытокСлужебный");
	...
	// Отображать сформировать цены для авторасчета
	НовоеУсловноеОформление = Список.КомпоновщикНастроек.ФиксированныеНастройки.УсловноеОформление.Элементы.Добавить();
	
	Оформление = НовоеУсловноеОформление.Оформление.Элементы.Найти("Отображать");
	Оформление.Значение 		= Ложь;
	Оформление.Использование 	= Истина;
	
	ГруппаИли = НовоеУсловноеОформление.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаИли.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
	
	Отбор = ГруппаИли.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ВидСравнения 		= ВидСравненияКомпоновкиДанных.Равно;
	Отбор.Использование 	= Истина;
	Отбор.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("Авто");
	...
	ОформляемоеПоле = НовоеУсловноеОформление.Поля.Элементы.Добавить();
	ОформляемоеПоле.Поле			= Новый ПолеКомпоновкиДанных("УстановкаЦен");
	ОформляемоеПоле.Использование	= Истина;
	
	// Пустой идентификатор формул
	НовоеУсловноеОформление = Список.КомпоновщикНастроек.ФиксированныеНастройки.УсловноеОформление.Элементы.Добавить();
	...
	Оформление.Значение 		= НСтр("ru='Пустой идентификатор формул'");
	Оформление.Использование 	= Истина;
	
	Отбор = НовоеУсловноеОформление.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ВидСравнения 		= ВидСравненияКомпоновкиДанных.НеЗаполнено;
	Отбор.Использование 	= Истина;
	Отбор.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("ИдентификаторФормул");
	
	ОформляемоеПоле = НовоеУсловноеОформление.Поля.Элементы.Добавить();
	ОформляемоеПоле.Поле			= Новый ПолеКомпоновкиДанных("СформироватьЦены");
	ОформляемоеПоле.Использование	= Истина;
...
Процедура СформироватьЦены(ВидЦен)
	...
	Если ДанныеВидаЦен.ТипВидаЦен = КэшЗначений.Статический
		ИЛИ ДанныеВидаЦен.ТипВидаЦен = КэшЗначений.ПроизвольныйЗапрос
		ИЛИ ДанныеВидаЦен.ТипВидаЦен = КэшЗначений.Расширение Тогда
