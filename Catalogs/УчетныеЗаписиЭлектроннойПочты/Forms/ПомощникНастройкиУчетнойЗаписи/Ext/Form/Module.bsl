...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КонтекстныйРежим Тогда
		Элементы.ИспользоватьДляОтправки.Доступность = Ложь;
	КонецЕсли;
	// Конец УНФ
	...
	// УНФ
	Элементы.ДляКогоУчетнаяЗапись.Видимость = Ложь;
	// Конец УНФ
...
Процедура ВыполнитьПроверкуНастроекЗапросНаРазрешенияВыполнен(РезультатЗапроса, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(УчетнаяЗаписьСсылка) Тогда 
		// УНФ
		РазослатьОповещения(УчетнаяЗаписьСсылка);
		// Конец УНФ
	КонецЕсли;
...
Функция НастройкиПоУмолчанию(АдресЭлектроннойПочты, Пароль)
	...
	// УНФ
	Возврат ЭлектроннаяПочтаУНФКлиентСервер.НастройкиПоУмолчанию(АдресЭлектроннойПочты, Пароль);
	// Конец УНФ
...
Процедура СоздатьУчетнуюЗапись()
	...
	Попытка
		...
		// УНФ
		ЭлектроннаяПочтаУНФ.ПриНастройкеУчетнойЗаписи(УчетнаяЗапись);
		// Конец УНФ
...
&НаКлиенте
Процедура РазослатьОповещения(УчетнаяЗаписьСсылка)
	
	ОбщегоНазначенияКлиент.ОповеститьОбИзмененииОбъекта(УчетнаяЗаписьСсылка);
	
	Оповестить(ЭлектроннаяПочтаУНФКлиентСервер.ИмяСобытияУчетнаяЗаписьОбновлена(), УчетнаяЗаписьСсылка);
	
	Если Параметры.РежимВыбора Тогда
		ОповеститьОВыборе(УчетнаяЗаписьСсылка);
	КонецЕсли;
