...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Выбран.Видимость = Истина;
	ОтборВыбранные = "Все";
...
Процедура ЭлементДеревоПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("ДанныеФормыЭлементДерева") Тогда
		...
		ПеренестиСПодчиненными(ИДПриемник, МассивИДИсточник.Значение, ДетализацияОтбора);
...
Процедура ОтборВыбранныеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	УстановитьОтборНаКлиенте(ВыбранноеЗначение);
	
КонецПроцедуры

&НаКлиенте
Процедура ТолькоДействителеныеПриИзменении(Элемент)
	УстановитьОтборНаКлиенте(ОтборВыбранные);
КонецПроцедуры
...
&НаКлиенте
Процедура УстановитьОтборНаКлиенте(ВыбранноеЗначение)
...
Процедура ЗаполнитьДеревоИзСохраненныхДанных(ИдентификаторКаталога, АдресНастроекСтруктурыКаталога)
	...
	Иначе	
		ТЗ = РегистрыСведений.СтруктураКаталогаИнтернетМагазина.СоздатьНаборЗаписей();
		ТЗ.Отбор.НастройкаИнтеграции.Установить(НастройкаИнтеграции);
		ТЗ.Отбор.ИдентификаторКаталога.Установить(ИдентификаторКаталога);
		ТЗ.Прочитать();
		СохраненныеДанные = ТЗ.Выгрузить();
		СохраненныеДанные.Индексы.Добавить( "РодительИдентификатор" );
	КонецЕсли;

	Если ТипЗнч( СохраненныеДанные ) = Тип( "ТаблицаЗначений" ) Тогда
		Если СохраненныеДанные.Колонки.Найти( "РодительИдентификатор" ) <> Неопределено Тогда
			СохраненныеДанные.Индексы.Добавить( "РодительИдентификатор" );
		КонецЕсли;
	КонецЕсли;
	...
	Если СохраненныеДанные <> Неопределено Тогда
		...
		Попытка
			ПреобразоватьВДеревоРекурсия(Дерево, СохраненныеДанные, "", Исключения);
		Исключение
...
&НаСервере
Процедура ПреобразоватьВДеревоРекурсия(Дерево, тТаблица, Идентификатор, Исключения)
	
	Если Исключения.Получить( Идентификатор ) <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Исключения.Вставить( Идентификатор, Истина );
	...
	Для Каждого Стр Из тМассив Цикл
		...
		Если ЗначениеЗаполнено(Стр.Идентификатор)
			...
			ПреобразоватьВДеревоРекурсия(НоваяСтрока, тТаблица, Стр.Идентификатор, Исключения);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПреобразоватьВТаблицуРекурсия(тДерево, СохраненныеДанные, РодительИдентификатор = "")
	...
	Для Каждого Стр Из тДерево.Строки Цикл
		...
		Иначе
			НоваяСтрока.НастройкаИнтеграции = НастройкаИнтеграции;
		КонецЕсли;
...
Функция ЗаписатьИЗакрытьСервер()
	...
	СохраненныеДанные.ЗаполнитьЗначения(НастройкаИнтеграции, "НастройкаИнтеграции");
...
&НаСервере
Процедура ПеренестиСПодчиненными(ИДПриемник, Источник, ДетализацияОтбора)
...
Процедура ОбновитьВыбранныеТовары(Показывать)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Номенклатура, "Действителен", ТолькоДействителеные
	, ВидСравненияКомпоновкиДанных.Равно, , ТолькоДействителеные);
