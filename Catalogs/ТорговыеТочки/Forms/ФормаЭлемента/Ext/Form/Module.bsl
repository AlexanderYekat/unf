...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РазмещеныНаФорме.Вставить(Справочники.ВидыКонтактнойИнформации.АдресТорговойТочки, Истина);
	...
	ПараметрыКонтактнойИнформации.РазмещеныНаФорме = РазмещеныНаФорме;
...
Функция ПараметрыДляАвтозаполненияФормыТС(Постановка=Истина)
	...
	Если Постановка Тогда
		ИмяФормыУведомления = "Отчет.РегламентированноеУведомлениеТС1.Форма.Форма2020_1";
...
Процедура РассчитатьТорговыйСбор()
	...
	Если Объект.ДатаНачалаОсуществленияДеятельности >= Дата("20250101") 
		ИЛИ Объект.КодОбъектаОсуществленияТорговли = "03" Тогда
		...
		Коэф = КоэффициентДефляторНаДату(Объект.ДатаНачалаОсуществленияДеятельности);
	Иначе
		Коэф = 1;
	КонецЕсли; 
	
	// домножаем ставки на дефлятор текущего года
	ДанныеТарифа.СтавкаДо50квм     = Окр(ДанныеТарифа.СтавкаДо50квм * Коэф, 2);
	ДанныеТарифа.СтавкаБольше50квм        = Окр(ДанныеТарифа.СтавкаБольше50квм * Коэф, 2);
	...
	Если Объект.КодОбъектаОсуществленияТорговли = "08"
		ИЛИ Объект.КодОбъектаОсуществленияТорговли = "03" Тогда
...
Функция ПолучитьДанныеТарифа()
	...
	Для Каждого ТекущаяСтрокаСтавки Из СтавкиСбораТаблицаЗначений Цикл
		Если ТекущаяСтрокаСтавки.ЗонаТарифа = ЗонаТарифа
			И ТекущаяСтрокаСтавки.ДействуетС <= Объект.ДатаНачалаОсуществленияДеятельности Тогда
