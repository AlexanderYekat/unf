...
Процедура КопироватьСтроки(Команда)
	...
	Если КопированиеТабличнойЧастиКлиент.МожноКопироватьСтроки(СвояНоменклатура, Элементы.СвояНоменклатура.ТекущиеДанные) Тогда
...
Процедура КопироватьСтрокиНаСервере(КоличествоСкопированных)
	...
	КопированиеТабличнойЧастиСервер.Копировать(
		СвояНоменклатура, 
		Элементы.СвояНоменклатура.ВыделенныеСтроки, 
		КоличествоСкопированных);
	
КонецПроцедуры

&НаСервере
Процедура ВставитьСтрокиНаСервере(КоличествоСкопированных, КоличествоВставленных) Экспорт
	
	Справочники.СегментыНоменклатуры.ВставитьСтроки(
		ЭтотОбъект,
		"СвояНоменклатура",
		Элементы,
		КоличествоСкопированных,
...
Функция ПолеОтбораДополнительногоРеквизита(Заголовок, ТипЗначения, Ссылка, УточнениеАдреса = Ложь)
	...
	ПолеДопРеквизита = СтрШаблон("Служебные реквизиты.Ссылка.%1", Заголовок);
