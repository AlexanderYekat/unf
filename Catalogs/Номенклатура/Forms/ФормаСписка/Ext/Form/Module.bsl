...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("КнопкаПеренестиВДокументЗаголовок") Тогда
		КнопкаПеренестиВДокументЗаголовок = Параметры.КнопкаПеренестиВДокументЗаголовок;
		Если Не ПустаяСтрока( КнопкаПеренестиВДокументЗаголовок ) Тогда
			Элементы.ПеренестиВДокумент.Заголовок = КнопкаПеренестиВДокументЗаголовок;
		КонецЕсли;
	КонецЕсли;
	...
	Поставщик = ?(Параметры.Свойство("Контрагент"), Параметры.Контрагент, Неопределено);
	ВыборПоНоменклатуреПоставщика = Параметры.Свойство("НоменклатураДляНоменклатурыПоставщика");
	Если ВыборПоНоменклатуреПоставщика Тогда
			
		ЭтаФорма.АвтоЗаголовок = Ложь;
		
		Элементы.СтраницыНоменклатуры.ТекущаяСтраница = Элементы.СтраницаНоменклатураПоставщиков;
		ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков";
		ЭтаФорма.Заголовок = НСтр("ru='Номенклатура поставщика'");
		Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюКнопкаВыбрать.Видимость = Истина;
		
		Если ИспользоватьСегментыНоменклатуры Тогда
			Элементы.ОтборКатегорииИерархияПереключатель.КоличествоКолонок = 2;
		КонецЕсли;
		
		НоменклатураДляНоменклатурыПоставщика = Параметры.НоменклатураДляНоменклатурыПоставщика;
		
		Если ЗначениеЗаполнено(НоменклатураДляНоменклатурыПоставщика.Номенклатура) Тогда
			УстановитьОтборДляНоменклатурыПоставщиковПоНоменклатуре();
		КонецЕсли;
	КонецЕсли;
	...
	ИерархияВключаяВложенные = ОбщегоНазначения.ХранилищеНастроекДанныхФормЗагрузить(
	ИмяФормы,
	"ВключаяВложенные",
	Ложь);
	
	Если ИерархияВключаяВложенные Тогда
		Элементы.ОтборИерархияКонтекстноеМенюИерархияВключаяВложенные.Картинка = БиблиотекаКартинок.НаследованиеВключено;
	Иначе
		Элементы.ОтборИерархияКонтекстноеМенюИерархияВключаяВложенные.Картинка = БиблиотекаКартинок.НаследованиеОтключено;
	КонецЕсли;
	...
	Если Параметры.Свойство("ИмяФормыВладельца") И Параметры.ИмяФормыВладельца = "Обработка.РабочееМестоКассира.Форма.ФормаРМК" Тогда
		ТипВладельцаФормы = "НовоеРМК";
	Иначе
		ТипВладельцаФормы = ?(Параметры.Свойство("ТипВладельца"), Параметры.ТипВладельца, Неопределено);
	КонецЕсли;
	...
	ИспользоватьНоменклатуруПоставщиковПриПодборе = УчетНоменклатурыПоставщиков И ЭтоПодбор И РаботаСЗакупками
													И (Параметры.Свойство("ИмяТабличнойЧасти") И Параметры.ИмяТабличнойЧасти = "Запасы");
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаВидыПодбора", "Видимость", ИспользоватьНоменклатуруПоставщиковПриПодборе И НЕ ВыборПоНоменклатуреПоставщика);
	...
	Если Параметры.РежимВыбора 
		...
		РежимВыбораГруппы = Истина;
	Иначе
		РежимВыбораГруппы = Ложь;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ГруппаКатегория) Тогда
		...
		Если РежимВыбора И Параметры.Свойство("ТекущаяСтрока") Тогда
			ТекущаяНоменклатураСтроки = Параметры.ТекущаяСтрока;
		КонецЕсли;
		
		Если ТипЗнч(ГруппаКатегория) = Тип("СправочникСсылка.Номенклатура") 
			И ОтборКатегорииИерархияПереключатель = Перечисления.ВидыОтборовНоменклатуры.ГруппыНоменклатуры Тогда
			ОтборИерархияТекущая = ГруппаКатегория;
			УстановитьОтборИерархияНаСервере();
		ИначеЕсли ОтборКатегорииИерархияПереключатель = Перечисления.ВидыОтборовНоменклатуры.КатегорииНоменклатуры Тогда
			...
			Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда 
				ОтборКатегорииНоменклатураПоставщиковПриАктивизацииСтрокиНаСервере();
			Иначе
	...
	РазныеВалюты = (ЭтоПодбор И ЗначениеЗаполнено(ВалютаПодбора) И Не ВалютаДокумента = ВалютаПодбора) Или (Не ЭтоПодбор И УчетВалютныхОпераций);
	...
	ИспользоватьАктВыполненныхРабот = ПолучитьФункциональнуюОпцию("ИспользоватьАктыВыполненныхРабот");
	...
	Если ТипЗнч(ОтборВидЦен) = Тип("СправочникСсылка.ВидыЦен") Тогда
		...
		МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.Недействителен", Ложь)); 
		
		НовыеПараметрыВыбора =  Новый ФиксированныйМассив(МассивПараметровВыбора);
		
		Элементы.ОтборВидЦен.ПараметрыВыбора = НовыеПараметрыВыбора;
	...
	Если ВыборПоНоменклатуреПоставщика Тогда 
		ЗаполнитьДеревоГруппПоставщика();
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(ЭтаФорма.ВладелецФормы) = Тип("ПолеФормы")
		И ЭтаФорма.ВладелецФормы.Имя = "НастройкаРМКБыстрыеТоварыНоменклатура" Тогда
		ТипВладельца = "НастройкиРМК";
	КонецЕсли;
	
	Элементы.ДекорацияВидПодбораНоменклатура.Шрифт = Новый Шрифт(Элементы.ДекорацияВидПодбораНоменклатура.Шрифт,,,Истина); 
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_НоменклатураПоставщикаГруппа" Тогда 
		...
		ЗаполнитьДеревоГруппПоставщика(НоваяГруппа, Ложь);
...
&НаКлиенте
Процедура ДекорацияВидПодбораНажатие(Элемент) 
	
	Элемент.Шрифт = Новый Шрифт(Элемент.Шрифт,,,Истина);
	
	Если Элемент.Имя = "ДекорацияВидПодбораНоменклатура" И (ТекущаяСтраницаНоменклатуры = "СписокЗапасы" Или ТекущаяСтраницаНоменклатуры = "ДеревоОстатки") Тогда
		Возврат
	КонецЕсли;
	
	Если Не Элемент.Имя = "ДекорацияВидПодбораНоменклатура" И ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		Возврат
	КонецЕсли;
	...
	ТекущаяСтраницаНоменклатуры = ?(ТекущаяСтраницаНоменклатуры = "СписокЗапасы", "СписокНоменклатураПоставщиков", "СписокЗапасы");
	
	Если Элемент.Имя = "ДекорацияВидПодбораНоменклатура" Тогда
		Элементы.ДекорацияПодбораНоменклатураПоставщика.Шрифт = Новый Шрифт(Элементы.ДекорацияПодбораНоменклатураПоставщика.Шрифт,,,Ложь);
	Иначе
		Элементы.ДекорацияВидПодбораНоменклатура.Шрифт = Новый Шрифт(Элементы.ДекорацияВидПодбораНоменклатура.Шрифт,,,Ложь); 
	КонецЕсли;
	
	УстанавливатьВидимостьИДоступность = Истина;
	
	Если Элементы.АналогиПредставлениеОтбора.Видимость Тогда 
		АналогиОтменаОтбора();
		УстанавливатьВидимостьИДоступность = Ложь;
	КонецЕсли;
	...
	Элементы.СтраницыНоменклатуры.ТекущаяСтраница = ?(Элементы.СтраницыНоменклатуры.ТекущаяСтраница = Элементы.СтраницаЗапасы, Элементы.СтраницаНоменклатураПоставщиков, Элементы.СтраницаЗапасы);
	
	ВыполнитьОперацииПриИзмененииВидаПодбораНаСервере(УстанавливатьВидимостьИДоступность);
	
	Если ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.КатегорииНоменклатуры")
		Или ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.СегментыНоменклатуры") Тогда
	...
	ИначеЕсли ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатуры") Тогда
		...
		УстановитьОтборПоИерархииПриИзмененииВидаПодбора();
		
	ИначеЕсли Не ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" 
		И ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатурыПоставщиков") Тогда
		
		Элементы.СтраницыКатегорииИерархия.ТекущаяСтраница = Элементы.КатегорииОтбор;
		
		ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.КатегорииНоменклатуры");
		ОтборКатегорииПриАктивизацииСтрокиНаКлиенте();
...
&НаСервере
Процедура ВыполнитьОперацииПриИзмененииВидаПодбораНаСервере(УстанавливатьВидимостьИДоступность)
	
	Если Не ТекущаяСтраницаНоменклатуры = "ДеревоОстатки" И УстанавливатьВидимостьИДоступность Тогда
...
Процедура ОтборИерархияПриАктивизацииСтроки(Элемент)
	...
	Если ОтборИерархияТекущая = НовоеЗначение И ОтборИерархияТекущаяПредставление = НовоеПредставление Тогда
		Возврат
	КонецЕсли;
	...
	Если ДанныеМеток.Количество() Тогда
		...
		УдалитьМеткиОтбораСервер(,СписокГрупп);
...
&НаСервере
Процедура УдалитьМеткиОтбораСервер(ИмяТаблицыМеток = "ДанныеМеток", СписокГрупп = Неопределено)
	
	УдалитьМеткиОтбора();
	
	ЭтаФорма[ИмяТаблицыМеток].Очистить();
	
	Если Не СписокГрупп = Неопределено Тогда
		РаботаСОтборами.ОбновитьЭлементыМеток(ЭтотОбъект, СписокГрупп, ИмяТаблицыМеток);
	КонецЕсли;
...
Процедура ПоказыватьОстаткиПриИзменении(Элемент)
	...
	Если Не ПоказыватьОстатки И ТекущаяСтраницаНоменклатуры = "ДеревоОстатки" Тогда
		...
		И НЕ ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков"); 
...
Процедура СписокЗапасыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "КартинкаОстатки" Или (ЭтоПодбор И Не Поле.Имя = "КартинкаКорзина") Тогда
		...
		Если Не ТекущиеДанные = Неопределено Тогда
			...
			СтруктураДанныхНоменклатурыДляРежимаОстатки.Вставить("Номинал", ТекущиеДанные.Номинал);
		Иначе
			Возврат;
		КонецЕсли;
...
Процедура ПослеЗакрытияВопросаПомеченНаУдаление(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда 
			...
			СтруктураПараметров= Новый Структура("Номенклатура, НоменклатураПоставщика, СтруктураНастроек, Склад, Характеристика, Партия");
			
			СтруктураПараметров.Номенклатура = ПараметрыОповещения.ВыбраннаяСтрока.Номенклатура;
			СтруктураПараметров.НоменклатураПоставщика = ПараметрыОповещения.ВыбраннаяСтрока;
			СтруктураПараметров.СтруктураНастроек = СтруктураСохраняемыхНастроек;
			СтруктураПараметров.Склад = ПустойСклад;
			СтруктураПараметров.Характеристика = ПустаяХарактеристика;
			СтруктураПараметров.Партия = ПустаяПартия;
...
&НаКлиенте
Процедура ОтборНоменклатураПоставщиковПриАктивизацииСтроки(Элемент) 
	
	Если ТекущаяСтраницаНоменклатуры = "ДеревоОстатки" 
		Или Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные = Неопределено 
		Или Не ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатурыПоставщиков") Тогда
		Возврат;
	КонецЕсли;
	
	НовоеЗначение = Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Значение;
	НовоеПредставление = Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Представление;
	
	Если ОтборИерархияТекущая = НовоеЗначение И ОтборИерархияТекущаяПредставление = НовоеПредставление Тогда
		Возврат
	КонецЕсли;
	
	ОтображатьКомандыПеремещения = ЗначениеЗаполнено(НовоеЗначение);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборНоменклатураПоставщиковПереместитьВверх", "Доступность", ОтображатьКомандыПеремещения);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборНоменклатураПоставщиковПереместитьВниз", "Доступность", ОтображатьКомандыПеремещения);
	...
	Для Каждого элОтбора Из ЭтотОбъект[ТекущаяСтраницаНоменклатуры].КомпоновщикНастроек.ФиксированныеНастройки.Отбор.Элементы Цикл
		...
		Если Строка(элОтбора.ЛевоеЗначение) = "НоменклатураПоставщикаРодитель" Тогда
			
			Если элОтбора.ПравоеЗначение = НовоеЗначение И элОтбора.Представление = НовоеПредставление И элОтбора.Использование Тогда
				Возврат;
			Иначе
				УстановитьОтборПоИерархииНоменклатураПоставщиков();
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	// отбор не был установлен ранее, и текущее значение отбора = <Все группы>
	Если НЕ Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Представление = ПредставлениеВсеГруппы() Тогда
		УстановитьОтборПоИерархииНоменклатураПоставщиков();
	КонецЕсли;
...
&НаКлиенте
Процедура КорзинаРасшифровкаПриАктивизацииЯчейки(Элемент)
	...
	Если Не Элемент.ТекущийЭлемент.Имя = "КорзинаРасшифровкаКоличество" Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.КорзинаРасшифровкаКоличество.ФорматРедактирования = ?(ТекущиеДанные.ЗапретитьВводДробногоКоличества, "ЧДЦ=0", "");
...
&НаКлиенте
Процедура ИзменитьНоменклатуруПоставщика(Команда) 
	
	Если Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПоказатьЗначение(, Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Значение);
...
Процедура УстановитьПометкуУдаленияКатегории(Команда)
	...
	Если НЕ ТипЗнч(ТекущаяСтрокаДерева.Значение) = Тип("СправочникСсылка.КатегорииНоменклатуры") Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ТекущаяСтрокаДерева.Значение) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИерархияВключаяВложенные(Команда)
	...
	ИерархияВключаяВложенные = Не ИерархияВключаяВложенные;
...
Процедура НастройкиОтборов(Команда) 
	ОткрытьНастройкиОтборов(); 
КонецПроцедуры

&НаКлиенте
Процедура НастройкиОтборовНоменклатураПоставщиков(Команда)
	ОткрытьНастройкиОтборов();
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьНастройкиОтборов()
	...
	СтруктураНастроек.Вставить("ПодборПоНоменклатуреПоставщиков", ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков");
...
Процедура ВосстановитьНастройкиФормы(СтруктураНастроек, ДополнительныеПараметры) Экспорт
	...
	Если ЦенаВСписке = 0 И СтруктураНастроек.ЦенаВСписке = 1 Тогда
		
		ТекущиеДанные = Элементы[ТекущаяСтраницаНоменклатуры].ТекущиеДанные;
		Если Не ТекущиеДанные = Неопределено Тогда
			ЦенаВСписке = СтруктураНастроек.ЦенаВСписке;
			ОбновитьНадписьЦеныВПравойПанели(ТекущиеДанные.Ссылка);
		КонецЕсли;
...
&НаКлиенте
Процедура КомандыГрупп(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("КомандыГруппПродолжение", ЭтотОбъект);
	
	КомандыСпискаГруппы = Новый СписокЗначений;
	
	Если Элементы.ОтборИерархияКонтекстноеМенюГруппаВыбрать.Видимость Тогда
		КомандыСпискаГруппы.Добавить("Выбрать", НСтр("ru = 'Выбрать'"),,БиблиотекаКартинок.ВыбратьЗначение);
	КонецЕсли;
	
	КомандыСпискаГруппы.Добавить("Создать", НСтр("ru = 'Создать'"),,БиблиотекаКартинок.СоздатьГруппу);
	Если ЗначениеЗаполнено(Элементы.ОтборИерархия.ТекущиеДанные.Значение) Тогда
		КомандыСпискаГруппы.Добавить("Изменить", НСтр("ru = 'Изменить'"),,БиблиотекаКартинок.Изменить);
		КомандыСпискаГруппы.Добавить("Скопировать", НСтр("ru = 'Скопировать'"),,БиблиотекаКартинок.СкопироватьОбъект);
		КомандыСпискаГруппы.Добавить("УстановитьПометкуУдаления", НСтр("ru = 'Установить пометку удаления'"),,БиблиотекаКартинок.Удалить);
	КонецЕсли;
	
	Если ИерархияВключаяВложенные Тогда
		КомандыСпискаГруппы.Добавить("ИерархияВключаяВложенные", НСтр("ru = 'Показывать номенклатуру вложенных групп         '"),, БиблиотекаКартинок.НаследованиеВключено);
	Иначе
		КомандыСпискаГруппы.Добавить("ИерархияВключаяВложенные", НСтр("ru = 'Показывать номенклатуру вложенных групп         '"),, БиблиотекаКартинок.НаследованиеОтключено);
	КонецЕсли;
	
	КомандыСпискаГруппы.Добавить("ДопУпорядочиваниеУвеличить", НСтр("ru = 'Вверх'"),,БиблиотекаКартинок.ПереместитьВверх);
	КомандыСпискаГруппы.Добавить("ДопУпорядочиваниеУменьшить", НСтр("ru = 'Вниз'"),,БиблиотекаКартинок.ПереместитьВниз);
	
	ПоказатьВыборИзСписка(ОписаниеОповещения, КомандыСпискаГруппы, ТекущийЭлемент);
	
КонецПроцедуры

&НаКлиенте
Процедура КомандыКатегорий(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("КомандыКатегорийПродолжение", ЭтотОбъект);
	
	КомандыСпискаКатегории = Новый СписокЗначений;
	
	КомандыСпискаКатегории.Добавить("Создать", НСтр("ru = 'Создать'"),,БиблиотекаКартинок.СоздатьЭлементСписка);
	Если ЗначениеЗаполнено(Элементы.ОтборКатегории.ТекущиеДанные.Значение) Тогда
		КомандыСпискаКатегории.Добавить("Изменить", НСтр("ru = 'Изменить'"),,БиблиотекаКартинок.Изменить);
		КомандыСпискаКатегории.Добавить("Скопировать", НСтр("ru = 'Скопировать'"),,БиблиотекаКартинок.СкопироватьОбъект);
		КомандыСпискаКатегории.Добавить("УстановитьПометкуУдаления", НСтр("ru = 'Установить пометку удаления        '"),,БиблиотекаКартинок.Удалить);
	КонецЕсли;
	КомандыСпискаКатегории.Добавить("ОткрытьСправочникКатегории", НСтр("ru = 'Список категорий        '"),,БиблиотекаКартинок.Справочник);
	КомандыСпискаКатегории.Добавить("ДопУпорядочиваниеУвеличить", НСтр("ru = 'Вверх'"),,БиблиотекаКартинок.ПереместитьВверх);
	КомандыСпискаКатегории.Добавить("ДопУпорядочиваниеУменьшить", НСтр("ru = 'Вниз'"),,БиблиотекаКартинок.ПереместитьВниз);
	
	ПоказатьВыборИзСписка(ОписаниеОповещения, КомандыСпискаКатегории, ТекущийЭлемент);
	
КонецПроцедуры

&НаКлиенте
Процедура КомандыСегментов(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("КомандыСегментовПродолжение", ЭтотОбъект);
	
	КомандыСпискаСегменты = Новый СписокЗначений;
	
	КомандыСпискаСегменты.Добавить("Изменить", НСтр("ru = 'Изменить        '"),,БиблиотекаКартинок.Изменить);
	
	ПоказатьВыборИзСписка(ОписаниеОповещения, КомандыСпискаСегменты, ТекущийЭлемент);

КонецПроцедуры

&НаКлиенте
Процедура КомандыНоменклатурыПоставщиков(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("КомандыНоменклатурыПоставщиковПродолжение", ЭтотОбъект);
	
	КомандыСпискаГруппы = Новый СписокЗначений;
	
	КомандыСпискаГруппы.Добавить("Создать", НСтр("ru = 'Создать                  '"),,БиблиотекаКартинок.СоздатьГруппу);
	Если ЗначениеЗаполнено(Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Значение) Тогда
		КомандыСпискаГруппы.Добавить("Изменить", НСтр("ru = 'Изменить'"),,БиблиотекаКартинок.Изменить);
		КомандыСпискаГруппы.Добавить("Скопировать", НСтр("ru = 'Скопировать'"),,БиблиотекаКартинок.СкопироватьОбъект);
		КомандыСпискаГруппы.Добавить("УстановитьПометкуУдаления", НСтр("ru = 'Установить пометку удаления         '"),,БиблиотекаКартинок.Удалить);
		КомандыСпискаГруппы.Добавить("ДопУпорядочиваниеУвеличить", НСтр("ru = 'Вверх'"),,БиблиотекаКартинок.ПереместитьВверх);
...
Процедура СоздатьНоменклатуруПоставщика(Команда)
	СоздатьГруппуНоменклатураПоставщика();
КонецПроцедуры

&НаКлиенте
Процедура СкоприроватьНоменклатуруПоставщика(Команда)
	СкопироватьГруппуПоставщика();
КонецПроцедуры

&НаКлиенте
Процедура УстановитьПометкуУдаленияНоменклатураПоставщика(Команда)
	УстановитьПометкуУдаленияГруппыПоставщика();
КонецПроцедуры

&НаКлиенте
Процедура ПереместитьВверхНоменклатураПоставщика(Команда)
	РеквизитДопУпорядочиванияУвеличитьНоменклатураПоставщика();
КонецПроцедуры

&НаКлиенте
Процедура ПереместитьВнизНоменклатураПоставщика(Команда)
	РеквизитДопУпорядочиванияУменьшитьНоменклатураПоставщика();
КонецПроцедуры
...
&НаСервере
Процедура ЗаполнитьДеревоГруппПоставщика(ГруппаТекущейСтроки = Неопределено, ОбновлятьСоставОтборов = Истина)
	
	Если ОбновлятьСоставОтборов Тогда
		
		ЭлементОтбора = Перечисления.ВидыОтборовНоменклатуры.ГруппыНоменклатурыПоставщиков;
		Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
			НомерПозицииСпискаВыбора = ?(ИспользоватьСегментыНоменклатуры, 3, 2);
			Элементы.ОтборКатегорииИерархияПереключатель.СписокВыбора.Вставить(НомерПозицииСпискаВыбора, ЭлементОтбора, НСтр("ru = 'Группы поставщика'"));
			
			Если ИспользоватьСегментыНоменклатуры Тогда
				
				ЭлементОтбораПоСегменту = Перечисления.ВидыОтборовНоменклатуры.СегментыНоменклатуры;
				ЭлементОтбораСпискаПоСегменту = Элементы.ОтборКатегорииИерархияПереключатель.СписокВыбора.НайтиПоЗначению(ЭлементОтбораПоСегменту);
				
				Если ЭлементОтбораСпискаПоСегменту <> Неопределено Тогда
					Элементы.ОтборКатегорииИерархияПереключатель.СписокВыбора.Удалить(ЭлементОтбораСпискаПоСегменту);
				КонецЕсли;
				
				Элементы.ОтборКатегорииИерархияПереключатель.СписокВыбора.Вставить(1, ЭлементОтбораПоСегменту,НСтр("ru = 'Сегменты'"));
				
			КонецЕсли;
			
		Иначе
			ЭлементОтбораПоГруппеПоставщика = Элементы.ОтборКатегорииИерархияПереключатель.СписокВыбора.НайтиПоЗначению(ЭлементОтбора);
			
			Если ЭлементОтбораПоГруппеПоставщика <> Неопределено Тогда
				Элементы.ОтборКатегорииИерархияПереключатель.СписокВыбора.Удалить(ЭлементОтбораПоГруппеПоставщика);
			КонецЕсли;
			...
			Если ИспользоватьСегментыНоменклатуры Тогда
				...
				Элементы.ОтборКатегорииИерархияПереключатель.СписокВыбора.Вставить(2, ЭлементОтбораПоСегменту,НСтр("ru = 'Сегменты'"));
	...
	|	НоменклатураПоставщиков.ПометкаУдаления КАК ПометкаУдаления,
	|	ВЫБОР
	|		КОГДА НЕ НоменклатураПоставщиков.ЭтоГруппа
	|			ТОГДА ВЫБОР
	|					КОГДА НоменклатураПоставщиков.ПометкаУдаления
	|						ТОГДА 5
	|					ИНАЧЕ 4
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	...
	|			КОНЕЦ
	|	КОНЕЦ КАК Картинка,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(НоменклатураПоставщиков.Ссылка) КАК Представление,
	|	НоменклатураПоставщиков.ЭтоГруппа КАК ЭтоГруппа,
	|	НоменклатураПоставщиков.РеквизитДопУпорядочиванияУНФ КАК РеквизитДопУпорядочиванияУНФ
	|ИЗ
	|	Справочник.НоменклатураПоставщиков КАК НоменклатураПоставщиков
	|ГДЕ
	|	НоменклатураПоставщиков.ЭтоГруппа
	|	И НоменклатураПоставщиков.Владелец = &Поставщик
	|
	|УПОРЯДОЧИТЬ ПО
	|	НоменклатураПоставщиков.ЭтоГруппа ИЕРАРХИЯ,
	|	РеквизитДопУпорядочиванияУНФ ИЕРАРХИЯ";
	...
	ЗначениеВРеквизитФормы(Дерево,"ОтборНоменклатураПоставщиков");
	
	ИдентификаторСтроки = Неопределено;
	Если ГруппаТекущейСтроки <> Неопределено Тогда
		ИдентификаторСтроки = ИдентификаторСтрокиДереваПоЗначению(ОтборНоменклатураПоставщиков, ГруппаТекущейСтроки);
	КонецЕсли;
	
	Если ИдентификаторСтроки <> Неопределено Тогда
		Элементы.ОтборНоменклатураПоставщиков.ТекущаяСтрока = ИдентификаторСтроки;
	КонецЕсли;
...
Процедура ОтборВидЦенПриИзменении(Элемент)
	...
	ТекущиеДанные = Элементы[ТекущаяСтраницаНоменклатуры].ТекущиеДанные;
	Если Не ТекущиеДанные = Неопределено Тогда
		ОтборВидЦенСервер(ТекущиеДанные.Ссылка);
	Иначе
		ОтборВидЦенСервер();
	КонецЕсли;
...
&НаСервере
Процедура ОтборВидЦенСервер(Номенклатура = Неопределено)
	
	УстановитьВидЦенДляСписков();
	УстановитьПараметрыЗапросовСписков();
	
	Если Не Номенклатура = Неопределено Тогда
		ОбновитьНадписьЦеныВПравойПанели(Номенклатура);
	КонецЕсли;
...
Процедура УстановитьВидЦенДляСписков(ОбновлятьВидимость = Истина)
	...
	КонецЕсли;
	ОтборПоВидуЦен = ТипЗнч(ОтборВидЦен) = Тип("СправочникСсылка.ВидыЦен");
	...
	Если ЗначениеЗаполнено(ОтборВидЦен) И Не УправлениеДоступом.ЧтениеРазрешено(ОтборВидЦен) Тогда
		...
		Если ОтборПоВидуЦен Тогда
			ОтборВидЦен = Справочники.ВидыЦен.ПустаяСсылка();
		ИначеЕсли ОтборПоВидуЦенКонтрагентов Тогда
			ОтборВидЦен = Справочники.ВидыЦенКонтрагентов.ПустаяСсылка();
		КонецЕсли;
		
	КонецЕсли;
	
	ОтборВидЦенВалютаЦены = ОтборВидЦен.ВалютаЦены;
	ОтборВидЦенНесколькоВалют = Ложь;
	
	Если ОтборПоВидуЦен Тогда
		ОтборВидЦенНесколькоВалют = ОтборВидЦен.НесколькоВалютДляЦен;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОтборВидЦен) Тогда
		
		ЗаголовокЦен = НСтр("ru = '<Вид цен не выбран>'");
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ВалютаПредставление) И ОтборПоВидуЦен И Не ОтборВидЦен.НесколькоВалютДляЦен Тогда
			СимвольноеПредставление = " " + ВалютаПредставление;
		ИначеЕсли ЗначениеЗаполнено(ВалютаПредставление) И ОтборПоВидуЦенКонтрагентов Тогда
			СимвольноеПредставление = " " + ВалютаПредставление;
		Иначе
	...
	Элементы.СписокНоменклатураПоставщиковЦена.Заголовок = ЗаголовокЦен;
	Элементы.ДеревоОстаткиЦена.Заголовок		= ЗаголовокЦен;
	Элементы.КорзинаРасшифровкаГруппаЦенаВВалюте.Заголовок = ЗаголовокЦен;
...
Процедура ПереформироватьЗапросыСписков()
	...
	Если Не ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		НоваяСтраницаНоменклатуры = "СписокЗапасы";
		УстановитьСтраницуСпискаНоменклатуры(НоваяСтраницаНоменклатуры);
	Иначе
		УстановитьВидимостьИДоступность();
...
Процедура СвернутьРазвернутьПанельОтборов(Элемент)
	...
	Если ТекущиеДанные = Неопределено Тогда
		...
		Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюДобавитьВКорзину.Видимость = Ложь;
		Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюУдалитьИзКорзины.Видимость = Ложь;
...
&НаКлиенте
Процедура УстановитьОтборПоИерархииНоменклатураПоставщиков()
	
	Если Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ЭтоОтборПоГруппе = ЗначениеЗаполнено(Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Значение);
	...
	Представление	= Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Представление;
	...
	Если ЭтоОтборПоГруппе Тогда
		...
		КонецЕсли;
		ПравоеЗначение = Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Значение;
		
	ИначеЕсли Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Представление = ПредставлениеВсеГруппы() Тогда
		
		Использование = Ложь;
		
	ИначеЕсли Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Представление = ПредставлениеНетГруппы() Тогда
		
		ПравоеЗначение = ПредопределенноеЗначение("Справочник.НоменклатураПоставщиков.ПустаяСсылка");
		
	КонецЕсли;

	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		ЭтотОбъект["СписокНоменклатураПоставщиков"],
		"НоменклатураПоставщикаРодитель",
		ПравоеЗначение,
		Сравнение,
		Представление,
		Использование);
	...
	ОтборИерархияТекущаяПредставление = Представление;
	
	Если РежимВыбора И ЗначениеЗаполнено(ТекущаяНоменклатураСтроки) Тогда
		Элементы.СписокНоменклатураПоставщиков.ТекущаяСтрока = ТекущаяНоменклатураСтроки;
		ТекущаяНоменклатураСтроки = Неопределено;
	КонецЕсли
...
Процедура УстановитьОтборПоИерархии()
	...
	Если РежимВыбора И ЗначениеЗаполнено(ТекущаяНоменклатураСтроки) Тогда
		Элементы.СписокЗапасы.ТекущаяСтрока = ТекущаяНоменклатураСтроки;
		ТекущаяНоменклатураСтроки = Неопределено;
	КонецЕсли
...
&НаКлиенте
Процедура ОтключитьОтборПоИерархииВнутреннейНоменклатурыСписокНоменклатураПоставщиков()
...
&НаКлиенте
Процедура ОтключитьОтборПоГруппамПоставщиков()
...
&НаКлиенте
Процедура ОтключитьОтборПоКатегорииСписокНоменклатураПоставщиков()
	
	Если Элементы.ОтборКатегории.ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПравоеЗначение = Элементы.ОтборКатегории.ТекущиеДанные.Значение;
	Сравнение		= ВидСравненияКомпоновкиДанных.Равно;
	Использование	= Ложь;
	Представление	= Элементы.ОтборКатегории.ТекущиеДанные.Представление;
...
&НаСервере
Процедура СохранитьНастройки(ИзФормы = Ложь, ЗакрытиеФормы = Ложь)
	...
	СохранитьОбщиеНастройки(ЗакрытиеФормы);
	...
	Если РежимВыбора И ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		
		СтрокаНастроек = "ПоказыватьЦены,ПоказыватьОстатки,ДействиеПриПоискеПоШК,ОстаткиВидимость,
		|ПодбиратьВесьОстаток,ОтборОстатки,ОтборВидЦен,ОтборКатегорииИерархияПереключатель";
		
		ИмяКлючаОбъекта = "СписокНоменклатураПоставщиков";
	...
	Если РежимВыбора Тогда
		...
		СтрокаНастроек = "ПоказыватьЦены,ПоказыватьОстатки,ДействиеПриПоискеПоШК,НастройкиПоиска,НастройкиПоискаКакИскать,
		|ПоискВДопРеквизитахИСведениях,ПоискВХарактеристиках,ПоискВШтрихКодах,ПоискВДопСведениях,ОстаткиВидимость,
	...
	Если ЭтоПодбор Тогда
		ИмяКлючаОбъектаФормаВыбора = "СписокНоменклатураФормаПодбораОбщая";
	Иначе
...
Процедура ВосстановитьНастройки()
	...
	Если РежимВыбора И ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		Если Не ЗначениеЗаполнено(ОтборКатегорииИерархияПереключатель) Тогда
			ОтборКатегорииИерархияПереключатель = Перечисления.ВидыОтборовНоменклатуры.КатегорииНоменклатуры;
		КонецЕсли;
	ИначеЕсли Не РежимВыбораГруппы Тогда
	...
	Иначе
		...
		ГруппаКатегория = Справочники.Номенклатура.ПустаяСсылка();
	КонецЕсли;
	...
	Если ЭтоПодбор Тогда
		...
		ЕстьПартииСНДС = ?(Параметры.Свойство("ЕстьПартииСНДС"), Параметры.ЕстьПартииСНДС, Ложь);
		...
		НалогообложениеНДСОрганизации = Неопределено;
		Если ЕстьПартииСНДС И ЗначениеЗаполнено(Организация) Тогда
			СтруктурнаяЕдиница = ?(Параметры.Свойство("СтруктурнаяЕдиница") И Не СтруктурнаяЕдиницаВТабличнойЧасти, Параметры.СтруктурнаяЕдиница, Неопределено);
			НалогообложениеНДСОрганизации = НалогиУНФ.НалогообложениеНДС(Организация,, Дата);
		КонецЕсли;
		
		Если ЕстьПартииСНДС И Не ИсключаемыйТипНДСКомиссия(ВидОперации, ТипВладельцаФормы) 
			И НалогообложениеНДСОрганизации = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
		...
		Иначе
			Если Не СтруктурнаяЕдиницаВТабличнойЧасти Тогда
				...
				Если ТипЗнч(ПараметрыСтруктурнаяЕдиница) = Тип("СправочникСсылка.СтруктурныеЕдиницы") Тогда
					Если Не ПараметрыСтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.МагазинГруппаСкладов Тогда
	...
	Если (Параметры.Свойство("Отбор") И Параметры.Отбор.Свойство("КатегорияНоменклатуры")) Или ОграничениеДоступаЛупа Тогда
		...
		Элементы.СтраницыКомандыКатегорийГруппСегментов.Видимость = Ложь;
		ВидимостьКатегорииГруппы = Ложь;
	Иначе
		
		Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
			ТекущаяНоменклатура = Параметры.ТекущаяСтрока.Номенклатура; 
			Элементы.СписокНоменклатураПоставщиков.ТекущаяСтрока = Параметры.ТекущаяСтрока;
		Иначе
			ТекущаяНоменклатура = Параметры.ТекущаяСтрока;
		КонецЕсли;
		
		ВидимостьКатегорииГруппы = ?(РежимВыбораГруппы, Ложь, Элементы.ОтборКатегорииИерархияПереключатель.Видимость);
		Если ОтборКатегорииИерархияПереключатель = Перечисления.ВидыОтборовНоменклатуры.ГруппыНоменклатуры Тогда
			ГруппаТекущейНоменклатуры = ?(РежимВыбораНоменклатураЗаполнена, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяНоменклатура, "Родитель"), ГруппаКатегория);
			...
			КонецЕсли;
		ИначеЕсли ОтборКатегорииИерархияПереключатель = Перечисления.ВидыОтборовНоменклатуры.КатегорииНоменклатуры Тогда
			ГруппаТекущейНоменклатуры = ?(РежимВыбораНоменклатураЗаполнена, ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяНоменклатура, "КатегорияНоменклатуры"), ГруппаКатегория);
			Если ЗначениеЗаполнено(ГруппаТекущейНоменклатуры) Тогда
			...
			КонецЕсли;
		ИначеЕсли ОтборКатегорииИерархияПереключатель = Перечисления.ВидыОтборовНоменклатуры.ГруппыНоменклатурыПоставщиков И Не ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
...
Функция СтруктураВыбора()
	...
	СтруктураВозврата.Вставить("НалогообложениеНДСПартии",);
	СтруктураВозврата.Вставить("СтатусПартии",);
	СтруктураВозврата.Вставить("Весовой",);
	СтруктураВозврата.Вставить("ВалютаЦены",);
	СтруктураВозврата.Вставить("ЗапретитьВводДробногоКоличества",);
	СтруктураВозврата.Вставить("НоменклатураПоставщика",);
...
Процедура ДобавитьНоменклатуруВКорзину(СтруктураВыбора)
	...
	НоменклатураПоставщика = ?(ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков"
								, СтруктураВыбора.НоменклатураПоставщика, ПредопределенноеЗначение("Справочник.НоменклатураПоставщиков.ПустаяСсылка"));
	СтруктураПоискаВКорзине.Вставить("НоменклатураПоставщика", НоменклатураПоставщика);
	...
	Если Не ЗначениеЗаполнено(СтруктурнаяЕдиница) Тогда 
		СтруктурнаяЕдиница = СкладПоУмолчанию(СтруктураВыбора.Номенклатура);
	КонецЕсли;
		
	СтруктураПоискаВКорзине.Вставить("СтруктурнаяЕдиница", СтруктурнаяЕдиница);
	
	СтруктураПоискаВКорзине.Вставить("Ячейка", ?(СтруктураВыбора.Ячейка = Неопределено, ПредопределенноеЗначение("Справочник.Ячейки.ПустаяСсылка"), СтруктураВыбора.Ячейка));
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		...
		СтрокаКорзины.Количество = ?((СтруктураВыбора.Весовой И ТипВладельцаФормы = "НовоеРМК"), 0, 1);
		СтрокаКорзины.СтавкаНДС = СтруктураВыбора.СтавкаНДСНоменклатуры;
		СтрокаКорзины.СтруктурнаяЕдиница = СтруктурнаяЕдиница;
		СтрокаКорзины.НоменклатураПоставщика = НоменклатураПоставщика;
		...
		ИсключаеиыйТипНДС = ИсключаемыйТипНДСКомиссия(ВидОперации, ТипВладельцаФормы);
		Если НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС")
			Или Не ЗначениеЗаполнено(НалогообложениеНДС) Тогда
			
			Если ТекущаяСтраницаНоменклатуры = "ДеревоОстатки" Тогда
				
				Если НЕ ИсключаеиыйТипНДС И СтруктураВыбора.СтатусПартии = ПредопределенноеЗначение("Перечисление.СтатусыПартий.ТоварыНаКомиссии") Тогда
					Если СтруктураВыбора.НалогообложениеНДСПартии = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС") Тогда
						СтавкаНДСНоменклатуры = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСБезНДС();
					Иначе
		...
		Иначе
			...
			Если ТекущаяСтраницаНоменклатуры = "ДеревоОстатки" Тогда
				Если НЕ ИсключаеиыйТипНДС И СтруктураВыбора.СтатусПартии = ПредопределенноеЗначение("Перечисление.СтатусыПартий.ТоварыНаКомиссии")
					И СтруктураВыбора.НалогообложениеНДСПартии = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС") Тогда
					СтрокаКорзины.СтавкаНДС = СтруктураДанныхНоменклатурыДляРежимаОстатки.СтавкаНДСНоменклатуры;
				Иначе
			...
			ИначеЕсли НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.НаЭкспорт") Тогда
				СтрокаКорзины.СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСНоль();
			Иначе
				СтрокаКорзины.СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСБезНДС();
			КонецЕсли;
	...
	СтрокаКорзины.ВалютаДокумента = ВалютаДокумента;
	...
	НесколькоВалютВидаЦен = Ложь;
	Если Не РаботаСЗакупками И ОтборВидЦенНесколькоВалют Тогда
		НесколькоВалютВидаЦен = Истина;
	КонецЕсли;
	
	СтрокаКорзины.ЦенаВВалютеПодбора = СтрокаКорзины.Цена;

	Если РазныеВалюты Или НесколькоВалютВидаЦен Тогда
		
		СтрокаКорзины.ЦенаВВалютеПодбора = СтрокаКорзины.Цена;
		СтрокаКорзины.Валюта = СтруктураВыбора.ВалютаЦены;
		
		Если НесколькоВалютВидаЦен Тогда
			СтрокаКорзины.ВалютаПредставление = СтруктураВыбора.ВалютаЦены;
		КонецЕсли;
		...
		ПодборНоменклатурыВДокументахКлиент.ПересчитатьСтрокуТабличнойЧастиВВалюту(СтрокаКорзины,
			СтруктураВыбора.ВалютаЦены, ВалютаПересчета, ДатаПересчета, СуммаВключаетНДС, СтруктураОкругления);
...
&НаКлиенте
Процедура ВыбратьВСпискеНоменклатураПоставщиков(Команда)
	ВыбратьСтрокуСпискаНоменклатураПоставщиков();
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьВИерархии(Команда)
...
&НаКлиенте
Процедура СоздатьГруппуНоменклатураПоставщика()
	
	РодительЗначение = ?(Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные = Неопределено, ПредопределенноеЗначение("Справочник.НоменклатураПоставщиков.ПустаяСсылка"),
		Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Значение);
		
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("ЗначенияЗаполнения",  Новый Структура("Родитель, ЭтоГруппа, Владелец", РодительЗначение, Истина, Поставщик));
	ПараметрыЗаполнения.Вставить("ЭтоГруппа",  Истина);
	
	ОткрытьФорму("Справочник.НоменклатураПоставщиков.ФормаГруппы", ПараметрыЗаполнения, ЭтаФорма);
	
КонецПроцедуры

&НаКлиенте
Процедура СкопироватьГруппуПоставщика()
	
	ОткрытьФорму("Справочник.НоменклатураПоставщиков.ФормаГруппы", Новый Структура("ЗначениеКопирования", Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Значение));
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьПометкуУдаленияГруппыПоставщика()
	
	Если Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные = Неопределено Или Не ЗначениеЗаполнено(Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.Значение) Тогда
		Возврат;
	КонецЕсли;
	
	ТекущаяСтрокаДерева = ОтборНоменклатураПоставщиков.НайтиПоИдентификатору(Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные.ПолучитьИдентификатор());
	ТекущаяСтрокаДерева.ПометкаУдаления = ИзменитьПометкуУдаленияСервер(ТекущаяСтрокаДерева.Значение);
...
&НаКлиенте
Процедура РеквизитДопУпорядочиванияУвеличитьНоменклатураПоставщика()
	
	ИзменитьПорядокГруппыНоменклатурыНоменклатураПоставщика(-1);
	
КонецПроцедуры

&НаКлиенте
Процедура РеквизитДопУпорядочиванияУменьшитьНоменклатураПоставщика()
...
Процедура СписокЗапасыПриАктивизацииСтроки(Элемент)
	...
	Если НЕ ТекущаяСтраницаНоменклатуры = "СписокЗапасы" Тогда
		Возврат
	КонецЕсли;
	...
	Если Не ТекущиеДанные = Неопределено И ТекущиеДанные.Свойство("Ссылка")
		И ТекущиеДанные.Ссылка = ТекущаяНомеклатура Тогда
		Возврат
	КонецЕсли;
	...
	Иначе
		...
		ПодключитьОбработчикОжидания("ОбновитьОстаткиПоНоменклатуреКлиент", 0.5, Истина);
	...
	ТекущаяНомеклатура = ТекущиеДанные.Ссылка;
...
&НаКлиенте
Процедура СписокНоменклатураПоставщиковВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя = "СписокНоменклатураПоставщиковКартинкаКорзина" Тогда
		СтандартнаяОбработка = Ложь;
		ДобавитьВКорзинуНоменклатураПоставщика(Истина);
		Возврат;
	КонецЕсли;
	
	Если ЭтоПодбор И Не Поле.Имя = "СписокНоменклатураПоставщиковКартинкаКорзина" Тогда
		СтандартнаяОбработка = Ложь;
		
		ТекущиеДанные = Элементы.СписокНоменклатураПоставщиков.ТекущиеДанные;
		Если Не ТекущиеДанные = Неопределено Тогда
			ДобавитьВКорзинуНоменклатураПоставщика(Истина);
		КонецЕсли;
...
&НаКлиенте
Процедура СписокНоменклатураПоставщиковПриАктивизацииСтроки(Элемент)
	...
	Если ПоказыватьКорзину Тогда
		...
		Если ТекущиеДанные.КартинкаКорзина= 1 Тогда
			Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюДобавитьВКорзину.Видимость = Истина;
			Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюУдалитьИзКорзины.Видимость = Истина;
		Иначе
			Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюДобавитьВКорзину.Видимость = Истина;
			Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюУдалитьИзКорзины.Видимость = Ложь;
		КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьОстаткиПоНоменклатуреКлиент()
	...
	Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		Номенклатура = Элементы[ТекущаяСтраницаНоменклатуры].ТекущиеДанные.Номенклатура;
	...
	ОбновитьПанельОстатков(Номенклатура, ТекущиеДанные.ЕдиницаИзмерения, Характеристика);
...
Процедура ОбновитьДанныеПоНоменклатуреКлиент()
	...
	Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" И Не ПоказыватьКартинку И Не ОстаткиВидимость Тогда
		Возврат
	КонецЕсли;
	...
	Иначе
		АдресКартинки = "";
		Изображения.Очистить();
		ОбновитьДанныеПоНоменклатуреСервер(Номенклатура, Характеристика, ТекущиеДанные.ЕдиницаИзмерения);
	КонецЕсли;
...
&НаКлиенте
Процедура ПеретащитьВКорзинуНоменклатураПоставщика(ПараметрыПеретаскивания)
	
	Если ЗапрашиватьКоличество Или ЗапрашиватьЦену Тогда
		ДобавитьВКорзинуНоменклатураПоставщика();
		Возврат;
	КонецЕсли;
	
	ДобавитьВКорзину(ПараметрыПеретаскивания.Значение,, "СписокНоменклатураПоставщиков");
...
Процедура ДобавитьВКорзину(ВыбранныеСтроки, Склад = Неопределено, ИмяТаблицыДобавления = "")
	...
	Если ТипЗнч(ВыбранныеСтроки) = Тип("Массив") Тогда
		...
		Для Каждого ВыделеннаяСтрока Из ВыбранныеСтроки Цикл
			...
			Если ИмяТаблицыДобавления = "СписокНоменклатураПоставщиков" Тогда
				ДобавитьСтрокуВКорзинуНоменклатураПоставщика(ВыделеннаяСтрока, Склад, СтрокаДобавлена, ИмяТаблицыДобавления);
			Иначе
	...
	ИначеЕсли ТипЗнч(ВыбранныеСтроки) = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		Если ИмяТаблицыДобавления = "СписокНоменклатураПоставщиков" Тогда
			ДобавитьСтрокуВКорзинуНоменклатураПоставщика(ВыбранныеСтроки, Склад, СтрокаДобавлена, ИмяТаблицыДобавления);
		Иначе
...
&НаКлиенте
Процедура ДобавитьСтрокуВКорзинуНоменклатураПоставщика(ВыделеннаяСтрока, Склад, СтрокаДобавлена = Истина, ИмяТаблицыДобавления = "")

	СтруктураВыбора = СтруктураВыбора();
	
	Если ТипЗнч(ВыделеннаяСтрока) = Тип("ДанныеФормыЭлементКоллекции") И ВыделеннаяСтрока.Свойство("Номенклатура") Тогда
	...
	СтруктураВыбора.НоменклатураПоставщика = ДанныеСтроки.Ссылка;
	...
	Если ТолькоНоменклатураИКоличество Тогда
		...
		СтруктураВыбора.Ячейка = ?(Не ПоказыватьЯчейкуВПеремещенииЗапасов, Неопределено, СтруктураВыбора.Ячейка);
	КонецЕсли;
		
	СтруктураВыбора.Количество = ?((СтруктураВыбора.Весовой И ТипВладельцаФормы = "НовоеРМК"), 0, 1);
...
Процедура ДобавитьСтрокуВКорзину(ВыделеннаяСтрока, Склад, СтрокаДобавлена = Истина, ИмяТаблицыДобавления = "")
	...
	Если ТекущаяСтраницаНоменклатуры = "СписокЗапасы" Тогда
		...
		Если ДанныеСтроки.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПодарочныйСертификат") Тогда
			СтруктураВыбора.Цена = ДанныеСтроки.Номинал;
		КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьВКорзинуИзСпискаНоменклатураПоставщиков(Команда)
	ДобавитьВКорзинуНоменклатураПоставщика(Истина);
КонецПроцедуры

&НаКлиенте
Процедура УдалитьИзКорзиныНоменклатураПоставщиков(Команда)
	ДобавитьВКорзинуНоменклатураПоставщика(Ложь);
КонецПроцедуры
...
&НаКлиенте
Процедура УдалитьЭлементИзКорзиныНоменклатураПоставщика(ТекущиеДанные, ДобавлениеИзСписка, Склад = Неопределено, ИмяТаблицыДобавления = Неопределено)
	
	Если ТекущиеДанные.КартинкаКорзина = 0 Тогда
		СписокСтрокДобавленияВКорзину.Удалить(0);
		ДобавитьУдалитьСтрокиИзКорзиныНоменклатураПоставщика(,,ИмяТаблицыДобавления);
		Возврат
	КонецЕсли;
	...
	Если Не ДобавлениеИзСписка Тогда
		...
		ПредопределенноеЗначение("Справочник.Ячейки.ПустаяСсылка"), ТекущиеДанные.Ячейка));
	Иначе
		СтруктураПоиска = Новый Структура("Номенклатура, СтруктурнаяЕдиница", ТекущиеДанные.Номенклатура, Склад);
	КонецЕсли;
	
	ПараметрыОповещения = Новый Структура("СтруктураПоиска, ДобавлениеИзСписка, ИмяТаблицыДобавления", СтруктураПоиска, ДобавлениеИзСписка, ИмяТаблицыДобавления);
	
	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтотОбъект, ПараметрыОповещения);
	ПоказатьВопрос(Оповещение, СтрШаблон(НСтр("ru='Удалить %1 из корзины?'"), """" +Строка(ТекущиеДанные.Номенклатура)+ """"), Режим, 0);
...
&НаКлиенте 
Процедура ДобавитьВКорзинуНоменклатураПоставщика(ДобавитьВКорзину = Истина, ИмяТаблицыДобавления = "")
	
	СписокСтрокДобавленияВКорзину.Очистить();
	
	ИмяТаблицыДобавления = ТекущаяСтраницаНоменклатуры;
	...
	Если СписокСтрокДобавленияВКорзину.Количество() Тогда
		СписокВладельцев = Новый СписокЗначений;
		ДобавитьУдалитьСтрокиИзКорзиныНоменклатураПоставщика(ДобавитьВКорзину, СписокВладельцев, ИмяТаблицыДобавления);
...
Процедура ДобавитьУдалитьСтрокиИзКорзины(ДобавитьВКорзину = Неопределено, СписокВладельцев = Неопределено, ИмяТаблицыДобавления = Неопределено)
	...
	Если СписокСтрокДобавленияВКорзину.Количество() Тогда
		...
		Для Каждого СтрокаСпискаДобавленияСтрок Из СписокСтрокДобавленияВКорзину Цикл
		
		ТекущиеДанные = СтрокаСпискаДобавленияСтрок.Значение;
		УдалитьПозицию = СтрокаСпискаДобавленияСтрок.Пометка;
		...
		Если ЗапрашиватьКоличество Или ЗапрашиватьЦену Тогда
			...
			СтруктураПараметров = Новый Структура("Количество, Цена, ЕдиницаИзмерения, Номенклатура, Характеристика,ЭтоНабор,
												...
												|СтавкаНДС, ТипНоменклатуры, ПодбиратьВесьОстаток, КоличествоОстаток,
												|ВалютаПодбора, ЗапретитьВводДробногоКоличества");
			
			СтруктураПараметров.Количество = ?((ДанныеПоНоменклатуре.Весовой И ТипВладельцаФормы = "НовоеРМК"), 0, 1);
			...
			СтруктураПараметров.ЗапретитьВводДробногоКоличества = ДанныеПоНоменклатуре.ЗапретитьВводДробногоКоличества;
	...
	Если ПоказыватьКорзину Тогда
		...
		Если Не ТекущиеДанные = Неопределено И ТекущиеДанные.Свойство("КартинкаКорзина") И ТекущиеДанные.КартинкаКорзина = 1 Тогда
...
&НаКлиенте
Процедура ДобавитьУдалитьСтрокиИзКорзиныНоменклатураПоставщика(ДобавитьВКорзину = Неопределено, СписокВладельцев = Неопределено, ИмяТаблицыДобавления = Неопределено)
	
	ДобавлениеИзСписка = Истина;
	...
	Если СписокСтрокДобавленияВКорзину.Количество() Тогда
		...
		Если ДобавитьВКорзину = Неопределено 
			Или (Не ДобавитьВКорзину = Неопределено И Не ДобавитьВКорзину) Тогда
			Если УдалитьПозицию Тогда
				УдалитьЭлементИзКорзиныНоменклатураПоставщика(ТекущиеДанные, ДобавлениеИзСписка, Склад, ИмяТаблицыДобавления);
				Возврат;
			КонецЕсли;
		КонецЕсли;
		
		Если Не УдалитьПозицию Тогда 
			СписокВладельцев.Добавить(ТекущиеДанные.Номенклатура);
		КонецЕсли;
		...
		Если ЗапрашиватьКоличество Или ЗапрашиватьЦену Тогда
			...
			Если ПоказыватьОстатки И ЗначениеЗаполнено(ОтборСклад) Тогда
				СтруктурнаяЕдиница = ОтборСклад;
			ИначеЕсли ЭтоПодбор Тогда
				СтруктурнаяЕдиница = СтруктурнаяЕдиницаДокумент;
			Иначе
			...
			|ЗапрашиватьКоличество, ЗапрашиватьЦену, Партия, СтруктурнаяЕдиница, Ячейка,
			|СтавкаНДС, ТипНоменклатуры, ПодбиратьВесьОстаток, КоличествоОстаток,
			|ВалютаПодбора, ЗапретитьВводДробногоКоличества, НоменклатураПоставщика");
			...
			СтруктураПараметров.СтруктурнаяЕдиница = СтруктурнаяЕдиница;
			СтруктураПараметров.Ячейка = Ячейка;
			СтруктураПараметров.НоменклатураПоставщика = ТекущиеДанные.Ссылка;
			...
			Если НЕ ПоказыватьЦены Тогда
				...
				СтруктураВыбора.Вставить("Номенклатура",	ДанныеПоНоменклатуре.Номенклатура);
				СтруктураВыбора.Вставить("ВидЦен",			ОтборВидЦен);
				СтруктураВыбора.Вставить("ВалютаДокумента", ВалютаПодбора);
				СтруктураВыбора.Вставить("Коэффициент",		1);
				СтруктураВыбора.Вставить("Характеристика", Характеристика);
			...
			СтруктураПараметров.ЕдиницаИзмерения = ДанныеПоНоменклатуре.ЕдиницаИзмерения;
			СтруктураПараметров.Номенклатура = ДанныеПоНоменклатуре.Номенклатура;
...
Процедура ДобавитьВКорзинуЦенаКоличество(СтруктураВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(СтруктураВыбора.СтруктурнаяЕдиница) Тогда
		СтруктураВыбора.СтруктурнаяЕдиница = СкладПоУмолчанию(СтруктураВыбора.Номенклатура);
	КонецЕсли;
	...
	Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		СтруктураПоискаВКорзине.Вставить("НоменклатураПоставщика", СтруктураВыбора.НоменклатураПоставщика);
	КонецЕсли;
	...
	СтрокаКорзины.НоменклатураПоставщика = СтруктураВыбора.НоменклатураПоставщика;
...
Процедура ОтборКатегорииИерархияПереключательПриИзменении(Элемент)
	...
	Если ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.КатегорииНоменклатуры") Тогда
		...
		Элементы.СтраницыКатегорииИерархия.ТекущаяСтраница = Элементы.КатегорииОтбор;
		Элементы.СтраницыКомандыКатегорийГруппСегментов.ТекущаяСтраница = Элементы.СтраницаКомандыКатегорий;
		ОтключитьОтборПоИерархии();
		ОтключитьОтборПоГруппамПоставщиков();
		ПодключитьОбработчикОжидания("ОтборКатегорииПриАктивизацииСтрокиНаКлиенте", 0.2, Истина);
	...
	ИначеЕсли ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатуры") Тогда
		...
		Элементы.СтраницыКомандыКатегорийГруппСегментов.ТекущаяСтраница = Элементы.СтраницаКомандыГрупп;
		ОтключитьОтборПоГруппамПоставщиков();
	...
	ИначеЕсли ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.СегментыНоменклатуры") Тогда
		...
		Элементы.СтраницыКомандыКатегорийГруппСегментов.ТекущаяСтраница = Элементы.СтраницаКомандыСегментов;
		...
		Если Элементы.ОтборИерархия.ТекущиеДанные <> Неопределено Тогда
		    Если Элементы.ОтборСегменты.ТекущаяСтрока = Неопределено Тогда //отбор на новом созданном элементе
				...
				Элементы.ОтборСегменты.ТекущаяСтрока = ИдентификаторСтроки;
			Иначе //восстановить ранее сделанный, отключенный отбор по иерархии
				УстановитьОтборПоСегментуНоменклатуры(ЭтотОбъект, ТекущаяСтраницаНоменклатуры);
			КонецЕсли;
	...
	ИначеЕсли ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатурыПоставщиков") Тогда
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокЗапасыКонтекстноеМенюПерейтиКРодителю", "Заголовок", НСтр("ru='Перейти к группе поставщика'"));
		
		Элементы.СтраницыКатегорииИерархия.ТекущаяСтраница = Элементы.ИерархияНоменклатураПоставщиковОтбор;
		Элементы.СтраницыКомандыКатегорийГруппСегментов.ТекущаяСтраница = Элементы.СтраницаКомандыНоменклатурыПоставщиков;
		
		Если Элементы.ОтборИерархия.ТекущиеДанные <> Неопределено Тогда
			Если Элементы.ОтборНоменклатураПоставщиков.ТекущаяСтрока = Неопределено Тогда //отбор на новом созданном элементе
				ИдентификаторСтроки = 0;
				ОбщегоНазначенияКлиентСервер.ПолучитьИдентификаторСтрокиДереваПоЗначениюПоля("Значение", ИдентификаторСтроки, ОтборИерархия.ПолучитьЭлементы(), Элементы.ОтборИерархия.ТекущиеДанные.Значение, Ложь);
				Элементы.ОтборНоменклатураПоставщиков.ТекущаяСтрока = ИдентификаторСтроки;
			Иначе //восстановить ранее сделанный, отключенный отбор по иерархии
		...
		ОтключитьОтборПоИерархииВнутреннейНоменклатурыСписокНоменклатураПоставщиков();
		Элементы.ОтборИерархия.ТекущаяСтрока = 0;
		
		ОтключитьОтборПоКатегорииСписокНоменклатураПоставщиков();
		Элементы.ОтборКатегории.ТекущаяСтрока = 0;
		
		Элементы.ОтборНоменклатураПоставщиков.ТекущаяСтрока = 0;
		ПодключитьОбработчикОжидания("ОтборКатегорииПриАктивизацииСтрокиНаКлиенте", 0.1, Истина);
		
	КонецЕсли;
	
	Если РежимВыбораГруппы И ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатуры") Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВыбратьГруппу", "Видимость", Истина);
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВыбратьГруппу", "Видимость", Ложь);
	КонецЕсли;
...
Процедура ОтборКатегорииПриАктивизацииСтрокиНаКлиенте()
	...
	Если ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатуры")
		Или ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатурыПоставщиков") Тогда
...
&НаСервере
Процедура ОтборКатегорииНоменклатураПоставщиковПриАктивизацииСтрокиНаСервере()
	КатегорииНоменклатурыСервер.УстановитьОтборПоКатегории(ЭтотОбъект, ЭтаФорма["СписокНоменклатураПоставщиков"], ОтборКатегорииТекущая);
...
Процедура ОтборКатегорииПриАктивизацииСтрокиНаСервере()
	...
	Если ДанныеМетокПП.Количество() Или ДанныеМетокККМ.Количество() Тогда
		...
		УдалитьМеткиОтбораСервер("ДанныеМетокПП" ,СписокГрупп);
		
		СписокГрупп.Очистить();
		СписокГрупп.Добавить("КатегорииОтборСвойстваХарактеристики");
		РаботаСОтборами.ОбновитьЭлементыМеток(ЭтотОбъект, СписокГрупп, "ДанныеМетокККМ");
		
		УдалитьМеткиОтбораСервер("ДанныеМетокККМ" ,СписокГрупп);
...
&НаСервере
Процедура УстановитьОтборИерархияНаСервере()
	
	ПравоеЗначение	= Неопределено;
	Сравнение		= ВидСравненияКомпоновкиДанных.Равно;
	Использование	= Истина;
	Представление	= Строка(ОтборИерархияТекущая);
	
	Если ИерархияВключаяВложенные Тогда
		Сравнение = ВидСравненияКомпоновкиДанных.ВИерархии;
	КонецЕсли;
	ПравоеЗначение = ОтборИерархияТекущая;
...
&НаКлиенте
Процедура СоздатьКатегорию()
	
	ЗначенияЗаполнения = Новый Структура;
	Если Элементы.ОтборКатегории.ТекущиеДанные <> Неопределено Тогда
		ЗначенияЗаполнения.Вставить("Родитель", РодительКатегории(Элементы.ОтборКатегории.ТекущиеДанные.Значение));
	КонецЕсли;
...
&НаКлиенте
Процедура ВыбратьСтрокуСпискаНоменклатураПоставщиков()
	
	Если НастройкиИзДокумента И Не ЭтоПодбор Тогда
		
		Если РежимВыбора И ЗначениеЗаполнено(ТипВладельца) Тогда
			
			ВыбраннаяСтрока = Элементы.СписокНоменклатураПоставщиков.ТекущаяСтрока;
			ТекущиеДанные = Элементы.СписокНоменклатураПоставщиков.ТекущиеДанные;
			...
			СтруктураПараметров.Номенклатура = ТекущиеДанные.Номенклатура;
			СтруктураПараметров.НоменклатураПоставщика = ВыбраннаяСтрока;
			СтруктураПараметров.СтруктураНастроек = СтруктураСохраняемыхНастроек;
			СтруктураПараметров.Склад = ПустойСклад;
			СтруктураПараметров.Характеристика = ТекущиеДанные.Характеристика;
			СтруктураПараметров.Партия = ПустаяПартия;
...
&НаСервере
Функция НоменклатураПоставщика(Номенклатура, Характеристика)
...
&НаКлиенте
Процедура УстановитьОтборПоИерархииПриИзмененииВидаПодбора()
		...
		ЭлементыОтбора = ЭтотОбъект[ТекущаяСтраницаНоменклатуры].КомпоновщикНастроек.ФиксированныеНастройки.Отбор.Элементы;
...
Функция ИсключаемыйТипНДСКомиссия(ВидОперации, ТипВладельца)
	
	Если ТипВладельца = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
		Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") Тогда
			Возврат Ложь
		Иначе
			Возврат Истина;
		КонецЕсли
	...
	Если ТипВладельца = Тип("ДокументСсылка.РасходнаяНакладная") Тогда
		
		МассивИсключаемыхТиповОпераций = Новый Массив;
		МассивИсключаемыхТиповОпераций.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику"));
		МассивИсключаемыхТиповОпераций.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ВозвратИзПереработки"));
		МассивИсключаемыхТиповОпераций.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ВозвратКомитенту"));
		МассивИсключаемыхТиповОпераций.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ВозвратСОтветХранения"));
		
		
		Если МассивИсключаемыхТиповОпераций.Найти(ВидОперации) = Неопределено Тогда
			Возврат Ложь
		Иначе
			Возврат Истина;
		КонецЕсли;
	...
	Если ТипВладельца = Тип("ДокументСсылка.ЗаказПокупателя")
		Или ТипВладельца = Тип("ДокументСсылка.ОтчетКомитенту")
		Или ТипВладельца = Тип("ДокументСсылка.ЧекККМ")
		Или ТипВладельца = Тип("ДокументСсылка.ОтчетОРозничныхПродажах")
		Или ТипВладельца = "НовоеРМК" Тогда
		Возврат Ложь
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция СкладПоУмолчанию(Номенклатура)
...
Функция ЭтоРаботаСЗакупками(ТипДокумента)
	...
	Если ЭтоПодбор Тогда
		...
		ЭтоВозвратПоставщику = ТипДокумента = Тип("ДокументСсылка.РасходнаяНакладная")
	...
	Иначе
		...
		ЭтоВозвратПоставщику = ТипДокумента = "РасходнаяНакладная"
			И ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику;
...
Процедура УстановитьОтборПоДеревуИерархии(ИмяДерева, ЗначениеСтроки)
	...
	ИначеЕсли ИмяДерева = "ОтборНоменклатураПоставщиков" Тогда
		СтрокаДерева = ИдентификаторСтрокиДереваПоЗначению(ОтборНоменклатураПоставщиков, ЗначениеСтроки);
		Элементы.ОтборНоменклатураПоставщиков.ТекущаяСтрока = СтрокаДерева; //В списке напротив Группы установить галочку - Использовать всегда
	КонецЕсли;
...
&НаКлиенте
Функция МассивНоменклатурыНоменклатурыПоставщикаИзКорзиныКлиент()
	
	МассивНоменклатуры = Новый Массив;
	
	Для Каждого ЭлементКорзины Из Корзина Цикл
		МассивНоменклатуры.Добавить(ЭлементКорзины.НоменклатураПоставщика);
	КонецЦикла;
...
&НаСервере
Функция МассивНоменклатурыПоставщикаИзКорзиныСервер()
...
&НаСервере
Процедура СохранитьОбщиеНастройки(ЗакрытиеФормы = Ложь)
...
Процедура УстановитьНастройкиФормыПриСоздании(Параметры)
	...
	Если НЕ ЗначениеЗаполнено(ТекущаяСтраницаНоменклатуры) Тогда
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		Элементы.СписокНоменклатураПоставщиковКартинкаКорзина.Видимость = ДоступныПродажи;
		Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюДобавитьВКорзину.Видимость = ДоступныПродажи И ПоказыватьКорзину;
		Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюУдалитьИзКорзины.Видимость = ДоступныПродажи И ПоказыватьКорзину;
...
Процедура УстановитьВидимостьИДоступностьПриСоздании()
	...
	Если РежимВыбораГруппы И ОтборКатегорииИерархияПереключатель = Перечисления.ВидыОтборовНоменклатуры.ГруппыНоменклатуры Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВыбратьГруппу", "Видимость", Истина);
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВыбратьГруппу", "Видимость", Ложь);
	КонецЕсли;
	...
	Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюДобавитьВКорзину.Видимость = НЕ Параметры.РежимВыбора И ПоказыватьКорзину;
	Элементы.СписокНоменклатураПоставщиковКонтекстноеМенюУдалитьИзКорзины.Видимость = НЕ Параметры.РежимВыбора И ПоказыватьКорзину;
	...
	Иначе
		...
		ИначеЕсли ОтборКатегорииИерархияПереключатель = Перечисления.ВидыОтборовНоменклатуры.СегментыНоменклатуры Тогда
			Элементы.СтраницыКатегорииИерархия.ТекущаяСтраница = Элементы.ИерархияСегментыОтбор;
			Элементы.СтраницыКомандыКатегорийГруппСегментов.ТекущаяСтраница = Элементы.СтраницаКомандыСегментов;
		ИначеЕсли ОтборКатегорииИерархияПереключатель = Перечисления.ВидыОтборовНоменклатуры.ГруппыНоменклатурыПоставщиков
			И РежимВыбора И ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
			Элементы.СтраницыКатегорииИерархия.ТекущаяСтраница = Элементы.ИерархияНоменклатураПоставщиковОтбор;
			Элементы.СтраницыКомандыКатегорийГруппСегментов.ТекущаяСтраница = Элементы.СтраницаКомандыНоменклатурыПоставщиков;
		КонецЕсли;
	КонецЕсли;
	...
	Элементы.СписокНоменклатураПоставщиковПоказатьОстатки.Пометка = ОстаткиВидимость;
	...
	Если Не ПечатьЭтикетокИЦенников Тогда
		...
		Если ЗначениеЗаполнено(ОтборВидЦен) Тогда
			ВалютаЦены = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтборВидЦен, "ВалютаЦены", Истина);
			ВалютаПредставление = УправлениеНебольшойФирмойПовтИсп.ПолучитьСимвольноеПредставлениеВалюты(ВалютаЦены);
			СимвольноеПредставление = ?(ЗначениеЗаполнено(ВалютаПредставление), " " + ВалютаПредставление, "");
			Элементы.КорзинаРасшифровкаГруппаЦенаВВалюте.Заголовок = Строка(ОтборВидЦен) + СимвольноеПредставление;
		КонецЕсли;
		...
		Если ЭтоПодбор И Не ВалютаПодбора = ВалютаДокумента Тогда
			...
			Элементы.КорзинаРасшифровкаЦенаВВалютеПодбора.Доступность = РазрешеноРедактированиеЦенДокументов;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборТипСертификат", "Видимость", ПолучитьФункциональнуюОпцию("ИспользоватьПодарочныеСертификаты"));
...
Процедура УстановитьПараметрыЗапросовСписков()
	...
	СписокНоменклатураПоставщиков.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("ТекущаяСтраницаНоменклатуры", ТекущаяСтраницаНоменклатуры);
	СписокЗапасы.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("ТекущаяСтраницаНоменклатуры", ТекущаяСтраницаНоменклатуры);
	
	Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		
		УстановитьПараметрСписка(СписокНоменклатураПоставщиков, "ОтборОстатки", Ложь);
		УстановитьПараметрСписка(СписокНоменклатураПоставщиков, "Поставщик", Поставщик);
		
		СписокНоменклатураПоставщиков.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("ПоказыватьЦены", ПоказыватьЦены);
		СписокНоменклатураПоставщиков.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Корзина", МассивНоменклатурыПоставщикаИзКорзиныСервер());
		СписокНоменклатураПоставщиков.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("ВидЦен", ОтборВидЦен);
		СписокНоменклатураПоставщиков.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Поставщик", Поставщик);
		
		СписокНоменклатураПоставщиков.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("УчетВалютныхОпераций", 
		ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций"));
		...
		Если ЭтоПодбор Или РежимВыбора Тогда
			Если ЗначениеЗаполнено(ПериодЦен) И Не НачалоДня(ПериодЦен) = НачалоДня(ТекущаяДатаСеанса()) Тогда
				УстановитьПараметрСписка(СписокНоменклатураПоставщиков, "ДатаСреза", ПериодЦен);
				СписокНоменклатураПоставщиков.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("ДатаСреза", ПериодЦен);
			КонецЕсли;
		КонецЕсли;
	...
	СписокЗапасы.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("ПоказыватьМинимальныеЦены", 
		ПоказыватьМинимальныеЦены);
	СписокЗапасы.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("ВидМинимальныхЦен", ВидМинимальныхЦен);
	СписокЗапасы.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("ПоказыватьЦены", ПоказыватьЦены);
	СписокЗапасы.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("УчетВалютныхОпераций", 
		ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций"));
	...
	Если ОтборОстатки = 1 И ЗначениеЗаполнено(ОтборОрганизация) Тогда
		...
		СписокОрганизаций.Добавить(ОрганизацииПередачи(ОтборОрганизация));
		УстановитьПараметрСписка(СписокЗапасы, "ВсеОрганизации", Ложь);
		УстановитьПараметрСписка(СписокЗапасы, "Организация", СписокОрганизаций);
	Иначе
		СписокОрганизаций = Новый СписокЗначений;
...
Процедура УстановитьВидимостьИДоступность(ЭтоСозданиеФормы = Ложь)
	...
	ПодборПоНоменклатуреПоставщиков = ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков";
	
	Если ПодборПоНоменклатуреПоставщиков Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокНоменклатураПоставщиковОстаток", "Видимость", ПоказыватьОстатки);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокНоменклатураПоставщиковЦенаВалюта", "Видимость", ПоказыватьЦены);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокНоменклатураПоставщиковПредставлениеВалютаЦены", "Видимость", ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций"));
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КорзинаРасшифровкаНоменклатураПоставщика", "Видимость", ПодборПоНоменклатуреПоставщиков);
 	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КомандыСписка", "Видимость", Не ПодборПоНоменклатуреПоставщиков);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Отборы", "Высота", ?(Не ПодборПоНоменклатуреПоставщиков, 7, 5));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "АдресКартинки", "ТекстНевыбраннойКартинки", 
																?(ПодборПоНоменклатуреПоставщиков, НСтр("ru = 'Изображение не задано'"), НСтр("ru = 'Добавить изображение'")));
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборСклад", "Видимость", ПоказыватьОстатки И ОтборОстатки = 1 И НЕ ПодборПоНоменклатуреПоставщиков);
	
	ИспользоватьНесколькоОрганизаций = ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций");
	
	ПоказыватьОтборПоОрганизации = Элементы.ОтборСклад.Видимость И Не УчетПоКомпанииВЦелом И ИспользоватьНесколькоОрганизаций И НЕ ПодборПоНоменклатуреПоставщиков;
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборОрганизация", "Видимость", ПоказыватьОтборПоОрганизации);
	...
	ФункциональнаяОпцияУчетПоЯчейкам = Константы.ФункциональнаяОпцияУчетПоЯчейкам.Получить();
	
	Если Не ФункциональнаяОпцияУчетПоЯчейкам
	...
	ПоказыватьЯчейкуВПеремещенииЗапасов = ФункциональнаяОпцияУчетПоЯчейкам И ИмяФормыВладельца = "Документ.ПеремещениеЗапасов.Форма.ФормаДокумента";
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КорзинаРасшифровкаЯчейка", "Видимость", ВидимостьЯчейки Или ПоказыватьЯчейкуВПеремещенииЗапасов);
	...
	//Цены
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЦенаВалюта", "Видимость", ПоказыватьЦены И ЦенаВСписке = 0);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЦеныДиапазон", "Видимость", ПоказыватьЦены И ЦенаВСписке = 0 И НЕ ПодборПоНоменклатуреПоставщиков);
	...
	, НЕ ТекущаяСтраницаНоменклатуры = "ДеревоОстатки" И ПоказыватьЦены И ЦенаВСписке = 1 И НЕ ПодборПоНоменклатуреПоставщиков);
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаАналогиНоменклатуры", "Видимость", ПоказыватьАналоги И НЕ ПодборПоНоменклатуреПоставщиков);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСопутствующиеТовары", "Видимость", ПоказыватьСопутствующиеТовары И НЕ ПодборПоНоменклатуреПоставщиков);
	КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокНоменклатураПоставщиковКартинкаКорзина", "Видимость", ПоказыватьКолонкаКорзина);
	
	РазрешитьДобавлениеВКорзинуИзРасшифровки = ПоказыватьКолонкаКорзина И Не ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков";
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "РасшифровкаОстатковКартинкаКорзина", "Видимость", РазрешитьДобавлениеВКорзинуИзРасшифровки);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "РасшифровкаОстатков", "РазрешитьНачалоПеретаскивания", РазрешитьДобавлениеВКорзинуИзРасшифровки);
	...
	Если ТекущаяСтраницаНоменклатуры = "ДеревоОстатки" Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СтраницыКомандыКатегорийГруппСегментов", "Видимость", Ложь);
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТипНоменклатурыОтбор", "Видимость", НЕ ПодборПоНоменклатуреПоставщиков);
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СтраницыКомандыКатегорийГруппСегментов", "Видимость", ВидимостьКатегорииГруппы);
		...
		ИначеЕсли ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатуры") Тогда
			...
			Элементы.СтраницыКатегорииИерархия.ТекущаяСтраница = Элементы.ИерархияГруппыОтбор;
			Элементы.СтраницыКомандыКатегорийГруппСегментов.ТекущаяСтраница = Элементы.СтраницаКомандыГрупп;
		ИначеЕсли ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатурыПоставщиков")
			И РежимВыбора И ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
			Элементы.СтраницыКатегорииИерархия.ТекущаяСтраница = Элементы.ИерархияНоменклатураПоставщиковОтбор;
			Элементы.СтраницыКомандыКатегорийГруппСегментов.ТекущаяСтраница = Элементы.СтраницаКомандыНоменклатурыПоставщиков;
		Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаОтбор", "Видимость", НЕ ПодборПоНоменклатуреПоставщиков);
	...
	Если ЭтоПодбор Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КорзинаРасшифровкаЯчейка", "Видимость"
			, (ПоказыватьИнформациюПоСкладам И ВидимостьЯчейки) Или ПоказыватьЯчейкуВПеремещенииЗапасов);
	...
	Если РежимВыбораГруппы Тогда
		ОтображатьКолонкуРасшифровкиПоОстаткам = Ложь;
	Иначе
	...
	Если Не ПравоДоступа("Добавление", Метаданные.Справочники.Номенклатура) Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СоздатьНоменклатуруСервис1СНоменклатура", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СкопироватьНоменклатуру", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗагрузкаДанныхИзExcel", "Видимость", Ложь);
	КонецЕсли;
	
	ОбновитьЗаголовокФильтров();
	
	Если ОбщегоНазначенияУНФ.ЭтоРозница() Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборТипРабота", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборТипВидРабот", "Видимость", Ложь);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	// Цены и номинал
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "СписокЗапасы.ТипНоменклатуры", Перечисления.ТипыНоменклатуры.ПодарочныйСертификат, ВидСравненияКомпоновкиДанных.Равно);
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "Цена");
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", Новый ПолеКомпоновкиДанных("СписокЗапасы.Номинал"));
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьОтборПоСегментуНоменклатуры(Форма, ИмяСписка = "СписокЗапасы")
...
Процедура ПереопределитьТекстЗапросаВСписках()
	...
	Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		ПереопределитьТекстЗапросаСписокНоменклатураПоставщиков();
	Иначе
		ПереопределитьТекстЗапросаСписокЗапасы();
	КонецЕсли;
...
&НаСервере
Процедура ПереопределитьТекстЗапросаСписокНоменклатураПоставщиков()
	...
	Если Не ПоказыватьОстатки Тогда
		...
		|	СправочникНоменклатураПоставщиков.Владелец КАК Поставщик,
		...
		|	СправочникНоменклатураПоставщиков.СрокПополнения КАК СрокПополнения,
		|	ВнутренняяНоменклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	0 КАК Цена,
		...
		|	НЕОПРЕДЕЛЕНО КАК ВалютаЦены,
		|	НЕОПРЕДЕЛЕНО КАК ПредставлениеВалютаЦены,
		|	ВЫБОР
		|		КОГДА ВнутренняяНоменклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)
		|			ТОГДА 0
		|		КОГДА ВнутренняяНоменклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПодарочныйСертификат)
		|			ТОГДА 4
		|		ИНАЧЕ 2
		|	КОНЕЦ + ВЫБОР
		|		КОГДА ВнутренняяНоменклатура.ИспользоватьХарактеристики
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ + ВЫБОР
		...
		|		ИНАЧЕ 0
		|	КОНЕЦ + ВЫБОР
		|		КОГДА ВнутренняяНоменклатура.ЭтоНабор
		|			ТОГДА 12
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ВариантКартинки,
		|	ВнутренняяНоменклатура.Родитель КАК Родитель,
		|	ВнутренняяНоменклатура.КатегорияНоменклатуры КАК КатегорияНоменклатуры,
		|	СправочникНоменклатураПоставщиков.Родитель КАК НоменклатураПоставщикаРодитель,
		|	ВнутренняяНоменклатура.Склад КАК Склад,
		|	ВнутренняяНоменклатура.Ячейка КАК Ячейка,
		|	ВнутренняяНоменклатура.ЭтоНабор КАК ЭтоНабор,
		|	ВнутренняяНоменклатура.ТипНоменклатуры КАК ТипНоменклатуры,
		|	ВнутренняяНоменклатура.ЗапретитьВводДробногоКоличества КАК ЗапретитьВводДробногоКоличества,
		|	ВнутренняяНоменклатура.Весовой КАК Весовой,
		|	СправочникНоменклатураПоставщиков.Идентификатор КАК Идентификатор,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ВнутренняяНоменклатура
		|		ПО СправочникНоменклатураПоставщиков.Номенклатура = ВнутренняяНоменклатура.Ссылка
		|ГДЕ
		|	НЕ СправочникНоменклатураПоставщиков.ЭтоГруппа
		|	И СправочникНоменклатураПоставщиков.Владелец = &Поставщик";
		...
		Если ИспользоватьСегментыНоменклатуры Тогда
			...
			|
			|{ГДЕ
			|	(СправочникНоменклатураПоставщиков.Номенклатура В
			|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		ОбщегоНазначения.УстановитьСвойстваДинамическогоСписка(Элементы.СписокНоменклатураПоставщиков, ПараметрыСписка);
	...
	|	СправочникНоменклатураПоставщиков.Наименование КАК Наименование,
	|	СправочникНоменклатураПоставщиков.Артикул КАК Артикул,
	|	ЕСТЬNULL(ОстаткиТоваровПоставщиков.Количество, 0) КАК КоличествоОстаток,
	|	СправочникНоменклатураПоставщиков.СрокПополнения КАК СрокПополнения,
	|	ВнутренняяНоменклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	0 КАК Цена,
	|	СправочникНоменклатураПоставщиков.Номенклатура КАК Номенклатура,
	...
	|ИЗ
	|	Справочник.НоменклатураПоставщиков КАК СправочникНоменклатураПоставщиков
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОстаткиТоваровПоставщиков КАК ОстаткиТоваровПоставщиков
	|		ПО СправочникНоменклатураПоставщиков.Ссылка = ОстаткиТоваровПоставщиков.НоменклатураПоставщика
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ВнутренняяНоменклатура
	|		ПО СправочникНоменклатураПоставщиков.Номенклатура = ВнутренняяНоменклатура.Ссылка
	|ГДЕ
	|	НЕ СправочникНоменклатураПоставщиков.ЭтоГруппа
	|	И СправочникНоменклатураПоставщиков.Владелец = &Поставщик
	|	И &ДопПоляШаблонУсловияПоОстаткам";
	
	Если ОтборОстатки = 0 Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И &ДопПоляШаблонУсловияПоОстаткам", "");
	ИначеЕсли ОтборОстатки = 1 Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДопПоляШаблонУсловияПоОстаткам", "ЕСТЬNULL(ОстаткиТоваровПоставщиков.Количество, 0) > 0");
...
Процедура ПереопределитьТекстЗапросаСписокЗапасы()
	...
	Если ОтборПоЦенам И ЦенаВСписке = 0 Тогда //Медленный способ с двумя левыми соединениями
		...
		Если Не ОтборПоВидуЦенКонтрагентов И ОтборВидЦен.НесколькоВалютДляЦен Тогда
...
Процедура ОстаткиОтменаОтбора()
	...
	Элементы.НазадКСпискуНоменклатуры.Видимость = Ложь;
...
Процедура УстановитьОтборАналоги(ТекущиеДанные, ОтборИзКорзины = Ложь)
	...
	Если Элементы.НазадКСпискуНоменклатуры.Видимость Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НазадКСпискуНоменклатуры", 
			"Видимость", Ложь);
		ЭтотОбъект.АвтоЗаголовок = Истина;
		ЭтотОбъект.Заголовок = "";
	КонецЕсли;
...
&НаКлиенте
Процедура НоменклатураПоставщикаПредставлениеОтбораОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ОтменитьОтборПоНоменклатуреПоставщиков();
	Элементы.НоменклатураПоставщикаПредставлениеОтбора.Видимость = Ложь;
	
КонецПроцедуры

&НаСервере
Процедура ОтменитьОтборПоНоменклатуреПоставщиков()
	
	ЕстьХарактеристика = ЗначениеЗаполнено(НоменклатураДляНоменклатурыПоставщика.Характеристика);
	
	УстановитьЗначениеОтбораДинамическогоСпискаНоменклатураПоставщиков("Номенклатура", НоменклатураДляНоменклатурыПоставщика.Номенклатура, Ложь);
	Если ЕстьХарактеристика Тогда
		УстановитьЗначениеОтбораДинамическогоСпискаНоменклатураПоставщиков("Характеристика", НоменклатураДляНоменклатурыПоставщика.Характеристика, Ложь);
	КонецЕсли;
...
&НаСервере
Процедура УстановитьОтборДляНоменклатурыПоставщиковПоНоменклатуре()
	
	ПредставлениеНоменклатуры = Строка(НоменклатураДляНоменклатурыПоставщика.Номенклатура);
	
	ЕстьХарактеристика = ЗначениеЗаполнено(НоменклатураДляНоменклатурыПоставщика.Характеристика);
	
	ПредставлениеХарактеристики = ?(ЕстьХарактеристика, Строка(НоменклатураДляНоменклатурыПоставщика.Характеристика), "");
	ТекстПредставления = СтрШаблон(НСтр("ru='Номенклатура: %1 %2'"), ПредставлениеНоменклатуры, ПредставлениеХарактеристики);
	
	УстановитьЗначениеОтбораДинамическогоСпискаНоменклатураПоставщиков("Номенклатура", НоменклатураДляНоменклатурыПоставщика.Номенклатура);
	Если ЕстьХарактеристика Тогда
		УстановитьЗначениеОтбораДинамическогоСпискаНоменклатураПоставщиков("Характеристика", НоменклатураДляНоменклатурыПоставщика.Характеристика);
	КонецЕсли;
	
	Элементы.НоменклатураПоставщикаПредставлениеОтбора.Заголовок = ИнформацияОбОтборе(ТекстПредставления);
	Элементы.НоменклатураПоставщикаПредставлениеОтбора.Видимость = Истина;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьЗначениеОтбораДинамическогоСпискаНоменклатураПоставщиков(ИмяПоляОтбора, ЗначениеОтбора = Неопределено, Использование = Истина)
	
	ВидСравненияОтбора = ВидСравненияКомпоновкиДанных.ВСписке;
	НайденныйЭлемент = ПолеОтбора(СписокНоменклатураПоставщиков.Отбор,ИмяПоляОтбора);
	
	Если Использование И НайденныйЭлемент = Неопределено Тогда
		
		ПолеОтбора = Новый ПолеКомпоновкиДанных(ИмяПоляОтбора);
		
		НовыйОтбор = СписокНоменклатураПоставщиков.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		НовыйОтбор.Использование = Истина;
		НовыйОтбор.ЛевоеЗначение = ПолеОтбора;
		НовыйОтбор.ПравоеЗначение = ЗначениеОтбора;;
		НовыйОтбор.ВидСравнения = ВидСравненияОтбора;
...
Процедура ОбновитьЗаголовокФильтров()
	...
	Если Не ПоказыватьОстатки Тогда
		ТекстОтборОстатки = "";
	ИначеЕсли ОтборОстатки = 1 И (Не ЗначениеЗаполнено(ОтборСклад) И Не ЗначениеЗаполнено(ОтборОрганизация)) 
		Или ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
...
&НаКлиенте
Процедура НазадКСпискуНоменклатуры(Команда)
...
Процедура ОбновитьДанныеПараметровДинамическихСписковКлиент()
	...
	Если ТекущаяСтраницаНоменклатуры = "СписокЗапасы" Или ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
			
			СписокЗапасы.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Корзина", МассивНоменклатурыИзКорзиныКлиент());
			СписокЗапасы.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("ПрямыеАналоги", ПрямыеАналоги);
			
			Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
				СписокНоменклатураПоставщиков.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Корзина", МассивНоменклатурыНоменклатурыПоставщикаИзКорзиныКлиент());
			КонецЕсли;
...
Функция ТаблицаДополнительныхРеквизитов(ОписаниеОбъекта)
	...
	ДополнительныеСвойстваОбъекта = ОписаниеОбъекта.ДополнительныеРеквизиты.Выгрузить();
	ОписаниеСвойств = УправлениеСвойствамиСлужебный.ЗначенияСвойств(ДополнительныеСвойстваОбъекта,
		Свойства_НаборыДополнительныхРеквизитовОбъекта, ВидСвойств);
...
Процедура ОбновитьДанныеПоНоменклатуреСервер(Номенклатура, Характеристика = Неопределено, ЕдиницаИзмерения = Неопределено)
	...
	Если ПоказыватьКартинку И Не ТекущаяСтраницаНоменклатуры = "ДеревоОстатки" Тогда
		ОбновитьИзображение(Номенклатура, Характеристика);
	КонецЕсли;
	
	// Сопутствующие товары
	Если ПоказыватьСопутствующиеТовары И Не ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
	...
	Если НЕ ТекущаяСтраницаНоменклатуры = "ДеревоОстатки"  Тогда
		...
		// Аналоги
		Если ПоказыватьАналоги И Не ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		...
		// Панель остатков
		ОбновитьПанельОстатков(Номенклатура, ЕдиницаИзмерения, Характеристика);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьНадписьЦеныВПравойПанели(Номенклатура)
	...
	Если ПоказыватьЦены И ЦенаВСписке = 1 И ЗначениеЗаполнено(ОтборВидЦен) Тогда
		...
		|					КОГДА ЦеныНоменклатурыСрезПоследних.Цена < ЕСТЬNULL(МинимальныеЦеныНоменклатурыСрезПоследних.Цена, 0)
		|						ТОГДА ВЫБОР
		|								КОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
		|									ТОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены
		|								ИНАЧЕ МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены
		|							КОНЕЦ
		|					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.ВалютаЦены
		|				КОНЕЦ
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ЦеныНоменклатурыСрезПоследних.ВалютаЦены = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
		|					ТОГДА ЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены
		|				ИНАЧЕ ЦеныНоменклатурыСрезПоследних.ВалютаЦены
		|			КОНЕЦ
		|	КОНЕЦ КАК ВалютаЦены
		|ИЗ
		...
		Если Выборка.Следующий() Тогда
			...
			ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%3%", Выборка.ВалютаЦены.СимвольноеПредставление);
		Иначе
...
&НаСервере
Процедура ОбновитьПанельОстатков(Номенклатура, ЕдиницаИзмерения, Характеристика = Неопределено)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СУММА(ВЫРАЗИТЬ(ОстаткиТоваров.Количество / &Коэффициент КАК ЧИСЛО(15, 3))) КАК СвободныйОстаток,
	|	СУММА(ВЫРАЗИТЬ(ОстаткиТоваров.Резерв / &Коэффициент КАК ЧИСЛО(15, 3))) КАК ВРезерве,
	|	СУММА((ВЫРАЗИТЬ(ОстаткиТоваров.Количество / &Коэффициент КАК ЧИСЛО(15, 3))) + (ВЫРАЗИТЬ(ОстаткиТоваров.Резерв / &Коэффициент КАК ЧИСЛО(15, 3)))) КАК ОстатокОбщий,
	...
	|	И ВЫБОР
	|			КОГДА НЕ &Характеристика = НЕОПРЕДЕЛЕНО
	|				ТОГДА ОстаткиТоваров.Характеристика = &Характеристика
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|
	|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("ВсеОрганизации", НЕ ЗначениеЗаполнено(ОтборОрганизация) Или ОтборОстатки = 0);
...
Функция ОрганизацииПередачи(Организация)
	...
	Если НЕ ЗначениеЗаполнено(Организация) ИЛИ НЕ ПередачаТоваровМеждуОрганизациями Тогда
		Возврат ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Организация);
	Иначе
		Возврат ИнтеркампаниСервер.ОрганизацииОстатков(Организация);
	КонецЕсли;
	
КонецФункции

&НаКлиенте
Процедура КомандыГруппПродолжение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныйЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ВыбранныйЭлемент.Значение = "Выбрать" Тогда
		ГруппаВыбрать(Неопределено);
	ИначеЕсли ВыбранныйЭлемент.Значение = "Создать" Тогда
		СоздатьГруппу();
	ИначеЕсли ВыбранныйЭлемент.Значение = "Изменить" Тогда
		ИзменитьГруппу();
	ИначеЕсли ВыбранныйЭлемент.Значение = "Скопировать" Тогда
		СкопироватьГруппу();
	ИначеЕсли ВыбранныйЭлемент.Значение = "УстановитьПометкуУдаления" Тогда
		УстановитьПометкуУдаленияГруппы(Неопределено);
	ИначеЕсли ВыбранныйЭлемент.Значение = "ИерархияВключаяВложенные" Тогда
		ИерархияВключаяВложенные(Неопределено);
	ИначеЕсли ВыбранныйЭлемент.Значение = "ДопУпорядочиваниеУвеличить" Тогда
		РеквизитДопУпорядочиванияУвеличить(Неопределено);
	ИначеЕсли ВыбранныйЭлемент.Значение = "ДопУпорядочиваниеУменьшить" Тогда
		РеквизитДопУпорядочиванияУменьшить(Неопределено);
	Иначе
		Возврат
	КонецЕсли;
...
&НаКлиенте
Процедура КомандыКатегорийПродолжение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = "Создать" Тогда
		СоздатьКатегорию();
	ИначеЕсли ВыбранныйЭлемент.Значение = "Изменить" Тогда
		ИзменитьКатегорию(Неопределено);
	ИначеЕсли ВыбранныйЭлемент.Значение = "Скопировать" Тогда
		СкопироватьКатегорию(Неопределено);
	ИначеЕсли ВыбранныйЭлемент.Значение = "УстановитьПометкуУдаления" Тогда
		УстановитьПометкуУдаленияКатегории(Неопределено);
	ИначеЕсли ВыбранныйЭлемент.Значение = "ОткрытьСправочникКатегории" Тогда
		ОткрытьСправочникКатегории(Неопределено);
	ИначеЕсли ВыбранныйЭлемент.Значение = "ДопУпорядочиваниеУвеличить" Тогда
		РеквизитДопУпорядочиванияКатегорииУвеличить(Неопределено);
	ИначеЕсли ВыбранныйЭлемент.Значение = "ДопУпорядочиваниеУменьшить" Тогда
		РеквизитДопУпорядочиванияКатегорииУменьшить(Неопределено);
	Иначе
		Возврат
	КонецЕсли;
...
&НаКлиенте
Процедура КомандыСегментовПродолжение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныйЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ВыбранныйЭлемент.Значение = "Изменить" Тогда
		ИзменитьСегмент(Неопределено);
	Иначе
		Возврат
	КонецЕсли;
...
&НаКлиенте
Процедура КомандыНоменклатурыПоставщиковПродолжение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = "Создать" Тогда
		СоздатьГруппуНоменклатураПоставщика();
	ИначеЕсли ВыбранныйЭлемент.Значение = "Изменить" Тогда
		ИзменитьНоменклатуруПоставщика(Неопределено);
	ИначеЕсли ВыбранныйЭлемент.Значение = "Скопировать" Тогда
		СкопироватьГруппуПоставщика();
	ИначеЕсли ВыбранныйЭлемент.Значение = "УстановитьПометкуУдаления" Тогда
		УстановитьПометкуУдаленияГруппыПоставщика();
...
&НаСервере
Процедура УдалитьМеткиОтбора(Метки = Неопределено)
...
Процедура ПолученШтрихкод(ДанныеШтрихкода) Экспорт
	...
	ДополнительныеДанныеПоШтрихкоду = ШтрихкодированиеУНФКлиент.ДополнительныеДанныеПоШтрихкоду(ДанныеШтрихкода.Штрихкод, "");
	...
	Если ДополнительныеДанныеПоШтрихкоду.ЕстьРазделительGS1 Тогда 
		ДанныеШтрихкода.Штрихкод = ДополнительныеДанныеПоШтрихкоду.ПредставлениеШтрихкода;		
	КонецЕсли;
	...
	Если Не МассивДанных = Неопределено И МассивДанных.Количество() Тогда
		...
		Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
			ТекущаяСтрока = МассивДанных[0].НоменклатураПоставщика;
		Иначе
			ТекущаяСтрока = Номенклатура;
		КонецЕсли;
		
		Элементы[ТекущаяСтраницаНоменклатуры].ТекущаяСтрока = ТекущаяСтрока;
		...
		Если НЕ ДействиеПриПоискеПоШК = 1 Тогда
			...
			ИначеЕсли НастройкиИзДокумента И ДействиеПриПоискеПоШК = 2 Тогда
				...
				Если Элементы.СписокЗапасы.РежимВыбора И ЗначениеЗаполнено(ТипВладельца) Тогда
					...
					ВыбраннаяСтрока = Элементы[ТекущаяСтраницаНоменклатуры].ТекущиеДанные;
					
					СтруктураСохраняемыхНастроек = СтруктураСохраняемыхНастроекФормыВыбора();
					
					Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
						ХарактеристикаВыбора = ВыбраннаяСтрока.Характеристика;
						НоменклатураПоставщика = ВыбраннаяСтрока.Ссылка;
					Иначе
						ХарактеристикаВыбора = ПредопределенноеЗначение("Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка");
					...
					СтруктураПараметров= Новый Структура("Номенклатура, СтруктураНастроек, Склад, Характеристика, Партия, НоменклатураПоставщика"
					, Номенклатура, СтруктураСохраняемыхНастроек, ПустойСклад, ХарактеристикаВыбора, ПустаяПартия, НоменклатураПоставщика);
	...
	ИначеЕсли НЕ ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		...
		ТекстВопроса = СтрШаблон(НСтр("ru='Создать новую позицию товара с штрихкодом %1'"), Штрихкод);
...
Процедура ВопросСоздатьНоменклатуруСНовымШтрихКодом(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Нет Тогда
		...
		// ЭлектронноеВзаимодействие.РаботаСНоменклатурой
		Если РаботаСНоменклатуройКлиент.ДоступнаФункциональностьПодсистемы() Тогда
			РаботаСНоменклатуройКлиент.НайтиНоменклатуруПоШтрихкодуВСервисе(Параметры.ШтрихКод);
			Возврат;
		КонецЕсли;
		// Конец ЭлектронноеВзаимодействие.РаботаСНоменклатурой
		
		СтрокаСообщения = НСтр("ru = 'Данные по штрихкоду не найдены: %1%'");
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "%1%", Параметры.Штрихкод);
		ОбщегоНазначенияКлиент.СообщитьПользователю(СтрокаСообщения);
	...
	СтруктураПараметров = Новый Структура("ЗначенияЗаполнения", ЗначенияЗаполнения);
	СтруктураПараметров.Вставить("Отбор", СписокЗапасы.Отбор);
	СтруктураПараметров.Вставить("ШтрихКодДляУстановкиПослеЗаписи", Параметры.Штрихкод);
	
	ОткрытьФорму("Справочник.Номенклатура.Форма.ФормаЭлемента", СтруктураПараметров, ЭтаФорма);
...
Процедура СписокЗапасыПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	УчетВалютныхОпераций = Настройки.ДополнительныеСвойства.УчетВалютныхОпераций;
	...
	Если ПоказыватьЦены Тогда
		...
		СтруктураОтображенияЦен.Вставить("УчетВалютныхОпераций", УчетВалютныхОпераций);
...
Процедура ВывестиЦены(СтруктураОтображенияЦен)
	...
	УчетВалютныхОпераций		= СтруктураОтображенияЦен.УчетВалютныхОпераций;
	
	Если Не ПоказыватьЦены Тогда
		Возврат;
	КонецЕсли;
	
	НесколькоВалютДляЦен = Ложь;
	...
	Если Не РаботаСЗакупками Тогда
		Если ПоказыватьМинимальныеЦены Тогда
			...
			("
			 ...
			 |	МинимальныеЦеныНоменклатурыСрезПоследних.Цена,
			 |	ВЫБОР КОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены <> ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) 
			 |		ТОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены 
			 |		ИНАЧЕ МинимальныеЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены
			 |	КОНЕЦ КАК ВалютаЦены
			 ...
			 |ПОМЕСТИТЬ КурсыМинимальныхЦен
			 |ИЗ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаСреза, 
			 |												Валюта В (ВЫБРАТЬ 
			 |															МинимальныеЦеныНоменклатуры.ВалютаЦены 
			 |														ИЗ МинимальныеЦеныНоменклатуры КАК МинимальныеЦеныНоменклатуры)) КАК КурсыВалютСрезПоследних
			 |ИНДЕКСИРОВАТЬ ПО
			 |	Валюта
			 |;
			 |
			 ...
			 |	ЦеныНоменклатуры.Коэффициент КАК Коэффициент,
			 |	ВЫБОР 
			 |		КОГДА ЦеныНоменклатуры.Цена < (ЕСТЬNULL(МинимальныеЦеныНоменклатуры.Цена, 0) 
			 |										* ЕСТЬNULL(КурсыМинимальныхЦен.Курс, 1)
			 |										* ЕСТЬNULL(КурсыМинимальныхЦен.Кратность, 1))
			 |		ТОГДА ЕСТЬNULL(МинимальныеЦеныНоменклатуры.Цена, 0) 
			 |				* ЕСТЬNULL(КурсыМинимальныхЦен.Курс, 1)
			 |				* ЕСТЬNULL(КурсыМинимальныхЦен.Кратность, 1)
			 ...
			 |										* ЕСТЬNULL(КурсыМинимальныхЦен.Курс, 1)
			 |										* ЕСТЬNULL(КурсыМинимальныхЦен.Кратность, 1))
			 |		ТОГДА МинимальныеЦеныНоменклатуры.ВалютаЦены
			 |		ИНАЧЕ ЦеныНоменклатуры.ВалютаЦены 
			 |	КОНЕЦ КАК ВалютаЦены,
			 |	ВЫБОР 
			 |		КОГДА ЦеныНоменклатуры.Цена < (ЕСТЬNULL(МинимальныеЦеныНоменклатуры.Цена, 0) 
			 |										* ЕСТЬNULL(КурсыМинимальныхЦен.Курс, 1)
			 |										* ЕСТЬNULL(КурсыМинимальныхЦен.Кратность, 1))
			 |		ТОГДА МинимальныеЦеныНоменклатуры.ВалютаЦены.СимвольноеПредставление
			 |		ИНАЧЕ ЦеныНоменклатуры.ВалютаЦены.СимвольноеПредставление 
			 |	КОНЕЦ КАК ПредставлениеВалютаЦены
			 |ИЗ ЦеныНоменклатуры КАК ЦеныНоменклатуры
			 |	ЛЕВОЕ СОЕДИНЕНИЕ МинимальныеЦеныНоменклатуры КАК МинимальныеЦеныНоменклатуры
			 |		ПО ЦеныНоменклатуры.Номенклатура = МинимальныеЦеныНоменклатуры.Номенклатура
			 |		И ЦеныНоменклатуры.Характеристика = МинимальныеЦеныНоменклатуры.Характеристика
			 |	ЛЕВОЕ СОЕДИНЕНИЕ КурсыМинимальныхЦен КАК КурсыМинимальныхЦен
			 |		ПО МинимальныеЦеныНоменклатуры.ВалютаЦены = КурсыМинимальныхЦен.Валюта");
		...
		Если ТипЗнч(ВидЦен) = Тип("СправочникСсылка.ВидыЦен") Тогда
			НесколькоВалютДляЦен = ВидЦен.НесколькоВалютДляЦен;
		КонецЕсли;
	Иначе
		Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если УчетВалютныхОпераций Тогда
			ДанныеСтрокиСписка.ВалютаЦены = Выборка.ВалютаЦены;
			Если НесколькоВалютДляЦен Тогда
				ДанныеСтрокиСписка.ВалютаЦеныПредставление = Выборка.ПредставлениеВалютаЦены;
			КонецЕсли;
		КонецЕсли;
		
		ДанныеСтрокиСписка.ЕдиницаИзмерения = Выборка.ЕдиницаИзмерения;
		ДанныеСтрокиСписка.КоличествоОстаток = Окр(ДанныеСтрокиСписка.КоличествоОстаток / Выборка.Коэффициент, 3,
...
&НаСервереБезКонтекста
Процедура СписокНоменклатураПоставщиковПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	
	Если НЕ Настройки.ДополнительныеСвойства.ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		Возврат;
	КонецЕсли;
	...
	УчетВалютныхОпераций = Настройки.ДополнительныеСвойства.УчетВалютныхОпераций;
	Поставщик = Настройки.ДополнительныеСвойства.Поставщик;
	...
	Для Каждого КлючИЗначение Из Строки Цикл
		...
		Ссылки.Добавить(СтрокаСписка.Номенклатура);
		Характеристики.Добавить(СтрокаСписка.Характеристика);
	...
	Если ПоказыватьЦены Тогда
		...
		СтруктураОтображенияЦен.Вставить("Строки", Строки);
		СтруктураОтображенияЦен.Вставить("Ссылки", Ссылки);
		СтруктураОтображенияЦен.Вставить("Характеристики", Характеристики);
		СтруктураОтображенияЦен.Вставить("ВидЦен", ВидЦен);
		СтруктураОтображенияЦен.Вставить("ПоказыватьЦены", ПоказыватьЦены);
		СтруктураОтображенияЦен.Вставить("УчетВалютныхОпераций", УчетВалютныхОпераций);
		СтруктураОтображенияЦен.Вставить("ДатаСреза", ДатаСреза);
		СтруктураОтображенияЦен.Вставить("Поставщик", Поставщик);
		ВывестиЦеныНоменклатурыПоставщиков(СтруктураОтображенияЦен);
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ВывестиЦеныНоменклатурыПоставщиков(СтруктураОтображенияЦен)
	
	Строки 						= СтруктураОтображенияЦен.Строки;
	Ссылки 						= СтруктураОтображенияЦен.Ссылки;
	Характеристики 				= СтруктураОтображенияЦен.Характеристики;
	ВидЦен 						= СтруктураОтображенияЦен.ВидЦен;
	ПоказыватьЦены 				= СтруктураОтображенияЦен.ПоказыватьЦены;
	УчетВалютныхОпераций		= СтруктураОтображенияЦен.УчетВалютныхОпераций;
	ДатаСреза 					= СтруктураОтображенияЦен.ДатаСреза;
	Поставщик 					= СтруктураОтображенияЦен.Поставщик;
	...
	|	ЦеныНоменклатурыКонтрагентовСрезПоследних.ВидЦенКонтрагента.ВалютаЦены.СимвольноеПредставление КАК ПредставлениеВалютаЦены
	|ИЗ
	|	РегистрСведений.ЦеныНоменклатурыКонтрагентов.СрезПоследних(
	|			&ДатаСреза,
	|			ВидЦенКонтрагента = &ВидЦен
	|				И Номенклатура В (&Ссылки)
	|				И Характеристика В (&Характеристики)) КАК ЦеныНоменклатурыКонтрагентовСрезПоследних
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НоменклатураПоставщиков КАК НоменклатураПоставщиков
	|		ПО ЦеныНоменклатурыКонтрагентовСрезПоследних.Номенклатура = НоменклатураПоставщиков.Номенклатура
	|			И ЦеныНоменклатурыКонтрагентовСрезПоследних.Характеристика = НоменклатураПоставщиков.Характеристика
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаСписка = Строки.Получить(Выборка.НоменклатураПоставщика);
		...
		Если УчетВалютныхОпераций Тогда
			ДанныеСтрокиСписка.ВалютаЦены = Выборка.ВалютаЦены;
			ДанныеСтрокиСписка.ПредставлениеВалютаЦены = Выборка.ПредставлениеВалютаЦены;
		КонецЕсли;
...
&НаКлиенте
Процедура ИзменитьПорядокГруппыНоменклатурыНоменклатураПоставщика(Шаг)
	
	ДанныеТекущейСтроки = Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные;
	Если ДанныеТекущейСтроки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ГруппаНоменклатурыСсылка) Тогда
		...
		ИзменитьПорядокГруппыНоменклатурыПоставщикаНаСервере(ГруппаНоменклатурыСсылка, Шаг);
		ЗаполнитьДеревоГруппПоставщика(ГруппаНоменклатурыСсылка, Ложь);
...
&НаСервереБезКонтекста
Процедура ИзменитьПорядокГруппыНоменклатурыПоставщикаНаСервере(ГруппаНоменклатурыСсылка, Шаг)
	...
	|	Справочник.НоменклатураПоставщиков КАК НоменклатураПоставщиков
	|ГДЕ
	|	НоменклатураПоставщиков.Ссылка <> &ГруппаНоменклатуры
	|	И НоменклатураПоставщиков.ЭтоГруппа
	|	И НоменклатураПоставщиков.Родитель = &РодительГруппыНоменклатуры
	|	И НоменклатураПоставщиков.РеквизитДопУпорядочиванияУНФ <= &РеквизитДопУпорядочивания
	|
	|УПОРЯДОЧИТЬ ПО
	|	НоменклатураПоставщиков.РеквизитДопУпорядочиванияУНФ УБЫВ";
...
Функция ОбработатьПодготовленныеДанные(РезультатЗагрузки)
	...
	ДеревоИерархии = ДанныеФормыВЗначение(РезультатЗагрузки.ДеревоИерархии, ТипДеревоЗначений);
	ПараметрыВызоваСервера.Вставить("ДеревоИерархии", ДеревоИерархии);
	...
	Если РезультатЗагрузки.НастройкиЗагрузкиДанных.Свойство("СоответствиеКартинок")
		...
		Для каждого ЭлементСоответствия Из РезультатЗагрузки.НастройкиЗагрузкиДанных.СоответствиеКартинок Цикл
			ДанныеКартинки = ПолучитьИзВременногоХранилища(ЭлементСоответствия.Значение);
...
&НаСервере
Процедура ОбновитьИзображение(Номенклатура, Характеристика = Неопределено)
	
	Файлы = Новый Массив;
	
	Если Не Характеристика = Неопределено Тогда
		
		ЭлементСКартинкой = Характеристика;
		РаботаСФайлами.ЗаполнитьПрисоединенныеФайлыКОбъекту(ЭлементСКартинкой, Файлы);
		ОснованияКартинка = ЭлементСКартинкой.ФайлКартинки;
		
		Если Не ЗначениеЗаполнено(ОснованияКартинка) Тогда
			ЭлементСКартинкой = Номенклатура;
			РаботаСФайлами.ЗаполнитьПрисоединенныеФайлыКОбъекту(ЭлементСКартинкой, Файлы);
			ОснованияКартинка = ЭлементСКартинкой.ФайлКартинки;
		КонецЕсли;
	...
	Для каждого Файл Из Файлы Цикл
		...
		Если НЕ ОтображатьФайлНаФорме(ЭлементСКартинкой, Файл) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура РасшифроватьОстатки()
	...
	Элементы.НазадКСпискуНоменклатуры.Видимость = Истина;
	ЭтотОбъект.АвтоЗаголовок = Ложь;
	ЭтотОбъект.Заголовок = НСтр("ru='Остатки номенклатуры'");
...
Процедура ЗаполнитьДеревоОстатков(Ссылка, ИспользуютсяХарактеристикиНоменклатуры = Неопределено, ИспользоватьОтбор = Ложь, ЭтоУслугаРабота = Неопределено)
	...
	Запрос.УстановитьПараметр("Номинал", ?(Ссылка.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.ПодарочныйСертификат, Ссылка.Номинал, 0));
	...
	Иначе
		...
		Если ИспользуютсяХарактеристикиНоменклатуры Тогда
			...
			Пока РезультатЗапроса.Следующий() Цикл
				...
				НоваяСтрока.НалогообложениеНДСПартии = НоваяСтрока.Партия.НалогообложениеНДС;
...
Функция НоменклатураПоставщиков(Ссылка)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЕСТЬNULL(ОстаткиТоваровПоставщиков.Количество, 0) КАК Остаток,
	...
	|	И НЕ НоменклатураПоставщиков.ПометкаУдаления
	|	И ЕСТЬNULL(ОстаткиТоваровПоставщиков.Количество, 0) > 0";
...
Функция ТекстЗапросаПоОстаткамДляДереваОстатковПолный(ИспользуютсяХарактеристикиНоменклатуры, ИспользуютсяПартииНоменклатуры, ЕстьОрдернаяСхема)
	...
	|		ПОЛНОЕ СОЕДИНЕНИЕ ХарактеристикиНоменклатуры КАК ХарактеристикиНоменклатуры
	...
	|ПОМЕСТИТЬ ЗаказыНаПроизводствоОстатки
	|ИЗ
	...
	|ИЗ
	|	ЗаказыНаПроизводствоОстатки КАК ЗаказыНаПроизводствоОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РазмещениеЗаказовОстатки КАК РазмещениеЗаказовОстатки
	...
	|	СУММА(Заказано.КоличествоОстаток) КАК КоличествоОстаток,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Заказано.КоличествоДокументовЗаказано) КАК КоличествоДокументовЗаказано
	|ПОМЕСТИТЬ ЗаказаноИтог
	|ИЗ
	...
	|	ЗаказыПоставщикамОстатки.Номенклатура КАК Номенклатура,
	|	ЗаказыПоставщикамОстатки.Характеристика КАК Характеристика
	|ПОМЕСТИТЬ ЗаказыПоставщикамОстаткиДокументы
	|ИЗ
	|	ЗаказыПоставщикамОстатки КАК ЗаказыПоставщикамОстатки
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ЗаказыПоставщикамОстаткиДокументы.ЗаказПоставщикуДатаПоступления) КАК ЗаказПоставщикуДатаПоступления,
	|	ЗаказыПоставщикамОстаткиДокументы.Номенклатура КАК Номенклатура,
	|	ЗаказыПоставщикамОстаткиДокументы.Характеристика КАК Характеристика
	|ПОМЕСТИТЬ ЗаказыПоставщикамОстаткиДокументыИтог
	|ИЗ
	|	ЗаказыПоставщикамОстаткиДокументы КАК ЗаказыПоставщикамОстаткиДокументы
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаказыПоставщикамОстаткиДокументы.Номенклатура,
	|	ЗаказыПоставщикамОстаткиДокументы.Характеристика
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	ЗаказаноИтог.КоличествоОстаток КАК КПоступлению,
	|	&Ссылка КАК Ссылка,
	...
	|	ЗаказаноИтог.КоличествоДокументовЗаказано КАК КоличествоДокументовЗаказано,
	|	ЗаказыПоставщикамОстаткиДокументыИтог.ЗаказПоставщикуДатаПоступления КАК ЗаказПоставщикуДатаПоступления
	|ПОМЕСТИТЬ ВТПромежуточныйВывод
	|ИЗ
	|	ИнформацияПоНоменклатуреИтог КАК ИнформацияПоНоменклатуреИтог
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказаноИтог КАК ЗаказаноИтог
	|		ПО ИнформацияПоНоменклатуреИтог.Ссылка = ЗаказаноИтог.Номенклатура
	|			И ИнформацияПоНоменклатуреИтог.Характеристика = ЗаказаноИтог.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказыПоставщикамОстаткиДокументыИтог КАК ЗаказыПоставщикамОстаткиДокументыИтог
	|		ПО ИнформацияПоНоменклатуреИтог.Ссылка = ЗаказыПоставщикамОстаткиДокументыИтог.Номенклатура
	|			И ИнформацияПоНоменклатуреИтог.Характеристика = ЗаказыПоставщикамОстаткиДокументыИтог.Характеристика
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ЗаказаноИтог.КоличествоОстаток,
	|	ЗаказаноИтог.КоличествоДокументовЗаказано,
	|	ЗаказыПоставщикамОстаткиДокументыИтог.ЗаказПоставщикуДатаПоступления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР КОГДА ЦеныНоменклатурыСрезПоследних.ВидЦен.НесколькоВалютДляЦен
	|				ТОГДА ВЫБОР КОГДА ЦеныНоменклатурыСрезПоследних.ВалютаЦены <> ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|							ТОГДА ЦеныНоменклатурыСрезПоследних.ВалютаЦены
	|							ИНАЧЕ ЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены
	|						КОНЕЦ
	|		ИНАЧЕ ВЫБОР КОГДА ЦеныНоменклатурыСрезПоследних.ВалютаЦены <> ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|						И ЦеныНоменклатурыСрезПоследних.ВалютаЦены <> ЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены 
	|					ТОГДА ЦеныНоменклатурыСрезПоследних.ВалютаЦены
	...
	|	ВТПромежуточныйВыводСЦенами.КПоступлению КАК КПоступлению,
	|	ВЫБОР
	|		КОГДА ВТПромежуточныйВыводСЦенами.МинимальнаяЦена <> 0 И &ПоказыватьМинимальныеЦены
	|			ТОГДА ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ВТПромежуточныйВыводСЦенами.МинимальнаяЦена <> 0 И &ПоказыватьМинимальныеЦены
	|			ТОГДА ВТПромежуточныйВыводСЦенами.ВалютаМинимальнойЦены
	|		ИНАЧЕ ВТПромежуточныйВыводСЦенами.ВалютаЦены
	|	КОНЕЦ КАК ВалютаЦены,
	...
	Если Не ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаПроизводство, Пользователи.АвторизованныйПользователь()) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ЗаказыНаПроизводствоОстатки.ЗаказНаПроизводство.Финиш", "ДАТАВРЕМЯ(1,1,1)");
	КонецЕсли;
...
Функция ТекстЗапросаПоОстаткамДляДереваОстатковБезПартий(ИспользуютсяХарактеристикиНоменклатуры, ИспользуютсяПартииНоменклатуры, ЕстьОрдернаяСхема)
	...
	|				КОНЕЦ
	|	КОНЕЦ КАК ВалютаЦены,
	|	ВЫБОР КОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВидЦен.НесколькоВалютДляЦен
	|				ТОГДА ВЫБОР КОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены <> ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|							ТОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены
	|							ИНАЧЕ МинимальныеЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены
	|						КОНЕЦ
	|		ИНАЧЕ ВЫБОР КОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены <> ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|						И МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены <> МинимальныеЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены 
	|					ТОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены
	|					ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|				КОНЕЦ
	|	КОНЕЦ КАК ВалютаМинимальнойЦены,
	...
	|ВЫБРАТЬ
	|	ВТПромежуточныйВыводСЦенами.ХарактеристикаПоказатель КАК ХарактеристикаПоказатель,
	|	ВТПромежуточныйВыводСЦенами.Артикул КАК ХарактеристикаАртикул,
	|	ВТПромежуточныйВыводСЦенами.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ВТПромежуточныйВыводСЦенами.КоличествоОстаток КАК КоличествоОстаток,
	|	ВТПромежуточныйВыводСЦенами.КоличествоВсего КАК КоличествоВсего,
	|	ВТПромежуточныйВыводСЦенами.Резерв КАК Резерв,
	...
	|ВЫБРАТЬ
	|	ВТВывод.ХарактеристикаПоказатель КАК ХарактеристикаПоказатель,
	|	ВТВывод.ХарактеристикаАртикул КАК ХарактеристикаАртикул,
	|	ВТВывод.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	&ДопПоле1,
	|	&ДопПоле2,
	|	&ДопПоле3 КАК ДопПоле3,
	...
	|	КОНЕЦ КАК Цена,
	|	ВТВывод.МинимальнаяЦена КАК МинимальнаяЦена,
	|	ВТВывод.ВалютаЦены КАК ВалютаЦены,
	|	ВТВывод.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ВТВывод.КартинкаКорзина КАК КартинкаКорзина,
	|	ВТВывод.Ссылка КАК Ссылка,
	|	ВТВывод.Номенклатура КАК Номенклатура,
	...
	|	ХарактеристикаПоказатель
	|ИТОГИ
	|	МАКСИМУМ(ХарактеристикаАртикул),
	|	СУММА(ДопПоле3),
	|	СУММА(КоличествоОстаток),
	|	СУММА(КоличествоВсего),
	|	СУММА(Резерв),
...
Функция ТекстЗапросаПоОстаткамДляДереваОстатковБезХарактеристик(ИспользуютсяХарактеристикиНоменклатуры, ИспользуютсяПартииНоменклатуры, ЕстьОрдернаяСхема)
	...
	|	ОстаткиТоваровИРезерв КАК ОстаткиТоваровИРезерв
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказыПоставщикамОстаткиДокументыИтог КАК ЗаказыПоставщикамОстаткиДокументыИтог
	|		ПО ОстаткиТоваровИРезерв.Номенклатура = ЗаказыПоставщикамОстаткиДокументыИтог.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказаноИтог КАК ЗаказаноИтог
	|		ПО ОстаткиТоваровИРезерв.Номенклатура = ЗаказаноИтог.Номенклатура
	|ГДЕ
	|	(НЕ ОстаткиТоваровИРезерв.Количество = 0
	|			ИЛИ НЕ ОстаткиТоваровИРезерв.Резерв = 0)
	|
	...
	|ИЗ
	|	ИнформацияПоНоменклатуреИтог КАК ИнформацияПоНоменклатуреИтог
	|		ПОЛНОЕ СОЕДИНЕНИЕ ПартииНоменклатуры КАК ПартииНоменклатуры
	...
	|	КОНЕЦ КАК МинимальнаяЦена,
	|	ВЫБОР
	|		КОГДА ЦеныНоменклатурыСрезПоследних.ВидЦен.НесколькоВалютДляЦен
	|			ТОГДА ВЫБОР
	|					КОГДА ЦеныНоменклатурыСрезПоследних.ВалютаЦены <> ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|						ТОГДА ЦеныНоменклатурыСрезПоследних.ВалютаЦены
	|					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены
	...
	|					И Номенклатура = &Ссылка
	|					И Актуальность) КАК ЦеныНоменклатурыСрезПоследних
	|		ПО ВТПромежуточныйВыводСЦенами.Ссылка = ЦеныНоменклатурыСрезПоследних.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
	|				,
	|				ВидЦен = &ВидМинимальныхЦен
	|					И Номенклатура = &Ссылка
	|					И Актуальность) КАК МинимальныеЦеныНоменклатурыСрезПоследних
	|		ПО ВТПромежуточныйВыводСЦенами.Ссылка = МинимальныеЦеныНоменклатурыСрезПоследних.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстЗапросаПоОстаткамДляДереваОстатковТолькоПоСкладам(ИспользуютсяХарактеристикиНоменклатуры, ИспользуютсяПартииНоменклатуры,ЕстьОрдернаяСхема)
	...
	|	КОНЕЦ КАК Цена,
	|	ВЫБОР
	|		КОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВидЦен = &ВидМинимальныхЦен
	|			ТОГДА ВЫБОР
	|					КОГДА НЕ МинимальныеЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения.Коэффициент ЕСТЬ NULL
	|						ТОГДА ВЫРАЗИТЬ(МинимальныеЦеныНоменклатурыСрезПоследних.Цена / МинимальныеЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения.Коэффициент КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ЕСТЬNULL(МинимальныеЦеныНоменклатурыСрезПоследних.Цена, 0)
...
Функция ТекстЗапросаХарактеристикиБезОстатковДляДереваОстатков()
	...
	|					И НЕ НоменклатураПоставщиков.ПометкаУдаления
	|					И НоменклатураПоставщиков.УдалитьОстаток > 0)
	...
	|				И ЦеныНоменклатурыСрезПоследних.ВидЦен.НесколькоВалютДляЦен
	|				И ЦеныНоменклатурыСрезПоследних.ВалютаЦены <> ЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены
	|			ТОГДА ЦеныНоменклатурыСрезПоследних.ВалютаЦены
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|	КОНЕЦ КАК ВалютаЦены,
	...
	|				И МинимальныеЦеныНоменклатурыСрезПоследних.ВидЦен.НесколькоВалютДляЦен
	|				И МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены <> МинимальныеЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены
	|			ТОГДА МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|	КОНЕЦ КАК ВалютаМинимальнойЦены,
	|	ЕСТЬNULL(ЗаказаноИтог.КоличествоОстаток, 0) КАК КПоступлению,
	|	ЗаказыПоставщикамОстаткиДокументыИтог.ЗаказПоставщикуДатаПоступления КАК ЗаказПоставщикуДатаПоступления,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказаноИтог КАК ЗаказаноИтог
	|		ПО ХарактеристикиНоменклатуры.Ссылка = ЗаказаноИтог.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказыПоставщикамОстаткиДокументыИтог КАК ЗаказыПоставщикамОстаткиДокументыИтог
	|		ПО ХарактеристикиНоменклатуры.Ссылка = ЗаказыПоставщикамОстаткиДокументыИтог.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
	|				,
	|				ВидЦен = &ВидМинимальныхЦен
	|					И Номенклатура = &Ссылка
	...
	|	КОНЕЦ КАК Цена,
	|	ВЫБОР
	|		КОГДА ВТПредварительнаяВыборкаСЦенами.МинимальнаяЦена <> 0 И &ПоказыватьМинимальныеЦены
	|			ТОГДА ВТПредварительнаяВыборкаСЦенами.ВалютаМинимальнойЦены
	|		ИНАЧЕ ВТПредварительнаяВыборкаСЦенами.ВалютаЦены
	|	КОНЕЦ КАК ВалютаЦены,
...
Функция ТекстЗапросаСтруктурныеЕдиницыБезОстатковДляДереваОстатков()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказаноИтог КАК ЗаказаноИтог
	|		ПО СтруктурныеЕдиницы.Номенклатура = ЗаказаноИтог.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказыПоставщикамОстаткиДокументыИтог КАК ЗаказыПоставщикамОстаткиДокументыИтог
	|		ПО СтруктурныеЕдиницы.Номенклатура = ЗаказыПоставщикамОстаткиДокументыИтог.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
	|				,
	|				ВидЦен = &ВидМинимальныхЦен
	|					И Номенклатура = &Ссылка
...
Процедура ВывестиДеревоОстатков(Ссылка, ДеревоРезультат, ИспользуютсяХарактеристикиНоменклатуры, ИспользуютсяПартииНоменклатуры
	...
	Если ИспользуютсяХарактеристикиНоменклатуры И Не ИспользуютсяПартииНоменклатуры Тогда
		...
		Пока ДеревоРезультат.Следующий() Цикл
			...
			Пока ВыборкаПоСкладам.Следующий() Цикл
				...
				ВалютаЦены = СтрокаПоСкладу.ВалютаЦены;
				Если НЕ ЗначениеЗаполнено(СтрокаПоСкладу.ЦенаПоказатель) Тогда
					СтрокаПоСкладу.ВалютаЦены = Неопределено;
				КонецЕсли;
			...
			Если Не РаботаСЗакупками И ОтборВидЦен.НесколькоВалютДляЦен И ВалютаЦены <> ОтборВидЦен.ВалютаЦены 
				И ЗначениеЗаполнено(СтрокаЗаголовок.ЦенаПоказатель) Тогда
				СтрокаЗаголовок.ВалютаЦены = ВалютаЦены;
			КонецЕсли;
	...
	ИначеЕсли ИспользуютсяХарактеристикиНоменклатуры И ИспользуютсяПартииНоменклатуры Тогда
		...
		Пока ДеревоРезультат.Следующий() Цикл
			...
			Пока ВыборкаПоПартиям.Следующий() Цикл
				...
				СтрокаПоПартии.НалогообложениеНДСПартии = СтрокаПоПартии.Партия.НалогообложениеНДС;
				...
				Если Не ЗначениеЗаполнено(СтрокаПоПартии.ЦенаПоказатель) Тогда
					СтрокаПоПартии.ВалютаЦены = Неопределено;
				КонецЕсли;
				...
				Пока ВыборкаПоСкладам.Следующий() Цикл
					...
					СтрокаПоСкладу.НалогообложениеНДСПартии = СтрокаПоСкладу.Партия.НалогообложениеНДС;
					СтрокаПоСкладу.ПредставлениеПоказателя = Строка(СтрокаПоСкладу.Показатель) + СтрокаОтправительИлиПолучатель(СтрокаПоСкладу.Склад);
	...
	ИначеЕсли Не ИспользуютсяХарактеристикиНоменклатуры И ИспользуютсяПартииНоменклатуры Тогда
		...
		Пока ВыборкаПоПартиям.Следующий() Цикл
			...
			Пока ВыборкаПоСкладам.Следующий() Цикл
				...
				Иначе
					...
					ВыборкаИтогДляЦены = ВыборкаПоСкладам.Выбрать();
					Если ВыборкаИтогДляЦены.Следующий() Тогда
						Цена = ВыборкаИтогДляЦены.Цена;
					КонецЕсли;
		...
		Если ВыборкаПоПартиям.Количество() Тогда
			...
			Если Не РаботаСЗакупками И ОтборВидЦен.НесколькоВалютДляЦен Или ВалютаЦены <> ОтборВидЦен.ВалютаЦены
				И ЗначениеЗаполнено(СтрокаЗаголовок.ЦенаПоказатель) Тогда
				СтрокаЗаголовок.ВалютаЦены = ВалютаЦены;
			КонецЕсли;
...
Функция ДанныеПоПоступлениям(Ссылка)
	...
	|	ЗаказыПоставщикамОстатки.Номенклатура КАК Номенклатура,
	|	ЗаказыПоставщикамОстатки.Характеристика КАК Характеристика
	|ПОМЕСТИТЬ ВТЗаказыПоставщикамОстаток
	|ИЗ
	|	РегистрНакопления.ЗаказыПоставщикам.Остатки(
	|			,
	|			ВЫБОР
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТЗаказыПоставщикамОстаток.КоличествоОстаток КАК Количество,
	|	ВТЗаказыПоставщикамОстаток.ЗаказПоставщику КАК Ссылка,
	|	МИНИМУМ(ГрафикДвиженияЗапасов.Период) КАК ДатаПоступления
	|ПОМЕСТИТЬ ВТИтог
	|ИЗ
	|	ВТЗаказыПоставщикамОстаток КАК ВТЗаказыПоставщикамОстаток
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ГрафикДвиженияЗапасов КАК ГрафикДвиженияЗапасов
	|		ПО ВТЗаказыПоставщикамОстаток.ЗаказПоставщику = ГрафикДвиженияЗапасов.Заказ
	|			И ВТЗаказыПоставщикамОстаток.Номенклатура = ГрафикДвиженияЗапасов.Номенклатура
	|			И ВТЗаказыПоставщикамОстаток.Характеристика = ГрафикДвиженияЗапасов.Характеристика
	|ГДЕ
	|	ВЫБОР
	|			КОГДА &ОтборПоОрганизации
	|				ТОГДА ГрафикДвиженияЗапасов.Организация В (&Организация)
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТЗаказыПоставщикамОстаток.ЗаказПоставщику,
	|	ВТЗаказыПоставщикамОстаток.КоличествоОстаток
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТИтог.Ссылка) КАК КоличествоДокументов,
	|	МИНИМУМ(ВТИтог.ДатаПоступления) КАК ДатаПоступления
	|ИЗ
	|	ВТИтог КАК ВТИтог";
...
Процедура ВывестиУровеньЯчеек(ВыборкаРодитель, Ссылка, СтрокаРодитель, ПроверятьВывод, СтруктураПоиска
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаРодитель.ЕстьЯчейка) И ВыводитьУровнень Тогда
			...
			СтрокаДерева.НалогообложениеНДСПартии = СтрокаДерева.Партия.НалогообложениеНДС;
			СтрокаНеДоступна(СтрокаДерева, Выборка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы);
			Если Не ЗначениеЗаполнено(СтрокаДерева.ЦенаПоказатель) Тогда
				СтрокаДерева.ВалютаЦены = Неопределено;
			КонецЕсли;
...
Процедура ПоказатьОстатки(Команда)
	...
	ПоказатьОстаткиНаКлиенте();
	
КонецПроцедуры

&НаКлиенте
Процедура ПоказатьОстаткиНаКлиенте()
	...
	ТекущиеДанные = Элементы[ТекущаяСтраницаНоменклатуры].ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Или ТекущаяСтраницаНоменклатуры = "ДеревоОстатки" Тогда 
		Возврат
	КонецЕсли;
	
	Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		ОбновитьПанельОстатков(ТекущиеДанные.Номенклатура, ТекущиеДанные.ЕдиницаИзмерения, ТекущиеДанные.Характеристика);
	Иначе
...
Процедура КорзинаРасшифровкаПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если Не ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		ПеретащитьВКорзину(ПараметрыПеретаскивания);
	Иначе
		ПеретащитьВКорзинуНоменклатураПоставщика(ПараметрыПеретаскивания);
	КонецЕсли;
...
Процедура ПоказатьСписокВыбораДокументов(Элемент)
	...
	Если ПолныеПрава Тогда
		...
		Если ИспользоватьАктВыполненныхРабот Тогда
...
Процедура КорзинаРасшифровкаНоменклатураПриИзменении(Элемент)
	...
	КорзинаРасшифровкаНоменклатураПриИзмененииПродолжить(СтрокаКорзины);
	
КонецПроцедуры // КорзинаНоменклатураПриИзменении()

&НаКлиенте
Процедура КорзинаРасшифровкаНоменклатураПриИзмененииПродолжить(СтрокаКорзины, ИзменениеНоменклатуры = Истина)
	...
	СтруктураДанные.Вставить("Поставщик", 			Поставщик);
	СтруктураДанные.Вставить("ВидОперации", 		ВидОперации);
	...
	Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" И ИзменениеНоменклатуры Тогда
		СтрокаКорзины.НоменклатураПоставщика = СтруктураДанные.НоменклатураПоставщика;
	КонецЕсли;
	...
	СтрокаКорзины.ЗапретитьВводДробногоКоличества = СтруктураДанные.ЗапретитьВводДробногоКоличества;
	
	СтрокаКорзины.ИспользоватьХарактеристики = СтруктураДанные.ИспользоватьХарактеристики;
	СтрокаКорзины.ПроверятьЗаполнениеХарактеристики = СтруктураДанные.ПроверятьЗаполнениеХарактеристики;
	
	Если СтруктураДанные.ИспользоватьХарактеристики Тогда 
		СтрокаКорзины.Характеристика = СтруктураДанные.Характеристика;
	КонецЕсли;
	
	СтрокаКорзины.ИспользоватьПартии = СтруктураДанные.ИспользоватьПартии;
	СтрокаКорзины.ПроверятьЗаполнениеПартий = СтруктураДанные.ПроверятьЗаполнениеПартий;
	
	Если СтруктураДанные.ИспользоватьПартии Тогда
			СтрокаКорзины.Партия = СтруктураДанные.Партия;
	КонецЕсли;
...
&НаКлиенте
Процедура КорзинаРасшифровкаНоменклатураПоставщикаПриИзменении(Элемент)
	
	СтрокаКорзины = Элементы.КорзинаРасшифровка.ТекущиеДанные;
	
	ЗаполнитьЗначенияСвойств(СтрокаКорзины, НоменклатураИХарактеристикаПоНоменклатуреПоставщика(СтрокаКорзины.НоменклатураПоставщика));
	
	КорзинаРасшифровкаНоменклатураПриИзмененииПродолжить(СтрокаКорзины, Ложь);
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	РаботаСКомиссионерамиКомитентамиСервер.СкорректироватьСтавкуНДСКомиссионногоТовара(СтруктураДанные, "НоменклатураСтавкаНДС");
	...
	СтруктураДанные.Вставить("НоменклатураПоставщика", НоменклатураВДокументахСервер.НоменклатураПоставщикаПоНоменклатуре(СтруктураДанные.Поставщик, СтруктураДанные.Номенклатура, СтруктураДанные.Характеристика));
	...
	СтруктураДанные.Вставить("ЗапретитьВводДробногоКоличества", СтруктураДанные.Номенклатура.ЗапретитьВводДробногоКоличества);
...
Процедура КорзинаРасшифровкаХарактеристикаПриИзменении(Элемент)
	...
	СтруктураДанные.Вставить("Номенклатура", 		СтрокаКорзины.Номенклатура);
	СтруктураДанные.Вставить("Характеристика", 		СтрокаКорзины.Характеристика);
	СтруктураДанные.Вставить("Партия", 				СтрокаКорзины.Партия);
	СтруктураДанные.Вставить("НалогообложениеНДС",	НалогообложениеНДС);
	СтруктураДанные.Вставить("Поставщик", 			Поставщик);
	СтруктураДанные.Вставить("ВидОперации", 		ВидОперации);
...
Процедура КорзинаРасшифровкаКоличествоПриИзменении(Элемент)
	...
	Если Элементы.КорзинаРасшифровкаРезерв.Видимость И ЗначениеЗаполнено(СтрокаКорзины.Резерв) Тогда
		СтрокаКорзины.Резерв = СтрокаКорзины.Количество;
	КонецЕсли;
...
Процедура ПеренестиВДокумент(Команда)
	...
	Иначе
		...
		Для Каждого СтрокаКорзины Из Корзина Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаКорзины.ЕдиницаИзмерения) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'В строке %1 корзины не заполнена единица измерения'"), НомерСтроки);
				КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Корзина", НомерСтроки, "ЕдиницаИзмерения");
...
Процедура ОбновитьКартинкуКорзинаРасшифровкаОстатков()
	...
	ПараметрыОтбора = Новый Структура("Номенклатура, Характеристика, СтруктурнаяЕдиница");
...
Процедура КорзинаРасшифровкаПартияПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(Организация) Или Не ЗначениеЗаполнено(НалогообложениеНДС) Тогда
		Возврат;
	КонецЕсли;
	...
	КорзинаРасшифровкаПартияПриИзмененииНаСервереБезКонтекста(СтруктураДанные);
...
&НаСервереБезКонтекста
Процедура КорзинаРасшифровкаПартияПриИзмененииНаСервереБезКонтекста(СтруктураДанные)
...
Процедура ПоказатьВСписке(Команда)
	...
	Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		Ссылка = ДанныеКорзины.НоменклатураПоставщика;
	Иначе
	...
	Если ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение(
		...
		КонецЕсли;
	ИначеЕсли ОтборКатегорииИерархияПереключатель = ПредопределенноеЗначение("Перечисление.ВидыОтборовНоменклатуры.ГруппыНоменклатурыПоставщиков") Тогда
		ТекущиеДанные = Элементы.ОтборНоменклатураПоставщиков.ТекущиеДанные;
		Если Не ТекущиеДанные = Неопределено И Не ТекущиеДанные.Представление = ПредставлениеВсеГруппы() Тогда
			ПоказатьВСпискеСервер(Ссылка, "ОтборНоменклатураПоставщиков");
		КонецЕсли;
	Иначе
		ТекущиеДанные = Элементы.ОтборИерархия.ТекущиеДанные;
		Если Не ТекущиеДанные = Неопределено И Не ТекущиеДанные.Представление = ПредставлениеВсеГруппы() Тогда
...
Процедура ПоказатьВСпискеКлиент()
	...
	Если ТекущаяСтраницаНоменклатуры = "СписокНоменклатураПоставщиков" Тогда
		Ссылка = ДанныеКорзины.НоменклатураПоставщика;
		Элементы.СписокНоменклатураПоставщиков.ТекущаяСтрока = Ссылка;
	Иначе
		Ссылка = ДанныеКорзины.Номенклатура;
...
Процедура ПоказатьВСпискеСервер(Ссылка, ВидДереваИерархии)
	...
	ИначеЕсли ВидДереваИерархии = "ОтборНоменклатураПоставщиков" Тогда
		УстановитьОтборПоДеревуИерархии("ОтборНоменклатураПоставщиков", Ссылка.Родитель)
	Иначе
...
Функция ПолучитьДанныеПоШтрихкодамВМассиве(ДанныеШтрихкода, СтруктурнаяЕдиница = Неопределено)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекДанные.Вставить("НоменклатураПоставщика", НоменклатураПоставщика(Выборка.Номенклатура, Выборка.Характеристика));
