...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Если НачальноеЗаполнениеЭлементовВыполнено() Тогда
		Возврат;
	КонецЕсли;
	
	// 1. Новые контрагенты
	Элемент = Элементы.Добавить();
	Элемент.Наименование = НСтр("ru='Новые за неделю'");
	Элемент.Комментарий = НСтр("ru='# Сегмент создан автоматически'");
	Элемент.ИмяШаблонаСКД = "Основная";
	...
	Элемент.СхемаКомпоновкиДанных = Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(АдресСКД));
	
	Если ОбщегоНазначенияУНФ.ЭтоУНФ() Тогда
		// 2. Давно не связывались
		Элемент = Элементы.Добавить();
		Элемент.Наименование = НСтр("ru='Не связывались больше месяца'");
		Элемент.Комментарий = НСтр("ru='# Сегмент создан автоматически'");
		Элемент.ИмяШаблонаСКД = "Основная";
	...
	Элемент = Элементы.Добавить();
	Элемент.Наименование = НСтр("ru='Есть заказы за последний месяц'");
	Элемент.Комментарий = НСтр("ru='# Сегмент создан автоматически'");
	Элемент.ИмяШаблонаСКД = "Основная";
...
Функция НачальноеЗаполнениеЭлементовВыполнено()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СегментыКонтрагентов.Ссылка КАК Ссылка
	|ИЗ
