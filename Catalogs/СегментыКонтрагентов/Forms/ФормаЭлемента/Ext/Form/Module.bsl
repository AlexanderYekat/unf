...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ТипЗнч(СКД) <> Тип("СхемаКомпоновкиДанных") Тогда
			СКД = Справочники.СегментыКонтрагентов.ПолучитьМакет(МакетСегментов);
		КонецЕсли;
	...
	Если ТекущийШаблонСхемыКомпоновкиДанных = "Основная" Тогда
		...
		НоваяСКД.НастройкиПоУмолчанию.ПараметрыДанных.УстановитьЗначениеПараметра("НаборСвойств_Справочник_Контрагенты",
...
Процедура КомпоновщикНастроекНастройкиОтборПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ТекущиеДанные.Свойство("ТипГруппы", ТипГруппы);
	
	Если Копирование Тогда
		Если ТипГруппы = Неопределено Тогда
			НовыйЭлементОтбора = КомпоновщикНастроек.Настройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			НовыйЭлементОтбора.ЛевоеЗначение = ТекущиеДанные.ЛевоеЗначение;	
			НовыйЭлементОтбора.Использование = Истина;
			НовыйЭлементОтбора.ВидСравнения = ТекущиеДанные.ВидСравнения;
			НовыйЭлементОтбора.ПравоеЗначение = ТекущиеДанные.ПравоеЗначение; 
		КонецЕсли;
		
		Если ТипГруппы <> Неопределено Тогда
			НовыйЭлементОтбора = КомпоновщикНастроек.Настройки.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
			НовыйЭлементОтбора.ТипГруппы = ТекущиеДанные.ТипГруппы;
		КонецЕсли;
...
&НаКлиенте
Процедура СоставКонтрагентовПоПравиламВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПоказатьЗначение(,ТекущиеДанныеСписка.Контрагент);
КонецПроцедуры
...
Процедура ЗаполнитьПоляТаблицыОтбора()
	...
	Для Каждого ПолеОтбора Из КомпоновщикНастроек.Настройки.Отбор.ДоступныеПоляОтбора.Элементы Цикл
		...
		Если Строка(ПолеОтбора.Поле) = "События" И ЭтоРозница Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого ПравилоОтбора Из ПолеОтбора.Элементы Цикл
			Если Строка(ПравилоОтбора.Поле) = "ЗаказыПокупателей.СостояниеЗаказНаряда" Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ДобавитьДополнительныеРеквизиты(ГруппаДобавления)
	...
	|	И ДополнительныеРеквизитыИСведения.НаборСвойств = &НаборСвойств_Справочник_Контрагенты
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДополнительныеРеквизитыИСведения.Заголовок";
	
	НаборСвойств_Справочник_Контрагенты = УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Контрагенты");
	Запрос.УстановитьПараметр("НаборСвойств_Справочник_Контрагенты", НаборСвойств_Справочник_Контрагенты);
		...
		Для Каждого ЭлементГруппы Из ГруппаПараметров.Элементы Цикл
			...
			Для Каждого РеквизитГруппы Из ЭлементГруппы.Элементы Цикл
				...
				Если РеквизитГруппы.Заголовок <> ПолеДопРеквизита 
					ИЛИ РеквизитГруппы.ТипЗначения <> ТипЗначения Тогда
					Продолжить;
				КонецЕсли;
...
Процедура ДобавитьПоляКонтактнойИнформации(ГруппаДобавления)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТипЗнчКИ = Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(1024));
		...
		Если Выборка.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			...
			НовоеПравило.ЭтоГруппа = Ложь;
			НовоеПравило.Картинка = 5;
			НовоеПравило.ТипЗначенияПоля = ТипЗнчКИ;
