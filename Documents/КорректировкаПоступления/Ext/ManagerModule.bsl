...
Функция ПолучитьИсправляемыйДокументПоступления(ДокПоступления, Исходный = Ложь) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ДокПоступления) 
		И ТипЗнч(ДокПоступления) = Тип("ДокументСсылка.АктОРасхождениях") Тогда
...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	|								ИНАЧЕ ТаблицаЗапасы.СуммаБезНДС + ТаблицаЗапасы.СуммаРасходовБезНДС
	|							КОНЕЦ
	|					ИНАЧЕ ВЫБОР
	|							КОГДА ТаблицаЗапасы.НДСВключатьВСтоимость
	|								ТОГДА ТаблицаЗапасы.Сумма
	|							ИНАЧЕ ТаблицаЗапасы.СуммаБезНДС
	|						КОНЕЦ
	|				КОНЕЦ
	|		КОНЕЦ) КАК Сумма,
	...
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ТаблицаЗапасы.ВключатьРасходыВСебестоимость
	|						ТОГДА ТаблицаЗапасы.СуммаБезНДС + ТаблицаЗапасы.СуммаРасходовБезНДС
	|					ИНАЧЕ ТаблицаЗапасы.СуммаБезНДС
	|				КОНЕЦ
...
Процедура СформироватьТаблицаСерииНоменклатуры(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаЗапасы.ВидОперацииДокументаПоступления = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя)
	|				ИЛИ ВременнаяТаблицаЗапасы.ВидОперацииДокументаПоступления = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтКомиссионера)
	|				ИЛИ ВременнаяТаблицаЗапасы.ВидОперацииДокументаПоступления = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПереработчика)
	|				ИЛИ ВременнаяТаблицаЗапасы.ВидОперацииДокументаПоступления = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратСОтветХранения)
	...
	|				И ВременнаяТаблицаЗапасы.ВидУчетаПоВЦеломИлиПоДокументам)
	|	И НЕ СерииНоменклатуры.Количество = 0";
...
Процедура СформироватьТаблицаСерииНоменклатурыКПоступлению(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	|		ПО ВременнаяТаблицаЗапасы.КлючСвязи = СерииНоменклатуры.КлючСвязи
	|ГДЕ
	|	ВременнаяТаблицаЗапасы.ОрдерныйСклад И ВременнаяТаблицаЗапасы.ОрдерныйСкладКПоступлению
	|	И НЕ СерииНоменклатуры.Количество = 0";
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства)
	...
	МоментКонтроля = Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Исключая);
	...
	Запрос.УстановитьПараметр("ВосстановлениеАванса", НСтр("ru='Восстановление предоплаты'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница'"));
	...
	ОсвобождатьАванс = СтруктураДополнительныеСвойства.ДляПроведения.ОсвобождатьАванс;
	...
	Если Не ОсвобождатьАванс Тогда
	...
	Если ОсвобождатьАванс Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПоставщиками",
			"ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПоставщикамиПредварительная");
		...
		|ИЗ
		|	РегистрНакопления.РасчетыСПоставщиками.Остатки(&МоментКонтроля, ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг)) КАК РасчетыСПоставщикамиОстатки
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	ТипРасчетов,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасчетыСПоставщиками.ВидДвижения КАК ВидДвижения,
		...
		|	ВременнаяТаблицаРасчетыСПоставщиками.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
		...
		|	ВременнаяТаблицаРасчетыСПоставщиками.Курс КАК Курс,
		|	ВременнаяТаблицаРасчетыСПоставщиками.Кратность КАК Кратность,
		|	ВременнаяТаблицаРасчетыСПоставщиками.ТипРасчетов КАК ТипРасчетов,
		|	ВременнаяТаблицаРасчетыСПоставщиками.Сумма КАК Сумма,
		|	ВременнаяТаблицаРасчетыСПоставщиками.СуммаВал КАК СуммаВал,
		|	ВременнаяТаблицаРасчетыСПоставщиками.СуммаРег КАК СуммаРег,
		|	ВременнаяТаблицаРасчетыСПоставщиками.СуммаДляОстатка КАК СуммаДляОстатка,
		|	ВременнаяТаблицаРасчетыСПоставщиками.СуммаВалДляОстатка КАК СуммаВалДляОстатка,
		|	ВременнаяТаблицаРасчетыСПоставщиками.СодержаниеПроводки КАК СодержаниеПроводки,
		|	ЕСТЬNULL(ВременнаяТаблицаРасчетыСПоставщикамиОстатки.СуммаОстаток, 0) КАК СуммаОстаток,
		|	ЕСТЬNULL(ВременнаяТаблицаРасчетыСПоставщикамиОстатки.СуммаВалОстаток, 0) КАК СуммаВалОстаток,
		|	ЕСТЬNULL(ВременнаяТаблицаРасчетыСПоставщикамиОстатки.СуммаРегОстаток, 0) КАК СуммаРегОстаток
		|ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПоставщикамиПредварительная КАК ВременнаяТаблицаРасчетыСПоставщиками
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаРасчетыСПоставщикамиОстатки КАК ВременнаяТаблицаРасчетыСПоставщикамиОстатки
		|		ПО ВременнаяТаблицаРасчетыСПоставщиками.Организация = ВременнаяТаблицаРасчетыСПоставщикамиОстатки.Организация
		|			И ВременнаяТаблицаРасчетыСПоставщиками.ТипРасчетов = ВременнаяТаблицаРасчетыСПоставщикамиОстатки.ТипРасчетов
		|			И ВременнаяТаблицаРасчетыСПоставщиками.Контрагент = ВременнаяТаблицаРасчетыСПоставщикамиОстатки.Контрагент
		|			И ВременнаяТаблицаРасчетыСПоставщиками.Договор = ВременнаяТаблицаРасчетыСПоставщикамиОстатки.Договор
		|			И ВременнаяТаблицаРасчетыСПоставщиками.Документ = ВременнаяТаблицаРасчетыСПоставщикамиОстатки.Документ
		|			И ВременнаяТаблицаРасчетыСПоставщиками.Заказ = ВременнаяТаблицаРасчетыСПоставщикамиОстатки.Заказ
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	Контрагент,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.ВидДвижения КАК ВидДвижения,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Дата КАК Дата,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Организация КАК Организация,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Контрагент КАК Контрагент,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.ВестиРасчетыПоДокументам КАК ВестиРасчетыПоДокументам,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СчетУчета КАК СчетУчета,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Договор КАК Договор,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Документ КАК Документ,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Заказ КАК Заказ,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Валюта КАК Валюта,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Курс КАК Курс,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Кратность КАК Кратность,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.ТипРасчетов КАК ТипРасчетов,
		|	ВЫБОР
		|		КОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаОстаток > ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Сумма
		|			ТОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаОстаток
		|		ИНАЧЕ ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Сумма
		|	КОНЕЦ КАК Сумма,
		|	ВЫБОР
		|		КОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВалОстаток > ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВал
		|			ТОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВалОстаток
		|		ИНАЧЕ ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВал
		|	КОНЕЦ КАК СуммаВал,
		|	ВЫБОР
		|		КОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаРегОстаток > ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаРег
		|			ТОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаРегОстаток
		|		ИНАЧЕ ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаРег
		|	КОНЕЦ КАК СуммаРег,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаДляОстатка КАК СуммаДляОстатка,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВалДляОстатка КАК СуммаВалДляОстатка,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СодержаниеПроводки КАК СодержаниеПроводки
		|ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПоставщиками
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов КАК ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов
		|ГДЕ
		|	(ВЫБОР
		|		КОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаОстаток > ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Сумма
		|			ТОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаОстаток
		...
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Дата,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Организация,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Контрагент,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.ВестиРасчетыПоДокументам,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СчетУчета,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Договор,
		|	&Ссылка,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Заказ,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Валюта,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Курс,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Кратность,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Аванс),
		|	ВЫБОР
		|		КОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаОстаток > ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Сумма
		|			ТОГДА -(ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаОстаток + ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Сумма)
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВалОстаток > ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВал
		|			ТОГДА -(ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВалОстаток + ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВал)
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаРегОстаток > ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаРег
		|			ТОГДА -(ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаРегОстаток + ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаРег)
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаДляОстатка,
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВалДляОстатка,
		|	&ВосстановлениеАванса
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов КАК ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов
		|ГДЕ
		|	(-ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаОстаток > ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.Сумма
		|			ИЛИ -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВалОстаток > ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаВал
		|			ИЛИ -ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаРегОстаток > ВременнаяТаблицаРасчетыСПоставщикамиДляРасчетаАвансов.СуммаРег)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	Контрагент,
...
Процедура СформироватьТаблицаОплатаДокументов(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	МассивДоступныхОпераций.Добавить(Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки);
	МассивДоступныхОпераций.Добавить(Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение);
...
Процедура СформироватьТаблицаЗакупкиДляКУДиР(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	// 4. Обойдем все уменьшенные предоплаты. Для каждой обойдем все товары, начиная с тех, что были уменьшены и оплачены
	// текущей предоплатой. ТЧ Запасы определяет порядок сторнирования, но сумму сторнирования определяет информация о
	// товарах, которые были оплачены текущей предоплатой. Для каждого товара поищем информацию об оплате и сторнируем ее.
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства) Экспорт
	...
	|	&ДокументПоступления КАК КорректируемыйДокумент,
	...
	|	КорректировкаПоступленияЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ВЫБОР
	...
	|	КорректировкаПоступленияЗапасы.СуммаРасходовБезНДС КАК СуммаРасходовБезНДС,
	...
	|	ЕСТЬNULL(ПолитикиУчетаСерий.ПолитикаУчетаСерий, КорректировкаПоступленияЗапасы.Номенклатура.ПолитикаУчетаСерий) КАК ПолитикаУчетаСерий,
	|	КорректировкаПоступленияЗапасы.Ссылка.ИсправляемыйДокументПоступления.ВидОперации КАК ВидОперацииДокументаПоступления,
	|	КорректировкаПоступленияЗапасы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	КорректировкаПоступленияЗапасы.ДокументПоступления КАК ДокументПоступления
	|ПОМЕСТИТЬ ВременнаяТаблицаИзмененияЗапасов
	|ИЗ
	|	Документ.КорректировкаПоступления.Запасы КАК КорректировкаПоступленияЗапасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПолитикиУчетаСерий КАК ПолитикиУчетаСерий
	|		ПО КорректировкаПоступленияЗапасы.Номенклатура = ПолитикиУчетаСерий.Владелец
	|			И КорректировкаПоступленияЗапасы.СтруктурнаяЕдиница = ПолитикиУчетаСерий.СтруктурнаяЕдиница
	...
	|	ИзмененияЗапасов.СтруктурнаяЕдиница.ОрдерныйСклад
	|		И ИзмененияЗапасов.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриПоступлении КАК ОрдерныйСклад,
	|	ВЫБОР
	|		КОГДА &УчетПоЯчейкам
	...
	|	ВЫБОР
	|		КОГДА ИзмененияЗапасов.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.НДСПоПриобретеннымЦенностям)
	|		ИНАЧЕ ИзмененияЗапасов.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям
	|	КОНЕЦ КАК СчетУчетаНДСПоПриобретеннымЦенностям,
	|	ВЫБОР
	|		КОГДА ИзмененияЗапасов.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИзмененияЗапасов.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям.Валютный
	...
	|				ТОГДА 0
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаУчета
	...
	|					ИНАЧЕ ИзмененияЗапасов.СуммаНДС * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * &КратностьДокумента)
	|				КОНЕЦ
	...
	|			ИНАЧЕ ИзмененияЗапасов.Всего * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * &КратностьДокумента)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Сумма,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаУчета
	|				ТОГДА ИзмененияЗапасов.Всего - ИзмененияЗапасов.СуммаНДС
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА (ИзмененияЗапасов.Всего - ИзмененияЗапасов.СуммаНДС) * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ (ИзмененияЗапасов.Всего - ИзмененияЗапасов.СуммаНДС) * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * &КратностьДокумента)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаБезНДС,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаУчета
	|				ТОГДА ИзмененияЗапасов.СуммаРасходов
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	...
	|			ИНАЧЕ ИзмененияЗапасов.СуммаРасходов * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * &КратностьДокумента)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРасходов,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА ИзмененияЗапасов.СуммаРасходов * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ИзмененияЗапасов.СуммаРасходов
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРасходовВал,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаУчета
	|				ТОГДА ИзмененияЗапасов.СуммаРасходовБезНДС
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА ИзмененияЗапасов.СуммаРасходовБезНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ИзмененияЗапасов.СуммаРасходовБезНДС * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * &КратностьДокумента)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРасходовБезНДС,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА ИзмененияЗапасов.СуммаРасходовБезНДС * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ИзмененияЗапасов.СуммаРасходовБезНДС
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРасходовБезНДСВал,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаУчета
	|				ТОГДА ИзмененияЗапасов.Себестоимость
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	...
	|			ИНАЧЕ ИзмененияЗапасов.Себестоимость * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * &КратностьДокумента)
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА ИзмененияЗапасов.Всего * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента * КурсыРегВалюты.Кратность)
	...
	|						ТОГДА ИзмененияЗапасов.СуммаНДС
	|					КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|						ТОГДА ИзмененияЗапасов.СуммаНДС * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента * КурсыРегВалюты.Кратность)
	...
	|	ИзмененияЗапасов.ПолитикаУчетаСерий КАК ПолитикаУчетаСерий,
	|	ИзмененияЗапасов.ВидОперацииДокументаПоступления КАК ВидОперацииДокументаПоступления,
	...
	|	ИзмененияЗапасов.СтавкаНДС.Ставка КАК СтавкаНДССтавка,
	|	ВременнаяТаблицаШапка.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	|	ВЫБОР
	|		КОГДА ИзмененияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА ИзмененияЗапасов.Ссылка
	|		КОГДА ИзмененияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ИзмененияЗапасов.ДокументПоступления
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументПоступления,
	|	ВЫБОР
	|		КОГДА ИзмененияЗапасов.СтруктурнаяЕдиница.ОрдерныйСклад
	|				И ИзмененияЗапасов.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриПоступлении
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОрдерныйСкладКПоступлению,
	|	ВЫБОР
	|		КОГДА ИзмененияЗапасов.СтруктурнаяЕдиница.УчетОстатковПоЯчейкам
	|			ТОГДА ИзмененияЗапасов.Ячейка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ЯчейкаКПоступлению,
	|	ИзмененияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|		ИЛИ ИзмененияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом) КАК ВидУчетаПоВЦеломИлиПоДокументам
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияРасходов.Ссылка.ВалютаДокумента = &ВалютаУчета
	|				ТОГДА ИзмененияРасходов.Всего
	...
	|			ИНАЧЕ ИзмененияРасходов.Всего * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * &КратностьДокумента)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Сумма,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияРасходов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА ИзмененияРасходов.Всего * КурсыРегВалюты.Курс * &КратностьДокумента / (&КурсДокумента * КурсыРегВалюты.Кратность)
	...
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ИзмененияРасходов.Ссылка.ВалютаДокумента = &ВалютаУчета
	|						ТОГДА ИзмененияРасходов.СуммаНДС
	|					КОГДА ИзмененияРасходов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	...
	|					ИНАЧЕ ИзмененияРасходов.СуммаНДС * &КурсДокумента * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * &КратностьДокумента)
	|				КОНЕЦ
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДС,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ВременнаяТаблицаШапка.НДСВключатьВСтоимость
	...
	|			ИНАЧЕ ВЫБОР
	|					КОГДА  ИзмененияРасходов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|						ТОГДА ИзмененияРасходов.СуммаНДС * КурсыРегВалюты.Курс *  &КратностьДокумента / (&КурсДокумента * КурсыРегВалюты.Кратность)
	|					ИНАЧЕ ИзмененияРасходов.СуммаНДС
	|				КОНЕЦ
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДСВал,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияРасходов.Ссылка.ВалютаДокумента = &ВалютаУчета
	...
	|	ВЫБОР
	|		КОГДА ИзмененияРасходов.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.НДСПоПриобретеннымЦенностям)
	|		ИНАЧЕ ИзмененияРасходов.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям
	|	КОНЕЦ КАК СчетУчетаНДСПоПриобретеннымЦенностям,
	|	ВЫБОР
	|		КОГДА ИзмененияРасходов.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИзмененияРасходов.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям.Валютный
	|	КОНЕЦ КАК СчетУчетаНДСПоПриобретеннымЦенностямВалютный,
	...
	|			КОНЕЦ * &КурсДокумента * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * &КратностьДокумента) КАК ЧИСЛО(15, 2))) КАК Сумма,
	|	СУММА(ВЫБОР
	|			КОГДА НЕ КорректировкаПоступленияПредоплата.ЕстьВДокументеПоступления
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КорректировкаПоступленияСерииНоменклатуры.КлючСвязи КАК КлючСвязи,
	|	КорректировкаПоступленияСерииНоменклатуры.Серия КАК Серия,
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияСерииНоменклатуры.Количество = 0
	|			ТОГДА 1
	|		ИНАЧЕ КорректировкаПоступленияСерииНоменклатуры.Количество
	|	КОНЕЦ КАК Количество,
	|	ВЫБОР
	...
	|ПОМЕСТИТЬ ВременнаяТаблицаСерииНоменклатурыПослеКорректировки
	|ИЗ
	|	Документ.КорректировкаПоступления.СерииНоменклатуры КАК КорректировкаПоступленияСерииНоменклатуры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы
	|		ПО (ВременнаяТаблицаЗапасы.КлючСвязи = КорректировкаПоступленияСерииНоменклатуры.КлючСвязи)
	|ГДЕ
	|	КорректировкаПоступленияСерииНоменклатуры.Ссылка = &Ссылка
	|	И &ИспользоватьСерииНоменклатуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КорректировкаПоступленияСерииНоменклатурыДоКорректировки.КлючСвязи КАК КлючСвязи,
	|	КорректировкаПоступленияСерииНоменклатурыДоКорректировки.Серия КАК Серия,
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияСерииНоменклатурыДоКорректировки.Количество = 0
	|			ТОГДА 1
	|		ИНАЧЕ КорректировкаПоступленияСерииНоменклатурыДоКорректировки.Количество
	|	КОНЕЦ КАК Количество,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаЗапасы.ПолитикаУчетаСерий = ЗНАЧЕНИЕ(Справочник.ПолитикаУчетаСерий.ПустаяСсылка)
	|			ТОГДА &ОстаткиСерийНоменклатуры
	...
	|			КОНЕЦ
	|	КОНЕЦ КАК ОстаткиСерийНоменклатуры
	|ПОМЕСТИТЬ ВременнаяТаблицаСерииНоменклатурыДоКорректировки
	|ИЗ
	|	Документ.КорректировкаПоступления.СерииНоменклатурыДоКорректировки КАК КорректировкаПоступленияСерииНоменклатурыДоКорректировки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы
	|		ПО (ВременнаяТаблицаЗапасы.КлючСвязи = КорректировкаПоступленияСерииНоменклатурыДоКорректировки.КлючСвязи)
	|ГДЕ
	|	КорректировкаПоступленияСерииНоменклатурыДоКорректировки.Ссылка = &Ссылка
	|	И &ИспользоватьСерииНоменклатуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыДоКорректировки.Количество, 0) > ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.Количество, 0)
	|			ТОГДА ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.Количество, 0) - ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыДоКорректировки.Количество, 0)
	|		ИНАЧЕ ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.Количество, 0) - ЕСТЬNULL(ВременнаяТаблицаСерииНоменклатурыДоКорректировки.Количество, 0)
	|	КОНЕЦ КАК Количество,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаСерииНоменклатурыДоКорректировки.КлючСвязи ЕСТЬ NULL
	|			ТОГДА ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.КлючСвязи
	|		ИНАЧЕ ВременнаяТаблицаСерииНоменклатурыДоКорректировки.КлючСвязи
	|	КОНЕЦ КАК КлючСвязи,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаСерииНоменклатурыДоКорректировки.ОстаткиСерийНоменклатуры ЕСТЬ NULL
	|			ТОГДА ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.ОстаткиСерийНоменклатуры
	|		ИНАЧЕ ВременнаяТаблицаСерииНоменклатурыДоКорректировки.ОстаткиСерийНоменклатуры
	|	КОНЕЦ КАК ОстаткиСерийНоменклатуры,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаСерииНоменклатурыДоКорректировки.Серия ЕСТЬ NULL
	|			ТОГДА ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.Серия
	|		ИНАЧЕ ВременнаяТаблицаСерииНоменклатурыДоКорректировки.Серия
	|	КОНЕЦ КАК Серия
	|ПОМЕСТИТЬ ВременнаяТаблицаСерииНоменклатуры
	|ИЗ
	|	ВременнаяТаблицаСерииНоменклатурыДоКорректировки КАК ВременнаяТаблицаСерииНоменклатурыДоКорректировки
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСерииНоменклатурыПослеКорректировки КАК ВременнаяТаблицаСерииНоменклатурыПослеКорректировки
	|		ПО ВременнаяТаблицаСерииНоменклатурыДоКорректировки.КлючСвязи = ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.КлючСвязи
	|			И ВременнаяТаблицаСерииНоменклатурыДоКорректировки.Серия = ВременнаяТаблицаСерииНоменклатурыПослеКорректировки.Серия
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СведенияПрослеживаемостиДокументов.СтранаПроисхождения КАК СтранаПроисхождения,
	|	СведенияПрослеживаемостиДокументов.Номенклатура КАК Комплектующие,
	|	СведенияПрослеживаемостиДокументов.Сумма КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблицаСведенияПрослеживаемости
	|ИЗ
	|	(ВЫБРАТЬ
	|		КорректировкаПоступленияСведенияПрослеживаемости.НомерСтроки КАК НомерСтроки,
	|		КорректировкаПоступленияСведенияПрослеживаемости.РНПТ КАК РНПТ,
	|		КорректировкаПоступленияСведенияПрослеживаемости.Количество КАК Количество,
	|		КорректировкаПоступленияСведенияПрослеживаемости.КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|		КорректировкаПоступленияСведенияПрослеживаемости.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|		ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|		КорректировкаПоступленияСведенияПрослеживаемости.СтранаПроисхождения КАК СтранаПроисхождения,
	|		КорректировкаПоступленияСведенияПрослеживаемости.Номенклатура КАК Номенклатура,
	|		КорректировкаПоступленияСведенияПрослеживаемости.Сумма КАК Сумма
	|	ИЗ
	...
	|	ВЫБРАТЬ
	|		0,
	|		ПриходнаяНакладнаяСведенияПрослеживаемости.РНПТ,
	|		-ПриходнаяНакладнаяСведенияПрослеживаемости.Количество,
	|		-ПриходнаяНакладнаяСведенияПрослеживаемости.КоличествоПрослеживаемости,
	|		ПриходнаяНакладнаяСведенияПрослеживаемости.ИдентификаторСтроки,
	|		ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
	|		ПриходнаяНакладнаяСведенияПрослеживаемости.СтранаПроисхождения,
	|		ПриходнаяНакладнаяСведенияПрослеживаемости.Номенклатура,
	|		-ПриходнаяНакладнаяСведенияПрослеживаемости.Сумма
	|	ИЗ
	|		Документ.ПриходнаяНакладная.СведенияПрослеживаемости КАК ПриходнаяНакладнаяСведенияПрослеживаемости
	|	ГДЕ
	|		ПриходнаяНакладнаяСведенияПрослеживаемости.Ссылка = &ИсправляемыйДокумент
	|		И &ВестиУчетПрослеживаемыхТоваров
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|		ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
	|		КорректировкаПоступленияСведенияПрослеживаемости.СтранаПроисхождения,
	|		КорректировкаПоступленияСведенияПрослеживаемости.Номенклатура,
	|		-КорректировкаПоступленияСведенияПрослеживаемости.Сумма
	|	ИЗ
	|		Документ.КорректировкаПоступления.СведенияПрослеживаемости КАК КорректировкаПоступленияСведенияПрослеживаемости
	|	ГДЕ
	|		КорректировкаПоступленияСведенияПрослеживаемости.Ссылка = &ИсправляемыйДокумент
	|		И &ВестиУчетПрослеживаемыхТоваров
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		0,
	|		АктОРасхожденияхСведенияПрослеживаемости.РНПТ,
	|		-АктОРасхожденияхСведенияПрослеживаемости.Количество,
	|		-АктОРасхожденияхСведенияПрослеживаемости.КоличествоПрослеживаемости,
	|		АктОРасхожденияхСведенияПрослеживаемости.ИдентификаторСтроки,
	|		ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
	|		АктОРасхожденияхСведенияПрослеживаемости.СтранаПроисхождения,
	|		АктОРасхожденияхСведенияПрослеживаемости.Номенклатура,
	|		-АктОРасхожденияхСведенияПрослеживаемости.Сумма
	|	ИЗ
	|		Документ.АктОРасхождениях.СведенияПрослеживаемости КАК АктОРасхожденияхСведенияПрослеживаемости
	|	ГДЕ
	|		АктОРасхожденияхСведенияПрослеживаемости.Ссылка = &ИсправляемыйДокумент
	...
	|	СведенияПрослеживаемостиДокументов.СтранаПроисхождения,
	|	СведенияПрослеживаемостиДокументов.Номенклатура,
	|	СведенияПрослеживаемостиДокументов.Сумма";
	...
	Запрос.УстановитьПараметр("ИсправляемыйДокумент", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаКорректировкаПоступления, "ДокументОснование", Истина));
	...
	// Серии
	СформироватьТаблицаСерииНоменклатуры(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства);
	СформироватьТаблицаСерииНоменклатурыКПоступлению(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства);
	
	// Прослеживаемость
	ПрослеживаемостьУНФ.СформироватьДвиженияПрослеживаемыхТоваров(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства);
	...
	// Взаиморасчеты
	СформироватьТаблицаОплатаДокументов(ДокументСсылкаКорректировкаПоступления, СтруктураДополнительныеСвойства);
...
Процедура ВыполнитьКонтроль(ДокументСсылкаКорректировкаПоступления, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
		...
		Если НЕ МассивРезультатов[4].Пустой() И Не ДополнительныеСвойства.ДляПроведения.ОсвобождатьАванс Тогда
		...
		Если НЕ МассивРезультатов[10].Пустой() Тогда
			...
			КонтрольОстатковУНФ.СерииНоменклатурыКПоступлению(ДокументОбъектПриходнаяНакладная, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Процедура ЗарегистрироватьЦеныПоставщика(ДокументСсылкаКорректировкаПоступления) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаЗаписей Цикл
		...
		Запись.ВидЦенКонтрагента = ДокументСсылкаКорректировкаПоступления.ВидЦенКонтрагента;
		Запись.Период = ДокументСсылкаКорректировкаПоступления.Дата;
		Запись.Номенклатура = СтрокаТаблицы.Номенклатура;
		Запись.Характеристика = СтрокаТаблицы.Характеристика;
		Запись.Цена = СтрокаТаблицы.Цена;
		Запись.Автор = СтрокаТаблицы.Автор;
		Запись.ЕдиницаИзмерения = СтрокаТаблицы.ЕдиницаИзмерения;
		Запись.ДокументРегистратор = СтрокаТаблицы.ДокументРегистратор;
...
Функция ДанныеПолученныхДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|ПОМЕСТИТЬ ДанныеСчетФаткурыОснования
	|ИЗ
	|	Документ.КорректировкаПоступления КАК КорректировкаПоступления
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученный.ДокументыОснования КАК СчетФактураПолученныйДокументыОснования
	|		ПО КорректировкаПоступления.ДокументОснование = СчетФактураПолученныйДокументыОснования.ДокументОснование
	|ГДЕ
	...
	|	ЕСТЬNULL(ДанныеСчетФаткурыОснования.Номер, НЕОПРЕДЕЛЕНО) КАК НомерИсправления,
	|	ЕСТЬNULL(ДанныеСчетФаткурыОснования.Дата, НЕОПРЕДЕЛЕНО) КАК ДатаИсправления,
	...
	|				И НЕ КорректировкаПоступления.Организация = КорректировкаПоступления.Организация.ГоловнаяОрганизация
	...
	|	Документ.КорректировкаПоступления КАК КорректировкаПоступления
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеСчетФаткурыОснования КАК ДанныеСчетФаткурыОснования
	|		ПО КорректировкаПоступления.Ссылка = ДанныеСчетФаткурыОснования.Ссылка
	|ГДЕ
	|	КорректировкаПоступления.Ссылка В(&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	|	КорректировкаПоступленияЗапасы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияЗапасы.ПрослеживаемыйТовар
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ПрослеживаемыйТоварЧисло,
	|	КорректировкаПоступленияЗапасы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	КорректировкаПоступленияЗапасы.Ссылка.СуммаВключаетНДС КАК СуммаВключаетНДС,
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияЗапасы.КодТНВЭД = ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка)
	|			ТОГДА КорректировкаПоступленияЗапасы.Номенклатура.ТоварнаяНоменклатураВЭД
	|		ИНАЧЕ КорректировкаПоступленияЗапасы.КодТНВЭД
	|	КОНЕЦ КАК КодТНВЭД
	...
	|	КорректировкаПоступленияЗапасы.ПрослеживаемыйТоварЧисло КАК ПрослеживаемыйТоварЧисло,
	|	КорректировкаПоступленияЗапасы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	КорректировкаПоступленияЗапасы.КодТНВЭД.ЕдиницаИзмерения.Код КАК КодТНВЭДЕдиницаИзмеренияКод,
	|	КорректировкаПоступленияЗапасы.КодТНВЭД.ЕдиницаИзмерения.Наименование КАК КодТНВЭДЕдиницаИзмеренияНаименование
	|ИЗ
	...
	|			ТОГДА КорректировкаПоступленияСведенияПрослеживаемости.Количество * ВЫБОР
	|					КОГДА ВременнаяТаблица_КорректировкаПоступленияЗапасы.СуммаВключаетНДС
	|						ТОГДА ВЫРАЗИТЬ((ВременнаяТаблица_КорректировкаПоступленияЗапасы.Сумма - ВременнаяТаблица_КорректировкаПоступленияЗапасы.СуммаНДС) / ВременнаяТаблица_КорректировкаПоступленияЗапасы.Количество КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ВременнаяТаблица_КорректировкаПоступленияЗапасы.Цена
	|				КОНЕЦ
	|		ИНАЧЕ КорректировкаПоступленияСведенияПрослеживаемости.Сумма
	|	КОНЕЦ КАК СтоимостьБезНалога
	|ИЗ
	|	ВременнаяТаблица_КорректировкаПоступленияЗапасы КАК ВременнаяТаблица_КорректировкаПоступленияЗапасы
	...
	|		ПО ВременнаяТаблица_КорректировкаПоступленияЗапасы.Ссылка = КорректировкаПоступленияСведенияПрослеживаемости.Ссылка
	|			И ВременнаяТаблица_КорректировкаПоступленияЗапасы.ИдентификаторСтроки = КорректировкаПоступленияСведенияПрослеживаемости.ИдентификаторСтроки";
	
	МассивРезультатов = Запрос.ВыполнитьПакет();
	Результат = МассивРезультатов[1].Выгрузить();
	
	ТЗСтрокЗапасы = МассивРезультатов[3].Выгрузить();
	ТЗСтрокРасходы = МассивРезультатов[5].Выгрузить();   
	ТзСтрокСведенияПрослеживаемости = МассивРезультатов[6].Выгрузить();
...
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц) Экспорт
	...
	НомераТаблиц.Вставить("ВременнаяТаблицаКорректируемыйДокумент",					НомераТаблиц.Количество());
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СуммыДокументовРегламентированныйУчет.Регистратор КАК ИсправляемыйДокумент,
	|	СУММА(СуммыДокументовРегламентированныйУчет.Всего) КАК СуммаРегл
	|ПОМЕСТИТЬ ВременнаяТаблицаКорректируемыйДокумент
	|ИЗ
	|	РегистрСведений.СуммыДокументовРегламентированныйУчет КАК СуммыДокументовРегламентированныйУчет
	|ГДЕ
	|	СуммыДокументовРегламентированныйУчет.Активность
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВЫБОР
	|		КОГДА НЕ ВременнаяТаблицаКорректируемыйДокумент.СуммаРегл ЕСТЬ NULL
	|			ТОГДА ВременнаяТаблицаКорректируемыйДокумент.СуммаРегл / Реквизиты.ИсправляемыйДокументПоступления.СуммаДокумента
	...
	|		ПО (ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКорректируемыйДокумент КАК ВременнаяТаблицаКорректируемыйДокумент
	|		ПО Реквизиты.ИсправляемыйДокументПоступления = ВременнаяТаблицаКорректируемыйДокумент.ИсправляемыйДокумент
	|ГДЕ
	...
	|	КорректировкаПоступленияЗапасы.Номенклатура.Штрихкод КАК ТоварШтрихкод,
	...
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияЗапасы.СтавкаНДС.Ставка > 0
	|			ТОГДА КорректировкаПоступленияЗапасы.Сумма / (100 + ВЫБОР
	|				КОГДА Реквизиты.СуммаВключаетНДС
	|					ТОГДА КорректировкаПоступленияЗапасы.СтавкаНДС.Ставка
	|				ИНАЧЕ 0
	|			КОНЕЦ) * КорректировкаПоступленияЗапасы.СтавкаНДС.Ставка * ВЫБОР
	|					КОГДА Реквизиты.ИспользоватьПересчет
	|						ТОГДА Реквизиты.Курс / Реквизиты.Кратность
	|					ИНАЧЕ 1
	|				КОНЕЦ
	...
	|	ВЫБОР
	|		КОГДА Реквизиты.СуммаВключаетНДС
	|			ТОГДА КорректировкаПоступленияЗапасы.Сумма - ВЫБОР
	|					КОГДА КорректировкаПоступленияЗапасы.СтавкаНДС.Ставка > 0
	|						ТОГДА КорректировкаПоступленияЗапасы.Сумма / (100 + ВЫБОР
	|				КОГДА Реквизиты.СуммаВключаетНДС
	|					ТОГДА КорректировкаПоступленияЗапасы.СтавкаНДС.Ставка
	|				ИНАЧЕ 0
	|			КОНЕЦ) * КорректировкаПоступленияЗапасы.СтавкаНДС.Ставка
	|					ИНАЧЕ 0
	|				КОНЕЦ
	|		ИНАЧЕ КорректировкаПоступленияЗапасы.Сумма
	|	КОНЕЦ * ВЫБОР
	...
	|	КорректировкаПоступленияРасходы.Номенклатура.Штрихкод,
	...
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияРасходы.СтавкаНДС.Ставка > 0
	|			ТОГДА КорректировкаПоступленияРасходы.Сумма / (100 + ВЫБОР
	|				КОГДА Реквизиты.СуммаВключаетНДС
	|					ТОГДА КорректировкаПоступленияРасходы.СтавкаНДС.Ставка
	|				ИНАЧЕ 0
	|			КОНЕЦ) * КорректировкаПоступленияРасходы.СтавкаНДС.Ставка * ВЫБОР
	|					КОГДА Реквизиты.ИспользоватьПересчет
	|						ТОГДА Реквизиты.Курс / Реквизиты.Кратность
	|					ИНАЧЕ 1
	|				КОНЕЦ
	...
	|	ВЫБОР
	|		КОГДА Реквизиты.СуммаВключаетНДС
	|			ТОГДА КорректировкаПоступленияРасходы.Сумма - ВЫБОР
	|					КОГДА КорректировкаПоступленияРасходы.СтавкаНДС.Ставка > 0
	|						ТОГДА КорректировкаПоступленияРасходы.Сумма / (100 + ВЫБОР
	|				КОГДА Реквизиты.СуммаВключаетНДС
	|					ТОГДА КорректировкаПоступленияРасходы.СтавкаНДС.Ставка
	|				ИНАЧЕ 0
	|			КОНЕЦ) * КорректировкаПоступленияРасходы.СтавкаНДС.Ставка
	|					ИНАЧЕ 0
	|				КОНЕЦ
	|		ИНАЧЕ КорректировкаПоступленияРасходы.Сумма
	|	КОНЕЦ * ВЫБОР
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаПоступления.СведенияПрослеживаемости КАК КорректировкаПоступленияСведенияПрослеживаемости
	|		ПО ТаблицаДокумента.Ссылка = КорректировкаПоступленияСведенияПрослеживаемости.Ссылка
	|			И ТаблицаДокумента.ИдентификаторСтроки = КорректировкаПоступленияСведенияПрослеживаемости.ИдентификаторСтроки
	|ГДЕ
	|	КорректировкаПоступленияСведенияПрослеживаемости.Ссылка = &ДокументОснование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			И ТаблицаДокумента.ИдентификаторСтроки = КорректировкаПоступленияСведенияПрослеживаемости.ИдентификаторСтроки
	|ГДЕ
	|	КорректировкаПоступленияСведенияПрослеживаемости.Ссылка = &ИсправляемыйДокументРеализации
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПриходнаяНакладнаяСведенияПрослеживаемости.Ссылка,
	|	ПриходнаяНакладнаяСведенияПрослеживаемости.НомерСтроки,
	|	ПриходнаяНакладнаяСведенияПрослеживаемости.ИдентификаторСтроки,
	|	ПриходнаяНакладнаяСведенияПрослеживаемости.РНПТ,
	|	ПриходнаяНакладнаяСведенияПрослеживаемости.Количество,
	|	ПриходнаяНакладнаяСведенияПрослеживаемости.КоличествоПрослеживаемости,
	...
	|		ПО ПриходнаяНакладнаяЗапасы.Ссылка = ПриходнаяНакладнаяСведенияПрослеживаемости.Ссылка
	|			И (ПриходнаяНакладнаяСведенияПрослеживаемости.ИдентификаторСтроки = ПриходнаяНакладнаяЗапасы.ИдентификаторСтроки)
	|ГДЕ
	|	ПриходнаяНакладнаяСведенияПрослеживаемости.Ссылка = &ИсправляемыйДокументРеализации";
