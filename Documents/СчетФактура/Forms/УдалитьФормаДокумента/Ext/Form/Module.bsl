...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если КонтрагентПередИзменением <> Объект.Контрагент Тогда
		...
		СтруктураДанные = ДанныеКонтрагентаДляИзменения(Объект.Контрагент);
...
Процедура ПерейтиКСпискуНажатие(Элемент)
	...
	ПараметрыПодбора.Вставить("АдресОснований", АдресДокументыОснованияВХранилище);
	...
	ПараметрыПодбора.Вставить("ДопустимыеТипы", ДопустимыеТипыДокументОснование);
	ПараметрыПодбора.Вставить("ТолькоПросмотрСпискаОснований", Истина);
...
Процедура РедактироватьЗачетПредоплаты(Команда)
	...
	ПараметрыПодбора.Вставить("Подбор", Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСчетФактура.Продажа"));
	...
	ПараметрыПодбора.Вставить("СуммаДокумента", Объект.УдалитьЗапасы.Итог("Всего") + СтоимостьДоставкиСНДС);
...
Процедура ОбработатьИзменениеДоговора()
	...
	Если ДоговорПередИзменением <> Объект.Договор Тогда
		...
		Если ЗначениеЗаполнено(Объект.Договор)
			И ЗначениеЗаполнено(ДанныеДоговора.ВалютаРасчетов)
...
Процедура ОбработатьИзмененияПоКнопкеЦеныИВалюты(Знач ВалютаДокументаПередИзменением, ПересчитатьЦены = Ложь, ТекстПредупреждения = "")
	...
	ОткрытьФорму("ОбщаяФорма.ЦеныИВалюта", СтруктураПараметров, ЭтотОбъект, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
