	...
	Для каждого ТекСтрока Из ДокументыОснования Цикл
		...
		Если ТипЗнч(ТекСтрока.ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			...
			Если ТипЗнч(РеквизитыОснования.ДокументОснование) = Тип("ДокументСсылка.АктОРасхожденияхПолученный") Тогда
				РеквизитыАктаОРасхождениях = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитыОснования.ДокументОснование, "ДокументОснование");
			...
			Если СчетаФактурыУНФ.ПолучитьПодчиненныйСчетФактуру(РеквизитыОснования.ДокументОснование) = Неопределено Тогда
				...
				ШаблонТекст = НСтр("ru = 'Перед добавлением %1 счета-фактуры создайте счет-фактуру у исправляемого документа'");
...
Процедура ЗаполнитьСведенияШапкиПоПоступлению(ДанныеЗаполнения, СведенияДокументовОснований)
	...
	ДокументРеализацииСсылка = ДанныеЗаполнения.ДокументОснование;
	Если НЕ ПустаяСтрока(ДокументРеализацииСсылка.ОснованиеПечати) Тогда
	...
	РеквизитыКорректировки = Документы.СчетФактура.ПолучитьПараметрыЗаполненияКорректировочногоСчетаФактурыВозвратОтПокупателя(ДанныеЗаполнения);
	Для каждого ЭлементСтруктуры Из РеквизитыКорректировки Цикл
...
Процедура ЗаполнитьСведения(ДанныеЗаполнения, СведенияДокументовОснований, СуммыДокументовОснований)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриходнаяНакладная") 
		И ДанныеЗаполнения.ПокупателюВыставляетсяКорректировочныйСчетФактура Тогда
...
Процедура ВыполнитьОбработкуЗаполнения()
	...
	Для каждого СтрокаТаблицы Из ДокументыОснования Цикл
		...
		Если ТипЗнч(СтрокаТаблицы.ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации")
			ИЛИ (ТипЗнч(СтрокаТаблицы.ДокументОснование) = Тип("ДокументСсылка.ПриходнаяНакладная") И СтрокаТаблицы.ДокументОснование.ПокупателюВыставляетсяКорректировочныйСчетФактура) Тогда
