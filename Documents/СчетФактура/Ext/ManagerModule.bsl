...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.СчетФактура.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.СчетФактура);
...
Функция ПолучитьПараметрыЗаполненияКорректировочногоСчетаФактуры(ДокументОснование) Экспорт
	...
	Если РеквизитыИсходногоСчетаФактуры.Следующий() Тогда
		...
		Иначе
			...
			РеквизитыИсходногоСчетаФактуры.Дата,
			РеквизитыИсходногоСчетаФактуры.Номер,
			"");
...
Функция ПолучитьДанныеДляПечатиКорректировочныхСчетовФактур(МассивОбъектов, ТекстЗапросаПоСчетамФактурам, ДополнитьДаннымиУПД = Ложь) Экспорт
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		Пока ВыборкаПоОснованиям.Следующий() Цикл
			...
			Если ТипЗнч(ВыборкаПоОснованиям.ДокументОснование) = Тип("ДокументСсылка.ОтчетКомиссионера") И ДополнитьДаннымиУПД Тогда
		...
		ВерсияПостановленияНДС1137 = РегламентированнаяОтчетностьПереопределяемый.ВерсияПостановленияНДС1137(ВыборкаСФ.Дата);
		...
		СворачиватьСтроки = ВерсияПостановленияНДС1137 < 5
		ИЛИ (ВерсияПостановленияНДС1137 >= 5 И ДокументыОснования.Количество() > 1 И КорректируетсяОдинСФ);
		...
		+"НомерГТД,"
		+"НаименованиеНоменклатуры,"  + ?(ЕстьСодержание, "Содержание,", "")
		...
		+"ИдентификаторСтроки,"
		+"Ссылка,"
		+"Акциз" + ?(ЕстьНаборы, ",НоменклатураНабора", "");
		...
		Если ДополнитьДаннымиУПД Тогда
			...
			Для каждого Основание Из ДокументыОснования Цикл
				Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПриходнаяНакладная")
					И Основание.ПокупателюВыставляетсяКорректировочныйСчетФактура Тогда
					СсылкаНаСчетФактуру = СчетаФактурыУНФ.ПолучитьПодчиненныйСчетФактуру(Основание.ДокументОснование);
					СсылкаНаСчетФактуру = ?(ЗначениеЗаполнено(СсылкаНаСчетФактуру), СсылкаНаСчетФактуру, Основание.ДокументОснование);
				Иначе
				...
				СсылкаНаСчетФактуру.Дата,
				СсылкаНаСчетФактуру.Номер,
				"");
...
Процедура ВывестиКорректировочныйСчетФактуруВТабличныйДокумент(ОписаниеПечатнойФормы, ДанныеДокумента, ЭтоУниверсальныйКорректировочныйДокумент = Ложь) Экспорт
	...
	Если ЭтоУниверсальныйКорректировочныйДокумент Тогда 
		НомерПостановления1137 = ПечатьДокументовУНФ.ВерсияПостановленияНДС1137(ДанныеШапки.Дата, ВыборкаСФ.ЕстьПрослеживаемыеТовары);
	Иначе
		НомерПостановления1137 = ПечатьДокументовУНФ.ВерсияПостановленияНДС1137(ВыборкаСФ.ДатаДокумента, ВыборкаСФ.ЕстьПрослеживаемыеТовары);
	КонецЕсли;
	...
	Если ЭтоУниверсальныйКорректировочныйДокумент Тогда
		...
		Если НомерПостановления1137 = "2023Кв4" Тогда 
			
			ПолныйПутьКМакетуПечатнойФормы = ПолныйПутьКМакетуПечатнойФормы + "2023Кв4";
	...
	Иначе
		...
		Если НомерПостановления1137 = "2023Кв4"
			И ВыборкаСФ.ЕстьПрослеживаемыеТовары Тогда
		    ПолныйПутьКМакетуПечатнойФормы = "Обработка.ПечатьСчетФактура.ПФ_MXL_СчетФактураКорректировочный2023Кв4";
		ИначеЕсли НомерПостановления1137 = "534" Тогда
			ПолныйПутьКМакетуПечатнойФормы = "Обработка.ПечатьСчетФактура.ПФ_MXL_СчетФактураКорректировочный534";
		ИначеЕсли ДанныеШапки.Дата >= НачалоИспользованияСФ981 Тогда
	...
	Если НомерПостановления1137 = "534"
		ИЛИ НомерПостановления1137 = "2023Кв4" Тогда
	...
	// Прослеживаемость
	ВыводитьРНПТ = (НомерПостановления1137 = "534" ИЛИ НомерПостановления1137 = "2023Кв4")
	...
	Если НЕ Макет.Области.Найти("РеквизитыКомиссионера") = Неопределено Тогда
		...
		Для Каждого СтрокаКомиссионер Из ВыборкаСФ.ТаблицаПокупатели Цикл
			ДанныеКомиссионера = ДанныеШапки.ИННПокупателя + ?(ЗначениеЗаполнено(ДанныеШапки.КПППокупателя), "/" + ДанныеШапки.КПППокупателя, "");
			ПредставлениеСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выставлен комиссионеру %1 %2, по корректировочному счету-фактуре №%3 от %4'")
			, ДанныеШапки.НаименованиеПокупателя, ДанныеКомиссионера, СтрокаКомиссионер.НомерСФ, СтрокаКомиссионер.ДатаСФ );
...
Функция ВывестиКорректировочныйСчетФактуруСРНПТВТабличныйДокумент(ДанныеДляПечати, ТабДокумент, НомерЛиста)
	...
	Для каждого СтрокаДокумента Из ТаблицаДокумента Цикл
		...
		Если ТипЗнч(СтрокаДокумента.Товар) = Тип("СправочникСсылка.Номенклатура") Тогда
			...
			СоответствиеРеквизитовОбъекту = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивТоваров, "ЭтоНабор,ВариантПечатиНабора");
		...
		Если ЗначениеЗаполнено(СтрокаДокумента.СведенияОПрослеживаемостиДоИзменения) Тогда
			...
			Если НомерПостановления1137 = "2023Кв4" Тогда
				СтоимПрослежДоИзменения = ТаблицаРНПТ.Итог("СтоимостьБезНалога"); 
			КонецЕсли;
			ПерваяСтрока = Истина;
		...
		Если ЗначениеЗаполнено(СтрокаДокумента.СведенияОПрослеживаемости) Тогда
			...
			Если НомерПостановления1137 = "2023Кв4" Тогда
				СтоимПрослежПослеИзменения = ТаблицаРНПТ.Итог("СтоимостьБезНалога"); 
			КонецЕсли;
			ПерваяСтрока = Истина;
			ОбластьЗапись.ГраницаСнизу  = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
			ОбластьЗапись.ГраницаСверху = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
		...
		Если НомерПостановления1137 = "2023Кв4" Тогда
			СтоимПрослежДельта = СтоимПрослежДоИзменения - СтоимПрослежПослеИзменения; 
		КонецЕсли;
		...
		Если НомерПостановления1137 = "2023Кв4" Тогда
			Если СтоимПрослежДельта > 0 Тогда
				СтоимПрослежДельтаУвеличилось = Ложь;
				КоличествоПрослеживаемостиДельтаСтруктура.СтоимПрослежУменьшение = КоличествоПрослеживаемостиДельтаСтруктура.СтоимПрослежУменьшение + СтоимПрослежДельта;
			Иначе
				СтоимПрослежДельтаУвеличилось = Истина;
				СтоимПрослежДельта = -СтоимПрослежДельта;
				КоличествоПрослеживаемостиДельтаСтруктура.СтоимПрослежУвеличение = КоличествоПрослеживаемостиДельтаСтруктура.СтоимПрослежУвеличение + СтоимПрослежДельта;
			КонецЕсли; 
		КонецЕсли;
		...
		Если НомерПостановления1137 = "2023Кв4" Тогда
			Если СтоимПрослежДельтаУвеличилось Тогда
				ОбластьМакетаСтрокаУвеличение.Параметры.СтоимПрослежУвеличение = СтоимПрослежДельта;
			Иначе
				ОбластьМакетаСтрокаУвеличение.Параметры.КоличествоПрослежУвеличение = "";
			КонецЕсли; 
		КонецЕсли;
		...
		Если НомерПостановления1137 = "2023Кв4" Тогда
			Если НЕ СтоимПрослежДельтаУвеличилось Тогда
				ОбластьМакетаСтрокаУменьшение.Параметры.СтоимПрослежУменьшение = СтоимПрослежДельта;
			Иначе
...
Процедура ВывестиКорректировочныйСчетФактуруБезРНПТВТабличныйДокумент(ДанныеДляПечати, ТабДокумент, НомерЛиста)
	...
	Если ЭтоУниверсальныйКорректировочныйДокумент И (НомерПостановления1137 = "534" ИЛИ НомерПостановления1137 = "2023Кв4") Тогда
		ОбластьНабор = Макет.ПолучитьОбласть("СтрокаНабора");
	КонецЕсли;
	...
	Для каждого СтрокаДокумента Из ТаблицаДокумента Цикл
		...
		ОбластьМакета = ?(ЭтоУниверсальныйКорректировочныйДокумент И 
		(НомерПостановления1137 = "534" ИЛИ НомерПостановления1137 = "2023Кв4")
		И ЭтоНабор И ВыводитьИерархиюПоНаборам, ОбластьНабор, ОбластьСтрока);
...
Функция ПодготовитьДанныеДляПечатиКорректировочныхСчетовФактур(ДокументОснование, СчетФактура, ВидСчетаФактуры, ИспользуетсяПостановлениеНДС1137) Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаПоступления") Тогда
		КонтрагентРезидентТаможенногоСоюза = Справочники.Контрагенты.КонтрагентРезидентТаможенногоСоюза(ДокументОснование.Контрагент);
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
		ИсправляемыйДокументРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "ДокументОснование");
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
		...
		НайденныеСтроки = ДокументОснование.Покупатели.НайтиСтроки(ПараметрыПоиска);
...
Функция ПодготовитьДанныеШапкиКорректировочногоСчетаФактуры(ВыборкаСФ, Реквизиты, ДополнитьДаннымиУПД) Экспорт
	...
	Если ВерсияПостановления1137 = "981" Или ВерсияПостановления1137 = "534"
		Или ВерсияПостановления1137 = "2023Кв4" Тогда
