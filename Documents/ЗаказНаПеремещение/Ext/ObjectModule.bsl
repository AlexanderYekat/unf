...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСборкуЗаказовНаПеремещение") Тогда
		СборкаЗаказов.УстановитьСтатусСборкиЗаказа(ЭтотОбъект);
	КонецЕсли;
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДокументСсылкаЗаказПокупателя) Экспорт
	...
	Если ТипЗнч(ДокументСсылкаЗаказПокупателя) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		ПеренестиСерииНоменклатуры(ДокументСсылкаЗаказПокупателя.СерииНоменклатуры, ДокументСсылкаЗаказПокупателя.Запасы);
	КонецЕсли;
...
Процедура ЗаполнитьПоПриходномуОрдеру(ДанныеЗаполнения) Экспорт
	...
	Для Каждого СтрокаСерии Из ДанныеЗаполнения.СерииНоменклатуры Цикл
...
Процедура ПеренестиСерииНоменклатуры(ТаблицаСерииНоменклатурыЗаказа, ТаблицаЗапасыЗаказа)
	...
	Для Каждого СтрокаСерийЗаказа Из ТаблицаСерииНоменклатурыЗаказа Цикл
		...
		СерииНоменклатурыУНФКлиентСервер.ДополнитьСтруктуруДаннымиДляПолученияСерийНоменклатуры(ЭтотОбъект, СтруктураДанные);
