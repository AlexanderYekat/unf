...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	ЗаполнитьПризнакОрганизацияПлательщикНДС();
...
Процедура НовоеСостояниеЗаказовРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	НСтр("ru = 'Укажите состояния, в которые будут переведены полностью отгруженные заказы покупателей после проведения маршрутного листа.'"),
	"СостояниеОтгружен");
...
Процедура ЗаказыПринятьВРаботуПриИзменении(Элемент)
	...
	ОбновитьИтогиОбъемИВес(Заказы, ИтогВес, ИтогОбъем);
...
&НаКлиенте
Процедура СформироватьДокументыОтгрузкиСоСчетамиФактурами(Команда)

	ПараметрыСовета = Новый Структура;
	ПараметрыСовета.Вставить("Заголовок", НСтр("ru = 'Формирование документов отгрузки'"));
	ПараметрыСовета.Вставить("ТекстСообщения", НСтр("ru = 'Сейчас будут сформированы расходные накладные и счета-фактуры по всем заказам из маршрутного листа (если они еще не были созданы). Отгрузка будет произведена со склада, указанного в заказе.'"));
	ПараметрыСовета.Вставить("ВидимостьБольшеНеПоказывать", Истина);
	ПараметрыСовета.Вставить("ЗаголовокКнопкиОк", НСтр("ru = 'Продолжить'"));
	ПараметрыСовета.Вставить("ФормироватьДополнительно", "СФ");
	Если ФормаПараметры.НеПредупреждатьПередФормированием Тогда
		СтруктураВозврата = Новый Структура;
		СтруктураВозврата.Вставить("БольшеНеПоказывать", ФормаПараметры.НеПредупреждатьПередФормированием);
		СтруктураВозврата.Вставить("КодВозврата", КодВозвратаДиалога.ОК);
...
&НаКлиенте
Процедура СформироватьДокументыОтгрузкиСУПД(Команда)

	ПараметрыСовета = Новый Структура;
	ПараметрыСовета.Вставить("Заголовок", НСтр("ru = 'Формирование документов отгрузки'"));
	ПараметрыСовета.Вставить("ТекстСообщения", НСтр("ru = 'Сейчас будут сформированы расходные накладные и УПД по всем заказам из маршрутного листа (если они еще не были созданы). Отгрузка будет произведена со склада, указанного в заказе.'"));
	ПараметрыСовета.Вставить("ВидимостьБольшеНеПоказывать", Истина);
	ПараметрыСовета.Вставить("ЗаголовокКнопкиОк", НСтр("ru = 'Продолжить'"));
	ПараметрыСовета.Вставить("ФормироватьДополнительно", "УПД");
	Если ФормаПараметры.НеПредупреждатьПередФормированием Тогда
		СтруктураВозврата = Новый Структура;
		СтруктураВозврата.Вставить("БольшеНеПоказывать", ФормаПараметры.НеПредупреждатьПередФормированием);
		СтруктураВозврата.Вставить("КодВозврата", КодВозвратаДиалога.ОК);
...
Процедура СформироватьДокументыОтгрузкиПродолжение(Результат, ПараметрыСовета) Экспорт
	...
	ФормироватьДополнительно = Неопределено;
	ПараметрыСовета.Свойство("ФормироватьДополнительно", ФормироватьДополнительно);
	ЗапуститьФоновоеЗадание(ФормироватьДополнительно);
...
&НаКлиенте
Процедура ЗапуститьФоновоеЗадание(ФормироватьДополнительно)
	
	УстановитьДоступностьФормы(Ложь);
	
	Задание = ЗаданиеФормированиеДокументовОтгрузки(ФормироватьДополнительно);
...
&НаСервере
Функция ЗаданиеФормированиеДокументовОтгрузки(ФормироватьДополнительно)
	...
	Если ФормироватьДополнительно <> Неопределено Тогда
		ПараметрыЗадания.Вставить("ФормироватьДополнительно", ФормироватьДополнительно);  
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Для каждого СтрокаТабличнойЧасти Из Форма.Заказы Цикл
		...
		КонецЕсли;
	КонецЦикла; 
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормированиеДокументовОтгрузки", "Доступность", Объект.Проведен И ЕстьНеотгруженные);
...
&НаСервере
Процедура ЗаполнитьПризнакОрганизацияПлательщикНДС()      
	
	НалогообложениеНДС = НалогиУНФ.НалогообложениеНДС(Объект.Организация);
	ОрганизацияПлательщикНДС = НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС;   
	
	// Видимость команд формирования документов
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СформироватьДокументыОтгрузкиСоСчетамиФактурами", "Видимость", ОрганизацияПлательщикНДС);
