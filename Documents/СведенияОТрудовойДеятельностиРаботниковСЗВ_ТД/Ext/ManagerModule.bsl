...
Функция ВыборкаПоСотрудникам(МассивОбъектов)
	...
	Запрос.Текст = 
		...
		|	СведенияОТрудовойДеятельностиРаботниковСЗВ_ТДМероприятия.СрочночтьТрудовогоДоговора КАК СрочночтьТрудовогоДоговора,
...
Функция ДанныеДляПроведенияПоМероприятияТрудовойДеятельностиПереданные(СсылкаНаДокумент, ТолькоПроведенные = Ложь) Экспорт
	...
	Запрос.Текст =
		...
		|	ТаблицаДокумента.СрочночтьТрудовогоДоговора КАК СрочночтьТрудовогоДоговора,
		...
		|	ДанныеМероприятий.СрочночтьТрудовогоДоговора КАК СрочночтьТрудовогоДоговора,
...
Функция РуководительПоФИО(Фамилия, Имя, Отчество)
	...
	ПохожиеФизическиеЛица = СотрудникиФормы.ПодобратьСписокФизЛиц(ПараметрыПоиска);
