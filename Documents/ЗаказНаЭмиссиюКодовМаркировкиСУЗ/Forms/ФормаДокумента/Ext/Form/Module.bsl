...
Процедура ОбновитьВидимостьДоступностьЭлементовФормы(Форма)
	...
	Форма.ИспользованиеСервисПровайдераЗапрещено =
		ЭтоТабачнаяПродукция Или ЭтоКомиссия Или ЭтоМех;
...
// 
// Параметры:
//  РезультатАвторизации - Неопределено, (см. ИнтерфейсИСМПОбщегоНазначенияКлиент.ПараметрыЗавершенияОбработки) - Результат авторизации.
...
Процедура ОбработатьПолученныеДанныеТСД() Экспорт
	...
	Если ЗагрузкаДанныхТСД.Обработано = ЗагрузкаДанныхТСД.Всего Тогда
		ОбработатьДобавленныеГрупповыеУпаковки(ЗагрузкаДанныхТСД);
...
Процедура ЗаполнитьСоставляющиеНаборов(МассивСтрок, КэшированныеЗначения, ДобавленныеСтроки)
	...
	Для Каждого КомплектующаяНабора Из КомплектующиеНаборов Цикл
		...
		Если КомплектующаяНабора.НомерСтрокиНабора <> НомерСтрокиНабора Тогда
			...
			ТекущаяСтрока = ЭтотОбъект.Объект.Товары[НомерСтроки - 1];
		...
		ТекущаяСтрока = ЭтотОбъект.Объект.Товары.Вставить(НомерСтроки);
