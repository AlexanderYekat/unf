...
//
/// Параметры:
// 	СтруктураПараметров - Неопределено, Структура из КлючИЗначение - Параметры для расчета:
...
Функция ЗаявкаНаЭмиссиюКодовМаркировкиJSON(ДокументСсылка, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(Шапка.ВидПродукции) Тогда
		...
		МассивЗаказов = МассивЗаказовПоТаблицеТоваров(ТабличнаяЧастьТовары, МаксимальноеКоличествоКодовВЗаказе, Шапка.ВидПродукции);
		...
		Для Каждого СодержаниеЗаказа Из МассивЗаказов Цикл
			...
			Для Каждого СтрокаТЧ Из СодержаниеЗаказа Цикл
				...
				Иначе
					СтрокаЗаказаКодовМаркировки.Вставить("serialNumberType", ИнтерфейсСУЗСлужебный.СпособФормированияСерийногоНомера(СтрокаТЧ.СпособФормированияСерийногоНомера)); // OPERATOR или SELF_MADE
					Если Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.Антисептики
						Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы
						Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.МорепродуктыПодконтрольныеВЕТИС Тогда
						СтрокаЗаказаКодовМаркировки.Вставить("templateId", ИнтерфейсСУЗСлужебный.ШаблонКодаМаркировки(СтрокаТЧ.Шаблон));
					Иначе
				...
				Если ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииИспользуетНаборыИлиГрупповыеУпаковки(Шапка.ВидПродукции)
...
Функция МассивЗаказовПоТаблицеТоваров(ВходнаяТаблицаТовары, МаксимальноеКоличествоКодовПоСтроке, ВидПродукции)
