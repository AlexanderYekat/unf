...
Функция СформироватьЗапросДляТ6(МассивОбъектов)
	...
	Запрос.Текст =
		...
		|	ДанныеДокументов.Организация КАК РаботаОрганизация,
		|	ВЫРАЗИТЬ(ДанныеДокументов.Организация КАК Справочник.Организации).НаименованиеСокращенное КАК РаботаОрганизацияНаименованиеСокращенное,
		|	ДанныеДокументов.КодПоОКПО КАК РаботаОрганизацияКодПоОКПО,
...
Процедура ЗаполнитьНачисленияВДокументе(ДокументОбъект) Экспорт
	...
	Если ДокументОбъект.ДатаОкончания >= ДокументОбъект.ДатаНачала
		...
		Пока ДатаНачалаОтпуска <= ДокументОбъект.ДатаОкончания Цикл
			...
			РабочихДнейЧасов = РасчетЗарплатыДляНебольшихОрганизаций.РабочихДнейЧасовВПериодеОрганизации(
				СтрокаНачислений.ДатаНачала, СтрокаНачислений.ДатаОкончания, ДокументОбъект.Организация);
