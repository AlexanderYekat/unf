...
Процедура УстановитьВидимостьРеквизитовРасшифровкиПлатежа(Элементы, СпособОплаты, ВидОперации)
	...
	Иначе
		...
		Элементы.ОплатитьКартой.Видимость 				= ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЭквайринга.ПоступлениеОплатыОтПокупателя");
...
Процедура УдалитьОплатуКартой(Команда)
	...
	Если Объект.СпособОплаты = ПредопределенноеЗначение("Перечисление.СпособыОплатыЭквайринг.ОплатаСБП") Тогда
		...
		Доступность = Ложь;
		Оповещение  = Новый ОписаниеОповещения("ВозвратПоСБПЗавершение", ЭтотОбъект);
