...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	|	ИЛИ ЭквайринговыйТерминал = ЗНАЧЕНИЕ(Справочник.СпособыОплаты.ПустаяСсылка)
	|	) ";
...
Функция ПолучитьДанныеДляФискализации(Ссылка) Экспорт
	...
	ОбщиеПараметры.ИдентификаторФискальнойЗаписи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		Ссылка, 
		"ИдентификаторЧекаВОчереди");

	Если ПустаяСтрока(ОбщиеПараметры.ИдентификаторФискальнойЗаписи) Тогда
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.ОперацияПоПлатежнымКартам.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.ОперацияПоПлатежнымКартам);
...
Процедура СформироватьТаблицаОплатаДокументов(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	МассивДоступныхОпераций.Добавить(Перечисления.ВидыОперацийЭквайринга.ВозвратОплатыПокупателю);
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаОперацияПоПлатежнымКартам, СтруктураДополнительныеСвойства)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	ЭквайринговыйТерминал = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ДокументСсылкаОперацияПоПлатежнымКартам, "ЭквайринговыйТерминал");
	СчетУчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭквайринговыйТерминал, "СчетУчета");
	Валютный = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетУчета, "Валютный");
...
Процедура СформироватьРасчетыПоЭквайрингу(ДокументСсылкаОперацияПоПлатежнымКартам, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СодержаниеПроводкиВозвратСБП", НСтр("ru = 'Возврат СБП'"));
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаОперацияПоПлатежнымКартам);
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВременнаяТаблицаРасшифровкаПлатежа.ЭквайринговыйТерминал.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыСпособовОплат.СБП), ЛОЖЬ)
	|			ТОГДА ВЫБОР
	...
	|						ТОГДА &СодержаниеПроводкиВозвратСБП
	|					ИНАЧЕ &СодержаниеПроводкиОплатаСБП
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ВременнаяТаблицаРасшифровкаПлатежа.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЭквайринга.ВозвратОплатыПокупателю)
	...
	// Оплата не зачитывает операции возврата, значит можем сразу добавить движения в таблицу. Операции возврата
	// потребуется распределять по остаткам расчетов в документе "ПоступлениеНаСчет" или "РасходСоСчета".
...
Процедура СформироватьТаблицаОплатаПлатежнымиКартами(ДокументСсылкаОперацияПоПлатежнымКартам, СтруктураДополнительныеСвойства)
	...
	|	И ДанныеДокумента.СпособОплаты <> ЗНАЧЕНИЕ(Перечисление.СпособыОплатыЭквайринг.ОплатаСБП)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаОплатаСБП(ДокументСсылкаОперацияПоПлатежнымКартам, СтруктураДополнительныеСвойства)
	...
	"ВЫБРАТЬ
	|	ДанныеДокумента.Дата КАК Период,
	|	ДанныеДокумента.ЭквайринговыйТерминал КАК СпособОплаты,
	|	&Организация КАК Организация,
	|	ТаблицаРасшифровкаПлатежа.СуммаУчета КАК Сумма,
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОперацияПоПлатежнымКартам, СтруктураДополнительныеСвойства) Экспорт
	...
	СформироватьТаблицаОплатаСБП(ДокументСсылкаОперацияПоПлатежнымКартам, СтруктураДополнительныеСвойства);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПКО") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПКО", НСтр("ru = 'Приходный кассовый ордер'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати));
