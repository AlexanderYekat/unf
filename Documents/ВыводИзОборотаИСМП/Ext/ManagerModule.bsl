...
Функция СтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаРозничнаяПродажа
		...
		Или Операция = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаРеализацияНезарегистрированномуУчастнику Тогда
...
Функция ВыводИзОборотаJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	Если Шапка.ОбъемноСортовойУчет Тогда
		...
		Для Каждого СтрокаТовары Из Товары Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Товары.Колонки.Добавить("КоличествоСтрок", ОбщегоНазначения.ОписаниеТипаЧисло(5));
		Товары.ЗаполнитьЗначения(1, "КоличествоСтрок");
		Товары.Свернуть(
			"GTIN",
			"Цена, Количество, КоличествоУпаковок, КоличествоПотребительскихУпаковок, Сумма, КоличествоСтрок");
		Для Каждого СтрокаТовары Из Товары Цикл
			Если СтрокаТовары.КоличествоСтрок > 1 Тогда
				СтрокаТовары.Цена = Окр(СтрокаТовары.Цена / СтрокаТовары.КоличествоСтрок, 2);
			КонецЕсли;
		КонецЦикла;
	...
	Для Каждого СтрокаТЧМаркированныеТовары Из ДанныеПоТоварам Цикл
		...
		Если НайденныеСтроки.Количество() = 1 Тогда
			...
			Если Шапка.ОбъемноСортовойУчет Тогда
				СтрокаТЧ.Вставить("gtin", СтрокаТЧМаркированныеТовары.GTIN);
				Если ЗначениеЗаполнено(СтрокаТЧМаркированныеТовары.КоличествоПотребительскихУпаковок) Тогда
					СтрокаТЧ.Вставить("gtin_quantity", СтрокаТЧМаркированныеТовары.КоличествоПотребительскихУпаковок);
				Иначе
					СтрокаТЧ.Вставить("gtin_quantity", СтрокаТЧМаркированныеТовары.КоличествоУпаковок);
				КонецЕсли;
				НоменклатураПоGTIN = ВесоваяНоменклатураПоGTIN[СтрокаТЧМаркированныеТовары.GTIN];
				Если НоменклатураПоGTIN <> Неопределено Тогда
					Вес = ВесВГраммахПоНоменклатуре(СтрокаТЧМаркированныеТовары.Количество, НоменклатураПоGTIN);
			...
			Если ИспользованиеЦены Тогда
				...
				ИначеЕсли ЗначениеЗаполнено(СтрокаТЧТовары.Сумма) Тогда
					Если ЗначениеЗаполнено(СтрокаТЧТовары.КоличествоПотребительскихУпаковок)
