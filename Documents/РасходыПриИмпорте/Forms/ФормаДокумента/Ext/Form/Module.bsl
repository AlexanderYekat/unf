...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗапасыДокументПартии", "Видимость",
		ОбменСБухгалтериейНастроен);
...
Процедура ЗапасыНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество)
	...
	СтрокаЗапасы.Количество			= НовоеКоличество;
	...
	СтрокаЗапасы.ЗапретитьВводДробногоКоличества = СтруктураДанные.ЗапретитьВводДробногоКоличества;
...
Процедура ОбработатьИзмененияПоКнопкеЦеныИВалюты(Знач ВалютаРасчетовПередИзменением, ПересчитатьЦены = Ложь,
	...
	ПараметрыФормыЦеныИВалюта = ЦеныИВалютаКлиентСервер.НовыеПараметрыФормыЦеныИВалюта(Объект, ТолькоПросмотр,
		Компания, Ложь, ПересчитатьЦены, ТекстПредупреждения);
...
Процедура ОткрытьФормуЦеныИВалютаЗавершение(РезультатФормыЦеныИВалюта, ДополнительныеПараметры) Экспорт
	...
	Если РезультатФормыЦеныИВалюта.ПересчитатьЦены Тогда
		...
		ЦенообразованиеКлиент.ПересчитатьЦеныТабличнойЧастиПоВалюте(ЭтотОбъект, ВалютаРасчетовПередИзменением,
			"Разделы", РезультатФормыЦеныИВалюта.КурсПересчетаЦен);
	...
	Если РезультатФормыЦеныИВалюта.СуммаВключаетНДС <> РезультатФормыЦеныИВалюта.ПредСуммаВключаетНДС Тогда
		...
		ЦенообразованиеКлиент.ПересчитаемСуммуТабличнойЧастиПоФлагуСуммаВключаетНДС(ЭтотОбъект, "Разделы");
...
Функция РассчитатьСуммуПошлиныГТД(Знач ДанныеСтроки, Знач ДанныеОбъекта, Знач ВалютаРеглУчета)
	...
	Иначе
		...
		ТаможеннаяСтоимостьВал = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(ДанныеСтроки.ТаможеннаяСтоимость,
...
Процедура ПриИзмененииСтоимостиСтавкиПошлины(НомерРаздела)
	...
	ДанныеСтроки.Вставить("ТаможеннаяСтоимость");
	ДанныеСтроки.Вставить("СтавкаПошлины");
	ДанныеСтроки.Вставить("СуммаПошлины");
...
Процедура РаспределитьСуммуСбораПоЗапасам()
	...
	РаспределитьСуммуТаможенногоСбораПоКолонкамТаблицыЗапасов(Объект.ТаможенныйСбор, Объект.Запасы, "СуммаСбора",
		"ФактурнаяСтоимость");
...
Функция ДанныеКонтрагентаДляИзменения(Знач Дата, Знач Контрагент, Знач Организация)
	...
	Результат.Вставить("СуммаВключаетНДС", Неопределено);
	Если ЗначениеЗаполнено(ДоговорПоУмолчанию.ВидЦен) Тогда
		Результат.СуммаВключаетНДС = Договор.ВидЦен.ЦенаВключаетНДС;
	КонецЕсли;
...
Процедура ОбработатьТаблицуНаСервере(ИдентификаторРаздела, ИзмененныеРазделы, Ошибки)
	...
	ИначеЕсли ЗапасыИзменениеСтрокДействие = Перечисления.ДействияГрупповогоИзмененияСтрок.УстановитьИсходноеМесто Тогда
		...
		ПомеченныеСтроки = Объект.Запасы.НайтиСтроки(Новый Структура("Пометка, НомерРаздела", Истина, СтрокаРаздела.НомерРаздела));
