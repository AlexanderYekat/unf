...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаОплатаСБП(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства)
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	// СБП
	СформироватьТаблицаОплатаСБП(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства);
	...
	СформироватьТаблицаОплатаДокументов(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства);
...
Функция ТекстЗапросаИнициализироватьДанныеДокумента()
	...
	|			ТОГДА ЧекККМВозврат.Ссылка
	|		ИНАЧЕ ЧекККМВозврат.ЧекККМ
	|	КОНЕЦ КАК Документ,
	|	ЧекККМВозврат.КассаККМ КАК КассаККМ,
	|	ЧекККМВозврат.КассаККМ.СчетУчета КАК КассаККМСчетУчета,
	|	ЧекККМВозврат.ВалютаДокумента КАК ВалютаДокумента,
	...
	|	ЧекККМВозврат.Контрагент.СчетУчетаРасчетовСПокупателем КАК СчетУчетаРасчетовСПокупателем
	...
	|	ВЫРАЗИТЬ(ЧекККМЗапасы.Сумма / ВЫБОР
	...
	|	ЧекККМЗапасы.Ссылка.Контрагент.СчетУчетаАвансовПокупателя КАК СчетУчетаАвансовПокупателя,
	...
	|	ЧекККМВозвратПредоплата.Ссылка.Контрагент.СчетУчетаАвансовПокупателя КАК СчетУчетаАвансовПокупателя,
	...
	|	ЧекККМВозвратПредоплата.Ссылка.Дата КАК Дата,
	|	ЧекККМВозвратПредоплата.Документ КАК Документ,
	|	ЧекККМВозвратПредоплата.Ссылка.Заказ КАК Заказ,
	|	ЧекККМВозвратПредоплата.Курс КАК Курс,
	|	ЧекККМВозвратПредоплата.Кратность КАК Кратность,
	|	ЧекККМВозвратПредоплата.Ссылка.КассаККМ КАК КассаККМ,
	|	ЧекККМВозвратПредоплата.Ссылка.КассаККМ.СчетУчета КАК КассаККМСчетУчета,
...
Процедура СформироватьТаблицаОплатаПлатежнымиКартами(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ЧекПробит", СтруктураДополнительныеСвойства.ДляПроведения.ЧекПробит);
	Запрос.УстановитьПараметр("Архивный", СтруктураДополнительныеСвойства.ДляПроведения.Архивный);
	Запрос.УстановитьПараметр("ККТОнлайн", ДокументСсылкаЧекККМВозврат.КассаККМ.ТипКассы = Перечисления.ТипыКассККМ.ФискальныйРегистратор);
...
Функция СформироватьПечатнуюФормуТоварногоЧека(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Товарный чек (возврат) № %1 от %2'"), НомерДокумента,
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КМ3") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "КМ3", НСтр("ru = 'КМ-3'"),СформироватьПечатнуюФормуКМ3(МассивОбъектов, ОбъектыПечати));
		
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТоварныйЧек") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ТоварныйЧек", НСтр("ru = 'Товарный чек'"), СформироватьПечатнуюФормуТоварногоЧека(МассивОбъектов, ОбъектыПечати));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаЧекККМВозврат, СтруктураДополнительныеСвойства)
	...
	|	&ЧекПробит
	|	И НЕ &Архивный
	|	И ТаблицаУправленческий.ВидОплаты В (ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.БанковскаяКарта), ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.Кредит), ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.СБП))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
