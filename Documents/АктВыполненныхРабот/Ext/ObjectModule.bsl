...
Процедура ЗаполнитьПоЗаказуПокупателя(ДокументСсылкаЗаказПокупателя) Экспорт
	...
	|	ЗаказПокупателя.СпециальныйНалоговыйРежим
	|ИЗ
	...
	Если ТаблицаОстатков.Количество() > 0 Тогда
		...
		Если ТаблицаОстатков.Итог("КоличествоОстаток") = 0 Тогда
			// Это последняя отгрузка по заказу, перенесем все рассчитанные подарки
			Подарки.Очистить();
			Подарки.Загрузить(МассивРезультатов[8].Выгрузить());
	...
	Если ИспользоватьАвтоматическиеСкидки Тогда
		...
		КонецЕсли;
		СкидкиНаценкиСервер.ПрименитьРассчитанныеСкидкиКОбъекту(ЭтотОбъект, РасчетныеСкидки, "РаботыИУслуги");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЕстьРучныеСкидки ИЛИ ЕстьАвтоматическиеСкидки ИЛИ ЕстьОплатаБонусами Тогда
		Для Каждого СтрокаРаботыИУслуги Из РаботыИУслуги Цикл
			...
			ТекСуммаОплатаБонусами = ?(ЕстьОплатаБонусами, СтрокаРаботыИУслуги.СуммаСкидкиОплатыБонусом, 0);
	...
	// Оплата бонусами разрешена
 	Если ЕстьОплатаБонусами Тогда
		СуммаОплатыБонусами = РаботыИУслуги.Итог("СуммаСкидкиОплатыБонусом");
