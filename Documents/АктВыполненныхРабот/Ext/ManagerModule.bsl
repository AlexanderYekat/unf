...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.АктВыполненныхРабот.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.АктВыполненныхРабот);
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуется > 0 Тогда
			...
			Если Окр(СуммаКСписанию, 2, 1) <> 0 Или Окр(СуммаБезНДСКСписанию, 2, 1) <> 0 Тогда
				...
				СтрокаТаблицаПродажи.СебестоимостьБезНДС = СуммаБезНДСКСписанию;
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаРаботыИУслуги КАК ТаблицаДоходыИРасходы
	|ГДЕ
	|	НЕ ТаблицаДоходыИРасходы.ЭтоАгентскаяУслуга
...
//
Процедура СформироватьТаблицаБонусныеБаллы(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
...
Процедура СформироватьТаблицаНачисленияБонусныхБаллов(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
...
Процедура СформироватьТаблицаЗапасыИАгентскиеУслугиПринятые(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаЗапасыИАгентскиеУслугиПринятые.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|			ТОГДА ТаблицаЗапасыИАгентскиеУслугиПринятые.ЗаказПокупателя
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Заказ,
	...
	|	СУММА(ТаблицаЗапасыИАгентскиеУслугиПринятые.Сумма) КАК СуммаРасчетов,
...
Процедура СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаРаботыИУслуги КАК ТаблицаДокумента
	|ГДЕ
	|	НЕ ТаблицаДокумента.ЭтоАгентскаяУслуга
...
Функция ПолучитьТекстЗапросаИнициализироватьДанныеДокумента()
	...
	|	ВЫБОР
	|		КОГДА АктВыполненныхРаботРаботыИУслуги.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.Продажи_НДС)
	|		ИНАЧЕ АктВыполненныхРаботРаботыИУслуги.Номенклатура.СчетУчетаНДСПоРеализации
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	АктВыполненныхРаботБонусныеБаллыКНачислению.НомерСтроки КАК НомерСтроки,
	|	АктВыполненныхРаботБонусныеБаллыКНачислению.Ссылка.Дата КАК Период,
	|	АктВыполненныхРаботБонусныеБаллыКНачислению.Ссылка.ДисконтнаяКарта КАК БонуснаяКарта,
	|	АктВыполненныхРаботРаботыИУслуги.Номенклатура КАК Номенклатура,
	|	АктВыполненныхРаботРаботыИУслуги.Характеристика КАК Характеристика,
	|	АктВыполненныхРаботБонусныеБаллыКНачислению.КоличествоБонусныхБаллов КАК Количество,
	|	АктВыполненныхРаботБонусныеБаллыКНачислению.ДатаНачисления КАК ДатаНачисления,
	|	АктВыполненныхРаботБонусныеБаллыКНачислению.ДатаСписания КАК ДатаСгорания,
	|	АктВыполненныхРаботБонусныеБаллыКНачислению.Ссылка КАК Ссылка,
	|	АктВыполненныхРаботБонусныеБаллыКНачислению.СкидкаНаценка КАК АналитикаНачисленияБонусов
	|ПОМЕСТИТЬ ВременнаяТаблицаНачисленияБонусов
	|ИЗ
	|	Документ.АктВыполненныхРабот.БонусныеБаллыКНачислению КАК АктВыполненныхРаботБонусныеБаллыКНачислению
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.АктВыполненныхРабот.РаботыИУслуги КАК АктВыполненныхРаботРаботыИУслуги
	|		ПО АктВыполненныхРаботБонусныеБаллыКНачислению.Ссылка = АктВыполненныхРаботРаботыИУслуги.Ссылка
	|			И АктВыполненныхРаботБонусныеБаллыКНачислению.КлючСвязи = АктВыполненныхРаботРаботыИУслуги.КлючСвязи
	|ГДЕ
	|	АктВыполненныхРаботБонусныеБаллыКНачислению.Ссылка = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	АктВыполненныхРаботПредоплата.НомерСтроки КАК НомерСтроки,
	|	НАЧАЛОПЕРИОДА(АктВыполненныхРаботПредоплата.Ссылка.Дата, ДЕНЬ) КАК Период,
	|	АктВыполненныхРаботПредоплата.Документ КАК БонуснаяКарта,
	|	АктВыполненныхРаботПредоплата.СуммаРасчетов КАК Количество,
	|	0 КАК КСписанию,
	|	АктВыполненныхРаботПредоплата.Ссылка КАК Ссылка
	...
	|	И АктВыполненныхРаботПредоплата.ОплатаБонусами = ИСТИНА
	|	И АктВыполненныхРаботПредоплата.Заказ = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	Документ.АктВыполненныхРабот.ПримененныеПромокоды КАК ВыданныйПромокод
	...
	|ИЗ
	|	Документ.АктВыполненныхРабот.ПримененныеПромокоды КАК ПредъявленныеПромокоды
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства) Экспорт
	...
	// Бонусы
	СформироватьТаблицаБонусныеБаллы(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства);
	СформироватьТаблицаНачисленияБонусныхБаллов(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства);
	// Промокоды
	СформироватьТаблицаПромокоды(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства); // Регистрация новых выданных промокодов
	СформироватьТаблицаПрименениеПромокодов(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства); // Гашение примененных промокодов
	СформироватьТаблицаДоступностьПромокодов(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства); // Оборот скидок, предоставленных по примененным промокодам
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|				И НЕ АктВыполненныхРабот.Организация = АктВыполненныхРабот.Организация.ГоловнаяОрганизация
	...
	|	АктВыполненныхРабот.ЭтоУниверсальныйДокумент КАК ЭтоУниверсальныйДокумент,
