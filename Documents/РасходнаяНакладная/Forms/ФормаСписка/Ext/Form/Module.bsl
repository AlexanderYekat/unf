...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Возврат", ТолькоВозвраты) Тогда
		...
		Элементы.ОтборКонтрагент.ПодсказкаВвода = НСтр("ru = 'Поставщик, ИНН, телефон'");
		Элементы.Контрагент.Заголовок = НСтр("ru = 'Поставщик / Покупатель'");
		
		Элементы.Страницы.ОтображениеСтраниц = ОтображениеСтраницФормы.Нет;
		Элементы.ГруппаСписок.ОтображатьЗаголовок = Ложь;
		...
		ЭтотОбъект.Заголовок = НСтр("ru = 'Расходные накладные: возвраты'");
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПодменюСоздатьВозвраты", "Видимость", Ложь);		
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СтраницаДокументыКОтгрузке", "Видимость", ПолучитьФункциональнуюОпцию("ИспользоватьОрдерныйСклад")); 
	
	СписокЗаказыПокупателей.Параметры.УстановитьЗначениеПараметра("ЗакладкаЗаказыОткрыта", Ложь);
	...
	Иначе
		РаботаСОтборами.ВосстановитьНастройкиОтборов(ЭтотОбъект, Список,,,,"Возвраты");
	КонецЕсли;
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		ЭтотОбъект,
		"УНФ.Документ.РасходнаяНакладная",
		"ФормаСписка",
		Неопределено,
		НСтр("ru='Новости: Расходные накладные'"),
		Ложь,
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СтраницаДокументыКОтгрузке", "Видимость", ИспользуемыеВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам);
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаЗаказыПокупателей Тогда
		ЗакладкаОткрыта(Истина);
	Иначе
		ЗакладкаОткрыта(Ложь);
	КонецЕсли;
	
	Элементы.СписокЗаказыПокупателейСоздатьРасходнуюНакладную.КнопкаПоУмолчанию = Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаЗаказыПокупателей";
	Элементы.СписокДокументыКОтгрузкеСоздатьРасходнуюНакладную.КнопкаПоУмолчанию = Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаДокументыКОтгрузке";
	
КонецПроцедуры

&НаСервере
Процедура ЗакладкаОткрыта(ЗакладкаЗаказыОткрыта)
	
	СписокЗаказыПокупателей.Параметры.УстановитьЗначениеПараметра("ЗакладкаЗаказыОткрыта", ЗакладкаЗаказыОткрыта);
...
Процедура СоздатьРасходнуюНакладную(Команда)
	...
	ЭтоСписокЗаказыПокупателей = Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаЗаказыПокупателей";
	ЭтоСписокДокументыКОтгрузке = Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаДокументыКОтгрузке";
	
	ИмяСписка = "";
	ИмяДокументов = "";
	ИмяМассиваДокументов = "";
	
	Если ЭтоСписокЗаказыПокупателей Тогда
		ИмяСписка = "СписокЗаказыПокупателей";
		ИмяМассиваДокументов = "МассивЗаказовПокупателей";
		ИмяДокументов = "Заказы";
	ИначеЕсли ЭтоСписокДокументыКОтгрузке Тогда
		ИмяСписка = "СписокДокументыКОтгрузке";
		ИмяМассиваДокументов = "МассивДокументовКОтгрузке";
		ИмяДокументов = "Расходные ордера";
	КонецЕсли;
	...
	МассивДокументов = Элементы[ИмяСписка].ВыделенныеСтроки;
	...
	Иначе
		...
		Если ЭтоСписокЗаказыПокупателей Тогда
			СтруктураДанных = ПроверитьКлючевыеРеквизитыЗаказов(МассивДокументов);
		ИначеЕсли ЭтоСписокДокументыКОтгрузке Тогда
			СтруктураДанных = ПроверитьКлючевыеРеквизитыДокументовПоступления(МассивДокументов, "Расход");
		КонецЕсли;
		...
		Если СтруктураДанных.СформироватьНесколькоДокументов Тогда
			...
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru = '(%1) отличаются данными (%2) шапки документов. Сформировать несколько расходных накладных?'"), ИмяДокументов, СтруктураДанных.ПредставлениеДанных);
			КонецЕсли; 
			
			ПоказатьВопрос(Новый ОписаниеОповещения("СоздатьРасходнуюНакладнуюЗавершение", ЭтотОбъект, Новый Структура("МассивДокументов", МассивДокументов)), ТекстСообщения, РежимДиалогаВопрос.ДаНет, 0);
			
		Иначе
			
			СтруктураЗаполнения = Новый Структура();
			СтруктураЗаполнения.Вставить(ИмяМассиваДокументов, МассивДокументов);
...
Процедура СоздатьПоШаблону(Команда)
	...
	ЗаполнениеОбъектовУНФКлиент.ПоказатьВыборШаблонаДляСозданияДокументаИзСписка("Документ.РасходнаяНакладная",
...
Процедура СоздатьВозвратКомитенту(Команда)
	...
	ПараметрыВозврата.Вставить("ВидОперацииВозврат", ТолькоВозвраты);
	ПараметрыВозврата.Вставить("ВидОперации", "ВозвратКомитенту");
	ОткрытьФорму("Документ.РасходнаяНакладная.ФормаОбъекта", ПараметрыВозврата);
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьВозвратИзПереработки(Команда)
	
	ПараметрыВозврата = Новый Структура();
	ПараметрыВозврата.Вставить("ВидОперацииВозврат", ТолькоВозвраты);
	ПараметрыВозврата.Вставить("ВидОперации", "ВозвратИзПереработки");
	ОткрытьФорму("Документ.РасходнаяНакладная.ФормаОбъекта", ПараметрыВозврата);
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьВозвратСОтветХранения(Команда)
	
	ПараметрыВозврата = Новый Структура();
	ПараметрыВозврата.Вставить("ВидОперацииВозврат", ТолькоВозвраты);
	ПараметрыВозврата.Вставить("ВидОперации", "ВозвратИзОтветХранения");
	ОткрытьФорму("Документ.РасходнаяНакладная.ФормаОбъекта", ПараметрыВозврата);
...
Процедура СоздатьВозвратПоставщику(Команда)
	...
	ПараметрыВозврата = Новый Структура();
	ПараметрыВозврата.Вставить("ВидОперацииВозврат", ТолькоВозвраты);
	ПараметрыВозврата.Вставить("ВидОперации", "ВозвратПоставщику");
	ОткрытьФорму("Документ.РасходнаяНакладная.ФормаОбъекта", ПараметрыВозврата);
...
Функция ПроверитьКлючевыеРеквизитыДокументовПоступления(МассивДокументов, ТипДокументаПроверки = "Приход")
	
	Возврат СкладскойУчетСервер.ПроверитьКлючевыеРеквизитыСкладскихОрдеров(МассивДокументов, ТипДокументаПроверки);
	
КонецФункции

&НаСервере
Процедура УстановитьОтборПоВидуОперацийВозврата()
	
	Если Не ТолькоВозвраты Тогда
		Возврат;
	КонецЕсли;
	
	ЭлементыОтбора = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(Список.Отбор, "ВидОперации");
	ОтборНеУстановлен = ЭлементыОтбора.Количество() = 0 ИЛИ НЕ ЭлементыОтбора[0].Использование;
	
	Если НЕ ОтборНеУстановлен Тогда
		Возврат;
	КонецЕсли;
...
Функция ПроверитьКлючевыеРеквизитыЗаказов(МассивЗаказов)
	...
	|	0 КАК КоличествоНоменклатураДоставки
	|ИЗ
	...
	|	0,
	|	0,
	|	КОЛИЧЕСТВО(ЗаказПокупателяШапка.НоменклатураДоставки)
	|ИЗ
	|	Документ.ЗаказПокупателя КАК ЗаказПокупателяШапка
	|ГДЕ
	|	ЗаказПокупателяШапка.Ссылка В(&МассивЗаказов)
	|	И ЗаказПокупателяШапка.НоменклатураДоставки <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	И НЕ ЗаказПокупателяШапка.СпособДоставки В (ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПустаяСсылка), ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз))
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказПокупателяШапка.НоменклатураДоставки) > 0";
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.КоличествоНоменклатураДоставки > 0 Тогда
				ПредставлениеДанных = ПредставлениеДанных + ?(ПустаяСтрока(ПредставлениеДанных), "Услуга доставки", ", Услуга доставки");
				СтруктураДанных.Вставить("ТекстСообщения", НСтр("ru = 'Заказы с доставкой могут быть отгружены только отдельными документами. Сформировать несколько расходных накладных?'"));
			КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораОтборОплата()
	...
	МассивИсключений = РаботаСОтборами.МассивИсключенийПоТипуДокумента("РасходнаяНакладная");
...
Процедура УдалитьМеткуОтбора(МеткаИД)
	...
	УстановитьОтборПоВидуОперацийВозврата();
...
Процедура СохранитьНастройкиОтборов()
	...
	Иначе
		РаботаСОтборами.СохранитьНастройкиОтборов(ЭтотОбъект,,,"Возвраты");
	КонецЕсли;
