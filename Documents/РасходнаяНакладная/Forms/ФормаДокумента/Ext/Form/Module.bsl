...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ЭтоУниверсальныйДокумент 
		...
		Если СтруктураСистемаНалогообложения.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Общая
...
Функция ЗаполнитьЦеныПоСуммеРасчетов() 
	...
	ТаблицаДокумента = Объект.Запасы.Выгрузить(, "Номенклатура, Характеристика, Партия, ЗаказПокупателя");
...
Функция ПолучитьДанныеХарактеристикаПриИзменении(СтруктураДанные)
	...
	Если СтруктураДанные.Свойство("УчетНоменклатурыПоставщиков") И СтруктураДанные.УчетНоменклатурыПоставщиков Тогда //BG
		СтруктураДанные.Вставить("НоменклатураПоставщика", НоменклатураВДокументахСервер.НоменклатураПоставщикаПоНоменклатуре(СтруктураДанные.Контрагент, СтруктураДанные.Номенклатура, СтруктураДанные.Характеристика));
	Иначе
		СтруктураДанные.Вставить("НоменклатураПоставщика", Неопределено);
	КонецЕсли;
...
Процедура ОбработатьДобавлениеСтрокиЗапасов(НоваяСтрока, ИмяТабличнойЧасти = "Запасы", ШтрихкодУпаковки = Неопределено, ПересчитыватьНДС = Ложь, РассчитатьЦены = Ложь)
	...
	СтруктураДанныеНоменклатуры.Вставить("СуммаВключаетНДС", Объект.СуммаВключаетНДС);
	СтруктураДанныеНоменклатуры.Вставить("СтруктурнаяЕдиница", Объект.СтруктурнаяЕдиница);
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь)
	...
	Если ЭтоПродажа(Объект.ВидОперации)
		...
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером") Тогда
			...
			НовыйПараметр= Новый ПараметрВыбора("ДоговорКомиссии", Объект.ДоговорКомиссии);
			НовыйМассив = Новый Массив();
			НовыйМассив.Добавить(НовыйПараметр);
			НовыеПараметры = Новый ФиксированныйМассив(НовыйМассив);
...
Процедура ОповещениеПослеСообщенияКассиру(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ВыдатьПодаркиНаВыбор Тогда
		ИДЗамера = ОценкаПроизводительностиКлиент.ЗамерВремени("РасходнаяНакладная_ВыбораПодарка", Истина, Ложь);
		ДополнительныеПараметры = Новый Структура;
