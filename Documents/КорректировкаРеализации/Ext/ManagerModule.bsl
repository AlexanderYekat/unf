...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.КорректировкаРеализации);
...
Процедура СформироватьТаблицаСерииНоменклатуры(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Если ДокументСсылкаРасходнаяНакладная.СерииНоменклатуры.Количество()=0
		И ДокументСсылкаРасходнаяНакладная.СерииНоменклатурыДоКорректировки.Количество()=0 Тогда
	...
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаЗапасы.ВидОперацииИсправляемогоДокумента = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ПередачаНаКомиссию)
...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаЗапасы.ПередачаНаКомиссию
	|			ТОГДА НЕОПРЕДЕЛЕНО
	...
	|		КОГДА ТаблицаЗапасы.ПередачаНаКомиссию
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаЗапасы.НеУчитыватьЗаказыПриПередачеНаКомиссию
	...
	|	ТаблицаЗапасы.ПередачаНаКомиссию,
	|	ВЫБОР
	|		КОГДА ТаблицаЗапасы.ПередачаНаКомиссию
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ТаблицаЗапасы.Заказ
...
Процедура СформироватьТаблицаЗапасыРаботы(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуется <> 0 Тогда
			...
			СтрокаТаблицыРасход.СодержаниеПроводки = СодержанияПроводок.СписаниеЗапасов();
...
Процедура СформироватьТаблицаЗапасыТовары(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	|	ЛОЖЬ КАК ФиксированнаяСтоимость,
	|	ВЫБОР
	|		КОГДА ТаблицаЗапасы.Количество < 0
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Возврат,
	|	ТаблицаЗапасы.СчетУчетаСебестоимость КАК СчетДт,
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяВсего < 0 Тогда
			...
			Если КоличествоТребуетсяРезерв <> 0 Тогда
				...
				СтрокаТаблицыРасход.СодержаниеПроводки = СодержанияПроводок.СписаниеЗапасовВРезерв();
				...
				СтрокаТаблицыПриход.СодержаниеПроводки = СодержанияПроводок.ПоступлениеЗапасовВРезерв();
				...
				СтрокаТаблицыРасход.Сумма = 0;
				СтрокаТаблицыРасход.Количество = КоличествоТребуетсяРезерв;
				СтрокаТаблицыРасход.СодержаниеПроводки = СодержанияПроводок.ПродажаЗапасовИзРезерва();
			...
			Если КоличествоТребуетсяСвободныйОстаток <> 0 Тогда
				...
				СтрокаТаблицыРасход.СодержаниеПроводки = СодержанияПроводок.ПродажаЗапасовИзСвободногоОстатка();
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяРезерв > 0 Тогда
			...
			Если КорректировкаВМинус Тогда
				...
				СуммаБезНДСКСписанию = -СуммаБезНДСКСписанию;
			КонецЕсли;
			...
			Если СтруктураДополнительныеСвойства.ПередачаНаКомиссию Тогда 
				...
				СтрокаТаблицыПриход.Количество = ОстатокКСписанию;
...
Процедура СформироватьТаблицаПродажи(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	|	СУММА(ТаблицаПродажи.СуммаБезСкидки) КАК СуммаБезСкидки,
	|	0 КАК Себестоимость,
	|	0 КАК СебестоимостьБезНДС,
...
Процедура СформироватьТаблицаПлатежныйКалендарь(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.ПередачаНаКомиссию Тогда 
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаПлатежныйКалендарь", Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПоложительнаяКурсоваяРазницаСчетУчета", ПланыСчетов.Управленческий.ПрочиеДоходы);
	Запрос.УстановитьПараметр("ОтрицательнаяКурсоваяРазницаСчетУчета", ПланыСчетов.Управленческий.ПрочиеРасходы);
	Запрос.УстановитьПараметр("ПередачаНаКомиссию", СтруктураДополнительныеСвойства.ПередачаНаКомиссию); 
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства) Экспорт
	...
	Если ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.РасходнаяНакладная")
		И ДокументРеализации.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаНаКомиссию Тогда 
		СтруктураДополнительныеСвойства.Вставить("ПередачаНаКомиссию", Истина);
	Иначе
		СтруктураДополнительныеСвойства.Вставить("ПередачаНаКомиссию", Ложь);
	КонецЕсли;
	...
	ИсправляемыйДокумент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ДокументСсылкаКорректировкаРеализации, "ДокументОснование", Истина);
	...
	СформироватьТаблицаЗапасыПереданные(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства); 
	...
	СформироватьТаблицаЗапасыПереданныеВРазрезеГТД(СтруктураДополнительныеСвойства); 
	...
	Если Не СтруктураДополнительныеСвойства.ПередачаНаКомиссию Тогда 
		// Эквайринг
	...
	// Серии номенклатуры
	СформироватьТаблицаСерииНоменклатуры(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства);
	СформироватьТаблицаСерииНоменклатурыКРасходу(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства);
	// Серии номенклатуры
	...
	СформироватьТаблицаОплатаДокументов(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства);
...
Функция ТекстЗапросаИнициализироватьДанныеДокумента() 
	...
	|	&ПередачаНаКомиссию КАК ПередачаНаКомиссию,
	|	ЕСТЬNULL(КорректировкаРеализации.Договор.НеУчитыватьЗаказыПриПередачеНаКомиссию, ЛОЖЬ) КАК НеУчитыватьЗаказыПриПередачеНаКомиссию
	...
	|	КорректировкаРеализацииЗапасы.Ссылка.ИсправляемыйДокументРеализации.ВидОперации КАК ВидОперацииИсправляемогоДокумента,
	|	ВЫБОР
	|		КОГДА КорректировкаРеализацииЗапасы.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА КорректировкаРеализацииЗапасы.ДокументПоступления
	|		КОГДА КорректировкаРеализацииЗапасы.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА КорректировкаРеализацииЗапасы.Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументПоступления,
	|	ВЫБОР
	|		КОГДА КорректировкаРеализацииЗапасы.СтруктурнаяЕдиница.УчетОстатковПоЯчейкам
	|			ТОГДА КорректировкаРеализацииЗапасы.Ячейка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ИзмененияЗапасов.Ссылка.Подразделение
	...
	|	ВЫБОР
	|		КОГДА &ПередачаНаКомиссию
	|			ТОГДА ИзмененияЗапасов.Ссылка.Контрагент
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
	...
	|	КОНЕЦ КАК СчетУчета,
	|	ВЫБОР
	|		КОГДА ИзмененияЗапасов.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.Продажи_НДС)
	|		ИНАЧЕ ИзмененияЗапасов.Номенклатура.СчетУчетаНДСПоРеализации
	...
	|	ВЫБОР
	|		КОГДА &ПередачаНаКомиссию
	|			ТОГДА ИзмененияЗапасов.Номенклатура
	...
	|	КОНЕЦ КАК Резерв,
	|	ИзмененияЗапасов.Ссылка.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	...
	|	ЕСТЬNULL(ПолитикиУчетаСерий.ПолитикаУчетаСерий, ИзмененияЗапасов.Номенклатура.ПолитикаУчетаСерий) КАК ПолитикаУчетаСерий,
	|	ИзмененияЗапасов.ВидОперацииИсправляемогоДокумента КАК ВидОперацииИсправляемогоДокумента,
	|	ИзмененияЗапасов.ДокументПоступления КАК ДокументПоступления,
	|	ИзмененияЗапасов.ЯчейкаКРасходу КАК ЯчейкаКРасходу,
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаУчета
	|				ТОГДА ИзмененияЗапасов.Цена * ИзмененияЗапасов.Количество
	|			КОГДА ИзмененияЗапасов.Ссылка.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА ИзмененияЗапасов.Цена * ИзмененияЗапасов.Количество * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ИзмененияЗапасов.Цена * ИзмененияЗапасов.Количество * ИзмененияЗапасов.Ссылка.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ИзмененияЗапасов.Ссылка.Кратность)
	...
	|		ПО ИзмененияЗапасов.Номенклатура = ПолитикиУчетаСерий.Владелец
	|			И ИзмененияЗапасов.СтруктурнаяЕдиница = ПолитикиУчетаСерий.СтруктурнаяЕдиница
	...
	|		СведенияПрослеживаемости.Номенклатура,
	|		-СведенияПрослеживаемости.Сумма
	|	ИЗ
	...
	|		ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
	|		0,
	|		АктОРасхожденияхПолученныйСведенияПрослеживаемости.РНПТ,
	|		-АктОРасхожденияхПолученныйСведенияПрослеживаемости.Количество,
	|		-АктОРасхожденияхПолученныйСведенияПрослеживаемости.КоличествоПрослеживаемости,
	|		АктОРасхожденияхПолученныйСведенияПрослеживаемости.ИдентификаторСтроки,
	|		АктОРасхожденияхПолученныйСведенияПрослеживаемости.СтранаПроисхождения,
	|		АктОРасхожденияхПолученныйСведенияПрослеживаемости.Номенклатура,
	|		-АктОРасхожденияхПолученныйСведенияПрослеживаемости.Сумма
	|	ИЗ
	|		Документ.АктОРасхожденияхПолученный.СведенияПрослеживаемости КАК АктОРасхожденияхПолученныйСведенияПрослеживаемости
	|	ГДЕ
	|		АктОРасхожденияхПолученныйСведенияПрослеживаемости.Ссылка = &ИсправляемыйДокумент
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КорректировкаРеализацииСерииНоменклатуры.КлючСвязи КАК КлючСвязи,
	|	КорректировкаРеализацииСерииНоменклатуры.Серия КАК Серия,
	|	ВЫБОР
	|		КОГДА КорректировкаРеализацииСерииНоменклатуры.Количество = 0
	|			ТОГДА 1
	|		ИНАЧЕ КорректировкаРеализацииСерииНоменклатуры.Количество
	|	КОНЕЦ КАК Количество,
	|	ВЫБОР
	...
	|ИЗ
	|	Документ.КорректировкаРеализации.СерииНоменклатуры КАК КорректировкаРеализацииСерииНоменклатуры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы
	|		ПО (ВременнаяТаблицаЗапасы.КлючСвязи = КорректировкаРеализацииСерииНоменклатуры.КлючСвязи)
	|ГДЕ
	|	КорректировкаРеализацииСерииНоменклатуры.Ссылка = &Ссылка
	|	И &ИспользоватьСерииНоменклатуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КорректировкаРеализацииСерииНоменклатурыДоКорректировки.КлючСвязи КАК КлючСвязи,
	|	КорректировкаРеализацииСерииНоменклатурыДоКорректировки.Серия КАК Серия,
	|	ВЫБОР
	|		КОГДА КорректировкаРеализацииСерииНоменклатурыДоКорректировки.Количество = 0
	|			ТОГДА 1
	|		ИНАЧЕ КорректировкаРеализацииСерииНоменклатурыДоКорректировки.Количество
	|	КОНЕЦ КАК Количество,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаЗапасы.ПолитикаУчетаСерий = ЗНАЧЕНИЕ(Справочник.ПолитикаУчетаСерий.ПустаяСсылка)
	|			ТОГДА &ОстаткиСерийНоменклатуры
	...
	|ИЗ
	|	Документ.КорректировкаРеализации.СерииНоменклатурыДоКорректировки КАК КорректировкаРеализацииСерииНоменклатурыДоКорректировки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы
	|		ПО (ВременнаяТаблицаЗапасы.КлючСвязи = КорректировкаРеализацииСерииНоменклатурыДоКорректировки.КлючСвязи)
	|ГДЕ
	|	КорректировкаРеализацииСерииНоменклатурыДоКорректировки.Ссылка = &Ссылка
	|	И &ИспользоватьСерииНоменклатуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ВыполнитьКонтроль(ДокументСсылкаКорректировкаРеализации, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
		...
		ИЛИ СтруктураВременныеТаблицы.ДвиженияЗапасыПереданныеИзменение Тогда 
		...
		// Отрицательный остаток по запасам переданным
...
Функция СформироватьПараметрыИсправленияКорректировочногоДокумента(ВидОперации, ДокументРеализации, ИсправляемыйДокументРеализации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидОперации)  
		...
		Если ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда 
			...
			ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(
			РеквизитыИсправляемогоДокумента.Дата,
		...
		Иначе
			...
			"НомерИсходногоДокумента",
			ПечатьДокументовУНФ.ПолучитьНомерНаПечатьСУчетомДатыДокумента(
			РеквизитыДокументаРеализации.Дата,
			РеквизитыДокументаРеализации.Номер,
			""));
...
Функция ПолучитьИсправляемыйДокументРеализации(ДокРеализации, Исходный = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ДокРеализации) И ТипЗнч(ДокРеализации) = Тип("ДокументСсылка.КорректировкаРеализации")
	...
	ИначеЕсли ЗначениеЗаполнено(ДокРеализации) И ТипЗнч(ДокРеализации) = Тип("ДокументСсылка.АктОРасхожденияхПолученный") Тогда
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаЗапасыПереданные(ДокументСсылкаКорректировкаРеализации, СтруктураДополнительныеСвойства) 
	
	Если НЕ СтруктураДополнительныеСвойства.ПередачаНаКомиссию Тогда
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПереданные", Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
...
Процедура СформироватьТаблицаЗапасыПереданныеВРазрезеГТД(СтруктураДополнительныеСвойства) 
	
	Если НЕ СтруктураДополнительныеСвойства.ПередачаНаКомиссию Тогда
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПереданныеВРазрезеГТД", Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыПереданныеВРазрезеГТД", РезультатЗапроса.Выгрузить());
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|				И НЕ КорректировкаРеализации.Организация = КорректировкаРеализации.Организация.ГоловнаяОрганизация
	...
	|ПОМЕСТИТЬ ВременнаяТаблица_КорректировкаРеализацииЗапасы
	|ИЗ
	|	(ВЫБРАТЬ
	|		КорректировкаРеализацииЗапасы.Номенклатура КАК Номенклатура,
	|		ВЫБОР
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Номенклатура КАК Номенклатура,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Содержание КАК Содержание,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ПредставлениеНоменклатуры КАК ПредставлениеНоменклатуры,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ТипНоменклатуры КАК ТипНоменклатуры,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ЗапасКод КАК ЗапасКод,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Код КАК Код,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Артикул КАК Артикул,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Штрихкод КАК Штрихкод,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.КодТНВЭД КАК КодТНВЭД,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ИспользоватьФаксимиле КАК ИспользоватьФаксимиле,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Характеристика КАК Характеристика,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ЕдиницаИзмеренияПоОКЕИ_Наименование КАК ЕдиницаИзмеренияПоОКЕИ_Наименование,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ЕдиницаИзмеренияПоОКЕИ_Код КАК ЕдиницаИзмеренияПоОКЕИ_Код,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ВидУпаковки КАК ВидУпаковки,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.КоличествоВОдномМесте КАК КоличествоВОдномМесте,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.КоличествоМест КАК КоличествоМест,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.КоэффициентЕдиницыИзмерения КАК КоэффициентЕдиницыИзмерения,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.МассаБрутто КАК МассаБрутто,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Количество КАК Количество,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Ссылка КАК Ссылка,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.СтавкаНДС КАК СтавкаНДС,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.СуммаСкидкиПоСтроке КАК СуммаСкидкиПоСтроке,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Цена КАК Цена,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Сумма КАК Сумма,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.СуммаНДС КАК СуммаНДС,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Всего КАК Всего,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.НомерСтроки КАК НомерСтроки,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.СтранаСсылка КАК СтранаСсылка,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.СтранаПредставление КАК СтранаПредставление,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.СтранаКод КАК СтранаКод,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ПредставлениеГТД КАК ПредставлениеГТД,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Вес КАК Вес,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.Объем КАК Объем,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ПрослеживаемыйТоварЧисло КАК ПрослеживаемыйТоварЧисло,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.КодТНВЭДЕдиницаИзмеренияКод КАК КодТНВЭДЕдиницаИзмеренияКод,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.КодТНВЭДЕдиницаИзмеренияНаименование КАК КодТНВЭДЕдиницаИзмеренияНаименование,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.ДатаОтгрузочногоДокумента КАК ДатаОтгрузочногоДокумента,
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы.НомерОтгрузочногоДокумента КАК НомерОтгрузочногоДокумента
	|ИЗ
	|	ВременнаяТаблица_КорректировкаРеализацииЗапасы КАК ВременнаяТаблица_КорректировкаРеализацииЗапасы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|					КОГДА ВременнаяТаблица_КорректировкаРеализацииЗапасы.СуммаВключаетНДС
	|						ТОГДА ВЫРАЗИТЬ((ВременнаяТаблица_КорректировкаРеализацииЗапасы.Сумма - ВременнаяТаблица_КорректировкаРеализацииЗапасы.СуммаНДС) / ВременнаяТаблица_КорректировкаРеализацииЗапасы.Количество КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ВременнаяТаблица_КорректировкаРеализацииЗапасы.Цена
	|				КОНЕЦ
	...
	|		ПО ВременнаяТаблица_КорректировкаРеализацииЗапасы.Ссылка = КорректировкаРеализацииСведенияПрослеживаемости.Ссылка
	|			И ВременнаяТаблица_КорректировкаРеализацииЗапасы.ИдентификаторСтроки = КорректировкаРеализацииСведенияПрослеживаемости.ИдентификаторСтроки";
...
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц) Экспорт
	...
	|	КОНЕЦ КАК Контрагент,
	|	ВЫБОР
	|		КОГДА Реквизиты.Грузополучатель.ГоловнойКонтрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	...
	|			ТОГДА ВременнаяТаблицаКорректируемыйДокумент.СуммаРегл / Реквизиты.ИсправляемыйДокументРеализации.СуммаДокумента
	...
	|		ПО Реквизиты.ИсправляемыйДокументРеализации = ВременнаяТаблицаКорректируемыйДокумент.ИсправляемыйДокумент,
	|	(ВЫБРАТЬ
	...
	|	ТаблицаНоменклатуры.Номенклатура.Штрихкод КАК ТоварШтрихкод,
	...
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатуры.СтавкаНДС.Ставка > 0
	|			ТОГДА ТаблицаНоменклатуры.Сумма / (100 + ВЫБОР
	|				КОГДА Реквизиты.СуммаВключаетНДС
	|					ТОГДА ТаблицаНоменклатуры.СтавкаНДС.Ставка
	|				ИНАЧЕ 0
	|			КОНЕЦ) * ТаблицаНоменклатуры.СтавкаНДС.Ставка * ВЫБОР
	|					КОГДА Реквизиты.ИспользоватьПересчет
	|						ТОГДА Реквизиты.Курс / Реквизиты.Кратность
	|					ИНАЧЕ 1
	|				КОНЕЦ
	...
	|	ВЫБОР
	|		КОГДА Реквизиты.СуммаВключаетНДС
	|			ТОГДА ТаблицаНоменклатуры.Сумма - ВЫБОР
	|					КОГДА ТаблицаНоменклатуры.СтавкаНДС.Ставка > 0
	|						ТОГДА ТаблицаНоменклатуры.Сумма / (100 + ВЫБОР
	|				КОГДА Реквизиты.СуммаВключаетНДС
	|					ТОГДА ТаблицаНоменклатуры.СтавкаНДС.Ставка
	|				ИНАЧЕ 0
	|			КОНЕЦ) * ТаблицаНоменклатуры.СтавкаНДС.Ставка
	|					ИНАЧЕ 0
	|				КОНЕЦ
	...
	|	КорректировкаРеализации.НоменклатураДоставки.Штрихкод,
	...
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА КорректировкаРеализации.СтавкаНДСДоставки.Ставка > 0
	|			ТОГДА КорректировкаРеализации.СтоимостьДоставки / (100 + ВЫБОР
	|				КОГДА Реквизиты.СуммаВключаетНДС
	|					ТОГДА КорректировкаРеализации.СтавкаНДСДоставки.Ставка
	|				ИНАЧЕ 0
	|			КОНЕЦ) * КорректировкаРеализации.СтавкаНДСДоставки.Ставка * ВЫБОР
	|					КОГДА Реквизиты.ИспользоватьПересчет
	|						ТОГДА Реквизиты.Курс / Реквизиты.Кратность
	|					ИНАЧЕ 1
	|				КОНЕЦ
	...
	|	ВЫБОР
	|		КОГДА Реквизиты.СуммаВключаетНДС
	|			ТОГДА КорректировкаРеализации.СтоимостьДоставки - ВЫБОР
	|					КОГДА КорректировкаРеализации.СтавкаНДСДоставки.Ставка > 0
	|						ТОГДА КорректировкаРеализации.СтоимостьДоставки / (100 + ВЫБОР
	|				КОГДА Реквизиты.СуммаВключаетНДС
	|					ТОГДА КорректировкаРеализации.СтавкаНДСДоставки.Ставка
	|				ИНАЧЕ 0
	|			КОНЕЦ) * КорректировкаРеализации.СтавкаНДСДоставки.Ставка
	|					ИНАЧЕ 0
	|				КОНЕЦ
	...
	|	ТаблицаНоменклатуры.НоменклатураНабора КАК НоменклатураНабора,
	|	ТаблицаНоменклатуры.ХарактеристикаНабора КАК ХарактеристикаНабора,
	|	ТаблицаНоменклатуры.СуммаВключаетНДС КАК СуммаВключаетНДС
	...
	|						ТОГДА ВЫРАЗИТЬ((РасходнаяНакладнаяЗапасы.Сумма - РасходнаяНакладнаяЗапасы.СуммаНДС) / РасходнаяНакладнаяЗапасы.Количество КАК ЧИСЛО(15, 2))
	...
	|			И (РасходнаяНакладнаяСведенияПрослеживаемости.ИдентификаторСтроки = РасходнаяНакладнаяЗапасы.ИдентификаторСтроки)
	|ГДЕ
	|	РасходнаяНакладнаяСведенияПрослеживаемости.Ссылка = &ИсправляемыйДокументРеализации
	|	И РасходнаяНакладнаяЗапасы.Ссылка = &ИсправляемыйДокументРеализации
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЗаказПокупателяСведенияПрослеживаемости.Ссылка,
	...
	|						ТОГДА ВЫРАЗИТЬ((ЗаказПокупателяЗапасы.Сумма - ЗаказПокупателяЗапасы.СуммаНДС) / ЗаказПокупателяЗапасы.Количество КАК ЧИСЛО(15, 2))
	...
	|		ПО ЗаказПокупателяЗапасы.Ссылка = ЗаказПокупателяСведенияПрослеживаемости.Ссылка
	|			И (ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки = ЗаказПокупателяЗапасы.ИдентификаторСтроки)
	|ГДЕ
	|	ЗаказПокупателяСведенияПрослеживаемости.Ссылка = &ИсправляемыйДокументРеализации
	|	И ЗаказПокупателяЗапасы.Ссылка = &ИсправляемыйДокументРеализации";
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "М15") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "М15", НСтр("ru = 'М15 (Накладная на отпуск материалов)'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "М15"));
