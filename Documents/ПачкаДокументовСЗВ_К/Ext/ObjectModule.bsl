					...
					Если УказаныТерриториальныеУсловия Тогда
						Если Запись.ТерриториальныеУсловия = Справочники.ТерриториальныеУсловияПФР.РКС
							...
							Если НЕ УказанРК Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для территориальных условий РКС, МКС, МКС-РКСР, МКСР и РКСМ должен быть указан районный коэффициент.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						Иначе
							Если УказанРК Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для территориальных условий, отличных от РКС, МКС, МКС-РКСР, МКСР и РКСМ, районный коэффициент указываться не должен.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
