	...
	Если ИмяСобытия = "ПодборПроизведен" 
		...
		Для Каждого НомерСтроки Из МассивНомеровИдентичныхСтрок Цикл
			...
			Если ИспользоватьСерииНоменклатурыОстатки <> Неопределено Тогда
				...
				СерииНоменклатурыУНФКлиентСервер.ОбновитьСерииНоменклатурыКоличество(Объект, СтрокаТабличнойЧасти, "СерииНоменклатурыОприходование");
...
Процедура ЗапасыНоменклатураОприходованиеПриИзменении(Элемент)
	...
	Если ЗапрашиватьСбросКоличества И СбрасыватьКоличество
		...
		ДопПараметр.Вставить("Элемент", Элемент);
		ДопПараметр.Вставить("Количество", СтрокаТабличнойЧасти.Количество);
		Оповещение = Новый ОписаниеОповещения("ЗапасыОприходованиеПодтверждениеСбросаКоличестваЗавершение",
	...
	Иначе
		...
		Иначе
			НовоеКоличество = СтрокаТабличнойЧасти.Количество;
		КонецЕсли;
		ЗапасыНоменклатураОприходованиеПриИзмененииПродолжить(Элемент, НовоеКоличество);
...
&НаКлиенте
Процедура ЗапасыОприходованиеПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗапасыНоменклатураОприходованиеПриИзмененииПродолжить(ДополнительныеПараметры.Элемент, НовоеКоличество);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗапасыНоменклатураОприходованиеПриИзмененииПродолжить(Элемент, НовоеКоличество)
...
Процедура ЗапасыПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущийЭлемент.Имя = "ЗапасыЕдиницаИзмеренияОприходование" Тогда
		Если Элемент.ТекущиеДанные <> Неопределено И Элемент.ТекущиеДанные.НоменклатураОприходование = Элемент.ТекущиеДанные.Номенклатура Тогда
			Отказ = Истина;
			ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Номенклатура списания и оприходования совпадают. Единицу измерения можно установить только для списываемой номенклатуры.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПоРасходномуОрдеру(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Объект.ДокументПоступления = Неопределено;
		ЗаполнитьПоСкладскомуОрдеру(Объект.ДокументОтгрузки);
	КонецЕсли;
...
Процедура ЗачестьТоварыПоПересортице(ТаблицаВариантов, Результат)
	...
	ТаблицаТоваров = ТаблицаВариантов.Скопировать( ,
		"НоменклатураОприходование, ХарактеристикаОприходование, ПартияОприходование, СерииНоменклатурыОприходование");
	ТаблицаТоваров.Свернуть(
		"НоменклатураОприходование, ХарактеристикаОприходование, ПартияОприходование, СерииНоменклатурыОприходование");
	Для Каждого ТекСтрокаВарианты Из ТаблицаТоваров Цикл
		
		Если ТекСтрокаВарианты.НоменклатураОприходование.Пустая() Тогда
			Продолжить;
		КонецЕсли;
		
		ОтборВарианты = Новый Структура;
		ОтборВарианты.Вставить("НоменклатураОприходование", ТекСтрокаВарианты.НоменклатураОприходование);
		ОтборВарианты.Вставить("ХарактеристикаОприходование", ТекСтрокаВарианты.ХарактеристикаОприходование);
		ОтборВарианты.Вставить("ПартияОприходование", ТекСтрокаВарианты.ПартияОприходование);
		ОтборВарианты.Вставить("СерииНоменклатурыОприходование", ТекСтрокаВарианты.СерииНоменклатурыОприходование);
		МассивСтрокВариантов = ТаблицаВариантов.НайтиСтроки(
			ОтборВарианты);
		
		ОтборЗапасыОприходование = Новый Структура;
		ОтборЗапасыОприходование.Вставить("НоменклатураОприходование", ТекСтрокаВарианты.НоменклатураОприходование);
		ОтборЗапасыОприходование.Вставить("ХарактеристикаОприходование", ТекСтрокаВарианты.ХарактеристикаОприходование);
		ОтборЗапасыОприходование.Вставить("ПартияОприходование", ТекСтрокаВарианты.ПартияОприходование);
		ОтборЗапасыОприходование.Вставить("СерииНоменклатурыОприходование", ТекСтрокаВарианты.СерииНоменклатурыОприходование);
		ОтборЗапасыОприходование.Вставить("Номенклатура", Справочники.Номенклатура.ПустаяСсылка());
		МассивСтрокТоварыОприходование = Объект.Запасы.НайтиСтроки(ОтборЗапасыОприходование);
		...
		Для Каждого Вариант Из МассивСтрокВариантов Цикл
			ОтборЗапасыСписываемые = Новый Структура;
			ОтборЗапасыСписываемые.Вставить("Номенклатура", Вариант.Номенклатура);
			ОтборЗапасыСписываемые.Вставить("Характеристика", Вариант.Характеристика);
			ОтборЗапасыСписываемые.Вставить("Партия", Вариант.Партия);
			ОтборЗапасыСписываемые.Вставить("СерииНоменклатуры", Вариант.СерииНоменклатуры);
			ОтборЗапасыСписываемые.Вставить("НоменклатураОприходование", Справочники.Номенклатура.ПустаяСсылка());
			МассивСтрокТоварыСписываемые = Объект.Запасы.НайтиСтроки(ОтборЗапасыСписываемые);
...
Функция ПолучитьСерииНоменклатурыИзХранилища(АдресВоВременномХранилище, КлючСтроки, ЭтоОприходование = Ложь)
	...
	ПараметрыИменаПолей.Вставить("ИмяТЧСерииНоменклатуры", ?(ЭтоОприходование, "СерииНоменклатурыОприходование",
		"СерииНоменклатуры"));
...
Функция ПараметрыПодбораСерийНоменклатуры(ТекущиеДанныеИдентификатор, ЭтоОприходование = Ложь)
	...
	Если ЭтоОприходование Тогда
		Возврат СерииНоменклатурыУНФ.ПараметрыПодбораСерийНоменклатуры(Объект, ЭтотОбъект.УникальныйИдентификатор,
			ТекущиеДанныеИдентификатор, , , "СерииНоменклатурыОприходование", , ЭтоОприходование);
	Иначе
		Возврат СерииНоменклатурыУНФ.ПараметрыПодбораСерийНоменклатуры(Объект, ЭтотОбъект.УникальныйИдентификатор,
