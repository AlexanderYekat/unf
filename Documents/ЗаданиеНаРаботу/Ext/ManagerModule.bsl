...
Процедура ЗаполнитьДатаНачалаОкончания(Параметры) Экспорт
	...
	Если Результат.Пустой() Тогда
		...
		ТекстСообщения = НСтр("ru='Процедура ЗаполнитьДатаНачалаОкончания завершила обработку. Запрос вернул пустую порцию документов.'");
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Документы.ЗаданиеНаРаботу,, ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Пока ЗаданияНаРаботу.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Документы.ЗаданиеНаРаботу, ЗаданияНаРаботу.Ссылка, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
	
	Если ОбъектовОбработано = 0 Тогда
		ТекстСообщения = НСтр("ru='Процедуре ЗаполнитьДатаНачалаОкончания не удалось обработать некоторые объекты (пропущены): %1'");
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.ЗаданиеНаРаботу.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета = "")
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Запрос.УстановитьПараметр("Организация", 		Константы.УчетПоКомпании.Компания(ТекущийДокумент.Организация));
		...
		ОбластьШапка.Параметры.ПредставлениеОрганизации = ПечатьДокументовУНФ.ОписаниеОрганизации(
			СведенияОбОрганизации, "ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
		
		ТабличныйДокумент.Вывести(ОбластьШапка);
		
		ОбластьШапкаТаблицы.Параметры.ТекстВидаЗадания = СтрШаблон(НСтр("ru = 'Задание является %1'"), ?(Шапка.ВидОперации
			= Перечисления.ВидыОперацийЗаданиеНаРаботу.Внешнее, НСтр("ru = 'внешним'"), НСтр("ru = 'внутренним'")));
		...
		ОбластьПодвал.Параметры.РасшифровкаОтветственного = ?(ЗначениеЗаполнено(Шапка.Должность), Строка(Шапка.Должность) + ", ", "") + Строка(Шапка.Сотрудник);
		ОбластьПодвал.Параметры.РасшифровкаПодписиМОЛ = ?(ЗначениеЗаполнено(Шапка.ДолжностьМОЛ), Строка(Шапка.ДолжностьМОЛ) + ", ", "") + Строка(Шапка.РасшифровкаПодписиМОЛ);
