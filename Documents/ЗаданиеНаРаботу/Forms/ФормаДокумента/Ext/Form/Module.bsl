...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаборСвойств_Документ_ЗаданиеНаРаботу = УправлениеСвойствами.НаборСвойствПоИмени("Документ_ЗаданиеНаРаботу");
	КэшЗначений.Вставить("НаборСвойств_Документ_ЗаданиеНаРаботу", НаборСвойств_Документ_ЗаданиеНаРаботу);
...
&НаКлиенте
Процедура УтвердилНажатие(Элемент)
...
Процедура ВидРаботПриИзменении(Элемент)
	...
	ЦенообразованиеКлиентСервер.ДополнитьСтруктуруДанныхНоменклатурыПолямиЦенообразования(Объект, Неопределено, СтруктураДанные, ,Истина);
...
Процедура РаботыНачалоДатаПриИзменении(Элемент)
	...
	РассчитатьПродолжительность(ДанныеСтроки);
	РассчитатьСумму(ДанныеСтроки);
	РассчитатьДатыНачалаОкончания(ДанныеСтроки);
	
КонецПроцедуры

&НаКлиенте
Процедура РассчитатьДатыНачалаОкончания(ДанныеСтроки)
	
	Если Не ЗначениеЗаполнено(Объект.ДатаНачала) Или ДанныеСтроки.ДатаНачала < Объект.ДатаНачала Тогда
		Объект.ДатаНачала = ДанныеСтроки.ДатаНачала;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Объект.ДатаОкончания) Или ДанныеСтроки.ДатаОкончания > Объект.ДатаОкончания Тогда
		Объект.ДатаОкончания = ДанныеСтроки.ДатаОкончания;
	КонецЕсли;
...
Процедура РаботыВидРаботПриИзменении(Элемент)
	...
	Если ДанныеСтроки.Свойство("МинимальнаяЦена") Тогда
...
Процедура РаботыЗаказчикОбработкаВыбораВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		ОткрытьФорму("Справочник.ДоговорыКонтрагентов.Форма.ФормаВыбораСКонтрагентом", , , , , ,
			Новый ОписаниеОповещения("РаботыЗаказчикОбработкаВыбораВыбораЗавершение", ЭтотОбъект));
...
Процедура РаботыКомментарийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("ОбщаяФорма.РедактированиеТекста", ПараметрыФормы, , , , ,
		Новый ОписаниеОповещения("РаботыКомментарийНачалоВыбораЗавершение", ЭтотОбъект,
		Новый Структура("ТекущиеДанные", ДанныеСтроки))); 
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", КэшЗначений.НаборСвойств_Документ_ЗаданиеНаРаботу);
...
//
// Параметры:
//  СтруктураРеквизитов - Структура - реквизиты, необходимые при пересчете
...
Процедура ОткрытьФормуРеквизитыПечати()
	...
	ОткрытьФорму("Обработка.РеквизитыПечати.Форма.РеквизитыПечатиЗаданиеНаРаботу",
...
Процедура НапоминанияПриСозданииНаСервере()
	...
	Если ПолеИнтервалНапоминания <> Неопределено Тогда
		ПолеИнтервалНапоминания.Ширина = 12;
	КонецЕсли;
