...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	|	И ДляВсехСтрок(ЗначениеРазрешено(Материалы.СтруктурнаяЕдиницаРезерв, ПустаяСсылка КАК Истина, Null КАК Истина))";
...
Процедура ИзменитьСостояниеЗаказа(Заказ, Состояние, ВариантЗавершения = Неопределено) Экспорт
	...
	Если СостояниеЗаказа = Состояние И Состояние <> Справочники.СостоянияЗаказовПоставщикам.Завершен Тогда
		Возврат;
	КонецЕсли;
	
	ЗаказЗавершен = (СостояниеЗаказа = Состояние И Состояние = Справочники.СостоянияЗаказовПоставщикам.Завершен
...
Функция СтатусОтгрузки(Заказ, МоментВремени = Неопределено) Экспорт
	...
	Проведен = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Заказ, "Проведен");
	ВариантЗавершения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Заказ, "ВариантЗавершения");
	
	Если НЕ Проведен ИЛИ ВариантЗавершения = Перечисления.ВариантыЗавершенияЗаказа.Отменен Тогда
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	Если НастройкиЗагрузкиДанных.Свойство("ПоложениеДатыВЗаказе") Тогда
		...
		ПолеВидимо = (НастройкиЗагрузкиДанных.ПоложениеДатыВЗаказе
...
Функция СформироватьЗаказПоставщику(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	ВНоменклатуреПоставщика = ПараметрыПечати.Свойство("ВНоменклатуреПоставщика") И ПараметрыПечати.ВНоменклатуреПоставщика;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("МассивОбъектов", МассивОбъектов);
	Запрос.УстановитьПараметр("ВНоменклатуреПоставщика", ВНоменклатуреПоставщика);
	...
	|		ВЫБОР
	|			КОГДА НЕ &ВНоменклатуреПоставщика
	|				ТОГДА ВЫБОР
	...
	|					КОНЕЦ
	|			ИНАЧЕ ЗаказПоставщику.Запасы.НоменклатураПоставщика.Наименование
	|		КОНЕЦ КАК ПредставлениеНоменклатуры,
	|		ВЫБОР
	|			КОГДА НЕ &ВНоменклатуреПоставщика
	|				ТОГДА ЗаказПоставщику.Запасы.Номенклатура.Артикул
	|			ИНАЧЕ ЗаказПоставщику.Запасы.НоменклатураПоставщика.Артикул
	|		КОНЕЦ КАК Артикул,
	|		Номенклатура.Штрихкод КАК Штрихкод,
	|		ВЫБОР
	|			КОГДА НЕ &ВНоменклатуреПоставщика
	|				ТОГДА ЗаказПоставщику.Запасы.Номенклатура.Код
	|			ИНАЧЕ ЗаказПоставщику.Запасы.НоменклатураПоставщика.Код
	|		КОНЕЦ КАК Код,
	|		ВЫБОР
	|			КОГДА НЕ &ВНоменклатуреПоставщика
	|				ТОГДА ЗаказПоставщику.Запасы.Характеристика
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
	|		КОНЕЦ КАК Характеристика,
	...
	|		НомерСтроки КАК НомерСтроки,
	|		ВЫБОР
	|			КОГДА (ВЫРАЗИТЬ(ЗаказПоставщику.Материалы.Номенклатура.НаименованиеПолное КАК СТРОКА(100))) = """"
	|				ТОГДА ЗаказПоставщику.Материалы.Номенклатура.Наименование
	|			ИНАЧЕ ЗаказПоставщику.Материалы.Номенклатура.НаименованиеПолное
	|		КОНЕЦ КАК ПредставлениеНоменклатуры,
	...
	|		ПО ЗаказПоставщику.Ответственный.Физлицо = ФИОФизЛицСрезПоследних.ФизическоеЛицо
	|ГДЕ
	...
	Пока Шапка.Следующий() Цикл
		...
		Если ОбластьЗаголовок <> Неопределено Тогда
			...
			Если Шапка.ВидОперации = Перечисления.ВидыОперацийЗаказПоставщику.ЗаказНаПереработку Тогда
				ПредставлениеДокумента = СтрШаблон(НСтр("ru ='Заказ на переработку № %1 от %2'"), НомерДокумента, ДатаДокумента);
			Иначе
		...
		ТаблицаМатериалы = Шапка.ТаблицаМатериалы.Выгрузить();
		...
		Итоги.Вставить("ЕстьМатериалы", ТаблицаМатериалы.Количество()>0);
		...
		ОбластиМакета.Вставить("ОбластьМакетаПодписи", ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет,
			"Подписи", "", Ошибки));
		ОбластиМакета.Вставить("ОбластьМакетаНаименованииТаблицы", ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет,
			"НаименованиеТаблицы", "", Ошибки));
		ОбластиМакета.Вставить("ОбластьМакетаШапкаТаблицыМатериалы", ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет,
			"ШапкаТаблицыМатериалы", "", Ошибки));
		ОбластиМакета.Вставить("ОбластьМакетаСтрокаМатериалы", ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет,
			"СтрокаМатериалы", "", Ошибки));
		ОбластиМакета.Вставить("ОбластьМакетаИтогоМатериалы", ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет,
			"ИтогоМатериалы", "", Ошибки));
		
		Если ОбластиМакета.ОбластьМакетаНаименованииТаблицы <> Неопределено Тогда

			ТабличныйДокумент.Вывести(ОбластиМакета.ОбластьМакетаНаименованииТаблицы);
		...
		Если Итоги.ЕстьМатериалы Тогда
			
			Если ОбластиМакета.ОбластьМакетаШапкаТаблицыМатериалы <> Неопределено Тогда
				ОбластиМакета.ОбластьМакетаШапкаТаблицыМатериалы.Параметры.Заполнить(ДанныеПечати);
				ТабличныйДокумент.Вывести(ОбластиМакета.ОбластьМакетаШапкаТаблицыМатериалы);
			КонецЕсли;
			
			Если ОбластиМакета.ОбластьМакетаСтрокаМатериалы <> Неопределено Тогда
				
				Для Каждого СтрокаТабличнойЧастиМатериалы Из ТаблицаМатериалы Цикл
					
					ДанныеПечати.Очистить();
					ДанныеПечати.Вставить("НомерСтроки", СтрокаТабличнойЧастиМатериалы.НомерСтроки);
					
					ДанныеПечати.Вставить("ДатаОтгрузки", СтрокаТабличнойЧастиМатериалы.ДатаОтгрузки);
					
					ПараметрыНоменклатуры.Очистить();
					ПараметрыНоменклатуры.Вставить("Содержание", Неопределено);
					ПараметрыНоменклатуры.Вставить("ПредставлениеНоменклатуры",
					СтрокаТабличнойЧастиМатериалы.ПредставлениеНоменклатуры);
					ПараметрыНоменклатуры.Вставить("ПредставлениеХарактеристики", СтрокаТабличнойЧастиМатериалы.Характеристика);
					ПараметрыНоменклатуры.Вставить("ПредставлениеАртикула", СтрокаТабличнойЧастиМатериалы.Артикул);
					...
					СтрокаТабличнойЧастиМатериалы));
					
					ДанныеПечати.Вставить("Количество", СтрокаТабличнойЧастиМатериалы.Количество);
					ДанныеПечати.Вставить("ЕдиницаИзмерения", СтрокаТабличнойЧастиМатериалы.ЕдиницаИзмерения);
					
					ОбластиМакета.ОбластьМакетаСтрокаМатериалы.Параметры.Заполнить(ДанныеПечати);
					ТабличныйДокумент.Вывести(ОбластиМакета.ОбластьМакетаСтрокаМатериалы);
					
				КонецЦикла;
			КонецЕсли;
			
			Если ОбластиМакета.ОбластьМакетаИтогоМатериалы <> Неопределено Тогда
				ТабличныйДокумент.Вывести(ОбластиМакета.ОбластьМакетаИтогоМатериалы);
			КонецЕсли;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ОписаниеПечатнойФормы <> Неопределено Тогда
		...
		СформироватьЗаказПоставщику(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати, ПараметрыПечати);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.ДополнительныеПараметры.Вставить("ВНоменклатуреПоставщика", Истина);
