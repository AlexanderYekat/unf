...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПолучитьФункциональнуюОпцию("ВыполнятьРасчетНДФЛПоПрогрессивнойШкалеУНФ") Тогда
	...
	Элементы.СтраницаПогашениеЗаймов.Видимость = ПолучитьФункциональнуюОпцию("КредитыИЗаймы");
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ИсточникВыбора) = Тип("ФормаКлиентскогоПриложения") И СтрНайти(ИсточникВыбора.ИмяФормы,
		"ФормаКалендаря") > 0 Тогда
...
Процедура НачисленияУдержанияСчетЗатратНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗарплатаИПерсоналКлиент.ПараметрыВыбораСчетаЗатрат(ЭтотОбъект);
...
&НаКлиенте
Процедура НачисленияУдержанияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)

	Если Поле.Имя = "НачисленияУдержанияСуммаСРасшифровкой" Тогда

		ТекущиеДанные = Элементы.НачисленияУдержания.ТекущиеДанные;

		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Сотрудник", ТекущиеДанные.Сотрудник);
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораМенюРасшифровки", ЭтотОбъект, ДополнительныеПараметры);
		СписокЭлементовМеню = Новый СписокЗначений;
		СписокЭлементовМеню.Добавить("ИзменитьЗначение", НСтр("ru = 'Изменить значение'"), ,
			БиблиотекаКартинок.Изменить);
		СписокЭлементовМеню.Добавить("РасшифроватьЗначение", НСтр("ru = 'Расшифровать значение'"), ,
			БиблиотекаКартинок.Отчет);

		ПоказатьВыборИзМеню(ОписаниеОповещения, СписокЭлементовМеню, );
...
Процедура НачисленияУдержанияВидНачисленияУдержанияПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(ДанныеТекущейСтроки.СчетЗатрат) Тогда
		...
		ЗарплатаИПерсоналКлиент.СчетЗатратПоУмолчаниюВТекущуюСтроку(ЭтотОбъект, Объект.СтруктурнаяЕдиница);
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыНалогиНаДоходы
...
Процедура СтраховыеВзносыСчетЗатратНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗарплатаИПерсоналКлиент.ПараметрыВыбораСчетаЗатрат(ЭтотОбъект, "СтраховыеВзносы");
...
Процедура Рассчитать(Команда)
	...
	Если Не ПроверитьЗаполнениеРеквизитовОрганизацииДляРасчетаВзносов(Объект.Организация, Объект.ПериодРегистрации,
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьРеквизитыОрганизацииЗавершение", ЭтотОбъект,
			Новый Структура("Расшифровка, ПроцедураВыполнения", Расшифровка, "РассчитатьПоФормуламИНалогиИВзносы")),
...
Процедура Заполнить(Команда)
	...
	ИначеЕсли Объект.НачисленияУдержания.Количество() > 0 Или Объект.НалогиНаДоходы.Количество() > 0
		Или Объект.ПогашениеЗаймов.Количество() > 0 Тогда // Прочие расчеты
...
КонецПроцедуры
&НаКлиенте
Процедура УдалитьСтрокиСПустойСуммой(Команда)

	МассивИдентификаторов = Новый Массив;
	Для Каждого СтрокаНачисления Из Объект.НачисленияУдержания Цикл

		Если СтрокаНачисления.Сумма = 0 Тогда
			МассивИдентификаторов.Добавить(СтрокаНачисления.ПолучитьИдентификатор());
		КонецЕсли;

	КонецЦикла;

	Количество = МассивИдентификаторов.Количество();
	Пока Количество > 0 Цикл

		СтрокаДляУдаления = Объект.НачисленияУдержания.НайтиПоИдентификатору(МассивИдентификаторов[Количество - 1]);
		Объект.НачисленияУдержания.Удалить(СтрокаДляУдаления);
...
Функция ЗаполнитьПоказатели(СтруктураДанных)
	...
	СтруктураВозврата.Вставить("Представление1", Справочники.ПоказателиРасчетов.ПустаяСсылка());
	СтруктураВозврата.Вставить("Значение1", 0);
	СтруктураВозврата.Вставить("Показатель2", "");
	СтруктураВозврата.Вставить("Представление2", Справочники.ПоказателиРасчетов.ПустаяСсылка());
	СтруктураВозврата.Вставить("Значение2", 0);
	СтруктураВозврата.Вставить("Показатель3", "");
	СтруктураВозврата.Вставить("Представление3", Справочники.ПоказателиРасчетов.ПустаяСсылка());
	СтруктураВозврата.Вставить("Значение3", 0);
	СтруктураВозврата.Вставить("СчетЗатрат", Неопределено);

	Если Не ЗначениеЗаполнено(СтруктураДанных.Сотрудник) Или Не ЗначениеЗаполнено(
		СтруктураДанных.ВидНачисленияУдержания) Тогда
	...
	ЗарплатаУНФВызовСервера.ДобавитьПараметрыВСтруктуру(СтруктураДанных.ВидНачисленияУдержания.Формула,
		СтруктураПараметров);

	Счетчик = 0;
	Для Каждого ПараметрСтруктуры Из СтруктураПараметров Цикл

		Если ПараметрСтруктуры.Ключ = "ОтработаноДней" Или ПараметрСтруктуры.Ключ = "ОтработаноЧасов"
		...
		ПараметрРасчета = Справочники.ПоказателиРасчетов.НайтиПоРеквизиту("Идентификатор", ПараметрСтруктуры.Ключ);
		Если Не ЗначениеЗаполнено(ПараметрРасчета) Тогда

			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтрШаблон(НСтр("ru = 'Не найден параметр %1 для формулы начисления (удержания) %2'"),
				ПараметрРасчета, СтруктураДанных.ВидНачисленияУдержания);
			Сообщение.Сообщить();
			Продолжить;
...
Процедура ЗаполнитьПоПодразделению()
	...
	|		ПлановыеНачисленияИУдержания.Актуальность,
	|		ЕСТЬNULL(Сотрудники.ГрафикРаботы, ПлановыеНачисленияИУдержания.ГрафикРаботы)
	|	ИЗ
	...
	Пока ВыборкаСотрудник.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ДругоеПодразделениеУвольнение Тогда
				ЗаменитьДатуМассив = Объект.НачисленияУдержания.НайтиСтроки(Новый Структура("ДатаОкончания, Сотрудник",
					КонецМесяца, Выборка.Сотрудник));
				Для Каждого ЭлементМассива Из ЗаменитьДатуМассив Цикл
					ЭлементМассива.ДатаОкончания = Выборка.ДатаДействияНачало - ?(Выборка.ДругоеПодразделениеУвольнение
						= 1, 60 * 60 * 24, 0);
				КонецЦикла;
				Продолжить;
			КонецЕсли;

			ЗаменитьДатуМассив = Объект.НачисленияУдержания.НайтиСтроки(
				Новый Структура("ДатаОкончания, Сотрудник, ВидНачисленияУдержания", КонецМесяца, Выборка.Сотрудник,
				Выборка.ВидНачисленияУдержания));
			Для Каждого ЭлементМассива Из ЗаменитьДатуМассив Цикл
			...
			Если ЗначениеЗаполнено(Выборка.ВидНачисленияУдержания) И Выборка.Актуальность Тогда
				...
				Иначе
					...
					Если Выборка.ВидНачисленияУдержания.Тип = Перечисления.ТипыНачисленийИУдержаний.Начисление Тогда
						...
						СчетНачисленияУдержанияУказанСОшибкой = (КорректныеТипыСчетовНачислений.Найти(ТипСчета)
							= Неопределено);
					...
					Если Объект.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение
						И СчетНачисленияУдержанияУказанСОшибкой Тогда
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.НачисленияУдержания Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.ВидНачисленияУдержания) Тогда
			Продолжить;
		КонецЕсли;
		МассивПовторов = РезультатЗапроса.НайтиСтроки(Новый Структура("Сотрудник, ВидНачисленияУдержания",
		...
		Для Счетчик = 1 По 3 Цикл
			...
			СтрокаТабличнойЧасти["Представление" + Счетчик] = Справочники.ПоказателиРасчетов.ПустаяСсылка();
		...
		СтруктураПараметров = Новый Структура;
		ЗарплатаУНФВызовСервера.ДобавитьПараметрыВСтруктуру(СтрокаТабличнойЧасти.ВидНачисленияУдержания.Формула,
			СтруктураПараметров);

		Если СтрокаТабличнойЧасти.ВидНачисленияУдержания = Справочники.ВидыНачисленийИУдержаний.ПремияЗаПродажи Тогда
			СтруктураПараметровРасчетаПремийПродавцам = Новый Структура;
			СтруктураПараметровРасчетаПремийПродавцам.Вставить("Организация", Объект.Организация);
			СтруктураПараметровРасчетаПремийПродавцам.Вставить("Подразделение", Объект.СтруктурнаяЕдиница);
			СтруктураПараметровРасчетаПремийПродавцам.Вставить("Сотрудник", СтрокаТабличнойЧасти.Сотрудник);
			СтруктураПараметровРасчетаПремийПродавцам.Вставить("ДатаПолученияПравил", НачалоДня(КонецМесяца(
				Объект.ПериодРегистрации)));
			СтруктураПараметровРасчетаПремийПродавцам.Вставить("НачалоПериода", НачалоМесяца(Объект.ПериодРегистрации));
			СтруктураПараметровРасчетаПремийПродавцам.Вставить("КонецПериода", КонецМесяца(Объект.ПериодРегистрации));

			СтрокаТабличнойЧасти.Сумма = ЗарплатаИПерсоналСервер.РассчитатьСуммуПремииПоЛичнымПродажам(
				СтруктураПараметровРасчетаПремийПродавцам);
		...
		Иначе
			...
			Для Каждого ПараметрСтруктуры Из СтруктураПараметров Цикл
				...
				ПараметрРасчета = Справочники.ПоказателиРасчетов.НайтиПоРеквизиту("Идентификатор",
					ПараметрСтруктуры.Ключ);
				Если Не ЗначениеЗаполнено(ПараметрРасчета) Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтрШаблон(НСтр("ru = 'Не найден параметр %1 для сотрудника в строке № %2'"),
						ПараметрРасчета, Объект.НачисленияУдержания.Индекс(СтрокаТабличнойЧасти) + 1);
					Сообщение.Сообщить();
				КонецЕсли;
				...
				Если ПараметрРасчета.ТипПоказателяРасчетов = Перечисления.ТипыПоказателейРасчета.Ручной Тогда
					Продолжить;
				КонецЕсли; 
				...
				СтрокаТабличнойЧасти["Значение" + Счетчик] = ЗарплатаИПерсоналСлужебный.РассчитатьЗначениеПараметра(
					СтруктураОтборов, ПараметрРасчета, СтрШаблон(" %1", НСтр("ru = 'для сотрудника в строке №'"))
					+ (Объект.НачисленияУдержания.Индекс(СтрокаТабличнойЧасти) + 1));

				ЗакончитьЗамерВремени(НачалоЗамераПараметр, "НачислениеЗарплаты_РассчитатьЗначениеПараметра", Строка(
					ПараметрРасчета.Идентификатор));
...
Процедура РассчитатьПоФормулам()
	...
	Для Каждого СтрокаНачислений Из Объект.НачисленияУдержания Цикл
		...
		Если СтрокаНачислений.РучнаяКорректировка Или Не ЗначениеЗаполнено(
			СтрокаНачислений.ВидНачисленияУдержания.Формула) Тогда
			Продолжить;
		КонецЕсли; 
		...
		Для Счетчик = 1 По 3 Цикл
			Если ЗначениеЗаполнено(СтрокаНачислений["Представление" + Счетчик]) Тогда
				СтруктураПараметров.Вставить(СтрокаНачислений["Показатель" + Счетчик], СтрокаНачислений["Значение"
					+ Счетчик]);
			КонецЕсли;
		КонецЦикла; 
...
Процедура РассчитатьНалогиИВзносы()
	...
	НаборыЗаписей = УчетСтраховыхВзносовУНФ.НаборыЗаписейРегистратора(Объект.Ссылка.Метаданные(), ?(
	...
	Запрос.УстановитьПараметр("ОбъединятьНакопительнуюИСтраховуюЧасть", Объект.Дата
		>= УчетСтраховыхВзносовУНФ.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР());
	...
	Запрос.УстановитьПараметр("ОбщийНалоговыйРежим",
		РегламентированнаяОтчетностьПереопределяемый.СсылкаНаОбъектПоИдентификатору(
		"Справочник.ВидыТарифовСтраховыхВзносовУНФ", "ОбщийНалоговыйРежим"));
	Запрос.УстановитьПараметр("УпрощенныйНалоговыйРежим",
		РегламентированнаяОтчетностьПереопределяемый.СсылкаНаОбъектПоИдентификатору(
		"Справочник.ВидыТарифовСтраховыхВзносовУНФ", "УпрощенныйНалоговыйРежим"));
	Запрос.УстановитьПараметр("ЕНВД", РегламентированнаяОтчетностьПереопределяемый.СсылкаНаОбъектПоИдентификатору(
		"Справочник.ВидыТарифовСтраховыхВзносовУНФ", "ЕНВД"));
	...
	УчетСтраховыхВзносовУНФ.СформироватьВТРасширенныхСведенийОДоходах(Объект.ПериодРегистрации, КонецМесяца(
		Объект.ПериодРегистрации), Объект.Организация, Запрос.МенеджерВременныхТаблиц, Истина);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	
	|	ВЫБРАТЬ
	|		РасчетыСФондамиПоСтраховымВзносамУНФ.Сотрудник,
	|	ВЫБОР
	|			КОГДА РасчетыСФондамиПоСтраховымВзносамУНФ.ВидОбязательногоСтрахованияСотрудников = ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ПФРПоСуммарномуТарифу)
	|				ТОГДА -РасчетыСФондамиПоСтраховымВзносамУНФ.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|	ВЫБОР
	|			КОГДА РасчетыСФондамиПоСтраховымВзносамУНФ.ВидОбязательногоСтрахованияСотрудников = ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ПФРНакопительная)
	|				ТОГДА -РасчетыСФондамиПоСтраховымВзносамУНФ.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|	ВЫБОР
	|			КОГДА РасчетыСФондамиПоСтраховымВзносамУНФ.ВидОбязательногоСтрахованияСотрудников = ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ПФРСтраховая)
	|				ТОГДА -РасчетыСФондамиПоСтраховымВзносамУНФ.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|	ВЫБОР
	|			КОГДА РасчетыСФондамиПоСтраховымВзносамУНФ.ВидОбязательногоСтрахованияСотрудников = ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ФФОМС)
	|				ТОГДА -РасчетыСФондамиПоСтраховымВзносамУНФ.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|	ВЫБОР
	|			КОГДА РасчетыСФондамиПоСтраховымВзносамУНФ.ВидОбязательногоСтрахованияСотрудников = ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ФСС)
	|				ТОГДА -РасчетыСФондамиПоСтраховымВзносамУНФ.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|	ВЫБОР
	|			КОГДА РасчетыСФондамиПоСтраховымВзносамУНФ.ВидОбязательногоСтрахованияСотрудников = ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ФССНесчастныеСлучаи)
	|				ТОГДА -РасчетыСФондамиПоСтраховымВзносамУНФ.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|		0
	...
	|	ГДЕ
	|		РасчетыСФондамиПоСтраховымВзносамУНФ.Регистратор = &Ссылка
	|		И РасчетыСФондамиПоСтраховымВзносамУНФ.Сотрудник В
	|				(ВЫБРАТЬ
	|					СписокСотрудников.ФизическоеЛицо
	|				ИЗ
	|					ВТФизЛицаОрганизаций КАК СписокСотрудников)
	|		И РасчетыСФондамиПоСтраховымВзносамУНФ.Период МЕЖДУ &НачалоГода И &КонецМесяца) КАК РанееИсчисленныеВзносы
	|
	|СГРУППИРОВАТЬ ПО
	|	РанееИсчисленныеВзносы.ФизическоеЛицо
	|;
	...
	Если ДатаДокумента > '20230101' Тогда
		Запрос.УстановитьПараметр("ПФРПоСуммарномуТарифу", Справочники.ВидыНачисленийИУдержаний.ВзносыПоЕдиномуТарифу);
	Иначе	
	...
	Для Каждого СтрокаВзносов Из Объект.Взносы Цикл
		...
		Если Объект.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение
			...
			Или ТипСчета = Перечисления.ТипыСчетов.ПрочиеОборотныеАктивы Или ТипСчета
			= Перечисления.ТипыСчетов.Кредиторы) Тогда
...
Процедура ОбработатьВременнуюТаблицуПредоставленныхИмущественныхВычетов(Запрос)
	...
	ТаблицаПредоставленногоИмущественногоВычета.Колонки.Добавить("Сотрудник",
		Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ТаблицаПредоставленногоИмущественногоВычета.Колонки.Добавить("КодВычета",
		Новый ОписаниеТипов("СправочникСсылка.ВычетыНДФЛ"));
	ТаблицаПредоставленногоИмущественногоВычета.Колонки.Добавить("СуммаИмущественногоВычета",
	...
	Для Каждого СтрокаВычета Из ТаблицаИмущественныхВычетов Цикл
		...
		Если СтрокаВычета.СуммаНачисления > СтрокаВычета.СуммаВычетов Тогда
			...
			Если СтрокаВычета.СуммаРасходов > СтрокаВычета.ПредоставленныйВычетПоРасходам Тогда
				...
				Если СтрокаВычета.СуммаРасходов - СтрокаВычета.ПредоставленныйВычетПоРасходам
					>= СтрокаВычета.СуммаНачисления - СтрокаВычета.СуммаВычетов Тогда
					...
					СтрокаПредоставленногоВычета.СуммаИмущественногоВычета = СтрокаВычета.СуммаНачисления
						- СтрокаВычета.СуммаВычетов;//спишем весь остаток начисления в вычет
				...
				Иначе
					...
					СтрокаПредоставленногоВычета.Сотрудник = СтрокаВычета.Сотрудник;
					СтрокаПредоставленногоВычета.КодВычета = Справочники.ВычетыНДФЛ.Код311; //вычет по расходам
					СтрокаПредоставленногоВычета.СуммаИмущественногоВычета = СтрокаВычета.СуммаРасходов
						- СтрокаВычета.ПредоставленныйВычетПоРасходам;// спишем весь остаток расходов
					...
					Если СтрокаВычета.ПроцентыПоКредиту - СтрокаВычета.ПредоставленныйВычетПоПроцентам > 0 Тогда
						...
						СуммаОстаткаСписания =  СтрокаВычета.СуммаНачисления - СтрокаВычета.СуммаВычетов
							- (СтрокаВычета.СуммаРасходов - СтрокаВычета.ПредоставленныйВычетПоРасходам);
...
Процедура ОбработатьВременнуюТаблицуПредоставленныхСоциальныхВычетов(Запрос)
	...
	ТаблицаПредоставленногоСоциальногоВычета.Колонки.Добавить("Сотрудник",
		Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ТаблицаПредоставленногоСоциальногоВычета.Колонки.Добавить("КодВычета",
		Новый ОписаниеТипов("СправочникСсылка.ВычетыНДФЛ"));
	ТаблицаПредоставленногоСоциальногоВычета.Колонки.Добавить("СуммаСоциальногоВычета", Новый ОписаниеТипов("Число",
		...
		Если СтрокаВычета.СуммаНачисления > СтрокаВычета.СуммаВычетов Тогда
			...
			Если СтрокаВычета.СуммаРасходов > СтрокаВычета.ПредоставленныйВычетПоРасходам Тогда
				...
				Если СтрокаВычета.СуммаРасходов - СтрокаВычета.ПредоставленныйВычетПоРасходам
					...
					СтрокаПредоставленногоВычета.СуммаСоциальногоВычета = СтрокаВычета.СуммаНачисления
						- СтрокаВычета.СуммаВычетов;//спишем весь остаток начисления в вычет
				...
				Иначе
					...
					СтрокаПредоставленногоВычета.Сотрудник = СтрокаВычета.Сотрудник;
					СтрокаПредоставленногоВычета.КодВычета = СтрокаВычета.КодВычета; //вычет по расходам
					СтрокаПредоставленногоВычета.СуммаСоциальногоВычета = СтрокаВычета.СуммаРасходов
						- СтрокаВычета.ПредоставленныйВычетПоРасходам;// спишем весь остаток расходов
...
Процедура РассчитатьНДФЛ(Запрос)
	...
	Если Запрос.Параметры.ПериодДокумента < ДатаНачалаПрогрессивнойШкалы Или Не ВыполнятьРасчетНДФЛПоПрогрессивнойШкалеУНФ Тогда
...
Процедура РассчитатьНДФЛПоОбычнойШкале(Запрос)
	...
	ТаблицаНачислений.Свернуть(
		"Сотрудник, ИсчисленПоСтавке", "Сумма");
...
Процедура РассчитатьНДФЛПоПрогрессивнойШкале(Запрос)
	...
	ТаблицаНачислений.Свернуть(
		"Сотрудник, ИсчисленПоСтавке, ИсчисленПоПовышеннойСтавке", "Сумма");

	ТаблицаНачислений.Колонки.Добавить("СуммаНалога", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
	ТаблицаНачислений.Колонки.Добавить("СуммаНалогаСПревышения", Новый ОписаниеТипов("Число",
		Новый КвалификаторыЧисла(15, 2))); 
		...
		Для Каждого СтрокаНачислено Из СтрокиНачисленийТекущегоСотрудника Цикл
			...
			Если СтрокаНачислено.Сумма > 0 Тогда
				Если (НачисленоВсего + НачисленоВТекущемДокументе) <= ГраницаНалоговойБазы Тогда
					...
					СтрокаНачислено.СуммаНалога = Окр(СтрокаНачислено.Сумма * СтрокаНачислено.ИсчисленПоСтавке / 100, 0,
						1);
				ИначеЕсли НачисленоВсего >= ГраницаНалоговойБазы Тогда
					// Все начисления по повышенной ставке НДФЛ. 					
					СтрокаНачислено.СуммаНалогаСПревышения = Окр(СтрокаНачислено.Сумма
						* СтрокаНачислено.ИсчисленПоПовышеннойСтавке / 100, 0, 1);
				Иначе
					...
					СтрокаНачислено.СуммаНалога = Окр(СуммаНачисленияПоОбычнойСтавке * СтрокаНачислено.ИсчисленПоСтавке
						/ 100, 0, 1);
					СтрокаНачислено.СуммаНалогаСПревышения = Окр(СуммаНачисленияПоПовышеннойСтавке
						* СтрокаНачислено.ИсчисленПоПовышеннойСтавке / 100, 0, 1);
				КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПроверитьЗаполнениеРеквизитовОрганизацииДляРасчетаВзносов(Организация, ДатаАнализа, ТекстВопроса,
	РассчитыватьБезВзносов, Расшифровка)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПрименяемыеТарифыСтраховыхВзносовУНФ.СрезПоследних(&ДатаАнализа, ) КАК ТарифыСтраховыхВзносов
	|		ПО (ТарифыСтраховыхВзносов.ГоловнаяОрганизация = &Организация)
	|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		Если Не Выборка.ИспользуетсяОтчетность Тогда
			...
			Возврат Истина;
		Иначе
			ЗаполненТарифНаТравматизм = Выборка.ТарифНаТравматизм > 0 Или (СистемаНалогообложения.ПрименяетсяАУСН
				И РегистрыСведений.РазмерФиксированныхВзносовОтНесчастныхСлучаевАУСН.РазмерФиксированныхВзносовОтНесчастныхСлучаевЗаполнен());
			...
			Если Не ЗаполненТарифНаТравматизм И Не ЗаполненТарифСтраховыхВзносов Тогда
				ТекстВопроса = НСтр(
					"ru = 'В организации не заполнены тариф на травматизм и тариф страховых взносов. Заполнить?'");
				ПоляСОшибками.Вставить("КлючТарифаСтраховыхВзносов.ВидТарифа", НСтр(
					"ru='Не указан тариф страховых взносов'"));
...
Процедура ПолучитьСчетЗатратВзноса(СтруктураПараметров)
	...
	СтруктураПараметров.СчетЗатрат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		СтруктураПараметров.ВидНачисленияУдержания, "СчетЗатрат");

КонецПроцедуры

&НаКлиенте
Процедура ПослеВыбораМенюРасшифровки(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт

	Если ВыбранныйЭлемент <> Неопределено И ВыбранныйЭлемент.Значение = "РасшифроватьЗначение" Тогда

		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("Организация", Объект.Организация);
		СтруктураПараметров.Вставить("Сотрудник", ДополнительныеПараметры.Сотрудник);

		Период = Новый СтандартныйПериод;
		Период.ДатаНачала = НачалоМесяца(Объект.ПериодРегистрации);
		Период.ДатаОкончания = КонецМесяца(Объект.ПериодРегистрации);

		СтруктураПараметров.Вставить("Период", Период);
		СтруктураПараметров.Вставить("ДатаПолученияПравил", КонецМесяца(Объект.ПериодРегистрации));

		ПараметрыФормыОтчетаРасшифровки = Новый Структура;
		ПараметрыФормыОтчетаРасшифровки.Вставить("СформироватьПриОткрытии", Истина);
		ПараметрыФормыОтчетаРасшифровки.Вставить("Отбор", СтруктураПараметров);

		ОткрытьФорму("Отчет.РасшифровкаНачисленийПремийПродавцам.Форма", ПараметрыФормыОтчетаРасшифровки, ЭтотОбъект);
...
Процедура ЗаполнитьЗаймыСотрудникам()
	...
	Запрос.УстановитьПараметр("ВидДоговораДоговорЗаймаСотруднику",
	...
	Пока Выборка.Следующий() Цикл
		...
		НачисленоПроцентов = (Выборка.СуммаПроцентовГрафик + Выборка.СуммаКомиссииГрафик)
			- (Выборка.ПроцентыВалНачислено + Выборка.КомиссияВалНачислено);
		НачисленоПроцентов = ?(НачисленоПроцентов < 0, 0, НачисленоПроцентов);

		ПогашеноПроцентов = (Выборка.СуммаПроцентовГрафик + Выборка.СуммаКомиссииГрафик) - (Выборка.ПроцентыВалПогашено
			+ Выборка.КомиссияВалПогашено);
