...
Процедура ЗаполнитьПоСчетуНаОплату(ДанныеЗаполнения, НомерСтроки = Неопределено, Сумма = Неопределено) Экспорт
	...
	Если Сумма <> Неопределено Тогда
		...
		|				ИЛИ ШапкаДокумента.ДокументОснование = ЗНАЧЕНИЕ(Документ.ОтчетКомиссионера.ПустаяСсылка)
		...
		|					ИЛИ ТИПЗНАЧЕНИЯ(ШапкаДокумента.ДокументОснование) = ТИП(Документ.ОтчетКомиссионера)
		|					ИЛИ ТИПЗНАЧЕНИЯ(ШапкаДокумента.ДокументОснование) = ТИП(Документ.ЗаказПокупателя)
		|						И ШапкаДокумента.ДокументОснование.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаряд))
	...
	Иначе
		...
		|				ИЛИ ТаблицаДокумента.Ссылка.ДокументОснование = ЗНАЧЕНИЕ(Документ.ОтчетКомиссионера.ПустаяСсылка)
		...
		|					ИЛИ ТИПЗНАЧЕНИЯ(ТаблицаДокумента.Ссылка.ДокументОснование) = ТИП(Документ.ОтчетКомиссионера)
		|					ИЛИ ТИПЗНАЧЕНИЯ(ТаблицаДокумента.Ссылка.ДокументОснование) = ТИП(Документ.ЗаказПокупателя)
		|						И ТаблицаДокумента.Ссылка.ДокументОснование.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаряд)
		|						И ВидыЗаказНарядовПорядокСостояний.НомерСтроки >= ВЫРАЗИТЬ(ТаблицаДокумента.Ссылка.ДокументОснование.ВидЗаказа КАК Справочник.ВидыЗаказНарядов).НомерСостоянияВыполнения)
		...
		|	ВЫРАЗИТЬ(ТаблицаДокумента.Ссылка.Контрагент.НаименованиеПолное КАК СТРОКА(1024)) КАК ПринятоОт,
...
Процедура ЗаполнитьПоРасходнойНакладной(ДанныеЗаполнения) Экспорт
	...
	|	ШапкаДокумента.Патент КАК Патент,
	...
	|	ВЫБОР
	|		КОГДА ШапкаДокумента.ПоложениеПроекта = ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
	|			ТОГДА ТаблицаДокумента.Проект
	|		ИНАЧЕ ШапкаДокумента.Проект
	|	КОНЕЦ КАК Проект
	|ИЗ
...
Процедура ЗаполнитьПоПередачеТоваровМеждуОрганизациями(ДанныеЗаполнения) Экспорт
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|" + РасчетыСлужебный.ТекстЗапросаДляУчетаОплатыДругимиДокументамиСОтборомПоОрганизации() + "
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Процедура ЗаполнитьПоОтчетуОРозничныхПродажах(ДанныеЗаполнения) Экспорт
	...
	Если НовыйМеханизмИнкассации
		И (ДанныеЗаполнения.КассаККМ.ТипКассы = Перечисления.ТипыКассККМ.ФискальныйРегистратор
		ИЛИ ДанныеЗаполнения.КассаККМ.ТипКассы = Перечисления.ТипыКассККМ.ККМED) Тогда
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.СуммаПлатежа - СуммаОсталосьРаспределить > 0 Тогда
			...
			СуммаДокумента = Выборка.СуммаПлатежа - СуммаОсталосьРаспределить;
			СуммаОсталосьРаспределить = 0;
		Иначе
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.СуммаПлатежа - СуммаОсталосьРаспределить > 0 Тогда
				...
				НоваяСтрока.СуммаПлатежа = Выборка.СуммаПлатежа - СуммаОсталосьРаспределить;
				НоваяСтрока.СуммаНДС = НоваяСтрока.СуммаПлатежа - (НоваяСтрока.СуммаПлатежа) / ((НоваяСтрока.СтавкаНДС.Ставка + 100) / 100);
				СуммаДокумента = СуммаДокумента + Выборка.СуммаПлатежа - СуммаОсталосьРаспределить;
				СуммаОсталосьРаспределить = 0;
			Иначе
				СуммаОсталосьРаспределить = СуммаОсталосьРаспределить - Выборка.СуммаПлатежа;
			КонецЕсли;
...
Процедура ЗаполнитьПоОтчетуКомиссионера(ДокументОснование) Экспорт
	...
	|	2,
	|	ОтчетКомиссионераЗапасыВозвраты.Всего
	|ИЗ
	...
	|	ВТСводнаяТаблицаЗапасыИтог.Заказ КАК Заказ,
	...
	|				ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ВТСводнаяТаблицаЗапасыИтог.Всего * ВЫБОР
	...
	|								ТОГДА ВТСводнаяТаблицаЗапасыИтог.СуммаВознаграждения * ЕСТЬNULL(ВЫРАЗИТЬ(ВЫБОР
	|												КОГДА ОтчетКомиссионера.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|													ТОГДА КурсыРегВалют.Курс * ОтчетКомиссионера.Кратность / (ОтчетКомиссионера.Курс * ЕСТЬNULL(КурсыРегВалют.Кратность, 1))
	|												ИНАЧЕ 1
	...
	|			КОГДА ОтчетКомиссионера.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионера)
	|					ИЛИ ВТСводнаяТаблицаЗапасыИтог.ВидОперации = 1
	|				ТОГДА ЕСТЬNULL(ВТСводнаяТаблицаЗапасыИтог.Всего - ВЫБОР
	|							КОГДА ОтчетКомиссионера.УдержатьКомиссионноеВознаграждение
	...
	|			КОГДА ОтчетКомиссионера.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионера)
	|					ИЛИ ВТСводнаяТаблицаЗапасыИтог.ВидОперации = 1
	|				ТОГДА ЕСТЬNULL(ВТСводнаяТаблицаЗапасыИтог.СуммаНДС, 0)
	|			КОГДА ОтчетКомиссионера.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионераОВозвратах)
	|					ИЛИ ВТСводнаяТаблицаЗапасыИтог.ВидОперации = 2
	|				ТОГДА ВЫБОР
	|						КОГДА НЕ ОтчетКомиссионера.УдержатьКомиссионноеВознаграждение
	...
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВТСводнаяТаблицаЗапасыИтог.Заказ,
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ДвижениеДенежныхСредствСервер.ДобавитьПервуюСтрокуРасшифровки(ЭтотОбъект);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИсключаяСвойства = ИсключаяСвойства + ", Курс, Кратность";
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ПокупкаВалюты
		...
		Если ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ПоступлениеОтПродажиВалюты Тогда
			ПроверяемыеРеквизиты.Добавить("ПродажаВалютыВалюта");
			ПроверяемыеРеквизиты.Добавить("ПродажаВалютыКратность");
			ПроверяемыеРеквизиты.Добавить("ПродажаВалютыКурс");
		КонецЕсли;
...
Функция ТипРасчетаДляФискализации() Экспорт
	...
	Если ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПоставщика Тогда
