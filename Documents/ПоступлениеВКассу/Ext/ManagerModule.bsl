...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.ПоступлениеВКассу.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
...
Процедура СформироватьТаблицаДенежныеСредства(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	|	ИЛИ ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.ПоступлениеОтПродажиВалюты))
	|	И &ОстатокСуммы > 0
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ИЛИ ВременнаяТаблицаРасшифровкаПлатежа.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.ПоступлениеОтПродажиВалюты))
	...
	|	ВременнаяТаблицаШапка КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.ПоступлениеОтПродажиВалюты)
	|	И ТаблицаДокумента.РасчетКурсовойРазницыПокупкаПродажаВалюты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура СформироватьТаблицаДенежныеСредстваКПоступлению(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	ОснованиеЗаполнено = ЗначениеЗаполнено(ДокументСсылкаПоступлениеВКассу.ДокументОснование) 
						И (ТипЗнч(ДокументСсылкаПоступлениеВКассу.ДокументОснование) = Тип("ДокументСсылка.ВыемкаНаличных") 
						ИЛИ ТипЗнч(ДокументСсылкаПоступлениеВКассу.ДокументОснование) = Тип("ДокументСсылка.РасходИзКассы"));
	
	Запрос.УстановитьПараметр("ОснованиеЗаполнено", ОснованиеЗаполнено);
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СодержаниеРозничнаяВыручка", СодержанияПроводок.РозничнаяВыручка());
	Запрос.УстановитьПараметр("СодержаниеСебестоимость", СодержанияПроводок.Себестоимость());
	Запрос.УстановитьПараметр("СодержаниеЛичныеСредства", СодержанияПроводок.ЛичныеСредства());
	Запрос.УстановитьПараметр("СодержаниеНаценка", СодержанияПроводок.Наценка());
	...
	|			КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.ПоступлениеОтПродажиВалюты)
...
Процедура СформироватьТаблицаДоходыИРасходыКассовыйМетод(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	|ГДЕ
	|	&КассовыйМетодУчетаДоходовИРасходов
	|	И ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.ПоступлениеОтПродажиВалюты)
	|	И &ОстатокСуммы > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура СформироватьТаблицаОплатаДокументов(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	МассивДоступныхОпераций.Добавить(Перечисления.ВидыОперацийПоступлениеВКассу.ОтПоставщика);
...
Функция ПолучитьТекстЗапросаИнициализироватьДанные()
	...
	|		КОГДА ТаблицаДокумента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.ПокупкаВалюты)
	|			ТОГДА ТаблицаДокумента.Ссылка.ВалютаДенежныхСредств
	|		КОГДА ТаблицаДокумента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.ПоступлениеОтПродажиВалюты)
	...
	|						ТОГДА ЛОЖЬ
	|					КОГДА КурсыВалютКассы.Кратность = 0
	|						ТОГДА ЛОЖЬ
	|					КОГДА (ВЫРАЗИТЬ(ТаблицаДокумента.СуммаУчета * (ТаблицаДокумента.Курс / ТаблицаДокумента.Кратность - КурсыВалютКассы.Курс / КурсыВалютКассы.Кратность) КАК ЧИСЛО(15, 2))) = 0
	|						ТОГДА ЛОЖЬ
	|					ИНАЧЕ ТаблицаДокумента.Ссылка.РасчетКурсовойРазницыПокупкаПродажаВалюты
	|				КОНЕЦ
	|		ИНАЧЕ ТаблицаДокумента.Ссылка.РасчетКурсовойРазницыПокупкаПродажаВалюты
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства) Экспорт
	...
	Если ДокументСсылкаПоступлениеВКассу.ВидОперации =
			ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.ПоступлениеОтПродажиВалюты") Тогда
		Запрос.УстановитьПараметр("ПродажаВалютыВалюта", ДокументСсылкаПоступлениеВКассу.ПродажаВалютыВалюта);
	Иначе	
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		|				ИЛИ ПоступлениеВКассу.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.ПоступлениеОтПродажиВалюты)
...
Процедура СформироватьТаблицаРасчетыПоКредитамИЗаймам(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("УчетРасчетовПоКредитамИЗаймам", СодержанияПроводок.ПоступлениеПоКредиту());
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаПоступлениеВКассу);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("КурсоваяРазница", СодержанияПроводок.КурсоваяРазница());
