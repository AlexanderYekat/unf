...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаЗапасы КАК ТаблицаУправленческий
	|ГДЕ
	|	ТаблицаУправленческий.СуммаНДСВознаграждения <> 0
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаУправленческий.УдержатьКомиссионноеВознаграждение
	|						ТОГДА ТаблицаУправленческий.СуммаВал - ТаблицаУправленческий.СебестоимостьВал + ТаблицаУправленческий.СуммаВознагражденияВал
	|					ИНАЧЕ ТаблицаУправленческий.СуммаВал - ТаблицаУправленческий.СебестоимостьВал
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ТаблицаУправленческий.Сумма - ТаблицаУправленческий.Себестоимость + ТаблицаУправленческий.СуммаВознаграждения,
	...
	Запрос.УстановитьПараметр("ЗадолженностьКомитенту", СодержанияПроводок.ЗадолженностьКомитенту());
...
Процедура СформироватьТаблицаЗапасыПринятые(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПриемЗапасовОтложеннаяВыручка", СодержанияПроводок.ПриемЗапасов());
	Запрос.УстановитьПараметр("АгентскаяУслуга", СодержанияПроводок.ОтчетОРеализацииАгентскойУслуги());
...
Процедура СформироватьТаблицаПродажи(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства)
	...
	|	И НЕ ТаблицаПродажи.УдержатьКомиссионноеВознаграждение
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаПродажи.Период,
	|	ТаблицаПродажи.Организация,
...
Процедура СформироватьТаблицаВыпускПродукции(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства)
	...
	Запрос.Параметры.СуммаВознагражденияБольшеНоля = Мин(Запрос.Параметры.СуммаВознагражденияБольшеНоля, НЕ ДокументСсылкаОтчетКомитенту.УдержатьКомиссионноеВознаграждение);
	Запрос.Текст =
	"ВЫБРАТЬ
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства)
	...
	|	0,
	|	СУММА(ТаблицаДоходыИРасходы.СуммаНДСВознаграждения),
	|	СУММА(ТаблицаДоходыИРасходы.СуммаНДСВознаграждения)
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаДоходыИРасходы
	|ГДЕ
	|	ТаблицаДоходыИРасходы.СуммаНДСВознаграждения <> 0
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.СуммаНДСВознаграждения КАК Сумма,
	|	&НачислениеНДСПродажи СодержаниеПроводки
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.СуммаНДСВознаграждения <> 0
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ОтчетКомитентуЗапасы.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.Продажи_НДС)
	|		ИНАЧЕ ОтчетКомитентуЗапасы.Номенклатура.СчетУчетаНДСПоРеализации
	...
	|	ОтчетКомитентуЗапасы.Ссылка.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	...
	|			КОГДА ОтчетКомитентуЗапасы.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|				ТОГДА ОтчетКомитентуЗапасы.Сумма * КурсыРегВалюты.Курс * ОтчетКомитентуЗапасы.Ссылка.Кратность / (ОтчетКомитентуЗапасы.Ссылка.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ОтчетКомитентуЗапасы.Сумма
	...
	СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаОтчетКомитенту, СтруктураДополнительныеСвойства);
	// Серии номенклатуры
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|				И НЕ ОтчетКомитенту.Организация = ОтчетКомитенту.Организация.ГоловнаяОрганизация
