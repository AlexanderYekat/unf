...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда	
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СчетФактураВТабличнойЧастиДокумента", "Видимость", Истина);
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСчетФактураНомерДата", "Видимость"
		, НЕ Объект.СчетФактураВТабличнойЧастиДокумента И Элементы.СчетФактураНадпись.Видимость И Не ЗначениеЗаполнено(КэшЗначений.СчетФактураСсылка));
		Элементы.ГруппаДанныеКонтрагента.Видимость = Ложь;
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СчетФактураВТабличнойЧастиДокумента", "Видимость", Ложь);
...
Процедура СчетФактураВТабличнойЧастиДокументаПриИзменении(Элемент)
	...
	Если Объект.СчетФактураВТабличнойЧастиДокумента Тогда
		СчетФактураСсылка = ПолучитьПодчиненныйСчетФактуру();
		Если Не СчетФактураСсылка = Неопределено Тогда
			СтрокаСообщения = НСтр("ru = 'Для документа введена счет-фактура. Установка флага невозможна.'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(СтрокаСообщения);
			Объект.СчетФактураВТабличнойЧастиДокумента = Ложь;
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не Объект.СчетФактураВТабличнойЧастиДокумента Тогда
		...
		Если ЕстьСчетФактурыВСтроках Тогда
			СтрокаСообщения = НСтр("ru = 'В строках табличных частей введены счет фактуры. Установка флага невозможна.'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(СтрокаСообщения);
