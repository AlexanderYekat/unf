...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	|	И ЗначениеРазрешено(Оплаты.Контрагент, NULL КАК ИСТИНА)";
...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаШапка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС)
	|			ТОГДА ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНДСЗакупки
	|		ИНАЧЕ ТаблицаДокумента.Сумма
	|	КОНЕЦ КАК СуммаБезНДС,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаШапка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС)
	|				И ВременнаяТаблицаШапка.НДСВключатьВСтоимость
...
Процедура СформироватьТаблицаЗапасыНаСкладах(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|ГДЕ
	|	НЕ (АвансовыйОтчетЗапасы.ОрдерныйСклад И АвансовыйОтчетЗапасы.ОрдерныйСкладКПоступлению)
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
...
Процедура СформироватьТаблицаЗапасыКПоступлениюНаСклады(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	АвансовыйОтчетЗапасы.Партия КАК Партия,
	|	АвансовыйОтчетЗапасы.Количество КАК Количество,
	|	АвансовыйОтчетЗапасы.ДокументОснование КАК ДокументОснование,
	|	АвансовыйОтчетЗапасы.ЯчейкаКПоступлению КАК Ячейка
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК АвансовыйОтчетЗапасы
	|ГДЕ
	|	АвансовыйОтчетЗапасы.ОрдерныйСклад И АвансовыйОтчетЗапасы.ОрдерныйСкладКПоступлению
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаОплаты.Курс КАК Курс,
	|	ВременнаяТаблицаОплаты.Кратность КАК Кратность
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВременнаяТаблицаОплаты.Курс,
	|	ВременнаяТаблицаОплаты.Кратность
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	Контрагент,
...
Процедура СформироватьТаблицаДоходыИРасходыОтложенные(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	ТаблицаЗапасыДоходыИРасходыОтложенные = РезультатЗапроса.Выгрузить();
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ОприходованиеЗапасов", СодержанияПроводок.ОприходованиеЗапасов());
	Запрос.УстановитьПараметр("ОплатаПоставщику", СодержанияПроводок.ОплатаПоставщику());
	Запрос.УстановитьПараметр("ПрочиеРасходы", СодержанияПроводок.ПрочиеРасходы());
	...
	|					ИНАЧЕ ТаблицаДокумента.СуммаВал - ТаблицаДокумента.СуммаНДСВал
	|				КОНЕЦ
	|		ИНАЧЕ 0
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ТаблицаУправленческий.СчетРасчетовСПодотчетниками,
	|	ВЫБОР
	|		КОГДА ТаблицаУправленческий.СчетРасчетовСПодотчетниками.Валютный
	|			ТОГДА ТаблицаУправленческий.Валюта
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВЫБОР
...
Процедура СформироватьТаблицаОплатаДокументов(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	РасчетыПроведениеДокументов.СформироватьТаблицаОплатаДокументовДенежныеДокументы(ДокументСсылка, СтруктураДополнительныеСвойства, Неопределено, Истина);
...
Процедура СформироватьТаблицаЗакупкиДляКУДиР(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ВременнаяТаблицаОплаты.Период < &ДатаПереходаНаНДС20
	|						ТОГДА 18
	|					ИНАЧЕ 20
	|				КОНЕЦ
	|		ИНАЧЕ 0
	...
	ТаблицаРезультат = РезультатЗапроса[7].Выгрузить();
	...
	ВыборкаЗапасовИРасходов = РезультатЗапроса[6].Выбрать();
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства) Экспорт
	...
	|	АвансовыйОтчетЗапасы.СчетУчетаНДСПоПриобретеннымЦенностям КАК СчетУчетаНДСПоПриобретеннымЦенностям,
	|	АвансовыйОтчетЗапасы.СчетУчетаНДСПоПриобретеннымЦенностямВалютный КАК СчетУчетаНДСПоПриобретеннымЦенностямВалютный,
	|	АвансовыйОтчетЗапасы.Номенклатура КАК Номенклатура,
	...
	|	&Ссылка КАК Документ,
	|	АвансовыйОтчетЗапасы.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	|	ВЫБОР
	|		КОГДА АвансовыйОтчетЗапасы.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА АвансовыйОтчетЗапасы.Ссылка
	|		КОГДА АвансовыйОтчетЗапасы.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА АвансовыйОтчетЗапасы.ДокументПоступления
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументОснование,
	|	ВЫБОР
	|		КОГДА АвансовыйОтчетЗапасы.СтруктурнаяЕдиница.ОрдерныйСклад
	|				И АвансовыйОтчетЗапасы.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриПоступлении
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОрдерныйСкладКПоступлению,
	|	ВЫБОР
	|		КОГДА АвансовыйОтчетЗапасы.СтруктурнаяЕдиница.УчетОстатковПоЯчейкам
	|			ТОГДА АвансовыйОтчетЗапасы.Ячейка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|		ВЫБОР
	|			КОГДА АвансовыйОтчетЗапасы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|				ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.НДСПоПриобретеннымЦенностям)
	|			ИНАЧЕ АвансовыйОтчетЗапасы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям
	|		КОНЕЦ КАК СчетУчетаНДСПоПриобретеннымЦенностям,
	|		ВЫБОР
	|			КОГДА АвансовыйОтчетЗапасы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ АвансовыйОтчетЗапасы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям.Валютный
	...
	|		АвансовыйОтчетЗапасы.ТоварыДляПроизводства КАК ТоварыДляПроизводства,
	|		АвансовыйОтчетЗапасы.Ссылка.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	|		&ОприходованиеЗапасов КАК СодержаниеПроводки,
	|		АвансовыйОтчетЗапасы.Ссылка КАК Ссылка,
	|		АвансовыйОтчетЗапасы.ДокументПоступления КАК ДокументПоступления
	|	ИЗ
	...
	|	АвансовыйОтчетРасходы.СчетУчетаНДСПоПриобретеннымЦенностям КАК СчетУчетаНДСПоПриобретеннымЦенностям,
	|	АвансовыйОтчетРасходы.СчетУчетаНДСПоПриобретеннымЦенностямВалютный КАК СчетУчетаНДСПоПриобретеннымЦенностямВалютный,
	...
	|	ИСТИНА КАК ФиксированнаяСтоимость,
	|	&Ссылка КАК Документ,
	|	АвансовыйОтчетРасходы.Проект КАК Проект,
	|	АвансовыйОтчетРасходы.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость
	...
	|		ВЫБОР
	|			КОГДА АвансовыйОтчетРасходы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|				ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.НДСПоПриобретеннымЦенностям)
	|			ИНАЧЕ АвансовыйОтчетРасходы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям
	|		КОНЕЦ КАК СчетУчетаНДСПоПриобретеннымЦенностям,
	|		ВЫБОР
	|			КОГДА АвансовыйОтчетРасходы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ АвансовыйОтчетРасходы.Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям.Валютный
	|		КОНЕЦ КАК СчетУчетаНДСПоПриобретеннымЦенностямВалютный,
	|		АвансовыйОтчетРасходы.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|		АвансовыйОтчетРасходы.Номенклатура КАК Номенклатура,
	|		АвансовыйОтчетРасходы.СтавкаНДС КАК СтавкаНДС,
	...
	|		ВЫБОР
	|			КОГДА АвансовыйОтчетРасходы.Ссылка.ПоложениеПроекта <> ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
	|				ТОГДА АвансовыйОтчетРасходы.Ссылка.Проект
	|			ИНАЧЕ АвансовыйОтчетРасходы.Проект
	|		КОНЕЦ КАК Проект,
	|		АвансовыйОтчетРасходы.Ссылка.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость
	|	ИЗ
	...
	|	АвансовыйОтчет.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость
...
Процедура ВыполнитьКонтроль(ДокументСсылкаАвансовыйОтчет, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияРасчетыСПодотчетникамиИзменение
	 ...
	 Или СтруктураВременныеТаблицы.ДвиженияЗапасыКПоступлениюНаСкладыИзменение Тогда
		...
		Если НЕ МассивРезультатов[0].Пустой() Тогда
			...
			КонтрольОстатковУНФ.ЗапасыНаСкладахСписком(ДокументОбъектАвансовыйОтчет, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		
		// Отрицательный остаток учета запасов и затрат.
		Если НЕ МассивРезультатов[1].Пустой() Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[1].Выбрать();
			КонтрольОстатковУНФ.ЗапасыСписком(ДокументОбъектАвансовыйОтчет, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		
		// Отрицательный остаток по расчетам с подотчетными лицами.
		Если НЕ МассивРезультатов[2].Пустой() Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[2].Выбрать();
			КонтрольОстатковУНФ.РасчетыСПодотчетниками(ДокументОбъектАвансовыйОтчет, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		
		// Отрицательный остаток по расчетам с поставщиками.
		Если НЕ МассивРезультатов[3].Пустой() Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[3].Выбрать();
			КонтрольОстатковУНФ.РасчетыСПоставщиками(ДокументОбъектАвансовыйОтчет, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		...
		Если НЕ МассивРезультатов[4].Пустой() Тогда
			...
			КонтрольОстатковУНФ.ЗапасыКПоступлению(ДокументОбъектАвансовыйОтчет, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Функция ДанныеПолученныхДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	"ВЫБРАТЬ
	|	АвансовыйОтчетЗапасы.Ссылка КАК Ссылка,
	|	МАКСИМУМ(АвансовыйОтчетЗапасы.НомерСтроки) КАК МаксимальныйНомерСтрокиЗапасов
	|ПОМЕСТИТЬ ЗанятыеНомераСтрок
	|ИЗ
	|	Документ.АвансовыйОтчет.Запасы КАК АвансовыйОтчетЗапасы
	|ГДЕ
	|	АвансовыйОтчетЗапасы.Ссылка В(&МассивОбъектов)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВЫБОР
	|		КОГДА АвансовыйОтчет.СчетФактураКонтрагент.ГоловнойКонтрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА АвансовыйОтчет.СчетФактураКонтрагент.ГоловнойКонтрагент
	|		ИНАЧЕ АвансовыйОтчет.СчетФактураКонтрагент
	|	КОНЕЦ КАК Организация,
	|	АвансовыйОтчет.СчетФактураКонтрагент КАК ОбособленноеПодразделениеПоставщика,
	|	АвансовыйОтчет.СчетФактураКонтрагент КАК Грузоотправитель,
	...
	|	ВЫБОР
	|		КОГДА АвансовыйОтчет.СчетФактураКонтрагент.ВидКонтрагента = ЗНАЧЕНИЕ(Перечисление.ВидыКонтрагентов.ФизическоеЛицо)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоФизическоеЛицо,
	|	ВЫБОР
	|		КОГДА АвансовыйОтчет.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|			ТОГДА 2
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК СтатусУПД,
	|	ЛОЖЬ КАК ЭтоСводныйСчетФактура,
	|	ЛОЖЬ КАК ЭтоКорректировка,
	|	АвансовыйОтчет.СчетФактураКонтрагент.БанковскийСчетПоУмолчанию КАК БанковскийСчет,
	|	НЕОПРЕДЕЛЕНО КАК ПредставлениеПодразделения,
	|	НЕОПРЕДЕЛЕНО КАК ПредставлениеСкладаСписания,
	|	ВЫБОР
	|		КОГДА АвансовыйОтчет.Организация.ГоловнаяОрганизация <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|				И НЕ АвансовыйОтчет.Организация = АвансовыйОтчет.Организация.ГоловнаяОрганизация
	|			ТОГДА АвансовыйОтчет.Организация.ГоловнаяОрганизация
	|		ИНАЧЕ АвансовыйОтчет.Организация
	|	КОНЕЦ КАК Контрагент,
	|	АвансовыйОтчет.Организация КАК ОбособленноеПодразделениеПокупателя,
	|	АвансовыйОтчет.Организация КАК Грузополучатель,
	|	АвансовыйОтчет.Организация.БанковскийСчетПоУмолчанию КАК БанковскийСчетКонтрагента,
	|	НЕОПРЕДЕЛЕНО КАК АдресДоставки,
	|	АвансовыйОтчет.ПодписьРуководителя.РасшифровкаПодписи КАК РасшифровкаПодписиКонтрагента,
	|	НЕОПРЕДЕЛЕНО КАК ДоверенностьНомер,
	|	НЕОПРЕДЕЛЕНО КАК ДоверенностьДата,
	|	НЕОПРЕДЕЛЕНО КАК ДоверенностьВыдана,
	|	НЕОПРЕДЕЛЕНО КАК ДоверенностьЛицо,
	|	АвансовыйОтчет.СчетФактураДоговорКонтрагента.Представление КАК Основание,
	|	АвансовыйОтчет.СчетФактураДоговорКонтрагента.Представление КАК ПредставлениеОснования,
	|	АвансовыйОтчет.СчетФактураДоговорКонтрагента КАК ОснованиеПечатиСсылка,
	|	ВЫБОР
	|		КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|			ТОГДА НациональнаяВалюта.Значение
	|		ИНАЧЕ АвансовыйОтчет.ВалютаДокумента
	|	КОНЕЦ КАК ВалютаДокумента,
	|	ВЫБОР
	|		КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|			ТОГДА НациональнаяВалюта.Значение.НаименованиеПолное
	|		ИНАЧЕ АвансовыйОтчет.ВалютаДокумента.НаименованиеПолное
	|	КОНЕЦ КАК ВалютаНаименование,
	|	ВЫБОР
	|		КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|			ТОГДА НациональнаяВалюта.Значение.Код
	|		ИНАЧЕ АвансовыйОтчет.ВалютаДокумента.Код
	|	КОНЕЦ КАК ВалютаКод,
	|	АвансовыйОтчет.СуммаВключаетНДС КАК СуммаВключаетНДС,
	|	АвансовыйОтчет.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	|	АвансовыйОтчет.Курс КАК Курс,
	|	АвансовыйОтчет.Кратность КАК Кратность,
	|	АвансовыйОтчет.СчетФактураДоговорКонтрагента.НомерДоговора КАК ОснованиеНомер,
	|	АвансовыйОтчет.СчетФактураДоговорКонтрагента.ДатаДоговора КАК ОснованиеДата,
	...
	|	ЕСТЬNULL(ЗанятыеНомераСтрок.МаксимальныйНомерСтрокиЗапасов, 0) КАК МаксимальныйНомерСтрокиЗапасов,
	...
	|		ВЫБОР
	|			КОГДА АвансовыйОтчет.Запасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	...
	|					И АвансовыйОтчет.Запасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	|				ТОГДА АвансовыйОтчет.Запасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору.Наименование
	|			ИНАЧЕ АвансовыйОтчет.Запасы.Номенклатура.ЕдиницаИзмерения.Наименование
	|		КОНЕЦ КАК ЕдиницаИзмеренияПоОКЕИ_Наименование,
	|		ВЫБОР
	|			КОГДА АвансовыйОтчет.Запасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|					И ПечатьПользовательскихЕдиницИзмерения.Значение = ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|					И АвансовыйОтчет.Запасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	|				ТОГДА АвансовыйОтчет.Запасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору.Код
	|			ИНАЧЕ АвансовыйОтчет.Запасы.Номенклатура.ЕдиницаИзмерения.Код
	|		КОНЕЦ КАК ЕдиницаИзмеренияПоОКЕИ_Код,
	|		ВЫБОР
	|			КОГДА АвансовыйОтчет.Запасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|					И ПечатьПользовательскихЕдиницИзмерения.Значение = ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|					И АвансовыйОтчет.Запасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	|				ТОГДА АвансовыйОтчет.Запасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору
	|			ИНАЧЕ АвансовыйОтчет.Запасы.Номенклатура.ЕдиницаИзмерения
	...
	|					И АвансовыйОтчет.Запасы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	|				ТОГДА 1
	|			КОГДА АвансовыйОтчет.Запасы.ЕдиницаИзмерения ССЫЛКА Справочник.КлассификаторЕдиницИзмерения
	|				ТОГДА 1
	|			ИНАЧЕ АвансовыйОтчет.Запасы.ЕдиницаИзмерения.Коэффициент
	|		КОНЕЦ КАК КоличествоВОдномМесте,
	|		ВЫБОР
	|			КОГДА АвансовыйОтчет.Запасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|					И ПечатьПользовательскихЕдиницИзмерения.Значение <> ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|				ТОГДА АвансовыйОтчет.Запасы.ЕдиницаИзмерения.Коэффициент
	|			ИНАЧЕ 1
	...
	|		ВЫБОР
	|			КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|					ИЛИ АвансовыйОтчет.Запасы.Ссылка.СчетФактураДоговорКонтрагента.РасчетыВУсловныхЕдиницах = ИСТИНА
	|						И АвансовыйОтчет.Запасы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(АвансовыйОтчет.Запасы.Цена * АвансовыйОтчет.Курс / АвансовыйОтчет.Кратность КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ АвансовыйОтчет.Запасы.Цена
	|		КОНЕЦ КАК Цена,
	|		ВЫБОР
	|			КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|					ИЛИ АвансовыйОтчет.Запасы.Ссылка.СчетФактураДоговорКонтрагента.РасчетыВУсловныхЕдиницах = ИСТИНА
	|						И АвансовыйОтчет.Запасы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(АвансовыйОтчет.Запасы.Сумма * АвансовыйОтчет.Курс / АвансовыйОтчет.Кратность КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ АвансовыйОтчет.Запасы.Сумма
	|		КОНЕЦ КАК Сумма,
	|		ВЫБОР
	|			КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|					ИЛИ АвансовыйОтчет.Запасы.Ссылка.СчетФактураДоговорКонтрагента.РасчетыВУсловныхЕдиницах = ИСТИНА
	|						И АвансовыйОтчет.Запасы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(АвансовыйОтчет.Запасы.СуммаНДС * АвансовыйОтчет.Курс / АвансовыйОтчет.Кратность КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ АвансовыйОтчет.Запасы.СуммаНДС
	|		КОНЕЦ КАК СуммаНДС,
	|		ВЫБОР
	|			КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|					ИЛИ АвансовыйОтчет.Запасы.Ссылка.СчетФактураДоговорКонтрагента.РасчетыВУсловныхЕдиницах = ИСТИНА
	|						И АвансовыйОтчет.Запасы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(АвансовыйОтчет.Запасы.Всего * АвансовыйОтчет.Курс / АвансовыйОтчет.Кратность КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ АвансовыйОтчет.Запасы.Всего
	|		КОНЕЦ КАК Всего,
	|		Ссылка КАК Ссылка,
	...
	|		ВЫБОР
	|			КОГДА АвансовыйОтчет.Расходы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|					И ПечатьПользовательскихЕдиницИзмерения.Значение = ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|					И АвансовыйОтчет.Расходы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	|				ТОГДА АвансовыйОтчет.Расходы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору.Наименование
	|			ИНАЧЕ АвансовыйОтчет.Расходы.Номенклатура.ЕдиницаИзмерения.Наименование
	|		КОНЕЦ КАК ЕдиницаИзмеренияПоОКЕИ_Наименование,
	|		ВЫБОР
	|			КОГДА АвансовыйОтчет.Расходы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|					И ПечатьПользовательскихЕдиницИзмерения.Значение = ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|					И АвансовыйОтчет.Расходы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	|				ТОГДА АвансовыйОтчет.Расходы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору.Код
	|			ИНАЧЕ АвансовыйОтчет.Расходы.Номенклатура.ЕдиницаИзмерения.Код
	|		КОНЕЦ КАК ЕдиницаИзмеренияПоОКЕИ_Код,
	|		ВЫБОР
	|			КОГДА АвансовыйОтчет.Расходы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|					И ПечатьПользовательскихЕдиницИзмерения.Значение = ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|					И АвансовыйОтчет.Расходы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)
	|				ТОГДА АвансовыйОтчет.Расходы.ЕдиницаИзмерения.ЕдиницаИзмеренияПоКлассификатору
	|			ИНАЧЕ АвансовыйОтчет.Расходы.Номенклатура.ЕдиницаИзмерения
	|		КОНЕЦ КАК ЕдиницаИзмерения,
	|		ВЫБОР
	|			КОГДА АвансовыйОтчет.Расходы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|					И ПечатьПользовательскихЕдиницИзмерения.Значение <> ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
	|				ТОГДА АвансовыйОтчет.Расходы.ЕдиницаИзмерения.Коэффициент
	|			ИНАЧЕ 1
	|		КОНЕЦ КАК КоэффициентЕдиницыИзмерения,
	...
	|		ВЫБОР
	|			КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|					ИЛИ АвансовыйОтчет.Расходы.Ссылка.СчетФактураДоговорКонтрагента.РасчетыВУсловныхЕдиницах = ИСТИНА
	|						И АвансовыйОтчет.Расходы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(АвансовыйОтчет.Расходы.Цена * АвансовыйОтчет.Курс / АвансовыйОтчет.Кратность КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ АвансовыйОтчет.Расходы.Цена
	|		КОНЕЦ КАК Цена,
	|		ВЫБОР
	|			КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|					ИЛИ АвансовыйОтчет.Расходы.Ссылка.СчетФактураДоговорКонтрагента.РасчетыВУсловныхЕдиницах = ИСТИНА
	|						И АвансовыйОтчет.Расходы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(АвансовыйОтчет.Расходы.Сумма * АвансовыйОтчет.Курс / АвансовыйОтчет.Кратность КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ АвансовыйОтчет.Расходы.Сумма
	|		КОНЕЦ КАК Сумма,
	|		ВЫБОР
	|			КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|					ИЛИ АвансовыйОтчет.Расходы.Ссылка.СчетФактураДоговорКонтрагента.РасчетыВУсловныхЕдиницах = ИСТИНА
	|						И АвансовыйОтчет.Расходы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(АвансовыйОтчет.Расходы.СуммаНДС * АвансовыйОтчет.Курс / АвансовыйОтчет.Кратность КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ АвансовыйОтчет.Расходы.СуммаНДС
	|		КОНЕЦ КАК СуммаНДС,
	|		ВЫБОР
	|			КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|					ИЛИ АвансовыйОтчет.Расходы.Ссылка.СчетФактураДоговорКонтрагента.РасчетыВУсловныхЕдиницах = ИСТИНА
	|						И АвансовыйОтчет.Расходы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(АвансовыйОтчет.Расходы.Всего * АвансовыйОтчет.Курс / АвансовыйОтчет.Кратность КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ АвансовыйОтчет.Расходы.Всего
	|		КОНЕЦ КАК Всего,
	|		Ссылка КАК Ссылка,
	|		Ссылка.Дата КАК ДатаОтгрузочногоДокумента,
	|		Ссылка.Номер КАК НомерОтгрузочногоДокумента,
	...
	|ИЗ
	|	Документ.АвансовыйОтчет КАК АвансовыйОтчет
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗанятыеНомераСтрок КАК ЗанятыеНомераСтрок
	|		ПО АвансовыйОтчет.Ссылка = ЗанятыеНомераСтрок.Ссылка,
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	АвансовыйОтчет.Запасы.НомерСтроки";
	
	ТаблицаДанныхОбъектов = Запрос.Выполнить().Выгрузить();
	
	Для каждого ДанныеОбъекта Из ТаблицаДанныхОбъектов Цикл
		
		СледующийНомерСтрокЗапасы = ДанныеОбъекта.МаксимальныйНомерСтрокиЗапасов + 1;
		Для каждого СтрокаРасхода Из ДанныеОбъекта.ТаблицаРасходы Цикл
			
			НоваяСтрока = ДанныеОбъекта.ТаблицаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаРасхода);
			
			НоваяСтрока.НомерСтроки = СледующийНомерСтрокЗапасы;
			СледующийНомерСтрокЗапасы = СледующийНомерСтрокЗапасы + 1;
			
		КонецЦикла;
		
	КонецЦикла;
	
	Возврат ТаблицаДанныхОбъектов;
	
КонецФункции

// Возвращает данные печатной формы счет-фактуры с отбором по счет-фактурам по строкам.
//
// Параметры:
//  СтруктураПараметров - структура параметров получения данных
// 
// Возвращаемое значение:
//  ТаблицаЗначений - таблица данных объектов печати.
//
Функция ДанныеПолученныхДокументовРегУчетСчетФактураВТабличнойЧасти(СтруктураПараметров) Экспорт 
	
	МассивОбъектов = СтруктураПараметров.МассивОбъектов;
	СчетФактура = СтруктураПараметров.СчетФактура;
	ИспользоватьФаксимиле = СтруктураПараметров.ИспользоватьФаксимиле;
	ПечатнаяФормаТолькоВРублях = СтруктураПараметров.ПечатнаяФормаТолькоВРублях;
	...
	|ГДЕ
	|	АвансовыйОтчет.Ссылка В(&МассивОбъектов)
	|	И (АвансовыйОтчет.Запасы.СчетФактура = &СчетФактура
	|			ИЛИ АвансовыйОтчет.Расходы.СчетФактура = &СчетФактура)
	|
	|УПОРЯДОЧИТЬ ПО
	|	АвансовыйОтчет.Запасы.НомерСтроки";
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Компания = Константы.УчетПоКомпании.Компания(Шапка.Организация);
		...
		Запрос.УстановитьПараметр("ПересчитыватьВНациональнуюВалюту",	НЕ ВИностраннойВалюте);
		...
		|	СУММА(ВЫБОР
		|			КОГДА &ПересчитыватьВНациональнуюВалюту
		...
		Запрос = Новый Запрос();
		Запрос.УстановитьПараметр("МассивАвансовыхДокументов", МассивАвансовыхДокументов);
		Запрос.УстановитьПараметр("ПересчитыватьВНациональнуюВалюту",	ВИностраннойВалюте);
		Запрос.УстановитьПараметр("Курс", 						Шапка.Курс);
		Запрос.УстановитьПараметр("Кратность", 					Шапка.Кратность);
		Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|	РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&НаДату, ФизическоеЛицо = &ФизЛицо) КАК ФИОФизическихЛиц");
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ДанныеПодотчетногоЛица.Фамилия) Тогда
				ПодотчетноеЛицо   = ФизическиеЛицаКлиентСервер.ФамилияИнициалы(ДанныеПодотчетногоЛица);
			Иначе
		...
		Израсходовано = ?(ВИностраннойВалюте, Шапка.СуммаДокументаВНацВалюте, Шапка.СуммаДокумента);
		...
		Если (НЕ ВИностраннойВалюте И Константы.ВалютаУчета.Получить() = НациональнаяВалюта)
			...
			ЗапросПоПроводкам.УстановитьПараметр("ПересчитыватьВНациональнуюВалюту",	НЕ ВИностраннойВалюте);
		...
		Пока Запасы.Следующий() Цикл
			...
			Иначе
				ОбластьМакета.Параметры.НаименованиеРасхода = ПечатьДокументовУНФ.ПредставлениеНоменклатурыДляПечати(
		...
		Пока Оплаты.Следующий() Цикл
			...
			Сумма = ?(ВИностраннойВалюте, 
						Окр(Оплаты.СуммаПлатежа * Шапка.Курс / Шапка.Кратность, 2), 
							Оплаты.СуммаПлатежа);
			ОбластьМакета.Параметры.НаименованиеРасхода = СтрШаблон(НСтр("ru = 'Оплата контрагенту %1'"),
				Оплаты.Контрагент);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АвансовыйОтчет") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АвансовыйОтчет", НСтр("ru = 'Авансовый отчет'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "АвансовыйОтчет"));
		
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АвансовыйОтчетВВалюте") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АвансовыйОтчетВВалюте", НСтр("ru = 'Авансовый отчет (упр.)'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "АвансовыйОтчетВВалюте"));
