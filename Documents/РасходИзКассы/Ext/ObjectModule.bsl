...
Процедура ЗаполнитьПоРасходДСПлан(ДокументСсылкаРасходДСПлан, Сумма = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Выборка.ДокументОснованиеЗаявки) 
		И ТипЗнч(Выборка.ДокументОснованиеЗаявки) = Тип("ДокументСсылка.НачислениеЗарплатыУНФ") Тогда
	...
	Если ПолучитьФункциональнуюОпцию("ОграничиватьДоступНаУровнеЗаписей") Тогда
		Касса = Справочники.Кассы.ПолучитьКассуСУчетомПравДоступаНаУровнеЗаписей(Выборка.Касса);
	КонецЕсли;
...
Процедура ЗаполнитьПоДополнительныеРасходы(ДокументСсылкаДополнительныеРасходы) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = РасшифровкаПлатежа.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
		СуммаДокумента = СуммаДокумента + Выборка.СуммаПлатежа;
	КонецЦикла;
	...
	ИЛИ ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.НашейОрганизации Тогда
		...
		Если Контрагент.ВестиРасчетыПоДокументам
			...
			Для Каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				...
				ИначеЕсли СтрокаРасшифровкаПлатежа.ПризнакАванса Тогда
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// БЗКБ
	ВидыОбязательногоСтрахованияСотрудников = ЕдиныйНалоговыйСчет.СооответсвияВидовНалоговВидуОбязательногоСтрахованияСотрудников();
	Если ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Зарплата
		...
		ЗарплатаКадры.ЗарегистрироватьОплатуВедомостей(
			...
			Отказ);
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Налоги Тогда
		Если (ЭтотОбъект.ВидНалога = Справочники.ВидыНалогов.НДФЛСотрудники
			ИЛИ ЭтотОбъект.ВидНалога = Справочники.ВидыНалогов.НДФЛДоходыСвышеПредельнойВеличины)
		...
		ИначеЕсли ВидыОбязательногоСтрахованияСотрудников.Получить(ЭтотОбъект.ВидНалога) <> Неопределено Тогда
