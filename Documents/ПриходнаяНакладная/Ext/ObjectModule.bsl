...
Процедура ЗаполнитьПоСтруктуре(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.Свойство("МассивЗаказовПоставщикам") Тогда
		ЗаполнитьПоЗаказуПоставщику(ДанныеЗаполнения);
	ИначеЕсли ДанныеЗаполнения.Свойство("МассивДокументовКПоступлению") Тогда
		ЗаполнитьПоМассивуПриходныхОрдеров(ДанныеЗаполнения);
	КонецЕсли;
...
Процедура ЗаполнитьПоРасходнойНакладной(ДанныеЗаполнения) Экспорт
	...
	ИначеЕсли ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером Тогда
		ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары;
	Иначе
	...
	Если ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтКомиссионера Тогда
		...
		ТаблицаОстатков = РаботаСКомиссионерамиКомитентамиСервер.ТаблицаОстатковТоваровУКомиссионера(ЭтотОбъект, , , ,
			Ложь);
		...
		ПараметрыПоиска.Вставить("ЗаказПокупателя");
	...
	// Перенос подарочных сертификатов и оплат бонусами из табличной части "Предоплата"
	Предоплата.Очистить();
	Для Каждого СтрокаТабличнойЧасти Из ДанныеЗаполнения.Предоплата Цикл
		...
		Если СтрокаТабличнойЧасти.ОплатаСертификатом Тогда
			Если СтрокаТабличнойЧасти.Документ.ЧастичноеПогашение Тогда
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТабличнойЧасти);
			КонецЕсли;
		ИначеЕсли СтрокаТабличнойЧасти.ОплатаБонусами Тогда
			НоваяСтрока = Предоплата.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТабличнойЧасти);
		КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьБонусныеПрограммы")
		...
		БонусныеБаллыКНачислению.Загрузить(ДанныеЗаполнения.БонусныеБаллыКНачислению.Выгрузить());
	КонецЕсли;
...
Процедура ЗаполнитьПоЗаказуПоставщику(ДанныеЗаполнения) Экспорт
	...
	|	ЗаказПоставщикуЗапасы.НоменклатураПоставщика
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки");
...
Процедура ЗаполнитьПоПриходномуОрдеру(ДанныеЗаполнения) Экспорт
	...
	Если ВидУчета = Перечисления.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам Тогда
		ТекстИсключения = НСтр("ru = 'Вид учета остатков на ордерном складе - по Приходным накладным. Ввод Приходной накладной на основании Складского ордера недоступен.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если ПоложениеСклада = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке И УчетОстатковПоСкладскимОрдерам Тогда
	...
	Если ЗначениеЗаполнено(Договор) Или НЕ ЗначениеЗаполнено(Контрагент) Тогда
		...
		ЦенообразованиеСервер.ПерезаполнитьЦеныТабличнойЧастиПоВидуЦенКонтрагентаПриЗаполненииНаОснованииОрдера(ЭтотОбъект);
		ЦенообразованиеСервер.ПересчитатьЦеныТабличнойЧастиДокументаОбъектаПоВалютеПриЗаполненииНаОснованииОрдера(ЭтотОбъект, ВалютаДокумента, "Запасы");
		
	КонецЕсли;
	
КонецПроцедуры // ЗаполнитьПоПриходныйОрдер()

// Процедура заполнения документа на основании массива приходных ордеров.
//
// Параметры:
//	ДанныеЗаполнения - Структура - Данные заполнения документа
//	
Процедура ЗаполнитьПоМассивуПриходныхОрдеров(ДанныеЗаполнения) Экспорт
	
	// Заполнение шапки документа.

	ПоложениеСкладаУстановлено = Ложь;
	МассивДокументов = ДанныеЗаполнения.МассивДокументовКПоступлению;
	...
	Если Не РазрешитьСкладыВТабличныхЧастях Тогда
		ПоложениеСклада = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке;
		ПоложениеСкладаУстановлено = Истина;
	КонецЕсли;
	...
	Если ВидУчета = Перечисления.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам Тогда
		ТекстИсключения = НСтр("ru = 'Вид учета остатков на ордерном складе - по Приходным накладным. Ввод Приходной накладной на основании Приходного ордера недоступен.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Договор) Или НЕ ЗначениеЗаполнено(Контрагент) Тогда
		...
		ДоговорСтруктура = Справочники.ДоговорыКонтрагентов.СтруктураДляЗаполненияПоДоговоруКонтрагента(Договор, Истина);
		ЗаполнениеОбъектовУНФ.ЗаполнитьДокумент(ЭтотОбъект, ДоговорСтруктура,, "ВидОперации,СтруктурнаяЕдиница,Организация");
		
		СкладскойУчетСервер.ЗаполнитьТабличнуюЧастьДокументаСУчетомОстатковКлючевыеРеквизитыИдентичны(ЭтотОбъект, МассивДокументов, "ПриходныйОрдер");
...
// Процедура заполнения документа на основании приходного ордера без учета остатков.
//
// Параметры:
//	ДанныеЗаполнения - Структура - Данные заполнения документа
//	
...
Процедура ЗаполнитьПоПоступлениюНаСчет(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.ВидОперации <> Перечисления.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты Тогда
		ТекстИсключения = НСтр(
			"ru = 'В поступлении на счет указана неподходящая операция для ввода приходной накладной. Приходная накладная вводится только на основании операции ""От курьерской компании или почты"".'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если СуммаВознаграждения = 0 Тогда
		ТекстИсключения = НСтр(
			"ru = 'В поступлении на счет отсутствует информация об удержаниях курьерской компанией. Для ввода приходной накладной должна быть заполнена колонка ""Удержано агентом"" расшифровки платежа.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(УслугаИзДоговора) Тогда
		ТекстИсключения = НСтр(
			"ru = 'Невозможно ввести приходную накладную. У указанного в поступлении на счет договора курьерской компании не выбрана услуга доставки.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			Договор = Справочники.ДоговорыКонтрагентов.ПоДокументуКонтрагентуОрганизацииИВидуОперации(Ссылка,
				Контрагент, Организация, ВидОперации);
		КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		ДоговорСтруктура = Справочники.ДоговорыКонтрагентов.СтруктураДляЗаполненияПоДоговоруКонтрагента(ДанныеЗаполнения, Истина);
		ЗаполнениеОбъектовУНФ.ЗаполнитьДокумент(ЭтотОбъект, ДоговорСтруктура);
	Иначе
		...
		СтратегияЗаполнения[Тип("ДокументСсылка.ПогашениеСДИЗЗЕРНО")] = "ЗаполнитьПоПогашениюСДИЗЗЕРНО";
	...
	Если (ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя 
		...
		И НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
	...
	ПодписьКладовщика = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктурнаяЕдиница, "ПодписьМОЛ");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
		...
		Если Не ПоставщикРезидентТаможенногоСоюза Тогда
			Для каждого СтрокаТабличнойЧасти Из Запасы Цикл
				Если СтрокаТабличнойЧасти.ПрослеживаемыйТовар Тогда
					СтрокаТабличнойЧасти.НомерГТД = Справочники.НомераГТД.ПустаяСсылка();
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	Иначе
	...
	ПокупателюВыставляетсяКорректировочныйСчетФактура = ?(Не ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя 
																			И Не ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары, Ложь, ПокупателюВыставляетсяКорректировочныйСчетФактура);
	
	СчетаФактурыУНФ.ПометитьНаУдалениеСчетФактурыНеверногоТипа(Ссылка, ПокупателюВыставляетсяКорректировочныйСчетФактура);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ТаблицаРасходы = Расходы.Выгрузить(, "Заказ, ЗаказПоставщику, Всего");
	ТаблицаРасходы.Свернуть("Заказ, ЗаказПоставщику", "Всего");
	
	ТаблицаПредоплата = Предоплата.Выгрузить(, "Документ, Заказ, СуммаПлатежа");
	ТаблицаПредоплата.Свернуть("Документ, Заказ", "СуммаПлатежа");
	...
	Для каждого Строка Из ТаблицаПредоплата Цикл
		...
		Если Контрагент.ВестиРасчетыПоЗаказам И Строка.Заказ <> Неопределено
			...
			Если ТипЗнч(Строка.Документ) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
				НайденнаяСтрокаРасходы = ?(НайденнаяСтрокаРасходы = Неопределено, ТаблицаРасходы.Найти(Строка.Заказ, "Заказ"), НайденнаяСтрокаРасходы);
			КонецЕсли;
	...
	Если ЭтоУниверсальныйДокумент И (ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика
		Или (ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя
		Или ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары)
		И НЕ ПокупателюВыставляетсяКорректировочныйСчетФактура) Тогда
	...
	Если ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары Тогда
...
Функция ПрослеживаемаяОперация()
	...
	Возврат (ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика
		...
		ИЛИ ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПриемНаКомиссию);
...
Процедура ЗаполнитьПоНакладнойЕГАИС(ТТНВходящаяЕГАИС) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СтруктурныеЕдиницы.ТипСтруктурнойЕдиницы, ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)) В (ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад), ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Розница), ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет))
	|			ТОГДА СтруктурныеЕдиницы.Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК СтруктурнаяЕдиница,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СтруктурныеЕдиницы.ТипСтруктурнойЕдиницы, ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)) = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
	|			ТОГДА СтруктурныеЕдиницы.Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|		ПО ТТНВходящаяЕГАИС.ТорговыйОбъект = СтруктурныеЕдиницы.Ссылка
	|ГДЕ
	...
	Если ЗначениеЗаполнено(Контрагент)
		...
		Договор = Справочники.ДоговорыКонтрагентов.ПоКонтрагентуПоОрганизации(Контрагент, Организация,
			СписокВидовДоговора);
	КонецЕсли;
...
Процедура ЗаполнитьТоварыПоступленияПоТТН(ТТНВходящаяЕГАИС) Экспорт
	...
	|			ТОГДА 1
	|		КОГДА ЕСТЬNULL(СпрНоменклатура.ОбъемДАЛ, 1) = 0
	|			ТОГДА 0
	|		ИНАЧЕ 1 / ЕСТЬNULL(СпрНоменклатура.ОбъемДАЛ, 1)
	|	КОНЕЦ КАК Количество,
	...
	|		КОГДА ЕСТЬNULL(СпрНоменклатура.ОбъемДАЛ, 1) = 0
	|			ТОГДА 1
	|		ИНАЧЕ ЕСТЬNULL(СпрНоменклатура.ОбъемДАЛ, 1)
	|	КОНЕЦ КАК Цена,
...
Процедура ЗаполнитьПоПогашениюСДИЗЗЕРНО(ДанныеЗаполнения) Экспорт
	
	ДанныеДокумента = ДанныеПрикладныхДокументовИзПогашенияСДИЗЗЕРНО(ДанныеЗаполнения);
	
	// Заполнение шапки.
	ВыборкаШапка = ДанныеДокумента.ДанныеШапки.Выбрать();
	...
	Если Не ЗначениеЗаполнено(Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Контрагент)
			И Не ЗначениеЗаполнено(Договор) Тогда
			Договор = Справочники.ДоговорыКонтрагентов.ПоКонтрагентуПоОрганизации(Контрагент, Организация,
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Перечисления.ВидыДоговоров.СПоставщиком));
		КонецЕсли;
	КонецЕсли;
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		// заполняем ставку НДС
		Если ЗначениеЗаполнено(ВыборкаТовары.ВидСтавкиНДС) Тогда
			НоваяСтрока.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ВыборкаТовары.ВидСтавкиНДС, ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
		ИначеЕсли ЗначениеЗаполнено(Организация) И ЗначениеЗаполнено(ВыборкаШапка.ВидСтавкиНДСПоУмолчанию) Тогда
			НоваяСтрока.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ВыборкаШапка.ВидСтавкиНДСПоУмолчанию, ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
		КонецЕсли;
...
Функция ДанныеПрикладныхДокументовИзПогашенияСДИЗЗЕРНО(ДокументСсылка)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПогашениеСДИЗЗерно.Организация КАК Организация,
	|	Организации.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	|	Организации.ВидСтавкиНДСПоУмолчанию КАК ВидСтавкиНДСПоУмолчанию,
	|	КлючиРеквизитовОрганизацийЗЕРНО.ОрганизацияКонтрагент КАК Контрагент,
	|	ПогашениеСДИЗЗерно.Комментарий КАК Комментарий,
	|	ПогашениеСДИЗЗерно.Дата КАК Дата,
	|	НЕ ПогашениеСДИЗЗерно.Проведен КАК ЕстьОшибкиПроведен
	|ИЗ
	...
	|		ПО ПогашениеСДИЗЗерно.Продавец = КлючиРеквизитовОрганизацийЗЕРНО.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
	|		ПО ПогашениеСДИЗЗерно.Организация = Организации.Ссылка
	|ГДЕ
	|	ПогашениеСДИЗЗерно.Ссылка = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПогашениеСДИЗЗЕРНОТовары.Номенклатура КАК Номенклатура,
	|	ПогашениеСДИЗЗЕРНОТовары.Характеристика КАК Характеристика,
	|	ПогашениеСДИЗЗЕРНОТовары.Серия КАК Серия,
	|	ПогашениеСДИЗЗЕРНОТовары.Количество КАК Количество,
	|	ПогашениеСДИЗЗЕРНОТовары.СтатусУказанияСерий КАК СтатусУказанияСерий,
	...
	|		ПО ПогашениеСДИЗЗЕРНОТовары.Номенклатура = СправочникНоменклатура.Ссылка
	|ГДЕ
	...
	Результат.Вставить("ДанныеШапки", РезультатПакета[0]);
	Результат.Вставить("Товары", РезультатПакета[1]);
