...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаПриходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ТаблицаРасчетыСПокупателями = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями;
...
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц) Экспорт
	...
	|	СУММА(РасходнаяНакладнаяЗапасы.Цена) КАК Цена,
	...
	|ИЗ
	|	РегистрНакопления.Продажи.Обороты(, &ДатаОснования, Запись, Документ = &ИсправляемыйДокументРеализации) КАК ПродажиОбороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РасходнаяНакладная.Запасы КАК РасходнаяНакладнаяЗапасы
	|		ПО ПродажиОбороты.Документ = РасходнаяНакладнаяЗапасы.Ссылка
	|			И ПродажиОбороты.Номенклатура = РасходнаяНакладнаяЗапасы.Номенклатура
	|			И ПродажиОбороты.Характеристика = РасходнаяНакладнаяЗапасы.Характеристика
	|			И ПродажиОбороты.Партия = РасходнаяНакладнаяЗапасы.Партия
	|			И (ПродажиОбороты.ЗаказПокупателя = ВЫБОР
	|				КОГДА РасходнаяНакладнаяЗапасы.Заказ = НЕОПРЕДЕЛЕНО
	|					ТОГДА ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|				ИНАЧЕ РасходнаяНакладнаяЗапасы.Заказ
	|			КОНЕЦ)
	|ГДЕ
	|	РасходнаяНакладнаяЗапасы.Ссылка = &ИсправляемыйДокументРеализации
	|	И НЕ ПродажиОбороты.Регистратор = &Регистратор
	|
	|СГРУППИРОВАТЬ ПО
	|	РасходнаяНакладнаяЗапасы.Характеристика,
	|	РасходнаяНакладнаяЗапасы.Номенклатура,
	|	РасходнаяНакладнаяЗапасы.НоменклатураНабора,
	|	РасходнаяНакладнаяЗапасы.ХарактеристикаНабора,
	...
	|	РасходнаяНакладнаяЗапасыОстатки.СуммаНДС КАК СуммаНДСДоИзмененияВалДокумента,
	|	РасходнаяНакладнаяЗапасыОстатки.СуммаНДС - ПриходнаяНакладнаяЗапасы.СуммаНДС * ВЫБОР
	|		КОГДА Реквизиты.ИспользоватьПересчет
	|			ТОГДА Реквизиты.Курс / Реквизиты.Кратность
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК СуммаНДСПослеИзменения,
	|	РасходнаяНакладнаяЗапасыОстатки.СуммаНДС - ПриходнаяНакладнаяЗапасы.СуммаНДС КАК СуммаНДСПослеИзмененияВалДокумента,
	|	ВЫБОР
	|		КОГДА НЕ СуммыДокументовРегламентированныйУчет.Всего ЕСТЬ NULL
	...
	|			И ПриходнаяНакладнаяЗапасы.КлючСвязи = СуммыДокументовРегламентированныйУчет.НомерСтрокиДокумента
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаСведенияПрослеживаемостиИтог.Ссылка КАК Ссылка,
	|	ТаблицаСведенияПрослеживаемостиИтог.НомерСтроки КАК НомерСтроки,
	|	ТаблицаСведенияПрослеживаемостиИтог.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ТаблицаСведенияПрослеживаемостиИтог.РНПТ КАК РНПТ,
	|	ТаблицаСведенияПрослеживаемостиДоИзменения.Количество - ТаблицаСведенияПрослеживаемостиИтог.Количество КАК Количество,
	|	ТаблицаСведенияПрослеживаемостиДоИзменения.КоличествоПрослеживаемости - ТаблицаСведенияПрослеживаемостиИтог.КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|	ВЫБОР
	...
	|ИЗ
	|	ТаблицаСведенияПрослеживаемостиИтог КАК ТаблицаСведенияПрослеживаемостиИтог
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаСведенияПрослеживаемостиДоИзменения КАК ТаблицаСведенияПрослеживаемостиДоИзменения
	|		ПО ТаблицаСведенияПрослеживаемостиИтог.ИдентификаторСтроки = ТаблицаСведенияПрослеживаемостиДоИзменения.ИдентификаторСтроки
	|			И ТаблицаСведенияПрослеживаемостиИтог.РНПТ = ТаблицаСведенияПрослеживаемостиДоИзменения.РНПТ";
