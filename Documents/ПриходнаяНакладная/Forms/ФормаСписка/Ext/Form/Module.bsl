...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Возврат", ТолькоВозвраты) Тогда
		...
		ЭтотОбъект.Заголовок = НСтр("ru = 'Приходные накладные: возвраты'");
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НомерКартинкиОплаты", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтборОплата", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаОтборОплата", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, 
			"Расхождения", 
			"Видимость", 
			Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, 
			"ГруппаОтборРасхождения", 
			"Видимость", 
			Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, 
			"ОтборРасхождения", 
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СтраницаДокументыКПоступлению", "Видимость", ПолучитьФункциональнуюОпцию("ИспользоватьОрдерныйСклад")); 
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СтраницаДокументыКПоступлению", "Видимость", ИспользуемыеВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновитьСписокДокументовКПоступлению" Тогда
...
Процедура СоздатьПриходнуюНакладную(Команда) 
	...
	ЭтоСписокЗаказыПоставщикам = Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаЗаказыПоставщикам";
	ЭтоСписокДокументыКПоступлению = Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаДокументыКПоступлению";
	
	ИмяСписка = "";
	ИмяДокументов = "";
	ИмяМассиваДокументов = "";
	
	Если ЭтоСписокЗаказыПоставщикам Тогда
		ИмяСписка = "СписокЗаказыПоставщикам";
		ИмяМассиваДокументов = "МассивЗаказовПоставщикам";
		ИмяДокументов = "Заказы";
	ИначеЕсли ЭтоСписокДокументыКПоступлению Тогда
		ИмяСписка = "СписокДокументыКПоступлению";
		ИмяМассиваДокументов = "МассивДокументовКПоступлению";
		ИмяДокументов = "Приходные ордера";
	КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли ЭтоСписокДокументыКПоступлению Тогда
			СтруктураДанных = ПроверитьКлючевыеРеквизитыДокументовПоступления(МассивДокументов);
		КонецЕсли;
	
		Если СтруктураДанных.СформироватьНесколькоДокументов Тогда

			ТекстСообщения = СтрШаблон(НСтр(
				"ru = '(%1) отличаются данными (%2) шапки документов. Сформировать несколько приходных накладных?'"), ИмяДокументов, СтруктураДанных.ПредставлениеДанных);

			ПоказатьВопрос(Новый ОписаниеОповещения("СоздатьПриходнуюНакладнуюЗавершение", ЭтотОбъект,
				Новый Структура("МассивДокументов", МассивДокументов)), ТекстСообщения, РежимДиалогаВопрос.ДаНет, 0);
...
Процедура СоздатьПриходнуюНакладнуюЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		МассивДокументовПоступления = СформироватьДокументыПоступленияИЗаписать(МассивДокументов);
...
Процедура СоздатьВозвратОтКомиссионера(Команда)
	...
	ПараметрыВозврата.Вставить("ВидОперации", "ВозвратОтКомиссионера");
	ОткрытьФорму("Документ.ПриходнаяНакладная.ФормаОбъекта", ПараметрыВозврата);
КонецПроцедуры

&НаКлиенте
Процедура СоздатьВозвратОтПереработчика(Команда)
	ПараметрыВозврата = Новый Структура();
	ПараметрыВозврата.Вставить("ВидОперацииВозврат", ТолькоВозвраты);
	ПараметрыВозврата.Вставить("ВидОперации", "ВозвратОтПереработчика");
	ОткрытьФорму("Документ.ПриходнаяНакладная.ФормаОбъекта", ПараметрыВозврата);
КонецПроцедуры
...
Процедура СоздатьВозвратОтПокупателя(Команда)
	ПараметрыВозврата = Новый Структура();
	ПараметрыВозврата.Вставить("ВидОперацииВозврат", ТолькоВозвраты);
	ПараметрыВозврата.Вставить("ВидОперации", "ВозвратОтПокупателя");
	ОткрытьФорму("Документ.ПриходнаяНакладная.ФормаОбъекта", ПараметрыВозврата);
КонецПроцедуры
...
Процедура СоздатьВозвратСОтветХранения(Команда)
	ПараметрыВозврата = Новый Структура();
	ПараметрыВозврата.Вставить("ВидОперацииВозврат", ТолькоВозвраты);
	ПараметрыВозврата.Вставить("ВидОперации", "ВозвратСОтветХранения");
	ОткрытьФорму("Документ.ПриходнаяНакладная.ФормаОбъекта", ПараметрыВозврата);
КонецПроцедуры
...
//
&НаСервере
Функция СформироватьДокументыПоступленияИЗаписать(МассивДокументов) 
	...
	Для каждого СтрокаМассива Из МассивДокументов Цикл
		...
		НовыйДокументПоступления.Дата = ТекущаяДатаСеанса();
		НовыйДокументПоступления.Заполнить(СтрокаМассива);
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Элементы.СписокЗаказыПоставщикамСоздатьПриходнуюНакладную.КнопкаПоУмолчанию = Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаЗаказыПоставщикам";
	Элементы.СписокДокументыКПоступлениюСоздатьПриходнуюНакладную.КнопкаПоУмолчанию = Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаДокументыКПоступлению";
