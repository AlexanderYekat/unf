...
Процедура ШапкаТабличнаяЧастьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ЭтоОтветХранение И Результат.Свойство("ПоложениеЗаказаПоставщикуВДокументахПоступления")Тогда	
...
Процедура ЗаполнитьСтавкуНДСПоОрганизацииНалогообложениеНДС(ЭтоОткрытие = Ложь)
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНДС", "Видимость"
		, Элементы.СчетФактураНадпись.Видимость И (Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя Или Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя)); 
	...
	Для Каждого ТекШтрихкод Из Контекст.МассивШтрихкодов Цикл
		...
		Иначе
			...
			Если НайденныеСтроки.Количество() = 0 Или НЕ ОбъединятьСтрокиПриДобавлении
				...
				Если УчетНоменклатурыПоставщиков И ЭтоПоступление(Объект.ВидОперации) Тогда //BG
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь, ЭтоНовыйОбъект = Ложь)
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаНДС", "Видимость"
		, Элементы.СчетФактураНадпись.Видимость И (Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя Или Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары));
