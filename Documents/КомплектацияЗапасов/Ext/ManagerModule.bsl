	...
	Если НастройкиЗагрузкиДанных.Свойство("ВидОперации") Тогда
		...
		Если НастройкиЗагрузкиДанных.ВидОперации = Перечисления.ВидыОперацийКомплектацияЗапасов.Разборка Тогда
			...
			ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДоляСтоимости", НСтр(
				"ru = 'ДоляСтоимости'"), ОписанияТиповПолей.ОписаниеТиповСтрока25,
		...
		Иначе
			...
			Если ПолучитьФункциональнуюОпцию("УчетГТД") Тогда
				...
				ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки,
					"СтранаПроисхождения", НСтр("ru = 'Страна происхождения'"),
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.КомплектацияЗапасов.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
...
Процедура СформироватьТаблицаЗапасыРазборка(ДокументСсылкаКомплектацияЗапасов, СтруктураДополнительныеСвойства)
	...
	Для каждого СтрокаТаблицы Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПродукция Цикл
		...
		КоличествоОстаток = 0;
		СебестоимостьКомплекта = 0;
		СебестоимостьКомплектаБезНДС = 0;
		...
		Если КоличествоОстаток > 0 И КоличествоОстаток > КоличествоТребуется Тогда
			...
			СебестоимостьКомплектаБезНДС = Окр(СуммаБезНДСОстаток * КоличествоТребуется / КоличествоОстаток , 2, 1);

			МассивСтрокОстатков[0].КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток - КоличествоТребуется;
			МассивСтрокОстатков[0].СуммаОстаток = МассивСтрокОстатков[0].СуммаОстаток - СебестоимостьКомплекта;
			МассивСтрокОстатков[0].СуммаБезНДСОстаток = МассивСтрокОстатков[0].СуммаБезНДСОстаток - СебестоимостьКомплектаБезНДС;

		ИначеЕсли КоличествоОстаток = КоличествоТребуется Тогда

			СебестоимостьКомплекта = СуммаОстаток;
			СебестоимостьКомплектаБезНДС = СуммаБезНДСОстаток;
	...
	Для каждого СтрокаТаблицы Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы Цикл
		...
		Если ВсегоДолей = 0 Тогда
			...
			СуммаБезНДСКСписанию = Окр(СебестоимостьКомплектаБезНДС / ОсталосьСтрок, 2);
		...
		Иначе
			...
			СуммаБезНДСКСписанию = Окр(СебестоимостьКомплектаБезНДС * СтрокаТаблицы.ДоляСтоимости / ВсегоДолей, 2);
		...
		СебестоимостьКомплекта = СебестоимостьКомплекта - СуммаКСписанию;
		СебестоимостьКомплектаБезНДС = СебестоимостьКомплектаБезНДС - СуммаБезНДСКСписанию;
		...
		НоваяСтрокаПриход.СуммаБезНДС = СуммаБезНДСКСписанию;
		НоваяСтрокаПриход.Количество = 0;
		НоваяСтрокаПриход.ЗатратыНаВыпуск = Ложь;
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаКомплектацияЗапасов, СтруктураДополнительныеСвойства) Экспорт
	...
	// Прослеживаемость
	ПрослеживаемостьУНФ.СформироватьДвиженияРеализацияТоваров(ДокументСсылкаКомплектацияЗапасов, СтруктураДополнительныеСвойства);
...
Функция ТекстЗапросаИнициализироватьДанныеДокумента()
	...
	|			ТОГДА КомплектацияЗапасов.ЗаказПокупателя
	...
	|	КомплектацияЗапасов.Количество КАК КоличествоДокумент,
	|	КомплектацияЗапасов.ЕдиницаИзмерения КАК ЕдиницаИзмеренияДокумент,
	...
	|	КомплектацияЗапасов.Ссылка КАК Регистратор,
	|	НЕОПРЕДЕЛЕНО КАК КорректируемыйДокумент,
	|	КомплектацияЗапасов.Дата КАК Дата,
	|	КомплектацияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов КАК ВидУчетаОрдерныхСкладов,
	|	ВЫБОР
	|		КОГДА КомплектацияЗапасов.СтруктурнаяЕдиница.ОрдерныйСклад
	|				И КомплектацияЗапасов.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриПоступлении
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОрдерныйСкладКПоступлению,
	|	ВЫБОР
	|		КОГДА КомплектацияЗапасов.СтруктурнаяЕдиница.ОрдерныйСклад
	|				И КомплектацияЗапасов.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОрдерныйСкладКОтгрузке,
	|	КомплектацияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом) КАК УчетПоСкладуВЦелом,
	|	ВЫБОР
	|		КОГДА КомплектацияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА КомплектацияЗапасов.Ссылка
	|		КОГДА КомплектацияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА КомплектацияЗапасов.ДокументПоступления
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|			ТОГДА КомплектацияЗапасов.Ссылка
	|		КОГДА КомплектацияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА КомплектацияЗапасов.ДокументОтгрузки
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументОтгрузки,
	|	ВЫБОР
	|		КОГДА КомплектацияЗапасов.СтруктурнаяЕдиница.УчетОстатковПоЯчейкам
	|			ТОГДА КомплектацияЗапасов.Ячейка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ЯчейкаКПоступлению,
	|	КомплектацияЗапасов.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам) КАК УчетОстатковПоПрочимДокументам
	...
	|	КомплектацияЗапасовЗапасы.Количество КАК КоличествоДокумент,
	|	КомплектацияЗапасовЗапасы.ЕдиницаИзмерения КАК ЕдиницаИзмеренияДокумент,
	...
	|	КомплектацияЗапасовЗапасы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	NULL КАК КорректируемыйДокумент,
	|	КомплектацияЗапасовЗапасы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	0 КАК СуммаБезНдс,
	|	КомплектацияЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов КАК ВидУчетаОрдерныхСкладов,
	|	ВЫБОР
	|		КОГДА КомплектацияЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад
	|				И КомплектацияЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриПоступлении
	|				И КомплектацияЗапасовЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКомплектацияЗапасов.Сборка)
	|			ТОГДА ИСТИНА
	|		КОГДА КомплектацияЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад
	|				И КомплектацияЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке
	...
	|	КОНЕЦ КАК ОрдерныйСкладКОтгрузке,
	|	ВЫБОР
	|		КОГДА КомплектацияЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА КомплектацияЗапасовЗапасы.Ссылка
	|		КОГДА КомплектацияЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА КомплектацияЗапасовЗапасы.Ссылка.ДокументПоступления
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументПоступления,
	|	ВЫБОР
	|		КОГДА КомплектацияЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА КомплектацияЗапасовЗапасы.Ссылка
	|		КОГДА КомплектацияЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА КомплектацияЗапасовЗапасы.Ссылка.ДокументОтгрузки
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументОтгрузки
	...
	|ИЗ
	|	Документ.КомплектацияЗапасов.СведенияПрослеживаемости КАК СведенияПрослеживаемости
	|ГДЕ
...
Процедура ИнициализироватьДанныеДокументаСборка(ДокументСсылкаКомплектацияЗапасов, СтруктураДополнительныеСвойства)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст = ТекстЗапросаИнициализироватьДанныеДокументаСборка();
	МассивРезультатов = Запрос.ВыполнитьПакет();
	...
	РезультатЗапросаКПоступлению = МассивРезультатов[9].Выгрузить();
	...
	РезультатЗапросаККРасходу= МассивРезультатов[10].Выгрузить();
...
Функция ТекстЗапросаИнициализироватьДанныеДокументаСборка()
	...
	|	ВременнаяТаблицаШапка.ЗаказПокупателя КАК КоррЗаказПокупателя,
	...
	|	ЗНАЧЕНИЕ(Справочник.Спецификации.ПустаяСсылка) КАК Спецификация,
	|	ВременнаяТаблицаШапка.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка) КАК ЗаказНаПроизводство,
	...
	|				И ВременнаяТаблицаШапка.ОрдерныйСкладКОтгрузке)
	...
	|ГДЕ
	|	НЕ(ВременнаяТаблицаШапка.ОрдерныйСклад
	|				И ВременнаяТаблицаШапка.ОрдерныйСкладКПоступлению)
	|	И ВременнаяТаблицаШапка.ТипСтруктурнойЕдиницы <> ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблицаШапка.Период,
	...
	|	ВременнаяТаблицаШапка.Характеристика КАК Характеристика,
	|	ВременнаяТаблицаШапка.Партия КАК Партия,
	|	ВременнаяТаблицаШапка.ДокументПоступления КАК ДокументОснование,
	|	ВременнаяТаблицаШапка.ЯчейкаКПоступлению КАК Ячейка,
	|	СУММА(ВременнаяТаблицаШапка.Количество) КАК Количество
	|ИЗ
	|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
	|ГДЕ
	|	ВременнаяТаблицаШапка.ОрдерныйСкладКПоступлению
	|
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблицаШапка.Период,
	|	ВременнаяТаблицаШапка.СтруктурнаяЕдиница,
	...
	|	ВременнаяТаблицаШапка.Характеристика,
	|	ВременнаяТаблицаШапка.Партия,
	|	ВременнаяТаблицаШапка.ДокументПоступления,
	|	ВременнаяТаблицаШапка.ЯчейкаКПоступлению
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
	|ГДЕ
	|	ВременнаяТаблицаШапка.ОрдерныйСкладКОтгрузке
	|
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблицаШапка.Период,
	|	ВременнаяТаблицаШапка.СтруктурнаяЕдиница,
	...
	|	ВременнаяТаблицаЗапасы.Характеристика,
	|	ВременнаяТаблицаЗапасы.Партия,
	|	ВременнаяТаблицаЗапасы.ДокументОтгрузки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	НЕ(ВременнаяТаблицаШапка.ОрдерныйСклад
	|				И ВременнаяТаблицаШапка.ОрдерныйСкладКПоступлению
	|				И (ВременнаяТаблицаШапка.УчетОстатковПоПрочимДокументам
	|					ИЛИ ВременнаяТаблицаШапка.УчетПоСкладуВЦелом))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВременнаяТаблицаСерииНоменклатуры.Серия КАК Серия,
	|	ВременнаяТаблицаСерииНоменклатуры.ОстаткиСерийНоменклатуры КАК ОстаткиСерийНоменклатуры,
	|	ВременнаяТаблицаШапка.Организация КАК Организация,
	|	ВременнаяТаблицаЗапасы.Номенклатура КАК Номенклатура,
	|	ВременнаяТаблицаЗапасы.Характеристика КАК Характеристика,
	|	ВременнаяТаблицаЗапасы.Партия КАК Партия,
...
Процедура ИнициализироватьДанныеДокументаРазборка(ДокументСсылкаКомплектацияЗапасов, СтруктураДополнительныеСвойства)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст = ТекстЗапросаИнициализироватьДанныеДокументаРазборка();
	МассивРезультатов = Запрос.ВыполнитьПакет();
...
СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗапасыНаСкладах", МассивРезультатов[2].Выгрузить());
	...
	РезультатЗапросаККРасходу= МассивРезультатов[9].Выгрузить();
	ОстаткиСерийНоменклатурыСтроки = РезультатЗапросаККРасходу.НайтиСтроки(ПараметрыОтбора);
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСерииНоменклатурыКРасходу", РезультатЗапросаККРасходу.Скопировать(ОстаткиСерийНоменклатурыСтроки));
...
Функция ТекстЗапросаИнициализироватьДанныеДокументаРазборка()
	...
	|	ВременнаяТаблицаШапка.Характеристика КАК Характеристика,
	|	ВременнаяТаблицаШапка.Партия КАК Партия,
	|	ВременнаяТаблицаШапка.ДокументОтгрузки КАК ДокументОснование,
	|	ВременнаяТаблицаШапка.ЯчейкаКПоступлению КАК Ячейка,
	|	СУММА(ВременнаяТаблицаШапка.Количество) КАК Количество
	|ИЗ
	|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
	...
	|	ВременнаяТаблицаШапка.Характеристика,
	|	ВременнаяТаблицаШапка.Партия,
	|	ВременнаяТаблицаШапка.ДокументОтгрузки,
	|	ВременнаяТаблицаШапка.ЯчейкаКПоступлению
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ВременнаяТаблицаЗапасы.Характеристика,
	|	ВременнаяТаблицаЗапасы.Партия,
	|	ВременнаяТаблицаЗапасы.ДокументПоступления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
		...
		Если Константы.ВестиУчетПрослеживаемыхТоваров.Получить()
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструПрослеживаемыеТовары(
				ДокументОбъектКомплектацияЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
		...
		Если НЕ МассивРезультатов[5].Пустой() Тогда
			...
			КонтрольОстатковУНФ.ЗапасыКПоступлению(ДокументОбъектКомплектацияЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		...
		Если НЕ МассивРезультатов[6].Пустой() Тогда
			...
			КонтрольОстатковУНФ.СерииНоменклатурыКПоступлению(ДокументОбъектКомплектацияЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		...
		Если НЕ МассивРезультатов[7].Пустой() Тогда
			...
			КонтрольОстатковУНФ.ЗапасыКРасходу(ДокументОбъектКомплектацияЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		...
		Если НЕ МассивРезультатов[8].Пустой() Тогда
			...
			КонтрольОстатковУНФ.СерииНоменклатурыКРасходуПоДокументам(ДокументОбъектКомплектацияЗапасов, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
