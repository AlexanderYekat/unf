...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка)
		...
		Срок = 1;
		Объект.ДатаПредоставления = ТекущаяДатаСеанса();
	...
	РаботаСФормойКлиентСервер.УстановитьКартинкуДляКомментария(Элементы.СтраницаВыдача, Объект.Комментарий);
	...
	НаборСвойств_Документ_ДоговорКредитаИЗайма = УправлениеСвойствами.НаборСвойствПоИмени(
		"Документ_ДоговорКредитаИЗайма");
...
Процедура ЗаполнитьГрафикПлатежейИНачислений(Команда)
	...
	Если Объект.СуммаДокумента = 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не заполнена сумма'");
...
Процедура СоздатьНапоминанияОбОплате(Команда)
	...
	ПараметрыОтбора.Вставить("АдресГрафикПлатежейИНачисленийВХранилище", АдресГрафикПлатежейИНачисленийВХранилище);
	...
	ПараметрыОтбора.Вставить("БанкКонтрагент", Объект.Контрагент);
	
	ОткрытьФорму("Документ.ДоговорКредитаИЗайма.Форма.ФормаСозданияНапоминаний", ПараметрыОтбора, ЭтотОбъект);
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	...
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", НаборСвойств_Документ_ДоговорКредитаИЗайма);
...
Процедура ЗаполнитьРазмерПлатежа()
	...
	Иначе
		...
		СтрокаВидДоговора = ?(Объект.ВидДоговора = ВидДоговораДоговорЗайма, НСтр("ru = 'займа'"), НСтр("ru = 'кредита'"));
		Если Объект.СуммаДокумента > 0 И Объект.ПроцентнаяСтавкаГодовая <> 0 Тогда
			ШаблонКомментария = НСтр("ru = '""Сумма %"" составляет [Процент]% от суммы [ВидДоговора]. Остаток долга = 0 ([Валюта]).'");
			Значения = Новый Структура;
			Значения.Вставить("Процент", Формат(НоваяСтрокаГрафика.СуммаПроцентов / Объект.СуммаДокумента * 100, "ЧДЦ=2; ЧН=0"));
			Значения.Вставить("ВидДоговора", СтрокаВидДоговора);
			Значения.Вставить("Валюта", Объект.ВалютаРасчетов);
			НоваяСтрокаГрафика.Комментарий = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ШаблонКомментария, Значения);
		КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаДоговора()
	...
	Если Объект.ВидДоговора = ПредопределенноеЗначение("Перечисление.ВидыДоговоровКредитаИЗайма.КредитПолученный") Тогда
		...
		Элементы.ГруппаВходящийНомерДата.Видимость = Истина;
		
		Элементы.СтраницаВыдача.Заголовок = НСтр("ru = 'Получение кредита'");
		Элементы.СчетЗатрат.Заголовок = НСтр("ru = 'Счет затрат'");
	...
	Иначе
		...
		Элементы.ГруппаВходящийНомерДата.Видимость = Ложь;
		
		Элементы.СтраницаВыдача.Заголовок = НСтр("ru = 'Выдача займа'");
		Элементы.СчетЗатрат.Заголовок = НСтр("ru = 'Счет доходов'");
...
Процедура НастроитьЭлементыПоТипуКомиссииИВидуДоговора()
	...
	МассивТиповСчетов.Добавить(Перечисления.ТипыСчетов.ПроцентыПоКредитам);
	МассивТиповСчетов.Добавить(Перечисления.ТипыСчетов.ДолгосрочныеОбязательства);
	МассивТиповСчетов.Добавить(Перечисления.ТипыСчетов.КредитыИЗаймы);
	...
	Иначе
		...
		МассивТиповСчетов.Добавить(Перечисления.ТипыСчетов.ПрочиеДоходы);
		
	КонецЕсли;
	
	НовыйПараметр = Новый ПараметрВыбора("Отбор.ТипСчета", Новый ФиксированныйМассив(МассивТиповСчетов));
...
Процедура СохранитьДокументКакШаблон(Параметр) Экспорт
	...
	ОтображаемыеРеквизиты = ОтображаемыеРеквизиты();
...
Функция ГрафикПогашенияНесколькоМесяцев(РазмерПлатежа)
	...
	Пока СледующаяДатаПлатежа < ДатаОкончания Цикл
		...
		Иначе
			...
			// Проценты начинают начисляться со следующего дня после выдачи.
			Если ТекущийМесяц = НачалоМесяца(ДатаПредоставления) И ДнейВМесяце > 0 Тогда
				ДнейВМесяце = ДнейВМесяце - 1;
			КонецЕсли;
		КонецЕсли;
		...
		Если ВидыПлатежейРазмерПогашения.Найти(ВидПлатежей) <> Неопределено Тогда
			...
			НачисленоОсновногоДолгаСлед = НачисленоОсновногоДолгаСлед;
		КонецЕсли;
		
		//НачислениеПроцентов = Окр(НачислениеПроцентовПред + НачислениеПроцентовСлед, 2);
		НачислениеПроцентов = НачислениеПроцентовПред + НачислениеПроцентовСлед;
		Если ВидыПлатежейРазмерПогашения.Найти(ВидПлатежей) <> Неопределено Тогда
			НачисленоОсновногоДолга = Окр(НачисленоОсновногоДолгаПред + НачисленоОсновногоДолгаСлед, 2);
		Иначе
	...
	// Возможно, что в результате распределения небольшая сумма долга не распределилась. Распределим ее на основной долг,
	// уменьшая проценты. Начнем с последней строки.
...
Процедура ЗаполнитьГрафикПлатежейИНачисленийНаСервере()
	...
	Для Каждого СтрокаГрафика Из ГрафикПогашения Цикл
		...
		Если Объект.ПроцентнаяСтавкаГодовая <> 0 Тогда
			ШаблонКомментария = НСтр("ru = '""Сумма %"" составляет [Процент]% от суммы [ВидДоговора]. Остаток долга = [ОстатокДолга] ([Валюта]).'");
			Значения.Вставить("Процент", Формат(НоваяСтрокаГрафика.СуммаПроцентов / Объект.СуммаДокумента * 100, "ЧДЦ=2; ЧН=0"));
			Значения.Вставить("ВидДоговора", СтрокаВидДоговора);
		Иначе
			ШаблонКомментария = НСтр("ru = 'Остаток долга = [ОстатокДолга] ([Валюта]).'");
		КонецЕсли;
		Значения.Вставить("ОстатокДолга", Формат(ОстатокДолга, "ЧДЦ=2; ЧН=0"));
		Значения.Вставить("Валюта", Объект.ВалютаРасчетов);
		НоваяСтрокаГрафика.Комментарий = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ШаблонКомментария, Значения);
...
// Процедура обновляет информацию в поле Комментарий табличной части ГрафикПлатежейИНачислений.
//
&НаКлиенте
...
Функция РазмерАннуитетногоПлатежа() Экспорт
	...
	// - в первом приближении считаем, что размер платежа равен платежу
	// при единовременной выдаче всех траншей
	// - получаем остаток долга после выполнения всех платежей с таким размером
	// - во втором приближении считаем, что размер платежа на 20% меньше, чем платеж
