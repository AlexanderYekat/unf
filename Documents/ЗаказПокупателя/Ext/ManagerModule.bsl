...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	|	И ЗначениеРазрешено(СтруктурнаяЕдиницаРезерв, ПустаяСсылка КАК Истина)
	|	И ДляВсехСтрок(ЗначениеРазрешено(Запасы.СтруктурнаяЕдиницаРезерв, ПустаяСсылка КАК Истина, Null КАК Истина))";
...
Процедура ИзменитьСостояниеЗаказаПокупателя(Заказ, Состояние, ВариантЗавершения = Неопределено, ПричинаОтмены = Неопределено, Заметки = Неопределено) Экспорт
	...
	СостояниеЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Заказ, "СостояниеЗаказа");
	ВариантЗавершенияЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Заказ, "ВариантЗавершения");
	
	Если СостояниеЗаказа = Состояние И Состояние <> Справочники.СостоянияЗаказовПокупателей.Завершен Тогда
		Возврат;
	КонецЕсли;
	
	ЗаказЗавершен = (СостояниеЗаказа = Состояние И Состояние = Справочники.СостоянияЗаказовПокупателей.Завершен
		И (ЗначениеЗаполнено(ВариантЗавершенияЗаказа) И ВариантЗавершенияЗаказа = ВариантЗавершения));
	
	Если ЗаказЗавершен Тогда
		Возврат;
	КонецЕсли;
...
Функция СтатусОплаты(Заказ) Экспорт
	...
	Возврат РасчетыПроведениеДокументов.СтатусОплаты(Заказ);
	
КонецФункции

Функция ПроцентОплаты(Заказ) Экспорт
	
	Возврат РасчетыПроведениеДокументов.ПроцентОплаты(Заказ);
...
// Возвращает структуру с  датой отгрузки, рассчитанной по параметрам, указанным в договоре контрагента
//
// Параметры:
//  ДатаДокумента       - Дата
...
//
Функция РассчитатьДатуОтгрузки(ДатаДокумента, ДоговорКонтрагента) Экспорт

	СтруктураНастроек = Обработки.НастройкаЗаполненияДатыОтгрузки.СтруктураНастроекДатыОтгрузки();
	ЗаполнитьЗначенияСвойств(СтруктураНастроек, ДоговорКонтрагента);
	
	Результат = Новый Структура("ДатаОтгрузки, ТекстОшибки");
	
	Если СтруктураНастроек.ВариантЗаполненияДатыОтгрузки = Перечисления.ВариантыЗаполненияДатыОтгрузки.ПустаяСсылка()
		ИЛИ СтруктураНастроек.ВариантЗаполненияДатыОтгрузки = Перечисления.ВариантыЗаполненияДатыОтгрузки.НеЗаполнять Тогда
		Результат.ДатаОтгрузки = Дата(1,1,1);
		Возврат Результат;
	КонецЕсли;
	
	Если СтруктураНастроек.ВидДнейРасчетаДатыОтгрузки = Перечисления.ВидыДнейРасчетаДатыОтгрузки.Календарные Тогда
		Результат.ДатаОтгрузки = ДатаДокумента + СтруктураНастроек.КоличествоДнейРасчетаДатыОтгрузки * 86400;
	Иначе
		ПроизводственныйКалендарь = СтруктураНастроек.ПроизводственныйКалендарьРасчетаДатыОтгрузки;
		КоличествоДней = СтруктураНастроек.КоличествоДнейРасчетаДатыОтгрузки;
		Результат.ДатаОтгрузки = ПолучитьРассчитаннуюДатуРабочегоДня(ПроизводственныйКалендарь, ДатаДокумента,
			 КоличествоДней);
		Если НЕ ЗначениеЗаполнено(Результат.ДатаОтгрузки) Тогда
				
			ТекстОшибки = НСтр("ru='Производственный календарь ""%1"" не заполнен на расчетную дату отгрузки (%2 с %3). '");
...
// Возвращает список заказов, которые все еще не будут исполнены после оформления продажи указанных товаров
//
// Параметры:
//  ТаблицаПродажи      - ТаблицаЗначений - 
//  СписокЗаказов  - Массив - Список заказов, которые нужно проверить
//
// Возвращаемое значение:
//   Массив - ДокументСсылка.ЗаказПокупателя - список документов, которые не исполняются полностью после оформления
//                                             продажи указанных товаров
//
Функция ЗаказыБудутЕщеОткрыты(ТаблицаПродажи, СписокЗаказов) Экспорт
	Запрос = Новый Запрос;
	Текст = 
	"ВЫБРАТЬ
	|	ТЗ.Заказ КАК ЗаказПокупателя,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОстатокПослеОтгрузки.ЗаказПокупателя КАК ЗаказПокупателя,
	|	СУММА(ОстатокПослеОтгрузки.КоличествоОстаток) КАК КоличествоОстаток
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОстатокЗаказа.ЗаказПокупателя КАК ЗаказПокупателя,
	|		ОстатокЗаказа.Номенклатура КАК Номенклатура,
	|		ОстатокЗаказа.Характеристика КАК Характеристика,
	|		ОстатокЗаказа.КоличествоОстаток КАК КоличествоОстаток
	|	ИЗ
	|		РегистрНакопления.ЗаказыПокупателей.Остатки(, ЗаказПокупателя В (&СписокЗаказов)) КАК ОстатокЗаказа
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		Отгружается.ЗаказПокупателя,
	|		Отгружается.Номенклатура,
	|		Отгружается.Характеристика,
	|		ВЫБОР
	|			КОГДА Отгружается.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|				ТОГДА -ВЫРАЗИТЬ(Отгружается.ЕдиницаИзмерения КАК Справочник.ЕдиницыИзмерения).Коэффициент * Отгружается.Количество
	|			ИНАЧЕ -Отгружается.Количество
	|		КОНЕЦ
	|	ИЗ
	|		вт_Продажа КАК Отгружается) КАК ОстатокПослеОтгрузки
	|
	|СГРУППИРОВАТЬ ПО
	|	ОстатокПослеОтгрузки.ЗаказПокупателя
	|
	|ИМЕЮЩИЕ
	|	СУММА(ОстатокПослеОтгрузки.КоличествоОстаток) > 0";  
	Если ТаблицаПродажи.Колонки.Найти("Заказ") = Неопределено Тогда
		Текст = СтрЗаменить(Текст, "ТЗ.Заказ","ТЗ.ЗаказПокупателя");
	КонецЕсли;
	Запрос.Текст = Текст;
	Запрос.УстановитьПараметр("ТаблицаТоваров", ТаблицаПродажи);
	Запрос.УстановитьПараметр("СписокЗаказов", СписокЗаказов);
	Результат = Новый Массив;
	
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Результат = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("ЗаказПокупателя");
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаДокумента" Или ВидФормы = "ФормаОбъекта" Тогда
		...
		Если Параметры.Свойство("ВидОперации")
			И ТипЗнч(Параметры.ВидОперации) = Тип("ПеречислениеСсылка.ВидыОперацийЗаказПокупателя") Тогда
...
Процедура ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, Параметры, Менеджер, Идентификатор = "")

	СтруктураПоиска = Новый Структура("Менеджер, Обработчик", Менеджер, "");
	СтрокиКоманд = КомандыСозданияНаОсновании.НайтиСтроки(СтруктураПоиска);
	Если СтрокиКоманд.Количество() > 0 Тогда
		ПоследняяКоманда = СтрокиКоманд[0]; 
		ЗаполнитьЗначенияСвойств(ПоследняяКоманда, Параметры);                         
		Если ЗначениеЗаполнено(Идентификатор) Тогда  
			ПоследняяКоманда.Идентификатор = Идентификатор;
		КонецЕсли;
	КонецЕсли;
...
Функция ПолноеИмяОбъекта()
	Возврат "Документ.ЗаказПокупателя";
КонецФункции
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	Если ПустаяСтрока(ПолноеИмяОбъектаЗаполнения) Или ПолноеИмяОбъектаЗаполнения
		= "Документ.ЗаказПокупателя.ТабличнаяЧасть.Запасы" Тогда

		ПоляЗагрузкиДанныхИзВнешнегоИсточникаЗапасы(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных, ОписанияТиповПолей);

	ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ЗаказПокупателя.ТабличнаяЧасть.Калькуляция" Тогда

		ПоляЗагрузкиДанныхИзВнешнегоИсточникаКалькуляция(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных, ОписанияТиповПолей);
		
	ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ЗаказПокупателя.ТабличнаяЧасть.Работы" Тогда

		ПоляЗагрузкиДанныхИзВнешнегоИсточникаРаботы(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных, ОписанияТиповПолей);
		
	ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ЗаказПокупателя.ТабличнаяЧасть.Материалы" Тогда

		ПоляЗагрузкиДанныхИзВнешнегоИсточникаМатериалы(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных, ОписанияТиповПолей);
		
	ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ЗаказПокупателя.ТабличнаяЧасть.МатериалыЗаказчика" Тогда

		ПоляЗагрузкиДанныхИзВнешнегоИсточникаМатериалыЗаказчика(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных,
			ОписанияТиповПолей);
...
Процедура ПриОпределенииОбразцовЗагрузкиДанных(НастройкиЗагрузкиДанных, УникальныйИдентификатор) Экспорт
	...
	ИначеЕсли ПолноеИмяОбъектаЗаполнения="Документ.ЗаказПокупателя.ТабличнаяЧасть.Работы" Тогда
		...
		Образец_xlsx = ПолучитьМакет("ОбразецЗагрузкиДанныхРаботы_xlsx");
		ОбразецЗагрузкиДанных_xlsx = ПоместитьВоВременноеХранилище(Образец_xlsx, УникальныйИдентификатор);
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_xlsx", ОбразецЗагрузкиДанных_xlsx);
		
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_mxl", "ОбразецЗагрузкиДанныхРаботы_mxl");
		
		Образец_csv = ПолучитьМакет("ОбразецЗагрузкиДанныхРаботы_csv");
		ОбразецЗагрузкиДанных_csv = ПоместитьВоВременноеХранилище(Образец_csv, УникальныйИдентификатор);
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_csv", ОбразецЗагрузкиДанных_csv);
		
	ИначеЕсли ПолноеИмяОбъектаЗаполнения="Документ.ЗаказПокупателя.ТабличнаяЧасть.Материалы" Тогда
		
		Образец_xlsx = ПолучитьМакет("ОбразецЗагрузкиДанныхМатериалы_xlsx");
		ОбразецЗагрузкиДанных_xlsx = ПоместитьВоВременноеХранилище(Образец_xlsx, УникальныйИдентификатор);
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_xlsx", ОбразецЗагрузкиДанных_xlsx);
		
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_mxl", "ОбразецЗагрузкиДанныхМатериалы_mxl");
		
		Образец_csv = ПолучитьМакет("ОбразецЗагрузкиДанныхМатериалы_csv");
		ОбразецЗагрузкиДанных_csv = ПоместитьВоВременноеХранилище(Образец_csv, УникальныйИдентификатор);
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_csv", ОбразецЗагрузкиДанных_csv);
		
	ИначеЕсли ПолноеИмяОбъектаЗаполнения="Документ.ЗаказПокупателя.ТабличнаяЧасть.МатериалыЗаказчика" Тогда
		
		Образец_xlsx = ПолучитьМакет("ОбразецЗагрузкиДанныхМатериалыЗаказчика_xlsx");
		ОбразецЗагрузкиДанных_xlsx = ПоместитьВоВременноеХранилище(Образец_xlsx, УникальныйИдентификатор);
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_xlsx", ОбразецЗагрузкиДанных_xlsx);
		
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_mxl", "ОбразецЗагрузкиДанныхМатериалыЗаказчика_mxl");
		
		Образец_csv = ПолучитьМакет("ОбразецЗагрузкиДанныхМатериалыЗаказчика_csv");
		ОбразецЗагрузкиДанных_csv = ПоместитьВоВременноеХранилище(Образец_csv, УникальныйИдентификатор);
		НастройкиЗагрузкиДанных.Вставить("ОбразецЗагрузкиДанных_csv", ОбразецЗагрузкиДанных_csv);
...
Процедура СопоставитьЗагружаемыеДанныеИзВнешнегоИсточника(ПараметрыСопоставления, АдресРезультата) Экспорт
	...
	Для Каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицыФормы,
			"КатегорияНоменклатуры_ВходящиеДанные") И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			СтрокаТаблицыФормы, "КатегорияНоменклатуры") Тогда
		...
		Если ПустаяСтрока(ПолноеИмяОбъектаЗаполнения) Или ПолноеИмяОбъектаЗаполнения
			...
			СопоставитьСтрокуЗагружаемыхДанныхЗапасы(СтрокаТаблицыФормы, НастройкиПоиска, НастройкиЗагрузкиДанных);

			ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы, ПолноеИмяОбъектаЗаполнения);

		ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ЗаказПокупателя.ТабличнаяЧасть.Калькуляция" Тогда

			СопоставитьСтрокуЗагружаемыхДанныхКалькуляция(СтрокаТаблицыФормы, НастройкиПоиска, НастройкиЗагрузкиДанных);

			ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы, ПолноеИмяОбъектаЗаполнения);

		ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ЗаказПокупателя.ТабличнаяЧасть.Работы" Тогда

			СопоставитьСтрокуЗагружаемыхДанныхРаботы(СтрокаТаблицыФормы, НастройкиПоиска, НастройкиЗагрузкиДанных);

			ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы, ПолноеИмяОбъектаЗаполнения);
			
		ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ЗаказПокупателя.ТабличнаяЧасть.Материалы" Тогда

			СопоставитьСтрокуЗагружаемыхДанныхМатериалы(СтрокаТаблицыФормы, НастройкиПоиска, НастройкиЗагрузкиДанных);

			ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы, ПолноеИмяОбъектаЗаполнения);
			
		ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ЗаказПокупателя.ТабличнаяЧасть.МатериалыЗаказчика" Тогда

			СопоставитьСтрокуЗагружаемыхДанныхМатериалыЗаказчика(СтрокаТаблицыФормы, НастройкиПоиска,
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ПрогрессСопоставленияДанных(ТаблицаСопоставленияДанных.Индекс(
			СтрокаТаблицыФормы), РазмерТаблицыДанных);
...
Процедура ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы, ПолноеИмяОбъектаЗаполнения = "") Экспорт
	...
	Если ПустаяСтрока(ПолноеИмяОбъектаЗаполнения) Или ПолноеИмяОбъектаЗаполнения
		...
		Если НоменклатураЗаполнена Тогда
			...
			ЭтоЗапас = (ДопустимыеТипыНоменклатуры.Найти(СтрокаТаблицыФормы.Номенклатура.ТипНоменклатуры)
				<> Неопределено);

			ЭтоПодразделение = Ложь;
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицыФормы,
				"СтруктурнаяЕдиницаРезерв") И ЗначениеЗаполнено(СтрокаТаблицыФормы.СтруктурнаяЕдиницаРезерв)
				И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				СтрокаТаблицыФормы.СтруктурнаяЕдиницаРезерв, "ТипСтруктурнойЕдиницы")
				И СтрокаТаблицыФормы.СтруктурнаяЕдиницаРезерв.ТипСтруктурнойЕдиницы
			...
			СтрокаТаблицыФормы[ИмяСлужебногоПоля] = Не СтрокаТаблицыФормы.Номенклатура.ЭтоНабор И ЭтоЗапас
				И СтрокаТаблицыФормы.Количество <> 0 И Не ЭтоПодразделение;
	...
	ИначеЕсли ПустаяСтрока(ПолноеИмяОбъектаЗаполнения) Или ПолноеИмяОбъектаЗаполнения
		= "Документ.ЗаказПокупателя.ТабличнаяЧасть.Работы" Тогда
		...
		Если НоменклатураЗаполнена Тогда
			...
			ЭтоРаботаУслуга = (ДопустимыеТипыНоменклатуры.Найти(СтрокаТаблицыФормы.Номенклатура.ТипНоменклатуры)
				<> Неопределено);
			...
			СтрокаТаблицыФормы[ИмяСлужебногоПоля] = Не СтрокаТаблицыФормы.Номенклатура.ЭтоНабор И ЭтоРаботаУслуга
				И СтрокаТаблицыФормы.Количество <> 0 И Не ЭтоПодразделение;
	...
	ИначеЕсли ПолноеИмяОбъектаЗаполнения = "Документ.ЗаказПокупателя.ТабличнаяЧасть.Калькуляция" Тогда
		...
		СтрокаТаблицыФормы[ИмяСлужебногоПоля] = ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура) И (ТипЗнч(
			СтрокаТаблицыФормы.Номенклатура) <> Тип("СправочникСсылка.Номенклатура")
			Или Не СтрокаТаблицыФормы.Номенклатура.ЭтоНабор);
	
	ИначеЕсли ПустаяСтрока(ПолноеИмяОбъектаЗаполнения) Или ПолноеИмяОбъектаЗаполнения
		= "Документ.ЗаказПокупателя.ТабличнаяЧасть.Материалы" Тогда
	...
	ИначеЕсли ПустаяСтрока(ПолноеИмяОбъектаЗаполнения) Или ПолноеИмяОбъектаЗаполнения
		= "Документ.ЗаказПокупателя.ТабличнаяЧасть.МатериалыЗаказчика" Тогда
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточникаКалькуляция(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных, ОписанияТиповПолей)
	Перем ОписаниеТиповКолонка;
	
	Если НастройкиЗагрузкиДанных.ЗагрузкаКалькуляции Тогда
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ЭтоРасход", НСтр(
			"ru = 'Это расход'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповБулево, , , , ,
			НастройкиЗагрузкиДанных.ЗагрузкаКалькуляции);
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Себестоимость", НСтр(
		"ru = 'Себестоимость'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_2);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СебестоимостьЕдиницы", НСтр(
		"ru = 'Себестоимость ед.'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_2);
		
КонецПроцедуры

Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточникаЗапасы(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных, ОписанияТиповПолей)
	Перем ПолеВидимо;
	Перем ВидимостьСклада;
	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.Номенклатура");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПоляНоменклатуры(ТаблицаПолейЗагрузки, ОписаниеТиповКолонка,
		ОписанияТиповПолей, НастройкиЗагрузкиДанных);
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Количество", НСтр(
		"ru = 'Количество'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_3, , , Истина);

	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Резерв", НСтр(
		"ru = 'Резерв'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_3, , , , ,
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Цена", НСтр(
		"ru = 'Цена'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_2, , , Ложь);
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НастройкиЗагрузкиДанных, "ПоложениеДатыОтгрузки") Тогда
		ПолеВидимо = (НастройкиЗагрузкиДанных.ПоложениеДатыОтгрузки
			= Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти);
	Иначе
		ПолеВидимо = Ложь;
	КонецЕсли;
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДатаОтгрузки", НСтр(
		"ru = 'Дата отгрузки'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповДата, , , , ,
		ПолеВидимо);
	...
	Если ПолучитьФункциональнуюОпцию("УчетПоНесколькимСкладам")
		...
		ВидимостьСклада = НастройкиЗагрузкиДанных.ПоложениеСклада
			= Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти;
	...
	КонецЕсли;
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СтруктурнаяЕдиницаРезерв",
		НСтр("ru = 'Склад'"), ОписанияТиповПолей.ОписаниеТиповСтрока50, ОписаниеТиповКолонка, , , , , ВидимостьСклада);

КонецПроцедуры

Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточникаРаботы(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных, ОписанияТиповПолей)
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ЭтоРабота", НСтр(
		"ru = 'Это работа'", ОбщегоНазначения.КодОсновногоЯзыка()), ОписанияТиповПолей.ОписаниеТиповБулево,
		ОписанияТиповПолей.ОписаниеТиповБулево, , , , , Ложь);
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Количество", НСтр(
		"ru = 'Норма времени'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_3, , , Истина);
		
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Кратность", НСтр(
		"ru = 'Кратность'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15, , , Ложь);

	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Коэффициент", НСтр(
		"ru = 'Коэффициент'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15, , , Ложь);
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточникаМатериалы(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных, ОписанияТиповПолей)
	Перем ВидимостьСклада;
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточникаМатериалыЗаказчика(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных, ОписанияТиповПолей)
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ДатаПоступления", НСтр(
		"ru = 'Дата поступления'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповДата);

КонецПроцедуры

Процедура СопоставитьСтрокуЗагружаемыхДанныхКалькуляция(СтрокаТаблицыФормы, НастройкиПоиска, НастройкиЗагрузкиДанных)
	Перем ЭтоЗагрузкаКалькуляции;
	ЭтоЗагрузкаКалькуляции = НастройкиЗагрузкиДанных.ЗагрузкаКалькуляции;

	Если ЭтоЗагрузкаКалькуляции Тогда
		// Это расход
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВБулево(СтрокаТаблицыФормы.ЭтоРасход,
			СтрокаТаблицыФормы.ЭтоРасход_ВходящиеДанные);
	КонецЕсли;

	Если ЭтоЗагрузкаКалькуляции И СтрокаТаблицыФормы.ЭтоРасход Тогда
		// Расход по наименованию
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьСчетУчетаЗатрат(СтрокаТаблицыФормы.Номенклатура,
			СтрокаТаблицыФормы.НоменклатураНаименование, ПланыСчетов.Управленческий.ПустаяСсылка());
	Иначе
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуРаботы") Или ПолучитьФункциональнуюОпцию(
			"ИспользоватьПодсистемуПроизводство") Тогда
			Если ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура) Тогда
				ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьСпецификацию(
					СтрокаТаблицыФормы.Спецификация, СтрокаТаблицыФормы.Спецификация_ВходящиеДанные,
		...
		Если ПолучитьФункциональнуюОпцию("УчетВРазличныхЕдиницахИзмерения") Тогда
			ЗначениеПоУмолчанию = ?(ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура),
				СтрокаТаблицыФормы.Номенклатура.ЕдиницаИзмерения, Справочники.КлассификаторЕдиницИзмерения.шт);
	...
	// Себестоимость
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.Себестоимость,
		СтрокаТаблицыФормы.Себестоимость_ВходящиеДанные, 0);
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(
		СтрокаТаблицыФормы.СебестоимостьЕдиницы, СтрокаТаблицыФормы.СебестоимостьЕдиницы_ВходящиеДанные, 0);

КонецПроцедуры

Процедура СопоставитьСтрокуЗагружаемыхДанныхЗапасы(СтрокаТаблицыФормы, НастройкиПоиска, НастройкиЗагрузкиДанных)
	// Номенклатура по ШтрихКоду, Артикулу, Наименованию, НаименованиеПолное
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьНоменклатуру(СтрокаТаблицыФормы, НастройкиПоиска);
	
	// Содержание
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СкопироватьСтрокуВЗначениеСтроковогоТипа(
		СтрокаТаблицыФормы.Содержание, СтрокаТаблицыФормы.Содержание_ВходящиеДанные);
	...
	СтрокаТаблицыФормы.СчетУчетаДоходов = ?(СтрокаТаблицыФормы.Номенклатура.ТипНоменклатуры = ПредопределенноеЗначение(
		"Перечисление.ТипыНоменклатуры.ПодарочныйСертификат"), ПланыСчетов.Управленческий.ПрочиеДоходы,
		ПланыСчетов.Управленческий.ПустаяСсылка());
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицыФормы, "НаправлениеДеятельности") Тогда
		...
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьНаправлениеДеятельности(
			СтрокаТаблицыФормы.НаправлениеДеятельности, СтрокаТаблицыФормы.НаправлениеДеятельности_ВходящиеДанные,
			ЗначениеПоУмолчанию);
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицыФормы, "СпособПополнения") Тогда
		...
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьСпособПополнения(
			СтрокаТаблицыФормы.СпособПополнения, СтрокаТаблицыФормы.СпособПополнения_ВходящиеДанные,
			ЗначениеПоУмолчанию);
	...
	Если ПолучитьФункциональнуюОпцию("РезервированиеЗапасов") Тогда
		...
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.Резерв,
			СтрокаТаблицыФормы.Резерв_ВходящиеДанные, 0);
	...
	// СуммаНДС
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.СуммаНДС,
		СтрокаТаблицыФормы.СуммаНДС_ВходящиеДанные, 0);
	
	// ДатаОтгрузки
	Если НастройкиЗагрузкиДанных.ПоложениеДатыВЗаказе = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда

		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВДату(СтрокаТаблицыФормы.ДатаОтгрузки,
			СтрокаТаблицыФормы.ДатаОтгрузки_ВходящиеДанные);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуРаботы") Или ПолучитьФункциональнуюОпцию(
		...
		Если ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура) Тогда
			...
			ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьСпецификацию(СтрокаТаблицыФормы.Спецификация,
				СтрокаТаблицыФормы.Спецификация_ВходящиеДанные, СтрокаТаблицыФормы.Номенклатура);
	...
	// Склад
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьСтруктурнуюЕдиницу(
		СтрокаТаблицыФормы.СтруктурнаяЕдиницаРезерв, СтрокаТаблицыФормы.СтруктурнаяЕдиницаРезерв_ВходящиеДанные,
		Справочники.СтруктурныеЕдиницы.ОсновнойСклад);

КонецПроцедуры

Процедура СопоставитьСтрокуЗагружаемыхДанныхРаботы(СтрокаТаблицыФормы, НастройкиПоиска, НастройкиЗагрузкиДанных)
	// Номенклатура по ШтрихКоду, Артикулу, Наименованию, НаименованиеПолное
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьНоменклатуру(СтрокаТаблицыФормы, НастройкиПоиска);
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицыФормы, "ЭтоРабота_ВходящиеДанные") Тогда

		СтрокаТаблицыФормы.ЭтоРабота = СтрокаТаблицыФормы.ЭтоРабота_ВходящиеДанные;
			
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицыФормы, "ЭтоУслуга")
			И Не СтрокаТаблицыФормы.ЭтоУслуга Тогда
			
			СтрокаТаблицыФормы.ЭтоРабота = Истина;
	...
	// Кратность
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.Кратность,
		СтрокаТаблицыФормы.Кратность_ВходящиеДанные, 1);
		
	// Коэффициент
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВЧисло(СтрокаТаблицыФормы.Коэффициент,
		СтрокаТаблицыФормы.Коэффициент_ВходящиеДанные, 1);
...
Процедура СопоставитьСтрокуЗагружаемыхДанныхМатериалы(СтрокаТаблицыФормы, НастройкиПоиска, НастройкиЗагрузкиДанных)
	// Номенклатура по ШтрихКоду, Артикулу, Наименованию, НаименованиеПолное
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьНоменклатуру(СтрокаТаблицыФормы, НастройкиПоиска);
...
Процедура СопоставитьСтрокуЗагружаемыхДанныхМатериалыЗаказчика(СтрокаТаблицыФормы, НастройкиПоиска,
	НастройкиЗагрузкиДанных)
	// Номенклатура по ШтрихКоду, Артикулу, Наименованию, НаименованиеПолное
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьНоменклатуру(СтрокаТаблицыФормы, НастройкиПоиска);
	...
	// ДатаПоступления
	ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.ПреобразоватьСтрокуВДату(СтрокаТаблицыФормы.ДатаПоступления,
		СтрокаТаблицыФормы.ДатаПоступления_ВходящиеДанные);
...
Процедура СоздатьПоставляемыеШаблоныСообщений() Экспорт
	...
	Попытка
		...
		ШаблоныСообщенийУНФ.СоздатьШаблонEmail(НаименованиеШаблонаЗаказПринят(), ПараметрыШаблона, ВыбранныеКомандыПечати,
		...
		ПараметрыШаблона.ТипШаблона = "Письмо";
		ПараметрыШаблона.ПолноеИмяТипаНазначения = "Документ.ЗаказПокупателя";
		ШаблоныСообщенийУНФ.СоздатьШаблонEmail(НаименованиеШаблонаЗаказОплачен(), ПараметрыШаблона, ВыбранныеКомандыПечати,
			ДанныеШаблона.Вложения);
...
Процедура ПриОпределенииСобытий(События, Предмет) Экспорт
	...
	Если Предмет = "ЗаказПокупателя"
		Или Предмет = "ЗаказНаряд" Тогда
		
		НовоеСобытие = События.Добавить();
		НовоеСобытие.Предмет       = Предмет;
...
Процедура ПриОпределенииДействий(Действия) Экспорт
	...
	ПредметыДействий = Новый Массив;
	ПредметыДействий.Добавить("ЗаказПокупателя");
	ПредметыДействий.Добавить("ЗаказНаряд");
	
	Для каждого ПредметДействия Из ПредметыДействий Цикл
		// 1.
		НовоеДействие = Действия.Добавить();
		НовоеДействие.Предмет       = ПредметДействия;
...
Процедура ВыполнитьДействие(Предмет, Действие, ЗначенияЗаполнения, ДополнительныеПараметры, Результат) Экспорт
	...
	ИначеЕсли Действие.Идентификатор = "ОповеститьОформлениеОтгрузкиОплаты" Тогда
		...
		// Обработка произойдет в событии ПослеОбработкиСобытия.
		Возврат;
...
Процедура ВыполнитьКонтроль(ДокументОбъектЗаказПокупателя, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
		...
		+ СформироватьТекстЗапросаПрослеживаемыеТовары() // [8]
		+ СформироватьТекстЗапросаЗапасыКРасходуСоСкладов() // [9]
		+ СформироватьТекстЗапросаСерииНоменклатурыКРасходу() // [10]
		+ СформироватьТекстЗапросаРезервыТоваровОрганизаций(); // [11]
		...
		Если Константы.ВестиУчетПрослеживаемыхТоваров.Получить()
			И НЕ МассивРезультатов[8].Пустой() Тогда
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструПрослеживаемыеТовары(
				ДокументОбъектЗаказПокупателя, ВыборкаИзРезультатаЗапроса, Отказ);
		...
		Если НЕ МассивРезультатов[9].Пустой() Тогда
			...
			КонтрольОстатковУНФ.ЗапасыКРасходу(ДокументОбъектЗаказПокупателя, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		...
		Если НЕ МассивРезультатов[10].Пустой() Тогда
			...
			КонтрольОстатковУНФ.СерииНоменклатурыКРасходу(ДокументОбъектЗаказПокупателя, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		
		// Отрицательный остаток по запасам с учетом резервов к передаче
		Если НЕ МассивРезультатов[11].Пустой() И НЕ Отказ Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[11].Выбрать();
			КонтрольОстатковУНФ.РезервыКПередачиСУчетомОстатковПоЗапасамНаСкладах(ДокументОбъектЗаказПокупателя, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Функция СформироватьТекстЗапросаОстаткиЗапасы()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Запасы.Остатки(&МоментКонтроля, Организация В (&МассивОрганизаций)) КАК ЗапасыОстатки
	...
	|				&МоментКонтроля,
	|				Организация В (&МассивОрганизаций)
	|					И (СтруктурнаяЕдиница, ЗаказПокупателя, Номенклатура, Характеристика, Партия) В
	|						(ВЫБРАТЬ
	...
	|						ИЗ
	|							ДвиженияЗапасыИзменение КАК ДвиженияЗапасыИзменение)) КАК РезервыТоваровОрганизацийОстатки
	|		ПО (&ПередачаТоваровМеждуОрганизациями)
	|			И ДвиженияЗапасыИзменение.СтруктурнаяЕдиница = РезервыТоваровОрганизацийОстатки.СтруктурнаяЕдиница
	|			И ДвиженияЗапасыИзменение.ЗаказПокупателя = РезервыТоваровОрганизацийОстатки.ЗаказПокупателя
...
Функция СформироватьТекстЗапросаПрослеживаемыеТовары()
...
Функция СформироватьТекстЗапросаЗапасыКРасходуСоСкладов()
...
Функция СформироватьТекстЗапросаСерииНоменклатурыКРасходу()
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДвиженияСерииНоменклатурыКРасходуИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(СерииНоменклатурыКРасходуОстатки.КоличествоОстаток, 0) < 0
	|			ТОГДА -(ЕСТЬNULL(ДвиженияСерииНоменклатурыКРасходуИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(СерииНоменклатурыКРасходуОстатки.КоличествоОстаток, 0))
	|		ИНАЧЕ ЕСТЬNULL(ДвиженияСерииНоменклатурыКРасходуИзменение.КоличествоИзменение, 0) + ЕСТЬNULL(СерииНоменклатурыКРасходуОстатки.КоличествоОстаток, 0)
	|	КОНЕЦ КАК ОстатокЗапасыНаСкладах,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СерииНоменклатурыКРасходуОстатки.КоличествоОстаток, 0) < 0
	|			ТОГДА -ЕСТЬNULL(СерииНоменклатурыКРасходуОстатки.КоличествоОстаток, 0)
	|		ИНАЧЕ ЕСТЬNULL(СерииНоменклатурыКРасходуОстатки.КоличествоОстаток, 0)
	|	КОНЕЦ КАК КоличествоОстатокЗапасыНаСкладах
	|ИЗ
...
Функция СформироватьТекстЗапросаРезервыТоваровОрганизаций()
	...
	|	ДвиженияРезервыТоваровОрганизацийИзменение.Ячейка КАК ЯчейкаПредставление,
	|	ЗапасыОстатки.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы КАК ТипСтруктурнойЕдиницы,
	|	ЗапасыОстатки.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмеренияПредставление,
...
Процедура ПриДобавленииПараметровШаблона(НазначениеШаблона, ПараметрыШаблона) Экспорт
	...
	Если НазначениеШаблона = Перечисления.НазначенияШаблоновПечатиОфисныхДокументов.КоммерческоеПредложение Тогда
		...
		ДобавитьПараметрыЗаказаПокупателяВШаблон(НазначениеШаблона, ПараметрыЗаказа);
	...
	ИначеЕсли НазначениеШаблона = Перечисления.НазначенияШаблоновПечатиОфисныхДокументов.ДоговорКонтрагентаЗаказ
		...
		ДобавитьПараметрыЗаказаПокупателяВШаблон(НазначениеШаблона, ПараметрыШаблона);
...
Процедура ПриВыводеТаблицыПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода = Неопределено) Экспорт
	...
	Если ИмяТаблицы = "ЗаказПокупателя.Запасы" Тогда
		ПриВыводеТаблицыЗапасыПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода);
	ИначеЕсли ИмяТаблицы = "ЗаказПокупателя.ПлатежныйКалендарь" Тогда
		ПриВыводеТаблицыПлатежныйКалендарьПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода);
	ИначеЕсли ИмяТаблицы = "ЗаказПокупателя.МатериалыЗаказчика" Тогда
		ПриВыводеТаблицыМатериалыЗаказчикаПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода);
	КонецЕсли;
...
Процедура ПриВыводеТаблицыЗапасыПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода)
	...
	|		ВЫБОР
	|			КОГДА Характеристика.ФайлКартинки <> ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатурыПрисоединенныеФайлы.ПустаяСсылка)
	|				ТОГДА Характеристика.ФайлКартинки
	|			ИНАЧЕ Номенклатура.ФайлКартинки
...
Процедура ПриВыводеТаблицыПлатежныйКалендарьПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода)
	...
	Для каждого Строка Из ДанныеЗаказа.ТаблицаПлатежныйКалендарь Цикл
	...
	Если ЗначениеЗаполнено(СтруктураИтогов.СуммаПервойОплаты) Тогда
		СтруктураИтогов.СуммаПервойОплатыПрописью = РаботаСКурсамиВалют.СформироватьСуммуПрописью(
			СтруктураИтогов.СуммаПервойОплаты, ДанныеЗаказа.ВалютаДокумента);
	КонецЕсли;
...
Процедура ПриВыводеТаблицыМатериалыЗаказчикаПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода)
	...
	Для каждого Строка Из ДанныеЗаказа.ТаблицаМатериалыЗаказчика Цикл
		ПрефиксПараметраСтроки = ИмяТаблицы + ".";
		ДанныеПечати = Новый Соответствие;
		ДанныеПечати.Вставить(ПрефиксПараметраСтроки + "НомерСтроки", Строка.НомерСтроки);
		ДанныеПечати.Вставить(ПрефиксПараметраСтроки + "Номенклатура", Строка.Номенклатура);
		ДанныеПечати.Вставить(ПрефиксПараметраСтроки + "Характеристика", Строка.Характеристика);
		ДанныеПечати.Вставить(ПрефиксПараметраСтроки + "Количество", Строка.Количество);
		ДанныеПечати.Вставить(ПрефиксПараметраСтроки + "ЕдиницаИзмерения", Строка.ЕдиницаИзмерения);
		ШаблоныПечатиОфисныхДокументов.ЗаполнитьЗначенияИзСтруктуры(ОписаниеПараметров, ДанныеПечати);
		Если ЭтоВыводВФайл Тогда
			ШаблоныПечатиОфисныхДокументов.ВывестиСтрокуТаблицыДокумента(ОписаниеПараметров, ПараметрыВывода);
		КонецЕсли;
...
Процедура ДобавитьПараметрыЗаказаПокупателяВШаблон(НазначениеШаблона, ПараметрыШаблона)
	...
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыШаблона, "ЗаказПокупателя.СуммаДокументаПрописью", НСтр("ru='Сумма документа (прописью)'"));
	...
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыСчетаДляОплаты, "ЗаказПокупателя.БанковскийСчетДляОплаты", НСтр("ru='Банковский счет (полное представление)'"));
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыСчетаДляОплаты, "ЗаказПокупателя.БанковскийСчетДляОплатыПодробно.БанковскийСчетДляОплатыБанкСВИФТБИК", НСтр("ru='SWIFT'"));
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыСчетаДляОплаты, "ЗаказПокупателя.БанковскийСчетДляОплатыПодробно.БанковскийСчетДляОплатыБанкНаименование", НСтр("ru='Банк'"));
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыСчетаДляОплаты, "ЗаказПокупателя.БанковскийСчетДляОплатыПодробно.БанковскийСчетДляОплатыБанкКод", НСтр("ru='БИК'"));
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыСчетаДляОплаты, "ЗаказПокупателя.БанковскийСчетДляОплатыПодробно.БанковскийСчетДляОплатыБанкКоррСчет", НСтр("ru='Корр. счет'"));
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыСчетаДляОплаты, "ЗаказПокупателя.БанковскийСчетДляОплатыПодробно.БанковскийСчетДляОплатыНомерСчета", НСтр("ru='Номер счета'"));
	...
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыНоменклатурыТаблицыЗапасы, "ЗаказПокупателя.Запасы.Номенклатура.Изображение", НСтр("ru='Изображение номенклатуры'"), ПрефиксТаблицаВладелец,,,, ШаблоныПечатиОфисныхДокументов.ВидПараметраИзображение());
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыНоменклатурыТаблицыЗапасы, "ЗаказПокупателя.Запасы.Номенклатура.ИзображениеХарактеристики", НСтр("ru='Изображение характеристики'"), ПрефиксТаблицаВладелец,,,, ШаблоныПечатиОфисныхДокументов.ВидПараметраИзображение());
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыНоменклатурыТаблицыЗапасы, "ЗаказПокупателя.Запасы.Номенклатура.ИзображениеАвто", НСтр("ru='Изображение номенклатуры/характеристики'"), ПрефиксТаблицаВладелец,,,, ШаблоныПечатиОфисныхДокументов.ВидПараметраИзображение());
	...
	Если НазначениеШаблона = Перечисления.НазначенияШаблоновПечатиОфисныхДокументов.ДоговорКонтрагентаЗаказНаряд Тогда
		ПараметрыТаблицыЗапасы = ШаблоныПечатиОфисныхДокументов.ДобавитьГруппуПараметровШаблона(ПараметрыШаблона, НСтр("ru='Материалы заказчика'"));
		ПрефиксТаблицаВладелец = "ЗаказПокупателя.МатериалыЗаказчика";
		ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыТаблицыЗапасы, "ЗаказПокупателя.МатериалыЗаказчика.НомерСтроки", НСтр("ru='Номер строки'"), ПрефиксТаблицаВладелец,,, ТипКоличество);
		ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыТаблицыЗапасы, "ЗаказПокупателя.МатериалыЗаказчика.Номенклатура", НСтр("ru='Материал (представление)'"), ПрефиксТаблицаВладелец);
		ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыТаблицыЗапасы, "ЗаказПокупателя.МатериалыЗаказчика.Количество", НСтр("ru='Количество'"), ПрефиксТаблицаВладелец,,, ТипКоличество);
		ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыТаблицыЗапасы, "ЗаказПокупателя.МатериалыЗаказчика.ЕдиницаИзмерения", НСтр("ru='Единица измерения'"), ПрефиксТаблицаВладелец);
	КонецЕсли;
	...
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ГруппаПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.ДатаПервойОплаты", НСтр("ru='Дата первой оплаты'"), "ДЛФ=Д", ШаблоныПечатиОфисныхДокументов.ДопустимыеФорматыДаты());
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ГруппаПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.ПроцентПервойОплаты", НСтр("ru='Процент первой оплаты'"));
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ГруппаПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.СуммаПервойОплаты", НСтр("ru='Сумма первой оплаты'"), ФорматЦены,, ТипЦены);
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ГруппаПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.СуммаПервойОплатыПрописью", НСтр("ru='Сумма первой оплаты (прописью)'"));
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ГруппаПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.СуммаНДСПервойОплаты", НСтр("ru='Сумма НДС первой оплаты'"), ФорматЦены,, ТипЦены);
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ГруппаПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.ДатаПоследнейОплаты", НСтр("ru='Дата последней оплаты'"), "ДЛФ=Д", ШаблоныПечатиОфисныхДокументов.ДопустимыеФорматыДаты());
	...
	ПрефиксТаблицаВладелец = "ЗаказПокупателя.ПлатежныйКалендарь";
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыТаблицыПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.НомерСтроки", НСтр("ru='Номер строки'"), ПрефиксТаблицаВладелец,,, ТипКоличество);
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыТаблицыПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.ДатаОплаты", НСтр("ru='Дата оплаты'"), ПрефиксТаблицаВладелец, "ДЛФ=Д", ШаблоныПечатиОфисныхДокументов.ДопустимыеФорматыДаты());
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыТаблицыПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.ПроцентОплаты", НСтр("ru='Процент оплаты'"), ПрефиксТаблицаВладелец, ФорматСкидки,, ТипСкидки);
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыТаблицыПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.СуммаОплаты", НСтр("ru='Сумма оплаты'"), ПрефиксТаблицаВладелец, ФорматЦены,, ТипЦены);
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыТаблицыПлатежныйКалендарь, "ЗаказПокупателя.ПлатежныйКалендарь.СуммаНДСОплаты", НСтр("ru='Сумма НДС'"), ПрефиксТаблицаВладелец, ФорматЦены,, ТипЦены);
...
Процедура ЗаполнитьЗначенияПараметровЗаказа(ОбъектПечати, ОписаниеПараметров, ДополнительныеПараметры)
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(ЗаказПокупателя.ТекстУсловийСчетаЗаказа КАК СТРОКА(1000)) <> """"
	|			ТОГДА ЗаказПокупателя.ТекстУсловийСчетаЗаказа
	|		ИНАЧЕ ЗаказПокупателя.УсловияСчетаЗаказа.ТекстУсловий
	|	КОНЕЦ КАК УсловияЗаказа,
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(ЗаказПокупателя.ТекстУсловийКоммерческогоПредложения КАК СТРОКА(1000)) <> """"
	|			ТОГДА ЗаказПокупателя.ТекстУсловийКоммерческогоПредложения
	|		ИНАЧЕ ЗаказПокупателя.УсловияКоммерческогоПредложения.ТекстУсловий
	|	КОНЕЦ КАК УсловияКоммерческогоПредложения,
	...
	|	ЗаказПокупателя.БанковскийСчет КАК БанковскийСчетДляОплаты,
	|	ЗаказПокупателя.БанковскийСчет.Банк.СВИФТБИК КАК БанковскийСчетДляОплатыБанкСВИФТБИК,
	|	ЗаказПокупателя.БанковскийСчет.Банк.Наименование КАК БанковскийСчетДляОплатыБанкНаименование,
	|	ЗаказПокупателя.БанковскийСчет.Банк.Код КАК БанковскийСчетДляОплатыБанкКод,
	|	ЗаказПокупателя.БанковскийСчет.Банк.КоррСчет КАК БанковскийСчетДляОплатыБанкКоррСчет,
	|	ЗаказПокупателя.БанковскийСчет.НомерСчета КАК БанковскийСчетДляОплатыНомерСчета,
	|	ЗаказПокупателя.Запасы.(
	|		НомерСтроки КАК НомерСтроки,
	|		ВЫБОР
	...
	ШаблоныПечатиОфисныхДокументов.ЗаполнитьЗначенияИзСтрокиТаблицыЗначения(ОписаниеПараметров, Результат.Колонки, ДанныеЗаказа, "ЗаказПокупателя.БанковскийСчетДляОплатыПодробно.");
	...
	ПриВыводеТаблицыПараметровШаблона(ДополнительныеПараметры, "ЗаказПокупателя.Запасы", ОписаниеПараметров);
	ПриВыводеТаблицыПараметровШаблона(ДополнительныеПараметры, "ЗаказПокупателя.ПлатежныйКалендарь", ОписаниеПараметров);
...
Процедура ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиДокумента(СтрокаТабличнойЧасти, ОписаниеПараметров, СтруктураИтогов, ПрефиксПараметраСтроки, ПараметрыВывода)
	...
	Если ЭтоВыводВФайл Тогда
		...
		Иначе
			...
			ЗаполнитьДанныеПечатиИзображениеНоменклатуры(ОписаниеПараметров, СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.ХарактеристикаФайлКартинки, ПрефиксПараметраНоменклатура + "ИзображениеХарактеристики", ПараметрыВывода);
			ЗаполнитьДанныеПечатиИзображениеНоменклатуры(ОписаниеПараметров, СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.НоменклатураХарактеристикаФайлКартинки, ПрефиксПараметраНоменклатура + "ИзображениеАвто", ПараметрыВывода);
		КонецЕсли;
...
Функция УниверсальныйЗапросПоДаннымДокумента(МассивОбъектов, ИспользоватьФаксимиле, Ошибки)
	...
	|		НоменклатураНабора КАК НоменклатураНабора,
	|		ХарактеристикаНабора КАК ХарактеристикаНабора,
	|		ВЫРАЗИТЬ(ЗаказПокупателя.Работы.Цена * ЗаказПокупателя.Работы.Количество КАК ЧИСЛО(15, 2)) КАК СуммаБезСкидки
	|	) КАК ТаблицаРаботыУслуги,
	|	ЗаказПокупателя.ПлатежныйКалендарь.(
	|		ДатаОплаты КАК ДатаОплаты,
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|				И НЕ ЗаказПокупателя.Организация = ЗаказПокупателя.Организация.ГоловнаяОрганизация
	...
	|		НоменклатураНабора КАК НоменклатураНабора,
	|		ХарактеристикаНабора КАК ХарактеристикаНабора,
	|		Ссылка.Финиш КАК ДатаОтгрузочногоДокумента,
	...
	|	) КАК ТаблицаРаботыУслуги,
	|	ЗаказПокупателя.СведенияПрослеживаемости.(
	|		Ссылка КАК Ссылка,
	|		НомерСтроки КАК НомерСтроки,
	|		РНПТ КАК РНПТ,
	|		Количество КАК Количество,
	...
	|	ЗаказПокупателя.ЭтоУниверсальныйДокумент КАК ЭтоУниверсальныйДокумент,
	...
	|	ЗаказПокупателяЗапасы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ЗаказПокупателяЗапасы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ЗаказПокупателяЗапасы.КодТНВЭД КАК КодТНВЭД,
	...
	|	ЗаказПокупателяЗапасы.Ссылка.Финиш КАК ДатаОтгрузочногоДокумента,
	...
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК КодТНВЭД,
	|	ЗаказПокупателяЗапасы.Штрихкод КАК Штрихкод,
	|	ЗаказПокупателяЗапасы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ЗаказПокупателяЗапасы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ВЫБОР
	|		КОГДА ЗаказПокупателяЗапасы.ПрослеживаемыйТовар
	|			ТОГДА 1
	|		ИНАЧЕ 0
	...
	|		КОГДА ЗаказПокупателяЗапасы.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)
	|			ТОГДА ВЫБОР
	|					КОГДА ЗаказПокупателяЗапасы.КодТНВЭД = ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка)
	|							ИЛИ ЗаказПокупателяЗапасы.Ссылка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|							ИЛИ НЕ ЗаказПокупателяЗапасы.Ссылка.Контрагент.СтранаРегистрации.УчастникЕАЭС
	|						ТОГДА ЗаказПокупателяЗапасы.Номенклатура.ТоварнаяНоменклатураВЭД.ЕдиницаИзмерения.Код
	|					ИНАЧЕ ЗаказПокупателяЗапасы.КодТНВЭД.ЕдиницаИзмерения.Код
	|				КОНЕЦ
	|		ИНАЧЕ """"
	...
	|							ИЛИ ЗаказПокупателяЗапасы.Ссылка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|							ИЛИ НЕ ЗаказПокупателяЗапасы.Ссылка.Контрагент.СтранаРегистрации.УчастникЕАЭС
	|						ТОГДА ЗаказПокупателяЗапасы.Номенклатура.ТоварнаяНоменклатураВЭД.ЕдиницаИзмерения.Наименование
	|					ИНАЧЕ ЗаказПокупателяЗапасы.КодТНВЭД.ЕдиницаИзмерения.Наименование
	|				КОНЕЦ
	|		ИНАЧЕ """"
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК НеобходимоВыделитьКакСоставНабора,
	|	ЗаказПокупателяРаботы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ЗаказПокупателяРаботы.Ссылка.СуммаВключаетНДС КАК СуммаВключаетНДС,
	|	ЗаказПокупателяРаботы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ЗаказПокупателяРаботы.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	...
	|	НЕОПРЕДЕЛЕНО КАК СтранаКод,
	|	НЕОПРЕДЕЛЕНО КАК ПредставлениеГТД,
	|	ЗаказПокупателяРаботы.Ссылка.Финиш КАК ДатаОтгрузочногоДокумента,
	...
	|	0 КАК СтоимостьБезНалога,
	|	ВЫБОР
	|		КОГДА ЗаказПокупателяРаботы.ПрослеживаемыйТовар
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ПрослеживаемыйТоварЧисло,
	|	ЗаказПокупателяРаботы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ЗаказПокупателяРаботы.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект,
	|	ЗаказПокупателяРаботы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	"""" КАК КодТНВЭДЕдиницаИзмеренияКод,
	|	"""" КАК КодТНВЭДЕдиницаИзмеренияНаименование
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗаказПокупателяСведенияПрослеживаемости.НомерСтроки КАК НомерСтроки,
	|	ЗаказПокупателяСведенияПрослеживаемости.РНПТ КАК РНПТ,
	|	ЗаказПокупателяСведенияПрослеживаемости.Количество КАК Количество,
	|	ЗаказПокупателяСведенияПрослеживаемости.КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|	ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ЗаказПокупателяСведенияПрослеживаемости.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ЗаказПокупателяСведенияПрослеживаемости.Номенклатура КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА ЗаказПокупателяСведенияПрослеживаемости.Сумма = 0
	|			ТОГДА ЗаказПокупателяСведенияПрослеживаемости.Количество * ВЫБОР
	|					КОГДА ВременнаяТаблица_ЗаказПокупателяЗапасы.СуммаВключаетНДС
	|						ТОГДА ВЫРАЗИТЬ((ВременнаяТаблица_ЗаказПокупателяЗапасы.Сумма - ВременнаяТаблица_ЗаказПокупателяЗапасы.СуммаНДС) / ВременнаяТаблица_ЗаказПокупателяЗапасы.Количество КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ВременнаяТаблица_ЗаказПокупателяЗапасы.Цена
	|				КОНЕЦ
	|		ИНАЧЕ ЗаказПокупателяСведенияПрослеживаемости.Сумма
	...
	|ИЗ
	|	ВременнаяТаблица_ЗаказПокупателяЗапасы КАК ВременнаяТаблица_ЗаказПокупателяЗапасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.СведенияПрослеживаемости КАК ЗаказПокупателяСведенияПрослеживаемости
	|		ПО ВременнаяТаблица_ЗаказПокупателяЗапасы.Ссылка = ЗаказПокупателяСведенияПрослеживаемости.Ссылка
	|			И ВременнаяТаблица_ЗаказПокупателяЗапасы.ИдентификаторСтроки = ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЗаказПокупателяСведенияПрослеживаемости.НомерСтроки,
	|	ЗаказПокупателяСведенияПрослеживаемости.РНПТ,
	|	ЗаказПокупателяСведенияПрослеживаемости.Количество,
	|	ЗаказПокупателяСведенияПрослеживаемости.КоличествоПрослеживаемости,
	|	ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки,
	|	ЗаказПокупателяСведенияПрослеживаемости.СтранаПроисхождения,
	|	ЗаказПокупателяСведенияПрослеживаемости.Номенклатура,
	|	ВЫБОР
	|		КОГДА ЗаказПокупателяСведенияПрослеживаемости.Сумма = 0
	|			ТОГДА ЗаказПокупателяСведенияПрослеживаемости.Количество * ВЫБОР
	|					КОГДА ВременнаяТаблица_ЗаказПокупателяРаботы.СуммаВключаетНДС
	|						ТОГДА ВЫРАЗИТЬ((ВременнаяТаблица_ЗаказПокупателяРаботы.Сумма - ВременнаяТаблица_ЗаказПокупателяРаботы.СуммаНДС) / ВременнаяТаблица_ЗаказПокупателяРаботы.Количество КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ВременнаяТаблица_ЗаказПокупателяРаботы.Цена
	|				КОНЕЦ
	|		ИНАЧЕ ЗаказПокупателяСведенияПрослеживаемости.Сумма
	|	КОНЕЦ,
	|	ЗаказПокупателяСведенияПрослеживаемости.Номенклатура.ТоварнаяНоменклатураВЭД.ЕдиницаИзмерения.Код,
	|	ЗаказПокупателяСведенияПрослеживаемости.Номенклатура.ТоварнаяНоменклатураВЭД.ЕдиницаИзмерения,
	|	ЗаказПокупателяСведенияПрослеживаемости.СтранаПроисхождения,
	|	ЗаказПокупателяСведенияПрослеживаемости.СтранаПроисхождения.Код
	|ИЗ
	|	ВременнаяТаблица_ЗаказПокупателяРаботы КАК ВременнаяТаблица_ЗаказПокупателяРаботы
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.СведенияПрослеживаемости КАК ЗаказПокупателяСведенияПрослеживаемости
	|		ПО ВременнаяТаблица_ЗаказПокупателяРаботы.Ссылка = ЗаказПокупателяСведенияПрослеживаемости.Ссылка
	|			И ВременнаяТаблица_ЗаказПокупателяРаботы.ИдентификаторСтроки = ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки";
	...
	Для Каждого ТекущаяСтрока Из Результат Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			Если ТекущаяСтрока.ЕстьПрослеживаемыеТовары Тогда
				...
				ЕстьПрослеживаемыеЗапасы = Истина;
			КонецЕсли;
		...
		ЕстьПрослеживаемыеРаботы = Ложь;
		Если НайденныеСтроки.Количество() > 0 Тогда
			ТекущаяСтрока.ТаблицаРаботыУслуги = ТЗСтрокРаботыУслуги.Скопировать(НайденныеСтроки);
			
			ТекущаяСтрока.ЕстьПрослеживаемыеТовары = (ТекущаяСтрока.ТаблицаРаботыУслуги.Итог("ПрослеживаемыйТоварЧисло") > 0);
			Если ТекущаяСтрока.ЕстьПрослеживаемыеТовары Тогда
				ДополнитьСведениямиОПрослеживаемости(ТзСтрокСведенияПрослеживаемости, ТекущаяСтрока.ТаблицаРаботыУслуги);
				ЕстьПрослеживаемыеРаботы = Истина;
			КонецЕсли;
			
		КонецЕсли;
		
		Если ЕстьПрослеживаемыеРаботы И НЕ ЕстьПрослеживаемыеЗапасы Тогда
			ТекущаяСтрока.ТаблицаЗапасы.Колонки.Добавить("СведенияОПрослеживаемости");
		КонецЕсли;
...
Функция НовыйТаблицаСведенийОПрослеживаемости()
	...
	ТаблицаСведенийОПрослеживаемости.Колонки.Добавить("КодТНВЭДЕдиницаИзмеренияКод");
	ТаблицаСведенийОПрослеживаемости.Колонки.Добавить("КодТНВЭДЕдиницаИзмеренияНаименование");
	ТаблицаСведенийОПрослеживаемости.Колонки.Добавить("СтранаПроисхожденияКод");
	ТаблицаСведенийОПрослеживаемости.Колонки.Добавить("ПредставлениеСтраны");
...
Процедура СформироватьЗаказПокупателя(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	...
	|	ЗаказПокупателя.СуммаНДСДоставки КАК СуммаНДСДоставки,
	|	ЗаказПокупателя.СтавкаНДСДоставки КАК СтавкаНДСДоставки,
	|	ЗаказПокупателя.УсловияЗаказаПокупателя КАК УсловияЗаказаПокупателя,
	|	ЗаказПокупателя.ТекстУсловийЗаказаПокупателя КАК ТекстУсловийЗаказаПокупателя,
	|	ЗаказПокупателя.Работы.(
	|		НомерСтроки КАК НомерСтроки,
	|		0 КАК НомерВариантаКП,
	|		ВЫБОР
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		Если НЕ ЕстьШапкаТаблицыСоСкидкойСуммаБезСкидки И ВыводитьСуммуБезСкидок Тогда
			...
			ТекстСообщения = НСтр(
			"ru = 'Изменился типовой макет печатной формы Заказ покупателя. Для корректной
		...
		Если ЕстьСкидки Тогда
			Если ВыводитьСуммуБезСкидок И ЕстьШапкаТаблицыСоСкидкойСуммаБезСкидки Тогда
				ОбластьМакета = Макет.ПолучитьОбласть("ШапкаТаблицыСоСкидкойСуммаБезСкидки");
				ТабличныйДокумент.Вывести(ОбластьМакета);
				ОбластьМакета = Макет.ПолучитьОбласть("СтрокаСоСкидкойСуммаБезСкидки");
				ОбластьГруппаРазделитель = Макет.ПолучитьОбласть("СтрокаСоСкидкойГруппировкаСуммаБезСкидки");
		...
		СтруктураИтогов.Вставить("СуммаБезСкидкиПоДокументу", 0);
		СтруктураИтогов.Вставить("СуммаБезСкидкиПоСтроке", 0);
		...
		ДанныеПечати.Вставить("РасшифровкаПодписиКонтрагента", Шапка.РасшифровкаПодписиКонтрагента);		
		ДанныеПечати.Вставить("ТекстДополнительныхУсловий", Шапка.ТекстУсловийЗаказаПокупателя);		
		...
		Если СтруктураИтогов.ЕстьСкидки Тогда
			...
			//Если ИспользоватьШапкуСуммаБезСкидки Тогда
			//	
			//	ОбластьМакета = ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(Макет, "ИтогоСуммаБезСкидки", "", Ошибки);
			//	Если ОбластьМакета <> Неопределено Тогда
			//		
			//		ОбластьМакета.Параметры.Заполнить(СтруктураИтогов);
			//		ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		Если ОбластьМакета <> Неопределено И ЗначениеЗаполнено(Шапка.УсловияЗаказаПокупателя) Тогда
			Если НЕ ПустаяСтрока(Шапка.ТекстУсловийЗаказаПокупателя) Тогда
				ТекстДополнительныхУсловий = Шапка.ТекстУсловийЗаказаПокупателя;
			Иначе
				ТекстДополнительныхУсловий = 
					ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Шапка.УсловияЗаказаПокупателя, "ТекстУсловий");
			КонецЕсли;
			...
			Для СчетчикСтрок = 1 По ЧислоСтрокТекста Цикл
				ДанныеПечати.Вставить("ТекстДополнительныхУсловий",
...
Процедура СформироватьЗаказНаряд(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЕСТЬNULL(ФИОФизическихЛиц.Фамилия, """") КАК Фамилия,
	|	ЕСТЬNULL(ФИОФизическихЛиц.Имя, """") КАК Имя,
	|	ЕСТЬNULL(ФИОФизическихЛиц.Отчество, """") КАК Отчество,
	...
	|			ПО ЗаказПокупателяИсполнители.Сотрудник.Физлицо = ФИОФизическихЛиц.ФизическоеЛицо
	|				И (ФИОФизическихЛиц.Период <= ЗаказПокупателяИсполнители.Ссылка.Дата)
	|	ГДЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц КАК ФИОФизическихЛиц
	|		ПО МаксимальныеПериоды.Физлицо = ФИОФизическихЛиц.ФизическоеЛицо
	|			И МаксимальныеПериоды.Период = ФИОФизическихЛиц.Период";
...
Процедура СформироватьКоммерческоеПредложение(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	...
	|	ВЫБОР
	|		КОГДА (ВЫРАЗИТЬ(ЗаказПокупателя.ТекстУсловийКоммерческогоПредложения КАК СТРОКА(1000))) = """"
	|			ТОГДА ЗаказПокупателя.УсловияКоммерческогоПредложения.ТекстУсловий
	|		ИНАЧЕ ЗаказПокупателя.ТекстУсловийКоммерческогоПредложения
	|	КОНЕЦ КАК ТекстУсловийКоммерческогоПредложения,
	|	ЗаказПокупателя.Ответственный КАК Ответственный,
	|	ЗаказПокупателя.ОжидаетсяВыборВариантаКП КАК ОжидаетсяВыборВариантаКП,
	|	ЗаказПокупателя.ОсновнойВариантКП КАК ОсновнойВариантКП,
	|	ЗаказПокупателя.КоличествоВариантовКП КАК КоличествоВариантовКП,
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		Если ЗначениеЗаполнено(Шапка.ДополнительныеУсловия) Тогда
			...
			Если ОбластьМакета <> Неопределено Тогда
				ТабличныйДокумент.Вывести(ОбластьМакета);
				ТекстДополнительныхУсловий = Шапка.ТекстУсловийКоммерческогоПредложения; 
				ЧислоСтрокТекста = СтрЧислоСтрок(ТекстДополнительныхУсловий);
				Для СчетчикСтрок = 1 По ЧислоСтрокТекста Цикл
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = Обработки.ПечатьТН.ПредставлениеПФ();
	КомандаПечати.СписокФорм = "ФормаЗаказНаряда,ФормаСпискаЗаказНаряда,РеквизитыПечатиЗаказНаряд";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
Процедура ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиЗаказПокупателя(СтрокаТабличнойЧасти, ДанныеПечати, ПараметрыНоменклатуры, СтруктураИтогов)
	...
	Если НЕ СтрокаТабличнойЧасти.ЭтоНабор Тогда
		...
		СтруктураИтогов.СуммаБезСкидкиПоДокументу = СтруктураИтогов.СуммаБезСкидкиПоДокументу + СтрокаТабличнойЧасти.СуммаБезСкидки;
	
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиЗаказНаряд(СтрокаТабличнойЧасти, ДанныеПечати,
	ПараметрыНоменклатуры, СтруктураИтогов)
	...
	Если СтруктураИтогов.ЕстьСкидки Тогда
		...
		ПараметрыСтрокиТЧ = Новый Структура;
		ПараметрыСтрокиТЧ.Вставить("Количество", 0); 
		ПараметрыСтрокиТЧ.Вставить("Цена", 0);
		ПараметрыСтрокиТЧ.Вставить("Сумма", 0);
		ПараметрыСтрокиТЧ.Вставить("ПроцентСкидкиНаценки", 0);
		ПараметрыСтрокиТЧ.Вставить("СуммаАвтоматическойСкидки", 0);
		ЗаполнитьЗначенияСвойств(ПараметрыСтрокиТЧ, СтрокаТабличнойЧасти);
		
		ДанныеПечати.Вставить("ПредставлениеСкидки", ПечатьДокументовУНФ.ПредставлениеСкидки(ПараметрыСтрокиТЧ,
			СтруктураИтогов));
...
Функция ТекстЗапросаИнициализироватьДанныеДокументаЗаказПокупателя()
	...
	|	&Организация КАК Организация,
	|	ЗаказПокупателяТовары.СтруктурнаяЕдиницаРезерв КАК СтруктурнаяЕдиница,
	|	ВЫБОР
	|		КОГДА ЗаказПокупателяТовары.СтруктурнаяЕдиницаРезерв.ОрдерныйСклад
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОрдерныйСклад,
	|	ЗаказПокупателяТовары.Номенклатура.СчетУчетаЗапасов КАК СчетУчета,
	|	ЗаказПокупателяТовары.Номенклатура.СчетУчетаЗатрат КАК СчетУчетаЗатрат,
	...
	|			ТОГДА ЗаказПокупателяТовары.Количество * ВЫРАЗИТЬ(ЗаказПокупателяТовары.ЕдиницаИзмерения КАК Справочник.ЕдиницыИзмерения).Коэффициент
	...
	|						ТОГДА ЗаказПокупателяТовары.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|					ИНАЧЕ ЗаказПокупателяТовары.СуммаНДС * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	|				КОНЕЦ
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА ЗаказПокупателяТовары.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ЗаказПокупателяТовары.Всего * ВременнаяТаблицаШапка.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ВременнаяТаблицаШапка.Кратность)
	...
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|						ТОГДА ЗаказПокупателяТовары.СуммаНДС * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность / (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	|					ИНАЧЕ ЗаказПокупателяТовары.СуммаНДС
	|				КОНЕЦ
	...
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|						ТОГДА ЗаказПокупателяТовары.СуммаНДС
	|					ИНАЧЕ ЗаказПокупателяТовары.СуммаНДС * ВременнаяТаблицаШапка.Курс / ВременнаяТаблицаШапка.Кратность
	|				КОНЕЦ
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДСРег,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА ЗаказПокупателяТовары.Всего * КурсыРегВалюты.Курс * ВременнаяТаблицаШапка.Кратность / (ВременнаяТаблицаШапка.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ЗаказПокупателяТовары.Всего
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаВал,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ВременнаяТаблицаШапка.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА ЗаказПокупателяТовары.Всего
	|			ИНАЧЕ ЗаказПокупателяТовары.Всего * ВременнаяТаблицаШапка.Курс / ВременнаяТаблицаШапка.Кратность
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаРег,
	|	ЗаказПокупателяТовары.Всего КАК СуммаРасчетовПринятыеПереданные,
	...
	|	ЗаказПокупателяТовары.СуммаНДС КАК СуммаНДСВалютаДокумента,
	|	ЗаказПокупателяТовары.Всего КАК ВсегоВалютаДокумента,
	|	ЗаказПокупателяТовары.Номенклатура КАК Номенклатура,
	|	ЗаказПокупателяТовары.КлючСвязи КАК КлючСвязи,
	|	ЕСТЬNULL(ПолитикиУчетаСерий.ПолитикаУчетаСерий, ЗаказПокупателяТовары.Номенклатура.ПолитикаУчетаСерий) КАК ПолитикаУчетаСерий
	...
	|			ПО ЗаказПокупателяТовары.Номенклатура = ПолитикиУчетаСерий.Владелец
	|				И ЗаказПокупателяТовары.СтруктурнаяЕдиницаРезерв = ПолитикиУчетаСерий.СтруктурнаяЕдиница
	|				И (&Организация = ПолитикиУчетаСерий.Организация)
	|		ПО ВременнаяТаблицаШапка.Ссылка = ЗаказПокупателяТовары.Ссылка
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗаказПокупателяБонусныеБаллыКНачислению.НомерСтроки КАК НомерСтроки,
	|	ЗаказПокупателяБонусныеБаллыКНачислению.Ссылка.Дата КАК Период,
	|	ЗаказПокупателяБонусныеБаллыКНачислению.Ссылка.ДисконтнаяКарта КАК БонуснаяКарта,
	|	ЗаказПокупателяЗапасы.Номенклатура КАК Номенклатура,
	|	ЗаказПокупателяЗапасы.Характеристика КАК Характеристика,
	|	ЗаказПокупателяБонусныеБаллыКНачислению.КоличествоБонусныхБаллов КАК Количество,
	|	ЗаказПокупателяБонусныеБаллыКНачислению.ДатаНачисления КАК ДатаНачисления,
	|	ЗаказПокупателяБонусныеБаллыКНачислению.ДатаСписания КАК ДатаСгорания,
	|	ЗаказПокупателяБонусныеБаллыКНачислению.Ссылка КАК Ссылка,
	|	ЗаказПокупателяБонусныеБаллыКНачислению.СкидкаНаценка КАК АналитикаНачисленияБонусов
	|ПОМЕСТИТЬ ВременнаяТаблицаНачисленияБонусов
	|ИЗ
	|	Документ.ЗаказПокупателя.БонусныеБаллыКНачислению КАК ЗаказПокупателяБонусныеБаллыКНачислению
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.Запасы КАК ЗаказПокупателяЗапасы
	|		ПО ЗаказПокупателяБонусныеБаллыКНачислению.Ссылка = ЗаказПокупателяЗапасы.Ссылка
	|			И ЗаказПокупателяБонусныеБаллыКНачислению.КлючСвязи = ЗаказПокупателяЗапасы.КлючСвязи
	|ГДЕ
	|	ЗаказПокупателяБонусныеБаллыКНачислению.Ссылка = &Ссылка
	|	И ЗаказПокупателяБонусныеБаллыКНачислению.НомерВариантаКП = ЗаказПокупателяБонусныеБаллыКНачислению.Ссылка.ОсновнойВариантКП
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗаказПокупателяПредоплата.НомерСтроки КАК НомерСтроки,
	|	НАЧАЛОПЕРИОДА(ЗаказПокупателяПредоплата.Ссылка.Дата, ДЕНЬ) КАК Период,
	|	ЗаказПокупателяПредоплата.Документ КАК БонуснаяКарта,
	|	ЗаказПокупателяПредоплата.СуммаРасчетов КАК Количество,
	|	0 КАК КСписанию,
	...
	|	И ЗаказПокупателяПредоплата.ОплатаБонусами = ИСТИНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПредъявленныеПромокоды.Ссылка КАК Ссылка,
	|	ПредъявленныеПромокоды.НомерСтроки КАК НомерСтроки,
	|	ПредъявленныеПромокоды.Промокод КАК Промокод,
	|	ПредъявленныеПромокоды.УсловиеПредоставленияСкидки КАК УсловиеПредоставленияСкидки,
	|	-1 КАК Доступно,
	|	ЗарегистрированныеПромокоды.ДействуетС КАК ДействуетС,
	|	ЗарегистрированныеПромокоды.ДействуетДо КАК ДействуетДо,
	|	ЗарегистрированныеПромокоды.Конечный КАК Конечный,
	|	ПредъявленныеПромокоды.ЭтоАктивацияПромокода КАК ЭтоАктивацияПромокода
	|ПОМЕСТИТЬ ВременнаяТаблицаПромокоды
	|ИЗ
	|	Документ.ЗаказПокупателя.ПримененныеПромокоды КАК ПредъявленныеПромокоды
...
Процедура ИнициализироватьДанныеДокументаЗаказПокупателя(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства, ДокументОбъектЗаказПокупателя) 
	...
	// Бонусы
	СформироватьТаблицаБонусныеБаллыЗаказПокупателя(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
	СформироватьТаблицаНачисленияБонусныхБалловЗаказПокупателя(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
	// Промокоды
	СформироватьТаблицаДоступностьПромокодовЗаказПокупателя(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства); // Гашение примененных промокодов
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаПрименениеПромокодов", Новый ТаблицаЗначений);
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаПромокоды", Новый ТаблицаЗначений);
	// Эквайринг
...
Процедура СформироватьТаблицаЗапасыЗаказПокупателя(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	|	КОНЕЦ КАК ОрдерныйСклад,
	|	ЗаказПокупателяЗапасы.ТипНоменклатуры КАК ТипНоменклатуры,
	|	ВЫРАЗИТЬ(ЗаказПокупателяЗапасы.Номенклатура КАК Справочник.Номенклатура).СчетУчетаЗапасов КАК СчетУчета,
	|	ЗаказПокупателяЗапасы.Номенклатура КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА &ИспользоватьХарактеристики
	|			ТОГДА ЗаказПокупателяЗапасы.Характеристика
	...
	|	СУММА(втЗапасы.СуммаБезНДСОстаток) КАК СуммаБезНДСОстаток,
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяРезерв > 0 Тогда
			...
			Если Окр(СуммаКСписанию, 2, 1) <> 0
				...
				ИЛИ КоличествоТребуетсяРезерв > 0 Тогда
...
Процедура СформироватьТаблицаРезервыТоваровОрганизацийЗаказПокупателя(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	|	ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) КАК ЗаказРезерваСписание,
	...
	|	ТаблицаЗапасы.Количество - ЕСТЬNULL(СерииСвернуто.Количество, 0)
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
...
//
Процедура СформироватьТаблицаБонусныеБаллыЗаказПокупателя(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	// Начисление бонусных балов произойдет только после отгрузки
	ТаблицаОплатаБонусами = РезультатЗапроса[1].Выгрузить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаБонусныеБаллы", ТаблицаОплатаБонусами);
	
КонецПроцедуры

Процедура СформироватьТаблицаНачисленияБонусныхБалловЗаказПокупателя(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	// Заказ покупателя не делает начисления - начисления будут выполнены только после отгрузки
	ТаблицаНачисленияБонусныхБаллов.Очистить();
...
Процедура СформироватьТаблицаДоступностьПромокодовЗаказПокупателя(ДокументСсылкаЧекККМ, СтруктураДополнительныеСвойства)
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст =
	"ВЫБРАТЬ
...
Функция ТекстЗапросаИнициализироватьДанныеДокументаЗаказНаряд()
	...
	|	ЗаказНаряд.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	...
	|	ЗаказНаряд.КодОперацииПрослеживаемости КАК КодОперацииПрослеживаемости,
	|	ЗаказНаряд.ТоварыПередаютсяВСоставеРабот КАК ТоварыПередаютсяВСоставеРабот
	|ПОМЕСТИТЬ ВременнаяТаблицаШапка
	|ИЗ
	...
	|	ВЫБОР
	|		КОГДА ЗаказНарядРаботы.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.Продажи_НДС)
	|		ИНАЧЕ ЗаказНарядРаботы.Номенклатура.СчетУчетаНДСПоРеализации
	...
	|	ЗаказНарядРаботы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ЗаказНарядРаботы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ЗаказНарядРаботы.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	...
	|	КОНЕЦ КАК СчетУчетаПродажи,
	|	ВЫБОР
	|		КОГДА ЗаказНарядТовары.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.Продажи_НДС)
	|		ИНАЧЕ ЗаказНарядТовары.Номенклатура.СчетУчетаНДСПоРеализации
	|	КОНЕЦ КАК СчетУчетаНДСПоРеализации,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаШапка.ВестиУчетРасходовПоДоговорамОбслуживания
	|			ТОГДА ВременнаяТаблицаШапка.НаправлениеДеятельности.СчетУчетаСебестоимостиПродаж
	...
	|	ЗаказНарядТовары.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	NULL КАК КорректируемыйДокумент,
	|	ЗаказНарядТовары.ИдентификаторСтроки КАК ИдентификаторСтроки1,
	|	ЗаказНарядТовары.КодТНВЭД КАК КодТНВЭД,
	|	ЗаказНарядТовары.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ВЫБОР
	|		КОГДА ЗаказНарядТовары.СтруктурнаяЕдиницаРезерв.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ЗаказНарядТовары.ДокументПоступления
	|		КОГДА ЗаказНарядТовары.СтруктурнаяЕдиницаРезерв.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА ЗаказНарядТовары.Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументПоступления,
	|	ВЫБОР
	|		КОГДА ЗаказНарядТовары.СтруктурнаяЕдиницаРезерв.УчетОстатковПоЯчейкам
	|			ТОГДА ЗаказНарядТовары.Ячейка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ЯчейкаКРасходу,
	|	ЗаказНарядТовары.СтруктурнаяЕдиницаРезерв.ИспользоватьОрдернуюСхемуПриОтгрузке КАК ИспользоватьОрдернуюСхемуПриОтгрузке,
	|	ЗаказНарядТовары.СтруктурнаяЕдиницаРезерв.ВидУчетаОрдерныхСкладов КАК ВидУчетаОрдерныхСкладов,
	|	ЗаказНарядТовары.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	|ПОМЕСТИТЬ ВременнаяТаблицаТовары
	|ИЗ
	...
	|	ВЫБОР
	|		КОГДА ЗаказНарядМатериалы.СтруктурнаяЕдиницаРезерв.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ЗаказНарядМатериалы.ДокументПоступления
	|		КОГДА ЗаказНарядМатериалы.СтруктурнаяЕдиницаРезерв.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА ЗаказНарядМатериалы.Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументПоступления,
	|	ВЫБОР
	|		КОГДА ЗаказНарядМатериалы.СтруктурнаяЕдиницаРезерв.УчетОстатковПоЯчейкам
	|			ТОГДА ЗаказНарядМатериалы.Ячейка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ЯчейкаКРасходу,
	|	ЗаказНарядМатериалы.СтруктурнаяЕдиницаРезерв.ИспользоватьОрдернуюСхемуПриОтгрузке КАК ИспользоватьОрдернуюСхемуПриОтгрузке,
	|	ЗаказНарядМатериалы.СтруктурнаяЕдиницаРезерв.ВидУчетаОрдерныхСкладов КАК ВидУчетаОрдерныхСкладов,
	|	ЗаказНарядМатериалы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ЗаказНарядМатериалы.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект,
	|	ЗаказНарядМатериалы.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар
	...
	|			И ЗаказНарядМатериалы.Ссылка = ЗаказНарядРаботы.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПолитикиУчетаСерий КАК ПолитикиУчетаСерий
	...
	|	ЗаказНарядРаботы.Номенклатура КАК Операция
	...
	|	И &ИспользоватьСерииНоменклатуры
	|	И (НЕ ВременнаяТаблицаТовары.ОрдерныйСклад
	|			ИЛИ ВременнаяТаблицаТовары.ОрдерныйСклад
	|				И ВременнаяТаблицаТовары.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			ИЛИ ВременнаяТаблицаМатериалы.ОрдерныйСклад
	|				И ВременнаяТаблицаМатериалы.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЗаказПокупателяСведенияПрослеживаемости.КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|	ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ЗаказПокупателяСведенияПрослеживаемости.Номенклатура КАК Комплектующие,
	|	ЗаказПокупателяСведенияПрослеживаемости.Сумма КАК Сумма,
	|	ЗаказПокупателяСведенияПрослеживаемости.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ЛОЖЬ КАК ОтразитьВОперациях,
	|	ЛОЖЬ КАК МатериалыПередаютсяВСоставеРабот
	|ПОМЕСТИТЬ ВременнаяТаблицаСведенияПрослеживаемости
	|ИЗ
	|	ВременнаяТаблицаТовары КАК ВременнаяТаблицаТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.СведенияПрослеживаемости КАК ЗаказПокупателяСведенияПрослеживаемости
	|		ПО ВременнаяТаблицаТовары.ИдентификаторСтроки = ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки
	|ГДЕ
	|	ЗаказПокупателяСведенияПрослеживаемости.Ссылка = &Ссылка
	...
	|	ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки,
	|	ЗаказПокупателяСведенияПрослеживаемости.Номенклатура,
	|	ЗаказПокупателяСведенияПрослеживаемости.Сумма,
	|	ЗаказПокупателяСведенияПрослеживаемости.СтранаПроисхождения,
	|	ЛОЖЬ,
	|	&МатериалыПередаютсяВСоставеРабот
	|ИЗ
	|	ВременнаяТаблицаРаботы КАК ВременнаяТаблицаРаботы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.СведенияПрослеживаемости КАК ЗаказПокупателяСведенияПрослеживаемости
	|		ПО ВременнаяТаблицаРаботы.ИдентификаторСтроки = ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки
	|ГДЕ
	|	ЗаказПокупателяСведенияПрослеживаемости.Ссылка = &Ссылка
	|	И &ВестиУчетПрослеживаемыхТоваров
	|
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.СведенияПрослеживаемости КАК ЗаказПокупателяСведенияПрослеживаемости
	|		ПО ВременнаяТаблицаМатериалы.ИдентификаторСтроки = ЗаказПокупателяСведенияПрослеживаемости.ИдентификаторСтроки
	|ГДЕ
	|	ЗаказПокупателяСведенияПрослеживаемости.Ссылка = &Ссылка
	|	И &ВестиУчетПрослеживаемыхТоваров
	|;
	...
	|ПОМЕСТИТЬ ВременнаяТаблицаПромокоды
	|ИЗ
	|	Документ.ЗаказПокупателя.ПримененныеПромокоды КАК ВыданныйПромокод
...
Процедура ИнициализироватьДанныеДокументаЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства, ДокументОбъектЗаказПокупателя) 
	...
	Запрос.УстановитьПараметр("МатериалыПередаютсяВСоставеРабот",	СтруктураДополнительныеСвойства.МатериалыПередаютсяВСоставеРабот);
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЭтапыПроизводства", МассивРезультатов[12].Выгрузить());
	...
	// Прослеживаемость
	ПрослеживаемостьУНФ.СформироватьДвиженияРеализацияТоваров(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства, "ВременнаяТаблицаТовары");
	...
	// Бонусы
	СформироватьТаблицаБонусныеБаллыЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
	СформироватьТаблицаНачисленияБонусныхБалловЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
	// Промокоды
	СформироватьТаблицаПромокодыЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства); // Регистрация новых выданных промокодов
	СформироватьТаблицаПрименениеПромокодовЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства); // Гашение примененных промокодов
	СформироватьТаблицаДоступностьПромокодовЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства); // Оборот скидок, предоставленных по примененным промокодам
	// Эквайринг
	...
	СформироватьТаблицаСерииНоменклатурыКРасходуЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
	// Подарочные сертификаты
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаДоходыИРасходыПоЗаказамПрогноз", МассивРезультатов[13].Выгрузить());
...
Процедура СформироватьТаблицаРасчетыСПокупателямиЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства, ДокументОбъектЗаказПокупателя)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(ВременнаяТаблицаШапка.ЭтоСостояниеВыполненияЗаказНаряда = ИСТИНА
	|			И ВременнаяТаблицаШапка.ВариантЗавершения <> ЗНАЧЕНИЕ(Перечисление.ВариантыЗавершенияЗаказа.Отменен), ЛОЖЬ) КАК ФормироватьДвиженияРасчетов
	|ИЗ
	|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка";
...
Процедура СформироватьТаблицаНачисленияИУдержанияЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаНачисленияИУдержания.Операция КАК Операция,
...
Процедура СформироватьТаблицаЗапасыРаботыЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	|		ЗапасыОстатки.Характеристика КАК Характеристика,
	|		ЗапасыОстатки.Партия КАК Партия,
	|		ЗапасыОстатки.СуммаБезНДСОстаток КАК СуммаБезНДС,
	...
	|					ТОГДА Запасы.СуммаБезНДС
	|				ИНАЧЕ -Запасы.СуммаБезНДС
	|			КОНЕЦ),
	|		СУММА(ВЫБОР
	|				КОГДА Запасы.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|					ТОГДА Запасы.Сумма
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуется > 0 Тогда
			...
			Если СтрокиОстатки.Количество()>0 Тогда
				...
				СуммаБезНДСКСписанию = СтрокиОстатки[0].СуммаБезНДС;
			КонецЕсли; 
			
			Если СуммаКСписанию <> 0 Или СуммаБезНДСКСписанию <> 0 Тогда
...
Процедура СформироватьТаблицаЗапасыМатериалыЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыПеремещение.Количество() - 1 Цикл
		...
		Если КоличествоРезервОтгрузкаПеремещение > 0
			...
			СуммаБезНДСОстатокОтгрузкаПеремещение = 0;
			...
			Если МассивСтрокОстатковОтгрузкаПеремещение.Количество() > 0 Тогда
				...
				СуммаБезНДСОстатокОтгрузкаПеремещение = МассивСтрокОстатковОтгрузкаПеремещение[0].СуммаБезНДСОстаток;
			КонецЕсли;
			...
			Если КоличествоОстатокОтгрузкаПеремещение > 0 И КоличествоОстатокОтгрузкаПеремещение > КоличествоРезервОтгрузкаПеремещение Тогда
				...
				СуммаБезНДСКСписаниюОтгрузкаПеремещение = Окр(СуммаБезНДСОстатокОтгрузкаПеремещение * КоличествоРезервОтгрузкаПеремещение / КоличествоОстатокОтгрузкаПеремещение , 2, 1);
				...
				МассивСтрокОстатковОтгрузкаПеремещение[0].СуммаБезНДСОстаток = МассивСтрокОстатковОтгрузкаПеремещение[0].СуммаБезНДСОстаток - СуммаБезНДСКСписаниюОтгрузкаПеремещение;
			...
			ИначеЕсли КоличествоОстатокОтгрузкаПеремещение = КоличествоРезервОтгрузкаПеремещение Тогда
				...
				СуммаБезНДСКСписаниюОтгрузкаПеремещение = СуммаБезНДСОстатокОтгрузкаПеремещение;
				
				МассивСтрокОстатковОтгрузкаПеремещение[0].КоличествоОстаток = 0;
				МассивСтрокОстатковОтгрузкаПеремещение[0].СуммаОстаток = 0;
				МассивСтрокОстатковОтгрузкаПеремещение[0].СуммаБезНДСОстаток = 0;
				
			Иначе
				СуммаКСписаниюОтгрузкаПеремещение = 0;
				СуммаБезНДСКСписаниюОтгрузкаПеремещение = 0;
			КонецЕсли;
			...
			СтрокаТаблицыРасходПеремещение.СуммаБезНДС = СуммаБезНДСКСписаниюОтгрузкаПеремещение;
			...
			// Приход.
			Если Окр(СуммаКСписаниюОтгрузкаПеремещение, 2, 1) <> 0
				ИЛИ Окр(СуммаБезНДСКСписаниюОтгрузкаПеремещение, 2, 1) <> 0
				ИЛИ КоличествоРезервОтгрузкаПеремещение > 0 Тогда
				...
				СтрокаТаблицыПриходПеремещение.СуммаБезНДС = СуммаБезНДСКСписаниюОтгрузкаПеремещение;
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоОтгрузкаРезерв > 0 Тогда
			...
			Если МассивСтрокОтгрузкаОстатков.Количество() > 0 Тогда
				...
				СуммаБезНДСОтгрузкаОстаток = МассивСтрокОтгрузкаОстатков[0].СуммаБезНДСОстаток;
			КонецЕсли;
			...
			Если КоличествоОтгрузкаОстаток > 0 И КоличествоОтгрузкаОстаток > КоличествоОтгрузкаРезерв Тогда
				...
				СуммаБезНДСОтгрузкаКСписанию = Окр(СуммаБезНДСОтгрузкаОстаток * КоличествоОтгрузкаРезерв / КоличествоОтгрузкаОстаток , 2, 1);
				...
				МассивСтрокОтгрузкаОстатков[0].СуммаБезНДСОстаток = МассивСтрокОтгрузкаОстатков[0].СуммаБезНДСОстаток - СуммаБезНДСОтгрузкаКСписанию;
				
			ИначеЕсли КоличествоОтгрузкаОстаток = КоличествоОтгрузкаРезерв Тогда
				
				СуммаОтгрузкаКСписанию = СуммаОтгрузкаОстаток;
				СуммаБезНДСОтгрузкаКСписанию = СуммаБезНДСОтгрузкаОстаток;
				
				МассивСтрокОтгрузкаОстатков[0].КоличествоОстаток = 0;
				МассивСтрокОтгрузкаОстатков[0].СуммаОстаток = 0;
				МассивСтрокОтгрузкаОстатков[0].СуммаБезНДСОстаток = 0;
				
			Иначе
				СуммаОтгрузкаКСписанию = 0;
				СуммаБезНДСОтгрузкаКСписанию = 0;
			КонецЕсли;
			...
			СтрокаТаблицыРасход.СуммаБезНДС = СуммаБезНДСОтгрузкаКСписанию;
			СтрокаТаблицыРасход.Количество = КоличествоОтгрузкаРезерв;
			
			// Отнесем списанные запасы либо на стоимость запасов на складе либо на затраты НЗП.
			Если Окр(СуммаОтгрузкаКСписанию, 2, 1) <> 0
				Или Окр(СуммаБезНДСОтгрузкаКСписанию, 2, 1) <> 0 Тогда
				...
				СтрокаТаблицыПриход.СуммаБезНДС = СуммаБезНДСОтгрузкаКСписанию;
				СтрокаТаблицыПриход.Количество = 0;
...
Процедура СформироватьТаблицаЗапасыИсполнителиЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	|	СУММА(ЕСТЬNULL(ТаблицаЗапасыИсполнители.Сумма, 0)) КАК СуммаБезНДС,
	|	СУММА(ЕСТЬNULL(ТаблицаЗапасыИсполнители.Сумма, 0)) КАК Сумма,
...
Процедура СформироватьТаблицаЗапасыТоварыЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
    ...
    |	СУММА(втЗапасыИтоги.СуммаБезНДСОстаток) КАК СуммаБезНДСОстаток,
		...
		Если КоличествоТребуетсяВсего > 0 Тогда
			...
			Если СтрокаТаблицаЗапасы.Выполнен 
				...
				Если МассивСтрокОстатковОтгрузка.Количество() > 0 Тогда
					...
					СуммаБезНДСОстатокОтгрузка = МассивСтрокОстатковОтгрузка[0].СуммаБезНДСОстаток;
				КонецЕсли;
				...
				Если КоличествоОстатокОтгрузка > 0 И КоличествоОстатокОтгрузка > КоличествоРезервОтгрузка Тогда
					...
					СуммаБезНДСКСписаниюОтгрузка = Окр(СуммаБезНДСОстатокОтгрузка * КоличествоРезервОтгрузка / КоличествоОстатокОтгрузка , 2, 1);
					...
					МассивСтрокОстатковОтгрузка[0].СуммаБезНДСОстаток = МассивСтрокОстатковОтгрузка[0].СуммаБезНДСОстаток - СуммаБезНДСКСписаниюОтгрузка;

				ИначеЕсли КоличествоОстатокОтгрузка = КоличествоРезервОтгрузка Тогда

					СуммаКСписаниюОтгрузка = СуммаОстатокОтгрузка;
					СуммаБезНДСКСписаниюОтгрузка = СуммаБезНДСОстатокОтгрузка;

					МассивСтрокОстатковОтгрузка[0].КоличествоОстаток = 0;
					МассивСтрокОстатковОтгрузка[0].СуммаОстаток = 0;
					МассивСтрокОстатковОтгрузка[0].СуммаБезНДСОстаток = 0;

				Иначе
					СуммаКСписаниюОтгрузка = 0;
					СуммаБезНДСКСписаниюОтгрузка = 0;
				КонецЕсли;
				...
				СтрокаТаблицыРасход.СуммаБезНДС = СуммаБезНДСКСписаниюОтгрузка;
				
				СтрокаТаблицыРасход.Количество = КоличествоРезервОтгрузка;
				
				СтрокаТаблицыРасход.СодержаниеПроводки = НСтр("ru='Продажа запасов из резерва'");
				
				Если Окр(СуммаКСписаниюОтгрузка, 2, 1) <> 0
					Или Окр(СуммаБезНДСКСписаниюОтгрузка, 2, 1) <> 0 Тогда
					...
					Если Не СтрокаТаблицаЗапасы.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПодарочныйСертификат") Тогда
						...
						СтрокаПродажи.СебестоимостьБезНДС = СуммаБезНДСКСписаниюОтгрузка;
						СтрокаПродажи.Склад = СтрокаТаблицаЗапасы.СтруктурнаяЕдиница;
					КонецЕсли;
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если СтрокаТаблицаЗапасы.ВидДвижения = ВидДвиженияНакопления.Расход 
			...
			Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				СтрокаТаблицаЗапасы.СуммаБезНДС = СтрокаТаблицаЗапасы.СуммаБезНДС + НайденнаяСтрока.СуммаБезНДС;
				Если СтрокаТаблицаЗапасы.Сумма <> 0 Или СтрокаТаблицаЗапасы.СуммаБезНДС <> 0 Тогда
					...
					СтрокаТаблицаПродажи.СебестоимостьБезНДС = НайденнаяСтрока.СуммаБезНДС;
...
Процедура СформироватьТаблицаСерииНоменклатурыЗаказНаряд(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	|		ВЫРАЗИТЬ(ВременнаяТаблицаЗапасы.СтруктурнаяЕдиница КАК Справочник.СтруктурныеЕдиницы) КАК СтруктурнаяЕдиница,
	...
	|		ВЫРАЗИТЬ(ВременнаяТаблицаМатериалы.СтруктурнаяЕдиницаЗапасов КАК Справочник.СтруктурныеЕдиницы),
	...
	|ГДЕ
	|	НЕ(СНОбщаяТаблица.СтруктурнаяЕдиница.ОрдерныйСклад
	|				И СНОбщаяТаблица.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке
	|				И (СНОбщаяТаблица.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|					ИЛИ СНОбщаяТаблица.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом)))
	|
	|СГРУППИРОВАТЬ ПО
...
//
Процедура СформироватьТаблицаСерииНоменклатурыКРасходуЗаказНаряд(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	|	СУММА(СНОбщаяТаблица.Количество) КАК Количество,
	|	СНОбщаяТаблица.ОстаткиСерийНоменклатуры КАК ОстаткиСерийНоменклатуры,
	|	СНОбщаяТаблица.ДокументОснование КАК ДокументОснование
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВременнаяТаблицаЗапасы.Период КАК Период,
	|		ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	...
	|		ВременнаяТаблицаЗапасы.Партия КАК Партия,
	|		ВЫРАЗИТЬ(ВременнаяТаблицаЗапасы.СтруктурнаяЕдиница КАК Справочник.СтруктурныеЕдиницы) КАК СтруктурнаяЕдиница,
	|		ВременнаяТаблицаЗапасы.ЯчейкаКРасходу КАК Ячейка,
	...
	|		ВременнаяТаблицаМатериалы.Партия,
	|		ВЫРАЗИТЬ(ВременнаяТаблицаМатериалы.СтруктурнаяЕдиницаЗапасов КАК Справочник.СтруктурныеЕдиницы),
	|		ВременнаяТаблицаМатериалы.ЯчейкаКРасходу,
	|		ВременнаяТаблицаМатериалы.ДокументПоступления,
	|		СерииНоменклатуры.Количество
	|	ИЗ
	|		ВременнаяТаблицаМатериалы КАК ВременнаяТаблицаМатериалы
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаСерииНоменклатурыМатериалы КАК СерииНоменклатуры
	...
	|		И ВременнаяТаблицаШапка.ВариантЗавершения <> ЗНАЧЕНИЕ(Перечисление.ВариантыЗавершенияЗаказа.Отменен)) КАК СНОбщаяТаблица
	|ГДЕ
	|	СНОбщаяТаблица.СтруктурнаяЕдиница.ОрдерныйСклад
	|			И СНОбщаяТаблица.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке
	|
	|СГРУППИРОВАТЬ ПО
	|	СНОбщаяТаблица.Период,
	...
	|	СНОбщаяТаблица.ВидДвижения,
	|	СНОбщаяТаблица.ОстаткиСерийНоменклатуры,
	|	СНОбщаяТаблица.ДокументОснование";
...
Процедура СформироватьТаблицаРезервыТоваровОрганизацийЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаСерииНоменклатуры.КлючСвязи КАК КлючСвязи,
	|	СУММА(ТаблицаСерииНоменклатуры.Количество) КАК Количество
	|ПОМЕСТИТЬ СерииМатериалыСвернуто
	|ИЗ
	|	ВременнаяТаблицаСерииНоменклатурыМатериалы КАК ТаблицаСерииНоменклатуры
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаСерииНоменклатуры.КлючСвязи
	|;
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	ТаблицаТовары.Финиш КАК Период,
	...
	|	ТаблицаТовары.ОрдерныйСклад КАК ОрдерныйСклад,
	|	ТаблицаШапка.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	...
	|	ВременнаяТаблицаТовары КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСерииНоменклатуры КАК ТаблицаСерииНоменклатуры
	|		ПО ТаблицаТовары.КлючСвязи = ТаблицаСерииНоменклатуры.КлючСвязи,
	|	ВременнаяТаблицаШапка КАК ТаблицаШапка
	|ГДЕ
	|	ТаблицаТовары.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)
	|	И (ТаблицаШапка.ЭтоСостояниеВыполненияЗаказНаряда
	|			ИЛИ ТаблицаТовары.Резерв > 0)
	|	И НЕ ТаблицаСерииНоменклатуры.Серия ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ТаблицаТовары.СтруктурнаяЕдиница,
	|	ТаблицаТовары.ОрдерныйСклад,
	|	ТаблицаШапка.ХозяйственнаяОперация,
	...
	|			ТОГДА ТаблицаТовары.Количество - ЕСТЬNULL(СерииСвернуто.Количество, 0)
	|		ИНАЧЕ ТаблицаТовары.Резерв - ЕСТЬNULL(СерииСвернуто.Количество, 0)
	|	КОНЕЦ
	|ИЗ
	|	ВременнаяТаблицаТовары КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ СерииСвернуто КАК СерииСвернуто
	|		ПО ТаблицаТовары.КлючСвязи = СерииСвернуто.КлючСвязи,
	|	ВременнаяТаблицаШапка КАК ТаблицаШапка
	|ГДЕ
	|	ТаблицаТовары.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)
	|	И (ТаблицаШапка.ЭтоСостояниеВыполненияЗаказНаряда
	...
	|	ТаблицаМатериалы.НомерСтроки,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
	|	ТаблицаМатериалы.Финиш,
	|	ТаблицаМатериалы.Организация,
	|	ТаблицаМатериалы.Номенклатура,
	|	ТаблицаМатериалы.Характеристика,
	|	ТаблицаМатериалы.Партия,
	|	ТаблицаМатериалы.СтруктурнаяЕдиницаЗапасов,
	|	ТаблицаМатериалы.ОрдерныйСклад,
	|	ТаблицаШапка.ХозяйственнаяОперация,
	|	ТаблицаМатериалы.Ячейка,
	|	ТаблицаМатериалы.НомерГТД,
	|	ТаблицаМатериалы.СтранаПроисхождения,
	|	ТаблицаСерииНоменклатуры.Серия,
	|	ТаблицаСерииНоменклатуры.ОстаткиСерийНоменклатуры,
	|	ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка),
	|	ТаблицаСерииНоменклатуры.Количество
	|ИЗ
	|	ВременнаяТаблицаМатериалы КАК ТаблицаМатериалы
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСерииНоменклатурыМатериалы КАК ТаблицаСерииНоменклатуры
	|		ПО ТаблицаМатериалы.КлючСвязиСерииНоменклатуры = ТаблицаСерииНоменклатуры.КлючСвязи,
	|	ВременнаяТаблицаШапка КАК ТаблицаШапка
	|ГДЕ
	|	ТаблицаМатериалы.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)
	|	И (ТаблицаШапка.ЭтоСостояниеВыполненияЗаказНаряда
	|			ИЛИ ТаблицаМатериалы.Резерв > 0)
	|	И НЕ ТаблицаСерииНоменклатуры.Серия ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ВЫБОР
	|		КОГДА ТаблицаШапка.ЭтоСостояниеВыполненияЗаказНаряда
	|			ТОГДА ТаблицаМатериалы.Количество - ЕСТЬNULL(СерииСвернуто.Количество, 0)
	|		ИНАЧЕ ТаблицаМатериалы.Резерв - ЕСТЬNULL(СерииСвернуто.Количество, 0)
	|	КОНЕЦ
	|ИЗ
	|	ВременнаяТаблицаМатериалы КАК ТаблицаМатериалы
	|		ЛЕВОЕ СОЕДИНЕНИЕ СерииМатериалыСвернуто КАК СерииСвернуто
	|		ПО ТаблицаМатериалы.КлючСвязиСерииНоменклатуры = СерииСвернуто.КлючСвязи,
	|	ВременнаяТаблицаШапка КАК ТаблицаШапка
	|ГДЕ
	|	ТаблицаМатериалы.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)
	|	И (ТаблицаШапка.ЭтоСостояниеВыполненияЗаказНаряда
...
// Таблицу значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаБонусныеБаллыЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
...
Процедура СформироватьТаблицаНачисленияБонусныхБалловЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
...
Процедура СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.Период,
	|	ТаблицаДокумента.Организация,
	|	ЗНАЧЕНИЕ(Справочник.ВидыНалогов.НДС),
	|	ТаблицаДокумента.СуммаНДС,
	|	&НачислениеНДСПродажи
	|ИЗ
	|	ВременнаяТаблицаТовары КАК ТаблицаДокумента,
	|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
	|ГДЕ
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаПромокодыЗаказНаряд(ДокументСсылкаЧекККМ, СтруктураДополнительныеСвойства)
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаПрименениеПромокодовЗаказНаряд(ДокументСсылкаЧекККМ, СтруктураДополнительныеСвойства)
...
Процедура СформироватьТаблицаДоступностьПромокодовЗаказНаряд(ДокументСсылкаЧекККМ, СтруктураДополнительныеСвойства)
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст =
	"ВЫБРАТЬ
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	СтруктураПодменюВажное = Новый Структура("Подменю, Важность", "ПодменюСоздатьНаОснованииПереопределен", "Важное");
	СтруктураПодменюОбычное = Новый Структура("Подменю, Важность", "ПодменюСоздатьНаОснованииПереопределен", "Обычное");
	СтруктураПодменюСмТакже = Новый Структура("Подменю, Важность", "ПодменюСоздатьНаОснованииПереопределен", "СмТакже");   
	
	ОбъектМетаданных = "Документ.СчетНаОплату"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.СчетНаОплату.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументСчетНаОплатуСоздатьНаОсновании");	  
 	КонецЕсли;
	
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям("Документ.ПоступлениеВКассу") 
		ИЛИ ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям("Документ.ПоступлениеНаСчет") Тогда
		ДобавитьКомандуДокументЗаказПокупателяВвестиФактическийПлатеж(КомандыСозданияНаОсновании, СтруктураПодменюОбычное);  
  	КонецЕсли;

	ОбъектМетаданных = "Документ.ПоступлениеВКассу"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ПоступлениеВКассу.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументПоступлениеВКассуСоздатьНаОсновании");	
  	КонецЕсли;

	ОбъектМетаданных = "Документ.ПоступлениеНаСчет"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ПоступлениеНаСчет.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументПоступлениеНаСчетСоздатьНаОсновании");	
	КонецЕсли;   
	
	ОбъектМетаданных = "Документ.ОперацияПоПлатежнымКартам"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ОперацияПоПлатежнымКартам.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		СтруктураПодменюОбычноеОплатаКартой = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(СтруктураПодменюОбычное);
		СтруктураПодменюОбычноеОплатаКартой.Вставить("Представление", НСтр("ru='Оплата картой'"));

		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычноеОплатаКартой, ОбъектМетаданных, "ФормаДокументОперацияПоПлатежнымКартамОплатаПлатежнойКартой");	
	КонецЕсли;   

	ОбъектМетаданных = "Документ.РасходныйОрдер"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.РасходныйОрдер.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументРасходныйОрдерСоздатьНаОсновании");	   
 	КонецЕсли;

	ОбъектМетаданных = "Документ.ЧекККМ"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ЧекККМ.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументЗаказПокупателяСоздатьЧекККМ");	   
 	КонецЕсли;

	ОбъектМетаданных = "Документ.РасходнаяНакладная"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		 Документы.РасходнаяНакладная.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		 ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументРасходнаяНакладнаяСоздатьНаОсновании");	
 	КонецЕсли;
	    
	ОбъектМетаданных = "Документ.АктВыполненныхРабот"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		 Документы.АктВыполненныхРабот.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);
		 ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументАктВыполненныхРаботСоздатьНаОсновании");	   
 	КонецЕсли;
	
	ОбъектМетаданных = "Документ.ПриходнаяНакладная"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ПриходнаяНакладная.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		СтруктураПодменюОбычноеВозвратОтПокупателя = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(СтруктураПодменюОбычное);
		СтруктураПодменюОбычноеВозвратОтПокупателя.Вставить("Представление", НСтр("ru='Возврат от покупателя'"));
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычноеВозвратОтПокупателя, ОбъектМетаданных, "ФормаДокументПриходнаяНакладнаяСоздатьНаОсновании");	
		
		ДобавитьКомандуДокументПриходнаяНакладнаяПриемВПереработку(КомандыСозданияНаОсновании, СтруктураПодменюОбычное); 
	КонецЕсли;
	
	ОбъектМетаданных = "Документ.ЗаказПоставщику"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		
		Документы.ЗаказПоставщику.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументЗаказПоставщикуСоздатьНаОсновании");	
		
		Обработки.РасчетПотребностей.ДобавитьКомандуЗаказыПоставщикам(КомандыСозданияНаОсновании, СтруктураПодменюОбычное);
		
		ДобавитьКомандуДокументЗаказПоставщикуПоКалькуляции(КомандыСозданияНаОсновании, СтруктураПодменюОбычное); 
		
	КонецЕсли;

	ОбъектМетаданных = "Обработка.ТорговыеПредложения"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		ДобавитьКомандуДокументЗаказыПоставщикамБизнесСеть(КомандыСозданияНаОсновании, СтруктураПодменюОбычное);       
 	КонецЕсли;
	
	ОбъектМетаданных = "Документ.СборкаЗапасов"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.СборкаЗапасов.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		СтруктураПодменюОбычноеПроизводство = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(СтруктураПодменюОбычное);
		СтруктураПодменюОбычноеПроизводство.Вставить("Представление", НСтр("ru='Производство'"));
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычноеПроизводство, ОбъектМетаданных, "ФормаДокументСборкаЗапасовСоздатьНаОсновании");	
		
		ДобавитьКомандуДокументСборкаЗапасовЧастичныйВыпуск(КомандыСозданияНаОсновании, СтруктураПодменюОбычное); 
	КонецЕсли;


	ОбъектМетаданных = "Документ.ЗаказНаПроизводство"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ЗаказНаПроизводство.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументЗаказНаПроизводствоСоздатьНаОсновании");	
	КонецЕсли; 
	
 	ОбъектМетаданных = "Документ.ПередачаТоваровМеждуОрганизациями"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ПередачаТоваровМеждуОрганизациями.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументПередачаТоваровМеждуОрганизациямиСоздатьНаОсновании");	
 	КонецЕсли;

	ОбъектМетаданных = "Документ.ЗаказНаПеремещение"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ЗаказНаПеремещение.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументЗаказНаПеремещениеСоздатьНаОсновании");	
 	КонецЕсли;

	ОбъектМетаданных = "Документ.ПеремещениеЗапасов"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ПеремещениеЗапасов.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументПеремещениеЗапасовСоздатьНаОсновании");	
 	КонецЕсли;
	
	ОбъектМетаданных = "Документ.РезервированиеЗапасов"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.РезервированиеЗапасов.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		СтруктураПодменюРезервирование = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(СтруктураПодменюОбычное);
		СтруктураПодменюРезервирование.Вставить("Представление", НСтр("ru='Резервирование (зарезервировать)'"));
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюРезервирование, ОбъектМетаданных, "ФормаДокументЗаказПокупателяРезервированиеСоздатьНаОсновании");	
		
		ДобавитьКомандуДокументРезервированиеСнятьРезерв(КомандыСозданияНаОсновании, СтруктураПодменюОбычное); 
	КонецЕсли;
	
	ОбъектМетаданных = "Документ.ЗаданиеНаРаботу"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		ДобавитьКомандуДокументЗаданиеНаРаботуКоманда(КомандыСозданияНаОсновании, СтруктураПодменюОбычное); 
 	КонецЕсли;

	ОбъектМетаданных = "Документ.ОтчетОПереработке"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ОтчетОПереработке.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументОтчетОПереработкеСоздатьНаОсновании");	
  	КонецЕсли;

	ОбъектМетаданных = "Документ.СдельныйНаряд"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.СдельныйНаряд.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументСдельныйНарядСоздатьНаОсновании");	
  	КонецЕсли;

	ОбъектМетаданных = "Документ.РаспределениеЗатрат"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.РаспределениеЗатрат.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументРаспределениеЗатратСоздатьНаОсновании");	
	КонецЕсли;   
	
	ОбъектМетаданных = "Документ.ПоступлениеДСПлан"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ПоступлениеДСПлан.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументПоступлениеДСПланСоздатьНаОсновании");	
 	КонецЕсли;

	ОбъектМетаданных = "Документ.ОтчетКомиссионераОСписании"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ОтчетКомиссионераОСписании.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументОтчетКомиссионераОСписанииСоздатьНаОсновании");	
 	КонецЕсли;

	ОбъектМетаданных = "Документ.ПереоценкаТоваровНаКомиссии"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ПереоценкаТоваровНаКомиссии.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументПереоценкаТоваровНаКомиссииСоздатьНаОсновании");	
 	КонецЕсли;

	ОбъектМетаданных = "Документ.КомплектацияЗапасов"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		ДобавитьКомандуКомплектация(КомандыСозданияНаОсновании, СтруктураПодменюОбычное);       
 	КонецЕсли;

	ОбъектМетаданных = "Документ.АктОРасхожденияхПолученный"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.АктОРасхожденияхПолученный.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументАктОРасхожденияхПолученныйСоздатьНаОсновании");	
 	КонецЕсли;

	ОбъектМетаданных = "Документ.Событие"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.Событие.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументСобытиеСоздатьНаОсновании");	
  	КонецЕсли;

	ОбъектМетаданных = "Документ.ЗакрытиеПланируемыхОплат"; 
	Если ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ОбъектМетаданных) Тогда
		Документы.ЗакрытиеПланируемыхОплат.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);  
		ИзменитьСведенияПоследнейКоманды(КомандыСозданияНаОсновании, СтруктураПодменюОбычное, ОбъектМетаданных, "ФормаДокументЗакрытиеПлатежногоКалендаряСоздатьНаОсновании");	
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОбработкиСозданияСвязанныхОбъектов") Тогда
		ДобавитьКомандуСозданиеСвязанныхОбъектов(КомандыСозданияНаОсновании, СтруктураПодменюОбычное);       
  	КонецЕсли;

	Если НЕ Параметры.Свойство("ПоУмолчанию") Тогда  
		...
		Для Каждого 	Команда Из КомандыСозданияНаОсновании Цикл 
			КомандаВНастройках = НастройкиПользователя.Найти(Команда.Представление, "Наименование");   
			Если КомандаВНастройках = Неопределено Тогда      
				МассивУдаляемыхКоманд.Добавить(Команда);
			ИначеЕсли НЕ КомандаВНастройках.Пометка Тогда      
				МассивУдаляемыхКоманд.Добавить(Команда);
			Иначе
				Команда.Порядок = НастройкиПользователя.Индекс(КомандаВНастройках) + 1; 
			КонецЕсли;
		КонецЦикла;
		Для Каждого 	Команда Из МассивУдаляемыхКоманд Цикл 
			КомандыСозданияНаОсновании.Удалить(Команда);     
		КонецЦикла;    
		
		ДобавитьКомандуНастройки(КомандыСозданияНаОсновании, СтруктураПодменюСмТакже);    
		
		Если НЕ Параметры.ЭтоФормаОбъекта Тогда      
			КопияТаблицы = КомандыСозданияНаОсновании.Скопировать();
			Для Каждого КомандаКопии Из КопияТаблицы Цикл    
				НоваяКоманда = КомандыСозданияНаОсновании.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяКоманда, КомандаКопии);
				НоваяКоманда.Подменю = "СписокКонтекстноеМенюСоздатьНаОснованииПереопределен"
			КонецЦикла;
		КонецЕсли;
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.ЗаказПокупателя);
...
//  СтрокаТаблицыЗначений, Неопределено - описание добавленной команды.
//
Функция ДобавитьКомандуНастройки(КомандыСозданияНаОсновании, Параметры) Экспорт
	
	МодульСозданиеНаОсновании = ОбщегоНазначения.ОбщийМодуль("СозданиеНаОсновании");
	КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.ЗаказПокупателя);
	Если КомандаСозданияНаОсновании <> Неопределено Тогда
		КомандаСозданияНаОсновании.Обработчик = "СозданиеНаОснованииКлиент.ОткрытьФормуНастройкиПунктовСозданияНаОсновании";
		КомандаСозданияНаОсновании.ДополнительныеПараметры.Вставить("ПолноеИмяОбъекта", ПолноеИмяОбъекта());
		
		КомандаСозданияНаОсновании.Представление = РегистрыСведений.НастройкиПунктовСозданияНаОсновании.НаименованиеПунктаНастроек();
		КомандаСозданияНаОсновании.Порядок = 90;   
...
Функция ДобавитьКомандуДокументЗаказПокупателяВвестиФактическийПлатеж(КомандыСозданияНаОсновании, Параметры)

	МодульСозданиеНаОсновании = ОбщегоНазначения.ОбщийМодуль("СозданиеНаОсновании");
	КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.ПоступлениеВКассу);
	Если КомандаСозданияНаОсновании <> Неопределено Тогда
		КомандаСозданияНаОсновании.Обработчик = "СозданиеНаОснованииКлиент.ДокументЗаказПокупателяВвестиФактическийПлатеж";
		
		КомандаСозданияНаОсновании.Представление = НСтр("ru='Ввести фактический платеж'");
		КомандаСозданияНаОсновании.Идентификатор = "ФормаДокументЗаказПокупателяВвестиФактическийПлатеж";
...
Функция ДобавитьКомандуДокументЗаказПоставщикуПоКалькуляции(КомандыСозданияНаОсновании, Параметры)

	МодульСозданиеНаОсновании = ОбщегоНазначения.ОбщийМодуль("СозданиеНаОсновании");
	КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.ЗаказПоставщику);
	Если КомандаСозданияНаОсновании <> Неопределено Тогда
		КомандаСозданияНаОсновании.Обработчик = "СозданиеНаОснованииКлиент.ДокументЗаказПоставщикуПоКалькуляции";
		
		КомандаСозданияНаОсновании.Представление = НСтр("ru='Заказ поставщику (по калькуляции)'");
		КомандаСозданияНаОсновании.Идентификатор = "ФормаДокументЗаказПоставщикуСоздатьПоКалькуляции";
...
Функция ДобавитьКомандуДокументЗаказыПоставщикамБизнесСеть(КомандыСозданияНаОсновании, Параметры)

	МодульСозданиеНаОсновании = ОбщегоНазначения.ОбщийМодуль("СозданиеНаОсновании");
	КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.ЗаказПоставщику);
	Если КомандаСозданияНаОсновании <> Неопределено Тогда
		КомандаСозданияНаОсновании.Обработчик = "СозданиеНаОснованииКлиент.ДокументЗаказыПоставщикамБизнесСеть";
		
		КомандаСозданияНаОсновании.Представление = НСтр("ru='Заказы поставщикам в 1С:Бизнес-сеть'");
		КомандаСозданияНаОсновании.Идентификатор = "ФормаОбщаяКомандаПоискТорговыхПредложений";
...
Функция ДобавитьКомандуДокументСборкаЗапасовЧастичныйВыпуск(КомандыСозданияНаОсновании, Параметры)
	...
	Если КомандаСозданияНаОсновании <> Неопределено Тогда
		КомандаСозданияНаОсновании.Обработчик = "СозданиеНаОснованииКлиент.ДокументСборкаЗапасовЧастичныйВыпуск";
		
		КомандаСозданияНаОсновании.Представление = НСтр("ru='Производство (частичный выпуск)'");
		КомандаСозданияНаОсновании.Идентификатор = "ФормаДокументЗаказПокупателяЧастичныйВыпуск";
...
Функция ДобавитьКомандуДокументРезервированиеСнятьРезерв(КомандыСозданияНаОсновании, Параметры)
	...
	Если КомандаСозданияНаОсновании <> Неопределено Тогда
		КомандаСозданияНаОсновании.Обработчик = "СозданиеНаОснованииКлиент.ДокументРезервированиеСнятьРезерв";
		
		КомандаСозданияНаОсновании.Представление = НСтр("ru='Резервирование (снять резерв)'");
		КомандаСозданияНаОсновании.Идентификатор = "ФормаДокументЗаказПокупателяРезервированиеСнятьСоздатьНаОсновании";
...
Функция ДобавитьКомандуДокументЗаданиеНаРаботуКоманда(КомандыСозданияНаОсновании, Параметры)
	...
	Если КомандаСозданияНаОсновании <> Неопределено Тогда
		КомандаСозданияНаОсновании.Обработчик = "СозданиеНаОснованииКлиент.ДокументЗаданиеНаРаботуКоманда";
		
		КомандаСозданияНаОсновании.Представление = НСтр("ru='Задание на работу'");
		КомандаСозданияНаОсновании.Идентификатор = "ФормаДокументЗаданиеНаРаботуСоздатьНаОсновании";
...
Функция ДобавитьКомандуКомплектация(КомандыСозданияНаОсновании, Параметры)

	МодульСозданиеНаОсновании = ОбщегоНазначения.ОбщийМодуль("СозданиеНаОсновании");
	КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.КомплектацияЗапасов);
	Если КомандаСозданияНаОсновании <> Неопределено Тогда
		КомандаСозданияНаОсновании.Обработчик = "СозданиеНаОснованииКлиент.Комплектация";
		
		КомандаСозданияНаОсновании.Представление = НСтр("ru='Комплектация'");
		КомандаСозданияНаОсновании.Идентификатор = "ФормаДокументЗаказПокупателяКомплектация";
...
Функция ДобавитьКомандуДокументПриходнаяНакладнаяПриемВПереработку(КомандыСозданияНаОсновании, Параметры)
	...
	Если КомандаСозданияНаОсновании <> Неопределено Тогда
		КомандаСозданияНаОсновании.Обработчик = "СозданиеНаОснованииКлиент.ДокументПриходнаяНакладнаяПриемВПереработку";
		
		КомандаСозданияНаОсновании.Представление = НСтр("ru='Прием в переработку'");
	 	КомандаСозданияНаОсновании.Идентификатор = "ФормаОбщаяКомандаПриходнаяНакладнаяПриемВПереработку";
...
Функция ДобавитьКомандуСозданиеСвязанныхОбъектов(КомандыСозданияНаОсновании, Параметры)
	...
	Если КомандаСозданияНаОсновании <> Неопределено Тогда
		...
		КомандаСозданияНаОсновании.Идентификатор = "ФормаДокументСозданиеСвязанныхОбъектов";
