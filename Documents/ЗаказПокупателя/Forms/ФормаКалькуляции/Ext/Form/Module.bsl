...
Процедура ЗаполнитьКалькуляцию()
	...
	Для каждого СтрКэш Из ТаблицаКэшСебестоимости Цикл
		...
		ПараметрыПоиска.Вставить("НомерВариантаКП");
		ПараметрыПоиска.Вставить("КлючСвязи");
		ПараметрыПоиска.Вставить("Номенклатура");
		ПараметрыПоиска.Вставить("Характеристика");
		ПараметрыПоиска.Вставить("Спецификация");
		ПараметрыПоиска.Вставить("ЕдиницаИзмерения");
		ЗаполнитьЗначенияСвойств(ПараметрыПоиска, СтрКэш);
		Строки = ДанныеЗаказа.Калькуляция.НайтиСтроки(ПараметрыПоиска);
...
Процедура ВывестиКалькуляцию()
	...
	Если ПоказанаКалькуляцияЗаказа Тогда
		...
		|	ТаблицаКалькуляции.Цена КАК Цена,
		|	ВЫРАЗИТЬ(ТаблицаКалькуляции.СтавкаНДС КАК Справочник.СтавкиНДС) КАК СтавкаНДС,
		...
		|	ТаблицаКалькуляции.Всего КАК Всего
		|ПОМЕСТИТЬ ТаблицаКалькуляцииПредварительно
		|ИЗ
		...
		|		КОГДА &ЦенаБезНДС
		|			ТОГДА (100 + ТаблицаКалькуляции.СтавкаНДС.Ставка) / 100
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК Цена,
		|	ТаблицаКалькуляции.ПроцентСкидкиНаценки КАК ПроцентСкидкиНаценки,
		|	ТаблицаКалькуляции.ПроцентАвтоматическойСкидки КАК ПроцентАвтоматическойСкидки,
		...
		|ИЗ
		|	ТаблицаКалькуляцииПредварительно КАК ТаблицаКалькуляции
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Для каждого СтрМатериала Из СтрокиСпецификаций Цикл
		...
		Для каждого СтрСостава Из СтрокиСостава Цикл
			Если ЗначениеЗаполнено(СтрСостава.Спецификация)
				...
				Возврат Ложь;
			КонецЕсли;
			Если ТипЗнч(СтрМатериала.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
				Коэффициент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрМатериала.ЕдиницаИзмерения, "Коэффициент");
			Иначе
				Коэффициент = 1;
			КонецЕсли;
	...
	Для каждого СтрокаТабличнойЧасти Из ДанныеЗаказа.Калькуляция Цикл
		...
		Если СтрокиЗапасы.Количество() > 0 Тогда
			...
			Если ТипЗнч(СтрокаЗапасы.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
				Коэффициент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаЗапасы.ЕдиницаИзмерения, "Коэффициент");
			Иначе
				Коэффициент = 1;
			КонецЕсли;
