...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоказыватьПредпросмотрЗаказовПокупателей =
		УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь,
			"ПоказыватьПредпросмотрЗаказовПокупателей") И НЕ ЭтоМобильныйКлиент;		
...
Процедура ЗаполнитьПанельПредпросмотра()
	...
	|	ЗаказПокупателяЗапасы.СуммаСкидкиНаценки + ЗаказПокупателяЗапасы.СуммаАвтоматическойСкидки +
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказВариантКП КАК ЗаказВариантКП
	|		ПО ЗаказПокупателяЗапасы.НомерВариантаКП = ЗаказВариантКП.ОсновнойВариантКП
	|ГДЕ
	...
	СодержаниеДокумента.Загрузить(МассивРезультатов[1].Выгрузить());
	КоличествоСтрокБольшеСта = МассивРезультатов[2].Выгрузить()[0].КоличествоСтрок > 100;
