...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	   ...
	   И НЕ ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ТекстПлательщика) Тогда
			...
			СтруктураДанные.Вставить("НаименованиеПлательщикаПриПеречисленииНалогов",
				Объект.Организация.НаименованиеПлательщикаПриПеречисленииНалогов);
			СтруктураДанные.Вставить("ТекстКорреспондента", Объект.БанковскийСчет.ТекстКорреспондента);
			СтруктураДанные.Вставить("НаименованиеПолное", Объект.Организация.НаименованиеПолное);
			СтруктураДанные.Вставить("ТекстБанкДляРасчетов", Документы.ПлатежноеПоручение.ПолучитьТекстБанкДляРасчетов(
	...
	УстановитьВидимостьРерулярныеПлатежи(Элементы.НадписьСледующийПлатеж);
...
//
&НаКлиенте
Процедура КонтрагентПриИзменении(Элемент, ВосстанавливатьСтавкиНДС = Истина)
...
Процедура ЗаполнитьПоОснованиюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		КонтрагентПриИзменении(Неопределено, Ложь);
		УстановитьДоступностьРеквизитовПеречисленияНалогов();
	Иначе
		УстановитьВидимостьПоДаннымКонтрагента();
	КонецЕсли;
...
Функция ДанныеКонтрагентаДляИзменения(Знач ПолучательПлатежа, ВалютаДокумента, ВидОперации)
	...
	Результат.Вставить("НаименованиеПолное", 	?(ЭтоРасчетыССотрудником, ПолучательПлатежа.Наименование, ПолучательПлатежа.НаименованиеПолное));
	Результат.Вставить("СчетКонтрагента", 	СчетКонтрагента);
	Результат.Вставить("ИННПолучателя", 		ПолучательПлатежа.ИНН);
	Результат.Вставить("КПППолучателя", 		?(НеобходимостьУказанияКПП И НЕ ЭтоРасчетыССотрудником, ПолучательПлатежа.КПП, ""));
	Результат.Вставить("НазначениеПлатежа",	ПолучательПлатежа.БанковскийСчетПоУмолчанию.ТекстНазначения);
	Результат.Вставить("ТекстКорреспондента",	ПолучательПлатежа.БанковскийСчетПоУмолчанию.ТекстКорреспондента);
	...
	Иначе
		Результат.Вставить("СтатьяДДСПоУмолчанию", ПолучательПлатежа.СтатьяДДСПоУмолчанию);
		...
		УстановитьВидимостьПоДаннымКонтрагента();
		Результат.Вставить("ДоговорВалютаКурсКратность", РаботаСКурсамиВалют.ПолучитьКурсВалюты(
				ДоговорПоУмолчанию.ВалютаРасчетов, Объект.Дата));
		
		Результат.Вставить("ВестиРасчетыПоДоговорам", ПолучательПлатежа.ВестиРасчетыПоДоговорам);
		Результат.Вставить("ВестиРасчетыПоДокументам", ПолучательПлатежа.ВестиРасчетыПоДокументам);
		Результат.Вставить("ВестиРасчетыПоЗаказам", ПолучательПлатежа.ВестиРасчетыПоЗаказам);
		Результат.Вставить("ВестиУчетОплатыПоСчетам", ПолучательПлатежа.ВестиУчетОплатыПоСчетам);
	КонецЕсли;
...
Процедура ЗаполнитьВидПлатежаИзНастроек()
	...
	ВидПлатежаИзНастроек = ОбщегоНазначения.ХранилищеНастроекДанныхФормЗагрузить(ИмяФормы, ЭтаФОрма.Элементы.ВидПлатежа.Имя, ""); // Не локализуется
...
Процедура СохранитьВидПлатежаВНастройки()
	...
	ОбщегоНазначения.ХранилищеНастроекДанныхФормСохранить(ИмяФормы, ЭтаФОрма.Элементы.ВидПлатежа.Имя, Объект.ВидПлатежа);
...
Функция НадписьСледующийПлатеж(ИсходныйДокумент)
	...
	Если ЗначениеЗаполнено(ДатаСледующегоПлатежа) Тогда
		...
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			//НСтр("ru = 'Следующий платеж %1 до %2'"),
			НСтр("ru = 'Следующий платеж %1'"),
			Формат(ДатаСледующегоПлатежа, "ДЛФ=D"));
			//Формат(Объект.ОкончаниеВыполнения, "ДФ=dd.MM.yyyy"));
