...
// Функция возвращает признак использования НДС в переданном виде операции
//
// Параметры:
//  ВидОперации	 - ПеречислениеСсылка.ВидыОперацийПлатежноеПоручение	 - Вид операции платежного поручения
// 
// Возвращаемое значение:
//  Булево - Признак используется НДС в данной операции
//
Функция ЭтоВидОперацииБезНДС(ВидОперации) Экспорт
...
Функция ЗаполнитьРеквизитыЕНП(ДокументОбъект) Экспорт
	
	ДокументОбъект.НазначениеПлатежа = НСтр("ru = 'Единый налоговый платеж'");
	...
	ДокументОбъект.КодОКАТО = ЕдиныйНалоговыйСчет.ОКТМОЕдиногоНалоговогоСчета();
	ДокументОбъект.ПоказательНомера = "0";
	ДокументОбъект.ПоказательПериода = "0";
	ДокументОбъект.ИдентификаторПлатежа = "0";
	ДокументОбъект.КодБК = ДокументОбъект.Налог.КодБК;
	...
	АдминистраторНалогаОрганизации = ДанныеГосударственныхОрганов.АдминистраторНалогаОрганизации(
		ДокументОбъект.Налог,
		ДокументОбъект.Организация,
		ДокументОбъект.Дата);
	ДокументОбъект.ПоказательОснования = ПлатежиВБюджетКлиентСервер.НезаполненноеЗначение();
	Получатель = АдминистраторНалогаОрганизации;
	Если ЗначениеЗаполнено(Получатель) Тогда
		ДокументОбъект.Контрагент = Получатель;
		ДокументОбъект.ИННПолучателя = Получатель.ИНН;
		ДокументОбъект.КПППолучателя = Получатель.КПП;
		
		ДокументОбъект.СчетКонтрагента
		= ЕдиныйНалоговыйСчет.ЕдиныйБанковскийСчетГосударственногоОргана(Получатель);
		
		ТекстКорреспондента = Получатель.БанковскийСчетПоУмолчанию.ТекстКорреспондента;
		ТекстБанкДляРасчетов = Документы.ПлатежноеПоручение.ПолучитьТекстБанкДляРасчетов(ДокументОбъект.СчетКонтрагента);
		Если ПустаяСтрока(ТекстКорреспондента) Тогда
			ДокументОбъект.ТекстПолучателя = Получатель.НаименованиеПолное;
			Если ЗначениеЗаполнено(ТекстБанкДляРасчетов) Тогда
				ДокументОбъект.ТекстПолучателя = ДокументОбъект.ТекстПолучателя + ТекстБанкДляРасчетов;
			КонецЕсли;
		Иначе
			ДокументОбъект.ТекстПолучателя = ТекстКорреспондента;
		КонецЕсли;
	КонецЕсли;
