...
Процедура ОбновитьНадписьКнопкиКОплатеНаКлиенте()
	...
	Элементы.КнопкаКОплате.Заголовок = СтрШаблон(НСтр("ru = 'К оплате: %1'"), Формат(Объект.СуммаДокумента,
		"ЧДЦ=2; ЧН=0.00"));
...
// 
&НаСервере
Процедура ЗаполнитьСтавкуНДСПоОрганизацииНалогообложениеНДС(НалогообложениеПередИзменением = Неопределено)
	
	Если НалогообложениеПередИзменением = Неопределено Тогда
		НалогообложениеПередИзменением = Объект.НалогообложениеНДС;
	КонецЕсли;
	
	Объект.НалогообложениеНДС = НалогиУНФ.НалогообложениеНДС(Объект.Организация, Объект.СтруктурнаяЕдиница, ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса()));
...
Процедура ЗаполнитьСтавкуНДСПоНалогообложениеНДС(НалогообложениеПередИзменением = Неопределено)
	...
	НалогообложениеНДСОрганизации = НалогиУНФ.НалогообложениеНДС(Объект.Организация, Объект.СтруктурнаяЕдиница, ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса()));
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	Если ЗначениеЗаполнено(СтруктураДанные.Номенклатура) И СтруктураДанные.Номенклатура.ИспользоватьПартии
		...
		Если ЗначениеЗаполнено(СтруктураДанные.Партия)
			И СтруктураДанные.Партия.Статус = Перечисления.СтатусыПартий.ТоварыНаКомиссии
			И Не ЗначениеЗаполнено(СтруктураДанные.Партия.ВладелецПартии) Тогда 
			ТекстСообщения = СтрШаблон(НСтр("ru = 'У комиссионной партии ""%1"" не заполнен владелец. Добавление партии пропущено.'"), СтруктураДанные.Партия.Наименование);
			СтруктураДанные.Партия = Справочники.ПартииНоменклатуры.ПустаяСсылка();
	...
	Иначе
		...
		Если СтруктураДанные.ИспользоватьПартии Тогда
			Если ПодборНоменклатурыИзСписка И ТипЗнч(СтруктураВыбораНоменклатуры) = Тип("Структура") Тогда
				...
				СтруктураВыбораНоменклатуры.Партия = ПредопределенноеЗначение("Справочник.ПартииНоменклатуры.ПустаяСсылка");
			Иначе
...
Процедура ПолучитьЗапасыИзХранилища(АдресЗапасовВХранилище, ИмяТабличнойЧасти, ЕстьХарактеристики, ЕстьПартии)
	...
	Для каждого СтрокаЗагрузки Из ТаблицаДляЗагрузки Цикл
		...
		Если ЗначениеЗаполнено(НоваяСтрока.Номенклатура) Тогда
			...
			ОбработатьДобавлениеСтрокиЗапасов(НоваяСтрока, ИмяТабличнойЧасти, КодАкцизнойМарки, Истина);
...
Процедура СформироватьЗаголовок(СтруктураСостояниеКассовойСмены)
	...
	Если СтруктураСостояниеКассовойСмены = Неопределено Тогда
		СтруктураСостояниеКассовойСмены = ПолучитьСостояниеКассовойСменыНаСервере(КассаККМ, ИспользоватьАгентскиеПлатежиИРазделениеВыручки);
	КонецЕсли;
...
Процедура ЗаполнитьТекущийЧекПоЗаказу(ПараметрыНовогоЧекаККМ)
	...
	Если ИспользоватьАгентскиеПлатежиИРазделениеВыручки Тогда
		АгентскиеПлатежиУНФ.ЗаполнитьДоговорПлатежногоАгентаВТЧСервер(Объект.Запасы);
...
&НаСервере
Процедура ОбработатьДобавлениеСтрокиЗапасов(НоваяСтрока, ИмяТабличнойЧасти = "Запасы", КодАкцизнойМарки, ДобавлениеИзПодбора = Ложь)
	...
	Иначе
		...
		НоваяСтрока.СуммаСкидкиНаценки = (НоваяСтрока.Количество * НоваяСтрока.Цена) - НоваяСтрока.СуммаАвтоматическойСкидки - НоваяСтрока.Сумма;
		...
		Если НоваяСтрока.ИспользоватьПартии Тогда
			НоваяСтрока.Партия = ?(ЗначениеЗаполнено(НоваяСтрока.Партия), НоваяСтрока.Партия, СтруктураДанные.Партия);
		КонецЕсли;
		// Конец Партии
		
		Если ЗначениеЗаполнено(НоваяСтрока.Партия)
			И НоваяСтрока.Партия.Статус = Перечисления.СтатусыПартий.ТоварыНаКомиссии
			И Не ЗначениеЗаполнено(НоваяСтрока.Партия.ВладелецПартии) Тогда
			НоваяСтрока.Партия = Справочники.ПартииНоменклатуры.ПустаяСсылка();
			ТекстСообщения = СтрШаблон(НСтр("ru = 'В строке %1 у комиссионной партии не заполнен владелец. Добавление партии пропущено.'"), НоваяСтрока.НомерСтроки);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
		
		Если ИмяТабличнойЧасти = "Запасы" И 
			ЗначениеЗаполнено(НоваяСтрока.Резерв) 
		...
		ЗаполнениеОбъектовУНФКлиентСервер.ЗаполнитьСтрокуПоШапке(НоваяСтрока, Объект, "Ответственный", "ПоложениеОтветственный");
		Если Не ЗначениеЗаполнено(НоваяСтрока.Ответственный) Тогда
			НоваяСтрока.Ответственный = Объект.Ответственный;
		КонецЕсли;
		
		Если Не ДобавлениеИзПодбора Тогда
	...
	Если ИмяТабличнойЧасти = "Запасы" Тогда
		Если СтруктураДанные.ТребуетсяИзменениеНалогообложениеНаСНДС Тогда
			...
			ЦеныИВалюта = ЦеныИВалютаКлиентСервер.ТекстНадписиЦеныИВалюта(Объект, Объект.ВалютаДокумента, КурсНациональнаяВалюта,УчетВалютныхОпераций);
...
//
&НаКлиенте
Процедура ПробитьЧек(РезультатОплаты)
	...
	Если ПечатьДокументовУНФКлиент.ПечатьЧекаНевозможна(ЭтаФорма, НСтр("ru = 'Не удалось выполнить проведение документа'"), Истина) Тогда
	...
	Если НЕ ИспользоватьПодключаемоеОборудование // Проверка на включенную ФО "Использовать ПО"
		...
		Если РезультатОплаты.СформироватьТоварныйЧек И НЕ Объект.Ссылка.Пустая() Тогда
			
			МассивЧековККМ = Новый Массив;
			МассивЧековККМ.Добавить(Объект.Ссылка);
			
			УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ЧекККМ", "ТоварныйЧек", МассивЧековККМ, ЭтотОбъект);
		...
		КонецЕсли;
		Если СписокРаспределенныхЧеков.Количество() > 0 Тогда
			Если РезультатОплаты.ОбщаяОплата Тогда
				ЗагрузитьСледующийРаспределенныйЧек();
				ПроверитьСостояниеКассовойСмены(Ложь, Истина);
				ОплатитьРаспределенныеЧеки(РезультатОплаты);
			Иначе
				Оплатить("");
			КонецЕсли;
		Иначе
	...
	Если ИдентификаторУстройстваФР <> Неопределено Тогда
		...
		ОбщиеПараметры = ЗаблокироватьФормуИПодготовитьДанныеДляПробития(ТранспортныйМодуль, ЕстьАлкогольнаяПродукцияЕГАИС);
		...
		ДополнительныеПараметры.Вставить("РезультатОплаты", РезультатОплаты);
...
Процедура ПроверитьКодМаркировкиСредствамиККТЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе 
		ОбщиеПараметры = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры, "ОбщиеПараметры");
		СсылкаНаОбъект = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбщиеПараметры, "ДокументОснование");
		...
		Если СписокРаспределенныхЧеков.Количество() > 0 Тогда
			ВосстановитьЗапасыИзРаспределенныхЧеков(Ложь);
			Если ДополнительныеПараметры.РезультатОплаты.ОбщаяОплата Тогда
				Элементы.ГруппаГрупповоеПробитиеЧеков.Видимость = Ложь;
				Элементы.ГруппаПродажаИВозврат.Видимость = Истина;
			КонецЕсли;
		КонецЕсли;
...
&НаСервере
Функция ЗаблокироватьФормуИПодготовитьДанныеДляПробития(ТранспортныйМодуль, ЕстьАлкогольнаяПродукцияЕГАИС)
	
	ЭтаФорма.Доступность = Ложь;
	Возврат ПодготовитьДанныеДляПробитияЧека(ТранспортныйМодуль, ЕстьАлкогольнаяПродукцияЕГАИС);
...
Процедура ПробитьЧекЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Исключение
			...
			ЗаполнитьОписаниеДляТЧЗапасыНаКлиенте();
			ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Не удалось выполнить запись документа'")); // Асинхронный метод.
			Возврат;
		КонецПопытки;
		
		Если Параметры.РезультатОплаты.СформироватьТоварныйЧек И НЕ Объект.Ссылка.Пустая() Тогда
		...
		КонецЕсли;
		Если СписокРаспределенныхЧеков.Количество() > 0 Тогда
			Если Параметры.РезультатОплаты.ОбщаяОплата Тогда
				ЗагрузитьСледующийРаспределенныйЧек();
				ПроверитьСостояниеКассовойСмены(Ложь, Истина);
				ОплатитьРаспределенныеЧеки(Параметры.РезультатОплаты);
			Иначе
				Оплатить("");
			КонецЕсли;
		Иначе
...
Процедура ВыемкаДенег(Команда, ПараметрыКассыККМ = Неопределено, ОповещениеЗакрытияСмены = Неопределено)
	...
	ИспользоватьБезПодключенияОборудования = ПараметрыКассыККМ.ИспользоватьБезПодключенияОборудования;
	ИзымаемаяСумма = РассчитатьСуммуВыемки(СтруктураСостояниеКассовойСмены.НаличностьВКассе, ПараметрыКассыККМ);
...
//
&НаКлиенте
Процедура ПробитьЧекВыполнить(Команда, РезультатОплаты)
...
Процедура ЗакрытьКассовуюСменуПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИДЗамера = ОценкаПроизводительностиКлиент.ЗамерВремени("РМК_ПолучитьСостояниеКассовойСменыНаСервере", Истина, Ложь);
	СостояниеКассовойСмены = ПолучитьСостояниеКассовойСменыНаСервере(КассаККМ, ИспользоватьАгентскиеПлатежиИРазделениеВыручки);
	ОценкаПроизводительностиКлиент.ЗавершитьЗамерВремени(ИДЗамера, Ложь);
...
Процедура АннулироватьЧекККМ(КассаККМ)
	...
	Если ИдентификаторУстройстваФР <> Неопределено Тогда
		...
		ОборудованиеЧекопечатающиеУстройстваКлиент.НачатьАннулированиеЧекаНаФискальномУстройстве(
			ОповещениеПриЗавершении, 
			УникальныйИдентификатор,        
...
Процедура СформироватьДокументВыемки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено И Результат <> 0 Тогда
		...
		Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("ОповещениеПоступленияВКассу") Тогда
			Результат.Удалить("ОповещениеПоступленияВКассу");
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция СписокДоступныхСкладов() 
	...
	|	НастройкиРМКСтруктурныеЕдиницы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница
	|ИЗ
	...
	|	СЕ.Ссылка
	|ИЗ
	|	Справочник.СтруктурныеЕдиницы КАК СЕ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НастройкиРМК.СтруктурныеЕдиницы КАК НастройкиРМКСтруктурныеЕдиницы
	|		ПО (НастройкиРМКСтруктурныеЕдиницы.Ссылка = &НастройкаРМК)
	|ГДЕ
	|	НастройкиРМКСтруктурныеЕдиницы.СтруктурнаяЕдиница ЕСТЬ NULL";
	Запрос.УстановитьПараметр("НастройкаРМК", НастройкаРМК);
	МассивСкладов = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("СтруктурнаяЕдиница");;
	Возврат МассивСкладов;
	
КонецФункции

&НаСервереБезКонтекста
Функция ЕстьДоступныеОтложенныеЧеки() 
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КассыККМ КАК спрКассаККМ
	|		ПО ЧекККМ.КассаККМ = спрКассаККМ.Ссылка
	|ГДЕ
	|	ЧекККМ.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Отложен), ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.ПустаяСсылка))
	|	И спрКассаККМ.СтруктурнаяЕдиница В
	|			(ВЫБРАТЬ
	|				СтруктурныеЕдиницы.СтруктурнаяЕдиница
	|			ИЗ
	|				втДоступныеСтруктурныеЕдиницы КАК СтруктурныеЕдиницы)";
	Запрос.УстановитьПараметр("НастройкаРМК", НастройкаРМК);
...
Процедура ВосстановитьНастройкиОтборов()
	...
	Если УчетПоКомпанииВЦелом Тогда
		//УНФ.ОтборыСписка
		РаботаСОтборами.ВосстановитьНастройкиОтборов(ЭтотОбъект, СписокЗаказов, "СписокЗаказов",,,,,"ДанныеМетокЗаказов",,"ДанныеОтборовЗаказов");
		//Конец УНФ.ОтборыСписка
		Возврат
	КонецЕсли;
	...
	Если ИспользоватьНастройкиПоУмолчанию Тогда
		...
		//УНФ.ОтборыСписка
		РаботаСОтборами.ВосстановитьНастройкиОтборов(ЭтотОбъект, СписокЗаказов, "СписокЗаказов",,,,,"ДанныеМетокЗаказов",СписокПолейИсключений,"ДанныеОтборовЗаказов");
		//Конец УНФ.ОтборыСписка
...
Процедура УстановитьОрганизациюПоВладельцуККМ()
	...
	Если Не УчетПоКомпанииВЦелом И ИспользоватьНастройкиПоУмолчанию Тогда
		...
		НайденныеСтроки = ДанныеМетокЗаказов.НайтиСтроки(ПараметрыОтбора);
		
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			ДанныеМетокЗаказов.Удалить(НайденнаяСтрока);
		КонецЦикла;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ВыдатьПодаркиНаВыбор И Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.Пробит") Тогда
		// Необходимо сначала выбрать подарок, и только после этого будет возможна запись документа и пробитие чека.
		//ТекстСообщения = НСтр("ru = 'Документ можно будет записать (в т.ч. пробить чек) после выбора подарка.'");
		//ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Элементы.ГруппаПродажаИВозврат.ТекущаяСтраница = Элементы.ГруппаОтложенные Тогда
		Если Источник = "ПодключаемоеОборудование"
			...
			Данные = Новый Массив();
			Если Параметр.Количество() > 1 И Параметр[1] <> Неопределено Тогда
			...
			Если ТипыМарок.Найти(ТипШтрихкода) = Неопределено Тогда
				ШтрихкодВBase64 = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ШтрихкодВBase64(Штрихкод);
				ДисконтныеКарты = ДисконтныеКартыУНФВызовСервера.НайтиДисконтныеКартыПоШтрихкоду(ШтрихкодВBase64, Истина);
	...
	Если СтрНачинаетсяС(ИмяСобытия, "ЗакрытиеФормыПроверкиИПодбораГосИС")
		И КэшДанныхГОСИС.Свойство("ТребуетсяПересчет") Тогда
		...
		ПересчитатьДокументНаКлиенте();
		КэшДанныхГОСИС.Удалить("ТребуетсяПересчет");
	КонецЕсли;
	...
	Если ИмяСобытия = "РМК_Запись_ЧекККМВозврат" Тогда
...
Процедура ИнициализацияФормы(пПриСозданииНаСервере = Истина)
	...
	РазрешеноРедактированиеЦенДокументов = УправлениеДоступомУНФ.РазрешеноРедактированиеЦенДокументов()
		Или Пользователи.РолиДоступны("ПравоИзменятьЦенуРМК");
	...
	ОтборПоКассеЧеки = КассаККМ;
	ОтборПоОткрытойСменеЧеки = Истина;
	УстановитьОтборОтложенныхЧековПоКассеККМ();		
	УстановитьОтборОтложенныхЧековПоОткрытойСмене();
...
Процедура СоздатьЧекККМНаВозврат(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ЧекККМНаВозврат.Пустая() Или МожноСоздатьЧекНаВозврат(ТекущиеДанные.Ссылка, ТекущиеДанные.СуммаДокумента) Тогда
			Если Не ПроверитьСостояниеКассовойСмены() Тогда
				Возврат;  
			КонецЕсли;
...
Процедура СоздатьЧекККМНаОснованииЗаказа(Команда)
	...
	Если НЕ ЧекККМ.Пустая() И ЕстьПередачаТоваров Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон(НСтр("ru = 'Ранее по заказу уже оформлялись чеки: %1'"),
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ОрганизацияЗаказаПодходит(ТекущиеДанные.Ссылка) Тогда
			...
			ПараметрыНовогоЧекаККМ = Новый Структура;
			ПараметрыНовогоЧекаККМ.Вставить("Основание", ТекущиеДанные.Ссылка);
			ПараметрыНовогоЧекаККМ.Вставить("КассаККМ", КассаККМ);
			ПараметрыНовогоЧекаККМ.Вставить("РМК", Истина);
			ПараметрыНовогоЧекаККМ.Вставить("ДоступныеСклады", СписокДоступныхСкладов);
...
&НаКлиенте
функция ПроверитьСостояниеКассовойСмены(ПоказыватьПредупреждение = Ложь, ПринудительноОткрыть = Ложь)
	...
	Если НЕ СтруктураСостояниеКассовойСмены.СменаОткрыта Тогда
		Если ПринудительноОткрыть Тогда 
		ОткрытьКассовуюСмену();
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Перед началом операции нужно открыть кассовую смену.'") + Символы.ПС +
				НСтр("ru = 'Открыть смену можно в журнале ""Чеки ККМ"".'"));  
			Возврат Ложь;    
		КонецЕсли;
...
Процедура ОткрытьСмену(Команда)
	...
	ПроверитьСостояниеКассовойСмены(Истина, Истина);
...
Процедура Оплатить(ТипыДенежныхСредств)
	...
	Иначе
		...
		Если РаспределениеНоменклатурыНастроено Тогда
			...
			Если ВыдатьПодаркиНаВыбор Тогда
				// ожидание выбора подарка
				ДействиеПослеВыдачиПодарка = "ПробитьЧек";
				Возврат;
			КонецЕсли;
			АдресСкидокДляСообщения = ПодготовитьСтруктуруСкидокДляСообщения(Объект.СкидкиНаценки);

			Отказ = Ложь;
			РаспределитьЗапасыПоЧекам(Отказ);			
			Если СписокРаспределенныхЧеков.Количество() > 0 Тогда
	...
	Если ИспользоватьАвтоматическиеСкидки Тогда
		...
		Если ВыдатьПодаркиНаВыбор Тогда
			...
			Возврат;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(АдресСкидокДляСообщения) Тогда
			АдресСкидокДляСообщения = ПодготовитьСтруктуруСкидокДляСообщения(Объект.СкидкиНаценки);  
		КонецЕсли;
	КонецЕсли;
...
Процедура ОплатитьФрагмент(СуммаДоРасчетаАвтоСкидок, ТипыДенежныхСредств)
	...
	ФормаСообщения = ПолучитьФорму("ОбщаяФорма.ФормаВыводаСообщений",,, УникальныйИдентификатор);
	Если НЕ ФормаСообщения = Неопределено И ФормаСообщения.Открыта() Тогда
 		ФормаСообщения.Активизировать();
	КонецЕсли;	
...
//
&НаСервере
Процедура ОбновитьДокументНаСервере(ПараметрОбъект, Идентификатор)
	
	ЗначениеВДанныеФормы(ДанныеФормыВЗначение(ПараметрОбъект, Тип("ДокументОбъект.ЧекККМ")), Объект);
	
	Если Не Объект.Ссылка.Пустая() Тогда
		РазблокироватьДанныеДляРедактирования(Объект.Ссылка, Идентификатор);
		Попытка
			ЗаблокироватьДанныеДляРедактирования(Объект.Ссылка, , УникальныйИдентификатор);
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Блокировка чека ККМ'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОплатитьЗавершение(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ОбновитьДокументНаСервере(Результат.Объект, Результат.Идентификатор);
		
		Если Результат.Кнопка = "Пробить чек" Тогда
			
			// РаспределениеПродаж
			Если Результат.ОбщаяОплата Тогда
			...
			// Конец РаспределениеПродаж
				...
				ПробитьЧекВыполнить(Команды.ПробитьЧек, Результат);
		...
		Иначе
			...
			АдресСкидокДляСообщения = Неопределено;
			ВосстановитьЗапасыИзРаспределенныхЧеков(Истина, СкидкиИзменились);
	...
	Если СписокРаспределенныхЧеков.Количество() = 0 Тогда
		...
		// Выводим сообщения после пробития последнего чека
		Если ЗначениеЗаполнено(АдресСкидокДляСообщения) Тогда
			...
			ОткрытьФорму("ОбщаяФорма.ФормаВыводаСообщений", ПараметрыФормы, , УникальныйИдентификатор, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			ВывестиСообщения = Ложь;
			УдалитьИзВременногоХранилища(АдресСкидокДляСообщения);
			АдресСкидокДляСообщения = "";
		КонецЕсли;
...
Функция ЗарезервироватьНаСервере(ОтменаРезервирования = Ложь)
	...
	Если ОтменаРезервирования Тогда
		...
		Если ЗначениеЗаполнено(ТекущийДокумент) И ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.ЧекККМ") Тогда
			...
			Если ТекущийДокумент.Статус = Перечисления.СтатусыЧековККМ.Пробит Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Нельзя снять резерв - чек ККМ пробит на фискальном регистраторе'"));
	...
	Иначе
		...
		Исключение
			...
			Если ЭтоНепредусмотреннаяОшибка(ОписаниеОшибки) Тогда
				Сообщение = Новый СообщениеПользователю;
...
Процедура ПродолжитьЧекНаСервере()
	...
	// ИнтеграцияГосИС
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ТребуетсяЗаполнениеСлужебныхРеквизитов");
	ДополнительныеПараметры.Вставить("ТребуетсяЗаполнениеКэша");
...
&НаСервере
Функция ЭтоНепредусмотреннаяОшибка(ОписаниеОшибки)
	СообщениеНеУдалосьПровести = НСтр("ru='Не удалось провести'");
	Возврат СтрНайти(ОписаниеОшибки, СообщениеНеУдалосьПровести) = 0;
КонецФункции
...
Процедура ЗначениеПоискаНоменклатурыПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ЗначениеПоискаНоменклатуры) Тогда
		...
		ДанныеНоменклатуры.Вставить("Номенклатура", ЗначениеПоискаНоменклатуры);
		...
		ДанныеНоменклатуры = ПолучитьДанныеНоменклатураПриИзменении(ДанныеНоменклатуры);
		
		Если ДанныеНоменклатуры.Свойство("Склад") Тогда
			Если СписокДоступныхСкладов.НайтиПоЗначению(ДанныеНоменклатуры.Склад) = Неопределено Тогда
				ДанныеНоменклатуры.Склад = Объект.СтруктурнаяЕдиница;
				ДанныеНоменклатуры.Ячейка = ПредопределенноеЗначение("Справочник.Ячейки.ПустаяСсылка");
			КонецЕсли;
		КонецЕсли;
		ДанныеНоменклатуры.Вставить("СписокДоступныхСкладов", СписокДоступныхСкладов);
		
		// Если требуется ввод акцизной марки, то нужно добавлять отдельную строку?
		ПараметрыПоиска = Новый Структура;
		ПараметрыПоиска.Вставить("Номенклатура");
		ПараметрыПоиска.Вставить("Характеристика");
		ПараметрыПоиска.Вставить("Партия");
		ПараметрыПоиска.Вставить("ЕдиницаИзмерения");
		ПараметрыПоиска.Вставить("НоменклатураНабора");
		ЗаполнитьЗначенияСвойств(ПараметрыПоиска, ДанныеНоменклатуры);
		...
		Если МассивСтрокТЧ.Количество() = 0 Тогда
			...
			// Агентские платежи
			Если ИспользоватьАгентскиеПлатежиИРазделениеВыручки Тогда
				НоваяСтрока.ДоговорПлатежногоАгента = ДанныеНоменклатуры.ДоговорКонтрагента;
				НоваяСтрока.ДанныеАгентскогоДоговора = ДанныеНоменклатуры.ДанныеАгентскогоДоговора;
				АгентскиеПлатежиУНФКлиентСервер.ВыделитьАгентскоеВознаграждениеВТаблице(Объект, НоваяСтрока);
		...
		Если РежимОстаткиИРезервы Тогда
			ЗаполнитьЗначенияОстатковНоменклатурыВТабличнойЧастиНаСервере(ДанныеНоменклатуры, ЗначениеПоискаНоменклатуры);
		КонецЕсли
...
Процедура ЧекНеПредъявленПриИзмененииНаСервере()
	...
	Если ЧекНеПредъявлен Тогда
		...
		Элементы.ДекорацияЗаголовокЧекиККМ.Заголовок = НСтр("ru = 'Создайте чек на возврат'");
		...
		Элементы.СоздатьЧекККМНаВозврат.Доступность = Истина;
...
Процедура КассаККМПриИзменении(Элемент)
	...
	ПредыдущаяКассаККМ = КассаККМ;
	Объект.КассаККМ = КассаККМ;
	Если ЗначениеЗаполнено(ОтборПоКассеЧеки) Тогда
		ОтборПоКассеЧеки = КассаККМ;
	КонецЕсли;
...
Процедура КассаККМПриИзмененииНаСервере(ПараметрыКассы)
	...
	ЗаполнитьСтавкуНДСПоОрганизацииНалогообложениеНДС(НалогообложениеПередИзменением);
...
&НаКлиенте
Процедура ОтборПоКассеККМЧекиПриИзменении(Элемент)
...
&НаКлиенте
Процедура ОтборПоОткрытойСменеЧекиПриИзменении(Элемент)
...
&НаКлиенте
Процедура ОтборПоКассеККМЧекиОчистка(Элемент, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокОтложенныхЧеков,
		"КассаККМ.СтруктурнаяЕдиница",
		СписокДоступныхСкладов(),
		ВидСравненияКомпоновкиДанных.ВСписке,
		НСтр("ru = 'Чек доступных складов'"),
		Истина,
...
Процедура ТЧПартияПриИзменении(ИмяТЧ)
	...
	СтруктураДанные.Вставить("НалогообложениеНДС",			Объект.НалогообложениеНДС);
	СтруктураДанные.Вставить("ДатаОбработки",				Объект.Дата);
	СтруктураДанные.Вставить("ОчиститьПартию", Ложь); 
	
	ЗапасыПартияПриИзмененииНаСервереБезКонтекста(СтруктураДанные);
	
	Если СтруктураДанные.ОчиститьПартию Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'У комиссионной партии ""%1"" не заполнен владелец. Выбор партии пропущен.'"), Строка(СтрокаТабличнойЧасти.Партия));
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		СтрокаТабличнойЧасти.Партия = СтруктураДанные.Партия;
	КонецЕсли;
...
Процедура ЗапасыПартияПриИзмененииНаСервереБезКонтекста(СтруктураДанные)
	...
	Если ЗначениеЗаполнено(СтруктураДанные.Партия)
		И СтруктураДанные.Партия.Статус = Перечисления.СтатусыПартий.ТоварыНаКомиссии
		И Не ЗначениеЗаполнено(СтруктураДанные.Партия.ВладелецПартии) Тогда 
		СтруктураДанные.ОчиститьПартию = Истина;
		СтруктураДанные.Партия = Справочники.ПартииНоменклатуры.ПустаяСсылка();
	КонецЕсли;
...
Процедура ЗаполнитьСоставЧекаИСсылкиНаДокументыНаСервере(ЧекККМ)
	...
	Для Каждого ТекСтрока Из ЧекККМ.Запасы Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если ЧекККМ.Запасы.Количество() = 0 Тогда
		СоставЧека = "";
	КонецЕсли;
	...
	Если ЕстьПН Тогда // Приходная накладная
		...
		Элементы.СоздатьЧекККМНаВозврат.Доступность = Ложь;
		ЧекККМНаВозврат								= Документы.ЧекККМВозврат.ПустаяСсылка();
		Элементы.ДекорацияРКО.Видимость				= Ложь;
		РКО											= Документы.РасходИзКассы.ПустаяСсылка();
...
Процедура ИзменитьКоличествоСПомощьюКалькулятора(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ПараметрыФормы.Вставить("НоменклатураХарактеристикаИПартия", ТекущиеДанные.НоменклатураХарактеристикаИПартия);
		ПараметрыФормы.Вставить("Цена", ТекущиеДанные.Цена);
		ПараметрыФормы.Вставить("Сумма", ТекущиеДанные.Сумма);
		ПараметрыФормы.Вставить("ПроцентСкидкиНаценки", ТекущиеДанные.ПроцентСкидкиНаценки);
		ПараметрыФормы.Вставить("ПроцентАвтоматическойСкидки", ТекущиеДанные.ПроцентАвтоматическойСкидки); 
			
		ОткрытьФорму("Документ.ЧекККМ.Форма.ФормаВводаКоличества", ПараметрыФормы, , , , , Оповещение);
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Действие недоступно для маркируемой продукции'");
		Сообщение.Поле = "Объект.Запасы";
...
Процедура РассчитатьСкидкиНаценкиКлиент()
	...
	СтруктураПараметры.Вставить("ВыводитьСообщенияКассиру", 		Истина);
	СтруктураПараметры.Вставить("РасчетПодарков", 					Истина);
	...
	Если ВывестиСообщения Тогда
		ИДЗамера = ОценкаПроизводительностиКлиент.ЗамерВремени("РМК_СообщениеКассиру", Истина, Ложь);
		ДополнительныеПараметры = Новый Структура;
		...
		ОткрытьФорму("ОбщаяФорма.ФормаВыводаСообщений", ПараметрыФормы, , УникальныйИдентификатор, , , ОбработчикОповещения, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	Иначе
...
Процедура ОбновитьВидимостьПодарков()
	...
	НужноВводитьСерииПодарков = Ложь;
	Если ЕстьПодарки Тогда
		СтрокиБезСерий = Объект.Подарки.НайтиСтроки(Новый Структура("СтатусыСерийНоменклатуры", 0));
		НужноВводитьСерииПодарков = НЕ СтрокиБезСерий.Количество() = Объект.Подарки.Количество();
	КонецЕсли;
	ОтображатьСтраницы = ЕстьПодарки И НужноВводитьСерииПодарков;
	Элементы.ГруппаПодарки.Видимость = ОтображатьСтраницы;
	ВидимостьСтраниц = ОтображениеСтраницФормы.Нет;
	Если ОтображатьСтраницы Тогда
		ВидимостьСтраниц = ОтображениеСтраницФормы.ЗакладкиСнизу;
	КонецЕсли;
	Элементы.СтраницыРеквизитовЧека.ОтображениеСтраниц = ВидимостьСтраниц;
...
Функция СкидкиИзменились()
	...
	Иначе
		...
		КонецЕсли;
		Если Не СкидкиИзменились Тогда
			СкидкиИзменились = СкидкиПодаркиИзменились(ПримененныеСкидки);
		КонецЕсли;		
	КонецЕсли;
...
Процедура РассчитатьСкидкиНаценкиНаСервере(СтруктураПараметры)
	...
	Для Каждого СтрокаПодарка Из Объект.Подарки Цикл
		ТабличныеЧастиУНФКлиентСервер.ЗаполнитьКлючСвязи(Объект.Подарки, СтрокаПодарка, "КлючСвязи");
		УстановитьСтатусСерийСтроки(СтрокаПодарка, "Подарки");
	КонецЦикла;
	ОбновитьВидимостьПодарков();
...
//
&НаСервере
Процедура ПеренестиСкидкиПодаркиВЗапасы(Знач ПримененныеСкидкиНаценки = Неопределено) Экспорт
	
	Если Объект.ОтменаПереносаПодарковВПродажи Тогда
		Возврат;
	КонецЕсли;

	Результат = СкидкиНаценкиСервер.ПеренестиПодаркиВЗапасы(Объект, ПримененныеСкидкиНаценки, АдресПримененныхСкидокВоВременномХранилище);
	Если Результат.Успех Тогда
		Для Каждого ОписаниеДобавленнойСтроки Из Результат.ДобавленныеСтрокиЗапасов Цикл 
			...
			ОбработатьДобавлениеСтрокиЗапасов(ДобавленнаяСтрокаЗапасов, "Запасы", КодАкцизнойМарки);
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗапасыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.СтруктурнаяЕдиница) Тогда
			ПараметрыОтбора.Вставить("Склад", СтрокаТабличнойЧасти.СтруктурнаяЕдиница);
		Иначе
			ПараметрыОтбора.Вставить("Склад", Объект.СтруктурнаяЕдиница);
		КонецЕсли;
		ПараметрыОтбора.Вставить("ЕдиницаИзмерения", СтрокаТабличнойЧасти.ЕдиницаИзмерения);
		ПараметрыОтбора.Вставить("ЗаказПокупателя", Объект.Заказ);
		ПараметрыОтбора.Вставить("ОрганизацияОбъекта", Объект.Организация);
		ПараметрыОтбора.Вставить("НомерВариантаКП");
		...
		Если Элемент.ТекущийЭлемент = ПолеДляПроверки Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				СписокДокументов = СписокДокументовПоЗаказу("ЗапасыПоступления", ПараметрыОтбора);
...
&НаСервере  
Функция ПодготовитьСтруктуруСкидокДляСообщения(Знач СкидкиНаценки)
	
	Результат = Неопределено;
    ВывестиСообщения = Ложь;
	ТаблицаСообщений = СкидкиНаценкиСервер.ПолучитьТаблицуСообщений(СкидкиНаценки, Перечисления.ТочкиВыдачиСообщенияКассиру.ПослеОформленияЧека);
	Если НЕ ТаблицаСообщений.Количество() = 0 Тогда
		...
		Результат = ПоместитьВоВременноеХранилище(структураРезультат, УникальныйИдентификатор);
...
Процедура ВыдатьПодаркиНаВыборНаСервере(Результат, ДополнительныеПараметры) Экспорт
	...
	Для каждого СтрокаПодарка Из ТаблицаПодарков Цикл
		...
		УстановитьСтатусСерийСтроки(НоваяСтрока, "Подарки");
...
//
&НаСервере
Процедура ПеренестиСкидкиПодаркиВТовары(ПримененныеСкидкиНаценки = неопределено) Экспорт
	...
	Для Каждого СтрокаПодарка Из ТаблицаСтрокПереносимыхПодарков Цикл
		...
		ОбработатьДобавлениеСтрокиЗапасов(СтрокаПеренесенногоПодарка, "Запасы", КодАкцизнойМарки);
...
&НаСервере
Функция СкидкиПодаркиИзменились(ПримененныеСкидки)
	СкидкиИзменились = Ложь;
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Если МассивДанныхШтрихкода <> Неопределено
			...
			Для Каждого ДанныеШтрихкода Из МассивДанныхШтрихкода Цикл
				...
				Если ЗначениеЗаполнено(ДанныеШтрихкода.Партия)
					И ДанныеШтрихкода.Партия.Статус = Перечисления.СтатусыПартий.ТоварыНаКомиссии
					И Не ЗначениеЗаполнено(ДанныеШтрихкода.Партия.ВладелецПартии) Тогда 
					ТекстСообщения = СтрШаблон(НСтр("ru = 'У комиссионной партии ""%1"" не заполнен владелец. Добавление партии пропущено.'"), ДанныеШтрихкода.Партия.Наименование);
					ДанныеШтрихкода.Партия = Справочники.ПартииНоменклатуры.ПустаяСсылка();
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
				КонецЕсли;
...
Процедура ЗаполнитьПоДаннымШтрихкодовЗавершение(СтруктураНоменклатуры, Параметры) Экспорт
	...
	Иначе
		...
		МассивСтрокТЧ = Объект.Запасы.НайтиСтроки(
			ПараметрыПоиска);
		Если МассивСтрокТЧ.Количество() = 0 Тогда
...
Процедура ПолученыШтрихкоды(ДанныеШтрихкодов) Экспорт
	...
	ИначеЕсли КэшДанныхГОСИС.Свойство("ИнтеграцияИСМП")
...
Процедура ПослеОбработкиШтрихкодовСервер(РезультатОбработкиШтрихкода, КэшированныеЗначения, ПараметрыСканирования)
	...
	Если Не РезультатОбработкиШтрихкода.ТребуетсяОбработкаШтрихкода Тогда
		...
		Для Каждого СтрокаДляОбработки Из СтрокиДляОбработки Цикл
			ИнтеграцияИСМПУНФ.ЗаполнитьСлужебныеРеквизитыИСМПВСтрокеТЧ(СтрокаДляОбработки);
...
Функция ДанныеЧекаДляВыведенияНаПринтерЧеков()
	...
	СсылкаЧека = Элементы.Список.ТекущаяСтрока;
	Возврат Документы.ЧекККМ.ПолучитьДанныеДляФискализации(СсылкаЧека);
...
Процедура ПоискПоШтрихкоду(Команда)
	...
	#Иначе
	ОткрытьФорму("Документ.ЧекККМ.Форма.ФормаВводаШтрихкода", Новый Структура("ТекШтрихкод, РежимУдаленияДоступен", ТекШтрихкод, Истина), , , , ,
			ОбработкаЗавершения);
	#КонецЕсли
	...
	Если РезультатОбработкиШтрихкода.ТребуетсяОбработкаШтрихкода Тогда
		...
		Если ШтрихкодДляОбработки.Свойство("Номенклатура") Тогда
			...
			Иначе
				...
				ПроверкаИПодборПродукцииИС.ЗаполнитьКешШтрихкодовУпаковок(ЭтотОбъект, НастройкиКэша);
				ПроверкаИПодборПродукцииИС.ПрименитьКешШтрихкодовУпаковок(ЭтотОбъект, НастройкиКэша);
...
// Процедура настраивает период динамического списка на сервере.
//
&НаСервере
Процедура УстановитьПериодНаСервере(ВидПериодаРМК, ИмяСписка, ПараметрСтандартныйПериод = Неопределено)
	...
	ИначеЕсли ВидПериодаРМК = ЗаВчерашнийДень Тогда
		...
		ЭтотОбъект[ИмяСписка].Параметры.УстановитьЗначениеПараметра("ОтборПоСмене", Ложь);
		Элементы[ИмяСписка].Обновить();
		Элементы[ИмяСписка].Период.ДатаНачала = НачалоДня(НачалоДня(ТекущаяДатаСеанса()) - 1);
		Элементы[ИмяСписка].Период.ДатаОкончания = НачалоДня(ТекущаяДатаСеанса()) - 1;
		Элементы[ИмяСписка].Обновить();
		Если ИмяСписка = "Список" Тогда
			Элементы.Дата.Видимость = Ложь;
...
Процедура ЗаполнитьБыстрыеТовары(ПриОткрытии = Ложь)
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА БыстрыеТовары.ПолеСортировки = """"
		|			ТОГДА ""999999""
		|		ИНАЧЕ БыстрыеТовары.ПолеСортировки
	...
	Для Каждого БыстрыйТовар Из ТаблицаРезультата Цикл
		...
		НоваяКнопка.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.Авто;
		НоваяКнопка.Видимость = Истина;
...
//
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьОписаниеДляСтрокиТЧЗапасы(Объект, Строка)
...
Процедура УстановитьМеткуИОтборСписка(ИмяПоляОтбораСписка, ГруппаРодительМетки, ВыбранноеЗначение, ПредставлениеЗначения="")
	...
	РаботаСОтборами.ПрикрепитьМеткуОтбора(ЭтотОбъект, ИмяПоляОтбораСписка, ГруппаРодительМетки, ВыбранноеЗначение, ПредставлениеЗначения, "СписокЗаказов",,"ДанныеМетокЗаказов",,,,"ДанныеОтборовЗаказов");
	РаботаСОтборами.УстановитьОтборСписка(ЭтотОбъект, СписокЗаказов, ИмяПоляОтбораСписка, "СписокЗаказов",,"ДанныеМетокЗаказов",,"ДанныеОтборовЗаказов");
...
Процедура Подключаемый_МеткаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	МеткаИД = Сред(Элемент.Имя, СтрДлина("МеткаЗ_")+1);
...
Процедура УдалитьМеткуОтбора(МеткаИД)
	...
	РаботаСОтборами.УдалитьМеткуОтбораСервер(ЭтотОбъект, СписокЗаказов, МеткаИД, "СписокЗаказов","ДанныеМетокЗаказов",,,,"ДанныеОтборовЗаказов");
...
Процедура СохранитьНастройкиОтборов()
	...
	РаботаСОтборами.СохранитьНастройкиОтборов(ЭтотОбъект, "СписокЗаказов",,,, "ДанныеМетокЗаказов", "ДанныеОтборовЗаказов");
...
Процедура Подключаемый_ОтборПриИзмененииНаСервере(ЭлементИмя, ЭлементРодительИмя)
	...
	РаботаСОтборами.Подключаемый_ОтборПриИзменении(ЭтотОбъект, ЭлементИмя, ЭлементРодительИмя, "СписокЗаказов","ДанныеМетокЗаказов","ДанныеОтборовЗаказов");
...
Процедура Подключаемый_ОтборОчисткаНаСервере(ЭлементИмя, ЭлементРодительИмя)
	...
	РаботаСОтборами.Подключаемый_ОтборОчистка(ЭтотОбъект, ЭлементИмя, "СписокЗаказов","ДанныеМетокЗаказов","ДанныеОтборовЗаказов");
...
Процедура СброситьВсеМеткиОтбораНаСервере()
	РаботаСОтборами.УдалитьМеткиОтбораСервер(ЭтотОбъект, СписокЗаказов,,"СписокЗаказов","ДанныеМетокЗаказов",,"ДанныеОтборовЗаказов");
КонецПроцедуры
...
&НаКлиенте
Процедура ОткрытьПодборСерииПодарка(ИмяТЧ)
		
	ТекущиеДанныеИдентификатор = Элементы[ИмяТЧ].ТекущиеДанные.ПолучитьИдентификатор();
	ПараметрыСерийНоменклатуры = ПараметрыПодбораСерийПодарка(ТекущиеДанныеИдентификатор);
...
&НаСервере
Функция ПараметрыПодбораСерийПодарка(ТекущиеДанныеИдентификатор)
...
Процедура ПодаркиСерииНоменклатурыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ОткрытьПодборСерииПодарка("Подарки");
	
КонецПроцедуры

&НаСервере
Процедура УстановитьСтатусСерийСтроки(СтрокаТЧ, ИмяТЧ = "Запасы")
	...
	Если ИмяТЧ = "Подарки" И СтруктураДанные.Свойство("ПараметрыПодбораСтатуса") Тогда
...
Процедура ЗапасыПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущийЭлемент.Имя = "ЗапасыЦена" ИЛИ Элемент.ТекущийЭлемент.Имя = "ЗапасыЦена_МК" Тогда
		Если НЕ ПродажаСоСклада И РазрешеноРедактированиеЦенДокументов Тогда
			Отказ = Истина;
			ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Цену можно установить в карточке номенклатуры. В рабочем месте кассира можно менять цену только при продаже с оптового склада.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьУсловноеОформлениеТабличнойЧастиДляХарактеристикИПартий()
			...
			ЗначениеПоиска = Элементы.Найти(НаименованиеПоляХарактеристика);
			ЗначениеПоискаПартия = Элементы.Найти(НаименованиеПоляПартия);
...
Процедура СпозиционироватьНаПоискеНоменклатура()
	...
	Если ТекущийЭлемент <> Элементы.ЗначениеПоискаНоменклатуры Тогда
		ТекущийЭлемент = Элементы.ЗначениеПоискаНоменклатуры;
		ЗначениеПоискаНоменклатуры = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка");
	КонецЕсли;
...
Процедура СпозиционироватьНаПоискеШК()
	...
	Если ТекущийЭлемент <> Элементы.ЗначениеПоискаНоменклатурыПоШтрихКоду Тогда
		ТекущийЭлемент = Элементы.ЗначениеПоискаНоменклатурыПоШтрихКоду;
...
Процедура ОбновитьСправочникНастройкиРМК()
	...
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = Не получилось поменять настройку быстрого поиска. '") + ОписаниеОшибки();
		Сообщение.Сообщить();
	КонецПопытки;
...
Процедура ПересчитатьОтложенныеЧеки()
	...
	Если ЗначениеЗаполнено(ОтборПоКассеЧеки) Тогда
		...
		Запрос.УстановитьПараметр("КассаККМ", ОтборПоКассеЧеки);
	Иначе
		Запрос.Текст = Запрос.Текст + 
		"
		|	И ЧекККМ.КассаККМ.СтруктурнаяЕдиница В (&СписокДоступныхСкладов)";
		Запрос.УстановитьПараметр("СписокДоступныхСкладов", СписокДоступныхСкладов());
	КонецЕсли;
	Если ОтборПоОткрытойСменеЧеки Тогда
		// Поиск в открытых сменах
		Запрос.Текст = Запрос.Текст + 
		"
		|	И ЧекККМ.КассоваяСмена В (
		|		Выбрать 
		|			ДокументОРП.Ссылка 
		|		Из Документ.ОтчетОРозничныхПродажах Как ДокументОРП 
		|		Где ДокументОРП.СтатусКассовойСмены = ЗНАЧЕНИЕ(Перечисление.СтатусыОтчетаОРозничныхПродажах.Открыта))";	
	Иначе
		// Поиск в закрытых сменах
		// Отложенные чеки в закрытых сменах могут появиться не ранее 3.0.2.64, тогда в отложенном чеке очищается кассовая смена
		Запрос.Текст = Запрос.Текст + 
		"
...
Процедура УправлениеСтраницейОтложенныеЧеки()
	...
	Элементы.ГруппаОтложенные.Видимость = ЕстьДоступныеОтложенныеЧеки();
	
	Если Элементы.ГруппаОтложенные.Видимость <> ТекущаяВидимость И КоличествоОтложенныхЧеков > 0 Тогда
...
&НаСервере
Процедура УстановитьОтборОтложенныхЧековПоКассеККМ()
...
&НаСервере
Процедура УстановитьОтборОтложенныхЧековПоОткрытойСмене()
	Если ОтборПоОткрытойСменеЧеки Тогда		
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			СписокОтложенныхЧеков,
			"КассоваяСмена.СтатусКассовойСмены",
			Перечисления.СтатусыОтчетаОРозничныхПродажах.Открыта,
			ВидСравненияКомпоновкиДанных.Равно,
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			...
			НСтр("ru = 'Отложенные чеки закрытых смен'"),
			Истина,
			РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный);
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(
			СписокОтложенныхЧеков,
...
&НаСервере
Процедура ВосстановитьЗапасыИзРаспределенныхЧеков(ПересчитыватьСкидки = Ложь, СкидкиИзменились = Ложь)
	...
	Для Каждого ЭлементСписка Из СписокРаспределенныхЧеков Цикл
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ЭлементСписка.Значение.Подарки, Объект.Подарки);
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ЭлементСписка.Значение.СерииПодарков, Объект.СерииПодарков);
...
&НаСервере
Процедура ЗаполнитьРаспределенныеЧеки(ЗапасыДляРаспределения, Отказ = Ложь)
	...
	КопияПодарки					= Объект.Подарки.Выгрузить();
	КопияСерииПодарков				= Объект.СерииПодарков.Выгрузить();
	...
	СтруктураРеквизитов.Вставить("ДисконтнаяКарта");
	...
	Для Каждого СтрокаЗапасов Из ЗапасыДляРаспределения Цикл
		...
		Для Каждого СтрокаМарок Из КопияАкцизныеМарки Цикл
			МаркаСоответствуетТовару = Ложь;
			ДанныеМарки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаМарок.АкцизнаяМарка, "Номенклатура, Характеристика, Серия");
			Если ДанныеМарки.Номенклатура = НоваяСтрокаЗапасов.Номенклатура
				И ДанныеМарки.Характеристика = НоваяСтрокаЗапасов.Характеристика
				И ?(ЗначениеЗаполнено(НоваяСтрокаЗапасов.Партия), ДанныеМарки.Серия = НоваяСтрокаЗапасов.Партия, ДанныеМарки.Серия = Неопределено) Тогда
				МаркаСоответствуетТовару = Истина;
			Иначе
				НастройкиЧастичногоВыбытия = РегистрыСведений.НастройкиЧастичногоВыбытияПродукцииИС.ПолучитьОписание(
					ДанныеМарки.Номенклатура, ДанныеМарки.Характеристика);
				Если НастройкиЧастичногоВыбытия.НоменклатураНастроена
					И НастройкиЧастичногоВыбытия.НоменклатураЧастичногоВыбытия = НоваяСтрокаЗапасов.Номенклатура
					И НастройкиЧастичногоВыбытия.ХарактеристикаЧастичногоВыбытия = НоваяСтрокаЗапасов.Характеристика
					И ?(ЗначениеЗаполнено(НоваяСтрокаЗапасов.Партия), ДанныеМарки.Серия = НоваяСтрокаЗапасов.Партия, ДанныеМарки.Серия = Неопределено) Тогда
					МаркаСоответствуетТовару = Истина;
				КонецЕсли;
			КонецЕсли;
			Если МаркаСоответствуетТовару Тогда
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		Для Каждого СтрокаОбщейСкидки Из КопияСкидкиНаценки Цикл
			// Некоторые общие скидки, например, подарки за условие по документу, или сообщения, могут фигурировать с пустым ключем связи
			// Такие скидки не относятся к конкретной строке товара, и будут добавлены в последний чек
			ЗаполнитьЗначенияСвойств(Объект.СкидкиНаценки.Добавить(), СтрокаОбщейСкидки);
		КонецЦикла;
		Объект.Подарки.Загрузить(КопияПодарки);
		Объект.СерииПодарков.Загрузить(КопияСерииПодарков);
		КопияСкидкиНаценки.Очистить();
		
		Объект.СуммаДокумента = Объект.Запасы.Итог("Всего");
		КассаККМПриИзмененииНаСервере(Новый Структура("ВалютаДенежныхСредств, РежимРаспределения"));
		ДокОбъект = РеквизитФормыВЗначение("Объект");
		ДокОбъект.ДополнительныеСвойства.Вставить("НеПроверятьСмену");
		// Скидки взяты из предварительного общего чека "как есть", пересчитывать не требуется.
		ДокОбъект.СкидкиРассчитаны = Истина;
		Если Не ДокОбъект.ПроверитьЗаполнение() Тогда
			ЕстьОшибки = Истина;
		КонецЕсли;
...
Процедура ОплатитьРаспределенныеЧеки(Результат)
	...
	Иначе
		Объект.ПолученоНаличными = Результат.Наличными;
		Сдача = Объект.ПолученоНаличными - Объект.СуммаДокумента;
		СдачаСтрока = Формат(Сдача, "ЧДЦ=2");
	КонецЕсли;
	...
	ПробитьЧекВыполнить(Команды.Найти("ПробитьЧек"), Результат);
...
//
&НаСервере
Процедура РаспределитьЗапасыПоЧекам(Отказ = Ложь)
	...
	Если ЕстьДругиеКассыСпецрежимы Тогда
		ЗаполнитьРаспределенныеЧеки(ЗапасыДляРаспределения, Отказ);
	Иначе
		СписокРаспределенныхЧеков.Очистить();
	КонецЕсли;
