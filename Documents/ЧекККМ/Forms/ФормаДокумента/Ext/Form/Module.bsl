...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ИзКорзины") Тогда
		Пользователь = Пользователи.ТекущийПользователь();
		Объект.КассаККМ = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь, "ОсновнаяКассаККМ");
	КонецЕсли;
...
Процедура НапечататьПоследнийСлипЧек(Команда)
	...
	Если НЕ ПустаяСтрока(ПоследнийСлипЧек) Тогда
		...
		ПараметрыПечатиТеста = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыПечатиТеста(ПоследнийСлипЧек);
		ОборудованиеЧекопечатающиеУстройстваКлиент.НачатьПечатьТекста(Оповещение, УникальныйИдентификатор, , ПараметрыПечатиТеста);
	Иначе
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			...
			Если МассивСтрокТЧ.Количество() = 0 Или НЕ ОбъединятьСтрокиПриДобавлении Тогда
				...
				Если ЗначениеЗаполнено(ДанныеШтрихкода.СтруктураДанныеНоменклатуры.Ячейка) Тогда
...
Процедура ДобавитьСтрокуОплаты(ДанныеСтроки)
	...
	Если ДанныеСтроки.ВидОплаты = ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.Бонусы") Тогда
		...
		СтрокиБонусов = Объект.БезналичнаяОплата.НайтиСтроки(Новый Структура("БонуснаяКарта", Объект.ДисконтнаяКарта));
