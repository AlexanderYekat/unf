...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПриемИПередачаВРемонт, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ПриемИПередачаВРемонт.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.Продажи_НДС)
	|		ИНАЧЕ ПриемИПередачаВРемонт.Номенклатура.СчетУчетаНДСПоРеализации
	...
	Если (СтруктураДополнительныеСвойства.ВариантРемонта = Перечисления.ВариантыРемонта.НашаМастерскаяПростойРемонт
		...
		СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаПриемИПередачаВРемонт, СтруктураДополнительныеСвойства);
	КонецЕсли;
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаПриемИПередачаВРемонт, СтруктураДополнительныеСвойства)
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|				И НЕ ПриемИПередачаВРемонт.Организация = ПриемИПередачаВРемонт.Организация.ГоловнаяОрганизация
...
Процедура СформироватьАктВозврата(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		|	ПриемИПередачаВРемонт.ТекстУсловийГарантийногоТалона,
		...
		ОбластьМакета.Параметры.ВремяПечати = "Дата и время печати: " + ТекущаяДатаСеанса();
		ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		Если ЗначениеЗаполнено(Шапка.УсловияГарантии)
			...
			Иначе				
				ТекстДополнительныхУсловий = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Шапка.УсловияГарантии, "ТекстУсловий");
			КонецЕсли;
...
Процедура СформироватьТалонПриемки(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		|	ПриемИПередачаВРемонт.ТекстУсловийПриемки,
		|	ПриемИПередачаВРемонт.ПодписьКладовщика.Должность КАК ДолжностьКладовщика,
		|	ПриемИПередачаВРемонт.ПодписьКладовщика.РасшифровкаПодписи КАК РасшифровкаПодписиКладовщика,
		|	ПриемИПередачаВРемонт.КонтактноеЛицоПодписант.Наименование КАК РасшифровкаПодписиКонтрагента,
		|	ПриемИПередачаВРемонт.Номенклатура.ИспользоватьСерииНоменклатуры КАК ИспользоватьСерииНоменклатуры
		...
		Если ЗначениеЗаполнено(Шапка.ФайлЛоготип) Тогда
			...
			Если ЗначениеЗаполнено(ДанныеКартинки) Тогда
				...
				КартинкаЛоготип = Новый Картинка(ДанныеКартинки);
				ПечатьДокументовУНФ.УстановитьКартинкуБезопасно(ОбластьМакета.Рисунки, "Логотип", КартинкаЛоготип);
		...
		ОбластьМакета = Макет.ПолучитьОбласть("ВремяПечати");
		ОбластьМакета.Параметры.ВремяПечати = НСтр("ru ='Дата и время печати: '") + ТекущаяДатаСеанса();
		ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		Если ЗначениеЗаполнено(Шапка.УсловияПриемки)
			...
			Если НЕ ПустаяСтрока(Шапка.ТекстУсловийПриемки) Тогда
				ТекстДополнительныхУсловий = Шапка.ТекстУсловийПриемки;
			Иначе				
				ТекстДополнительныхУсловий = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Шапка.УсловияПриемки, "ТекстУсловий");
			КонецЕсли;
