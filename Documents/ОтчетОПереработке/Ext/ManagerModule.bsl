...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.ОтчетОПереработке.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.ОтчетОПереработке);
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаОтчетОПереработке, СтруктураДополнительныеСвойства)
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетОПереработке, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ОтчетОПереработкеПродукция.Номенклатура.СчетУчетаНДСПоРеализации = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.Продажи_НДС)
	|		ИНАЧЕ ОтчетОПереработкеПродукция.Номенклатура.СчетУчетаНДСПоРеализации
	...
	|				И ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	ОтчетОПереработкеПродукция.Ссылка.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	|	ВЫРАЗИТЬ(ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ОтчетОПереработкеПродукция.Ссылка.ДокументПоступления
	|		КОГДА ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА ОтчетОПереработкеПродукция.Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументПоступления,
	|	ВЫБОР
	|		КОГДА ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.УчетОстатковПоЯчейкам
	|			ТОГДА ОтчетОПереработкеПродукция.Ссылка.Ячейка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ЯчейкаКРасходу,
	|	ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад
	|		И ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке
	|		И НЕ ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом)
	|		И НЕ ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПустаяСсылка) КАК ОрдерныйСкладДляДвиженияСерий,
	|	ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке КАК ОрдерныйСкладКОтгрузке,
	|	ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом) КАК УчетПоСкладуВЦелом,
	|	ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам) КАК УтетПоПрочимДокументам,
	|	ОтчетОПереработкеПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам) КАК УтетПоСкладскимОрдерам
	...
	|	ОтчетОПереработкеЗапасы.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад
	|		И ОтчетОПереработкеЗапасы.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке
	|		И НЕ ОтчетОПереработкеЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом)
	|		И НЕ ОтчетОПереработкеЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПустаяСсылка) КАК ОрдерныйСкладДляДвиженияСерий
	...
	|				И ОтчетОПереработкеОтходы.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриОтгрузке
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОрдерныйСклад,
	...
	|	КОНЕЦ КАК Количество,
	|	ВЫБОР
	|		КОГДА ОтчетОПереработкеОтходы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ОтчетОПереработкеОтходы.Ссылка.ДокументПоступления
	|		КОГДА ОтчетОПереработкеОтходы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА ОтчетОПереработкеОтходы.Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументПоступления,
	|	ВЫБОР
	|		КОГДА ОтчетОПереработкеОтходы.Ссылка.СтруктурнаяЕдиница.УчетОстатковПоЯчейкам
	|			ТОГДА ОтчетОПереработкеОтходы.Ссылка.Ячейка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ЯчейкаКРасходу
	...
	|			ИЛИ ОтчетОПереработкеСерииНоменклатуры.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад
	|				И ВременнаяТаблицаПродукция.УтетПоСкладскимОрдерам)";
	...
	СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаОтчетОПереработке, СтруктураДополнительныеСвойства);
	// Серии номенклатуры
	СформироватьТаблицаСерииНоменклатуры(ДокументСсылкаОтчетОПереработке, СтруктураДополнительныеСвойства);
	СформироватьТаблицаСерииНоменклатурыКРасходу(ДокументСсылкаОтчетОПереработке, СтруктураДополнительныеСвойства);
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
	 ...
	 Или СтруктураВременныеТаблицы.ДвиженияСерииНоменклатурыКРасходуИзменение Тогда
		...
		Если НЕ МассивРезультатов[5].Пустой() Тогда
			...
			КонтрольОстатковУНФ.ЗапасыКРасходу(ДокументОбъектОтчетОПереработке, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		...
		Если НЕ МассивРезультатов[6].Пустой() Тогда
			...
			КонтрольОстатковУНФ.СерииНоменклатурыКРасходуПоДокументам(ДокументОбъектОтчетОПереработке, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|				И НЕ ОтчетОПереработке.Организация = ОтчетОПереработке.Организация.ГоловнаяОрганизация
...
Функция УниверсальныйЗапросПоДаннымДокумента(МассивОбъектов, ЭтоПечатьАктаВыполненныхРабот, ИспользоватьФаксимиле)
	...
	|	ОтчетОПереработке.Комментарий КАК Комментарий,
	|	ОтчетОПереработке.Продукция.(
	|		НомерСтроки КАК НомерСтроки,
	|		Содержание КАК Содержание,
	|		ВЫБОР
	...
	|		ВЫБОР
	|			КОГДА &ЭтоПечатьАктаВыполненныхРабот = ИСТИНА
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|			ИНАЧЕ ОтчетОПереработке.Продукция.Номенклатура.ТипНоменклатуры
	...
	|	ОтчетОПереработке.Запасы.(
	|		Ссылка КАК Ссылка,
	|		ВЫБОР
	|			КОГДА (ВЫРАЗИТЬ(ОтчетОПереработке.Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000))) = """"
	|				ТОГДА ОтчетОПереработке.Запасы.Номенклатура.Наименование
	|			ИНАЧЕ ВЫРАЗИТЬ(ОтчетОПереработке.Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000))
	|		КОНЕЦ КАК ПредставлениеНоменклатуры,
	|		НЕОПРЕДЕЛЕНО КАК Содержание,
	|		Номенклатура.Склад КАК Склад,
	|		Номенклатура.Ячейка КАК Ячейка,
	...
	|			КОГДА &ЭтоПечатьАктаВыполненныхРабот = ИСТИНА
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|			ИНАЧЕ ОтчетОПереработке.Запасы.Номенклатура.ТипНоменклатуры
	|		КОНЕЦ КАК ТипНоменклатуры,
	|		Номенклатура.Код КАК Код
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "ОтчетОПереработке");
	Если ПечатнаяФорма <> Неопределено Тогда
		
		ПечатнаяФорма.ТабличныйДокумент = Новый ТабличныйДокумент;
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ОтчетОПереработке_ОтчетОПереработке";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ОтчетОПереработке.ПФ_MXL_ОтчетОПереработке";
		ПечатнаяФорма.СинонимМакета = НСтр("ru ='Отчет о переработке'");
		...
		СформироватьОтчетОПереработке(ПечатнаяФорма, ДанныеОбъектовПечати, ОбъектыПечати);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ОтчетОПереработке";
	КомандаПечати.Представление = НСтр("ru = 'Отчет о переработке'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 26;
	
КонецПроцедуры

Функция СформироватьОтчетОПереработке(ОписаниеПечатнойФормы, ДанныеОбъектовПечати, ОбъектыПечати)
	...
	ЕстьПоложениеЯчейкиОтправителя = (ДанныеОбъектовПечати.Колонки.Найти("ПоложениеЯчейкиОтправителя") <> Неопределено);
	ЕстьПоложениеЯчейкиПолучателя = (ДанныеОбъектовПечати.Колонки.Найти("ПоложениеЯчейкиПолучателя") <> Неопределено);
	...
	Для Каждого ДанныеОбъекта Из ДанныеОбъектовПечати Цикл
		...
		ЯчейкаВТабличнойЧасти = Ложь;
		Если ОписаниеПечатнойФормы.ИмяМакета = "БланкТоварногоНаполненияОтправитель" Тогда
			ЯчейкаВТабличнойЧасти = ЕстьПоложениеЯчейкиОтправителя И ДанныеОбъекта.ПоложениеЯчейкиОтправителя = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти;
		ИначеЕсли ОписаниеПечатнойФормы.ИмяМакета = "БланкТоварногоНаполненияПолучатель" Тогда
			ЯчейкаВТабличнойЧасти = ЕстьПоложениеЯчейкиПолучателя И ДанныеОбъекта.ПоложениеЯчейкиПолучателя = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти;
		КонецЕсли;
		
		Если СкладВШапке И ПолучитьФункциональнуюОпцию("УчетПоЯчейкам") И Не ЯчейкаВТабличнойЧасти Тогда
		...
		Если ОбластьМакета <> Неопределено И ОбластьМакетаШапка <> Неопределено Тогда
			...
			Если ЕстьТЧЗапасы Тогда
				...
				ДанныеПечати.Вставить("ТипНоменклатурыТаблицы", НСтр("ru ='Продукция'"));
			...
			Если ЕстьТЧМатериалы И ДанныеОбъекта.ТаблицаМатериалы.Количество() Тогда
...
Процедура СформироватьТаблицаСерииНоменклатуры(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	|				И (ВременнаяТаблицаЗапасы.УчетПоСкладуВЦелом
	|					ИЛИ ВременнаяТаблицаЗапасы.УтетПоПрочимДокументам))";
