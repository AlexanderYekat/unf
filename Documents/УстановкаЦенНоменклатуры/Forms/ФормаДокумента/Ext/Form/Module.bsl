...
Процедура ПересчитатьПроцентИзменения(ДанныеТекущейСтроки, Идентификатор)
	...
	Если ДанныеТекущейСтроки[ИмяПоляЦенаДо] > 0 Тогда 
		...
		ДанныеТекущейСтроки[ИмяПоляПроцентИзменения] = ИзменениеЦены / ДанныеТекущейСтроки[ИмяПоляЦенаДо] * 100;			
	Иначе
...
Процедура ЗаполнитьМассивВсехВидовЦенРекурсивно(МассивВидовЦен, ВыбранныеВидыЦен) 
	...
	Для Каждого ЭлементСоответсвия Из ВыбранныеВидыЦен Цикл 
		
		ТекущийВидЦен = ЭлементСоответсвия.Ключ;
		ЗависимыеВидыЦен = ЭлементСоответсвия.Значение;
...
Процедура ЗаполнитьЗависимыеИдентификаторыРекурсивно(ДанныеИдентификаторов, БазовыйВидЦен, ПодчиненныеВидыЦен,
	...
	Для Каждого ЭлементСоответсвия Из ПодчиненныеВидыЦен Цикл 
...
Процедура ПослеДобавленияНоменклатуры(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.ВыборПроизведен Тогда
		...
		Если Результат.Свойство("ЗаполнятьНаОснованиеПрихода") 
			И Результат.ЗаполнятьНаОснованиеПрихода Тогда 
			
			ДополнительныеПараметры.Вставить("ЗаполнятьНаОснованиеПрихода", Истина);
		КонецЕсли;
