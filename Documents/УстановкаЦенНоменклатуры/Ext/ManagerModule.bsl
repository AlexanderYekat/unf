...
Процедура ЗаполнитьПоказательПроцентИзмененияЦены(СтрокаПриемник, ИмяТабличнойЧасти, ИдентификаторВидаЦен) 
	...
	Если СтрокаПриемник[ИмяКолонкиЦенаДо] > 0 Тогда
		
		СтрокаПриемник[ИмяКолонкиПроцентИзменения] = (СтрокаПриемник[ИмяКолонкиЦенаНовая] - СтрокаПриемник[ИмяКолонкиЦенаДо]) 
			/ СтрокаПриемник[ИмяКолонкиЦенаДо] * 100;  
		
		Если СтрокаПриемник[ИмяКолонкиЦенаНовая] = 0 Тогда 
...
Процедура ЗаполнитьПоказателиИзмененияИНаценкиЗависимыхЦен(СтрокаПриемник, ИмяТабличнойЧасти, 
		...
		ЭтоНовыйДокумент, ЗначенияРеквизитовЦеновыхГрупп, ЗначенияРеквизитовЗависимыхВидовЦен) 
	...
	// % изменения    
	Если СтрокаПриемник[ИмяКолонкиЦенаДо_ТекущийВидЦен] > 0  Тогда
		СтрокаПриемник[ИмяКолонкиПроцентИзменения] = 
			(СтрокаПриемник[ИмяКолонкиЦенаНовая_ТекущийВидЦен] - СтрокаПриемник[ИмяКолонкиЦенаДо_ТекущийВидЦен])
				/ СтрокаПриемник[ИмяКолонкиЦенаДо_ТекущийВидЦен] * 100;  
	Иначе 
		СтрокаПриемник[ИмяКолонкиПроцентИзменения] = 0;
	КонецЕсли;
...
Процедура КопироватьДобавлениемСтрокТаблицЗначений(ТаблицаИсточник, ТаблицаПриемник, ПараметрыКопирования) Экспорт
	...
	Перем ИспользоватьРежимПодбора;
	...
	Если ТипЗнч(ТаблицаИсточник) = Тип("ТаблицаЗначений")
		...
		МассивНоменклатуры = ТаблицаКопирования.ВыгрузитьКолонку("Номенклатура");
		...
		Для каждого СтрокаИсточник Из ТаблицаКопирования Цикл
			...
			Если ПараметрыКопирования.Свойство("ВыбранныеВидыЦен") Тогда
				...
				Для Каждого СтрокаВидыЦен Из КоллекцияВидовЦен Цикл 
					...
					ИдентификаторВидаЦен = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийВидЦен, "ИдентификаторФормул");
					Если НЕ ЗначениеЗаполнено(ИдентификаторВидаЦен) Тогда
						Продолжить;
					КонецЕсли;

					ЗаполнитьПоказательПроцентИзмененияЦены(НоваяСтрока, ИмяТабличнойЧасти, ИдентификаторВидаЦен); 
						...
						Для Каждого СтрокаЗависимыхВидовЦен Из КолекцияЗависимыеВидыЦен Цикл
							...
							ЗаполнитьПоказателиИзмененияИНаценкиЗависимыхЦен(НоваяСтрока, ИмяТабличнойЧасти, ИспользоватьРежимПодбора, 
								ЗависимыйВидЦен, ИдентификаторВидаЦен, ПересчетНаценки, ПараметрыКопирования.ЭтоНовый, 
									ЗначенияРеквизитовЦеновыхГрупп, ЗначенияРеквизитовЗависимыхВидовЦен);  			
						КонецЦикла;
