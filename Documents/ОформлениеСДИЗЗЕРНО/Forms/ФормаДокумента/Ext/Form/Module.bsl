...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИнтеграцияЗЕРНОКлиентСервер.ЭтоВыборПартииЗЕРНО(ВыбранноеЗначение) Тогда
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			ОбновитьДанные(
				НоваяСтрока.Партия,
				Новый Структура("ИмяПоля, ИдентификаторСтроки", "ТоварыПартия", НоваяСтрока.ПолучитьИдентификатор()));
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Функция ПунктНазначенияСписокАвтоподбораОбщий(ТекстДляАвтоПодбора)
	...
	СписокАвтоподбора = ИнтеграцияЗЕРНОВызовСервера.СписокАвтоподбораПоОписаниюТипов(
