...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаборСвойств_Документ_ПереоценкаТоваровНаКомиссии = УправлениеСвойствами.НаборСвойствПоИмени(
		"Документ_ПереоценкаТоваровНаКомиссии");
	КэшЗначений.Вставить("НаборСвойств_Документ_ПереоценкаТоваровНаКомиссии",
		НаборСвойств_Документ_ПереоценкаТоваровНаКомиссии);
	...
	РассчитатьСтаруюЦенуПриСозданииНаСервере();
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) И ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ПерезаполнитьЦеныТабличнойЧастиПоВидуЦен(ЭтаФорма, ИмяТЧЗапасы());
	КонецЕсли;
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если КонтрагентПередИзменением <> Объект.Контрагент Тогда
		...
		СтруктураДанные = ПолучитьДанныеКонтрагентПриИзменении(Объект.Дата, Объект.Контрагент, Объект.Организация);
...
Процедура ДоговорПриИзменении(Элемент)
	...
	Объект.ДокументОснование = Неопределено;
	ОбработатьИзменениеДоговора(ДоговорПередИзменением);
...
&НаКлиенте
Процедура ЗапасыЦенаСтараяПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.Запасы.ТекущиеДанные;
	РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти)
КонецПроцедуры

&НаКлиенте
Процедура ЗапасыСуммаСтараяПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.Запасы.ТекущиеДанные;
	РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти, Истина)
КонецПроцедуры
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", КэшЗначений.НаборСвойств_Документ_ПереоценкаТоваровНаКомиссии);
...
Процедура ЗагрузкаИзФайлаЗапасы(Команда)
	...
	НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "Документ.ПереоценкаТоваровНаКомиссии.ТабличнаяЧасть.Запасы";
	
	НастройкиЗагрузкиДанных.Вставить("ПолноеИмяТабличнойЧасти", "ПереоценкаТоваровНаКомиссии.Запасы");
...
&НаСервере
Процедура РассчитатьСтаруюЦенуПриСозданииНаСервере()
	Для Каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл
		...
		СтрокаТабличнойЧасти.ЦенаСтарая = Окр(СтрокаТабличнойЧасти.СуммаСтарая/СтрокаТабличнойЧасти.Количество, 2, РежимОкругления.Окр15как20);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПоказатьСкрытьПанельРедактирования(ИмяТЧ, ИзменяетДанные = Неопределено)
	...
	ГрупповоеИзменениеСтрокСервер.ПоказатьСкрытьПанельРедактирования(ЭтотОбъект,
		НаборЭлементовГрупповогоИзмененияСтрокСервер(ИмяТЧ), СостояниеПерехода, ИзменяетДанные);
...
Процедура ОбновитьПодвалФормы()
	ИтогВсего = Объект.Запасы.Итог("Сумма");
	Объект.СуммаОтклонений = Объект.Запасы.Итог("Отклонение");
КонецПроцедуры
...
Процедура ПолучитьЗапасыИзХранилища(АдресЗапасовВХранилище, ИмяТабличнойЧасти, ИдентификаторыНовыхСтрок = Неопределено, МассивНомеровИдентичныхСтрок = Неопределено)
	...
	Для каждого СтрокаЗагрузки Из ТаблицаДляЗагрузки Цикл
		...
		Если ЭтоПодборПоОстаткам Тогда
			НоваяСтрока.СуммаСтарая = СтрокаЗагрузки.СуммаРасчетов;
			
			Если НЕ НоваяСтрока.Количество = 0 Тогда
				НоваяСтрока.ЦенаСтарая = Окр(СтрокаЗагрузки.СуммаРасчетов/НоваяСтрока.Количество, 2, РежимОкругления.Окр15как20);
			КонецЕсли;
...
Процедура РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти, РассчитатьЦену = Ложь)
	...
	Если Не РассчитатьЦену Тогда
		...
		СтрокаТабличнойЧасти.Отклонение = СтрокаТабличнойЧасти.Сумма - СтрокаТабличнойЧасти.СуммаСтарая;
	Иначе
		...
		СтрокаТабличнойЧасти.ЦенаСтарая = ?(КоличествоСтроки=0, 0, СтрокаТабличнойЧасти.СуммаСтарая / КоличествоСтроки);
		СтрокаТабличнойЧасти.Отклонение = СтрокаТабличнойЧасти.Сумма - СтрокаТабличнойЧасти.СуммаСтарая;
	КонецЕсли;
...
Процедура КонтрагентПриИзмененииФрагмент(ДоговорПередИзменением, СтруктураДанные)
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Договор) И ИзменилсяВидЦен Тогда
		...
		Если ПересчетНеобходим Тогда
			...
			ПараметрыОповещения.Вставить("ДоговорПередИзменением", ДоговорПередИзменением);
			ПараметрыОповещения.Вставить("ВалютаРасчетовПередИзменением", ВалютаРасчетовПередИзменением);
			ПараметрыОповещения.Вставить("СтруктураДанные", СтруктураДанные);
			ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьИзменениеДоговораЗавершение", ЭтотОбъект,
				ПараметрыОповещения);
...
&НаКлиенте
Процедура ОбработатьИзменениеДоговора(ДоговорПередИзменением)
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Договор) 
		...
		Если ПересчетНеобходим Тогда
			...
			ПараметрыОповещения.Вставить("ДоговорПередИзменением", ДоговорПередИзменением);
			ПараметрыОповещения.Вставить("ВалютаРасчетовПередИзменением", ВалютаРасчетовПередИзменением);
			ПараметрыОповещения.Вставить("СтруктураДанные", ДанныеДоговора);
			ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьИзменениеДоговораЗавершение", ЭтотОбъект,
				ПараметрыОповещения);
			ПоказатьВопрос(ОписаниеОповещения, Сообщение, РежимДиалогаВопрос.ДаНет);
...
Процедура ПолученыШтрихкоды(ДанныеШтрихкодов) Экспорт
	...
	Если НеизвестныеШтрихкоды.Количество() > 0 Тогда
		...
		ОткрытьФорму("РегистрСведений.ШтрихкодыНоменклатуры.Форма.РегистрацияШтрихкодовНоменклатуры", ПараметрыФормы,ЭтотОбъект,,,, ОписаниеОповещения);
...
Процедура ОткрытьФормуНастройки(ПередаватьВидДокумента)
	...
	Если ПередаватьВидДокумента Тогда
		ПараметрыДиалога.Вставить("ВидДокумента", НСтр("ru='переоценка товаров на комиссии'"));
	КонецЕсли;
