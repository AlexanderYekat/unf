...
Функция СведенияОРазрешительнойДокументацииJSONОтчетПроизводственнойЛинииJSON(РезультатЗапроса, ДополнительныеПараметры, РеквизитыОрганизации)
	...
	Если Шапка.Операция = Перечисления.ВидыОперацийИСМП.СведенияОКодахИдентификацииДляВводаВОборот
		Или Шапка.Операция = Перечисления.ВидыОперацийИСМП.СведенияОРазрешительнойДокументацииДляВводаВОборот Тогда
		
		ЗаполнитьСообщениеСведенияОРазрешительнойДокументации(ТелоЗапроса, Шапка, ИсточникДанных, ДокументыСертификации,
			РезультатОбработкиДанныхОтчета)
		
	Иначе
		
		Если Не ЗаполнитьСведенияКорректировки(СтрокаТЧ, Шапка, ДокументыСертификации) Тогда
...
// Заполенение разрешительной документации, ТН ВЭД, даты производства, тип эмиссии
Процедура ЗаполнитьСообщениеКорректировкаСведенийКМ(ГруппаКодов, ИсточникДанных, Товары, СообщениеJSON, ДополнительныеПараметры)
	...
	ВидПродукции                = ДополнительныеПараметры.ВидПродукции;
	ПараметрыНормализацииПрочее = ДополнительныеПараметры.ПараметрыНормализацииПрочее;
	ДокументыСертификации       = ДополнительныеПараметры.ДокументыСертификации;
	...
	Если ДокументыСертификации.Количество() Тогда
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		// permitDocsOperation
		// licencesOperation
		// licences [licence_number, licence_date]
		...
		Для Каждого СтрокаШтрихкоды Из НайденныеСтроки Цикл
			// тут может быть КИН после обновления ТАПИ
...
Процедура ЗаполнитьСообщениеСведенияОРазрешительнойДокументации(ТелоЗапроса, Шапка, ИсточникДанных, ДокументыСертификации,
		РезультатОбработкиДанныхОтчета)
	
	Если ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияМОТП(Шапка.ВидПродукции)
...
Функция ЗаполнитьСведенияКорректировки(ЗаполняемыеСведения, Шапка, ДокументыСертификации)
