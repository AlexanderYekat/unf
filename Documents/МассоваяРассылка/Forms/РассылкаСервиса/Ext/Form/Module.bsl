...
&НаСервере
Процедура ОбработатьАдреснуюКингу()
	Если ЭтоСозданиеНовойАдреснойКнигиСервиса Тогда
		СоздатьНовуюАдреснуюКнигуСервиса();
	Иначе
		Если ЗначениеЗаполнено(Объект.ПредлагаемаяСкидка) Тогда
			// Это может быть рассылка промокодов
			ОбновитьПромокоды();
		КонецЕсли;		
	КонецЕсли;
КонецПроцедуры
...
Процедура СоздатьНовуюАдреснуюКнигуСервиса()
	...
	Для каждого ДанныеПолучателя Из Получатели Цикл
		...
		ДанныеПолучателяКниги.Промокод = ДанныеПолучателя.Промокод;
	КонецЦикла;
...
Процедура ОбновитьПромокоды()
	...
	Для каждого ДанныеПолучателя Из Получатели Цикл
		...
		ДанныеПолучателяКниги.Промокод = ДанныеПолучателя.Промокод;
	КонецЦикла;
	МассовыеРассылкиИнтеграция.ОбновитьПромокоды(ДанныеАдреснойКниги);
...
Функция СоздатьМассовуюРассылкуЗавершение(ДанныеРассылкиАдрес)
	...
	Если ЗначениеЗаполнено(ДанныеРассылки.Идентификатор) Тогда
		...
		КонецЕсли;
		Объект.Состояние = Перечисления.СостоянияОтправкиРассылки.ВнешнийСервис;
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Получатели.Промокод",, ВидСравненияКомпоновкиДанных.НеЗаполнено);
...
Процедура ОтразитьПромокодыНаФорме()
	...
	Для Каждого СтрокаПромокода Из Объект.Промокоды Цикл
		...
		СтрокиОбновить = Получатели.НайтиСтроки(Отбор);
...
Процедура РаспределитьПромокоды(ЗаполнениеОбязательно = Ложь)
	...
	СтрокиАвто = Получатели.НайтиСтроки(Отбор);
	...
	Если ЗаполнениеОбязательно И СтрокиАвто.Количество() > 0 Тогда
		...
		Иначе
			...
			ВызватьИсключение(НСтр("ru = 'Невозможно сгенерировать промокоды, проверьте шаблон генерации в условиях скидки'"));
		КонецЕсли;
	КонецЕсли;	
	СчетчикПромокодов = 0;
	...
	Для Каждого СтрокаАвто Из СтрокиАвто Цикл
		...
		Если ЗаполнениеОбязательно И Не ЕстьДоступныйПромокод Тогда
			ВызватьИсключение(НСтр("ru = 'Не удалось создать промокоды для рассылки'"));
		ИначеЕсли Не ЕстьДоступныйПромокод Тогда
			Продолжить;
		КонецЕсли; 
...
Процедура ПолучателиПромокодПриИзменении(Элемент)
	...
	СтрокаПолучателя = Получатели.НайтиПоИдентификатору(ТекущаяСтрока);
...
Процедура УправлениВидимостьюПромокодов()
	...
	ЗаголовокПолучателей = НСтр("ru = 'Получатели'");
	Если ИспользоватьПромокоды Тогда 
		ЗаголовокПолучателей = НСтр("ru = 'Получатели промокодов'");
	КонецЕсли;
	Элементы.ГруппаПодсказкаШаблонГенерации.Видимость = Ложь;
	Элементы.ГруппаПолучатели.Заголовок = ЗаголовокПолучателей;
...
Процедура ОбработатьВыбранныеПромокоды(Значение, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ОбщийПромокод", ОбщийПромокод) И ОбщийПромокод Тогда
		...
		СтрокиАвто = Получатели.НайтиСтроки(Новый Структура("Промокод", ""));
