...
Функция СформироватьДвиженияПоРегиструУзлыКорректировкиСтоимостиСписания(Отказ)
	...
	Запрос.УстановитьПараметр("ДатаКон", ДополнительныеСвойства.ДляПроведения.КонечнаяДатаПериода);
	...
	|	ВЫБОР
	|		КОГДА СУММА(ВложенныйЗапрос.Количество) = 0
	|				И СУММА(ВложенныйЗапрос.СуммаБезНДСДляКоличества) = 0
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.СуммаБезНДС) / ВЫБОР
	|					КОГДА СУММА(ВложенныйЗапрос.Количество) = 0
	|						ТОГДА СУММА(ВложенныйЗапрос.СуммаБезНДСДляКоличества)
	|					ИНАЧЕ СУММА(ВложенныйЗапрос.Количество)
	...
	|	ВЫБОР
	|		КОГДА СУММА(ВложенныйЗапрос.Количество) = 0
	|				И СУММА(ВложенныйЗапрос.СуммаИнтДляКоличества) = 0
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.СуммаИнт) / ВЫБОР
	|					КОГДА СУММА(ВложенныйЗапрос.Количество) = 0
	|						ТОГДА СУММА(ВложенныйЗапрос.СуммаИнтДляКоличества)
	|					ИНАЧЕ СУММА(ВложенныйЗапрос.Количество)
	|				КОНЕЦ КАК ЧИСЛО(23, 10))
	|	КОНЕЦ КАК СуммаИнт,
	|	ВЫБОР
	|		КОГДА СУММА(ВложенныйЗапрос.Количество) = 0
	|				И СУММА(ВложенныйЗапрос.СуммаБезНДСИнтДляКоличества) = 0
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.СуммаБезНДСИнт) / ВЫБОР
	|					КОГДА СУММА(ВложенныйЗапрос.Количество) = 0
	|						ТОГДА СУММА(ВложенныйЗапрос.СуммаБезНДСИнтДляКоличества)
	|					ИНАЧЕ СУММА(ВложенныйЗапрос.Количество)
	|				КОНЕЦ КАК ЧИСЛО(23, 10))
	|	КОНЕЦ КАК СуммаБезНДСИнт
	...
	|			ВЫБОР
	|				КОГДА УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|					ТОГДА УчетЗатрат.СуммаБезНДС
	|				ИНАЧЕ -УчетЗатрат.СуммаБезНДС
	|			КОНЕЦ КАК СуммаБезНДСДляКоличества,
	|			ВЫБОР
	|				КОГДА УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	...
	|			ВЫБОР
	|				КОГДА УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|					ТОГДА УчетЗатрат.СуммаИнт
	|				ИНАЧЕ -УчетЗатрат.СуммаИнт
	|			КОНЕЦ КАК СуммаИнтДляКоличества,
	|			ВЫБОР
	|				КОГДА УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|						И (УчетЗатрат.ФиксированнаяСтоимость)
	|					ТОГДА УчетЗатрат.СуммаИнт
	...
	|			ВЫБОР
	|				КОГДА УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|					ТОГДА УчетЗатрат.СуммаБезНДСИнт
	|				ИНАЧЕ -УчетЗатрат.СуммаБезНДСИнт
	|			КОНЕЦ КАК СуммаБезНДСИнтДляКоличества,
	|			ВЫБОР
	|				КОГДА УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|						И (УчетЗатрат.ФиксированнаяСтоимость)
	|					ТОГДА УчетЗатрат.СуммаБезНДСИнт
	...
	|				КОГДА УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|					ТОГДА УчетЗатрат.СуммаБезНДС
	|				ИНАЧЕ 0 - УчетЗатрат.СуммаБезНДС
	|			КОНЕЦ,
	|			ВЫБОР
	|				КОГДА УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|					ТОГДА УчетЗатрат.СуммаБезНДС
	...
	|				КОГДА УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|					ТОГДА УчетЗатрат.СуммаБезНДСИнт
	|				ИНАЧЕ 0 - УчетЗатрат.СуммаБезНДСИнт
	|			КОНЕЦ,
	|			ВЫБОР
	|				КОГДА УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|					ТОГДА УчетЗатрат.СуммаБезНДСИнт
	...
	|			УчетЗатратОстатки.СуммаБезНДСОстаток,
	|			УчетЗатратОстатки.СуммаБезНДСОстаток,
	|			УчетЗатратОстатки.СуммаОстаток,
	|			УчетЗатратОстатки.СуммаОстаток,
	|			УчетЗатратОстатки.СуммаИнтОстаток,
	|			УчетЗатратОстатки.СуммаИнтОстаток,
	|			УчетЗатратОстатки.СуммаБезНДСИнтОстаток,
	|			УчетЗатратОстатки.СуммаБезНДСИнтОстаток
	|		ИЗ
	...
	Если Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("УчетЗатратСДатамиДопРасходов") <> Неопределено Тогда
		
		Запрос.Текст = "УНИЧТОЖИТЬ УчетЗатратСДатамиДопРасходов;
					|	УНИЧТОЖИТЬ НачОстаткиПартийFIFO;
					|	УНИЧТОЖИТЬ УчетЗатратПоступлениеПартий";
		Запрос.Выполнить();
	...
	|	0 КАК СуммаОстаток,
	|	0 КАК СуммаИнтОстаток,
	|	0 КАК СуммаБезНДСИнтОстаток
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	УчетЗатрат.ВидДвижения КАК ВидДвижения,
	|	УчетЗатрат.Организация КАК Организация,
	|	УчетЗатрат.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	УчетЗатрат.СчетУчета КАК СчетУчета,
	|	УчетЗатрат.Номенклатура КАК Номенклатура,
	...
	|	УчетЗатрат.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
	|	ВЫБОР
	|		КОГДА УчетЗатрат.Регистратор ССЫЛКА Документ.ДополнительныеРасходы
	|				И УчетЗатрат.ДокументПродажи <> НЕОПРЕДЕЛЕНО
	|				И УчетЗатрат.ДокументПродажи ССЫЛКА Документ.ПриходнаяНакладная
	|				И УчетЗатрат.ДокументПродажи <> ЗНАЧЕНИЕ(Документ.ПриходнаяНакладная.ПустаяСсылка)
	|			ТОГДА ВЫРАЗИТЬ(УчетЗатрат.ДокументПродажи КАК Документ.ПриходнаяНакладная).Дата
	|		ИНАЧЕ УчетЗатрат.Период
	|	КОНЕЦ КАК Период,
	|	УчетЗатрат.Количество КАК Количество,
	|	УчетЗатрат.СуммаБезНДС КАК СуммаБезНДС,
	|	УчетЗатрат.Сумма КАК Сумма,
	|	УчетЗатрат.СуммаИнт КАК СуммаИнт,
	|	УчетЗатрат.СуммаБезНДСИнт КАК СуммаБезНДСИнт
	|ПОМЕСТИТЬ УчетЗатратСДатамиДопРасходов
	|ИЗ
	|	РегистрНакопления.Запасы КАК УчетЗатрат
	|ГДЕ
	|	УчетЗатрат.Период МЕЖДУ &ДатаНач И &ДатаКон
	|	И (УчетЗатрат.КоррСчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ИЛИ УчетЗатрат.Регистратор ССЫЛКА Документ.ОтчетПереработчика)
	|	И УчетЗатрат.Организация = &Организация
	|	И (УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				И НЕ УчетЗатрат.Возврат
	|			ИЛИ УчетЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|				И УчетЗатрат.Возврат)
	|	И НЕ(УчетЗатрат.Регистратор ССЫЛКА Документ.СборкаЗапасов)
	|	И НЕ(УчетЗатрат.Регистратор ССЫЛКА Документ.КомплектацияЗапасов)
	|	И НЕ(УчетЗатрат.Регистратор ССЫЛКА Документ.ДополнительныеРасходы
	|				И УчетЗатрат.ДокументПродажи <> НЕОПРЕДЕЛЕНО
	|				И УчетЗатрат.ДокументПродажи ССЫЛКА Документ.ПриходнаяНакладная
	|				И УчетЗатрат.ДокументПродажи <> ЗНАЧЕНИЕ(Документ.ПриходнаяНакладная.ПустаяСсылка)
	|				И (ВЫРАЗИТЬ(УчетЗатрат.ДокументПродажи КАК Документ.ПриходнаяНакладная).Дата < &ДатаНач
	|					ИЛИ ВЫРАЗИТЬ(УчетЗатрат.ДокументПродажи КАК Документ.ПриходнаяНакладная).Дата > &ДатаКон))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	УчетЗатратОстатки.СуммаБезНДСОстаток КАК СуммаБезНДС,
	|	УчетЗатратОстатки.СуммаОстаток КАК Сумма,
	|	УчетЗатратОстатки.СуммаИнтОстаток КАК СуммаИнт,
	|	УчетЗатратОстатки.СуммаБезНДСИнтОстаток КАК СуммаБезНДСИнт
	|ПОМЕСТИТЬ НачОстаткиПартийFIFO
	|ИЗ
	|	РегистрНакопления.Запасы.Остатки(
	|			&ДатаНач,
	|			Организация = &Организация
	...
	|ПОМЕСТИТЬ УчетЗатратПоступлениеПартий
	|ИЗ
	|	НачОстаткиПартийFIFO КАК УчетЗатрат
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	УчетЗатрат.СуммаБезНДС,
	|	УчетЗатрат.Сумма,
	|	УчетЗатрат.СуммаИнт,
	|	УчетЗатрат.СуммаБезНДСИнт,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|ИЗ
	|	УчетЗатратСДатамиДопРасходов КАК УчетЗатрат
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ПериодыПартий
	|ИЗ
	|	УчетЗатратПоступлениеПартий КАК УчетЗатрат
	...
	|ГДЕ
	|	УчетЗатрат.Период < ТаблицаОстатковПартий.Период
	|
	|СГРУППИРОВАТЬ ПО
	|	УчетЗатрат.Организация,
	|	УчетЗатрат.СтруктурнаяЕдиница,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ УчетЗатратПоступлениеПартий КАК УчетЗатрат
	...
	|	ВЫБОР
	|		КОГДА ТаблицаОстатковПартий.КоличествоОстаток >= ЕСТЬNULL(ВнешниеПоступленияЗаПериод.Количество, 0)
	|			ТОГДА ТаблицаОстатковПартий.СуммаБезНДСОстаток + ЕСТЬNULL(ВнешниеПоступленияЗаПериод.СуммаБезНДС, 0)
	|		ИНАЧЕ ТаблицаОстатковПартий.СуммаБезНДСОстаток + (ВЫРАЗИТЬ(ВнешниеПоступленияЗаПериод.СуммаБезНДС * ТаблицаОстатковПартий.КоличествоОстаток / ВнешниеПоступленияЗаПериод.Количество КАК ЧИСЛО(23, 10)))
	...
	|	ВЫБОР
	|		КОГДА ТаблицаОстатковПартий.КоличествоОстаток >= ЕСТЬNULL(ВнешниеПоступленияЗаПериод.Количество, 0)
	|			ТОГДА ТаблицаОстатковПартий.СуммаИнтОстаток + ЕСТЬNULL(ВнешниеПоступленияЗаПериод.СуммаИнт, 0)
	|		ИНАЧЕ ТаблицаОстатковПартий.СуммаИнтОстаток + (ВЫРАЗИТЬ(ВнешниеПоступленияЗаПериод.СуммаИнт * ТаблицаОстатковПартий.КоличествоОстаток / ВнешниеПоступленияЗаПериод.Количество КАК ЧИСЛО(23, 10)))
	|	КОНЕЦ КАК СуммаИнтОстаток,
	|	ВЫБОР
	|		КОГДА ТаблицаОстатковПартий.КоличествоОстаток >= ЕСТЬNULL(ВнешниеПоступленияЗаПериод.Количество, 0)
	|			ТОГДА ТаблицаОстатковПартий.СуммаБезНДСИнтОстаток + ЕСТЬNULL(ВнешниеПоступленияЗаПериод.СуммаБезНДСИнт, 0)
	|		ИНАЧЕ ТаблицаОстатковПартий.СуммаБезНДСИнтОстаток + (ВЫРАЗИТЬ(ВнешниеПоступленияЗаПериод.СуммаБезНДСИнт * ТаблицаОстатковПартий.КоличествоОстаток / ВнешниеПоступленияЗаПериод.Количество КАК ЧИСЛО(23, 10)))
	|	КОНЕЦ КАК СуммаБезНДСИнтОстаток,
	|	ТаблицаОстатковПартий.КоличествоОстатокНаКонецПериода КАК КоличествоОстатокНаКонецПериода,
	|	ВнешниеПоступленияЗаПериод.Период КАК Период
	...
	|	ТаблицаТекущихОстатковПартий.СуммаБезНДСОстаток КАК СуммаБезНДСОстаток,
	|	ТаблицаТекущихОстатковПартий.СуммаОстаток КАК СуммаОстаток,
	|	ТаблицаТекущихОстатковПартий.СуммаИнтОстаток КАК СуммаИнтОстаток,
	|	ТаблицаТекущихОстатковПартий.СуммаБезНДСИнтОстаток КАК СуммаБезНДСИнтОстаток,
	...
	|			ТОГДА 0
	|		ИНАЧЕ (ТаблицаУзлыКорректировкиСтоимостиСписания.СуммаБезНДС * ТаблицаУзлыКорректировкиСтоимостиСписания.Количество - ЕСТЬNULL(ТаблицаОстатковПартий.СуммаБезНДСОстаток, 0)) / (ТаблицаУзлыКорректировкиСтоимостиСписания.Количество - ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстатокНаКонецПериода, 0) + ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстаток, 0))
	|	КОНЕЦ КАК СуммаБезНДС,
	|	ВЫБОР
	|		КОГДА ТаблицаУзлыКорректировкиСтоимостиСписания.Количество - ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстатокНаКонецПериода, 0) + ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстаток, 0) = 0
	|			ТОГДА 0
	...
	|		КОГДА ТаблицаУзлыКорректировкиСтоимостиСписания.Количество - ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстатокНаКонецПериода, 0) + ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстаток, 0) = 0
	|			ТОГДА 0
	|		ИНАЧЕ (ТаблицаУзлыКорректировкиСтоимостиСписания.СуммаИнт * ТаблицаУзлыКорректировкиСтоимостиСписания.Количество - ЕСТЬNULL(ТаблицаОстатковПартий.СуммаИнтОстаток, 0)) / (ТаблицаУзлыКорректировкиСтоимостиСписания.Количество - ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстатокНаКонецПериода, 0) + ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстаток, 0))
	|	КОНЕЦ КАК СуммаИнт,
	|	ВЫБОР
	|		КОГДА ТаблицаУзлыКорректировкиСтоимостиСписания.Количество - ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстатокНаКонецПериода, 0) + ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстаток, 0) = 0
	|			ТОГДА 0
	|		ИНАЧЕ (ТаблицаУзлыКорректировкиСтоимостиСписания.СуммаБезНДСИнт * ТаблицаУзлыКорректировкиСтоимостиСписания.Количество - ЕСТЬNULL(ТаблицаОстатковПартий.СуммаБезНДСИнтОстаток, 0)) / (ТаблицаУзлыКорректировкиСтоимостиСписания.Количество - ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстатокНаКонецПериода, 0) + ЕСТЬNULL(ТаблицаОстатковПартий.КоличествоОстаток, 0))
	|	КОНЕЦ КАК СуммаБезНДСИнт
	|ИЗ
	...
	|				ИЛИ УчетЗатрат.СуммаБезНДС <> 0)) КАК УчетЗатратДляРасчетов
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			УчетЗатрат.Организация КАК Организация,
	|			УчетЗатрат.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|			УчетЗатрат.СчетУчета КАК СчетУчета,
	...
	|						И УчетЗатрат.Возврат)
	|			И УчетЗатрат.Организация = &Организация
	|	И НЕ(УчетЗатрат.Регистратор ССЫЛКА Документ.ПеремещениеЗапасов И УчетЗатрат.Регистратор.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПеремещениеЗапасов.ВозвратИзЭксплуатации)) 
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|	УзлыКорректировкиСтоимостиСписания.СуммаБезНДС КАК СуммаБезНДС,
	|	УзлыКорректировкиСтоимостиСписания.Сумма,
	|	УзлыКорректировкиСтоимостиСписания.СуммаИнт КАК СуммаИнт,
	|	УзлыКорректировкиСтоимостиСписания.СуммаБезНДСИнт КАК СуммаБезНДСИнт
...
Функция РешитьСЛУ()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗапасыПредварительно.Организация КАК Организация,
	|	ЗапасыПредварительно.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ЗапасыПредварительно.СчетУчета КАК СчетУчета,
	|	ЗапасыПредварительно.Номенклатура КАК Номенклатура,
	|	ЗапасыПредварительно.Характеристика КАК Характеристика,
	|	ЗапасыПредварительно.Партия КАК Партия,
	|	ЗапасыПредварительно.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ЗапасыПредварительно.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
	|	ЗапасыПредварительно.ЗаказПродажи КАК ЗаказПродажи,
	|	ЗапасыПредварительно.ДокументПродажи КАК ДокументПродажи,
	|	ЗапасыПредварительно.ВидДвижения КАК ВидДвижения,
	|	ЗапасыПредварительно.Возврат КАК Возврат,
	|	ЗапасыПредварительно.ФиксированнаяСтоимость КАК ФиксированнаяСтоимость,
	|	ЗапасыПредварительно.Количество КАК Количество,
	|	ЗапасыПредварительно.СуммаБезНДС КАК СуммаБезНДС,
	|	ЗапасыПредварительно.Сумма КАК Сумма
	|ПОМЕСТИТЬ ЗапасыПредварительно
	|ИЗ
	|	РегистрНакопления.Запасы КАК ЗапасыПредварительно
	|ГДЕ
	|	ЗапасыПредварительно.Организация = &Организация
	|	И ЗапасыПредварительно.Период МЕЖДУ &ДатаНач И &ДатаКон
	|	И ЗапасыПредварительно.ДокументПродажи <> НЕОПРЕДЕЛЕНО
	|	И КОНЕЦПЕРИОДА(ЗапасыПредварительно.ДокументПродажи.Дата, МЕСЯЦ) = КОНЕЦПЕРИОДА(ЗапасыПредварительно.Период, МЕСЯЦ)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументПродажи,
	|	Организация,
	...
	|	-СУММА(УчетЗапасовИЗатрат.СуммаБезНДС) КАК СуммаБезНДС,
	|	-СУММА(УчетЗапасовИЗатрат.Сумма) КАК Сумма
	|ПОМЕСТИТЬ УчетЗатратВозвратыТекПериода
	|ИЗ
	|	ЗапасыПредварительно КАК УчетЗапасовИЗатрат
	|ГДЕ
	|	УчетЗапасовИЗатрат.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	...
	|	СУММА(ЕСТЬNULL(УчетЗапасовИЗатрат.СуммаБезНДС, 0)) КАК СуммаБезНДСПродано,
	|	СУММА(ЕСТЬNULL(УчетЗапасовИЗатрат.Сумма, 0)) КАК СуммаПродано,
	|	УчетЗатратВозвратыТекПериода.Количество КАК КоличествоВозврат,
	|	УчетЗатратВозвратыТекПериода.СуммаБезНДС КАК СуммаБезНДСВозврат,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗапасыПредварительно КАК УчетЗапасовИЗатрат
	...
	|	УчетЗатратВозвратыТекПериода.СуммаБезНДС,
	...
	|		УчетЗатратВозвраты.СуммаБезНДСПродано,
	|		УчетЗатратВозвраты.СуммаПродано,
	|		УчетЗатратВозвраты.КоличествоВозврат,
	|		УчетЗатратВозвраты.СуммаБезНДСВозврат,
	...
	|			И УчетЗатратВозвраты.Организация = УзлыКорректировкиСтоимостиСписания.Организация
	|			И УчетЗатратВозвраты.СтруктурнаяЕдиница = УзлыКорректировкиСтоимостиСписания.СтруктурнаяЕдиница
	|			И УчетЗатратВозвраты.СчетУчета = УзлыКорректировкиСтоимостиСписания.СчетУчета
	|			И УчетЗатратВозвраты.Номенклатура = УзлыКорректировкиСтоимостиСписания.Номенклатура
	|			И УчетЗатратВозвраты.Характеристика = УзлыКорректировкиСтоимостиСписания.Характеристика
	|			И УчетЗатратВозвраты.Партия = УзлыКорректировкиСтоимостиСписания.Партия
	|			И УчетЗатратВозвраты.ЗаказПокупателя = УзлыКорректировкиСтоимостиСписания.ЗаказПокупателя
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерУзла
	|ИТОГИ ПО
	...
	ТаблицаВозвратов = РезультатЗапроса[4].Выгрузить();
	ТаблицаВозвратов.Очистить();
	
	ОбходПоОрганизация = РезультатЗапроса[4].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
					...
					Пока ОбходПоХарактеристика.Следующий() Цикл
						...
						Пока ОбходПоПартия.Следующий() Цикл
							...
							Пока ОбходПоДокументПродажи.Следующий() Цикл
								ОбходПоСуммаБезНДСВозврат = ОбходПоДокументПродажи.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
								Пока ОбходПоСуммаБезНДСВозврат.Следующий() Цикл
									ОбходПоСуммаВозврат = ОбходПоСуммаБезНДСВозврат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
									Пока ОбходПоСуммаВозврат.Следующий() Цикл
										...
										Пока ОбходПоКоличествоВозврат.Следующий() Цикл
											...
											СуммаБезНДСОсталосьРаспределить = ОбходПоКоличествоВозврат.СуммаБезНДСВозврат;
											...
											Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
												Если КоличествоОсталосьРаспределить > 0 Тогда
													...
													Если КоличествоОсталосьРаспределить <= ВыборкаДетальныеЗаписи.КоличествоПродано Тогда
														...
														НоваяСтрока.СуммаБезНДСРаспределено = СуммаБезНДСОсталосьРаспределить;
														СуммаБезНДСОсталосьРаспределить = 0;
													Иначе
														...
														НоваяСтрока.СуммаБезНДСРаспределено = ВыборкаДетальныеЗаписи.СуммаБезНДСПродано;
														СуммаБезНДСОсталосьРаспределить = СуммаБезНДСОсталосьРаспределить - ВыборкаДетальныеЗаписи.СуммаБезНДСПродано;
													КонецЕсли;
												КонецЕсли;
											КонецЦикла;
										КонецЦикла;
	...
	Пока (ТекущееОтклонение > ТребуемаяТочность * ТребуемаяТочность) И (КоличествоИтераций < 100) Цикл
		...
		|	СУММА(ВЫРАЗИТЬ(ВЫБОР
		|				КОГДА УзлыКорректировкиСтоимостиСписания.Количество <> 0
		|					ТОГДА ВЫРАЗИТЬ(ТаблицаРешений.СуммаИнт КАК ЧИСЛО(23, 10)) * ВЫБОР
		|							КОГДА УчетЗатрат.Количество = 0
		|								ТОГДА ВЫРАЗИТЬ(УчетЗатрат.СуммаИнт КАК ЧИСЛО(23, 10))
		|							ИНАЧЕ ВЫРАЗИТЬ(УчетЗатрат.Количество КАК ЧИСЛО(23, 10))
		|						КОНЕЦ / ВЫРАЗИТЬ(УзлыКорректировкиСтоимостиСписания.Количество КАК ЧИСЛО(23, 10))
		|				ИНАЧЕ 0
		|			КОНЕЦ КАК ЧИСЛО(23, 10))) КАК СуммаИнт,
		|	СУММА(ВЫРАЗИТЬ(ВЫБОР
		|				КОГДА УзлыКорректировкиСтоимостиСписания.Количество <> 0
		|					ТОГДА ВЫРАЗИТЬ(ТаблицаРешений.СуммаБезНДСИнт КАК ЧИСЛО(23, 10)) * ВЫБОР
		|							КОГДА УчетЗатрат.Количество = 0
		|								ТОГДА ВЫРАЗИТЬ(УчетЗатрат.СуммаБезНДСИнт КАК ЧИСЛО(23, 10))
		|							ИНАЧЕ ВЫРАЗИТЬ(УчетЗатрат.Количество КАК ЧИСЛО(23, 10))
		|						КОНЕЦ / ВЫРАЗИТЬ(УзлыКорректировкиСтоимостиСписания.Количество КАК ЧИСЛО(23, 10))
		|				ИНАЧЕ 0
		|			КОНЕЦ КАК ЧИСЛО(23, 10))) КАК СуммаБезНДСИнт
		...
		|ВЫБРАТЬ
		|	УзлыКорректировкиСтоимостиСписания.НомерУзла КАК НомерУзла,
		|	УзлыКорректировкиСтоимостиСписания.СуммаБезНДС + ЕСТЬNULL(ВременнаяТаблицаРешений.СуммаБезНДС, 0) КАК СуммаБезНДС,
		|	УзлыКорректировкиСтоимостиСписания.Сумма + ЕСТЬNULL(ВременнаяТаблицаРешений.Сумма, 0) КАК Сумма,
		|	УзлыКорректировкиСтоимостиСписания.СуммаИнт + ЕСТЬNULL(ВременнаяТаблицаРешений.СуммаИнт, 0) КАК СуммаИнт,
		|	УзлыКорректировкиСтоимостиСписания.СуммаБезНДСИнт + ЕСТЬNULL(ВременнаяТаблицаРешений.СуммаБезНДСИнт, 0) КАК СуммаБезНДСИнт
		|ПОМЕСТИТЬ ТаблицаРешений
		|ИЗ
		|	РегистрСведений.УзлыКорректировкиСтоимостиСписания КАК УзлыКорректировкиСтоимостиСписания
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаРешений КАК ВременнаяТаблицаРешений
...
Процедура СформироватьДвиженияПоРегиструУчетаЗатрат(НаборЗаписей, НаборЗаписейУправленческий, СтрокаДвижения, СуммаИлиСтруктура, ФиксированнаяСтоимость, СодержаниеПроводки = Неопределено)
	...
	Сумма = СуммаИлиСтруктура.СуммаКорректировки;
	СуммаБезНДС = СуммаИлиСтруктура.СуммаКорректировкиБезНДС;
...
Процедура СформироватьКорректирующиеДвиженияПоРегиструУчетаЗатрат(ТаблицаОшибок)
	...
	|					ИЛИ УчетЗатрат.ДокументПродажи.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПеремещениеЗапасов.ПередачаВЭксплуатацию))
	...
	|	КОНЕЦ КАК Количество,
	|	ЕСТЬNULL(УчетЗатрат.СуммаБезНДС, 0) КАК СуммаБезНДС,
	|	ЕСТЬNULL(УчетЗатрат.Сумма, 0) КАК Сумма,
	|	ЕСТЬNULL(УчетЗатрат.СуммаИнт, 0) КАК СуммаИнт,
	|	ЕСТЬNULL(УчетЗатрат.СуммаБезНДСИнт, 0) КАК СуммаБезНДСИнт,
	|	ЕСТЬNULL(ТаблицаРешений.СуммаБезНДС, 0) КАК ЦенаБезНДС,
	|	ЕСТЬNULL(ТаблицаРешений.Сумма, 0) КАК Цена,
	|	ЕСТЬNULL(ТаблицаРешений.СуммаИнт, 0) КАК ЦенаИнт,
	|	ЕСТЬNULL(ТаблицаРешений.СуммаБезНДСИнт, 0) КАК ЦенаИнтБезНДС,
	...
	|	УчетЗатрат.ДокументПродажи КАК ДокументПродажи,
	|	ВЫБОР
	|		КОГДА УчетЗатрат.ДокументПродажи ССЫЛКА Документ.РасходнаяНакладная
	|			ТОГДА УчетЗатрат.ДокументПродажи.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику)
	|		ИНАЧЕ ЛОЖЬ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СуммаПеремещенияБезНДС = ВыборкаДетальныеЗаписи.ЦенаБезНДС * ВыборкаДетальныеЗаписи.Количество;
		СуммаКорректировкиБезНДС = СуммаПеремещенияБезНДС - ВыборкаДетальныеЗаписи.СуммаБезНДС;
		
		СуммаПеремещенияИнт = ВыборкаДетальныеЗаписи.ЦенаИнт * ВыборкаДетальныеЗаписи.Количество;
		СуммаКорректировкиИнт = СуммаПеремещенияИнт - ВыборкаДетальныеЗаписи.СуммаИнт;
		
		СуммаПеремещенияИнтБезНДС = ВыборкаДетальныеЗаписи.ЦенаИнтБезНДС * ВыборкаДетальныеЗаписи.Количество;
		СуммаКорректировкиИнтБезНДС = СуммаПеремещенияИнтБезНДС - ВыборкаДетальныеЗаписи.СуммаБезНДСИнт;
		
		СуммыКорректировкиСтруктура = Новый Структура;
		СуммыКорректировкиСтруктура.Вставить("СуммаКорректировки", СуммаКорректировки);
		СуммыКорректировкиСтруктура.Вставить("СуммаКорректировкиБезНДС", СуммаКорректировкиБезНДС);
		СуммыКорректировкиСтруктура.Вставить("СуммаКорректировкиИнт", СуммаКорректировкиИнт);
		СуммыКорректировкиСтруктура.Вставить("СуммаКорректировкиИнтБезНДС", СуммаКорректировкиИнтБезНДС);
		
		Если Окр(СуммаКорректировки, 2) <> 0 Или Окр(СуммаКорректировкиБезНДС, 2) <> 0 Тогда
			...
			Если ВыборкаДетальныеЗаписи.КоррСчетУчета = ДополнительныеСвойства.ДляПроведения.ПустойСчет Тогда
				...
				Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДокументПродажи)
				   И ((ТипЗнч(ВыборкаДетальныеЗаписи.ДокументПродажи) = Тип("ДокументСсылка.РасходнаяНакладная")
					 И НЕ ВыборкаДетальныеЗаписи.ЭтоВозвратПоставщику)
				  ...
				  ИЛИ ТипЗнч(ВыборкаДетальныеЗаписи.ДокументПродажи) = Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями")) Тогда
					...
					НоваяСтрока.СебестоимостьБезНДС = СуммаКорректировкиБезНДС;
					НоваяСтрока.СебестоимостьИнт = СуммаКорректировкиИнт;
					НоваяСтрока.СебестоимостьБезНДСИнт = СуммаКорректировкиИнтБезНДС;
					
					Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ЗаказПродажи)
						И ВыборкаДетальныеЗаписи.УчетПотребностиПоСкладам = Истина Тогда
					...
					НоваяСтрока.СодержаниеПроводки = СодержанияПроводок.ОтражениеРасходов();
					
					Если УчетПоПроектам Тогда
						Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыборкаДетальныеЗаписи.ДокументПродажи, "Проект") Тогда
							НоваяСтрока.Проект = ВыборкаДетальныеЗаписи.ДокументПродажи.Проект;
						КонецЕсли;
					КонецЕсли;
					...
					НоваяСтрока.Содержание = СодержанияПроводок.ОтражениеРасходов();
					НоваяСтрока.Сумма = СуммаКорректировки;
					
				ИначеЕсли ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДокументПродажи)
						И ТипЗнч(ВыборкаДетальныеЗаписи.ДокументПродажи) = Тип("ДокументСсылка.РасходнаяНакладная")
						И ВыборкаДетальныеЗаписи.ЭтоВозвратПоставщику Тогда
					...
					Если СуммаКорректировки < 0 Тогда
						...
						НоваяСтрока.СуммаРасходов = СуммаКорректировки;
						НоваяСтрока.СодержаниеПроводки = СодержанияПроводок.ПрочиеРасходы();
					Иначе
					...
					Если СуммаКорректировки < 0 Тогда
						...
						НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
						НоваяСтрока.Сумма = - СуммаКорректировки;
						НоваяСтрока.Содержание = СодержанияПроводок.ПрочиеРасходы();
					Иначе
				...
				ИначеЕсли ВыборкаДетальныеЗаписи.ПеремещениеВРозницуСуммовойУчет Тогда
					...
					НоваяСтрока.СодержаниеПроводки = СодержанияПроводок.ПеремещениеВРозницу();
					НоваяСтрока.Себестоимость = СуммаКорректировки;
					
					// Движения по регистру Управленческий.
					ЗначенияРеквизитовТочка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
						ВыборкаДетальныеЗаписи.РозничнаяСтруктурнаяЕдиница, "СчетУчетаВРознице, СчетУчетаНаценки", Истина);
					...
					НоваяСтрока.Организация = ВыборкаДетальныеЗаписи.Организация;
					НоваяСтрока.СценарийПланирования = Справочники.СценарииПланирования.Фактический;
					НоваяСтрока.СчетДт = ЗначенияРеквизитовТочка.СчетУчетаВРознице;
					НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
					НоваяСтрока.Содержание = СодержанияПроводок.ПеремещениеВРозницу();
					НоваяСтрока.Сумма = СуммаКорректировки; 
					...
					НоваяСтрока.СценарийПланирования = Справочники.СценарииПланирования.Фактический;
					НоваяСтрока.СчетДт = ЗначенияРеквизитовТочка.СчетУчетаВРознице;
					НоваяСтрока.СчетКт = ЗначенияРеквизитовТочка.СчетУчетаНаценки;
					НоваяСтрока.Содержание = СодержанияПроводок.ТорговаяНаценка();
					НоваяСтрока.Сумма = - СуммаКорректировки; 
				...
				Иначе
					...
					Иначе
						...
						НоваяСтрока.СчетДт = ВыборкаДетальныеЗаписи.СчетУчетаСписания;
						НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
						НоваяСтрока.Содержание = СодержанияПроводок.СписаниеЗапасовНаПроизвольныйСчет();
						НоваяСтрока.Сумма = СуммаКорректировки;
...
Процедура РаспределитьСуммыБезКоличества(ВидОперации, ТаблицаОшибок)
	...
	ДатаКон = ДополнительныеСвойства.ДляПроведения.КонечнаяДатаПериода;
	...
	|				И (УчетЗатратОстатки.СуммаБезНДСОстаток МЕЖДУ -1 И 1
	|					ИЛИ УчетЗатратОстатки.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|			ТОГДА УчетЗатратОстатки.СуммаБезНДСОстаток
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаБезНДС,
	|	ВЫБОР
	|		КОГДА УчетЗатратОстатки.КоличествоОстаток = 0
	...
	Пока НЕ Результат.Пустой() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			СуммаКорректировкиБезНДС = ВыборкаДетальныеЗаписи.СуммаБезНДС;
			...
			Если ВыборкаДетальныеЗаписи.КоррСчетУчета = ДополнительныеСвойства.ДляПроведения.ПустойСчет Тогда
				...
				ИначеЕсли ВыборкаДетальныеЗаписи.ПеремещениеВРозницуСуммовойУчет Тогда
					...
					НоваяСтрока.СчетДт = ЗначенияРеквизитовТочка.СчетУчетаВРознице;
					НоваяСтрока.СчетКт = ЗначенияРеквизитовТочка.СчетУчетаНаценки;
					НоваяСтрока.Содержание = НСтр("ru = 'Торговая наценка'");
					НоваяСтрока.Сумма = - СуммаКорректировки; 
...
Процедура РассчитатьСтоимостьВозвратов(ОкончательныйРасчет = Ложь)
	...
	|	-СУММА(Запасы.СуммаБезНДС) КАК СуммаБезНДС,
	...
	|	И Запасы.КоррСчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	...
	|	Запасы.ЗаказПродажи.УчетПотребностиПоСкладам
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Запасы.СуммаБезНДС КАК СуммаБезНДС,
	|	Запасы.Сумма КАК Сумма
	|ПОМЕСТИТЬ Продажи
	|ИЗ
	|	РегистрНакопления.Запасы КАК Запасы
	|ГДЕ
	|	Запасы.Организация = &Организация
	|	И Запасы.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И НЕ Запасы.Возврат
	|	И Запасы.КоррСчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|	И Запасы.ДокументПродажи В
	|			(ВЫБРАТЬ
	|				ВтВозвраты.ДокументПродажи
	|			ИЗ
	|				ВтВозвраты)
	|;
	|
	...
	|	ТаблицаВозвратов.СуммаБезНДС КАК СуммаБезНДСВозвратов,
	...
	|	СУММА(ТаблицаПродажи.СуммаБезНДС) КАК СуммаБезНДСПродаж,
	|	СУММА(ТаблицаПродажи.Сумма) КАК СуммаПродаж
	|ИЗ
	|	ВтВозвраты КАК ТаблицаВозвратов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Продажи КАК ТаблицаПродажи
	...
	Запрос.УстановитьПараметр("КонецПериода", ДополнительныеСвойства.ДляПроведения.КонечнаяДатаПериода);
	...
	Если ОкончательныйРасчет Тогда
		...
		Запрос.УстановитьПараметр("ГраницаДатаКон", Новый 
			Граница(ДополнительныеСвойства.ДляПроведения.КонечнаяДатаПериода, ВидГраницы.Включая));
		Запрос.Текст =
		"ВЫБРАТЬ
		|	&Организация КАК Организация,
		...
		|	ТаблицаНоменклатуры.СуммаБезНДСВозвратов КАК СуммаБезНДСВозвратов,
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Иначе
			...
			ЦенаПродажиБезНДС = ВыборкаДетальныеЗаписи.СуммаБезНДСПродаж / ВыборкаДетальныеЗаписи.КоличествоПродаж;
			СуммаВозвратов = ЦенаПродажиБезНДС * ВыборкаДетальныеЗаписи.КоличествоВозвратов;
			СуммаКорректировкиБезНДС = ВыборкаДетальныеЗаписи.СуммаБезНДСВозвратов - ЦенаПродажиБезНДС * ВыборкаДетальныеЗаписи.КоличествоВозвратов;
		КонецЕсли;
		...
		Если ОкончательныйРасчет И ТаблицаОстатков <> Неопределено И Окр(СуммаКорректировки, 2) <> 0 Тогда
			...
			СтруктураПоиска.Вставить("ЗаказНаПроизводство");
		...
		Если (НЕ Окр(СуммаКорректировки, 2) = 0) ИЛИ (Окр(СуммаКорректировкиБезНДС, 2) <> 0) Тогда
			
			СуммыКорректировкиСтруктура = Новый Структура("СуммаКорректировки, СуммаКорректировкиБезНДС", СуммаКорректировки, СуммаКорректировкиБезНДС);
...
Процедура ЗакрытьОстаткиБезКоличества()
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЭтапыПроизводства") 
		ИЛИ НЕ ПолучитьФункциональнуюОпцию("ИспользоватьЗакрытиеСуммБезКоличества") Тогда
		Возврат;
	КонецЕсли;
	...
	|	ВЫБОР
	|		КОГДА ЗапасыОстаткиИОбороты.СтруктурнаяЕдиница ССЫЛКА Справочник.СтруктурныеЕдиницы
	|			ТОГДА ВЫРАЗИТЬ(ЗапасыОстаткиИОбороты.СтруктурнаяЕдиница КАК Справочник.СтруктурныеЕдиницы).ТипСтруктурнойЕдиницы
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.ПустаяСсылка)
	...
	|	ЗапасыОстаткиИОбороты.СуммаБезНДСКонечныйОстаток КАК СуммаБезНДС,
	|	ЗапасыОстаткиИОбороты.СуммаКонечныйОстаток КАК Сумма,
	|	ЗапасыОстаткиИОбороты.Номенклатура.СчетУчетаЗатрат КАК СчетУчетаЗатрат,
	|	ЗапасыОстаткиИОбороты.Номенклатура.СчетУчетаЗатрат.ТипСчета КАК СчетУчетаЗатратТипСчета,
	|	ЗапасыОстаткиИОбороты.Номенклатура.НаправлениеДеятельности КАК НаправлениеДеятельности
	|ИЗ
	...
	|	И (ЗапасыОстаткиИОбороты.СуммаКонечныйОстаток <> 0 ИЛИ ЗапасыОстаткиИОбороты.СуммаБезНДСКонечныйОстаток <> 0)";
	...
	НаборЗаписейДоходыИРасходы = РегистрыНакопления.ДоходыИРасходы.СоздатьНаборЗаписей();
	НаборЗаписейДоходыИРасходы.Отбор.Регистратор.Установить(ДополнительныеСвойства.ДляПроведения.Ссылка);
	
	НаборЗаписейУправленческий = РегистрыБухгалтерии.Управленческий.СоздатьНаборЗаписей();
	НаборЗаписейУправленческий.Отбор.Регистратор.Установить(ДополнительныеСвойства.ДляПроведения.Ссылка);
	
	Выборка = Результат.Выбрать();
	Пока Выборка.Следующий() Цикл
		  
		// Движения по регистру Запасы.
		НоваяСтрока = НаборЗаписейЗапасы.Добавить();
		НоваяСтрока.ВидДвижения = ВидДвиженияНакопления.Расход;
		НоваяСтрока.Период = Дата;
		НоваяСтрока.Регистратор = Ссылка;
		...
		НоваяСтрока.СодержаниеПроводки = СодержанияПроводок.ЗакрытиеОстаткаСуммБезКоличества();
		...
		Если Выборка.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение Тогда
			НоваяСтрока.СтруктурнаяЕдиница = Выборка.СтруктурнаяЕдиница;
		Иначе
			НоваяСтрока.СтруктурнаяЕдиница = Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
		КонецЕсли;
		НоваяСтрока.ЗаказПокупателя = Выборка.ЗаказПокупателя;
		НоваяСтрока.НаправлениеДеятельности = Выборка.НаправлениеДеятельности;
		Если Выборка.СчетУчетаЗатратТипСчета = Перечисления.ТипыСчетов.Расходы
			ИЛИ Выборка.СчетУчетаЗатратТипСчета = Перечисления.ТипыСчетов.ПрочиеРасходы Тогда
			НоваяСтрока.СчетУчета = Выборка.СчетУчетаЗатрат;
			НоваяСтрока.СуммаРасходов = Выборка.Сумма;
		...
		Если Выборка.СчетУчетаЗатратТипСчета = Перечисления.ТипыСчетов.Расходы
			ИЛИ Выборка.СчетУчетаЗатратТипСчета = Перечисления.ТипыСчетов.ПрочиеРасходы Тогда
			НоваяСтрока.СчетДт = Выборка.СчетУчетаЗатрат;
		...
		Иначе
			...
			НоваяСтрока.Сумма = - Выборка.Сумма;
		КонецЕсли;
		НоваяСтрока.Содержание = СодержанияПроводок.ЗакрытиеОстаткаСуммБезКоличества();
...
Функция СформироватьТаблицуБазыРаспределения(БазаРаспределения, МассивСчетовУчета, ФильтрПоСтруктурнаяЕдиница, ФильтрПоЗаказ) Экспорт
	...
	Если БазаРаспределения = Перечисления.БазыРаспределенияРасходов.ОбъемВыпуска Тогда
		...
		|	ВыпускПродукцииОбороты.КоличествоОборот КАК БазаБезНДС,
	...
	ИначеЕсли БазаРаспределения = Перечисления.БазыРаспределенияРасходов.ПрямыеЗатраты Тогда
		...
		|	УчетЗатрат.СчетУчета КАК СчетУчета,
		|	УчетЗатрат.СчетУчета.ТипСчета КАК СчетУчетаТипСчета,
		|	УчетЗатрат.СуммаБезНДСКонечныйОстаток КАК БазаБезНДС,
...
Процедура РаспределитьЗатраты(Отказ, ТаблицаОшибок)
	...
	|	УчетЗапасовИЗатрат.ЗаказПокупателя КАК ЗаказПокупателя,
	|	УчетЗапасовИЗатрат.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
	|	УчетЗапасовИЗатрат.СуммаБезНДСОстаток КАК СуммаБезНДС,
		...
		Пока ОбходПоСтруктурнаяЕдиница.Следующий() Цикл
			...
			Пока ОбходПоЗаказ.Следующий() Цикл
				...
				Пока ОбходПоСчетамУчета.Следующий() Цикл
					...
					ВсегоБазаРаспределенияБезНДС = ТаблицаБазы.Итог("БазаБезНДС");
					...
					Если ОбходПоСчетамУчета.Сумма <> 0 Тогда
						...
						СуммаБезНДСКРаспределению = ОбходПоСчетамУчета.СуммаБезНДС;
						СуммаБезНДСРаспределили = 0;
						...
						Для каждого НаправлениеРаспределения Из ТаблицаБазы Цикл
							...
							СуммаБезНДСЗатрат = ?(СуммаБезНДСКРаспределению = 0 Или ВсегоБазаРаспределенияБезНДС = 0,
							0,
							Окр(НаправлениеРаспределения.База / ВсегоБазаРаспределенияБезНДС * СуммаБезНДСКРаспределению, 2, 1));
							
							СуммаБезНДСРаспределили = СуммаБезНДСРаспределили + СуммаБезНДСЗатрат;
							...
							Если ТаблицаБазы.Индекс(НаправлениеРаспределения) = КоличествоНаправлений Тогда
								...
								СуммаРаспределили = СуммаКРаспределению;
								
								СуммаБезНДСЗатрат = СуммаБезНДСЗатрат + СуммаБезНДСКРаспределению - СуммаБезНДСРаспределили;
								СуммаБезНДСРаспределили = СуммаБезНДСКРаспределению;
							КонецЕсли;
							
							СуммыКорректировкиСтруктура = Новый Структура();
							СуммыКорректировкиСтруктура.Вставить("СуммаКорректировки", СуммаЗатрат);
							СуммыКорректировкиСтруктура.Вставить("СуммаКорректировкиБезНДС", СуммаБезНДСЗатрат);
...
Процедура РассчитатьФинансовыйРезультат(Отказ, ТаблицаОшибок)
	...
	ОсновноеНаправление = Справочники.НаправленияДеятельности.ОсновноеНаправление;
	СчетУчетаПрибылиПоОсновномуНаправлению = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновноеНаправление, "СчетУчетаПрибыли");
	...
	Пока ВыборкаРезультатЗапроса.Следующий() Цикл
		...
		НоваяСтрока.НаправлениеДеятельности = ?(ЗначениеЗаполнено(ВыборкаРезультатЗапроса.НаправлениеДеятельности), 
			ВыборкаРезультатЗапроса.НаправлениеДеятельности, ОсновноеНаправление);
		...
		Если ВыборкаРезультатЗапроса.СуммаДоходов <> 0 Тогда
			...
			НоваяСтрока.СчетКт = ?(
				...
				СчетУчетаПрибылиПоОсновномуНаправлению);
					...
					Пока ОбходПоСчетамУчета.Следующий() Цикл
						...
						Если ТаблицаБазы.Количество() = 0
						 ИЛИ ВсегоБазаРаспределения = 0 Тогда
							...
							СтрокаТаблицы.НаправлениеДеятельности = ОсновноеНаправление;
							...
							СтрокаТаблицы.СчетУчетаПрибыли = СчетУчетаПрибылиПоОсновномуНаправлению;
						 ...
						 ИЛИ ОбходПоСчетамУчета.СуммаРасходов <> 0 Тогда
							...
							Для каждого НаправлениеРаспределения Из ТаблицаБазы Цикл
								...
								Если СуммаЗатрат <> 0 Тогда
									...
									НоваяСтрока.СодержаниеПроводки = СодержанияПроводок.ФинансовыйРезультат();
									...
									НоваяСтрока.Содержание = СодержанияПроводок.ФинансовыйРезультат();
...
Процедура РассчитатьКурсовыеРазницы(Отказ, ТаблицаОшибок)
	...
	// Создадим набор записей регистра накопления ДвиженияДенежныхСредств.
	НаборЗаписейДвиженияДенежныхСредств = РегистрыНакопления.ДвиженияДенежныхСредств.СоздатьНаборЗаписей();
	НаборЗаписейДвиженияДенежныхСредств.Отбор.Регистратор.Установить(Ссылка);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НоваяСтрока = НаборЗаписейДвиженияДенежныхСредств.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаДетальныеЗаписи);
		НоваяСтрока.СценарийПланирования = Справочники.СценарииПланирования.Фактический;
		НоваяСтрока.ХозяйственнаяОперация = ХозяйственнаяОперация;
		Если ВыборкаДетальныеЗаписи.ВидДвиженияУправленческий = ВидДвиженияНакопления.Приход Тогда
			НоваяСтрока.СуммаПриход = ВыборкаДетальныеЗаписи.Сумма;
		Иначе
			НоваяСтрока.СуммаРасход = -ВыборкаДетальныеЗаписи.Сумма;
		КонецЕсли;
	...
	НаборЗаписейДвиженияДенежныхСредств.Записать(Ложь);
...
Процедура ДобавитьРеквизитыВДополнительныеСвойстваДляПроведения(СтруктураДополнительныеСвойства)
	...
	КонечнаяДатаПериода = КонецМесяца(Дата);
	...
	СтруктураДополнительныеСвойства.ДляПроведения.Вставить("КонечнаяДатаПериода", КонечнаяДатаПериода);
...
Процедура СвернутьНаборЗаписейЗапасы(НаборЗаписейРегистра)
	...
	|		ИЛИ СУММА(Запасы.СуммаБезНДС) <> 0)";
...
Процедура СвернутьНаборЗаписейПродажи(НаборЗаписейРегистра)
	...
	|	СУММА(Продажи.СебестоимостьИнт) КАК СебестоимостьИнт,
	|	СУММА(Продажи.СебестоимостьБезНДСИнт) КАК СебестоимостьБезНДСИнт
	|ИЗ
	...
	|		ИЛИ СУММА(Продажи.СебестоимостьБезНДС) <> 0
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если СоответствиеРегистровБлокировки.Количество() > 0 Тогда
		...
		Диапазон = Новый Диапазон(ДополнительныеСвойства.ДляПроведения.НачальнаяДатаПериода, ДополнительныеСвойства.ДляПроведения.КонечнаяДатаПериода);
		Для каждого РегистрБлокировки Из СоответствиеРегистровБлокировки Цикл
			...
			Если 	РегистрБлокировки.Ключ = "РегистрНакопления.Запасы"
				ИЛИ РегистрБлокировки.Ключ = "РегистрНакопления.Продажи"
				ИЛИ РегистрБлокировки.Ключ = "РегистрНакопления.ДенежныеСредстваВКассахККМ" Тогда
				ЭлементБлокировки.УстановитьЗначение("ДвижениеЧекаВОткрытойСмене", Ложь);
			КонецЕсли;
		КонецЦикла;
	...
	Если РасчетПрямыхЗатрат Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьРаспределениеЗатрат") Тогда
	...
	Если ЗачетАвансаПоЕдиномуНалоговомуСчету Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Проведение документа: %1 (%2)'"), СокрЛП(ЭтотОбъект), "Зачет аванса по единому налоговому счету");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ХозяйственнаяОперация = ХозяйственныеОперацииСервер.ХозяйственнаяОперацияПоВидуОперацииДокумента(Неопределено,
		Ссылка);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Запрос.УстановитьПараметр("ЗачетАвансаПоЕдиномуНалоговомуСчету", ЗачетАвансаПоЕдиномуНалоговомуСчету); 
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	ЗакрытиеМесяца.ЗачетАвансаПоЕдиномуНалоговомуСчету КАК ЗачетАвансаПоЕдиномуНалоговомуСчету
	|ИЗ
	...
	|			ИЛИ ЗакрытиеМесяца.ЗачетАвансаПоЕдиномуНалоговомуСчету = &ЗачетАвансаПоЕдиномуНалоговомуСчету
	|				И ЗакрытиеМесяца.ЗачетАвансаПоЕдиномуНалоговомуСчету)";
	Выборка = Запрос.Выполнить().Выбрать();
	МассивНастроек = Новый Массив;
	
	ПоляНастроек = Новый Структура;
	ПоляНастроек.Вставить("РасчетПрямыхЗатрат");
	ПоляНастроек.Вставить("РаспределениеЗатрат");
	ПоляНастроек.Вставить("РасчетФактическойСебестоимости");
	ПоляНастроек.Вставить("РасчетФинансовогоРезультата");
	ПоляНастроек.Вставить("РасчетКурсовыхРазниц");
	ПоляНастроек.Вставить("РасчетСебестоимостиВРозницеСуммовойУчет");  
	ПоляНастроек.Вставить("ЗачетАвансаПоЕдиномуНалоговомуСчету");  
...
Функция ЗаполнитьСтруктуруШапкиДокумента(Отказ, Заголовок) Экспорт
	...
	Отказ               = Не ПрослеживаемостьПереопределяемый.УчетнаяПолитикаСуществует(ГоловнаяОрганизация, Дата);
	...
	СтруктураШапки.Вставить("Предприниматель",         Истина);
	
	ВсяОрганизация = ПрослеживаемостьПереопределяемый.ВсяОрганизация(Организация);
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Если ЗачетАвансаПоЕдиномуНалоговомуСчету Тогда
		ОтменитьФормированиеЗаписейУСНВПериодеПоВиду(
		Организация,
		Дата,
		Перечисления.ВидыЗаписейФормированийКУДиР.РасходыЕНП, ЭтотОбъект.Ссылка); 		
	КонецЕсли;
	
КонецПроцедуры

Функция ОтменитьФормированиеЗаписейУСНВПериодеПоВиду(Организация, ПериодФормирования
	...
	|ГДЕ
	|	ЗаписиУСН.Дата МЕЖДУ &НачалоМесяца И &КонецМесяца
	...
	Запрос.УстановитьПараметр("НачалоМесяца", НачалоМесяца(ПериодФормирования));
	Запрос.УстановитьПараметр("КонецМесяца", КонецМесяца(ПериодФормирования)); 
...
#Область ЗачетАвансаПоЕдиномуНалоговомуСчету
Процедура ЗачетАвансаПоЕдиномуНалоговомуСчету(СтруктураШапкиДокумента, Отказ)
	
	// ПОДГОТОВКА ПРОВЕДЕНИЯ ПО ДАННЫМ ДОКУМЕНТА
	
	ПараметрыПроведения = ПодготовитьПараметрыЗачетАвансаПоЕдиномуНалоговомуСчету(
...
Функция ТекстЗапросаЗачетАвансаПоЕдиномуНалоговомуСчету(НомераТаблиц)
	...
	|	КОНЕЦПЕРИОДА(ЗакрытиеМесяца.Дата, МЕСЯЦ) КАК Период,
	|	ИСТИНА КАК ВыдаватьСообщения,
	|	ЗакрытиеМесяца.ЗачетАвансаПоЕдиномуНалоговомуСчету КАК ВидОперации
	|ИЗ
	|	Документ.ЗакрытиеМесяца КАК ЗакрытиеМесяца
	|ГДЕ
