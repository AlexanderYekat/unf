...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства)
	...
	|ГДЕ
	|	ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПрочегоКонтрагента)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок,
	|	НомерСтроки";
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Ссылка.ПризнакАванса
	|			ТОГДА ТаблицаДокумента.Ссылка.КонтрагентИсточник.СчетУчетаАвансовПоставщику
	...
	|	&КорректировкаДолга
	|ИЗ
	|	Документ.Взаимозачет.ПрочиеРасчеты КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка
	|	И ТаблицаДокумента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПрочегоКонтрагента)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок,
	|	НомерСтроки";
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаЗакупкиПоставщиковДляКУДиР(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства) Экспорт
	...
	|			ИЛИ ДопРеквизитыДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачет.ПрочийВзаимозачет)
	|				И ДопРеквизитыДокумента.ВидПрочегоВзаимозачета = ЗНАЧЕНИЕ(Перечисление.ВидыПрочегоВзаимозачета.ДолгПрочегоКонтрагентаИДолгПередПоставщиком))
	...
	|			ИЛИ НЕ ВременнаяТаблицаРасчетыСПоставщиками.ПризнакАванса
	|				И ДопРеквизитыДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВзаимозачет.Взаимозачет))
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("КомментарийПрочий", НСтр("ru='Корректировка долга прочего контрагента'"));
	...
	|				ТОГДА 0
	|			ИНАЧЕ ТаблицаДокумента.СуммаУчета * ТаблицаДокумента.Ссылка.СуммаРасчетов / ТаблицаДокумента.Ссылка.СуммаУчета
	|		КОНЕЦ),
	|	СУММА(ТаблицаДокумента.СуммаУчета),
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Ссылка.ДокументРасчетов = НЕОПРЕДЕЛЕНО
	...
	|	ВременнаяТаблицаРасшифровкаПлатежа.Ссылка КАК Ссылка,
	|	ВременнаяТаблицаРасшифровкаПлатежа.Ссылка.ВидОперации КАК ВидОперации,
	|	&Организация КАК Организация,
	...
	|	ВременнаяТаблицаРасшифровкаПлатежа.Ссылка.ВидОперации,
	...
	|	ТаблицаДокумента.СуммаУчета КАК СуммаДляОстатка,
	|	ТаблицаДокумента.СуммаРасчетов КАК СуммаВалДляОстатка,
	|	&УчетРасчетовПоПрочимОперациям КАК СодержаниеПроводки,
	|	&КомментарийПрочий КАК Комментарий,
	|	ТаблицаДокумента.СчетУчета КАК СчетУчета,
	|	ТаблицаДокумента.Ссылка.Дата КАК Период
	|ИЗ
	|	Документ.Взаимозачет.ПрочиеРасчеты КАК ТаблицаДокумента
	...
	Если СтруктураВременныеТаблицы.ДвиженияРасчетыСПокупателямиИзменение
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияРасчетыСПоставщикамиИзменение Тогда
		...
		Если НЕ МассивРезультатов[1].Пустой() Тогда
			...
			Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
				Контрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаВзаимозачет, "Контрагент");
				КонтрагентИсточник = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаВзаимозачет, "КонтрагентИсточник");
				Если Контрагент = КонтрагентИсточник Тогда
					Прервать; // Для переноса задолженности по аналитике внутри одного контрагента контроль не нужен
				КонецЕсли;
				Если ВыборкаИзРезультатаЗапроса.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
					ТекстСообщения = НСтр(
				...
				ТекстСообщенияОбщий = НСтр("ru = 'Ошибка:
									  |Нет возможности зафиксировать расчеты с поставщиками'");
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщенияОбщий, , , , Отказ);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктВзаимозачета") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АктВзаимозачета", НСтр("ru = 'Акт взаимозачета'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "АктВзаимозачета"));
