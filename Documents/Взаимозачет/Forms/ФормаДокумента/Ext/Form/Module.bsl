...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОтображатьСчетаУчета Тогда
		Элементы.ТипКорректировки.СписокВыбора.Добавить(Истина, НСтр("ru = 'Увеличение долга'"));
		Элементы.ТипКорректировки.СписокВыбора.Добавить(Ложь, НСтр("ru = 'Уменьшение долга'"));
	КонецЕсли;
...
Процедура КонтрагентПолучательПриИзменении(Элемент)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.ПереуступкаДолгаПокупателя")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.ПереуступкаДолгаПоставщику")
		Тогда	
...
&НаКлиенте
Процедура ДекорацияПодписьРуководителяНажатие(Элемент)
...
Процедура ДоговорПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Договор) Тогда
		Объект.Курс = ?(ДанныеДоговора.ВалютаКурсКратность.Курс = 0, 1, ДанныеДоговора.ВалютаКурсКратность.Курс);
		Объект.Кратность = ?(ДанныеДоговора.ВалютаКурсКратность.Кратность = 0, 1,
			ДанныеДоговора.ВалютаКурсКратность.Кратность);
	КонецЕсли;
...
&НаКлиенте
Процедура ТипКорректировкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение Тогда
		ТипКорректировки = НСтр("ru = 'Увеличение долга'");
		Если Объект.ВидОперации = ВидОперацииКорректировкаДолгаПокупателя Тогда
			Объект.Корреспонденция = ПредопределенноеЗначение("ПланСчетов.Управленческий.ПрочиеДоходы");
		Иначе
			Объект.Корреспонденция = ПредопределенноеЗначение("ПланСчетов.Управленческий.ПрочиеРасходы");
		КонецЕсли;
	Иначе
		ТипКорректировки = НСтр("ru = 'Уменьшение долга'");
		Если Объект.ВидОперации = ВидОперацииКорректировкаДолгаПокупателя Тогда
			Объект.Корреспонденция = ПредопределенноеЗначение("ПланСчетов.Управленческий.ПрочиеРасходы");
		Иначе
			Объект.Корреспонденция = ПредопределенноеЗначение("ПланСчетов.Управленческий.ПрочиеДоходы");
...
Процедура ДебиторДоговорПриИзменении(Элемент)
	...
	ДанныеДоговора = ДанныеДоговораДляИзменения(ДатаДокумента, СтрокаТабличнойЧасти.Договор);
	
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Договор) Тогда
		СтрокаТабличнойЧасти.Курс = ?(ДанныеДоговора.ВалютаКурсКратность.Курс = 0, 1,
			ДанныеДоговора.ВалютаКурсКратность.Курс);
		СтрокаТабличнойЧасти.Кратность = ?(ДанныеДоговора.ВалютаКурсКратность.Кратность = 0, 1,
			ДанныеДоговора.ВалютаКурсКратность.Кратность);
	КонецЕсли;
...
Процедура ДебиторДокументПриИзменении(Элемент)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.ЗачетАвансовПоставщику")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.ЗачетАвансовПокупателя") Тогда
...
#Область ОбработчикиСобытийЭлементовТаблицыПрочиеРасчеты

&НаКлиенте
Процедура ПрочиеРасчетыДоговорПриИзменении(Элемент)
	
	СтрокаТабличнойЧасти = Элементы.ПрочиеРасчеты.ТекущиеДанные;
...
&НаКлиенте
Процедура ПрочиеРасчетыСуммаРасчетовПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПрочиеРасчетыКурсПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПрочиеРасчетыКратностьПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПрочиеРасчетыСуммаУчетаПриИзменении(Элемент)
...
Процедура ПодборРасчетовСПрочимиКонтрагентами(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ТекКонтрагент) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для использования подбора необходимо заполнить контрагента'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПрочегоКонтрагента") Тогда
		АдресТЧВХранилище = ПоместитьПрочиеРасчетыВХранилище();
	Иначе
	...
	ПараметрыПодбора.Вставить("Ссылка", Объект.Ссылка);
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПрочегоКонтрагента") Тогда
		ПараметрыПодбора.Вставить("СчетУчета", Неопределено);
	Иначе
...
Процедура ПодборРасчетовСПрочимиКонтрагентамиЗавершение(Результат1, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.OK Тогда
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПрочегоКонтрагента") Тогда
			ПолучитьПрочиеРасчетыИзХранилища(АдресДебиторВХранилище);
		Иначе
			Если Объект.ВидПрочегоВзаимозачета = ПредопределенноеЗначение("Перечисление.ВидыПрочегоВзаимозачета.ДолгПокупателяИДолгПрочемуКонтрагенту") Тогда
...
Процедура НастроитьПодсказкуДляКорреспонденции(Форма)
	...
	Если Форма.Объект.ВидОперации = Форма.ВидОперацииКорректировкаДолгаПокупателя Тогда
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли Форма.Объект.ВидОперации = Форма.ВидОперацииКорректировкаДолгаПоставщику
		ИЛИ Форма.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПрочегоКонтрагента") Тогда
		Если Форма.Объект.Корреспонденция.Пустая() Тогда
			Форма.Элементы.Корреспонденция.ОтображениеПодсказки = ОтображениеПодсказки.Авто;
			Форма.Элементы.Корреспонденция.МаксимальнаяШирина = СтандартнаяМаксимальнаяШирина;
		Иначе
...
&НаСервере
Функция ПоместитьПрочиеРасчетыВХранилище()
	
	Возврат ПоместитьВоВременноеХранилище(
		Объект.ПрочиеРасчеты.Выгрузить(,
			"Договор,
			|СуммаРасчетов,
			|СуммаУчета,
			|Курс,
...
&НаСервере
Процедура ПолучитьПрочиеРасчетыИзХранилища(АдресПрочиеРасчетыВХранилище)
	
	ТаблицаПрочиеРасчеты = ПолучитьИзВременногоХранилища(АдресПрочиеРасчетыВХранилище);
	Объект.ПрочиеРасчеты.Очистить();
	Для каждого СтрокаПрочиеРасчеты Из ТаблицаПрочиеРасчеты Цикл
		Строка = Объект.ПрочиеРасчеты.Добавить();
		ЗаполнитьЗначенияСвойств(Строка, СтрокаПрочиеРасчеты);
	КонецЦикла;
...
Функция ДанныеДоговораДляИзменения(Знач Дата, Знач Договор)
	...
	Результат.Вставить("ВалютаКурсКратность", РаботаСКурсамиВалют.ПолучитьКурсВалюты(Договор.ВалютаРасчетов, Дата));
	Возврат Результат;
...
Процедура УстановитьПараметрыВыбора()
	...
	Если ПолучитьФункциональнуюОпцию("ПередачаТоваровМеждуОрганизациями") Тогда
		...
		Элементы.КредиторДоговор.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый Массив);	
		ПараметрыВыбораДоговора = Новый Массив;
		Значения = Новый Массив;
		Значения.Добавить(Объект.Контрагент);
		Значения.Добавить(Объект.Организация);
		Параметр = Новый ПараметрВыбора("Отбор.Владелец", Новый ФиксированныйМассив(Значения));
		ПараметрыВыбораДоговора.Добавить(Параметр);
		Элементы.КредиторДоговор.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораДоговора);
...
Процедура УстановитьСвязиПараметровВыбора()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.Взаимозачет") Тогда
		...
		НовыйМассив.Добавить(НоваяСвязь);
		Если ПолучитьФункциональнуюОпцию("ПередачаТоваровМеждуОрганизациями") Тогда
			НоваяСвязь = Новый СвязьПараметраВыбора("Отбор.ДоговорПередачи", "Элементы.Кредитор.ТекущиеДанные.Договор");
			НовыйМассив.Добавить(НоваяСвязь);
		КонецЕсли;
		НовыеСвязи = Новый ФиксированныйМассив(НовыйМассив);
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.ТипКорректировки.Видимость = Ложь;
	
	Элементы.КурсКратность.Видимость = ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций");
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.Взаимозачет") Тогда
		...
		Элементы.ИтогиСуммаПрочийКонтрагент.Видимость = Ложь;
		...
		Элементы.РасчетыСПрочимКонтрагентом.Видимость = Ложь;
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПокупателя") Тогда
		...
		Если НЕ ОтображатьСчетаУчета Тогда
			Элементы.ТипКорректировки.Видимость = Истина;
			Элементы.Корреспонденция.Видимость = Ложь;
			Если Объект.Корреспонденция = ПредопределенноеЗначение("ПланСчетов.Управленческий.ПрочиеДоходы") Тогда
				ТипКорректировки = НСтр("ru = 'Увеличение долга'");
			Иначе
				ТипКорректировки = НСтр("ru = 'Уменьшение долга'");
			КонецЕсли;
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПоставщику") Тогда
		...
		Если НЕ ОтображатьСчетаУчета Тогда
			Элементы.ТипКорректировки.Видимость = Истина;
			Элементы.Корреспонденция.Видимость = Ложь;
			Если Объект.Корреспонденция = ПредопределенноеЗначение("ПланСчетов.Управленческий.ПрочиеРасходы") Тогда
				ТипКорректировки = НСтр("ru = 'Увеличение долга'");
			Иначе
				ТипКорректировки = НСтр("ru = 'Уменьшение долга'");
			КонецЕсли;
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПрочегоКонтрагента") Тогда
		
		// Подвал
		Элементы.ИтогиСуммаДебитор.Видимость = Ложь;
		Элементы.ИтогиСуммаКредитор.Видимость = Ложь;
		Элементы.ИтогиСуммаПрочийКонтрагент.Видимость = Истина;
		// Конец Подвал
		
		Элементы.Получатель.Видимость = Ложь;
		Элементы.РасчетыСДебитором.Видимость = Ложь;
		Элементы.РасчетыСКредитором.Видимость = Ложь;
		Элементы.РасчетыСПрочимКонтрагентом.Видимость = Истина;
		Элементы.Корреспонденция.Видимость = Истина;
		Элементы.Контрагент.Видимость = Ложь;
		Элементы.КонтрагентПолучатель.Видимость = Истина;
		Элементы.КонтрагентПолучатель.Заголовок = НСтр("ru = 'Прочий контрагент'");
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.ЗачетАвансовПокупателя")
		...
		Если Объект.ВидОперации = ПредопределенноеЗначение(
			"Перечисление.ВидыОперацийВзаимозачет.ЗачетАвансовПокупателя") Тогда
			ОкончаниеАвансы = НСтр("ru = 'от покупателя'");
		Иначе
			ОкончаниеАвансы = НСтр("ru = 'поставщику'");
		КонецЕсли;
		
		Если Объект.ВидОперации = ПредопределенноеЗначение(
			"Перечисление.ВидыОперацийВзаимозачет.ЗачетАвансовПокупателя") Тогда
			ОкончаниеДолги = НСтр("ru = 'покупателя'");
		Иначе
			ОкончаниеДолги = НСтр("ru = 'перед поставщиком'");
		КонецЕсли;
		
		Элементы.ИтогиСуммаДебитор.Заголовок = СтрШаблон(НСтр("ru = 'Авансы %1'"), ОкончаниеАвансы);
		Элементы.ИтогиСуммаКредитор.Заголовок = СтрШаблон(НСтр("ru = 'Долги %1'"), ОкончаниеДолги);
		// Конец Подвал
...
Процедура УстановитьДоступныеТипы()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.ПереуступкаДолгаПокупателя")
		...
		Массив.Добавить(Тип("ДокументСсылка.ЧекККМВозврат"));
		...
		КонецЕсли;
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.КорректировкаДолгаПокупателя") Тогда
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.ЗачетАвансовПокупателя") Тогда
		...
		МассивДебитор.Добавить(Тип("ДокументСсылка.КорректировкаРеализации"));
		МассивДебитор.Добавить(Тип("ДокументСсылка.ПриходнаяНакладная"));
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВзаимозачет.ЗачетАвансовПоставщику") Тогда
		...
		МассивДебитор = Новый Массив();
		МассивДебитор.Добавить(Тип("ДокументСсылка.АвансовыйОтчет"));
		МассивДебитор.Добавить(Тип("ДокументСсылка.РасходИзКассы"));
		МассивДебитор.Добавить(Тип("ДокументСсылка.Взаимозачет"));
		МассивДебитор.Добавить(Тип("ДокументСсылка.РасходСоСчета"));
		МассивДебитор.Добавить(Тип("ДокументСсылка.КорректировкаПоступления"));
		МассивДебитор.Добавить(Тип("ДокументСсылка.РасходнаяНакладная"));
...
КонецПроцедуры // УстановитьДоступныеТипы()
...
Функция ПолучитьПараметрыФормыВыбора(Документ, Организация, Контрагент, Договор, ВидОперации, ИмяТабличнойЧасти = "")
	...
	Если ПолучитьФункциональнуюОпцию("ПередачаТоваровМеждуОрганизациями")
		И (ВидОперации = Перечисления.ВидыОперацийВзаимозачет.ПрочийВзаимозачет
		ИЛИ ВидОперации = Перечисления.ВидыОперацийВзаимозачет.Взаимозачет
		ИЛИ ВидОперации = Перечисления.ВидыОперацийВзаимозачет.КорректировкаДолгаПокупателя
		ИЛИ ВидОперации = Перечисления.ВидыОперацийВзаимозачет.КорректировкаДолгаПоставщику) Тогда
		Если ИмяТабличнойЧасти = "Кредитор" Тогда
			ПараметрыФормы.Вставить("ЭтоВзаимозачетСПоставщиком", Истина);
		ИначеЕсли ИмяТабличнойЧасти = "Дебитор" Тогда
			ПараметрыФормы.Вставить("ЭтоВзаимозачетСПокупателем", Истина);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьДанныеКонтрагентПриИзменении(СсылкаНаДокумент, Контрагент, Организация);
	...
	ДоговорПоУмолчанию = Справочники.ДоговорыКонтрагентов.ПоДокументуКонтрагентуОрганизацииИВидуОперации(
		СсылкаНаДокумент, Контрагент, Организация);
	ДанныеКонтрагента.Вставить("Договор", ДоговорПоУмолчанию);
...
Процедура ОпределитьВидимостьНастроекУчетаВНалогообложении()
	...
	Если Не ПлательщикУСН Тогда
		Элементы.ГруппаУчетВНУ.Видимость = Ложь;
	Иначе
		ПоказыватьУчетВНУ = Объект.ВидОперации = Перечисления.ВидыОперацийВзаимозачет.КорректировкаДолгаПоставщику
			Или Объект.ВидОперации = Перечисления.ВидыОперацийВзаимозачет.Взаимозачет
			Или Объект.ВидОперации = Перечисления.ВидыОперацийВзаимозачет.ПрочийВзаимозачет
			И Объект.ВидПрочегоВзаимозачета = Перечисления.ВидыПрочегоВзаимозачета.ДолгПрочегоКонтрагентаИДолгПередПоставщиком;
		Если ПоказыватьУчетВНУ Тогда
