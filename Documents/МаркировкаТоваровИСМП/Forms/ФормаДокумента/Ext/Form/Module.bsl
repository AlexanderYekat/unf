...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Элементы.НомерПартии.Видимость = ЭтоКреслаКоляски И (ЭтоОперацияНанесения Или ЗначениеЗаполнено(ОперацияНанесения));
	Элементы.НомерСерии.Видимость  = Элементы.НомерПартии.Видимость;
...
Процедура ЗаполнитьПредставлениеСертификацииЛицензииНаНедра()
	...
	Если Объект.ОтчетПроизводственнойЛинии Тогда
		...
		Если ЗначениеЗаполнено(Объект.ВидДокументаСертификации) Тогда
			...
			ДанныеЗаполненияСертификации.НомерСкважины     = Объект.НомерСкважины;
		...
		Если ЗначениеЗаполнено(Объект.НомерЛицензииНаПользованиеНедрами) Тогда
			...
			ЛицензияНаПользованиеНедрами = ИнтеграцияИСМПКлиентСервер.ПредставлениеЛицензииНаНедра(ДанныеЗаполненияЛицензииНаНедра);
	...
	Иначе
		...
		Для Каждого Строка Из Объект.Товары Цикл
			...
			ЗаполнитьПредставлениеСертификации(Строка, ИспользоватьНомерСкважины);
			Если ИспользоватьНомерСкважины Тогда
				ЗаполнитьПредставлениеЛицензии(Строка);
			КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ЗаполнитьПредставлениеСертификации(ДанныеСтроки, ИспользоватьНомерСкважины)
...
&НаСервереБезКонтекста
Процедура ЗаполнитьПредставлениеЛицензии(ДанныеСтроки)
	...
	ДанныеСтроки.ЛицензияНаПользованиеНедрами = ИнтеграцияИСМПКлиентСервер.ПредставлениеЛицензииНаНедра(ДанныеЗаполненияЛицензииНаНедра);
...
Процедура ЗаполнитьДоступныеОперации()
	...
	Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(Объект.ВидПродукции, Ложь, Истина, Ложь) Тогда
		...
		Если (ИспользуетсяПодсистемаВЕТИС Или Не ЭтоПродукцияПодконтрольнаяВЕТИС) И Не ЭтоМех Тогда
			Элементы.Операция.СписокВыбора.Добавить(
	...
	Для Каждого ДанныеШтрихкода Из ШтрихКоды Цикл
		...
		Если ЗначениеЗаполнено(EAN) Тогда
			...
			Если НайденныеСтроки <> Неопределено 
				...
				Если НайденныеСтроки[0].ВидПродукции <> Объект.ВидПродукции Тогда
					ТекстСообщения = СтрШаблон(
						...
						НайденныеСтроки[0].ВидПродукции,
						Объект.ВидПродукции);
				...
				ИначеЕсли (Номенклатура <> НайденныеСтроки[0].Номенклатура
...
Процедура ОбработатьПолученныеДанныеТСД()
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Штрихкод.Штрихкод) Тогда
			...
			ОбработатьКодМаркировки(Штрихкод);
