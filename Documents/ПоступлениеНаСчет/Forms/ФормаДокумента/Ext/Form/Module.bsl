...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) И Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтПокупателя") Тогда
	...
	НаборСвойств_Документ_ПоступлениеНаСчет = УправлениеСвойствами.НаборСвойствПоИмени("Документ_ПоступлениеНаСчет");
	КэшЗначений.Вставить("НаборСвойств_Документ_ПоступлениеНаСчет", НаборСвойств_Документ_ПоступлениеНаСчет);
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам") Тогда
		...
		Для Каждого ТекущаяСтрока Из Объект.ЭквайринговыеОперации Цикл
			ПересчитатьСуммуПлатежаИКомиссииЭквайрингаВСтроке(ТекущаяСтрока);
		КонецЦикла;
	...
	ФормыДокументовДеньги.ОпределитьВидимостьНастроекУчетаВНалогообложении(ЭтотОбъект, НастройкиНУ);
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам") 
		И СтруктураДанные.Свойство("ДоговорЭквайринга") Тогда
		ДоговорЭквайринга = СтруктураДанные.ДоговорЭквайринга;
...
&НаКлиенте
Процедура ПродажаВалютыПересчитатьСумму()
	...
	Если Объект.РасшифровкаПлатежа.Количество() = 1 Тогда
		...
		СтрокаТабличнойЧасти.СуммаПлатежа = Объект.СуммаДокумента;
		СтрокаТабличнойЧасти.СуммаРасчетов = Объект.СуммаДокумента;
...
Процедура Подбор(Команда)
	...
	ПараметрыПодбора.Вставить("ЭквайринговыйТерминал", Объект.ЭквайринговыйТерминал);
...
Процедура ЗаполнитьПоОснованиюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		// Возможно, что будет прерывание при заполнении на основании приема и передачи в ремонт и тогда банковский счет
		// останется пустым.
...
Процедура ЗаполнитьПоОснованиюЗавершениеНаСервере()
	...
	Если НалогообложениеЗаполненоВОбработкеЗаполнения Тогда
		Налогообложение = НалогиУНФ.НалогообложениеПоОснованию(Объект.Организация, Объект.Дата, Объект.ДокументОснование, КэшЗначений.НастройкиНалогообложения);
		Объект.УчитыватьВНУ = Налогообложение.УчитыватьВНУ;
		Объект.Патент = Налогообложение.Патент;
	КонецЕсли;
	ЗаполнитьНалоговыйУчетСУчетомРасшифровки();
...
Процедура НапечататьЧек(Команда)
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		ОтборОборудования = МенеджерОборудованияКлиентСервер.СписокОборудованияОтбор();
		ОтборОборудования.Вставить("ТипыПО", ЧекопечатающееОборудование());
		ОтборОборудования.Вставить("Организация", Объект.Организация);
		МенеджерОборудованияКлиент.ВыбратьУстройство(ОписаниеОповещения, ОтборОборудования,
				НСтр("ru='Выберите оборудование'"), НСтр("ru='Оборудование не подключено.'"));
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", КэшЗначений.НаборСвойств_Документ_ПоступлениеНаСчет);
...
Процедура ЗаполнитьПоДоговоруКредитаНаСервере()
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.СуммаДокумента < Выборка.ОсновнойДолгВалРасход Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'По договору кредита уже выдано %1 (%2).'"),
				Выборка.ОсновнойДолгВалРасход, Выборка.ВалютаРасчетов);
		ИначеЕсли Выборка.СуммаДокумента = Выборка.ОсновнойДолгВалРасход Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'По договору кредита уже выдана вся сумма: %1 (%2).'"),
				Выборка.ОсновнойДолгВалРасход, Выборка.ВалютаРасчетов);
		Иначе
...
Процедура ЗаполнитьПоДоговоруЗайма(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.ФормаЗаполненияРасшифровкиПлатежаПоКредитамИЗаймам", ПараметрыОтбора, ЭтотОбъект, , , ,
		Новый ОписаниеОповещения("ЗаполнитьПоДоговоруЗаймаЗавершение", ЭтотОбъект));
...
Процедура ПолучитьРасшифровкаПлатежаИзХранилища(АдресРасшифровкаПлатежаВХранилище, Очищать = Истина)
	...
	ЗаполнятьНУ = ФормыДокументовДеньгиКлиентСервер.ВидОперацииРасшифровкаСНалоговымУчетом(Объект.ВидОперации)
...
Процедура ПересчитатьСуммыДокумента(Курс, Кратность, ПересчитатьСуммуПлатежа, ДополнительныеПараметры)
	...
	Если КэшЗначений.Свойство("СуммаПродажи",СуммаПродажи) Тогда
		Если Объект.РасшифровкаПлатежа.Количество() = 1 Тогда
			...
			ДвиженияДенежныхСредствКлиентСервер.ЗаполнитьПризнакСпособаРасчета(Объект,, СуммаПродажи, ЧекПробит);
		Иначе
...
Функция ПолучитьДанныеКонтрагентПриИзменении(Контрагент, Организация, Дата)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам Тогда
		Результат.Вставить("ДоговорЭквайринга", ЭквайринговыеОперацииСервер.ПолучитьДоговорЭквайринга(Контрагент, Организация));
	КонецЕсли;
...
Процедура ОрганизацияПриИзмененииНаСервере(ДанныеВалюты)
	...
	ВалютаДенежныхСредств = Объект.Организация.БанковскийСчетПоУмолчанию.ВалютаДенежныхСредств;
	...
	УстановитьВидимостьКурсаВалюты(Объект.БанковскийСчет.ВалютаДенежныхСредств);
	УстановитьНастройкиУчетаВНалогообложении(Ложь);
	
	// Видимость итогов НДС в зависимости от настроек налогообложения организации
	Элементы.СуммаНДС.Видимость = Объект.НалогообложениеНДС = ПредопределенноеЗначение(
		"Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС");
	...
	Если Объект.РасшифровкаПлатежа.Количество() > 0 Тогда
		Для Каждого ТекущаяСтрока Из Объект.РасшифровкаПлатежа Цикл
			Если ВестиРасчетыПоДоговорам И НЕ УчетПоКомпании Тогда
				ТекущаяСтрока.Договор = Неопределено;
			ИначеЕсли ТекущаяСтрока.Договор.Пустая() Тогда
				ТекущаяСтрока.Договор = Справочники.ДоговорыКонтрагентов.ПоДокументуКонтрагентуОрганизацииИВидуОперации(
...
Процедура ВидОперацииПриИзмененииНаСервере(ЭтоЗаполнениеПоОснованию = Ложь)
	...
	ФормыДокументовДеньги.ВидОперацииПриИзмененииНаСервере(ЭтотОбъект);
...
Процедура ОбработатьИзменениеДоговораКонтрагента(СтрокаТабличнойЧасти = Неопределено)
	...
	Иначе
		СтрокаТабличнойЧасти.СтатьяДДС = РасчетыРаботаСФормамиВызовСервера.ПолучитьСтатьюДДСПоУмолчаниюДляСтрокиРасшифровки(
			...
			СтрокаТабличнойЧасти.СтатьяДДС);
		Если Объект.ВидОперации = ВидОперацииОтПокупателя Или Объект.ВидОперации = ВидОперацииОтНашейОрганизации Тогда
			СтрокаТабличнойЧасти.СпособЗачета = РасчетыРаботаСФормамиВызовСервера.СпособРазнесенияОплатыДляДоговора(СтрокаТабличнойЧасти.Договор, Объект.Контрагент);
		Иначе
			СтрокаТабличнойЧасти.СпособЗачета = СпособЗачетаВручную;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОпределитьНеобходимостьПересчетаСуммПриИзмененииКурса(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия = КодВозвратаДиалога.Да Тогда
		...
		Если Объект.РасшифровкаПлатежа.Количество() > 0 Тогда
			...
			// Конец Прочие расчеты
			ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПокупкаВалюты")
...
Процедура СоздатьОбновитьДокументыПереносаДолгаНаСервере(ДокОбъект)
	...
	Для Каждого ТекущаяСтрока Из ДокОбъект.РасшифровкаПлатежаОтАгента Цикл
		...
		Если СоздатьОбновить Тогда
			...
			СтруктураДляЗаполнения.Вставить("ИндексСтрокиРасшифровки", ДокОбъект.РасшифровкаПлатежаОтАгента.Индекс(
				ТекущаяСтрока));
			...
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не получилось провести документ переноса долга.
												|%1'"), ПодробноеПредставлениеОшибки);
				КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("РасшифровкаПлатежаОтАгента",
					ТекущаяСтрока.НомерСтроки, "КорректировкаДолга");
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокументПереносаДолга, КонтекстноеПоле);
		...
		ИначеЕсли ПометитьНаУдаление И НЕ ТекущаяСтрока.КорректировкаДолга.Пустая() И НЕ ТекущаяСтрока.КорректировкаДолга.ПометкаУдаления Тогда
			...
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не получилось пометить на удаление документ переноса долга.
												|%1'"), ПодробноеПредставлениеОшибки);
				КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("РасшифровкаПлатежаОтАгента",
					ТекущаяСтрока.НомерСтроки, "КорректировкаДолга");
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокументПереносаДолга, КонтекстноеПоле);
		...
		ИначеЕсли НЕ ПометитьНаУдаление И НЕ СоздатьОбновить И НЕ ТекущаяСтрока.КорректировкаДолга.Пустая() Тогда
			...
			Если ТекущаяСтрока.КорректировкаДолга.Проведен Тогда
				...
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
					ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
					ТекстСообщения = СтрШаблон(НСтр("ru = 'Не получилось отменить проведение документа переноса долга.
													|%1'"), ПодробноеПредставлениеОшибки);
					КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("РасшифровкаПлатежаОтАгента",
						ТекущаяСтрока.НомерСтроки, "КорректировкаДолга");
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокументПереносаДолга, КонтекстноеПоле);
			...
			ИначеЕсли ТекущаяСтрока.КорректировкаДолга.ПометкаУдаления Тогда
				...
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
					ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
					ТекстСообщения = СтрШаблон(НСтр("ru = 'Не получилось снять пометку удаления с документа переноса долга.
													|%1'"), ПодробноеПредставлениеОшибки);
					КонтекстноеПоле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("РасшифровкаПлатежаОтАгента",
						ТекущаяСтрока.НомерСтроки, "КорректировкаДолга");
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокументПереносаДолга, КонтекстноеПоле);
...
Процедура ПокупкаВалютыСуммаУчетаПриИзменении(Элемент)
	...
	Объект.Курс = ?(
		Объект.СуммаДокумента = 0,
		1,
		Объект.СуммаУчета / Объект.СуммаДокумента * Объект.Кратность);
...
&НаКлиенте
Процедура ПродажаВалютыДоговорНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
Процедура УстановитьВидимостьПечатиЧека()
	...
	ВидОперацииПодразумеваетЧек = Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя
...
Процедура ОчиститьРеквизитыНеОтносящиесяКОперации()
	...
	ИЛИ Объект.ВидОперации = ВидОперацииОтНашейОрганизации Тогда
		...
		Объект.Курс = 0;
		Объект.Кратность = 0;
		Объект.ПродажаВалютыВалюта = Неопределено;
		Объект.ПродажаВалютыКурс = 0;
		Объект.ПродажаВалютыКратность = 0;
		Объект.АналитикаПрочихДоходовИРасходов = Неопределено;
...
Процедура УстановитьВидимостьПризнакаСпособаРасчета(Знач НапечататьЧекВидимость = неопределено)
	...
	Элементы.РасшифровкаПлатежаПризнакСпособаРасчета.Видимость = НапечататьЧекВидимость
		И Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтПокупателя");
КонецПроцедуры
...
Процедура ЭквайрингПодборЗавершение(Результат1, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.OK Тогда
		...
		Для Каждого ТекущаяСтрока Из Объект.ЭквайринговыеОперации Цикл
			...
			ТекущаяСтрока.НалоговыйУчет = РаботаСФормойДокументаКлиентСервер.ПредставлениеНалоговогоУчетаСтроки(
				ТекущаяСтрока, ВидимостьУчитыватьВНУ, ВидимостьПатент);
		КонецЦикла;
...
Процедура ЭквайринговыйТерминалПриИзмененииНаСервере()
	...
	Если Объект.ПоложениеЭквайринговогоТерминала = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке
		И ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам Тогда
...
Процедура ЭквайринговыйТерминалНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Контрагент.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Сначала выберите банк-эквайер.'");
		Сообщение.Поле = "Объект.Контрагент";
		Сообщение.Сообщить();
...
&НаКлиентеНаСервереБезКонтекста
Процедура ПересчитатьСуммуПлатежаИКомиссииЭквайрингаВСтроке(ТекущаяСтрока)
	...
	ТекущаяСтрока.СуммаКомиссииИтогоПоСтроке = ТекущаяСтрока.СуммаРасчетовКомиссии
		+ ТекущаяСтрока.СуммаРасчетовКомиссииВозврата;
...
Процедура ОткрытьФормуВыбораПериодаРасчета()
	...
	ПараметрыЗаполнения.Вставить("БанковскийСчет", Объект.БанковскийСчет);
	ПараметрыЗаполнения.Вставить("ЭквайринговыйТерминал", Объект.ЭквайринговыйТерминал);
	ПараметрыЗаполнения.Вставить("Организация", Объект.Организация);
	ПараметрыЗаполнения.Вставить("Ссылка", Объект.Ссылка);
	ПараметрыЗаполнения.Вставить("ВалютаДенежныхСредств", Объект.ВалютаДенежныхСредств);
	ПараметрыЗаполнения.Вставить("ПоложениеЭквайринговогоТерминала", Объект.ПоложениеЭквайринговогоТерминала);
	ПараметрыЗаполнения.Вставить("РасшифровкаПлатежа", ПоместитьРасшифровкаПлатежаВХранилище()); 
	
	ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения("ОткрытьФормуВыбораПериодаРасчетаЗавершение", ЭтотОбъект);
	ОткрытьФорму("Документ.ПоступлениеНаСчет.Форма.ФормаВыбораПериода", ПараметрыЗаполнения, ЭтотОбъект,
		УникальныйИдентификатор, , , ОписаниеОповещенияОЗавершении, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПодборЭквайринговыхОпераций(Команда)
	...
	ПараметрыПодбора = Новый Структура;
	ПараметрыПодбора.Вставить("АдресЭквайринговыеОперацииВХранилище", АдресЭквайринговыеОперацииВХранилище);
	ПараметрыПодбора.Вставить("Компания", Компания);
	ПараметрыПодбора.Вставить("Дата", Объект.Дата);
	ПараметрыПодбора.Вставить("Контрагент", Объект.Контрагент);
	ПараметрыПодбора.Вставить("Ссылка", Объект.Ссылка);
...
Процедура ЭквайринговыеОперацииЭквайринговыйТерминалПриИзменении(Элемент)
	...
	Если НалоговыйУчетТерминала.СобственныеНастройкиНалоговогоУчета Тогда
		...
		ВыбраннаяСтрока.НалоговыйУчет = РаботаСФормойДокументаКлиентСервер.ПредставлениеНалоговогоУчетаСтроки(
			СтрокаЭквайринга, ВидимостьУчитыватьВНУ, ВидимостьПатент);
...
Процедура ДоговорЭквайрингаПриИзменении(Элемент)
	...
	Если Объект.ПоложениеЭквайринговогоТерминала = ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти") И Объект.ЭквайринговыеОперации.Количество() > 0 Тогда
		Для Каждого ЭлементКоллекции Из Объект.ЭквайринговыеОперации Цикл
			ОбработатьИзменениеТерминалаВТЧ(ЭлементКоллекции);
		КонецЦикла;
	Иначе
		ПроверитьСоответствиеТерминалаДоговору(); 
		ОбработатьИзменениеТерминалаВШапке(Элемент);
	КонецЕсли;
	ДоговорЭквайрингаПриИзмененииНаСервере();
...
&НаСервере
Процедура ПроверитьСоответствиеТерминалаДоговору()
	
	Если Объект.ЭквайринговыйТерминал.Договор <> ДоговорЭквайринга Тогда
		Объект.ЭквайринговыйТерминал = Справочники.СпособыОплаты.ПустаяСсылка();
	КонецЕсли; 
...
Процедура ЭквайринговыеОперацииПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		СтрокаЭквайринг.НалоговыйУчет = РаботаСФормойДокументаКлиентСервер.ПредставлениеНалоговогоУчетаСтроки(
			СтрокаЭквайринг, ВидимостьУчитыватьВНУ, ВидимостьПатент);
	КонецЕсли; 
