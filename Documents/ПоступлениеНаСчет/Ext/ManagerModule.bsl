...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.ПоступлениеНаСчет.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		...
		КомандаСозданияНаОсновании = МодульСозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.Документы.ПоступлениеНаСчет);
...
Процедура СформироватьТаблицаДенежныеСредства(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПоступлениеДенежныхСредств", СодержанияПроводок.ПоступлениеДенежныхСредств());
	...
	|			ИЛИ ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОтПродажиВалюты))
	|	И &ОстатокСуммы > 0
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ИЛИ ВременнаяТаблицаРасшифровкаПлатежа.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОтПродажиВалюты))
	...
	|ГДЕ
	|	ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПокупкаВалюты)
	|	И ТаблицаДокумента.РасчетКурсовойРазницыПокупкаПродажаВалюты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	Если ДокументСсылкаПоступлениеНаСчет.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПокупкаВалюты
		И ТЗРезультатЗапроса.Количество() > 1 Тогда
		ИтогСуммаУчета = 0;
...
Процедура СформироватьТаблицаРасчетыСПодотчетниками(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ПогашениеДолгаПодотчетника", СодержанияПроводок.ПогашениеДолгаПодотчетника());
	Запрос.УстановитьПараметр("КурсоваяРазница", СодержанияПроводок.КурсоваяРазница());
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	|	&Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.КорреспонденцияТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.Доходы)
	...
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.Прочее),
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.КурсоваяРазницаПриПродажеВалюты > 0
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПрочиеДоходы)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПрочиеРасходы)
	|	КОНЕЦ,
	|	ТаблицаДокумента.ВалютаДенежныхСредств,
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.КурсоваяРазницаПриПродажеВалюты > 0
	|			ТОГДА ТаблицаДокумента.КурсоваяРазницаПриПродажеВалюты
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.КурсоваяРазницаПриПродажеВалюты < 0
	...
	|		ПО (КурсыВалютУчетаСрезПоследних.Валюта = &ВалютаУчета)
	|ГДЕ
	|	ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОтПродажиВалюты)
	|	И ТаблицаДокумента.РасчетКурсовойРазницыПокупкаПродажаВалюты
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок,
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КурсоваяРазница", СодержанияПроводок.КурсоваяРазница());
	Запрос.УстановитьПараметр("Содержание", СодержанияПроводок.ОприходованиеДенежныхСредствСоСчета());
	Запрос.УстановитьПараметр("СодержаниеВзносНаличных", СодержанияПроводок.ОприходованиеДенежныхСредствИзКассы());
	Запрос.УстановитьПараметр("ВозвратНалога", СодержанияПроводок.ВозвратНалога());
	Запрос.УстановитьПараметр("СодержаниеПокупкаВалюты", СодержанияПроводок.ПокупкаВалюты());
	...
	Запрос.УстановитьПараметр("ИспользуютсяПереводыВПути",РегистрыСведений.ПрименениеПереводовВПути.ИспользуютсяПереводыВПути(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата,СтруктураДополнительныеСвойства.ДляПроведения.Организация));
	Запрос.УстановитьПараметр("СодержаниеПродажаВалюты", СодержанияПроводок.ПродажаВалюты());
	Запрос.УстановитьПараметр("ВалютаУчета", СтруктураДополнительныеСвойства.ВалютаУчета);
...
Функция ТекстЗапросаТаблицаУправленческий()
	...
	|			КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОтПродажиВалюты)
	|				ТОГДА &СодержаниеПродажаВалюты
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.КурсоваяРазницаПриПродажеВалюты > 0
	|				И ТаблицаДокумента.Корреспонденция.Валютный
	|			ТОГДА ТаблицаДокумента.ВалютаДенежныхСредств
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВЫБОР
	...
	|		КОГДА ТаблицаДокумента.КурсоваяРазницаПриПродажеВалюты < 0
	|			ТОГДА -ТаблицаДокумента.КурсоваяРазницаПриПродажеВалюты
	|		ИНАЧЕ ТаблицаДокумента.КурсоваяРазницаПриПродажеВалюты
	|	КОНЕЦ,
	|	&КурсоваяРазница
	|ИЗ
	|	ВременнаяТаблицаШапка КАК ТаблицаДокумента
...
Функция ТекстЗапросаСформироватьТаблицаДоходыИРасходыКассовыйМетод()
	...
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.Прочее),
	|	ТаблицаДокумента.Статья,
	|	ТаблицаДокумента.КурсоваяРазницаПриПродажеВалюты,
	|	0
	|ИЗ
	|	ВременнаяТаблицаШапка КАК ТаблицаДокумента
	|ГДЕ
	|	&КассовыйМетодУчетаДоходовИРасходов
	|	И ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОтПродажиВалюты)
	|	И &ОстатокСуммы > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура СформироватьТаблицаОплатаДокументов(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	МассивДоступныхОпераций.Добавить(Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика);
...
Процедура СформироватьТаблицаДвиженияДенежныхСредств(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.Проект,
	|	ТаблицаДокумента.Подразделение,
	|	СУММА(ТаблицаДокумента.КурсоваяРазницаПриПродажеВалюты),
	|	СУММА(&ОстатокСуммы),
	|	0,
	|	0
	|ИЗ
...
Функция ПолучитьТекстЗапросаИнициализироватьДанные()
	...
	|ИЗ
	|	РегистрСведений.КурсыВалют.СрезПоследних(&МоментВремени, Валюта В (&ВалютаУчета, &ВалютаДС, &ВалютаДоговораКредитаЗайма, &ПродажаВалютыВалюта)) КАК КурсыВалютСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		КОГДА ТаблицаДокумента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОтПродажиВалюты)
	|			ТОГДА ТаблицаДокумента.Ссылка.ПродажаВалютыВалюта
	...
	|		КОГДА ТаблицаДокумента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОтПродажиВалюты)
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаДокумента.ПродажаВалютыКратность = 0
	|						ТОГДА 0
	|					КОГДА КурсыВалютЦБДляПродажиВалюты.Кратность = 0
	|						ТОГДА 0
	|					ИНАЧЕ ВЫРАЗИТЬ(ТаблицаДокумента.СуммаУчета * (ТаблицаДокумента.ПродажаВалютыКурс / ТаблицаДокумента.ПродажаВалютыКратность - КурсыВалютЦБДляПродажиВалюты.Курс / КурсыВалютЦБДляПродажиВалюты.Кратность) КАК ЧИСЛО(15, 2))
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КурсоваяРазницаПриПродажеВалюты,
	|	ВЫБОР
	...
	|					КОГДА КурсыВалютЦБДляПродажиВалюты.Кратность = 0
	|						ТОГДА ЛОЖЬ
	|					КОГДА (ВЫРАЗИТЬ(ТаблицаДокумента.СуммаУчета * (ТаблицаДокумента.ПродажаВалютыКурс / ТаблицаДокумента.ПродажаВалютыКратность - КурсыВалютЦБДляПродажиВалюты.Курс / КурсыВалютЦБДляПродажиВалюты.Кратность) КАК ЧИСЛО(15, 2))) = 0
	|						ТОГДА ЛОЖЬ
	|					ИНАЧЕ ТаблицаДокумента.Ссылка.РасчетКурсовойРазницыПокупкаПродажаВалюты
	|				КОНЕЦ
	|		КОГДА ТаблицаДокумента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеНаСчет.ПокупкаВалюты)
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаДокумента.Кратность = 0
	|						ТОГДА ЛОЖЬ
	|					КОГДА КурсыВалютБанковскогоСчета.Кратность = 0
	|						ТОГДА ЛОЖЬ
	|					КОГДА (ВЫРАЗИТЬ(ТаблицаДокумента.СуммаУчета * (ТаблицаДокумента.Курс / ТаблицаДокумента.Кратность - КурсыВалютБанковскогоСчета.Курс / КурсыВалютБанковскогоСчета.Кратность) КАК ЧИСЛО(15, 2))) = 0
	|						ТОГДА ЛОЖЬ
	|					ИНАЧЕ ТаблицаДокумента.Ссылка.РасчетКурсовойРазницыПокупкаПродажаВалюты
	|				КОНЕЦ
	|		ИНАЧЕ ТаблицаДокумента.Ссылка.РасчетКурсовойРазницыПокупкаПродажаВалюты
	|	КОНЕЦ КАК РасчетКурсовойРазницыПокупкаПродажаВалюты
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыВалютБанковскогоСчета
	|		ПО (КурсыВалютБанковскогоСчета.Валюта = &ВалютаДС)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыВалютЦБДляПродажиВалюты
	|		ПО (КурсыВалютЦБДляПродажиВалюты.Валюта = &ПродажаВалютыВалюта)
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка";
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства) Экспорт
	...
	Если ДокументСсылкаПоступлениеНаСчет.ВидОперации =
			ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ПоступлениеОтПродажиВалюты") Тогда
		Запрос.УстановитьПараметр("ПродажаВалютыВалюта", ДокументСсылкаПоступлениеНаСчет.ПродажаВалютыВалюта);
	Иначе	
		Запрос.УстановитьПараметр("ПродажаВалютыВалюта", СтруктураДополнительныеСвойства.ВалютаУчета);
	КонецЕсли;
...
Процедура ВыполнитьКонтроль(ДокументСсылкаПоступлениеНаСчет, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияДенежныеСредстваИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияРасчетыСПокупателямиИзменение Тогда
		...
		|			И ДвиженияРасчетыПоЭквайрингуИзменение.ВидОплаты = РасчетыПоЭквайрингуОстатки.ВидОплаты
		|ГДЕ
		|	(&УдалениеПроведения
...
Процедура СформироватьТаблицаРасчетыПоЭквайрингу(ДокументСсылкаПоступлениеНаСчет, СтруктураДополнительныеСвойства)
	...
	Если ПараметрыЭквайринга.КонтрольВзаиморасчетовЭквайринг Тогда
		...
		|	ВЫБОР
		|		КОГДА ПоступлениеНаСчетЭквайринговыеОперации.ВидОплаты = ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.ВыдачаНаличных)
		...
		|	ПоступлениеНаСчетЭквайринговыеОперации.ВидОплаты КАК ВидОплаты
		|ИЗ
		|	Документ.ПоступлениеНаСчет.ЭквайринговыеОперации КАК ПоступлениеНаСчетЭквайринговыеОперации
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыВалютУчета
		|		ПО (КурсыВалютУчета.Валюта = &ВалютаУчета)
		...
		|	ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.ПустаяСсылка)
		|ИЗ
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Заказ", НСтр(
		"ru = 'Заказ покупателя'"), ОписанияТиповПолей.ОписаниеТиповСтрока50, ОписаниеТиповЗаказ);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "НомерЗаказаАгента", НСтр(
		"ru = '№ заказа агента'"), ОписанияТиповПолей.ОписаниеТиповСтрока1000,
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "Документ", НСтр(
		"ru = 'Документ'"), ОписанияТиповПолей.ОписаниеТиповСтрока50, ОписаниеТиповКолонка);

	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ПолученоОтКлиента", НСтр(
		"ru = 'Получено от клиента'"), ОписанияТиповПолей.ОписаниеТиповСтрока25,
		ОписанияТиповПолей.ОписаниеТиповЧисло15_2);
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "УдержаноАгентом", НСтр(
		"ru = 'Удержано агентом'"), ОписанияТиповПолей.ОписаниеТиповСтрока25, ОписанияТиповПолей.ОписаниеТиповЧисло15_2);
