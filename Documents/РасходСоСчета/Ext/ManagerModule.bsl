...
Процедура СформироватьТаблицаДенежныеСредства(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("РасходДенежныхСредств", СодержанияПроводок.РасходДенежныхСредств());
	Запрос.УстановитьПараметр("ПеремещениеДенежныхСредств", СодержанияПроводок.ПеремещениеДенежныхСредств());
	...
	// БЗКБ
	Если Справочники.Организации.НужноПоказыватьЭлементыБЗКБДляОрганизации(ДокументСсылкаРасходСоСчета.Организация) Тогда
		Если ДокументСсылкаРасходСоСчета.ВыплатаРегламентированнойЗаработнойПлаты.Количество() > 0
			И ДокументСсылкаРасходСоСчета.ВыплатаЗаработнойПлаты.Количество() = 0 Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "Документ.РасходСоСчета.ВыплатаЗаработнойПлаты", "Документ.РасходСоСчета.ВыплатаРегламентированнойЗаработнойПлаты");
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "Документ.ПлатежнаяВедомость.Сотрудники", "Документ.ВедомостьНаВыплатуЗарплатыВБанк.Зарплата");
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПлатежнаяВедомостьСотрудники.Сотрудник", "ПлатежнаяВедомостьСотрудники.ФизическоеЛицо");
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПлатежнаяВедомостьСотрудники.СуммаПлатежа", "ПлатежнаяВедомостьСотрудники.КВыплате");
		КонецЕсли;
	КонецЕсли;
	// Конец БЗКБ
...
Процедура СформироватьТаблицаРасчетыСПодотчетниками(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеДолгаПодотчетника", СодержанияПроводок.ВозникновениеДолгаПодотчетника());
	Запрос.УстановитьПараметр("КурсоваяРазница", СодержанияПроводок.КурсоваяРазница());
	Запрос.УстановитьПараметр("ВалютаУчета",	Константы.ВалютаУчета.Получить());
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаРасчетыСПерсоналомБЗКБ(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	
	Если ДокументСсылкаРасходСоСчета.ВыплатаРегламентированнойЗаработнойПлаты.Количество() > 0
		И ДокументСсылкаРасходСоСчета.ВыплатаЗаработнойПлаты.Количество() = 0
		И Справочники.Организации.НужноПоказыватьЭлементыБЗКБДляОрганизации(ДокументСсылкаРасходСоСчета.Организация) Тогда
		...
		Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
		Запрос.УстановитьПараметр("УчетПоНесколькимПодразделениям", ПолучитьФункциональнуюОпцию("УчетПоНесколькимПодразделениям"));
		Запрос.УстановитьПараметр("ПогашениеОбязательствПередПерсоналом", СодержанияПроводок.ПогашениеОбязательствПередПерсоналом());
		Запрос.УстановитьПараметр("КурсоваяРазница", СодержанияПроводок.КурсоваяРазница());
		Запрос.УстановитьПараметр("ВалютаУчета",	Константы.ВалютаУчета.Получить());
		...
		|				ВыплатаЗаработнойПлаты.Ведомость
		|			ИЗ
		|				Документ.РасходСоСчета.ВыплатаРегламентированнойЗаработнойПлаты КАК ВыплатаЗаработнойПлаты
		|			ГДЕ
		|				ВыплатаЗаработнойПлаты.Ссылка = &Ссылка
		|				И ВыплатаЗаработнойПлаты.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Зарплата))";
		...
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Документ.ВедомостьНаВыплатуЗарплатыВБанк");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		...
		|	&Организация КАК Организация,
		|	ВыплатаЗаработнойПлаты.Ссылка.ВалютаДенежныхСредств КАК Валюта,
		|	ПлатежнаяВедомостьСотрудники.Сотрудник.СчетРасчетовСПерсоналом КАК СчетУчета,
		|	ВыплатаЗаработнойПлаты.Ссылка.БанковскийСчет КАК БанковскийСчет,
		|	ПлатежнаяВедомостьСотрудники.КВыплате КАК СуммаВал,
		|	ВЫРАЗИТЬ(ПлатежнаяВедомостьСотрудники.КВыплате * КурсыВалютКассы.Курс * КурсыВалютУчета.Кратность / (КурсыВалютУчета.Курс * КурсыВалютКассы.Кратность) КАК ЧИСЛО(15, 2)) КАК Сумма,
		|	&ПогашениеОбязательствПередПерсоналом КАК СодержаниеПроводки
		|ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПерсоналомБЗКБ
		|ИЗ
		|	Документ.ВедомостьНаВыплатуЗарплатыВБанк.Зарплата КАК ПлатежнаяВедомостьСотрудники
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РасходСоСчета.ВыплатаРегламентированнойЗаработнойПлаты КАК ВыплатаЗаработнойПлаты
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&МоментВремени, ) КАК КурсыВалютКассы
		|			ПО ВыплатаЗаработнойПлаты.Ссылка.ВалютаДенежныхСредств = КурсыВалютКассы.Валюта
		|		ПО ПлатежнаяВедомостьСотрудники.Ссылка = ВыплатаЗаработнойПлаты.Ведомость
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&МоментВремени, Валюта = &ВалютаУчета) КАК КурсыВалютУчета
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("КурсоваяРазница", СодержанияПроводок.КурсоваяРазница());
	Запрос.УстановитьПараметр("ВалютаУчета",	Константы.ВалютаУчета.Получить());
	Запрос.УстановитьПараметр("ПроектВШапке", ДокументСсылкаРасходСоСчета.ПоложениеПроекта = 
		Перечисления.ПоложениеРеквизитаНаФорме.ВШапке);
	Запрос.УстановитьПараметр("ПодразделениеВШапке", ДокументСсылкаРасходСоСчета.ПоложениеПодразделения = 
		Перечисления.ПоложениеРеквизитаНаФорме.ВШапке);
	...
	|	&Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Ссылка.Корреспонденция.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.Расходы)
	|			ТОГДА ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Ссылка.Корреспонденция.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.Расходы)
	|			ТОГДА ТаблицаДокумента.Ссылка.Заказ
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|	ТаблицаДокумента.Ссылка.Корреспонденция КАК СчетУчета,
	|	ТаблицаДокумента.Ссылка.АналитикаПрочихДоходовИРасходов КАК Аналитика,
	|	&ОтражениеРасходов КАК СодержаниеПроводки,
	|	0 КАК СуммаДоходов,
	|	ВЫРАЗИТЬ(ТаблицаДокумента.СуммаПлатежа * КурсыВалютРасчетов.Курс * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * КурсыВалютРасчетов.Кратность) КАК ЧИСЛО(15, 2)) КАК СуммаРасходов,
	|	ВЫРАЗИТЬ(ТаблицаДокумента.СуммаПлатежа * КурсыВалютРасчетов.Курс * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * КурсыВалютРасчетов.Кратность) КАК ЧИСЛО(15, 2)) КАК Сумма
	|ИЗ
	...
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка
	|	И (ТаблицаДокумента.Ссылка.Корреспонденция.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.Расходы)
	|			ИЛИ ТаблицаДокумента.Ссылка.Корреспонденция.ТипСчета = ЗНАЧЕНИЕ(Перечисление.ТипыСчетов.ПрочиеРасходы))
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КурсоваяРазница", СодержанияПроводок.КурсоваяРазница());
	Запрос.УстановитьПараметр("Содержание", СодержанияПроводок.СписаниеДенежныхСредств());
	Запрос.УстановитьПараметр("СодержаниеСнятиеНаличных", СодержанияПроводок.ОприходованиеДенежныхСредствВКассу());
	Запрос.УстановитьПараметр("ОплатаНалога", СодержанияПроводок.ОплатаНалога());
	Запрос.УстановитьПараметр("СодержаниеОплатаОтБанкаЭквайрера", СодержанияПроводок.ОплатаОтБанкаЭквайера());
	Запрос.УстановитьПараметр("СодержаниеКомиссияБанкаЭквайрера", СодержанияПроводок.КомиссияБанкаЭквайера());
	Запрос.УстановитьПараметр("СодержаниеОплатаОтБанка", СодержанияПроводок.ОплатаОтБанка());
	Запрос.УстановитьПараметр("СодержаниеКомиссияБанка", СодержанияПроводок.КомиссияБанка());
	Запрос.УстановитьПараметр("СодержаниеЛичныеСредства", СодержанияПроводок.ЛичныеСредства());
	Запрос.УстановитьПараметр("ВалютаУчета",	Константы.ВалютаУчета.Получить());
	Запрос.УстановитьПараметр("ИспользуютсяПереводыВПути",РегистрыСведений.ПрименениеПереводовВПути.ИспользуютсяПереводыВПути(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата,СтруктураДополнительныеСвойства.ДляПроведения.Организация));
	...
	|	ТаблицаДокумента.СодержаниеПроводки
	|ИЗ
	|	ВременнаяТаблицаРасчетыСПерсоналомБЗКБ КАК ТаблицаДокумента
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок,
	|	НомерСтроки";
...
Процедура СформироватьТаблицаРасчетыПоНалогам(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	|	И НЕ ТаблицаДокумента.ВидНалога = ЗНАЧЕНИЕ(Справочник.ВидыНалогов.ЕдиныйНалоговыйПлатеж)";
...
Процедура СформироватьТаблицаОплатаДокументов(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	МассивДоступныхОпераций.Добавить(Перечисления.ВидыОперацийРасходСоСчета.Покупателю);
...
Процедура СформироватьНДФЛРасчетыНалоговыхАгентовСБюджетом(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	ДокументСсылкаРасходИзКассы.Организация,
	ДокументСсылкаРасходИзКассы.Дата);
	
	Если ПлательщикЕНП Тогда
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаНДФЛРасчетыНалоговыхАгентовСБюджетом", Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
	...
	Если ВыборкаПоНДФЛ.Следующий() Тогда
		...
		Если РасчетПоПрогрессивнойШкале Тогда
			СтрокиРаспределения = СотрудникиУНФ.РаспределитьПеречисленныйНалогПоФизическимЛицамСПревышения(ДокументСсылкаРасходИзКассы, ДокументСсылкаРасходИзКассы.Организация,ВыборкаПоНДФЛ.Период , ВыборкаПоНДФЛ.Сумма, ВыборкаПоНДФЛ.ПериодРегистрации);
		Иначе
			СтрокиРаспределения = СотрудникиУНФ.РаспределитьПеречисленныйНалогПоФизическимЛицам(ДокументСсылкаРасходИзКассы, ДокументСсылкаРасходИзКассы.Организация,ВыборкаПоНДФЛ.Период , ВыборкаПоНДФЛ.Сумма, ВыборкаПоНДФЛ.ПериодРегистрации);
		КонецЕсли; 
...
Процедура СформироватьТаблицаРасчетыНалогоплательщиковСБюджетомПоНДФЛУНФ(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	
	УчетНДФЛУНФ.СформироватьТаблицаРасчетыНалогоплательщиковСБюджетомПоНДФЛУНФ(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства);
...
Процедура СформироватьРасчетыСФондамиПоСтраховымВзносам(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Если ПлательщикЕНП Тогда
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРасчетыСФондамиПоСтраховымВзносам", Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	РасчетыСФондамиПоСтраховымВзносамУНФ.Период,
	|	РасчетыСФондамиПоСтраховымВзносамУНФ.Организация,
	|	РасчетыСФондамиПоСтраховымВзносамУНФ.ВидДвижения,
	|	РасчетыСФондамиПоСтраховымВзносамУНФ.ЭтоСтраховыеВзносы,
	|	РасчетыСФондамиПоСтраховымВзносамУНФ.ВидОбязательногоСтрахованияСотрудников,
	|	РасчетыСФондамиПоСтраховымВзносамУНФ.Сумма
	|ИЗ
	|	ВТРасчетыСФондамиПоСтраховымВзносам КАК РасчетыСФондамиПоСтраховымВзносамУНФ";
	...
	ТаблицаПоВзносам.Колонки.Добавить("ВидОбязательногоСтрахованияСотрудников",Новый ОписаниеТипов("ПеречислениеСсылка.ВидыОбязательногоСтрахованияСотрудниковУНФ"));
	...
	Если ВыборкаПоВзносам.Следующий() Тогда
		СтрокиРаспределения = СотрудникиУНФ.РаспределитьПеречисленныйВзносПоФизическимЛицам(ДокументСсылкаРасходИзКассы, ДокументСсылкаРасходИзКассы.Организация,ВыборкаПоВзносам.Период , ВыборкаПоВзносам.Сумма, ВыборкаПоВзносам.ВидОбязательногоСтрахованияСотрудников);
		Для Каждого СтрокаТЗ Из СтрокиРаспределения Цикл
...
Процедура СформироватьТаблицаЗакупкиПоставщиковДляКУДиР(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВременнаяТаблицаРасчетыСПоставщиками.Валюта КАК ВалютаДоговора,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТЗакупкиПоставщиковДляКУДиР.ВалютаДоговора КАК ВалютаДоговора,
	...
	|	ЕСТЬNULL(ВТЗакупкиПоставщиковДляКУДиР.СтавкаНДС, 0) КАК СтавкаНДС,
	|	0 КАК Количество,
	|	0 КАК СуммаНДС,
	...
	|		ПО ВТЗакупкиПоставщиковДляКУДиР.ВалютаДоговора = КурсыВалютСрезПоследних.Валюта
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СУММА(ПередачаТоваровМеждуОрганизациямиЗапасы.Количество),
	|	СУММА(ПередачаТоваровМеждуОрганизациямиЗапасы.Всего),
	|	ЕСТЬNULL(ПередачаТоваровМеждуОрганизациямиЗапасы.СтавкаНДС.Ставка, 0),
	|	НЕ ПередачаТоваровМеждуОрганизациямиЗапасы.ТоварыДляПроизводства
	|ИЗ
	|	ВТТоварныеДокументы КАК ВТТоварныеДокументы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПередачаТоваровМеждуОрганизациями.Запасы КАК ПередачаТоваровМеждуОрганизациямиЗапасы
	|		ПО (ПередачаТоваровМеждуОрганизациямиЗапасы.Ссылка = ВТТоварныеДокументы.ТоварныйДокумент)
	|			И (ПередачаТоваровМеждуОрганизациямиЗапасы.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас))
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТТоварныеДокументы.ТоварныйДокумент,
	|	ПередачаТоваровМеждуОрганизациямиЗапасы.Номенклатура,
	|	ПередачаТоваровМеждуОрганизациямиЗапасы.ЕдиницаИзмерения,
	|	ЕСТЬNULL(ПередачаТоваровМеждуОрганизациямиЗапасы.СтавкаНДС.Ставка, 0),
	|	ПередачаТоваровМеждуОрганизациямиЗапасы.ТоварыДляПроизводства
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Пока ВыборкаДокументовСТоварами.Следующий() Цикл
		...
		Для Каждого ДокументОплаты Из СвободныеОплаты Цикл
			Если СуммаЗаписи > 0 Тогда
				...
				Если УчетВалюты И ДокументОплаты.ВалютаДоговора <> НацВалюта Тогда
					НовСтр.Сумма = НовСтр.Сумма * ДокументОплаты.Курс / ДокументОплаты.Кратность;
				КонецЕсли;
...
Процедура СформироватьТаблицаДвиженияДенежныхСредств(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Если Справочники.Организации.НужноПоказыватьЭлементыБЗКБДляОрганизации(ДокументСсылкаРасходСоСчета.Организация) Тогда
		Если ДокументСсылкаРасходСоСчета.ВыплатаРегламентированнойЗаработнойПлаты.Количество() > 0
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПлатежнаяВедомостьСотрудники.Сотрудник", "ПлатежнаяВедомостьСотрудники.ФизическоеЛицо");
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПлатежнаяВедомостьСотрудники.СуммаПлатежа", "ПлатежнаяВедомостьСотрудники.КВыплате");
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВыплатаЗаработнойПлаты.Проект", "НЕОПРЕДЕЛЕНО");
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВыплатаЗаработнойПлаты.Подразделение", "НЕОПРЕДЕЛЕНО");
		КонецЕсли;
	КонецЕсли;
	// Конец БЗКБ
	...
	КурсовыеРазницыУНФ.ДополнитьДвиженияДенежныхСредствКурсовойРазницей(СтруктураДополнительныеСвойства);
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьДвиженияНаЕдиномНалоговомСчете(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаДокумента.Дата КАК Период,
	|	ТаблицаДокумента.Ссылка КАК ПлатежныйДокумент,
	|	ТаблицаДокумента.Ссылка КАК Регистратор,
	|	ВЫРАЗИТЬ(ТаблицаДокумента.СуммаДокумента * КурсВалютыДокумента.Курс * КурсВалютыУчета.Кратность / (КурсВалютыУчета.Курс * КурсВалютыДокумента.Кратность) КАК ЧИСЛО(15, 2)) КАК Сумма,
	|	ТаблицаДокумента.Организация.ГоловнаяОрганизация КАК ГоловнаяОрганизация
	|ИЗ
	|	Документ.РасходСоСчета КАК ТаблицаДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ КурсыВалютНалоги КАК КурсВалютыДокумента
	|		ПО ТаблицаДокумента.ВалютаДенежныхСредств = КурсВалютыДокумента.Валюта
	|		ЛЕВОЕ СОЕДИНЕНИЕ КурсыВалютНалоги КАК КурсВалютыУчета
	|		ПО (КурсВалютыУчета.Валюта = &ВалютаУчета)
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка
	|	И ТаблицаДокумента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Налоги)
	|	И ТаблицаДокумента.ВидНалога = ЗНАЧЕНИЕ(Справочник.ВидыНалогов.ЕдиныйНалоговыйПлатеж)";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРасчетыПоЕдиномуНалоговомуСчету", РезультатЗапроса.Выгрузить());
	
КонецПроцедуры // СформироватьДвиженияНаЕдиномНалоговомСчете()
...
Функция ПолучитьТекстЗапросаИнициализироватьДанные()
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СтатьяДДС = ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ПФРНакопительная)
	...
	|						ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ПФРПоСуммарномуТарифу)
	|					ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ПФРСтраховая)
	|				КОНЕЦ
	|		КОГДА РасходСоСчета.ВидНалога = ЗНАЧЕНИЕ(Справочник.ВидыНалогов.ФОМССотрудники)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ФФОМС)
	|		КОГДА РасходСоСчета.ВидНалога = ЗНАЧЕНИЕ(Справочник.ВидыНалогов.ФСССотрудники)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ФСС)
	|		КОГДА РасходСоСчета.ВидНалога = ЗНАЧЕНИЕ(Справочник.ВидыНалогов.ФССТравматизмСотрудники)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудниковУНФ.ФССНесчастныеСлучаи)
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("ОбъединятьНакопительнуюИСтраховуюЧасть", ?(ЗначениеЗаполнено(ДокументСсылкаРасходСоСчета.ДокументОснование), ДокументСсылкаРасходСоСчета.ДокументОснование.Дата >= УчетСтраховыхВзносовУНФ.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР(), Истина));
	...
	СформироватьТаблицаРасчетыСПерсоналомБЗКБ(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства);
	СформироватьТаблицаРасчетыНалогоплательщиковСБюджетомПоНДФЛУНФ(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства);
	// Прочие расчеты
	...
	СформироватьДвиженияНаЕдиномНалоговомСчете(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства); 
	//Конец Единый налоговый счет
...
Процедура СформироватьТаблицаРасчетыПоЭквайрингу(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ОплатаПлатежнойКартой", СодержанияПроводок.ОплатаОтБанкаЭквайера());
	Запрос.УстановитьПараметр("ОплатаКредитом", СодержанияПроводок.ОплатаОтБанкаПоПродажеВКредит());
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
...
Процедура СформироватьТаблицаДоходыИРасходыЭквайринг(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Для каждого ТекущаяСтрокаРасчетов Из ТаблицаРасчетыПоЭквайрингу Цикл
		...
		Движение.СодержаниеПроводки = СодержанияПроводок.КомиссияБанкаЭквайера();
...
Процедура СформироватьТаблицаРасчетыПоКредитамИЗаймам(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("УчетРасчетовПоЗаймамСотрудникам", СодержанияПроводок.ВыдачаЗаймаСотруднику());
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаРасходСоСчета);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("КурсоваяРазница", СодержанияПроводок.КурсоваяРазница());
...
Функция КонтрагентДолженБытьЗаполнен(ВидОперации) Экспорт
	...
	Возврат ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику
