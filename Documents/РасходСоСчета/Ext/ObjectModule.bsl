...
Процедура ЗаполнитьПоРасходДСПлан(ДокументСсылкаРасходДСПлан, Сумма = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Выборка.ДокументОснованиеЗаявки) Тогда
		...
		Если ТипЗнч(Выборка.ДокументОснованиеЗаявки) = Тип("ДокументСсылка.НачислениеЗарплатыУНФ") Тогда
...
Процедура ЗаполнитьПоПлатежноеПоручение(ДокументСсылкаПлатежноеПоручение) Экспорт
	...
	Если ЭтотОбъект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата Тогда
		...
		Запрос.Текст = 
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ПлатежноеПоручениеВыплатаРегламентированнойЗаработнойПлаты.НомерСтроки КАК НомерСтроки,
			|	ПлатежноеПоручениеВыплатаРегламентированнойЗаработнойПлаты.Ведомость КАК Ведомость,
			|	ПлатежноеПоручениеВыплатаРегламентированнойЗаработнойПлаты.СуммаПлатежа КАК СуммаПлатежа
			|ИЗ
			|	Документ.ПлатежноеПоручение.ВыплатаРегламентированнойЗаработнойПлаты КАК ПлатежноеПоручениеВыплатаРегламентированнойЗаработнойПлаты
			|ГДЕ
			|	ПлатежноеПоручениеВыплатаРегламентированнойЗаработнойПлаты.Ссылка = &ДокументСсылкаПлатежноеПоручение";
		
		Запрос.УстановитьПараметр("ДокументСсылкаПлатежноеПоручение", ДокументСсылкаПлатежноеПоручение);
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		
		ВыборкаРасшифровка = МассивРезультатов[0].Выбрать();
		...
		// БЗКБ
		ВыборкаРегламентированнаяЗарплата = МассивРезультатов[1].Выбрать();
		Пока ВыборкаРегламентированнаяЗарплата.Следующий() Цикл
			НоваяСтрока = ВыплатаРегламентированнойЗаработнойПлаты.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаРегламентированнаяЗарплата);
		КонецЦикла;
		// Конец БЗКБ
	...
	Иначе
		...
		Иначе
			...
			Запрос.Текст = 
				...
				|	ПлатежноеПоручениеРасшифровкаПлатежа.Проект КАК Проект,
				|	ПлатежноеПоручениеРасшифровкаПлатежа.Подразделение КАК Подразделение
				|ИЗ
...
Процедура ЗаполнитьПоПриходнаяНакладная(ДокументСсылкаПриходнаяНакладная) Экспорт
	...
	|	МАКСИМУМ(ШапкаДокумента.НомерЧекаККМ > 0) КАК ЧекПробит
	|ИЗ
	...
	Для Каждого ТекущаяСтрока Из ТаблицаДанных Цикл
		...
		Если НЕ ЧекПробит Тогда
			НоваяСтрока.ПризнакСпособаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиСпособаРасчета.ПередачаСПолнойОплатой");
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьПоОтчетКомиссионера(ДокументСсылкаОтчетКомиссионера) Экспорт
	...
	Запрос.Текст =
		...
		|	1 КАК ВидОперации
		|ПОМЕСТИТЬ ВТСводнаяТаблицаЗапасы
		|ИЗ
		...
		|	ОтчетКомиссионераЗапасыВозвраты.СуммаВознаграждения,
		|	ОтчетКомиссионераЗапасыВозвраты.СуммаНДСВознаграждения,
		|	ОтчетКомиссионераЗапасыВозвраты.ЗаказПокупателя,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(ВТСводнаяТаблицаЗапасы.Сумма) КАК Сумма,
		|	СУММА(ВТСводнаяТаблицаЗапасы.СуммаНДС) КАК СуммаНДС,
		|	СУММА(ВТСводнаяТаблицаЗапасы.СуммаВознаграждения) КАК СуммаВознаграждения,
		|	СУММА(ВТСводнаяТаблицаЗапасы.СуммаНДСВознаграждения) КАК СуммаНДСВознаграждения,
		|	ВТСводнаяТаблицаЗапасы.ЗаказПокупателя КАК ЗаказПокупателя,
		|	ВТСводнаяТаблицаЗапасы.СтавкаНДС КАК СтавкаНДС,
		|	ВТСводнаяТаблицаЗапасы.ВидОперации КАК ВидОперации
		|ПОМЕСТИТЬ ВТСводнаяТаблицаЗапасыИтог
		|ИЗ
		|	ВТСводнаяТаблицаЗапасы КАК ВТСводнаяТаблицаЗапасы
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТСводнаяТаблицаЗапасы.СтавкаНДС,
		|	ВТСводнаяТаблицаЗапасы.ЗаказПокупателя,
		|	ВТСводнаяТаблицаЗапасы.ВидОперации
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Покупателю)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Покупателю)
		|	КОНЕЦ КАК ВидОперации,
		|	&Ссылка КАК ДокументОснование,
		|	ВЫБОР
		|		КОГДА ШапкаДокумента.Организация.БанковскийСчетПоУмолчанию.ВалютаДенежныхСредств = ШапкаДокумента.Договор.ВалютаРасчетов
		...
		|	ВЫБОР
		|		КОГДА ШапкаДокумента.Контрагент.ВестиРасчетыПоЗаказам
		|			ТОГДА ВТСводнаяТаблицаЗапасыИтог.ЗаказПокупателя
		...
		|	ВЫБОР
		|		КОГДА ШапкаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионера)
		|				ИЛИ ВТСводнаяТаблицаЗапасыИтог.ВидОперации = 1
		|			ТОГДА ШапкаДокумента.Договор.ДоговорУслугиКомиссионногоВознаграждения
		|		КОГДА ШапкаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионераОВозвратах)
		|				ИЛИ ВТСводнаяТаблицаЗапасыИтог.ВидОперации = 2
		...
		|									ТОГДА ВТСводнаяТаблицаЗапасыИтог.СуммаВознаграждения
		|								ИНАЧЕ ВТСводнаяТаблицаЗапасыИтог.СуммаВознаграждения + ВТСводнаяТаблицаЗапасыИтог.СуммаНДСВознаграждения
		|							КОНЕЦ * ВЫБОР
		|								КОГДА ШапкаДокумента.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
		|									ТОГДА КурсыРегВалют.Курс * ШапкаДокумента.Кратность / (ШапкаДокумента.Курс * ЕСТЬNULL(КурсыРегВалют.Кратность, 1))
		|								ИНАЧЕ 1
		...
		|				ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ВЫБОР
		|								КОГДА ШапкаДокумента.СуммаВключаетНДС
		|									ТОГДА ВТСводнаяТаблицаЗапасыИтог.Сумма - ВТСводнаяТаблицаЗапасыИтог.СуммаВознаграждения
		|								ИНАЧЕ ВТСводнаяТаблицаЗапасыИтог.Сумма - (ВТСводнаяТаблицаЗапасыИтог.СуммаВознаграждения + ВТСводнаяТаблицаЗапасыИтог.СуммаНДСВознаграждения)
		|							КОНЕЦ * ВЫБОР
		|								КОГДА ШапкаДокумента.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
		|									ТОГДА КурсыРегВалют.Курс * ШапкаДокумента.Кратность / (ШапкаДокумента.Курс * ЕСТЬNULL(КурсыРегВалют.Кратность, 1))
		|								ИНАЧЕ 1
		...
		|	ВТСводнаяТаблицаЗапасыИтог.СтавкаНДС КАК СтавкаНДС,
		|	СУММА(ВЫБОР
		|			КОГДА ШапкаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионера)
		|					ИЛИ ВТСводнаяТаблицаЗапасыИтог.ВидОперации = 1
		|				ТОГДА ЕСТЬNULL(ВТСводнаяТаблицаЗапасыИтог.СуммаНДСВознаграждения, 0)
		|			КОГДА ШапкаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетКомиссионераОВозвратах)
		|					ИЛИ ВТСводнаяТаблицаЗапасыИтог.ВидОперации = 2
		|				ТОГДА ЕСТЬNULL(ВТСводнаяТаблицаЗапасыИтог.СуммаНДС, 0) - ЕСТЬNULL(ВТСводнаяТаблицаЗапасыИтог.СуммаНДСВознаграждения, 0)
		|		КОНЕЦ) КАК СуммаНДС
		|ИЗ
		...
		|			И ШапкаДокумента.Организация = ВложенныйЗапрос.Владелец,
		|	Константа.НациональнаяВалюта КАК КонстантаНациональнаяВалюта,
		|	ВТСводнаяТаблицаЗапасыИтог КАК ВТСводнаяТаблицаЗапасыИтог
		|ГДЕ
		|	ШапкаДокумента.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТСводнаяТаблицаЗапасыИтог.СтавкаНДС,
		|	ШапкаДокумента.Организация,
		|	ШапкаДокумента.Контрагент,
		|	ШапкаДокумента.ВалютаДокумента,
		|	ВТСводнаяТаблицаЗапасыИтог.ЗаказПокупателя,
		|	ВЫБОР
		|		КОГДА ШапкаДокумента.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
		|			ТОГДА ШапкаДокумента.Курс
		|		ИНАЧЕ КурсыВалютыРасчетов.Курс
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументСсылкаНачислениеНалогов) Экспорт
	...
	|	ТаблицаДокументаНалоги.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ТаблицаДокументаНалоги.СрокУплаты КАК СрокУплатыНалога
	|ИЗ
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.СрокУплатыНалога > '20230101' Тогда
			ЭтотОбъект.ВидНалога = Справочники.ВидыНалогов.ЕдиныйНалоговыйПлатеж;
			ЭтотОбъект.Контрагент = ДанныеГосударственныхОрганов.АдминистраторНалогаОрганизации(ЭтотОбъект.ВидНалога, ЭтотОбъект.Организация, Выборка.СрокУплатыНалога);
			ЭтотОбъект.СрокУплаты = РегламентированнаяОтчетностьУСН.ОпределитьСрокПлатежаПоЕНП(Выборка.СрокУплатыНалога);  
			ПоказателиНалоговогоПериода = ПлатежиВБюджетКлиентСервер.ПоказателиНалоговогоПериода(ЭтотОбъект.Организация, ЭтотОбъект.ВидНалога, Выборка.СрокУплатыНалога);
				ЭтотОбъект.НазначениеПлатежа = Справочники.ВидыНалогов.НазначениеПлатежа(
				ЭтотОбъект.ВидНалога,,
				ЭтотОбъект.Организация,
				Макс(ПоказателиНалоговогоПериода.Период, Выборка.СрокУплатыНалога),
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если РасшифровкаПлатежа.Количество() = 0 Тогда
		РасшифровкаПлатежа.Добавить();
		РасшифровкаПлатежа[0].СуммаПлатежа = СуммаДокумента;
		РасшифровкаПлатежа[0].СуммаРасчетов = СуммаДокумента;
		РасшифровкаПлатежа[0].СпособЗачета =
			РасчетыРаботаСФормамиВызовСервера.СпособРазнесенияОплатыДляДоговора(РасшифровкаПлатежа[0].Договор, Контрагент);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику
		...
		Если Контрагент.ВестиРасчетыПоДокументам
			...
			Для Каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				...
				ИначеЕсли УчитыватьВНУ И СтрокаРасшифровкаПлатежа.ПризнакАванса Тогда
					// Если возврат аванса по патенту, а организация на УСН, то флаг "УСН" должен быть сброшен.
					Если ТипЗнч(СтрокаРасшифровкаПлатежа.Документ) = Тип("ДокументСсылка.ПоступлениеВКассу")
						Или ТипЗнч(СтрокаРасшифровкаПлатежа.Документ) = Тип("ДокументСсылка.ПоступлениеНаСчет") Тогда
						
						УСНИПатентДокументаВРасшифровке = УправлениеНебольшойФирмойВызовСервера.ЗначенияРеквизитовОбъекта(
							СтрокаРасшифровкаПлатежа.Документ, "ПоложениеНастроекНалоговогоУчета, УчитыватьВНУ, Патент");
							
						УстановленУчитыватьВНУ = ?(ПоложениеНастроекНалоговогоУчета = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти,
													СтрокаРасшифровкаПлатежа.УчитыватьВНУ, УчитыватьВНУ);
													
						Если УстановленУчитыватьВНУ
							И УСНИПатентДокументаВРасшифровке.ПоложениеНастроекНалоговогоУчета <> Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти
							И НЕ УСНИПатентДокументаВРасшифровке.УчитыватьВНУ И НЕ УСНИПатентДокументаВРасшифровке.Патент.Пустая() Тогда
							ТекстСообщения = СтрШаблон(НСтр(
								"ru = 'В строке %1 списка ""Расшифровка платежа"" указан документ с патентом.
								|Чтобы вернуть деньги по патенту сбросьте флаг ""Налоговый учет (УСН)"" в шапке.'"),
								СтрокаРасшифровкаПлатежа.НомерСтроки);
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата Тогда
		...
		Если (СуммаПлатежа <> СуммаДокумента И СуммаПлатежаРегУчет = 0)
			...
			Если СуммаПлатежа <> СуммаДокумента И (СуммаПлатежаРегУчет = 0 ИЛИ СуммаПлатежа <> 0) Тогда
				ТекстСообщения = СтрШаблон(НСтр(
			...
			Если СуммаПлатежаРегУчет <> СуммаДокумента И СуммаПлатежаРегУчет <> 0 Тогда
				...
				СуммаДокумента, СуммаПлатежаРегУчет, СокрЛП(ВалютаДенежныхСредств));
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "СуммаДокумента", , Отказ);
			КонецЕсли;
		КонецЕсли;
		...
		// БКЗБ
		Если ВыплатаРегламентированнойЗаработнойПлаты.Количество() > 0 И ВалютаДенежныхСредств <> Константы.НациональнаяВалюта.Получить()
			И Справочники.Организации.НужноПоказыватьЭлементыБЗКБДляОрганизации(ЭтотОбъект.Организация) Тогда
			ТекстСообщения = НСтр(
				"ru = 'Выплата регламентированной зарплаты не поддерживается для валюты, которая отличается от национальной.'");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для каждого СтрокаТЧ Из РасшифровкаПлатежа Цикл
		...
		Если СтрокаТЧ.СуммаПлатежа > 0 И СтрокаТЧ.СуммаРасчетов = 0 Тогда
			СтрокаТЧ.СуммаРасчетов = СтрокаТЧ.СуммаПлатежа;
		КонецЕсли;	
	КонецЦикла;
	...
	ИначеЕсли ФормыДокументовДеньгиКлиентСервер.ВидОперацииРасшифровкаСНалоговымУчетом(ВидОперации)
		И ПоложениеНастроекНалоговогоУчета = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
	...
	Если ЗначениеЗаполнено(ПлатежноеПоручениеОснование)
		И ТипЗнч(ПлатежноеПоручениеОснование) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СотрудникиУНФ.ОтразитьНДФЛРасчетыНалоговыхАгентовСБюджетом(ДополнительныеСвойства, Движения, Отказ);
	ПроведениеДокументовУНФ.ОтразитьДвижения("ЗакупкиДляКУДиР", ТаблицыДляДвижений, Движения, Отказ);
	ПроведениеДокументовУНФ.ОтразитьДвижения("РасчетыНалогоплательщиковСБюджетомПоНДФЛУНФ", ТаблицыДляДвижений, Движения, Отказ);
	
	//Единый налоговый счет
	ПроведениеДокументовУНФ.ОтразитьДвижения("РасчетыПоЕдиномуНалоговомуСчету", ТаблицыДляДвижений, Движения, Отказ);
	...
	// БЗКБ
	Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата
		И ВыплатаРегламентированнойЗаработнойПлаты.Количество() > 0
		И Справочники.Организации.НужноПоказыватьЭлементыБЗКБДляОрганизации(ЭтотОбъект.Организация) Тогда
		Ведомости	= ОбщегоНазначенияКлиентСервер.СвернутьМассив(ВыплатаРегламентированнойЗаработнойПлаты.ВыгрузитьКолонку("Ведомость"));
		ФизЛица = Новый Массив;
		Для Каждого СтрокаВыплаты Из ВыплатаРегламентированнойЗаработнойПлаты Цикл
			Если ЗначениеЗаполнено(СтрокаВыплаты.ФизическоеЛицо) Тогда
				ФизЛица.Добавить(СтрокаВыплаты.ФизическоеЛицо);
			КонецЕсли;
		КонецЦикла;
		...
		ЗарплатаКадры.ЗарегистрироватьОплатуВедомостей(
			...
			Организация,
			Ведомости,
			?(ФизЛица.Количество() > 0, ФизЛица, Неопределено), 
			Дата, // Дата платежа
			Отказ);
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Налоги Тогда
		Если ЭтотОбъект.ВидНалога = Справочники.ВидыНалогов.ФССТравматизмСотрудники
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "ИспользуетсяОтчетность") Тогда
			
			ПеречисленныйНалог = Новый ТаблицаЗначений;
			ПеречисленныйНалог.Колонки.Добавить("МесяцНалоговогоПериода", Новый ОписаниеТипов("Дата"));
			ПеречисленныйНалог.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(15, 2)));
			ПеречисленныйНалог.Колонки.Добавить("ПлатежноеПоручениеНомер", Новый ОписаниеТипов("Строка"));
			ПеречисленныйНалог.Колонки.Добавить("ПлатежноеПоручениеДата", Новый ОписаниеТипов("Дата"));
			СтрокаПеречисленныйНалог = ПеречисленныйНалог.Добавить();
			СтрокаПеречисленныйНалог.Сумма = ЭтотОбъект.СуммаДокумента;
			СтрокаПеречисленныйНалог.МесяцНалоговогоПериода = ЭтотОбъект.СрокУплаты;
			
			Ставка = Неопределено;
			ФизическиеЛица = Неопределено;
			РегистрацияВНалоговомОргане = Неопределено;
			ЗаписыватьДвижения = Ложь;
			ДатаОперации = ЭтотОбъект.СрокУплаты;
			...
			ЗаписыватьДвижения = Ложь;
			РасчетыСФондом = Новый ТаблицаЗначений;
			РасчетыСФондом.Колонки.Добавить("Период", Новый ОписаниеТипов("Дата"));
			РасчетыСФондом.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(15, 2)));
			РасчетыСФондом.Колонки.Добавить("ВидОбязательногоСтрахованияСотрудников", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыОбязательногоСтрахованияСотрудников"));
			РасчетыСФондом.Колонки.Добавить("ЭтоСтраховыеВзносы", Новый ОписаниеТипов("Булево"));
			РасчетыСФондом.Колонки.Добавить("ГодЗадолженностиПоЕСН", Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(4, 0)));
			РасчетыСФондом.Колонки.Добавить("ПлатежноеПоручениеНомер", Новый ОписаниеТипов("Строка"));
			РасчетыСФондом.Колонки.Добавить("ПлатежноеПоручениеДата", Новый ОписаниеТипов("Дата"));
			
			СтрокаРасчетыСФондом = РасчетыСФондом.Добавить();
			СтрокаРасчетыСФондом.Период = ЭтотОбъект.СрокУплаты;
			СтрокаРасчетыСФондом.Сумма = ЭтотОбъект.СуммаДокумента;
			СтрокаРасчетыСФондом.ВидОбязательногоСтрахованияСотрудников = Перечисления.ВидыОбязательногоСтрахованияСотрудников.ФССНесчастныеСлучаи;
			СтрокаРасчетыСФондом.ЭтоСтраховыеВзносы = Истина;
			СтрокаРасчетыСФондом.ГодЗадолженностиПоЕСН = Год(ЭтотОбъект.СрокУплаты);
			Если ЗначениеЗаполнено(ДокументОснование) И ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
				РеквизитыПлатежногоПоручения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "Дата,Номер");
				СтрокаРасчетыСФондом.ПлатежноеПоручениеНомер = РеквизитыПлатежногоПоручения.Номер;
				СтрокаРасчетыСФондом.ПлатежноеПоручениеДата = РеквизитыПлатежногоПоручения.Дата;
			КонецЕсли;
			
			УчетСтраховыхВзносов.СформироватьУплатуВзносовВФонд(Движения, Отказ, Организация, РасчетыСФондом, ЗаписыватьДвижения);
		КонецЕсли;
	КонецЕсли;
	// Конец БЗКБ
	...
	Если Не Отказ Тогда
		СчетаФактурыУНФ.ПровестиПодчиненныеСчетаФактуры(Ссылка, Номер, Дата, ДополнительныеСвойства, Истина);
	КонецЕсли;
