...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) 
		И (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты")
	...
	НаборСвойств_Документ_РасходСоСчета = УправлениеСвойствами.НаборСвойствПоИмени("Документ_РасходСоСчета");
	КэшЗначений.Вставить("НаборСвойств_Документ_РасходСоСчета", НаборСвойств_Документ_РасходСоСчета);
...
Процедура ДокументОснованиеНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки,
	...
	ФормыДокументовДеньгиКлиент.ДокументОснованиеНадписьОбработкаНавигационнойСсылки(ЭтотОбъект, Элемент,
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если Объект.РасшифровкаПлатежа.Количество() = 1 Тогда 
		...
		Объект.РасшифровкаПлатежа[0].СтавкаНДС = РасчетыРаботаСФормамиВызовСервера.СтавкаНДСПоОрганизацииНалогообложениеНДСИСпособуЗаполнения(
			Объект.Организация, Объект.Дата, ДанныеКонтрагента.СпособЗаполненияСтавкиНДС);
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ПрочиеРасчеты") Тогда
		...
		Объект.Корреспонденция = ДанныеКонтрагента.КорреспонденцияПрочихРасчетов;
...
Процедура УчитыватьВНУПриИзменении(Элемент)
	...
	Если ФормыДокументовДеньгиКлиентСервер.ВидОперацииРасшифровкаСНалоговымУчетом(Объект.ВидОперации) Тогда
		Для Каждого Строка Из Объект.РасшифровкаПлатежа Цикл
			...
			Если Строка.УчитыватьВНУ Тогда
				Строка.Патент = ПредопределенноеЗначение("Справочник.Патенты.ПустаяСсылка");
			КонецЕсли;
...
Процедура РасшифровкаПлатежаСчетНаОплатуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		ОповещениеОЗакрытииФормыВыбора = Новый ОписаниеОповещения("РасшифровкаПлатежаСчетНаОплатуНачалоВыбораЗавершение", ЭтотОбъект);
		ПараметрыДляФормыВыбора.Вставить("РежимВыбора", Истина);
...
Процедура Подбор(Команда)
	...
	ПараметрОповещения = Новый Структура("АдресРасшифровкаПлатежаВХранилище", АдресРасшифровкаПлатежаВХранилище);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПодборЗавершение", ЭтотОбъект, ПараметрОповещения);
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", КэшЗначений.НаборСвойств_Документ_РасходСоСчета);
...
Функция ПолучитьДанныеБанковскийСчетПриИзменении(Дата, БанковскийСчет, СчетКонтрагента)
	...
	Результат.Вставить("ВалютаКурсКратность", РаботаСКурсамиВалют.ПолучитьКурсВалюты(
		БанковскийСчет.ВалютаДенежныхСредств, Дата));
	Результат.Вставить("ВалютаДенежныхСредств", БанковскийСчет.ВалютаДенежныхСредств);
	Результат.Вставить("СчетКонтрагента", Неопределено);
	Если ЗначениеЗаполнено(СчетКонтрагента) И СчетКонтрагента.ВалютаДенежныхСредств
		= БанковскийСчет.ВалютаДенежныхСредств Тогда
		Результат.СчетКонтрагента = СчетКонтрагента;
	КонецЕсли;
	
	ОбновитьВидимостьИОстаткиДС(Объект.БанковскийСчет);
	
	УстановитьЗаголовкиКолонокТабличныхЧастей(Результат.ВалютаДенежныхСредств);
...
Процедура ОрганизацияПриИзмененииНаСервере(ДанныеВалюты)
	...
	Если Объект.БанковскийСчет.Владелец <> Объект.Организация Тогда
	...
	Если ВалютаДенежныхСредств <> Объект.СчетКонтрагента.ВалютаДенежныхСредств Тогда
	...
	Если Объект.РасшифровкаПлатежа.Количество() > 0 Тогда
		Для Каждого ТекущаяСтрока Из Объект.РасшифровкаПлатежа Цикл
			...
			ИначеЕсли ТекущаяСтрока.Договор.Пустая() Тогда
				ТекущаяСтрока.Договор = ДоговорПоУмолчанию;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ДанныеКонтрагентаДляИзменения(Знач Контрагент, Знач Организация, Знач Дата)
	...
	Результат.Вставить("КорреспонденцияПрочихРасчетов", Контрагент.СчетУчетаРасчетовСПоставщиком);
	// Конец Прочие расчеты
	Результат.Вставить("СпособЗаполненияСтавкиНДС", Справочники.ДоговорыКонтрагентов.СпособЗаполненияСтавкиНДСПоДоговору(
			ДоговорПоУмолчанию));
	
	ФормыДокументовДеньги.УстановитьВидимостьРеквизитовРасчетов(ЭтотОбъект);
	...
	Если ЗначениеЗаполнено(Контрагент.СтатьяДДСПоУмолчанию) Тогда
		Результат.Вставить("УчитыватьВНУ", РегламентированнаяОтчетностьУСН.НужноУчитыватьВНУ(
			Контрагент.СтатьяДДСПоУмолчанию, Объект.ВидОперации, Организация, Дата));
	Иначе
	...
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаЗаймаСотруднику") И
		...
		Если ЗначениеЗаполнено(Объект.Контрагент)
			...
			Если НЕ ЗначениеЗаполнено(Объект.РасшифровкаПлатежа[0].Договор) Тогда
				...
				СписокВидовДоговоров = МенеджерСправочника.ПолучитьСписокВидовДоговораДляДокумента(Объект.Ссылка,
					Объект.ВидОперации);
				Объект.РасшифровкаПлатежа[0].Договор = МенеджерСправочника.ПоКонтрагентуПоОрганизации(
...
Процедура ЗаполнитьРеквизитыПлатежаВБюджетПоУмолчанию()
	...
	Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Налоги") Тогда
...
Процедура УстановитьВидимостьПечатиЧека()
	...
	ВидОперацииПодразумеваетЧек = Объект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Покупателю
...
Процедура УстановитьВидимостьПризнакаСпособаРасчета(Знач НапечататьЧекВидимость = неопределено)
	...
	Элементы.РасшифровкаПлатежаПризнакСпособаРасчета.Видимость = НапечататьЧекВидимость
		И (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты")
		ИЛИ Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВозвратПродажиВКредит")
		ИЛИ Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Покупателю"));
...
Процедура ЗаполнитьПоДоговоруКредита(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьПоДоговоруКредитаЗавершение", ЭтотОбъект);
	...
	Если Результат = КодВозвратаДиалога.OK Тогда
		...
		Для Каждого ТекущаяСтрока Из Объект.ЭквайринговыеОперации Цикл
			...
			ТекущаяСтрока.НалоговыйУчет = РаботаСФормойДокументаКлиентСервер.ПредставлениеНалоговогоУчетаСтроки(
				ТекущаяСтрока, ВидимостьУчитыватьВНУ);
...
Процедура ПодборЭквайринговыхОпераций(Команда)
	...
	Если Объект.ПоложениеЭквайринговогоТерминала = ПредопределенноеЗначение(
		"Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти") Тогда
		ПараметрыПодбора.Удалить("ЭквайринговыйТерминал");
	КонецЕсли;
	
	ПараметрОповещения = Новый Структура("АдресЭквайринговыеОперацииВХранилище", АдресЭквайринговыеОперацииВХранилище);
	ОписаниеОповещения = Новый ОписаниеОповещения("ЭквайрингПодборЗавершение", ЭтотОбъект, ПараметрОповещения);
	ОткрытьФорму("ОбщаяФорма.ФормаПодбораДолговПоЭквайрингу", ПараметрыПодбора, , , , , ОписаниеОповещения);
...
Процедура ПриЗавершенииВыбораНастроекНалогообложения(СтруктураРезультата, ДополнительныеПараметры) Экспорт
	...
	Если СтруктураРезультата.КодВозвратаДиалога = КодВозвратаДиалога.OK И СтруктураРезультата.БылиВнесеныИзменения Тогда
		...
		ВыбраннаяСтрока.НалоговыйУчет = РаботаСФормойДокументаКлиентСервер.ПредставлениеНалоговогоУчетаСтроки(
			ВыбраннаяСтрока, ВидимостьУчитыватьВНУ);
...
Процедура ЭквайринговыеОперацииЭквайринговыйТерминалПриИзменении(Элемент)
	...
	Если НалоговыйУчетТерминала.СобственныеНастройкиНалоговогоУчета Тогда
		...
		ВыбраннаяСтрока.НалоговыйУчет = РаботаСФормойДокументаКлиентСервер.ПредставлениеНалоговогоУчетаСтроки(
			СтрокаЭквайринга, ВидимостьУчитыватьВНУ);
...
Процедура ЭквайринговыеОперацииПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		СтрокаЭквайринг.НалоговыйУчет = РаботаСФормойДокументаКлиентСервер.ПредставлениеНалоговогоУчетаСтроки(
			СтрокаЭквайринг, ВидимостьУчитыватьВНУ);
	КонецЕсли; 
...
Процедура ЗаполнитьНалоговыйУчетСУчетомРасшифровки()
	...
	Если Объект.РасшифровкаПлатежа.Количество() > 0 Тогда
		ЗаполнитьЗначенияСвойств(Объект, Объект.РасшифровкаПлатежа[0], "УчитыватьВНУ");
	КонецЕсли;
