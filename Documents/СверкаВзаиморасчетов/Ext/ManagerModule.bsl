...
Функция ОписаниеРасчетногоДокументаОрганизации(ДокументСсылка, ДанныеДокумента, НомерДокумента, ДатаДокумента) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПриходнаяНакладная") 
		...
		ДобавитьКОписанию = " (возврат товаров выкупленных комиссионером)";
...
Процедура ЗаполнитьСведенияКонтрагентаПоДаннымОрганизации(ДанныеОрганизации, ДанныеКонтрагента, ОрганизациюВПредставление = Ложь, УказатьСчетФактуру = Ложь, Организация = Неопределено) Экспорт
	...
	Для каждого СтрокаОрганизации Из ДанныеОрганизации Цикл
		...
		СтрокаКонтрагента.ДатаОперации = СтрокаОрганизации.ДатаОперации;
		СтрокаКонтрагента.НаименованиеОперации = СтрокаОрганизации.НаименованиеОперации;
...
Функция ТекстЗапросаДляФормированияЭлектронногоДокументаДанныеОтправителя() Экспорт
	...
	|	АктСверкиВзаиморасчетовПоДаннымОрганизации.РасчетныйДокумент КАК РасчетныйДокумент,
	|	АктСверкиВзаиморасчетовПоДаннымОрганизации.РасчетныйДокумент.Организация КАК Организация,
...
Функция СформироватьПечатнуюФормуАктБезРасхождений(ОписаниеПечатнойФормы, ДокументПечати, ОбъектыПечати, ИспользоватьФаксимиле)
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Если Шапка.СортироватьПоДоговорам Тогда
			...
			ТаблицаДоговоров = ВыборкаДокументов.ДоговорыКонтрагентов.Выгрузить();
			
			Для Каждого СтрокаДанных Из ТаблицаДоговоров Цикл
				
				Договор = СтрокаДанных.Договор;
				ДоговорУжеОбработан = ДанныеОрганизации.Найти(Договор, "Договор") <> Неопределено;
				
				Если ДоговорУжеОбработан Тогда
					Продолжить;
				КонецЕсли;
				...
				ОстатокНаКонец = НачальноеСальдоПоДоговору;
			
				ОбластьМакетаДоговорИтоги.Параметры.СуммаДолгОрганизации = 0;
				ОбластьМакетаДоговорИтоги.Параметры.СуммаДолгКонтрагента = 0;
				ОбластьМакетаДоговорИтоги.Параметры.ДолгОрганизацииДоговорКонец = Макс(ОстатокНаКонец, 0);
				ОбластьМакетаДоговорИтоги.Параметры.ДолгКонтрагентаДоговорКонец = Макс(-ОстатокНаКонец, 0);
...
Процедура СформироватьПечатнуюФормуАктБезРасхожденийФрагмент(ПередаваемыеПараметры)
	...
	ПередаваемыеПараметры.СуммаДолгКонтрагента = СуммаДолгКонтрагента;
	ПередаваемыеПараметры.СуммаДолгОрганизации = СуммаДолгОрганизации;
	ПередаваемыеПараметры.НачальноеСальдоПоДоговору = НачальноеСальдоПоДоговору;
