...
Функция ДанныеЗаполняемыеИзПервичногоДокумента()
	...
	Если ДанныеВходящегоЗапроса <> Неопределено Тогда
		...
		КонецЕсли;
		Если ИспользоватьПараметрыРасчетаИзВходящегоЗапроса(ДанныеВходящегоЗапроса, РеквизитыПервичногоДокумента) Тогда
...
Процедура ЗаполнитьРеквизитыРасчетаСреднегоЗаработка(Реквизиты, ДанныеДляРасчетаСреднегоЗаработка)
	...
	Если Реквизиты.ВидПособия = Перечисления.ПособияНазначаемыеФСС.ЕжемесячноеПособиеПоУходуЗаРебенком Тогда
		ВидЗаработка = Перечисления.СпособыУчетаГодовогоЗаработкаСФР.ВсеСтрахователиЗаТекущийПериодРаботы;
	ИначеЕсли ПараметрыРасчета.УчитыватьЗаработокПредыдущихСтрахователей Тогда
...
Функция ИспользоватьПараметрыРасчетаИзВходящегоЗапроса(ДанныеВходящегоЗапроса, РеквизитыПервичногоДокумента)
	// Параметры расчета из входящего запроса используются только в том случае, 
	// если во входящем запросе есть такие данные
	// и годы расчета во входящем запросе удотворяет волеизъявлению застрахованного лица
	// которое отражено в данных больничного (или другого первичного документа).
	Возврат ПриоритетУДанныхВходящегоЗапроса
		...
		И ?(РеквизитыПервичногоДокумента.Свойство("ПервыйРасчетныйГод")
				...
				И ЗначениеЗаполнено(РеквизитыПервичногоДокумента.ВторойРасчетныйГод),
			ДанныеВходящегоЗапроса.ПервыйРасчетныйГод = РеквизитыПервичногоДокумента.ПервыйРасчетныйГод
				И ДанныеВходящегоЗапроса.ВторойРасчетныйГод = РеквизитыПервичногоДокумента.ВторойРасчетныйГод,
			Истина);
КонецФункции
...
Функция ЗаполнитьДатуНачалаСобытия(ПараметрыФиксации)
	РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента();
	Если Не ЗначениеЗаполнено(ОбщегоНазначенияБЗК.ЗначениеСвойства(РеквизитыПервичногоДокумента, "ДатаНачалаСобытия")) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗаполнитьВерсиюСпецификации(ПараметрыФиксации)
	...
	// Правила подбора версий спецификации исходя из совместимости спецификаций:
	// - Когда версия запроса < 2.34,  тогда версия ответа = <последняя меньше чем 2.34>.
	// - Когда версия запроса >= 2.34, тогда версия ответа = последняя.
	Если ЗначениеЗаполнено(ВходящийЗапрос) Тогда
		РеквизитыЗапроса = ДанныеВходящегоЗапроса();
		Если РеквизитыЗапроса.ВерсияСпецификации = "" Тогда // Версия запроса < 2.34.
			Реквизиты.ВерсияСпецификации = ""; // Версия ответа = <последняя меньше чем 2.34>.
		КонецЕсли;
	КонецЕсли;
...
Процедура ДополнитьСтруктуруДаннымиВходящегоЗапроса(Реквизиты, РеквизитыЗапроса, ИменаПолей, УжеЗаполненныеДанные = Неопределено)
