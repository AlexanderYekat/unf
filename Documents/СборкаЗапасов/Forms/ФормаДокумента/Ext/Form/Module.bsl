...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаборСвойств_Документ_СборкаЗапасов = УправлениеСвойствами.НаборСвойствПоИмени("Документ_СборкаЗапасов");
	КэшЗначений.Вставить("НаборСвойств_Документ_СборкаЗапасов", НаборСвойств_Документ_СборкаЗапасов);
	...
	МенеджерОборудованияУНФ.УстановитьВидимостьЭлементовПоДоступномуПодключаемомуОборудованию(Элементы,
		"ЗапасыЗагрузитьДанныеИзТСД, ПродукцияЗагрузитьДанныеИзТСД",
		"ЗапасыПолучитьВес");
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ОбновитьБракованныеЭтапыНаФорме(ЭтотОбъект);
	КонецЕсли;
	...
	РаботаСФормойДокумента.НастроитьВидимостьГруппыИнформации(ЭтотОбъект, "ГруппаИнформацияПоНовымРеквизитам",
		"ПоказыватьИнформациюПоНовойСхемеРеквизитовПечати");  
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ОценкаПроизводительностиКлиент.ЗамерВремени("Проведение"
			...
			Если НЕ НулеваяСтоимость() Тогда
				ПараметрыОтбора = Новый Структура;
				ПараметрыОтбора.Вставить("Цена", 0);
				НайденныеСтроки = Объект.Отходы.НайтиСтроки(ПараметрыОтбора);
				...
				Если Отказ Тогда
					...
					Сообщение.Текст = СтрШаблон(НСтр("ru = 'Для отхода ""%1"" не найдена цена по виду цен ""%2""'"),Строка.Номенклатура,ЦеныИВалюта);
					Сообщение.Сообщить();	
					КонецЦикла; 
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ПодборПроизведен" 
		...
		Для Каждого НомерСтроки Из МассивНомеровИдентичныхСтрок Цикл
			...
			ИначеЕсли ИмяТабличнойЧасти="Отходы" Тогда
				РассчитатьСуммуВСтрокеТабличнойЧасти("Отходы", СтрокаТабличнойЧасти);
			КонецЕсли;
...
&НаСервере
Функция НулеваяСтоимость()
	Возврат Объект.ВариантУчетаСтоимостиОтходов = ПредопределенноеЗначение("Перечисление.ВариантУчетаСтоимостиОтходов.Нулевая")
КонецФункции
...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	Объект.СкладскойОрдерПродукция = Неопределено;
	Объект.СкладскойОрдерЗапасы = Неопределено;
...
Процедура СтруктурнаяЕдиницаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница) Тогда
		...
		Если ЗначениеЗаполнено(СтруктураДанные.СтруктурнаяЕдиницаПродукции) Тогда
			...
			Элементы.ЯчейкаПродукцииСборка.Доступность = ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаПродукции);
			Элементы.ЯчейкаПродукцииРазборка.Доступность = ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаПродукции);
			
		Иначе
			
			Объект.СтруктурнаяЕдиницаПродукции = Объект.СтруктурнаяЕдиница;
			Объект.ЯчейкаПродукции = Объект.Ячейка;
			Элементы.ЯчейкаПродукцииСборка.Доступность = ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница);
			Элементы.ЯчейкаПродукцииРазборка.Доступность = ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница);
		...
		Если ЗначениеЗаполнено(СтруктураДанные.СтруктурнаяЕдиницаЗапасов) Тогда
			...
			Элементы.ЯчейкаЗапасовСборка.Доступность = ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаЗапасов);
			Элементы.ЯчейкаЗапасовРазборка.Доступность = ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаЗапасов);
			
		Иначе
			
			СтруктурнаяЕдиницаЗапасов = Объект.СтруктурнаяЕдиница;
			ЯчейкаЗапасов = Объект.Ячейка;
			Элементы.ЯчейкаЗапасовСборка.Доступность = ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница);
			Элементы.ЯчейкаЗапасовРазборка.Доступность = ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница);
		...
		Если ЗначениеЗаполнено(СтруктураДанные.СтруктурнаяЕдиницаОтходов) Тогда
			...
			Элементы.ЯчейкаОтходов.Доступность = ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаОтходов);
			
		Иначе
			
			Объект.СтруктурнаяЕдиницаОтходов = Объект.СтруктурнаяЕдиница;
			Объект.ЯчейкаОтходов = Объект.Ячейка;
			Элементы.ЯчейкаОтходов.Доступность = ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница);
	...
	Если ТипСтруктурнойЕдиницы = ПредопределенноеЗначение("Перечисление.ТипыСтруктурныхЕдиниц.Склад") Тогда
		Объект.ВариантУчетаСтоимостиОтходов = ПредопределенноеЗначение("Перечисление.ВариантУчетаСтоимостиОтходов.Нулевая"); 
		Элементы.ВариантУчетаСтоимостиОтходов.Видимость = Ложь;
		ИзменениеВариантаУчетаСтоимостиОтходов();
	ИначеЕсли ТипСтруктурнойЕдиницы = ПредопределенноеЗначение("Перечисление.ТипыСтруктурныхЕдиниц.Подразделение") Тогда
		Объект.ВариантУчетаСтоимостиОтходов = ПредопределенноеЗначение("Перечисление.ВариантУчетаСтоимостиОтходов.Нулевая"); 
		Элементы.ВариантУчетаСтоимостиОтходов.Видимость = Истина;
		ИзменениеВариантаУчетаСтоимостиОтходов();
	КонецЕсли;
КонецПроцедуры // СтруктурнаяЕдиницаПриИзменении()

&НаКлиенте
Процедура ИзменениеВариантаУчетаСтоимостиОтходов()
	Если ФактическаяЦенаКорректна() Тогда
			ВариантУчетаСтоимостиОтходовПриИзмененииНаСервере();	
			ЦенообразованиеКлиент.ПерезаполнитьЦеныТабличнойЧастиПоВидуЦен(ЭтотОбъект, "Отходы", Ложь);
			ЦеныИВалюта = ЦеныИВалютаКлиентСервер.ТекстНадписиЦеныИВалюта(Объект, ВалютаРасчетов, КурсНациональнаяВалюта,
			Ложь);
		Иначе
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Не заполнен вид цен учета отходов'");
			Сообщение.Сообщить();
		КонецЕсли;
КонецПроцедуры
...
Процедура ПродукцияПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПоискПоШтрихКоду") И ДополнительныеПараметры.ЭтоПоискПоШтрихКоду Тогда
		...
		ДополнительныеПараметры.Вставить("ИмяТабличнойЧастиСерииНоменклатуры", "СерииНоменклатурыПродукция");
...
Процедура ПродукцияНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество)
	...
	Если ДоступенОрдерныйУчет Тогда
		...
		Если Не СтруктураДанные.УказыватьДокументПоступления = УчетОстатковПоСкладскимОрдерамКОтгрузке Тогда
			Если СтруктураДанные.УказыватьДокументПоступления Тогда
				УчетОстатковПоСкладскимОрдерамКОтгрузке = СтруктураДанные.УказыватьДокументПоступления;
			Иначе
				УчетОстатковПоСкладскимОрдерамКОтгрузке = УчетОстатковВРазрезеСкладскихОрдеров(Ложь);
			КонецЕсли;
			ПриИзмененииВидаУчетаСклада(ЭтотОбъект);
...
&НаКлиенте
Процедура ПродукцияНомерГТДПриРНПТПриИзменении(Элемент)
...
Процедура СтруктурнаяЕдиницаПродукцииРазборкаПриИзменении(Элемент)
	...
	Элементы.ЯчейкаПродукцииРазборка.Доступность = (ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаЗапасов));
...
&НаКлиенте
Процедура ПродукцияБракНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ТекущаяСтрока = Элементы.Продукция.ТекущиеДанные;
	Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.Спецификация)
		ИЛИ (НЕ ЗначениеЗаполнено(Объект.ЗаказНаПроизводство) И НЕ ПроизводствоПодЗаказ(ТекущаяСтрока)) Тогда
	...
	СтруктураОтбора = Новый Структура;
	СтруктураОтбора.Вставить("КлючСвязи", КлючСвязи);
	Строки = Объект.БракованныеЭтапы.НайтиСтроки(СтруктураОтбора);
	Для каждого СтрокаТабличнойЧасти Из Строки Цикл
		МассивЭтапов.Добавить(СтрокаТабличнойЧасти.Этап);
	КонецЦикла;
	...
	СтруктураОткрытия.Вставить("ВыполненныеЭтапы", МассивЭтапов);
	СтруктураОткрытия.Вставить("Спецификация", ТекущаяСтрока.Спецификация);
	СтруктураОткрытия.Вставить("Номенклатура", ТекущаяСтрока.Номенклатура);
	СтруктураОткрытия.Вставить("Характеристика", ТекущаяСтрока.Характеристика);
	СтруктураОткрытия.Вставить("Регистратор", Объект.Ссылка);
	...
	СтруктураОткрытия.Вставить("Заказ", ТекущаяСтрока.ЗаказПокупателя);
	Иначе
	СтруктураОткрытия.Вставить("Заказ", ?(ЗначениеЗаполнено(Объект.ЗаказПокупателя),Объект.ЗаказПокупателя,Объект.ЗаказНаПроизводство));
	КонецЕсли;	
	ОткрытьФорму("Документ.СборкаЗапасов.Форма.БракованныеЭтапы", СтруктураОткрытия, Элемент, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
&НаКлиенте
Процедура ПродукцияБракОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Объект.БракованныеЭтапы.Очистить();
	
	Для каждого Этап Из ВыбранноеЗначение Цикл
		НоваяСтрока = Объект.БракованныеЭтапы.Добавить();
		НоваяСтрока.КлючСвязи = Этап.КлючСвязи;
		НоваяСтрока.Этап = Этап.Этап;
		НоваяСтрока.Количество = Этап.Количество;
	КонецЦикла;
...
Процедура ПродукцияСтруктурнаяЕдиницаПриИзменении(Элемент)
	...
	Если ДоступенОрдерныйУчет Тогда
		...
		ЗначениеДоИзменения = УчетОстатковПоСкладскимОрдерамКОтгрузке;
		...
		Если Не ЗначениеДоИзменения = УчетОстатковПоСкладскимОрдерамКОтгрузке Тогда
			ПриИзмененииВидаУчетаСклада(ЭтотОбъект);
		КонецЕсли;
...
Процедура ЗапасыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе
		СтруктураПараметров.Вставить("ЗаказПокупателя", Объект.ЗаказПокупателя);
	КонецЕсли;
	СтруктураПараметров.Вставить("НомерВариантаКП", 0);
	СтруктураПараметров.Вставить("ОрганизацияОбъекта", Объект.Организация);
...
Процедура СтруктурнаяЕдиницаЗапасовРазборкаПриИзменении(Элемент)
	...
	Элементы.ЯчейкаЗапасовРазборка.Доступность = (ЗначениеЗаполнено(Объект.СтруктурнаяЕдиницаПродукции));
...
Процедура ЗапасыПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПоискПоШтрихКоду") И ДополнительныеПараметры.ЭтоПоискПоШтрихКоду Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Результат);
		ДополнительныеПараметры.Вставить("ОбработкаПослеУточненияОбъединенияКоличества", Истина);
		ДополнительныеПараметры.Вставить("ИмяТабличнойЧастиСерииНоменклатуры", "СерииНоменклатуры");
		ЗаполнитьПоДаннымШтрихкодовЗавершение(Неопределено, ДополнительныеПараметры);
...
Процедура РаспределениеЗапасовНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество)
	...
	ТекСтр.Количество = НовоеКоличество;
	ТекСтр.Спецификация = СтруктураДанные.Спецификация;
	...
	ТекСтр.ЗапретитьВводДробногоКоличества = СтруктураДанные.ЗапретитьВводДробногоКоличества;
...
Процедура ОтходыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	КонецЕсли;
	РассчитатьСуммуВСтрокеТабличнойЧасти("Отходы");
КонецПроцедуры
 // ОтходыНоменклатураПриИзменении()
...
Процедура ОтходыПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПодбор") И ДополнительныеПараметры.ЭтоПодбор Тогда
		...
		Для Каждого НомерСтроки Из МассивНомеровИдентичныхСтрок Цикл
			...
			СтрокаТабличнойЧасти = Объект["Отходы"][НомерСтроки];
...
&НаКлиенте
Процедура ОтходыХарактеристикаПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.Отходы.ТекущиеДанные;
...
&НаКлиенте
Процедура ВариантУчетаСтоимостиОтходовПриИзменении(Элемент)
	Если ФактическаяЦенаКорректна() Тогда
			ВариантУчетаСтоимостиОтходовПриИзмененииНаСервере();	
			ЦенообразованиеКлиент.ПерезаполнитьЦеныТабличнойЧастиПоВидуЦен(ЭтотОбъект, "Отходы", Ложь);
			ЦеныИВалюта = ЦеныИВалютаКлиентСервер.ТекстНадписиЦеныИВалюта(Объект, ВалютаРасчетов, КурсНациональнаяВалюта,
...
&НаСервере 
Функция ФактическаяЦенаКорректна()
	Если Объект.ВариантУчетаСтоимостиОтходов = ПредопределенноеЗначение("Перечисление.ВариантУчетаСтоимостиОтходов.ПоВидуЦен") тогда
		Если Константы.ВидЦеныОтходов.Получить().Пустая() Тогда
			Возврат Ложь
		Иначе
			Возврат Истина
		КонецЕсли;
...
&НаКлиенте
Процедура ЦеныИВалютыНажатие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
&НаСервере
Процедура ВариантУчетаСтоимостиОтходовПриИзмененииНаСервере()
	Если Объект.ВариантУчетаСтоимостиОтходов = ПредопределенноеЗначение("Перечисление.ВариантУчетаСтоимостиОтходов.ПоВидуЦен") Тогда
	Объект.ВидЦен = Константы.ВидЦеныОтходов.Получить();	
	Иначе
...
Процедура ЗаполнитьПоРасходномуОрдеру(Результат, ДополнительныеПараметры) Экспорт
	...
	Ответ = Результат;
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ЗаполнитьПоСкладскомуОрдеру(Объект.СкладскойОрдерЗапасы);
	КонецЕсли;
...
Процедура ЗаполнитьПоПриходномуОрдеру(Результат, ДополнительныеПараметры) Экспорт
	...
	Ответ = Результат;
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ЗаполнитьПоСкладскомуОрдеру(Объект.СкладскойОрдерПродукция);
	КонецЕсли;
...
Процедура ДобавитьДополнительныйРеквизит(Команда)
	...
	ПараметрыФормы.Вставить("ТекущийНаборСвойств", КэшЗначений.НаборСвойств_Документ_СборкаЗапасов);
...
Процедура ОпределитьВидимостьНастроекУчетаВНалогообложении()
	...
	ВидимостьРасходыУСН = Объект.ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка;
	...
	Если Не ЗначениеЗаполнено(Объект.ПоложениеНастроекНалоговогоУчета) Тогда
	...
	УчитыватьВНУ = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеНастройки("УСНПроизводствоПоУмолчанию") И ВидимостьРасходыУСН;
...
Процедура ОткрытьФормуНастройки(ПередаватьВидДокумента)
	...
	Если ПередаватьВидДокумента Тогда
		ПараметрыДиалога.Вставить("ВидДокумента", НСтр("ru = 'производство'"));
	КонецЕсли;
...
Процедура НастройкаСбросаКоличества(Команда)
	...
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("СтрокаПоиска", "Сбрасывать количество при изменении номенклатуры");
...
Функция УчетОстатковВРазрезеСкладскихОрдеров(Поступление = Истина, СтруктурнаяЕдиница = Неопределено)
	...
	Если Не Поступление И НЕ СтруктурнаяЕдиница = Неопределено Тогда
		Возврат СкладскойУчетСервер.УчетОстатковВРазрезеСкладскихОрдеров(СтруктурнаяЕдиница, Поступление);
	КонецЕсли;
	
	Если Поступление Тогда
		
		Возврат СкладскойУчетСервер.УчетОстатковВРазрезеСкладскихОрдеров(Объект.СтруктурнаяЕдиницаПродукции);
		
	Иначе 
		
		Если Объект.ПоложениеСклада = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
			
			Если Объект.ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка Тогда
			...
			Если Объект.ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Разборка Тогда
				...
				Если Объект.ПоложениеСклада = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
					МассивКонтроля = СкладскойУчетСервер.МассивКонтроляПоЗапасамКПоступлениюПоСкладскимОрдерам(Объект.Продукция.Выгрузить(,"СтруктурнаяЕдиница"));
					Возврат МассивКонтроля.Количество();
				КонецЕсли;
			КонецЕсли;
		...
		Возврат СкладскойУчетСервер.УчетОстатковВРазрезеСкладскихОрдеров(Объект.СтруктурнаяЕдиницаЗапасов, Поступление);
...
Процедура ПриИзмененииВидаУчетаСклада(Форма)
	...
	Если Форма.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСборкаЗапасов.Сборка") Тогда
		
		Если ЗначениеЗаполнено(Форма.Объект.СтруктурнаяЕдиницаПродукции) Тогда
			Форма.Элементы.ГруппаДокументПриходныйОрдерПродукция.Видимость = Форма.УчетОстатковПоСкладскимОрдерамКПоступлению;
			Форма.Элементы.ГруппаДокументРасходныйОрдерПродукция.Видимость = Ложь;
		Иначе
			Форма.Элементы.ГруппаДокументПриходныйОрдерПродукция.Видимость = Ложь;
			Форма.Элементы.ГруппаДокументРасходныйОрдерПродукция.Видимость = Ложь;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Форма.Объект.СтруктурнаяЕдиницаЗапасов) Тогда
			Форма.Элементы.ГруппаДокументРасходныйОрдерЗапасы.Видимость = Форма.УчетОстатковПоСкладскимОрдерамКОтгрузке;
			Форма.Элементы.ГруппаДокументПриходныйОрдерЗапасы.Видимость = Ложь;
		Иначе
			Форма.Элементы.ГруппаДокументРасходныйОрдерЗапасы.Видимость = Ложь;
			Форма.Элементы.ГруппаДокументПриходныйОрдерЗапасы.Видимость = Ложь;
		КонецЕсли;
		
	ИначеЕсли Форма.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСборкаЗапасов.Разборка") Тогда
		
		Если ЗначениеЗаполнено(Форма.Объект.СтруктурнаяЕдиницаЗапасов) Тогда
			Форма.Элементы.ГруппаДокументПриходныйОрдерПродукция.Видимость = Ложь;
			Форма.Элементы.ГруппаДокументРасходныйОрдерПродукция.Видимость = Форма.УчетОстатковПоСкладскимОрдерамКОтгрузке;
		Иначе
			Форма.Элементы.ГруппаДокументПриходныйОрдерПродукция.Видимость = Ложь;
			Форма.Элементы.ГруппаДокументРасходныйОрдерПродукция.Видимость = Ложь;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Форма.Объект.СтруктурнаяЕдиницаПродукции) Тогда
			Форма.Элементы.ГруппаДокументРасходныйОрдерЗапасы.Видимость = Ложь;
			Форма.Элементы.ГруппаДокументПриходныйОрдерЗапасы.Видимость = Форма.УчетОстатковПоСкладскимОрдерамКПоступлению;
		Иначе
			Форма.Элементы.ГруппаДокументРасходныйОрдерЗапасы.Видимость = Ложь;
			Форма.Элементы.ГруппаДокументПриходныйОрдерЗапасы.Видимость = Ложь;
		КонецЕсли;
	...
	Форма.Элементы.РучноеРаспределение.Видимость = Форма.Элементы.ГруппаДокументПриходныйОрдерЗапасы.Видимость Или Форма.Элементы.ГруппаДокументРасходныйОрдерЗапасы.Видимость;
	Форма.Элементы.РучноеРаспределениеГруппаОрдера.Видимость = НЕ Форма.Элементы.ГруппаДокументПриходныйОрдерЗапасы.Видимость И Не Форма.Элементы.ГруппаДокументРасходныйОрдерЗапасы.Видимость;
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	Иначе
		НалогообложениеНДСОрганизации =  ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС")
	КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(СтруктураДанные.Номенклатура.ВидСтавкиНДС) Тогда
		СтруктураДанные.Вставить("СтавкаНДС", Справочники.СтавкиНДС.СтавкаНДС(СтруктураДанные.Номенклатура.ВидСтавкиНДС, ДатаОбработки));
	Иначе
		СтруктураДанные.Вставить("СтавкаНДС", Справочники.СтавкиНДС.СтавкаНДС(СтруктураДанные.Организация.ВидСтавкиНДСПоУмолчанию, ДатаОбработки));
	КонецЕсли;
...
Процедура ОткрытьФормуРеквизитыПечати()
	...
	ОткрытьФорму("Обработка.РеквизитыПечати.Форма.РеквизитыПечатиПроизводство", Новый Структура("КонтекстПечати", Объект),
...
Процедура СохранитьНастройкуПоказыватьИнформациюПоНовойСхеме()
	...
	РаботаСФормойДокумента.СохранитьВидимостьГруппыИнформации(ЭтотОбъект.ИмяФормы,
		"ПоказыватьИнформациюПоНовойСхемеРеквизитовПечати", Ложь);
...
Процедура ОбработатьИзмененияПоКнопкеЦеныИВалюты(Знач ВалютаРасчетовПередИзменением, ПересчитатьЦены = Ложь,
	...
	ЦенообразованиеКлиент.ДополнитьСтруктуруВызоваФормыЦеныИВалютыСтруктурнойЕдиницей(Объект, Объект.Отходы,
		Элементы.Отходы.ТекущиеДанные, ПараметрыФормыЦеныИВалюта, Не РезервированиеЗапасов);
		
	Если Элементы.ЗапасыПартия.Видимость Тогда
		РаботаСНоменклатуройКлиентУНФ.ДополнитьСтруктуруВызоваФормыЦеныИВалютыМассивомПартийНоменклатуры(Объект.Отходы, ПараметрыФормыЦеныИВалюта);
	КонецЕсли;
...
Процедура ОткрытьФормуЦеныИВалютаЗавершение(РезультатФормыЦеныИВалюта, ДополнительныеПараметры) Экспорт
	...
	Если РезультатФормыЦеныИВалюта.ПерезаполнитьЦены Тогда
		...
		ЦенообразованиеКлиент.ПерезаполнитьЦеныТабличнойЧастиПоВидуЦен(ЭтотОбъект, "Отходы", Истина);
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.РучноеРаспределениеГруппаОрдера.ТолькоПросмотр = Форма.ТолькоПросмотр;
...
Процедура УправлениеВидимостьюЭтапов(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПродукцияБрак", 				    		"Видимость", ОтображатьЭтапы);
...
Процедура ВставитьСтроки(ИмяТЧ)
	...
	Если ИмяТЧ = "Отходы" Тогда
		РассчитатьСуммуВСтрокеТабличнойЧасти(ИмяТЧ);
	КонецЕсли;
...
Функция ПараметрыПодбораСерийНоменклатуры(ТекущиеДанныеИдентификатор, ИмяТЧ, ИмяТЧСерииНоменклатуры)
	...
	Если ИмяТЧ = "Запасы" И Объект.ВидОперации = ПредопределенноеЗначение(
		"Перечисление.ВидыОперацийСборкаЗапасов.Сборка") Тогда
		РежимПодбора = Истина;
	ИначеЕсли ИмяТЧ = "Продукция" И Объект.ВидОперации = ПредопределенноеЗначение(
		"Перечисление.ВидыОперацийСборкаЗапасов.Разборка") Тогда
		РежимПодбора = Истина;
	Иначе
		РежимПодбора = Ложь;
	КонецЕсли;
	...
	Возврат СерииНоменклатурыУНФ.ПараметрыПодбораСерийНоменклатуры(Объект, ЭтотОбъект.УникальныйИдентификатор,
		ТекущиеДанныеИдентификатор, РежимПодбора, ИмяТЧ, ИмяТЧСерииНоменклатуры);
...
Процедура ОбновитьПодсказкуРаспределение(ПрочитатьЗначение = Ложь)
	...
	Если РучноеРаспределениеПоУмолчанию Тогда
	...
	Элементы.РучноеРаспределениеГруппаОрдераРасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(ЭлементыЗаголовка);	// АПК:1356 Используются предварительно локализованные строки с гиперссылками
...
Процедура ОбновитьЭтапыНаФорме(Форма)
	...
	Для каждого СтрокаТабличнойЧасти Из Объект.Продукция Цикл
		...
		КонецЦикла;
	КонецЦикла; 	
	ОбновитьБракованныеЭтапыНаФорме(Форма);
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьБракованныеЭтапыНаФорме(Форма)
	...
	Для каждого СтрокаТабличнойЧасти Из Объект.Продукция Цикл
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("КлючСвязи", СтрокаТабличнойЧасти.КлючСвязи);
		СтрокиЭтапы = Объект.БракованныеЭтапы.НайтиСтроки(СтруктураОтбора);
		СтрокаТабличнойЧасти.Брак = "";
		Для каждого СтрокаЭтап Из СтрокиЭтапы Цикл
			СтрокаТабличнойЧасти.Брак = СтрокаТабличнойЧасти.Брак + ?(ПустаяСтрока(СтрокаТабличнойЧасти.Брак), "", "; ")
			 + Строка(СтрокаЭтап.Этап) + "("+Строка(СтрокаЭтап.Количество)+")" ;
...
&НаКлиенте
Процедура ОтходыЦенаПриИзменении(Элемент)
		РассчитатьСуммуВСтрокеТабличнойЧасти("Отходы");
КонецПроцедуры

&НаКлиенте
Процедура ВариантУчетаСтоимостиОтходовРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПереходВЕБВ" Тогда
		СтандартнаяОбработка = Ложь;
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("СтрокаПоиска", "ВидЦеныОтходов");
...
&НаКлиенте
Процедура ПродукцияБракОчистка(Элемент, СтандартнаяОбработка)
	Объект.БракованныеЭтапы.Очистить();
КонецПроцедуры
