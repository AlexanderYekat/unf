...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаЗапасы Из Продукция Цикл
		...
		Если СтрокаЗапасы.ПрослеживаемыйКомплект Тогда
			ТекстСообщения = НСтр("ru = 'Продукция не может быть прослеживаемой комплектуемой номенклатурой. Выберите другую номенклатуру.'");
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Продукция", СтрокаЗапасы.НомерСтроки, "ПродукцияНоменклатура");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОрдерныйСклад") Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка Тогда
			...
			УчетОстатковПоСкладскимОрдерам = СкладскойУчетСервер.УчетОстатковВРазрезеСкладскихОрдеров(СтруктурнаяЕдиницаЗапасов, Ложь);
			Если УчетОстатковПоСкладскимОрдерам Тогда  
				ПроверяемыеРеквизиты.Добавить("СкладскойОрдерЗапасы");
			КонецЕсли;
			
			УчетОстатковПоСкладскимОрдерам = СкладскойУчетСервер.УчетОстатковВРазрезеСкладскихОрдеров(СтруктурнаяЕдиницаПродукции);
			Если УчетОстатковПоСкладскимОрдерам Тогда  
		...
		Иначе
			...
			УчетОстатковПоСкладскимОрдерам = СкладскойУчетСервер.УчетОстатковВРазрезеСкладскихОрдеров(СтруктурнаяЕдиницаЗапасов);
			Если УчетОстатковПоСкладскимОрдерам Тогда  
				ПроверяемыеРеквизиты.Добавить("СкладскойОрдерЗапасы");
			КонецЕсли;
			
			УчетОстатковПоСкладскимОрдерам = СкладскойУчетСервер.УчетОстатковВРазрезеСкладскихОрдеров(СтруктурнаяЕдиницаПродукции, Ложь);
			Если УчетОстатковПоСкладскимОрдерам Тогда  
				ПроверяемыеРеквизиты.Добавить("СкладскойОрдерПродукция");
			КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОрдерныйСклад") Тогда
		...
		МассивКонтроляПоЗапасамКПоступлениюПоСкладскимОрдерам = СкладскойУчетСервер.МассивКонтроляПоЗапасамКПоступлениюПоСкладскимОрдерам(МассивСтруктурныхЕдиницПоступление);
		МассивКонтроляПоЗапасамКОтгрузкеПоСкладскимОрдерам = СкладскойУчетСервер.МассивКонтроляПоЗапасамКОтгрузкеПоСкладскимОрдерам(МассивСтруктурныхЕдиницОтгрузка);
...
Процедура ЗаполнитьПоПриходномуОрдеру(ДокументСсылкаПриходныйОрдер) Экспорт
	...
	Если ВидУчета = Перечисления.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам Тогда
		ТекстИсключения = НСтр("ru = 'Вид учета остатков на ордерном складе - по Сборке запасов. Ввод Сборки запасов на основании Складского ордера недоступен.'");
	...
	УчетОстатковПоСкладскимОрдерам = СкладскойУчетСервер.УчетОстатковВРазрезеСкладскихОрдеров(СтруктурнаяЕдиницаОстатков);
	...
	Пока ВыборкаЗапасы.Следующий() Цикл
		...
		Иначе
			...
			КоличествоСтрокиВЕдиницахХранения = НоваяСтрокаПродукция.Количество * НоваяСтрокаПродукция.ЕдиницаИзмерения.Коэффициент;
			...
			НоваяСтрокаПродукция.Количество = КоличествоВСтрокеЗапасов/ НоваяСтрокаПродукция.ЕдиницаИзмерения.Коэффициент;;
			
		КонецЕсли;
		
		Если СерииНоменклатурыУНФ.ИспользоватьСерииНоменклатурыОстатки() = Истина Тогда
			НоваяСтрокаПродукция.СерииНоменклатуры = СерииНоменклатурыУНФ.ПредставлениеСерийНоменклатуры(СерииНоменклатурыПродукция, НоваяСтрокаПродукция.КлючСвязи)
		КонецЕсли;
...
Процедура ЗаполнитьПоРасходномуОрдеру(ДокументСсылкаРасходныйОрдер) Экспорт
	...
	УчетОстатковПоСкладскимОрдерам = СкладскойУчетСервер.УчетОстатковВРазрезеСкладскихОрдеров(СтруктурнаяЕдиницаОстатков, Ложь);
...
Процедура ЗаполнитьПоСкладскомуОрдеруБезУчетаОстатков(ДанныеЗаполнения, Приходный = Истина) Экспорт
	...
	Если ИмяТабличнойЧасти = "Продукция" Тогда
		...
		СерииНоменклатурыУНФ.УдалитьСерииНоменклатурыВТабличнойЧастиВЗависимостиОтПолитики(ЭтотОбъект,,, "СерииНоменклатурыПродукция");
	Иначе
...
Процедура ЗаполнитьПоЗаказуНаПроизводство(ДанныеЗаполнения) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЭтапыПроизводства") И НЕ ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Разборка Тогда	
	Запрос.Текст = ТекстЗапросаПоЗаказуНаПроизводствоСЭтапами(); 
	Иначе
	Запрос.Текст = ТекстЗапросаПоЗаказуНаПроизводствоБезЭтапов();
    КонецЕсли;
	...
	Запрос.УстановитьПараметр("ЗаказНаПроизводство", ДанныеЗаполнения);
	Запрос.УстановитьПараметр("ПоЗаказуПокупателя", ЗначениеЗаполнено(ДанныеЗаполнения.ЗаказПокупателя));
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	...
	Если ТаблицаОстатков.Количество() > 0 Тогда   
		...
		КонецЕсли;
		НовыйКлючСвязи = Неопределено;
		//Заполняем ТЧ Продукция
		Пока ВыборкаПродукция.Следующий() Цикл
			Если НовыйКлючСвязи = Неопределено Тогда
				НовыйКлючСвязи = ВыборкаПродукция.КлючСвязи;
			КонецЕсли;
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьЭтапыПроизводства") Тогда
				СтруктураКоличества = Новый Структура("КоличествоПланфакт",неопределено);
				ЗаполнитьЗначенияСвойств(СтруктураКоличества, ВыборкаПродукция);
				Если СтруктураКоличества.КоличествоПланфакт = неопределено ИЛИ НЕ ЗначениеЗаполнено(СтруктураКоличества.КоличествоПланфакт) тогда
					СтруктураДляПоиска.Вставить("Количество", ВыборкаПродукция.Количество);
				Иначе
					СтруктураДляПоиска.Вставить("Количество", ВыборкаПродукция.КоличествоПланФакт);
				КонецЕсли;
			КонецЕсли;
			МассивПродукции = Продукция.НайтиСтроки(СтруктураДляПоиска);
			Если СборкаСЭтапами И МассивПродукции.Количество() > 0 Тогда
				Если ЗначениеЗаполнено(ВыборкаПродукция.ВидПроизводства) Тогда
					НовыйЭтап = ВыполненныеЭтапы.Добавить();
					НовыйЭтап.КлючСвязи = МассивПродукции[0].КлючСвязи;
					НовыйЭтап.Этап = ВыборкаПродукция.Этап;
				КонецЕсли;
			Иначе
			...
			Если СборкаСЭтапами Тогда
				...
				Иначе
					НоваяСтрока.Количество = ВыборкаПродукция.КоличествоПланФакт;
				КонецЕсли;
					 
					МассивСтрокОстатков[0].КоличествоОстаток = МассивСтрокОстатков[0].КоличествоОстаток + КоличествоКСписанию;
				
				СтруктураСвойств = Новый Структура("ВидПроизводства",неопределено);
				ЗаполнитьЗначенияСвойств(СтруктураСвойств, ВыборкаПродукция);
				Если НЕ  СтруктураСвойств.ВидПроизводства = неопределено тогда
					Если ЗначениеЗаполнено(ВыборкаПродукция.ВидПроизводства) Тогда
						НовыйЭтап = ВыполненныеЭтапы.Добавить();
	...
	// Заполнение по спецификации.
		...
		ЗаполнятьПоСпецификации = Ложь;
		МассивСпецификаций = Продукция.ВыгрузитьКолонку("Спецификация");
			Для Каждого ПроверяемаяСпецификация из МассивСпецификаций Цикл
				Если не ПроверяемаяСпецификация.Ссылка.Пустая() Тогда
					ЗаполнятьПоСпецификации = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если ЗаполнятьПоСпецификации Тогда
		...
		Если ЗначениеЗаполнено(ВыборкаИзРезультатаЗапроса.Номенклатура) Тогда
			...
			Иначе
				...
				Если ВыборкаИзРезультатаЗапроса.ЕстьСборка Тогда
					НоваяСтрока.Количество = ВыборкаИзРезультатаЗапроса.Количество - ВыборкаИзРезультатаЗапроса.КоличествоСобрано;
					КоличествоКСписанию =
					(ВыборкаИзРезультатаЗапроса.Количество - ВыборкаИзРезультатаЗапроса.КоличествоСобрано) * ВыборкаИзРезультатаЗапроса.Коэффициент;
				Иначе
...
Процедура ЗаполнитьПоСтруктуреНаОснованииЗаказаПокупателя(ДанныеЗаполнения)
	...
	Иначе
		ЗначенияРеквизитов = Новый Структура("Проведен, ОжидаетсяВыборВариантаКП, ВидОперации", Истина, Ложь, Перечисления.ВидыОперацийЗаказПокупателя.ПустаяСсылка());
	КонецЕсли; 
	Документы.ЗаказПокупателя.ПроверитьВозможностьВводаНаОснованииЗаказаПокупателя(ДанныеЗаполнения.Основание, ЗначенияРеквизитов);
	ПоЗаказуНаПереработку = (ЗначенияРеквизитов.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПереработку);
	ЭтоРазборка = (ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Разборка);
	...
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Запрос.УстановитьПараметр("ВидОперации", ВидОперации);
	Документы.ЗаказПокупателя.ДобавитьТаблицуЗапасыВМенеджерВременныхТаблиц(ДанныеЗаполнения.Основание, Запрос.МенеджерВременныхТаблиц, Ложь);
...
Процедура ЗаполнитьПоСтруктуреНаОснованииЗаказаНаПроизводство(ДанныеЗаполнения)
	...
	Если ДанныеЗаполнения.Основание.ВидОперации <> Перечисления.ВидыОперацийЗаказНаПроизводство.Разборка И Продукция.Количество() > 0 Тогда
...
Функция ТекстЗапросаПоЗаказуНаПроизводствоБезЭтапов() 
...
Функция ТекстЗапросаПоЗаказуНаПроизводствоСЭтапами()
	Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗаказыОстатки.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
	|	ЗаказыОстатки.Номенклатура КАК Номенклатура,
	|	ЗаказыОстатки.Характеристика КАК Характеристика,
	...
	|		ЗаказыОстатки.КоличествоОстаток КАК КоличествоОстаток
	|	ИЗ
	|		РегистрНакопления.ЗаказыНаПроизводство.Остатки(, ЗаказНаПроизводство = &ЗаказНаПроизводство
	|		И Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)) КАК ЗаказыОстатки
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ВЫБОР
	|		КОГДА
	|			ЗаказНаПроизводствоПродукция.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказНаПроизводство.Разборка)
	|		И (ЗаказНаПроизводствоПродукция.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|		ИЛИ ЗаказНаПроизводствоПродукция.Ссылка.УчетПотребностиПоСкладам)
	|		И ЗаказНаПроизводствоПродукция.СтруктурнаяЕдиница <> ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
	|			ТОГДА ЗаказНаПроизводствоПродукция.СтруктурнаяЕдиница
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
	|	КОНЕЦ КАК СтруктурнаяЕденица,
	|	ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка) КАК Ячейка,
	...
	|	ЗаказНаПроизводствоПродукция.Спецификация.ВидПроизводства КАК СпецификацияВидПроизводства
	|ПОМЕСТИТЬ ВТПродукцияСЭтапами
	|ИЗ
	|	Константа.ФункциональнаяОпцияИспользоватьЭтапыПроизводства КАК ФункциональнаяОпцияИспользоватьЭтапыПроизводства,
	|	Документ.ЗаказНаПроизводство.Продукция КАК ЗаказНаПроизводствоПродукция
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЭтапыПроизводства КАК ЭтапыПроизводства
	|		ПО ЗаказНаПроизводствоПродукция.Номенклатура = ЭтапыПроизводства.Номенклатура
	|		И ЗаказНаПроизводствоПродукция.Характеристика = ЭтапыПроизводства.Характеристика
	|		И ЗаказНаПроизводствоПродукция.Спецификация = ЭтапыПроизводства.Спецификация
	|		И ВЫБОР
	|			КОГДА &ПоЗаказуПокупателя = ИСТИНА
	|				ТОГДА ЗаказНаПроизводствоПродукция.ЗаказПокупателя = ЭтапыПроизводства.Заказ
	|			ИНАЧЕ ЗаказНаПроизводствоПродукция.Ссылка = ЭтапыПроизводства.Заказ
	|		КОНЕЦ
	|ГДЕ
	...
	|	ЗаказНаПроизводствоПродукция.ТипНоменклатуры,
	|	ЗаказНаПроизводствоПродукция.Номенклатура.СтранаПроисхождения,
	...
	|	ЗаказНаПроизводствоПродукция.ПодразделениеЗавершающегоЭтапа,
	|	ВЫБОР
	|		КОГДА
	|			ЗаказНаПроизводствоПродукция.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказНаПроизводство.Разборка)
	|		И (ЗаказНаПроизводствоПродукция.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	...
	|	ЗаказНаПроизводствоПродукция.Спецификация.ВидПроизводства,
	...
	|	Продукция.ТипНоменклатуры КАК ТипНоменклатуры,
	|	Продукция.СтранаПроисхождения КАК СтранаПроисхождения,
	...
	|	Продукция.СтруктурнаяЕденица КАК СтруктурнаяЕденица,
	|	Продукция.Ячейка КАК Ячейка,
	|	Продукция.ЗаказПокупателя КАК ЗаказПокупателя,
	|	Продукция.КлючСвязи КАК КлючСвязи,
	|	Продукция.Этап КАК Этап,
	|	СУММА(Продукция.КоличествоПлан) КАК КоличествоПланФакт,
	|	Продукция.ВидПроизводства
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВТПродукцияСЭтапами.НомерСтроки КАК НомерСтроки,
	|		ВТПродукцияСЭтапами.Номенклатура КАК Номенклатура,
	|		ВТПродукцияСЭтапами.ТипНоменклатуры КАК ТипНоменклатуры,
	|		ВТПродукцияСЭтапами.СтранаПроисхождения КАК СтранаПроисхождения,
	|		ВТПродукцияСЭтапами.Характеристика КАК Характеристика,
	|		ВТПродукцияСЭтапами.Партия КАК Партия,
	|		ВТПродукцияСЭтапами.Количество КАК Количество,
	|		ВТПродукцияСЭтапами.Коэффициент КАК Коэффициент,
	|		ВТПродукцияСЭтапами.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|		ВТПродукцияСЭтапами.Спецификация КАК Спецификация,
	|		ВТПродукцияСЭтапами.ПодразделениеЗавершающегоЭтапа КАК ПодразделениеЗавершающегоЭтапа,
	|		ВТПродукцияСЭтапами.СтруктурнаяЕденица КАК СтруктурнаяЕденица,
	|		ВТПродукцияСЭтапами.Ячейка КАК Ячейка,
	|		ВТПродукцияСЭтапами.ЗаказПокупателя КАК ЗаказПокупателя,
	|		ВТПродукцияСЭтапами.КлючСвязи КАК КлючСвязи,
	|		ВТПродукцияСЭтапами.Этап КАК Этап,
	|		СУММА(ВТПродукцияСЭтапами.КоличествоПлан) КАК КоличествоПлан,
	|		ВТПродукцияСЭтапами.СпецификацияВидПроизводства КАК ВидПроизводства
	|	ИЗ
	|		ВТПродукцияСЭтапами КАК ВТПродукцияСЭтапами
	|	СГРУППИРОВАТЬ ПО
	|		ВТПродукцияСЭтапами.НомерСтроки,
	|		ВТПродукцияСЭтапами.Номенклатура,
	|		ВТПродукцияСЭтапами.ТипНоменклатуры,
	|		ВТПродукцияСЭтапами.СтранаПроисхождения,
	|		ВТПродукцияСЭтапами.Характеристика,
	|		ВТПродукцияСЭтапами.Партия,
	|		ВТПродукцияСЭтапами.Количество,
	|		ВТПродукцияСЭтапами.Коэффициент,
	|		ВТПродукцияСЭтапами.ЕдиницаИзмерения,
	|		ВТПродукцияСЭтапами.Спецификация,
	|		ВТПродукцияСЭтапами.ПодразделениеЗавершающегоЭтапа,
	|		ВТПродукцияСЭтапами.СтруктурнаяЕденица,
	|		ВТПродукцияСЭтапами.Ячейка,
	|		ВТПродукцияСЭтапами.ЗаказПокупателя,
	|		ВТПродукцияСЭтапами.КлючСвязи,
	|		ВТПродукцияСЭтапами.Этап,
	|		ВТПродукцияСЭтапами.СпецификацияВидПроизводства) КАК Продукция
	|СГРУППИРОВАТЬ ПО
	|	Продукция.НомерСтроки,
	|	Продукция.Номенклатура,
	|	Продукция.ТипНоменклатуры,
	|	Продукция.СтранаПроисхождения,
	...
	|	Продукция.ПодразделениеЗавершающегоЭтапа,
	|	Продукция.СтруктурнаяЕденица,
	|	Продукция.Ячейка,
...
Процедура ПроверитьЗаполнениеЭтаповПроизводства(Отказ, ОтменаПроведения = Ложь)
	...
	ТаблицаКонтроляДублей.Свернуть("ЗаказПокупателя, ЗаказНаПроизводство, Номенклатура, Характеристика, Партия, Этап", "КоличествоСтрок");
