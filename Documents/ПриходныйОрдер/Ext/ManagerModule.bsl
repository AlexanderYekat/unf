...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПриходныйОрдер, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫБОР
	|		КОГДА &УчетПоЯчейкам
	|			ТОГДА ПриходныйОрдерЗапасы.Ячейка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Ячейка,
	...
	|	ВЫБОР
	|		КОГДА ПриходныйОрдерЗапасы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ПриходныйОрдерЗапасы.Ссылка
	|		ИНАЧЕ ПриходныйОрдерЗапасы.ДокументПоступления
	...
	|	ВЫБОР
	|		КОГДА &УчетПоЯчейкам
	|				И ПриходныйОрдерЗапасы.Ссылка.СтруктурнаяЕдиница.УчетОстатковПоЯчейкам
	|			ТОГДА ПриходныйОрдерЗапасы.Ячейка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Ячейка,
	|	ПриходныйОрдерЗапасы.Номенклатура КАК Номенклатура,
	...
	|			ТОГДА ПриходныйОрдерЗапасы.Ссылка
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ПриходныйОрдерЗапасы.ДокументПоступления = ЗНАЧЕНИЕ(Документ.ПриходныйОрдер.ПустаяСсылка)
	|					ТОГДА НЕОПРЕДЕЛЕНО
	|				ИНАЧЕ ПриходныйОрдерЗапасы.ДокументПоступления
	|			КОНЕЦ
	|	КОНЕЦ КАК ДокументОснование
...
Процедура ВыполнитьКонтроль(ДокументСсылкаПриходныйОрдер, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт 
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
		...
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ
		|	И &КонтрольОстатковПоЗапасамКПоступлению
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки
		|;
		...
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияСерииНоменклатурыИзменение.Организация) КАК ОрганизацияПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияСерииНоменклатурыИзменение.СтруктурнаяЕдиница) КАК СтруктурнаяЕдиницаПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияСерииНоменклатурыИзменение.Номенклатура) КАК НоменклатураПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияСерииНоменклатурыИзменение.Характеристика) КАК ХарактеристикаПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияСерииНоменклатурыИзменение.Партия) КАК ПартияПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияСерииНоменклатурыИзменение.Ячейка) КАК ЯчейкаПредставление,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ДвиженияСерииНоменклатурыИзменение.Серия) КАК СерияПредставление,
		|	СерииНоменклатурыОстатки.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы КАК ТипСтруктурнойЕдиницы,
		|	ПРЕДСТАВЛЕНИЕССЫЛКИ(СерииНоменклатурыОстатки.Номенклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияПредставление,
		...
		|				&МоментКонтроля,
		|				(Организация, СтруктурнаяЕдиница, Номенклатура, Характеристика, Партия, Ячейка, Серия) В
		|					(ВЫБРАТЬ
		...
		|						ДвиженияСерииНоменклатурыИзменение.Ячейка КАК Серия
		|					ИЗ
		|						ДвиженияСерииНоменклатурыИзменение КАК ДвиженияСерииНоменклатурыИзменение)) КАК СерииНоменклатурыОстатки
		|		ПО ДвиженияСерииНоменклатурыИзменение.Организация = СерииНоменклатурыОстатки.Организация
		...
		Запрос.УстановитьПараметр("КонтрольОстатковПоЗапасамКПоступлению", ДополнительныеСвойства.КонтрольОстатковПоЗапасамКПоступлению);
		...
		Если ДополнительныеСвойства.КонтрольОстатковПоЗапасамКПоступлению И НЕ МассивРезультатов[1].Пустой() Тогда
			ДокументОбъектПриходныйОрдер = ДокументСсылкаПриходныйОрдер.ПолучитьОбъект();
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[1].Выбрать();
			КонтрольОстатковУНФ.ЗапасыКПоступлению(ДокументОбъектПриходныйОрдер, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		...
		Если НЕ МассивРезультатов[2].Пустой() Тогда
			...
			КонтрольОстатковУНФ.СерииНоменклатуры(ДокументОбъектПриходныйОрдер, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		
		// Отрицательный остаток по сериям номенклатуры к поступлению.
		Если ДополнительныеСвойства.КонтрольОстатковПоЗапасамКПоступлению И НЕ МассивРезультатов[3].Пустой() Тогда
			ДокументОбъектПриходныйОрдер = ДокументСсылкаПриходныйОрдер.ПолучитьОбъект();
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[3].Выбрать();
			КонтрольОстатковУНФ.СерииНоменклатурыКПоступлению(ДокументОбъектПриходныйОрдер, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПриходныйОрдер") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПриходныйОрдер", НСтр("ru = 'Приходный ордер'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ПриходныйОрдер"));
		
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "М4") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "М4", НСтр("ru = 'М-4'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "М4"));
		
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТОРГ4") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ТОРГ4", НСтр("ru = 'Т-4'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ТОРГ4"));
