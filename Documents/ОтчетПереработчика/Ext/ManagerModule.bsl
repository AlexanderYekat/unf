...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаУправленческий.Ссылка.Дата,
	|	ТаблицаУправленческий.Ссылка.Организация,
	...
	|			ТОГДА ТаблицаУправленческий.Ссылка.ВалютаДокумента
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТаблицаУправленческий.СчетУчетаНДСПоПриобретеннымЦенностямВалютный
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ТаблицаУправленческий.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщиком,
	|	ВЫБОР
	|		КОГДА ТаблицаУправленческий.Ссылка.Контрагент.СчетУчетаРасчетовСПоставщиком.Валютный
	|			ТОГДА ТаблицаУправленческий.Ссылка.ВалютаДокумента
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	ВЫБОР
	...
	|	&ОтражениеНДС
	|ИЗ
	|	УслугиВВалюте КАК ТаблицаУправленческий
	|ГДЕ
...
Процедура ИнициализироватьДанныеПоУслуге(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства, СуммаУслуги) Экспорт
	...
	|	ОтчетПереработчика.Всего - ОтчетПереработчика.СуммаНДС КАК СуммаБезНДС
	|ПОМЕСТИТЬ Услуги
	|ИЗ
	...
	|	ОтчетПереработчикаРасходы.Ссылка.НДСВключатьВСтоимость,
	|	ОтчетПереработчикаРасходы.Всего,
	|	ОтчетПереработчикаРасходы.СуммаНДС,
	|	ОтчетПереработчикаРасходы.Всего - ОтчетПереработчикаРасходы.СуммаНДС
	|ИЗ
	...
	|	Услуги.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	|	Услуги.СчетУчетаЗатрат КАК СчетУчетаЗатрат,
	|	ВЫБОР
	|		КОГДА Услуги.Расход.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Управленческий.НДСПоПриобретеннымЦенностям)
	|		ИНАЧЕ Услуги.Расход.СчетУчетаНДСПоПриобретеннымЦенностям
	|	КОНЕЦ КАК СчетУчетаНДСПоПриобретеннымЦенностям,
	|	ВЫБОР
	|		КОГДА Услуги.Расход.СчетУчетаНДСПоПриобретеннымЦенностям = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ Услуги.Расход.СчетУчетаНДСПоПриобретеннымЦенностям.Валютный
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА Услуги.НДСВключатьВСтоимость
	|				ТОГДА 0
	|			ИНАЧЕ ВЫБОР
	|					КОГДА Услуги.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|						ТОГДА Услуги.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|					ИНАЧЕ Услуги.СуммаНДС * Услуги.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * Услуги.Кратность)
	|				КОНЕЦ
	...
	|			ИНАЧЕ ВЫБОР
	|					КОГДА Услуги.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|						ТОГДА Услуги.СуммаНДС * КурсыРегВалюты.Курс * Услуги.Кратность / (Услуги.Курс * КурсыРегВалюты.Кратность)
	|					ИНАЧЕ Услуги.СуммаНДС
	|				КОНЕЦ
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДСВал,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА Услуги.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|				ТОГДА Услуги.СуммаБезНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ Услуги.СуммаБезНДС * Услуги.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * Услуги.Кратность)
	...
	|	СУММА(ВЫБОР
	|			КОГДА УслугиВВалюте.НДСВключатьВСтоимость
	|				ТОГДА УслугиВВалюте.Сумма
	|			ИНАЧЕ УслугиВВалюте.Сумма - УслугиВВалюте.СуммаНДС
	|		КОНЕЦ) КАК Сумма,
	|	СУММА(ВЫБОР
	|			КОГДА УслугиВВалюте.НДСВключатьВСтоимость
	|				ТОГДА УслугиВВалюте.СуммаВал
	|			ИНАЧЕ УслугиВВалюте.СуммаВал - УслугиВВалюте.СуммаНДСВал
	|		КОНЕЦ) КАК СуммаВал,
	|	СУММА(УслугиВВалюте.СуммаБезНДС) КАК СуммаБезНДС
	|ИЗ
	...
	|				ТОГДА УслугиВВалюте.Сумма
	|			ИНАЧЕ УслугиВВалюте.Сумма - УслугиВВалюте.СуммаНДС
	|		КОНЕЦ * ОтчетПереработчикаПродукция.Количество / ВТКоличествоПродукции.КоличествоПродукции КАК ЧИСЛО(15, 2)) КАК Сумма,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА УслугиВВалюте.НДСВключатьВСтоимость
	|				ТОГДА УслугиВВалюте.СуммаВал
	|			ИНАЧЕ УслугиВВалюте.СуммаВал - УслугиВВалюте.СуммаНДСВал
	|		КОНЕЦ * ОтчетПереработчикаПродукция.Количество / ВТКоличествоПродукции.КоличествоПродукции КАК ЧИСЛО(15, 2)) КАК СуммаВал,
	|	ВЫРАЗИТЬ(УслугиВВалюте.СуммаБезНДС * ОтчетПереработчикаПродукция.Количество / ВТКоличествоПродукции.КоличествоПродукции КАК ЧИСЛО(15, 2)) КАК СуммаБезНДС,
	|	ВЫБОР
	...
	|							ТОГДА УслугиВВалюте.СуммаВал
	|						ИНАЧЕ УслугиВВалюте.СуммаВал - УслугиВВалюте.СуммаНДСВал
	|					КОНЕЦ * ОтчетПереработчикаПродукция.Количество / ВТКоличествоПродукции.КоличествоПродукции КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаВалКт,
	...
	Запрос.УстановитьПараметр("ОтражениеРасходовПоПереработке", СодержанияПроводок.ОтражениеРасходовПоПереработке());
...
Процедура СформироватьТаблицаЗапасыКПоступлениюНаСклады(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства)
	...
	|	ОтчетПереработчикаПродукция.ДокументОснование КАК ДокументОснование
	|ИЗ
	|	ВременнаяТаблицаПродукция КАК ОтчетПереработчикаПродукция
	|ГДЕ
	|	ОтчетПереработчикаПродукция.ОрдерныйСклад
	|	И ОтчетПереработчикаПродукция.ОрдерныйСкладКПоступлению
	...
	|	ОтчетПереработчикаПродукция.ДокументОснование
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		КОНЕЦ),
	|	ВЫБОР
	|		КОГДА ОтчетПереработчикаОтходы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА ОтчетПереработчикаОтходы.Ссылка
	|		КОГДА ОтчетПереработчикаОтходы.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ОтчетПереработчикаОтходы.Ссылка.ДокументПоступления
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ
	|ИЗ
	|	Документ.ОтчетПереработчика.Отходы КАК ОтчетПереработчикаОтходы
	...
	|			И ОтчетПереработчикаОтходы.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриПоступлении
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам)
	|			ТОГДА ОтчетПереработчикаПродукция.Ссылка
	|		КОГДА ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоСкладскимОрдерам)
	|			ТОГДА ОтчетПереработчикаПродукция.Ссылка.ДокументПоступления
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ДокументОснование,
	|	ВЫБОР
	|		КОГДА ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад
	|				И ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ИспользоватьОрдернуюСхемуПриПоступлении
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОрдерныйСкладКПоступлению,
	|	ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом) КАК УчетПоСкладуВЦелом,
	|	ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам) КАК УтетПоПрочимДокументам
	...
	|				И (ТаблицаПродукция.УчетПоСкладуВЦелом
	|					ИЛИ ТаблицаПродукция.УтетПоПрочимДокументам))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТаблицаПродукция.Ссылка = &Ссылка
	|	И &ИспользоватьСерииНоменклатуры
	|	И ТаблицаПродукция.ОрдерныйСклад
	|	И ТаблицаПродукция.ОрдерныйСкладКПоступлению
	|	И НЕ ТаблицаПродукция.УчетПоСкладуВЦелом";
	...
	РезультатЗапросаКПоступлению = МассивРезультатов[6].Выгрузить();
...
Процедура СформироватьТаблицуРаспределенияМатериалов(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства, ТаблицаПродукция) Экспорт
	...
	|				ТОГДА 1
	|			ИНАЧЕ ТаблицаМатериалы.Количество
	|		КОНЕЦ * ТаблицаМатериалы.ЕдиницаИзмерения.Коэффициент / ТаблицаМатериалы.КоличествоПродукции *
	|			ТаблицаСоставПродукции.КоррКоличество
	...
	|		ИНАЧЕ ВЫБОР
	|			КОГДА
	|				СборкаЗапасовЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|				ТОГДА СборкаЗапасовЗапасы.Номенклатура.СчетУчетаЗапасов
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетПереработчикаРаспределениеЗапасов.НомерСтроки КАК НомерСтроки,
	|	ОтчетПереработчикаРаспределениеЗапасов.КлючСвязиПродукция КАК КлючСвязи,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка КАК Ссылка,
	|	ОтчетПереработчикаРаспределениеЗапасов.ЗаказПокупателя КАК КоррЗаказПокупателя,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.ДокументОснование КАК ДокументОснование,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.Контрагент КАК Контрагент,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.Договор КАК Договор,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.Курс КАК Курс,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.Кратность КАК Кратность,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.ВалютаДокумента КАК ВалютаДокумента,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.Дата КАК Период,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ОтчетПереработчикаРаспределениеЗапасов.ЕдиницаИзмерения.Коэффициент КАК Коэффициент,
	|	ОтчетПереработчикаРаспределениеЗапасов.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад КАК ОрдерныйСклад,
	|	ВЫБОР
	|		КОГДА &УчетПоЯчейкам
	|			ТОГДА ОтчетПереработчикаРаспределениеЗапасов.Ссылка.Ячейка
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
	|	КОНЕЦ КАК Ячейка,
	|	ВЫБОР
	|		КОГДА
	|			ОтчетПереработчикаРаспределениеЗапасов.Ссылка.СтруктурнаяЕдиница = ОтчетПереработчикаРаспределениеЗапасов.Ссылка.СтруктурнаяЕдиница
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
	|		ИНАЧЕ ОтчетПереработчикаРаспределениеЗапасов.Ссылка.СтруктурнаяЕдиница
	|	КОНЕЦ КАК СтруктурнаяЕдиницаЗапасов,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиницаЗапасовНаСклад,
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад КАК ОрдерныйСкладЗапасов,
	|	ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиницаПродукции,
	|	ОтчетПереработчикаРаспределениеЗапасов.Номенклатура.СчетУчетаЗапасов КАК СчетУчета,
	|	ВЫБОР
	|		КОГДА
	|			ОтчетПереработчикаРаспределениеЗапасов.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
	|		ИЛИ ОтчетПереработчикаРаспределениеЗапасов.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
	|		ИЛИ ОтчетПереработчикаРаспределениеЗапасов.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ДавальческоеСырье)
	|			ТОГДА ОтчетПереработчикаРаспределениеЗапасов.Номенклатура.СчетУчетаЗапасов
	|		ИНАЧЕ ВЫБОР
	|			КОГДА
	|				ОтчетПереработчикаРаспределениеЗапасов.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|				ТОГДА ОтчетПереработчикаРаспределениеЗапасов.Номенклатура.СчетУчетаЗапасов
	|			ИНАЧЕ ОтчетПереработчикаРаспределениеЗапасов.Номенклатура.СчетУчетаЗатрат
	|		КОНЕЦ
	|	КОНЕЦ КАК СчетУчетаЗапасов,
	|	ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗапасов КАК КоррСчетУчета,
	|	ВЫБОР
	|		КОГДА
	|			ОтчетПереработчикаРаспределениеЗапасов.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
	|		ИЛИ ОтчетПереработчикаРаспределениеЗапасов.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
	...
	|		ИНАЧЕ ВЫБОР
	|			КОГДА
	|				ОтчетПереработчикаПродукция.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|				ТОГДА ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗапасов
	|			ИНАЧЕ ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗатрат
	|		КОНЕЦ
	|	КОНЕЦ КАК СчетУчетаПродукции,
	|	ОтчетПереработчикаРаспределениеЗапасов.Номенклатура КАК Номенклатура,
	|	ОтчетПереработчикаПродукция.Номенклатура КАК КоррНоменклатура,
	|	ВЫБОР
	|		КОГДА &ИспользоватьХарактеристики
	|			ТОГДА ОтчетПереработчикаРаспределениеЗапасов.Характеристика
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК Характеристика,
	|	ОтчетПереработчикаПродукция.Характеристика КАК КоррХарактеристика,
	|	ВЫБОР
	|		КОГДА &ИспользоватьПартии
	|			ТОГДА ОтчетПереработчикаРаспределениеЗапасов.Партия
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК Партия,
	|	ВЫБОР
	|		КОГДА &ИспользоватьПартии
	|			ТОГДА ОтчетПереработчикаРаспределениеЗапасов.Партия.Статус
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ПустаяСсылка)
	|	КОНЕЦ КАК ПартияСтатус,
	|	ОтчетПереработчикаПродукция.Партия КАК КоррПартия,
	|	ОтчетПереработчикаПродукция.Спецификация КАК КоррСпецификация,
	|	ОтчетПереработчикаРаспределениеЗапасов.Спецификация КАК Спецификация,
	|	ОтчетПереработчикаРаспределениеЗапасов.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ВЫБОР
	|		КОГДА
	|			ТИПЗНАЧЕНИЯ(ОтчетПереработчикаРаспределениеЗапасов.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	|			ТОГДА ОтчетПереработчикаРаспределениеЗапасов.Количество
	|		ИНАЧЕ ОтчетПереработчикаРаспределениеЗапасов.Количество *
	|			ОтчетПереработчикаРаспределениеЗапасов.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК Количество,
	|	ВЫБОР
	|		КОГДА
	|			ТИПЗНАЧЕНИЯ(ОтчетПереработчикаРаспределениеЗапасов.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	...
	|			ОтчетПереработчикаРаспределениеЗапасов.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК ИсходноеКоличество,
	|	ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗапасов КАК СчетДт,
	|	ОтчетПереработчикаПродукция.Номенклатура.СчетУчетаЗапасов КАК СчетДтПродукции,
	|	ВЫБОР
	|		КОГДА
	|			ОтчетПереработчикаРаспределениеЗапасов.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ОтветственноеХранение)
	|		ИЛИ ОтчетПереработчикаРаспределениеЗапасов.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
	...
	|	КОНЕЦ КАК СчетКтПродукции,
	|	ЛОЖЬ КАК Распределено,
	|	ОтчетПереработчикаРаспределениеЗапасов.СтранаПроисхождения,
	|	ОтчетПереработчикаРаспределениеЗапасов.НомерГТД,
	|	ОтчетПереработчикаРаспределениеЗапасов.Всего
	|ИЗ
	|	Документ.ОтчетПереработчика.Продукция КАК ОтчетПереработчикаПродукция
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика.РаспределениеЗапасов КАК ОтчетПереработчикаРаспределениеЗапасов
	|		ПО ОтчетПереработчикаПродукция.Ссылка = ОтчетПереработчикаРаспределениеЗапасов.Ссылка
	|		И ОтчетПереработчикаПродукция.КлючСвязи = ОтчетПереработчикаРаспределениеЗапасов.КлючСвязиПродукция
	|ГДЕ
	|	ОтчетПереработчикаРаспределениеЗапасов.Ссылка = &Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	...
	ТаблицаРаспределения = МассивРезультатов[3].Выгрузить();
	Если ДокументСсылкаОтчетПереработчика.РучноеРаспределение Тогда 
	...
	ПараметрыПоиска.Вставить("Номенклатура",СтрокаРаспределения.Номенклатура);
	ПараметрыПоиска.Вставить("Характеристика",СтрокаРаспределения.Характеристика);
	СтрокиТаблицыматериалов = ТаблицаМатериалов.НайтиСтроки(ПараметрыПоиска);
		Если СтрокиТаблицыматериалов.Количество() > 0 Тогда
			СтрокаРаспределения.Всего = (СтрокиТаблицыматериалов[0].Всего/СтрокиТаблицыматериалов[0].Количество)*СтрокаРаспределения.Количество;
		Иначе
			СтрокаРаспределения.Всего = 0;
		КонецЕсли;	
	КонецЦикла;
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРаспределенияМатериаловСборка", ТаблицаРаспределения);
...
Процедура ВыполнитьКонтроль(ДокументСсылкаОтчетПереработчика, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыИзменение
		...
		Если НЕ МассивРезультатов[7].Пустой() Тогда
			...
			КонтрольОстатковУНФ.ЗапасыКПоступлению(ДокументОбъектОтчетПереработчика, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		...
		Если НЕ МассивРезультатов[8].Пустой() Тогда
			...
			КонтрольОстатковУНФ.СерииНоменклатурыКПоступлению(ДокументОбъектОтчетПереработчика, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Функция ДанныеПолученныхДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ОтчетПереработчика.Контрагент.ГоловнойКонтрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА ОтчетПереработчика.Контрагент.ГоловнойКонтрагент
	|		ИНАЧЕ ОтчетПереработчика.Контрагент
	...
	|	ВЫБОР
	|		КОГДА ОтчетПереработчика.Контрагент.ВидКонтрагента = ЗНАЧЕНИЕ(Перечисление.ВидыКонтрагентов.ФизическоеЛицо)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоФизическоеЛицо,
	|	ВЫБОР
	|		КОГДА ОтчетПереработчика.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|			ТОГДА 2
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК СтатусУПД,
	...
	|	ВЫБОР
	|		КОГДА ОтчетПереработчика.Организация.ГоловнаяОрганизация <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|				И НЕ ОтчетПереработчика.Организация = ОтчетПереработчика.Организация.ГоловнаяОрганизация
	|			ТОГДА ОтчетПереработчика.Организация.ГоловнаяОрганизация
	|		ИНАЧЕ ОтчетПереработчика.Организация
	...
	|		ИНАЧЕ ОтчетПереработчика.ВалютаДокумента
	|	КОНЕЦ КАК ВалютаДокумента,
	|	ВЫБОР
	|		КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|			ТОГДА НациональнаяВалюта.Значение.НаименованиеПолное
	|		ИНАЧЕ ОтчетПереработчика.ВалютаДокумента.НаименованиеПолное
	|	КОНЕЦ КАК ВалютаНаименование,
	|	ВЫБОР
	|		КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|			ТОГДА НациональнаяВалюта.Значение.Код
	|		ИНАЧЕ ОтчетПереработчика.ВалютаДокумента.Код
	|	КОНЕЦ КАК ВалютаКод,
	...
	|	ОтчетПереработчика.Ссылка.Дата КАК ДатаОтгрузочногоДокумента,
	|	ОтчетПереработчика.Ссылка.Номер КАК НомерОтгрузочногоДокумента,
	|	ОтчетПереработчика.ПоложениеРасходов КАК ПоложениеРасходов,
	|	ОтчетПереработчика.Расходы.(
	|		НомерСтроки КАК НомерСтроки,
	|		Расход КАК Номенклатура,
	|		НЕОПРЕДЕЛЕНО КАК Содержание,
	|		ВЫБОР
	|			КОГДА (ВЫРАЗИТЬ(ОтчетПереработчика.Расходы.Расход.НаименованиеПолное КАК СТРОКА(1000))) = """"
	|				ТОГДА ОтчетПереработчика.Расходы.Расход.Наименование
	|			ИНАЧЕ ВЫРАЗИТЬ(ОтчетПереработчика.Расходы.Расход.НаименованиеПолное КАК СТРОКА(1000))
	...
	|		ВЫБОР
	|			КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|					ИЛИ ОтчетПереработчика.Расходы.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	|						И ОтчетПереработчика.Расходы.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|				ТОГДА ВЫРАЗИТЬ(ВЫБОР
	|							КОГДА ОтчетПереработчика.СуммаВключаетНДС = ИСТИНА
	|									И ОтчетПереработчика.Расходы.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|								ТОГДА ОтчетПереработчика.Расходы.Сумма / ((ОтчетПереработчика.Расходы.СтавкаНДС.Ставка + 100) / 100)
	|							ИНАЧЕ ОтчетПереработчика.Расходы.Сумма
	|						КОНЕЦ * ОтчетПереработчика.Ссылка.Курс / ОтчетПереработчика.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	...
	|			ИНАЧЕ ОтчетПереработчика.Расходы.СуммаНДС
	|		КОНЕЦ КАК СуммаНДС,
	|		ВЫБОР
	|			КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|					ИЛИ ОтчетПереработчика.Расходы.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	...
	|			ИНАЧЕ ОтчетПереработчика.Расходы.Всего
	|		КОНЕЦ КАК Всего,
	|		Ссылка КАК Ссылка,
	...
	|	НЕОПРЕДЕЛЕНО КАК Содержание,
	|	ВЫБОР
	|		КОГДА (ВЫРАЗИТЬ(ОтчетПереработчика.Расход.НаименованиеПолное КАК СТРОКА(1000))) = """"
	|			ТОГДА ОтчетПереработчика.Расход.Наименование
	|		ИНАЧЕ ВЫРАЗИТЬ(ОтчетПереработчика.Расход.НаименованиеПолное КАК СТРОКА(1000))
	|	КОНЕЦ КАК ПредставлениеНоменклатуры,
	...
	|	ВЫБОР
	|		КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|				ИЛИ ОтчетПереработчика.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	|					И ОтчетПереработчика.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|			ТОГДА ВЫРАЗИТЬ(ВЫБОР
	|						КОГДА ОтчетПереработчика.СуммаВключаетНДС = ИСТИНА
	...
	|						ИНАЧЕ ОтчетПереработчика.Сумма
	|					КОНЕЦ * ОтчетПереработчика.Ссылка.Курс / ОтчетПереработчика.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	|					КОГДА ОтчетПереработчика.СуммаВключаетНДС = ИСТИНА
	|							И ОтчетПереработчика.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	...
	|				ИЛИ ОтчетПереработчика.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	|					И ОтчетПереработчика.Ссылка.ВалютаДокумента <> НациональнаяВалюта.Значение
	|			ТОГДА ВЫРАЗИТЬ(ОтчетПереработчика.СуммаНДС * ОтчетПереработчика.Ссылка.Курс / ОтчетПереработчика.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ ОтчетПереработчика.СуммаНДС
	|	КОНЕЦ КАК СуммаНДС,
	|	ВЫБОР
	|		КОГДА &ПечатнаяФормаТолькоВРублях = ИСТИНА
	|				ИЛИ ОтчетПереработчика.Ссылка.Договор.РасчетыВУсловныхЕдиницах = ИСТИНА
	...
	|						ИНАЧЕ ОтчетПереработчика.Сумма + ОтчетПереработчика.СуммаНДС
	|					КОНЕЦ * ОтчетПереработчика.Ссылка.Курс / ОтчетПереработчика.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	|					КОГДА ОтчетПереработчика.СуммаВключаетНДС = ИСТИНА
	|							И ОтчетПереработчика.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	...
	|ГДЕ
	|	ОтчетПереработчика.Ссылка В(&МассивОбъектов)";
