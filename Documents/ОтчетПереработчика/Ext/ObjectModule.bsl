...
Процедура ЗаполнитьПоПриходныйОрдер(ДокументСсылкаПриходныйОрдер) Экспорт
	...
	Если ВидУчета = Перечисления.ВидыУчетаОрдерныхСкладов.УчетОстатковПоПрочимДокументам Тогда
		ТекстИсключения = НСтр("ru = 'Вид учета остатков на ордерном складе - по Отчет переработчика. Ввод Отчет переработчика на основании Складского ордера недоступен.'");
	...
	Пока ВыборкаЗапасы.Следующий() Цикл
		...
		Если СерииНоменклатурыУНФ.ИспользоватьСерииНоменклатурыОстатки() = Истина Тогда
			НоваяСтрокаПродукция.СерииНоменклатуры = СерииНоменклатурыУНФ.ПредставлениеСерийНоменклатуры(СерииНоменклатуры, НоваяСтрокаПродукция.КлючСвязи)
		КонецЕсли;
		
		НоваяСтрокаПродукция.Спецификация = Справочники.Спецификации.СпецификацияПоУмолчанию(НоваяСтрокаПродукция.Номенклатура, НоваяСтрокаПродукция.Характеристика);
...
Процедура ЗаполнитьПоСкладскомуОрдеруБезУчетаОстатков(ДанныеЗаполнения, Приходный = Истина) Экспорт
	...
	Для Каждого СтрокаЗапасы Из ДанныеЗаполнения.Запасы Цикл
		...
		НоваяСтрока.Спецификация = Справочники.Спецификации.СпецификацияПоУмолчанию(СтрокаЗапасы.Номенклатура, СтрокаЗапасы.Характеристика);
