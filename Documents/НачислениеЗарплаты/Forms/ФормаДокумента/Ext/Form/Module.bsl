...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если Параметры.ЗначенияЗаполнения.Свойство("ПерваяПоловинаМесяца") Тогда
			Если РасчетЗарплатыДляНебольшихОрганизаций.ЭтоДатаВыплатыАванса(Объект.Организация, Объект.МесяцНачисления, Объект.Дата) Тогда
	...
	РасчетЗарплатыДляНебольшихОрганизаций.ПриСозданииФормыДокументаНачислениеЗарплаты(ЭтотОбъект);
...
Процедура ДобавитьСотрудников(Знач СписокСотрудников)
	...
	Если РезультатНачисления.Количество() > 0 Тогда
		...
		РасчетЗарплатыДляНебольшихОрганизаций.СформироватьТабличнуюЧастьСотрудники(Объект);
		РассчитатьИтогиПоДокументу();
	...
	Если ОбновляемыеСотрудники <> Неопределено Тогда
		...
		Для каждого ОбновляемыйСотрудник Из ОбновляемыеСотрудники Цикл
			...
			Иначе
				...
				СтрокаСотрудника.НДФЛ = ТаблицаНДФЛ.Итог("Налог") + ТаблицаНДФЛ.Итог("НалогСПревышения") + СтрокаСотрудника.НДФЛОтпускаБольничные;
...
Процедура ПрочитатьДатуВыплаты()
	...
	Если Объект.ПерваяПоловинаМесяца Тогда
		ДатаВыплаты = ДатыВыплаты.Аванс;
	Иначе
		ДатаВыплаты = ДатыВыплаты.Зарплата;
	КонецЕсли;
КонецПроцедуры
