...
Процедура ОчисткаПулаКодовМаркировки() Экспорт
	...
	|		ПО ПулКодовМаркировкиСУЗ.ШтрихкодУпаковки = КегиНаОборудованииРозливаИСМП.КодМаркировки
	|ГДЕ
...
Процедура УдалениеНеиспользованныхКодовМаркировки() Экспорт
	...
	|ГДЕ
	|	ПулКодовМаркировкиСУЗ.ЗаказНаЭмиссию <> ЗНАЧЕНИЕ(Документ.ЗаказНаЭмиссиюКодовМаркировкиСУЗ.ПустаяСсылка)
	|	И ПулКодовМаркировкиСУЗ.ЗаказНаЭмиссию.ТипОплатыКодовМаркировки <> ЗНАЧЕНИЕ(Перечисление.ТипыОплатыКодовМаркировкиСУЗ.ОплатаПоЭмиссии)
	|;
	|ВЫБРАТЬ
...
Процедура ИнициализироватьКомпоновщикНастроекДляПечатиТаблицыКодовМаркировки(КомпоновщикНастроек) Экспорт
	...
	Если СхемаКомпоновкиДанных = Неопределено Тогда
		СхемаКомпоновкиДанных = РегистрыСведений.ПулКодовМаркировкиСУЗ.ПолучитьМакет("ПоляШаблонаИСМПТаблица");
	КонецЕсли;
...
Процедура ОтразитьАгрегациюКодовМаркировкиВПуле(ДеревоМаркированнойПродукции) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Запись = РегистрыСведений.ПулКодовМаркировкиСУЗ.СоздатьМенеджерЗаписи();
...
Функция НастройкиУдаленияНеиспользованныхКодовМаркировки()
	...
	Строка = ТаблицаНастроек.Добавить();
	Строка.ВидПродукции = Перечисления.ВидыПродукцииИС.МорепродуктыПодконтрольныеВЕТИС;
	Строка.ДатаЭмиссии  = СрокХранения;
