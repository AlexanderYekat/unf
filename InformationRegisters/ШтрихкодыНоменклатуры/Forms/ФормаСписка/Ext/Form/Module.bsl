...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьШтрихКода" Тогда
		...
		Если Не ФормаВладельца = Неопределено И Параметр.Номенклатура = ФормаВладельца.Объект.Ссылка 
 		...
 		ИначеЕсли Параметр.Свойство("ШтрихКодУдален") И Параметр.ШтрихКодУдален Тогда
...
Процедура СписокПередУдалением(Элемент, Отказ)
	...
	УдаляемыйШтрихКод = ТекущиеДанные.ШтрихКод;
...
Процедура СписокПослеУдаления(Элемент)
	...
	ФормаВладельца = ЭтаФорма.ВладелецФормы;
	Если ФормаВладельца = Неопределено ИЛИ Не ЗначениеЗаполнено(УдаляемыйШтрихКод) Тогда 
		Возврат
	КонецЕсли;
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("Номенклатура", ФормаВладельца.Объект.Ссылка);
	ПараметрыОповещения.Вставить("ШтрихКод", УдаляемыйШтрихКод);
