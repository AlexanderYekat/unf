...
//
&НаКлиенте
Процедура СинхронизироватьПорциюОбъектовЗавершение(РезультатЗапроса, ПараметрыОповещения) Экспорт
	...
	Если РезультатЗапроса.Статус = "Выполнено" Тогда
		...
		Результат = СинхронизироватьПорциюОбъектовНаСервереЗавершение(РезультатЗапроса.РезультатДлительнойОперации);
		Если Результат.Успешно Тогда
	...
	ИначеЕсли РезультатЗапроса.Статус = "Ошибка" Тогда
		
		ОбработатьИсключение(РезультатЗапроса.КраткоеПредставлениеОшибки);
