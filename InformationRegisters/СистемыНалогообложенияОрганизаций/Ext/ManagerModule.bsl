...
Функция ОпределитьСистемуНалогообложенияОрганизации(Организация, Знач Дата) экспорт
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		Возврат Новый Структура("СистемаНалогообложения, ПлательщикУСН, ПлательщикЕНВД, ПрименяетсяПатент, ПрименяетсяАУСН, ПлательщикЕНП"
		,Перечисления.СистемыНалогообложения.Общая, Ложь, Ложь, Ложь, Ложь, Ложь);;
	КонецЕсли;
...
Функция ПолучитьТипСистемыНалогообложенияККТОрганизации(Организация, ДатаДокумента = Неопределено) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		ИначеЕсли Выборка.СистемаНалогообложения = Перечисления.СистемыНалогообложения.ОсобыйПорядок Тогда
			Если Выборка.ПлательщикЕНВД Тогда
				Возврат Перечисления.ТипыСистемНалогообложенияККТ.ЕНВД;
			ИначеЕсли Выборка.ПрименяетсяПатент Тогда
				Возврат Перечисления.ТипыСистемНалогообложенияККТ.Патент;
			Иначе
				Возврат Перечисления.ТипыСистемНалогообложенияККТ.ОСН;
			КонецЕсли;
		ИначеЕсли Выборка.ОбъектНалогообложения = Перечисления.ВидыОбъектовНалогообложения.Доходы Тогда
