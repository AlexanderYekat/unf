...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АдресСайта") Тогда
		АдресСайта = Параметры.АдресСайта;
	КонецЕсли;
	...
	Если Параметры.Свойство("ТипСайта") Тогда
		ТипСайта = Параметры.ТипСайта;
	КонецЕсли;
	
	Если Параметры.Свойство("ОсновнаяОрганизация") Тогда
		ОсновнаяОрганизация = Параметры.ОсновнаяОрганизация;
	КонецЕсли;
	...
	Если Параметры.Свойство("ЭтоЗагрузка") Тогда
		ЭтоЗагрузка = Истина;
	КонецЕсли;
...
Функция ЗапуститьЗагрузкуВФонеСервер(ИмяКоманды)
	...
	Если НЕ ЭтоЗагрузка Тогда
		Если ЗначениеЗаполнено(НастройкаИнтеграции) Тогда
			...
			Попытка
				...
				ЭлементБлокировки = Блокировка.Добавить("Справочник.НастройкиИнтеграцииСИнтернетМагазином");
				ЭлементБлокировки.УстановитьЗначение("Ссылка", НастройкаИнтеграции);
				ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
				Блокировка.Заблокировать();
				
				НастройкаИнтеграцииОбъект = НастройкаИнтеграции.ПолучитьОбъект();
				НастройкаИнтеграцииОбъект.ТипСайта = ТипСайта;
				НастройкаИнтеграцииОбъект.ПротоколОбменаCMS = ТипСайта;
				НастройкаИнтеграцииОбъект.АдресСайта = АдресСайта;
				НастройкаИнтеграцииОбъект.Логин = ИмяПользователя;
				НастройкаИнтеграцииОбъект.Пароль = Пароль;
		...
		Иначе
			...
			Попытка
				...
				Блокировка = Новый БлокировкаДанных;
				ЭлементБлокировки = Блокировка.Добавить("Справочник.НастройкиИнтеграцииСИнтернетМагазином");
				ЭлементБлокировки.УстановитьЗначение("АдресСайта", АдресСайта);
				ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
				Блокировка.Заблокировать();
				...
				|ГДЕ
				|	НастройкиИнтеграцииСИнтернетМагазином.АдресСайта = &АдресСайта
				...
				Если РезультатЗапроса.Пустой() Тогда
					...
					НастройкаИнтеграцииОбъект = Справочники.НастройкиИнтеграцииСИнтернетМагазином.СоздатьЭлемент();
					НастройкаИнтеграцииОбъект.Заполнить(Неопределено);
					НастройкаИнтеграцииОбъект.ТипСайта = ТипСайта;
					НастройкаИнтеграцииОбъект.ПротоколОбменаCMS = ТипСайта;
					НастройкаИнтеграцииОбъект.АдресСайта = АдресСайта;
					НастройкаИнтеграцииОбъект.Логин = ИмяПользователя;
					НастройкаИнтеграцииОбъект.Пароль = Пароль;
					НастройкаИнтеграцииОбъект.ВыгружатьНаСайт = Истина;
					НастройкаИнтеграцииОбъект.ОрганизацияДляПодстановкиВЗаказы = ОсновнаяОрганизация;
					НастройкаИнтеграцииОбъект.ВыгружатьКартинки = ЗагружатьКартинки;
					НастройкаИнтеграцииОбъект.ВидыЦен.Добавить().ВидЦен = ВидЦен;
					НастройкаИнтеграцииОбъект.ДобавитьНастройкиСоздания();
					НастройкаИнтеграцииОбъект.НастройкаЗавершена = Истина;
					
					НастройкаИнтеграцииОбъект.СпособУстановкиДатыОтгрузкиЗаказа
					= ПредопределенноеЗначение("Перечисление.СпособыУстановкиДатыДляМагазинаСоцСетей.ТекущаяДата");
					
					НастройкаИнтеграцииОбъект.УчетнаяЗаписьДляОповещения
					= ПредопределенноеЗначение("Справочник.УчетныеЗаписиЭлектроннойПочты.СистемнаяУчетнаяЗаписьЭлектроннойПочты");
					
					НастройкаИнтеграцииОбъект.ТипИсточникаИнтеграции = ПредопределенноеЗначение("Перечисление.ТипыИсточниковИнтеграции.Сайт");
					
					НастройкаИнтеграцииОбъект.Записать();
					
					НастройкаИнтеграции = НастройкаИнтеграцииОбъект.Ссылка;
	...
	ПараметрыЗадания.Вставить("НастройкаИнтеграции", НастройкаИнтеграции);
	...
	ПараметрыЗадания.Вставить("НачалоЗагрузки", ТекущаяДатаСеанса());
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	Если ЭтоЗагрузка И ТипЗнч(Настройки) = Тип("Соответствие") Тогда
		Настройки.Вставить("АдресСайта", АдресСайта);
		Настройки.Вставить("ИмяПользователя", ИмяПользователя);
		Настройки.Вставить("Пароль", Пароль);
		Настройки.Вставить("ТипСайта", ТипСайта);
	КонецЕсли;
