...
Процедура ОтработатьИзменениеОрганизации_ПослеУстановкиРежимаСУЦ(Результат, ВходящийКонтекст) Экспорт
	...
	ПосоветоватьОбратитсяКАдминистраторуПриОтсутствииПрав();
	УправлениеФормой();
...
Функция ПроверитьКраткоеНаименование(МастерДалее = Истина, ВыводитьСообщения = Истина)
	...
	Если ЭтоЮридическоеЛицо Тогда
		...
		ТекстОшибки = "";
		Если НЕ ЗначениеЗаполнено(СокрЛП(КраткоеНаименование)) 
			ИЛИ ОбработкаЗаявленийАбонента.НаименованиеПустое(ЭтотОбъект) Тогда 
...
Функция ПроверитьЮрАдрес(МастерДалее = Истина, ВыводитьСообщения = Истина)
	...
	Результат = КонтекстЭДОСервер.ПроверитьАдресОрганизацииЗаявления(
		ЭтотОбъект,
		АдресЮридическийЗначение, 
		"УказательЮрАдреса", 
...
Процедура ИзменитьОформлениеИННВладельца()
	...
	Элементы.ГруппаВладелецЭЦПИНН.Видимость = РезультатПроверки.ЕстьОшибка;
	...
	Иначе
		...
		Если ТребуетсяПодключение = Истина Тогда // Может быть Неопределено, если не заполнена переопределяемая
			...
			Если НастройкиДляЭДО <> Неопределено Тогда
				...
				ИначеЕсли ВыбраноОблачнаяПодпись ИЛИ ЭтоМультиРежим Тогда
					ПодключитьЭДО = Ложь;
				Иначе
...
Процедура ОбновитьРеквизитыСотрудника(ДанныеСотрудникаПредыдущие, НеобходимоОбновитьСертификат)
	...
	Если ПопытатьсяОбновитьРеквизит(
			...
			"ЭлектроннаяПочта", 
			ЭлектроннаяПочта, 
			ДанныеСотрудника) Тогда
			
		ЭлектроннаяПочтаДляПаролей  = ДанныеСотрудника.ЭлектроннаяПочта;
		ЭлектроннаяПочта 			= ДанныеСотрудника.ЭлектроннаяПочта;
...
&НаКлиенте
Процедура ПриОткрытии_ПослеОпределенияВозможностиБезбумажногоПодписания(Результат, ВходящийКонтекст) Экспорт
...
Процедура ПосоветоватьОбратитсяКАдминистраторуПриОтсутствииПрав(ВыполняемоеОповещение = Неопределено) Экспорт
	...
	Показывать = МультирежимКлиентСервер.ПоказыватьСообщениеПроОтсутствиеПрав(ЭтотОбъект);
...
Процедура ИзменитьОформлениеТарифа()
	...
	Элементы.ГруппаТарифа.Видимость = ПоддерживаетсяМультирежим;
