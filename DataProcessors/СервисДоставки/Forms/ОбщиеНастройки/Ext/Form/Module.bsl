...
Процедура ЗагрузитьРезультатПолученияСпискаПунктовВыдачиКлиента(АдресРезультата)
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаСклад Из ПоискСтрок Цикл
			...
			СтрокаСклад.СкладАдрес = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформацииОбъекта(
				Выборка.Значение,
				Справочники.ВидыКонтактнойИнформации.ФактАдресСтруктурнойЕдиницы);//!!!!! УНФ, было "АдресСклада"
...
Процедура СоответствиеСкладовСкладПриИзмененииНаСервере()
	...
	ТекущиеДанные = СоответствиеСкладов.НайтиПоИдентификатору(Элементы.СоответствиеСкладов.ТекущаяСтрока);
	ТекущиеДанные.СкладАдрес = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформацииОбъекта(ТекущиеДанные.Склад, Справочники.ВидыКонтактнойИнформации.ФактАдресСтруктурнойЕдиницы); //!!!!! УНФ, было "АдресСклада"
...
Процедура ОбновитьЗаголовокСоответствиеПунктовВыдачи()
	...
	КоличествоЗаполненных = КоличествоЗаписей - СоответствиеСкладов.НайтиСтроки(Новый Структура("Склад", ПредопределенноеЗначение("Справочник.СтруктурныеЕдиницы.ПустаяСсылка"))).Количество(); //!!!!! УНФ было "Склады"
