...
Процедура ПолучитьДанныеПоПеремещениюЗапасов(ДанныеДокумента, ТекущийДокумент)
	...
	|	ВЫБОР
	|		КОГДА ПеремещениеЗапасов.Грузоотправитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА ПеремещениеЗапасов.СтруктурнаяЕдиница.Организация
	|		ИНАЧЕ ПеремещениеЗапасов.Грузоотправитель
	|	КОНЕЦ КАК Грузоотправитель,
	|	ВЫБОР
	|		КОГДА ПеремещениеЗапасов.Грузополучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА ПеремещениеЗапасов.СтруктурнаяЕдиницаПолучатель.Организация
	|		ИНАЧЕ ПеремещениеЗапасов.Грузополучатель
	|	КОНЕЦ КАК Грузополучатель,
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		// Дата и части даты
		ОбластьПодвал.Параметры.ДатаДокументаДень = """" + Формат(Шапка.ДатаДокумента, "ДФ=dd") + """";
		МесяцДатыДокумента = Формат(Шапка.ДатаДокумента, "ДФ=MMMM");
		МесяцДатыДокументаСоСклонением = ПолучитьСклоненияСтроки(
		МесяцДатыДокумента,
		,
		"ПД=Родительный")[0];
		ОбластьПодвал.Параметры.ДатаДокументаМесяц = НРег(МесяцДатыДокументаСоСклонением);
		ОбластьПодвал.Параметры.ДатаДокументаГод = Формат(Шапка.ДатаДокумента, "ДФ=yyyy");
		// Конец Дата и части даты
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТТН") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ТТН",
			НСтр("ru = 'Товарно-транспортная накладная'"), ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ПараметрыПечати));
