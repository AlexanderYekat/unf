...
Процедура ВРаботеПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	СписокПриПолученииДанныхНаСервере(Строки, ИмяЭлемента);
...
Процедура ЗадачиНаСегодняНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	СписокНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания);
...
Процедура ЗадачиБезСрокаПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для Каждого КлючИЗначение Из Строки Цикл
		...
		Данные.Картинка = КартинкаПоТипуЗадачи(Данные.Ссылка);
		Данные.ЗадачаПросрочена = ЗадачаПросрочена(Данные.Начало, Данные.Окончание);
		
		Если ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.ЗадачаСотрудника") Тогда
			Данные.Содержание = СтрокаТегов(Данные.Ссылка.Теги);
		КонецЕсли;
		
		НайденнаяСтрока = НастройкиДоступныхКалендарей.Найти(Данные.Календарь);
		Если НайденнаяСтрока = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Данные.ЦветКалендаря = "■";
		КлючИЗначение.Значение.Оформление["ЦветКалендаря"].УстановитьЗначениеПараметра(
			"ЦветТекста", НайденнаяСтрока.ЦветКалендаря);
...
&НаСервереБезКонтекста
Процедура СписокПриПолученииДанныхНаСервере(Строки, НазваниеСписка)
	...
	Для Каждого КлючИЗначение Из Строки Цикл
		...
		Данные.Картинка = КартинкаПоТипуЗадачи(Данные.Ссылка);
		Данные.ДатаОбъединенная = СтрокаДаты(Данные.Начало, Данные.Окончание, НазваниеСписка);
		Данные.ЗадачаПросрочена = ЗадачаПросрочена(Данные.Начало, Данные.Окончание);
...
Процедура УстановитьУсловноеОформлениеНаВесьДень()
	...
	Для каждого Цвет Из ЦветаПоказанныхКалендарей Цикл
		...
		РаботаСФормой.ДобавитьОформляемоеПоле(НовоеУсловноеОформление,
			Элементы.ЗадачиНаВесьДеньКалендарьПредставление.Имя);
		РаботаСФормой.ДобавитьОформляемоеПоле(НовоеУсловноеОформление,
			Элементы.ЗадачиНаВесьДеньКалендарьДатаСтрокой.Имя);
...
Процедура ПрочитатьДоступныеКалендари()
	...
	Если СохраненВариантЦвета Тогда
		...
		ЦветаПоНомеруКартинки = ЦветаПоНомеруКартинки();
	...
	Для Каждого СтрокаТаблицы Из ТаблицаКалендарей Цикл
		...
		Иначе
			...
			НоваяСтрока.ЦветКалендаря = РаботаСЦветомСервер.СлучайныйСветлыйЦвет();
...
Процедура УстановитьПредставлениеПериода(Форма)
	...
	ИначеЕсли Форма.ВариантПериода = "Неделя" Тогда
		...
		Форма.ПредставлениеПериодаКалендарь = СтрШаблон("%1 - %2",
...
Процедура ДобавитьЭлементыДосокКалендаря(Календари,ПрефиксКалендаря,ГруппаДобавления)
	...
	Для Каждого Календарь Из Календари Цикл
		...
		Если НЕ Календарь.Недействителен Тогда
			ДекорацияКартинкаМеню.ЦветФона = Календарь.ЦветКалендаря;
			ДекорацияКартинкаМеню.ЦветТекста = Календарь.ЦветКалендаря;
...
Процедура УстановитьОбязательностьИспользованияПолей(РеквизитДинамическийСписок)
	...
	РеквизитДинамическийСписок.УстановитьОбязательноеИспользование("ЦветКалендаря", Истина);
...
&НаКлиенте
Процедура СинхронизироватьСВнешнимиСервисами(Команда)
	СинхронизироватьНаКлиенте();
КонецПроцедуры
