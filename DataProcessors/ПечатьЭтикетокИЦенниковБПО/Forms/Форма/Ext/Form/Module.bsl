...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.РежимПечати) Тогда
		Если Параметры.Свойство("РежимПечати") Тогда
			Объект.РежимПечати = Параметры.РежимПечати;
		Иначе
...
&НаКлиенте
Процедура Подключаемый_Перезаполнить(Команда)
	
	ТоварыДляПечати = ПечатьДокументовУНФКлиент.ПерезаполнитьСУчетомОтбора(ЭтотОбъект);
	
	Объект.Товары.Очистить();
	
	Для Каждого СтрокаДляПечати Из ТоварыДляПечати Цикл
		НоваяСтрока = Объект.Товары.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаДляПечати);
	КонецЦикла;
...
Процедура Печать(Команда)
	...
	ОчиститьРаспечатанныеЦенникиИзДанныхМобильногоПриложения();
...
Процедура ВыбратьИзДанныхМобильногоПриложения(Команда)
	...
	СтруктураПараметров.Вставить("ЭтоПечатьЭтикетокИЦенниковБПО", Истина);
	...
	ОткрытьФорму("Обработка.ПечатьЭтикетокИЦенников.Форма.ФормаПечатьИзРегистра",СтруктураПараметров , ЭтотОбъект,,,, ОбработчикЗаполненияТабличнойЧасти, РежимБлокировки);
...
Процедура ЗагрузитьНастройкиОтбора()
	...
	СхемаКомпоновкиДанных = ПечатьЭтикетокИЦенниковБПОПереопределяемый.МакетСхемыКомпоновкиДанныхДляОтбора(ЭтотОбъект);
...
&НаСервере
Процедура ЗаполнитьОбработкуПоПакетамЦенников(Параметры)
	
	Объект.Товары.Очистить();
	
	Для Каждого Товар Из Параметры.СтруктураТоваровБПО Цикл
		
		НоваяСтрока = Объект.Товары.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Товар);
		
		НоваяСтрока = СтруктураТоваровИзМобильногоПриложения.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Товар);
		
	КонецЦикла;
	
	Если Параметры.Свойство("ВидЦен") Тогда
		Объект.ВидЦены = Параметры.ВидЦен;
	КонецЕсли;
...
&НаСервере
Процедура ОчиститьРаспечатанныеЦенникиИзДанныхМобильногоПриложения()
	
	Для каждого Строка Из СтруктураТоваровИзМобильногоПриложения Цикл
		
		Отбор = Новый Структура;
		Отбор.Вставить("НоменклатураБПО", Строка.НоменклатураБПО);
		Отбор.Вставить("ХарактеристикаБПО", Строка.ХарактеристикаБПО);
		Отбор.Вставить("Партия", Строка.Партия);
		Отбор.Вставить("УпаковкаБПО", Строка.УпаковкаБПО);
		Отбор.Вставить("Выбран", Истина);
		СтрокиМассив = Объект.Товары.НайтиСтроки(Отбор);
		
		Для каждого СтрокаТаблицыТоваров Из СтрокиМассив Цикл
			
			Набор = РегистрыСведений.ЦенникиКПечати.СоздатьНаборЗаписей();
			Набор.Отбор.Номенклатура.Установить(СтрокаТаблицыТоваров.НоменклатураБПО);
			Набор.Отбор.Характеристика.Установить(СтрокаТаблицыТоваров.ХарактеристикаБПО);
			Набор.Отбор.Партия.Установить(СтрокаТаблицыТоваров.Партия);
			Набор.Отбор.ЕдиницаИзмерения.Установить(СтрокаТаблицыТоваров.УпаковкаБПО);
			Набор.Записать();
	...
	СтруктураТоваровИзМобильногоПриложения.Очистить();
...
Процедура КомпоновщикНастроекНастройкиОтборПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		...
		УстановитьЭлементОтбораСКД("Организация", Организация, Истина);
...
Процедура СохранитьНастройкиНаСервере()
	...
	ПараметрыЗакрытия.Вставить("ПечатьНаОбычномПринтере", Объект.ПечатьНаОбычномПринтере);
	ПараметрыЗакрытия.Вставить("ВидЦены", Объект.ВидЦены);
	ПараметрыЗакрытия.Вставить("ВидЦеныАкционный", Объект.ВидЦеныАкционный);
	ПараметрыЗакрытия.Вставить("ТорговыйОбъектБПО", Объект.ТорговыйОбъектБПО);
	Если ПечатьПоОтбору Тогда
		ПараметрыЗакрытия.Вставить("НастройкиОтбора", Новый ХранилищеЗначения(КомпоновщикНастроек.Настройки));
	КонецЕсли;
	
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПечатьЭтикетокИЦенниковНастройкиФормыБПО", "", ПараметрыЗакрытия);
...
Процедура ВосстановитьНастройкиНаСервере()
	...
	Настройки = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ПечатьЭтикетокИЦенниковНастройкиФормыБПО", "");
	...
	КонецЕсли;
	Если Настройки.Свойство("ВидЦеныАкционный", ЗначениеСохраненнойНастройки) Тогда
		Объект.ВидЦеныАкционный = ЗначениеСохраненнойНастройки;
	КонецЕсли;
	Если Настройки.Свойство("ТорговыйОбъектБПО", ЗначениеСохраненнойНастройки) Тогда
		Объект.ТорговыйОбъектБПО = ЗначениеСохраненнойНастройки;
	КонецЕсли;
	Если Настройки.Свойство("ПечатьНаОбычномПринтере", ЗначениеСохраненнойНастройки) Тогда
		Объект.ПечатьНаОбычномПринтере = ЗначениеСохраненнойНастройки;
	КонецЕсли;
