...
Функция СформироватьПФ(ОписаниеПечатнойФормы, ДанныеОбъектовПечати, ЭтоУПД) Экспорт
	...
	Для Каждого ДанныеОбъекта Из ДанныеОбъектовПечати Цикл
		...
		И НомерПостановления1137 <> "2023Кв4");
		...
		Если ОбластиМакета.ОбластьСтроки <> Неопределено Тогда
			...
			Для Каждого СтрокаТабличнойЧасти Из ДанныеОбъекта.ТаблицаЗапасы Цикл
				...
				Если НомерПостановления1137 = "534" ИЛИ
					НомерПостановления1137 = "2023Кв4" Тогда
		...
		ИначеЕсли ЭтоУПД
			И	(ОбъектПечати.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС
				...
				(ТипЗнч(ОбъектПечати) = Тип("ДокументСсылка.РасходнаяНакладная")
					И ОбъектПечати.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ПередачаНаКомиссию"))) Тогда
