...
// АПК:1036-вкл
	...
	Если НЕ ЭтоСокращенныйРежимНастройкиРабочегоМестаКассира Тогда
		...
		ИначеЕсли ИмяТекущейСтраницы = "СтраницаВидыОплат" Тогда
			...
			Элементы.НастройкаПОЭквайринговыйТерминал.Доступность = НастройкаРМК.ИспользоватьОплатуПлатежнымиКартами;
...
Процедура ЗаполнениеДаннымиПереходаСервер(ИмяСтраницы, ИтераторНаправления, ПараметрыКлиент)
	...
	ИначеЕсли ИмяСтраницы = "СтраницаПодключаемоеОборудование" Тогда
		...
		ОтображатьСозданиеККТ = ТекущийТипОборудования = ТипыПодключаемогоОборудования.Получить("ФискальныйРегистратор") И ИспользуемоеККТ = 0;
		Элементы.НастройкаПодключенияККМ.Видимость = ОтображатьСозданиеККТ;
		
		ОтображатьСозданиеЭТ = ТекущийТипОборудования = ТипыПодключаемогоОборудования.Получить("ЭквайринговыйТерминал") И ИспользуемыеЭквайринговыеТерминалы = 0;
		...
		Если ТекущийТипОборудования = ТипыПодключаемогоОборудования.Получить("ФискальныйРегистратор") 
			ИЛИ ТекущийТипОборудования = ТипыПодключаемогоОборудования.Получить("ЭквайринговыйТерминал") Тогда
				Элементы.ПодключаемоеОборудование.КнопкаСоздания = Истина;
		Иначе
				Элементы.ПодключаемоеОборудование.КнопкаСоздания = Ложь;
		КонецЕсли;
		Если ТекущийТипОборудования = ТипыПодключаемогоОборудования.Получить("ФискальныйРегистратор") Тогда
			ЗаполнитьКассыККМ();
...
Процедура ЗаполнитьТаблицуПодключаемогоОборудования()
	...
	Если НастройкаПОФискальныеРегистраторы Тогда
		ДобавитьСтрокиПодключаемоеОборудования(
			ТипыОборудования.Получить("ФискальныйРегистратор"),, Истина);
	КонецЕсли;
...
Процедура СохранитьИзмененияКассыККМ(НастраиваемаяКассаККМ, СверятьДанныеССервисомОФД)
	...
	Если ЗначениеЗаполнено(НастраиваемаяКассаККМ) Тогда
		...
		КассаККМОбъект = НастраиваемаяКассаККМ.ПолучитьОбъект();
		КассаККМОбъект.СверятьДанныеССервисомОФД = СверятьДанныеССервисомОФД;
		// АПК:1327-выкл: ответственное чтение не требуется, так как конкурентная работа с данными маловероятна
		КассаККМОбъект.Записать();
		// АПК:1327-вкл:
...
Процедура ДрайверОборудованияНастройкаСоздание(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОбработкаДобавленияДравейра = Новый ОписаниеОповещения("ОбновитьСписокДрайверовОборудования", ЭтаФорма);
	ОткрытьФорму("Справочник.ДрайверыОборудования.Форма.ФормаСписка",,ЭтаФорма,,,,ОбработкаДобавленияДравейра,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
