...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ОтборИсходящий 		= Параметры.Исходящий;
	ОтборБанковскийСчетКонтрагента = Параметры.БанковскийСчетКонтрагента;
	ОтборБанковскийСчетОрганизации = Параметры.БанковскийСчетОрганизации;
...
&НаКлиенте
Процедура Настр_АнализироватьИсториюВыбораЗначенийРеквизитовПриИзменении(Элемент)
...
&НаКлиенте
Процедура Настр_АвтоматическиПодставлятьДокументыПриИзменении(Элемент)
	
	КритическоеИзменение = Истина;
	Если Настр_АвтоматическиПодставлятьДокументы
		И Настр_СпособЗачета = ПредопределенноеЗначение("Перечисление.СпособыЗачетаИРаспределенияПлатежей.Авто") Тогда
		
		Настр_СпособЗачета = ПредопределенноеЗначение("Перечисление.СпособыЗачетаИРаспределенияПлатежей.Вручную");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура Настр_ИспользоватьПомощникСозданияКонтрагентовСчетовДоговоровПриИзменении(Элемент)
	
	КритическоеИзменение = Истина;
	Если Настр_ИспользоватьПомощникСозданияСправочников Тогда
		Настр_АвтоматическоеСозданиеНенайденныхЭлементов = Ложь;
	КонецЕсли;	
	УправлениеФормой(ЭтотОбъект);
...
Процедура Настр_АвтоматическоеСозданиеНенайденныхЭлементовПриИзменении(Элемент)
	...
	КритическоеИзменение = Истина;
	Если Настр_АвтоматическоеСозданиеНенайденныхЭлементов Тогда
		Настр_ИспользоватьПомощникСозданияСправочников = Ложь;
	КонецЕсли;	
	УправлениеФормой(ЭтотОбъект);
...
&НаКлиенте
Процедура Настр_СпособЗачетаПриИзменении(Элемент)
	
	Если Настр_АвтоматическиПодставлятьДокументы
		И Настр_СпособЗачета = ПредопределенноеЗначение("Перечисление.СпособыЗачетаИРаспределенияПлатежей.Авто") Тогда
		Настр_АвтоматическиПодставлятьДокументы = Ложь;
	КонецЕсли;
...
Процедура ОткрытьСписокПоследнихВыбранныхЗначений(Команда)
	...
	Отбор.Вставить("Контрагент", ОтборКонтрагент);
	Отбор.Вставить("БанковскийСчетКонтрагента", ОтборБанковскийСчетКонтрагента);
	Отбор.Вставить("БанковскийСчетОрганизации", ОтборБанковскийСчетОрганизации);
	//Отбор.Вставить("Исходящий", ОтборИсходящий);
	Если ЗначениеЗаполнено(ОтборДоговор) Тогда
		Отбор.Вставить("Договор", ОтборДоговор);
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура("Отбор", Отбор);

	ОткрытьФорму("РегистрСведений.ИсторияЗначенийРеквизитовПриЗагрузкеИзКлиентаБанка.ФормаСписка", ПараметрыФормы, , , , , ,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПодготовитьФормуНаСервере()
	...
	Элементы.Настр_Программа.СписокВыбора.ЗагрузитьЗначения(
		УчетДенежныхСредствСервер.СписокСовместимыхПрограммКлиентовБанка(Истина));
		
	Элементы.ПроводитьПриЗагрузкеПоступлениеНаРасчетныйСчет.Заголовок = "Поступление на счет";
	Элементы.ПроводитьПриЗагрузкеСписаниеСРасчетногоСчета.Заголовок = "Расход со счета";
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.Настр_АвтоматическоеСозданиеНенайденныхЭлементов.Доступность = Не Форма.Настр_ИспользоватьПомощникСозданияСправочников;
	Элементы.Настр_ИспользоватьПомощникСозданияСправочников.Доступность = Не Форма.Настр_АвтоматическоеСозданиеНенайденныхЭлементов;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ЗаполнитьТаблицуПлатежныхДокументов(ПлатежноеПоручение)
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	УчетДенежныхСредствСервер.УстановитьБанковскийСчет(Настр_БанковскийСчет, Организация, ВалютаРегламентированногоУчета);
...
Процедура ПрочитатьНастройкиБанковскогоСчета()
	...
	Настр_АнализироватьИсториюВыбораЗначенийРеквизитов	= Настройки.АнализироватьИсториюВыбораЗначенийРеквизитов;
	Настр_АвтоматическиПодставлятьДокументы				= Настройки.АвтоматическиПодставлятьДокументы;
	Настр_ИспользоватьПомощникСозданияСправочников		= Настройки.ИспользоватьПомощникСозданияСправочников;
	Настр_СтатьяДДСВходящий								= Настройки.СтатьяДДСВходящий;
	Настр_СтатьяДДСИсходящий							= Настройки.СтатьяДДСИсходящий;
	Настр_СпособЗачета									= Настройки.СпособЗачета;
	Настр_НеУдалятьДокументыКоторыхНетВВыписке			= Настройки.НеУдалятьДокументыКоторыхНетВВыписке;
...
Процедура ПрочитатьНастройкиПользователя()
	...
	Настр_ПроводитьПриЗагрузкеСписаниеСРасчетногоСчета
		= Настройки.СозданиеДокументов[Тип("ДокументСсылка.РасходСоСчета")].ПроводитьДокумент;
	Настр_ПроводитьПриЗагрузкеПоступлениеНаРасчетныйСчет
		= Настройки.СозданиеДокументов[Тип("ДокументСсылка.ПоступлениеНаСчет")].ПроводитьДокумент;
...
Процедура ЗаписатьНастройки()
	...
	НастройкиЗагрузки.СозданиеДокументов[Тип("ДокументСсылка.РасходСоСчета")].ПроводитьДокумент
		= Настр_ПроводитьПриЗагрузкеСписаниеСРасчетногоСчета;
	НастройкиЗагрузки.СозданиеДокументов[Тип("ДокументСсылка.ПоступлениеНаСчет")].ПроводитьДокумент
		= Настр_ПроводитьПриЗагрузкеПоступлениеНаРасчетныйСчет;
	...
	Если ЗначениеЗаполнено(Настр_СоглашениеПрямогоОбменаСБанками) Тогда
		НастройкиЗагрузки.Вставить("СохранятьНастройкиДляАвтовыписки",
			Настр_СоглашениеПрямогоОбменаСБанками.ДоступноАвтоматическоеПолучениеВыписки);
	Иначе
		НастройкиЗагрузки.Вставить("СохранятьНастройкиДляАвтовыписки", Ложь);
	КонецЕсли;
	...
	ТаблицаДокументов = ЗаполнитьТаблицуПлатежныхДокументов(Настр_ПлатежноеПоручение);
	...
	НастройкиОбменаСКлиентомБанка.АнализироватьИсториюВыбораЗначенийРеквизитов	= Настр_АнализироватьИсториюВыбораЗначенийРеквизитов;
	НастройкиОбменаСКлиентомБанка.АвтоматическиПодставлятьДокументы				= Настр_АвтоматическиПодставлятьДокументы;
	НастройкиОбменаСКлиентомБанка.ИспользоватьПомощникСозданияСправочников		= Настр_ИспользоватьПомощникСозданияСправочников;
	НастройкиОбменаСКлиентомБанка.СтатьяДДСВходящий								= Настр_СтатьяДДСВходящий;
	НастройкиОбменаСКлиентомБанка.СтатьяДДСИсходящий							= Настр_СтатьяДДСИсходящий;
	НастройкиОбменаСКлиентомБанка.СпособЗачета									= Настр_СпособЗачета;
	НастройкиОбменаСКлиентомБанка.НеУдалятьДокументыКоторыхНетВВыписке			= Настр_НеУдалятьДокументыКоторыхНетВВыписке;
...
Процедура СохранитьНастройкиЗаполнения()
	...
	ИсходящиеПараметры.Вставить("АнализироватьИсториюВыбораЗначенийРеквизитов",	Настр_АнализироватьИсториюВыбораЗначенийРеквизитов);
	ИсходящиеПараметры.Вставить("АвтоматическиПодставлятьДокументы",  			Настр_АвтоматическиПодставлятьДокументы);
	ИсходящиеПараметры.Вставить("ИспользоватьПомощникСозданияСправочников",		Настр_ИспользоватьПомощникСозданияСправочников);
	ИсходящиеПараметры.Вставить("СтатьяДДСВходящий",							Настр_СтатьяДДСВходящий);
	ИсходящиеПараметры.Вставить("СтатьяДДСИсходящий",							Настр_СтатьяДДСИсходящий);
	ИсходящиеПараметры.Вставить("СпособЗачета",									Настр_СпособЗачета);
	ИсходящиеПараметры.Вставить("НеУдалятьДокументыКоторыхНетВВыписке",			Настр_НеУдалятьДокументыКоторыхНетВВыписке);
