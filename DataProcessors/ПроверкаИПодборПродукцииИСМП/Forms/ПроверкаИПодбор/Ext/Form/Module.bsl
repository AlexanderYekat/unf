...
Процедура ОбработатьИПроверитьПереданныеПараметры(Отказ)
	...
	ПараметрыСканированияСервер = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(ПроверяемыйДокумент,, ВидМаркируемойПродукции, ПараметрыРежимаИсправленияОшибок);
...
Процедура УправлениеЭлементамиФормыПриСоздании()
	...
	Если Не ЕстьОшибкиПроверкиККТ
		...
		ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования(
			ПроверяемыйДокумент, Неопределено, ВидМаркируемойПродукции, ПараметрыРежимаИсправленияОшибок);
...
Функция ТребуетсяВопросПередЗавершениемПроверки(ПропуститьПроверкуКоличества = Ложь)
	...
	ЕстьРасхожденияПоКодам = ИтогиПроверкиПриЗавершении.КоличествоОтсутствует > 0
	                         ...
	                         Или ИтогиПроверкиПриЗавершении.КоличествоНеЧислилосьОСУ > 0
	                         Или ИтогиПроверкиПриЗавершении.КоличествоОтсутствуетОСУ > 0;
				...
				Иначе
					...
					Иначе
						СоздаватьАктОРасхождениях =	ДополнительныеПараметры.СоздаватьАктОРасхождениях
							...
							Или ДополнительныеПараметры.ИтогиПроверкиПриЗавершении.КоличествоНепроверенныхОСУ > 0
							Или ДополнительныеПараметры.ИтогиПроверкиПриЗавершении.КоличествоОтложенныхОСУ > 0;
					КонецЕсли;
				КонецЕсли;
...
Процедура ИнициализироватьПараметрыСканирования(НачальнаяИнициализация = Ложь)
	...
	Если ПараметрыСканирования = Неопределено Или НачальнаяИнициализация Тогда
		ПараметрыСканирования = ШтрихкодированиеОбщегоНазначенияИСКлиент.ПараметрыСканирования(
			ВладелецФормы, Неопределено, ВидМаркируемойПродукции, ПараметрыРежимаИсправленияОшибок);
	КонецЕсли;
	...
	Если ИдентификаторТекущейПроверяемойУпаковки <> -1 Тогда
		...
		Если ТекущаяУпаковка <> Неопределено
			...
			Если ДанныеВыбора = Неопределено Тогда
				...
				ДанныеВыбора.Вставить("Номенклатура");
				ДанныеВыбора.Вставить("Характеристика");
				ДанныеВыбора.Вставить("Серия");
...
Процедура ДобавитьПредставлениеКоличестваУпаковок(СтрокиСКоличествомУпаковок, ПредставлениеУпаковки, Количество)
	Если Количество = 0 Тогда
		Возврат;
	КонецЕсли;
	СтрокаСКоличеством = ПолучитьСклоненияСтрокиПоЧислу(ПредставлениеУпаковки, Количество,, "ЧС=Количественное", "ПД=Именительный")[0];
