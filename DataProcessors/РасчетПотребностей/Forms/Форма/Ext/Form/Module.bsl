...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СпособПополнения", ОтборСпособПополнения) Тогда
	...
	ПроверкаПрав 								  = Справочники.ВариантыРасчетаПотребностей.ПустаяСсылка();
	ЕстьПравоСохраненияВариантаРасчетаПотребности = ПравоДоступа("Изменение", ПроверкаПрав.Метаданные());
	
	Элементы.ГруппаПроверкаПраваСохранениеНастроек.Видимость = ЕстьПравоСохраненияВариантаРасчетаПотребности;
...
Процедура УправлениеФормой(Знач ИмяРеквизита = "")
	...
	Если ПустаяСтрока(ИмяРеквизита) Тогда
		...
		ИначеЕсли ПараметрыФормы.Свойство("ЗаголовокПодборки") Тогда	
...
Процедура УстановитьУсловноеОформление()
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Запасы.Номенклатура", ОтборПоЗаказу);
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Шрифт", Новый Шрифт(, , Истина)); // Полужирный шрифт
...
Функция ЗаполнитьФильтрыИзПараметровФормы()
	...
	Если Параметры.Свойство("ЗаказПокупателя") Тогда
		...
		Если ПараметрыФормы.УчетПотребностиПоСкладам Тогда
			Склад 					= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.ЗаказПокупателя, "СтруктурнаяЕдиницаРезерв");
		КонецЕсли;

		ОтборПоЗаказу				= Параметры.ЗаказПокупателя;
...
Процедура ОбновитьПараметрыФормы()
	...
	Если Параметры.Свойство("ЗаказПокупателя", ЗаказПокупателя) Тогда
	...
	Если Параметры.Свойство("ЗаголовокПодборки", ЗаголовокПодборки) Тогда

		НужноСохранитьНастройкиОтборов  = Ложь;
		ПараметрыФормы.Вставить("ЗаголовокПодборки", ЗаголовокПодборки);
		
	ИначеЕсли Параметры.Свойство("Номенклатура") Тогда
		Параметры.Свойство("ЗаголовокПодборкиСпискаНоменклатуры", ЗаголовокПодборки);
		НужноСохранитьНастройкиОтборов = Ложь;
	КонецЕсли;                      
...
Процедура ОбновитьКурсыВалют()
	...
	Если Не ПараметрыФормы.Свойство("КурсыВалют") Тогда
...
&НаСервере
Процедура ВосстановитьНастройкиОтборов(Знач СохраненныйВариантРасчетаПотребностей = "")
	...
	Если РеквизитыФормы.Свойство("Настройки", СохраненныеНастройки)
		И ЗначениеЗаполнено(СохраненныеНастройки) Тогда
...
Процедура ВывестиРасшифровкуПотребности()
	...
	Если Элементы.ЗапасыСписок.ТекущиеДанные <> Неопределено Тогда
		...
		ЗаполнитьРасшфировкуСписка(ФоновоеЗаданиеАдресРезультата, ПоНоменклатуре);
...
&НаСервере
Процедура ЗаполнитьРасшфировкуСписка(АдресРезультата, ПоНоменклатуре)
	...
	ПолеЗаказ 					= Новый ПолеКомпоновкиДанных("ЗаказПокупателя");
		
	Для Каждого ЭлементОтбора Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
			
		Если ЭлементОтбора.Использование
			И ЭлементОтбора.ЛевоеЗначение = ПолеЗаказ Тогда
			ОтборПоЗаказу = ЭлементОтбора.ПравоеЗначение[0].Значение;
		КонецЕсли;
...
Процедура ОбновитьНаСервере(АдресРезультата)
	...
	Иначе
		...
		ЗаполнитьСписокНаФорме(СтруктураРезультат.Дерево.Строки, ЗапасыСписок, ОтборПоЗаказу, Склад);
...
&НаСервереБезКонтекста
Процедура ЗаполнитьСписокНаФорме(Строки, ЗапасыСписок, ЗаказПокупателя, Склад)
 		...
 		Для Каждого СтрокаРасчетаПотребностей Из СтрокаНоменклатуры.Строки Цикл
			...
			ИначеЕсли СтрокаРасчетаПотребностей.ИмяГруппировки = ИмяГруппировкиПоступление Тогда
				...
				Для Каждого СтрокаРасшифровки Из СтрокаРасчетаПотребностей.Строки Цикл
					...
					ИначеЕсли СтрокаРасшифровки.Заказ = ЗаказПокупателя Тогда
			...
			ИначеЕсли СтрокаРасчетаПотребностей.ИмяГруппировки = ИмяГруппировкиПотребность Тогда
				...
				Для Каждого СтрокаРасшифровки Из СтрокаРасчетаПотребностей.Строки Цикл
					...
					Если СтрокаРасшифровки.ИмяГруппировки = ИмяГруппировкиЗаказ Тогда
						...
						Если СтрокаРасшифровки.Заказ = ЗаказПокупателя Тогда
	...
	Для каждого СтрокаИсточник Из ТекСтр.ПолучитьЭлементы() Цикл
		...
		Если Изменено Тогда               
			...
			Если СтрокаИсточник.Лимит > 0
				...
				Лимит 					= ?(ЦелоеЧисло, ОкруглитьПотребность(СтрокаИсточник.Лимит), СтрокаИсточник.Лимит);
				СтрокаИсточник.Заказать = МИН(Лимит, Распределить);
...
Процедура РаспределитьПоЗаказам(ТекСтр, НеПересчитыватьГруппировки, ЦелоеЧисло = Ложь)
	...
	Для каждого ВложеннаяСтрока Из ТекСтр.ПолучитьЭлементы() Цикл
		...
		КонецЕсли; 
		Дефицит	 	 = ?(ЦелоеЧисло, ОкруглитьПотребность(ВложеннаяСтрока.Дефицит), ВложеннаяСтрока.Дефицит);
		Заказать 	 = Мин(Дефицит, Распределить);
...
Процедура РаспределитьПоПериодам(ТекСтр, НеПересчитыватьГруппировки, ЦелоеЧисло = Ложь)
	...
	Для каждого ВложеннаяСтрока Из ТекСтр.ПолучитьЭлементы() Цикл
		...
		КонецЕсли; 
		Дефицит	 	= ?(ЦелоеЧисло, ОкруглитьПотребность(ВложеннаяСтрока.Дефицит), ВложеннаяСтрока.Дефицит);
		Заказать 	= Мин(Распределить, Дефицит); 
		Изменено 	= (ВложеннаяСтрока.Заказать <> Заказать);
...
Процедура ОбновитьМеткиФильтров()
	...
	Для Каждого ОписаниеМетки Из ДанныеМеток Цикл
		...
		Если ТипЗнч(ОписаниеМетки.Метка) = Тип("СписокЗначений") И ЗначениеЗаполнено(ЗаголовокПодборки) Тогда
			ПредставлениеМетки = ЗаголовокПодборки;
		Иначе
...
Процедура ПоказатьСкрытьПанельРедактирования(ИзменяетДанные = Неопределено)
	...
	Элементы.ЗапасыСписокОтступ.Видимость = Элементы.ЗапасыПометка.Видимость;
