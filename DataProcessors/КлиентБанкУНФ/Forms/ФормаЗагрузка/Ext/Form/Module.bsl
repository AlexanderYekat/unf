...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("БанковскийСчетОрганизации") И ЗначениеЗаполнено(Параметры.БанковскийСчетОрганизации) Тогда
		Объект.БанковскийСчет = Параметры.БанковскийСчетОрганизации;
		ЭтаФорма.Заголовок = СтрШаблон(НСтр("ru = 'Загрузка выписок по счету: %1'"),
			Параметры.БанковскийСчетОрганизации.Наименование);
...
Функция ЗаполнитьДокументыНаИмпорт(ИмпортТекстДляРазбора)
	...
	Операция = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "Обработки.КлиентБанкУНФ.РазобратьБанковскуюВыписку", КонтекстФормы);
	Возврат Операция;
...
Процедура ПослеОбработкиБанковскойВыписки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПослеЗагрузкиВыписокВ1С Тогда
		...
		Если НЕ Результат.Свойство("СтруктураДанныхЗагрузки")
			ИЛИ ТипЗнч(Результат.СтруктураДанныхЗагрузки) <> Тип("Структура") Тогда
			Возврат;
		КонецЕсли;
		...
		Если НЕ Результат.СтруктураДанныхЗагрузки.Свойство("СообщенияПользователю") Тогда
			Результат.СтруктураДанныхЗагрузки.Вставить("СообщенияПользователю", Новый ФиксированныйМассив(Новый Массив));
		КонецЕсли;
		...
		#Иначе
			...
			ОткрытьФорму("Обработка.КлиентБанкУНФ.Форма.ФормаЗагрузкаДокументовЗавершена",
				ПараметрыЗавершения,
...
Процедура НастроитьФормуМобильныйКлиент()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПериод", "Заголовок", НСтр("ru = 'Период'"));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПериод", "ОтображатьЗаголовок", Истина);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗагрузкаНачПериода", "Заголовок", НСтр("ru = 'С'"));
...
Процедура НастроитьВидимостьЭлементовНаСервере(ПолноеОбновлениеСтраницыПодвала = Ложь) Экспорт
	...
	Если КоличествоНеНайденныхКонтрагентов = 0 И КоличествоНеНайденныхРСчетов = 0 Тогда
		...
		Элементы.ДекорацияНеНайденныеРеквизиты.Заголовок = НСтр("ru = 'Новых контрагентов: 0. Р/счетов: 0'");
	Иначе
		Элементы.ДекорацияНеНайденныеРеквизиты.Гиперссылка = Истина;
		Элементы.ДекорацияНеНайденныеРеквизиты.Заголовок = СтрШаблон(НСтр(
			"ru = 'Новых контрагентов: %1. Р/счетов: %2'"), КоличествоНеНайденныхКонтрагентов,
			КоличествоНеНайденныхРСчетов);
	КонецЕсли;
...
Процедура ВывестиТаблицуРезультата()
	...
	Макет = Обработки.КлиентБанкУНФ.ПолучитьМакет("ПротоколЗагрузки");
...
Процедура ЭлектроннаяВыпискаБанкаНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ВыпискиБанка.Количество() > 1 Тогда
		...
		ОткрытьФорму("Обработка.КлиентБанкУНФ.Форма.ФормаСпискаВыписок", ПараметрыФормы);
...
Процедура ЗагрузкаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "ЗагрузкаНеВерныйФорматСуммыКартинка" Тогда
		...
		Иначе
			...
			ОткрытьФорму("Обработка.КлиентБанкУНФ.Форма.ФормаПросмотраСтроки", Новый Структура("ДанныеСтроки", СтруктураСтроки), ЭтаФорма, УникальныйИдентификатор,
...
Процедура Настройка(Команда)
	...
	ОткрытьФорму("Обработка.КлиентБанкУНФ.Форма.ФормаНастройка",
	...
	Для каждого СтрокаДокумента Из Объект.Загрузка Цикл
		...
		Если ТипЗнч(СтрокаДокумента.Контрагент) = Тип("Строка")
		 ИЛИ ТипЗнч(СтрокаДокумента.СчетКонтрагента) = Тип("Строка") Тогда
			...
			Обработки.КлиентБанкУНФ.СписокНенайденных(СтрокаДокумента, Объект.БанковскийСчет, ВременнаяТаблицаКонтрагентов, КоличествоНеНайденныхКонтрагентов, КоличествоНеНайденныхРСчетов);
...
Процедура ДекорацияНеНайденныеРеквизитыНажатие(Элемент)
	...
	ОткрытьФорму("Обработка.КлиентБанкУНФ.Форма.ФормаНеНайденныхКонтрагентов", ПараметрыФормыПросмотраФайла, ЭтотОбъект);
...
Процедура ОткрытьФормуРедактированияНазначения(ДанныеСтроки)
	...
	ОткрытьФорму("Обработка.КлиентБанкУНФ.Форма.РедактированиеНазначения", ПараметрыФормы, , , , , ОписаниеОповещения,
...
Процедура ОткрытьФормуРедактированияНазначенияМножественная()
	...
	ОткрытьФорму(
		"Обработка.КлиентБанкУНФ.Форма.РедактированиеНазначения", 
...
Процедура ЗагрузитьТекущийПлатеж(Команда)
	...
	Если Элементы.Загрузка.ТекущиеДанные = Неопределено Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Выберите платеж (строку), который хотите загрузить'");
...
Процедура ОткрытьФайл(Команда)
	...
	ОткрытьФорму("Обработка.КлиентБанкУНФ.Форма.ФормаФайла", ПараметрыФормыПросмотраФайла, ЭтотОбъект);
...
Процедура ОткрытьФормуРедактированияКонтрагентов(ДанныеСтроки, ИмяЗакладкиНаФорме)
	...
	ОткрытьФорму(
		"Обработка.КлиентБанкУНФ.Форма.РедактированиеКонтрагентаДоговора",
		СтруктураПараметров,
		,
		,
		,
...
Процедура ОткрытьФормуРедактированияСтатьиДДС(ДанныеСтроки)
	...
	ОткрытьФорму("Обработка.КлиентБанкУНФ.Форма.РедактированиеСтатьиДДС", ПараметрыФормы, , , , , ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОткрытьФормуРедактированияСтатьиДДСМножественная()
	...
	ОткрытьФорму(
		"Обработка.КлиентБанкУНФ.Форма.РедактированиеСтатьиДДС", 
...
Процедура ОткрытьФормуРедактированияСчетаКорреспонденции(ДанныеСтроки)
	...
	ОткрытьФорму(
		"Обработка.КлиентБанкУНФ.Форма.РедактированиеКорреспонденции", 
...
Функция ЗагрузкаПлатежноеПоручениеПриИзмененииНаСервере(ПлатежноеПоручение)
	...
	ИначеЕсли ПлатежноеПоручение.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.РасчетыПоКредитам") Тогда
...
Функция ДлительнаяОперацияЗагрузкиВыписки()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(УникальныйИдентификатор);
	Операция = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "Обработки.КлиентБанкУНФ.Загрузить", КонтекстФормы);
...
Процедура ПослеЗагрузкиБанковскойВыпискиНаСервере(Результат)
	...
	Если КонтекстФормы.ОбработкаПрервана Тогда
		ОбщегоНазначения.СообщитьПользователю(КонтекстФормы.ТекстСообщения, ЭтаФорма);
		Возврат;
	КонецЕсли;
