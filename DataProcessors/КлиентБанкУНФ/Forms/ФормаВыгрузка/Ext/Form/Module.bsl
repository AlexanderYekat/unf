...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("БанковскийСчетОрганизации")
	   И ЗначениеЗаполнено(Параметры.БанковскийСчетОрганизации) Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ru = 'Выгрузка платежных поручений из 1С в банк'"); //: " + Параметры.БанковскийСчетОрганизации.Наименование;
	Иначе
...
Процедура Настройка(Команда)
	...
	ОткрытьФорму("Обработка.КлиентБанкУНФ.Форма.ФормаНастройка", ПараметрыОткрытияФормы);
...
Процедура ЗаполнитьВыгрузка()
	...
	Запрос = Новый Запрос;
	Запрос.Текст = Обработки.КлиентБанкУНФ.ПолучитьТекстЗапросаПоЗаполнениюТабличнойЧастиВыгрузка();
...
Процедура ПоказатьФормуКонтроляБезопасности(ИмяФайла, АдресВоВременномХранилище)
	...
	Если Объект.КонтролироватьБезопасностьОбменаСБанком
		...
		ОткрытьФорму("Обработка.КлиентБанкУНФ.Форма.КонтрольБезопасностиОбменаСБанком", ПараметрыФормы, , , , , );
	КонецЕсли;
...
Процедура ЗаписатьОшибкуВЖурнал(ТекстОшибки)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'КлиентБанкУНФ.СохранитьФайлВыгрузки'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура УправлениеФормойНаСервере()
	...
	КонецЕсли;
	Элементы.ВыгрузкаСуммаДокумента.Заголовок = СтрШаблон(НСтр("ru = 'Сумма, %1'"), ПредставлениеВалютыДС);
