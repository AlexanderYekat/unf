...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НеприменениеККТ Тогда
		ЗаполнениеОбъектовУНФ.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ДополнительныйРеквизит");
	КонецЕсли;
...
Процедура ЗаполнитьНаОсновании(Основание, Отказ) Экспорт
	...
	Если ФискальнаяОперация.ОбщиеПараметры = Неопределено Тогда
		ДанныеXMLКорректируемогоЧека = ФискальнаяОперация.ДанныеXML.Получить();
		ОбщиеПараметры = ОборудованиеЧекопечатающиеУстройстваВызовСервера.ЗагрузитьДанныеФискализацииИзXML(ДанныеXMLКорректируемогоЧека);
	Иначе
		ОбщиеПараметры = ФискальнаяОперация.ОбщиеПараметры;
	...
	Иначе
		Контрагент = ОбщиеПараметры.СведенияОПокупателе.Покупатель;
	КонецЕсли;
	Если ЗначениеЗаполнено(ОбщиеПараметры.ПолучательИНН) Тогда
		КонтрагентИНН = ОбщиеПараметры.ПолучательИНН;
	Иначе
		КонтрагентИНН = ОбщиеПараметры.СведенияОПокупателе.ПокупательИНН;
	КонецЕсли;
	...
	Если ТипЗнч(ФискальнаяОперация.ДокументОснование) = Тип("ДокументСсылка.ЧекККМКоррекции") Тогда
		...
		НеприменениеККТОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ФискальнаяОперация.ДокументОснование, 
																	"НеприменениеККТ");
		
		Если НеприменениеККТОснования Тогда
...
Функция ДанныеФискальнойОперации(Основание)
	...
	|	ФискальныеОперации.ДанныеXML
	|ИЗ
	|	Документ.ЧекККМКоррекции КАК ЧекКоррекции
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФискальныеОперации КАК ФискальныеОперации
	|		ПО ЧекКоррекции.Ссылка = ФискальныеОперации.ДокументОснование
	|ГДЕ
	|	ЧекКоррекции.Проведен
	|	И ЧекКоррекции.ПробитЧек
	|	И ЧекКоррекции.ДокументРасчетов = &ДокументРасчетов
	|	И ЧекКоррекции.НеприменениеККТ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Если ВыборкаПоследняяФискальнаяОперация.Следующий() Тогда
		...
		ФискальнаяОперация = НоваяСтруктураФискальнаяОперация();
		...
		Если ВыборкаДатаРасчетов.Следующий() Тогда
			ФискальнаяОперация.ДатаРасчета = ВыборкаДатаРасчетов.ДатаРасчетов;
		Иначе
			ФискальнаяОперация.ДатаРасчета = Основание.Дата;
		КонецЕсли;
		
		ФискальнаяОперация.ДокументРасчетов = ДокументРасчетов;
	...
	Иначе
		...
		ФискальнаяОперация.ДатаРасчета = Основание.Дата;
		ФискальнаяОперация.ДокументОснование = Основание;
		ФискальнаяОперация.Организация = Основание.Организация;
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("СтруктурнаяЕдиница", Основание.Метаданные()) Тогда
			ФискальнаяОперация.СтруктурнаяЕдиница = Основание.СтруктурнаяЕдиница;
		ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("СтруктурнаяЕдиницаРезерв", Основание.Метаданные()) Тогда
			ФискальнаяОперация.СтруктурнаяЕдиница = Основание.СтруктурнаяЕдиницаРезерв;
		КонецЕсли;
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЧекККМ")
			Или ТипЗнч(Основание) = Тип("ДокументСсылка.ЧекККМВозврат") Тогда
			ФискальнаяОперация.ОбщиеПараметры = РабочееМестоКассира.ПодготовитьДанныеДляПробитияЧека(Основание);
		Иначе
			ФискальнаяОперация.ОбщиеПараметры = РозничныеПродажиСервер.ПолучитьДанныеДляФискализации(Основание);
		КонецЕсли;
...
Процедура ЗаполнитьПозицииЧекаИзXML(ПозицииЧекаXML)
	...
	Для Каждого ПозицияЧека Из ПозицииЧекаXML Цикл
		...
		Если Не ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
			Продолжить;
		КонецЕсли;
...
Функция НоваяСтруктураФискальнаяОперация()
	
	ФискальнаяОперация = Новый Структура;
	ФискальнаяОперация.Вставить("ДатаРасчета");
	ФискальнаяОперация.Вставить("ДокументРасчетов");
	...
	ФискальнаяОперация.Вставить("ОбщиеПараметры");
	ФискальнаяОперация.Вставить("ЕстьКорректирующийДокумент", Ложь);
