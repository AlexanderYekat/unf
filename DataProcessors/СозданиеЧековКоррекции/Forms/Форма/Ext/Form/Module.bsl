...
Процедура ПробитьЧекВыполнитьЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		ЗаписатьЧекКоррекцииПослеПробития(РезультатВыполнения);
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Для Каждого КодСНО Из МассивКодовСНО Цикл
			...
			Попытка
				...
				ПоддерживаемыеСНОКассойККМ.Добавить(ОборудованиеЧекопечатающиеУстройстваКлиентСервер.СистемаНалогообложенияККТПоКоду(КодСНОЧисло));
			Исключение
			КонецПопытки;
...
Процедура СоздатьЧеки(Команда)
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если (Объект.НеприменениеККТ ИЛИ Объект.ТипКоррекции = 2) И Объект.ЧекСкорректированныйПроведен Тогда
			
			ПробитьЧекВыполнить(Объект.ЧекСкорректированный, Ложь);
			
		ИначеЕсли Объект.ЧекСторноПроведен И Объект.ЧекСкорректированныйПроведен Тогда
...
Процедура УправлениеВидомостьюИДоступностью()
	...
	Если Объект.НеприменениеККТ Тогда
		Элементы.ГруппаЧекСторно.Видимость = Ложь;
		Элементы.ДополнительныйРеквизит.Видимость = Ложь;
		Элементы.ФормаСоздатьЧеки.Заголовок = Нстр("ru = 'Создать чек'");
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ЗаписатьЧекКоррекцииПослеПробития(РезультатВыполнения)
	
	ЧекКоррекцииОбъект = РезультатВыполнения.ПараметрыФискализации.ДокументОснование.ПолучитьОбъект();
	
	ЧекКоррекцииОбъект.НомерСмены = РезультатВыполнения.НомерСменыККТ;
	ЧекКоррекцииОбъект.НомерЧека = РезультатВыполнения.НомерЧекаККТ;
...
Процедура ЗаполнитьСпискиВыбораЭлементов()
	...
	Если Не Объект.НеприменениеККТ Тогда
		Элементы.ОписаниеКоррекции.СписокВыбора.Добавить(НСтр("ru = 'Ошибка в указании ставки НДС при применении ККТ'"));
	КонецЕсли;
