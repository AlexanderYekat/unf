...
Функция ПолучитьИРазобратьШтрихкодыДокумента(Документ, ПараметрыСканирования)
	...
	Для Каждого СтрокаТаблицы Из ШтрихкодыУпаковокПоДокументу Цикл
		...
		ДанныеШтрихкода.Штрихкод = СтрокаТаблицы.Штрихкод;
...
Процедура ВыполнитьОтдельныеПроверкиПродавцаИПокупателя(ПараметрыОбработки, ЭтоДокументПриобретения)
	...
	Если ОрганизацияЗарегистрированаВГИСМТ
		И ПараметрыОбработки.ТипДокумента = Перечисления.ТипыДокументовГИСМТ.УПД Тогда
...
Функция СформироватьИтоговыйРезультатПоИтогамОтдельныхПроверок(ПараметрыОбработки)
...
Функция НаличиеОшибокВКодахМаркировки(ПараметрыОбработки)
	...
	Если РезультатЗапросаСтатусов.ТребуетсяОбновлениеКлючаСессии Тогда
		РезультатПроверки.ТребуетсяОбновлениеКлючаСессии = Истина;
		Возврат РезультатПроверки;
	ИначеЕсли РезультатЗапросаСтатусов.СтатусыКодовМаркировки = Неопределено Тогда
...
Функция ЗапроситьИдентификатораАбонентаВЭДОLite(Организация, ИНН)
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			ВозвращаемоеЗначение.ИдентификаторАбонента = РезультатОтправкиЗапроса.ТекстВходящегоСообщенияJSON;
