...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтображатьБонусы = ПолучитьФункциональнуюОпцию("ИспользоватьБонусныеПрограммы");
 	ОтображатьСертификаты = ПолучитьФункциональнуюОпцию("ИспользоватьПодарочныеСертификаты");
	ОтображатьДисконтныеКарты = ПолучитьФункциональнуюОпцию("ИспользоватьДисконтныеКарты");
	ОтображатьПрочее = ОтображатьСчетаУчета 
								ИЛИ ОтображатьБонусы
								ИЛИ ОтображатьСертификаты
								ИЛИ ОтображатьДисконтныеКарты;
	...
	// ГТД и РНПТ
	ИменаТЧИПолей = ИменаТЧИПолейДляТаблицыЗапасы_БазовыеПоляСПрослеживаемостью(, "ЗапасыВРазрезеГТД");
	ИменаТЧИПолей = ИменаТЧИПолейДляТаблицыЗапасы_БазовыеПоляСПрослеживаемостью(ИменаТЧИПолей, "ЗапасыПереданныеВРазрезеГТД");
	ИменаТЧИПолей = ИменаТЧИПолейДляТаблицыЗапасы_БазовыеПоляСПрослеживаемостью(ИменаТЧИПолей, "ЗапасыПринятыеВРазрезеГТД");
	...
	Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДКомиссионер.Видимость = КэшЗначений.Свойство("ВестиУчетПрослеживаемыхТоваров") И КэшЗначений.ВестиУчетПрослеживаемыхТоваров;
	...
	Настройки.Вставить("ЗапасыПринятые");
	Настройки.Вставить("ЗапасыПереданные");
	Настройки.Вставить("ЗапасыВРазрезеГТД");
	Настройки.Вставить("РасчетыСПоставщиками");
	Настройки.Вставить("РасчетыСПокупателями");
	Настройки.Вставить("РасчетыСПерсоналом");
	Настройки.Вставить("ВнеоборотныеАктивы");
	Настройки.Вставить("РасчетыПоКредитамИЗаймам");
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ПриСозданииНаСервере(
		Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.РасчетыПоКредитамИЗаймам,
		Настройки.РасчетыПоКредитамИЗаймам, ЭтотОбъект, Ложь);
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ОтображатьСчетаУчета Тогда
		ПоследняяСтраница = 4;
	КонецЕсли;
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗакупкиКУДиРОтдельнымДокументомПриИзменении()
	ОбновитьОтображениеРеквизитовЗакупокУСН();	
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияЗакупкиУСНВключитьОтчетностьНажатие(Элемент)
...
Процедура ВводНачальныхОстатковТоварыЗапасыПередУдалением(Элемент, Отказ)
	...
	ТабличныеЧастиУНФКлиентСервер.УдалитьСтрокиПоКлючуСвязи(ВводНачальныхОстатковТовары.ЗакупкиДляКУДИР, СтрокаТабличнойЧасти);
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
Процедура ВводНачальныхОстатковТоварыЗапасыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ЗапасыНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличествоПриИзмененииНоменклатуры, Стр);
...
Процедура ЗапасыПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПоискПоШтрихКоду") И ДополнительныеПараметры.ЭтоПоискПоШтрихКоду Тогда
		...
		ДополнительныеПараметры.Вставить("ИмяТекущейСтраницы", "СтраницаСобственныеТовары");
...
&НаКлиенте
Процедура ЗапасыНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество, СтрокаТЧ = Неопределено)
	...
	СтруктураДанные.Вставить("ДатаОбработки", ДатаОстатков);
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыСтавкаНДСПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасы.ТекущиеДанные;
	РассчитатьСуммуНДС(СтрокаТабличнойЧасти);
КонецПроцедуры
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыСуммаНДСПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасы.ТекущиеДанные;
	СтрокаТабличнойЧасти.СуммаБезНДС = СтрокаТабличнойЧасти.Сумма - СтрокаТабличнойЧасти.СуммаНДС;
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыСуммаБезНДСПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасы.ТекущиеДанные;
	СтрокаТабличнойЧасти.СуммаНДС = СтрокаТабличнойЧасти.Сумма - СтрокаТабличнойЧасти.СуммаБезНДС;
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыНомерДокументаКУДиРПриИзменении(Элемент)
	ОбновитьСтрокиЗапасыКУДиР(Элементы.ВводНачальныхОстатковТоварыЗапасы.ТекущаяСтрока, "НомерДокументаКУДиР");
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыДатаДокументаКУДиРПриИзменении(Элемент)
	ОбновитьСтрокиЗапасыКУДиР(Элементы.ВводНачальныхОстатковТоварыЗапасы.ТекущаяСтрока, "ДатаДокументаКУДиР");
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыСуммаОплатыКУДиРПриИзменении(Элемент)
	ОбновитьСтрокиЗапасыКУДиР(Элементы.ВводНачальныхОстатковТоварыЗапасы.ТекущаяСтрока, "СуммаОплатыКУДиР");
КонецПроцедуры
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыСуммаНДСКУДиРПриИзменении(Элемент)
	ОбновитьСтрокиЗапасыКУДиР(Элементы.ВводНачальныхОстатковТоварыЗапасы.ТекущаяСтрока, "СуммаНДСКУДиР");
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПриАктивизацииЯчейки(Элемент)
	...
	Если Не Элемент.ТекущийЭлемент = Неопределено И Элемент.ТекущийЭлемент.Имя = "ВводНачальныхОстатковТоварыЗапасыКоличество" Тогда
		Элементы.ВводНачальныхОстатковТоварыЗапасыКоличество.ФорматРедактирования = ?(СтрокаТабличнойЧасти.ЗапретитьВводДробногоКоличества И УстанавливатьФорматРедактированияЗапретаДробногоКоличества, "ЧДЦ=0", "");
	КонецЕсли;
...
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеПриАктивизацииЯчейки(Элемент)
	...
	Если Не Элемент.ТекущийЭлемент = Неопределено И Элемент.ТекущийЭлемент.Имя = "ВводНачальныхОстатковТоварыЗапасыПринятыеКоличество" Тогда
		Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеКоличество.ФорматРедактирования = ?(СтрокаТабличнойЧасти.ЗапретитьВводДробногоКоличества И УстанавливатьФорматРедактированияЗапретаДробногоКоличества, "ЧДЦ=0", "");
	КонецЕсли;
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(Стр, ДанныеДляЗаполнения, ИменаРеквизитовАвтозаполнения);
	Стр.Номенклатура = ВыбранноеЗначение;
	ЗапасыПринятыеНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличествоПриИзмененииНоменклатуры, Стр);
...
Процедура ЗапасыПринятыеПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПоискПоШтрихКоду") И ДополнительныеПараметры.ЭтоПоискПоШтрихКоду Тогда
		...
		ДополнительныеПараметры.Вставить("ИмяТекущейСтраницы", "СтраницаПринятыеТовары");
		ЗаполнитьПоДаннымШтрихкодовЗавершение(Неопределено, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ЗапасыПринятыеНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество, СтрокаТЧ = Неопределено)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеПриАктивизацииЯчейки(Элемент)
	...
	Если Не Элемент.ТекущийЭлемент = Неопределено И Элемент.ТекущийЭлемент.Имя = "ВводНачальныхОстатковТоварыЗапасыПереданныеКоличество" Тогда
		Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеКоличество.ФорматРедактирования = ?(СтрокаТабличнойЧасти.ЗапретитьВводДробногоКоличества И УстанавливатьФорматРедактированияЗапретаДробногоКоличества, "ЧДЦ=0", "");
	КонецЕсли;
...
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(Стр, ДанныеДляЗаполнения, ИменаРеквизитовАвтозаполнения);
	Стр.Номенклатура = ВыбранноеЗначение;
	ЗапасыПереданныеНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличествоПриИзмененииНоменклатуры, Стр);
...
Процедура ЗапасыПереданныеПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПоискПоШтрихКоду") И ДополнительныеПараметры.ЭтоПоискПоШтрихКоду Тогда
		...
		ДополнительныеПараметры.Вставить("ИмяТекущейСтраницы", "СтраницаПереданныеТовары");
		ЗаполнитьПоДаннымШтрихкодовЗавершение(Неопределено, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ЗапасыПереданныеНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество, СтрокаТЧ = Неопределено)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДХарактеристикаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДПартияПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТД.ТекущиеДанные;
	// Прослеживаемость
	УдалитьСвязанныеЗаписи(СтрокаТабличнойЧасти);
	// Конец Прослеживаемость
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДКоличествоПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТД.ТекущиеДанные;
	// Прослеживаемость
	УдалитьСвязанныеЗаписи(СтрокаТабличнойЧасти);
	// Конец Прослеживаемость
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДЕдиницаИзмеренияПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТД.ТекущиеДанные;
	// Прослеживаемость
	УдалитьСвязанныеЗаписи(СтрокаТабличнойЧасти);
	// Конец Прослеживаемость
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	СтрокаТабличнойЧасти = ВводНачальныхОстатковТовары.ЗапасыВРазрезеГТД.НайтиПоИдентификатору(ВыбраннаяСтрока);
	
	// Прослеживаемость
	Если Поле = Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДРНПТ И СтрокаТабличнойЧасти.ПрослеживаемыйТовар Тогда
		ОткрытьФормуПодбораРНПТ(ЭтаФорма, "ЗапасыВРазрезеГТД");
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
	
	Если Поле.Имя = "ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДПрослеживаемыйТовар" Тогда
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДПриАктивизацииЯчейки(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТД.ТекущиеДанные;
	...
	Если ЭтоНоваяСтрокаЗапасы
		И Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТД.ТекущийЭлемент = Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДРНПТ 
	...
	Если Не Элемент.ТекущийЭлемент = Неопределено И Элемент.ТекущийЭлемент.Имя = "ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДКоличество" Тогда
		Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДКоличество.ФорматРедактирования = ?(СтрокаТабличнойЧасти.ЗапретитьВводДробногоКоличества И УстанавливатьФорматРедактированияЗапретаДробногоКоличества, "ЧДЦ=0", "");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ЭтоНоваяСтрокаЗапасы = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДПриРНПТПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДРНПТНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОткрытьФормуПодбораРНПТ(ЭтаФорма, "ЗапасыВРазрезеГТД");
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДРНПТОчистка(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДПередУдалением(Элемент, Отказ)
		// Подчиненные табличные части
	Если НЕ Отказ Тогда
		Для Каждого ВыделеннаяСтрока Из Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТД.ВыделенныеСтроки Цикл
			ТекущиеДанныеСтроки = Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТД.ДанныеСтроки(ВыделеннаяСтрока);
			// Прослеживаемость
...
Процедура ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Стр.Номенклатура = ВыбранноеЗначение;
	ЗапасыВРазрезеГТДНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличествоПриИзмененииНоменклатуры, Стр);
...
Процедура ЗапасыВРазрезеГТДПодтверждениеСбросаКоличестваЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ЭтоПоискПоШтрихКоду") И ДополнительныеПараметры.ЭтоПоискПоШтрихКоду Тогда
		...
		ДополнительныеПараметры.Вставить("ИмяТекущейСтраницы", "СтраницаТоварыВРазрезеГТД");
		ЗаполнитьПоДаннымШтрихкодовЗавершение(Неопределено, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ЗапасыВРазрезеГТДНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество, СтрокаТЧ = Неопределено)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДХарактеристикаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДПартияПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТД.ТекущиеДанные;
	// Прослеживаемость
	УдалитьСвязанныеЗаписи(СтрокаТабличнойЧасти);
	// Конец Прослеживаемость
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДКоличествоПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТД.ТекущиеДанные;
	// Прослеживаемость
	УдалитьСвязанныеЗаписи(СтрокаТабличнойЧасти);
	// Конец Прослеживаемость
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДЕдиницаИзмеренияПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТД.ТекущиеДанные;
	// Прослеживаемость
	УдалитьСвязанныеЗаписи(СтрокаТабличнойЧасти);
	// Конец Прослеживаемость
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	СтрокаТабличнойЧасти = ВводНачальныхОстатковТовары.ЗапасыПереданныеВРазрезеГТД.НайтиПоИдентификатору(ВыбраннаяСтрока);
	
	// Прослеживаемость
	Если Поле = Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДРНПТ И СтрокаТабличнойЧасти.ПрослеживаемыйТовар Тогда
		ОткрытьФормуПодбораРНПТ(ЭтаФорма, "ЗапасыПереданныеВРазрезеГТД");
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
	
	Если Поле.Имя = "ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДПрослеживаемыйТовар" Тогда
		Если СтрокаТабличнойЧасти.ПрослеживаемыйТовар Тогда
			ПрослеживаемостьФормыКлиентУНФ.ПоказатьПредупреждениеПрослеживаемыйТовар();
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДПриАктивизацииЯчейки(Элемент)
	...
	// Прослеживаемость
	Если ЭтоНоваяСтрокаЗапасы
		И Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТД.ТекущийЭлемент = Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДРНПТ 
		И Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.РНПТ) Тогда
		ОткрытьФормуПодбораРНПТ(ЭтаФорма, "ЗапасыПереданныеВРазрезеГТД");
	КонецЕсли;
	// Конец Прослеживаемость
	...
	Если Не Элемент.ТекущийЭлемент = Неопределено И Элемент.ТекущийЭлемент.Имя = "ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДКоличество" Тогда
		Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДКоличество.ФорматРедактирования = ?(СтрокаТабличнойЧасти.ЗапретитьВводДробногоКоличества И УстанавливатьФорматРедактированияЗапретаДробногоКоличества, "ЧДЦ=0", "");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ЭтоНоваяСтрокаЗапасы = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДРНПТНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОткрытьФормуПодбораРНПТ(ЭтаФорма, "ЗапасыПереданныеВРазрезеГТД");
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДРНПТОчистка(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДПриРНПТНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДПередУдалением(Элемент, Отказ)
	// Подчиненные табличные части
	Если НЕ Отказ Тогда
		Для Каждого ВыделеннаяСтрока Из Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТД.ВыделенныеСтроки Цикл
			ТекущиеДанныеСтроки = Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТД.ДанныеСтроки(ВыделеннаяСтрока);
			// Прослеживаемость
			УдалитьСвязанныеЗаписи(ТекущиеДанныеСтроки);
			// Конец Прослеживаемость
		КонецЦикла;
...
Процедура ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Стр.Номенклатура = ВыбранноеЗначение;
	ЗапасыПереданныеВРазрезеГТДНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличествоПриИзмененииНоменклатуры, Стр);
	Если ЗначениеЗаполнено(Стр.СтранаПроисхождения) Тогда
		УдаляемыйРеквизит = "СтранаПроисхождения";
		ИменаРеквизитовАвтозаполнения = СтрЗаменить(ИменаРеквизитовАвтозаполнения, "," + УдаляемыйРеквизит, "");
		ИменаРеквизитовАвтозаполнения = СтрЗаменить(ИменаРеквизитовАвтозаполнения, УдаляемыйРеквизит, "");
...
&НаКлиенте
Процедура ЗапасыПереданныеВРазрезеГТДНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество, СтрокаТЧ = Неопределено)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДХарактеристикаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДПартияПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТД.ТекущиеДанные;
	// Прослеживаемость
	УдалитьСвязанныеЗаписи(СтрокаТабличнойЧасти);
	// Конец Прослеживаемость
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДКоличествоПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТД.ТекущиеДанные;
	// Прослеживаемость
	УдалитьСвязанныеЗаписи(СтрокаТабличнойЧасти);
	// Конец Прослеживаемость
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДЕдиницаИзмеренияПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТД.ТекущиеДанные;
	// Прослеживаемость
	УдалитьСвязанныеЗаписи(СтрокаТабличнойЧасти);
	// Конец Прослеживаемость
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	СтрокаТабличнойЧасти = ВводНачальныхОстатковТовары.ЗапасыПринятыеВРазрезеГТД.НайтиПоИдентификатору(ВыбраннаяСтрока);
	
	// Прослеживаемость
	Если Поле = Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДРНПТ И СтрокаТабличнойЧасти.ПрослеживаемыйТовар Тогда
		ОткрытьФормуПодбораРНПТ(ЭтаФорма, "ЗапасыПринятыеВРазрезеГТД");
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
	
	Если Поле.Имя = "ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДПрослеживаемыйТовар" Тогда
		Если СтрокаТабличнойЧасти.ПрослеживаемыйТовар Тогда
			ПрослеживаемостьФормыКлиентУНФ.ПоказатьПредупреждениеПрослеживаемыйТовар();
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДПриАктивизацииЯчейки(Элемент)
	...
	// Прослеживаемость
	Если ЭтоНоваяСтрокаЗапасы
		И Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТД.ТекущийЭлемент = Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДРНПТ 
		И Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.РНПТ) Тогда
		ОткрытьФормуПодбораРНПТ(ЭтаФорма, "ЗапасыПринятыеВРазрезеГТД");
	КонецЕсли;
	// Конец Прослеживаемость
	...
	Если Не Элемент.ТекущийЭлемент = Неопределено И Элемент.ТекущийЭлемент.Имя = "ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДКоличество" Тогда
		Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДКоличество.ФорматРедактирования = ?(СтрокаТабличнойЧасти.ЗапретитьВводДробногоКоличества И УстанавливатьФорматРедактированияЗапретаДробногоКоличества, "ЧДЦ=0", "");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ЭтоНоваяСтрокаЗапасы = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДПриРНПТНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДРНПТНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОткрытьФормуПодбораРНПТ(ЭтаФорма, "ЗапасыПринятыеВРазрезеГТД")
КонецПроцедуры

&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДРНПТОчистка(Элемент, СтандартнаяОбработка)
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТД.ТекущиеДанные;
...
&НаКлиенте
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДПередУдалением(Элемент, Отказ)
	// Подчиненные табличные части
	Если НЕ Отказ Тогда
		Для Каждого ВыделеннаяСтрока Из Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТД.ВыделенныеСтроки Цикл
			ТекущиеДанныеСтроки = Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТД.ДанныеСтроки(ВыделеннаяСтрока);
			// Прослеживаемость
			УдалитьСвязанныеЗаписи(ТекущиеДанныеСтроки);
			// Конец Прослеживаемость
		КонецЦикла;
...
Процедура ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Стр.Номенклатура = ВыбранноеЗначение;
	ЗапасыПринятыеВРазрезеГТДНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличествоПриИзмененииНоменклатуры, Стр);
	Если ЗначениеЗаполнено(Стр.СтранаПроисхождения) Тогда
		УдаляемыйРеквизит = "СтранаПроисхождения";
		ИменаРеквизитовАвтозаполнения = СтрЗаменить(ИменаРеквизитовАвтозаполнения, "," + УдаляемыйРеквизит, "");
		ИменаРеквизитовАвтозаполнения = СтрЗаменить(ИменаРеквизитовАвтозаполнения, УдаляемыйРеквизит, "");
...
&НаКлиенте
Процедура ЗапасыПринятыеВРазрезеГТДНоменклатураПриИзмененииПродолжить(Элемент, НовоеКоличество, СтрокаТЧ = Неопределено)
...
#Область РасчетыСПрочимиКонтрагентами

&НаКлиенте
Процедура ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиРасчетыСПрочимиКонтрагентамиКонтрагентПриИзменении(Элемент)
	
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиРасчетыСПрочимиКонтрагентами.ТекущиеДанные;
	
	СтруктураДанные = ПолучитьДанныеКонтрагентПриИзменении(СтрокаТабличнойЧасти.Контрагент, Организация, "РасчетыСПрочимиКонтрагентами");
	СтрокаТабличнойЧасти.Договор = СтруктураДанные.Договор;
...
&НаКлиенте
Процедура ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиРасчетыСПрочимиКонтрагентамиСуммаВалПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиРасчетыСПрочимиКонтрагентамиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	Если НЕ ОтображатьСчетаУчета И НоваяСтрока Тогда
		Элемент.ТекущиеДанные.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Управленческий.РасчетыСРазнымиДебиторамиИКредиторами");
	КонецЕсли;
...
Процедура ДатаОстатковПриИзмененииНаСервере()
	...
	// Прослеживаемость
	КэшЗначений.Вставить("ВестиУчетПрослеживаемыхТоваров", ПрослеживаемостьУНФ.ВедетсяУчетПрослеживаемыхТоваров(ДатаОстатков));
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	ЗагрузитьВводыОстатков();
	Если НЕ ЗначениеЗаполнено(ДатаОстатков) Тогда
		ДатаОстатков = ОбщегоНазначенияКлиент.ДатаСеанса();
	КонецЕсли;
...
Процедура НастройкаСбросаКоличества(Команда)
	...
	СтатистикаИспользованияФормКлиент.ПриИнтерактивномДействии(ЭтотОбъект, 
		Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДКонтекстноеМенюНастройкаСбросаКоличества, "Нажатие");
...
Процедура ЗагрузитьВводыОстатков()
	...
	КонецЕсли; 
	Если ЗначениеЗаполнено(ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами.Ссылка) Тогда
		ЗначениеВРеквизитФормы(Документы.ВводНачальныхОстатков.СоздатьДокумент(), "ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами");
	КонецЕсли;
	Если ЗначениеЗаполнено(ВводНачальныхОстатковСкидкиБонусы.Ссылка) Тогда
		ЗначениеВРеквизитФормы(Документы.ВводНачальныхОстатков.СоздатьДокумент(), "ВводНачальныхОстатковСкидкиБонусы");
	КонецЕсли; 
	Если ЗначениеЗаполнено(ВводНачальныхОстатковРасчетыПоКредитамИЗаймам.Ссылка) Тогда
		РеквизитОбъект = Документы.ВводНачальныхОстатков.СоздатьДокумент();
		ЗначениеВРеквизитФормы(РеквизитОбъект, "ВводНачальныхОстатковРасчетыПоКредитамИЗаймам");
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("Комментарий",
		Документы.ВводНачальныхОстатков.КомментарийДокументВведенПомощникомВводаОстатков());
	...
	Пока ВыборкаРезультатЗапроса.Следующий() Цикл
		Если ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.ДенежныеСредства Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковБанкИКасса");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.РасчетыСПодотчетниками Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковПодотчет");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.Запасы Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковТовары");
			ЗаполнитьЦены(Ложь,, Ложь);
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.РасчетыСПоставщикамиИПокупателями Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковРасчетыСКонтрагентами");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.РасчетыСПерсоналом Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковЗарплата");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.РасчетыПоНалогам Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковНалоги");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.Эквайринг Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковЭквайринг");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.Имущество Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковИмущество");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.ПрочиеРазделы Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковПрочее");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.ЗакупкиТоваровДляУСН Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковЗакупкиТоваровДляУСН");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.РасчетыСФондамиПоСтраховымВзносам Тогда
		...
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.РасчетыСПрочимиКонтрагентами Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.РасчетыПоКредитамИЗаймам Тогда
			РеквизитОбъект = ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект();
			ЗначениеВРеквизитФормы(РеквизитОбъект, "ВводНачальныхОстатковРасчетыПоКредитамИЗаймам");
		ИначеЕсли ВыборкаРезультатЗапроса.РазделУчета = Перечисления.РазделыУчета.СкидкиБонусы Тогда
			ЗначениеВРеквизитФормы(ВыборкаРезультатЗапроса.Ссылка.ПолучитьОбъект(), "ВводНачальныхОстатковСкидкиБонусы");
		КонецЕсли;
	...
	УстановитьВидимостьРеквизитовТЧ("РасчетыСПоставщиками");
	УстановитьВидимостьРеквизитовТЧ("РасчетыСПокупателями");
	УстановитьВидимостьРеквизитовТЧ("РасчетыСПрочимиКонтрагентами");
	УстановитьВидимостьРеквизитовТЧ("ЗапасыПереданные");
	УстановитьВидимостьРеквизитовТЧ("ЗапасыПринятые");
	УстановитьВидимостьРеквизитовТЧ("ПрочиеРазделы");
	УстановитьВидимостьРеквизитовТЧ("РасчетыПоКредитамИЗаймам");
...
Процедура УстановитьВидимостьИДоступность(ИмяСобытия = "")
	...
	УстановитьВидимостьШага4();	
	
	Элементы.СтраницаРасчетыПоНалогам.Видимость = ПолучитьФункциональнуюОпцию("ОтображатьСчетаУчета");
	
	Элементы.СтраницаРасчетыПоКредитамИЗаймам.Видимость = ПолучитьФункциональнуюОпцию("КредитыИЗаймы");

	УстановитьОтборыПоЭквайрингу();
	
	// Прослеживаемость
	ПрослеживаемостьФормыКлиентСерверУНФ.НастроитьВидимостьКолонокРНПТ(ЭтотОбъект, "ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДГруппаРНПТ", "ВводНачальныхОстатковТоварыЗапасыВРазрезеГТДПодсказкаРНПТ");
	ПрослеживаемостьФормыКлиентСерверУНФ.НастроитьВидимостьКолонокРНПТ(ЭтотОбъект, "ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДГруппаРНПТ", "ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТДПодсказкаРНПТ");
	ПрослеживаемостьФормыКлиентСерверУНФ.НастроитьВидимостьКолонокРНПТ(ЭтотОбъект, "ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДГруппаРНПТ", "ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТДПодсказкаРНПТ");
	// Конец Прослеживаемость
...
Процедура УстановитьВидимостьКурсов()
	...
	Если ПустаяСтрока(ВалютаУчетаПредставление) Тогда
		ВалютаУчетаПредставление = Строка(ВалютаУчета);
	КонецЕсли; 
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВводНачальныхОстатковТоварыЗапасыСуммаОплатыКУДиР", "Заголовок", 
		?(ОтображатьКурсСебестоимости, 
		СтрШаблон(НСтр("ru = 'Оплачено без НДС (УСН), %1'"), ВалютаУчетаПредставление), 
		НСтр("ru = 'Оплачено без НДС (УСН)'")));
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВводНачальныхОстатковЗакупкиТоваровДляУСНЗакупкиДляКУДИРСумма", "Заголовок", 
		?(ОтображатьКурсСебестоимости, 
		СтрШаблон(НСтр("ru = 'Оплачено без НДС, %1'"), ВалютаУчетаПредставление), 
		НСтр("ru = 'Оплачено без НДС'")));
...
Процедура ОбновитьЭлементыФормы(ПолноеОбновление = Ложь)
	...
	Для каждого Стр Из ВводНачальныхОстатковБанкИКасса.ДенежныеСредства Цикл
		...
		Если ТипЗнч(Стр.БанковскийСчетКасса)=Тип("СправочникСсылка.Кассы") И ОтображатьСчетаУчета Тогда
...
&НаСервере
Функция СохранятьРасчетыСПрочимиКонтрагентами()
	
	Возврат ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами.РасчетыСПрочимиКонтрагентами.Количество() > 0
	ИЛИ (ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами.РасчетыСПрочимиКонтрагентами.Количество() = 0
	И НЕ ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами.Ссылка.Пустая()
	И ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами.Проведен);
	
КонецФункции

&НаСервере
Функция СохранятьСкидкиБонусы()
	
	Возврат
		ВводНачальныхОстатковСкидкиБонусы.БонусныеБаллы.Количество() > 0
		ИЛИ ВводНачальныхОстатковСкидкиБонусы.ПодарочныеСертификаты.Количество() > 0
		ИЛИ ВводНачальныхОстатковСкидкиБонусы.НакопленныеПродажиПоДисконтнымКартам.Количество() > 0
		ИЛИ (
				ВводНачальныхОстатковСкидкиБонусы.БонусныеБаллы.Количество() = 0
				И ВводНачальныхОстатковСкидкиБонусы.ПодарочныеСертификаты.Количество() = 0
				И ВводНачальныхОстатковСкидкиБонусы.НакопленныеПродажиПоДисконтнымКартам.Количество() = 0
				И НЕ ВводНачальныхОстатковСкидкиБонусы.Ссылка.Пустая()
				И ВводНачальныхОстатковСкидкиБонусы.Проведен);
...
Процедура ЗаписатьИзмененияФормы(ЗакончитьВвод = Ложь, Отказ = Ложь)
	...
	КомментарийДокументВведенПомощникомВводаОстатков
		= Документы.ВводНачальныхОстатков.КомментарийДокументВведенПомощникомВводаОстатков();
	...
	Если СохранятьБанкИКасса() Тогда
		...
		ВводНачальныхОстатковБанкИКассаОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковБанкИКассаОбъект.РазделУчета = Перечисления.РазделыУчета.ДенежныеСредства;
	...
	Если СохранятьПодотчет() Тогда
		...
		ВводНачальныхОстатковПодотчетОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковПодотчетОбъект.РазделУчета = Перечисления.РазделыУчета.РасчетыСПодотчетниками;
	...
	Если СохранятьЗапасы() Тогда
		...
		ВводНачальныхОстатковТоварыОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковТоварыОбъект.РазделУчета = Перечисления.РазделыУчета.Запасы;
	...
	Если СохранятьРасчетыСКонтрагентами() Тогда
		...
		ВводНачальныхОстатковРасчетыСКонтрагентамиОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковРасчетыСКонтрагентамиОбъект.РазделУчета = Перечисления.РазделыУчета.РасчетыСПоставщикамиИПокупателями;
	...
	Если СохранятьРасчетыСПерсоналом() Тогда
		...
		ВводНачальныхОстатковЗарплатаОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковЗарплатаОбъект.РазделУчета = Перечисления.РазделыУчета.РасчетыСПерсоналом;
	...
	Если СохранятьНалоги() Тогда
		...
		ВводНачальныхОстатковНалогиОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковНалогиОбъект.РазделУчета = Перечисления.РазделыУчета.РасчетыПоНалогам;
	...
	Если СохранятьЭквайринг() Тогда
		...
		ВводНачальныхОстатковЭквайрингОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковЭквайрингОбъект.РазделУчета = Перечисления.РазделыУчета.Эквайринг;
	...
	Если СохранятьВнеоборотныеАктивы() Тогда
		...
		ВводНачальныхОстатковИмуществоОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковИмуществоОбъект.РазделУчета = Перечисления.РазделыУчета.Имущество;
	...
	Если СохранятьПрочее() Тогда
		...
		ВводНачальныхОстатковПрочееОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковПрочееОбъект.РазделУчета = Перечисления.РазделыУчета.ПрочиеРазделы;
	...
	Если СохранятьЗакупкиТоваровДляУСН() Тогда
		...
		ВводНачальныхОстатковЗакупкиТоваровДляУСНОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковЗакупкиТоваровДляУСНОбъект.РазделУчета = Перечисления.РазделыУчета.ЗакупкиТоваровДляУСН;
	...
	Если СохранятьРасчетыСФондамиПоСтраховымВзносам() Тогда
		...
		ВводНачальныхОстатковРасчетыСФондамиПоСтраховымВзносамОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковРасчетыСФондамиПоСтраховымВзносамОбъект.РазделУчета = Перечисления.РазделыУчета.РасчетыСФондамиПоСтраховымВзносам;
		Если НЕ ВводНачальныхОстатковРасчетыСФондамиПоСтраховымВзносамОбъект.ПроверитьЗаполнение() Тогда
			Отказ = Истина;
		КонецЕсли; 
	КонецЕсли;
	
	Если СохранятьРасчетыСПрочимиКонтрагентами() Тогда
		ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиОбъект = РеквизитФормыВЗначение("ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами");
		ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиОбъект.Дата = ДатаОстатков;
		ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиОбъект.Организация = Организация;
		ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиОбъект.Автоформирование = Истина;
		ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиОбъект.РазделУчета = Перечисления.РазделыУчета.РасчетыСПрочимиКонтрагентами;
		Если НЕ ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиОбъект.ПроверитьЗаполнение() Тогда
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
	
	Если СохранятьРасчетыПоКредитамИЗаймам() Тогда
		ВводНачальныхОстатковРасчетыПоКредитамИЗаймамОбъект =
			РеквизитФормыВЗначение("ВводНачальныхОстатковРасчетыПоКредитамИЗаймам");
		ВводНачальныхОстатковРасчетыПоКредитамИЗаймамОбъект.Дата = ДатаОстатков;
		ВводНачальныхОстатковРасчетыПоКредитамИЗаймамОбъект.Организация = Организация;
		ВводНачальныхОстатковРасчетыПоКредитамИЗаймамОбъект.Автоформирование = Истина;
		ВводНачальныхОстатковРасчетыПоКредитамИЗаймамОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковРасчетыПоКредитамИЗаймамОбъект.РазделУчета = Перечисления.РазделыУчета.РасчетыПоКредитамИЗаймам;
		Если НЕ ВводНачальныхОстатковРасчетыПоКредитамИЗаймамОбъект.ПроверитьЗаполнение() Тогда
			Отказ = Истина;
		КонецЕсли; 
	КонецЕсли;
	
	Если СохранятьСкидкиБонусы() Тогда
		ВводНачальныхОстатковСкидкиБонусыОбъект = РеквизитФормыВЗначение("ВводНачальныхОстатковСкидкиБонусы");
		ВводНачальныхОстатковСкидкиБонусыОбъект.Дата = ДатаОстатков;
		ВводНачальныхОстатковСкидкиБонусыОбъект.Организация = Организация;
		ВводНачальныхОстатковСкидкиБонусыОбъект.Автоформирование = Истина;
		ВводНачальныхОстатковСкидкиБонусыОбъект.Комментарий = КомментарийДокументВведенПомощникомВводаОстатков;
		ВводНачальныхОстатковСкидкиБонусыОбъект.РазделУчета = Перечисления.РазделыУчета.СкидкиБонусы;
		Если НЕ ВводНачальныхОстатковСкидкиБонусыОбъект.ПроверитьЗаполнение() Тогда
 			Отказ = Истина;
		КонецЕсли; 
	КонецЕсли; 
	...
	Если СохранятьРасчетыСПрочимиКонтрагентами() Тогда
		Если ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами.РасчетыСПрочимиКонтрагентами.Количество() > 0 Тогда
			ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиОбъект.Записать(РежимЗаписиДокумента.Проведение);
		Иначе
			ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиОбъект.Записать(РежимЗаписиДокумента.ОтменаПроведения);
		КонецЕсли; 
		ЗначениеВРеквизитФормы(ВводНачальныхОстатковРасчетыСПрочимиКонтрагентамиОбъект, "ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами");
	КонецЕсли;
	
	Если СохранятьРасчетыПоКредитамИЗаймам() Тогда
		Если ВводНачальныхОстатковРасчетыПоКредитамИЗаймам.РасчетыПоКредитамИЗаймам.Количество() > 0 Тогда
			ВводНачальныхОстатковРасчетыПоКредитамИЗаймамОбъект.Записать(РежимЗаписиДокумента.Проведение);
		Иначе
			ВводНачальныхОстатковРасчетыПоКредитамИЗаймамОбъект.Записать(РежимЗаписиДокумента.ОтменаПроведения);
		КонецЕсли; 
		ЗначениеВРеквизитФормы(ВводНачальныхОстатковРасчетыПоКредитамИЗаймамОбъект,
			"ВводНачальныхОстатковРасчетыПоКредитамИЗаймам");
	КонецЕсли;

	Если СохранятьСкидкиБонусы() Тогда
		Если ВводНачальныхОстатковСкидкиБонусы.БонусныеБаллы.Количество() > 0 
			ИЛИ ВводНачальныхОстатковСкидкиБонусы.ПодарочныеСертификаты.Количество() > 0
			ИЛИ ВводНачальныхОстатковСкидкиБонусы.НакопленныеПродажиПоДисконтнымКартам.Количество() > 0 Тогда
			ВводНачальныхОстатковСкидкиБонусыОбъект.Записать(РежимЗаписиДокумента.Проведение);
		Иначе
			ВводНачальныхОстатковСкидкиБонусыОбъект.Записать(РежимЗаписиДокумента.ОтменаПроведения);
		КонецЕсли; 
		ЗначениеВРеквизитФормы(ВводНачальныхОстатковСкидкиБонусыОбъект, "ВводНачальныхОстатковСкидкиБонусы");
	КонецЕсли;
...
Процедура ВыполнитьПроверкиПриПереходеНаСледующуюСтраницу(Отказ)
	...
	ИначеЕсли ТекущийНомерСтраницы = 2 Тогда
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковТовары.ЗапасыВРазрезеГТД Цикл
			...
			Если Не ТекСтрока.ПрослеживаемыйТовар Тогда
				ПроверитьКолонкуТаблицы(
				"ВводНачальныхОстатковТовары.ЗапасыВРазрезеГТД",
				ТекСтрока,
				"НомерГТД",
		...
		Для каждого ЭлементМассива Из ИменаТЧ Цикл
			...
			Для каждого ТекСтрока Из ВводНачальныхОстатковТовары[ЭлементМассива] Цикл
				...
				Для каждого ЭлементаСоответствия Из СоответствиеРеквизитовИСообщений Цикл
					...
					Если ЭлементаСоответствия.Ключ = "НомерГТД" И Не ТекСтрока.ПрослеживаемыйТовар Тогда
...
&НаСервере
Функция СохранятьРасчетыПоКредитамИЗаймам()
	
	Возврат ВводНачальныхОстатковРасчетыПоКредитамИЗаймам.РасчетыПоКредитамИЗаймам.Количество() > 0
	ИЛИ (ВводНачальныхОстатковРасчетыПоКредитамИЗаймам.РасчетыПоКредитамИЗаймам.Количество() = 0
	И НЕ ВводНачальныхОстатковРасчетыПоКредитамИЗаймам.Ссылка.Пустая()
	И ВводНачальныхОстатковРасчетыПоКредитамИЗаймам.Проведен);
	
КонецФункции

&НаСервере
Процедура УстановитьВидимостьШага4()
	ОтображатьСчетаУчета = ПолучитьФункциональнуюОпцию("ОтображатьСчетаУчета");
	ОтображатьБонусы = ПолучитьФункциональнуюОпцию("ИспользоватьБонусныеПрограммы");
 	ОтображатьСертификаты = ПолучитьФункциональнуюОпцию("ИспользоватьПодарочныеСертификаты");
	ОтображатьДисконтныеКарты = ПолучитьФункциональнуюОпцию("ИспользоватьДисконтныеКарты");
	ОтображатьПрочее = ОтображатьСчетаУчета 
								...
								ИЛИ ОтображатьСертификаты
								ИЛИ ОтображатьДисконтныеКарты;
	Элементы.Шаг4.Видимость = ОтображатьПрочее;
	Элементы.ОтметкаШаг4.Видимость = Элементы.Шаг4.Видимость;
	Для Каждого Элемент Из Элементы.СтраницыПрочие.ПодчиненныеЭлементы Цикл
		Если Элемент = Элементы.СтраницаБонусы Тогда
			Элемент.Видимость = ОтображатьБонусы;
		ИначеЕсли Элемент = Элементы.СтраницаПодарочныеСертификаты Тогда
			Элемент.Видимость = ОтображатьСертификаты;
		ИначеЕсли Элемент = Элементы.СтраницаНакопленныеПродажиПоДК Тогда
			Элемент.Видимость = ОтображатьДисконтныеКарты;
		Иначе
			Элемент.Видимость = ОтображатьСчетаУчета;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗаполнитьСтавкуНДСНаСервере()
	...
	СтруктураСоСтавкойНДС = Новый Структура("ДатаОбработки", ДатаОстатков);
	Для Каждого ТекущаяСтрока Из ВводНачальныхОстатковТовары.Запасы Цикл
		СтруктураСоСтавкойНДС.Вставить("Номенклатура", ТекущаяСтрока.Номенклатура);
		СтруктураСоСтавкойНДС.Вставить("Организация", Организация);
		ДополнитьСтруктуруСтавкойНДС(СтруктураСоСтавкойНДС);
		ТекущаяСтрока.СтавкаНДС = СтруктураСоСтавкойНДС.СтавкаНДС;
...
&НаСервереБезКонтекста
Процедура ДополнитьСтруктуруСтавкойНДС(СтруктураДанные)
	
	Если СтруктураДанные.Свойство("ДатаОбработки") = Ложь Тогда
...
Процедура ПолучитьСтруктуруВидовОпераций()
	...
	Если НЕ Элементы.СтраницаПринятыеТовары.Видимость И НЕ Элементы.СтраницаПереданныеТовары.Видимость И НЕ Элементы.СтраницаТоварыВРазрезеГТД.Видимость И НЕ Элементы.СтраницаЗакупкиТоваровДляУСН.Видимость Тогда
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	ДополнитьСтруктуруСтавкойНДС(СтруктураДанные);
...
Процедура ЗаполнитьПризнакиИспользованияХарактеристик()
	...
	// Контроль ввода дробного количества
	ЗаполнитьПрочиеДополнительныеРеквизитыТабличныхЧастей();
...
&НаСервере
Процедура ЗаполнитьПрочиеДополнительныеРеквизитыТабличныхЧастей()
	
	МассивНоменклатурыДляПроверкиДробногоКоличества = Новый Массив;
	
	ТаблицаНоменклатуры = ВводНачальныхОстатковТовары["Запасы"].Выгрузить(,"Номенклатура");
	...
	Для Каждого ИмяТаблицыДокумента Из МассивИменТаблицВводаНачальныхОстатков Цикл
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ВводНачальныхОстатковТовары[ИмяТаблицыДокумента].Выгрузить(,"Номенклатура"), ТаблицаНоменклатуры);
	КонецЦикла;
	
	МассивИменТаблицВводаНачальныхОстатков.Добавить("Запасы");
	...
	Если МассивНоменклатурыДляПроверкиДробногоКоличества.Количество() Тогда
		
		Для Каждого ИмяТаблицыДокумента Из МассивИменТаблицВводаНачальныхОстатков Цикл
			
			Для Каждого СтрокаТаблицы Из ВводНачальныхОстатковТовары[ИмяТаблицыДокумента] Цикл
...
Процедура УстановитьВидимостьРеквизитовТЧ(ИмяТабличнойЧасти, Знач ВестиРасчетыПоДоговорам = Ложь, Знач ВестиРасчетыПоДокументам = Ложь, Знач ВестиРасчетыПоЗаказам = Ложь, Знач ВестиУчетОплатыПоСчетам = Ложь, Знач ВалютныйСчет = Ложь )
	...
	ИначеЕсли ИмяТабличнойЧасти = "ПрочиеРазделы" Тогда 
		ЗаполнитьПризнакВалютныйСчета();
	ИначеЕсли ИмяТабличнойЧасти = "РасчетыСПрочимиКонтрагентами" Тогда
		ЗаполнитьСлужебныеРеквизитыПоКонтрагентуВКоллекции(ВводНачальныхОстатковРасчетыСПрочимиКонтрагентами[ИмяТабличнойЧасти]);
	КонецЕсли;
	...
	ИначеЕсли ИмяТабличнойЧасти = "ПрочиеРазделы" Тогда
		Для каждого ТекСтрока Из ВводНачальныхОстатковПрочее[ИмяТабличнойЧасти] Цикл
			...
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли ИмяТабличнойЧасти = "РасчетыПоКредитамИЗаймам" Тогда
		УчетВалютныхОпераций = ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций");
	КонецЕсли; 
	...
	ИначеЕсли ИмяТабличнойЧасти = "РасчетыПоКредитамИЗаймам" Тогда 
		Элементы.ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамДоговорКредитаЗаймаВалютаРасчетов.Видимость =
			УчетВалютныхОпераций;
		Элементы.ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамКурс.Видимость = УчетВалютныхОпераций;
		Элементы.ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамКратность.Видимость = УчетВалютныхОпераций;
		Элементы.ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамОсновнойДолг.Видимость = УчетВалютныхОпераций;
		Элементы.ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамПроценты.Видимость = УчетВалютныхОпераций;
		Элементы.ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамКомиссия.Видимость = УчетВалютныхОпераций;
	КонецЕсли;
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			...
			МассивСтрокТЧ = ВводНачальныхОстатковТовары[ИмяТабличнойЧасти].НайтиСтроки(Новый Структура("Номенклатура,Характеристика,Партия,ЕдиницаИзмерения",ДанныеШтрихкода.Номенклатура,ДанныеШтрихкода.Характеристика,ДанныеШтрихкода.Партия,ДанныеШтрихкода.ЕдиницаИзмерения));
				...
				ДопПараметр = Новый Структура("ЭтоПоискПоШтрихКоду, ДанныеШтрихкода, ТекШтрихкод, СтруктураДанные, ИмяТабличнойЧасти, ИмяТекущейСтраницы", Истина, ДанныеШтрихкода, ТекШтрихкод, СтруктураДанные, ИмяТабличнойЧасти, ИмяТекущейСтраницы);
...
&НаКлиенте
Процедура ЗагрузкаДанныхИзВнешнегоИсточникаБонусы(Команда)
...
&НаКлиенте
Процедура ЗагрузкаДанныхИзВнешнегоИсточникаПодарочныеСертификаты(Команда)
...
&НаКлиенте
Процедура ЗагрузкаДанныхИзВнешнегоИсточникаНакопленныеПродажиПоДК(Команда)
...
Процедура ОбработатьПодготовленныеДанныеЗапасыВРазрезеГТД(АдресРезультатаЗагрузки)
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru='Загрузка данных'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,
			Метаданные.Справочники.Номенклатура, , ПредставлениеОшибки);
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ОбработатьПодготовленныеДанныеБонусныеБаллы(АдресРезультатаЗагрузки)
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				НоваяСтрока = ВводНачальныхОстатковСкидкиБонусы.БонусныеБаллы.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы,
	...
	Исключение
		...
		ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка данных'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДисконтныеКарты, , ПодробноеПредставлениеОшибки);
...
Процедура ОбработатьПодготовленныеДанныеПодарочныеСертификаты(РезультатЗагрузки)
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				НоваяСтрока = ВводНачальныхОстатковСкидкиБонусы.ПодарочныеСертификаты.Добавить();
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();		
		ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка данных'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.Номенклатура, , ПодробноеПредставлениеОшибки);
	...
	Попытка
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна Тогда
				...
				НоваяСтрока = ВводНачальныхОстатковСкидкиБонусы.НакопленныеПродажиПоДисконтнымКартам.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы,
...
Процедура ОбработкаВыбораРНПТННаКлиенте(ВыбранноеЗначение)
	...
	Если Элементы.Страницы.ТекущаяСтраница.Имя = "Шаг2" Тогда
		Если Элементы.СтраницыОстаткиТоваров.ТекущаяСтраница.Имя = "СтраницаТоварыВРазрезеГТД" Тогда
			СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыВРазрезеГТД.ТекущиеДанные;
		ИначеЕсли Элементы.СтраницыОстаткиТоваров.ТекущаяСтраница.Имя = "СтраницаТоварыПереданныеВРазрезеГТД" Тогда
			СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыПереданныеВРазрезеГТД.ТекущиеДанные;
		ИначеЕсли Элементы.СтраницыОстаткиТоваров.ТекущаяСтраница.Имя = "СтраницаТоварыПринятыеВРазрезеГТД" Тогда
			СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковТоварыЗапасыПринятыеВРазрезеГТД.ТекущиеДанные;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаВыбораГТДНаКлиенте(ВыбранноеЗначение)
	...
	Если СтрокаТабличнойЧасти <> Неопределено И ЗначениеЗаполнено(ИмяТабличнойЧасти) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.НомерГТД) Тогда
			...
			Если ДатаГТД > КонецДня(ДатаОстатков) Тогда
				
				ТекстСообщения = НСтр("ru ='Выбранная ГТД датирована более поздней датой, чем текущий документ'");
				ДополнениеТекста = СтрШаблон("ВводНачальныхОстатковТовары.%1[%2].РНПТ", ИмяТабличнойЧасти, СтрокаТабличнойЧасти.ПолучитьИдентификатор());
...
Процедура УдалитьСвязанныеЗаписи(СтрокаЗапасы)
	...
	МассивСтрокСведенияПрослеживаемости = ВводНачальныхОстатковТовары.СведенияПрослеживаемости.НайтиСтроки(СтруктураОтбора);
	Для Каждого СтрокаТЧ Из МассивСтрокСведенияПрослеживаемости Цикл
		ВводНачальныхОстатковТовары.СведенияПрослеживаемости.Удалить(СтрокаТЧ);
	КонецЦикла;
...
Процедура ОбновитьПризнакПрослеживаемости()
	...
	ПрослеживаемостьФормыУНФ.ОбновитьПризнакПрослеживаемости(ВводНачальныхОстатковТовары.ЗапасыВРазрезеГТД, ВводНачальныхОстатковТовары.СведенияПрослеживаемости, ИспользуетсяПрослеживаемость);
	ПрослеживаемостьФормыУНФ.ОбновитьПризнакПрослеживаемости(ВводНачальныхОстатковТовары.ЗапасыПереданныеВРазрезеГТД, ВводНачальныхОстатковТовары.СведенияПрослеживаемости, ИспользуетсяПрослеживаемость);
	ПрослеживаемостьФормыУНФ.ОбновитьПризнакПрослеживаемости(ВводНачальныхОстатковТовары.ЗапасыПринятыеВРазрезеГТД, ВводНачальныхОстатковТовары.СведенияПрослеживаемости, ИспользуетсяПрослеживаемость);
	
	Для Каждого СтрокаЗапасы Из ВводНачальныхОстатковТовары.ЗапасыВРазрезеГТД Цикл
		ПрослеживаемостьФормыКлиентСерверУНФ.ЗаполнитьПредставлениеРНПТ(ЭтотОбъект, СтрокаЗапасы);
	КонецЦикла;
	
	Для Каждого СтрокаЗапасы Из ВводНачальныхОстатковТовары.ЗапасыПереданныеВРазрезеГТД Цикл
		ПрослеживаемостьФормыКлиентСерверУНФ.ЗаполнитьПредставлениеРНПТ(ЭтотОбъект, СтрокаЗапасы);
	КонецЦикла;
	
	Для Каждого СтрокаЗапасы Из ВводНачальныхОстатковТовары.ЗапасыПринятыеВРазрезеГТД Цикл
		ПрослеживаемостьФормыКлиентСерверУНФ.ЗаполнитьПредставлениеРНПТ(ЭтотОбъект, СтрокаЗапасы);
	КонецЦикла;
...
&НаСервере
Функция ИменаТЧИПолейДляТаблицыЗапасы_БазовыеПоляСПрослеживаемостью(пИменаТЧИПолей = Неопределено, пИмяТЧ = "Запасы")
	
	ИменаТЧСтруктура = Новый Структура;
	ИменаТЧСтруктура.Вставить("ИмяПоляПроверки", "ВводНачальныхОстатковТовары." + пИмяТЧ + ".СтранаПроисхождения");
	ИменаТЧСтруктура.Вставить("ИмяПоляПрослеживаемость", "ВводНачальныхОстатковТовары." + пИмяТЧ + ".ПрослеживаемыйТовар");
	ИменаТЧСтруктура.Вставить("ИмяПоляОформления", "ВводНачальныхОстатковТовары" + пИмяТЧ + "РНПТ");
	ИменаТЧСтруктура.Вставить("ИмяПоляНомерГТД", "ВводНачальныхОстатковТовары" + пИмяТЧ + "НомерГТД");
	ИменаТЧСтруктура.Вставить("ИмяГруппыРНПТ", "ВводНачальныхОстатковТовары" + пИмяТЧ + "ГруппаРНПТ");
	ИменаТЧСтруктура.Вставить("ИмяПоляРНПТ", "ВводНачальныхОстатковТовары."+пИмяТЧ+".РНПТ");
...
&НаКлиенте
Процедура ОткрытьФормуПодбораРНПТ(Форма, ИмяТаблицы)
	...
	АдресТаблицыРНПТВХранилище = ПоместитьРНПТВХранилище(ИдентификаторСтроки, ЭтаФорма.УникальныйИдентификатор);
	ПрослеживаемостьФормыКлиентУНФ.ОткрытьФормуПодбораРНПТПомощникВводаНачальныхОстатков(Форма, ИмяТаблицы, АдресТаблицыРНПТВХранилище);
КонецПроцедуры

&НаСервере
Функция ПоместитьРНПТВХранилище(ИдентификаторСтроки, ИдентификаторФормы)
	
	СтрокиСРНПТ = ВводНачальныхОстатковТовары.СведенияПрослеживаемости.НайтиСтроки(Новый Структура("ИдентификаторСтроки", ИдентификаторСтроки));
	ТаблицаРНПТ = ВводНачальныхОстатковТовары.СведенияПрослеживаемости.Выгрузить(СтрокиСРНПТ);
...
// Устанавливает видимость/доступность элементов, относящихся к отчетности УСН, исходя из настроек организации
&НаСервере
Процедура ОбновитьОтображениеЗакупокУСН()
	Если ТипЗнч(КэшЗначений) <> Тип("Структура") Тогда
		КэшЗначений = Новый Структура;
	КонецЕсли;
	КэшЗначений.Вставить("УСНДоходыРасходы", Ложь);
	КэшЗначений.Вставить("ВестиУчетЗакупокДляКУДИР", Ложь);
	
	ВидимостьНастройкиУСН = Ложь;
	ДоступностьЗакупкиУСН = Ложь;
	Если ЗначениеЗаполнено(Организация) Тогда
		РеквизитыОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Организация, "ИспользуетсяОтчетность, ЮридическоеФизическоеЛицо");
		...
		КэшЗначений.УСНДоходыРасходы = СистемаНалогообложения.ПлательщикУСН
										И СистемаНалогообложения.ОбъектНалогообложения = Перечисления.ВидыОбъектовНалогообложения.ДоходыМинусРасходы;
		КэшЗначений.ВестиУчетЗакупокДляКУДИР = РеквизитыОрганизации.ИспользуетсяОтчетность И КэшЗначений.УСНДоходыРасходы;
		
		ВидимостьНастройкиУСН = КэшЗначений.УСНДоходыРасходы И РеквизитыОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо;
		ДоступностьЗакупкиУСН = КэшЗначений.ВестиУчетЗакупокДляКУДИР;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВводНачальныхОстатковТоварыЗакупкиКУДиРОтдельнымДокументом", "Видимость", ВидимостьНастройкиУСН); 
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВводНачальныхОстатковТоварыЗакупкиКУДиРОтдельнымДокументом", "Доступность", ДоступностьЗакупкиУСН); 
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаИспользуетсяОтчетность", "Видимость", ВидимостьНастройкиУСН И НЕ ДоступностьЗакупкиУСН); 
	
	ОбновитьОтображениеРеквизитовЗакупокУСН();
КонецПроцедуры

// Переключает видимость колонок/таблицы закупок УСН в зависимости от выбранного способа их заполнения 
&НаСервере
Процедура ОбновитьОтображениеРеквизитовЗакупокУСН()
	
	ВидимостьТаблицыЗакупкиУСН = Ложь;
	ВидимостьКолонокЗакупкиУСН = Ложь;
	Если КэшЗначений.ВестиУчетЗакупокДляКУДиР Тогда
		Если ВводНачальныхОстатковТовары.ЗакупкиКУДиРОтдельнымДокументом Тогда
			ВидимостьТаблицыЗакупкиУСН = Истина;
			Если ВводНачальныхОстатковЗакупкиТоваровДляУСН.ЗакупкиДляКУДИР.Количество() = 0 Тогда
				Для Каждого СтрокаТабличнойЧасти Из ВводНачальныхОстатковТовары.ЗакупкиДляКУДИР Цикл
					ЗаполнитьЗначенияСвойств(ВводНачальныхОстатковЗакупкиТоваровДляУСН.ЗакупкиДляКУДИР.Добавить(), СтрокаТабличнойЧасти);
				КонецЦикла;
			КонецЕсли;
		Иначе
			ВидимостьКолонокЗакупкиУСН = Истина;
			ВводНачальныхОстатковЗакупкиТоваровДляУСН.ЗакупкиДляКУДИР.Очистить();
			Для Каждого СтрокаЗапасы Из ВводНачальныхОстатковТовары.Запасы Цикл
				ДанныеЗакупок = ТабличныеЧастиУНФКлиентСервер.СтрокиПоКлючуСвязи(ВводНачальныхОстатковТовары.ЗакупкиДляКУДИР, СтрокаЗапасы.КлючСвязи); 
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВводНачальныхОстатковЗакупкиТоваровДляУСНЗакупкиДляКУДИР", "Видимость", ВидимостьТаблицыЗакупкиУСН); 
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВводНачальныхОстатковТоварыЗапасыГруппаЗакупкиУСН", "Видимость", ВидимостьКолонокЗакупкиУСН); 
	
КонецПроцедуры

// Заполняет в документе ВводНачальныхОстатковТовары таблицу ЗакупкиДляКУДИР введенными пользователем в таблицу Запасы данными
&НаСервере
Процедура ОбновитьСтрокиЗапасыКУДиР(ИдентификаторСтрокиЗапасы, ИзмененнаяКолонка = "")
	СтрокаЗапасы = ВводНачальныхОстатковТовары.Запасы.НайтиПоИдентификатору(ИдентификаторСтрокиЗапасы);
	
	ИмяКолонкиКлючСвязи = "КлючСвязи";
	ТабличныеЧастиУНФКлиентСервер.ЗаполнитьКлючСвязи(ВводНачальныхОстатковТовары.Запасы, СтрокаЗапасы, ИмяКолонкиКлючСвязи);
	
	Если ЗначениеЗаполнено(СтрокаЗапасы.НомерДокументаКУДиР) И ЗначениеЗаполнено(СтрокаЗапасы.ДатаДокументаКУДиР) Тогда
		МассивСтрокЗапасыКУДиР = ТабличныеЧастиУНФКлиентСервер.СтрокиПоКлючуСвязи(ВводНачальныхОстатковТовары.ЗакупкиДляКУДИР, СтрокаЗапасы.КлючСвязи, ИмяКолонкиКлючСвязи);
		Если МассивСтрокЗапасыКУДиР.Количество() = 0 Тогда
			СтрокаЗакупкиКУДиР = ВводНачальныхОстатковТовары.ЗакупкиДляКУДИР.Добавить();
		...
		КонецЦикла;
	Иначе
		ТабличныеЧастиУНФКлиентСервер.УдалитьСтрокиПоКлючуСвязи(ВводНачальныхОстатковТовары.ЗакупкиДляКУДИР, СтрокаЗапасы, ИмяКолонкиКлючСвязи);
	КонецЕсли;
КонецПроцедуры

#КонецОбласти
 
#Область ОбработчикиСобытийЭлементовТаблицыФормыВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймам

&НаКлиенте
Процедура ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамДоговорКредитаЗаймаПриИзменении(Элемент)
	
	ЗаполнитьСтрокуКредитыИЗаймы();
...
&НаКлиенте
Процедура ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамОсновнойДолгВалПриИзменении(Элемент)
	
	СтрокаТабличнойЧасти = Элементы.ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймам.ТекущиеДанные;
...
&НаКлиенте
Процедура ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамПроцентыВалПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамКомиссияВалПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймамДоговорКредитаЗаймаНачалоВыбора(Элемент,
	...
	ПараметрыОткрытия.Вставить("РежимВыбора", Истина);
	ОтборФормы = Новый Структура;
	ОтборФормы.Вставить("Организация", Организация);
	ПараметрыОткрытия.Вставить("Отбор", ОтборФормы);
	ОповещениеОЗакрытииДоговорКредитаЗайма = Новый ОписаниеОповещения("ВыбратьДоговорКредитаЗайма", ЭтотОбъект);
	ОткрытьФорму("Документ.ДоговорКредитаИЗайма.ФормаВыбора", ПараметрыОткрытия,
		Элементы.ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймам, , , , ОповещениеОЗакрытииДоговорКредитаЗайма,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьДоговорКредитаЗайма(ВыбранноеЗначение, Параметры) Экспорт

	Если ВыбранноеЗначение <> Неопределено Тогда
		ТекущаяСтрока = Элементы.ВводНачальныхОстатковКредитыИЗаймыРасчетыПоКредитамИЗаймам.ТекущиеДанные;
		ТекущаяСтрока.ДоговорКредитаЗайма = ВыбранноеЗначение;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьСтрокуКредитыИЗаймы()
	...
	КурсыВалют = ЦенообразованиеСервер.ПолучитьКурсыВалют(Валюта, ВалютаУчета, ДатаОстатков);

	СтрокаТабличнойЧасти.Курс = КурсыВалют.КурсНач;
	СтрокаТабличнойЧасти.Кратность = КурсыВалют.КратностьНач;
	
	ОстаткиЗадолженности = ОстаткиЗадолженностиПоГрафикуКредитаЗайма(ДоговорКредитаЗайма, ДатаОстатков);
