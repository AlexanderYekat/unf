...
&НаКлиенте
Процедура АдресРесурсаОсновнойПубликацииБилайн(Элемент)
	ОбновитьАдресПубликации();
КонецПроцедуры
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Оповестить("Телефония_ОбновлениеНастроек");
	Закрыть(Истина);
КонецПроцедуры
...
Процедура ПрочитатьНастройки()
	...
	// Билайн
	НастройкиПубликацииБилайн = НастройкиПубликации.Получить(Перечисления.ДоступныеАТС.Билайн);
	Если ТипЗнч(НастройкиПубликацииБилайн) = Тип("Структура") Тогда
		НастройкиПубликацииБилайн.Свойство("АдресРесурсаОсновнойПубликации", АдресРесурсаОсновнойПубликацииБилайн);
	КонецЕсли;
...
Процедура СохранитьНастройки()
	...
	Если Не ЗначениеЗаполнено(АдресСервераОсновнойПубликации)
		...
		И Не ЗначениеЗаполнено(АдресРесурсаОсновнойПубликацииБилайн)
	...
	Иначе
		...
		НастройкиПубликации.Вставить(Перечисления.ДоступныеАТС.Билайн, Новый Структура("АдресРесурсаОсновнойПубликации", АдресРесурсаОсновнойПубликацииБилайн));
		НастройкиПубликации.Вставить(Перечисления.ДоступныеАТС.МТТ, Новый Структура("АдресРесурсаОсновнойПубликации", АдресРесурсаОсновнойПубликацииУниверсальныйItoolabs));
...
Процедура ОбновитьАдресПубликации()
	...
	ПолныйПутьПубликацииБилайн                = ПолныйПутьПубликации(Перечисления.ДоступныеАТС.Билайн, АдресРесурсаОсновнойПубликацииБилайн);
...
Процедура УправлениеФормой()
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Элементы.НастройкиMango.Видимость                 = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.MangoOffice");
		Элементы.НастройкиДомRu.Видимость                 = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.ДомRu");
		Элементы.НастройкиЯндекс.Видимость                = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Яндекс");
		Элементы.НастройкиРостелеком.Видимость            = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Ростелеком");
		Элементы.НастройкиВестКоллСПб.Видимость           = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.ВестКоллСПб");
		Элементы.НастройкиДеловаяСетьИркутск.Видимость    = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.ДеловаяСетьИркутск");
		Элементы.НастройкиБилайн.Видимость                = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Билайн");
		Элементы.НастройкиУниверсальныйItoolabs.Видимость = ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.Энфорта")
			...
			ИЛИ ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.МТТ")
			ИЛИ ИспользуемаяАТС = ПредопределенноеЗначение("Перечисление.ДоступныеАТС.УниверсальныйItoolabs");
			Если Элементы.НастройкиУниверсальныйItoolabs.Видимость Тогда
				Элементы.НастройкиУниверсальныйItoolabs.Заголовок = "Сервис интеграции:" + ИспользуемаяАТС;
			КонецЕсли;
	КонецЕсли;
