...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЗаписьКалендаря") И ЗначениеЗаполнено(Параметры.ЗаписьКалендаря) Тогда
...
Процедура Подключаемый_ПроверитьВыполнениеДлительнойОперацииНаКлиенте()
	...
	Если СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "РасчетПоСтраховымВзносам") > 0 
		ИЛИ СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "6НДФЛ") > 0 
		ИЛИ СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "ПерсСведенияОФизлицах") > 0 Тогда
	...
	Если СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "РасчетПоСтраховымВзносам") = 0 
		И СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "6НДФЛ") = 0 
		И СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "ПерсСведенияОФизлицах") = 0 Тогда
...
Процедура ОбработатьПараметрСобытияКалендаря()
	...
	СобытиеКалендаряДанные = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СобытиеКалендаря, "Задача,ДатаДокументаОбработкиСобытия,ДатаНачалаДокументов,ДатаОкончанияДокументов, ДатаОкончанияСобытия");
	...
	Элементы.ГруппаДекорацииОписанияСтраховыеВзносыЗаСотрудников.Видимость = Ложь; 
	Элементы.ГруппаДекорацииОписанияПерсСведенияОФизлицах.Видимость = Ложь;
	...
	Если СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ЕдиныйНалог Тогда
		...
		ЕстьПатенты = 
		Справочники.Патенты.ПолучитьПатентыОрганизации(Организация, СобытиеКалендаряДанные.ДатаДокументаОбработкиСобытия).Количество() > 0;
		
		Элементы.ГруппаЗаполнения5.Видимость = ЕстьПатенты
		И РегламентированнаяОтчетностьУСН.УпрощенныйЗачетФиксированныхВзносов(
		Организация, СобытиеКалендаряДанные.ДатаДокументаОбработкиСобытия); 
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.АвансовыйПлатежПоУСН Тогда
		...
		Элементы.ГруппаЗаполнения3.Видимость = ЕстьПатенты
		И РегламентированнаяОтчетностьУСН.УпрощенныйЗачетФиксированныхВзносов(
		Организация, СобытиеКалендаряДанные.ДатаДокументаОбработкиСобытия);
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыИП Тогда
		...
		Если Год(ПериодДокументовОкончание) > 2022 Тогда
			ТекущиеВзносы = РегистрыСведений.ФиксированныеВзносыИП.ФиксированныеВзносыИПНаДату(НачалоГода(ПериодДокументовОкончание));
			Элементы.Декорация14.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='• Сумма взносов - фиксированная. По единому тарифу: %1 руб.'"), РегламентированнаяОтчетностьУСН.ВзносыПоЕдиномуТарифуПодлежащиеУплате(Организация, ПериодДокументовОкончание));
			Элементы.ГруппаДекорацииОписания24.Видимость = Ложь;
		КонецЕсли;
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогиСотрудников Тогда
		...
		Заголовок = НСтр("ru='Налоги сотрудников НДФЛ за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Рассчитать'"); 
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыЗаСотрудников Тогда
		
		Заголовок = НСтр("ru='Страховые взносы сотрудников за '") 
		+ ПредставлениеПериода(
		ПериодДокументовНачало,
		ПериодДокументовОкончание,
		"ФП=Истина");
		
		Элементы.ДекорацияНадписьПериодовДокументовНалоговСотрудников1.Заголовок = 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.ДекорацияНадписьПериодовДокументовРСВ.Заголовок,
		ПредставлениеПериода(
		ПериодДокументовНачало,
		...
		Если День(СобытиеКалендаряДанные.ДатаОкончанияСобытия) > 20 Тогда
			Элементы.ДекорацияТекстОписание51.Видимость = Ложь;
			Элементы.ДекорацияТекстОписание53.Видимость = Истина;
		Иначе
			Элементы.ДекорацияТекстОписание51.Видимость = Истина;
			Элементы.ДекорацияТекстОписание53.Видимость = Ложь;
		КонецЕсли;
		
		Элементы.ГруппаДекорацииОписанияСтраховыеВзносыЗаСотрудников.Видимость = Истина;
		
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Рассчитать'");		
	// Форма 4-ФСС
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Форма4ФСС Тогда
		
		Если ПериодДокументовНачало >= РегламентированнаяОтчетностьУСН.ДатаОбязательногоПримененияЕНП() Тогда
			Заголовок = НСтр("ru='Отчет ЕФС-1, раздел 2 '") 
			+ ПредставлениеПериода(
			ПериодДокументовНачало,
			ПериодДокументовОкончание,
			"ФП=Истина");			
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.РасчетПоСтраховымВзносам Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать'");     
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ПерсСведенияОФизлицах Тогда
		
		Заголовок = НСтр("ru='Персонифицированные сведения о физических лицах за '") 
		+ ПредставлениеПериода(
		ПериодДокументовНачало,
		ПериодДокументовОкончание,
		"ФП=Истина");
		
		Элементы.ДекорацияНадписьПериодовДокументовПерсСведенияОФизлицах.Заголовок = 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Элементы.ДекорацияНадписьПериодовДокументовПерсСведенияОФизлицах.Заголовок,
		ПредставлениеПериода(
		ПериодДокументовНачало,
		...
		Элементы.ГруппаДекорацииОписанияПерсСведенияОФизлицах.Видимость = Истина;
...
Процедура ВыполнитьРасчетНажатие()	
	...
	Если Результат.Свойство("ИмяФормыРезультата") Тогда
		...
		Если СтрНайти(ИмяФормыРезультата, "РасчетПоСтраховымВзносам") > 0
			ИЛИ СтрНайти(ИмяФормыРезультата, "Форма6НДФЛ") > 0 
			ИЛИ СтрНайти(ИмяФормыРезультата, "ПерсСведенияОФизлицах") > 0 Тогда
...
&НаКлиенте
Процедура ДекорацияПереходКПапкамДокументовИзАвПлатежа1ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ДекорацияТекстОписание22ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	ОбработкаНавигационнойСсылкиСотрудникиИНачисление(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка);
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияТекстОписание25ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	ОбработкаНавигационнойСсылкиСотрудникиИНачисление(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка);
КонецПроцедуры  
...
&НаКлиенте
Процедура ОбработкаНавигационнойСсылкиСотрудникиИНачисление(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;          
	
	ИспользуетсяРегламентированнаяЗарплата = РегламентированнаяОтчетностьСотрудники.ИспользуетсяРегламентированнаяЗарплата(Организация); 
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "СведенияПоСотрудникам" Тогда
		ОткрытьФорму("Справочник.Сотрудники.ФормаСписка");
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "НачисленияПоСотрудникам" И ИспользуетсяРегламентированнаяЗарплата = Ложь Тогда
		ОткрытьФорму("ЖурналДокументов.ДокументыПоЗарплате.ФормаСписка", Новый Структура("Организация", Организация)); 
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "НачисленияПоСотрудникам" И ИспользуетсяРегламентированнаяЗарплата Тогда	
		ОткрытьФорму("ОбщаяФорма.МенюФункцийБЗКБ");
	КонецЕсли;	
