...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ВзносСДоходов + ВзносПоЕдиномуТарифуЕН > 0 Тогда
		Элементы.ДекорацияНалоговыйВычет.Видимость = Истина;
		Элементы.ДекорацияМинус4.Видимость = Истина;
		Элементы.СуммаНалоговыхВычетов.Видимость = Истина;
		...
		Элементы.ВзносОплаченные.Видимость = СуммаСтраховыхВзносовЕН <> 0; 	
		...
		Элементы.СуммаСтраховыхВзносовКУплатеЕН.Видимость = Ложь; 		
	Иначе
		Элементы.ДекорацияНалоговыйВычет.Видимость = Ложь;
		Элементы.ДекорацияМинус5.Видимость = Ложь;
		...
		Элементы.ВзносОплаченные.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПолучитьДанныеОтчетности()
	...
	МассивПоказателей.Добавить(ПланыВидовХарактеристик.ПоказателиОтчетности.ВзносПоЕдиномуТарифуЕН);
	
	Запрос.УстановитьПараметр("МассивПоказателей", МассивПоказателей);
	ВсегоКУплатеДоУменьшения = 0;
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.КодОтчетности = "СуммаЕНПовышенная" Тогда	
			...
			ВсегоКУплатеДоУменьшения = ВсегоКУплатеДоУменьшения + Выборка.ЗначениеПоказателя; 
		ИначеЕсли Выборка.КодОтчетности = "СуммаДоходовЗаГодЕН" Тогда	
			Элементы.ГруппаСтрока2.Видимость = (Выборка.ЗначениеПоказателя > 0);
			ЭтаФорма[Выборка.КодОтчетности] = Формат(Выборка.ЗначениеПоказателя, "ЧДЦ=2; ЧРГ=; ЧН=" ) + " р.";	
			
		ИначеЕсли Выборка.КодОтчетности = "ВзносПоЕдиномуТарифуЕН" Тогда
		...
		Иначе
			...
			Если Выборка.КодОтчетности = "СуммаЕН" Тогда
				ВсегоКУплатеДоУменьшения = ВсегоКУплатеДоУменьшения + Выборка.ЗначениеПоказателя; 
			КонецЕсли; 	
	...
	Если УпрощенныйЗачетФиксированныхВзносов Тогда
		...
		//РегламентированнаяОтчетностьУСН.РассчитатьФиксированныеСтраховыеВзносы(ДатаДокументаОбработкиСобытия, Объект.Организация).ЕдиныйТарифВсего;		
		...
		СрокПоЕдиномуТарифу = Объект.СобытиеКалендаря.ДатаНачалаДокументов;
		...
		Если (ВзносПоЕдиномуТарифуЕН + ВзносСДоходов) > 0 Тогда
			// Выводим расчет с учетом взносов, подлежащих уплате
			Если ДоляДоходовПоУСН < 100 И ДоляДоходовПоУСН > 0 Тогда
				// Есть совмещение с ПСН 			
				Элементы.СуммаНалоговыхВычетов.Подсказка = НСтр("ru = 'Налоговый вычет = (По единому тарифу + Взнос с доходов + Уплаченные взносы) * Доля доходов по УСН'")
				+ Символы.ПС + СтрШаблон("(%1 + %2 + %3) * %4",
				Формат(ВзносПоЕдиномуТарифуЕН, ФорматЧиселВПояснении),
				Формат(ВзносСДоходов, ФорматЧиселВПояснении),
				Формат(ДоляДоходовПоУСН, ФорматЧиселВПояснении),
				Формат(СуммаСтраховыхВзносовЕН, ФорматЧиселВПояснении));  
			Иначе
				Элементы.СуммаНалоговыхВычетов.Подсказка = НСтр("ru = 'Налоговый вычет = По единому тарифу + Взнос с доходов + Уплаченные взносы'")
...
&НаКлиенте
Процедура ВзносОплаченныеНажатие(Элемент, СтандартнаяОбработка)
