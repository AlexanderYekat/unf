...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВзносСДоходов + ВзносПоЕдиномуТарифу > 0 Тогда
		...
		Элементы.ДоляДоходовПоПатенту.Видимость = Истина;
		Элементы.ГруппаВзносыПодлежащиеУплатеРасчет.Видимость = Истина;
		Элементы.ГруппаРасходыСтраховыеВзносыВсего.Видимость = Ложь;
		Элементы.ПояснениеРасчетаВзносыПодлежащиеУплате.Заголовок = СтрШаблон("(%1 + %2) * %3%%",
		ВзносПоЕдиномуТарифу,
		ВзносСДоходов,
		ДоляДоходовПоПатенту);
		...
		Элементы.ДоляДоходовПоПатенту.Видимость = ДоляДоходовПоПатенту < 100 И ДоляДоходовПоПатенту <> 0;   
		
	Иначе
		Элементы.ГруппаВзносыПодлежащиеУплате.Видимость = Ложь;
		Элементы.ДоляДоходовПоПатенту.Видимость = Ложь;
		Элементы.ГруппаВзносыПодлежащиеУплатеРасчет.Видимость = Ложь;
		Элементы.ГруппаРасходыСтраховыеВзносыВсего.Видимость = Истина;
	...
	ПериодСобытияНачало= Объект.СобытиеКалендаря.ДатаНачалаСобытия; 
...
Процедура Пересчитать(Команда)
	...
	// Уменьшение на страховые взносы
	ЗаполнитьСписокУведомленийНаСервере(Объект.Организация, ПериодСобытияНачало);  
	ОформитьРасходыСтраховыеВзносы(ЭтотОбъект, ПериодСобытияНачало);
...
Процедура ПолучитьДанныеОтчетности()
	...
	|	И ЗначенияПоказателейОтчетности.ЗаписьКалендаря = &ЗаписьКалендаря");
	...
	Запрос.УстановитьПараметр("ЗаписьКалендаря", Объект.СобытиеКалендаря);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ПоказательОтчетности = ПланыВидовХарактеристик.ПоказателиОтчетности.СуммаПатентДоУменьшения Тогда
			СуммаПатентДоУменьшения = Выборка.ЗначениеПоказателя;
		КонецЕсли;
	...
	Если УпрощенныйЗачетФиксированныхВзносов Тогда
		...
		Если КонецКвартала(ДатаДокументаОбработкиСобытия) = КонецГода(ДатаДокументаОбработкиСобытия) Тогда
		...
		Объект.Организация,
		ДатаДокументаОбработкиСобытия, Периодичность);
		...
		ДатаДокументаОбработкиСобытия,
		Истина); 
		ВзносыСДоходов = РегламентированнаяОтчетностьУСН.ВзносыСДоходовПодлежащиеУплате(Объект.Организация, ДатаДокументаОбработкиСобытия);
		ВзносСДоходов = ВзносыСДоходов.Всего;    
		
		ТекущийГод = Год(ДатаДокументаОбработкиСобытия); 
		
		Элементы.ВзносСДоходов.Подсказка = СтрШаблон(НСтр("ru = 'Остаток взноса за %1 г. %2 руб. из %3 руб.'"),
...
Процедура РаспечататьКвитанциюЗавершение() Экспорт
	...
	СтруктураПечати = ВыгрузитьДокументВФайл(Новый Структура("НачислениеНалогов, СобытиеКалендаря", ДокументВзаиморасчетовСБюджетом, Объект.СобытиеКалендаря), "Квитанция");
...
Функция ВыгрузитьДокументВФайл(Объект, ВидФайла)
	...
	Если ВидФайла = "Квитанция" Тогда
		ТабДок = Документы.НачислениеНалогов.СформироватьКвитанцию(Объект.НачислениеНалогов, , Объект.СобытиеКалендаря);
...
Функция ТаблицаНалогиДляУведомления() Экспорт
	...
	Если СуммаКУплате > 0 Тогда 
		...
		НоваяСтрока.Налог                       = Справочники.ВидыНалогов.Патент;
