...
Процедура Печать(Команда)
	...
	ПараметрыПечати = ПолучитьПараметрыПечати(РежимПечати);
...
Процедура УдалитьНепроверяемыеОтборыКомпоновщика(Компоновщик)
	...
	Для Индекс = 1 По Количество Цикл
		...
		Если ТипЗнч(ЭлементОтбора)=Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			...
			Если Строка(ПолеОтбора) = "БезСерий" И ЭлементОтбора.Представление = "" Тогда
...
Процедура ЗаполнитьТаблицуТоваровНаСервере(Сохранять = Истина)
	...
	СтруктураТоваровИзПриложения.Очистить();
...
Процедура ЗаполнитьОбработкуПоПакетамЦенников(Параметры)
	...
	СтруктураТоваров.Колонки.Добавить("Партия",             Новый ОписаниеТипов("СправочникСсылка.ПартииНоменклатуры"));
	СтруктураТоваров.Колонки.Добавить("ЕдиницаИзмерения",             Новый ОписаниеТипов("СправочникСсылка.ЕдиницыИзмерения"));
	СтруктураТоваров.Колонки.Добавить("КоличествоЦенников",   Новый ОписаниеТипов("Число"));
	
	Для Каждого Строка Из Параметры.СтруктураТоваров Цикл
		
		Товар = СтруктураТоваров.Добавить();
		Товар.Номенклатура       = Строка.Номенклатура;
		Товар.Характеристика     = Строка.Характеристика;
		Товар.Партия             = Строка.Партия;
	...
	СтруктураТоваровИзПриложения.Загрузить(СтруктураТоваров);
...
Процедура ОчиститьРаспечатанныеЦенники()
	...
	Для каждого Строка Из СтруктураТоваровИзПриложения Цикл
		...
		Для каждого СтрокаТаблицыТоваров Из СтрокиМассив Цикл
			...
			Набор.Отбор.Номенклатура.Установить(СтрокаТаблицыТоваров.Номенклатура);
			Набор.Отбор.Характеристика.Установить(СтрокаТаблицыТоваров.Характеристика);
			Набор.Отбор.Партия.Установить(СтрокаТаблицыТоваров.Партия);
			Набор.Отбор.ЕдиницаИзмерения.Установить(СтрокаТаблицыТоваров.ЕдиницаИзмерения);
			Набор.Записать();
...
&НаСервере
Функция ПолучитьПараметрыПечати(РежимПечати)
	
	ПараметрыПечати = Новый Структура;
	ПараметрыПечати.Вставить("Режим", Объект.Режим);
	ПараметрыПечати.Вставить("РежимПечати", РежимПечати);
	ПараметрыПечати.Вставить("ВидЦен", Объект.ВидЦен);
	ПараметрыПечати.Вставить("Организация", Объект.Организация);
	ПараметрыПечати.Вставить("ЦеныПоВидуЦены", Объект.ЦеныПоВидуЦены);
	ПараметрыПечати.Вставить("ЦеныНаДату", Объект.ЦеныНаДату);
	ПараметрыПечати.Вставить("ЦеныНаДатуАкционные", Объект.ЦеныНаДатуАкционные);
	ПараметрыПечати.Вставить("ВидЦенАкционные", Объект.ВидЦенАкционные);
