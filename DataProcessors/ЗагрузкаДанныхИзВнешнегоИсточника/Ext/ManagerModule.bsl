...
Процедура ПредварительнаяОбработкаДанных(ПараметрыВыполнения, АдресХранилища) Экспорт
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.СформироватьНастройкиОбновленияСвойств(ПараметрыВыполнения.ДеревоПолей,
		ПараметрыВыполнения.НастройкиЗагрузкиДанных);

	Если ПараметрыВыполнения.ТаблицаСопоставленияДанных.Колонки.Найти("НомерГТД_ВходящиеДанные") <> Неопределено Тогда

		ПроверитьКорректностьЗаполненияНомеровГТД(ПараметрыВыполнения.ТаблицаСопоставленияДанных,
			ПараметрыВыполнения.Ошибки);
	...
	Если Не ПараметрыВыполнения.ПропуститьСтраницу Тогда
		...
		ПараметрыВыполнения.ЗапретитьПереходДалее = (ПараметрыВыполнения.Ошибки.Найти(Истина, "Критичная")
			<> Неопределено);
...
Процедура ДобавитьКолонкиТаблицыСопоставлений(ЭтотОбъект, ТаблицаСопоставленияДанных, НастройкиЗагрузкиДанных,
		ПакетнаяЗагрузка = Ложь) Экспорт  
...
Процедура СоздатьДеревоПолейЗагрузкиДоступныхПользователю(ДеревоПолей, НастройкиЗагрузкиДанных) Экспорт
	...
	Для каждого ГруппаПолей Из ГруппыИПоля.Строки Цикл
		...
		Если ЭтоПользовательскаяГруппаПолей Тогда
			...
			НоваяСтрока 					= ДеревоПолей.Строки.Добавить();
			НоваяСтрока.ПредставлениеПоля	= ГруппаПолей.ПредставлениеГруппы;
...
Процедура ДобавитьДополнительныйРеквизитПоиска(ЭтотОбъект, ДополнительныйРеквизитПоиска) Экспорт
	
	Элементы = ЭтотОбъект.Элементы;
	ПутьРеквизита = "ТаблицаСопоставленияДанных";
	ОписаниеТиповСтрока1024 = Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(1024));
	МассивРеквизитов = Новый Массив;
	ПрефиксПоля = ЗагрузкаДанныхИзВнешнегоИсточника.ПрефиксДополнительногоРеквизитаПоиска();
	
	Значение = ПрефиксПоля + ДополнительныйРеквизитПоиска.ИдентификаторДляФормул;
	
	Если Элементы.Найти(Значение) <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	НовыйРеквизит = Новый РеквизитФормы(Значение, ОписаниеТиповСтрока1024, ПутьРеквизита, ДополнительныйРеквизитПоиска.Заголовок);
	МассивРеквизитов.Добавить(НовыйРеквизит);
	
	ЭтотОбъект.ИзменитьРеквизиты(МассивРеквизитов);
	
	НовыйЭлемент 				= Элементы.Добавить(Значение, Тип("ПолеФормы"), Элементы.ТаблицаСопоставленияДанных);
	НовыйЭлемент.Вид			= ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным	= "ТаблицаСопоставленияДанных." + Значение;
	НовыйЭлемент.Заголовок		= ДополнительныйРеквизитПоиска.Заголовок;
	НовыйЭлемент.ТолькоПросмотр = Истина;
	НовыйЭлемент.Ширина			= 8;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ИдентификаторДела = "ЗагрузитьНачальныеОстатки";
	
	Если НЕ ПравоДоступа("Использование", Метаданные.Обработки.ПомощникВводаНачальныхОстатков)
		ИЛИ НЕ ПравоДоступа("Использование", Метаданные.Обработки.ЗагрузкаДанныхИзВнешнегоИсточника)
	...
	Дело.Важное			= Истина;
	Дело.Представление	= НСтр("ru = 'Загрузить остатки из Excel'");
	Дело.Количество		= 0;
	Дело.Форма			= "Обработка.ЗагрузкаДанныхИзВнешнегоИсточника.Форма.ПомощникЗагрузкиДанныхИзВнешнегоИсточника";
	Дело.ПараметрыФормы	= Новый Структура("НастройкиЗагрузкиДанных", НастройкиЗагрузкиДанных);
	Дело.Владелец		= НСтр("ru='Начальные остатки'");
	Дело.Подсказка		= "";
...
Процедура ПроверитьЗаполнениеТабличногоДокументаИЗаполнитьТаблицуФормы(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.НастройкиЗагрузкиДанных.Свойство("ЗагрузкаИзФайлаOzon") Тогда
		ЗагрузкаИзФайлаOzon = ПараметрыВыполнения.НастройкиЗагрузкиДанных.ЗагрузкаИзФайлаOzon;  
	КонецЕсли;

	ДополнительныйРеквизитПоиска = ПараметрыВыполнения.НастройкиЗагрузкиДанных.НастройкиПоиска.ДополнительныйРеквизитПоиска;

	Если ДополнительныйРеквизитПоиска <> Неопределено Тогда

		ДополнительныйРеквизитПоискаИдентификатор = ДополнительныйРеквизитПоиска.ИдентификаторДляФормул;

	Иначе

		ДополнительныйРеквизитПоискаИдентификатор = "";
	...
	Для НомерСтроки = 2 По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		Если ПараметрыВыполнения.НастройкиЗагрузкиДанных.ФиксированныйШаблон Тогда
			...
			Для номерКолонкиДопРеквизита = 13 По 22 Цикл
				...
				ЗначениеЯчейки = ТабличныйДокумент.ПолучитьОбласть(НомерСтроки,
					номерКолонкиДопРеквизита).ТекущаяОбласть.Текст;
		...
		Для Каждого ГруппаИлиПоле Из КопияГруппыИПоля.Строки Цикл
			...
			Если ПустаяСтрока(ГруппаИлиПоле.ИмяГруппыПолей) Тогда
				...
				Если ГруппаИлиПоле.ИмяПоля
					= ЗагрузкаДанныхИзВнешнегоИсточника.ИмяПоляДобавленияДополнительныхРеквизитов() Тогда
					...
					Для Каждого ПолеИзГруппыПолейДополнительныхРеквизитов Из ГруппаИлиПоле.Строки Цикл
						...
						ЗначениеЯчейки = ТабличныйДокумент.ПолучитьОбласть(НомерСтроки,
							ПолеИзГруппыПолейДополнительныхРеквизитов.НомерКолонки).ТекущаяОбласть.Текст;

						ИмяРеквизита = ПолеИзГруппыПолейДополнительныхРеквизитов.ИмяПоля + ПостФикс;
						Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрокаДанных,
							ИмяРеквизита) Тогда
				...
				ИначеЕсли ГруппаИлиПоле.ИмяПоля = "СуммаВозвращено" И ЗагрузкаИзФайлаOzon Тогда 
					
					ЗначениеЯчейки = ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 6).ТекущаяОбласть.Текст;  
					
					НоваяСтрокаДанных[ГруппаИлиПоле.ИмяПоля + ПостФикс] = ЗначениеЯчейки;
					НоваяСтрокаДанных.СуммаВозвращено = ЗначениеЯчейки;
					
					Продолжить; 
					
				ИначеЕсли ГруппаИлиПоле.ИмяПоля = "Сумма" И ЗагрузкаИзФайлаOzon Тогда  
					
					ЗначениеЯчейки = ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 9).ТекущаяОбласть.Текст;  
					
					НоваяСтрокаДанных[ГруппаИлиПоле.ИмяПоля + ПостФикс] = ЗначениеЯчейки;
					НоваяСтрокаДанных.Сумма = ЗначениеЯчейки;
					
					Продолжить;  
					
				ИначеЕсли ГруппаИлиПоле.ИмяПоля = "Доплата" И ЗагрузкаИзФайлаOzon Тогда   
					
					ЗначениеЯчейки = ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 10).ТекущаяОбласть.Текст;  
					
					НоваяСтрокаДанных[ГруппаИлиПоле.ИмяПоля + ПостФикс] = ЗначениеЯчейки;
					НоваяСтрокаДанных.Доплата = ЗначениеЯчейки; 
					
					Продолжить;
					
				ИначеЕсли ГруппаИлиПоле.ИмяПоля = "ДоплатаВозвращено" И ЗагрузкаИзФайлаOzon Тогда 
					
					ЗначениеЯчейки = ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 12).ТекущаяОбласть.Текст;  
					
					НоваяСтрокаДанных[ГруппаИлиПоле.ИмяПоля + ПостФикс] = ЗначениеЯчейки;
					НоваяСтрокаДанных.ДоплатаВозвращено = ЗначениеЯчейки;
					
					Продолжить;  
					
				ИначеЕсли ГруппаИлиПоле.ИмяПоля = "КоличествоВозвращено" И ЗагрузкаИзФайлаOzon Тогда 
					
					ЗначениеЯчейки = ТабличныйДокумент.ПолучитьОбласть(НомерСтроки, 11).ТекущаяОбласть.Текст;  
					
					НоваяСтрокаДанных[ГруппаИлиПоле.ИмяПоля + ПостФикс] = ЗначениеЯчейки;
					НоваяСтрокаДанных.КоличествоВозвращено = ЗначениеЯчейки;
				...
				ЗначениеЯчейки = ТабличныйДокумент.ПолучитьОбласть(НомерСтроки,
					ГруппаИлиПоле.НомерКолонки).ТекущаяОбласть.Текст;
				
				Если ГруппаИлиПоле.ИмяПоля = "Картинка" Или ГруппаИлиПоле.ИмяПоля = "КартинкаПоУмолчанию" Тогда
					Если ТабличныйДокумент.ПолучитьОбласть(НомерСтроки,
						ГруппаИлиПоле.НомерКолонки).Рисунки.Количество() > 0 Тогда
							
							Картинка = ТабличныйДокумент.ПолучитьОбласть(НомерСтроки,
								ГруппаИлиПоле.НомерКолонки).Рисунки[0].Картинка;
							ИмяФайлаКартинки = СтрШаблон("%1%2%3_%4.png", КаталогВременныхФайлов(),
								ПолучитьРазделительПутиСервера(), НСтр("ru='Картинка'"), Формат(ТекущаяДатаСеанса(),
								"ДФ=ddMMyyyy_hhmmss;"));
							Картинка.Записать(ИмяФайлаКартинки);
							ЗначениеЯчейки = ИмяФайлаКартинки;							
				...
				Если ГруппаИлиПоле.НомерЦветаОригинал = 1 И Не ЗначениеЗаполнено(ЗначениеЯчейки) Тогда

					ТекстОшибки = НСтр(
						"ru = 'В колонке {%1} присутствуют незаполненные ячейки. При обработке данные строки будут пропущены.'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
						ГруппаИлиПоле.ПредставлениеПоля);
					МестоВозникновения = НСтр("ru = 'Строка № %1.'");
					МестоВозникновения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(МестоВозникновения,
						НомерСтроки);
			...
			Иначе
				...
				Для Каждого ПолеИзГруппыПолей Из ГруппаИлиПоле.Строки Цикл
					...
					ЗначениеЯчейки = ТабличныйДокумент.ПолучитьОбласть(НомерСтроки,
						ПолеИзГруппыПолей.НомерКолонки).ТекущаяОбласть.Текст;
					ПриведенноеИмяПоля = ИдентификаторДляФормул(ПолеИзГруппыПолей.ИмяПоля);
					Если ВРег(ПриведенноеИмяПоля) = ВРег(ДополнительныйРеквизитПоискаИдентификатор) Тогда

						НоваяСтрокаДанных[ЗагрузкаДанныхИзВнешнегоИсточника.ПрефиксДополнительногоРеквизитаПоиска()
							+ ДополнительныйРеквизитПоискаИдентификатор] = ЗначениеЯчейки;
					...
					Если ПолеИзГруппыПолей.НомерЦветаОригинал = 1 И Не ЗначениеЗаполнено(ЗначениеЯчейки) Тогда

						ТекстОшибки = НСтр(
							"ru = 'В колонке {%1} присутствуют незаполненные ячейки. При обработке данные строки будут пропущены.'");
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
							ПолеИзГруппыПолей.ПредставлениеПоля);
						МестоВозникновения = НСтр("ru = 'Строка № %1.'");
						МестоВозникновения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							МестоВозникновения, НомерСтроки);
		...
		ТекстПрогресса = СтрШаблон(НСтр("ru ='Проверено %1 из %2 строк...'"), НомерСтроки,
...
Процедура СоздатьШаблонДереваПолейЗагрузкиДоступныхПользователю(ДеревоПолей)
	...
	ОписаниеТиповЧисло1_0 = Новый ОписаниеТипов("Число", , , , Новый КвалификаторыЧисла(1, 0,
		ДопустимыйЗнак.Неотрицательный));
	ОписаниеТиповЧисло2_0 = Новый ОписаниеТипов("Число", , , , Новый КвалификаторыЧисла(2, 0,
		ДопустимыйЗнак.Неотрицательный));
	ОписаниеТиповОТ = Новый ОписаниеТипов("ОписаниеТипов");
	ОписаниеТипаДопРеквизитов = Новый ОписаниеТипов("СправочникСсылка.НаборыДополнительныхРеквизитовИСведений");
	...
	ДеревоПолей.Колонки.Добавить("ПредставлениеГруппы",		ОписаниеТиповСтрока256,,);
	...
	ДеревоПолей.Колонки.Добавить("ДопРеквизитВладелец",	ОписаниеТипаДопРеквизитов);
	...
	Для каждого Поле Из ГруппаПолей.Строки Цикл
		...
		Если Поле.Видимость Тогда
			...
			ИначеЕсли Поле.ДопРеквизитПризнак = Истина 
				...
				НоваяСтрока.ДопРеквизитВладелец = Поле.ДопРеквизитВладелец;
...
Функция ПолучитьСоответствиеДополнительныхПолейСсылки(ГруппаПолей)
	
	Результат = Новый Соответствие;
	КоллекцияТипов = ГруппаПолей.ТипПолучаемогоЗначения;
	
	Если КоллекцияТипов.СодержитТип(Тип("СправочникСсылка.Контрагенты")) Тогда
		
		Результат.Вставить("Наименование", ПолучитьСтруктуруПоля("Наименование", 
			НСтр("ru ='Контрагент (наименование)'")));
			
		Результат.Вставить("ИНН_КПП", ПолучитьСтруктуруПоля("ИНН", НСтр("ru = 'ИНН'"), 11));
		
		Результат.Вставить("РасчетныйСчет", ПолучитьСтруктуруПоля("БанковскийСчетПоУмолчанию", 	
			НСтр("ru = 'Контрагент (расчетный счет)'"), 11));
		
	ИначеЕсли КоллекцияТипов.СодержитТип(Тип("СправочникСсылка.Номенклатура")) Тогда
		
		Результат.Вставить("Код", ПолучитьСтруктуруПоля("Код", НСтр("ru = 'Код'"), 9));
		
		Результат.Вставить("Штрихкод", ПолучитьСтруктуруПоля("Штрихкод", НСтр("ru = 'Штрихкод'"), 9));
		
		Результат.Вставить("Артикул", ПолучитьСтруктуруПоля("Артикул", НСтр("ru = 'Артикул'"), 9));
		
		Результат.Вставить("НоменклатураНаименование", ПолучитьСтруктуруПоля("Наименование", 
			НСтр("ru = 'Номенклатура (наименование)'")));
			
		Результат.Вставить("НоменклатураНаименованиеПолное", ПолучитьСтруктуруПоля("НаименованиеПолное", 
			НСтр("ru = 'Номенклатура (полное наименование)'")));
		
		Результат.Вставить("РодительКод", ПолучитьСтруктуруПоля("РодительКод", НСтр("ru = 'Группа (код)'"), 9));
		
		Результат.Вставить("РодительНаименование", ПолучитьСтруктуруПоля("РодительНаименование", 
			НСтр("ru = 'Группа (наименование)'"), 9));
		
	ИначеЕсли КоллекцияТипов.СодержитТИп(Тип("СправочникСсылка.НоменклатураПоставщиков")) Тогда
		
		Результат.Вставить("АртикулПоставщика", ПолучитьСтруктуруПоля("Артикул", 
			НСтр("ru = 'Артикул поставщика'")));
			
		Результат.Вставить("ИдентификаторПоставщика", ПолучитьСтруктуруПоля("Идентификатор", 
			НСтр("ru = 'Идентификатор поставщика'")));
			
		Результат.Вставить("НоменклатураПоставщиковНаименование", ПолучитьСтруктуруПоля("Наименование", 
			НСтр("ru = 'Номенклатура поставщиков (наименование)'")));
		
	ИначеЕсли КоллекцияТипов.СодержитТип(Тип("СправочникСсылка.ХарактеристикиНоменклатуры")) Тогда
		
		Результат.Вставить("ХарактеристикаНаименованиеДляПечати", ПолучитьСтруктуруПоля("НаименованиеДляПечати", 
			НСтр("ru = 'Характеристика (наименование для печати)'")));
		
		Результат.Вставить("ХарактеристикаНаименование", ПолучитьСтруктуруПоля("Наименование", 
			НСтр("ru = 'Характеристика (наименование)'")));
			
		Результат.Вставить("ХарактеристикаАртикул", ПолучитьСтруктуруПоля("Артикул", 
			НСтр("ru = 'Характеристика (артикул)'"), 9));
		
	ИначеЕсли КоллекцияТипов.СодержитТИп(Тип("СправочникСсылка.КонтактныеЛица")) Тогда
		
		Результат.Вставить("Наименование", ПолучитьСтруктуруПоля("Наименование", 
			НСтр("ru = 'ФИО (наименование)'")));
			
		Результат.Вставить("Код", ПолучитьСтруктуруПоля("Код", НСтр("ru = 'Код'"), 9));
		
	ИначеЕсли КоллекцияТипов.СодержитТИп(Тип("СправочникСсылка.Лиды")) Тогда
		
		Результат.Вставить("Наименование", ПолучитьСтруктуруПоля("Наименование", 
			НСтр("ru = 'Лид (наименование)'")));
...
Функция ПолучитьСтруктуруПоля(Поле, Представление, ШиринаПоля = 4)
	
	Результат = Новый Структура;
	Результат.Вставить("Поле", Поле);
	Результат.Вставить("Представление", Представление);
	Результат.Вставить("ШиринаПоля", ШиринаПоля);
...
Процедура ДобавитьРеквизитыИзПользовательскойГруппыПолей(ЭтотОбъект, ГруппаПолей, ПутьРеквизита, БлокируемыеРеквизиты = Неопределено)
	...
	СоответствиеДополнительныхПолейСсылки = ПолучитьСоответствиеДополнительныхПолейСсылки(ГруппаПолей);
	...
	НовыйЭлементВходящие 				= Элементы.Добавить(ГруппаПолей.ИмяГруппыПолей, Тип("ПолеФормы"), Элементы.ТаблицаСопоставленияДанных);
	НовыйЭлементВходящие.Вид			= ВидПоляФормы.ПолеВвода;
	НовыйЭлементВходящие.ПутьКДанным	= "ТаблицаСопоставленияДанных." + ГруппаПолей.ИмяГруппыПолей;
	НовыйЭлементВходящие.Заголовок		= ГруппаПолей.ПредставлениеПоля;
	НовыйЭлементВходящие.РежимРедактирования = РежимРедактированияКолонки.Вход;
	НовыйЭлементВходящие.ОтметкаНезаполненного = ГруппаПолей.ОбязательноеЗаполнениеГруппы;
	НовыйЭлементВходящие.АвтоОтметкаНезаполненного = ГруппаПолей.ОбязательноеЗаполнениеГруппы;
	НовыйЭлементВходящие.КнопкаСоздания = Ложь;
	НовыйЭлементВходящие.Видимость = Истина;
	
	Если ГруппаПолей.ОбязательноеЗаполнениеГруппы Тогда
		НовыйЭлементВходящие.ФиксацияВТаблице = ФиксацияВТаблице.Лево;
	КонецЕсли;
	...
	ГруппаНайденныеДанные = Элементы.Добавить("ГруппаНайденные" + ГруппаПолей.ИмяГруппыПолей, Тип("ГруппаФормы"), ГруппаПервогоУровня);
	ГруппаНайденныеДанные.Группировка = ГруппировкаКолонок.Горизонтальная;
	ГруппаНайденныеДанные.ОтображатьЗаголовок = Ложь;
	
	ГруппаВходящиеДанные = Элементы.Добавить("ГруппаВходящая" + ГруппаПолей.ИмяГруппыПолей, Тип("ГруппаФормы"), ГруппаПервогоУровня);
	ГруппаВходящиеДанные.Группировка = ГруппировкаКолонок.Горизонтальная;
	ГруппаВходящиеДанные.ОтображатьЗаголовок = Ложь;
	...
	Для каждого СтрокаГруппы Из ГруппаПолей.Строки Цикл
		...
		НовыйЭлементВходящие 				= Элементы.Добавить(СтрокаГруппы.ИмяПоля, Тип("ПолеФормы"), ГруппаВходящиеДанные);
		НовыйЭлементВходящие.Вид			= ВидПоляФормы.ПолеВвода;
		НовыйЭлементВходящие.ПутьКДанным	= "ТаблицаСопоставленияДанных." + СтрокаГруппы.ИмяПоля;
		НовыйЭлементВходящие.Заголовок		= СтрокаГруппы.ПредставлениеПоля;
		НовыйЭлементВходящие.ТолькоПросмотр = Истина;
		НовыйЭлементВходящие.Ширина 		= 4;
		НовыйЭлементВходящие.РастягиватьПоГоризонтали = Ложь;
		
		ИмяПоля = СтрокаГруппы.ИмяПоля;
		Если СоответствиеДополнительныхПолейСсылки.Получить(ИмяПоля)<>Неопределено Тогда
			
			НовыйЭлемент 				= Элементы.Добавить(ГруппаПолей.ИмяГруппыПолей + СтрокаГруппы.ИмяПоля, Тип("ПолеФормы"), ГруппаНайденныеДанные);
			НовыйЭлемент.Вид			= ВидПоляФормы.ПолеВвода;
			
			ПолеСсылки = СтрЗаменить(СоответствиеДополнительныхПолейСсылки[ИмяПоля].Поле, ГруппаПолей.ИмяГруппыПолей, "");
			ПредставлениеСсылки = СоответствиеДополнительныхПолейСсылки[ИмяПоля].Представление;
			
			ПутьКДанным = "ТаблицаСопоставленияДанных." + ГруппаПолей.ИмяГруппыПолей + "." + ПолеСсылки;
			
			НовыйЭлемент.ПутьКДанным	= ПутьКДанным;
			НовыйЭлемент.Заголовок		= ПредставлениеСсылки;
			НовыйЭлемент.ТолькоПросмотр = Истина;
			НовыйЭлемент.Ширина 		= СоответствиеДополнительныхПолейСсылки[ИмяПоля].ШиринаПоля;
			НовыйЭлемент.РастягиватьПоГоризонтали = Ложь;
			
			НовыйЭлементВходящие.Ширина = СоответствиеДополнительныхПолейСсылки[ИмяПоля].ШиринаПоля;
...
Функция ИдентификаторДляФормул(СтрокаПредставления)
