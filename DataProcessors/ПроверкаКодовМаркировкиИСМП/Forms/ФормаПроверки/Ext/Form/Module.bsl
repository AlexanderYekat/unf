...
Процедура УстановитьПоляПредставленияСтроки(Форма, ТекущиеДанные)
	...
	Если ОбщегоНазначенияИСКлиентСерверПовтИсп.ПоддерживаетсяЧастичноеВыбытие(СтрокаДанных["ВидПродукции"])
		...
		И СтрокаДанных["ЧастичноеВыбытиеОстаток"] <> Неопределено Тогда
		
		ДанныеЧастичногоВыбытия = Новый Массив();
		Если СтрокаДанных["ВидПродукции"] = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Пиво") Тогда
			
			СтрокиЧастичноеВыбытиеПива = Новый Массив();
			СтрокиЧастичноеВыбытиеПива.Добавить(
				СтрШаблон(
					НСтр("ru = 'Остаток по данным ГИС МТ: %1 из %2 л.'"),
					СтрокаДанных["ЧастичноеВыбытиеОстаток"],
					СтрокаДанных["ЕмкостьПотребительскойУпаковки"]));
					
			Если СтрокаДанных["ЧастичноеВыбытиеОстатокСУчетомОбъемаСлива"] <> Неопределено
				И ЗначениеЗаполнено(СтрокаДанных["ЧастичноеВыбытиеОбъемСливаПредставление"]) Тогда
					
				СтрокиЧастичноеВыбытиеПива.Добавить(СтрокаДанных["ЧастичноеВыбытиеОбъемСливаПредставление"]);
				СтрокиЧастичноеВыбытиеПива.Добавить(
					СтрШаблон(
						НСтр("ru = 'Расчетный остаток: %1 л.'"),
						СтрокаДанных["ЧастичноеВыбытиеОстатокСУчетомОбъемаСлива"]));
				
			КонецЕсли;
			
			ДанныеЧастичногоВыбытия.Добавить(СтрСоединить(СтрокиЧастичноеВыбытиеПива, Символы.ПС));
			
		Иначе
			
			ДанныеЧастичногоВыбытия.Добавить(
				СтрШаблон(
					НСтр("ru = 'Доступно %1 из %2'"),
					СтрокаДанных["ЧастичноеВыбытиеОстаток"],
					СтрокаДанных["ЕмкостьПотребительскойУпаковки"]));
		...
		Если СтрокаДанных["ЧастичноеВыбытиеНеКорректныйОстаток"] Тогда
			ДанныеЧастичногоВыбытия.Добавить(" ");
			ДанныеЧастичногоВыбытия.Добавить(НСтр("ru = '(не корректно)'"));
		КонецЕсли;
		
		ЗначениеПодсказкиЧастичноеВыбытие = Новый ФорматированнаяСтрока(ДанныеЧастичногоВыбытия);
	...
	Если ЗначениеЗаполнено(СтрокаДанных["ИННАгента"]) Тогда
		Если НаименованиеАгента.Количество() Тогда
...
Процедура ЗаполнитьСтрокуДереваПоДаннымШтрихкода(НайденныйИдентификаторСтроки, ДанныеШтрихкода, ПараметрыОбработки)
	...
	Если СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво
		...
		ДанныеПодключения = РегистрыСведений.КегиНаОборудованииРозливаИСМП.ПолучитьПодключениеКОборудованию(СтрокаДанных.ШтрихкодУпаковки)[СтрокаДанных.ШтрихкодУпаковки];
		Если ДанныеПодключения <> Неопределено
			И ЗначениеЗаполнено(ДанныеПодключения.ОбъемСлива) Тогда
			
			ДнейСлива  = Цел((НачалоДня(ТекущаяДатаСеанса()) - НачалоДня(ДанныеПодключения.ДатаПодключения)) / (60 * 60 * 24));
			
			Если ДнейСлива > 0 Тогда
				ОбъемСлива = ДанныеПодключения.ОбъемСлива * ДнейСлива;
