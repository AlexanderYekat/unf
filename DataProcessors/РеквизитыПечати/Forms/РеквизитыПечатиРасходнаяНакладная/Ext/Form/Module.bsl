...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИменаРеквизитов = 
		...
		|Вес, Объем, ДанныеОТранспортировке, СопроводительныеДокументы,
		|НомерСчетаНаОплату,
	...
	Если НЕ ЗначениеЗаполнено(КонтекстПечати.ДанныеОТранспортировке)
		И (ЗначениеЗаполнено(КонтекстПечати.Вес)
		ИЛИ ЗначениеЗаполнено(КонтекстПечати.Объем)) Тогда
			ЗаполнитьПодсказкуДанныеОТранспортировке();
	КонецЕсли;
...
&НаКлиенте
Процедура ДанныеОТранспортировкеПриИзменении(Элемент)
	
	ЗафиксироватьИзменениеЗначенияРеквизита("ДанныеОТранспортировке");
	Модифицированность = Истина;
...
Процедура СопроводительныеДокументыПриИзменении(Элемент)
	...
	ЗафиксироватьИзменениеЗначенияРеквизита("СопроводительныеДокументы");
	Модифицированность = Истина;
...
&НаСервере
Процедура ЗаполнитьПодсказкуДанныеОТранспортировке()
	
	МассивСтрок = Новый Массив;
	Если ЗначениеЗаполнено(КонтекстПечати.Вес) Тогда
		ШаблонВеса = НСтр("ru = 'масса брутто: %1'");
		ВесПрописью = ПечатьДокументовУНФ.ИнформацияОГрузеПрописью(КонтекстПечати.Вес, Истина);
		СтрокаВеса = СтрШаблон(ШаблонВеса, ВесПрописью);
		МассивСтрок.Добавить(СтрокаВеса);
	КонецЕсли;

	Если ЗначениеЗаполнено(КонтекстПечати.Объем) Тогда
		ШаблонОбъема = НСтр("ru = 'объем: %1'");
		ОбъемПрописью = ПечатьДокументовУНФ.ИнформацияОГрузеПрописью(КонтекстПечати.Объем, Ложь);
		СтрокаОбъема = СтрШаблон(ШаблонОбъема, ОбъемПрописью);
		МассивСтрок.Добавить(СтрокаОбъема);
	КонецЕсли;
	
	ДанныеОТранспортировке = СтрСоединить(МассивСтрок, ", ");
	
	Элементы.ДанныеОТранспортировке.ПодсказкаВвода = ДанныеОТранспортировке;
	
КонецПроцедуры

&НаКлиенте
Процедура ДанныеОТранспортировкеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	
	Если НЕ ЗначениеЗаполнено(КонтекстПечати.ДанныеОТранспортировке)
		И НЕ ПустаяСтрока(Элементы.ДанныеОТранспортировке.ПодсказкаВвода) Тогда
		КонтекстПечати.ДанныеОТранспортировке = Элементы.ДанныеОТранспортировке.ПодсказкаВвода;
	КонецЕсли;
