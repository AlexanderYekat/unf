...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтаФорма.ИсточникКатегории = ПредопределенноеЗначение("Перечисление.ИсточникиКатегорийДляМаркетплейса.ВидНоменклатуры") Тогда
		Элементы.ИмпортируемыеТоварыКатегорияИлиНоменклатураДляСопоставленияКатегорииOzon.Заголовок = "Категория для сопоставления с категорией Ozon";
	...
	ИначеЕсли ЭтаФорма.ИсточникКатегории = ПредопределенноеЗначение("Перечисление.ИсточникиКатегорийДляМаркетплейса.ИерархияНоменклатуры") Тогда
		Элементы.ИмпортируемыеТоварыКатегорияИлиНоменклатураДляСопоставленияКатегорииOzon.Заголовок = "Номенклатура или группа для сопоставления с категорией Ozon";
...
Процедура ИзменитьРеквизитыФормы()
	...
	ЭлементЗначениеОбъединенияВОднуКарточку.Заголовок = "Значение для объединения в одну карточку";
...
Процедура УстановитьВидимостьДоступность()
	...
	И ВариантЗагрузкиДанных = ИнтеграцияСМаркетплейсомOzonКлиентСервер.ВариантЗагрузкиЗагрузитьНесопоставленныеКарточкиТоваров() Тогда
		Элементы.ЗаписатьЗагруженныеДанные.Заголовок = "Создать и записать данные в 1С";
	Иначе
		Элементы.ЗаписатьЗагруженныеДанные.Заголовок = "Записать данные в 1С";
	КонецЕсли;
...
Процедура ЗагрузитьИСоздатьВсеТоварыИзOzonЗавершениеФоновогоЗадания( Результат, ДополнительныеПараметры ) Экспорт
	...
	Иначе
		ПоказатьПредупреждение( Новый ОписаниеОповещения, "Не найдено новых описаний товаров для импорта на сайте Ozon." );
		ФормаРежимОтображенияЭлементыОбновить( ИнтеграцияСМаркетплейсомOzonКлиентСервер.ФормаРежимОтображенияОткрытие() ); 
	КонецЕсли;
...
&НаСервере
Процедура ЗагрузитьИСоздатьВсеТоварыИзOzonРезультатОбработатьНаСервере( АдресРезультата ) Экспорт
