...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ИгнорироватьКонтрольОстатков") Тогда
		ИгнорироватьКонтрольОстатков = Параметры.ИгнорироватьКонтрольОстатков;
	Иначе
		ИгнорироватьКонтрольОстатков = Ложь;
	КонецЕсли;
	
	Если Параметры.Свойство("СерииДоКорректировки") Тогда
		СерииДоКорректировки = Параметры.СерииДоКорректировки;
	Иначе
		СерииДоКорректировки = Ложь;
	КонецЕсли;
	
	ЭтаФорма.ТолькоПросмотр = СерииДоКорректировки;
	...
	Если КонтрольОстатковСерийНоменклатуры И Не ИгнорироватьКонтрольОстатков Тогда
		
		СерииНоменклатурыОстатки.ТекстЗапроса = ТекстЗапросаСерииОстатки();
		
		Если Не СтруктурнаяЕдиница = Неопределено Тогда
			ИспользуетсяСтараяОрдернаяСхема = (СтруктурнаяЕдиница.ВидУчетаОрдерныхСкладов = Перечисления.ВидыУчетаОрдерныхСкладов.ПоСкладуВЦелом
																							И ТипЗнч(Параметры.ДокСсылка) = Тип("ДокументСсылка.РасходныйОрдер"));
		Иначе
			ИспользуетсяСтараяОрдернаяСхема = Истина;
		КонецЕсли;
		
		СерииНоменклатурыОстатки.Параметры.УстановитьЗначениеПараметра("ИспользуетсяСтараяОрдернаяСхема", ИспользуетсяСтараяОрдернаяСхема);
		...
		Если ПолучитьФункциональнуюОпцию("ПередачаТоваровМеждуОрганизациями") Тогда
			СерииНоменклатурыОстатки.Параметры.УстановитьЗначениеПараметра("Организации", 
				ИнтеркампаниСервер.ОрганизацииОстатков(Параметры.Организация));
		Иначе
			СерииНоменклатурыОстатки.Параметры.УстановитьЗначениеПараметра("Организации", 
	...
	Иначе //без остатков
		...
		СерииНоменклатурыОстатки.Параметры.УстановитьЗначениеПараметра("ИспользуетсяСтараяОрдернаяСхема", Истина);
	...
	Если Параметры.Свойство("ОтключитьОбщееОповещение") Тогда
		ОтключитьОбщееОповещение = Параметры.ОтключитьОбщееОповещение;
	КонецЕсли;
	...
	НастроитьРежимПодбораМеховыхИзделий();
	//- ГИСМ
...
Процедура УстановитьДоступностьЭлементовФормы()
	...
	Если СерииДоКорректировки Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаКнопкиНовые", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаКнопкиВыбранные", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСтрелки", "Доступность", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СерииНоменклатурыВыбранныеКонтекстноеМеню", "Доступность", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СерииНоменклатурыНовыеКонтекстноеМеню", "Доступность", Ложь);
	КонецЕсли;
...
Функция ТекстЗапросаСерииОстатки()
	...
	|		И СерииНоменклатуры.Организация В(&Организации)
	...
	|	ГДЕ
	|		НЕ &ИспользуетсяСтараяОрдернаяСхема
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		И &ЭтоПриходныйОрдер
	|		И НЕ &ИспользуетсяСтараяОрдернаяСхема
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		И &ЭтоРасходныйОрдер
	|		И НЕ &ИспользуетсяСтараяОрдернаяСхема) КАК ВложенныйЗапросПереопределяемый
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаСерииНоменклатуры()
	...
	|ГДЕ
	|	&ИспользуетсяСтараяОрдернаяСхема";
...
Процедура Завершить(Команда)
	...
	Если СохраненоУспешно Тогда
		...
		СтруктураВозврата.Вставить("КлючСтроки", КлючСвязи);
		СтруктураВозврата.Вставить("АдресВоВременномХранилище", АдресВоВременномХранилище);
		СтруктураВозврата.Вставить("ЭтоОприходование", ЭтоОприходование);
		СтруктураВозврата.Вставить("СерииДоКорректировки", СерииДоКорректировки);
		
		Если НЕ ОтключитьОбщееОповещение Тогда
...
Процедура ВыбратьНомер(Команда)
	...
	Иначе
		ДобавитьСерияВыбранный(Элементы.СерииОстатки.ТекущиеДанные.Серия, Элементы.СерииОстатки.ТекущиеДанные.Количество);
	КонецЕсли; 
...
Процедура ДобавитьСерияВыделенные(СерииНоменклатуры)
	...
	СерииСОстатками.Очистить();
	
	Для Каждого СерияНоменклатуры Из СерииНоменклатуры Цикл
		
		Серия = Элементы.СерииОстатки.ДанныеСтроки(СерияНоменклатуры).Серия;
		Количество = Элементы.СерииОстатки.ДанныеСтроки(СерияНоменклатуры).Количество;
		
		СписокВыбранных.Добавить(Серия);
		
		НоваяСтрока = СерииСОстатками.Добавить();
		НоваяСтрока.Серия = Серия;
		НоваяСтрока.Количество = ?(ИспользоватьУникальныеСерии, 1, Количество);
...
Процедура ДобавитьВыделенные(СписокВыбранных)
	...
	Если Не ИспользоватьУникальныеСерии Тогда
		...
		Пока Результат.Следующий() Цикл
			...
			НайденныеСтрокиКоличество = СерииСОстатками.НайтиСтроки(ПараметрыПоиска);
			Если НайденныеСтрокиКоличество.Количество() И НайденныеСтрокиКоличество[0].Количество > 0 И НайденныеСтрокиКоличество[0].Количество < 1 Тогда
				КолиечествоСерий = НайденныеСтрокиКоличество[0].Количество;
			КонецЕсли;
			
			Если НайденныеСтроки.Количество() Тогда
				НайденныеСтроки[0].Количество = НайденныеСтроки[0].Количество + КолиечествоСерий;
				Продолжить;
			КонецЕсли;
			
			НоваяСтрока = Объект.СерииНоменклатуры.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Результат);
			НоваяСтрока.Количество = КолиечествоСерий;
...
&НаКлиенте
Процедура ДобавитьСерияВыбранный(Серия, КоличествоВСтроке = 1)
	...
	Иначе
		...
		КоличествоДляДобавления = ?(КоличествоВСтроке <= 1 И КоличествоВСтроке > 0, КоличествоВСтроке, 1);
		
		Если НайденныеСтроки.Количество() Тогда
			НайденныеСтроки[0].Количество = НайденныеСтроки[0].Количество + КоличествоДляДобавления;
		Иначе
			НоваяСтрока = Объект.СерииНоменклатуры.Добавить();
			НоваяСтрока.Серия = Серия;
			НоваяСтрока.Количество = КоличествоДляДобавления;
			НоваяСтрока.НовыйНомер = Строка(Серия);
		КонецЕсли;
...
Процедура СерииВыбранныеПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Для каждого стр Из МассивСерийНоменклатуры Цикл
		...
		Если ДанныеСтроки<>Неопределено Тогда
			ДобавитьСерияВыбранный(ДанныеСтроки.Серия, ДанныеСтроки.Количество);
		КонецЕсли;
	КонецЦикла;
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрихкодов)
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		ИначеЕсли СтруктураДанные.ОтборТипНоменклатуры.Найти(ДанныеШтрихкода[0].ТипНоменклатуры) = Неопределено Тогда
		...
		Иначе
			...
			Иначе
				...
				Если Не ИспользоватьУникальныеСерии Тогда
					УжеЕстьТакойЭлемент[0].Количество = УжеЕстьТакойЭлемент[0].Количество + 1;
				КонецЕсли;
...
Процедура ЗаполнитьПоДаннымШтрихкодовЗавершение(СтруктураНоменклатуры, Параметры) Экспорт
	...
	Если ТипЗнч(ДанныеШтрихкода) = Тип("Структура") И ДанныеШтрихкода.Свойство("ТипНоменклатуры") 
		И СтруктураДанные.ОтборТипНоменклатуры.Найти(ДанныеШтрихкода.ТипНоменклатуры) = Неопределено Тогда
...
&НаСервере
Процедура НастроитьРежимПодбораМеховыхИзделий()
	
	ДанныеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "ВидМаркировки, ВидПродукцииИС");
	ИспользуетсяПодсистемаГИСМ = НЕ ИнтеграцияГИСМ.ПодсистемаНеИспользуется();
	
	Если ИспользуетсяПодсистемаГИСМ
		И ЗначениеЗаполнено(ДанныеНоменклатуры.ВидМаркировки)
		И ДанныеНоменклатуры.ВидМаркировки <> Перечисления.ВидыМаркировки.НеМаркируется
		ИЛИ ДанныеНоменклатуры.ВидПродукцииИС = Перечисления.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха
		И НЕ ИспользуетсяПодсистемаГИСМ Тогда
		// Подбор изделий из натурального меха.
