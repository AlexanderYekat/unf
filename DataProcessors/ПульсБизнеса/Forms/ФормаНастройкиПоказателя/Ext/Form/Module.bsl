...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Остаток") Тогда
		...
		ДляУдаления = НастройкиПоказателей.НайтиСтроки(Новый Структура("Остаток", НЕ Параметры.Остаток));
		
		Для каждого СтрокаНастроек Из ДляУдаления Цикл
			НастройкиПоказателей.Удалить(СтрокаНастроек);
		КонецЦикла;
		
	КонецЕсли;
	
	Параметры.Свойство("Цвет", ВыбранныйЦвет);
	Если НЕ ЗначениеЗаполнено(ВыбранныйЦвет) Тогда
		ВыбранныйЦвет  = "Цвет1"; // Не локализуется
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	МассивЦветов = МассивЦветов();
	
	Если ЗначениеЗаполнено(ВыбранныйЦвет) Тогда
		
		Если Сред(ВыбранныйЦвет, 1, 4) = "Цвет" Тогда
			НомерЦвета = Число(Сред(ВыбранныйЦвет, 5, 1)) - 1;
			ИзменитьДанныеПоЦвету(МассивЦветов[НомерЦвета]);
		КонецЕсли;
...
Процедура ЦветНажатие(Элемент)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ОбработчикОповещенияЦветНажатие", ЭтотОбъект);
	
	ПараметрыОткрытияФормы = Новый Структура;
	ПараметрыОткрытияФормы.Вставить("МассивЦветов", МассивЦветов());
...
Процедура СохранитьНастройку(Команда)
	...
	Результат.Вставить("Цвет", ВыбранныйЦвет);
...
&НаСервере
Процедура ИзменитьДанныеПоЦвету(Знач НовыйЦвет)
	
	МассивЦветов = МассивЦветов();
	
	НомерЦвета = 1;
	Для Каждого ПредопределенныйЦвет Из МассивЦветов Цикл
		
		Цвет = ПредопределенныйЦвет;
		
		Если Цвет = НовыйЦвет Тогда
			ВыбранныйЦвет = "Цвет" + НомерЦвета;
			Прервать;
		КонецЕсли;
	...
	Элементы.ЦветВыбора.ЦветТекста = Цвет;
	Элементы.ЦветВыбора.ЦветФона = Цвет;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция МассивЦветов()
	
	Цвета = Новый Массив;
	
	Цвета.Добавить(Новый Цвет(255, 248, 191));
	Цвета.Добавить(Новый Цвет(209, 234, 209));
	Цвета.Добавить(Новый Цвет(255, 228, 191));
	Цвета.Добавить(Новый Цвет(215, 207, 235));
	Цвета.Добавить(Новый Цвет(202, 226, 250));
	Цвета.Добавить(Новый Цвет(250, 203, 202)); 
...
Процедура ОбработчикОповещенияЦветНажатие(НовыйЦвет, ДополнительныеПараметры) Экспорт
	...
	Если НовыйЦвет <> Неопределено И НовыйЦвет <> КодВозвратаДиалога.Отмена Тогда
		ИзменитьДанныеПоЦвету(НовыйЦвет);
	КонецЕсли;
