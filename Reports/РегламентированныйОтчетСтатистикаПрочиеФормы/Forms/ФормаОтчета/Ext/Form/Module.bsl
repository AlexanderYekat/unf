...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнформацияЗагруженныхСправочников = РегламентированнаяОтчетность.ИнформацияЗагруженныхСправочников(СтруктураИнформацияСправочников["ИсточникОтчета"]);
...
Процедура ПодготовитьДанныеОтчетаПоШаблону(ДанныеШаблонаОтчета, Отказ = Ложь, ЗагружатьСписки = Истина);
	...
	Если ЗагружатьСписки Тогда 
		...
		Для каждого УзелСправочник Из УзлыСправочники Цикл
			...
			Если ДочерниеУзлыКоличество > МаксимумДляЗагрузки Или ДочерниеУзлыКоличество = 0 Тогда 
...
Функция НачатьВыполнениеЗагрузкиНаСервере()
	Попытка
		...
		Результат = ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения,
			"РегламентированнаяОтчетность.ЗагрузитьСправочникиШаблонаВРегистр",
