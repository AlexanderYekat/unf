...
Процедура ДеревоПлатежейПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущиеДанные.ТипАналитики = ПредопределенноеЗначение("Перечисление.АналитикаПлатежногоКалендаря.Документ") Тогда
			...
			Если РазрешеноМенятьСтатусУтвержденияПлатежа Тогда
		...
		// Для платежных поручений отключаем группы контекстного меню
		Если ТипЗнч(ТекущиеДанные.Аналитика) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
			Элементы.ДеревоПлатежейКонтекстноеМенюГруппаИзменениеТекущегоДокумента.Доступность = Ложь;
			Элементы.ДеревоПлатежейКонтекстноеМенюГруппаСоздание.Доступность = Ложь;
			Элементы.ДеревоПлатежейКонтекстноеМенюГруппаУтверждение.Доступность = Ложь;
		КонецЕсли;
...
Процедура ДеревоПлатежейПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущийЭлемент.Имя = "ДеревоПлатежей" + "Аналитика" И Элемент.ТекущиеДанные.ТипАналитики = ПредопределенноеЗначение("Перечисление.АналитикаПлатежногоКалендаря.Документ") Тогда //Переход к колонке с суммой
		Для каждого Период Из ПериодыПланирования Цикл
			...
			Если Элемент.ТекущиеДанные.Свойство(ИмяКолонки) И Элемент.ТекущиеДанные[ИмяКолонки] <> 0 Тогда
	...
	Иначе
		...
		Если Элементы.ДеревоПлатежейКонтекстноеМенюОткрытьОбъект.Доступность Тогда
			СписокКоманд.Добавить("ОткрытьОбъект", НСтр("ru = 'Открыть'"), , БиблиотекаКартинок.ПолеВводаОткрыть);
		КонецЕсли;
		
		Если Элементы.ДеревоПлатежейКонтекстноеМенюГруппаИзменениеТекущегоДокумента.Доступность Тогда
			СписокКоманд.Добавить("ИзменитьДату", НСтр("ru = 'Изменить дату'"), ,
				БиблиотекаКартинок.ПлатежныйКалендарьИзменитьДату);
			СписокКоманд.Добавить("ИзменитьСчетКассу", НСтр("ru = 'Изменить счет / кассу'"), ,
				БиблиотекаКартинок.Изменить);
			СписокКоманд.Добавить("Отменить", НСтр("ru = 'Отменить'"), , БиблиотекаКартинок.ПлатежныйКалендарьОтменить);
			СписокКоманд.Добавить("Скопировать", НСтр("ru = 'Скопировать'"), ,
				БиблиотекаКартинок.СкопироватьЭлементСписка);
		КонецЕсли;
		
		СписокКоманд.Добавить("СоздатьПлатежноеПоручение", НСтр("ru = 'Платежное поручение'"), ,
			БиблиотекаКартинок.СоздатьЭлементСписка);
		СписокКоманд.Добавить("ДобавитьРасход", НСтр("ru = 'Заявка на расход денег'"), ,
			БиблиотекаКартинок.СоздатьЭлементСписка);
		СписокКоманд.Добавить("ДобавитьПриход", НСтр("ru = 'Поступление денег (план)'"), ,
			БиблиотекаКартинок.СоздатьЭлементСписка);
		СписокКоманд.Добавить("ДобавитьПеремещение", НСтр("ru = 'Перемещение денег (план)'"), ,
			БиблиотекаКартинок.СоздатьЭлементСписка);
		...
		Если Элементы.ДеревоПлатежейКонтекстноеМенюГруппаУтверждение.Доступность Тогда
			Если Элементы.ДеревоПлатежейКонтекстноеМенюУтвердить.Доступность Тогда
				СписокКоманд.Добавить("Утвердить", НСтр("ru = 'Утвердить'"), ,
					БиблиотекаКартинок.ПлатежныйКалендарьУтвердить);
			КонецЕсли;
			Если Элементы.ДеревоПлатежейКонтекстноеМенюОтменитьУтверждение.Доступность Тогда
				СписокКоманд.Добавить("ОтменитьУтверждение", НСтр("ru = 'Отклонить'"), ,
					БиблиотекаКартинок.ПлатежныйКалендарьОтклонить);
			КонецЕсли;
		КонецЕсли;
		
		ПоказатьВыборИзМеню(Новый ОписаниеОповещения("ОкончаниеВыбораИзСпискаКонтекстногоМеню", ЭтаФорма),
			СписокКоманд, Элемент);
...
&НаКлиенте
Процедура СоздатьЗакрытиеПК(Команда)
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаполнитьПриСоздании", Истина);
	ОткрытьФорму("Документ.ЗакрытиеПланируемыхОплат.Форма.ФормаДокумента", ПараметрыФормы);
КонецПроцедуры
...
Процедура ИнициализироватьРеквизитыФормы()
	...
	// Наличие роли для изменения статуса оплаты
	РазрешеноМенятьСтатусУтвержденияПлатежа = ФормыДокументовДеньги.РазрешеноМенятьСтатусОплаты();
	...
	Элементы.ПредставлениеВариантаПересчета.СписокВыбора.Добавить(
		Перечисления.ПлатежныйКалендарьВариантыПересчета.ВалютаУправленческогоУчета, СтрШаблон(НСтр(
		"ru = 'В валюте учета - %1'"), ВалютаУчета));
	Элементы.ПредставлениеВариантаПересчета.СписокВыбора.Добавить(
		Перечисления.ПлатежныйКалендарьВариантыПересчета.ВалютаНациональная, СтрШаблон(НСтр(
		"ru = 'В национальной валюте - %1'"), ВалютаНациональная));
	Элементы.ПредставлениеВариантаПересчета.СписокВыбора.Добавить(
		Перечисления.ПлатежныйКалендарьВариантыПересчета.ВалютаПлатежа, НСтр("ru = 'В валюте платежа'"));
...
Функция ДобавитьЭлементДерева(Выборка, ТипАналитики, Родитель, ПараметрыГруппировок)
	...
	ДвиженияОтсутствуют = Выборка.СуммаОборот = 0 И Выборка.Приход = 0;
	
	Если ТипАналитики = Перечисления.АналитикаПлатежногоКалендаря.БанковскийСчетКасса И Выборка.СуммаОстаток = 0
		И ДвиженияОтсутствуют Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если ТипАналитики = Перечисления.АналитикаПлатежногоКалендаря.Статья И ДвиженияОтсутствуют Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если ТипАналитики = Перечисления.АналитикаПлатежногоКалендаря.Контрагент И ДвиженияОтсутствуют Тогда
		Возврат Неопределено;
	КонецЕсли;
			...
			Если СтрокаРодитель.ТипАналитики = Перечисления.АналитикаПлатежногоКалендаря.БанковскийСчетКасса Тогда
				...
				Если ИндексНачалаПересчета <> -1 Тогда
					...
					Для Итератор = ИндексНачалаПересчета По ПериодыПланирования.Количество() - 1 Цикл
						...
						Сумма = СтрокаРодитель[ИмяТекущейКолонки];
						
						Если Объект.ВидДиаграммы = Перечисления.ВариантыДиаграммыПлатежныйКалендарь.Сводная Тогда
	
							АналитикаСерии = НСтр("ru = 'Сводно (по всем счетам / кассам)'");
							ТекущаяСерия = ДиаграммаПлатежногоКалендаря.УстановитьСерию(АналитикаСерии);
							ТекущаяТочка = ДиаграммаПлатежногоКалендаря.УстановитьТочку(ТекущийПериод);
							
							ЗначениеДиаграммы = ДиаграммаПлатежногоКалендаря.ПолучитьЗначение(ТекущаяТочка, ТекущаяСерия).Значение;
							Сумма = ?(ЗначениеДиаграммы = Неопределено, 0, ЗначениеДиаграммы) + СуммаОборот;
							
						КонецЕсли;
						
						ОбновитьДиаграммуПриИзмененииДерева(ДиаграммаПлатежногоКалендаря, Объект.ВидДиаграммы, ТекущийПериод,
							СтрокаРодитель.Аналитика, Сумма);
...
Функция ПолучитьДанныеЯчейки(ИдентификаторСтроки = Неопределено)
	...
	РеквизитыАналитики.Вставить(ПредопределенноеЗначение(
		"Перечисление.АналитикаПлатежногоКалендаря.БанковскийСчетКасса"), "БанковскийСчетКасса");
	РеквизитыАналитики.Вставить(ПредопределенноеЗначение(
		"Перечисление.АналитикаПлатежногоКалендаря.Документ"), "Документ");
	РеквизитыАналитики.Вставить(ПредопределенноеЗначение(
		"Перечисление.АналитикаПлатежногоКалендаря.Организация"), "Организация");
	РеквизитыАналитики.Вставить(ПредопределенноеЗначение(
		"Перечисление.АналитикаПлатежногоКалендаря.Статья"), "Статья");
	РеквизитыАналитики.Вставить(ПредопределенноеЗначение(
		"Перечисление.АналитикаПлатежногоКалендаря.Контрагент"), "Контрагент");
	РеквизитыАналитики.Вставить(ПредопределенноеЗначение(
		"Перечисление.АналитикаПлатежногоКалендаря.ТипДенежныхСредств"), "ТипДенежныхСредств");
	
	ДанныеЯчейки = Новый Структура;
	ДанныеЯчейки.Вставить("Организация");
	ДанныеЯчейки.Вставить("ТипДенежныхСредств");
	ДанныеЯчейки.Вставить("БанковскийСчетКасса");
	ДанныеЯчейки.Вставить("Статья");
	ДанныеЯчейки.Вставить("Контрагент");
	ДанныеЯчейки.Вставить("Документ");
	ДанныеЯчейки.Вставить("Валюта");
	ДанныеЯчейки.Вставить("СуммыВСтроке");
	ДанныеЯчейки.Вставить("ИдентификаторСтроки");
	...
	Для каждого Период Из ПериодыПланирования Цикл
		...
		Если ТекущиеДанные.Свойство(ИмяКолонки) И ЗначениеЗаполнено(ТекущиеДанные[ИмяКолонки]) Тогда
	...
	Если Элементы.ДеревоПлатежей.ТекущийЭлемент <> Неопределено И ИдентификаторСтроки = Неопределено Тогда
		...
		Если ИмяКолонки = "ДеревоПлатежейАналитика" ИЛИ ИмяКолонки = "ДеревоПлатежей_ПустаяКолонка" Тогда
			Если СуммыВСтроке.Количество() > 0 Тогда
				ДанныеЯчейки.Вставить("Период", СуммыВСтроке[0].Период);
				ДанныеЯчейки.Вставить("Сумма",  СуммыВСтроке[0].Сумма);
			Иначе
...
Процедура ПереместитьСуммуПоСтроке(СтараяДата, НоваяДата, СтрокаДерева, Сумма)
	...
	Если ПериодыПланирования.НайтиПоЗначению(ДатаДляНовойКолонки) <> Неопределено Тогда
		...
		ИмяЭлемента = СтрШаблон("ДеревоПлатежей%1", КолонкаНовогоПериода);
		Элементы.ДеревоПлатежей.ТекущийЭлемент = Элементы[ИмяЭлемента];
	КонецЕсли;
...
Процедура ПередУдалениемСтрокиДерева(СтрокаДерева)
	...
	ПараметрыОтбора = Новый Структура("Аналитика,ИдентификаторСтрокиДляОтбора", СтрокаДерева.Аналитика,
		СтрокаДерева.ИдентификаторСтрокиДляОтбора);
	СтрокиПодробнойИнформации = ПодробнаяИнформацияТекущейАналитики.НайтиСтроки(ПараметрыОтбора);
...
Функция ИзменитьДанныеПланированияНаСервере(ДокументПланирования, ИзмененныеРеквизиты, ПланироватьПоДням)
	...
	ИмяСобытия = НСтр("ru = 'ПлатежныйКалендарь.ИзменитьДанныеПланирования'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИзменитьСтатусУтвержденияДокумента(СсылкаНаДокумент, СтатусУтверждения)
	...
	Если СтатусУтверждения <> СсылкаНаДокумент.СтатусУтвержденияПлатежа Тогда
		...
		Исключение
			ТекстСообщения = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ИмяСобытия = НСтр("ru = 'ПлатежныйКалендарь.ИзменитьСтатусУтвержденияДокумента'",
...
Функция ОтменитьПланирование(СсылкаНаДокумент)
	...
	Попытка
		ДокументОбъект = Документы.ЗакрытиеПланируемыхОплат.СоздатьДокумент();
