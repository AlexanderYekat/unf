...
Процедура СохранитьНастройкиОтборов(Объект, ДанныеМеток) Экспорт
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(ИмяКлючаОбъекта, ИмяКлючаОбъекта+"_ВыводитьОтклонение",     Объект.ВыводитьОтклонение);
...
Процедура ВосстановитьНастройкиОтборов(Объект, ДанныеМеток) Экспорт
	...
	СохраненноеЗначение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(ИмяКлючаОбъекта, ИмяКлючаОбъекта+"_ВыводитьОтклонение");
	Если СохраненноеЗначение <> Неопределено Тогда
		Объект.ВыводитьОтклонение = СохраненноеЗначение;
	Иначе
		Объект.ВыводитьОтклонение = Ложь;
	КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат, ДанныеОтчета) Экспорт
	...
	Если ДанныеОтчета.ВидОтчетаОтбор = Перечисления.ВидыФинансовыхОтчетов.Баланс Тогда
		Области.ШапкаИтого.Параметры.Заголовок = НСтр("ru = 'НА НАЧАЛО'");
	Иначе
		Области.ШапкаИтого.Параметры.Заголовок = НСтр("ru = 'ИТОГО'");
	КонецЕсли;
	
	ДокументРезультат.Присоединить(Области.ШапкаИтого);
	
	Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт И ДанныеОтчета.ВыводитьОтклонение Тогда
		ДокументРезультат.Присоединить(Области.ШапкаОтклонение);
	КонецЕсли;
	...
	Для каждого Период Из ДанныеОтчета.ПериодыПланирования Цикл
		...
		Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт И ДанныеОтчета.ВыводитьОтклонение 
			И ЭтоАктивныйПериод(Период, ДанныеОтчета.АктивныйПериод, ДанныеОтчета.Периодичность) Тогда
			ДокументРезультат.Присоединить(Области.ШапкаОтклонениеАктивныйПериод);
		КонецЕсли;
		
	КонецЦикла;
	
	Если ДанныеОтчета.Свойство("ГруппировкаАнализаБизнеса")
		И НЕ ЗначениеЗаполнено(ДанныеОтчета.ГруппировкаАнализаБизнеса) Тогда
		
		ДанныеОтчета.ГруппировкаАнализаБизнеса = Перечисления.ГруппировкаАнализаБизнеса.Показатели;
	...
	Пока ВыборкаСуммПоказателей.Следующий() Цикл
			...
			СтрокаДерева = ПолучитьСтрокуДереваПоПоказателю(ДанныеОтчета.ВидОтчетаОтбор, ВыборкаСуммПоказателей.Показатель, ДанныеОтчета);
	...
	Для каждого ИдентификаторПоказателя Из МассивПоказателейДляПересчета Цикл
		...
		Если ЗависимыеЭлементы.Количество() > 0 Тогда
			Для каждого Период Из ДанныеОтчета.ПериодыПланирования Цикл
				...
				ОбновитьЗависимыеЭлементыПоФормуле(ДанныеОтчета.ВидОтчетаОтбор, ИдентификаторПоказателя, ИмяКолонки, ДанныеОтчета);
			...
			Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт Тогда
				ОбновитьЗависимыеЭлементыПоФормуле(ДанныеОтчета.ВидОтчетаОтбор, ИдентификаторПоказателя, "ИтогоПлан", ДанныеОтчета);
				ОбновитьЗависимыеЭлементыПоФормуле(ДанныеОтчета.ВидОтчетаОтбор, ИдентификаторПоказателя, "ИтогоФакт", ДанныеОтчета);
			Иначе
				ОбновитьЗависимыеЭлементыПоФормуле(ДанныеОтчета.ВидОтчетаОтбор, ИдентификаторПоказателя, ИмяКолонкиИтог, ДанныеОтчета);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Если НЕ ДанныеОтчета.ПоказыватьПустыеСтроки Тогда
		УдалитьПустыеСтрокиДереваРекурсивно(ДанныеОтчета.ДеревоПоказателейБаланс, ДанныеОтчета);
	КонецЕсли;
	
	ДокументРезультат.НачатьАвтоГруппировкуСтрок();
	ВывестиДеревоРекурсивно(ДокументРезультат, Области, ДанныеОтчета, ДанныеОтчета.ДеревоПоказателейБаланс.Строки, 0);
...
Процедура ВывестиОтчет(ДанныеОтчета, ДокументРезультат, Области)
	...
	ТаблицаПодключаемыхПоказателей = ПоказателиБизнеса.ПолучитьТаблицуПодключаемыхПоказателей(ДанныеОтчета);
	...
	Если ДанныеОтчета.ГруппировкаАнализаБизнеса <> Перечисления.ГруппировкаАнализаБизнеса.Показатели Тогда
		...
		Пока ВыборкаАналитика.Следующий() Цикл
			...
			ЗаполнитьДеревоПоказателей(ДанныеОтчета, ВыборкаАналитика.Выбрать(), ТаблицаПодключаемыхПоказателей, НоваяСтрока.Строки, ВыборкаАналитика.Аналитика);
		КонецЦикла;
		
	Иначе
		ЗаполнитьДеревоПоказателей(ДанныеОтчета, Запрос.Выполнить().Выбрать(), ТаблицаПодключаемыхПоказателей, ИтоговоеДерево.Строки);
	КонецЕсли;
...
Процедура ЗаполнитьДеревоПоказателей(ДанныеОтчета, Выборка, ТаблицаПодключаемыхПоказателей, ИтоговоеДерево, Аналитика = Неопределено)
...
Функция ИнициализироватьОбластиОтчета(ДанныеОтчета)
	...
	Области.Вставить("ШапкаОтклонение",  					Макет.ПолучитьОбласть("ШапкаОтчета|Отклонение"));
	Области.Вставить("ШапкаОтклонениеАктивныйПериод",  		Макет.ПолучитьОбласть("ШапкаОтчета|ОтклонениеАктивныйПериод"));
	Области.Вставить("ДоходОтклонениеАбсолютное",  			Макет.ПолучитьОбласть("Доход|ОтклонениеАбсолютное"));
	Области.Вставить("РасходОтклонениеАбсолютное",  		Макет.ПолучитьОбласть("Расход|ОтклонениеАбсолютное"));
	Области.Вставить("ГруппаОтклонениеАбсолютное",  		Макет.ПолучитьОбласть("Группа|ОтклонениеАбсолютное"));
	Области.Вставить("ФормулаОтклонениеАбсолютное",  		Макет.ПолучитьОбласть("Формула|ОтклонениеАбсолютное"));
	Области.Вставить("АналитикаОтклонениеАбсолютное",  		Макет.ПолучитьОбласть("Аналитика|ОтклонениеАбсолютное"));
	Области.Вставить("ДоходОтклонениеОтносительное",  		Макет.ПолучитьОбласть("Доход|ОтклонениеОтносительное"));
	Области.Вставить("РасходОтклонениеОтносительное",  		Макет.ПолучитьОбласть("Расход|ОтклонениеОтносительное"));
	Области.Вставить("ГруппаОтклонениеОтносительное",  		Макет.ПолучитьОбласть("Группа|ОтклонениеОтносительное"));
	Области.Вставить("ФормулаОтклонениеОтносительное",  	Макет.ПолучитьОбласть("Формула|ОтклонениеОтносительное"));
	Области.Вставить("АналитикаОтклонениеОтносительное",  	Макет.ПолучитьОбласть("Аналитика|ОтклонениеОтносительное"));
...
Процедура ЗаполнитьСуммыВСтрокеДерева(СтрокаДерева, ВыборкаСуммПоказателей, ДанныеОтчета)
	...
	Если ЭтоПланФакт Тогда
		Если ВыборкаСуммПоказателей.Период < ДанныеОтчета.АктивныйПериод Тогда // Факт
			СтрокаДерева[ИмяКолонки] =ВыборкаСуммПоказателей.СуммаФакт;
		ИначеЕсли ВыборкаСуммПоказателей.Период > ДанныеОтчета.АктивныйПериод Тогда // План
			СтрокаДерева[ИмяКолонки] = ВыборкаСуммПоказателей.СуммаПлан;
		Иначе // План-Факт
			СтрокаДерева[ИмяКолонки] = ВыборкаСуммПоказателей.СуммаФакт;
			СтрокаДерева[ИмяКолонки+"План"] = ВыборкаСуммПоказателей.СуммаПлан;
		КонецЕсли;
	Иначе
		...
		Если Период > ВыборкаСуммПоказателей.Период Тогда
			...
			Если ЭтоПланФакт Тогда
				Если Период < ДанныеОтчета.АктивныйПериод Тогда // Факт
					СтрокаДерева[ИмяКолонкиБудущегоПериода] = ВыборкаСуммПоказателей.СуммаФакт;
				ИначеЕсли Период > ДанныеОтчета.АктивныйПериод Тогда // План
					СтрокаДерева[ИмяКолонкиБудущегоПериода] = ВыборкаСуммПоказателей.СуммаПлан;
				Иначе // План-Факт
					СтрокаДерева[ИмяКолонкиБудущегоПериода] = ВыборкаСуммПоказателей.СуммаФакт;
					СтрокаДерева[ИмяКолонкиБудущегоПериода+"План"] = ВыборкаСуммПоказателей.СуммаПлан;
				КонецЕсли;
			Иначе
...
Процедура ВывестиДеревоРекурсивно(ДокументРезультат, Области, ДанныеОтчета, Строки, Уровень)
	...
	Для каждого ТекущаяСтрока Из Строки Цикл
		...
		Если ТекущаяСтрока.ТипПоказателя = Перечисления.ТипыПоказателейБизнеса.Доход Тогда
			...
			ОтклонениеАбсолютное = Области.ДоходОтклонениеАбсолютное;
			ОтклонениеОтносительное = Области.ДоходОтклонениеОтносительное;
		...
		ИначеЕсли ТекущаяСтрока.ТипПоказателя = Перечисления.ТипыПоказателейБизнеса.Расход Тогда
			...
			ОтклонениеАбсолютное = Области.РасходОтклонениеАбсолютное;
			ОтклонениеОтносительное = Области.РасходОтклонениеОтносительное;
		...
		ИначеЕсли ТекущаяСтрока.ТипПоказателя = Перечисления.ТипыПоказателейБизнеса.Группа Тогда
			...
			ОтклонениеАбсолютное = Области.ГруппаОтклонениеАбсолютное;
			ОтклонениеОтносительное = Области.ГруппаОтклонениеОтносительное;
		...
		ИначеЕсли ТекущаяСтрока.ТипПоказателя = Перечисления.ТипыПоказателейБизнеса.Формула Тогда
			...
			ОтклонениеАбсолютное = Области.ФормулаОтклонениеАбсолютное;
			ОтклонениеОтносительное = Области.ФормулаОтклонениеОтносительное;
		...
		Иначе // Аналитика
			...
			ОтклонениеАбсолютное = Области.АналитикаОтклонениеАбсолютное;
			ОтклонениеОтносительное = Области.АналитикаОтклонениеОтносительное;
		...
		Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт Тогда
			...
			Если ДанныеОтчета.ВыводитьОтклонение Тогда
				Отклонения = РассчитатьОтклонения(ТекущаяСтрока.ИтогоПлан, ТекущаяСтрока.ИтогоФакт);
				ОтклонениеАбсолютное.Параметры.Сумма = Отклонения.Абсолютное;
				ОтклонениеОтносительное.Параметры.Сумма = Отклонения.Относительное;
				ДокументРезультат.Присоединить(ОтклонениеАбсолютное);
				ДокументРезультат.Присоединить(ОтклонениеОтносительное);
			КонецЕсли;
			...
			Если ЭтоАктивныйПериод(Период, ДанныеОтчета.АктивныйПериод, ДанныеОтчета.Периодичность) Тогда
				...
				Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт Тогда
					...
					Если ДанныеОтчета.ВыводитьОтклонение Тогда
						...
						Отклонения = РассчитатьОтклонения(ТекущаяСтрока[ИмяКолонки+"План"], ТекущаяСтрока[ИмяКолонки]);
						ОтклонениеАбсолютное.Параметры.Сумма = Отклонения.Абсолютное;
						ОтклонениеОтносительное.Параметры.Сумма = Отклонения.Относительное;
	...
	Для каждого СтрокаДерева Из Дерево.Строки Цикл
		...
		Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт Тогда
			Если СтрокаДерева.Строки.Количество() = 0 И СтрокаДерева.ИтогоПлан = 0 И СтрокаДерева.ИтогоФакт = 0 Тогда
				ЕстьСуммаВПериоде = Ложь;
				Для каждого Период Из ДанныеОтчета.ПериодыПланирования Цикл
					ИмяКолонки = СформироватьИмяКолонкиПоПериоду(Период);
					Если СтрокаДерева[ИмяКолонки] <> 0 Тогда
						ЕстьСуммаВПериоде = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				Если НЕ ЕстьСуммаВПериоде Тогда
...
Функция РассчитатьОтклонения(СуммаПлан, СуммаФакт)
	
	Отклонения = Новый Структура;
	
	АбсолютноеОтклонение = СуммаПлан - СуммаФакт;
	
	Если СуммаФакт = 0 И СуммаПлан = 0 Тогда
		ОтносительноеОтклонение = 0;
	ИначеЕсли СуммаФакт = 0 Тогда
		ОтносительноеОтклонение = 100;
	ИначеЕсли СуммаПлан = 0 Тогда
		ОтносительноеОтклонение = -100;
	Иначе
		Если АбсолютноеОтклонение > 0 Тогда
			ОтносительноеОтклонение = ((СуммаПлан - АбсолютноеОтклонение) / СуммаПлан) * 100;
		ИначеЕсли АбсолютноеОтклонение < 0 Тогда
			ОтносительноеОтклонение = (АбсолютноеОтклонение * -1 / СуммаПлан) * -100;
		Иначе
			ОтносительноеОтклонение = 0;
		КонецЕсли;
	КонецЕсли;
	
	Отклонения.Вставить("Абсолютное", АбсолютноеОтклонение);
	Отклонения.Вставить("Относительное", ОтносительноеОтклонение);
	
	Возврат Отклонения;
	
КонецФункции

Процедура ДобавитьСтрокуДерева(ИтоговоеДерево, ДанныеСтроки, ДанныеОтчета, МассивПоказателейДляПересчета)
	
	СтрокаДерева = ИтоговоеДерево.Найти(ДанныеСтроки.Показатель, "Показатель", Истина);
	Если СтрокаДерева = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ИмяКолонкиИтог = "Итого";
	ИмяКолонки = СформироватьИмяКолонкиПоПериоду(ДанныеСтроки.Период);
	
	Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт Тогда
		
		Если ДанныеСтроки.Период < ДанныеОтчета.АктивныйПериод Тогда // Факт
			СтрокаДерева[ИмяКолонки] = СтрокаДерева[ИмяКолонки] + ДанныеСтроки.СуммаФакт;
		ИначеЕсли ДанныеСтроки.Период > ДанныеОтчета.АктивныйПериод Тогда // План
			СтрокаДерева[ИмяКолонки] = СтрокаДерева[ИмяКолонки] + ДанныеСтроки.СуммаПлан;
		Иначе // План-Факт
			СтрокаДерева[ИмяКолонки] = СтрокаДерева[ИмяКолонки] + ДанныеСтроки.СуммаФакт;
			СтрокаДерева[ИмяКолонки+"План"] = СтрокаДерева[ИмяКолонки+"План"] + ДанныеСтроки.СуммаПлан;
		КонецЕсли;
		
		СтрокаДерева["ИтогоФакт"] = СтрокаДерева["ИтогоФакт"] + ДанныеСтроки.СуммаФакт;
		СтрокаДерева["ИтогоПлан"] = СтрокаДерева["ИтогоПлан"] + ДанныеСтроки.СуммаПлан;
		
		
		РодительСтроки = СтрокаДерева.Родитель;
		Пока РодительСтроки <> Неопределено Цикл
			Если СтрокаДерева.НеИспользоватьВИтогах = Истина ИЛИ РодительСтроки.НеИспользоватьВИтогах = Истина Тогда
				Прервать;
			КонецЕсли;
			РодительСтроки[ИмяКолонки] = РодительСтроки[ИмяКолонки] + СтрокаДерева[ИмяКолонки];
			Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт И ДанныеСтроки.Период = ДанныеОтчета.АктивныйПериод Тогда
				РодительСтроки[ИмяКолонки+"План"] = РодительСтроки[ИмяКолонки+"План"] + СтрокаДерева[ИмяКолонки+"План"];
			КонецЕсли;
			РодительСтроки["ИтогоФакт"] = РодительСтроки["ИтогоФакт"] + ДанныеСтроки.СуммаФакт;
			РодительСтроки["ИтогоПлан"] = РодительСтроки["ИтогоПлан"] + ДанныеСтроки.СуммаПлан;
			РодительСтроки = РодительСтроки.Родитель;
		КонецЦикла;
		
	Иначе
		
		СтрокаДерева[ИмяКолонки] = СтрокаДерева[ИмяКолонки] + ДанныеСтроки.Сумма;
		СтрокаДерева[ИмяКолонкиИтог] = СтрокаДерева[ИмяКолонкиИтог] + ДанныеСтроки.Сумма;
		...
		Пока РодительСтроки <> Неопределено Цикл
			Если СтрокаДерева.НеИспользоватьВИтогах = Истина ИЛИ РодительСтроки.НеИспользоватьВИтогах = Истина Тогда
				Прервать;
			КонецЕсли;
			РодительСтроки[ИмяКолонки] = РодительСтроки[ИмяКолонки] + ДанныеСтроки.Сумма;
			РодительСтроки[ИмяКолонкиИтог] = РодительСтроки[ИмяКолонкиИтог] + ДанныеСтроки.Сумма;
			РодительСтроки = РодительСтроки.Родитель;
		КонецЦикла;
		
	КонецЕсли;
	
	МассивПоказателейДляПересчета.Добавить(ДанныеСтроки.ИдентификаторПоказателя);
	
КонецПроцедуры

Процедура РассчитатьФормулу(ДанныеОтчета, ИтоговоеДерево, СтрокаФормулы, ИдентификаторыПоказателейДляФормул)
	...
	Для каждого Период Из ДанныеОтчета.ПериодыПланирования Цикл
		...
		ИмяКолонки = СформироватьИмяКолонкиПоПериоду(Период);
		РассчитатьЗначениеКолонкиПоФормуле(СтрокаФормулы, ИмяКолонки, Идентификаторы, ИдентификаторыПоказателейДляФормул);
		
		Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт И Период = ДанныеОтчета.АктивныйПериод Тогда
			РассчитатьЗначениеКолонкиПоФормуле(СтрокаФормулы, ИмяКолонки+"План", Идентификаторы, ИдентификаторыПоказателейДляФормул);
		КонецЕсли;
		
	КонецЦикла;
	
	Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт Тогда
		РассчитатьЗначениеКолонкиПоФормуле(СтрокаФормулы, "ИтогоПлан", Идентификаторы, ИдентификаторыПоказателейДляФормул);
		РассчитатьЗначениеКолонкиПоФормуле(СтрокаФормулы, "ИтогоФакт", Идентификаторы, ИдентификаторыПоказателейДляФормул);
	Иначе
		РассчитатьЗначениеКолонкиПоФормуле(СтрокаФормулы, "Итого", Идентификаторы, ИдентификаторыПоказателейДляФормул);
	КонецЕсли;
	
	ТекущаяСтрока = СтрокаФормулы;
	Пока ТекущаяСтрока.Родитель <> Неопределено Цикл
		Если ТекущаяСтрока.НеИспользоватьВИтогах = Истина Тогда
			Прервать;
		КонецЕсли;
		
		Для каждого Период Из ДанныеОтчета.ПериодыПланирования Цикл
			ИмяКолонки = СформироватьИмяКолонкиПоПериоду(Период);
			ТекущаяСтрока.Родитель[ИмяКолонки] = ТекущаяСтрока.Родитель[ИмяКолонки] + СтрокаФормулы[ИмяКолонки];
			Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт И Период = ДанныеОтчета.АктивныйПериод Тогда
				ТекущаяСтрока.Родитель[ИмяКолонки+"План"] = ТекущаяСтрока.Родитель[ИмяКолонки+"План"] + СтрокаФормулы[ИмяКолонки+"План"];
			КонецЕсли;
		КонецЦикла;
		Если ДанныеОтчета.ПланФакт = Перечисления.ПланФакт.ПланФакт Тогда
			ТекущаяСтрока.Родитель["ИтогоПлан"] = ТекущаяСтрока.Родитель["ИтогоПлан"] + СтрокаФормулы["ИтогоПлан"];
			ТекущаяСтрока.Родитель["ИтогоФакт"] = ТекущаяСтрока.Родитель["ИтогоФакт"] + СтрокаФормулы["ИтогоФакт"];
		Иначе
			ТекущаяСтрока.Родитель["Итого"] = ТекущаяСтрока.Родитель["Итого"] + СтрокаФормулы["Итого"];
		КонецЕсли;
...
Процедура РассчитатьЗначениеКолонкиПоФормуле(СтрокаФормулы, ИмяКолонки, Идентификаторы, ИдентификаторыПоказателейДляФормул)
