...
//   СправочникСсылка.КассыККМ   - Искомая по регистрационному номеру касса
//
...
Функция РозничныйВидЦены(КассовыйАппарат = Неопределено) Экспорт
	...
	Если КассовыйАппарат <> Неопределено Тогда
		...
		// Определение вида цены прайс листа
		Запрос = Новый Запрос(
		"ВЫБРАТЬ ПЕРВЫЕ 1
...
Функция ОтменаРегистрацииКассыНаСервереТарификации(ИдентификаторЛицензии, УдалениеОбъекта = Истина,
	...
	ТарифноеОграничение = ТарификацияУНФ.ОграничениеКоличествоКассПоТарифу();
...
Функция ПолучитьПроксиМенеджераСервиса()
	...
	Определение = Новый WSОпределения(Адрес, Логин, Пароль, ИнтернетПрокси, 20);
	Прокси = Новый WSПрокси(Определение, "http://www.1c.ru/1cFresh/PrivateAPI/1.0", "PrivateAPI", "PrivateAPISoap", ИнтернетПрокси, 20);
