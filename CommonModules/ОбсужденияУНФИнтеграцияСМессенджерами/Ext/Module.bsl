...
Процедура ДобавитьНеотвеченныеСообщенияВКонтактЦентр() Экспорт
	...
	Для каждого Сообщение Из НовыеСообщения Цикл
		...
		Для каждого Участник Из УчастникиОбсуждения Цикл
			СотрудникПользователя = ОбсужденияУНФ.СотрудникПользователя(Участник);
			Если ЗначениеЗаполнено(СотрудникПользователя) Тогда
				СотрудникиОбсуждения.Добавить(СотрудникПользователя);
			КонецЕсли;
		КонецЦикла;
...
Функция ПолучитьИнтеграцию(ИдентификаторИнтеграции)
	...
	// АПК:488-выкл Обход платформенной проверки конфигурации: метод доступен, начиная с версии 8.3.17.
