...
Процедура ОбработатьЗаявки(МенеджерОбмена, ОбъектВсеИзменения, РезультатОбработки, ПовторнаяЗагрузка)
	...
	ТипОбъектаИмяВОбъектеВсеИзменения = МенеджерОбмена.МенеджерКонвертации.ТипОбъектаИмяВОбъектеВсеИзменения;
	ОбрабатываемыеЗаявки 	= ОбрабатываемыеЗаявки();
	ТипОбъектаТипЗаявки 	= ТипОбъектаТипЗаявки();
	...
	Для каждого ТипОбъекта Из ОбрабатываемыеЗаявки Цикл
		ТипЗаявки = ТипОбъектаТипЗаявки[ТипОбъекта];
...
Функция ДанныеФизическихЛицДляВыгрузки(Приложение, ФизическиеЛица, ПроверятьОшибкиЗаполнения) Экспорт
	...
	ИменаСвойствСклоненияФИО = ИменаСвойствСклонения();
...
Функция ДанныеДолжностей(ТаблицаИзменений)
	...
	ПараметрыСклонения = СклонениеПредставленийОбъектовКлиентСервер.ПараметрыСклонения();
	ИменаСвойствСклонения = ИменаСвойствСклонения();
	
	Для каждого СтрокаТЗ Из Данные Цикл
		НоваяСтрока = ТаблицаДанных.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЗ);
		Склонения = Новый Структура(ИменаСвойствСклонения);
		ЗаполнитьЗначенияСвойств(Склонения, СклонениеПредставленийОбъектов.ДанныеСклонения(СтрокаТЗ.Наименование, ПараметрыСклонения));
		НоваяСтрока.Склонения = Склонения;
	КонецЦикла;
...
Функция ТипОбъектаТипЗаявки()

	ТипОбъектаТипЗаявки = Новый Соответствие;
	ПеречислениеТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом;
	ПеречислениеТипЗаявки = Перечисления.ТипыЗаявокКабинетСотрудника;
	
	ТипОбъектаТипЗаявки.Вставить(ПеречислениеТипОбъекта.ЗапросСправки2НДФЛ, 			ПеречислениеТипЗаявки.СправкаНДФЛ);
	ТипОбъектаТипЗаявки.Вставить(ПеречислениеТипОбъекта.ЗаявкаНаКомпенсациюОтпуска, 	ПеречислениеТипЗаявки.ЗаявлениеНаКомпенсациюОтпуска);
	ТипОбъектаТипЗаявки.Вставить(ПеречислениеТипОбъекта.ЗаявкаИзменениеЛичныхДанных, 	ПеречислениеТипЗаявки.ИзменениеЛичныхДанных);
	ТипОбъектаТипЗаявки.Вставить(ПеречислениеТипОбъекта.ЗаявкаДСВ, 						ПеречислениеТипЗаявки.ЗаявлениеНаУдержаниеДСВ);
	ТипОбъектаТипЗаявки.Вставить(ПеречислениеТипОбъекта.ЗапросСправкиСМестаРаботы, 		ПеречислениеТипЗаявки.СправкаСМестаРаботы);
	ТипОбъектаТипЗаявки.Вставить(ПеречислениеТипОбъекта.ЗаявкаНаОтпуск, 				ПеречислениеТипЗаявки.ЗаявлениеНаОтпуск);
	ТипОбъектаТипЗаявки.Вставить(ПеречислениеТипОбъекта.ЗаявкаОстаткиОтпусков, 			ПеречислениеТипЗаявки.СправкаОбОстаткеОтпуска);
	ТипОбъектаТипЗаявки.Вставить(ПеречислениеТипОбъекта.ЗаявкаНалоговыйВычет, 			ПеречислениеТипЗаявки.ЗаявлениеНаНалоговыеВычеты);
	ТипОбъектаТипЗаявки.Вставить(ПеречислениеТипОбъекта.Отсутствие, 					ПеречислениеТипЗаявки.СогласованиеОтсутствия);
	ТипОбъектаТипЗаявки.Вставить(ПеречислениеТипОбъекта.ОбращениеСотрудника, 			ПеречислениеТипЗаявки.ОбращениеСотрудника);
	
	Возврат ТипОбъектаТипЗаявки;

КонецФункции

Функция ОбрабатываемыеЗаявки()
