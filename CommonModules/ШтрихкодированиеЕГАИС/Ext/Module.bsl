...
Процедура ДополнитьДанныеПоШтрихкодамСпецификой(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	ЗаполнитьТаблицуДанныеКодовМаркировкиОписаниемНоменклатуры(ДанныеПоШтрихкодам.ДанныеКодовМаркировки);
...
Процедура ДобавитьКолонкиТаблицыДанныеКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования = Неопределено, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Если ПараметрыСканирования <> Неопределено
		И ШтрихкодированиеОбщегоНазначенияИС.ТребуетсяЧастичноеВыбытие(ПараметрыСканирования, ДанныеПоШтрихкодам) Тогда
...
Процедура ЗаполнитьДанныеНеизвестнойМаркируемойПродукции(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	ЗаполнитьТаблицуДанныеКодовМаркировкиОписаниемНоменклатуры(СтрокиДанныхСАкцизнымиМарками);
...
Процедура ЗаполнитьТаблицуДанныеКодовМаркировкиОписаниемНоменклатуры(ДанныеКодовМаркировки) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Строка = ДанныеКодовМаркировки[Выборка.ИндексСтроки];
		Строка.Коэффициент = Выборка.КоличествоВПотребительскойУпаковке;
		Если ЧастичноеВыбытиеИспользуется Тогда
			Строка.ЧастичноеВыбытиеВариантУчета   = Выборка.ВариантЧастичногоВыбытия;
			Строка.ЕмкостьПотребительскойУпаковки = Выборка.ЕмкостьПотребительскойУпаковки;
			Строка.ЧастичноеВыбытиеНоменклатура   = Выборка.НоменклатураЧастичногоВыбытия;
			Строка.ЧастичноеВыбытиеХарактеристика = Выборка.ХарактеристикаЧастичногоВыбытия;
			Строка.ЧастичноеВыбытие               = Строка.ЧастичноеВыбытиеВыведено > 0;
			Строка.ЧастичноеВыбытиеОстаток        = Макс(Строка.ЕмкостьПотребительскойУпаковки - Строка.ЧастичноеВыбытиеВыведено, 0);
		КонецЕсли;
	КонецЦикла;
