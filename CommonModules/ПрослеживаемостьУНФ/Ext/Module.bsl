...
// Возвращает признак применения Постановления Правительства от 15.12.2023 № 2162
// В котором добавлено, что по определенным группам товаров уведомление о перемещении 
// должно подаваться в срок до пересечения товаром границы РФ с государством - членом ЕАЭС
//
// Параметры
// Дата - тип дата, в данном параметре передается
//        дата на которую необходимо определить признак применения постановления
//
// Возвращаемое значение:
//  Булево - признак применения
//           Истина - постановление применяется
//           Ложь   - постановление не применяется
//
Функция УведомлениеПодаетсяДоМоментаПересеченияГраницы(Дата) Экспорт
	
	Если Дата < '20240118000000' Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли
...
//
// Параметры:
//  Дата - дата, в данном параметре передается дата поступления прослеживаемых товаров
//  ЭтоКорректировочноеУведомление - Булево - истина, если функция вызывается из корректировочного уведомления
//
// Возвращаемое значение:
//  СрокУведомления - дата, день до которого нужно отправить уведомление о ввозе
...
//  ЭтоПрослеживаемяОперация - Булево - Если в документе несколько операций, то сюда нужно передать признак, что
//                                      операция прослеживаемая.
//
// Возвращаемое значение:
...
Функция ТребуетсяПодобратьРНПТ(пТабличнаяЧасть, Знач пДата, Знач пЭтоПрослеживаемяОперация) Экспорт
	...
	Если пЭтоПрослеживаемяОперация Тогда
		ВестиУчетПрослеживаемыхТоваров = ВедетсяУчетПрослеживаемыхТоваров(пДата);
...
Процедура ОбновитьПризнакПрослеживаемости(ТабличнаяЧасть, ДатаДокумента, ПрослеживаемаяОперация = Истина) Экспорт
	...
	Если НЕ ВедетсяУчетПрослеживаемыхТоваров(ДатаДокумента) ИЛИ НЕ ПрослеживаемаяОперация Тогда
...
Процедура ПроверитьЗаполнениеДанныхПрослеживаемости(ДокументОбъект, ДанныеПроверки, Отказ) Экспорт
	...
	ТабличнаяЧасть = ДокументОбъект.Метаданные().ТабличныеЧасти.Найти(ДанныеПроверки.ИмяТЧ);
	ИспользуютсяКомплекты = Ложь;
	Если Не ТабличнаяЧасть = Неопределено Тогда
		ИспользуютсяКомплекты = НЕ (ТабличнаяЧасть.Реквизиты.Найти("ПрослеживаемыйКомплект") = Неопределено);
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ДокументОбъект[ДанныеПроверки.ИмяТЧ] Цикл
		...
		Если СтрокаТаблицы.ПрослеживаемыйТовар Тогда
			...
			Если ИспользуютсяКомплекты И СтрокаТаблицы.ПрослеживаемыйКомплект Тогда
				Продолжить
			КонецЕсли;
...
Процедура СформироватьДвиженияПоступлениеТоваров(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	СтруктураДополнительныеСвойства.ДляПроведения.Вставить("ПриводитьНомерКНомеруНаПечать", Ложь);
...
Процедура ПодготовитьВременнуюТаблицуШапкаПрослеживаемость(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
		...
		|	ВременнаяТаблицаШапка.Ссылка.Организация КАК Организация,
		|	ВременнаяТаблицаШапка.Контрагент КАК Контрагент,
		|	ВременнаяТаблицаШапка.КодОперацииПрослеживаемости КАК КодОперацииПрослеживаемости,
		|	ВременнаяТаблицаШапка.ТоварыПередаютсяВСоставеРабот КАК ТоварыПередаютсяВСоставеРабот,
		|	ВременнаяТаблицаШапка.Контрагент.СтранаРегистрации КАК СтранаРегистрации,
		|	ВременнаяТаблицаШапка.Ссылка.Договор.ВидДоговора КАК ВидДоговора,
		|	ВременнаяТаблицаШапка.Ссылка.ЭтоУниверсальныйДокумент КАК ЭтоУниверсальныйДокумент,
		|	ЕСТЬNULL(ВТ_Реквизиты.ЕстьНДС, ЛОЖЬ) КАК ЕстьНДС
		|ПОМЕСТИТЬ ВТ_ШапкаПрослеживаемость
		|ИЗ
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка,
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РасходнаяНакладная")
		...
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя")
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомитенту")Тогда
		ТекстЗапроса = ТекстВТ_Реквизиты()
		+ ОбщегоНазначенияУНФКлиентСервер.ТекстРазделителяЗапросовПакета()
		+
		"ВЫБРАТЬ
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеЗапасов")
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КомплектацияЗапасов") Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ВременнаяТаблицаШапка.Ссылка КАК Ссылка,
		|	НЕОПРЕДЕЛЕНО КАК ВидОперации,
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВременнаяТаблицаЗапасы", "ВременнаяТаблицаТовары");
	КонецЕсли;
...
Процедура ПодготовитьВременнуюТаблицуЗапасыПрослеживаемость(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Если ВОтчетеНужнаСебестоимостьБезНДС(ДокументСсылка) Тогда
		ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ ВТ_ПредварительнаяТаблица
		|ИЗ
		...
		// В некоторых случаях выполнение запроса выдает ошибку, что тип номенклатуры не может быть выбран в запросе.
		// Из-за этого здесь реализован обход, который позволяет обойти эту ошибку.
		ТаблизаЗапасы = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.СкопироватьКолонки("СуммаБезНДС, ВидДвижения, ИдентификаторСтроки");
		ТаблизаЗапасы.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
		ТаблизаЗапасы.Колонки.Добавить("ПрослеживаемыйТовар", Новый ОписаниеТипов("Булево"));
		Для Каждого ТекущаяСтрока Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы Цикл
			НоваяСтрока = ТаблизаЗапасы.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрока);
			Если НоваяСтрока.СуммаБезНДС = Неопределено Тогда
				НоваяСтрока.СуммаБезНДС = 0;
			КонецЕсли;
		КонецЦикла;
		
		Запрос.УстановитьПараметр("ТаблицаЗапасы", ТаблизаЗапасы);
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
		...
		|	ВременнаяТаблицаМатериалы.ИдентификаторСтроки,
		|	ВременнаяТаблицаМатериалы.ПрослеживаемыйТовар,
		|	ИСТИНА,
		|	ЛОЖЬ
		|ИЗ
		...
		|	0,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыСтавокНДС.БезНДС),
		|	ВременнаяТаблицаРаботы.ИдентификаторСтроки,
		|	ВременнаяТаблицаРаботы.ПрослеживаемыйТовар,
		|	ЛОЖЬ,
		|	ИСТИНА
		|ИЗ
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаЗапасыВозвраты.Номенклатура КАК Номенклатура,
		|	ВременнаяТаблицаЗапасыВозвраты.Себестоимость - ВременнаяТаблицаЗапасыВозвраты.СуммаНДСРег КАК СуммаБезНДС,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыСтавокНДС.БезНДС) КАК ВидСтавкиНДС,
		|	ВременнаяТаблицаЗапасыВозвраты.ИдентификаторСтроки КАК ИдентификаторСтроки,
		|	ВременнаяТаблицаЗапасыВозвраты.КлючСвязи КАК КлючСвязи,
		|	ВременнаяТаблицаЗапасыВозвраты.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
		|	ВременнаяТаблицаЗапасыВозвраты.Количество КАК Количество
		|ПОМЕСТИТЬ ВТ_ЗапасыПрослеживаемостьВозвраты
		|ИЗ
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
		...
		|ИЗ
		|	ВременнаяТаблицаЗапасы КАК ВременнаяТаблицаЗапасы";
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КомплектацияЗапасов") Тогда
		ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ ВТ_ЗапасыПрослеживаемость";
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СборкаЗапасов")
		И ДокументСсылка.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийСборкаЗапасов.Сборка") Тогда
		ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	ВременнаяТаблицаЗапасыПрослеживаемость.Номенклатура,
		|	ВременнаяТаблицаЗапасыПрослеживаемость.ПрослеживаемыйТовар,
		|	ВременнаяТаблицаЗапасыПрослеживаемость.ИдентификаторСтроки";
	ИначеЕсли ВОтчетеНужнаСебестоимостьБезНДС(ДокументСсылка) Тогда
		ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		|	ВТ_ПредварительнаяТаблица.Номенклатура КАК Номенклатура,
		|	СУММА(ВТ_ПредварительнаяТаблица.СуммаБезНДС) КАК СуммаБезНДС,
		|	ВТ_ПредварительнаяТаблица.ВидСтавкиНДС КАК ВидСтавкиНДС,
		|	ВТ_ПредварительнаяТаблица.ИдентификаторСтроки КАК ИдентификаторСтроки,
		|	ВТ_ПредварительнаяТаблица.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар
		|ПОМЕСТИТЬ ВТ_ЗапасыПрослеживаемость
		|ИЗ
		|	ВТ_ПредварительнаяТаблица КАК ВТ_ПредварительнаяТаблица
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ПредварительнаяТаблица.ПрослеживаемыйТовар,
		|	ВТ_ПредварительнаяТаблица.ИдентификаторСтроки,
		|	ВТ_ПредварительнаяТаблица.ВидСтавкиНДС,
		|	ВТ_ПредварительнаяТаблица.Номенклатура";
	КонецЕсли;
	...
	Если ВОтчетеНужнаСебестоимостьБезНДС(ДокументСсылка) Тогда
		...
		ТекстЗапроса = ТекстЗапроса
		+
		"УНИЧТОЖИТЬ ВТ_ПредварительнаяТаблица";
	КонецЕсли;
...
Функция ВОтчетеНужнаСебестоимостьБезНДС(ДокументСсылка)
	
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПеремещениеЗапасов")
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеЗапасов")
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РаспределениеЗатрат")
		Или (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СборкаЗапасов")
		И ДокументСсылка.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСборкаЗапасов.Сборка")) Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
...
Процедура СформироватьДвиженияОперацииСПрослеживаемымиТоварами(ДокументСсылка, Реквизиты, СтруктураДополнительныеСвойства, ТекстЗапроса) Экспорт
	...
	Если Не МетаданныеДокумента.Реквизиты.Найти("ЭтоУниверсальныйДокумент") = Неопределено Тогда
		Запрос.УстановитьПараметр("ЭтоУниверсальныйДокумент", ДокументСсылка.ЭтоУниверсальныйДокумент);
	Иначе
		Запрос.УстановитьПараметр("ЭтоУниверсальныйДокумент", Ложь);
	КонецЕсли;
...
Функция ТекстЗапросаОперацииСПрослеживаемымиТоварамиПоступлениеТоваров(ДокументСсылка)
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОприходованиеЗапасов") Тогда
		...
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|			ТОГДА ВТ_ЗапасыПрослеживаемость.Номенклатура
		|		ИНАЧЕ ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие
		|	КОНЕЦ КАК Номенклатура,
		|	ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие КАК Комплектующие,
		|	ВременнаяТаблицаСведенияПрослеживаемости.РНПТ КАК РНПТ,
		|	СУММА(ВременнаяТаблицаСведенияПрослеживаемости.Количество) КАК Количество,
		|	СУММА(ВременнаяТаблицаСведенияПрослеживаемости.Сумма) КАК СуммаБезНДС,
		...
		|	ВТ_ЗапасыПрослеживаемость.Номенклатура,
		|	ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие,
		...
		|	СУММА(ВЫБОР
		|			КОГДА ВТ_ПодготовленныеДанныеПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|				ТОГДА ЕСТЬNULL(ВТ_СуммыБезНДС.СуммаБезНДС, 0)
		|			ИНАЧЕ ВТ_ПодготовленныеДанныеПрослеживаемости.СуммаБезНДС
	...
	Иначе 
		...
		|	ЕСТЬNULL(ВТ_ШапкаПрослеживаемость.НомерВходящегоДокумента, """") КАК НомерДокумента,
		|	ВЫБОР
		|		КОГДА &ЭтоУниверсальныйДокумент
		...
		|					ИЛИ ВТ_ШапкаПрослеживаемость.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ПриемНаКомиссию))
		...
		|	СУММА(ВременнаяТаблицаСведенияПрослеживаемости.Сумма),
		...
		|			ИЛИ ВТ_ШапкаПрослеживаемость.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары))
		|
		|СГРУППИРОВАТЬ ПО
		|	ЕСТЬNULL(ВТ_ШапкаПрослеживаемость.ДатаВходящегоДокумента, ДАТАВРЕМЯ(1, 1, 1)),
		|	ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие,
		...
		|	ВТ_ПодготовленныеДанныеПрослеживаемости.ТипДокументаВПрослеживаемости,
		|	ВТ_ПодготовленныеДанныеПрослеживаемости.НомерДокумента,
		|	ВТ_ПодготовленныеДанныеПрослеживаемости.Комплектующие";
	КонецЕсли;
...
Функция ТекстЗапросаОперацииСПрослеживаемымиТоварамиРеализацияТоваров(ДокументСсылка)
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СборкаЗапасов") Тогда
		...
		|	СУММА(ВЫБОР
		|			КОГДА ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|				ТОГДА ВТ_ЗапасыПрослеживаемость.СуммаБезНДС
		|			ИНАЧЕ ВременнаяТаблицаСведенияПрослеживаемости.Сумма
		|		КОНЕЦ) КАК СуммаБезНДС,
		|	НАЧАЛОПЕРИОДА(ВТ_ШапкаПрослеживаемость.Дата, КВАРТАЛ) КАК ПериодОперации,
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда 
		...
		|	МИНИМУМ(ВременнаяТаблицаСведенияПрослеживаемости.НомерСтроки) КАК НомерСтроки,
		|	СУММА(ВременнаяТаблицаСведенияПрослеживаемости.Количество) КАК Количество,
		|	СУММА(ВТ_ЗапасыПрослеживаемость.Количество) КАК КоличествоЗапасы,
		|	СУММА(ВременнаяТаблицаСведенияПрослеживаемости.КоличествоПрослеживаемости) КАК КоличествоПрослеживаемости,
		|	СУММА(ВЫБОР
		|			КОГДА ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|				ТОГДА ВТ_ЗапасыПрослеживаемость.СуммаБезНДС
		...
		|	И НЕ ОтчетКомиссионераОПродажахПокупатели.Ссылка.ИспользоватьНовыйВидДокумента
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ШапкаПрослеживаемость.Номер,
		|	ВременнаяТаблицаСведенияПрослеживаемости.РНПТ,
		...
		|	МИНИМУМ(ВременнаяТаблицаСведенияПрослеживаемости.НомерСтроки),
		|	СУММА(-ВременнаяТаблицаСведенияПрослеживаемости.Количество),
		|	СУММА(ВТ_ЗапасыПрослеживаемость.Количество),
		|	СУММА(-ВременнаяТаблицаСведенияПрослеживаемости.КоличествоПрослеживаемости),
		|	СУММА(-ВЫБОР
		|			КОГДА ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|				ТОГДА ВТ_ЗапасыПрослеживаемость.СуммаБезНДС
		...
		|	МИНИМУМ(ВременнаяТаблицаСведенияПрослеживаемости.НомерСтроки),
		|	СУММА(ВЫБОР
		|			КОГДА ВременнаяТаблицаСведенияПрослеживаемости.Количество > ВТ_ЗапасыПрослеживаемость.Количество
		|				ТОГДА ВТ_ЗапасыПрослеживаемость.Количество
		|			ИНАЧЕ ВременнаяТаблицаСведенияПрослеживаемости.Количество
		|		КОНЕЦ),
		|	СУММА(ВТ_ЗапасыПрослеживаемость.Количество),
		|	СУММА(ВЫБОР
		|			КОГДА ВременнаяТаблицаСведенияПрослеживаемости.КоличествоПрослеживаемости > ВТ_ЗапасыПрослеживаемость.Количество
		|				ТОГДА ВТ_ЗапасыПрослеживаемость.Количество
		|			ИНАЧЕ ВременнаяТаблицаСведенияПрослеживаемости.КоличествоПрослеживаемости
		|		КОНЕЦ),
		|	СУММА(ВЫБОР
		|			КОГДА ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|				ТОГДА ВТ_ЗапасыПрослеживаемость.СуммаБезНДС
		...
		|	ВременнаяТаблицаСведенияПрослеживаемости.РНПТ <> НЕОПРЕДЕЛЕНО
		|	И ВременнаяТаблицаСведенияПрослеживаемости.РНПТ <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
		|	И ОтчетКомиссионераОПродажахПокупатели.Ссылка.ИспользоватьНовыйВидДокумента
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ШапкаПрослеживаемость.Номер,
		|	ВременнаяТаблицаСведенияПрослеживаемости.РНПТ,
		...
		|ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|			ТОГДА ВТ_ЗапасыПрослеживаемостьВозвраты.Номенклатура
		|		ИНАЧЕ ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.Комплектующие
		|	КОНЕЦ,
		|	ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.РНПТ,
		|	ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.Комплектующие,
		|	МИНИМУМ(ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.НомерСтроки),
		|	СУММА(ВЫБОР
		|			КОГДА ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.Количество > ВТ_ЗапасыПрослеживаемостьВозвраты.Количество
		|				ТОГДА -ВТ_ЗапасыПрослеживаемостьВозвраты.Количество
		|			ИНАЧЕ -ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.Количество
		|		КОНЕЦ),
		|	СУММА(-ВТ_ЗапасыПрослеживаемостьВозвраты.Количество),
		|	СУММА(ВЫБОР
		|			КОГДА ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.КоличествоПрослеживаемости > ВТ_ЗапасыПрослеживаемостьВозвраты.Количество
		|				ТОГДА -ВТ_ЗапасыПрослеживаемостьВозвраты.Количество
		|			ИНАЧЕ -ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.КоличествоПрослеживаемости
		|		КОНЕЦ),
		|	СУММА(-ВЫБОР
		|			КОГДА ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|				ТОГДА ВТ_ЗапасыПрослеживаемостьВозвраты.СуммаБезНДС
		|			ИНАЧЕ ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.Сумма
		|		КОНЕЦ),
		|	НАЧАЛОПЕРИОДА(ВТ_ШапкаПрослеживаемость.Дата, КВАРТАЛ),
		|	ВТ_ШапкаПрослеживаемость.Дата,
		|	ВТ_ШапкаПрослеживаемость.Номер,
		...
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПустаяСсылка)
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ОтчетКомиссионераПокупателиВозвраты.Покупатель.ИНН = """"
		|					ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ВозвратОтРозничногоПокупателя)
		|				ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПолучениеКомитентомСведенийОВозвратеТовараКомиссионеру)
		|			КОНЕЦ
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ОтчетКомиссионераПокупателиВозвраты.СчетФактура = ЗНАЧЕНИЕ(Документ.СчетФактура.ПустаяСсылка)
		|			ТОГДА ВТ_ШапкаПрослеживаемость.Ссылка
		|		ИНАЧЕ ОтчетКомиссионераПокупателиВозвраты.СчетФактура
		|	КОНЕЦ,
		|	ВТ_ШапкаПрослеживаемость.Контрагент
		|ИЗ
		|	ВТ_ШапкаПрослеживаемость КАК ВТ_ШапкаПрослеживаемость,
		|	ВТ_ЗапасыПрослеживаемостьВозвраты КАК ВТ_ЗапасыПрослеживаемостьВозвраты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаСведенияПрослеживаемости КАК ВременнаяТаблицаСведенияПрослеживаемостиВозвраты
		|		ПО ВТ_ЗапасыПрослеживаемостьВозвраты.ИдентификаторСтроки = ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.ИдентификаторСтроки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетКомиссионера.ПокупателиВозвраты КАК ОтчетКомиссионераПокупателиВозвраты
		|		ПО ВТ_ЗапасыПрослеживаемостьВозвраты.КлючСвязи = ОтчетКомиссионераПокупателиВозвраты.КлючСвязи
		|			И (ОтчетКомиссионераПокупателиВозвраты.Ссылка = &ДокументСсылка)
		|ГДЕ
		|	ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.РНПТ <> НЕОПРЕДЕЛЕНО
		|	И ВременнаяТаблицаСведенияПрослеживаемостиВозвраты.РНПТ <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|				ИЛИ ВТ_ЗапасыПрослеживаемость.ЭтоМатериал
		|			ТОГДА ВТ_ЗапасыПрослеживаемость.Номенклатура
		|		ИНАЧЕ ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие
		|	КОНЕЦ КАК Номенклатура,
		|	ВременнаяТаблицаСведенияПрослеживаемости.РНПТ КАК РНПТ,
		...
		|	ВТ_ШапкаПрослеживаемость.Номер КАК НомерДокумента,
		|	ВЫБОР
		|		КОГДА ВТ_ЗапасыПрослеживаемость.ЭтоМатериал
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.ТипыДокументов.Акт)
		|		КОГДА ВТ_ЗапасыПрослеживаемость.ЭтоРабота
		|			ТОГДА ВЫБОР
		|					КОГДА &ЭтоУниверсальныйДокумент
		|						ТОГДА ЗНАЧЕНИЕ(Справочник.ТипыДокументов.УПД)
		...
		|	ВЫБОР
		|		КОГДА ВТ_ЗапасыПрослеживаемость.ЭтоМатериал
		|			ТОГДА ВТ_ШапкаПрослеживаемость.КодОперацииПрослеживаемости
		|		КОГДА ВТ_ШапкаПрослеживаемость.ЕстьНДС
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПустаяСсылка)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.Реализация)
		|	КОНЕЦ КАК КодОперации,
		|	ВТ_ШапкаПрослеживаемость.Ссылка КАК ДокументОперации,
		|	ВТ_ШапкаПрослеживаемость.ЭтоУниверсальныйДокумент КАК ЭтоУниверсальныйДокумент,
		|	ВЫБОР
		|		КОГДА ВТ_ЗапасыПрослеживаемость.ЭтоМатериал
		|			ТОГДА НЕОПРЕДЕЛЕНО
		|		ИНАЧЕ ВТ_ШапкаПрослеживаемость.Контрагент
		|	КОНЕЦ КАК Контрагент,
		|	ВТ_ЗапасыПрослеживаемость.ЭтоМатериал КАК ЭтоМатериал,
		|	ВТ_ЗапасыПрослеживаемость.ЭтоРабота КАК ЭтоРабота,
		|	ВТ_ШапкаПрослеживаемость.ТоварыПередаютсяВСоставеРабот КАК ТоварыПередаютсяВСоставеРабот
		|ПОМЕСТИТЬ ВТ_ПодготовленныеДанныеПрослеживаемости
		|ИЗ
		|	ВТ_ШапкаПрослеживаемость КАК ВТ_ШапкаПрослеживаемость,
		|	ВТ_ЗапасыПрослеживаемость КАК ВТ_ЗапасыПрослеживаемость
		...
		|	ВременнаяТаблицаСведенияПрослеживаемости.РНПТ <> НЕОПРЕДЕЛЕНО
		|	И ВременнаяТаблицаСведенияПрослеживаемости.РНПТ <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
		|	И НЕ(ВТ_ЗапасыПрослеживаемость.ЭтоМатериал
		|				И ВТ_ШапкаПрослеживаемость.ТоварыПередаютсяВСоставеРабот)
		|	И НЕ(ВТ_ЗапасыПрослеживаемость.ЭтоРабота
		|				И НЕ ВТ_ШапкаПрослеживаемость.ТоварыПередаютсяВСоставеРабот)
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ШапкаПрослеживаемость.Ссылка,
		|	ВТ_ШапкаПрослеживаемость.ЭтоУниверсальныйДокумент,
		|	ВТ_ЗапасыПрослеживаемость.ЭтоМатериал,
		|	ВТ_ЗапасыПрослеживаемость.ЭтоРабота,
		|	ВТ_ШапкаПрослеживаемость.ТоварыПередаютсяВСоставеРабот,
		|	ВременнаяТаблицаСведенияПрослеживаемости.Комплектующие,
		|	ВТ_ШапкаПрослеживаемость.Номер,
		|	ВТ_ЗапасыПрослеживаемость.Номенклатура,
		|	ВременнаяТаблицаСведенияПрослеживаемости.РНПТ,
		|	ВТ_ШапкаПрослеживаемость.Дата,
		|	НАЧАЛОПЕРИОДА(ВТ_ШапкаПрослеживаемость.Дата, КВАРТАЛ),
		|	ВТ_ЗапасыПрослеживаемость.ИдентификаторСтроки,
		|	ВЫБОР
		|		КОГДА ВТ_ЗапасыПрослеживаемость.ЭтоМатериал
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ПорядокОтраженияВОтчетностиПоПрослеживаемости.ОтчетОбОперациях)
		|		КОГДА ВТ_ШапкаПрослеживаемость.ЕстьНДС
		...
		|	СУММА(ВЫБОР
		|			КОГДА ВТ_ПодготовленныеДанныеПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|					И НЕ ВТ_ПодготовленныеДанныеПрослеживаемости.ЭтоМатериал
		|					И НЕ ВТ_ПодготовленныеДанныеПрослеживаемости.ЭтоРабота
		|				ТОГДА ЕСТЬNULL(ВТ_СуммыБезНДС.СуммаБезНДС, 0)
		|			ИНАЧЕ ВТ_ПодготовленныеДанныеПрослеживаемости.СуммаБезНДС
		|		КОНЕЦ) КАК СуммаБезНДС
		|ИЗ
	...
	Иначе // РН
		...
		|			ИЛИ ВТ_ШапкаПрослеживаемость.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером))
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ШапкаПрослеживаемость.Ссылка,
		|	ВТ_ШапкаПрослеживаемость.Контрагент,
...
Процедура СформироватьДвиженияПрослеживаемыхТоваров(ДокументСсылка, СтруктураДополнительныеСвойства, ИмяВременнойТаблицыЗапасы = "ВременнаяТаблицаЗапасы") Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РасходнаяНакладная") 
		И ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером Тогда
		Запрос.Текст = ТекстЗапросаДвиженияПрослеживаемыхТоваровВыкупУКомиссионера();
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПриходнаяНакладная") 
		И ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары Тогда 
		Запрос.Текст = ТекстЗапросаДвиженияПрослеживаемыхТоваровВозвратВыкупленногоУКомиссионера();
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомиссионера") 
		...
		Если СтруктураДополнительныеСвойства.Свойство("ИспользоватьНовыйВидДокумента") И СтруктураДополнительныеСвойства.ИспользоватьНовыйВидДокумента Тогда 
			Запрос.Текст = ТекстЗапросаДвиженияПрослеживаемыхКомиссионныхТоваровОбщий(Истина);
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КомплектацияЗапасов") Тогда
		...
		Запрос.Текст = ТекстЗапросаДвиженияПрослеживаемыхТоваровКомплектацияЗапасов();
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
		И ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		Запрос.Текст = ТекстЗапросаДвиженияПрослеживаемыхТоваровЗаказНаряд();
	Иначе
...
//
Процедура ПодобратьРНПТОстатки(ДокументОбъект, Отказ, ИмяТабличнойЧасти = "Запасы") Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
		...
		Иначе
			...
			ДанныеПроверки.Вставить("ИмяТЧ", ИмяТабличнойЧасти);
	...
	ДокументОбъект.СведенияПрослеживаемости.Свернуть("РНПТ, ИдентификаторСтроки, Номенклатура, СтранаПроисхождения", "Количество, КоличествоПрослеживаемости, Сумма");
...
// Процедура - Подобрать РНПТ по данным продаж товаров переданных на комиссию
//
// Параметры:
//  ДокументОбъект	 - ДокументОбъект	 - объект документа, в котором следует заполнить таблицу РНПТ
//  Отказ			 - Булево			 - признак ошибки подбора
//  ТаблицаТоваров			 - Табличная часть			 - таблица товаров для подбора РНПТ
//
Процедура ПодобратьРНПТКомиссия(ДокументОбъект, Отказ, ИмяТаблицыТоваров = "Запасы") Экспорт

	ТаблицаТоваров = ДокументОбъект[ИмяТаблицыТоваров].Выгрузить();
	
	Если ТаблицаТоваров.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ОтчетКомиссионера")
		И ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ПродажаИВозврат Тогда
		
		Если ИмяТаблицыТоваров = "Запасы" Тогда
			ТаблицаРНПТ = ТаблицаРНПТПередачНаКомиссию(ДокументОбъект, ТаблицаТоваров);
		ИначеЕсли ИмяТаблицыТоваров = "ЗапасыВозвраты" Тогда
	...
	Для Каждого СтрокаТабличнойЧасти Из ДокументОбъект[ИмяТаблицыТоваров] Цикл
		...
		Иначе
			...
			// Проверка заполнения табличной части
			ДанныеПроверки = Новый Структура;
			ДанныеПроверки.Вставить("ИмяТЧ", ИмяТаблицыТоваров);
			ДанныеПроверки.Вставить("ИмяСписка", НСтр("ru = 'Товары'"));
			ПроверитьЗаполнениеДанныхПрослеживаемости(ДокументОбъект, ДанныеПроверки, Отказ);
...
Функция ПолучитьНачальныеДанные(Период, Организация) Экспорт
	...
	|ГДЕ
	|	ЕСТЬNULL(ТаблицаОстатковБезОснованияБезИнвентаризации.Количество, 0) <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
//  СтруктураДополнительныеСвойства - Дополнительные свойства для проведения
//  СтруктураДополнительныеСвойства - Дополнительные свойства для проведения
//

Процедура СформироватьДвиженияПрослеживаемыхТоваровВЕАЭС(Движения, СтруктураДополнительныеСвойства, ИмяВременнойТаблицыЗапасы = "ВременнаяТаблицаЗапасы") Экспорт
...
Функция ОтказОтключитьУчетПрослеживаемыхТоваров() Экспорт
	...
	Возврат НСтр("ru='Нельзя изменить значение опции Учет прослеживаемых товаров, потому что в программе зафиксированы остатки номеров РНПТ'");
...
Процедура ОбработкаПроведенияУведомления(ПараметрыПроведения, Движения, Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(ТаблицаДвиженийРНПТ)
		...
		УстановитьСвойстваРНПТ(РазличныеРНПТ, ПараметрыПроведения.Реквизиты[0]);
	КонецЕсли;
...
Функция ТекстЗапросаДвиженияПрослеживаемыхТоваровВыкупУКомиссионера()
	...
	|	СведенияПрослеживаемости.РНПТ КАК РНПТ,
	|	ВЫБОР
	|		КОГДА СведенияПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА ТаблицаЗапасы.СтранаПроисхождения
	|		ИНАЧЕ СведенияПрослеживаемости.СтранаПроисхождения
	...
	|	СведенияПрослеживаемости.Комплектующие КАК Комплектующие
	|ИЗ
	|	ВременнаяТаблицаСведенияПрослеживаемости КАК СведенияПрослеживаемости
	...
	|	&Контрагент,
	|	СведенияПрослеживаемости.Комплектующие
	|ИЗ
	|	ВременнаяТаблицаСведенияПрослеживаемости КАК СведенияПрослеживаемости
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
	|		ПО СведенияПрослеживаемости.ИдентификаторСтроки = ТаблицаЗапасы.ИдентификаторСтроки,
	...
	|	Шапка.Ссылка,
	|	СведенияПрослеживаемости.НомерСтроки,
	|	СведенияПрослеживаемости.ВидДвижения,
	|	&Организация,
	|	СведенияПрослеживаемости.РНПТ,
	|	ВЫБОР
	|		КОГДА СведенияПрослеживаемости.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				И СведенияПрослеживаемости.СтранаПроисхождения = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|			ТОГДА ТаблицаЗапасы.СтранаПроисхождения
	|		КОГДА СведенияПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА ТаблицаЗапасы.СтранаПроисхождения
	|		ИНАЧЕ СведенияПрослеживаемости.СтранаПроисхождения
...
Функция ТекстЗапросаДвиженияПрослеживаемыхТоваровВозвратВыкупленногоУКомиссионера() 
...
Функция ТекстЗапросаДвиженияПрослеживаемыхТоваровЗаказНаряд()
	...
	|		КОГДА СведенияПрослеживаемости.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				И СведенияПрослеживаемости.СтранаПроисхождения = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|			ТОГДА ТаблицаМатериалы.СтранаПроисхождения
	|		КОГДА СведенияПрослеживаемости.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА ТаблицаМатериалы.СтранаПроисхождения
	|		ИНАЧЕ СведенияПрослеживаемости.СтранаПроисхождения
	|	КОНЕЦ,
	...
	|ИЗ
	|	ВременнаяТаблицаСведенияПрослеживаемости КАК СведенияПрослеживаемости
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаМатериалы КАК ТаблицаМатериалы
	|		ПО СведенияПрослеживаемости.ИдентификаторСтроки = ТаблицаМатериалы.ИдентификаторСтроки,
	|	ВременнаяТаблицаШапка КАК Шапка
	|ГДЕ
	|	НЕ &ЭтоПоступлениеИзЕАЭС";
...
Функция ТекстЗапросаДвиженияПрослеживаемыхТоваровКомплектацияЗапасов()
	...
	|	СведенияПрослеживаемости.НомерСтроки,
	|	ВЫБОР
	|		КОГДА &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКомплектацияЗапасов.Сборка)
...
Функция ТекстЗапросаДвиженияПрослеживаемыхКомиссионныхТоваровОбщий(ИспользоватьНовыйВидОтчетаКомиссионера = Ложь) 
	
	Если ИспользоватьНовыйВидОтчетаКомиссионера Тогда 
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	Шапка.Дата КАК Период,
		|	Шапка.Ссылка КАК Регистратор,
		...
		|	ТаблицаЗапасы.Характеристика КАК Характеристика,
		|	ТаблицаЗапасы.Партия КАК Партия,
		|	ТаблицаЗапасы.Количество КАК КоличествоЗапасы,
		|	ВЫБОР
		|		КОГДА СведенияПрослеживаемости.Количество > ТаблицаЗапасы.Количество
		|			ТОГДА ТаблицаЗапасы.Количество
		|		ИНАЧЕ СведенияПрослеживаемости.Количество
		|	КОНЕЦ КАК Количество,
		|	ВЫБОР
		|		КОГДА СведенияПрослеживаемости.КоличествоПрослеживаемости > ТаблицаЗапасы.Количество
		|			ТОГДА ТаблицаЗапасы.Количество
		|		ИНАЧЕ СведенияПрослеживаемости.КоличествоПрослеживаемости
		...
		|	Шапка.Дата,
		|	Шапка.Ссылка,
		|	СведенияПрослеживаемостиВозвраты.НомерСтроки,
		|	СведенияПрослеживаемостиВозвраты.ВидДвижения,
		|	&Организация,
		|	СведенияПрослеживаемостиВозвраты.РНПТ,
		|	ВЫБОР
		|		КОГДА СведенияПрослеживаемостиВозвраты.Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|			ТОГДА ТаблицаЗапасыВозвраты.СтранаПроисхождения
		|		ИНАЧЕ СведенияПрослеживаемостиВозвраты.СтранаПроисхождения
		|	КОНЕЦ,
		|	СведенияПрослеживаемостиВозвраты.Комплектующие,
		|	ТаблицаЗапасыВозвраты.Номенклатура,
		|	ТаблицаЗапасыВозвраты.Характеристика,
		|	ТаблицаЗапасыВозвраты.Партия,
		|	ТаблицаЗапасыВозвраты.Количество,
		|	ВЫБОР
		|		КОГДА СведенияПрослеживаемостиВозвраты.Количество > ТаблицаЗапасыВозвраты.Количество
		|			ТОГДА ТаблицаЗапасыВозвраты.Количество
		|		ИНАЧЕ СведенияПрослеживаемостиВозвраты.Количество
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА СведенияПрослеживаемостиВозвраты.КоличествоПрослеживаемости > ТаблицаЗапасыВозвраты.Количество
		|			ТОГДА ТаблицаЗапасыВозвраты.Количество
		|		ИНАЧЕ СведенияПрослеживаемостиВозвраты.КоличествоПрослеживаемости
		|	КОНЕЦ,
		|	&Контрагент,
		|	ЕСТЬNULL(ТаблицаЗапасыВозвраты.КорректируемыйДокумент, Шапка.КорректируемыйДокумент),
		|	&Контрагент
		|ИЗ
		|	ВременнаяТаблицаСведенияПрослеживаемостиВозвраты КАК СведенияПрослеживаемостиВозвраты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаЗапасыВозвраты КАК ТаблицаЗапасыВозвраты
		|		ПО СведенияПрослеживаемостиВозвраты.ИдентификаторСтроки = ТаблицаЗапасыВозвраты.ИдентификаторСтроки,
		|	ВременнаяТаблицаШапка КАК Шапка
		|ГДЕ
		|	НЕ &ЭтоПоступлениеИзЕАЭС
		|;
		...
		|	ВтИтог.Регистратор КАК Регистратор,
		...
		|	ВтИтог.Комплектующие КАК Комплектующие,
		|	ВтИтог.Номенклатура КАК Номенклатура,
		|	ВтИтог.Характеристика КАК Характеристика,
		|	ВтИтог.Партия КАК Партия,
		|	СУММА(ВтИтог.КоличествоЗапасы) КАК КоличествоЗапасы,
		|	СУММА(ВтИтог.Количество) КАК Количество,
		|	СУММА(ВтИтог.КоличествоПрослеживаемости) КАК КоличествоПрослеживаемости,
		|	ВтИтог.Контрагент КАК Контрагент,
		|	ВтИтог.КорректируемыйДокумент КАК КорректируемыйДокумент,
		...
		|	ВтИтог.Регистратор,
		|	ВтИтог.Комиссионер,
		|	ВтИтог.КорректируемыйДокумент,
		|	ВтИтог.Контрагент,
		|	ВтИтог.Партия,
		|	ВтИтог.Характеристика,
		|	ВтИтог.Номенклатура,
		|	ВтИтог.Комплектующие,
...
Процедура СообщитьОшибкуПодбора(ДокументОбъект, СтрокаТабличнойЧасти, ОсталосьПодобрать, Отказ)
	...
	Иначе
		ТекстОшибки = СтрШаблон(
		НСтр("ru = 'По учету прослеживаемости не удалось подобрать %1 %2 запаса %3'"),
		Формат(ОсталосьПодобрать, "ЧГ=0"),
		СтрокаТабличнойЧасти.ЕдиницаИзмерения,
		ПредставлениеНоменклатуры);
	КонецЕсли;
...
Функция ТаблицаРНПТОстатки(ДокументОбъект, ТаблицаЗапасов)
	...
	|ИЗ
	|	ТаблицаЗапасов КАК ТаблицаЗапасов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрослеживаемыеТовары.Остатки(
	|				&МоментСписания,
	|				Организация = &Организация
	|					И Комплектующие = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|					И Комиссионер = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)) КАК ПрослеживаемыеТоварыОстатки
...
Функция ТаблицаРНПТПродажПоступлений(ДокументОбъект, ТаблицаТоваров)
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ОтчетКомитенту") Тогда 
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РасходнаяНакладная") 
		И (ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику
		Или ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратКомитенту) Тогда 
		Если ТипЗнч(ДокументОбъект.ДокументОснование) <> Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
			Возврат Неопределено;
		КонецЕсли; 
		ИсходныеДокументы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДокументОбъект.ДокументОснование);
	...
	|		РегистрНакопления.ПрослеживаемыеТовары КАК ПрослеживаемыеТовары
	|	ГДЕ
	|		ВЫРАЗИТЬ(ПрослеживаемыеТовары.Регистратор КАК Документ.УведомлениеОВвозеПрослеживаемыхТоваров).ПервичныйДокумент В (&ИсходныеДокументы)) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ПодобратьРНПТИзОстатков(ДокументОбъект, ОсталосьПодобрать, СтрокаТабличнойЧасти, ТаблицаРНПТОстатки)
	...
	Для Каждого ПодобранныйРНПТ Из ОстаткиРНПТ Цикл
		...
		Иначе    
			...
			Если ПодобранныйРНПТ.КоличествоПрослеживаемости = 0 Тогда
				Прервать;
			КонецЕсли;
