...
//                     - Массив Из см. ШтрихкодированиеИСМП.НовыйЭлементКоллекцииУпаковокДляРаспределенияПоТоварам - Исходные штрихкоды упаковок, используя частичное выбытие.
...
Функция СодержимоеИКодыОСУ(ТаблицаУпаковок, ТаблицаТоваров, ПараметрыСканирования) Экспорт

	СодержимоеРезультат = Содержимое(ТаблицаУпаковок.ВыгрузитьКолонку("Штрихкод"),,, Истина);
...
Процедура ЗаполнитьСведенияОМаркировкеАктОРасхождениях_2019(Приемник, Источник, ДанныеШтрихкодовУпаковок,
	...
	КонецЕсли;
	Для Каждого СтрокаУпаковки Из СтрокиУпаковок Цикл
		Если СтрокаУпаковки.ВидПродукции = Перечисления.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха Тогда
			Прервать;
