...
Функция КатегорииОпераций() Экспорт
	...
	ПередачаДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.ПодключениеКОборудованиюРозлива,
		НСтр("ru = 'Подключение кега к оборудованию для розлива'"));
	
	ПередачаДанных.Вставить(
		Перечисления.ВидыОперацийИСМП.ПодключениеКОборудованиюРозливаРасчетСтатуса,
		НСтр("ru = 'Подключение кега к оборудованию для розлива'"));
	...
	Попытка
		...
		Если ЭтоПовторноеФормированиеСообщения
			...
			Если ЭлементОчереди.Назначение = Перечисления.НазначениеСообщенийИСМП.ИСМП Тогда
				...
				ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ПодключениеКОборудованиюРозливаРасчетСтатуса Тогда
...
// 
// Параметры:
//  СообщенияJSON - Массив из См. ИнтеграцияИСМПСлужебный.СтруктураСообщенияJSON- массив сообщений для заполнения (см. ИнтеграцияИСМПСлужебный.СтруктураСообщенияJSON).
//  ОбщийЭлементОчереди -  СтрокаТаблицыЗначений - (См. ИнтеграцияИСМП.ИнициализиоватьОчередьСообщений).
...
//  СообщениеJSON - См. ИнтеграцияИСМПСлужебный.СтруктураСообщенияJSON.
//
// Возвращаемое значение:
...
Процедура ОбработкаЗагрузкиПереданногоДокумента(ЭлементОчереди, ПараметрыОбмена, РезультатОбработкиЗаявки, ПараметрыОбновленияСтатуса)
	...
	ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ПодключениеКОборудованиюРозлива Тогда
...
Процедура ЗагрузитьРезультатОбработкиАбстрактнойОперацииРасчетСтатуса(ЭлементОчереди, ПараметрыОбмена)
	...
	Если ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ОтчетОбИспользованииРасчетСтатуса
		Или ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотРасчетСтатуса
		Или ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборот
		Или ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ПодключениеКОборудованиюРозливаРасчетСтатуса
	...
	Попытка
		...
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			...
			ИнтеграцияИСМПСлужебный.ДобавитьЗаписьВПротоколОбмена(
				ПротоколаОбменаXML,
				Реквизиты);
...
Функция ЗапроситьСтатусПулаКодовМаркировкиПоСтрокеЗаказаИПолучитьКоды(ЭлементОчереди, ПараметрыОбмена, НастройкаОбменаСУЗ)
	...
	ПараметрыЗапроса = ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса;
	ТребуетсяAPIv3   = (ЭлементОчереди.ВидПродукции = Перечисления.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха);
		...
		Если СтатусПула = Перечисления.СтатусыПулаКодовМаркировкиСУЗ.Готов Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(РезультатЗапросаКодовМаркировки.ТекстОшибки) Тогда
					...
					КонецЕсли;
				ИначеЕсли ТребуетсяAPIv3 Тогда
					РезультатОбработкиПула.ОбработкаПулаЗавершена = Истина;
...
Процедура ОбработкаДокументаОбъемноСортовогоУчета(ДокументПриемка, ДанныеШтрихкодов)
	...
	Для Каждого СтрокаДанных Из ДанныеШтрихкодов Цикл
		...
		Если СтрокиПоEAN.Количество() = 1 Тогда
			...
			Если СтрокаПоEAN.ПроизвольнаяЕдиницаУчета Тогда
...
Процедура ЗагрузитьРезультатОбработкиАбстрактнойОперации(ЭлементОчереди, ПараметрыОбмена)
	...
	Если ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.Агрегация
		...
		Или ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ПодключениеКОборудованиюРозливаРасчетСтатуса Тогда
		СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаВыполнена;
	Иначе
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииВводВОборотИмпортСФТС(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если Не ЕстьОшибки Тогда
		...
		Если ЭтоМолочнаяПродукцияИСМП Тогда
			...
			Если ЭлементОчереди.ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС Тогда
				Настройки.УказыватьИдентификаторПроисхожденияВЕТИС = Истина;
				Настройки.ЗаполнятьВложенныеКоды = Истина;
			КонецЕсли;
			Настройки.Вставить("ИмяРеквизитаКодМаркировкиДополнительное", "ki");
		КонецЕсли;
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииПриемкаИСМП(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если ОперацияПодтверждениеПриемки Тогда
		...
		Если ЭтоПриемкаИзЕАЭСПриОСУ Тогда
			...
			ЗаполнитьРезультатПроверкиПриемкиТоваровИСМППриОСУ(РезультатПроверки, Товары, ШтрихкодыУпаковок);
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииПодключениеКОборудованиюРозлива(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Если ДанныеСтатуса.ОсобоеСостояние = Перечисления.СтатусыКодовМаркировкиИСМП.ПодключенКОборудованиюРозлива Тогда
		...
		НоваяСтрока.ДанныеПодключения = РегистрыСведений.КегиНаОборудованииРозливаИСМП.ИнициализироватьДанныеПодключенияКОборудованию();
	...
	Если ТаблицыБлокируемыхСсылок.Количество() Тогда
		...
		ЭлементБлокировки = БлокировкаДанных.Добавить(Метаданные.РегистрыСведений.КегиНаОборудованииРозливаИСМП.ПолноеИмя());
		ЭлементБлокировки.ИсточникДанных = ТаблицыБлокируемыхСсылок;
		...
		Для Каждого СтрокаТаблицы Из ТаблицыБлокируемыхСсылок Цикл
			...
			ДанныеПодключения = РегистрыСведений.КегиНаОборудованииРозливаИСМП.ПолучитьПодключениеКОборудованию(
			...
			Если ДанныеПодключения.Статус = Перечисления.СтатусыПодключенияКОборудованиюРозливаИСМП.Подключено Тогда
				РегистрыСведений.КегиНаОборудованииРозливаИСМП.УстановитьПодключениеКОборудованию(
					СтрокаТаблицы.ДанныеПодключения,
					ДанныеПодключения);
			Иначе
				РегистрыСведений.КегиНаОборудованииРозливаИСМП.УдалитьПодключениеКОборудованию(СтрокаТаблицы.КодМаркировки);
			КонецЕсли;
...
Процедура ЗаполнитьРезультатПроверкиПриемкиТоваровИСМППриОСУ(РезультатПроверки, Товары, ШтрихкодыУпаковок)
				...
				Если СообщениеМожноЗагружать(ЭлементОчереди, ПараметрыОбмена, НайденныеЗависимости, ДатаСледующейИтерации) Тогда
					...
					Если ЭлементОчереди.Назначение = Перечисления.НазначениеСообщенийИСМП.ИСМП Тогда
						...
						ИначеЕсли ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.АгрегацияСоздание
							...
							Или ЭлементОчереди.Операция = Перечисления.ВидыОперацийИСМП.ПодключениеКОборудованиюРозлива Тогда
