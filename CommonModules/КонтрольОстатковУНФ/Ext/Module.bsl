...
Процедура ЗапасыНаСкладахСписком(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ТипЗнч(ДокОбъект) = Тип("ДокументОбъект.ЧекККМ") ИЛИ ТипЗнч(ДокОбъект) = Тип("ДокументОбъект.ЧекККМВозврат") Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Ошибка проведения документа'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка, ДокОбъект,, ТекстСообщения);
		КонецЕсли;
...
// Выполняет контроль корректности количества запасов при пересчете единиц измерения
//  (после пересчета количество не должно равняться нулю)
//
// Параметры:
//  МенеджерВременныхТаблиц	 - МенеджерВременныхТаблиц	 - Менеджер временных таблиц, содержащий таблицу запасов
//  Отказ					 - Булево					 - признак наличия ошибок.
//  ИмяВременнойТаблицы		 - Строка					 - Имя временной таблицы запасов
//
Процедура КонтрольПересчетаКоличества(МенеджерВременныхТаблиц, Отказ, ИмяВременнойТаблицы = "ВременнаяТаблицаЗапасы") Экспорт
	...
	|	ВЫРАЗИТЬ(ТаблицаЗапасы.Номенклатура КАК Справочник.Номенклатура).ЕдиницаИзмерения КАК ЕдиницаИзмеренияБазовая,
	|	ТаблицаЗапасы.Характеристика КАК Характеристика,
	|	ТаблицаЗапасы.Партия КАК Партия,
	|	СУММА(ТаблицаЗапасы.КоличествоДокумент) КАК КоличествоДокумент,
	|	ТаблицаЗапасы.ЕдиницаИзмеренияДокумент КАК ЕдиницаИзмеренияДокумент
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
	|ГДЕ
	|	ТаблицаЗапасы.ЕдиницаИзмеренияДокумент ССЫЛКА Справочник.ЕдиницыИзмерения
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаЗапасы.Характеристика,
	|	ТаблицаЗапасы.Номенклатура,
	|	ТаблицаЗапасы.Партия,
	|	ТаблицаЗапасы.ЕдиницаИзмеренияДокумент
	|
	|ИМЕЮЩИЕ
	|	СУММА(ТаблицаЗапасы.КоличествоДокумент) <> 0 И
	|	СУММА(ВЫРАЗИТЬ(ТаблицаЗапасы.Количество КАК ЧИСЛО(15, 3))) = 0";
	Если НЕ ПустаяСтрока(ИмяВременнойТаблицы) И ИмяВременнойТаблицы <> "ВременнаяТаблицаЗапасы" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВременнаяТаблицаЗапасы", ИмяВременнойТаблицы);
	КонецЕсли;
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ШаблонСообщения = НСтр("ru = 'Номенклатура: %1.
			|Результат пересчета количества %2 %3 в базовую единицу %4 равен нулю.
			|Некорректно выбрана базовая единица номенклатуры.'");
		ПредставлениеНоменклатуры = Справочники.Номенклатура.Представление(Выборка.Номенклатура, Выборка.Характеристика, 
			Выборка.Партия);
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ПредставлениеНоменклатуры, Выборка.КоличествоДокумент, 
			Выборка.ЕдиницаИзмеренияДокумент, Выборка.ЕдиницаИзмеренияБазовая);
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , , , Отказ);
	КонецЦикла;
	
КонецПроцедуры

// Сообщает об ошибках проведения по регистру Запасы к поступлению на склады.
//
// Параметры:
//  ДокОбъект - ДокументОбъект - проводимый документ,
//  ВыборкаЗаписей - ВыборкаИзРезультатаЗапроса - выборка из результата запроса контроля остатков,
//  Отказ - Булево - признак наличия ошибок.
Процедура ЗапасыКПоступлению(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	
	ЗаголовокОшибки = НСтр("ru = 'Ошибка:'");
	
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр(
		"ru = 'Не хватает запасов к поступлению %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление% в ячейке %Ячейка% по документу %Документ%'");
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыводитьВЗаголовокРасшифровку Тогда
			...
			Или (Не УчетПоНесколькимПодразделениям И ВыборкаЗаписей.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение) Тогда
				ПредставлениеСтруктурнойЕдиницы = "";
			Иначе
				ПредставлениеСтруктурнойЕдиницы = Справочники.СтруктурныеЕдиницы.Представление(ВыборкаЗаписей.СтруктурнаяЕдиницаПредставление);
			КонецЕсли;
			ТекстЗаголовкаСообщения = СтрЗаменить(ШаблонЗаголовкаСообщения, "%СтруктурнаяЕдиницаПредставление%",
				ПредставлениеСтруктурнойЕдиницы);
			ТекстЗаголовкаСообщения = СтрЗаменить(ТекстЗаголовкаСообщения, "%ТипСтруктурнойЕдиницы%",
			...
			ТекстЗаголовкаСообщения = СтрЗаменить(ТекстЗаголовкаСообщения, "%Документ%",
				ВыборкаЗаписей.ДокументПредставление);
				
			ТекстЗаголовкаСообщения = СтрЗаменить(ТекстЗаголовкаСообщения, "%Ячейка%",
				?(ЗначениеЗаполнено(ВыборкаЗаписей.ЯчейкаПредставление),ВыборкаЗаписей.ЯчейкаПредставление, НСтр("ru = '<Без ячейки>'")));
...
// Сообщает об ошибках проведения по регистру Запасы к расходу со складов.
//
// Параметры:
//  ДокОбъект - ДокументОбъект - проводимый документ,
//  ВыборкаЗаписей - ВыборкаИзРезультатаЗапроса - выборка из результата запроса контроля остатков,
//  Отказ - Булево - признак наличия ошибок.
Процедура ЗапасыКРасходу(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	
	ЗаголовокОшибки = НСтр("ru = 'Ошибка:'");
	
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр(
		"ru = 'Не хватает запасов к расходу %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление% в ячейке %Ячейка% по документу %Документ%'");
...
//                      * СерияПредставление - Строка.
//  Отказ			 - Булево - признак наличия ошибок.
Процедура СерииНоменклатурыКРасходуПоДокументам(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт //BG
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		КоличествоОстаток = ?(ТипЗнч(ДокОбъект.Ссылка) = Тип("ДокументСсылка.РасходныйОрдер"), ВыборкаЗаписей.ОстатокЗапасыНаСкладах, -ВыборкаЗаписей.ОстатокЗапасыНаСкладах);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОстаток%", КоличествоОстаток);
		
		Количество = ?(ТипЗнч(ДокОбъект.Ссылка) = Тип("ДокументСсылка.РасходныйОрдер"), ВыборкаЗаписей.КоличествоОстатокЗапасыНаСкладах, -ВыборкаЗаписей.КоличествоОстатокЗапасыНаСкладах);
...
//                      * СерияПредставление - Строка.
//  Отказ			 - Булево - признак наличия ошибок.
Процедура СерииНоменклатурыКПоступлению(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт //BG
	
	ШаблонЗаголовкаСообщения = НСтр("ru = 'Ошибка:'") + Символы.ПС + НСтр(
		"ru = 'Не хватает серий к поступлению %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%'");
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		КоличествоОстаток = ?(ТипЗнч(ДокОбъект.Ссылка) = Тип("ДокументСсылка.ПриходныйОрдер"), ВыборкаЗаписей.ОстатокЗапасыНаСкладах, -ВыборкаЗаписей.ОстатокЗапасыНаСкладах);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОстаток%", КоличествоОстаток);
		
		Количество = ?(ТипЗнч(ДокОбъект.Ссылка) = Тип("ДокументСсылка.ПриходныйОрдер"), -ВыборкаЗаписей.КоличествоОстатокЗапасыНаСкладах, ВыборкаЗаписей.КоличествоОстатокЗапасыНаСкладах);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Количество%", Количество);
...
//  ВыборкаЗаписей - ВыборкаИзРезультатаЗапроса - выборка из результата запроса контроля остатков,
//  Отказ - Булево - признак наличия ошибок.
Процедура РезервыКПередачиСУчетомОстатковПоЗапасамНаСкладах(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	
	ЗаголовокОшибки = НСтр("ru = 'Ошибка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru = 'Не хватает запасов с учетом изменения резервов к передаче'");
	ОбщегоНазначения.СообщитьПользователю(ТекстЗаголовкаСообщения, ДокОбъект, , , Отказ);
