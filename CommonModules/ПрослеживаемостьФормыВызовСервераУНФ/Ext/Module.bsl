...
Функция ПоместитьРНПТВХранилище(ДокументОбъект, ИдентификаторСтроки, ИдентификаторФормы, ИмяТаблицыПрослеживаемости = "СведенияПрослеживаемости") Экспорт 
	
	СтрокиСРНПТ = ДокументОбъект[ИмяТаблицыПрослеживаемости].НайтиСтроки(Новый Структура("ИдентификаторСтроки", ИдентификаторСтроки)); 
	ТаблицаРНПТ = ДокументОбъект[ИмяТаблицыПрослеживаемости].Выгрузить(СтрокиСРНПТ); 
...
Функция ПоместитьРНПТВХранилищеРаботы(ДокументОбъект, ИдентификаторСтроки, КлючСвязи, ИдентификаторФормы, ИмяТаблицыПрослеживаемости = "СведенияПрослеживаемости") Экспорт
	
	СтрокиСРНПТ = ДокументОбъект[ИмяТаблицыПрослеживаемости].НайтиСтроки(Новый Структура("ИдентификаторСтроки", ИдентификаторСтроки)); 
	
	Если СтрокиСРНПТ.Количество() Тогда
		ТаблицаРНПТ = ДокументОбъект[ИмяТаблицыПрослеживаемости].Выгрузить(СтрокиСРНПТ);
		АдресТаблицыРНПТВХранилище = ПоместитьВоВременноеХранилище(ТаблицаРНПТ, ИдентификаторФормы);
	Иначе
		
		ПараметрыПоиска = Новый Структура("КлючСвязи, ПрослеживаемыйТовар", КлючСвязи, Истина);
		Материалы = ДокументОбъект.Материалы.НайтиСтроки(ПараметрыПоиска);
		
		ТаблицаРНПТ = ДокументОбъект[ИмяТаблицыПрослеживаемости].Выгрузить();
		ТаблицаРНПТ.Очистить();
		Для Каждого СтрокаМатериалов Из Материалы Цикл 
			
			СтрокиСРНПТ = ДокументОбъект[ИмяТаблицыПрослеживаемости].НайтиСтроки(Новый Структура("ИдентификаторСтроки", СтрокаМатериалов.ИдентификаторСтроки));
			
			Для Каждого СтрокаРНПТ Из СтрокиСРНПТ Цикл
				
				НоваяСтрока = ТаблицаРНПТ.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаРНПТ);
				
				НоваяСтрока.Номенклатура = СтрокаМатериалов.Номенклатура;
				НоваяСтрока.СтранаПроисхождения = СтрокаМатериалов.СтранаПроисхождения;
				
				Если ЗначениеЗаполнено(НоваяСтрока.Количество) И Не ЗначениеЗаполнено(НоваяСтрока.КоличествоПрослеживаемости) Тогда
...
Функция ИсходныйДокументРекурсивно(ИсходныйДокумент) Экспорт
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаПоступления")
		ИЛИ ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаРеализации")
		ИЛИ ТипЗнч(Основание) = Тип("ДокументСсылка.АктОРасхождениях")
		ИЛИ ТипЗнч(Основание) = Тип("ДокументСсылка.АктОРасхожденияхПолученный") Тогда
