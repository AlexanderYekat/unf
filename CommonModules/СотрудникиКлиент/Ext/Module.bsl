...
Процедура ПроверитьОднофамильцевДанныеФизическихЛиц(РезультатВыбора, ДополнительныеПараметры) Экспорт 
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		Если ИзФормыСотрудника Тогда
			...
			Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
				...
				МодульПодборПерсоналаКлиент.ПроверитьОднофамильцевСотрудникаСредиКандидатовЗавершение(РезультатВыбора,
					ДополнительныеПараметры);
			Иначе	
				УстановитьФизическоеЛицоВФормеСотрудника(Форма, РезультатВыбора.ФизическоеЛицо);
			КонецЕсли;
		...
		Иначе
			...
			Иначе
				ОбработатьВыборФизическогоЛицаОднофамильца(Форма, РезультатВыбора.ФизическоеЛицо);
			КонецЕсли;
