...
Процедура РасчетПоказателей6НДФЛ(ИмяРасчета, НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные = Ложь, НомерКорректировки = 0) Экспорт
	...
	ИначеЕсли ИмяРасчета = "РасчетПоказателей_6НДФЛ_2024Кв1" Тогда
		РасчетПоказателей_6НДФЛ_2024Кв1(НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные, НомерКорректировки);
...
Процедура РасчетПоказателей_6НДФЛ_2024Кв2(НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные, НомерКорректировки)
	...
	Если ОписаниеТаблиц.Свойство("Раздел1НалогиВозвраты") Тогда
		...
		|	НалогиПоДатам.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ВЫБОР
		|		КОГДА НалогиПоДатам.КрайнийСрокУплаты МЕЖДУ &НачалоГода И &ОкончаниеПериода
		...
		|	0,
		|	ВЫБОР
		|		КОГДА ВозвратыПоДатам.КрайнийСрокУплаты МЕЖДУ &НачалоГода И &ОкончаниеПериода
...
Процедура РасчетПоказателей_6НДФЛ_2024Кв1(НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные, НомерКорректировки)
	...
	Если ОписаниеТаблиц.Свойство("Приложение1Налогоплательщики") Тогда
		...
		Если НомерКорректировки > 0 Тогда
			...
			Иначе 
				РеквизитыРегистрации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(?(ТипЗнч(РегистрацияВНалоговомОргане) = Тип("Массив"), РегистрацияВНалоговомОргане[0], РегистрацияВНалоговомОргане), "КодПоОКТМО,КПП");
				ПолучитьПереданныеДанныеОтчета2024(Организация, СокрЛП(РеквизитыРегистрации.КодПоОКТМО), СокрЛП(РеквизитыРегистрации.КПП), ОкончаниеПериода, Запрос.МенеджерВременныхТаблиц, НомерКорректировки, "ФормаОтчета2024Кв1");
			КонецЕсли;
		КонецЕсли;
