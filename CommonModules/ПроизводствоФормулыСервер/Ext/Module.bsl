...
Процедура ПроверитьФормулу(Ошибки, Формула, ФиксированныеЗначения, ОграничениеТипа, КатегорияНоменклатуры) Экспорт
	...
	Если СтрЧислоВхождений(ТекстФормулы, "{") <>  СтрЧислоВхождений(ТекстФормулы, "}") тогда
		ТекстОшибки = НСтр("ru ='Количество открытых фигурных скобок не равно количеству закрытых.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Формула", ТекстОшибки, "");
	КонецЕсли;
	
	//ПроверитьАрифметическиеОперанды(ТекстФормулы)
	

	ТаблицаОперандов = ПолучитьТаблицуОперандовФормулы(ТекущаяДатаСеанса(), Формула);
	СтруктураФормулы = КонструкторФормулСлужебный.ЭлементыФормулы(СтрЗаменить(ТекстФормулы,Символы.ПС,""));
	//Очистим массив от лишних элементов
	СоставФормулы = Новый Массив;
	Для Каждого Элемент из СтруктураФормулы.ВсеЭлементы Цикл
		Если  СтрЧислоВхождений(ФорматнаяСтрока(),СокрЛП(Элемент)) >0 ИЛИ  СокрЛП(Элемент) = "" Тогда
			Продолжить
		Иначе
			СоставФормулы.Добавить(Элемент);
		КонецЕсли;
	КонецЦикла; 
	МассивОператоров = МассивАрифметическихОператоров();
	МассивЛогическихОпераций = МассивЛогическихОпераций();
	...
	КонецЦикла;
	Если СоответствиеОперандов = Неопределено Тогда
		ТекстОшибки = НСтр("ru ='Формула содержит ошибки операндов, возможно в формуле заданы параметры которых нет у элементов'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Формула", ТекстОшибки, "");
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонСообщения, РасчетныеДанные.ТекстОшибки);
...
//Функция которая определяет тип произвольного значения
Функция ПолучитьТипОперанда(Операнд)
	Попытка
		Возврат ТипЗнч(Булево(Операнд))
	Исключение
		Попытка
			Возврат ТипЗнч(Дата(Операнд))
		Исключение
			Попытка 
				Возврат типЗнч(Число(Операнд))
			Исключение
				Возврат типЗнч(Строка(Операнд))
			КонецПопытки;
		КонецПопытки;
	КонецПопытки;
КонецФункции

//Возвращает строку с лишними символами
Функция ФорматнаяСтрока()
	Возврат "()Цел#ЕСЛИ#ТОГДА#ИНАЧЕЕСЛИ#ИНАЧЕ#КОНЕЦЕСЛИ"+ Символы.ВК + Символы.ВТаб + Символы.НПП + Символы.ПС +
...
//
// Параметры:
//  ЗаказОбъект - ДокументОбъект.ЗаказПокупателя, ДокументОбъект.ЗаказНаПроизводство, ДанныеФормыСтруктура - Объект
//      заказа, при вызове из формы объект должен быть записан.
	...
	Для каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		...
		Для каждого СтрокаСостава Из Данные["Спецификация.Состав"] Цикл
			...
			Если СтрокаСостава.ИспользуетсяСопоставление Тогда
				...
				КонецЕсли;
			ИначеЕсли СтрокаСостава.ИспользуетсяРасширение Тогда
			ПолучитьРасшифровкуПодключаемойПозиции(СтрокаСостава,СтруктураРеквизитов,СтрокаТабличнойЧасти,Данные)	
			Иначе
		...
		Для каждого СтрокаОперации Из Данные["Спецификация.Операции"] Цикл
			...
			КонецЕсли; 
		КонецЦикла; 
		Если СтрокаТабличнойЧасти.Спецификация.ИспользуетсяРасширение Тогда
		ПолучитьРасшифровкуПодключаемойСпецификации(ИзменяемаяИнформация,СтрокаТабличнойЧасти.Спецификация,Данные,СтрокаТабличнойЧасти.Спецификация.ИдентификаторСпецификации)
		КонецЕсли;
	КонецЦикла;
...
// Возвращает таблицу расшифровки данных подключаемого показателя
//
// Параметры:
//     СтрокаСостава - СтрокаТабличнойЧасти - Строка табличной части состав
//     СтрокаТабличнойЧасти - СтрокаТабличнойЧасти - Строка табличной части продукция
//     СтруктураРеквизитов - Структура - Описывает структуру реквизитов которые в дальнейшем будут возвращены и обработаны
//     Данные - Структура - Контекст обрабатываемой операции
// Возвращаемое значение:
//      Структура - Описывает структуру реквизитов которые в дальнейшем будут обработаны в состав
//      Спецификации
//
Функция ПолучитьРасшифровкуПодключаемойПозиции(СтрокаСостава,СтруктураРеквизитов,СтрокаТабличнойЧасти,Данные) Экспорт
	
	ПараметрыПодключаемойПроцедуры = СформироватьПараметрыПодключаемойПроцедуры(СтрокаСостава, СтруктураРеквизитов, Данные);
	
	ИмяПроцедуры = ИмяПроцедурыПоИдентификатору(СтрокаСостава.ИдентификаторСпецификации, Истина);
	
	Попытка
		ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяПроцедуры, ПараметрыПодключаемойПроцедуры);
	Исключение
		ТекстОшибки = "Процедура расшифровки спецификации %1 не найдена. Проверьте подключено ли расширение.";
		ЗаписьЖурналаРегистрации(НСтр("ru='ПроизводствоФормулыСервер.ПолучитьРасшифровкуПодключаемойПозиции'",
								ОбщегоНазначения.КодОсновногоЯзыка()),
								УровеньЖурналаРегистрации.Ошибка,
								Метаданные.ОбщиеМодули.ПроизводствоФормулыСервер,
								,
								СтрШаблон(ТекстОшибки, СтрокаТабличнойЧасти.Спецификация));
	КонецПопытки;
...
//
// Параметры:
//     Состав - ТабличнаяЧасть - Соастав спецификации;
//     Операции - ТабличнаяЧасть - Операции спецификации;
//     Спецификация - СправочникСсылка - Ссылка на спецификацию;
//     Данные - Структура - Контекст обрабатываемой операции;
//     ИдентификаторСпецификации - УникальныйИдентификатор - Идентификатор спецификации
//	ИзменяемаяИнформация - Соответсвие - данное соответсвие далее преобразовывается в состав спецификации.
//
Процедура ПолучитьРасшифровкуПодключаемойСпецификации(ИзменяемыеДанные,Спецификация,Данные,ИдентификаторСпецификации) Экспорт
	
	ПараметрыПодключаемойПроцедуры = СформироватьПараметрыПодключаемойПроцедурыСпецификации(ИзменяемыеДанные,Спецификация,Данные);
	
	ИмяПроцедуры = ИмяПроцедурыСпецификацииПоИдентификатору(ИдентификаторСпецификации, Истина);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='ПроизводствоФормулыСервер.ПолучитьРасшифровкуПодключаемойПозиции'",
								...
								Метаданные.ОбщиеМодули.ПроизводствоФормулыСервер,
								,
								СтрШаблон(ТекстОшибки, ИдентификаторСпецификации));
	КонецПопытки;
...
Функция ВыбратьПараметрические(Спецификации) Экспорт
	...
	|			ИЛИ Спецификации.ИспользуетсяРасширение)";
...
Процедура ПроверитьЗаполнениеДопРеквизитовСпецификаций(Объект, ИмяТЧ, Отказ) Экспорт
	...
	|		ПО (ДополнительныеРеквизитыОбщие.Ссылка = &НаборСвойств_Справочник_Спецификации_Общие)
	|ГДЕ
...
Функция МассивАрифметическихОператоров() Экспорт
	МассивОператоров = Новый Массив;
	МассивОператоров.Добавить("+");
	МассивОператоров.Добавить("-");
	МассивОператоров.Добавить("*");
	МассивОператоров.Добавить("/");
	МассивОператоров.Добавить("%");
	...
	МассивОператоров.Добавить("<=");
	МассивОператоров.Добавить("<>");
	МассивОператоров.Добавить("Макс");
	МассивОператоров.Добавить("Мин");
	МассивОператоров.Добавить("Цел");
	МассивОператоров.Добавить("Окр");
	МассивОператоров.Добавить("%1");
	МассивОператоров.Добавить("%5");
	МассивОператоров.Добавить("%20");
	МассивОператоров.Добавить("%50");
	МассивОператоров.Добавить("НЕОПРЕДЕЛЕНО ");
	Возврат МассивОператоров;
КонецФункции

Функция МассивЛогическихОпераций() Экспорт
	МассивОператоров = Новый Массив;
	МассивОператоров.Добавить("=");
	Возврат МассивОператоров;
КонецФункции

Функция МассивУсловныхОператоров() Экспорт
	МассивОператоров = Новый Массив;
	МассивОператоров.Добавить("#ЕСЛИ");
	МассивОператоров.Добавить("#ИНАЧЕЕСЛИ");
	МассивОператоров.Добавить("#ТОГДА");
	МассивОператоров.Добавить("#ИНАЧЕ");
	МассивОператоров.Добавить("#КОНЕЦЕСЛИ");
	Возврат МассивОператоров;
КонецФункции
...
Функция СформироватьПараметрыПодключаемойПроцедуры(СтрокаСостава, СтруктураРеквизитов, Данные)
	
		
	ПараметрыПроцедуры = Новый Массив;
	ПараметрыПроцедуры.Добавить(СтрокаСостава);
	ПараметрыПроцедуры.Добавить(СтруктураРеквизитов);
...
Функция СформироватьПараметрыПодключаемойПроцедурыСпецификации(ИзменяемыеДанные,Спецификация,Данные)
	
		
	ПараметрыПроцедуры = Новый Массив;
	ПараметрыПроцедуры.Добавить(ИзменяемыеДанные);
	ПараметрыПроцедуры.Добавить(Спецификация);
	ПараметрыПроцедуры.Добавить(Данные);
	Возврат ПараметрыПроцедуры;
	
КонецФункции

Функция ИмяПроцедурыПоИдентификатору(ИдентификаторПоказателя, ЭтоРасшифровка = Ложь)
	...
	АдаптированныйИдентификатор = СтрЗаменить(АдаптированныйИдентификатор," ","");
		...
		ИмяПроцедуры = СтрШаблон("ПодключаемыеПроцедуры.ПолучитьРасшифровкуПодключаемойПозиции_%1", АдаптированныйИдентификатор);
...
Функция ИмяПроцедурыСпецификацииПоИдентификатору(ИдентификаторПоказателя, ЭтоРасшифровка = Ложь)
	
	АдаптированныйИдентификатор = СтрЗаменить(ИдентификаторПоказателя, "-", "_");
	АдаптированныйИдентификатор = СтрЗаменить(АдаптированныйИдентификатор," ","");
	
		ИмяПроцедуры = СтрШаблон("ПодключаемыеПроцедуры.ПолучитьРасшифровкуПодключаемойСпецификации_%1", АдаптированныйИдентификатор);
...
Функция СоответствиеОперандовДляПроверки(КатегорияНоменклатуры)
	...
	Если ЗначениеЗаполнено(КатегорияНоменклатуры) Тогда
		...
		Схема.Параметры.НаборСвойств_Справочник_Номенклатура_Общие.Значение = УправлениеСвойствами.НаборСвойствПоИмени(
...
Функция ТекстЗапросаВыборкиДанных()
	...
	|		Свойство.Представление КАК Представление) КАК ДополнительныеРеквизиты
	|ИЗ
	...
	|		ИспользуетсяРасширение,
	|		ИдентификаторСпецификации) КАК Состав,
	...
	|		КлючСвязи КАК КлючСвязи,
	|		ИспользуетсяСопоставление КАК ИспользуетсяСопоставление,
	|		Описание КАК Описание) КАК Операции,
	...
	|		КлючСтрокиПравил КАК КлючСтрокиПравил) КАК СопоставлениеСостава,
	...
	|		РеквизитСопоставления КАК РеквизитСопоставления,
	|		ЗначениеРеквизита КАК ЗначениеРеквизита,
	|		КлючСтрокиПравил КАК КлючСтрокиПравил) КАК СопоставлениеОпераций,
	|	Спецификации.ФиксированныеЗначения.(
	|		НомерСтроки КАК НомерСтроки,
	|		Идентификатор КАК Идентификатор,
	|		Значение КАК Значение) КАК ФиксированныеЗначения
	|ИЗ
	|	Справочник.Спецификации КАК Спецификации
	|ГДЕ
	...
	|		Свойство КАК Свойство,
	|		Значение КАК Значение,
	|		ТекстоваяСтрока КАК ТекстоваяСтрока) КАК ДополнительныеРеквизиты,
	...
	|		КлючСвязи КАК КлючСвязи,
	|		ПодразделениеЗавершающегоЭтапа КАК ПодразделениеЗавершающегоЭтапа) КАК Продукция
	|ИЗ
...
Процедура РасчетДанныхПоФормуле(Знач ФормулаСтрокой, СтруктураОперандов, РасчетныеДанные)
	...
	КонецЕсли;
    Если СтрНайти(ФормулаСтрокой, "#ЕСЛИ") > 0 Тогда
	МассивСлов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивСлов(ФормулаСтрокой);
	МассивКонецЕсли = Новый Массив;
	ОткрытыеИначеЕсли = 0;
	Для каждого Слово Из МассивСлов Цикл
		
		Если ВРЕГ(Слово) = "ИНАЧЕЕСЛИ" Тогда
			ОткрытыеИначеЕсли = ОткрытыеИначеЕсли + 1;
		КонецЕсли;
		
		Если ВРЕГ(Слово) = "КОНЕЦЕСЛИ" Тогда
			РезСтр = ") ";
			ВремСтр = ") ";
			Для Итератор=1 По ОткрытыеИначеЕсли Цикл
				РезСтр = РезСтр + ВремСтр;
			КонецЦикла;
			МассивКонецЕсли.Добавить(РезСтр);
			ОткрытыеИначеЕсли = 0;
		КонецЕсли;
	...
	ФормулаСтрокой = СтрЗаменить(ФормулаСтрокой, "#ИНАЧЕЕСЛИ",	",?(");
	ФормулаСтрокой = СтрЗаменить(ФормулаСтрокой, "#ИНАЧЕ",		",");
	ФормулаСтрокой = СтрЗаменить(ФормулаСтрокой, Символы.ПС,	"");
		
	
	СтрокаИначеЕсли = "#КОНЕЦЕСЛИ";
	ДлинаОперанда = СтрДлина(СтрокаИначеЕсли);
	Для каждого Операнд Из МассивКонецЕсли Цикл
		Позиция = СтрНайти(ФормулаСтрокой, СтрокаИначеЕсли);
		ФормулаСтрокой = Лев(ФормулаСтрокой,Позиция-1) + Операнд + Сред(ФормулаСтрокой, Позиция + ДлинаОперанда);
	КонецЦикла;
	КонецЕсли;
	Попытка
		Если не Найти(ФормулаСтрокой,"Параметры.Результат = ") Тогда 
			ФормулаСтрокой = "Параметры.Результат = " + ФормулаСтрокой
		КонецЕсли;
		ФормулаСтрокой = СтрЗаменить(ФормулаСтрокой,"{","");
		ФормулаСтрокой = СтрЗаменить(ФормулаСтрокой,"}","");
		ПараметрыРасчета.Вставить("Результат",РасчетныеДанные.Результат);
		ОбщегоНазначения.ВыполнитьВБезопасномРежиме(ФормулаСтрокой, ПараметрыРасчета);
		РасчетныеДанные.Результат = ПараметрыРасчета.Результат;
	Исключение
