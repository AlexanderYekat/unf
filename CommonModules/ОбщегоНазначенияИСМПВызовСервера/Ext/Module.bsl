...
Функция ПолучитьСокращеннуюИнформациюПоКМ(ПолныйКодМаркировки, НормализованныйШтрихкод, ВидПродукции, СоставКодаМаркировки, ПредставлениеНоменклатуры, ПараметрыСканирования, ЭтоОперацияПробития = Истина) Экспорт
	...
	Результат = ИнтерфейсИСМПОбщегоНазначения.СокращеннаяИнформацияПоКМПриРозничнойПродаже(ТаблицаМаркировки, ПараметрыСканирования.Организация, ПараметрыСканирования.НомерФискальногоНакопителя,, ЭтоОперацияПробития);
	...
	Иначе
		...
		Если Не ДанныеКодаМаркировки = Неопределено Тогда
			...
			СтруктураВозврата.РазрешительныйРежимДатаЗапросаГИСМТ = Формат(ОбщегоНазначенияИС.ДатаВСтрокуUNIX(ТекущаяДатаСеанса()), "ЧРГ=; ЧГ=0;");
...
Процедура ПолучитьСокращеннуюИнформациюПоКМДлительнаяОперация(Параметры, АдресРезультата) Экспорт
	...
	Для Каждого ЭлементДанных Из Параметры.ДанныеДляЗапроса Цикл
		...
		Результат.Вставить(ЭлементДанных.ИндексЭлемента, СтруктураДанныхГИСМТ);
