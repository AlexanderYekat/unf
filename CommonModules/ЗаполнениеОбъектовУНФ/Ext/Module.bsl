...
Процедура ЗаполнитьДокумент(ДокументОбъект, Знач ДанныеЗаполнения, Знач СтратегияЗаполнения = Неопределено, ИсключаяСвойства = "") Экспорт
	...
	Справочники.Проекты.ЗаполнитьПроектВДокументе(ДанныеЗаполнения, ДокументОбъект);
	...
	Если ДанныеЗаполнения.Свойство("ТолькоДополнитьЗначениямиИзНастроек")
		И ДанныеЗаполнения.ТолькоДополнитьЗначениямиИзНастроек Тогда
		ЗаполнитьЗначенияСвойств(ДокументОбъект, ДанныеЗаполнения);
		Возврат;
	КонецЕсли;
...
Функция ЭтоЗаполнениеПоШаблону(ДанныеЗаполнения) Экспорт
	...
	Если ТипЗнч(ШаблонДокумента) <> Тип("СправочникСсылка.ШаблоныДокументовУНФ") Тогда
		Возврат Ложь;
	КонецЕсли;
...
// Заполнить договор по умолчанию.
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект - документ, в котором требуется заполнить договор по-умолчанию.
Процедура ЗаполнитьДоговорПоУмолчанию(ДокументОбъект) Экспорт
	...
	ВестиРасчетыПоДоговорам = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.Контрагент,
		"ВестиРасчетыПоДоговорам");
	ВестиРасчетыПоДоговорам = ОписаниеТиповБулево.ПривестиЗначение(ВестиРасчетыПоДоговорам);
...
// Функция возвращает признак необходимости определять положение ответственного в документе (шапка или табличная часть)
//
// Параметры:
//  Документ - ДокументСсылка - Ссылка на документ, для которого проверяется возможность указания ответственного в
//                              табличной части
// 
// Возвращаемое значение:
//  Булево - Возможность указания ответственного в табличной части
//
Функция ЭтоДокументОтгрузкиСПоложениемОтветственного(Документ) Экспорт
...
// Удаляет реквизит "Договор" из массива проверяемых реквизитов когда расчеты по договорам не ведутся.
// 
// Параметры:
// 	ПроверяемыеРеквизиты - Массив из Строка - проверяемые реквизиты
// 	ДокументОбъект - ДокументОбъект - документ, для которого отключается проверка заполнения реквизита "Договор"
Процедура УдалитьПроверяемыйРеквизитДоговор(ПроверяемыеРеквизиты, ДокументОбъект) Экспорт
	...
	УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Договор");
...
Процедура УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, УдаляемыйРеквизит) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(ПроверяемыеРеквизиты, УдаляемыйРеквизит);
...
Процедура ДополнитьЗначениямиИзНастроек(ДанныеЗаполнения, ДокументОбъект)
	...
	ДополнитьПоложениеДатыПоступленияВЗаказеПокупателя(ЗначенияИзНастроек, ДокументОбъект);
	...
	ДополнитьПоложениеНастроекНалоговогоУчета(ЗначенияИзНастроек, ДокументОбъект);
...
Процедура ДополнитьПоложениеОтветственный(ЗначенияИзНастроек, ДокументОбъект)
	...
	Если НЕ ЭтоДокументОтгрузкиСПоложениемОтветственного(ДокументОбъект.Ссылка) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДополнитьПоложениеДатыПоступленияВЗаказеПокупателя(ЗначенияИзНастроек, ДокументОбъект)
	
	Если ТипЗнч(ДокументОбъект.Ссылка) <> Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		Возврат;
	КонецЕсли;
	
	Если ДокументОбъект.ВидОперации <> Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПереработку Тогда
		Возврат;
	КонецЕсли;
	
	Если ОтсутствуетНезаполненныйРеквизит("ПоложениеДатыПоступления", ДокументОбъект) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПоложениеДатыПоступленияПоУмолчанию) Тогда
		ЗначенияИзНастроек.Вставить("ПоложениеДатыПоступления", ПоложениеДатыПоступленияПоУмолчанию);
	Иначе
		ЗначенияИзНастроек.Вставить("ПоложениеДатыПоступления", Перечисления.ПоложениеРеквизитаНаФорме.ВШапке);
	КонецЕсли;
...
Процедура ДополнитьПоложениеНастроекНалоговогоУчета(ЗначенияИзНастроек, ДокументОбъект)
	
	Если ОтсутствуетНезаполненныйРеквизит(
		"ПоложениеНастроекНалоговогоУчета",
		ДокументОбъект) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПоложениеНастроекНалоговогоУчета) Тогда
		ЗначенияИзНастроек.Вставить("ПоложениеНастроекНалоговогоУчета", ПоложениеНастроекНалоговогоУчета);
	Иначе
		ЗначенияИзНастроек.Вставить("ПоложениеНастроекНалоговогоУчета", Перечисления.ПоложениеРеквизитаНаФорме.ВШапке);
	КонецЕсли;
...
Процедура ДополнитьПредопределеннымиЭлементамиСправочников(ДанныеЗаполнения, ДокументОбъект)
	...
	ДополнитьПервымДоступнымВидомЦенВозвратов(ПредопределенныеЭлементыСправочников, ДокументОбъект);
...
Процедура ДополнитьПредопределеннымиСтруктурнымиЕдиницами(ПредопределенныеЭлементыСправочников, ДокументОбъект)
	...
	Если Не ОтсутствуетНезаполненныйРеквизит("СтруктурнаяЕдиница", ДокументОбъект) Тогда
		...
		Если ЗначениеЗаполнено(ПредопределеннаяСтруктурнаяЕдиница)
			...
			Реквизит = ДокументОбъект.Ссылка.Метаданные().Реквизиты["СтруктурнаяЕдиница"];
			Если ТипСтруктурнойЕдиницыЕстьВПараметрахВыбора(Реквизит.ПараметрыВыбора) Тогда
				ТипСтруктурнойЕдиницы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПредопределеннаяСтруктурнаяЕдиница, "ТипСтруктурнойЕдиницы", Истина);
				Если ТипСтруктурнойЕдиницыВПараметрыВыбора(ТипСтруктурнойЕдиницы, Реквизит.ПараметрыВыбора) Тогда
					ПредопределенныеЭлементыСправочников.Вставить(
					"СтруктурнаяЕдиница",
...
Процедура ДополнитьПервымДоступнымВидомЦенВозвратов(ПредопределенныеЭлементыСправочников, ДокументОбъект)
	
	Если ОтсутствуетНезаполненныйРеквизит("ВидЦенВозврата", ДокументОбъект) Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПриходнаяНакладная") Тогда
		...
		|	Справочник.ВидыЦен КАК ВидыЦен
		|ГДЕ
		|	ВидыЦен.ИмяПредопределенныхДанных = &ИмяПредопределенныхДанныхВидаЦен
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|ИЗ
		|	Справочник.ВидыЦен КАК ВидыЦен
		|ГДЕ НЕ ВидыЦен.Недействителен");
		
		Запрос.УстановитьПараметр("ИмяПредопределенныхДанныхВидаЦен", "Оптовая");
		
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РасходнаяНакладная") Тогда
	...
	Выборка = РезультатЗапроса.Выбрать();
	Если Выборка.Следующий() Тогда
		ПредопределенныеЭлементыСправочников.Вставить("ВидЦенВозврата", Выборка.Ссылка);
	КонецЕсли; 
...
Процедура РазыменоватьПоляСобытияКонтрагент(РазыменованныеПоля)
	...
	|			ИЛИ СобытиеУчастники.Контакт ССЫЛКА Справочник.АдресатыПисем
	|				И (СвязиАдресатКонтактCRM.КонтактCRM ССЫЛКА Справочник.Контрагенты
	|					ИЛИ СвязиАдресатКонтактCRM.КонтактCRM ССЫЛКА Справочник.КонтактныеЛица
	|					ИЛИ СвязиАдресатКонтактCRM.КонтактCRM ССЫЛКА Справочник.Лиды))
	...
	Если ТипЗнч(Выборка.Контакт) = Тип("СправочникСсылка.АдресатыПисем") Тогда
		КонтактСобытия = Выборка.КонтактCRM;
	Иначе
		КонтактСобытия = Выборка.Контакт;
	КонецЕсли;
	
	Если ТипЗнч(КонтактСобытия) = Тип("СправочникСсылка.Контрагенты") Тогда
		
		Контрагент = КонтактСобытия;
		
	ИначеЕсли ТипЗнч(КонтактСобытия) = Тип("СправочникСсылка.КонтактныеЛица") Тогда
		
		КонтрагентыКонтакта = Справочники.КонтактныеЛица.СвязанныеКонтрагенты(КонтактСобытия);
		Если КонтрагентыКонтакта.Количество() = 1 Тогда
			Контрагент = КонтрагентыКонтакта[0];
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(КонтактСобытия) = Тип("СправочникСсылка.Лиды") Тогда
		
		Контрагент = Справочники.Лиды.ПолучитьСоздатьКонтрагента(КонтактСобытия);
...
Функция ТипСтруктурнойЕдиницыЕстьВПараметрахВыбора(ПараметрыВыбора)
