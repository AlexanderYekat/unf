...
Процедура ФормаОтчетаПриЗагрузкеВариантаНаСервере(Форма, НовыеНастройкиКД) Экспорт
	...
	// Отключение блокировки изменения периода в контекстных отчетах
	Если ТипЗнч(Форма.ФормаПараметры.Отбор) = Тип("Структура") Тогда
		ИменаПараметров = СтрРазделить("Период,СтПериод,НачалоПериода,КонецПериода", ",");
		Для каждого ИмяПараметра Из ИменаПараметров Цикл
			Если Форма.ФормаПараметры.Отбор.Свойство(ИмяПараметра) Тогда
				Форма.ФормаПараметры.Отбор.Удалить(ИмяПараметра)
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ОбновитьЗначенияПараметровПериода(Настройки, ЭтоОтчетУНФ = Истина) Экспорт
	...
	СтПериодИспользование = (ПараметрСтПериод <> Неопределено И ПараметрСтПериод.Использование 
		И ТипЗнч(ПараметрСтПериод.Значение) = Тип("СтандартныйПериод"));
	ПараметрПериод = Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Период"));
	ПериодИспользование = (ПараметрПериод <> Неопределено И ПараметрПериод.Использование 
		И ТипЗнч(ПараметрПериод.Значение) = Тип("СтандартныйПериод"));
	СрезИспользование = (ПараметрПериод <> Неопределено И ПараметрПериод.Использование 
		И ТипЗнч(ПараметрПериод.Значение) = Тип("Дата"));
	ПараметрНачалоПериода = Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("НачалоПериода"));
	НачалоПериодаИспользование = (ПараметрНачалоПериода <> Неопределено И ПараметрНачалоПериода.Использование 
		И (ТипЗнч(ПараметрНачалоПериода.Значение) = Тип("Дата") 
		ИЛИ ТипЗнч(ПараметрНачалоПериода.Значение) = Тип("СтандартнаяДатаНачала")));
	ПараметрКонецПериода = Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("КонецПериода"));
	КонецПериодаИспользование = (ПараметрКонецПериода <> Неопределено И ПараметрКонецПериода.Использование 
		И (ТипЗнч(ПараметрКонецПериода.Значение) = Тип("Дата") 
		ИЛИ ТипЗнч(ПараметрКонецПериода.Значение) = Тип("СтандартнаяДатаНачала")));
	
	Если НЕ ЭтоОтчетУНФ Тогда
		
		НачалоПериода = '0001-01-01';
		КонецПериода = '0001-01-01';
		Если СтПериодИспользование Тогда
			...
			КонецЕсли;
		ИначеЕсли ПериодИспользование Тогда
			...
			КонецЕсли;
		ИначеЕсли КонецПериодаИспользование Тогда
			...
			ИначеЕсли ТипЗнч(ПараметрКонецПериода.Значение) = Тип("СтандартнаяДатаНачала") 
				И ЗначениеЗаполнено(ПараметрКонецПериода.Значение) Тогда
			...
			Если ПараметрНачалоПериода <> Неопределено И ПараметрНачалоПериода.Использование Тогда
				...
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли СрезИспользование Тогда
		...
		Если КонецПериода >= НачалоПериода Тогда
			Если ПараметрСтПериод <> Неопределено И ТипЗнч(ПараметрСтПериод.Значение) = Тип("СтандартныйПериод") 
				И СтПериодИспользование Тогда
				ПараметрСтПериод.Значение = Новый СтандартныйПериод(НачалоПериода, КонецПериода);
				ПараметрСтПериод.Использование = Истина;
			КонецЕсли;
			Если ПараметрПериод <> Неопределено И ТипЗнч(ПараметрПериод.Значение) = Тип("СтандартныйПериод") 
				И ПериодИспользование Тогда
				ПараметрПериод.Значение = Новый СтандартныйПериод(НачалоПериода, КонецПериода);
				ПараметрПериод.Использование = Истина;
			КонецЕсли;
			Если ПараметрПериод <> Неопределено И ТипЗнч(ПараметрПериод.Значение) = Тип("Дата") 
				И НЕ СтПериодИспользование Тогда
				ПараметрПериод.Значение = КонецПериода;
				ПараметрПериод.Использование = Истина;
			КонецЕсли;
			Если ПараметрКонецПериода <> Неопределено И ТипЗнч(ПараметрКонецПериода.Значение) = Тип("Дата") 
				И НЕ СтПериодИспользование И НЕ ПериодИспользование Тогда
				ПараметрКонецПериода.Значение = КонецПериода;
				ПараметрКонецПериода.Использование = Истина;
			КонецЕсли;
			Если ПараметрКонецПериода <> Неопределено И ТипЗнч(ПараметрКонецПериода.Значение) = Тип("СтандартнаяДатаНачала") 
				И НЕ СтПериодИспользование И НЕ ПериодИспользование Тогда
				ПараметрКонецПериода.Значение = Новый СтандартнаяДатаНачала(КонецПериода);
				ПараметрКонецПериода.Использование = Истина;
			КонецЕсли;
			Если ПараметрНачалоПериода <> Неопределено И ТипЗнч(ПараметрНачалоПериода.Значение) = Тип("Дата") 
				И НЕ СтПериодИспользование И НЕ ПериодИспользование Тогда
				ПараметрНачалоПериода.Значение = НачалоПериода;
				ПараметрНачалоПериода.Использование = Истина;
			КонецЕсли;
			Если ПараметрНачалоПериода <> Неопределено И ТипЗнч(ПараметрНачалоПериода.Значение) = Тип("СтандартнаяДатаНачала") 
				И НЕ СтПериодИспользование И НЕ ПериодИспользование Тогда
	...
	// Срез на дату включает последнюю секунду дня
...
// Добавление команды отчета Копирование строки в документы
//
// Параметры:
//  Форма	 - 	ФормаКлиентскогоПриложения - форма отчета
//
Процедура ДобавитьКомандуКопироватьСтроки(Форма) Экспорт

	Перем ПереопределяемыеКоманды, ДанныеКоманды;

	Если Не Форма.НастройкиОтчета.Свойство("ПереопределяемыеКоманды", ПереопределяемыеКоманды) Тогда
		Возврат;
	КонецЕсли;

    Группа	   			= Форма.Элементы.КонтекстноеМенюОбластиДанныхБуферОбмена;
	ИмяКоманды 			= "КопироватьСтрокиВБуфер";
	
	Если ПереопределяемыеКоманды.Свойство(ИмяКоманды, ДанныеКоманды) Тогда
		
		КомандаФормы 				= Форма.Команды.Добавить(ИмяКоманды);
		КомандаФормы.Действие 		= "Подключаемый_Команда"; 
		КомандаФормы.Заголовок 		= НСтр("ru = 'Копировать строки в документ'");

		НовыйЭлемент 				= Форма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), Группа);
		НовыйЭлемент.Вид 			= ВидКнопкиФормы.КнопкаКоманднойПанели;
		НовыйЭлемент.ИмяКоманды 	= ИмяКоманды;
		НовыйЭлемент.Картинка		= БиблиотекаКартинок.КопированиеСтрок;

		Настройки				  = Форма.Отчет.КомпоновщикНастроек.Настройки;
		ПолеКоличество			  = "";
		
		ДанныеКоманды.Вставить("ВариантыКоманды", Новый СписокЗначений);
		
		ВариантыКоманды			  = Новый Массив;
		ВариантыКоманды.Добавить(ПолеКоличество);
		
		Если ДанныеКоманды.СоответствиеСтрок.Свойство("Количество", ПолеКоличество) Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВариантыКоманды, СтрРазделить(ПолеКоличество, ","));
		КонецЕсли;		
		
		Для Ном = 1 По ВариантыКоманды.Количество() Цикл
			
			ДанныеКомандыМеню			= ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ДанныеКоманды);
            ВариантКоманды				= ВариантыКоманды[Ном-1];
			
			Если ЗначениеЗаполнено(ВариантКоманды) Тогда
				ДанныеКомандыМеню.СоответствиеСтрок.Вставить("Количество", ВариантКоманды);	
			Иначе
				ДанныеКомандыМеню.СоответствиеСтрок.Удалить("Количество");
			КонецЕсли;
			
			ПредставлениеКомандыМеню	= Новый Массив;
            КомандаИспользуется			= Истина;
			
			Для Каждого КлючСоответствия Из ДанныеКомандыМеню.СоответствиеСтрок Цикл
				
				ПолеВыбора = Настройки.ДоступныеПоляВыбора.Элементы.Найти(Новый ПолеКомпоновкиДанных(КлючСоответствия.Значение));

				Если ПолеВыбора = Неопределено Тогда
					КомандаИспользуется = Ложь;
					Прервать;
				КонецЕсли;
				
				ПредставлениеКомандыМеню.Добавить(ПолеВыбора.Заголовок);
				
			КонецЦикла;

			Если КомандаИспользуется Тогда
				ДанныеКоманды.ВариантыКоманды.Добавить(ВариантКоманды, СтрСоединить(ПредставлениеКомандыМеню, ", "));
			КонецЕсли;
...
//  Форма	 - 	ФормаКлиентскогоПриложения - форма отчета
//
Процедура ДобавитьКомандуИзменитьВыделенное(Форма) Экспорт
    ...
    Группа	   		= Форма.Элементы.КонтекстноеМенюОбластиДанныхБуферОбмена;
	ИмяКоманды 		= "ГрупповоеИзменениеОбъектов";
	
	Если ПереопределяемыеКоманды.Свойство(ИмяКоманды) Тогда
		
		КомандаФормы 			  = Форма.Команды.Добавить(ИмяКоманды);
		КомандаФормы.Действие 	  = "Подключаемый_Команда"; 
		КомандаФормы.Заголовок 	  = НСтр("ru = 'Изменить выделенное'");
		КомандаФормы.Подсказка    = НСтр("ru = 'Изменить выделенные поля отчета'");

		НовыйЭлемент 			  = Форма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), Группа);
		НовыйЭлемент.Вид 		  = ВидКнопкиФормы.КнопкаКоманднойПанели;
		НовыйЭлемент.ИмяКоманды   = ИмяКоманды;
		НовыйЭлемент.Отображение  = ОтображениеКнопки.Авто;
...
#Область УстаревшиеПроцедурыИФункции
 ...
 // Устарела. Преобразовать настройки варианта, сохраненные не библиотечной формой отчетов
//
// Параметры:
//  Форма			 - ФормаКлиентскогоПриложения	 - Форма отчета
...
Процедура ПереопределитьТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы) Экспорт
	...
	Настройки = Новый Структура;
	Настройки.Вставить("ОсновнойТип", Тип("СправочникСсылка.КонтактныеЛица"));
	Настройки.Вставить("ВидКИ", УправлениеКонтактнойИнформацией.ВидКонтактнойИнформацииПоИмени("EmailКонтактногоЛица"));
	РассылкаОтчетов.ДобавитьЭлементВТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы, Настройки);
...
Функция ТекстЗапросаТаблицаОтчетов(Параметры)
	...
	Иначе
		...
		|	ПредопределенныеВариантыОтчетовРазмещение.Ссылка.Включен
		|		И ПредопределенныеВариантыОтчетовРазмещение.Ссылка.ВидимостьПоУмолчанию КАК Использование,
