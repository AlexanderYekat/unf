...
//
Процедура ПерезаполнитьЦеныТабличнойЧастиПоВидуЦен(ФормаДокумента, ИмяТабличнойЧасти, ПересчитатьСкидки = Ложь, 
	ПересчитыватьЦеныСЗаказом = Истина) Экспорт
	...
	СтруктураДанных.Вставить("ЭтоНовыйОбъект", 		НЕ ЗначениеЗаполнено(ФормаДокумента.Объект.Ссылка));
	
	ЕстьВидОперации = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаДокумента.Объект, "ВидОперации");
	
	Если ЕстьВидОперации И ФормаДокумента.Объект.ВидОперации 
			= ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя") Тогда
			
		СтруктураДанных.Вставить("ВидЦен",			ФормаДокумента.Объект.ВидЦенВозврата);
	...
	Для каждого СтрокаТЧ Из ФормаДокумента.Объект[ИмяТабличнойЧасти] Цикл
		...
		ЦенообразованиеКлиентСервер.ДополнитьСтруктуруДанныхНоменклатурыПолямиЦенообразования(ФормаДокумента.Объект, СтрокаТЧ, СтрокаТабличнойЧасти, СкладВШапке, ПрименитьОбщийВидЦен);
	...
	Для каждого СтрокаТЧ Из ТабличнаяЧастьДокумента Цикл
		...
		Если СтрокаТЧ.Свойство("СтруктурнаяЕдиница")
			...
			Если ЗначениеЗаполнено(ИмяРеквизитаСтруктурнаяЕдиница)
				И ЕстьСтруктурнаяЕдиницаВТЧ 
				И НЕ СкладВШапке Тогда
...
Процедура ПерезаполнитьЦеныТабличнойЧастиПоВидуЦенКонтрагента(ФормаДокумента, ИмяТабличнойЧасти) Экспорт
	...
	Если ЕстьВидОперации И ФормаДокумента.Объект.ВидОперации 
		...
		СтруктураДанных.Вставить("ВидЦенКонтрагента",			ФормаДокумента.Объект.ВидЦенВозврата);
...
//  ПараметрыФормыЦеныИВалюта - см. ЦеныИВалютаКлиентСервер.НовыеПараметрыФормыЦеныИВалюта
//  ПрименитьОбщийВидЦен - Булево - если Истина, то будет подобрана структурная единица независимо от объекта. По
//                                  умолчанию Ложь;
//
Процедура ДополнитьСтруктуруВызоваФормыЦеныИВалютыСтруктурнойЕдиницей(Объект, ТабличнаяЧасть,
	ТекущаяСтрокаТабличнойЧасти, ПараметрыФормыЦеныИВалюта, ПрименитьОбщийВидЦен = Ложь) Экспорт
	...
	Если СкладВШапке Тогда
		...
		СтруктурнаяЕдиница = ЦенообразованиеКлиентСервер.ПолучитьЗначениеСкладаВОбъекте(Объект,
			ТекущаяСтрокаТабличнойЧасти, ИмяРеквизита, СкладВШапке);
		ПараметрыФормыЦеныИВалюта.Вставить("СтруктурнаяЕдиница", СтруктурнаяЕдиница);
	...
	Иначе
		...
		Если ТекущаяСтрокаТабличнойЧасти <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ИмяРеквизита) Тогда				
				СтруктурныеЕдиницы.Добавить(ТекущаяСтрокаТабличнойЧасти[ИмяРеквизита]);
				
				Для ИндексСтроки = 1 По ТабличнаяЧасть.Количество() - 1 Цикл
				
					СтрокаТабличнойЧасти = ТабличнаяЧасть[ИндексСтроки];
					СтруктурныеЕдиницы.Добавить(СтрокаТабличнойЧасти[ИмяРеквизита]);
				
				КонецЦикла;
			Иначе
				СтруктурныеЕдиницы.Добавить(Неопределено);
			КонецЕсли;
					
			ПараметрыФормыЦеныИВалюта.Вставить("СтруктурныеЕдиницы", СтруктурныеЕдиницы);
...
// Процедура - Переопределить вид цен возвратов для подбора
//
// Параметры:
//  ПараметрыПодбора - Структура - см. ПодборНоменклатурыВДокументахКлиент.ЗаполнитьЗначенияПараметровПодбораНоменклатуры
//  Объект			 - ДанныеФормыСтруктура - данные формы структура документа для подбора номенклатуры
//
Процедура ПереопределитьВидЦенВозвратовДляПодбора(ПараметрыПодбора, Объект) Экспорт
	...
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПриходнаяНакладная")
		...
		ПараметрыПодбора.ВидЦен = Объект.ВидЦенВозврата;
		
	КонецЕсли;
	
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РасходнаяНакладная")
		И Объект.ВидОперации 
...
// Функция - см. ЦенообразованиеВызовСервера.НовыйСпособВидовЦенДляВозвратовИспользуется
//
// Возвращаемое значение: 
//	Булево - признак использования нового способа видов цен для возвратов
//
...
// Параметры:
//  ПредставлениеНоменклатуры - Соответствие - см. ЦенообразованиеКлиент.ПредставлениеНоменклатурыСтрокой
//  ПараметрыКартинки - Структура - см. ЦенообразованиеКлиент.ПредставлениеНоменклатурыСтрокой
//
// Возвращаемое значение: 
	...
	Для Каждого ГруппаВидЦены Из Форма.ВидыЦен.ПолучитьЭлементы() Цикл
		Для каждого СтрокаВидЦены Из ГруппаВидЦены.ПолучитьЭлементы() Цикл
			...
			Если СтрокаВидЦены.Использование
				И СтрокаВидЦены.Вид = ВидПроизвольныйЗапрос
				И ЗначениеЗаполнено(СтрокаВидЦены.АдресСхемыКомпоновкиДанных) Тогда
