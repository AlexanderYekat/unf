...
Процедура ОткрытьФормуПодбораНоменклатуры(ФормаВладелец, ИмяТабличнойЧасти = "Запасы", ПараметрыПодбора = Неопределено, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	Иначе
		...
		// Добавляет в параметры открытия формы поддбора переданные данные с формы
		Если ТипЗнч(ПараметрыПодбора) = Тип("Структура") И ПараметрыПодбора.Свойство("ПолностьюОбновить") Тогда
			
			Если ПараметрыПодбора.Свойство("ЕстьПартииСНДС") Тогда
				ЕстьПартииСНДС = ПараметрыПодбора.ЕстьПартииСНДС;
			КонецЕсли;
			
			Если ПараметрыПодбора.Свойство("Контрагент") Тогда
				Контрагент = ПараметрыПодбора.Контрагент;
			КонецЕсли;
	...
	ПараметрыПодбора.Вставить("ЕстьПартииСНДС", ЕстьПартииСНДС);
	...
	Если ОповещениеПриЗавершении = Неопределено Тогда
		ОповещениеПриЗавершении = Новый ОписаниеОповещения("ПриЗакрытииПодбора", ЭтотОбъект);
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыВидимостиНаборовКомплектов(ФормаВладелец, ПолноеИмяФормыПодбора, ПараметрыПодбора)
	...
	Если Найти(ПолноеИмяФормыПодбора, "Справочники.Номенклатура.ФормаСписка") > 0 Тогда
		...
		Если СтрНайти(ФормаВладелец.ИмяФормы, "ЗаказПоставщику") > 0
			Или СтрНайти(ФормаВладелец.ИмяФормы, "РасходныйОрдер") > 0
			Или СтрНайти(ФормаВладелец.ИмяФормы, "ПриходныйОрдер") > 0 Тогда
				ПоказыватьНаборыКомплекты = Ложь;
		КонецЕсли;
...
Процедура ЗаполнитьВидыЦенНоменклатуры(ФормаВладелец, ПолноеИмяФормыПодбора, ПараметрыПодбора)
	...
	// Вид цен для реализации
		...
		Если ФормаВладелец.ИмяФормы = "Документ.СчетФактура.Форма.ФормаДокумента" Тогда
			...
			ИначеЕсли ФормаВладелец.ИмяФормы = "Документ.РасходнаяНакладная.Форма.ФормаДокумента" 
				И ФормаВладелец.ЭтотОбъект.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику") Тогда
	...
	ЦенообразованиеКлиент.ПереопределитьВидЦенВозвратовДляПодбора(ПараметрыПодбора, ФормаВладелец.Объект);
