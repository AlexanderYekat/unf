...
Процедура ПриСозданииНаСервере(Форма, Объект, Знач ДополнительныеПараметры = Неопределено, УдалитьПоложениеЗаголовкаКИ = "",
	...
	// УНФ Переименовать переменную ДополнительныеПараметры в ДополнительныеПараметрыВывода.
	...
	ДополнительныеПараметрыВывода.ПоложениеКнопкиДобавить = НастройкиКИ.ПоложениеКнопкиДобавить;
	ДополнительныеПараметрыВывода.ШиринаПоляКомментарий = НастройкиКИ.ШиринаПоляКомментарий;
	...
	// УНФ
	УправлениеКонтактнойИнформациейУНФ.ПриСозданииНаСервере(Форма, Объект, ДополнительныеПараметры);
	// Конец УНФ
...
//    УдалитьОтложеннаяИнициализация - Массив - устарел,следует использовать ДополнительныеПараметры.
//
Процедура ПриСозданииНаСервереСвязанныхВладельцевКИ(Форма , ДанныеВладельцевКИ, ДополнительныеПараметры = Неопределено, УдалитьПоложениеЗаголовкаКИ = "",
	Знач УдалитьИсключаемыеВиды = Неопределено, УдалитьОтложеннаяИнициализация = Ложь) Экспорт
	
	Если ТипЗнч(ДанныеВладельцевКИ) = Тип("ДанныеФормыЭлементКоллекции") Тогда
		КоллекцияДанныеВладельцевКИ = Новый Массив;
		КоллекцияДанныеВладельцевКИ.Добавить(ДанныеВладельцевКИ);
	Иначе
		КоллекцияДанныеВладельцевКИ = ДанныеВладельцевКИ;
	КонецЕсли;
	
	Для каждого СтрокаДанныеВладельцевКИ Из КоллекцияДанныеВладельцевКИ Цикл
		...
		// Кэширование часто используемых значений
		СсылкаНаОбъект             = СтрокаДанныеВладельцевКИ.Ссылка;
		МетаданныеОбъекта          = СсылкаНаОбъект.Метаданные();
		ПолноеИмяОбъектаМетаданных = МетаданныеОбъекта.ПолноеИмя();
		ИмяОбъекта                 = МетаданныеОбъекта.Имя;
		ЭтоОсновнойЯзык            = ОбщегоНазначения.ЭтоОсновнойЯзык();
		
		НастройкиКИ = НастройкиПодсистемы(СтрокаДанныеВладельцевКИ.Ссылка);
		...
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(СтрокаДанныеВладельцевКИ.Ссылка)) Тогда
			ТекстЗапроса = "ВЫБРАТЬ
			|	КонтактнаяИнформация.Представление КАК Представление,
			|	КонтактнаяИнформация.НомерСтроки КАК НомерСтроки,
			|	КонтактнаяИнформация.Вид КАК Вид, 
			...
			КонтактнаяИнформация = Запрос.Выполнить().Выгрузить();
		Иначе
			КонтактнаяИнформация = СтрокаДанныеВладельцевКИ.КонтактнаяИнформация.Выгрузить();
		...
		КонтактнаяИнформация.Сортировать("РеквизитДопУпорядочивания, НомерСтроки");
		СоздатьРеквизитыКонтактнойИнформации(Форма, ДобавляемыеРеквизиты, ИмяОбъекта, РазмещеныНаФорме, КонтактнаяИнформация, 
			ОтложеннаяИнициализация, ОбработкаНавигационнойСсылки, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения);
		...
		ПараметрыКонтактнойИнформации = ПараметрыВыводаКонтактнойИнформации(Форма, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения,
			ПоложениеЗаголовкаКИ, ОтложеннаяИнициализация, ДополнительныеПараметрыВывода);
		ПараметрыКонтактнойИнформации.Владелец                     = СсылкаНаОбъект;
		ПараметрыКонтактнойИнформации.ПараметрыАдреса.ТипПомещения = ТипПомещения;
		ПараметрыКонтактнойИнформации.ОбработкаНавигационнойСсылки = ОбработкаНавигационнойСсылки;
		...
		Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
			Форма.Элементы[СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения].ШрифтЗаголовка = ШрифтыСтиля.ВажнаяНадписьШрифт;
		КонецЕсли;
		
		ИмяГруппы = "ГруппаЗначенияКомандыКИ" + СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения;
		ГруппаЗначенияКомандыКИ = Форма.Элементы.Найти(ИмяГруппы);
			
		Если ГруппаЗначенияКомандыКИ = Неопределено Тогда
			Родитель = Родитель(Форма, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения);
			ГруппаЗначенияКомандыКИ = Форма.Элементы.Добавить(ИмяГруппы, Тип("ГруппаФормы"), Родитель);
			ГруппаЗначенияКомандыКИ.Вид = ВидГруппыФормы.ОбычнаяГруппа;
			ГруппаЗначенияКомандыКИ.Заголовок = НСтр("ru='Значения контактной информации'");
			ГруппаЗначенияКомандыКИ.ОтображатьЗаголовок = Ложь;
		...
		ИмяГруппы = "ГруппаЗначенияКИ"+СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения;
		ГруппаЗначенияКИ = Форма.Элементы.Найти(ИмяГруппы);
		...
		Для Каждого СтрокаКИ Из КонтактнаяИнформация Цикл
			...
			Если СтрокаКИ.ЭтоРеквизитТабличнойЧасти Тогда
				СоздатьЭлементыТабличнойЧасти(Форма, ИмяОбъекта, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения, СтрокаКИ, ДанныеВидаКонтактнойИнформации);
				Продолжить;
			КонецЕсли;
			...
			Если ОтложеннаяИнициализация Тогда
				...
				ДобавитьРеквизитВОписание(Форма, СтрокаКИ, ДанныеВидаКонтактнойИнформации, ЭтоНовыйВидКИ,, 
					СтатическийЭлемент, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения);
				Если СтатическийЭлемент Тогда
					ПодготовитьСтатическийЭлемент(Форма, СтрокаКИ, СозданныеЭлементы, СозданныйЭлемент,
						ПараметрыКонтактнойИнформации.ОтображатьИконки, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения);
				КонецЕсли;
				Продолжить;
			КонецЕсли;
			
			ДобавитьРеквизитВОписание(Форма, СтрокаКИ, ДанныеВидаКонтактнойИнформации, ЭтоНовыйВидКИ,, 
				НЕ СтрокаКИ.ЭтоИсторическаяКонтактнаяИнформация, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения);
			...
			Иначе
				...
				Если НЕ СтрокаКИ.ЭтоИсторическаяКонтактнаяИнформация Тогда
					ДобавитьСтрокуКонтактнойИнформации(Форма, СтрокаКИ, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения, ЭтоНовыйВидКИ, КоличествоАдресов, СледующаяСтрока, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения);
				КонецЕсли;
		...
		ОбновитьКонтекстноеМеню(Форма, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения);
		...
		Если Не ОтложеннаяИнициализация И РазрешитьДобавлениеПолей
			И Форма.ПараметрыКонтактнойИнформации[СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения].СписокДобавляемыхЭлементов.Количество() > 0 Тогда
			ДобавитьКнопкуДополнительногоПоляКонтактнойИнформации(Форма, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения);
		Иначе
			ДобавитьПояснениеПоСбросуНастроекФормы(Форма, СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения, ОтложеннаяИнициализация);
		КонецЕсли;
		
		Если ТипЗнч(Форма.СозданныеИмяЭлементаДляРазмещенияКИ) <> Тип("Структура") Тогда
			Форма.СозданныеИмяЭлементаДляРазмещенияКИ = Новый Структура;
		КонецЕсли;
		Форма.СозданныеИмяЭлементаДляРазмещенияКИ.Вставить(СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения, Истина);
		
	КонецЦикла;
	
	// УНФ
	УправлениеКонтактнойИнформациейУНФ.ПриСозданииНаСервереСвязанныхВладельцевКИ(Форма, ДанныеВладельцевКИ,
		ДополнительныеПараметры);
	// Конец УНФ
	
КонецПроцедуры

Процедура ПриЧтенииНаСервереСвязанныхВладельцевКИ(Форма, ДанныеВладельцевКИ, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация") Экспорт
	...
	Если ТипЗнч(Форма.СозданныеИмяЭлементаДляРазмещенияКИ) = Тип("Структура") Тогда
		Для каждого КлючЗначение Из Форма.СозданныеИмяЭлементаДляРазмещенияКИ Цикл
			Форма.СозданныеИмяЭлементаДляРазмещенияКИ[КлючЗначение.Ключ] = Ложь;
		КонецЦикла;
		Для каждого СтрокаДанныеВладельцевКИ Из КоллекцияДанныеВладельцевКИ Цикл
			Если Форма.СозданныеИмяЭлементаДляРазмещенияКИ.Свойство(СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения) Тогда
				Форма.СозданныеИмяЭлементаДляРазмещенияКИ[СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения] = Истина;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для каждого СтрокаДанныеВладельцевКИ Из КоллекцияДанныеВладельцевКИ Цикл
		СписокРеквизитовФормы = Форма.ПолучитьРеквизиты();
		ИмяЭлементаДляРазмещения = СтрокаДанныеВладельцевКИ.ИмяЭлементаДляРазмещения;
		
		ПервыйЗапуск = Истина;
		ТолькоУдалитьБезПеречитывания = Ложь;
		Если ТипЗнч(Форма.СозданныеИмяЭлементаДляРазмещенияКИ) = Тип("Структура") Тогда
			Если Форма.СозданныеИмяЭлементаДляРазмещенияКИ.Свойство(ИмяЭлементаДляРазмещения) Тогда
				ПервыйЗапуск = Ложь;
				ТолькоУдалитьБезПеречитывания = Не Форма.СозданныеИмяЭлементаДляРазмещенияКИ[ИмяЭлементаДляРазмещения];
			КонецЕсли;
		КонецЕсли;
		
		Если ПервыйЗапуск Тогда
			ПриСозданииНаСервереСвязанныхВладельцевКИ(Форма, СтрокаДанныеВладельцевКИ);
			Продолжить;
		КонецЕсли;
		...
		Иначе
			...
			МассивУдаляемыхРеквизитов = Новый Массив;
			ИмяОбъекта = СтрокаДанныеВладельцевКИ.Ссылка.Метаданные().Имя;
		...
		Если ТолькоУдалитьБезПеречитывания Тогда
			Форма.СозданныеИмяЭлементаДляРазмещенияКИ.Удалить(ИмяЭлементаДляРазмещения);
			Продолжить;
		КонецЕсли;
		...
		ДополнительныеПараметры.ПоложениеЗаголовкаКИ = ПоложениеЗаголовкаКИ;
		ДополнительныеПараметры.ОтложеннаяИнициализация = ОтложеннаяИнициализация;
		ПриСозданииНаСервереСвязанныхВладельцевКИ(Форма, СтрокаДанныеВладельцевКИ, ДополнительныеПараметры);
...
Процедура ПриЗаписиНаСервереСвязанныхВладельцевКИ(Форма, Объект, СтрокаДанныеВладельцаКИ, Отказ = Ложь) Экспорт
	
	КонтактнаяИнформация = КонтактнаяИнформацияИзРеквизитовФормы(Форма, Объект, СтрокаДанныеВладельцаКИ.ИмяЭлементаДляРазмещения);
	Объект.КонтактнаяИнформация.Очистить();
	УстановитьКонтактнуюИнформациюОбъекта(Объект, КонтактнаяИнформация);
...
Процедура СоздатьРеквизитыКонтактнойИнформации(Знач Форма, Знач ДобавляемыеРеквизиты, Знач ИмяОбъекта, Знач РазмещеныНаФорме,
	...
	Знач ПостфиксИмяРеквизита = "")
	// УНФ: Добавлен параметр ПостфиксИмяРеквизита.
		...
		Иначе
			...
			Если СозданныйРеквизит = Неопределено Тогда
				...
				Если Не ОтложеннаяИнициализация Тогда
					...
					// УНФ
					НовыйРеквизитФормы = Новый РеквизитФормы(ОбъектКонтактнойИнформации.ИмяРеквизита + ПостфиксИмяРеквизита,
						ТипРеквизита,, ОбъектКонтактнойИнформации.Наименование, Истина);
					ДобавляемыеРеквизиты.Добавить(НовыйРеквизитФормы);
					// Конец УНФ
					Если ЕстьПолеКомментарий Тогда					
						// УНФ
						НовыйРеквизитКомментарий = Новый РеквизитФормы(
							ОбъектКонтактнойИнформации.ИмяРеквизитаКомментарий + ПостфиксИмяРеквизита, ТипРеквизита,,
							ОбъектКонтактнойИнформации.Наименование, Истина);
	...
	Если НЕ ОбщегоНазначения.ЭтоМобильныйКлиент() И ПараметрыКонтактнойИнформацииЭлемента.ПоложениеКнопкиДобавить = "Авто" Тогда
		...
		Иначе
			...
			ИначеЕсли ПоследняяГруппа <> Неопределено И НЕ ЕстьКнопкаКонтактнойИнформации(ПоследняяГруппа, Истина)
				...
				// УНФ
				Декорация = Форма.Элементы.Добавить("ОтступДобавить" + ИмяЭлементаДляРазмещения, Тип("ДекорацияФормы"),
					ГруппаКоманды);
				// Конец УНФ
...
Процедура ДобавитьСтрокуКонтактнойИнформации(Форма, Результат, ИмяЭлементаДляРазмещения, ЭтоНовыйВидКИ = Ложь,
		...
		ПостфиксИмяРеквизита = "") // УНФ Добавлен параметр ПостфиксИмяРеквизита.
	...
	Если ДобавлениеНовогоЗначения Тогда
		...
		Если ТипЗнч(ДобавляемыйВид)= Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
			...
			ИнформацияОВидеКИ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДобавляемыйВид,
				"Тип, Наименование, ВидРедактирования, ВидПоляДругое, ВводитьНомерПоМаске, МаскаНомераТелефона, ПометкаУдаления, ХранитьИсториюИзменений");
			ИнформацияОВидеКИ.Вставить("Вид", ДобавляемыйВид);
			ИнформацияОВидеКИ.Вставить("Представление", "");
			ИнформацияОВидеКИ.Вставить("Комментарий", "");
			// Конец УНФ
		Иначе
	...
	// УНФ
	ОтборПоВиду = Новый Структура("Вид, ЭтоИсторическаяКонтактнаяИнформация, ИмяЭлементаДляРазмещения", ДобавляемыйВид,
		Ложь, ИмяЭлементаДляРазмещения);
	// Конец УНФ
	...
	Если ДобавлениеНовогоЗначения Тогда
		...
		// УНФ
		Если Результат.Свойство("ПостфиксИмяРеквизита") Тогда
			Если Не СтрЗаканчиваетсяНа(ИмяРеквизита, Результат.ПостфиксИмяРеквизита) Тогда
				ИмяРеквизита = ИмяРеквизита + Результат.ПостфиксИмяРеквизита;
			КонецЕсли;
			Если Не СтрЗаканчиваетсяНа(ИмяРеквизитаКомментарий, Результат.ПостфиксИмяРеквизита) Тогда
				ИмяРеквизитаКомментарий = ИмяРеквизитаКомментарий + Результат.ПостфиксИмяРеквизита;
			КонецЕсли;
		КонецЕсли;
		// Конец УНФ
	...
	// УНФ
	Если ПостфиксИмяРеквизита <> "" Тогда
		Если Не СтрЗаканчиваетсяНа(ИмяРеквизита, ПостфиксИмяРеквизита) Тогда
			ИмяРеквизита = ИмяРеквизита + ПостфиксИмяРеквизита;
		КонецЕсли;
		Если Не СтрЗаканчиваетсяНа(ИмяРеквизитаКомментарий, ПостфиксИмяРеквизита) Тогда
			ИмяРеквизитаКомментарий = ИмяРеквизитаКомментарий + ПостфиксИмяРеквизита;
		КонецЕсли;
	КонецЕсли;
	// Конец УНФ
...
Процедура ДобавитьРеквизитВОписание(Форма, СтрокаКонтактнойИнформации, ДанныеВидаКонтактнойИнформации, ЭтоНовыйВидКИ,
	ЭтоРеквизитТабличнойЧасти = Ложь, ЗаполнятьЗначениеРеквизита = Истина,
	ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация",
	ПостфиксИмяРеквизита = "") // УНФ добавлен параметр ПостфиксИмяРеквизита.
	
	// УНФ
	СтрокаКонтактнойИнформации.ИмяРеквизита = СтрокаКонтактнойИнформации.ИмяРеквизита + ПостфиксИмяРеквизита;
	// Конец УНФ
...
Процедура УдалитьКомандыИЭлементыФормы(Форма, ИмяЭлементаДляРазмещения)
	...
	Для Каждого УдаляемыйЭлемент Из ДобавленныеЭлементы Цикл
		...
		Иначе
			...
			ЭлементСуществует = Форма.Элементы.Найти(УдаляемыйЭлемент.Значение) <> Неопределено;
...
Функция КонтактнаяИнформацияИзРеквизитовФормы(Форма, Объект,
	ИмяЭлементаДляРазмещения = Неопределено) // УНФ добавлен параметр ИмяЭлементаДляРазмещения.
	...
	Для Каждого СтрокаТаблицы Из УправлениеКонтактнойИнформациейКлиентСервер.ОписаниеКонтактнойИнформацииНаФорме(Форма) Цикл
		...
		// УНФ
		Если ИмяЭлементаДляРазмещения <> Неопределено И СтрокаТаблицы.ИмяЭлементаДляРазмещения <> ИмяЭлементаДляРазмещения Тогда
			Продолжить;
		КонецЕсли;
		// Конец УНФ
