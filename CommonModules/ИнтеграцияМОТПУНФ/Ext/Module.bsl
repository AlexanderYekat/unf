...
Процедура ПриЗаполненииИнформацииПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN, ПарамтерыПоискаРМК = Неопределено) Экспорт
	...
	ДанныеПоШтрихкодам.Колонки.Добавить("ЕдиницаИзмерения",	Метаданные.ОпределяемыеТипы.Упаковка.Тип);
	...
	Для Каждого Штрихкод Из ДанныеПоШтрихКодамСоответствие Цикл
		...
		Для Каждого НоменклатураСоответствия Из Штрихкод.Значение Цикл
			...
			Если ЗначениеЗаполнено(Штрихкод.Значение) Тогда
				...
				НоваяСтрока.ЕдиницаИзмерения = НоменклатураСоответствия.ЕдиницаИзмерения;
			КонецЕсли;
	...
	|	ДанныеПоШтрихкодам.ЕдиницаИзмерения КАК ЕдиницаИзмерения
	...
	|	ДанныеПоШтрихкодам.ЕдиницаИзмерения КАК Упаковка,
	|	ЕСТЬNULL(ЕдиницыИзмерения.Коэффициент, 1) КАК Количество,
	|	ПРЕДСТАВЛЕНИЕ(ДанныеПоШтрихкодам.Номенклатура) КАК ПредставлениеНоменклатуры,
	|	ДанныеПоШтрихкодамEAN.ШтрихкодEAN КАК ШтрихкодEAN,
	|	СправочникНоменклатура.ВидПродукцииИС КАК ВидПродукции,
	|	ВЫБОР
	|		КОГДА СправочникНоменклатура.Ссылка ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ &ОпределениеПризнакаМаркируемаяПродукция
	...
	|		ПО (ДанныеПоШтрихкодам.Номенклатура = СправочникНоменклатура.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЕдиницыИзмерения КАК ЕдиницыИзмерения
	|		ПО (ДанныеПоШтрихкодам.ЕдиницаИзмерения = ЕдиницыИзмерения.Ссылка)";
	
	ОпределениеМаркируемаяПродукция = ИнтеграцияИСУНФ.ОпределениеПризнакаМаркируемаяПродукцияТекстаЗапроса("СправочникНоменклатура");
	...
	ДанныеПоШтрихкодамРезультат = ДанныеПоШтрихкодамEAN;
	...
	Если Не ПарамтерыПоискаРМК = Неопределено Тогда
		...
		Если ПарамтерыПоискаРМК.Свойство("ДанныеРучногоСопоставления") Тогда
			...
			Если ТипЗнч(ДанныеРучногоСопоставления.Упаковка) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
				НоваяСтрока.Упаковка = ДанныеРучногоСопоставления.Упаковка;
			КонецЕсли;
			НоваяСтрока.ШтрихкодEAN = ДанныеРучногоСопоставления.Штрихкод;
			НоваяСтрока.ВидПродукции = НоваяСтрока.Номенклатура.ВидПродукцииИС;
			...
			Если ДанныеШтрихкодаРМК.ЗначенияПоиска.Количество() > 0 Тогда
				Для Каждого ЗначениеПоиска Из ДанныеШтрихкодаРМК.ЗначенияПоиска Цикл
					Если ЗначениеПоиска.ТипОбъекта = Тип("СправочникСсылка.ШтрихкодыУпаковокТоваров")
						...
						СтрокаДанных.ВидПродукции              = ЗначениеПоиска.Номенклатура.ВидПродукцииИС;
						...
						СтрокаДанных.Характеристика =
							?(ЗначениеЗаполнено(ЗначениеПоиска.Характеристика), ЗначениеПоиска.Характеристика, СтрокаДанных.Характеристика);
						СтрокаДанных.Упаковка =
							?(ЗначениеЗаполнено(ЗначениеПоиска.Упаковка), ЗначениеПоиска.Упаковка, СтрокаДанных.Упаковка);
						СтрокаДанных.Серия =
							?(ЗначениеЗаполнено(ЗначениеПоиска.СерияНоменклатуры), ЗначениеПоиска.СерияНоменклатуры, СтрокаДанных.Серия);
						НайденаНоменклатура = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
...
Процедура ОтразитьРезультатыСканированияВДокументе(ПараметрыОкончанияСканирования) Экспорт
	...
	Если ТипПроверяемогоДокумента = Тип("ДокументСсылка.ПриходнаяНакладная")
		ИЛИ ТипПроверяемогоДокумента = Тип("ДокументСсылка.ОтчетПереработчика") Тогда
		
		ОтразитьРезультатыПроверкиИПодбораВДокументеПоступления(ПараметрыОкончанияСканирования);
		
	ИначеЕсли ТипПроверяемогоДокумента = Тип("ДокументСсылка.РасходнаяНакладная")
		ИЛИ ТипПроверяемогоДокумента = Тип("ДокументСсылка.КорректировкаРеализации")
		ИЛИ ТипПроверяемогоДокумента = Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями")
		ИЛИ ТипПроверяемогоДокумента = Тип("ДокументСсылка.ОтчетОПереработке")
		ИЛИ ТипПроверяемогоДокумента = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
...
Процедура ПриОпределенииПараметровИнтеграцииФормыПроверкиИПодбора(Форма, ПараметрыИнтеграции, ВидПродукции) Экспорт
	...
	Если ИнтеграцияИСМПУНФКлиентСервер.ЭтоДокументПоНаименованию(Форма, "РасходнаяНакладная") Тогда
		...
		ПараметрыИнтеграции.Вставить("ИмяКолонкиКоличество", "КоличествоВБазовыхЕдиницахГОСИС");
		
	ИначеЕсли ИнтеграцияИСМПУНФКлиентСервер.ЭтоДокументПоНаименованию(Форма, "КорректировкаРеализации") Тогда
		
		ПараметрыИнтеграции.ИмяРодительскойГруппыФормы                 = "ГруппаЗапасы";
		ПараметрыИнтеграции.ИмяСледующейКолонки                        = "ЗапасыГруппаКоличество";
		ПараметрыИнтеграции.РазмещатьЭлементыИнтерфейса                = Истина;
		ПараметрыИнтеграции.ИспользоватьКолонкуСтатусаПроверкиПодбора  = Истина;
		ПараметрыИнтеграции.ИспользоватьБезМаркируемойПродукции        = Истина;
		ПараметрыИнтеграции.БлокироватьТабличнуюЧастьТоварыПриПроверке = Ложь;
		ПараметрыИнтеграции.ИспользоватьСтатусПроверкаЗавершена        = Ложь;
		ПараметрыИнтеграции.ИмяТабличнойЧастиСерии                     = "СерииНоменклатуры";
	...
	ИначеЕсли ИнтеграцияИСМПУНФКлиентСервер.ЭтоДокументПоНаименованию(Форма, "ПередачаТоваровМеждуОрганизациями") Тогда
		...
		ПараметрыИнтеграции.ДоступныОбъемноСортовыеКоды                = Истина;
		ПараметрыИнтеграции.ИмяТабличнойЧастиСерии                     = "СерииНоменклатуры";
		ПараметрыИнтеграции.ЕстьПравоИзменение                         = ПравоДоступа("Изменение", Метаданные.Документы.ПередачаТоваровМеждуОрганизациями);
		ПараметрыИнтеграции.Вставить("ИмяКолонкиКоличество", "КоличествоВБазовыхЕдиницахГОСИС");
		
	ИначеЕсли ИнтеграцияИСМПУНФКлиентСервер.ЭтоДокументПоНаименованию(Форма, "ОтчетОПереработке") Тогда
		...
		ПараметрыИнтеграции.ИспользоватьСтатусПроверкаЗавершена        = Ложь;
		ПараметрыИнтеграции.ДоступныОбъемноСортовыеКоды                = Истина;
		ПараметрыИнтеграции.ЕстьПравоИзменение                         = ПравоДоступа("Изменение", Метаданные.Документы.ОтчетОПереработке);
		ПараметрыИнтеграции.Вставить("ИмяКолонкиКоличество", "КоличествоВБазовыхЕдиницахГОСИС");
		
	ИначеЕсли ИнтеграцияИСМПУНФКлиентСервер.ЭтоДокументПоНаименованию(Форма, "ОтчетПереработчика") Тогда
		...
		ПараметрыИнтеграции.ИспользоватьСтатусПроверкаЗавершена        = Ложь;
		ПараметрыИнтеграции.ДоступныОбъемноСортовыеКоды                = Истина;
		ПараметрыИнтеграции.ЕстьПравоИзменение                         = ПравоДоступа("Изменение", Метаданные.Документы.ОтчетПереработчика);
		ПараметрыИнтеграции.Вставить("ИмяКолонкиКоличество", "КоличествоВБазовыхЕдиницахГОСИС");
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.ЗаказПокупателя.Форма.ФормаЗаказНаряда" Тогда
		...
		ПараметрыИнтеграции.ИмяРодительскойГруппыФормы                 = "ЗНГруппаЗапасы";
		ПараметрыИнтеграции.РазмещатьЭлементыИнтерфейса                = Истина;
		ПараметрыИнтеграции.ЕстьПравоИзменение                         = ПравоДоступа("Изменение", Форма.Объект.Ссылка.Метаданные());
		ПараметрыИнтеграции.ЭтоДокументПриобретения                    = Ложь;
		ПараметрыИнтеграции.ЕстьЭлектронныйДокумент                    = Ложь;
...
Процедура ПроверитьМаркируемуюПродукциюДокумента(Ссылка, Отказ) Экспорт
	...
	Если (ТипДокумента = Тип("ДокументСсылка.РасходнаяНакладная") И НЕ СуществуетОтгрузкаТоваровИСМПВведеннаяНаОснованииДокумента(Ссылка))
		ИЛИ ТипДокумента = Тип("ДокументСсылка.ПриходнаяНакладная")
		...
		ИЛИ ТипДокумента = Тип("ДокументСсылка.ОтчетОПереработке")
		ИЛИ ТипДокумента = Тип("ДокументСсылка.ОтчетПереработчика")
		ИЛИ ТипДокумента = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
...
Процедура РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаТовары, СтруктураДанные)
	
	ТабличныеЧастиУНФКлиентСервер.РассчитатьСуммыВСтрокеТЧ(СтрокаТовары, СтруктураДанные);
...
// Переносит результат проверки и подбора продукции ИС МП в исходящий документ.
//   Общая схема:
...
Процедура ОтразитьРезультатыПроверкиИПодбораВИсходящемДокументе(ПараметрыОкончанияСканирования)
	...
	ТипОбъекта = ТипЗнч(ПараметрыОкончанияСканирования.ПроверяемыйДокумент);
	...
	Если ТипОбъекта = Тип("ДокументСсылка.РасходнаяНакладная") ИЛИ ТипОбъекта = Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями")
		ИЛИ ТипОбъекта = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерии(ДокументОбъект, ПараметрыОкончанияСканирования);
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ОтчетОПереработке") Тогда
		ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерии(ДокументОбъект, ПараметрыОкончанияСканирования, "Продукция");
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		ОтразитьИзменениеКоличестваВТабличнойЧастиТоварыСерииДокументаКорректировки(ДокументОбъект, ПараметрыОкончанияСканирования);
	КонецЕсли;
...
// Переносит результат проверки и подбора продукции ИС МП во входящий документ.
//   Общая схема:
...
Процедура ОтразитьРезультатыПроверкиИПодбораВДокументеПоступления(ПараметрыОкончанияСканирования)
	...
	ТипОбъекта = ТипЗнч(ПараметрыОкончанияСканирования.ПроверяемыйДокумент);
	Если ТипОбъекта = Тип("ДокументСсылка.ОтчетПереработчика") Тогда
	...
	Если Не ПараметрыОкончанияСканирования.СоздаватьАктОРасхождениях Тогда
		...
		ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерии(ДокументОбъект, ПараметрыОкончанияСканирования, ИмяТЧЗапасы);
	...
	Если ПараметрыОкончанияСканирования.СоздаватьАктОРасхождениях Тогда
		...
		СоздатьАктОРасхожденияхПоРезультатамПроверкиИПодбораИСМП(
...
// Переносит маркируемую продукцию из формы проверки и подбора в документ
//   * Удаляет из табличных частей Запасы, Серии отсутствующую в данных проверки маркируемую продукцию.
//   * Не меняет прочие товарные строки.
// 
// Параметры:
//  ДокументОбъект  - документ к изменению;
//  ПараметрыОкончанияПроверки - Структура - (См. ПроверкаИПодборИСМП.ЗафиксироватьРезультатПроверкиИПодбора).
//
Процедура ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерии(ДокументОбъект, ПараметрыОкончанияСканирования, ИмяТЧЗапасы = "Запасы", ИмяТЧСерии = "СерииНоменклатуры")
	
	ТаблицаПодобраннойПровереннойПродукции = ПараметрыОкончанияСканирования.ТаблицаПодобраннойПровереннойПродукции;
	
	Запрос = Новый Запрос;
	
	ТаблицаТовары = ДокументОбъект[ИмяТЧЗапасы].Выгрузить();
	...
	Запрос.УстановитьПараметр("Подобрано", ТаблицаПодобраннойПровереннойПродукции);
	...
	|	ВЫБОР
	|		КОГДА ВТ_Серии.Серия ЕСТЬ NULL
	|			ТОГДА ВТ_Товары.Количество * ВЫБОР
	...
	Пока ВыборкаРасхождения.Следующий() Цикл
		...
		Если Недостача > 0 Тогда
			Для Каждого СтрокаТаблицыТовары Из СтрокиТовары Цикл
				...
				ИзмененныеСтроки.Вставить(СтрокаТаблицыТовары);
		...
		Иначе
			...
			Иначе
				...
				НоменклатураВДокументахСервер.ЗаполнитьДанныеВСтрокеТабличнойЧасти(ДокументОбъект, ИмяТЧЗапасы, НоваяСтрокаТовары);
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрокаТовары, "ЕдиницаИзмерения")
					И ЗначениеЗаполнено(НоваяСтрокаТовары.Номенклатура) Тогда
					НоваяСтрокаТовары.ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрокаТовары.Номенклатура, "ЕдиницаИзмерения");
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(ВыборкаРасхождения.Серия) Тогда
					...
					НоваяСтрокаТовары.СерииНоменклатуры = СерииНоменклатурыУНФ.ПредставлениеСерийНоменклатуры(ТаблицаСерии,
						НоваяСтрокаТовары.КлючСвязи);
				КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ИзмененныеСтроки Цикл
		...
		СтрокаТаблицыТовары = КлючИЗначение.Ключ;
		Если СтрокаТаблицыТовары.Количество = 0 Тогда
			СтрокаТаблицыТовары.Удалить = Истина;
		Иначе
			...
			СтрокаТаблицыТовары.СерииНоменклатуры = СерииНоменклатурыУНФ.ПредставлениеСерийНоменклатуры(ТаблицаСерии, СтрокаТаблицыТовары.КлючСвязи);
		КонецЕсли;
	...
	ДокументОбъект[ИмяТЧЗапасы].Загрузить(ТаблицаТовары);
	
КонецПроцедуры

// Переносит маркируемую продукцию из формы проверки и подбора в документ корректировки
//   * Удаляет из табличных частей Запасы, Серии отсутствующую в данных проверки маркируемую продукцию.
//   * Не меняет прочие товарные строки.
// 
// Параметры:
//  ДокументОбъект           - ДокументОбъект.КорректировкаРеализации, ДокументаОбъект.АктОРасхождениях - документ к изменению
//  ПараметрыОкончанияПроверки - Структура - (См. ПроверкаИПодборИСМП.ЗафиксироватьРезультатПроверкиИПодбора).
//
Процедура ОтразитьИзменениеКоличестваВТабличнойЧастиТоварыСерииДокументаКорректировки(ДокументОбъект, ПараметрыОкончанияСканирования)
	...
	Запрос.УстановитьПараметр("Подобрано", ПараметрыОкончанияСканирования.ТаблицаПодобраннойПровереннойПродукции);
	Запрос.УстановитьПараметр("ВидПродукцииИС", ПараметрыОкончанияСканирования.ВидПродукцииИС);
	...
	|	Товары.Характеристика,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(Товары.Серия) = ТИП(Справочник.СерииНоменклатуры)
	|				И НЕ Товары.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|			ТОГДА Товары.Серия
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	-Товары.Количество
	|ИЗ
	|	ВТ_Подобрано КАК Товары
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
		...
		Если Недостача>0 Тогда
			Для Каждого СтрокаТаблицыТовары Из СтрокиТовары Цикл
				...
				СтрокаТаблицыТовары.Удалить = СтрокаТаблицыТовары.Количество = 0
					И СтрокаТаблицыТовары.КоличествоДоИзменения = 0 И СтрокаТаблицыТовары.КоличествоДоКорректировки = 0;
...
Процедура ОтразитьИзмененияТабличнойЧастиШтрихкодыУпаковок(ДокументОбъект, ТаблицаШтрихкодовВерхнегоУровня, ВидПродукцииИС) Экспорт
	...
	Для Каждого ЭлементВНаличии Из ТаблицаШтрихкодовВерхнегоУровня Цикл
		СтруктураПоиска = Новый Структура("ШтрихкодУпаковки", ЭлементВНаличии.ШтрихкодУпаковки);
...
Процедура СоздатьАктОРасхожденияхПоРезультатамПроверкиИПодбораИСМП(ПараметрыОкончанияСканирования, Проводить)
	
	СформированныйАкт = ИнтеграцияИСМПУНФ.СформированныйАктОРасхождениях(ПараметрыОкончанияСканирования.ПроверяемыйДокумент);
	...
	Если СформированныйАкт = Неопределено Тогда
		...
		ДокументОбъект.Заполнить(ПараметрыОкончанияСканирования.ПроверяемыйДокумент);
	Иначе
...
Процедура ЗаполнитьТаблицуРасхожденийШтриховыхКодов(ДокументОбъект, ПараметрыОкончанияСканирования)
	...
	Для Каждого СтрокаТаблицы Из ПараметрыОкончанияСканирования.ТаблицаРасхожденийШтриховыхКодов Цикл
		...
		НоваяСтрока.ТипРасхождения = ?(СтрокаТаблицы.ЭтоИзлишек,
		                               Перечисления.ТипыРасхожденийИСМП.Излишек,
		                               Перечисления.ТипыРасхожденийИСМП.Недостача);
