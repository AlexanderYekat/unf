...
Функция ПолучитьЗаказыВебСервис(ДатаИзменения, КодНастройкиИнтеграции = Неопределено) Экспорт
	...
	Возврат ПолучитьЗаказы(ДатаИзменения, КодНастройкиИнтеграции);
...
Функция ПолучитьКлассификаторИКаталогВебСервис(ДатаИзменения, КодГруппы, КодНастройкиИнтеграции = Неопределено) Экспорт
	...
	Возврат ПолучитьКлассификаторИКаталог(ДатаИзменения, КодГруппы, КодНастройкиИнтеграции);
...
Функция ПолучитьОстаткиИЦеныВебСервис(ДатаИзменения, КодГруппы, КодСклада, КодОрганизации, КодНастройкиИнтеграции = Неопределено) Экспорт
	...
	Возврат ПолучитьОстаткиИЦены(ДатаИзменения, КодГруппы, КодСклада, КодОрганизации, КодНастройкиИнтеграции);
	
КонецФункции // ПолучитьОстаткиИЦеныВебСервис()

Функция ПолучитьКартинкуВебСервис(ИдентификаторТовараСтрока) Экспорт
	
	Возврат ПолучитьКартинку(ИдентификаторТовараСтрока);
...
Процедура ВыполнитьОбмен( НастройкаИнтеграции, Параметры, РежимЗапускаОбмена, ВыгружатьТолькоИзменения = Истина, ЭтоРеглЗадание = Ложь ) 
	...
	ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог( "Начало выполнения обмена данными", НастройкаИнтеграции );
...
Функция ВыгрузитьПакетПредложений(Параметры, ИмяФайлаПрайса, ПакетCML)
	...
	Исключение
		...
		ТекстОшибки = "ВыгрузитьПакетПредложений: Ошибка: " + ПодробноеПредставлениеОшибки;
		ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог( ТекстОшибки, , Истина );
...
Функция ВыгрузитьКлассификаторИКаталог(Параметры, ИмяФайлаКаталога, ПакетCML)
	...
	Исключение
		...
		ТекстОшибки = "ВыгрузитьКлассификаторИКаталог: Ошибка: " + ПодробноеПредставлениеОшибки;
		ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог( ТекстОшибки, , Истина );
...
Функция СоздатьОбновитьЗаказПоДокументуXDTO( ДокументXDTO, РанееЗагруженныеЗаказы, СтруктураСтатистики, Параметры, ОписаниеОшибки)
	...
	Если ПропуститьДокумент
		И Параметры.Свойство("ВариантОбновленияЗаказовИнтернетМагазина")
		И Параметры.ВариантОбновленияЗаказовИнтернетМагазина = Перечисления.ВариантыОбновленияЗаказовИнтернетМагазина.ОбновлятьТолькоНеПроведенные
		Тогда
		...
		Если НЕ Параметры.ОбменЧерезВебСервис Тогда
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				ВыполнитьДействияПередЗаписьюЗаказа(ДокументОбъект, Параметры, Истина);
...
Функция ВыгрузитьЗаказыВФайл(Параметры, СтруктураСтатистики, ОписаниеОшибки)
	...
	Пока Параметры.ВыполнятьОбменЗаказы Цикл
		...
		Исключение
			...
			ТекстОшибки = "ВыгрузитьЗаказыВФайл: Ошибка: " + ПодробноеПредставлениеОшибки;
			ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог( ТекстОшибки, , Истина );
...
Функция ПолучитьЗаказыСОплатойИОтгрузкой(МассивИзменений, МассивЗагруженныхДокументов, Параметры)
	...
	СхемаКомпоновки = Справочники.НастройкиИнтеграцииСИнтернетМагазином.ПолучитьМакет("СхемаВыгрузкиЗаказов");
...
Процедура ПодготовитьДанныеДляВыгрузки(Параметры)
	...
	Или ФормироватьОтчетПоДаннымДляВыгрузки Тогда
		...
		Исключение
			КомпоновщикНастроекОтборыПредставление = НСтр( "ru = '<ПодготовитьДанныеДляВыгрузки: Ошибка при получении элементов отбора>: '" )
			 + ПодробноеПредставлениеОшибки( ИнформацияОбОшибке() );
		КонецПопытки;
...
////	Параметры - Структура
////
Функция ЗаполнитьСтруктуруИзменений(Параметры) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	ОстаткиТоваровИзменения.Номенклатура
	|ИЗ
	...
	|ГДЕ
	|	втНоменклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	втХарактеристика.Номенклатура,
	|	""Товары""
	|ИЗ
	|	втХарактеристика КАК втХарактеристика
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|
	|ВЫБРАТЬ
	|	ВТ_ЦенаКоличество.Номенклатура,
	|	""Товары""
	|ИЗ
	|	ВТ_ЦенаКоличество КАК ВТ_ЦенаКоличество
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	НоменклатураПрисоединенныеФайлыИзменения.Ссылка.ВладелецФайла,
	|	""Товары""
	|ИЗ
	...
	|
	|ВЫБРАТЬ
	|	ХарактеристикиНоменклатурыПрисоединенныеФайлыИзменения.Ссылка.ВладелецФайла.Владелец, 
	|	""Товары""
	|ИЗ
	...
	|
	|ВЫБРАТЬ
	|	ХарактеристикиНоменклатурыПрисоединенныеФайлыИзменения.Ссылка,
	|	""Файлы""
	|ИЗ
	|		 Справочник.ХарактеристикиНоменклатурыПрисоединенныеФайлы Как ХарактеристикиНоменклатурыПрисоединенныеФайлы
	|		 ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатурыПрисоединенныеФайлы.Изменения КАК ХарактеристикиНоменклатурыПрисоединенныеФайлыИзменения
	...
	|ГДЕ
	|	ЗаказыИзменения.Узел = &УзелОбменаЗаказы
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ВЫРАЗИТЬ(ОплатаСчетовИЗаказов.СчетНаОплату КАК Документ.ЗаказПокупателя) ССЫЛКА Документ.ЗаказПокупателя
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	втНоменклатура КАК втНоменклатура
	|ГДЕ
	|	втНоменклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ОтклоненияОтГрафиковРаботыРесурсовИзменения.РесурсПредприятия ССЫЛКА Справочник.КлючевыеРесурсы");
...
Процедура ПолучитьСтруктуруИзменений(Параметры)
	...
	Если Параметры.Свойство("НастройкаИнтеграции") И Параметры.ВыгружатьТолькоИзменения Тогда
		СтруктураИзменений = ЗаполнитьСтруктуруИзменений(Параметры);
	Иначе
		СтруктураИзменений = Новый Структура;
		СтруктураИзменений.Вставить("Товары", Новый Массив);
		СтруктураИзменений.Вставить("Услуги", Новый Массив);
...
Функция ПолучитьЗаказы(ДатаИзменения, КодНастройкиИнтеграции = Неопределено)
	...
	Иначе
		ЗаказыXDTO = СформироватьЗаказыXDTO_ВерсияCommerceML205a(МассивИзменений, СтруктураСтатистики, Параметры);
	КонецЕсли;
...
Функция ПолучитьКлассификаторИКаталог(ДатаИзменения, КодГруппы, КодНастройкиИнтеграции = Неопределено)
	...
	Исключение
		...
		ТекстОшибки = "ПолучитьКлассификаторИКаталог: Ошибка: КлассификаторXDTO: " + ПодробноеПредставлениеОшибки( ИнформацияОбОшибке() );
		ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог( ТекстОшибки, , Истина );
	...
	Исключение
		...
		ТекстОшибки = "ПолучитьКлассификаторИКаталог: Ошибка: КаталогXDTO: " + ПодробноеПредставлениеОшибки( ИнформацияОбОшибке() );
		ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог( ТекстОшибки, , Истина );
...
Функция ПолучитьОстаткиИЦены(ДатаИзменения, КодГруппы, КодСклада, КодОрганизации, КодНастройкиИнтеграции = Неопределено)
	...
	Исключение
		...
		ТекстОшибки = "ПолучитьОстаткиИЦены: Ошибка: ПакетПредложенийXDTO: " + ПодробноеПредставлениеОшибки( ИнформацияОбОшибке() );
		ОбменСВнешнимиСистемамиЛогирование.ЗаписатьВЛог( ТекстОшибки, , Истина );
...
Функция ПолучитьКартинку(ИдентификаторТовараСтрока)
	...
	Если Выборка.Следующий() Тогда
		...
		ДвоичныеДанныеФайла = ПолучитьДвоичныеДанныеФайлаВебСервер(Выборка);
		
		Попытка
			КартинкаXDTO = КартинкаСериализаторXDTO.ЗаписатьXDTO(ДвоичныеДанныеФайла);
		Исключение
...
Процедура ПодготовитьДанныеДляВыгрузкиНоменклатуры(Параметры)
	...
	СхемаВыгрузкиТоваров = Справочники.НастройкиИнтеграцииСИнтернетМагазином.ПолучитьМакет("СхемаВыгрузкиТоваровВебСервис");
...
Процедура ПодготовитьДанныеДляВыгрузкиОстатковИЦен(Параметры)
	...
	Параметры.Вставить("ВыборкаЦен", МассивРезультатовЗапроса[17].Выбрать());
	Параметры.Вставить("ВыборкаВидовЦен", МассивРезультатовЗапроса[20].Выбрать());
	Параметры.Вставить("ВыборкаСкладов", МассивРезультатовЗапроса[24].Выбрать());
...
Функция ПолучитьДвоичныеДанныеФайлаВебСервер(ДанныеФайла)
	...
	Иначе
		...
		ИмяФайла = ПодготовитьПутьДляПлатформы(ПлатформаWindows,
			ПолучитьПутьТомаДляПлатформы(ПлатформаWindows, ДанныеФайла.Том) + "\" + ДанныеФайла.ПутьКФайлу);
