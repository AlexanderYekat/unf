...
Процедура РассчитатьАвтоматическиеСкидки(ПараметрыРасчета, Результат) Экспорт
	...
	Если ПараметрыРасчета.Свойство("ВернутьМаксимальнуюСуммуОплатыБонусами") И ПараметрыРасчета.ВернутьМаксимальнуюСуммуОплатыБонусами = Истина Тогда
...
Процедура ЗаполнитьТаблицуУправляемыхСкидок(Результат, ПараметрыРасчета) Экспорт
	...
	Если ПараметрыРасчета = Неопределено Тогда
		ПараметрыРасчета = Новый Структура("МаксимальныйПроцетСкидки, МаксимальныйПроцетНаценки, ИспользоватьОтборы", 0, 0, Ложь);
	Иначе
	...
	|					И &МаксимальныйПроцетСкидки > 0
	|				ТОГДА ВидыСкидокНаценок.Процент <= &МаксимальныйПроцетСкидки
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ВидыСкидокНаценок.Процент < 0
	|							И &МаксимальныйПроцетНаценки < 0
	|						ТОГДА ВидыСкидокНаценок.Процент >= &МаксимальныйПроцетНаценки
	|					ИНАЧЕ ИСТИНА
	|				КОНЕЦ
	|		КОНЕЦ
	|
	...
	Запрос.УстановитьПараметр("МаксимальныйПроцетСкидки", ПараметрыРасчета.МаксимальныйПроцетСкидки);
	Запрос.УстановитьПараметр("МаксимальныйПроцетНаценки", ПараметрыРасчета.МаксимальныйПроцетНаценки);
	Запрос.УстановитьПараметр("ДатаСеанса", ТекущаяДатаСеанса());
	Запрос.УстановитьПараметр("ИспользуетсяОтбор", ПараметрыРасчета.ИспользоватьОтборы);
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
