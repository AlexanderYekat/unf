	...
	Если ДанныеДляПроведения <> Неопределено Тогда
		...
		Для Каждого ДанныеСотрудника Из ДанныеДляПроведения Цикл
			...
			Если ПараметрыФормирования.ДополнитьСведениямиОКодахПоОКЗ
				...
				ИначеЕсли ЗначениеЗаполнено(ДанныеСотрудника.ТрудоваяФункция) Тогда
...
Функция СвойстваДоговоровСотрудников(КадровыеДанныеСотрудников, ДанныеДляПроведения = Неопределено) Экспорт
	...
	Для Каждого КадровыеДанныеСотрудника Из КадровыеДанныеСотрудников Цикл
		...
		ДанныеСотрудника = Новый Структура("СрочночтьТрудовогоДоговора,УдаленностьРаботы,СокращенностьГрафика");
		...
		КонецЕсли;
		ДанныеСотрудника.СрочночтьТрудовогоДоговора = СрочностьТрудовогоДоговора(КадровыеДанныеСотрудника.ВидЗанятости,
	...
	Если ИсточникиДанных.Получить("СведенияОПоследнихМероприятияхТрудовойДеятельности") <> Неопределено Тогда
		...
		ТекстЗапросаПоследнихМероприятий =
			...
			+	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			 	...
			 	|	Мероприятия.СрочночтьТрудовогоДоговора КАК СрочночтьТрудовогоДоговора,
...
Функция МероприятияПоСпискуИдентификаторов(Организация, ИдМероприятий) Экспорт
	...
	ТекстЗапроса =
		...
		|	ЕСТЬNULL(МероприятияПереданные.СрочночтьТрудовогоДоговора, Мероприятия.СрочночтьТрудовогоДоговора) КАК СрочночтьТрудовогоДоговора,
...
Процедура УстановитьУсловноеОформлениеТаблицыМероприятий(УправляемаяФорма, Организация, ДатаОтменыРедактируется, ФормаОбъекта = Истина, ДатаДокумента = Неопределено) Экспорт
	...
	Для Каждого ИмяПоля Из ВсеНастраиваемыеПоля Цикл
		...
		Если ИмяПоля = "СрочночтьТрудовогоДоговора"
...
Процедура ЗаполнитьДанныеДоговора(ДанныеСотрудника, СвойстваДоговора) Экспорт
	Если СвойстваДоговора <> Неопределено Тогда
		Если Не ЗначениеЗаполнено(ДанныеСотрудника.СрочночтьТрудовогоДоговора) Тогда
			ДанныеСотрудника.СрочночтьТрудовогоДоговора = СвойстваДоговора.СрочночтьТрудовогоДоговора;
		КонецЕсли;
...
Функция ДанныеТрудовойДеятельностиСотрудниковПоТаблицеОтборов(МенеджерВременныхТаблиц, ТолькоРазрешенные, Организации, МероприятияВсехСотрудников = Ложь, ИсключаемыйРегистратор = Неопределено, ПараметрыЗаполнения = Неопределено)
	...
	Запрос.Текст =
		...
		|	КПередачеМероприятияТрудовойДеятельности.СрочночтьТрудовогоДоговора КАК СрочночтьТрудовогоДоговора,
...
Функция ПредставлениеВыполняемойФункции(ВыборкаДанных, Знач ПредставлениеДляСТДР = Ложь) Экспорт 
	...
	Если Не ПредставлениеДляСТДР И ВыборкаДанных.Дата >= ПерсонифицированныйУчетКлиентСервер.ДатаПостановленияЕФС1_2023() Тогда
		Если ЗначениеЗаполнено(ВыборкаДанных.СрочночтьТрудовогоДоговора) Тогда
			СрочночтьТрудовогоДоговора = КодВидаДоговора(ВыборкаДанных);
			Если СрочночтьТрудовогоДоговора <> Неопределено Тогда
				СоставПредставления.Добавить(СрочночтьТрудовогоДоговора);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция КодВидаДоговора(ВыборкаДанных) Экспорт
	Если ВыборкаДанных.СрочночтьТрудовогоДоговора = Перечисления.СрочностиТрудовогоДоговора.БессрочныйТрудовойДоговор Тогда
		Возврат "0";
	ИначеЕсли ВыборкаДанных.СрочночтьТрудовогоДоговора = Перечисления.СрочностиТрудовогоДоговора.СрочныйТрудовойДоговорДо6Месяцев Тогда
		Возврат "0.1";
	ИначеЕсли ВыборкаДанных.СрочночтьТрудовогоДоговора = Перечисления.СрочностиТрудовогоДоговора.СрочныйТрудовойДоговорБолее6Месяцев Тогда
		Возврат "0.2";
	ИначеЕсли ВыборкаДанных.СрочночтьТрудовогоДоговора = Перечисления.СрочностиТрудовогоДоговора.БессрочныйТрудовойДоговорПоСовместительству Тогда
		Возврат "1";
	ИначеЕсли ВыборкаДанных.СрочночтьТрудовогоДоговора = Перечисления.СрочностиТрудовогоДоговора.СрочныйТрудовойДоговорПоСовместительствуДо6Месяцев Тогда
		Возврат "1.1";
	ИначеЕсли ВыборкаДанных.СрочночтьТрудовогоДоговора = Перечисления.СрочностиТрудовогоДоговора.СрочныйТрудовойДоговорПоСовместительствуБолее6Месяцев Тогда
		Возврат "1.2";
	КонецЕсли;
	Возврат Неопределено
КонецФункции
...
Функция ЗапросВТМероприятияТрудовойДеятельности(ТолькоРазрешенные, ИсключаемыйРегистратор, ИмяВТМероприятияТрудовойДеятельности)
	...
	ТекстЗапроса =
		...
		|	МероприятияПрочие.СрочночтьТрудовогоДоговора КАК СрочночтьТрудовогоДоговора,
		...
		|	Мероприятия.СрочночтьТрудовогоДоговора,
		...
		|	МероприятияПрочие.СрочночтьТрудовогоДоговора,
...
Функция ЗапросВТКПередачеМероприятияТрудовойДеятельности(ТолькоРазрешенные, ИсключаемыйРегистратор, ПараметрыЗаполнения, ИмяВТКПередачеМероприятияТрудовойДеятельности)
	...
	ТекстЗапросаВТКПередачеМероприятияТрудовойДеятельности =
		...
		|	МероприятияДоНачалаУчета.СрочночтьТрудовогоДоговора КАК СрочночтьТрудовогоДоговора,
		...
		|	МероприятияОтчетногоПериода.СрочночтьТрудовогоДоговора,
...
Функция ЗапросВТВсеМероприятияДоНачалаУчета(ТолькоРазрешенные, ИсключаемыйРегистратор, ИмяВТВсеМероприятияДоНачалаУчета)
	...
	Запрос.Текст =
		...
		|	ВсеМероприятияПрочиеДоНачалаУчета.СрочночтьТрудовогоДоговора КАК СрочночтьТрудовогоДоговора,
		...
		|	ВсеМероприятияПрочиеДоНачалаУчета.СрочночтьТрудовогоДоговора,
		...
		|	МероприятияТрудовойДеятельности.СрочночтьТрудовогоДоговора КАК СрочночтьТрудовогоДоговора,
...
Функция ЗапросВТМероприятияДоНачалаУчета(ТолькоРазрешенные, ИсключаемыйРегистратор, ИмяВТМероприятияДоНачалаУчета)
	...
	ТекстЗапросаМероприятияКПередаче =
		...
		|	РанееПереданныеОтмененные.СрочночтьТрудовогоДоговора КАК СрочночтьТрудовогоДоговора,
		...
		|	МероприятияДоНачалаУчета.СрочночтьТрудовогоДоговора,
