...
Процедура ПередПериодическойОтправкойДанныхКлиентаНаСервер(Параметры) Экспорт
	...
	Если ПараметрПриложения = Неопределено
		Или Не МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ПараметрПриложения.ОбновитьПараметрыНастроек
		И Не ПараметрПриложения.НастроенАвтоматическийОбмен Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПараметрПриложения.НастроенАвтоматическийОбмен Тогда
