...
Процедура ВыполнитьРегистрациюНаСервере(Ссылка, Режим, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Ложь, ОбластьДанных = 0) Экспорт
	...
	ОбменСКассовымСерверомШтрихМ.ВыполнитьПодключениеКассовыхАппаратов(КассыДляРегистрации, Режим,
...
//    ОсвободитьЛицензию - Булево - флаг для освобождения лицензии на сервере тарификации;
...
Процедура ОбработатьРезультатПодключенияКассы(ИнформацияОПодключении, ЕстьОшибки, СообщениеОбОшибке) Экспорт
	...
	Если ЗначениеЗаполнено(КассовыйАппарат) Тогда
		...
		Если ИнформацияОПодключении.ОсвободитьЛицензию И РаботаВМоделиСервиса.РазделениеВключено() Тогда
			ОтменаРегистрации = 
				ТарификацияУНФ.ОтменаРегистрацииКассыНаСервереТарификации(МенеджерЗаписиКассы.ИдентификаторТарификации,
				Ложь, Истина);
...
Процедура ОбработатьРезультатОчисткиТоваров(ИнформацияОПодключении, ЕстьОшибки, СообщениеОбОшибке) Экспорт
	...
	Если ЗначениеЗаполнено(КассовыйАппарат) Тогда
		...
		Если ИнформацияОПодключении.ОсвободитьЛицензию И РаботаВМоделиСервиса.РазделениеВключено() Тогда
			ОтменаРегистрации = 
				ТарификацияУНФ.ОтменаРегистрацииКассыНаСервереТарификации(КассаОбъект.ИдентификаторТарификации, Ложь,
				Истина);
			Если НЕ ОтменаРегистрации Тогда
				СообщениеОбОшибке = СообщениеОбОшибке +
...
Функция ТекстОтветаСервераШтрихМПоДокументу(ИнформацияПоДокументу)
	...
	СериализуемаяИнформация = ПодготовитьСериализуемуюИнформациюПоДокументу(ИнформацияПоДокументу);
	
	Запись = Новый ЗаписьJSON();
	Запись.УстановитьСтроку(Новый ПараметрыЗаписиJSON(,"	",));
	
	СериализаторXDTO.ЗаписатьJSON(Запись, СериализуемаяИнформация, НазначениеТипаXML.Явное);
	Возврат Запись.Закрыть();
	
КонецФункции


// Преобразует структуру информацияПоДокументу с несериализуемыми данными в подходящий для JSON формат
...
Процедура ВыполнитьЗагрузкуЧекаККМКакПриходнаяНакладная(ИнформацияПоДокументу, КассовыйАппарат, ВидОперации, Проводить, ЕстьОшибки, СообщениеОбОшибке, СозданныйДокумент = Неопределено, ЧекПродажи = Неопределено)
	...
	Иначе
		...
		ПриходнаяНакладная.Договор = Справочники.ДоговорыКонтрагентов.ПоДокументуКонтрагентуОрганизацииИВидуОперации(
			ПриходнаяНакладная, ПриходнаяНакладная.Контрагент, ПриходнаяНакладная.Организация,
			ПриходнаяНакладная.ВидОперации);
		
	КонецЕсли;
	
	// заполняем потенциально изменяемую шапку
...
Процедура ВыполнитьЗагрузкуЧекаККМКакРасходнаяНакладная(ИнформацияПоДокументу, КассовыйАппарат, ВидОперации, Проводить, ЕстьОшибки, СообщениеОбОшибке, СозданныйДокумент = Неопределено)
	...
	Иначе
		...
		РасходнаяНакладная.Договор = Справочники.ДоговорыКонтрагентов.ПоДокументуКонтрагентуОрганизацииИВидуОперации(
			РасходнаяНакладная, РасходнаяНакладная.Контрагент, РасходнаяНакладная.Организация,
...
Процедура ВыполнитьЗагрузкуОперацииВозвратаРасхода(ИнформацияПоДокументу, КассовыйАппарат, Проводить,  ЕстьОшибки, СообщениеОбОшибке, СозданныеДокументы = Неопределено)
	...
	СозданныйРасходНаличныхИзКассы = Неопределено;
	ВыполнитьЗагрузкуВнесенияНаличных(
		...
		ЕстьОшибки,
		СообщениеОбОшибке3,
		СозданныйРасходНаличныхИзКассы);
	СозданныеДокументы.Добавить(СозданныйРасходНаличныхИзКассы);
...
Функция ПолучитьЭквайринговыйТерминалПоУмолчанию(ПолученныеДанные)
	НайденныйЭТ = Справочники.СпособыОплаты.ПолучитьЭквайринговыйТерминалПоУмолчанию(ПолученныеДанные.КассаККМ);
	
	Если ЗначениеЗаполнено(НайденныйЭТ) Тогда
		Возврат НайденныйЭТ;
	КонецЕсли;
	
	НовыйЭТ = Справочники.СпособыОплаты.СоздатьЭлемент();
...
Функция ИнформацияПоТарифам() Экспорт
	...
	Попытка
		ИдентификаторПоставщика = ТарификацияУНФ.ИдентификаторПоставщикаКасса();
		ИдентификаторУслугиКасса = ТарификацияУНФ.ИдентификаторУслугиКасса();
		ИнформацияПоТарифам = Тарификация.КоличествоЛицензийУникальнойУслуги(ИдентификаторПоставщика,
			ИдентификаторУслугиКасса);
	Исключение
