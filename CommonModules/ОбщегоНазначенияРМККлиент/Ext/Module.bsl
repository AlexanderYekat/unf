...
Процедура ПроверитьОшибкиЗаполненияКартыЛояльности(Форма, ЕстьОшибки) Экспорт
	...
	Если ЗначениеЗаполнено(Форма.НоваяКартаВидКарты) Тогда
		...
		Если ЗначениеЗаполнено(ДанныеВидаКарты.Идентификатор) Тогда
			...
			ПроверкаЗаполненностиНомера = ГенерироватьНомер И ЭтоИменнаяКарта;
		КонецЕсли;
	КонецЕсли;
...
Функция ИменаРеквизитовФормыРМКДляДобавленияСтроки() Экспорт
	...
	ИменаРеквизитов = "ОсновнаяОрганизация, ОтображатьОстаток, ИмяФормы,
		|РозничныйВидЦены, ПараметрыСобытийПО, ФильтрПоискаПоШтрихкоду, ИспользоватьВыборСкладаПриПродаже";
	ОбщегоНазначенияРМККлиентПереопределяемый.
...
//
Процедура ПроверкаКодаМаркировкиСредствамиККТПоДаннымШтрихкода(ПараметрыСканирования, РезультатОбработкиШтрихкода, ФормаВладелец, ОписаниеОповещения) Экспорт
