...
Процедура ПовторноОбработатьВходящиеСообщенияСЭДО(ПараметрыОбновления = Неопределено, ТипыЧислом = Неопределено, Идентификаторы = Неопределено, Страхователи = Неопределено) Экспорт
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(Страхователи) Тогда
...
Процедура ЗагрузитьРезультатДоставкиСообщенияСЭДО(Страхователь, Сообщение, Результат)
	...
	Если Сообщение.ТипЧислом = 86 Тогда
		// Сведения о застрахованном лице.
...
Функция УполномоченныйПредставитель(Организация, Дата = Неопределено) Экспорт
	...
	Результат.ФизическоеЛицо   = ОтветственныеЛица.УполномоченныйПоПрямымВыплатамФСС;
	Результат.Должность        = ОтветственныеЛица.УполномоченныйПоПрямымВыплатамФССДолжность;
	Результат.ОснованиеПодписи = ОтветственныеЛица.УполномоченныйПоПрямымВыплатамФССОснованиеПодписи;
