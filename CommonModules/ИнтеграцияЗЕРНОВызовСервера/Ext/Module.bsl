...
Функция ДанныеАдресаОбъекта(Ссылка) Экспорт
	...
	Иначе
		...
		ДанныеАдреса.Вставить("КомментарийАдреса",
			ИнтеграцияЗЕРНО.КомментарийКонтактнойИнформации(ДанныеАдреса.Адрес));
	КонецЕсли;
...
Функция ТекстЗапросаАдресаДляАвтоподбора(ОписаниеТипов, ИмяВременнойТаблицы = "") Экспорт

	ТекстЗапроса = "";
	
	Если ОписаниеТипов.Типы().Количество() > 0 Тогда
		СобытияФормЗЕРНОПереопределяемый.ТекстЗапросаАдресаДляАвтоподбора(ОписаниеТипов, ТекстЗапроса, ИмяВременнойТаблицы);
	КонецЕсли;
...
Функция ТекстЗапросаАдресаДляАвтоподбораИС(ОписаниеТипов, ИмяВременнойТаблицы = "") Экспорт
	...
	Для Каждого Тип Из Типы Цикл
		...
		ИначеЕсли Тип = Тип("СправочникСсылка.РеестрЭлеваторовЗЕРНО") Тогда
			ТекстПодзапроса = "
			|ВЫБРАТЬ ПЕРВЫЕ 10
			|	ТаблицаСправочника.Ссылка КАК ВладелецАдреса,
...
Функция ТекстЗапросаАдресаДляАвтоподбораПоОписаниюТипов(ОписаниеТипов) Экспорт
	...
	Для Каждого Тип Из Типы Цикл
		Если Тип = Тип("СправочникСсылка.ЗемельныеУчасткиИС")
			Или Тип = Тип("СправочникСсылка.РеестрЭлеваторовЗЕРНО") Тогда
			ТипыИС.Добавить(Тип);
		КонецЕсли;
	КонецЦикла;
...
Функция СписокАвтоподбораПоОписаниюТипов(ОписаниеТипов, ТекстДляАвтоПодбора,
	...
	Если СтрДлина(ТекстДляАвтоПодбора) >= МинимальнаяДлинаСтрокиПоиска Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Данные.Вставить("КомментарийАдреса",   ИнтеграцияЗЕРНО.КомментарийКонтактнойИнформации(Данные.Адрес));
