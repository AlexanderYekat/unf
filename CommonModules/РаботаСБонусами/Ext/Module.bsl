...
Функция ПолучитьМаксимальнуюСуммуОплатыДокумента(ТабличнаяЧасть, ДисконтнаяКарта) Экспорт
	...
	ЕстьОплатаБонусами = Неопределено;
	Для Каждого СуммаОплаты Из МаксимальныеСуммыОплаты Цикл
		СтрокаТЧ = СуммаОплаты.Ключ;		
		Если ЕстьОплатаБонусами = Неопределено Тогда
			ЕстьОплатаБонусами = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТЧ, "СуммаСкидкиОплатыБонусом");
		КонецЕсли;
		СуммаОплатыБонусами = 0;
		Если ЕстьОплатаБонусами Тогда
			СуммаОплатыБонусами = СтрокаТЧ.СуммаСкидкиОплатыБонусом; 
		КонецЕсли;
		СуммаТовара = СтрокаТЧ.Сумма + СуммаОплатыБонусами;
