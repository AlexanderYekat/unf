...
//  ОбновлятьЗаписанныйОбъект - Булево - обновить реквизиты для записанного объекта.
//
Процедура ЗаполнитьСвойстваНоменклатурыПоКатегории(Номенклатура, ОбновлятьЗаписанныйОбъект = Ложь, ТолькоЗаполненныеСвойства = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Номенклатура.Ссылка) И НЕ ОбновлятьЗаписанныйОбъект Тогда
		Возврат;
	КонецЕсли;
	
	#Область ФормированиеПеречняСвойств
	
	ЗаполняемыеСвойства = "Весовой, МетодОценки, НаправлениеДеятельности, Склад, Ячейка, ЦеноваяГруппа,
		| СрокИсполненияЗаказа, ВидСтавкиНДС, ПодакцизныйТовар, ЗапретитьВводДробногоКоличества";
	СвойстваСПроверкойЗаполнения = "ЕдиницаИзмерения, СпособПополнения";
	...
	Если Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас
		...
		ЗаполняемыеСвойства = ЗаполняемыеСвойства + ", ИспользоватьХарактеристики, ПроверятьЗаполнениеХарактеристики, ПризнакПредметаРасчета";
		
	КонецЕсли;
	
	Если Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Запас Тогда
		
		ЗаполняемыеСвойства = ЗаполняемыеСвойства + ", СтранаПроисхождения, ИспользоватьНомерПартии,
		| ИспользоватьСерииНоменклатуры, ИспользоватьСрокГодностиСерии, ИспользоватьДатуПроизводстваСерии,
		| ТочностьУказанияСрокаГодностиСерии, ИспользоватьУникальныеСерии, ПолитикаУчетаСерий";
		
		// ИСМП.
		ЗаполняемыеСвойства = ЗаполняемыеСвойства + ", ВидПродукцииИС, ТабачнаяПродукция, ОбувнаяПродукция";
		
		// ЕГАИС.
		СвойстваСПроверкойЗаполнения = СвойстваСПроверкойЗаполнения + ", Производитель, ПроизводительИмпортерАлкогольнойПродукции, ВидАлкогольнойПродукции";
		ЗаполняемыеСвойства = ЗаполняемыеСвойства + ", ИмпортнаяАлкогольнаяПродукция, ПродаетсяВРозлив";
		
		// ВЕТИС.
		ЗаполняемыеСвойства = ЗаполняемыеСвойства + ", ПодконтрольнаяПродукцияВЕТИС, ИспользоватьПроизводителяВЕТИСПартии, ИспользоватьЗаписьСкладскогоЖурналаВЕТИСПартии,
		| ИспользоватьИдентификаторПартииВЕТИСПартии, ИспользоватьДатуПроизводстваПартии, ИспользоватьСрокГодностиПартии, АвтоматическиГенерироватьПартии";
		
		// ГИСМ.
		ЗаполняемыеСвойства = ЗаполняемыеСвойства + ", ВидМаркировки, КиЗГИСМВид, КиЗГИСМСпособВыпускаВОборот, КиЗГИСМРазмер, КиЗГИСМGTIN, ТНВЭДГИСМ";
		
	КонецЕсли;
	
	ПереченьСвойств = ЗаполняемыеСвойства + ", " + СвойстваСПроверкойЗаполнения +
		", ТипНоменклатурыПоУмолчанию, СчетУчетаЗапасов, СчетУчетаЗатрат, СчетУчетаДоходов, СчетУчетаНДСПоПриобретеннымЦенностям, СчетУчетаНДСПоРеализации";
	ДанныеКатегории = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Категория, ПереченьСвойств);
	
	Если ТолькоЗаполненныеСвойства Тогда
		СвойстваСПроверкойЗаполнения = СвойстваСПроверкойЗаполнения + ", " + ЗаполняемыеСвойства;
		ЗаполняемыеСвойства = "";
	КонецЕсли;
	
	#КонецОбласти
	
	#Область ЗаполнениеСвойств
	
	Если ЗначениеЗаполнено(ЗаполняемыеСвойства) Тогда
		ЗаполнитьЗначенияСвойств(Номенклатура, ДанныеКатегории, ЗаполняемыеСвойства);
		
		Если ЗначениеЗаполнено(Номенклатура.ПолитикаУчетаСерий) И НЕ Номенклатура.ИспользоватьСерииНоменклатуры Тогда
			Номенклатура.ПолитикаУчетаСерий = Неопределено;
		КонецЕсли;
		
	КонецЕсли;
	
	ПереченьСвойстваСПроверкой = СтрРазделить(СтрЗаменить(СвойстваСПроверкойЗаполнения, Символы.ПС, ""), ", ", Ложь);
	Для Каждого Свойство Из ПереченьСвойстваСПроверкой Цикл
		Если ЗначениеЗаполнено(ДанныеКатегории[Свойство])Тогда
			Номенклатура[Свойство] = ДанныеКатегории[Свойство];
		КонецЕсли;
	КонецЦикла;
	
	Если ДанныеКатегории.Свойство("ИспользоватьХарактеристики") И НЕ ДанныеКатегории.ИспользоватьХарактеристики Тогда
		Номенклатура.ИспользоватьХарактеристики = Справочники.Номенклатура.ЕстьХарактеристики(Номенклатура.Ссылка);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДанныеКатегории.ТипНоменклатурыПоУмолчанию)Тогда
		Номенклатура.ТипНоменклатуры = ДанныеКатегории.ТипНоменклатурыПоУмолчанию;
	КонецЕсли;
	
	Если Номенклатура.ТипНоменклатуры = ДанныеКатегории.ТипНоменклатурыПоУмолчанию
		И (ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаЗапасов) И НЕ Номенклатура.СчетУчетаЗапасов = ДанныеКатегории.СчетУчетаЗапасов
		Или ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаЗатрат) И НЕ Номенклатура.СчетУчетаЗатрат = ДанныеКатегории.СчетУчетаЗатрат
		Или ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаДоходов) И НЕ Номенклатура.СчетУчетаДоходов = ДанныеКатегории.СчетУчетаДоходов
		Или ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаНДСПоПриобретеннымЦенностям) И НЕ Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ДанныеКатегории.СчетУчетаНДСПоПриобретеннымЦенностям
		Или ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаНДСПоРеализации) И НЕ Номенклатура.СчетУчетаНДСПоРеализации = ДанныеКатегории.СчетУчетаНДСПоРеализации) Тогда
		
		Если ЗначениеЗаполнено(Номенклатура.Ссылка)
		...
		Иначе
			Номенклатура.СчетУчетаЗапасов = ?(ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаЗапасов), ДанныеКатегории.СчетУчетаЗапасов, Номенклатура.СчетУчетаЗапасов);
			Номенклатура.СчетУчетаЗатрат = ?(ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаЗатрат), ДанныеКатегории.СчетУчетаЗатрат, Номенклатура.СчетУчетаЗатрат);
			Номенклатура.СчетУчетаДоходов = ?(ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаДоходов), ДанныеКатегории.СчетУчетаДоходов, Номенклатура.СчетУчетаДоходов);
			Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям = ?(ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаНДСПоПриобретеннымЦенностям), ДанныеКатегории.СчетУчетаНДСПоПриобретеннымЦенностям, Номенклатура.СчетУчетаНДСПоПриобретеннымЦенностям);
			Номенклатура.СчетУчетаНДСПоРеализации = ?(ЗначениеЗаполнено(ДанныеКатегории.СчетУчетаНДСПоРеализации), ДанныеКатегории.СчетУчетаНДСПоРеализации, Номенклатура.СчетУчетаНДСПоРеализации);
		КонецЕсли;
		
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Номенклатура.ВидМаркировки) Тогда
		Номенклатура.ВидМаркировки = Перечисления.ВидыМаркировки.НеМаркируется;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Номенклатура.ПризнакПредметаРасчета) Тогда
		Номенклатура.ПризнакПредметаРасчета = РозничныеПродажиСервер.ПолучитьОжидаемыйПризнакПредметаРасчета(Номенклатура.ТипНоменклатуры, Номенклатура.ВидПродукцииИС, Номенклатура.ВидМаркировки, Номенклатура.ПодакцизныйТовар);
	КонецЕсли;
...
// Возвращает значение заполнения категории из настроек пользовтеля.
// 
// Возвращаемое значение:
//  Категория - Ссылка на категорию номенклатуры.
//
...
// Записывает значение категории в настройки пользователя.
//
// Параметры:
//  Категория - СправочникСсылка.КатегорииНоменклатуры - Ссылка на категорию.
//
