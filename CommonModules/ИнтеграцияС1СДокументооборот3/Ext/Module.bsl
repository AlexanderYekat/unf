...
Функция ВыполнитьДействиеЗадачиЗапрос(Прокси, ДанныеПоЗадаче) Экспорт
	...
	Если ДанныеПоЗадаче.ТребуемоеИнтерактивноеДействие.ВыбратьИсполнителяЗадачи Тогда
		...
		Запрос.interactiveActionResult.Добавить(РезультатXDTO);
...
Процедура ЗаполнитьОбработкуВФорме(Форма, ДанныеОбОбработкеXDTO) Экспорт
	...
	ДоступнаПечать = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("3.0.14.1");
...
Процедура ПриСозданииНаСервереФормыДействия(Форма, Параметры) Экспорт
	...
	Форма.ДоступнаНастройкаОтложенногоНачалаВыполнения =
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("3.0.14.1");
...
Функция УчастникДействияXDTO(Прокси, Исполнитель, ОсновнойОбъектАдресации = Неопределено,
	...
	Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.14.1") Тогда
		Возврат УчастникЗадачXDTO(
			Прокси,
			Исполнитель,
			ОсновнойОбъектАдресации,
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Не корректно задан тип участника действия: %1'"), Исполнитель.Тип);
...
Процедура ЗаполнитьПечатныеФормыОбъектовМетаданных(metadataObjects)
	...
	Для Каждого ОбъектМетаданныхXDTO Из metadataObjects Цикл
		...
		Для Каждого Строка Из ТаблицаПечатныхФорм Цикл
			...
			Попытка
				ПечатнаяФормаXDTO.additionalParameters = ОбщегоНазначения.ЗначениеВJSON(Строка.ДополнительныеПараметры);
			Исключение
...
Процедура ЗаполнитьДанныеДействия(Форма, ФормаXDTO, ИзменениеДействияРазрешено, Разрешения)
	...
	Если ИзменениеДействияРазрешено Тогда
		РазрешенияТаблицей = Форма.РеквизитФормыВЗначение("Разрешения");
...
//   ПараметрыГруппыФормы - см. ИнтеграцияС1СДокументооборот3.СтруктураГруппыФормы
//
...
Функция СтруктураПоляНадписи(ПолеНадписиXDTO)
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ПолеНадписиXDTO, "titleLocation") Тогда
		...
		ИначеЕсли ПолеНадписиXDTO.titleLocation = "Left" Тогда
			ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
		...
		ИначеЕсли ПолеНадписиXDTO.titleLocation = "Bottom" Тогда
			ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Низ;
	...
	Иначе
		ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
	КонецЕсли;
	СтруктураПоляНадписи.Вставить("ПоложениеЗаголовка", ПоложениеЗаголовка);
