...
Функция ПолучитьНачалоИмениМетки(ИмяТЧДанныеМеток) Экспорт
	...
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокПлатежныхПоручений" Тогда
	...
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокОперацииПоПлатежнымКартам" Тогда
		НачалоИмениМетки = "МеткаЭ_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокБанковскиеВыписки" Тогда
	...
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокВозвратКомитенту" Тогда
		НачалоИмениМетки = "МеткаВЗ_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокОтчетКомитенту" Тогда
		НачалоИмениМетки = "МеткаКМ_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокПриемНаКомиссию" Тогда
		НачалоИмениМетки = "МеткаПН_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокСписаниеЗапасов" Тогда
		НачалоИмениМетки = "МеткаСЗ_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокЗаказов" Тогда
		НачалоИмениМетки = "МеткаЗ_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокПП" Тогда
		НачалоИмениМетки = "МеткаПП_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокОтчетПереработчика" Тогда
		НачалоИмениМетки = "МеткаОП_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокОтчетОПереработке" Тогда
		НачалоИмениМетки = "МеткаООП_"; 
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокПриходнаяНакладная" Тогда
		НачалоИмениМетки = "МеткаПН_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокПриходныйОрдер" Тогда
		НачалоИмениМетки = "МеткаПО_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокРасходныйОрдер" Тогда
		НачалоИмениМетки = "МеткаРО_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокВыкупКомиссионером" Тогда
		НачалоИмениМетки = "МеткаВЫ_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокВозвратВыкупленногоОтКомиссионера" Тогда
		НачалоИмениМетки = "МеткаВВ_";
	ИначеЕсли ИмяТЧДанныеМеток = "ДанныеМетокРегулярныеПлатежи" Тогда
...
Процедура ОбновитьЭлементыМеток(Форма, СписокГруппФормыДляУдаленияДобавленныхЭлементов=Неопределено, ИмяТЧДанныеМеток = "ДанныеМеток", СтруктураИменПредставленияПериода = Неопределено) Экспорт
	...
	РаботаСОтборамиКлиентСервер.УстановитьОтображениеКомандыСброситьВсеОтборы(Форма, ИмяТЧДанныеМеток);
...
Процедура УдалитьМеткуОтбораСервер(Форма, СписокОтбора, МеткаИД, ИмяСписка = "", ИмяТЧДанныеМеток = "ДанныеМеток", УстановитьНовыйОтбор = Истина,
	...
	Если УстановитьНовыйОтбор Тогда
		...
		Иначе
			УстановитьПараметрЗапросаСписка(Форма, СписокОтбора, ИмяПоляОтбора, ИмяПараметраЗапроса, ИмяТЧДанныеМеток);
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьМеткиОтбораСервер(Форма, СписокОтбора, Метки = Неопределено, ИмяСписка = "", ИмяТЧДанныеМеток = "ДанныеМеток", СтруктураИменПредставленияПериода = Неопределено, ИмяТЧДанныеОтборов = "ДанныеОтборов") Экспорт
	...
	Если Метки = Неопределено Тогда
		...
		Для Каждого Метка Из ДанныеМеток Цикл
			...
			Если ОтборИМеткиДоступны Тогда
				...
				Метки.Добавить(НомерСтрокиМетки);
				НомерСтрокиМетки = НомерСтрокиМетки + 1;
			КонецЕсли;
	...
	Для каждого ГруппаСОтбором Из СписокГруппФормыДляУдаленияДобавленныхЭлементов Цикл
		ЭлементГруппаСОтбором = Элементы[ГруппаСОтбором];
		Если СтрНачинаетсяС(ЭлементГруппаСОтбором.Имя, "ГруппаОтборОтДо") Или СтрНачинаетсяС(ЭлементГруппаСОтбором.Имя, "ГруппаОтборСПо") Тогда
			Для каждого ЭлементОтбор Из ЭлементГруппаСОтбором.ПодчиненныеЭлементы Цикл
				Форма[ЭлементОтбор.ПутьКДанным] = Неопределено;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого МеткаИД Из Метки Цикл
		...
		Если ИмяПараметраЗапроса="" Тогда
			УстановитьОтборСписка(Форма, СписокОтбора, ИмяПоляОтбора, ИмяСписка,,ИмяТЧДанныеМеток,,ИмяТЧДанныеОтборов);
		Иначе
			УстановитьПараметрЗапросаСписка(Форма, СписокОтбора, ИмяПоляОтбора, ИмяПараметраЗапроса, ИмяТЧДанныеМеток);
		КонецЕсли;
	...
	ОбновитьЭлементыМеток(Форма, СписокГруппФормыДляУдаленияДобавленныхЭлементов, ИмяТЧДанныеМеток, СтруктураИменПредставленияПериода);
...
Функция НомерКартинкиПоСтатусуОплаты(ЗначениеМетки) Экспорт
	...
	Если ЗначениеМетки = "БезОплатыСегодня"
		Или ЗначениеМетки = "БезОтгрузкиСегодня" Тогда
		Возврат 4;
	КонецЕсли;
...
Процедура НастройкаОтборовЗавершение(Форма, АдресВыбранныеОтборы, АдресУдаленныеОтборы, СтруктураИмен = Неопределено) Экспорт
	...
	Для каждого ИмяГруппыРодителя Из УдаляемыеМетки Цикл
		...
		Для каждого СтрокаДанныеМеток Из СтрокиДляУдаления Цикл
			...
			УдалитьМеткуОтбораСервер(Форма, Форма[ИмяРеквизитаСписка], МеткаИД);
...
Процедура Подключаемый_ОтборПриИзменении(Форма, ЭлементИмя, ЭлементРодительИмя, ИмяРеквизитаСписка="Список", ИмяТЧДанныеМеток="ДанныеМеток", ИмяТЧДанныеОтборов="ДанныеОтборов") Экспорт
	...
	Если СтрНачинаетсяС(ЭлементРодительИмя, "ГруппаОтборСПо_") Или СтрНачинаетсяС(ЭлементРодительИмя, "ГруппаОтборОтДо_") Тогда
		...
		Для каждого Метка Из МассивМеток Цикл
			Если СтрНайти(Метка.ПредставлениеМетки, Форма.Элементы[ЭлементИмя].Заголовок) > 0 Тогда
				УдалитьМеткуОтбораСервер(Форма, Форма[ИмяРеквизитаСписка], Форма[ИмяТЧДанныеМеток].Индекс(Метка), ИмяРеквизитаСписка, ИмяТЧДанныеМеток,,,,ИмяТЧДанныеОтборов);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	ПрикрепитьМеткуОтбора(Форма, ИмяПоляОтбора, ЭлементРодительИмя, ВыбранноеЗначение, ПредставлениеЗначения,ИмяРеквизитаСписка,,ИмяТЧДанныеМеток,,,ИмяОтбора,ИмяТЧДанныеОтборов);
	УстановитьОтборСписка(Форма, Форма[ИмяРеквизитаСписка], ИмяПоляОтбора,ИмяРеквизитаСписка,,ИмяТЧДанныеМеток,ИмяОтбора,ИмяТЧДанныеОтборов);
	...
	Если СтрНачинаетсяС(ЭлементИмя, "ОтборС_") 
		...
		Для каждого Метка Из МассивМеток Цикл
			Если СтрНайти(Метка.ПредставлениеМетки, Форма.Элементы[ЭлементИмя].Заголовок) > 0 Тогда
				...
				УдалитьМеткуОтбораСервер(Форма, Форма[ИмяСписка], МеткаИД,ИмяСписка,ИмяТЧДанныеМеток,,,,ИмяТЧДанныеОтборов);
...
Процедура ПеренастроитьПравуюПанельСОтборами(ВыбранныеОтборы, УдаляемыеМетки, УдаляемыеРеквизиты, УдаляемыеЭлементы, Форма, ИмяГруппыОтборов)
	...
	Для каждого ГруппаОтбора Из Форма.Элементы[ИмяГруппыОтборов].ПодчиненныеЭлементы Цикл
		Если ГруппаОтбора.Вид = ВидГруппыФормы.ОбычнаяГруппа Тогда
			...
			Если ЭтоПредопределенныйОтбор И Не ОтборДоступенПоФО Тогда
				Продолжить;
			КонецЕсли;
...
// Заменяет отбор, установленный пользователем в сохраненной настройке списка, на отбор, установленный программно при
// создании формы списка. Вызывается при восстановлении пользовательских настроек динамического списка
