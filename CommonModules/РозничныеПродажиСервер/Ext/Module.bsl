...
Функция ДанныеДляИСМП(ДокументОбъект, ОбщиеПараметры = Неопределено) Экспорт
	...
	Для Каждого СтрокаТовары Из Запасы Цикл
		...
		Для Каждого СтрокаРаспределения Из РаспределениеПоСтроке Цикл
			...
			СтрокаТоварыРазобранные.Цена             = Окр((Сумма + СуммаСкидки) / ?(Коэффициент = 0, 1, Коэффициент), 2);
...
Функция ПолучитьДанныеПродажЗаСмену(КассоваяСмена, ДанныеСмены) Экспорт
	...
	ДанныеСмены.Вставить("СуммаНаличные",  РезультатЗапроса[16].Выгрузить()[0].СуммаНаличными + (РезультатЗапроса[20].Выгрузить()[0].СуммаЧеков - РезультатЗапроса[19].Выгрузить()[0].СуммаБезналичныеПоЧекам - ДанныеСмены.СуммаПредполата));
