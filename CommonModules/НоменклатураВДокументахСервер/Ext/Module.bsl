...
// Возвращает массив типов документов для проверки ввода дробного количества
//
// Возвращаемое значение:
//  Массив - массив типов документов для проверки ввода дробного количества
//
Функция МассивТиповДокументовДляПроверкиДробногоКоличества() Экспорт
	...
	МассивТипов.Добавить(Тип("ДокументСсылка.ПереоценкаВРозницеСуммовойУчет"));
	МассивТипов.Добавить(Тип("ДокументСсылка.ПереоценкаТоваровНаКомиссии"));
	...
	МассивТипов.Добавить(Тип("ДокументСсылка.РасходыПриИмпорте"));
	МассивТипов.Добавить(Тип("ДокументСсылка.РезервированиеЗапасов"));
...
#Область РаботаСНоменклатуройПоставщика

// Возвращает данные номенклатуры для передачи в заполняемые документы.
//
// Параметры:
//  НоменклатураПоставщика - СправочникСсылка.НоменклатураПоставщиков - элемент справочника Номенклатура поставщиков
// 
// Возвращаемое значение:
//  Число - значение остатка у поставщика
//
Функция ОстатокНоменклатурыПоставщика(НоменклатураПоставщика) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОстаткиТоваровПоставщиков.НоменклатураПоставщика КАК НоменклатураПоставщика,
	|	ОстаткиТоваровПоставщиков.Количество КАК Количество
	|ИЗ
	|	РегистрСведений.ОстаткиТоваровПоставщиков КАК ОстаткиТоваровПоставщиков
	|ГДЕ
	|	ОстаткиТоваровПоставщиков.НоменклатураПоставщика = &НоменклатураПоставщика";
...
// Записывает данные остатка в регистр Остатки товаров поставщиков.
//
// Параметры:
//  НоменклатураПоставщика - СправочникСсылка.НоменклатураПоставщиков - элемент справочника Номенклатура поставщиков, для которого сохраняется остаток
//  Количество - Число - значение остатка
// 
Процедура ЗаписатьЗначениеОстаткаНоменклатурыПоставщика(НоменклатураПоставщика, Количество) Экспорт
		...
		Если Количество > 0 Тогда
			...
			НоваяЗапись.НоменклатураПоставщика = НоменклатураПоставщика;
...
// Возвращает данные номенклатуры поставщика по внутренней номенклатуре.
//
// Параметры:
//  Поставщик - СправочникСсылка.Контрагенты - элемент справочника Контрагенты
//  Номенклатура - СправочникСсылка.Номенклатура - элемент справочника Номенклатура
//  Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры - элемент справочника Характеристики номенклатуры
// 
// Возвращаемое значение:
//  НоменклатураПоставщика - СправочникСсылка.НоменклатураПоставщика
//
Функция НоменклатураПоставщикаПоНоменклатуре(Поставщик , Номенклатура, Характеристика) Экспорт
...
// Возвращает номенклатуру и характеристику по номенклатуре поставщика.
//
// Параметры:
//  НоменклатураПоставщика - СправочникСсылка.НоменклатураПоставщиков - элемент справочника Номенклатура поставщиков
// 
// Возвращаемое значение:
//  Структура - Номенклатура и Характеристика номенклатуры
//
Функция НоменклатураИХарактеристикаПоНоменклатуреПоставщика(НоменклатураПоставщика) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", НоменклатураПоставщика);
	...
	СтруктураВозврата = Новый Структура("Номенклатура, Характеристика");
...
// Заполняет строки табличной части документа соответствием номенклатуры поставщиков для номенклатуры.
//
// Параметры:
//  Объект - ДокументОбъект - документ содержащий табличную часть для заполнения
//  ИмяТабличнойЧасти - Строка - имя табличной части документа для заполнения
//  ДополнительныеПараметры - Структура - дополнительные параметры для заполнения
// 
Процедура ЗаполнитьТабличнуюЧастьНоменклатуройПоставщикаПоНоменклатуре(Объект, ИмяТабличнойЧасти = "Запасы", ДополнительныеПараметры = Неопределено) Экспорт
	
	ТабличнаяЧасть = Объект[ИмяТабличнойЧасти]; 
	
	НоменклатураИХарактеристика = ТабличнаяЧасть.Выгрузить(,"Номенклатура, Характеристика");
	НоменклатураИХарактеристика.Свернуть("Номенклатура, Характеристика");
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("НоменклатураИХарактеристика", НоменклатураИХарактеристика);
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НоменклатураИХарактеристика.Номенклатура КАК Номенклатура,
	|	НоменклатураИХарактеристика.Характеристика КАК Характеристика
	|ПОМЕСТИТЬ ВТНоменклатураИХарактеристика
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТНоменклатураИХарактеристика.Номенклатура КАК Номенклатура,
	|	ВТНоменклатураИХарактеристика.Характеристика КАК Характеристика,
	|	НоменклатураПоставщиков.Ссылка КАК НоменклатураПоставщика
	|ИЗ
	|	ВТНоменклатураИХарактеристика КАК ВТНоменклатураИХарактеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НоменклатураПоставщиков КАК НоменклатураПоставщиков
	|		ПО ВТНоменклатураИХарактеристика.Номенклатура = НоменклатураПоставщиков.Номенклатура
	|			И ВТНоменклатураИХарактеристика.Характеристика = НоменклатураПоставщиков.Характеристика
	|ГДЕ
	|	НЕ НоменклатураПоставщиков.Ссылка ЕСТЬ NULL";
	...
	Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		...
		ЗаполнитьЗначенияСвойств(ПараметрыПоиска, СтрокаТабличнойЧасти);
		
		НайденныеСтроки = ТаблицаСоответствия.НайтиСтроки(ПараметрыПоиска);
		
		Если Не НайденныеСтроки.Количество() = 1 Тогда
			Продолжить
		КонецЕсли;
		
		СтрокаТабличнойЧасти.НоменклатураПоставщика = НайденныеСтроки[0].НоменклатураПоставщика;
...
Процедура ЗаполнитьДанныеВСтрокеТабличнойЧасти(Объект, ИмяТабличнойЧасти, СтрокаТабличнойЧасти, ИзКорзины = Ложь) Экспорт
	...
	Если НоменклатураВДокументахКлиентСервер.ЕстьРеквизитОбъекта("Номенклатура", СтрокаТабличнойЧасти) Тогда
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	ДанныеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураДанные.Номенклатура,
		"ЕдиницаИзмерения, ТипНоменклатуры, СтранаПроисхождения, ВидСтавкиНДС, ФиксированнаяСтоимость, ТоварнаяНоменклатураВЭД,
		|ИспользоватьХарактеристики, ИспользоватьСерииНоменклатуры, Склад, Ячейка");
	
	СтруктураДанные.Вставить("БазоваяЕдиницаИзмерения", ДанныеНоменклатуры.ЕдиницаИзмерения);
	...
	СтруктураДанные.Вставить("Ячейка", ДанныеНоменклатуры.Ячейка);
	...
	ИначеЕсли ЗначениеЗаполнено(ДанныеНоменклатуры.ВидСтавкиНДС) Тогда
		СтруктураДанные.Вставить("СтавкаНДС", Справочники.СтавкиНДС.СтавкаНДС(ДанныеНоменклатуры.ВидСтавкиНДС, ?(ЗначениеЗаполнено(СтруктураДанные.ДатаОбработки), СтруктураДанные.ДатаОбработки, ТекущаяДатаСеанса())));
	Иначе
	...
	Если СтруктураДанные.Свойство("КодТНВЭД") Тогда
		СтруктураДанные.КодТНВЭД = ДанныеНоменклатуры.ТоварнаяНоменклатураВЭД;
	КонецЕсли;
	...
	Если СтруктураДанные.Свойство("ХарактеристикиИспользуются") Тогда
		СтруктураДанные.ХарактеристикиИспользуются = 
			...
			И ДанныеНоменклатуры.ИспользоватьХарактеристики;
	КонецЕсли;
	...
	Если СтруктураДанные.Свойство("СтатусУказанияСерий") Тогда
		СтруктураДанные.СтатусУказанияСерий = 
			...
			И ДанныеНоменклатуры.ИспользоватьСерииНоменклатуры;
	КонецЕсли;
...
Функция СформироватьГарантийныеТалоныДокумента(ТабличныйДокумент, ТекущийДокумент, Ошибки) Экспорт
	...
	|	ДокПечати.ТекстУсловийГарантийногоТалона,
	|	ДокПечати.Запасы.(
	...
	Если ЗначениеЗаполнено(Шапка.ФайлЛоготип) Тогда
		...
		Если ЗначениеЗаполнено(ДанныеКартинки) Тогда
			...
			ПечатьДокументовУНФ.УстановитьКартинкуБезопасно(ОбластьМакета.Рисунки, "Логотип", КартинкаЛоготипа);
	...
	Если ЗначениеЗаполнено(Шапка.УсловияГарантийногоТалона) И
		...
		Если НЕ ПустаяСтрока(Шапка.ТекстУсловийГарантийногоТалона) Тогда
			ТекстДополнительныхУсловий = Шапка.ТекстУсловийГарантийногоТалона;
		Иначе
			ТекстДополнительныхУсловий = 
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Шапка.УсловияГарантийногоТалона, "ТекстУсловий");
		КонецЕсли;
...
// Заполняет поля табличной части Запасы признаками использования характеристик и партий
//
// Параметры:
//  ОткрытиеФормы - Булево - Устанавливается в значение "Истина", если заполнение происходит при открытии формы. 
//  ИменаТЧДляИсключения - Строка, Массив - Перечисляются табличные части, которые не следует обрабатывать. 
Процедура ЗаполнитьПризнакиИспользованияХарактеристик(Объект, ОткрытиеФормы = Ложь, ИменаТЧДляИсключения = Неопределено) Экспорт
	
	СерииНоменклатурыУНФ.ЗаполнитьПризнакиИспользованияСерий(Объект);
	ЗаполнитьПрочиеДополнительныеРеквизитыТабличныхЧастей(Объект);
	...
	МассивИменТабличныхЧастейИсключений = Новый Массив;
	Если ТипЗнч(ИменаТЧДляИсключения) = Тип("Строка") Тогда
		ИменаТЧ = СтрРазделить(ИменаТЧДляИсключения,",");
		Для каждого ИмяТЧ Из ИменаТЧ Цикл
			МассивИменТабличныхЧастейИсключений.Добавить(СокрЛП(ИмяТЧ));
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти	Цикл
		...
		Если НЕ ТабличнаяЧасть.Реквизиты.Найти("Характеристика") = Неопределено
			И МассивИменТабличныхЧастейИсключений.Найти(ТабличнаяЧасть.Имя) = Неопределено
			Тогда
...
// Заполняет поля табличных частей необходимыми признаками для доп. обработки данных в табличной части. 
//
// Параметры:
//  Объект - Объект - Объект формы
Процедура ЗаполнитьПрочиеДополнительныеРеквизитыТабличныхЧастей(Объект) Экспорт
	
	ИмяОсновнойТабличнойЧасти = "Запасы";
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
		ИмяОсновнойТабличнойЧасти = "РаботыИУслуги";
	КонецЕсли;
	
	ИмяТаблицыВозвраты = "Возвраты";
	
	ЭтоЧекККМ = ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЧекККМ");
	ЭтоВводНачальныхОстатков = ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ВводНачальныхОстатков");
	ЭтоЗаказНаряд = ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") И Объект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд;
	ЭтоЗаказПокупателя = ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
								И (Объект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу 
								Или Объект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПереработку);
	...
	ПоддерживаемыеТипыДокументовКомиссия = МассивТиповДокументовДляПроверкиКомиссииСНДС();
	ПоддерживаемыеТипыДокументовСкладскойУчет = МассивТиповОбрабатываемыхДокументовСкладскойУчет();
	ПоддерживаемыеТипыДокументовДляПроверкиДробногоКоличества = МассивТиповДокументовДляПроверкиДробногоКоличества();
	
	ОбработкаТабличныхЧастейПоКомиссии = ПриемТоваровНаКомиссию И НЕ ПоддерживаемыеТипыДокументовКомиссия.Найти(ТипЗнч(Объект.Ссылка)) = Неопределено;
	ОбработкаТабличныхЧастейСкладскойУчет = ИспользоватьОрдерныйСклад И НЕ ПоддерживаемыеТипыДокументовСкладскойУчет.Найти(ТипЗнч(Объект.Ссылка)) = Неопределено;
	ОбработкаТабличныхЧастейДробноеКоличество = НЕ ПоддерживаемыеТипыДокументовДляПроверкиДробногоКоличества.Найти(ТипЗнч(Объект.Ссылка)) = Неопределено;
	
	ОбрабатыватьТабличнуюЧастьРасходы = Ложь;
	ОбрабатыватьТабличнуюЧастьВозвраты = Ложь;
	ОбрабатыватьТабличнуюЧастьПродукция = Ложь;
	ОбрабатыватьТабличнуюЧастьМатериалы = Ложь;
	ОбрабатыватьТабличнуюЧастьОтходы = Ложь;
	ОбрабатыватьТабличнуюЧастьСпецоснастка = Ложь;
	ОбрабатыватьТабличнуюЧастьИнвентарьИХозяйственныеПринадлежности = Ложь;
	ОбрабатыватьТабличнуюЧастьРаспределениеЗапасов = Ложь;
	
	//Признак запрета ввода дробного количества
	МассивНоменклатурыДляПроверкиДробногоКоличества = Новый Массив;
	Если ОбработкаТабличныхЧастейДробноеКоличество Тогда
		
			ТаблицаНоменклатуры = Объект[ИмяОсновнойТабличнойЧасти].Выгрузить(,"Номенклатура");
			
			Если Не МассивТиповДокументовПоИмениТабличнойЧасти("Расходы").Найти(ТипЗнч(Объект.Ссылка)) = Неопределено Тогда
				ОбрабатыватьТабличнуюЧастьРасходы = Истина;
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Объект["Расходы"].Выгрузить(,"Номенклатура"), ТаблицаНоменклатуры);
			КонецЕсли;
			
			Если Не МассивТиповДокументовПоИмениТабличнойЧасти("Продукция").Найти(ТипЗнч(Объект.Ссылка)) = Неопределено Тогда
				ОбрабатыватьТабличнуюЧастьПродукция = Истина;
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Объект["Продукция"].Выгрузить(,"Номенклатура"), ТаблицаНоменклатуры);
			КонецЕсли;
			
			Если Не МассивТиповДокументовПоИмениТабличнойЧасти("Отходы").Найти(ТипЗнч(Объект.Ссылка)) = Неопределено Тогда
				ОбрабатыватьТабличнуюЧастьОтходы = Истина;
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Объект["Отходы"].Выгрузить(,"Номенклатура"), ТаблицаНоменклатуры);
			КонецЕсли;
			
			Если Не МассивТиповДокументовПоИмениТабличнойЧасти("РаспределениеЗапасов").Найти(ТипЗнч(Объект.Ссылка)) = Неопределено Тогда
				ОбрабатыватьТабличнуюЧастьРаспределениеЗапасов = Истина;
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Объект["РаспределениеЗапасов"].Выгрузить(,"Номенклатура"), ТаблицаНоменклатуры);
			КонецЕсли;
			
			Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
				ОбрабатыватьТабличнуюЧастьВозвраты = Истина;
				ИмяТаблицыВозвраты = "ЗапасыВозвраты";
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Объект[ИмяТаблицыВозвраты].Выгрузить(,"Номенклатура"), ТаблицаНоменклатуры);
			КонецЕсли;
			
			Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПеремещениеЗапасов") Тогда
				ОбрабатыватьТабличнуюЧастьСпецоснастка = Истина;
				ОбрабатыватьТабличнуюЧастьИнвентарьИХозяйственныеПринадлежности = Истина;
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Объект["Спецоснастка"].Выгрузить(,"Номенклатура"), ТаблицаНоменклатуры);
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Объект["ИнвентарьИХозяйственныеПринадлежности"].Выгрузить(,"Номенклатура"), ТаблицаНоменклатуры);
			КонецЕсли;
			
			Если ЭтоЗаказНаряд 
				Или ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
				ОбрабатыватьТабличнуюЧастьМатериалы = Истина;
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Объект["Материалы"].Выгрузить(,"Номенклатура"), ТаблицаНоменклатуры);
			КонецЕсли;
			
			Если ЭтоВводНачальныхОстатков Тогда
				
				МассивИменТаблицВводаНачальныхОстатков = Новый Массив;
				
				МассивИменТаблицВводаНачальныхОстатков.Добавить("ЗапасыПереданные");
				МассивИменТаблицВводаНачальныхОстатков.Добавить("ЗапасыПринятые");
				МассивИменТаблицВводаНачальныхОстатков.Добавить("ЗапасыВРазрезеГТД");
				МассивИменТаблицВводаНачальныхОстатков.Добавить("ЗапасыПереданныеВРазрезеГТД");
				МассивИменТаблицВводаНачальныхОстатков.Добавить("ЗапасыПринятыеВРазрезеГТД");
				
				Для Каждого ИмяТаблицыДокумента Из МассивИменТаблицВводаНачальныхОстатков Цикл
					ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Объект[ИмяТаблицыДокумента].Выгрузить(,"Номенклатура"), ТаблицаНоменклатуры);
				КонецЦикла;
			...
			|	Справочник.Номенклатура КАК Номенклатура
			|ГДЕ
			|	Номенклатура.Ссылка В(&ТаблицаНоменклатуры)
			|	И Номенклатура.ЗапретитьВводДробногоКоличества";
			
			МассивНоменклатурыДляПроверкиДробногоКоличества = Запрос.Выполнить().Выгрузить();
			
	КонецЕсли;
	
	ОбработкаТабличныхЧастейДробноеКоличество = ОбработкаТабличныхЧастейДробноеКоличество И МассивНоменклатурыДляПроверкиДробногоКоличества.Количество();
	
	//Признак НДС в комиссионных партиях
	МассивКомиссионныхПартийСНДС = Новый Массив;
	Если ОбработкаТабличныхЧастейПоКомиссии Тогда
		
		Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЧекККМ") Тогда
			КопияТабличнойЧастиДляПроверки = Объект[ИмяОсновнойТабличнойЧасти].Выгрузить(Новый Структура());
			КолонкаКомиссияСНДС = КопияТабличнойЧастиДляПроверки.Колонки.Найти("КомиссияСНДС");
			
			Если КолонкаКомиссияСНДС = Неопределено Тогда
				ОбработкаТабличныхЧастейПоКомиссии = Ложь;
			КонецЕсли;
		...
		Если ОбработкаТабличныхЧастейПоКомиссии Тогда
			...
			МассивПартий = Объект[ИмяОсновнойТабличнойЧасти].Выгрузить(,"Партия");
			...
			МассивКомиссионныхПартийСНДС = Запрос.Выполнить().Выгрузить();
			
		КонецЕсли;
		
	КонецЕсли;
	
	ОбработкаТабличныхЧастейПоКомиссии = ОбработкаТабличныхЧастейПоКомиссии И МассивКомиссионныхПартийСНДС.Количество();
	
	//Признак указания складского ордера в табличной части документа
	ИмяРеквизитаСтруктурнаяЕдиница = "СтруктурнаяЕдиница";
	МассивСтруктурныхЕдиницСОстаткамиПоОрдерам = Новый Массив;
	Если ОбработкаТабличныхЧастейСкладскойУчет Тогда
		
		Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПриходнаяНакладная") 
			Или ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РасходнаяНакладная") Тогда
			Если Объект.ПоложениеДокументаПоступления = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке Тогда
				ОбработкаТабличныхЧастейСкладскойУчет = Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	ОбработкаТабличныхЧастейСкладскойУчет = ОбработкаТабличныхЧастейСкладскойУчет И МассивСтруктурныхЕдиницСОстаткамиПоОрдерам.Количество() И Не ЭтоЗаказПокупателя;
	
	Если Не ОбработкаТабличныхЧастейПоКомиссии И Не ОбработкаТабличныхЧастейСкладскойУчет И Не ОбработкаТабличныхЧастейДробноеКоличество Тогда
		Возврат
	КонецЕсли;
	
	Для Каждого СтрокаЗапасы Из Объект[ИмяОсновнойТабличнойЧасти] Цикл

		Если ОбработкаТабличныхЧастейСкладскойУчет Тогда
			Если Не ЗначениеЗаполнено(СтрокаЗапасы[ИмяРеквизитаСтруктурнаяЕдиница]) Тогда
				СтрокаЗапасы.УказыватьДокументПоступления = Ложь;
			КонецЕсли;
		...
		Если ОбработкаТабличныхЧастейПоКомиссии Тогда
			Если Не ЗначениеЗаполнено(СтрокаЗапасы.Партия) Тогда
				СтрокаЗапасы.КомиссияСНДС = Ложь;
			КонецЕсли;
			СтрокаЗапасы.КомиссияСНДС = Не МассивКомиссионныхПартийСНДС.Найти(СтрокаЗапасы.Партия) = Неопределено;
		КонецЕсли;
		
		Если ОбработкаТабличныхЧастейДробноеКоличество Тогда
			
			Если ЭтоЧекККМ И Не СтрокаЗапасы.Свойство("ЗапретитьВводДробногоКоличества") Тогда
				Продолжить
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СтрокаЗапасы.Номенклатура) Тогда
				СтрокаЗапасы.ЗапретитьВводДробногоКоличества = Ложь;
			КонецЕсли;
			СтрокаЗапасы.ЗапретитьВводДробногоКоличества = Не МассивНоменклатурыДляПроверкиДробногоКоличества.Найти(СтрокаЗапасы.Номенклатура) = Неопределено;
			
		КонецЕсли;
		
	КонецЦикла;
	
	Если ОбрабатыватьТабличнуюЧастьМатериалы Тогда
		...
		Для Каждого СтрокаМатериалы Из Объект.Материалы Цикл
			...
			Если ОбработкаТабличныхЧастейСкладскойУчет Тогда
				...
				Если Не ЗначениеЗаполнено(СтрокаМатериалы[ИмяРеквизитаСтруктурнаяЕдиница]) Тогда
					СтрокаМатериалы.УказыватьДокументПоступления = Ложь;
				КонецЕсли;
			...
			Если ОбработкаТабличныхЧастейДробноеКоличество Тогда
				Если Не ЗначениеЗаполнено(СтрокаМатериалы.Номенклатура) Тогда
					СтрокаМатериалы.ЗапретитьВводДробногоКоличества = Ложь;
				КонецЕсли;
				СтрокаМатериалы.ЗапретитьВводДробногоКоличества = Не МассивНоменклатурыДляПроверкиДробногоКоличества.Найти(СтрокаМатериалы.Номенклатура) = Неопределено;
			КонецЕсли;
	...
	Если ОбработкаТабличныхЧастейДробноеКоличество Тогда
		...
		Если ОбрабатыватьТабличнуюЧастьРасходы Тогда 
			
			Для Каждого СтрокаРасходы Из Объект.Расходы Цикл
				
				Если Не ЗначениеЗаполнено(СтрокаРасходы.Номенклатура) Тогда
					СтрокаРасходы.ЗапретитьВводДробногоКоличества = Ложь;
				КонецЕсли;
				СтрокаРасходы.ЗапретитьВводДробногоКоличества = Не МассивНоменклатурыДляПроверкиДробногоКоличества.Найти(СтрокаРасходы.Номенклатура) = Неопределено;
				
			КонецЦикла;
			
		КонецЕсли;
		
		Если ОбрабатыватьТабличнуюЧастьВозвраты Тогда 
			
			Для Каждого СтрокаВозвраты Из Объект[ИмяТаблицыВозвраты] Цикл
				
				Если Не ЗначениеЗаполнено(СтрокаВозвраты.Номенклатура) Тогда
					СтрокаВозвраты.ЗапретитьВводДробногоКоличества = Ложь;
				КонецЕсли;
				СтрокаВозвраты.ЗапретитьВводДробногоКоличества = Не МассивНоменклатурыДляПроверкиДробногоКоличества.Найти(СтрокаВозвраты.Номенклатура) = Неопределено;
				
			КонецЦикла;
			
		КонецЕсли;
		
		Если ОбрабатыватьТабличнуюЧастьПродукция Тогда 
			
			Для Каждого СтрокаПродукция Из Объект.Продукция Цикл
				
				Если Не ЗначениеЗаполнено(СтрокаПродукция.Номенклатура) Тогда
					СтрокаПродукция.ЗапретитьВводДробногоКоличества = Ложь;
				КонецЕсли;
				СтрокаПродукция.ЗапретитьВводДробногоКоличества = Не МассивНоменклатурыДляПроверкиДробногоКоличества.Найти(СтрокаПродукция.Номенклатура) = Неопределено;
				
			КонецЦикла;
			
		КонецЕсли;
		
		Если ОбрабатыватьТабличнуюЧастьОтходы Тогда 
			
			Для Каждого СтрокаОтходы Из Объект.Отходы Цикл
				
				Если Не ЗначениеЗаполнено(СтрокаОтходы.Номенклатура) Тогда
					СтрокаОтходы.ЗапретитьВводДробногоКоличества = Ложь;
				КонецЕсли;
				СтрокаОтходы.ЗапретитьВводДробногоКоличества = Не МассивНоменклатурыДляПроверкиДробногоКоличества.Найти(СтрокаОтходы.Номенклатура) = Неопределено;
				
			КонецЦикла;
			
		КонецЕсли;
		
		Если ОбрабатыватьТабличнуюЧастьСпецоснастка Тогда 
			
			Для Каждого СтрокаСпецоснастка Из Объект.Спецоснастка Цикл
				
				Если Не ЗначениеЗаполнено(СтрокаСпецоснастка.Номенклатура) Тогда
					СтрокаСпецоснастка.ЗапретитьВводДробногоКоличества = Ложь;
				КонецЕсли;
				СтрокаСпецоснастка.ЗапретитьВводДробногоКоличества = Не МассивНоменклатурыДляПроверкиДробногоКоличества.Найти(СтрокаСпецоснастка.Номенклатура) = Неопределено;
				
			КонецЦикла;
			
		КонецЕсли;
		
		Если ОбрабатыватьТабличнуюЧастьИнвентарьИХозяйственныеПринадлежности Тогда 
			
			Для Каждого СтрокаИнвентарьИХозяйственныеПринадлежности Из Объект.ИнвентарьИХозяйственныеПринадлежности Цикл
				
				Если Не ЗначениеЗаполнено(СтрокаИнвентарьИХозяйственныеПринадлежности.Номенклатура) Тогда
					СтрокаИнвентарьИХозяйственныеПринадлежности.ЗапретитьВводДробногоКоличества = Ложь;
				КонецЕсли;
				СтрокаИнвентарьИХозяйственныеПринадлежности.ЗапретитьВводДробногоКоличества = Не МассивНоменклатурыДляПроверкиДробногоКоличества.Найти(СтрокаИнвентарьИХозяйственныеПринадлежности.Номенклатура) = Неопределено;
				
			КонецЦикла;
			
		КонецЕсли;
		
		Если ОбрабатыватьТабличнуюЧастьРаспределениеЗапасов Тогда 
			
			Для Каждого СтрокаРаспределениеЗапасов Из Объект.РаспределениеЗапасов Цикл
				
				Если Не ЗначениеЗаполнено(СтрокаРаспределениеЗапасов.Номенклатура) Тогда
					СтрокаРаспределениеЗапасов.ЗапретитьВводДробногоКоличества = Ложь;
				КонецЕсли;
				СтрокаРаспределениеЗапасов.ЗапретитьВводДробногоКоличества = Не МассивНоменклатурыДляПроверкиДробногоКоличества.Найти(СтрокаРаспределениеЗапасов.Номенклатура) = Неопределено;
		...
		Если ЭтоВводНачальныхОстатков Тогда
			...
			Для Каждого ИмяТаблицыДокумента Из МассивИменТаблицВводаНачальныхОстатков Цикл
				...
				Для Каждого СтрокаТаблицы Из Объект[ИмяТаблицыДокумента] Цикл
					
					Если Не ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) Тогда
						СтрокаТаблицы.ЗапретитьВводДробногоКоличества = Ложь;
					КонецЕсли;
					
					СтрокаТаблицы.ЗапретитьВводДробногоКоличества = Не МассивНоменклатурыДляПроверкиДробногоКоличества.Найти(СтрокаТаблицы.Номенклатура) = Неопределено;
...
Процедура ПроверитьЗаполнениеХарактеристик(Объект, Отказ, ПроверятьЗаполнениеПартий = Ложь, ИменаТЧДляИсключенияПроверкиПартий = Неопределено) Экспорт
	...
	Для Каждого ИмяТабличнойЧасти Из МассивИменТабличныхЧастей Цикл
		...
		Если ЭтоДокументПересортицаЗапасов Тогда
			ТаблицаРеквизитов = ТаблицаЗначенийРеквизитовНоменклатурыТабличнойЧастиПересортица(Объект[ИмяТабличнойЧасти]);
		Иначе
		...
		Для Каждого СтрокаЗапасы Из Объект[ИмяТабличнойЧасти] Цикл
			...
			Если ЭтоДокументПересортицаЗапасов Тогда
				...
				ПроверятьЗаполнениеХарактеристикиОприходование = ТаблицаРеквизитов[НомерСтроки-1].ПроверятьЗаполнениеХарактеристикиОприходование;
				ПроверятьЗаполнениеПартийОприходование = ТаблицаРеквизитов[НомерСтроки-1].ПроверятьЗаполнениеПартийОприходование;
				
				ПроверятьЗаполнениеХарактеристикВТабЧастиОприходование = ПроверятьЗаполнениеХарактеристикиОприходование И ИспользоватьХарактеристики;
				ПроверятьЗаполнениеПартийВТабЧастиОприходование = ПроверятьЗаполнениеПартийОприходование И ИспользоватьПартии;
				
				Если ПроверятьЗаполнениеХарактеристикВТабЧастиОприходование
				...
				Если ПроверятьПартии
					И ПроверятьЗаполнениеПартийВТабЧастиОприходование
...
Процедура ОбновитьУсловноеОформлениеТабличнойЧастиДляХарактеристик(Форма, СоответствиеИменТабличныхЧастей = Неопределено) Экспорт  
	...
	//// Код для серий номенклатуры.
	//Если ПолучитьФункциональнуюОпцию("ИспользоватьСерииНоменклатуры") Тогда
	//	
	//	ЗначениеПоискаПолеСерия = Форма.Элементы.Найти("ЗапасыСерииНоменклатуры");
	//	ЗначениеПоискаПолеСтатус = Форма.Элементы.Найти("ЗапасыСтатусыСерийНоменклатуры");
	//	
	//	Если НЕ ЗначениеПоискаПолеСерия = Неопределено И НЕ ЗначениеПоискаПолеСтатус = Неопределено Тогда
	//		
	//		ИмяПоляИспользоватьСерии = "Объект.Запасы.СтатусыСерийНоменклатуры";
	//		
	//		НовоеУсловноеОформление = Форма.УсловноеОформление.Элементы.Добавить();
	//		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, ИмяПоляИспользоватьСерии, 0, ВидСравненияКомпоновкиДанных.Равно);
	//		
	...
	//		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, ИмяПоляИспользоватьСерии, 0, ВидСравненияКомпоновкиДанных.Равно);
	//		
	//		РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "ЗапасыСтатусыСерийНоменклатуры");
		...
		Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
			...
			Если ИспользоватьПартии И НЕ ТабличнаяЧасть.Реквизиты.Найти("Партия") = Неопределено Тогда
				...
				Если НЕ ЗначениеПоиска = Неопределено И ТипЗнч(ЗначениеПоиска.Родитель) = Тип("ТаблицаФормы") Тогда
					...
					СписокИсключенийВидовОпераций.Добавить(Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратКомитенту);
					СписокИсключенийВидовОпераций.Добавить(Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратИзПереработки);
					СписокИсключенийВидовОпераций.Добавить(Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратСОтветХранения);
...
// Заполняет значения колонки "ОстатокОбщий" для таблицы "ТЧНоменклатура" формы документа.
//
// Параметры:
//  ТЧНоменклатура     - ДанныеФормыКоллекция - таблица формы.
//  НоменклатураОтбора - Массив.  
Процедура ЗаполнитьЗначенияОстатковТЧНоменклатураУстановкаЦен(ТЧНоменклатура, НоменклатураОтбора = Неопределено) Экспорт
		
	Если ТЧНоменклатура.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;         
				
	ТаблицаНоменклатуры = ДанныеФормыВЗначение(ТЧНоменклатура, Тип("ТаблицаЗначений"));
	
	Если НоменклатураОтбора = Неопределено Тогда    
		
		ВнешняяТаблицаНоменклатуры = ТаблицаНоменклатуры.Скопировать(, "Номенклатура");
	Иначе 
					
		МассивСтрокКопировать = СформироватьМассивСтрокДляКопированияУстановкаЦен(ТаблицаНоменклатуры, НоменклатураОтбора);   	
		Если МассивСтрокКопировать.Количество() = 0 Тогда  
			Возврат;
		КонецЕсли;  
				
		ВнешняяТаблицаНоменклатуры = ТаблицаНоменклатуры.Скопировать(МассивСтрокКопировать, "Номенклатура"); 
		
	КонецЕсли;
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("ВнешняяТаблицаНоменклатуры", ВнешняяТаблицаНоменклатуры);

	Запрос.Текст = ТекстЗапросаОстаткиУстановкаЦенНоменклатуры(Ложь);
	...
	ЗаполнитьКолонкуОстатокОбщийУстановкаЦен(ТЧНоменклатура, ТаблицаОстатков);
	
КонецПроцедуры 

// Возвращает ТаблицуЗначений, с заполненной колонкой "ОстатокОбщий".
//
// Параметры:
//  ТЧНоменклатура 		- ТаблицаЗначений.
//  НоменклатураОтбора 	- Массив (необязательный). 
//
// Возвращаемое значение:
// 	ТаблицаЗначений
Функция ЗаполнитьЗначенияОстатковТЧНоменклатураУстановкаЦен_ВФоне(ТЧНоменклатура,
	НоменклатураОтбора = Неопределено) Экспорт
	...
	Если НоменклатураОтбора = Неопределено Тогда 
		...
		ВнешняяТаблицаНоменклатуры = ТЧНоменклатура.Скопировать(, "Номенклатура");		
	Иначе 
				
		МассивСтрокКопировать = СформироватьМассивСтрокДляКопированияУстановкаЦен(ТЧНоменклатура, НоменклатураОтбора); 	
		Если МассивСтрокКопировать.Количество() = 0 Тогда  
			Возврат Неопределено;
		КонецЕсли; 
		
		ВнешняяТаблицаНоменклатуры = ТЧНоменклатура.Скопировать(МассивСтрокКопировать, "Номенклатура"); 		
	КонецЕсли;
...
// Возвращает Список значений, в котором хранятся значения остатков в разрезе складов
//
// Параметры:
//  ИмяТаблицыДанныхФормы 	- Строка.
//  СтруктураПараметров 	- Структура с дополнительными параметрами. 
//
// Возвращаемое значение:
// 	СписокЗначений	
Функция СформироватьСписокСкладовОстаткиУстановкаЦен(ИмяТаблицыДанныхФормы, СтруктураПараметров) Экспорт
	...
	Запрос.Текст = ТекстЗапросаСписокСкладовОстаткиУстановкаЦенНоменклатуры(ИмяТаблицыДанныхФормы);
...
// Заполняет значения колонки "ОстатокОбщий" для формы документа.
//
// Параметры:
// 	ТЧХарактеристики 	- ДанныеФормыКоллекция 	- таблица формы.
// 	НоменклатураОтбора 	- Ссылка 				- элемент справочника "Номенклатура", которая является владельцем характеристики. 
Процедура ЗаполнитьЗначенияОстатковНоменклатурыТЧХарактеристикиУстановкаЦен(ТЧХарактеристики, НоменклатураОтбора) Экспорт
	
	Если ТЧХарактеристики.Количество() = 0 Тогда
		Возврат;
	КонецЕсли; 		 
	
	ТаблицаХарактеристик = ДанныеФормыВЗначение(ТЧХарактеристики, Тип("ТаблицаЗначений")); 	
	МассивХарактеристик = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаХарактеристик, "Характеристика");
		
	Запрос = Новый Запрос;	 
	
	Запрос.УстановитьПараметр("НоменклатураОтбора"	, НоменклатураОтбора); 
	Запрос.УстановитьПараметр("МассивХарактеристик"	, МассивХарактеристик);	
	
	Запрос.Текст = ТекстЗапросаОстаткиУстановкаЦенНоменклатуры(Истина); 
	...
	ЗаполнитьКолонкуОстатокОбщийУстановкаЦен(ТЧХарактеристики, ТаблицаОстатков);
...
//
// Параметры:
//  ТЧХарактеристики 	- ТаблицаЗначений.
//  НоменклатураОтбора 	- Ссылка - элемент справочника "Номенклатура", которая является владельцем характеристики 
//
// Возвращаемое значение:
// 	ТаблицаЗначений
Функция ЗаполнитьЗначенияОстатковНоменклатурыТЧХарактеристикиУстановкаЦен_ВФоне(ТЧХарактеристики,
	НоменклатураОтбора) Экспорт
	
	Если ТЧХарактеристики.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли; 		 
		
	МассивХарактеристик = ОбщегоНазначения.ВыгрузитьКолонку(ТЧХарактеристики, "Характеристика");
...
// Заполняет колонки табличной части документа Заказ на перемещение значениями по остаткам и резервам
//
// Параметры:
//  Объект - Ссылка на Заказ на производство
//  СтруктураДанные - Структура дополнительных параметров
// 
Процедура ЗаполнитьНачальныеДанныеДляОбработкиТабличнойЧастиЗаказНаПеремещение(Объект, СтруктураДанные) Экспорт
	...
	Если Не СтрокиДляВыгрузки = Неопределено Тогда
		ТаблицаНоменклатуры = Объект.Запасы.Выгрузить(СтрокиДляВыгрузки,"Номенклатура, Характеристика, Партия, Количество, ЕдиницаИзмерения, ЗаказПокупателя");
		СтрокиОбходаЗапасов = СтрокиДляВыгрузки;
	Иначе
		ТаблицаНоменклатуры = Объект.Запасы.Выгрузить(,"Номенклатура, Характеристика, Партия, Количество, ЕдиницаИзмерения, ЗаказПокупателя");
		СтрокиОбходаЗапасов = Объект.Запасы;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("Характеристика", МассивХарактеристик);
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Отправитель", Объект.СтруктурнаяЕдиницаРезерв);
	Запрос.УстановитьПараметр("Получатель", Объект.СтруктурнаяЕдиницаПолучатель);
	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	Запрос.УстановитьПараметр("УчетПотребностиПоСкладам", ПолучитьФункциональнуюОпцию("УчетПотребностиПоСкладам"));
	
	МассивСкладов = Новый Массив;
	МассивСкладов.Добавить(Объект.СтруктурнаяЕдиницаРезерв);
	МассивСкладов.Добавить(Объект.СтруктурнаяЕдиницаПолучатель);
	
	Запрос.УстановитьПараметр("СтруктурныеЕдиницы", МассивСкладов);
	
	Запрос.Текст = ТекстЗапросаОстаткиИРезервыЗаказНаПеремещение();
	...
	ПараметрыОтбора.Вставить("ЗаказПокупателя");
	...
	Для Каждого СтрокаТабличнойЧасти Из СтрокиОбходаЗапасов Цикл
		...
		Если СтрокаОстатка = Неопределено Тогда
			СтрокаТабличнойЧасти.НеОтгружено = 0;
			СтрокаТабличнойЧасти.СвободноОтправитель = 0;
			СтрокаТабличнойЧасти.ОстатокОбщий = 0;
			СтрокаТабличнойЧасти.СвободноПолучатель = 0;
			СтрокаТабличнойЧасти.КПоступлению = 0;
			СтрокаТабличнойЧасти.МаксимальныйОстаток = 0;
			СтрокаТабличнойЧасти.МинимальныйОстаток = 0;
			Продолжить
		КонецЕсли;
		
		СтрокаТабличнойЧасти.СвободноОтправитель = Окр(СтрокаОстатка.СвободноОтправитель/КоэффициентЕдиницыИзмерения,3);
		СтрокаТабличнойЧасти.СвободноПолучатель = Окр(СтрокаОстатка.СвободноПолучатель/КоэффициентЕдиницыИзмерения,3);
		СтрокаТабличнойЧасти.ОстатокОбщий = Окр(СтрокаОстатка.ОстатокВсего/КоэффициентЕдиницыИзмерения,3);
		СтрокаТабличнойЧасти.МаксимальныйОстаток = Окр(СтрокаОстатка.МаксимальныйОстаток/КоэффициентЕдиницыИзмерения,3);
		СтрокаТабличнойЧасти.МинимальныйОстаток = Окр(СтрокаОстатка.МинимальныйОстаток/КоэффициентЕдиницыИзмерения,3);
		СтрокаТабличнойЧасти.НеОтгружено = Окр(СтрокаОстатка.НеОтгружено/КоэффициентЕдиницыИзмерения,3);
		СтрокаТабличнойЧасти.Заказано = Окр(СтрокаОстатка.Заказано/КоэффициентЕдиницыИзмерения,3);
		
		
		Если СтрокаОстатка.КоличествоДокументов > 1 Тогда
			
			РасшифровкаСчетчика = ПредметИсчисления(
			СтрокаОстатка.КоличествоДокументов,
			НСтр("ru = 'документ'"),
			НСтр("ru = 'документа'"),
			НСтр("ru = 'документов'"),
			"м");
			
			СтрокаТабличнойЧасти.ДатаПоступления = Строка(СтрокаОстатка.КоличествоДокументов) + РасшифровкаСчетчика;
		Иначе
			СтрокаТабличнойЧасти.ДатаПоступления = ?(Не ЗначениеЗаполнено(СтрокаОстатка.ДатаПоступления),"", СтрокаОстатка.ДатаПоступления);
		КонецЕсли;
...
Процедура ЗаполнитьЗначенияОстатковНоменклатурыВТабличнойЧасти(Объект, СтруктураДанные = Неопределено,  НоменклатураОтбора = Неопределено, ИмяТЧ = "Запасы", ТипДокумента = Неопределено) Экспорт
	...
	Если Не ТаблицаНоменклатуры.Колонки.Найти("ЯчейкаДоступна") = Неопределено
		И Не ТаблицаНоменклатуры.Колонки.Найти("СтруктурнаяЕдиница") = Неопределено Тогда
		
		СкладыСДоступнымиЯчейками = МассивСкладовСДоступнымиЯчейками();
		
		Если СкладыСДоступнымиЯчейками.Количество() Тогда
			
			Для Каждого СтрокаТаблицыНоменклатура Из ТаблицаНоменклатуры Цикл
				Если Не СкладыСДоступнымиЯчейками.Найти(СтрокаТаблицыНоменклатура.СтруктурнаяЕдиница) = Неопределено Тогда
					СтрокаТаблицыНоменклатура.ЯчейкаДоступна = Истина;
				КонецЕсли;
			КонецЦикла;
	...
	Для Каждого СтрокаТабЗапасы Из СвернутаяТаблицаЗапасы Цикл
		...
		Если КоличествоДляРаспределения > 0 И МассивСтрокОстаткиОбщие.Количество() Тогда
			...
			Для каждого СтрокаМассиваСтрокОстатки Из МассивСтрокОстаткиОбщие Цикл
				...
				ТабличныеЧастиУНФКлиентСервер.ЗаполнитьКлючСвязи(Объект[ИмяТЧ], НоваяСтрока, "КлючСвязи");
...
Процедура ЗаполнитьКолонкуРезервПоОстаткамЗаказПокупателяБезРазбиения(Объект, ПараметрыОтбора = Неопределено, ИмяТЧ = "Запасы", ТипДокумента = "ЗаказПокупателя") Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	Если ПолучитьФункциональнуюОпцию("ПередачаТоваровМеждуОрганизациями") Тогда
		Запрос.УстановитьПараметр("Организации", ИнтеркампаниСервер.ОрганизацииОстатков(Объект.Организация));
	Иначе
...
//
Процедура ЗаполнитьКоличествоПоОстаткамИРезервамПродажа(Объект, ПараметрыОтбора = Неопределено, СтруктураОтбораНоменклатуры = Неопределено, ИмяТЧ = "Запасы") Экспорт
	...
	Если СтруктураОтбораНоменклатуры = Неопределено Тогда
	...
	СтрокиДляВыгрузки = Объект[ИмяТЧ].НайтиСтроки(СтруктураОтбораНоменклатуры);
	...
	Запрос.Текст = ТекстЗапросаЗаполнитьКоличествоПоОстаткамИРезервам();
	
	Если ПолучитьФункциональнуюОпцию("ПередачаТоваровМеждуОрганизациями") Тогда
		Организации = ИнтеркампаниСервер.ОрганизацииОстатков(Объект.Организация);
	Иначе
		Организации = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Константы.УчетПоКомпании.Компания(Объект.Организация));
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ОтборПоСкладам", Ложь);
	Запрос.УстановитьПараметр("СписокДоступныхСкладов", Неопределено);
	Если ПараметрыОтбора <> Неопределено Тогда
		Если ПараметрыОтбора.Свойство("СписокДоступныхСкладов") Тогда
			Запрос.УстановитьПараметр("ОтборПоСкладам", Истина);
			Запрос.УстановитьПараметр("СписокДоступныхСкладов", ПараметрыОтбора.СписокДоступныхСкладов);
		КонецЕсли;
	КонецЕсли;
	...
	Если УчетПоЯчейкам Тогда
		СтруктураПараметров = Новый Структура("Номенклатура, Организация, Регистратор", МассивНоменклатуры, Организации, Объект.Ссылка);
...
Процедура ЗаполнитьКоличествоПоОстаткамИРезервамПроизводство(Объект, ПараметрыОтбора = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ТаблицаЗапасы", ТаблицаЗапасы);
	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	Запрос.УстановитьПараметр("Организации", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Организация));
...
// Возвращает список документов по Заказу на перемещение
//
// Параметры:
//  Объект - Обрабатываемый документ
//  ТипОперации - Тип операции формирования списка
//  СтруктураПараметров - Структура параметров для формирования списка
//  ИмяТЧ - Имя табличной части 
//
// Возвращаемое значение:
//  СписокЗначений - Список документов по Заказу на перемещение
//
Функция СписокДокументовПоПеремещению(Объект, ТипОперации, СтруктураПараметров, ИмяТЧ = "Запасы") Экспорт
	...
	Отправитель = СтруктураПараметров.Отправитель;

	ЗаказПокупателя = ?(ЗначениеЗаполнено(ЗаказПокупателя), ЗаказПокупателя, Документы.ЗаказПокупателя.ПустаяСсылка());
	
	ОрганизацияОбъекта = СтруктураПараметров.Организация;
	...
	Если ТипОперации = "Заказано" Тогда
		...
		Запрос.Текст = ТекстЗапросаЗаказаноЗаказНаПеремещение();
		...
		Пока Выборка.Следующий() Цикл
			...
			СписокВозврата.Добавить(Выборка.Ссылка, КоличествоТовара +" "+ Строка(ЕдиницаИзмерения)+". " +"Поставка: " + Формат(Выборка.ДатаПоступления,"ДЛФ=D") + ". " + Строка(Выборка.Ссылка));
		КонецЦикла;
	...
	ИначеЕсли ТипОперации = "ЗапасыПоступления" Тогда
		...
		Запрос.Текст = ТекстЗапросаЗапасыПоступленияСвободныеПоПеремещению();
...
Процедура ЗаполнитьПризнакТипаЗапасНоменклатуры(Объект, ИмяТЧ = "Запасы") Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект[ИмяТЧ] Цикл
		СтрокаТабличнойЧасти.ТипНоменклатурыЗапас = (ЗначенияТипов.Получить(СтрокаТабличнойЧасти.Номенклатура) = Перечисления.ТипыНоменклатуры.Запас
												 ИЛИ ЗначенияТипов.Получить(СтрокаТабличнойЧасти.Номенклатура) = Перечисления.ТипыНоменклатуры.ПодарочныйСертификат);
	КонецЦикла;
...
Функция МассивТиповОбрабатываемыхДокументовСкладскойУчет()
...
Функция МассивТиповДокументовПоИмениТабличнойЧасти(ИмяТабличнойЧасти)
	...
	Если ИмяТабличнойЧасти = "РаспределениеЗапасов" Тогда
		МассивТипов.Добавить(Тип("ДокументСсылка.СборкаЗапасов"));
		МассивТипов.Добавить(Тип("ДокументСсылка.РаспределениеЗатрат"));
	КонецЕсли;
...
// Заполняет колонку "ОстатокОбщий" в документе Установка цен значениями по остаткам
//   
// Параметры:
//  СтрокиОбхода 	- ТаблицаЗначений, ДанныеФормыКоллекция 
//  ТаблицаОстатков - ТаблицаЗначений 
Процедура ЗаполнитьКолонкуОстатокОбщийУстановкаЦен(СтрокиОбхода, ТаблицаОстатков)
	
	ОбходСУчетомХарактеристик = ПолучитьПризнакОбходаСтрокПоХарактеристикамУстановкаЦен(СтрокиОбхода);
	
	Если НЕ ОбходСУчетомХарактеристик Тогда
		ПараметрыОтбора = Новый Структура("Номенклатура");  
	Иначе
		ПараметрыОтбора = Новый Структура("Номенклатура, Характеристика"); 
	КонецЕсли;
	
	Для Каждого СтрокаТаблицы Из СтрокиОбхода Цикл  
		
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.ЕдиницаИзмеренияШаблон) 
			Или ТипЗнч(СтрокаТаблицы.ЕдиницаИзмеренияШаблон) = Тип("СправочникСсылка.КлассификаторЕдиницИзмерения") Тогда
			КоэффициентЕдиницыИзмерения = 1;
		Иначе
			КоэффициентЕдиницыИзмерения = СтрокаТаблицы.ЕдиницаИзмеренияШаблон.Коэффициент;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) 
			Или ТипЗнч(СтрокаТаблицы.Номенклатура) = Тип("Строка") Тогда 
			Продолжить; 
		КонецЕсли;
		
		ПараметрыОтбора.Номенклатура = СтрокаТаблицы.Номенклатура;
		
		Если ОбходСУчетомХарактеристик Тогда
			ПараметрыОтбора.Характеристика = СтрокаТаблицы.Характеристика;
		КонецЕсли;
		
		НайденныеСтрокиОстатки = ТаблицаОстатков.НайтиСтроки(ПараметрыОтбора); 	
		Если НайденныеСтрокиОстатки.Количество() > 0 Тогда
			СтрокаОстатка = НайденныеСтрокиОстатки[0]; 
		Иначе
			СтрокаОстатка = Неопределено;
		КонецЕсли; 
		...
		СтрокаТаблицы.ОстатокОбщий = ОстатокОбщийСКоэффициентом; 
	КонецЦикла;    

КонецПроцедуры

// Возвращает признак необходимости обхода строк по характеристикам.
//
// Параметры:
//  СтрокиОбхода - ТаблицаЗначений, ДанныеФормыКоллекция
Функция ПолучитьПризнакОбходаСтрокПоХарактеристикамУстановкаЦен(СтрокиОбхода) 
	
	ИмяКолонкиПоиска = "Характеристика"; 
	
	Если ТипЗнч(СтрокиОбхода) = Тип("ДанныеФормыКоллекция") Тогда			
		ПризнакОбходаСтрок = СтрокиОбхода.Выгрузить().Колонки.Найти(ИмяКолонкиПоиска) <> Неопределено;		
	Иначе  	
	    ПризнакОбходаСтрок = СтрокиОбхода.Колонки.Найти(ИмяКолонкиПоиска) <> Неопределено; 		
	КонецЕсли;
	
	Возврат ПризнакОбходаСтрок;	
КонецФункции

// Возвращает массив строк для копирования
//
// Параметры:
//  ТаблицаПоискаСтрок 	- ТаблицаЗначений.
//  НоменклатураОтбора 	- Массив (необязательный). 
//
// Возвращаемое значение:
// 	Строки для копирования - Массив
Функция СформироватьМассивСтрокДляКопированияУстановкаЦен(ТаблицаПоискаСтрок, НоменклатураОтбора) 
	
	МассивСтрокКопировать = Новый Массив; 
	
	Для Каждого ЭлементМассива Из НоменклатураОтбора Цикл 
		
		МассивНайденныхСтрок = ТаблицаПоискаСтрок.НайтиСтроки(Новый Структура("Номенклатура", ЭлементМассива));
		Если МассивНайденныхСтрок.Количество() > 0 Тогда 
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивСтрокКопировать, МассивНайденныхСтрок);
		КонецЕсли;  
		
	КонецЦикла;   
		
	Возврат МассивСтрокКопировать;
КонецФункции

Функция МассивСкладовСДоступнымиЯчейками()
	...
	|	Справочник.СтруктурныеЕдиницы КАК СтруктурныеЕдиницы
	|ГДЕ
	|	НЕ СтруктурныеЕдиницы.ОрдерныйСклад
...
Функция ТекстЗапросаОстаткиУстановкаЦенНоменклатуры(ТекстЗапросаСХарактеристиками)
	
	Если ТекстЗапросаСХарактеристиками Тогда 
		
		Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЗапасыНаСкладахОстатки.Номенклатура КАК Номенклатура,
		|	ЗапасыНаСкладахОстатки.Характеристика КАК Характеристика,
		|	ЗапасыНаСкладахОстатки.КоличествоОстаток КАК ОстатокОбщий
		|ИЗ
		|	РегистрНакопления.ЗапасыНаСкладах.Остатки(
		|			,
		|			Номенклатура = &НоменклатураОтбора
		|				И Характеристика В (&МассивХарактеристик)
		|				И СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы В (ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад), ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Розница))) КАК ЗапасыНаСкладахОстатки"; 
	...
	Иначе 
		...
		|ИЗ
		|	&ВнешняяТаблицаНоменклатуры КАК ВнешняяТаблица
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура
		|;
		...
		|	ЗапасыНаСкладахОстатки.КоличествоОстаток КАК ОстатокОбщий
		|ИЗ
		|	РегистрНакопления.ЗапасыНаСкладах.Остатки(, СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы В (ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад), ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Розница))) КАК ЗапасыНаСкладахОстатки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтНоменклатура КАК ВтНоменклатура
		|		ПО ЗапасыНаСкладахОстатки.Номенклатура = ВтНоменклатура.Номенклатура";
...
Функция ТекстЗапросаСписокСкладовОстаткиУстановкаЦенНоменклатуры(ИмяТаблицыДанныхФормы) 
		
	Если ИмяТаблицыДанныхФормы = "ТЧНоменклатура" Тогда 
...
Функция ТекстЗапросаОстаткиИРезервыЗаказНаПеремещение()
	...
	|ПОМЕСТИТЬ ВТНоменклатура
	|ИЗ
	|	&ТаблицаНоменклатура КАК ТаблицаЗапасы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|ПОМЕСТИТЬ ОстаткиНаСкладеВсего
	|ИЗ
	...
	|	СУММА(ЕСТЬNULL(ЗаказыПоставщикамОстатки.КоличествоОстаток, 0)) КАК КПоступлению,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПоставщикамОстатки.ЗаказПоставщику) КАК КоличествоДокументов
	|ПОМЕСТИТЬ ЗаказаноОстатки
	|ИЗ
	...
	|	СУММА(ЕСТЬNULL(ЗаказыНаПроизводствоОстатки.КоличествоОстаток, 0)),
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыНаПроизводствоОстатки.ЗаказНаПроизводство)
	|ИЗ
	...
	|	СУММА(ЕСТЬNULL(ЗаказыНаПеремещениеОстатки.КоличествоОстаток, 0)),
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыНаПеремещениеОстатки.ЗаказНаПеремещение)
	|ИЗ
	...
	|				И ТипДвижения = ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Поступление)
	|				И НЕ ЗаказНаПеремещение = &Ссылка) КАК ЗаказыНаПеремещениеОстатки
	|ГДЕ
	|	&УчетПотребностиПоСкладам
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗаказаноОстатки.Номенклатура КАК Номенклатура,
	|	ЗаказаноОстатки.Характеристика КАК Характеристика,
	|	СУММА(ЗаказаноОстатки.КоличествоДокументов) КАК КоличествоДокументов,
	|	СУММА(ЗаказаноОстатки.КПоступлению) КАК КПоступлению
	|ПОМЕСТИТЬ ЗаказаноОстаткиИтог
	|ИЗ
	|	ЗаказаноОстатки КАК ЗаказаноОстатки
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаказаноОстатки.Номенклатура,
	|	ЗаказаноОстатки.Характеристика
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	ВТНоменклатура.Характеристика КАК Характеристика,
	|	ВТНоменклатура.Партия КАК Партия,
	|	СУММА(ЕСТЬNULL(ЗаказыНаПеремещениеОстатки.КоличествоОстаток, 0)) КАК НеОтгружено
	|ПОМЕСТИТЬ НеОтгруженоПоПеремещению
	|ИЗ
	|	ВТНоменклатура КАК ВТНоменклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыНаПеремещение.Остатки(
	|				,
	|				ЗаказНаПеремещение = &Ссылка
	|					И ТипДвижения = ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Поступление)) КАК ЗаказыНаПеремещениеОстатки
	|		ПО ВТНоменклатура.Номенклатура = ЗаказыНаПеремещениеОстатки.Номенклатура
	|			И ВТНоменклатура.Характеристика = ЗаказыНаПеремещениеОстатки.Характеристика
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТНоменклатура.Номенклатура,
	|	ВТНоменклатура.Партия,
	...
	|	ЗаказыНаПеремещениеОстатки.Номенклатура,
	|	ЗаказыНаПеремещениеОстатки.Характеристика,
	|	ЗаказыНаПеремещениеОстатки.ЗаказНаПеремещение.ДатаПеремещения
	|ИЗ
	|	РегистрНакопления.ЗаказыНаПеремещение.Остатки(
	|			,
	|			Организация = &Организация
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗаказаноПоПериодам.Номенклатура КАК Номенклатура,
	|	ЗаказаноПоПериодам.Характеристика КАК Характеристика,
	|	МИНИМУМ(ЗаказаноПоПериодам.ДатаПоступления) КАК ДатаПоступления
	|ПОМЕСТИТЬ ЗаказаноПоПериодамИтог
	|ИЗ
	|	ЗаказаноПоПериодам КАК ЗаказаноПоПериодам
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаказаноПоПериодам.Номенклатура,
	|	ЗаказаноПоПериодам.Характеристика
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗаказаноОстаткиИтог.Номенклатура КАК Номенклатура,
	|	ЗаказаноОстаткиИтог.Характеристика КАК Характеристика,
	|	СУММА(ЗаказаноОстаткиИтог.КоличествоДокументов) КАК КоличествоДокументов,
	|	МИНИМУМ(ЗаказаноПоПериодамИтог.ДатаПоступления) КАК ДатаПоступления,
	|	СУММА(ЗаказаноОстаткиИтог.КПоступлению) КАК КПоступлению
	|ПОМЕСТИТЬ ЗаказаноИтог
	|ИЗ
	|	ЗаказаноОстаткиИтог КАК ЗаказаноОстаткиИтог
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказаноПоПериодамИтог КАК ЗаказаноПоПериодамИтог
	|		ПО ЗаказаноОстаткиИтог.Номенклатура = ЗаказаноПоПериодамИтог.Номенклатура
	|			И ЗаказаноОстаткиИтог.Характеристика = ЗаказаноПоПериодамИтог.Характеристика
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаказаноОстаткиИтог.Характеристика,
	|	ЗаказаноОстаткиИтог.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	ВТНоменклатура.Характеристика КАК Характеристика,
	|	ВТНоменклатура.Партия КАК Партия,
	|	0 КАК ОстатокПолучатель,
	|	ЕСТЬNULL(ЗапасыОстатки.КоличествоОстаток, 0) КАК ОстатокОтправитель
	|ПОМЕСТИТЬ ОстаткиНаСкладеОтправительИПолучатель
	|ИЗ
	|	ВТНоменклатура КАК ВТНоменклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Запасы.Остатки(
	|				,
	|				Организация = &Организация
	|					И ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|					И СтруктурнаяЕдиница = &Отправитель) КАК ЗапасыОстатки
	|		ПО ВТНоменклатура.Номенклатура = ЗапасыОстатки.Номенклатура
	|			И ВТНоменклатура.Характеристика = ЗапасыОстатки.Характеристика
	|			И ВТНоменклатура.Партия = ЗапасыОстатки.Партия
	|			И ВТНоменклатура.ЗаказПокупателя = ЗапасыОстатки.ЗаказПокупателя
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТНоменклатура.Номенклатура,
	|	ВТНоменклатура.Партия,
	|	ВТНоменклатура.Характеристика,
	|	ЕСТЬNULL(ЗапасыОстатки.КоличествоОстаток, 0)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|				Организация = &Организация
	|					И ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|					И СтруктурнаяЕдиница = &Получатель) КАК ЗапасыОстатки
	|		ПО ВТНоменклатура.Номенклатура = ЗапасыОстатки.Номенклатура
	|			И ВТНоменклатура.Характеристика = ЗапасыОстатки.Характеристика
	|			И ВТНоменклатура.Партия = ЗапасыОстатки.Партия
	|			И ВТНоменклатура.ЗаказПокупателя = ЗапасыОстатки.ЗаказПокупателя
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ОстаткиНаСкладеОтправительИПолучатель.Номенклатура КАК Номенклатура,
	|	ОстаткиНаСкладеОтправительИПолучатель.Характеристика КАК Характеристика,
	|	ОстаткиНаСкладеОтправительИПолучатель.Партия КАК Партия,
	|	СУММА(ОстаткиНаСкладеОтправительИПолучатель.ОстатокПолучатель) КАК ОстатокПолучатель,
	|	СУММА(ОстаткиНаСкладеОтправительИПолучатель.ОстатокОтправитель) КАК ОстатокОтправитель
	|ПОМЕСТИТЬ ОстаткиНаСкладеОтправительИПолучательИтог
	|ИЗ
	|	ОстаткиНаСкладеОтправительИПолучатель КАК ОстаткиНаСкладеОтправительИПолучатель
	|
	|СГРУППИРОВАТЬ ПО
	|	ОстаткиНаСкладеОтправительИПолучатель.Характеристика,
	|	ОстаткиНаСкладеОтправительИПолучатель.Партия,
	|	ОстаткиНаСкладеОтправительИПолучатель.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	ВТНоменклатура.Характеристика КАК Характеристика,
	|	ВТНоменклатура.Партия КАК Партия,
	|	СУММА(ЕСТЬNULL(ЗаказыНаПеремещениеОстатки.КоличествоОстаток, 0)) КАК КоличествоДвижение
	|ПОМЕСТИТЬ ОстатокПоЗаказамОтправитель
	|ИЗ
	|	ВТНоменклатура КАК ВТНоменклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыНаПеремещение.Остатки(
	|				,
	...
	|					И Склад = &Отправитель) КАК ЗаказыНаПеремещениеОстатки
	|		ПО ВТНоменклатура.Номенклатура = ЗаказыНаПеремещениеОстатки.Номенклатура
	|			И ВТНоменклатура.Характеристика = ЗаказыНаПеремещениеОстатки.Характеристика
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВТНоменклатура.Партия КАК Партия,
	|	СУММА(ЕСТЬNULL(ЗаказыНаПеремещениеОстатки.КоличествоОстаток, 0)) КАК КоличествоДвижение
	|ПОМЕСТИТЬ ОстатокПоЗаказамПолучатель
	|ИЗ
	|	ВТНоменклатура КАК ВТНоменклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыНаПеремещение.Остатки(
	|				,
	...
	|	ВТНоменклатура.Характеристика КАК Характеристика,
	|	ВТНоменклатура.Партия КАК Партия,
	|	СУММА(ЕСТЬNULL(ЗаказыНаПеремещение.Количество, 0)) КАК КоличествоДвижение
	|ПОМЕСТИТЬ ДвижениеДокумента
	|ИЗ
	|	ВТНоменклатура КАК ВТНоменклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыНаПеремещение КАК ЗаказыНаПеремещение
	|		ПО ВТНоменклатура.Номенклатура = ЗаказыНаПеремещение.Номенклатура
	|			И ВТНоменклатура.Характеристика = ЗаказыНаПеремещение.Характеристика
	|ГДЕ
	|	ЗаказыНаПеремещение.Регистратор = &Ссылка
	|	И ЗаказыНаПеремещение.ТипДвижения = ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Отгрузка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТНоменклатура.Партия,
	|	ВТНоменклатура.Характеристика,
	...
	|ПОМЕСТИТЬ УправлениеЗапасами
	|ИЗ
	|	ВТНоменклатура КАК ВТНоменклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УправлениеЗапасами КАК УправлениеЗапасами
	|		ПО ВТНоменклатура.Номенклатура = УправлениеЗапасами.Номенклатура
	|			И ВТНоменклатура.Характеристика = УправлениеЗапасами.Характеристика
	|ГДЕ
	|	УправлениеЗапасами.Организация = &Организация
	|	И УправлениеЗапасами.Склад = &Получатель
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ОстаткиНаСкладеОтправительИПолучательИтог.ОстатокОтправитель, 0) КАК СвободноОтправитель,
	|	ЕСТЬNULL(ОстаткиНаСкладеОтправительИПолучательИтог.ОстатокПолучатель, 0) КАК СвободноПолучатель,
	|	ВТНоменклатура.Номенклатура КАК Номенклатура,
	|	ВТНоменклатура.Характеристика КАК Характеристика,
	|	ВТНоменклатура.Партия КАК Партия,
	|	ЕСТЬNULL(ОстаткиНаСкладеВсего.КоличествоОстаток, 0) КАК ОстатокВсего,
	|	ЗаказаноИтог.ДатаПоступления КАК ДатаПоступления,
	|	ЕСТЬNULL(ЗаказаноИтог.КоличествоДокументов, 0) КАК КоличествоДокументов,
	|	ВТНоменклатура.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ЕСТЬNULL(УправлениеЗапасами.МаксимальныйУровеньЗапаса, 0) КАК МаксимальныйОстаток,
	|	ЕСТЬNULL(УправлениеЗапасами.МинимальныйУровеньЗапаса, 0) КАК МинимальныйОстаток,
	|	ЕСТЬNULL(ЗаказаноИтог.КПоступлению, 0) КАК КПоступлению,
	|	ЕСТЬNULL(НеОтгруженоПоПеремещению.НеОтгружено, 0) КАК НеОтгружено,
	|	ЕСТЬNULL(ОстатокПоЗаказамПолучатель.КоличествоДвижение, 0) КАК Заказано
	|ИЗ
	|	ВТНоменклатура КАК ВТНоменклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиНаСкладеОтправительИПолучательИтог КАК ОстаткиНаСкладеОтправительИПолучательИтог
	|		ПО ВТНоменклатура.Номенклатура = ОстаткиНаСкладеОтправительИПолучательИтог.Номенклатура
	|			И ВТНоменклатура.Характеристика = ОстаткиНаСкладеОтправительИПолучательИтог.Характеристика
	|			И ВТНоменклатура.Партия = ОстаткиНаСкладеОтправительИПолучательИтог.Партия
	|			И ВТНоменклатура.Номенклатура = ОстаткиНаСкладеОтправительИПолучательИтог.Номенклатура
	|			И ВТНоменклатура.Характеристика = ОстаткиНаСкладеОтправительИПолучательИтог.Характеристика
	|			И ВТНоменклатура.Партия = ОстаткиНаСкладеОтправительИПолучательИтог.Партия
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаказаноИтог КАК ЗаказаноИтог
	|		ПО ВТНоменклатура.Номенклатура = ЗаказаноИтог.Номенклатура
	|			И ВТНоменклатура.Характеристика = ЗаказаноИтог.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОстатокПоЗаказамОтправитель КАК ОстатокПоЗаказамОтправитель
	|		ПО ВТНоменклатура.Номенклатура = ОстатокПоЗаказамОтправитель.Номенклатура
	|			И ВТНоменклатура.Характеристика = ОстатокПоЗаказамОтправитель.Характеристика
	|			И ВТНоменклатура.Партия = ОстатокПоЗаказамОтправитель.Партия
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОстатокПоЗаказамПолучатель КАК ОстатокПоЗаказамПолучатель
	|		ПО ВТНоменклатура.Номенклатура = ОстатокПоЗаказамПолучатель.Номенклатура
	|			И ВТНоменклатура.Характеристика = ОстатокПоЗаказамПолучатель.Характеристика
	|			И ВТНоменклатура.Партия = ОстатокПоЗаказамПолучатель.Партия
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиНаСкладеВсего КАК ОстаткиНаСкладеВсего
	|		ПО ВТНоменклатура.Номенклатура = ОстаткиНаСкладеВсего.Номенклатура
	|			И ВТНоменклатура.Характеристика = ОстаткиНаСкладеВсего.Характеристика
	|			И ВТНоменклатура.Партия = ОстаткиНаСкладеВсего.Партия
	|		ЛЕВОЕ СОЕДИНЕНИЕ НеОтгруженоПоПеремещению КАК НеОтгруженоПоПеремещению
	|		ПО ВТНоменклатура.Номенклатура = НеОтгруженоПоПеремещению.Номенклатура
	|			И ВТНоменклатура.Характеристика = НеОтгруженоПоПеремещению.Характеристика
	|			И ВТНоменклатура.Партия = НеОтгруженоПоПеремещению.Партия
	|		ЛЕВОЕ СОЕДИНЕНИЕ УправлениеЗапасами КАК УправлениеЗапасами
	|		ПО ВТНоменклатура.Номенклатура = УправлениеЗапасами.Номенклатура
	|			И ВТНоменклатура.Характеристика = УправлениеЗапасами.Характеристика
...
Функция ТаблицаЗначенийРеквизитовНоменклатурыТабличнойЧасти(ТабличнаяЧасть)
	...
	|	ЕСТЬNULL(Номенклатура.ЭтоГруппа, ЛОЖЬ) КАК ЭтоГруппа,
	|	ЕСТЬNULL(Номенклатура.ПроверятьЗаполнениеХарактеристики, ЛОЖЬ) КАК ПроверятьЗаполнениеХарактеристики,
	|	ЕСТЬNULL(Номенклатура.ПроверятьЗаполнениеПартий, ЛОЖЬ) КАК ПроверятьЗаполнениеПартий,
	|	ЕСТЬNULL(Номенклатура.ТипНоменклатуры, ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПустаяСсылка)) КАК ТипНоменклатуры
	|ИЗ
	|	Итог КАК Итог
...
Функция ТаблицаЗначенийРеквизитовНоменклатурыТабличнойЧастиПересортица(ТабличнаяЧасть)
	
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("ТабличнаяЧасть", ТабличнаяЧасть.Выгрузить(,"НомерСтроки, Номенклатура, НоменклатураОприходование"));
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТабличнаяЧасть.Номенклатура КАК НоменклатураТабЧасти,
	|	ТабличнаяЧасть.НоменклатураОприходование КАК НоменклатураОприходованиеТабЧасти,
	|	ТабличнаяЧасть.НомерСтроки КАК НомерСтроки
	|ПОМЕСТИТЬ Итог
	|ИЗ
	|	&ТабличнаяЧасть КАК ТабличнаяЧасть
	...
	|	ЕСТЬNULL(Номенклатура.ПроверятьЗаполнениеПартий, ЛОЖЬ) КАК ПроверятьЗаполнениеПартий,
	|	Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры,
	|	Итог.НоменклатураОприходованиеТабЧасти КАК НоменклатураОприходование,
	|	ЕСТЬNULL(НоменклатураОприходование.ПроверятьЗаполнениеПартий, ЛОЖЬ) КАК ПроверятьЗаполнениеПартийОприходование,
	|	ЕСТЬNULL(НоменклатураОприходование.ПроверятьЗаполнениеХарактеристики, ЛОЖЬ) КАК ПроверятьЗаполнениеХарактеристикиОприходование
	|ИЗ
	|	Итог КАК Итог
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК Номенклатура
	|		ПО Итог.НоменклатураТабЧасти = Номенклатура.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК НоменклатураОприходование
	|		ПО Итог.НоменклатураОприходованиеТабЧасти = НоменклатураОприходование.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
...
Функция ТекстЗапросаЗаполнитьКоличествоПоОстаткамИРезервам()
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ СтруктурныеЕдиницы.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|		КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|					И (НЕ &ОтборПоСкладам
	|						ИЛИ СтруктурнаяЕдиница В (&СписокДоступныхСкладов))) КАК ЗапасыОстатки
...
Функция ТекстЗапросаОстаткиИРезервы()
	...
	|					И ЗаказПокупателя = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
	|					И ТИПЗНАЧЕНИЯ(СтруктурнаяЕдиница) = ТИП(Справочник.СтруктурныеЕдиницы)) КАК ЗапасыОстаткиОбщие
	...
	|							ТОГДА ЗаказПокупателя = &ЗаказПокупателя
	|					КОНЕЦ
	|					И НЕ ТИПЗНАЧЕНИЯ(ИсточникОбеспечения) = ТИП(Документ.ЗаказНаПеремещение)) КАК РазмещениеЗаказовОстатки
...
Функция ТекстЗапросаОстаткиИРезервыВРазрезеЗаказов()
	...
	|							ТОГДА СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы В (ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад), ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Розница))
	|						ИНАЧЕ ИСТИНА
	...
	|			ЗаказыНаПроизводствоОстатки.Организация) КАК ВложенныйЗапрос
	|	ГДЕ
	|		ВложенныйЗапрос.ЗаказаноБезРезерва > 0
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ВложенныйЗапрос.Номенклатура,
	|		ВложенныйЗапрос.Характеристика,
	...
	|			
	|			СГРУППИРОВАТЬ ПО
	|				ГрафикДвиженияЗапасов.Период,
	|				ЗаказыПоставщикамОстатки.Организация,
	|				ЗаказыПоставщикамОстатки.Номенклатура,
	|				ЗаказыПоставщикамОстатки.Характеристика
	|			
	|			ИМЕЮЩИЕ
	|				СУММА(ЗаказыПоставщикамОстатки.КоличествоОстаток - ЕСТЬNULL(РазмещениеЗаказовОстатки.КоличествоОстаток, 0)) > 0
	|			
	|			ОБЪЕДИНИТЬ ВСЕ
	|			
	|			ВЫБРАТЬ
	...
	|			
	|			ИМЕЮЩИЕ
	|				СУММА(ЗаказыНаПроизводствоОстатки.КоличествоОстаток - РазмещениеЗаказовОстатки.КоличествоОстаток) > 0) КАК ВложенныйЗапрос
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ВложенныйЗапрос.Организация,
	|			ВложенныйЗапрос.Характеристика,
...
Функция ТекстЗапросаЗаказаноЗаказНаПеремещение()
	
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТИПЗНАЧЕНИЯ(ЗаказыНаПеремещениеОстатки.ЗаказНаПеремещение) КАК ТипДокумента,
	|	ЗаказыНаПеремещениеОстатки.ЗаказНаПеремещение.ДатаПеремещения КАК ДатаПоступления,
	...
	|				И Характеристика = &Характеристика
	|				И ТипДвижения = ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Поступление)
	|				И НЕ ЗаказНаПеремещение = &ЗаказНаПеремещение) КАК ЗаказыНаПеремещениеОстатки
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаказыНаПеремещениеОстатки.ЗаказНаПеремещение,
	|	ТИПЗНАЧЕНИЯ(ЗаказыНаПеремещениеОстатки.ЗаказНаПеремещение),
	|	ЗаказыНаПеремещениеОстатки.ЗаказНаПеремещение.ДатаПеремещения,
	|	ЗаказыНаПеремещениеОстатки.КоличествоОстаток
	|
	|УПОРЯДОЧИТЬ ПО
	|	ТипДокумента";
...
Функция ТекстЗапросаЗапасыПоступленияСвободныеПоПеремещению()
	...
	|						И Номенклатура В (&Номенклатура)
	|						И Характеристика В (&Характеристика)
	|						И Склад = &Отправитель) КАК ЗаказыПоставщикамОстатки
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ГрафикДвиженияЗапасов КАК ГрафикДвиженияЗапасов
	|				ПО ЗаказыПоставщикамОстатки.ЗаказПоставщику = ГрафикДвиженияЗапасов.Заказ
	|					И ЗаказыПоставщикамОстатки.Номенклатура = ГрафикДвиженияЗапасов.Номенклатура
	|					И ЗаказыПоставщикамОстатки.Характеристика = ГрафикДвиженияЗапасов.Характеристика
	...
	|		ГДЕ
	|			ЗаказыПоставщикамОстатки.КоличествоОстаток - ЕСТЬNULL(РазмещениеЗаказовОстатки.КоличествоОстаток, 0) > 0
	|			И ГрафикДвиженияЗапасов.Склад = &Отправитель
	|			И &УчетПотребностиПоСкладам
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|						И Номенклатура В (&Номенклатура)
	|						И Характеристика В (&Характеристика)
	|						И Склад = &Отправитель) КАК ЗаказыНаПроизводствоОстатки
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РазмещениеЗаказов.Остатки(
	|						,
	|						Организация = &Организация
	|							И Номенклатура В (&Номенклатура)
	...
	|						И Характеристика В (&Характеристика)
	|						И Склад = &Отправитель
	|						И ТипДвижения = ЗНАЧЕНИЕ(Перечисление.ТипыДвиженийЗапасов.Поступление)) КАК ЗаказыНаПеремещениеОстатки) КАК ВложенныйЗапрос1
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ВложенныйЗапрос1.Количество,
	|		ВложенныйЗапрос1.Ссылка) КАК ВложенныйЗапрос";
...
Функция ТекстЗапросаОстаткиОбщие(ЕстьСсылка = Ложь, ЭтоЗаказ = Истина)
	...
	Если ЕстьСсылка Тогда
		...
		Если ЭтоЗаказ Тогда
			...
			|			КОГДА НЕ ТИПЗНАЧЕНИЯ(&Ссылка) = ТИП(Документ.СборкаЗапасов)
			|					И НЕ ТИПЗНАЧЕНИЯ(&Ссылка) = ТИП(Документ.ЗаказНаПроизводство)
			|				ТОГДА СтруктурныеЕдиницы.ТипСтруктурнойЕдиницы В (ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад), ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Розница))
			|			ИНАЧЕ ИСТИНА
			|		КОНЕЦ
			|;
			|
...
Функция ОстаткиПоЯчейкам(СтруктураПараметров, ТаблицаЗапасы, ИмяКолонкиЗаказ = "Заказ")
	...
	Если ТипЗнч(СтруктураПараметров.Организация) = Тип("Массив") Тогда
		Организации = СтруктураПараметров.Организация;
	Иначе
		Организации = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СтруктураПараметров.Организация);
	КонецЕсли;
	Запрос.УстановитьПараметр("Организации", Организации);
	Запрос.УстановитьПараметр("Номенклатура", СтруктураПараметров.Номенклатура);
	Запрос.УстановитьПараметр("ТаблицаЗапасы", ТаблицаЗапасы);
