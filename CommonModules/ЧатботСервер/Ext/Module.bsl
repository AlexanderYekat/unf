...
Функция ОтправитьСообщение(структураОтвет, Параметры)
	...
	Если ДанныеКартинки <> Неопределено Тогда
		ТипКонтента = СтрШаблон("image/%1", ДанныеКартинки.Расширение);
		ДвоичныеДанныеКартинки = ПолучитьИзВременногоХранилища(ДанныеКартинки.СсылкаНаДвоичныеДанныеФайла);
		ДвоичныеДанныеСообщение = ДвоичныеДанныеДляОтправкиВСообщении(ДвоичныеДанныеКартинки, ДанныеКартинки.ИмяФайла, ТипКонтента, "photo");
		
		Разделитель = СтрЗаменить(Строка(Новый УникальныйИдентификатор()), "-", "");
		ДвоичныеДанныеТела = СобратьТелоСообщенияИзБинарногоСодержимого(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДвоичныеДанныеСообщение), Разделитель);
		HTTPЗапрос.Заголовки.Вставить("Content-Type", СтрШаблон("multipart/form-data;charset=UTF-8;boundary=%1", Разделитель));
		HTTPЗапрос.Заголовки.Вставить("Content-Length", Формат(ДвоичныеДанныеТела.Размер(), "ЧН=0; ЧГ=0;"));
	...
	Если HTTPОтвет.КодСостояния = 200 Тогда
		...
		Иначе
			ТекстОшибки = НСтр("ru='Ошибка. Ответ сервера:
			|%1
			|%2'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, HTTPОтвет.КодСостояния, HTTPОтвет.ПолучитьТелоКакСтроку());
			ЗаписьЖурналаРегистрации(ИмяСобытияЖР(), УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
		КонецЕсли;
	Иначе
		ТекстОшибки = НСтр("ru='Ошибка. Ответ сервера:
...
Функция ДвоичныеДанныеДляОтправкиВСообщении(ДвоичныеДанныеФайла, ИмяФайла, ТипКонтента, ИмяСообщения)
	
	Поток = Новый ПотокВПамяти();
	ЗаписьДанных = Новый ЗаписьДанных(Поток);
	ЗаписьДанных.ЗаписатьСтроку(СтрШаблон("Content-Disposition: form-data;name=%1;filename=%2", ИмяСообщения, ИмяФайла));
	ЗаписьДанных.ЗаписатьСтроку(СтрШаблон("Content-Type: image%1", ТипКонтента));
...
Функция СобратьТелоСообщенияИзБинарногоСодержимого(СписокДвоичныхДанных, Разделитель)
	
	Тело = Новый ПотокВПамяти();
	ЗаписьДанных = Новый ЗаписьДанных(Тело);
	Для каждого ДвоичныеДанные Из СписокДвоичныхДанных Цикл
...
Функция СоздатьОбновитьЗаказ(ПоляЧата, Параметры)
	...
	Исключение
		ИмяСобытия = ИмяСобытияЖР() + "." + НСтр("ru='Проведение заказ-наряда'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Попытка
		...
		Возврат ДокументОбъект.Номер;
	Исключение
		ИмяСобытия = ИмяСобытияЖР() + "." + НСтр("ru='Запись заказ-наряда'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СоединениеLongPolling(НастройкиЧатбота)
	...
	// Заданный таймаут так же передается в заголовках запроса для telegram.
	// Необходимо установить таймаут соединения с запасом по времени, чтобы успеть обработать корректный ответ,
	// когда за переданный в заголовке таймаут telegram ничего не вернул.
...
Функция СоздатьИзменитьСлужебногоПользователяЧатбота(Включить = Ложь) Экспорт
	...
	Попытка
		Если ПользовательИзСправочника = Неопределено Тогда
			...
			ИмяСобытия = ИмяСобытияЖР() + "." + НСтр("ru='СозданиеСлужебногоПользователя'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Иначе
			...
			ИмяСобытия = ИмяСобытияЖР() + "." + НСтр("ru='ИзменениеДоступаСлужебногоПользователя'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияЖР()
	...
	Возврат НСтр("ru='Чатбот'", ОбщегоНазначения.КодОсновногоЯзыка());
