...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	УправлениеКонтактнойИнформациейУНФ.ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти);
	
КонецПроцедуры

// Устарела. Будет удалена в следующей версии.// Смотри также ОбновлениеИнформационнойБазыПереопределяемый.ПриНастройкеНачальногоЗаполненияЭлемента
//
// Параметры:
...
Процедура ПриНачальномЗаполненииЭлемента(Объект, Данные, ДополнительныеПараметры) Экспорт
	...
	УправлениеКонтактнойИнформациейУНФ.ПриНачальномЗаполненииЭлемента(Объект, Данные, ДополнительныеПараметры);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Форма, Отказ) Экспорт
	...
	Для Каждого СтрокаТаблицы Из Форма.КонтактнаяИнформация Цикл
		...
		ИначеЕсли НЕ ПустаяСтрока(СтрокаТаблицы.Представление) И СвойстваВида.ПроверятьКорректность Тогда
			...
			ЕстьОшибки = ЕстьОшибки ИЛИ РезультатПроверки <> 0;
...
Процедура ОбновитьЭлементыКонтактнойИнформации(Форма, ШиринаПоляВида = 8) Экспорт
	...
	Для каждого ИдентификаторСтроки Из ИдентификаторыСтрокТабличнойЧасти Цикл
		...
		Для Каждого ДанныеКИ Из СтрокиКонтактнойИнформации Цикл
			...
			ИндексКИ = XMLСтрока(Форма.КонтактнаяИнформация.Индекс(ДанныеКИ));
			...
			ДекорацияДействие.Гиперссылка = ПризнакГиперссылкиПоТипуКИ(ДанныеКИ.Тип, Форма.ИспользуетсяРаботаССобытиями);
...
Процедура ДобавитьКонтактнуюИнформацию(Форма, ДобавляемыйВид, УстановитьВыводВФормеВсегда = Ложь, ИдентификаторСтрокиТабличнойЧасти = 0) Экспорт
	...
	Форма.ТекущийЭлемент = Форма.Элементы["ПредставлениеКИ_" + XMLСтрока(ИндексВставки)];
...
// Функция - Признак гиперссылки по типу КИ
//
// Параметры:
//  ТипКИ				 - ПеречислениеСсылка.ТипыКонтактнойИнформации	 - тип для которого получается признак
//  ИспользуютсяСобытия	 - Булево - признак использования событий в приложении
// 
// Возвращаемое значение:
//  Булево - признак гиперссылки КИ
//
Функция ПризнакГиперссылкиПоТипуКИ(ТипКИ,ИспользуютсяСобытия) Экспорт
...
Процедура ВидКонтактнойИнформацииПриСозданииНаСервере(Форма) Экспорт
	
	СкрываемыеЭлементы = Новый Массив;
	СкрываемыеЭлементы.Добавить("МаскаПриВводеНомераТелефона");
	СкрываемыеЭлементы.Добавить("МаскаТелефона");
...
Процедура ДобавитьРеквизитыФормыКонтактнойИнформации(Форма, ЕстьКолонкаИдентификаторСтрокиТабличнойЧасти,
	...
	ДобавитьПризнакРаботыССобытиями = Истина;
	...
	Для Каждого Реквизит Из СписокРеквизитовФормы Цикл
		...
		ИначеЕсли Реквизит.Имя = "ИспользуетсяРаботаССобытиями" Тогда
			ДобавитьПризнакРаботыССобытиями = Ложь;
		КонецЕсли;
	КонецЦикла;
	...
	Если ДобавитьПризнакРаботыССобытиями Тогда
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("ИспользуетсяРаботаССобытиями", ОписаниеБулево));
	КонецЕсли;
