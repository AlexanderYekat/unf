...
// * СтрокиБезШтрихкодов - Соответствие из КлючИЗначение - Устарел. Товар распределяется всегда.
...
Функция РаспределениеШтрихкодовУпаковокПоТоварам(ПараметрыРаспределения, Товары, МассивУпаковок) Экспорт
	...
	Для Каждого СтрокаШтрихкод Из ТаблицаШтрихкоды Цикл
		...
		Для Каждого СтрокаТовары Из НайденныеСтрокиТовары Цикл
			...
			РезультатСписания.Количество    = СписатьКоличествоТоваров;
...
// Используется для автоматического заполнения кодов маркировки по номенклатуре подключенного кега к оборудованию розлива.
// Возвращает штрихкод упаковки с данными частичного выбытия, которые сохраняются в документе розничной продажи пива.
//
// Параметры:
//  ПараметрыСканирования - см. ШтрихкодированиеОбщегоНазначенияИСКлиент.ПараметрыСканирования
...
// * ДобавленныеКодыМаркировки - Соответствие из КлючИЗначение:
//                             * Ключ - СтрокаТаблицыЗначений, СтрокаТабличнойЧасти, ДанныеФормыЭлементКоллекции - Исходная строка Товаров
//                             * Значение - см. НовыйЭлементКоллекцииУпаковокДляРаспределенияПоТоварам
// * ИзмененныеКодыМаркировки - Соответствие из КлючИЗначение:
//                             * Ключ - СтрокаТаблицыЗначений, СтрокаТабличнойЧасти, ДанныеФормыЭлементКоллекции - Исходная строка Товаров
//                             * Значение - см. НовыйЭлементКоллекцииУпаковокДляРаспределенияПоТоварам
...
Функция ДобавитьИзменитьКодМаркировкиКегаНаОборудованииРозлива(ПараметрыСканирования, Товары, ШтрихкодыУпаковок = Неопределено) Экспорт

	ВозвращаемоеЗначение = Новый Структура();
	ВозвращаемоеЗначение.Вставить("ЕстьИзменения",             Ложь);
	ВозвращаемоеЗначение.Вставить("ДобавленныеКодыМаркировки", Новый Соответствие());
	ВозвращаемоеЗначение.Вставить("ИзмененныеКодыМаркировки",  Новый Соответствие());
	ВозвращаемоеЗначение.Вставить("УстановленныеСерии",        Новый Соответствие());
	
	Если Не (ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(Перечисления.ВидыПродукцииИС.Пиво)
		И ШтрихкодированиеОбщегоНазначенияИС.ТребуетсяЧастичноеВыбытие(ПараметрыСканирования)
		И ПараметрыСканирования.ВидОперацииИСМП = Перечисления.ВидыОперацийИСМП.ВыводИзОборотаРозничнаяПродажа) Тогда
		Возврат ВозвращаемоеЗначение;
	КонецЕсли;
	
	ТаблицаТовары = РегистрыСведений.КегиНаОборудованииРозливаИСМП.НоваяТаблицаПоискаКодовМаркировкиНаОборудованииРозлива();
	...
	Для Каждого СтрокаТаблицы Из ИсточникВложенныхШтрихкодов Цикл
		...
		КешКодовМаркировки.Вставить(ШтрихкодУпаковки, СтрокаТаблицы);
	...
	КодыНаОборудованииРозлива = РегистрыСведений.КегиНаОборудованииРозливаИСМП.КодыМаркировкиПодключенныеКОборудованиюРозлива(
		ТаблицаТовары, ПараметрыСканирования,, Истина);
	КодыНаОборудованииРозлива.Индексы.Добавить("Номенклатура,Характеристика");
	КодыНаОборудованииРозлива.Индексы.Добавить("Номенклатура,Характеристика,Серия");
	
	Если КодыНаОборудованииРозлива.Количество() = 0 Тогда
		Возврат ВозвращаемоеЗначение;
	КонецЕсли;
	...
	ПараметрыЗаполнения = ПараметрыЗаполненияТаблицыИзДереваУпаковок();
	ПараметрыЗаполнения.ТребуетсяЧастичноеВыбытие = Истина;
	...
	Для Каждого СтрокаТаблицы Из КодыНаОборудованииРозлива Цикл
		
		СтруктураПоиска = Новый Структура("Номенклатура,Характеристика");
		Если СтрокаТаблицы.УчитыватьСерии Тогда
			СтруктураПоиска.Вставить("Серия");
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаТаблицы);
		Если КодыНаОборудованииРозлива.НайтиСтроки(СтруктураПоиска).Количество() <> 1 Тогда
			Продолжить;
		КонецЕсли;
		...
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, ИсходнаяСтрокаТовары);
		КоличествоЧастичноеВыбытие = ИсходнаяСтрокаТовары.Количество;
		СтрокиШтрихкодов = ТаблицаШтрихкоды.НайтиСтроки(СтруктураПоиска);
		Для Каждого СтрокаШтрихкода Из СтрокиШтрихкодов Цикл
			КоличествоЧастичноеВыбытие = КоличествоЧастичноеВыбытие - СтрокаШтрихкода.Количество;
		КонецЦикла;
		
		Если КоличествоЧастичноеВыбытие = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаШтрихкодаТекущегоКега = КешКодовМаркировки[СтрокаТаблицы.КодМаркировки];
		Если СтрокаШтрихкодаТекущегоКега = Неопределено Тогда
			Если КоличествоЧастичноеВыбытие > 0 Тогда
				ЭлементКоллекции = НовыйЭлементКоллекцииУпаковокДляРаспределенияПоТоварам();
				ЭлементКоллекции.ШтрихкодУпаковки             = СтрокаТаблицы.КодМаркировки;
				ЭлементКоллекции.ЧастичноеВыбытиеКоличество   = КоличествоЧастичноеВыбытие;
				ЭлементКоллекции.ЧастичноеВыбытиеВариантУчета = СтрокаТаблицы.ВариантЧастичногоВыбытия;
				Если СтрокаТаблицы.ВариантЧастичногоВыбытия = Перечисления.ВариантыУчетаЧастичногоВыбытияИС.НастроеннаяНоменклатура Тогда
					ЭлементКоллекции.ЧастичноеВыбытиеНоменклатура   = ТаблицаТовары[СтрокаТаблицы.ИндексИсходнойСтроки].Номенклатура;
					ЭлементКоллекции.ЧастичноеВыбытиеХарактеристика = ТаблицаТовары[СтрокаТаблицы.ИндексИсходнойСтроки].Характеристика;
				КонецЕсли;
				ВозвращаемоеЗначение.ДобавленныеКодыМаркировки.Вставить(
					ИсходнаяСтрокаТовары,
					ЭлементКоллекции);
		...
		Иначе
			ЭлементКоллекции = НовыйЭлементКоллекцииУпаковокДляРаспределенияПоТоварам();
			ЭлементКоллекции.ШтрихкодУпаковки             = СтрокаТаблицы.КодМаркировки;
			ЭлементКоллекции.ЧастичноеВыбытиеКоличество   = СтрокаШтрихкодаТекущегоКега.ЧастичноеВыбытиеКоличество + КоличествоЧастичноеВыбытие;
			ЭлементКоллекции.ЧастичноеВыбытиеВариантУчета = СтрокаТаблицы.ВариантЧастичногоВыбытия;
			Если СтрокаТаблицы.ВариантЧастичногоВыбытия = Перечисления.ВариантыУчетаЧастичногоВыбытияИС.НастроеннаяНоменклатура Тогда
				ЭлементКоллекции.ЧастичноеВыбытиеНоменклатура   = ТаблицаТовары[СтрокаТаблицы.ИндексИсходнойСтроки].Номенклатура;
				ЭлементКоллекции.ЧастичноеВыбытиеХарактеристика = ТаблицаТовары[СтрокаТаблицы.ИндексИсходнойСтроки].Характеристика;
			КонецЕсли;
			ВозвращаемоеЗначение.ИзмененныеКодыМаркировки.Вставить(
				ИсходнаяСтрокаТовары,
				ЭлементКоллекции);
			ВозвращаемоеЗначение.ЕстьИзменения = Истина;
		КонецЕсли;

		Если ВозвращаемоеЗначение.ЕстьИзменения
			И СерииИспользюутся
			И Не ЗначениеЗаполнено(ИсходнаяСтрокаТовары.Серия)
			И ЗначениеЗаполнено(СтрокаТаблицы.Серия) Тогда
			ВозвращаемоеЗначение.УстановленныеСерии.Вставить(ИсходнаяСтрокаТовары, СтрокаТаблицы.Серия);
		КонецЕсли;
