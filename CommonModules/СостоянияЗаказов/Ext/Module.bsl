...
// Перемещает состояние в табличной части вида заказа
// 
// Параметры:
//  ВидЗаказа - СправочникСсылка.ВидыЗаказовПокупателей,СправочникСсылка.ВидыЗаказНарядов
...
Процедура ПроверитьОчиститьИсториюСостояний(ЗаказСсылка, ПредыдущееСостояние, ТекущееСостояние, ВидЗаказа = Неопределено) Экспорт
	...
	Если ВидЗаказа <> Неопределено И ПорядокТекущего = Неопределено Тогда
		ИмяСобытия = НСтр("ru='СостоянияЗаказов'", ОбщегоНазначения.КодОсновногоЯзыка());
		ТекстОшибки = СтрШаблон(НСтр("ru='Нельзя записать заказ: новое состояние заказа ""%1"" не принадлежит виду заказа ""%2"".'"),
			ТекущееСостояние, ВидЗаказа);
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,, ЗаказСсылка, ТекстОшибки);
...
Функция ЗаказПрошелСинхронизируемоеСостояние(Заказ, СинхронизируемоеСостояние, СинхронизируемыйВариантЗавершения = Неопределено, ТекущееСостояние = Неопределено)
	...
	Если Не ЗначениеЗаполнено(ДанныеЗаказа.СостояниеЗаказа) Тогда
		Возврат Ложь;
	КонецЕсли;
