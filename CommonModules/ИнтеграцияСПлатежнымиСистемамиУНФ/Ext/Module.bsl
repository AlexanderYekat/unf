...
Процедура ПриОпределенииНастроекИнтеграции(Настройки) Экспорт
	...
	Настройки.Вставить("ИспользоватьЧастичныеОплаты", 	Истина);
...
//
Процедура ПриНастройкеЭлементовФормыИнтеграции(НастройкиФормы, ОбщиеЭлементы, ДополнительныеПараметры) Экспорт
	...
	СпособОплаты	   		   = ?(Не ИспользоватьНесколькоВидовОплаты(), 
		ВидОплатыПоУмолчанию(Организация),
		НастройкиФормы.ЗначенияНастроекОплаты.СпособОплаты);
		
	Организация 			   = ?(Не ИспользоватьНесколькоОрганизаций(),
		ОрганизацияПоУмолчанию(),
		НастройкиФормы.ЗначенияНастроекОплаты.Организация);
	...
	Если ЗначениеЗаполнено(НастройкиПодключения) Тогда
		...
		ТекущаяИнтеграция		   = НастройкиПодключения[0].Интеграция;            
		...
		Для Каждого СвойствоЭлемента Из НастройкиЭлемента Цикл	
			ОбщиеЭлементы.ДекорацияДополнительнаяИнформация[СвойствоЭлемента.Ключ] = СвойствоЭлемента.Значение;
		КонецЦикла;
...
Процедура ПриФормированииЗаказаНаОплатуСБП(ДокументОплаты, 
	...
	СистемаБыстрыхПлатежей.ПараметрыПодключенияПоНастройке(ТорговаяТочка);											
	Если ДокументОплатыСтруктура <> Неопределено Тогда
		...
		ЗаполнитьНазначениеПлатежа(ЗаказНаОплату, ДокументОплаты, ДокументОплатыСтруктура);
		
	Иначе

		ПараметрыОплаты		= СистемаБыстрыхПлатежей.ПараметрыПодключенияПоНастройке(ТорговаяТочка).ПараметрыОплаты.c2b;
		РеквизитыОплаты 	= НастройкиСБПОснованияПлатежа(ДокументОплаты);
		
		Для Каждого ПараметрОплаты Из ПараметрыОплаты Цикл
			РеквизитыОплаты.Вставить(ПараметрОплаты.Имя, ПараметрОплаты.Значение);
		КонецЦикла;
		
		ЗаказНаОплату.СуммаОплаты					= РеквизитыОплаты.ИтогПоЧеку;
		ЗаказНаОплату.ДатаОплаты					= РеквизитыОплаты.Дата;
		ЗаказНаОплату.ОтложенноеПолучениеСтатуса	= Истина;
		ЗаказНаОплату.СрокЖизниQRКода				= 24*60*10; // 10 дней
		
		Если ТипЗнч(ДокументОплаты) = Тип("ДокументСсылка.ПлатежнаяСсылкаСБПc2b") Тогда
			ЗаполнитьНазначениеПлатежа(ЗаказНаОплату, ДокументОплаты.ОснованиеПлатежа, РеквизитыОплаты);	
		Иначе
			ЗаполнитьНазначениеПлатежа(ЗаказНаОплату, ДокументОплаты, РеквизитыОплаты);	
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДокументОперации)
		...
		Попытка
			...
			Если ДокументОплаты <> Неопределено Тогда
				...
				Если ПодключаемоеОборудованиеУНФ.ЕстьОблачнаяКассаАвтоматическойФискализации(
					...
					ДокументОплаты.ИдентификаторЧекаВОчереди = ?(ЗначениеЗаполнено(ДокументОплаты.ИдентификаторЧекаВОчереди),
						...
						XMLСтрока(ДокументОплаты.Ссылка));
	...
	Если ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ОперацияПоПлатежнымКартам") Тогда
		...
		Если ОбъектОбработан Тогда
			...
			Попытка
				...
				ОбъектДокумента.Статус 			= ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.Пробит");
...
Процедура НастройкаДекорацииПодключенияКассовыхСсылок(НастройкиЭлемента, Интеграция, Организация, ТорговыйОбъект, СпособОплаты) Экспорт
	...
	Если НастройкиТорговойТочки.НастройкиСБПc2b.КассовыеСсылки Тогда
		...
		НастройкиЭлемента.Вставить("ГоризонтальноеПоложение",		 ГоризонтальноеПоложениеЭлемента.Право);
		НастройкиЭлемента.Вставить("ГоризонтальноеПоложениеВГруппе", ГоризонтальноеПоложениеЭлемента.Право);
...
Функция НастройкиСБПОснованияПлатежа(ОснованиеПлатежа)
	...
	Если ТипЗнч(ОснованиеПлатежа) = Тип("ДокументСсылка.ПлатежнаяСсылкаСБПc2b") Тогда
	...
	Иначе
		...
		Если МетаданныеОснования.Реквизиты.Найти("Подразделение") <> Неопределено Тогда
			...
			РеквизитыПлатежа.Вставить("Подразделение",	"Подразделение");
		ИначеЕсли МетаданныеОснования.Реквизиты.Найти("СтруктурнаяЕдиницаПродажи") <> Неопределено Тогда
			...
			РеквизитыПлатежа.Вставить("Подразделение",	"СтруктурнаяЕдиницаПродажи");
		ИначеЕсли МетаданныеОснования.Реквизиты.Найти("СтруктурнаяЕдиница") <> Неопределено Тогда
			...
			РеквизитыПлатежа.Вставить("Подразделение",	"СтруктурнаяЕдиница");
		Иначе
...
Процедура ЗаполнитьНазначениеПлатежа(ЗаказНаОплату, ДокументОплаты, ДокументОплатыСтруктура)
	
	ПоИмени 		= Новый Структура("Идентификатор", ДокументОплаты.Метаданные().Имя);
	НайденныеСтроки = ЗаказНаОплату.ШаблоныНазначений.НайтиСтроки(ПоИмени);
	
	Если НайденныеСтроки.Количество() > 0 Тогда

		ЗаказНаОплату.НазначениеПлатежа = НайденныеСтроки[0].НазначениеПлатежа;
		
		РасширитьПоляНазначения(ЗаказНаОплату.НазначениеПлатежа, ДокументОплатыСтруктура, "СерийныйНомер");
		РасширитьПоляНазначения(ЗаказНаОплату.НазначениеПлатежа, ДокументОплатыСтруктура, "РегистрационныйНомер");
