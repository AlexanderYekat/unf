...
Функция ПодключенТелеграм() Экспорт
	...
	Попытка
		...
		Для Каждого Интеграции Из ОбщегоНазначения.ВычислитьВБезопасномРежиме("СистемаВзаимодействия.ПолучитьИнтеграции()") Цикл
...
// См. ЭлектроннаяПочтаУНФПереопределяемый.ПослеСозданияСобытияНаОснованииИнтернетПочтовогоСообщения
//
Процедура ПослеСозданияСобытияНаОснованииИнтернетПочтовогоСообщения(Событие, ИнтернетПочтовоеСообщение) Экспорт
	
	Если Событие.ВходящееИсходящееСобытие <> Перечисления.ВходящееИсходящееСобытие.Исходящее Тогда
		ОписаниеВходящего = ОписаниеВходящего();
		ОписаниеВходящего.Ответственный = Событие.Ответственный;
		ДобавитьВоВходящее(Событие.Ссылка, ОписаниеВходящего);
	КонецЕсли;
...
Процедура ПриИзмененииОтветственногоВИсточнике(Источник, ОтветственныйДо, ОтветственныйТекущий, ЭтоНовый);
	...
	УдалитьИзВходящего(Источник);
	
	ОписаниеВходящего = ОписаниеВходящего();
	ОписаниеВходящего.Ответственный = ОтветственныйТекущий;
	ДобавитьВоВходящее(Источник, ОписаниеВходящего);
