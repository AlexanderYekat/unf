...
Функция ПолучитьНовыйОбъект(Прокси, Тип, Предмет = Неопределено, Колонки = Неопределено,
	...
	Если Предмет <> Неопределено Тогда
		...
		ИначеЕсли Настройки.ИспользоватьИнтеграциюДО2 Тогда
			МодульИнтеграцияС1СДокументооборот = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияС1СДокументооборот");
			Возврат МодульИнтеграцияС1СДокументооборот.ПолучитьНовыйОбъект(Прокси, Тип, Предмет);
...
Функция СоздатьDMLink(Прокси, ИдентификаторОбъектаДО = "", ТипОбъектаДО = "", ОбъектИС = Неопределено) Экспорт
	...
	Если ИдентификаторОбъектаДО <> "" И ТипОбъектаДО <> "" Тогда
...
Функция КомандыПечатиФормы(ИмяФормы) Экспорт
	...
	Иначе
		...
		Для Каждого КомандаПечати Из КомандыПечатиФормы Цикл
			Попытка
				КомандаПечати.ДополнительныеПараметрыВСтроку = ОбщегоНазначения.ЗначениеВJSON(
					КомандаПечати.ДополнительныеПараметры);
			Исключение
				КомандаПечати.ДополнительныеПараметрыВСтроку = "";
			КонецПопытки;
		КонецЦикла;
