...
Процедура СформироватьТаблицаДоходыИРасходыКассовыйМетодЭквайринг(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	// В этой процедуре нам нужно связать отложенную отгрузку и нераспределенную оплату 
	...
	|ПОМЕСТИТЬ ДоходыИРасходыКассовыйМетодЭквайрингОстаткиОбъединение
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДоходыИРасходыКассовыйМетодЭквайрингОстаткиОбъединение.Документ,
	|	ДоходыИРасходыКассовыйМетодЭквайрингОстаткиОбъединение.Организация,
	|	ДоходыИРасходыКассовыйМетодЭквайрингОстаткиОбъединение.НаправлениеДеятельности,
	|	ДоходыИРасходыКассовыйМетодЭквайрингОстаткиОбъединение.Статья,
	|	СУММА(ДоходыИРасходыКассовыйМетодЭквайрингОстаткиОбъединение.СуммаДоходовОстаток) КАК СуммаДоходовОстаток
	|ПОМЕСТИТЬ ДоходыИРасходыКассовыйМетодЭквайрингОстатки
	|ИЗ
	|	ДоходыИРасходыКассовыйМетодЭквайрингОстаткиОбъединение КАК ДоходыИРасходыКассовыйМетодЭквайрингОстаткиОбъединение
	|
	|СГРУППИРОВАТЬ ПО
	|	ДоходыИРасходыКассовыйМетодЭквайрингОстаткиОбъединение.Документ,
	|	ДоходыИРасходыКассовыйМетодЭквайрингОстаткиОбъединение.Организация,
	...
	Пока ВыборкаРезультатаЗапроса.Следующий() И НеРаспределено > 0 Цикл
		...
		СуммаКСписаниюПоДоходамИРасходамКМ = ВыборкаРезультатаЗапроса.СуммаКСписанию - СуммаКСписаниюВБуферномРегистре;
		...
		// Цикл №1. Зачесть сумму СуммаКСписаниюПоДоходамИРасходамКМ.
		Если СуммаКСписаниюПоДоходамИРасходамКМ > 0 Тогда
			СуммаКСписанию = СуммаКСписаниюПоДоходамИРасходамКМ;
...
Процедура ЗаполнитьЭквайринговыеОперации(Объект, ДатаНачала = '00010101', ДатаОкончания = '00010101', Договор = Неопределено) Экспорт
	...
	|	РасчетыПоЭквайрингуОстатки.ВидОплаты КАК ВидОплаты
	...
	|	ВременнаяТаблицаРасчетыПоЭквайрингуОстаткиМинусДвиженияДокумента.ВидОплаты КАК ВидОплаты
	...
	|	ВременнаяТаблицаРасчетыПоЭквайрингуОстаткиМинусДвиженияДокумента.ВидОплаты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|			ИЛИ ВременнаяТаблицаРасчетыПоЭквайрингуОстатки.ВидОплаты = ЗНАЧЕНИЕ(Перечисление.ВидыБезналичныхОплат.ВыдачаНаличных)
	|			ТОГДА ВременнаяТаблицаРасчетыПоЭквайрингуОстатки.СуммаВалОстаток
	|		ИНАЧЕ 0
	...
	|	ВременнаяТаблицаРасчетыПоЭквайрингуОстатки.ВидОплаты
	...
	|		ИНАЧЕ &Патент
	|	КОНЕЦ КАК Патент,
	|	ВременнаяТаблицаДляЗаполненияЭквайринговыхОпераций.ВидОплаты КАК ВидОплаты
	|ИЗ
...
Процедура ЗаполнитьРасшифровкуПлатежаПоОперациямЭквайринга(Объект) Экспорт
	...
	ТаблицаЭквайринговыхОпераций.Колонки.Добавить("ВидОплатыДляСвертки", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыБезналичныхОплат"));
	
	Если ТипДокумента = Тип("ДокументСсылка.ПоступлениеНаСчет") Тогда
		КолонкиСвертки = "УчитыватьВНУ, НалогообложениеНДС, Патент, ВидОплатыДляСвертки";
		
		Для Каждого ТекущаяСтрокаДоСвертки Из ТаблицаЭквайринговыхОпераций Цикл
			Если ТекущаяСтрокаДоСвертки.ВидОплаты = ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.ВыдачаНаличных") Тогда
				ТекущаяСтрокаДоСвертки.ВидОплатыДляСвертки = ТекущаяСтрокаДоСвертки.ВидОплаты;
			КонецЕсли;
		КонецЦикла;
	Иначе //РасходСоСчета
	...
	Для Каждого ТекущаяСтрока Из ТаблицаЭквайринговыхОпераций Цикл
		...
		Если ТипДокумента = Тип("ДокументСсылка.ПоступлениеНаСчет") Тогда //Данный реквизит присутствует только в Поступлении
			...
			НоваяСтрока.ВидОплаты = ТекущаяСтрока.ВидОплатыДляСвертки;
		КонецЕсли; 
...
// Функция возвращает договор эквайринга
//
// Параметры:
//  Контрагент	 - СправочникСсылка.Контрагент	 - Банк-эквайер
//  Организация	 - СправочникСсылка.Организация	 - Организация, с которой заключен договор эквайринга
// 
// Возвращаемое значение:
//  СправочникСсылка.ДоговорыКонтрагентов - Действующий договор эквайринга
//
Функция ПолучитьДоговорЭквайринга(Контрагент, Организация) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ОсновныеДоговорыКонтрагента.Договор КАК Договор
		|ПОМЕСТИТЬ ВТ_ОсновныеДоговоры
		|ИЗ
		|	РегистрСведений.ОсновныеДоговорыКонтрагента КАК ОсновныеДоговорыКонтрагента
		|ГДЕ
		|	ОсновныеДоговорыКонтрагента.Организация = &Организация
		|	И ОсновныеДоговорыКонтрагента.Контрагент = &Контрагент
		|	И ОсновныеДоговорыКонтрагента.Договор.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоров.Прочее)
		|	И ОсновныеДоговорыКонтрагента.Договор.ЭтоДоговорЭквайринга
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ДоговорыКонтрагентов.Ссылка КАК Ссылка,
		|	ВТ_ОсновныеДоговоры.Договор КАК Договор
		|ИЗ
		|	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОсновныеДоговоры КАК ВТ_ОсновныеДоговоры
		|		ПО ДоговорыКонтрагентов.Ссылка = ВТ_ОсновныеДоговоры.Договор
		|ГДЕ
		|	ДоговорыКонтрагентов.Владелец = &Контрагент
		|	И ДоговорыКонтрагентов.Организация = &Организация
		|	И ДоговорыКонтрагентов.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоров.Прочее)
...
// Функция возвращает параметры эквайринга для документа
//
// Параметры:
//  Документ - ДокументСсылка.ПоступлениеНаСчет	 - Документ для которого получаются параметры
// 
// Возвращаемое значение:
//  Структура - Параметры эквайринга
//
...
// Функция определяет есть ли договоры эквайринга с разнесением оплат
// 
// Возвращаемое значение:
//  Булево - Признак наличия договоров эквайринга с разнесением оплат
//
...
// Процедура заполняет поля комиссии в документе
//
// Параметры:
//  ДокументОбъект	 - ДокументОбъект.ПоступлениеНаСчет	 - Документ для заполнения
//
Процедура ЗаполнитьПоляКомиссии(ДокументОбъект) Экспорт
	...
	ТаблицаВидовКартИПроцентовКомиссии = Справочники.СпособыОплаты.ВидыПлатежныхКартИПроцентыКомиссии(ДокументОбъект.ЭквайринговыйТерминал);
