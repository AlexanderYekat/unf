...
Процедура ЗаполнитьСпособЗачетаВРасшифровкеПлатежа(пОбъект) Экспорт
	...
	Если ЗначениеЗаполнено(пОбъект.ДокументОснование) И ТипЗнч(пОбъект.ДокументОснование) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
		Возврат;
	КонецЕсли;
...
// Процедура - Обновляет значение аналитики в зависимости от значения реквизита ШапкаТабличнаяЧасть
//
// Параметры:
...
Процедура ОбработатьРеквизитыШапкаТабличнаяЧастьПередЗаписью(Объект) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Объект.ПоложениеСтатьи) Тогда
		Объект.ПоложениеСтатьи = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.ПоложениеПроекта) Тогда
		Объект.ПоложениеПроекта = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке;
	КонецЕсли;
...
//
Процедура РаспределитьСуммуПоКолонкеТаблицы(Знач РаспределяемаяСумма, Таблица, ИмяКолонки, ИмяКолонкиБазы = "") Экспорт
	
	// Точности результата и распределяемой суммы должны совпадать, иначе возможны ошибки округления
	РаспределяемаяСумма = Окр(РаспределяемаяСумма, 2);
...
Процедура СформироватьТаблицаОплатаСчетовИЗаказов(ДокументСсылка, СтруктураДополнительныеСвойства, МассивДоступныхОпераций) Экспорт
	...
	|	0 КАК Сумма,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаРасчетыСПокупателями.Заказ.Кратность = 0
	...
	Если Запрос.Параметры.ЭтоЗаказНаряд Тогда
		...
		|	0 КАК Сумма,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаШапка.Кратность = 0
		|			ТОГДА ВременнаяТаблицаРасчетыСПокупателями.СуммаВал
...
Процедура СформироватьТаблицаОплатаСчетовИЗаказовДенежныеДокументы(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	Если ЭтоЧекККМ Тогда
		ДобавитьТекстЗапросаОплатыАвто = Истина;
	Иначе
		ВидОперации = ВидОперацииДокумента(ДокументСсылка);
		Если ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя
			...
			ДобавитьТекстЗапросаОплатыАвто = Истина;
		Иначе
			ДобавитьТекстЗапросаОплатыАвто = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Если ЭтоЧекККМ Или ДобавитьТекстЗапросаОплатыАвто Тогда
		Запрос.Текст = Запрос.Текст + "
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ДокументОснование", ДокументСсылка.Метаданные()) Тогда
			ТекстОплатыАвто = ТекстОплатыАвто + "
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|	ВЫРАЗИТЬ(ВременнаяТаблицаРасчетыСПокупателями.Документ.ДокументОснование КАК Документ.СчетНаОплату),
			|	ВЫБОР
			|		КОГДА НЕ ВременнаяТаблицаРасчетыСПокупателями.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Аванс)
			|			ТОГДА 0
			|		КОГДА ВременнаяТаблицаШапка.ВалютаДенежныхСредств = ВЫРАЗИТЬ(ВременнаяТаблицаРасчетыСПокупателями.Документ.ДокументОснование КАК Документ.СчетНаОплату).ВалютаДокумента
			|			ТОГДА ВременнаяТаблицаРасчетыСПокупателями.СуммаПлатежа
			|		КОГДА ВременнаяТаблицаРасчетыСПокупателями.Договор.ВалютаРасчетов = ВЫРАЗИТЬ(ВременнаяТаблицаРасчетыСПокупателями.Документ.ДокументОснование КАК Документ.СчетНаОплату).ВалютаДокумента
			|			ТОГДА ВременнаяТаблицаРасчетыСПокупателями.СуммаВал
			|		ИНАЧЕ ВЫРАЗИТЬ(ВременнаяТаблицаРасчетыСПокупателями.СуммаПлатежа * КурсыВалютКассы.Курс * КурсыВалютСчета.Кратность / (КурсыВалютСчета.Курс * КурсыВалютКассы.Кратность) КАК ЧИСЛО(15, 2))
			|	КОНЕЦ,
			|	ВЫБОР
			...
			|		ИНАЧЕ ВЫРАЗИТЬ(ВременнаяТаблицаРасчетыСПокупателями.СуммаПлатежа * КурсыВалютКассы.Курс * КурсыВалютСчета.Кратность / (КурсыВалютСчета.Курс * КурсыВалютКассы.Кратность) КАК ЧИСЛО(15, 2))
			|	КОНЕЦ,
			|	ВременнаяТаблицаРасчетыСПокупателями.СтавкаНДС,
			|	ВременнаяТаблицаРасчетыСПокупателями.СуммаНДС
			|ИЗ
			|	ВременнаяТаблицаРасчетыСПокупателями КАК ВременнаяТаблицаРасчетыСПокупателями
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
			|		ПО (ИСТИНА)
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&МоментВремени, ) КАК КурсыВалютСчета
			|		ПО (ВЫРАЗИТЬ(ВременнаяТаблицаРасчетыСПокупателями.Документ.ДокументОснование КАК Документ.СчетНаОплату).ВалютаДокумента = КурсыВалютСчета.Валюта)
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&МоментВремени, ) КАК КурсыВалютКассы
			|		ПО (ВременнаяТаблицаШапка.ВалютаДенежныхСредств = КурсыВалютКассы.Валюта)
			|ГДЕ
			|	ВременнаяТаблицаШапка.Контрагент.ВестиУчетОплатыПоСчетам
			|	И ВременнаяТаблицаРасчетыСПокупателями.СпособЗачета = ЗНАЧЕНИЕ(Перечисление.СпособыЗачетаИРаспределенияПлатежей.Авто)
			|	И ВременнаяТаблицаРасчетыСПокупателями.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
			|	И ЕСТЬNULL(ВЫРАЗИТЬ(ВременнаяТаблицаРасчетыСПокупателями.Документ.ДокументОснование КАК Документ.СчетНаОплату), ЗНАЧЕНИЕ(Документ.СчетНаОплату.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Документ.СчетНаОплату.ПустаяСсылка)
			|	И (ВЫРАЗИТЬ(ВременнаяТаблицаРасчетыСПокупателями.Документ.ДокументОснование КАК Документ.СчетНаОплату)) <> НЕОПРЕДЕЛЕНО
			|	И (ВременнаяТаблицаРасчетыСПокупателями.Сумма <> 0
			|			ИЛИ ВременнаяТаблицаРасчетыСПокупателями.СуммаВал <> 0)";
		КонецЕсли;
		...
		Если НЕ ЭтоПоступлениеДС Тогда
			...
			ТекстОплатыАвто = СтрЗаменить(ТекстОплатыАвто, "Документ.СчетНаОплату", "Документ.СчетНаОплатуПоставщика");
		КонецЕсли;
...
Процедура СформироватьТаблицаОплатаДокументовДенежныеДокументы(ДокументСсылка, СтруктураДополнительныеСвойства, МассивДоступныхОпераций, КонтрагентВДвижениях = Ложь) Экспорт
	...
	ИначеЕсли ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя
		...
		ИЛИ ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Покупателю
		ИЛИ ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Покупателю
		ИЛИ ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийЭквайринга.ПоступлениеОплатыОтПокупателя
		ИЛИ ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийЭквайринга.ВозвратОплатыПокупателю Тогда
	...
	Для Каждого ТекущаяСтрока Из ТекущаяТаблицаРасчетов Цикл
		...
		Если ТекущаяСтрока.ВидДвижения = ВидДвиженияНакопления.Расход
			И ТекущаяСтрока.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
			Коэффициент = 1;
		ИначеЕсли ТекущаяСтрока.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
			Коэффициент = -1;
		Иначе	
			Коэффициент = 0;
		КонецЕсли;
		...
		Если Коэффициент <> 0 Тогда
			...
			Если ТекущаяСтрока.Документ = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			...
			НоваяСтрока.СуммаОплаты = НоваяСтрока.СуммаОплаты * Коэффициент;
			НоваяСтрока.СуммаОплатыАвто = НоваяСтрока.СуммаОплатыАвто * Коэффициент;
			НоваяСтрока.СуммаНДС = НоваяСтрока.СуммаНДС * Коэффициент;
...
// Формирует таблицу значений, содержащую данные для проведения по регистру ОплатаДокументов для счетов, заказов и
// документов отгрузки. Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаОплатаДокументовНакладные(ДокументСсылка, СтруктураДополнительныеСвойства, МассивДоступныхОпераций) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПриходнаяНакладная")
		И СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Свойство("ТаблицаРасчетыСПокупателямиДополнение")
		И СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателямиДополнение.Количество() > 0 Тогда
	...
	СтруктураЗначения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка,
		"Договор,ВалютаДокумента,Курс,Кратность");
	ВалютаРасчетовПоДоговору = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураЗначения.Договор, "ВалютаРасчетов"); 
	ВалютыДоговораИДокументаСовпадают = (ВалютаРасчетовПоДоговору = СтруктураЗначения.ВалютаДокумента);
	...
	Для Каждого ТекущаяСтрока Из ТекущаяТаблицаРасчетов Цикл
		...
		Если ТекущаяСтрока.ВидДвижения = ВидДвиженияНакопления.Расход
			...
			Если ВалютыДоговораИДокументаСовпадают Тогда
		...
		ИначеЕсли ТекущаяСтрока.ВидДвижения = ВидДвиженияНакопления.Приход
				И ТекущаяСтрока.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
...
Процедура СформироватьДвиженияПоВзаиморасчетамДляЗаказа(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	|	ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка) КАК НомерСертификата,
	|	ЛОЖЬ КАК ОплатаСертификатом,
	|	РасчетыСПокупателями.ОплатаБонусами КАК ОплатаБонусами,
	|	ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка) КАК ПодарочныйСертификат,
	|	ЛОЖЬ КАК ЭтоОплатаЗаказа
	...
	|ГДЕ
	|	НЕ РасчетыСПокупателями.ОплатаБонусами
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
...
Процедура СформироватьДвиженияПоВзаиморасчетамОплатаАвтоматически(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.УчетВалютныхОпераций Тогда
		...
		КурсыВТаблице = Движения.Скопировать();
		КурсыВТаблице.Свернуть("Курс,Кратность");
		
		Если КурсыВТаблице.Количество() = 1 Тогда
...
Процедура СформироватьДвиженияПоВзаиморасчетамДляДокументовОтгрузки(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	ОсвобождатьАванс = СтруктураДополнительныеСвойства.ДляПроведения.Свойство("ОсвобождатьАванс")
		И СтруктураДополнительныеСвойства.ДляПроведения.ОсвобождатьАванс;
		
	Если ТребуетсяЗачетАвансов И Не ОсвобождатьАванс Тогда
...
Процедура ПодготовитьТаблицуВзаиморасчетовСУчетомАвансов(ДокументСсылка, СтруктураДополнительныеСвойства, ЭтоРасчетыСПоставщиком) Экспорт
	...
	ПараметрыОтбора.Вставить("Контрагент");
	ПараметрыОтбора.Вставить("Договор");
	ПараметрыОтбора.Вставить("ТипРасчетов");
	ПараметрыОтбора.Вставить("Документ");
	ПараметрыОтбора.Вставить("Заказ");
	Для каждого СтрокаРучногоАванса Из СтрокиАванса Цикл
		
		ЗаполнитьЗначенияСвойств(ПараметрыОтбора, СтрокаРучногоАванса);
		
		МассивОстатков = ОстаткиАвансовВРегистре.НайтиСтроки(ПараметрыОтбора);
...
Процедура ВывестиРасшифровкуПлатежа(пРасшифровкаПлатежа)
	...
	Иначе
		Для каждого ТекущаяСтрока Из пРасшифровкаПлатежа Цикл
			...
			СоставСтроки.Добавить(СтрШаблон("%1.", ТекущаяСтрока.НомерСтроки));
			Если ТекущаяСтрока.ПризнакАванса Тогда
				СоставСтроки.Добавить(НСтр("ru = 'Предоплата'"));
			Иначе
				СоставСтроки.Добавить(НСтр("ru = 'Оплата'"));
			КонецЕсли;
			СоставСтроки.Добавить(ТекущаяСтрока.СпособЗачета);
			СоставСтроки.Добавить(НСтр("ru = 'на'"));
			СоставСтроки.Добавить(ТекущаяСтрока.СуммаПлатежа);
			СоставСтроки.Добавить(НСтр("ru = 'по договору'"));
			СоставСтроки.Добавить(ТекущаяСтрока.Договор);
			Если ЗначениеЗаполнено(ТекущаяСтрока.Заказ) Тогда
				СоставСтроки.Добавить(НСтр("ru = 'по заказу'"));
				СоставСтроки.Добавить(ТекущаяСтрока.Заказ);
			КонецЕсли;
			Если ЗначениеЗаполнено(ТекущаяСтрока.Документ) Тогда
				СоставСтроки.Добавить(НСтр("ru = 'по документу'"));
				СоставСтроки.Добавить(ТекущаяСтрока.Документ);
			КонецЕсли;
			ТекстСообщения = СтрСоединить(СоставСтроки, " ");
...
Процедура ОбработатьТаблицуДокументовДляИзмененияПриОтгрузке(ДополнительныеСвойства, Отказ) Экспорт
	...
	Для Каждого ТекущаяСтрока Из ТаблицаДокументовДляИзменения Цикл
		...
		Если ТекущаяСтрока.ВариантИзмененияДанных <> Перечисления.ВариантыИзмененияДанныхВДокументахИзПомощника.Удаление
			...
			НеПолучилосьИзменитьДокумент = СтрШаблон(НСтр("ru = 'Не получилось изменить документ ""%1"".'"),
				ТекущаяСтрока.Документ);
			СоставСообщения.Добавить(НеПолучилосьИзменитьДокумент);
			Если ЗначениеЗаполнено(ТекущаяСтрока.Заказ) Тогда
				ВозможнаяПричина = СтрШаблон(НСтр("ru = 'Возможная причина: не найдена строка с заказом ""%1"".'"),
					ТекущаяСтрока.Заказ);
				СоставСообщения.Добавить(ВозможнаяПричина);
				СоставСообщения.Добавить(НСтр("ru = 'Рекомендация: укажите заказ в документе оплаты.'"));
			КонецЕсли;
			СоставСообщения.Добавить();
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтрСоединить(СоставСообщения, Символы.ПС);
			Если ЗначениеЗаполнено(ТекущаяСтрока.Документ) Тогда
...
// Процедура формирует временную таблицу предоплаты для кассового метода
//
// Параметры:
//  СтруктураДополнительныеСвойства	 - Структура	 - Структура свойств документа при проведении
...
// Процедура обновляет даты зачета в таблице доходы расходы кассовым методом
//
// Параметры:
//  СтруктураДополнительныеСвойства			 - Структура	 - Структура свойств документа при проведении
//  ТаблицаЗапасыДоходыИРасходыОтложенные	 - ТаблицаЗначений	 - Таблица с движениями по регистру Доходы и расходы отложенные
//
Процедура ОбновитьДатыВТаблицеДоходыРасходыОтложенные(СтруктураДополнительныеСвойства, ТаблицаЗапасыДоходыИРасходыОтложенные) Экспорт
	
	Для каждого СтрокаТаблицы Из ТаблицаЗапасыДоходыИРасходыОтложенные Цикл
		
		СтрокаТаблицы.Период = СтруктураДополнительныеСвойства.ДляПроведения.Дата;
...
// Функция возвращает статус оплаты заказа
//
// Параметры:
//  Заказ		 - ДокументСсылка	 - Заказ покупателя или Заказ поставщику
// 
// Возвращаемое значение:
//  Перечисление.СтатусОплаты - статус оплаты заказа
//
Функция СтатусОплаты(Заказ) Экспорт
	...
	Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЗаказПокупателя", "ЗаказПоставщику"); 
	КонецЕсли;	
...
// Функция возвращает процент оплаты заказа
//
// Параметры:
//  Заказ		 - ДокументСсылка	 - Заказ покупателя или Заказ поставщику
// 
// Возвращаемое значение:
//  Число - процент оплаты заказа
//
Функция ПроцентОплаты(Заказ) Экспорт
	...
	|				И ЕСТЬNULL(ФактОплаты.СуммаОплаты, 0) + ЕСТЬNULL(ФактОплаты.СуммаАванса, 0) = 0
	|			ТОГДА 0
	|		КОГДА ЕСТЬNULL(ФактОплаты.СуммаОплаты, 0) + ЕСТЬNULL(ФактОплаты.СуммаАванса, 0) - ФактОплаты.Сумма >= 0
	|			ТОГДА 100
	|		КОГДА ЕСТЬNULL(ФактОплаты.СуммаОплаты, 0) + ЕСТЬNULL(ФактОплаты.СуммаАванса, 0) > 0
...
// Функция возвращает сумму задолженности по документу расчетов
//
// Параметры:
//  ДокументСсылка		 - ДокументСсылка	 - Документ расчетов
// 
// Возвращаемое значение:
//  Число - сумма задолженности по документу покупателя
//
Функция СуммаЗадолженностиПоДокументу(ДокументСсылка) Экспорт
	
	Результат = Неопределено;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	Регистр.Сумма - Регистр.СуммаАванса - Регистр.СуммаОплаты КАК Сумма
	               |ИЗ
	               |	РегистрСведений.ФактОплатыДокументов КАК Регистр
	               |ГДЕ
	               |	Регистр.Документ = &Ссылка";
	
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	
	ТипДокумента = ТипЗнч(ДокументСсылка);
	Если Метаданные.РегистрыСведений.ФактОплатыЗаказов.Измерения.СчетНаОплату.Тип.СодержитТип(ТипДокумента) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ФактОплатыДокументов", "ФактОплатыЗаказов");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Документ", "СчетНаОплату");
	КонецЕсли;
...
// Функция возвращает суммы оплат по СБП по документу расчетов
//
// Параметры:
//  ДокументСсылка		 - ДокументСсылка	 - Документ расчетов
// 
// Возвращаемое значение:
//  Число - сумма оплат по СБП по документу покупателя
//
Функция СуммаОплатСБППоДокументу(ДокументСсылка) Экспорт
	
	Результат = 0;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	СУММА(ОперацияПоПлатежнымКартамРасшифровкаПлатежа.СуммаРасчетов) КАК Сумма
	               |ИЗ
	               ...
	               |	И ОперацияПоПлатежнымКартамРасшифровкаПлатежа.Ссылка.СпособОплаты = ЗНАЧЕНИЕ(Перечисление.СпособыОплатыЭквайринг.ОплатаСБП)
	               |	И (ОперацияПоПлатежнымКартамРасшифровкаПлатежа.Документ = &Ссылка
	               |			ИЛИ ОперацияПоПлатежнымКартамРасшифровкаПлатежа.Заказ = &Ссылка
	               |			ИЛИ ОперацияПоПлатежнымКартамРасшифровкаПлатежа.СчетНаОплату = &Ссылка)
	               |
	               |ИМЕЮЩИЕ
	               |	СУММА(ОперацияПоПлатежнымКартамРасшифровкаПлатежа.СуммаРасчетов) > 0";
...
// Функция возвращает максимальную сумму оплаты по СБП по документу расчетов
// Реализует следующий алгоритм:
// 1. заказ 1000 руб, оплата налом 100 руб, макс. к оплате = 900 (даже если документ не проведен, его проведут потом)
// 2. заказ 1000 руб, оплата СБП 100 руб, макс к оплате = 1000 руб. (не считаем СБП)
// 3. заказ 1000 руб, оплата налом 100 руб, оплата СБП 100 руб, макс. к оплате 900 руб
//	(по формуле: остаток взаиморасчетов (800 руб) + оплата СБП (100 руб.)
//
// Для непроведенного документа запись в РС ФактОплатыЗаказов может как быть (в редких случаях), так и отсутствовать.
// Обработаны оба случая. Первый случай обработан в запросе, второй - в коде.
//
// Параметры:
//  ДокументСсылка		 - ДокументСсылка	 - Документ расчетов
// 
// Возвращаемое значение:
//  Число - максимальная сумма оплаты по СБП по документу расчетов
//
Функция МаксимальнаяСуммаОплатыСБППоДокументу(ДокументСсылка) Экспорт
	
	Результат = 0;
	
	ТипДокумента = ТипЗнч(ДокументСсылка);
	Если Не Метаданные.РегистрыСведений.ФактОплатыДокументов.Измерения.Документ.Тип.СодержитТип(ТипДокумента)
		И Не Метаданные.РегистрыСведений.ФактОплатыЗаказов.Измерения.СчетНаОплату.Тип.СодержитТип(ТипДокумента) Тогда
		Возврат Результат;
	КонецЕсли;
	
	Если ТипДокумента = Тип("ДокументСсылка.Взаимозачет") Тогда
		ИмяРеквизитаСуммаДокумента = "СуммаУчета";
	Иначе	
		ИмяРеквизитаСуммаДокумента = "СуммаДокумента";
	КонецЕсли;
	
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка,
		ИмяРеквизитаСуммаДокумента + ", Проведен");
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |ВЫБРАТЬ
	                |	ВЫБОР
	                |		КОГДА &ДокументПроведен
	                |			ТОГДА Регистр.Сумма
	                |		ИНАЧЕ &СуммаДокумента
	                |	КОНЕЦ - Регистр.СуммаАванса - Регистр.СуммаОплаты КАК Сумма
	                |ИЗ
	                |	РегистрСведений.ФактОплатыДокументов КАК Регистр
	                |ГДЕ
	                |	Регистр.Документ = &Ссылка";
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.УстановитьПараметр("СуммаДокумента", РеквизитыДокумента[ИмяРеквизитаСуммаДокумента]);
	Запрос.УстановитьПараметр("ДокументПроведен", РеквизитыДокумента.Проведен);
	
	Если Метаданные.РегистрыСведений.ФактОплатыЗаказов.Измерения.СчетНаОплату.Тип.СодержитТип(ТипДокумента) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ФактОплатыДокументов", "ФактОплатыЗаказов");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Регистр.Документ", "Регистр.СчетНаОплату");
	КонецЕсли;
	
	МассивРезультатов = Запрос.ВыполнитьПакет();
	
	ВыборкаПоОплатеСБП = МассивРезультатов[0].Выбрать();
	
	Если ВыборкаПоОплатеСБП.Следующий() Тогда
		СуммаОплатыСБП = ВыборкаПоОплатеСБП.Сумма;
	Иначе	
		СуммаОплатыСБП = 0;
	КонецЕсли;
	
	ВыборкаПоПолнойОплате = МассивРезультатов[1].Выбрать();
	
	Если ВыборкаПоПолнойОплате.Следующий() Тогда
		Результат = ВыборкаПоПолнойОплате.Сумма;
	Иначе	
		Результат = РеквизитыДокумента[ИмяРеквизитаСуммаДокумента];
	КонецЕсли;
	
	Результат = Результат + СуммаОплатыСБП;
	
	Возврат Макс(Результат, 0);
	
КонецФункции

// Выполняет контроль превышения суммы оплаты над суммой документа
// 
// Параметры:
...
//
Процедура ВыполнитьКонтрольСуммыОплаты(СуммаОплаты, СуммаДокумента, Отказ) Экспорт
	...
	Если СуммаОплаты > СуммаДокумента Тогда
		РасчетыРаботаСФормамиВызовСервера.СообщитьОПревышенииСуммыПлатежаОтказ(Отказ);
	КонецЕсли;
	
КонецПроцедуры

// Устанавливает курс и кратность для документов Корректировка реализации / поступления
// 
// Параметры:
// 	ДокументСсылка - ДокументСсылка - .
// 	СтруктураДополнительныеСвойства - Структура - .
//
Процедура УстановитьКурсИКратностьДляКорректировок(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	
	ИсправляемыйДокумент = Неопределено;
	
	ЭтоКорректировкаРеализации = 
		ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаРеализации");
	
	ЭтоКорректировкаПоступления = 
		ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаПоступления");

	Если ЭтоКорректировкаРеализации И ЗначениеЗаполнено(ДокументСсылка.ИсправляемыйДокументРеализации) Тогда
		ИсправляемыйДокумент = ДокументСсылка.ИсправляемыйДокументРеализации;	
	КонецЕсли;
	
	Если ЭтоКорректировкаПоступления И ЗначениеЗаполнено(ДокументСсылка.ИсправляемыйДокументПоступления) Тогда
		ИсправляемыйДокумент = ДокументСсылка.ИсправляемыйДокументПоступления;
	КонецЕсли;
		
	КурсИКратность = КурсИКратностьДляКорректировок(ИсправляемыйДокумент);
	Если Не КурсИКратность.ВыполненаУстановкаКурсаДляКорректировок Тогда
		КурсИКратность = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Курс, Кратность");
	КонецЕсли;
	
	СтруктураДополнительныеСвойства.Вставить("КурсДокумента", КурсИКратность.Курс);
	СтруктураДополнительныеСвойства.Вставить("КратностьДокумента", КурсИКратность.Кратность);
	
КонецПроцедуры

// Возвращает курс и кратность для документов Корректировка реализации / поступления
// 
// Параметры:
// 	ИсправляемыйДокумент - ДокументСсылка - .
// 
// Возвращаемое значение:
// 	Структура - содержит курс и кратность исправляемого документа
//
Функция КурсИКратностьДляКорректировок(ИсправляемыйДокумент) Экспорт
	
	Результат = Новый Структура("ВыполненаУстановкаКурсаДляКорректировок", Ложь);
	...
	Если ЗначениеЗаполнено(ИсправляемыйДокумент) Тогда
		...
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      ...
		                      |	И СуммыДокументовРегламентированныйУчет.Регистратор = &ИсправляемыйДокумент
		                      |
		                      |ИМЕЮЩИЕ
		                      |	СУММА(ЕСТЬNULL(СуммыДокументовРегламентированныйУчет.Всего, 0)) <> 0");
		...
		Если Выборка.Следующий() Тогда
			...
			Результат.Вставить("Курс", Выборка.СуммаРегл / СуммаИсправляемогоДокумента);
			Результат.Вставить("Кратность", 1);
			Результат.Вставить("ВыполненаУстановкаКурсаДляКорректировок", Истина);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьТекстЗапросаОстаткиАвансов(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	|ГДЕ
	|	НЕ(ТИПЗНАЧЕНИЯ(&Регистратор) = ТИП(Документ.ПриходнаяНакладная)
	|				И ТИПЗНАЧЕНИЯ(ВТ_Предоплата.Документ) = ТИП(Документ.ОтчетКомиссионера))
	|СГРУППИРОВАТЬ ПО
...
Функция ПолучитьСтруктуруДанныхДокумента(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Иначе
		ТаблицаПредоплаты = МассивРезультатов[0].Выгрузить();
		Если НЕ ТаблицаПредоплаты.Колонки.Найти("ОплатаБонусами") = Неопределено Тогда
			НеБонусы = ТаблицаПредоплаты.НайтиСтроки(Новый Структура("ОплатаБонусами", Ложь));
			ОплатаБезБонусов = ТаблицаПредоплаты.Скопировать(НеБонусы);
			ТаблицаПредоплаты = ОплатаБезБонусов;
		КонецЕсли;
		
		СтруктураДанныхДокумента.Вставить("Предоплата", ТаблицаПредоплаты);
	КонецЕсли;
...
Процедура СформироватьДвиженияПоВзаиморасчетамДляПередачиТоваровМеждуОрганизациями(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.ПараметрыДокумента.ЭтоРасчетыСПоставщиком Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Предоплата КАК ВТ_Предоплата
			|ГДЕ", "Предоплата КАК ВТ_Предоплата ГДЕ ВТ_Предоплата.ЭтоРасчетыСПоставщиком И");
	Иначе
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Предоплата КАК ВТ_Предоплата
			|ГДЕ", "Предоплата КАК ВТ_Предоплата ГДЕ НЕ ВТ_Предоплата.ЭтоРасчетыСПоставщиком И");
	КонецЕсли;
...
Процедура СформироватьТаблицуРасчетовДляФормированияДвижений(СтруктураДанныхДокумента, ДокументСсылка, СтруктураДополнительныеСвойства, ТаблицаДвиженийПоВзаиморасчетам, ТребуемыеРеквизитыШапки = "", ЗапросПараметры = Неопределено)
	...
	Если ОсвобождатьАванс Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПокупателямиПредварительная",
			"ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПокупателямиДляРасчетаАвансовПредварительная");
		...
		|ИЗ
		|	РегистрНакопления.РасчетыСПокупателями.Остатки(&МоментКонтроля, ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг)) КАК РасчетыСПокупателямиОстатки
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	ТипРасчетов,
		...
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаВал КАК СуммаВал,
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаРег КАК СуммаРег,
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаДляОстатка КАК СуммаДляОстатка,
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаВалДляОстатка КАК СуммаВалДляОстатка,
		|	ТаблицаДвиженийПоВзаиморасчетам.СодержаниеПроводки КАК СодержаниеПроводки,
		|	ТаблицаДвиженийПоВзаиморасчетам.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	ТаблицаДвиженийПоВзаиморасчетам.СпособЗачета КАК СпособЗачета,
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаПлатежа КАК СуммаПлатежа,
		...
		|	ТаблицаДвиженийПоВзаиморасчетам.ПодарочныйСертификат КАК ПодарочныйСертификат,
		|	ТаблицаДвиженийПоВзаиморасчетам.НомерСертификата КАК НомерСертификата,
		|	ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателямиОстатки.СуммаОстаток, 0) КАК СуммаОстаток,
		|	ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателямиОстатки.СуммаВалОстаток, 0) КАК СуммаВалОстаток,
		|	ЕСТЬNULL(ВременнаяТаблицаРасчетыСПокупателямиОстатки.СуммаРегОстаток, 0) КАК СуммаРегОстаток
		|ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПокупателямиДляРасчетаАвансов
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПокупателямиДляРасчетаАвансовПредварительная КАК ТаблицаДвиженийПоВзаиморасчетам
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаРасчетыСПокупателямиОстатки КАК ВременнаяТаблицаРасчетыСПокупателямиОстатки
		|		ПО ТаблицаДвиженийПоВзаиморасчетам.Организация = ВременнаяТаблицаРасчетыСПокупателямиОстатки.Организация
		|			И ТаблицаДвиженийПоВзаиморасчетам.ТипРасчетов = ВременнаяТаблицаРасчетыСПокупателямиОстатки.ТипРасчетов
		|			И ТаблицаДвиженийПоВзаиморасчетам.Контрагент = ВременнаяТаблицаРасчетыСПокупателямиОстатки.Контрагент
		|			И ТаблицаДвиженийПоВзаиморасчетам.Договор = ВременнаяТаблицаРасчетыСПокупателямиОстатки.Договор
		|			И ТаблицаДвиженийПоВзаиморасчетам.Документ = ВременнаяТаблицаРасчетыСПокупателямиОстатки.Документ
		|			И ТаблицаДвиженийПоВзаиморасчетам.Заказ = ВременнаяТаблицаРасчетыСПокупателямиОстатки.Заказ
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	Контрагент,
		...
		|	ТаблицаДвиженийПоВзаиморасчетам.ТипРасчетов КАК ТипРасчетов,
		|	ВЫБОР
		|		КОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаОстаток > ТаблицаДвиженийПоВзаиморасчетам.Сумма
		|			ТОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаОстаток
		|		ИНАЧЕ ТаблицаДвиженийПоВзаиморасчетам.Сумма
		|	КОНЕЦ КАК Сумма,
		|	ВЫБОР
		|		КОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаВалОстаток > ТаблицаДвиженийПоВзаиморасчетам.СуммаВал
		|			ТОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаВалОстаток
		|		ИНАЧЕ ТаблицаДвиженийПоВзаиморасчетам.СуммаВал
		|	КОНЕЦ КАК СуммаВал,
		|	ВЫБОР
		|		КОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаРегОстаток > ТаблицаДвиженийПоВзаиморасчетам.СуммаРег
		|			ТОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаРегОстаток
		|		ИНАЧЕ ТаблицаДвиженийПоВзаиморасчетам.СуммаРег
		|	КОНЕЦ КАК СуммаРег,
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаДляОстатка КАК СуммаДляОстатка,
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаВалДляОстатка КАК СуммаВалДляОстатка,
		|	ТаблицаДвиженийПоВзаиморасчетам.СодержаниеПроводки КАК СодержаниеПроводки,
		...
		|ПОМЕСТИТЬ ВременнаяТаблицаРасчетыСПокупателямиПредварительная
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПокупателямиДляРасчетаАвансов КАК ТаблицаДвиженийПоВзаиморасчетам
		|ГДЕ
		|	(ВЫБОР
		|		КОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаОстаток > ТаблицаДвиженийПоВзаиморасчетам.Сумма
		|			ТОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаОстаток
		...
		|	ТаблицаДвиженийПоВзаиморасчетам.НомерСтроки,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
		|	ТаблицаДвиженийПоВзаиморасчетам.Период,
		|	ТаблицаДвиженийПоВзаиморасчетам.Организация,
		|	ТаблицаДвиженийПоВзаиморасчетам.Контрагент,
		|	ТаблицаДвиженийПоВзаиморасчетам.Договор,
		|	&Ссылка,
		|	ТаблицаДвиженийПоВзаиморасчетам.Заказ,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Аванс),
		|	ВЫБОР
		|		КОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаОстаток > ТаблицаДвиженийПоВзаиморасчетам.Сумма
		|			ТОГДА -(ТаблицаДвиженийПоВзаиморасчетам.СуммаОстаток + ТаблицаДвиженийПоВзаиморасчетам.Сумма)
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаВалОстаток > ТаблицаДвиженийПоВзаиморасчетам.СуммаВал
		|			ТОГДА -(ТаблицаДвиженийПоВзаиморасчетам.СуммаВалОстаток + ТаблицаДвиженийПоВзаиморасчетам.СуммаВал)
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА -ТаблицаДвиженийПоВзаиморасчетам.СуммаРегОстаток > ТаблицаДвиженийПоВзаиморасчетам.СуммаРег
		|			ТОГДА -(ТаблицаДвиженийПоВзаиморасчетам.СуммаРегОстаток + ТаблицаДвиженийПоВзаиморасчетам.СуммаРег)
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаДляОстатка,
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаВалДляОстатка,
		|	&ВосстановлениеАванса,
		|	ТаблицаДвиженийПоВзаиморасчетам.ХозяйственнаяОперация,
		|	ТаблицаДвиженийПоВзаиморасчетам.СпособЗачета,
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаПлатежа,
		|	ТаблицаДвиженийПоВзаиморасчетам.ВалютаПлатежа,
		|	ТаблицаДвиженийПоВзаиморасчетам.ИндексСтрокиДокумента,
		|	ТаблицаДвиженийПоВзаиморасчетам.СуммаНДС,
		|	ТаблицаДвиженийПоВзаиморасчетам.СсылкаНаДокумент,
		|	ТаблицаДвиженийПоВзаиморасчетам.СтавкаНДС,
		|	ТаблицаДвиженийПоВзаиморасчетам.ДокументОплаты,
		|	ТаблицаДвиженийПоВзаиморасчетам.ЭтоОплатаЗаказа,
		|	ТаблицаДвиженийПоВзаиморасчетам.Курс,
		|	ТаблицаДвиженийПоВзаиморасчетам.Кратность,
		|	ТаблицаДвиженийПоВзаиморасчетам.ПодарочныйСертификат,
		|	ТаблицаДвиженийПоВзаиморасчетам.НомерСертификата
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПокупателямиДляРасчетаАвансов КАК ТаблицаДвиженийПоВзаиморасчетам
		|ГДЕ
		|	(-ТаблицаДвиженийПоВзаиморасчетам.СуммаОстаток > ТаблицаДвиженийПоВзаиморасчетам.Сумма
		|			ИЛИ -ТаблицаДвиженийПоВзаиморасчетам.СуммаВалОстаток > ТаблицаДвиженийПоВзаиморасчетам.СуммаВал
		|			ИЛИ -ТаблицаДвиженийПоВзаиморасчетам.СуммаРегОстаток > ТаблицаДвиженийПоВзаиморасчетам.СуммаРег)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	Контрагент,
	...
	|ИЗ
	|	ВременнаяТаблицаРасчетыСПокупателямиПредварительная КАК ТаблицаДвиженийПоВзаиморасчетам";
		...
		Иначе
			...
			Для каждого СтрокаОстатка Из МассивОстатков Цикл
				...
				Иначе
					...
					Если СтруктураДополнительныеСвойства.НациональнаяВалюта = СтруктураДополнительныеСвойства.ВалютаУчета Тогда
						КоэффициентВалютаРасчетов = СтрокаОстатка.СуммаОстаток / СтрокаОстатка.СуммаВалОстаток;
						СтрокаРучногоАванса.СуммаОстаток = СтрокаРучногоАванса.СуммаВалОстаток * КоэффициентВалютаРасчетов;
						СтрокаРучногоАванса.СуммаРегОстаток = СтрокаРучногоАванса.СуммаОстаток;
					ИначеЕсли СтруктураДополнительныеСвойства.НациональнаяВалюта = ДокументОбъект.ДоговорВалютаРасчетов Тогда
						СтрокаРучногоАванса.СуммаРегОстаток = СтрокаРучногоАванса.СуммаВалОстаток;
					Иначе
...
Функция ВидОперацииДокумента(ДокументСсылка)
	
	МетаданныеДокумента = ДокументСсылка.Метаданные();
	ЕстьРеквизитВидОперации = МетаданныеДокумента.Реквизиты.Найти("ВидОперации") <> Неопределено;
	
	Если Не ЕстьРеквизитВидОперации Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ВидОперации");
...
//
// Возвращаемое значение:
// 	ТаблицаЗначений - ПустаяТаблицаВзаиморасчетовЗачетАвансов.
