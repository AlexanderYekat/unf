...
Процедура ПроверитьЗаполнениеАкцизныхМарок(Объект, Отказ, НаДату = Неопределено) Экспорт
	...
	Если НаДату = Неопределено Тогда
		Для Каждого ВидПродукцииИС Из ОбщегоназначенияИСМП.ВидыПродукцииОбязательнойМаркировки() Цикл
			ВидыПродукцииИС.Добавить(ВидПродукцииИС);
		КонецЦикла;
	Иначе
		Для Каждого ВидПродукцииИС Из ОбщегоНазначенияИСМПВызовСервера.УчитываемыеВидыМаркируемойПродукции(НаДату, Ложь) Цикл
			ВидыПродукцииИС.Добавить(ВидПродукцииИС);
		КонецЦикла;
	КонецЕсли;
...
Функция ИННКППGLNОрганизации(Организация, Подразделение) Экспорт
	...
	|	СтруктурныеЕдиницы.КПП КАК КПППодразделения
	|ИЗ
	|	Справочник.Организации КАК Организации
	...
	Если ЗначениеЗаполнено(Подразделение) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПодразделениеГИСМ", "ОрганизацииДляОбменаГИСМ.Подразделение = &Подразделение");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПодразделение", "СтруктурныеЕдиницы.Ссылка = &Подразделение");
		Запрос.УстановитьПараметр("Подразделение", Подразделение);
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПодразделениеГИСМ", "ИСТИНА");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПодразделение", "ЛОЖЬ");
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Выборка.КПППодразделения) Тогда
			Результат.Вставить("КПП", Выборка.КПППодразделения);
		Иначе
...
Процедура ОбработатьДанныеШтрихкодовДереваУпаковок(ДеревоУпаковок, ПараметрыШтрихкодаДляОбработки, Форма = Неопределено, УпаковкаВерхнегоУровня = Неопределено) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если ДанныеШтрихкодаДляОбработки.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар
			...
			Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ДанныеШтрихкодаДляОбработки.ВидПродукции) И СтрокаДерева.СоставКодаМаркировки.Свойство("МРЦ") Тогда
