				...
				Для Каждого СтрокаТовара Из ВерхнийУровень.ТаблицаТоваров Цикл
					...
					Если МинимальнаяЦена = Неопределено И Параметры.Свойство("ТаблицаЦен") Тогда
						...
						Для Каждого СтрокаЦены Из СтрокиМинимальнойЦены Цикл
							...
							ПараметрыМинимальнойЦены.Вставить("СуммаВключаетНДС", СтрокаЦены.СуммаВключаетНДС);
							ПараметрыМинимальнойЦены.Вставить("СтавкаНДС", СтрокаЦены.СтавкаНДС);
...
Процедура ИсключитьЗаписиСтарыхЦенИзТаблицы(ТаблицаНоменклатуры) Экспорт
	...
	Запрос.УстановитьПараметр("ВнешняяТаблицаНоменклауры", ТаблицаНоменклатуры.Скопировать(, "ВидЦен,Номенклатура,Характеристика"));
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ТаблицаТоварыБезЦен = Неопределено;
	Если НЕ РезультатЗапроса.Пустой() Тогда 	
		ТаблицаТоварыБезЦен = РезультатЗапроса.Выгрузить();	
	КонецЕсли;
	...
	Пока ПоследняяЗапись >= 0 Цикл
		...
		Если ТекущаяСтрока.ЦенаСтарая = ТекущаяСтрока.Цена Тогда
			...
			Если ТаблицаТоварыБезЦен = Неопределено Тогда
			...
			МассивНайденныхСтрок = ТаблицаТоварыБезЦен.НайтиСтроки(СтруктураПоиска);
