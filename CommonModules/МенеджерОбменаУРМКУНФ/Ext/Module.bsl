...
Процедура ВыполнитьПроцедуруМодуляМенеджера(ИмяПроцедуры, Параметры) Экспорт
	...
	Если ИмяПроцедуры = "ПОД_Справочник_ШтрихкодыУпаковокТоваров_Отправка_ПриОбработке" Тогда 
...
Процедура РегистрацияИзмененийНачальнойВыгрузкиДанных(Знач Получатель, СтандартнаяОбработка = Ложь, Отбор = Неопределено) Экспорт
	...
	ЗначенияРеквизитов 		= ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Получатель,
								"ВариантНастройки, ИспользоватьОтборПоОрганизациям, ДатаНачалаВыгрузкиДокументов, Организации, ВыгружатьФизическихЛиц, ВыгружатьФизическихЛицЧастично, СписокФизическихЛицДляВыгрузки");
	...
	ЗарегистрироватьДанныеПоДатеНачалаВыгрузкиИОрганизациям(Получатель,
		ЗначенияРеквизитов.ДатаНачалаВыгрузкиДокументов, ОтборОрганизации, ОтборФизическиеЛица, Отбор);
...
Процедура ПОД_Документ_ПриходнаяНакладная_Отправка_ПриОбработке(ДанныеИБ, ИспользованиеПКО, КомпонентыОбмена)
	...
	Для каждого Строка Из ДанныеИБ.ШтрихкодыУпаковок Цикл

		Если ЗначениеЗаполнено(Строка.ШтрихкодУпаковки) Тогда
			МаркаОбъект = Строка.ШтрихкодУпаковки.ПолучитьОбъект();
...
Процедура ОтложеннаяОбработка_ЧекККМ(Объект, КомпонентыОбмена)
	...
	Если АкцизныеМарки.Количество() Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КегиНаОборудованииРозливаИСМП КАК КегиНаОборудованииРозливаИСМП
		|		ПО (ШтрихкодыУпаковокТоваров.Ссылка = КегиНаОборудованииРозливаИСМП.КодМаркировки)
		|ГДЕ
		|	СправочникНоменклатура.ВидПродукцииИС = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.Пиво)
		|	И КегиНаОборудованииРозливаИСМП.КодМаркировки ЕСТЬ NULL
...
Процедура ПКО_Справочник_Номенклатура_Отправка_ПриОтправкеДанных(ДанныеИБ, ДанныеXDTO, КомпонентыОбмена, СтекВыгрузки)
	...
	Иначе
		...
		Если СоответствиеПКПД[ДанныеИБ.ВидПродукцииИС] <> Неопределено			
			...
			ДанныеXDTO.Вставить("ВидПродукцииИС", ДанныеИБ.ВидПродукцииИС);
		Иначе
			Инструкция = Новый Структура;
			Инструкция.Вставить("ИмяПКО",   "Перечисление_ВидыПродукцииИС");
			Инструкция.Вставить("Значение", Перечисления.ВидыПродукцииИС.ПустаяСсылка());
...
Функция ПолучитьСтрокиТЧБезналичнаяОплата(ДанныеXDTO, КомпонентыОбмена, ДляВозвратТовара = Ложь)
	...
	Если ДанныеXDTO.Свойство("ОплатаПлатежнымиКартамиПодарочныхСертификатов")
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ОплатаПлатежнымиКартами, ДанныеXDTO.ОплатаПлатежнымиКартамиПодарочныхСертификатов);
	КонецЕсли;
