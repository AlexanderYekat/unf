...
Процедура ОпределитьВидОплатыРазрешенныйВид(РазрешенныйВид, ВидОплаты) Экспорт
	...
	Если ВидОплаты = "Наличные"
		ИЛИ ВидОплаты = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.ВидыБезналичныхОплат.БанковскаяКарта") 
		ИЛИ ВидОплаты = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.ВидыБезналичныхОплат.ОплатаВРассрочку")
		ИЛИ ВидОплаты = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.ВидыБезналичныхОплат.Кредит") Тогда
...
Процедура ПодключитьКегКОборудованиюРозлива(Форма, ТекущаяСтрока, ОповещениеОЗавершении) Экспорт
	...
	ФормаПодключения = ОткрытьФорму(
		"ОбщаяФорма.ПодключенияКегаКОборудованиюРозливаИСМП",
		ПараметрыФормы,
		Форма,,,,
...
Процедура ОтключитьКегОтОборудованияРозлива(Форма, ОповещениеОЗавершении) Экспорт
	...
	Если ЗначениеЗаполнено(Форма.Объект.Организация) Тогда
		...
		СписокОрганизаций.Добавить(Форма.Объект.Организация);
		
		СтруктураБыстрогоОтбора = Новый Структура;
		СтруктураБыстрогоОтбора.Вставить("Организация", Форма.Объект.Организация);
		СтруктураБыстрогоОтбора.Вставить("Организации", СписокОрганизаций);
		ПараметрыФормы.Вставить("СтруктураБыстрогоОтбора", СтруктураБыстрогоОтбора);
	КонецЕсли;
	
	ОткрытьФорму(
		"ОбщаяФорма.СписокКегНаОборудованииРозливаИСМП",
		ПараметрыФормы,
		Форма,,,,
		ОповещениеОЗавершении);
...
Процедура ЗакрытьКассовуюСменуПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ДополнительныеПараметры.ПараметрыКассыККМ.СоздаватьВыемку Тогда
	...
	Иначе
		Оповещение = Новый ОписаниеОповещения("ЗакрытьКассовуюСменуЗавершение",
			ОбщегоНазначенияРМКУНФКлиент,
			ДополнительныеПараметры);
		ВыемкаДенег(ДополнительныеПараметры.ПараметрыКассыККМ, Оповещение, ДополнительныеПараметры.КассаККМ, ДополнительныеПараметры.УникальныйИдентификатор);
	КонецЕсли;
