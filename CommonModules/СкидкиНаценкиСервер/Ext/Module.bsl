...
Процедура РассчитатьСкидку(СтрокаДерева, Параметры)
	...
	Если ПараметрыСкидки.СпособПредоставления = Перечисления.СпособыПредоставленияСкидокНаценок.Процент Тогда
		...
		Для Каждого Товар Из Товары Цикл
			...
			Сумма = Товар.Сумма;
			Если ПараметрыСкидки.УменьшатьНаСуммуОплатыБаллами Тогда
...
Процедура РассчитатьСкидкиРекурсивно(ДеревоСкидок, Параметры, РассчетБонусов = Ложь)
	...
	Для Каждого СтрокаДерева Из ДеревоСкидок.Строки Цикл
		...
		Если СтрокаДерева.СпособПредоставления = Перечисления.СпособыПредоставленияСкидокНаценок.Округление Тогда
			...
			Продолжить;
		ИначеЕсли РассчетБонусов И НЕ СтрокаДерева.ЭтоПравилоНачисленияБонусов И НЕ СтрокаДерева.ЭтоГруппа Тогда
			// Считаем бонусы, пропустим все скидки
			Продолжить;
		ИначеЕсли НЕ РассчетБонусов И СтрокаДерева.ЭтоПравилоНачисленияБонусов И НЕ СтрокаДерева.ЭтоГруппа Тогда
...
Процедура РассчитатьОкругления(ДеревоСкидок, Параметры, ТаблицаДанных, РасчетБонусов = Ложь)
	...
	Для Каждого СтрокаДерева Из ДеревоСкидок.Строки Цикл
		...
		Если СтрокаДерева.СпособПредоставления <> Перечисления.СпособыПредоставленияСкидокНаценок.Округление Тогда
			Продолжить;
		ИначеЕсли Не (СтрокаДерева.ЭтоПравилоНачисленияБонусов = РасчетБонусов) Тогда
			Продолжить;
...
Функция СтруктураРассчитанныхСкидок(ДеревоСкидок, Параметры)
	...
	Если Параметры.РассчитыватьБонусы Тогда
		...
		ДеревоСкидокКопия = ОбщегоНазначения.СкопироватьРекурсивно(ДеревоСкидок);
		ДеревоКопияРезультатТолькоСкидки = ДеревоСкидок.Скопировать();
		УдалитьВетвиБонусовРекурсивно(ДеревоКопияРезультатТолькоСкидки);
		ПромежуточныеДанные = Новый Структура;
		ПромежуточныеДанные.Вставить("ДеревоСкидок", 		 ДеревоКопияРезультатТолькоСкидки);
		ПромежуточныеДанные.Вставить("ТаблицаСкидкиНаценки", ВозвращаемыеДанные.ТаблицаСкидкиНаценки.Скопировать());
		ПромежуточныеДанные.Вставить("ТаблицаСообщений", 	 ПустаяТаблицаСообщений());
		ПромежуточныеДанные.Вставить("ТаблицаПодарков", 	 ПустаяТаблицаПодарков());
		ПромежуточныеПараметры = ОбщегоНазначения.СкопироватьРекурсивно(Параметры);
			
		ИмяТЧЗапасы = "Запасы";
		Если ТипЗнч(ПромежуточныеПараметры.Регистратор) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
			ИмяТЧЗапасы = "РаботыИУслуги";
		ИначеЕсли ТипЗнч(ПромежуточныеПараметры.Регистратор) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
			Если Ложь Тогда
				ИмяТЧЗапасы = "Работы";
			КонецЕсли;
		КонецЕсли;
		ВиртуальныйОбъект = ПолучитьВиртуальныйОбъектДляРасчетаСкидок(ПромежуточныеДанные, ПромежуточныеПараметры, ИмяТЧЗапасы);
		// Если бонусы считаются до всех скидок, то не будем применять скидки к виртуальному объекту

		Если НЕ (ПромежуточныеПараметры.Свойство("НачислятьБонусыДоСкидок") И ПромежуточныеПараметры.НачислятьБонусыДоСкидок) Тогда
			ПрименитьСкидкиКВиртуальномуОбъекту(ВиртуальныйОбъект, ПромежуточныеДанные, ИмяТЧЗапасы);
		КонецЕсли;

		ПрименитьТаблицуТоваровКДеревуСкидокРекурсивно(ДеревоСкидокКопия, ВиртуальныйОбъект[ИмяТЧЗапасы]);
		ПромежуточныеПараметры.Товары = ВиртуальныйОбъект[ИмяТЧЗапасы];

		РассчитатьСкидкиРекурсивно(ДеревоСкидокКопия, ПромежуточныеПараметры, Истина);
		ТаблицаДанныхРасчетаБонусов = Неопределено;
		РассчитатьСкидкиПоГруппеСовместногоПрименения(ДеревоСкидокКопия, ПромежуточныеПараметры, Истина, ТаблицаДанныхРасчетаБонусов, Истина);
		
		РассчитатьОкругления(ДеревоСкидокКопия, ПромежуточныеПараметры, ТаблицаДанныхРасчетаБонусов, Истина);
		...
		Для Каждого СтрокаРассчитанногоБонуса Из ТаблицаДанныхРасчетаБонусов Цикл
			Для Каждого СтрокаНачисления Из СтрокаРассчитанногоБонуса.Расшифровка Цикл
				...
				Если Не БонуснаяПрограмма.ОтсрочкаНачисления ИЛИ БонуснаяПрограмма.КоличествоПериодовОтсрочки = 0 Тогда
					НоваяСтрокаТЗ.ДатаНачисления = ПромежуточныеПараметры.ТекущаяДата;
				Иначе
					НоваяСтрокаТЗ.ДатаНачисления = РаботаСБонусами.ПрибавитьКДатеПериод(
																	ПромежуточныеПараметры.ТекущаяДата,
...
Процедура РассчитатьОкруглениеБонусов(СтрокаДерева, Параметры, ТаблицаНачисленныхБонусов)
	...
	Если ПараметрыСкидки.ОбластьПредоставления = Перечисления.ВариантыОбластейОграниченияСкидокНаценок.ВДокументе Тогда
		...
		Если СуммаСкидкиКРаспределению<>0 Тогда
			...
			Для Каждого Начисление Из ТаблицаНачисленныхБонусов Цикл
				...
				НоваяСтрока.Сумма = Окр(ПроцентСкидкиНаценкиОкругления * Начисление.Сумма, 2);
				СуммаСкидкиКРаспределению = СуммаСкидкиКРаспределению - НоваяСтрока.Сумма;
	...
	ИначеЕсли ПараметрыСкидки.ОбластьПредоставления = Перечисления.ВариантыОбластейОграниченияСкидокНаценок.ВСтроке Тогда
		...
		Для Каждого Начисление Из ТаблицаНачисленныхБонусов Цикл
			...
			СуммаСкидки = Окр(ЗначениеСкидкиНаценки, 2, РежимОкругления.Окр15как10);
...
Функция ПодготовитьПараметрыРасчетаСкидок(ПакетЗапросов, ПараметрыРасчета, ВходныеПараметры)	
	...
	ТаблицаЦен = ТаблицаЦенТоваров(ТЗТовары, Параметры.СкидкиНаценкиПоВидуЦен, ПараметрыРасчета.ТекущаяДата, ПараметрыРасчета.ВалютаДокумента);
	...
	Если ЦенообразованиеСерверПовтИсп.МинимальныеЦеныИспользуются()
		...
		Для Каждого СтрокаТовары Из ТЗТовары Цикл 			
			...
			Если НайденаСтрока <> Неопределено И ЗначениеЗаполнено(НайденаСтрока.ВидМинимальныхЦен) Тогда
...
Функция ПеренестиПодаркиВЗапасы(Объект, ПримененныеСкидкиНаценки = Неопределено, АдресПримененныхСкидокВоВременномХранилище = "", ИмяТЧ = "Запасы") Экспорт
	...
	Для Каждого СтрокаПодарка Из ТаблицаСтрокПереносимыхПодарков Цикл
		...
		//СтрокиТоваров.Добавить(СтрокаПеренесенногоПодарка);
			...
			Если ИскатьВТЧПодарков И КоличествоПодарковВыдать > 0 Тогда
				...
				Для Каждого СтрокаЗачитываемогоПодарка Из ЗачитываемыеПодарки Цикл				
					Если КоличествоПодарковВыдать <= 0 Тогда
						Прервать;
					КонецЕсли;
					КоличествоУпаковокПодарковВыдано = Мин(СтрокаЗачитываемогоПодарка.Количество, КоличествоПодарковВыдать);
					ЗачитываемоеКоличествоУпаковок = Макс(КоличествоПодарковВыдать - КоличествоУпаковокПодарковВыдано, 0);
					ЗачитываемоеКоличество = ЗачитываемоеКоличествоУпаковок * КоэффициентУпаковки;
					ЗачитываемаяСумма = 0;
					Если НЕ ЗачитываемоеКоличествоУпаковок = 0 Тогда
						ЗачитываемаяСумма = Окр(ПодарокКВыдаче.Сумма * (КоличествоУпаковокПодарковВыдано / ЗачитываемоеКоличествоУпаковок), 2);
					КонецЕсли;					
					...
					КоличествоПодарковВыдать = КоличествоПодарковВыдать - КоличествоУпаковокПодарковВыдано;
				КонецЦикла;
			КонецЕсли;
...
Процедура УдалитьУсловияЗаВидОплаты(ТаблицаУсловий, ВыбранныеВидыОплаты = Неопределено)
	...
	КонецЦикла;
	//УсловияПрочиеВидыОплаты.Выгрузить();
	//МассивУсловияУдалить = Новый Массив;
	//Для Каждого Условие Из УсловияПрочиеВидыОплаты Цикл
	//	ОтборНеСработавшихУсловий = Новый Структура("Ссылка", Условие);
	//	Для Каждого СтрокаУсловия Из ТаблицаУсловий.НайтиСтроки(ОтборНеСработавшихУсловий) Цикл
	//		МассивУсловияУдалить.Добавить(СтрокаУсловия);
	//	КонецЦикла;
	//КонецЦикла;
	//Для Каждого СтрокаУдалить Из МассивУсловияУдалить Цикл 
	//	ТаблицаУсловий.Удалить(СтрокаУдалить);
	//КонецЦикла;
КонецПроцедуры 
...
Функция ТаблицаЦенТоваров(ТаблицаТоваров, ТаблицаСкидокПоВидуЦен, ДатаДокумента, ВалютаДокумента)
	...
	|				(ВЫБРАТЬ
	|					ФильрЦен.ВидЦен,
	|					ФильрЦен.Номенклатура,
	|					ФильрЦен.Характеристика
	|				ИЗ
	|					втФильрЦен КАК ФильрЦен)) КАК ЦеныНоменклатурыСрезПоследних
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЕдиницыИзмерения КАК ЕдиницаИзмеренияЦены
	|		ПО ЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения = ЕдиницаИзмеренияЦены.Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|	втТовары.Характеристика КАК Характеристика,
	|	втТовары.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ЕСТЬNULL(ПараметрыСертификата.ЕдиницаИзмерения, ЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияСкидки,
	|	ЕСТЬNULL(ПараметрыСертификата.Номинал, ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0)) КАК ЦенаСкидки,
	|	ЕСТЬNULL(ЕдиницаИзмеренияТовара.Коэффициент, 1) / ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.КоэффициентЕдиницыИзмерения, 1) КАК КоэффициентПриведенияКЕдиницеЦеныСкидки,
	|	втТовары.ЦенаВДокументе КАК Цена,
	|	втТовары.Количество КАК Количество,
	|	ВЫБОР
