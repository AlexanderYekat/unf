...
//
Функция НайтиДисконтныеКартыПоШтрихкоду(Штрихкод, BASE64 = Ложь) Экспорт
	
	Если BASE64 Тогда
		Штрихкод = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.Base64ВШтрихкод(Штрихкод);
	КонецЕсли;
	
	Возврат ДисконтныеКартыУНФСервер.НайтиДисконтныеКарты(Штрихкод, Перечисления.ТипыКодовКарт.Штрихкод);
...
Функция ПолучитьПроцентСкидкиПоВидуСкидокНаценок(ВидСкидкиНаценки) Экспорт
...
Функция ВычислитьПроцентСкидкиПоДисконтнойКарте(Знач ДатаСкидки, ДисконтнаяКарта, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Возврат ДисконтныеКартыУНФСервер.ВычислитьПроцентСкидкиПоДисконтнойКарте(ДатаСкидки, ДисконтнаяКарта, ДополнительныеПараметры);
...
Функция НайтиДисконтныеКартыПоДаннымСоСчитывателяМагнитныхКарт(Данные, ТипКода) Экспорт
	...
	Если ТипЗнч(Данные) = Тип("Массив") Тогда
			...
			Для Каждого Структура Из РасшифрованныеДанные Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					Если ЗначениеЗаполнено(Выборка.Ссылка) Тогда
						НоваяСтрока = ДисконтныеКартыУНФСервер.ПолучитьСтруктуруДанныхДисконтнойКарты();
		...
		Иначе
			...
			ВозвращаемоеЗначение = ДисконтныеКартыУНФСервер.НайтиДисконтныеКартыПоМагнитномуКоду(КодКарты);
		КонецЕсли;
	Иначе
		КодКарты = Данные;
		ПодготовитьКодКартыПоНастройкамПоУмолчанию(КодКарты);
...
Процедура ПодготовитьКодКартыПоНастройкамПоУмолчанию(КодКарты) Экспорт
	...
	// Данные на второй дорожке содержат номер, который заключен между префиксом и суффиксом. 
...
//         - Поле
//         - ЗначениеПоля
Функция РасшифроватьКодМагнитнойКарты(ДанныеДорожек)
