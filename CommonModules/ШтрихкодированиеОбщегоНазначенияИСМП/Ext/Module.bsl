...
Процедура ПроверитьНаОшибкиПоПравилам(ДанныеПоШтрихкодам, ПравилаПроверкиНаОшибки, ПараметрыСканирования, ЕстьОшибки)
	...
	Если ПравилаПроверкиНаОшибки.Свойство("КонтрольЗавершенностиАгрегации")
		И ПравилаПроверкиНаОшибки.КонтрольЗавершенностиАгрегации.Количество() > 0 Тогда

		Для Каждого СтрокаДанных Из ПравилаПроверкиНаОшибки.КонтрольЗавершенностиАгрегации Цикл

			Если ЗначениеЗаполнено(СтрокаДанных.ТекстОшибки) Тогда
				Продолжить;
			КонецЕсли;

			СтрокаДанных.ТекстОшибки = НСтр(
				"ru = 'Агрегацию логистических и групповых упаковок, упаковок наборов
				|необходимо выполнять в форме проверки и подбора.'");

		КонецЦикла;

		ПравилаПроверкиНаОшибки.КонтрольЗавершенностиАгрегации.Очистить();
...
Процедура ЗаполнитьДанныеИдентификатораРозничнойПродажиПоОбязательнымТоварнымГруппам(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаДанныхПоШтрихкодам Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ПараметрыКодаМаркировки <> Неопределено Тогда
			...
			Для Каждого КлючИЗначение Из ПараметрыКодаМаркировки Цикл
...
Процедура ЗаполнитьРезультатОбработкиШтрихкодаЧастичноеВыбытие(ДанныеШтрихкода, РезультатОбработки, ПараметрыСканирования) Экспорт
	...
	Если Не ПараметрыСканирования.ЭтоПроверкаКодовМаркировкиИСМП
		...
		И ЗначениеЗаполнено(ДанныеШтрихкода.ЧастичноеВыбытиеВариантУчета) Тогда
		
		Если ДанныеШтрихкода.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИС.ТекущаяНоменклатура
...
Процедура ПроверитьНаОшибкиТаблицуДанныеКодовМаркировки(ДанныеПоШтрихкодам, ПравилаПроверкиНаОшибки, ПараметрыСканирования, ЕстьОшибки)
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтрольЗавершенностиАгрегации") Тогда
			Если СтрокаДанных.СтрокаДерева = Неопределено
				...
				ЕстьОшибки = Истина;
				ПравилаПроверкиНаОшибки.КонтрольЗавершенностиАгрегации.Добавить(СтрокаДанных);
			КонецЕсли;
		КонецЕсли;

		Если ДанныеПоШтрихкодам.ЭтоСканированиеВФормеПроверкиИПодбора И Не ПараметрыСканирования.РежимБРМК Тогда
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтрольСтатусов")
			...
			Если ОтсутствуетДопустимыйСтатус
				И Не ЭтоПродажаВыбывшегоЧастично(
					СтрокаДанных,
...
Функция ПравилаПроверкиНаОшибки(ПараметрыСканирования, ДанныеПоШтрихкодам)
	...
	Если ПараметрыСканирования.ДопустимыеСтатусыИСМП.Количество() > 0
		И ОбщегоНазначенияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования, ДанныеПоШтрихкодам.ОтключитьЗапросВСервисПоОперацииИПродукции)
	...
	НеобходимаПроверкаПолногоКодаМаркировки = Ложь;
	...
	Для Каждого ДопустимыйВидПродукции Из ПараметрыСканирования.ДопустимыеВидыПродукции Цикл
		...
		Если ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(ДопустимыйВидПродукции,
			...
			НеобходимаПроверкаПолногоКодаМаркировки = Истина;
			Прервать;
	...
	Если ПараметрыСканирования.ТребоватьПолныйКодМаркировкиИСМП
		...
		Или НеобходимаПроверкаПолногоКодаМаркировки Тогда
	...
	Если ПараметрыСканирования.ВидОперацииИСМП = Перечисления.ВидыОперацийИСМП.ПодключениеКОборудованиюРозлива Тогда
	...
	Если ПараметрыСканирования.ЭтоМаркировкаТоваровИСМП
		...
		ПравилаПроверкиНаОшибки.Вставить("КонтрольЗавершенностиАгрегации", Новый Массив);
	КонецЕсли;
