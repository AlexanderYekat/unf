...
Функция ФайлыМЧДДляСохранениеНаДиск(СписокМЧД) Экспорт
	...
	РеквизитыДоверенностей = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(
		СписокМЧД, "Наименование,ИмяФайлаДоверенности,ПодписанЭП,Доверенность,Дата");
	Для Каждого МЧД Из СписокМЧД Цикл
		
		Реквизиты = РеквизитыДоверенностей.Получить(МЧД);
		ИмяФайла = Справочники.МашиночитаемыеДоверенностиКЭДО.ИмяВыгружаемогоФайла(
			Реквизиты.ИмяФайлаДоверенности, Реквизиты.Дата, МЧД);
		...
		ВывестиВФайлАрхиваМЧД(ФайлАрхива, МЧД, ИмяВременногоКаталога, Реквизиты);
...
Функция ДанныеСертификатовОрганизаций(Организации) Экспорт
	...
	ДанныеСертификатов.Вставить("ОрганизацииКОбновлению", Новый Соответствие);
	...
	Если ЗначениеЗаполнено(СертификатыОрганизаций) Тогда
		
		ДанныеСертификатов.Сертификаты = ОбщегоНазначения.СкопироватьРекурсивно(СертификатыОрганизаций);
		Для Каждого СертификатыОрганизации Из СертификатыОрганизаций Цикл
			УдалитьСертификат = Ложь;
			Если ТипЗнч(СертификатыОрганизации.Значение) <> Тип("Соответствие") Тогда
				УдалитьСертификат = Истина;
			Иначе
				Для Каждого ДанныеСертификата Из СертификатыОрганизации.Значение Цикл
					Если ТипЗнч(ДанныеСертификата.Значение) <> Тип("Массив") Тогда
						УдалитьСертификат = Истина;
					Иначе
						Для Каждого СвойстваСертификата Из ДанныеСертификата.Значение Цикл
							Если ТипЗнч(СвойстваСертификата) <> Тип("Структура")
								Или Не СвойстваСертификата.Свойство("РольПодписанта")
								Или Не ЗначениеЗаполнено(СвойстваСертификата.РольПодписанта) Тогда
						...
						КонецЦикла;
					КонецЕсли;
					Если УдалитьСертификат Тогда
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			Если УдалитьСертификат Тогда
				ДанныеСертификатов.Сертификаты.Удалить(СертификатыОрганизации.Ключ);
			КонецЕсли;
		КонецЦикла;
		
		Если Не ОбщегоНазначения.КоллекцииИдентичны(ДанныеСертификатов.Сертификаты, СертификатыОрганизаций) Тогда
			СертификатыОрганизаций = ОбщегоНазначения.СкопироватьРекурсивно(ДанныеСертификатов.Сертификаты);
		КонецЕсли;
		
		ВсеСертификаты = Новый Массив;
		Для Каждого СертификатыОрганизации Из СертификатыОрганизаций Цикл
			Если ЗначениеЗаполнено(СертификатыОрганизации.Значение) Тогда
				Для Каждого ДанныеСертификата Из СертификатыОрганизации.Значение Цикл
					Если ТипЗнч(ДанныеСертификата.Ключ) = Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования") Тогда
						ВсеСертификаты.Добавить(ДанныеСертификата.Ключ);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
		Если ЗначениеЗаполнено(ВсеСертификаты) Тогда
			ДействующиеСертификаты = ДействующиеСертификатыПодписей(ВсеСертификаты);
			Для Каждого СертификатыОрганизации Из СертификатыОрганизаций Цикл
				Для Каждого СертификатОрганизации Из СертификатыОрганизации.Значение Цикл
					Если ДействующиеСертификаты.Найти(СертификатОрганизации.Ключ) = Неопределено Тогда
						СертификатыОрганизаций.Удалить(СертификатыОрганизации.Ключ);
						ДанныеСертификатов.ОрганизацииКОбновлению.Вставить(СертификатыОрганизации.Ключ, Истина);
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
		КонецЕсли;
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
			...
			Если ЗначениеЗаполнено(ДанныеСертификатов.Сертификаты.Получить(Выборка.Организация))
				И ДанныеСертификатов.ОрганизацииКОбновлению.Получить(Выборка.Организация) = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Выборка.Сертификат) Тогда
				...
				КонецЕсли;
				Если ТипЗнч(СертификатыПоУмолчанию) = Тип("Соответствие") Тогда
					СертификатыПоУмолчанию = ОбщегоНазначения.ВыгрузитьКолонку(СертификатыПоУмолчанию, "Ключ");
				КонецЕсли;
				...
				Если СертификатыПоУмолчанию.Количество() > 0
					...
					ДанныеСертификатов.ОрганизацииКОбновлению.Вставить(Выборка.Организация, Истина);
...
Функция ЗапросСертификатыОрганизацииПользователя(Организации = Неопределено)
	...
	Запрос.Текст =
		...
		|	И СертификатыПодписей.Программа <> ЗНАЧЕНИЕ(Справочник.ПрограммыЭлектроннойПодписиИШифрования.ПустаяСсылка)
		...
		|	И СертификатыПодписей.Пользователь = &Пользователь
		|	И СертификатыПодписей.Ссылка.Программа <> ЗНАЧЕНИЕ(Справочник.ПрограммыЭлектроннойПодписиИШифрования.ПустаяСсылка)
...
Процедура ВывестиВФайлАрхиваМЧД(ФайлАрхива, МЧД, ИмяВременногоКаталога, РеквизитыДоверенности = Неопределено)
	
	Если РеквизитыДоверенности = Неопределено Тогда
		РеквизитыДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			МЧД, "Наименование,ИмяФайлаДоверенности,ПодписанЭП,Доверенность");
	КонецЕсли;
	
	ИмяФайла = Справочники.МашиночитаемыеДоверенностиКЭДО.ИмяВыгружаемогоФайла(
		РеквизитыДоверенности.ИмяФайлаДоверенности, РеквизитыДоверенности.Дата, МЧД);
	
	ИмяФайла = ИмяФайлаКЭДО(ИмяФайла);
	
	ПолноеИмяФайла = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(
		ИмяВременногоКаталога)+ ИмяФайла;
	ДвоичныеДанныеФайла = РеквизитыДоверенности.Доверенность.Получить();
	ДвоичныеДанныеФайла.Записать(ПолноеИмяФайла);
	
	ФайлАрхива.Добавить(ПолноеИмяФайла);
	
	Если РеквизитыДоверенности.ПодписанЭП Тогда
...
Функция ДействующиеСертификатыПодписей(Сертификаты)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	СертификатыПодписей.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования КАК СертификатыПодписей
		|ГДЕ
		|	СертификатыПодписей.Подписание
		|	И НЕ СертификатыПодписей.Отозван
		|	И СертификатыПодписей.ДействителенДо >= &ДействителенДо
		|	И СертификатыПодписей.Программа <> ЗНАЧЕНИЕ(Справочник.ПрограммыЭлектроннойПодписиИШифрования.ПустаяСсылка)
		|	И НЕ СертификатыПодписей.ПометкаУдаления
		|	И СертификатыПодписей.Ссылка В(&Сертификаты)
		|	И СертификатыПодписей.Пользователь = &Пользователь
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	СертификатыПодписей.Ссылка
		|ИЗ
		|	Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Пользователи КАК СертификатыПодписей
		|ГДЕ
		|	СертификатыПодписей.Ссылка.Подписание
		|	И НЕ СертификатыПодписей.Ссылка.Отозван
		|	И СертификатыПодписей.Ссылка.ДействителенДо >= &ДействителенДо
		|	И СертификатыПодписей.Ссылка.Программа <> ЗНАЧЕНИЕ(Справочник.ПрограммыЭлектроннойПодписиИШифрования.ПустаяСсылка)
		|	И НЕ СертификатыПодписей.Ссылка.ПометкаУдаления
		|	И СертификатыПодписей.Ссылка В(&Сертификаты)
		|	И СертификатыПодписей.Пользователь = &Пользователь";
	...
	УстановитьПривилегированныйРежим(Истина);
	СертификатыПодписей = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат СертификатыПодписей;
...
Процедура ПослеПодписанияФайловСертификатом(Знач Результат, Знач ДанныеПодписейКЭДО) Экспорт
	...
	Для Каждого ДанныеПодписи Из ДанныеПодписей Цикл
		ЭлектроннаяПодписьКЭДО.ЗарегистрироватьНаборСвойствПодписи(
			ДанныеПодписи.Объект, ДанныеПодписи.СвойстваПодписи, ДанныеПодписейКЭДО);
	КонецЦикла;
