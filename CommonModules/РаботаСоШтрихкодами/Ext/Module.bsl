...
Функция ДанныеПоискаПоШтрихкоду(Штрихкод, Форма) Экспорт
	...
	Если РеквизитыФормыРМК.ИмяФормы = "Обработка.РабочееМестоКассира.Форма.ФормаРМК" И ТипЗнч(РеквизитыФормыРМК) = Тип("Структура") Тогда
		НесколькоЗначенийПоиска = СтруктураПараметров.ЗначенияПоиска.Количество() > 1;
		...
		Для Каждого СтрокаЗначения Из СтруктураПараметров.ЗначенияПоиска Цикл
			...
			Если СтрокаЗначения.Свойство("Номенклатура") И ЗначениеЗаполнено(СтрокаЗначения.Номенклатура) Тогда
				...
				Если НЕ НайденнаяНоменклатура = Неопределено Тогда
					...
					СтрокаТовара = ПродажиРМК.СтрокаТовара(НайденнаяНоменклатура);
