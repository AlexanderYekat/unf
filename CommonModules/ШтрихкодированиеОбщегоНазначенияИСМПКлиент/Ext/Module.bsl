...
Процедура НачатьПроверкуКодовМаркировкиСредствамиККТ(ПараметрыНачалаПроверки) Экспорт
	...
	Для Каждого ЭлементПроверки Из ПараметрыПроверки.ЭлементыПроверки Цикл
		...
		РезультатПроверки.ТребуетсяРазрешительныйЗапросГИСМТ = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(ЭлементПроверки.ВидПродукции,
			ПараметрыПроверки.ПараметрыСканирования.ВидОперацииИСМП,
...
Функция ФормаБлокировкиПоПараметрамПроверки(ПараметрыПроверки) Экспорт
	...
	Если ПараметрыПроверки.ФормаВспомогательная = Неопределено
		Или ПараметрыПроверки.РежимБРМК Тогда
...
Процедура ПроверкаИдентификатораГИСМТПоКодуМаркировки(ПараметрыПроверки)
	...
	Для Каждого ЭлементПроверки Из ЭлементыПроверки Цикл
		...
		Если Не ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(ЭлементПроверки.ВидПродукции,
...
Процедура ПолучениеИдентификатораГИСМТЗавершение(Результат, ПараметрыПроверки) Экспорт
	...
	Для Каждого КлючИЗначение Из ДанныеОтвета.ДанныеИдентификаторов Цикл
		...
		ДанныеОтвета    = КлючИЗначение.Значение;
		
		Если ДанныеОтвета.ТребуетсяОбновлениеКлючаСессии Тогда
		...
		ЭлементПроверки.РазрешительныйРежимИдентификаторЗапросаГИСМТ = ДанныеОтвета.РазрешительныйРежимИдентификаторЗапросаГИСМТ;
		ЭлементПроверки.РазрешительныйРежимДатаЗапросаГИСМТ          = ДанныеОтвета.РазрешительныйРежимДатаЗапросаГИСМТ;
		ЭлементПроверки.РазрешительныйРежимАдресСервера              = ДанныеОтвета.РазрешительныйРежимАдресСервера;
		ЭлементПроверки.РазрешительныйРежимТелоЗапросаJSON           = ДанныеОтвета.РазрешительныйРежимТелоЗапросаJSON;
		ЭлементПроверки.РазрешительныйРежимТелоОтветаJSON            = ДанныеОтвета.РазрешительныйРежимТелоОтветаJSON;
		ЭлементПроверки.РазрешительныйРежимКодОтвета                 = ДанныеОтвета.РазрешительныйРежимКодОтвета;
		...
		Если ЗначениеЗаполнено(ЭлементПроверки.РазрешительныйРежимДатаЗапросаГИСМТ) Тогда
			...
			Если ДанныеОтвета.ЧастичноеВыбытиеОстаток <> Неопределено И
				ЭлементПроверки.Количество > ДанныеОтвета.ЧастичноеВыбытиеОстаток Тогда
				
				ТекстОшибкиОстатокВКеге = СтрШаблон(
					НСтр("ru = 'Остаток в кеге по данным ГИС МТ: %1 л., требуется: %2 л.'"),
					ДанныеОтвета.ЧастичноеВыбытиеОстаток,
		...
		ИначеЕсли Не ДанныеОтвета.АварийныйРежим Тогда
			
			РезультатЭлементаПроверки.ТекстОшибкиГИСМТ   = ДанныеОтвета.ОписаниеОшибки;
			ПараметрыПроверки.Результат.ТекстОшибкиГИСМТ = ДанныеОтвета.ОписаниеОшибки;
			ПараметрыПроверки.Результат.ЕстьОшибки       = Истина;
