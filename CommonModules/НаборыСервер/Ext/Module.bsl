...
Процедура ДобавитьИнформациюОНабореПриПолученииДанныхНоменклатуры(СтруктураДанные) Экспорт
	...
	Если СтруктураДанные.ЭтоНабор Тогда
		...
		|	КурсВалютыВидЦен.Валюта
		|ПОМЕСТИТЬ КурсВалютыВидЦен
		|ИЗ
		...
		|	ВЫБОР
		|		КОГДА ISNULL(ЦеныНоменклатурыСрезПоследних.ВалютаЦены,
		|			ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
		...
		|ИЗ
		|	РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ДатаОбработки, ВидЦен = &ВидЦен
		...
		|			ИЛИ
		|				СоставНаборов.СпособРасчетаЦеныНабора = ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаЦеныНабора.НазначаетсяИРаспределяетсяПоЦене))
		|
		|		ОБЪЕДИНИТЬ ВСЕ
		|
		|		ВЫБРАТЬ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КурсВалютыМинимальныйВидЦен.Курс,
		|	КурсВалютыМинимальныйВидЦен.Кратность,
		|	КурсВалютыМинимальныйВидЦен.Валюта
		|ПОМЕСТИТЬ КурсВалютыМинимальныйВидЦен
		|ИЗ
		|	РегистрСведений.КурсыВалют.СрезПоследних(&ДатаОбработки,) КАК КурсВалютыМинимальныйВидЦен
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Валюта
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МинимальныеЦеныНоменклатурыСрезПоследних.Номенклатура КАК Номенклатура,
		|	МинимальныеЦеныНоменклатурыСрезПоследних.Характеристика КАК Характеристика,
		|	МинимальныеЦеныНоменклатурыСрезПоследних.Цена КАК Цена,
		|	МинимальныеЦеныНоменклатурыСрезПоследних.ВидЦен КАК ВидЦен,
		|	МинимальныеЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	ВЫБОР
		|		КОГДА ISNULL(МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены,
		|			ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
		...
		|	КОНЕЦ КАК ВалютаЦены
		|ПОМЕСТИТЬ МинимальныеЦеныНоменклатурыСрезПоследних
		|ИЗ
		|	РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ДатаОбработки, ВидЦен = &ВидМинимальныхЦен
		|	И (Номенклатура, Характеристика) В
		|		(ВЫБРАТЬ
		|			СоставНаборов.Номенклатура,
		|			СоставНаборов.Характеристика
		...
		|	ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.ВидЦен.ПорядокОкругления,
		|		ЗНАЧЕНИЕ(Перечисление.ПорядкиОкругления.Окр0_01)) КАК ПорядокОкругления,
		...
		|	ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена * КурсВалютыВидЦен.Курс * КурсВалютыДокумента.Кратность /
		|		(КурсВалютыДокумента.Курс * КурсВалютыВидЦен.Кратность) * СоставНаборов.Коэффициент / ВЫБОР
		...
		|		ИНАЧЕ 1
		|	КОНЕЦ, 0) КАК Цена,
		|	ЕСТЬNULL(МинимальныеЦеныНоменклатурыСрезПоследних.Цена * КурсВалютыМинимальныйВидЦен.Курс *
		|		КурсВалютыДокумента.Кратность / (КурсВалютыДокумента.Курс * КурсВалютыМинимальныйВидЦен.Кратность) *
		|		СоставНаборов.Коэффициент / ВЫБОР
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ КурсВалютыВидЦен КАК КурсВалютыВидЦен
		|			ПО КурсВалютыВидЦен.Валюта = ЦеныНоменклатурыСрезПоследних.ВалютаЦены
		|		ПО ((СоставНаборов.ФиксированнаяСтоимость ИЛИ &ВидРабот = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ МинимальныеЦеныНоменклатурыСрезПоследних КАК МинимальныеЦеныНоменклатурыСрезПоследних
		|			ЛЕВОЕ СОЕДИНЕНИЕ КурсВалютыМинимальныйВидЦен КАК КурсВалютыМинимальныйВидЦен
		|			ПО КурсВалютыМинимальныйВидЦен.Валюта = МинимальныеЦеныНоменклатурыСрезПоследних.ВалютаЦены
		|		ПО ((СоставНаборов.ФиксированнаяСтоимость ИЛИ &ВидРабот = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
		...
		Пока Выборка.Следующий() Цикл
			...
			Если СтруктураДанные.Свойство("ВидОперации") И РаботаСКомиссионерамиКомитентамиСервер.ЭтоВозвратРасходнаяНакладная(СтруктураДанные.ВидОперации) Тогда
				НалогообложениеНДСОрганизации = Неопределено;
			ИначеЕсли СтруктураДанные.Свойство("Организация") Тогда
			...
			// Интеграция ГосИС
			ИнтеграцияИСМПУНФ.ЗаполнитьСлужебныеРеквизитыИСМП(СтруктураПолей);
			...
			Если Не СтруктураДанные.Свойство("ТребуетсяИзменениеНалогообложениеНаСНДС") И СтруктураДанные.Свойство("НалогообложениеНДС") Тогда
				СтруктураПолей.Вставить("ТребуетсяИзменениеНалогообложениеНаСНДС", Ложь);
				СтруктураПолей.Вставить("НалогообложениеНДС", СтруктураДанные.НалогообложениеНДС);
				РаботаСКомиссионерамиКомитентамиСервер.СкорректироватьСтавкуНДСКомиссионногоТовара(СтруктураПолей);
				СтруктураДанные.Вставить("ТребуетсяИзменениеНалогообложениеНаСНДС", СтруктураПолей.ТребуетсяИзменениеНалогообложениеНаСНДС);
			ИначеЕсли СтруктураДанные.Свойство("НалогообложениеНДС") И Не СтруктураДанные.ТребуетсяИзменениеНалогообложениеНаСНДС Тогда
				СтруктураПолей.Вставить("ТребуетсяИзменениеНалогообложениеНаСНДС", Ложь);
				СтруктураПолей.Вставить("НалогообложениеНДС", СтруктураДанные.НалогообложениеНДС);
				РаботаСКомиссионерамиКомитентамиСервер.СкорректироватьСтавкуНДСКомиссионногоТовара(СтруктураПолей);
				СтруктураДанные.ТребуетсяИзменениеНалогообложениеНаСНДС = СтруктураПолей.ТребуетсяИзменениеНалогообложениеНаСНДС;
			КонецЕсли;
...
Процедура ОбновитьСоставНабораВДокументе(СтруктураДанных, Объект) Экспорт
	...
	БуферЗначений.Колонки.Добавить("ЗаказПокупателя");
	БуферЗначений.Колонки.Добавить("Заказ");
...
Процедура ОбновитьЦеныСоставаНабора(СтруктураДанных, Объект) Экспорт
	...
	Для каждого СтрокаТЧ Из СтруктураДанных.СоставНабора Цикл
		...
		Если СтруктураДанныхЦены.Свойство("ВидЦен") Тогда
			ЦенообразованиеКлиентСервер.ДополнитьСтруктуруДанныхНоменклатурыПолямиЦенообразования(Объект, СтрокаТЧ, СтрокаТЧ);	
		КонецЕсли;
...
Процедура КомпоноватьТабличнуюЧастьПоНаборам(ДанныеОбъектовПечати, ИмяТабличнойЧасти, Ошибки = Неопределено, ПроверятьРазделители = Ложь) Экспорт
	...
	Для каждого ОбъектПечати Из ДанныеОбъектовПечати Цикл
		...
		Для каждого СтрокаНабора Из ДобавленныеНаборы Цикл
			...
			НомерИДатаОтгрузочногоДокументаСтруктура = Новый Структура;
			НомерИДатаОтгрузочногоДокументаСтруктура.Вставить("ДатаОтгрузочногоДокумента", '00010101');
			НомерИДатаОтгрузочногоДокументаСтруктура.Вставить("НомерОтгрузочногоДокумента", "");
			СтавкаНДСНабора = Неопределено;
			Для каждого СтрокаСоставаНабора Из СоставНабора Цикл
				
				ЗаполнитьЗначенияСвойств(НомерИДатаОтгрузочногоДокументаСтруктура, СтрокаСоставаНабора);
			...
			ДанныеСтроки.Вставить("Ссылка", ОбъектПечати.Ссылка);
			
			ЗаполнитьЗначенияСвойств(ДобавляемаяСтрока, ДанныеСтроки);
			ЗаполнитьЗначенияСвойств(ДобавляемаяСтрока, НомерИДатаОтгрузочногоДокументаСтруктура);
...
//                                  строками состава одного набора
//
Процедура КомпоноватьТабличнуюЧастьПоНаборамДокументыКорректировки(ДанныеОбъектовПечати, ИмяТабличнойЧасти, Ошибки = Неопределено, ПроверятьРазделители = Ложь) Экспорт
	
	ПоляСуммирования = "Сумма, СуммаБезСкидки, Скидка, Всего, СуммаНДС, СуммаБезНДС, СуммаСНДС, СуммаНДСВНациональнойВалюте, ВсегоВНациональнойВалюте, 
	|СуммаВВалютеДокумента, СуммаНДСВВалютеДокумента, ВсегоВВалютеДокумента, Вес, МассаБрутто, Объем, СуммаДоИзменения, СуммаНДСДоИзменения, ВсегоДоИзменения, 
	|СтоимостьБезНДСДоИзменения, СтоимостьБезНДСПослеИзменения, СтоимостьСНДСДоИзменения, СтоимостьСНДСПослеИзменения, РазницаБезНДСУвеличение, РазницаБезНДСУменьшение, 
	|РазницаНДСУвеличение, РазницаНДСУменьшение, РазницаСНДСУвеличение, РазницаСНДСУменьшение, СуммаНДСПослеИзменения";
	
	ПоляЗапас = "Запас, ПредставлениеНоменклатуры, ЗапасНаименование, ТоварНаименование, НоменклатураНаименование, НаименованиеНоменклатуры";
	ПоляАртикул = "Артикул, ТоварКод, НоменклатураАртикул, ПредставлениеКодаНоменклатуры";
	ПоляКод = "Код, КодПродукции, ЗапасКод, ТоварКодТНВЭД";
	ПоляЕдиницаИзмерения = "ЕдиницаИзмерения, ЕдиницаИзмеренияПоОКЕИ_Наименование, БазоваяЕдиницаНаименование, ЕдиницаХранения, НаименованиеЕдиницыИзмерения";
	ПоляКодЕдиницыИзмерения = "ЕдиницаИзмеренияКод, ЕдиницаИзмеренияПоОКЕИ_Код";
	ПоляКоличество = "КоличествоДоИзменения, КоличествоПослеИзменения";
	...
	Для каждого ОбъектПечати Из ДанныеОбъектовПечати Цикл
		...
		Для каждого СтрокаНабора Из ДобавленныеНаборы Цикл
			...
			ДанныеСтроки.Вставить("Товар", СтрокаНабора.НоменклатураНабора);
			...
			Если СтрокаНабора.ВыводитьИтоги Тогда
				...
				// Цена
				Если ДанныеСтроки.Свойство("Сумма") И ДанныеСтроки.Сумма>0 И ДанныеСтроки.Количество>0 Тогда
					ДанныеСтроки.Вставить("Цена", Окр(ДанныеСтроки.Сумма/ДанныеСтроки.Количество, 2));
				Иначе
					ДанныеСтроки.Вставить("Цена", 0);
				КонецЕсли;
				
				Если ДанныеСтроки.Свойство("СуммаДоИзменения") И ДанныеСтроки.СуммаДоИзменения>0 И ДанныеСтроки.КоличествоДоИзменения>0 Тогда
					ДанныеСтроки.Вставить("ЦенаДоИзменения", Окр(ДанныеСтроки.СуммаДоИзменения/ДанныеСтроки.КоличествоДоИзменения, 2));
				Иначе
					ДанныеСтроки.Вставить("ЦенаДоИзменения", 0);
				КонецЕсли;
...
// Устарела. Подготавливает наборы в таблице товаров к печати в чеке. Наборы с вариантом печати "Только набор" сворачиваются
//
// Параметры:
...
Процедура РазвернутьНаборы(Документ, ИмяТабличнойЧасти = "Запасы") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Для каждого СтрокаНабора Из СтрокиТЧ Цикл
			...
			ПараметрыНабора.Вставить("ДатаОбработки", ЦенообразованиеСервер.ПолучитьДатуОбработкиЦен(
				Не ЗначениеЗаполнено(Документ.Ссылка), Документ.Дата));
