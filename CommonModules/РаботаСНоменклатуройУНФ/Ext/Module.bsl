...
//  * КоличествоВУпаковке                                        - Число  - количество штук в 1 единице (упаковке)
//      товара. Например: 1 пачка фломастеров содержит 10 штук фломастеров.
...
//  * КоличествоВУпаковкеЕдиницаИзмеренияНаименованиеПолное      - Строка - полное наименование единицы измерения
//                                                                          количества штук.
//  * КоличествоВУпаковкеЕдиницаИзмеренияМеждународноеСокращение - Строка - международное сокращение единицы измерения
//                                                                          количества штук.
//  * КоличествоВУпаковкеЕдиницаИзмеренияБазоваяЕдиница          - Строка - код ОКЕИ базовой величины для единицы
//                                                                          измерения количества штук.
//  * КоличествоВУпаковкеЕдиницаИзмеренияЧислитель               - Число  - числитель отношения единицы измерения
//                                                                          количества штук к базовой величине.
//  * КоличествоВУпаковкеЕдиницаИзмеренияЗнаменатель             - Число  - знаменатель отношения единицы измерения
//                                                                          количества штук к базовой величине.
//  * ВесЧислитель                               - Число  - числитель отношения единицы измерения веса к единице
//                                                          измерения товара.
//  * ВесЗнаменатель                             - Число  - знаменатель отношения единицы измерения веса к единице
//                                                          измерения товара.
...
//  * ДлинаЧислитель                               - Число  - числитель отношения единицы измерения длины к единице
//                                                            измерения товара.
//  * ДлинаЗнаменатель                             - Число  - знаменатель отношения единицы измерения длины к единице
//                                                            измерения товара.
...
//  * ОбъемЧислитель                               - Число  - числитель отношения единицы измерения объема к единице
//                                                            измерения товара.
//  * ОбъемЗнаменатель                             - Число  - знаменатель отношения единицы измерения объема к единице
//                                                            измерения товара.
...
//  * ПлощадьЧислитель                               - Число  - числитель отношения единицы измерения площади к единице
//                                                              измерения товара.
//  * ПлощадьЗнаменатель                             - Число  - знаменатель отношения единицы измерения площади к
...
//  * ПлощадьЕдиницаИзмеренияЗнаменатель             - Число  - знаменатель отношения единицы измерения площади к
//                                                              базовой величине.
...
// Если в конфигурации нет возможности разделить номенклатуру на категории, следует инициализировать таблицу значениями
// NULL по всем полям. Минимально необходимо описать 2 уровня иерархии.
...
//  * ГлубинаЕдиницаИзмеренияЗнаменатель             - Число  - знаменатель отношения единицы измерения глубины к
//                                                              базовой величине.
//
// Классификаторы:
//  * Номенклатура   - ОпределяемыйТип.НоменклатураРаботаСНоменклатурой - ссылка на товар.
...
//
// При необходимости возможно добавление в пакет данных выгрузки произвольных дополнительных сведений общего характера,
// без привязки к товарной позиции. Для этого в тексте запроса можно описать любое нужное количество дополнительных
// запросов без создания временной таблицы. Важно, чтобы все эти запросы не использовали никакие вышеописанные временные
// таблицы, относящиеся к номенклатуре, так как выполняться эти запросы будут отдельно от запроса по номенклатуре,
// поскольку речь идет о сведениях, общих для всего пакета данных в целом. Если добавляемые в пакет дополнительные
// сведения имеют сложную структуру (не одномерный массив полей, а многомерный массив), то имя таблицы источника данных
// нужно сформировать по шаблону ДополнительныеДанные_ИмяТаблицы. В этом случае в пакет добавится дополнительное
// свойство ИмяТаблицы, значение которого будет заполнено данными запроса. Если таблица с префиксом
// ДополнительныеДанные_ в секции ИЗ не найдена, то все полученные запросом дополнительные сведения добавляются в пакет
// как одномерный массив пар значений <имя поля> - <значение поля>.
//
// Параметры:
...
#Область ПроцедурыПроверкиВыполненияОбработкиОбъектов
Функция ПризнакПредметаРасчетаЗаполнен(Параметры) Экспорт 
	Результат = Истина;
	ПризнакПредметаРасчета = Неопределено;
	НаименованиеППР = "ПризнакПредметаРасчета";
	НаименованиеТипаНоменклатуры = "ТипНоменклатуры";
	НаименованиеТипаНоменклатурыПоУмолчанию = "ТипНоменклатурыПоУмолчанию";
	Если ТипЗнч(Параметры.Данные) = Тип("СправочникСсылка.Номенклатура") Тогда
		ПереченьРеквизитов = "" + НаименованиеППР + ", " + НаименованиеТипаНоменклатуры;
		Данные = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.Данные, ПереченьРеквизитов);
	ИначеЕсли ТипЗнч(Параметры.Данные) = Тип("СправочникСсылка.КатегорииНоменклатуры") Тогда
		НаименованиеТипаНоменклатуры = НаименованиеТипаНоменклатурыПоУмолчанию;
		ПереченьРеквизитов = "" + НаименованиеППР + ", " + НаименованиеТипаНоменклатуры;
		Данные = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.Данные, ПереченьРеквизитов);
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Параметры.Данные, НаименованиеТипаНоменклатуры) Тогда
		Данные = Параметры.Данные;
	Иначе                     
		НаименованиеТипаНоменклатуры = НаименованиеТипаНоменклатурыПоУмолчанию;
		Данные = Параметры.Данные; 
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Параметры.Данные, НаименованиеТипаНоменклатуры)
		И Данные[НаименованиеТипаНоменклатуры] <> Перечисления.ТипыНоменклатуры.Операция
		И Данные[НаименованиеТипаНоменклатуры] <> Перечисления.ТипыНоменклатуры.ВидРабот 
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Параметры.Данные, НаименованиеППР) Тогда
		Результат = ЗначениеЗаполнено(Данные[НаименованиеППР]);
	КонецЕсли;
