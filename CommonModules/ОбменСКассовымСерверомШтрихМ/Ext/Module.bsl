...
Процедура ОбменДаннымиССерверомШтрихМ() Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Регламентное задание ""Обмен данными с сервером Штрих-М"".Потоковый обмен'", КодЯзыка);
	
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'Получены настройки.'", КодЯзыка));
	КонецЕсли;
	...
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'Завершение обмена.'", КодЯзыка));
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолучитьКассовыеДокументыДляКассовыхАппаратов(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Документы для кассовых аппаратов.'", КодЯзыка);
	...
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'HTTP Соединение установлено.'", КодЯзыка));
	КонецЕсли;
	...
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'Начало обмена.'", КодЯзыка));
	КонецЕсли;
...
Процедура ВыполнитьОчисткуТоваровНаКассовыхАппаратах(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Очистка всех товаров на кассовых аппаратах.'", КодЯзыка);
	...
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'Начало очистки.'", КодЯзыка));
	КонецЕсли;
		
	Настройки.Вставить("ТипМетода", "cashregister_clear_unsent_packets");
	ВыполнитьОчисткуСообщенийДляККТ(Соединение.HTTPСоединение, Настройки, ИнформацияПоКассовымАппаратам,
	...
	Иначе
		Если Настройки.ПодробныйЖурнал Тогда
			ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
				НСтр("ru = 'Очищена очередь сообщений.'", КодЯзыка));
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		Если Настройки.ПодробныйЖурнал Тогда
			ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
				НСтр("ru = 'Завершение очистки.'", КодЯзыка));
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
//  ОбластьДанных - Число - разделитель в модели сервиса.
//
Процедура ВыполнитьПодключениеКассовыхАппаратов(ИнформацияПоКассовымАппаратам, Режим, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Настройки.ИдентификаторОбластиНаСервереШтрихМ) Тогда
		...
		Если ЗначениеЗаполнено(ИдентификаторОбласти) Тогда
			...
			Если Настройки.ПодробныйЖурнал Тогда
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Получение идентификатора области.'", КодЯзыка),
					УровеньЖурналаРегистрации.Информация,,,
					НСтр("ru = 'Получен идентификатор области.'", КодЯзыка) + СокрЛП(ИдентификаторОбласти));
			КонецЕсли;
		Иначе
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Получение идентификатора области.'", КодЯзыка),
	...
	// Возможна ситуация, когда касса не отключена в другой Agent_node.
	// Такие кассы должны быть перерегистрированы в новую Agent_node.
	// При перерегистрации проверится токен, этим исключается перерегистрация "чужой" кассы.
	ФильтрДляПеререгистрации = Новый Структура();
	ФильтрДляПеререгистрации.Вставить("Результат", "cashregister_already_authorized_by_another_node");
	
	СтрокиКассДляПеререгистрации = ИнформацияПоКассовымАппаратам.НайтиСтроки(ФильтрДляПеререгистрации);
	
	Если СтрокиКассДляПеререгистрации.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	КассыДляПеререгистрации = ИнформацияПоКассовымАппаратам.СкопироватьКолонки();
	
	Для каждого Строка Из СтрокиКассДляПеререгистрации Цикл
		СтрокаДляПеререгистрации = КассыДляПеререгистрации.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаДляПеререгистрации, Строка,, "Результат, СообщениеОбмена");
	КонецЦикла;
	
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'Начало обмена перерегистрация из узла.'", КодЯзыка));
	КонецЕсли;
	...
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'Завершение обмена перерегистрация из узла.'", КодЯзыка));
	КонецЕсли;
КонецПроцедуры
...
//
// Параметры:
//  РеквизитыКассы - Структура - содержит реквизиты подключаемого/отключаемого кассового аппарата:
//    РегистрационныйНомер - Строка - регистрационный номер кассового аппарата;
//    Токен - Строка - сервисный пароль кассового аппарата;
//    ИдентификаторОбластиНаСервереШтрихМ - Строка - область подключения кассового аппарата на сервере Штрих(agent_node_id);
//  Режим - Число - метод, который необходимо выполнить с кассовыми аппаратами.
//    1 - Подключение;
//    2 - Отключение;
//    3 - Переподключение.
...
//  ОбластьДанных - Число - разделитель в модели сервиса.
//
Процедура ВыполнитьРегистрациюНаСервере(РеквизитыКассы, Режим, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Ложь, ОбластьДанных = 0) Экспорт
	...
	ВыполнитьПодключениеКассовыхАппаратов(КассыДляРегистрации, Режим,
		ЕстьОшибки, СообщениеОбОшибке, ВестиЛог, ОбластьДанных);
КонецПроцедуры

// Выполняет выгрузку настроек на кассовый аппарат.
//
Процедура ВыполнитьОбновлениеНастроекНаККТ(РеквизитыКассы, ЕстьОшибки, СообщениеОбОшибке) Экспорт
	
	КодЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	Настройки = НастройкиОбменаДаннымиССерверомШтрихМ();
	
	ЗаголовокЖурнала = НСтр("ru = 'Выгрузка настроек ККТ.'", КодЯзыка);
	...
	Если НЕ Настройки.ВыполнятьОбменССерверомШтрихМ Тогда
		Если Настройки.НетПодключенныхКасс Тогда
			СообщениеОбОшибке = НСтр("ru = 'Нет подключенных ККТ.'", КодЯзыка);
			УровеньЖурнала = УровеньЖурналаРегистрации.Информация;
		Иначе
			СообщениеОбОшибке = НСтр("ru = 'Не настроен обмен с сервером.'", КодЯзыка);
			ЕстьОшибки = Истина;
	...
	Настройки.Вставить("ТипМетода", "settings_upload");
	...
	ОбновитьНастройкиКасс(Соединение.HTTPСоединение, Настройки, РеквизитыКассы, ЕстьОшибки, СообщениеОбОшибке);
...
// Выполняет запрос состояния кассового аппарата.
//
// Параметры:
//  ИнформацияПоКассовымАппаратам - ТаблицаЗначений - информация по ККМ для подключения, колонки:
//    РегистрационныйНомер - Строка - регистрационный номер кассового аппарата;
...
//  ОбластьДанных - Число - разделитель в модели сервиса.
//
Процедура ВыполнитьЗапросСостоянияКассовыхАппаратов(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	
	КодЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	Настройки = НастройкиОбменаДаннымиССерверомШтрихМ();
	
	ЗаголовокЖурнала = НСтр("ru = 'Запрос состояния кассовых аппаратов.'", КодЯзыка);
	...
	Настройки.Вставить("ТипМетода", "cashregister_upload_status");
	ЗапроситьСтатусКассовыхАппаратов(Соединение.HTTPСоединение, Настройки, ИнформацияПоКассовымАппаратам,
		ЕстьОшибки, СообщениеОбОшибке);
...
// Выполняет очистку очереди сообщений для кассовых аппаратов.
//
// Параметры:
//  ИнформацияПоКассовымАппаратам - ТаблицаЗначений - информация по ККМ для очистки, колонки:
//    РегистрационныйНомер - Строка - регистрационный номер кассового аппарата;
//    Результат - Строка - строка результата обмена;
//    СообщениеОбмена - Строка - строка сообщения обмена.
//  ЕстьОшибки - Булево - флаг ошибки при обработке;
...
//  ОбластьДанных - Число - разделитель в модели сервиса.
//
Процедура ВыполнитьОчисткуСообщенийДляКассовыхАппаратов(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	
	КодЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	Настройки = НастройкиОбменаДаннымиССерверомШтрихМ();
	
	ЗаголовокЖурнала = НСтр("ru = 'Очистка сообщений для кассовых аппаратов Штрих-М.'", КодЯзыка);
	
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'Получены настройки обмена.'", КодЯзыка));
	КонецЕсли;
	...
	Если Настройки.ПодробныйЖурнал Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала,
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("ru = 'Начало очистки сообщений.'", КодЯзыка));
	КонецЕсли;
...
Процедура ВыполнитьВыборочнуюОчисткуТоваровНаКассовыхАппаратах(ТоварыИКассы, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Выборочная очистка товаров на кассовых аппаратах.'", КодЯзыка);
...
Процедура ВыполнитьВыгрузкуТоваров(ТоварыИКассы, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина, ОбластьДанных = 0) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Выгрузка товаров на ККТ.'", КодЯзыка);
...
//
// Параметры:
//  РегистрационныйНомер - Строка - рег.номер кассового аппарата;
//  ЕстьОшибки - Булево - флаг ошибки при обработке;
//  СообщениеОбОшибке - Строка - строка сообщения об ошибке;
//  ВестиЛог - Булево - флаг ведения записей в журнале регистрации;
//  ОбластьДанных - Число - разделитель в модели сервиса.
//
Процедура ВыполнитьВыгрузкуТоваровДляКассы(РегистрационныйНомер, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Ложь, ОбластьДанных = 0) Экспорт
	...
	КассыДляРегистрации.Колонки.Добавить("Результат");
	КассыДляРегистрации.Колонки.Добавить("СообщениеОбмена");
	КассыДляРегистрации.Колонки.Добавить("СообщениеВыполнения");
	КассыДляРегистрации.Колонки.Добавить("ОбластьДанныхВспомогательныеДанные");
	КассыДляРегистрации.Колонки.Добавить("ИдентификаторОбластиШтрих");
	
	СтрокаКассы = КассыДляРегистрации.Добавить();
	СтрокаКассы.РегистрационныйНомер = РегистрационныйНомер;
	
	ВыполнитьОчисткуСообщенийДляКассовыхАппаратов(КассыДляРегистрации, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог, ОбластьДанных);
	...
	ГруппыИТоварыДляВыгрузки = ОбменСКассовымСерверомШтрихМПереопределяемый.ГруппыИТовары();
	...
	ВыполнитьВыгрузкуТоваров(ТоварыИКассыДляВыгрузки, ЕстьОшибки, СообщениеОбОшибке);
	...
	ВыполнитьВыборочнуюОчисткуТоваровНаКассовыхАппаратах(ТоварыИКассыДляОчистки, ЕстьОшибки, СообщениеОбОшибке);
	
	Если ЕстьОшибки Тогда
		Возврат;
	КонецЕсли;
	
	ПроверитьСтатусЗагрузкиТоваровНаКассовыхАппаратах(ТоварыИКассыДляВыгрузки, ЕстьОшибки, СообщениеОбОшибке);
КонецПроцедуры
...
Процедура ПроверитьСтатусЗагрузкиТоваровНаКассовыхАппаратах(ИнформацияПоКассовымАппаратам, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Проверка актуальности товаров.'", КодЯзыка);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Справочники.ЗарегистрированныеКассы.ЗаполнитьТипПодключенияККТ";
КонецПроцедуры
...
Процедура ЗаписатьВПротоколКассовогоАппарата(РегистрационныйНомер, Сообщение, ОбластьДанных = 0) Экспорт
	...
	Если ЗначениеЗаполнено(ЗарегистрированнаяКасса) Тогда
		...
		Если КассаОбъект <> Неопределено Тогда
			КассаОбъект.СообщениеПриРегистрации = ЗаписьВПротоколОбмена(КассаОбъект.СообщениеПриРегистрации, Сообщение);
...
Функция ЗарегистрированнаяКассаНаСервере(ИнформацияОКассе, Создавать, СообщениеОбОшибке, ОбластьДанных = 0) Экспорт
	...
	Иначе
		...
		|	И ЗарегистрированныеКассы.ОбластьДанныхВспомогательныеДанные = &ОбластьДанных");
	...
	Если НЕ ЗначениеЗаполнено(ЗарегистрированнаяКасса) Тогда
		...
		Иначе
			СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Не найдена зарегистрированная касса с номером: %1 '",
				ОбщегоНазначения.КодОсновногоЯзыка()), ИнформацияОКассе.РегистрационныйНомер);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки(Типы) Экспорт
	...
	Типы.Добавить(Метаданные.Справочники.ДанныеОбменаШтрихМПЭЙ);
	Типы.Добавить(Метаданные.Справочники.УдалитьДанныеОбменаШтрихМПЭЙ)
...
Функция УзелОбменаШтрих() Экспорт
	...
	Если УзелОбменаШтрихСсылка.Пустая() Тогда
		Если РаботаВМоделиСервиса.РазделениеВключено() И НЕ РаботаВМоделиСервиса.СеансЗапущенБезРазделителей() Тогда
	...
	Если Не ЗначениеЗаполнено(ГлавныйУзелОбмена.Наименование) Тогда
		...
		ГлавныйУзелОбъект.Наименование = НСтр("ru='Центральный'", ОбщегоНазначения.КодОсновногоЯзыка());
		Попытка
...
// Устанавливает использование регламентных заданий обмена.
//
Процедура УстановитьИспользованиеРегламентногоЗадания() Экспорт
	...
	ИспользоватьОбменССерверомШтрихМ = ПолучитьФункциональнуюОпцию("ИспользоватьОбменССерверомШтрихМ");
	
	УзелОбмена = ОбменСКассовымСерверомШтрихМПовтИсп.УзелОбменаШтрих();
	
	ВыполнятьОбменССерверомШтрихМ = ИспользоватьОбменССерверомШтрихМ
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелОбмена, "ВыполнятьОбменССерверомШтрихМ");
...
Процедура ВыполнитьПотоковыйОбменДаннымиССерверомШтрихМ(Настройки)
	...
	КодЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	ЗаголовокЖурнала = НСтр("ru = 'Регламентное задание ""Обмен данными с сервером Штрих-М"".'", КодЯзыка);
	
	HTTPСоединение = Неопределено;
	Соединение = HTTPСоединение(Настройки);
	Если Соединение.HTTPСоединение = Неопределено Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала + НСтр("ru = 'Установка соединения'", КодЯзыка),
	...
	Если НЕ ЗначениеЗаполнено(Настройки.ИдентификаторОбластиНаСервереШтрихМ) Тогда
		...
		Если Отказ Тогда
			ЗаписьЖурналаРегистрации(ЗаголовокЖурнала + НСтр("ru = 'Получение идентификатора области'", КодЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
			Возврат;
		КонецЕсли;
		ИдентификаторОбласти = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Настройки.НастройкаОбмена,
	...
	Если Отказ Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала + НСтр("ru = 'Проверка принадлежности касс к текущей области'", КодЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
	КонецЕсли;
	...
	Если НЕ ЕстьПодключенныеКассы(Настройки.ИдентификаторОбластиНаСервереШтрихМ) Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала + НСтр("ru = 'Проверка регистрации касс в текущей области'", КодЯзыка),
	...
	Если Отказ Тогда
		ЗаписьЖурналаРегистрации(ЗаголовокЖурнала + НСтр("ru = 'Получение кассовых документов'", КодЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтвет_КассовыеДокументы(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Если ТипЗнч(Результат[ИмяРеквизитаОтвета]) = Тип("Массив") Тогда
		РеквизитыКонтейнераДокументов = ОбменСКассовымСерверомШтрихМПовтИсп.РеквизитыКонтейнераДокументов();
		...
		ТаблицаКонтейнераДокументов = ТаблицаЗначенийПоСтруктуре(РеквизитыКонтейнераДокументов);
	...
	Если НЕ ИмяРеквизитаОтвета = "fetch_documents_response" Тогда
		...
		Иначе
			СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Ошибка записи кассового аппарата: %1'",
				ОбщегоНазначения.КодОсновногоЯзыка()), СокрЛП(ДополнительныеПараметры.РеквизитыКассы.Ссылка));
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОтвет_КассовыеДокументы_Документы(ДанныеДляОбработки, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаКонтейнера Из ТаблицаКонтейнераДокументов Цикл
		...
		Если НЕ ЕстьОшибки Тогда
			ДатаУспешногоЧека = ДатаИзСтрокиФорматаISO8601(СтрокаКонтейнера.doc_timestamp);
		КонецЕсли;
		...
		ИначеЕсли КассовыеАппаратыПоРегНомеру.Количество() > 1 Тогда
			...
			// Это нештатная ситуация, требующая разбора.Отметим предупреждением в журнале регистрации.
...
Процедура ЗарегистрироватьКассы(Соединение, Настройки, Кассы, Отказ, СообщениеОбОшибке) Экспорт
...
Функция КассыДляПеререгистрации(ИдентификаторОбластиНаСервереШтрихМ)
	...
	|	ЗарегистрированныеКассы.РегистрационныйНомер КАК РегистрационныйНомер,
	|	ЗарегистрированныеКассы.ОбластьДанныхВспомогательныеДанные КАК ОбластьДанныхВспомогательныеДанные,
	|	ЗарегистрированныеКассы.Токен КАК Токен,
	...
	|ГДЕ
	|	ЗарегистрированныеКассы.СостояниеПодключения = ЗНАЧЕНИЕ(Перечисление.СостоянияРегистрацииКассовогоАппарата.Подключен)
	|	И ЗарегистрированныеКассы.ИдентификаторЭвотор = """"
...
Функция НоваяЗарегистрированнаяКасса(ИнформацияОКассе, СообщениеОбОшибке, ОбластьДанных = 0)
	...
	НоваяКасса.ТипПодключенияККТ = Перечисления.ТипыПодключенияККТ.СервисШтрихМ;
...
Процедура ОбработатьРезультатПодключенияКасс(КассыДляРегистрации, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаКассы Из КассыДляРегистрации Цикл
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			СообщениеПротокол = СокрЛП(СтрокаКассы.Результат) + " " + СтрокаКассы.СообщениеОбмена;
			КассаОбъект.СообщениеПриРегистрации = ЗаписьВПротоколОбмена(КассаОбъект.СообщениеПриРегистрации, СообщениеПротокол);
...
Процедура ОбновитьНастройкиКасс(Соединение, Настройки, НастройкиКасс, Отказ, СообщениеОбОшибке)
	
	ПараметрыМетода = Новый Массив;
	
	РегистрируемыеКассы = Новый Массив;
	РеквизитыКассы = Новый Структура;
	РеквизитыКассы.Вставить("cashregister_number", НастройкиКасс.РегистрационныйНомер);
	РегистрируемыеКассы.Добавить(РеквизитыКассы);
	ПараметрыМетода.Добавить(Новый Структура("cashregister_info", РегистрируемыеКассы));
	
	ДобавитьИдентификатор(ПараметрыМетода, Настройки);
	
	ОбновляемыеНастройки = Новый Массив;
	ЗначениеНастройкиСНО = Новый Структура("value", НастройкиКасс.НастройкаСНО);
	ОбновляемыеНастройки.Добавить(ЗначениеНастройкиСНО);
	ПараметрыМетода.Добавить(Новый Структура("settings", ОбновляемыеНастройки));
	...
	СтрокаКассы = КассыДляРегистрации.Добавить();
	СтрокаКассы.РегистрационныйНомер = НастройкиКасс.РегистрационныйНомер;
	
	ЗарегистрированнаяКасса = ЗарегистрированнаяКассаНаСервере(СтрокаКассы, Ложь, СообщениеОбОшибке);
	Если ЗарегистрированнаяКасса = Неопределено Тогда
		ЕстьОшибки = Истина;
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление настроек.'"),
			УровеньЖурналаРегистрации.Предупреждение,,, СообщениеОбОшибке);
		Возврат;
	Иначе
		СтрокаКассы.Ссылка = ЗарегистрированнаяКасса;
	КонецЕсли;
	
	ДанныеДляЗапроса.Вставить("ДополнительныеПараметры", КассыДляРегистрации);
	
	ВыполнитьЗапросКСерверуШтрихМ(Соединение, Настройки, ДанныеДляЗапроса, Отказ, СообщениеОбОшибке);
КонецПроцедуры

Процедура ОбработатьОтвет_НастройкаКасс(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		Для каждого Элемент Из Результат Цикл
			...
			СтрокаТаблицыКасс.СообщениеОбмена = СтруктураОтвета.message;
		КонецЦикла;
		ОбработатьРезультатОбновленияНастроек(ДополнительныеПараметры, Отказ, СообщениеОбОшибке);
	Иначе
		СообщениеОбОшибке = НСтр("ru = 'Ошибка формата ответного сообщения (регистрация касс).'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		Отказ = Истина;
...
Процедура ОбработатьРезультатОбновленияНастроек(КассыДляРегистрации, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаКассы Из КассыДляРегистрации Цикл
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Исключение
				Сообщение = Сообщение
					+ СтрШаблон(НСтр("ru = 'При обновлении настроек ошибочно обработан регистрационный номер ККМ: %1'", КодЯзыка),
						СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
				Продолжить;
			КонецПопытки;
			...
			Если НРег(СтрокаКассы.Результат) = "auth_required"
				ИЛИ НРег(СтрокаКассы.Результат) = "invalid_cashregister_number" Тогда
				Подключен = Ложь;
				ОсвободитьЛицензию = Истина;
				СообщениеПротокол = СокрЛП(СтрокаКассы.Результат) + " " + СокрЛП(СтрокаКассы.СообщениеОбмена);
			Иначе
				Подключен = Истина;
				Сообщение = Сообщение + СтрокаКассы.СообщениеОбмена + Символы.ПС;
				СообщениеПротокол = НСтр("ru = 'Настройки успешно обновлены'", КодЯзыка);
			КонецЕсли;
			...
			Если НЕ Подключен Тогда
				...
				ИнформацияОПодключении.Вставить("Подключен", Подключен);
				ИнформацияОПодключении.Вставить("ОсвободитьЛицензию", ОсвободитьЛицензию);
				ИнформацияОПодключении.Вставить("ЗаписатьТекущиеЦены", Ложь);
				ИнформацияОПодключении.Вставить("ПроверитьЗагрузкуТоваров", Ложь);
	...
	Иначе
		Если ЗначениеЗаполнено(Сообщение) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен Штрих. Обновление настроек ККМ.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, Сообщение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура ЗапроситьСтатусКассовыхАппаратов(Соединение, Настройки, Кассы, Отказ, СообщениеОбОшибке)
...
Процедура ОбработатьОтвет_СостояниеКасс(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры) Экспорт
	
	Если Результат.Свойство("cashregister_upload_status_response") Тогда
		Если ТипЗнч(Результат.cashregister_upload_status_response) = Тип("Массив") Тогда
			
			СтруктураОтветаСостояние = ОбменСКассовымСерверомШтрихМПовтИсп.РеквизитыСостоянияКассы();
			ТаблицаКасс = ДополнительныеПараметры.Кассы;
			
			Для каждого Элемент Из Результат.cashregister_upload_status_response Цикл
				ЗаполнитьЗначенияСвойств(СтруктураОтветаСостояние, Элемент);
				
				СтрокаТаблицыКасс = ТаблицаКасс.Найти(СтруктураОтветаСостояние.cashregister_number, "РегистрационныйНомер");
				Если СтрокаТаблицыКасс = Неопределено Тогда
					СтрокаТаблицыКасс = ТаблицаКасс.Добавить();
					СтрокаТаблицыКасс.РегистрационныйНомер = СтруктураОтветаСостояние.cashregister_number;
				КонецЕсли;
				
				СостояниеКассовогоАппарата = Новый Структура();
				СостояниеКассовогоАппарата.Вставить("ДатаОбмена", СтруктураОтветаСостояние.last_seen_at);
				СостояниеКассовогоАппарата.Вставить("КоличествоНепереданныхПакетов", СтруктураОтветаСостояние.unsent_packets_count);
				СостояниеКассовогоАппарата.Вставить("РазмерНепереданныхПакетов", СтруктураОтветаСостояние.unsent_packets_size_in_bytes);
				
				СтрокаТаблицыКасс.СостояниеККТ = СостояниеКассовогоАппарата;
			КонецЦикла;
			ОбработатьРезультатСостояниеКасс(ТаблицаКасс, Отказ, СообщениеОбОшибке);
		Иначе
			СообщениеОбОшибке = НСтр("ru = 'Ошибка формата ответного сообщения (состояние касс).'",
			ОбщегоНазначения.КодОсновногоЯзыка());
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если Результат.Свойство("add_info") Тогда
		Если ТипЗнч(Результат.add_info) = Тип("Массив") Тогда
			...
			Для каждого Элемент Из Результат.add_info Цикл
				...
				СтрокаТаблицыКасс.Результат = СтруктураОтвета.info_type;
				СтрокаТаблицыКасс.СообщениеОбмена = СтруктураОтвета.message;
				СтрокаТаблицыКасс.СообщениеВыполнения = НСтр("ru = 'Состояние кассового аппарата успешно обновлено.'",
					ОбщегоНазначения.КодОсновногоЯзыка());
			КонецЦикла;
			ОбработатьРезультатИнформацияПоКассам(ТаблицаКасс, Отказ, СообщениеОбОшибке);
			ТаблицаКасс = Неопределено;
		Иначе
			СообщениеОбОшибке = НСтр("ru = 'Ошибка формата ответного сообщения (состояние касс).'",
				ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОбработатьРезультатСостояниеКасс(КассыДляРегистрации, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаКассы Из КассыДляРегистрации Цикл
		...
		Если НЕ ТипЗнч(СтрокаКассы.СостояниеККТ) = Тип("Структура") Тогда
			Сообщение = Сообщение
				+ СтрШаблон(НСтр("ru = 'Ошибочная структура ответа сервера для номера ККМ: %1'", КодЯзыка),
					СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
			Продолжить;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Исключение
				Сообщение = Сообщение
					+ СтрШаблон(НСтр("ru = 'Не найдена касса по номеру ККМ: %1'", КодЯзыка),
						СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
				Продолжить;
			КонецПопытки;
			
			Если СтрокаКассы.СостояниеККТ.Свойство("ДатаОбмена") И ЗначениеЗаполнено(СтрокаКассы.СостояниеККТ.ДатаОбмена) Тогда
				ДатаПоследнегоОбмена = ДатаИзСтрокиФорматаISO8601(СтрокаКассы.СостояниеККТ.ДатаОбмена);
			Иначе
				ДатаПоследнегоОбмена = НСтр("ru = 'не определена'");
			КонецЕсли;
			
			СообщениеПротокол = СтрШаблон(НСтр("ru = 'Состояние ККТ № %1:
			|	дата сеанса обмена с ККТ - %2
			|	количество пакетов для ККТ - %3
			|	размер пакетов(b) для ККТ) - %4
			|'", КодЯзыка),
				СтрокаКассы.РегистрационныйНомер, ДатаПоследнегоОбмена,
				СтрокаКассы.СостояниеККТ.КоличествоНепереданныхПакетов, СтрокаКассы.СостояниеККТ.РазмерНепереданныхПакетов);
		...
		Иначе
			Сообщение = Сообщение
				+ СтрШаблон(НСтр("ru = 'Не найдена ссылка на кассу по номеру ККМ: %1'", КодЯзыка),
					СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
		КонецЕсли;
	КонецЦикла;
	...
	Иначе
		Если ЗначениеЗаполнено(Сообщение) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен Штрих. Запрос состояния ККМ.'", КодЯзыка),
				УровеньЖурналаРегистрации.Предупреждение,,, Сообщение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура ОбработатьРезультатИнформацияПоКассам(КассыДляРегистрации, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаКассы Из КассыДляРегистрации Цикл
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Исключение
				Сообщение = Сообщение
					+ СтрШаблон(НСтр("ru = 'При запросе ошибочно обработан регистрационный номер ККМ: %1'", КодЯзыка),
						СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
				Продолжить;
			КонецПопытки;
			...
			Иначе
				Подключен = Истина;
				Сообщение = Сообщение + СтрокаКассы.СообщениеОбмена + Символы.ПС;
				СообщениеПротокол = СокрЛП(СтрокаКассы.СообщениеВыполнения);
			КонецЕсли;
...
Процедура ОбработатьОтвет_ОчисткаСообщенийКасс(Результат, Отказ, СообщениеОбОшибке, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		...
		Для каждого Элемент Из Результат Цикл
			...
			СтрокаТаблицыКасс.Результат = СтруктураОтвета.info_type;
			СтрокаТаблицыКасс.СообщениеОбмена = СтруктураОтвета.message;
			СтрокаТаблицыКасс.СообщениеВыполнения = НСтр("ru = 'Сообщения для ККТ очищены.'",
				ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЦикла;
		ОбработатьРезультатИнформацияПоКассам(ТаблицаКасс, Отказ, СообщениеОбОшибке);
		ТаблицаКасс = Неопределено;
...
Процедура ВыполнитьОчисткуСообщенийДляККТ(Соединение, Настройки, Кассы, Отказ, СообщениеОбОшибке)
...
Процедура ЗаполнитьИнформациюОРезультатеПодключения(Результат, ИнформацияОПодключении, ПроверитьЗагрузкуТоваров = Ложь)
	
	Если НРег(Результат) = "goods_cleared" Тогда
		ИнформацияОПодключении.ПроверитьЗагрузкуТоваров = Истина;
		ИнформацияОПодключении.ЗаписатьТекущиеЦены = Истина;
	ИначеЕсли НРег(Результат) = "goods_uploaded" Тогда
		ИнформацияОПодключении.ПроверитьЗагрузкуТоваров = ПроверитьЗагрузкуТоваров;
		ИнформацияОПодключении.ЗаписатьТекущиеЦены = Истина;
	ИначеЕсли НРег(Результат) = "invalid_token" Тогда
		ИнформацияОПодключении.ОсвободитьЛицензию = Истина;
		ИнформацияОПодключении.Подключен = Ложь;
	ИначеЕсли НРег(Результат) = "invalid_cashregister_number" Тогда
		ИнформацияОПодключении.ОсвободитьЛицензию = Истина;
		ИнформацияОПодключении.Подключен = Ложь;
		ИнформацияОПодключении.Вставить("Товары", Неопределено);
	ИначеЕсли НРег(Результат) = "kkm_token_changed" Тогда
		ИнформацияОПодключении.ОсвободитьЛицензию = Истина;
		ИнформацияОПодключении.Подключен = Ложь;
	ИначеЕсли НРег(Результат) = "auth_required" Тогда
		ИнформацияОПодключении.ОсвободитьЛицензию = Истина;
		ИнформацияОПодключении.Подключен = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗарегистрироватьТовары(Соединение, Настройки, ТоварыДляОбмена, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаТоваров Из Товары Цикл
		...
		РеквизитыТоваров.Вставить("alcohol_excisable", СтрокаТоваров.Маркируемый);
		РеквизитыТоваров.Вставить("alcohol_kind_code", СтрокаТоваров.КодАлкогольнойПродукции);
		РеквизитыТоваров.Вставить("section", СтрокаТоваров.Отдел);
		ОбменСКассовымСерверомШтрихМПереопределяемый.ДополнитьРеквизитамиМаркируемойПродукции(РеквизитыТоваров, СтрокаТоваров.Ссылка);
	...
	Для каждого СтрокаКассы Из КассыДляРегистрации Цикл
		...
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Попытка
				КассаОбъект = СтрокаКассы.Ссылка.ПолучитьОбъект();
			Исключение
				Сообщение = Сообщение + СтрШаблон(НСтр("ru = 'При очистке товаров ошибочно обработан ККМ с заводским номером: %1'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС + ОписаниеОшибки() + Символы.ПС;
				Продолжить;
			КонецПопытки;
			
			Если КассаОбъект = Неопределено Тогда
				Сообщение = Сообщение + СтрШаблон(НСтр("ru = 'При очистке товаров ошибочно обработан ККМ с заводским номером: %1. Объект не найден.'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
				Продолжить;
			КонецЕсли;
			...
			ИнформацияОПодключении = Новый Структура();
			ИнформацияОПодключении.Вставить("Подключен", Ложь);
			ИнформацияОПодключении.Вставить("ОсвободитьЛицензию", Ложь);
			ИнформацияОПодключении.Вставить("ЗаписатьТекущиеЦены", Ложь);
			ИнформацияОПодключении.Вставить("ПроверитьЗагрузкуТоваров", Ложь);
			ИнформацияОПодключении.Вставить("РегистрационныйНомер", КассаОбъект.РегистрационныйНомер);
			ИнформацияОПодключении.Вставить("ОбластьВспомогательныхДанных", КассаОбъект.ОбластьДанныхВспомогательныеДанные);
			...
			ИнформацияОПодключении.Подключен = 
				КассаОбъект.СостояниеПодключения = Перечисления.СостоянияРегистрацииКассовогоАппарата.Подключен;
				
			ЗаполнитьИнформациюОРезультатеПодключения(СтрокаКассы.Результат, ИнформацияОПодключении);
			...
			КонецЕсли;
		Иначе
			Сообщение = Сообщение + СтрШаблон(НСтр("ru = 'При очистке товаров ошибочно обработан ККМ с заводским номером: %1. Не найдена ссылка на объект.'",
				КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
		КонецЕсли;
	КонецЦикла;
	Если ЕстьОшибка Тогда
	...
	Для каждого СтрокаКассы Из Кассы Цикл
		...
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Попытка
				КассаОбъект = СтрокаКассы.Ссылка.ПолучитьОбъект();
			Исключение
				Сообщение = Сообщение + СтрШаблон(НСтр("ru = 'При проверке товаров ошибочно обработан ККМ с заводским номером: %1'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС + ОписаниеОшибки() + Символы.ПС;
				Продолжить;
			КонецПопытки;
			
			Если КассаОбъект = Неопределено Тогда
				Сообщение = Сообщение + СтрШаблон(НСтр("ru = 'При проверке товаров ошибочно обработан ККМ с заводским номером: %1. Объект не найден.'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
				Продолжить;
			КонецЕсли;
			...
			ИнформацияОПодключении.Вставить("ОсвободитьЛицензию", Ложь);
			ИнформацияОПодключении.Вставить("ЗаписатьТекущиеЦены", Ложь);
			ИнформацияОПодключении.Вставить("ПроверитьЗагрузкуТоваров", Истина);
			ИнформацияОПодключении.Вставить("РегистрационныйНомер", КассаОбъект.РегистрационныйНомер);
			ИнформацияОПодключении.Вставить("ОбластьВспомогательныхДанных", КассаОбъект.ОбластьДанныхВспомогательныеДанные);
			ИнформацияОПодключении.Вставить("ИдентификаторОбластиНаСервереШтрихМ",
				КассаОбъект.ИдентификаторОбластиНаСервереШтрихМ);
			...
			КонецЕсли;
		Иначе
			Сообщение = Сообщение + СтрШаблон(НСтр("ru = 'При проверке товаров ошибочно обработан ККМ с заводским номером: %1. Не найдена ссылка на объект.'",
				ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
		КонецЕсли;
	КонецЦикла;
	Если ЕстьОшибка Тогда
		Отказ = Истина;
	...
	Для каждого СтрокаКассы Из КассыДляРегистрации Цикл
		...
		Если ЗначениеЗаполнено(СтрокаКассы.Ссылка) Тогда
			...
			Попытка
				КассаОбъект = СтрокаКассы.Ссылка.ПолучитьОбъект();
			Исключение
				Сообщение = Сообщение + СтрШаблон(НСтр("ru = 'При регистрации товаров ошибочно обработан ККМ с заводским номером: %1'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС + ОписаниеОшибки() + Символы.ПС;
				Продолжить;
			КонецПопытки;
			
			Если КассаОбъект = Неопределено Тогда
				Сообщение = Сообщение + СтрШаблон(НСтр("ru = 'При регистрации товаров ошибочно обработан ККМ с заводским номером: %1. Объект не найден.'",
					КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
				Продолжить;
			КонецЕсли;
			...
			ЗаполнитьИнформациюОРезультатеПодключения(СтрокаКассы.Результат, ИнформацияОПодключении, Истина);
			...
			КонецЕсли;
		Иначе
			Сообщение = Сообщение + СтрШаблон(НСтр("ru = 'При регистрации товаров ошибочно обработан ККМ с заводским номером: %1. Не найдена ссылка на объект.'",
				КодЯзыка), СтрокаКассы.РегистрационныйНомер) + Символы.ПС;
		КонецЕсли;
	КонецЦикла;
	Если ЕстьОшибка Тогда
...
Функция НастройкиОбменаДаннымиССерверомШтрихМ(ПроверятьРегистрациюКасс = Истина) Экспорт
	
	НастройкиОбмена = Новый Структура();
	НастройкиОбмена.Вставить("ВыполнятьОбменССерверомШтрихМ", Ложь);
	НастройкиОбмена.Вставить("НетПодключенныхКасс", Ложь);
	НастройкиОбмена.Вставить("ПодробныйЖурнал", Ложь);
	НастройкиОбмена.Вставить("НастройкаОбмена", Неопределено);
	...
	НастройкиОбмена.Вставить("ПодробныйЖурнал", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкиОбмена.НастройкаОбмена,
		"ПодробноеВедениеЖурналаРегистрацииДляОтладки"));
	// Регистрация касс не проверяется только при получении настроек для регистрации касс.
	// В остальных случаях - проверяется. 
	// Исключение - первый обмен с сервером, когда еще не получен ИдентификаторОбластиНаСервереШтрихМ.
	// В этом случае, если сеанс разделенный, обмен не проводить. Если сеанс неразделенный, провести обмен в любом случае.
	Если РаботаВМоделиСервиса.РазделениеВключено() Тогда
		ИдентификаторОбластиНаСервереШтрихМ = 
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкиОбмена.НастройкаОбмена, "ИдентификаторОбластиНаСервереШтрихМ");
		Если НЕ ЗначениеЗаполнено(ИдентификаторОбластиНаСервереШтрихМ) Тогда
			Если РаботаВМоделиСервиса.СеансЗапущенБезРазделителей() Тогда
				ПроверятьРегистрациюКасс = Ложь;
			Иначе
				Если Не ПроверятьРегистрациюКасс Тогда
					ПроверятьРегистрациюКасс = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьЗапросКСерверуШтрихМ(Соединение, Настройки, Данные, Отказ, СообщениеОбОшибке) Экспорт

	HTTPОтвет = Неопределено;
	
	HTTPЗапрос = Новый HTTPЗапрос(Настройки.ПутьНаСервере + "/" + Настройки.ТипМетода);
	HTTPЗапрос.Заголовки.Вставить("Content-Type", "application/json");
	HTTPЗапрос.Заголовки.Вставить("request_signature", Настройки.Подпись);
	
	СтрокаJSON = СтрокаJSON(Данные.ПараметрыМетода);
	
	Если СтрокаJSON <> Неопределено Тогда
		
		Если Настройки.ПодробныйЖурнал Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Текст НТТР запроса к серверу Штрих-М.'"),
				УровеньЖурналаРегистрации.Информация,,, СокрЛП(СтрокаJSON));
		КонецЕсли;
		...
		Исключение
			...
			СообщениеОбОшибке = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			Отказ = Истина;
			ЗаписьЖурналаРегистрации(НСтр("ru = 'НТТР запрос к серверу Штрих-М.'"),
				УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
			Возврат;
		КонецПопытки;
	...
	Иначе
		HTTPЗапрос = Неопределено;
		Отказ = Истина;
		СообщениеОбОшибке = НСтр("ru = 'Ошибка записи запроса в JSON.'")
	КонецЕсли;
...
Процедура ОбработатьОтветСервера(Результат, Настройки, Отказ, СообщениеОбОшибке)
	...
	Если Результат.КодСостояния >= 400 И Результат.КодСостояния < 500 Тогда
		СообщениеОбОшибке = НСтр("ru = 'Код статуса больше 4XX, ошибка запроса. Код статуса: %1'", КодЯзыка);
		Отказ = Истина;
	КонецЕсли;
	...
	Если Результат.КодСостояния >= 500 И Результат.КодСостояния < 600 Тогда
		СообщениеОбОшибке = НСтр("ru = 'Код статуса больше 5XX, ошибка сервера. Код статуса: %1'", КодЯзыка);
		Отказ = Истина;
	КонецЕсли;
	
	// Обрабатываем перенаправление.
	Если Результат.КодСостояния >= 300 И Результат.КодСостояния < 400 Тогда
		СообщениеОбОшибке = НСтр("ru = 'Код статуса больше 3XX, Перенаправление. Код статуса: %1'", КодЯзыка);
		Если Результат.КодСостояния = 302 Тогда
			...
			Иначе
				СообщениеОбОшибке = НСтр("ru = 'Постоянное перенаправление. Код статуса: %1'", КодЯзыка);
				Отказ = Истина;
			КонецЕсли;
		Иначе
			Отказ = Истина;
	...
	Если Отказ Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ответ сервера Штрих-М.'", КодЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
	КонецЕсли;
КонецПроцедуры
	...
	Если ОбъектJSON.Свойство("response_error") Тогда
		...
		Если НЕ КодРезультата = 0 Тогда
			...
			Если КодОшибки = 1 Тогда
				СообщениеОбОшибке = НСтр("ru = 'Несуществующий идентификатор сервиса.'", КодЯзыка);
			ИначеЕсли КодОшибки = 2 Тогда
				СообщениеОбОшибке = НСтр("ru = 'Неверная подпись.'", КодЯзыка);
			ИначеЕсли КодОшибки = 3 Тогда
				СообщениеОбОшибке = НСтр("ru = 'Неверный тип запроса.'", КодЯзыка);
			ИначеЕсли КодОшибки = 4 Тогда
				СообщениеОбОшибке = НСтр("ru = 'Неверный формат запроса.'", КодЯзыка);
			ИначеЕсли КодОшибки = 5 Тогда
				СообщениеОбОшибке = НСтр("ru = 'Ошибка сервера.'", КодЯзыка);
			Иначе
				СообщениеОбОшибке = НСтр("ru = 'Ошибка формата ответного сообщения.'", КодЯзыка);
			КонецЕсли;
	...
	Если ОбъектJSON.Свойство("response") Тогда
		...
		ИначеЕсли ОбъектJSON.response.Свойство("goods_clear_response") Тогда
			ОбработатьОтвет_ОчисткаТоваров(ОбъектJSON.response.goods_clear_response,
				Отказ, СообщениеОбОшибке, ДополнительныеПараметры);
		ИначеЕсли ОбъектJSON.response.Свойство("settings_upload_response") Тогда
			ОбработатьОтвет_НастройкаКасс(ОбъектJSON.response.settings_upload_response,
				Отказ, СообщениеОбОшибке, ДополнительныеПараметры);
		ИначеЕсли ОбъектJSON.response.Свойство("cashregister_upload_status_response") Тогда
			ОбработатьОтвет_СостояниеКасс(ОбъектJSON.response,
				Отказ, СообщениеОбОшибке, ДополнительныеПараметры);
		ИначеЕсли ОбъектJSON.response.Свойство("cashregister_clear_unsent_packets_response") Тогда
			ОбработатьОтвет_ОчисткаСообщенийКасс(ОбъектJSON.response.cashregister_clear_unsent_packets_response,
				Отказ, СообщениеОбОшибке, ДополнительныеПараметры);
		КонецЕсли;
	Иначе
		СообщениеОбОшибке = НСтр("ru = 'Ошибка формата ответного сообщения.'", КодЯзыка);
...
Функция ЗаписьВПротоколОбмена(Протокол, НоваяЗапись)
	
	Возврат СокрЛП(Прав(Протокол + Символы.ПС + Формат(ТекущаяДатаСеанса(), "ДЛФ=DT")+ " " + НоваяЗапись, 1024));
КонецФункции
...
Функция СтрокаJSON(Знач ПараметрыМетода)
	...
	Если ПараметрыЗапроса.Свойство("settings") Тогда
		Для каждого ЗначениеНастроек Из ПараметрыЗапроса.settings Цикл
			ЗначениеНастройки = ЗначениеНастроек.value;
			ПараметрыЗаписиJSON = Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Авто,,,, Истина, Истина, Истина, Истина, Истина);
			ЗаписьJSON = Новый ЗаписьJSON;
			ЗаписьJSON.УстановитьСтроку(ПараметрыЗаписиJSON);
			ЗаписатьJSON(ЗаписьJSON, ЗначениеНастройки);
			СтрокаJSON = ЗаписьJSON.Закрыть();
			ЗначениеНастроек.value = СтрокаJSON;
		КонецЦикла;
	КонецЕсли;
