...
Процедура ВыполнитьФормированиеПоДоходам(Организация, ПериодФормирования, Результат, МенеджерВТ) Экспорт
	...
	ПолучитьВТДенежныеДокументыНалогообложениеВТаблице(МенеджерВТ, Организация, НачалоКвартала(ПериодФормирования), КонецДня(ПериодФормирования));
	...
	|	СУММА(ТаблицаДокумента.СуммаРасчетовВозврата) КАК СуммаДокумента,
	|	СУММА(ТаблицаДокумента.СуммаРасчетовКомиссииВозврата) КАК СуммаКомиссииДокумента
	...
	|		КОНЕЦ) КАК СуммаКомиссииДокумента
	...
	|
	|ВЫБРАТЬ
	|	&КомментарийПоступлениеВКассу,
	|	ПоступлениеВКассу.Ссылка,
	|	ПоступлениеВКассу.Контрагент,
	|	ПоступлениеВКассу.Контрагент.НаименованиеПолное,
	|	НЕОПРЕДЕЛЕНО,
	|	"""",
	...
	|ГДЕ
	|	ПоступлениеВКассуРасшифровкаПлатежа.УчитыватьВНУ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ПоступлениеНаСчетРасшифровкаПлатежа.УчитыватьВНУ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	-ЕСТЬNULL(ВТВозвратОплатыНаКарты.СуммаДокумента, РасходСоСчета.СуммаДокумента),
	|	ЕСТЬNULL(ВТВозвратОплатыНаКарты.СуммаКомиссииДокумента, РасходСоСчета.СуммаКомиссииДокумента),
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДенежныеДокументыНалогообложениеВТаблице КАК ВТДенежныеДокументыНалогообложениеВТаблице
	|		ПО РасходСоСчета.Ссылка = ВТДенежныеДокументыНалогообложениеВТаблице.ДенежныйДокумент
	|ГДЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДенежныеДокументыНалогообложениеВТаблице КАК ВТДенежныеДокументыНалогообложениеВТаблице
	|		ПО РасходИзКассы.Ссылка = ВТДенежныеДокументыНалогообложениеВТаблице.ДенежныйДокумент
	|ГДЕ
	...
	Запрос.УстановитьПараметр("КомментарийПоступлениеВКассу",
							  НСтр("ru='0. Поступление в кассу с видами операций, которые входят в массив'"));
	Запрос.УстановитьПараметр("КомментарийОтчетОРозничныхПродажах",
							  НСтр("ru='1. ОРП'"));
	Запрос.УстановитьПараметр("КомментарийПоступлениеНаСчет",
							  НСтр("ru='2. Поступление на счет с видами операций, которые входят в массив'"));
	Запрос.УстановитьПараметр("КомментарийПоступлениеНаСчетКурьерФакт",
							  НСтр("ru='3. Поступление на счет от курьерской компании или почты (фактическая сумма)'"));
	Запрос.УстановитьПараметр("КомментарийПоступлениеНаСчетКурьерУдержано",
							  НСтр("ru='4. Поступление на счет от курьерской компании или почты (удержаная сумма)'"));
	Запрос.УстановитьПараметр("КомментарийРасходСоСчета",
							  НСтр("ru='5. Расход со счета с операциями из массива'"));
	Запрос.УстановитьПараметр("КомментарийРасходИзКассы",
							  НСтр("ru='6. Расход из кассы с операциями из массива'"));
	Запрос.УстановитьПараметр("КомментарийОтчетКомиссионера",
							  НСтр("ru='7. Отчет комиссионера, в котором удержано вознаграждение'"));
	Запрос.УстановитьПараметр("КомментарийОтчетКомитенту",
							  НСтр("ru='8. Отчет комитенту, в котором удержано вознаграждение'"));
	Запрос.УстановитьПараметр("КомментарийВзаимозачет",
							  НСтр("ru='9. Взаимозачет с операцией Взаимозачет'"));
	Запрос.УстановитьПараметр("КомментарийПрочийВзаимозачет",
	...
	Если ДокументФормирования <> Неопределено Тогда
		...
		Пока ВыборкаЗаписейУСН.Следующий() Цикл
			...
			Если ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКредитам И 
					ВыборкаЗаписейУСН.СуммаКомиссииДокумента > 0 Тогда
				НоваяСтрокаУСНКомиисия = ДокументФормирования.ЗаписиКУДиР.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаУСНКомиисия, НоваяСтрокаУСН);
				НоваяСтрокаУСНКомиисия.ДоходБаза = 0;
				...
				НоваяСтрокаУСНКомиисия.РасходБаза = ВыборкаЗаписейУСН.СуммаКомиссииДокумента;
				НоваяСтрокаУСНКомиисия.РасходВсего = ВыборкаЗаписейУСН.СуммаКомиссииДокумента;
				НоваяСтрокаУСНКомиисия.Содержание = НСтр("ru = 'Комиссия банка за продажи в кредит'");
			КонецЕсли;
			
			Если ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты И 
					ВыборкаЗаписейУСН.СуммаКомиссииДокумента <> 0 Тогда
				НоваяСтрокаУСНКомиисия = ДокументФормирования.ЗаписиКУДиР.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаУСНКомиисия, НоваяСтрокаУСН);
				НоваяСтрокаУСНКомиисия.ДоходБаза = 0;
				...
				Если ВыборкаЗаписейУСН.СуммаКомиссииДокумента < 0 Тогда
					НоваяСтрокаУСНКомиисия.Содержание = НСтр("ru = 'Компенсация комиссии банка за операции эквайринга'");
				Иначе
					НоваяСтрокаУСНКомиисия.Содержание = НСтр("ru = 'Комиссия банка за операции эквайринга'");
				КонецЕсли;
			КонецЕсли;
...
Процедура ВыполнитьФормированиеПоПрочимРасходам(Организация, ПериодФормирования, Результат, МенеджерВТ)
	...
	Запрос.УстановитьПараметр("ЕНП", Справочники.ВидыНалогов.ЕдиныйНалоговыйПлатеж);
...
Функция ВыполнитьРасчетЕдиногоНалога(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено, ФиксироватьСтатусСобытия=Истина) Экспорт
	...
	УпрощенныйЗачетФиксированныхВзносов = РегламентированнаяОтчетностьУСН.УпрощенныйЗачетФиксированныхВзносов(Организация, КонецДня(ПериодРасчета));
	...
	Если СтруктураРасчетаЕдиногоНалога.ОбъектНалогообложения = Перечисления.ВидыОбъектовНалогообложения.Доходы Тогда
		...
		КонецДня(ПериодРасчета), СтруктураРасчетаЕдиногоНалога); 
		
		НалоговыйВычет = ВзносыПодлежащиеУплате;
	КонецЕсли;  	
	
	Если СтруктураРасчетаЕдиногоНалога.ОбъектНалогообложения = Перечисления.ВидыОбъектовНалогообложения.ДоходыМинусРасходы Тогда
...
Функция ВыполнитьРасчетАвансовогоПлатежа(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если НЕ УчетВалютныхОпераций ИЛИ НациональнаяВалюта = ВалютаУчета Тогда
		...
		|			Организация = &Организация
		|				И ВидНалога В (&ВидыВзаиморасчетов)
		|				И СрокУплаты В (&СрокиОплатыПоЕНС)) КАК РасчетыПоНалогамОбороты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Иначе
		...
		|	И РасчетыПоНалогам.ВидНалога В(&ВидыВзаиморасчетов)
		|	И РасчетыПоНалогам.СрокУплаты В(&СрокиОплатыПоЕНС)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Если СтруктураРасчетаАвансовогоПлатежа.ОбъектНалогообложения = Перечисления.ВидыОбъектовНалогообложения.ДоходыМинусРасходы Тогда
...
Процедура РассчитатьСуммуПлатежа(НачалоПериода, ОкончаниеПериода, Организация, СтруктураДанных, ЭтоАвансовыйПлатеж = Истина)
	...
	Пока Выборка.Следующий() Цикл 
		...
		Если НалоговаяБазаЗаГод > ГраницаДоходовСПревышения Тогда 
	...
	Если СтруктураДекларацииПоУСН.ДокументОтчетности <> Неопределено Тогда
		СтруктураДекларацииПоУСН.ПараметрыФормыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			...
			Если (ПериодРасчета >= '20140101' И СтруктураДекларацииПоУСН.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2009Кв1")
				...
				ИЛИ (ПериодРасчета >= '20231231' И СтруктураДекларацииПоУСН.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2021Кв1") Тогда
	...
	Если СтруктураРасчетПоСтраховымВзносам.ДокументОтчетности <> Неопределено Тогда
		СтруктураРасчетПоСтраховымВзносам.ПараметрыФормыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			...
			Если (ПериодРасчета >= '20200101' И СтруктураРасчетПоСтраховымВзносам.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2017Кв1")
				...
				ИЛИ (ПериодРасчета >= '20231231' И СтруктураРасчетПоСтраховымВзносам.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2023Кв1") Тогда
			// Форма документа отчетности не соответствует периоду формирования
...
Функция ВыполнитьРасчетВзносовВПФРиФСС(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	ИначеЕсли Год(ПериодРасчета) >= 2023 Тогда	
		...
		Если УпрощенныйЗачетФиксированныхВзносов Или ГодовойРасчет Тогда
			...
			СтруктураРасчетаВзносовВПФРиФСС.ЕдиныйТарифЗаСебя = ВзносыПоЕдиномуТарифуПодлежащиеУплате;
		Иначе
...
Функция ВзносыИППодлежащиеУплате(Организация, НачалоПериода, КонецПериода, СтруктураРасчетаНалога = Неопределено)
	...
	ВзносПоЕдиномуТарифу = РассчитатьФиксированныеСтраховыеВзносы(КонецПериода, Организация).ЕдиныйТарифВсего;
	ВзносСДоходов = ВзносыСДоходовПодлежащиеУплате(Организация, КонецПериода).Всего;
	...
	УчитыватьПСН = (КонецПериода >= Дата(2021, 1, 1))
	...
	ВзносыПодлежащиеУплате = (ВзносПоЕдиномуТарифу + ВзносСДоходов) * КоэффициентСпецРежим;   
	...
	Возврат Окр(ВзносыПодлежащиеУплате - ЗачтеноПоЗаявлениямЗаТекущийГод, 0);
...
Функция КоэффициентСпецРежим(Организация, Период, Периодичность, УчитыватьПСН)
	
	Результат = 0;
	
	Если УчитыватьПСН Тогда
		Результат = ДоляДоходовПатентнойСистемыНалогообложения(Организация, Период, Периодичность);
	КонецЕсли;
...
Функция УпрощенныйЗачетФиксированныхВзносов(Организация, ПериодСобытия, УчитыватьПереходныйПериод = Истина) Экспорт
	...
	ПлательщикНДФЛ = Организация.ИПИспользуетТрудНаемныхРаботников;
...
Функция ВзносыСДоходовПодлежащиеУплате(Организация, НалоговыйПериод, СобытиеКалендаря = Неопределено, ПредварительныйРасчет = Ложь) Экспорт
	...
	СтруктураРасчетаВзносовВПФРиФСС = НоваяСтруктураРасчетаВзносовСДохода(Организация,
	КонецГода(ПериодПрошлыйГод), Неопределено);            
	ВыполнитьРасчетВзносовВПФРПриДоходахСвыше300тр(Организация,
	КонецГода(ПериодПрошлыйГод), Неопределено, СтруктураРасчетаВзносовВПФРиФСС);
...
//  Число
//
Функция ВзносСДоходовПодлежащийУплатеЗаПериод(Организация, НалоговыйПериод, ПериодСобытия, СобытиеКалендаря = Неопределено, ПредварительныйРасчет = Ложь,
	ТекущийГод = Ложь) Экспорт
	...
	ЭтоРасчетВзносаИП = СобытиеКалендаря <> Неопределено И ТипЗнч(СобытиеКалендаря) = Тип("СправочникСсылка.КалендарьПодготовкиОтчетности")
	И (СобытиеКалендаря.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыПриДоходахСвыше300ТР
	...
	Если (ЗначениеЗаполнено(СрокУплаты) И ВзносСДоходовПодлежитУплатеВТекущемПериоде(НалоговыйПериод, СрокУплаты))
		...
		СтруктураРасчетаВзносовВПФРиФСС = НоваяСтруктураРасчетаВзносовСДохода(Организация,
				ПериодСобытия, СобытиеКалендаря);
		...
		Для Каждого ТекущийПериод Из ПериодыУплатыВзносаСДоходов Цикл
			...
			ВыполнитьРасчетВзносовВПФРПриДоходахСвыше300тр(Организация,
				ДатаОкончания, СобытиеКалендаря, СтруктураРасчетаВзносовВПФРиФСС); 
		...
		Если ПериодСобытия < ДатаПереходаНаЕНС И Результат > 0 Тогда
			...
			ИначеЕсли Не ТекущийГод Тогда
				...
				ВзносСДоходовПрошлыйГод = ВзносыСДоходовПодлежащиеУплате(Организация, ПрошлыйГод).ЗаПредыдущийГод;
	...
	// Проверка предельной величины взноса
			ЭтоПредельнаяСуммаВзноса = (СтруктураРасчетаВзносовВПФРиФСС.ПФРСвыше300ТР
				>= СтруктураРасчетаВзносовВПФРиФСС.МаксимальнаяСуммаСтраховыхВзносов);
			СтруктураРасчетаВзносовВПФРиФСС.ПФРСвыше300ТР = ?(ЭтоПредельнаяСуммаВзноса,
				СтруктураРасчетаВзносовВПФРиФСС.МаксимальнаяСуммаСтраховыхВзносов,
	...
	// запись состояние события календаря
			Если СобытиеКалендаря <> Неопределено ИЛИ (СобытиеКалендаря <> Неопределено И Результат = 0) И НЕ ПредварительныйРасчет Тогда
				...
				СтруктураРасчетаВзносовВПФРиФСС.ПФРСвыше300ТР - СтруктураРасчетаВзносовВПФРиФСС.НачисленоСНачалаГода));  			
				
				ОтразитьЗначенияПоказателейОтчетности(СтруктураРасчетаВзносовВПФРиФСС); 
				
				//запись для упрощенного только за год
				Если Не УпрощенныйЗачетФиксированныхВзносов(Организация, ПериодСобытия) ИЛИ 
					КонецКвартала(НалоговыйПериод) = КонецГода(НалоговыйПериод)  Тогда  
...
Функция ВзносыПодлежащиеУплатеИспользованныеДляУменьшенияНалога(Организация, Период, РасчетДля_2023_Года = Ложь) Экспорт
	...
	Если РассчитыватьУСН Тогда
		Результат.УСН_Доход = ВзносыИспользованныеДляУменьшенияУСН(Организация, Период);
	КонецЕсли;
...
//
Функция ВзносыИспользованныеДляУменьшенияУСН(Организация, Период) Экспорт
	...
	УчтенныеВзносыПодлежащиеУплате = Неопределено;
...
Функция ВзносыПоЕдиномуТарифуПодлежащиеУплате(Организация, НалоговыйПериод, ОплатаПатента = Ложь) Экспорт
	...
	Возврат РазмерыСтраховыхВзносов(НалоговыйПериод, Организация).ЕдиныйТариф;
...
//			статус того, что событие рассчитано
//
Процедура ВыполнитьРасчетВзносовВПФРПриДоходахСвыше300тр(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено, СтруктураРасчетаВзносовВПФРиФСС) Экспорт
...
Функция ПоказателиРасходовСтраховыеВзносы(Организация, Патент, ДатаНачала, ДатаОкончания, СтруктураРасчетаПатент, СобытиеКалендаря) Экспорт
	...
	Если УпрощенныйЗачетФиксированныхВзносов(Организация, Срок) Тогда
		...
		ВзносыПодлежащиеУплатеИспользованныеДляУменьшенияНалога(Организация, Срок);
...
Процедура ВыполнитьФормированияЗаписейПатента(Патент) Экспорт
	...
	|ГДЕ
	|	РасходСоСчетаРасшифровкаПлатежа.Ссылка.Дата МЕЖДУ &НачалоПериода И &ОкончаниеПериода
	|	И РасходСоСчетаРасшифровкаПлатежа.Ссылка.Проведен
	|	И РасходСоСчетаРасшифровкаПлатежа.Ссылка.Организация = &Организация
	|	И РасходСоСчетаРасшифровкаПлатежа.ПризнакАванса
	|	И РасходСоСчетаРасшифровкаПлатежа.Документ.Патент = &Патент
	...
	|ГДЕ
	|	РасходИзКассыРасшифровкаПлатежа.Ссылка.Дата МЕЖДУ &НачалоПериода И &ОкончаниеПериода
	|	И РасходИзКассыРасшифровкаПлатежа.Ссылка.Проведен
	|	И РасходИзКассыРасшифровкаПлатежа.Ссылка.Организация = &Организация
	|	И РасходИзКассыРасшифровкаПлатежа.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Покупателю)
	|	И РасходИзКассыРасшифровкаПлатежа.ПризнакАванса
	|	И РасходИзКассыРасшифровкаПлатежа.Документ.Патент = &Патент
	|	И НЕ РасходИзКассыРасшифровкаПлатежа.Ссылка.УчитыватьВНУ
	|	И ВТДенежныеДокументыНалогообложениеВТаблице.ВидОперации ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура ПолучитьВТПредоплатаПоЧекамККМ(МенеджерВременныхТаблиц, НачалоПериода, ОкончаниеПериода)
	...
	|	И ОтчетОРозничныхПродажах.КассаККМ.ТипКассы = ЗНАЧЕНИЕ(Перечисление.ТипыКассККМ.ФискальныйРегистратор)
	|	И (ОтчетОРозничныхПродажах.ПоложениеНастроекНалоговогоУчета = ЗНАЧЕНИЕ(Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти)
	|			ИЛИ НЕ ОтчетОРозничныхПродажах.УчитыватьВНУ
