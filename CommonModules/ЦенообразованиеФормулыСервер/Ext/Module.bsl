...
//
Процедура РассчитатьДанныеКоллекции(КоллекцияНоменклатуры, ТаблицаОперандов, ПараметрыРасчета,
	ИспользуетсяТекущееЗначение = Ложь, ЭтоКоллекцияСИндексами = Ложь) Экспорт
	
	Перем КурсыВалютДокумента;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыРасчета, "КурсыВалютДокумента") Тогда
		КурсыВалютДокумента = ПараметрыРасчета.КурсыВалютДокумента;
	КонецЕсли;
	
	СоответствиеОперандов		= Новый Соответствие;
	ТранспонироватьТаблицыКоллекцийИОперандов(КоллекцияНоменклатуры, ТаблицаОперандов, СоответствиеОперандов,
		ИспользуетсяТекущееЗначение, ЭтоКоллекцияСИндексами);
	
	Если ПараметрыРасчета.Свойство("СоответствиеБазовыхЦен") Тогда
		
		Для каждого ЭлементСоответствия Из ПараметрыРасчета.СоответствиеБазовыхЦен Цикл
		
			СтрокаКоллекции = КоллекцияНоменклатуры.Найти(ЭлементСоответствия.Ключ, "ИдентификаторСтроки");
			Если СтрокаКоллекции <> Неопределено Тогда
				
				Для каждого ЭлементСтруктуры Из ЭлементСоответствия.Значение Цикл
					
					ИмяПоляЗначения = "Значение_" + ЭлементСтруктуры.Ключ;
					Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаКоллекции, ИмяПоляЗначения) Тогда
						СтрокаКоллекции[ИмяПоляЗначения] = ЭлементСтруктуры.Значение.ЗначениеЦены;
					КонецЕсли;
					
					ИмяПоляКурс = "Курс_" + ЭлементСтруктуры.Ключ;
					ИмяПоляКратность = "Кратность_" + ЭлементСтруктуры.Ключ;
						
					Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыРасчета, "ВалютаЦеныРасчетной")
						И ЭлементСтруктуры.Значение.ВалютаЦены <> ПараметрыРасчета.ВалютаЦеныРасчетной 
						И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаКоллекции, ИмяПоляКурс)
						И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаКоллекции, ИмяПоляКратность) 
						И КурсыВалютДокумента <> Неопределено Тогда

						СтрокаКоллекции[ИмяПоляКурс] = КурсыВалютДокумента[ЭлементСтруктуры.Значение.ВалютаЦены].Курс;
						СтрокаКоллекции[ИмяПоляКратность] = КурсыВалютДокумента[ЭлементСтруктуры.Значение.ВалютаЦены].Кратность;
	...
	РасчетныеДанные.Вставить("Цена");
	РасчетныеДанные.Вставить("ВидЦен");
	РасчетныеДанные.Вставить("Курс");
	РасчетныеДанные.Вставить("Кратность");
	РасчетныеДанные.Вставить("ЕдиницаИзмерения");
	РасчетныеДанные.Вставить("ОшибкаРасчета");
	РасчетныеДанные.Вставить("ТекстОшибки");
	...
	Пока КоличествоЗаписей > 0 Цикл
		...
		Если СтрокаКоллекции.Цена = 0 Тогда
			...
			РасчетныеДанные.ВидЦен			= СтрокаКоллекции.ВидЦен;
			
			ПодготовитьДанныеСтрокиКоллекции(СоответствиеОперандов, СтрокаКоллекции, ТаблицаОперандов, РасчетныеДанные);

			РасчетДанныхПоФормуле(СтрокаКоллекции.Формула, СоответствиеОперандов, РасчетныеДанные,
				ОкруглятьВБольшуюСторону, ПравилоОкругления);
...
Процедура ИсправитьИдентификаторФормул(ВидыЦен, Идентификатор) Экспорт
	...
	Если НайтиВидЦенПоИдентификатору(Идентификатор, ВидыЦен).ИдентификаторЗанят Тогда
		...
		СформироватьНовыйИдентификаторВидаЦен(Идентификатор, ВидыЦен.Наименование);
...
Процедура ПарсингФормулыНаОперанды(ПериодЗаписи, Формула, Операнды, КэшФормул = Неопределено)
	...
	Пока КоличествоОперандов > 0 Цикл
		...
		Результат		= НайтиВидЦенПоИдентификатору(Идентификатор, Неопределено, КэшФормул);
...
Функция ПолучитьТаблицуОперандовФормулы(ПериодЗаписи, ХранилищеФормул, КэшФормул = Неопределено) Экспорт
	...
	Если ТипЗнч(ХранилищеФормул) = Тип("Строка") Тогда
		...
		ПарсингФормулыНаОперанды(ПериодЗаписи, ХранилищеФормул, Операнды, КэшФормул);
		
	ИначеЕсли ТипЗнч(ХранилищеФормул) = Тип("Массив") Тогда
		
		Для каждого ЭлементМассива Из ХранилищеФормул Цикл
			
			ПарсингФормулыНаОперанды(ПериодЗаписи, ЭлементМассива, Операнды, КэшФормул);
...
Функция НайтиВидЦенПоИдентификатору(Идентификатор, ИсключаяСсылку = Неопределено, КэшФормул = Неопределено) Экспорт
	
	Результат = Неопределено;
	Если КэшФормул <> Неопределено Тогда
		Если КэшФормул.Свойство(Идентификатор, Результат) Тогда
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Справочник.ВидыЦен.Ссылка КАК ВидЦен,
	|	ИСТИНА КАК ЭтоЦеныНоменклатуры
	|ГДЕ
	|	Справочник.ВидыЦен.ИдентификаторФормул = &Идентификатор
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Справочник.ВидыЦенКонтрагентов.Ссылка,
	|	ЛОЖЬ
	|ГДЕ
	|	Справочник.ВидыЦенКонтрагентов.ИдентификаторФормул = &Идентификатор");
	...
	Если НЕ КэшФормул = Неопределено Тогда
		
		КэшФормул.Вставить(Идентификатор, Результат);
...
Процедура РасчетДанныхПоФормуле(Знач ФормулаСтрокой, СтруктураОперандов, РасчетныеДанные, ОкруглятьВБольшуюСторону = Неопределено, ПравилоОкругления = Неопределено)
	...
	Если РасчетныеДанные = Неопределено Тогда
		...
		РасчетныеДанные = Новый Структура;
		РасчетныеДанные.Вставить("Цена", 0);
		РасчетныеДанные.Вставить("Курс", 1);
		РасчетныеДанные.Вставить("ВидЦен", Справочники.ВидыЦен.ПустаяСсылка());
		РасчетныеДанные.Вставить("Кратность", 1);
		РасчетныеДанные.Вставить("ЕдиницаИзмерения", Неопределено);
		РасчетныеДанные.Вставить("ОшибкаРасчета", Ложь);
		РасчетныеДанные.Вставить("ТекстОшибки", "");
	...
	Попытка
		...
		ПараметрыОкругления = Новый Структура("ОкруглятьВБольшуюСторону, ПравилоОкругления, ПсихологическоеОкругление");
		
		ЦенообразованиеСервер.ЗаполнитьПараметрыОкругленияЦены(РасчетныеДанные.ВидЦен, РассчитаннаяЦена, ПараметрыОкругления);

		Если ЗначениеЗаполнено(РассчитаннаяЦена)
			И ЗначениеЗаполнено(ПараметрыОкругления.ОкруглятьВБольшуюСторону)
			И ЗначениеЗаполнено(ПараметрыОкругления.ПравилоОкругления) Тогда
			
			РасчетныеДанные.Цена = ОкруглитьЧислоПоПравилу(РассчитаннаяЦена, ПараметрыОкругления.ОкруглятьВБольшуюСторону, ПараметрыОкругления.ПравилоОкругления);
...
Процедура ТранспонироватьТаблицыКоллекцийИОперандов(КоллекцияНоменклатуры, ТаблицаОперандов, СоответствиеОперандов, ИспользуетсяТекущееЗначение, ЭтоКоллекцияСИндексами)
	...
	Если КоллекцияНоменклатуры.Колонки.Найти("ИдентификаторСтроки") = Неопределено Тогда
		
		КоллекцияНоменклатуры.Колонки.Добавить("ИдентификаторСтроки", ОбщегоНазначения.ОписаниеТипаЧисло(10));
		
	КонецЕсли; 
	
	Если КоллекцияНоменклатуры.Колонки.Найти("Рассчитывать") = Неопределено Тогда
		
		КоллекцияНоменклатуры.Колонки.Добавить("Рассчитывать", Новый ОписаниеТипов("Булево"));
		КоллекцияНоменклатуры.ЗаполнитьЗначения(Истина, "Рассчитывать");
		
	КонецЕсли;
	
	Если КоллекцияНоменклатуры.Колонки.Найти("ЗначениеЦеныСтарой") = Неопределено Тогда
		
		КоллекцияНоменклатуры.Колонки.Добавить("ЗначениеЦеныСтарой", ОбщегоНазначения.ОписаниеТипаЧисло(10, 2));
	...
	ШаблонИмениПараметра=
		...
		|	,%1.Курс КАК Курс_%1
		|	,%1.ВидЦен.ВалютаЦены КАК ВалютаЦены_%1
	...
	"ВЫБРАТЬ
	|	КоллекцияЗначенийПараметра.ВидЦен КАК ВидЦен,
	...
	|	,КоллекцияНоменклатуры.ИдентификаторСтроки
	|	,КоллекцияНоменклатуры.Рассчитывать
	|	,КоллекцияНоменклатуры.ЗначениеЦеныСтарой
