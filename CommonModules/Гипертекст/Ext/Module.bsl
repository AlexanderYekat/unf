...
Процедура ПеренестиКартинкиВСоответствие(ТекстHTML, Соответствие) Экспорт
	...
	Для Каждого Картинка Из МассивSRCКартинок Цикл
		...
		ИначеЕсли Не СтрНачинаетсяС(Картинка, "e1c") И Не СтрНачинаетсяС(Картинка, "http") Тогда
...
Функция HTMLСТегамиHtmlИBody(ТекстHTML) Экспорт
	...
	Если Не КонецТега Тогда
		ТекстHTML = СтрШаблон("%1%2</html>", ТекстHTML);
		КонецТега = СтрНайти(ТекстHTML, "</html");
	КонецЕсли;
	
	ПраваяКавычка = СтрНайти(ТекстHTML, ">", , ЕстьТегHtml);
	
	Возврат СтрШаблон(
	"<html><body>%1</body></html>%1%2",
	Сред(ТекстHTML, ПраваяКавычка + 1,
	КонецТега - ПраваяКавычка - 1));
