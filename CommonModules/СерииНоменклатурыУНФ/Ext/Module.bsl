...
Процедура УстановитьНастройкиСерийПоУмолчаниюГИСМ(Объект) Экспорт
	...
	Если НЕ (Объект.ВидМаркировки = Перечисления.ВидыМаркировки.НеМаркируется
...
Функция ПолучитьСерииНоменклатурыИзХранилища(Объект, АдресЗапасовВХранилище, КлючСвязи, ПараметрыИменаПолей=Неопределено) Экспорт
	...
	Если ПараметрыИменаПолей<>Неопределено И ТипЗнч(ПараметрыИменаПолей)=Тип("Структура") Тогда
		...
		КонецЕсли;
		Если ПараметрыИменаПолей.Свойство("СерииДоКорректировки") Тогда
			СерииДоКорректировки=ПараметрыИменаПолей.СерииДоКорректировки;
		КонецЕсли;
		Если ПараметрыИменаПолей.Свойство("ЭтоПеремещение") Тогда
			ЭтоПеремещение=ПараметрыИменаПолей.ЭтоПеремещение;
		КонецЕсли;
		Если ПараметрыИменаПолей.Свойство("ТипДокумента") Тогда
			ТипОбъекта=ПараметрыИменаПолей.ТипДокумента;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого СтрокаЗапасов Из СтрокиЗапасов Цикл
		...
		ИначеЕсли СерииДоКорректировки Тогда
			СтрокаЗапасов.СерииНоменклатурыДоКорректировки = СтроковоеПредставлениеСерийНоменклатуры;
		ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.МаркировкаТоваровГИСМ") Тогда
			СтрокаЗапасов.СерийныеНомера = СтроковоеПредставлениеСерийНоменклатуры;
		Иначе
	...
	Если ТипОбъекта = Тип("ДокументСсылка.ЗаказПокупателя") 
		И Объект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		
		Если ИмяТЧЗапасы = "Материалы" Тогда
			ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, "Материалы", ДополнительныеСвойства,"СерииНоменклатурыМатериалы");
		ИначеЕсли СерииДоКорректировки Тогда
			ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект,, ДополнительныеСвойства, "СерииНоменклатурыДоКорректировки");
		Иначе
			ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект,, ДополнительныеСвойства);
		КонецЕсли;
		
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ПересортицаЗапасов") Тогда
		
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект,, ДополнительныеСвойства);
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект,, ДополнительныеСвойства,"СерииНоменклатурыОприходование");
		
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.СборкаЗапасов") Тогда
		
		Если ИмяТЧЗапасы = "Продукция" Тогда
			ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, "Продукция", ДополнительныеСвойства,"СерииНоменклатурыПродукция");
		Иначе
			ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект,, ДополнительныеСвойства);
		КонецЕсли;
		
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ОтчетОПереработке")
		ИЛИ ТипОбъекта = Тип("ДокументСсылка.ОтчетПереработчика") Тогда
		
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, "Продукция", ДополнительныеСвойства);
		
	Иначе
		Если ИмяТЧЗапасы = "Подарки" Тогда
			ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, "Подарки", ДополнительныеСвойства,"СерииПодарков");
		Иначе
			ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, ИмяТЧЗапасы, ДополнительныеСвойства, ИмяТЧСерииНоменклатуры);
		КонецЕсли;
		...
		Если ЭтоПеремещение Тогда
			...
			ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, , ДополнительныеСвойства, ИмяТЧСерииНоменклатуры);
		КонецЕсли;
...
Функция ПараметрыПодбораСерийНоменклатуры(ДокОбъект, УИДФормы, ИДСтроки, РежимПодбора = Неопределено, ИмяТЧ = "Запасы", ИмяТЧСерийНоменклатуры = "СерииНоменклатуры", ИмяПоляКлючСвязи = "КлючСвязи",
	...
	Если РежимПодбора=Неопределено И ИспользоватьСерииНоменклатурыОстатки() = Истина Тогда
		РежимПодбора = Истина;
	...
	Если ТипЗнч(ДокОбъект.Ссылка) = Тип("ДокументСсылка.РасходнаяНакладная")
		И ДокОбъект.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером Тогда
		ПараметрыСерийНоменклатуры.Вставить("ИгнорироватьКонтрольОстатков", Истина);
	КонецЕсли;
	...
	ДанныеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекСтрокаДанные.Номенклатура, "ВидМаркировки, ВидПродукцииИС, ИспользоватьСерииНоменклатуры");
	Если ИнтеграцияГИСМ.ПодсистемаНеИспользуется() Тогда
		ИспользоватьНомерСерии = ДанныеНоменклатуры.ВидПродукцииИС = Перечисления.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха
			И ДанныеНоменклатуры.ИспользоватьСерииНоменклатуры;
		ИспользоватьRFIDМеткиСерии = ИспользоватьНомерСерии;
	Иначе
		ИспользоватьНомерСерии = ДанныеНоменклатуры.ВидМаркировки = Перечисления.ВидыМаркировки.МаркируемаяПродукция;
		ИспользоватьRFIDМеткиСерии = ДанныеНоменклатуры.ВидМаркировки = Перечисления.ВидыМаркировки.МаркируемаяПродукция
			ИЛИ ДанныеНоменклатуры.ВидМаркировки = Перечисления.ВидыМаркировки.КонтрольныйИдентификационныйЗнак;
	КонецЕсли;
	
	НастройкиСерий = Новый Структура;
	НастройкиСерий.Вставить("ИспользоватьНомерСерии", ИспользоватьНомерСерии);
	НастройкиСерий.Вставить("ИспользоватьRFIDМеткиСерии", ИспользоватьRFIDМеткиСерии);
...
Функция ПодготовитьПараметрыСерийНоменклатуры(ДокОбъект, ТекСтрокаДанные, УИДФормы, ИмяТЧ = "Запасы", ИмяТЧСерийНоменклатуры = "СерииНоменклатуры", ИмяПоляКлючСвязи = "КлючСвязи",
	...
	Если ИмяТЧСерийНоменклатуры = "СерииНоменклатурыДоКорректировки" Тогда
		ИмяКолонкиСтатус = "ЗапасыСтатусыСерийНоменклатурыДоКорректировки";
	Иначе
...
//  ИмяТабличнойЧасти - Строка - Строковое представление табличной части с номенклатурой
//  КлючСвязи - Число - ключ связи строк
//  ИмяТабличнойЧастиСерии - Строка - Имя табличной части с сериями
//
Процедура ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, ИмяТабличнойЧасти = "Запасы", 
	ДополнительныеСвойства = Неопределено, ИмяТабличнойЧастиСерии = "СерииНоменклатуры") Экспорт
	...
	ЗаказНарядВыполнен = Ложь;
	Если ЭтоЗаказПокупателя И Объект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		ЗаказНарядВыполнен = СостоянияЗаказов.ЭтоСостояниеВыполненияЗаказНаряда(Объект.СостояниеЗаказа, 
			Объект.ВидЗаказа);
	КонецЕсли;
	ПоРезерву = ЭтоЗаказПокупателя И НЕ ЗаказНарядВыполнен И ПолучитьФункциональнуюОпцию("РезервированиеЗапасов") 
		И ПолучитьФункциональнуюОпцию("ПередачаТоваровМеждуОрганизациями");
	...
	Иначе
		...
		ТипДокумента = ОбщегоНазначения.СтроковоеПредставлениеТипа(ТипЗнч(Объект.Ссылка));
	...
	ИначеЕсли ИмяТабличнойЧастиСерии = "СерииНоменклатурыДоКорректировки" Тогда
		ИмяПоляСтатуса = "СтатусыСерийНоменклатурыДоКорректировки";
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаНоменклатурыДокумента.Количество КАК Количество", "ТаблицаНоменклатурыДокумента.КоличествоДоКорректировки КАК Количество");
	Иначе
	...
	ИмяПоляКоличество = ?(Не ИмяТабличнойЧастиСерии = "СерииНоменклатурыДоКорректировки", "Количество", "КоличествоДоИзменения");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(НайденнаяСтрока.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") Тогда
			КоэффициентЕдиницыИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НайденнаяСтрока.ЕдиницаИзмерения, 
				"Коэффициент", Истина);
		КонецЕсли;
		
		Если ПоРезерву И НайденнаяСтрока.Резерв <> 0 Тогда
			Количество = ?(НайденнаяСтрока.Резерв < 0, -НайденнаяСтрока.Резерв, НайденнаяСтрока.Резерв);
		Иначе
			Количество = ?(НайденнаяСтрока[ИмяПоляКоличество] < 0, -НайденнаяСтрока[ИмяПоляКоличество], НайденнаяСтрока[ИмяПоляКоличество]);
		КонецЕсли;
...
Функция СтатусСерийНоменклатурыПриИзменении(СтруктураДанных) Экспорт
	...
	Если ПараметрыПодбораСтатуса.ТипДокумента = "Заказ-наряд" Тогда
		ТипДокумента = ПараметрыПодбораСтатуса.ТипДокумента;
	Иначе
		ТипДокумента = ОбщегоНазначения.СтроковоеПредставлениеТипа(ПараметрыПодбораСтатуса.ТипДокумента);
	КонецЕсли;
...
Процедура ЗаполнитьСерииНоменклатурыВНаличии(ДокОбъект, ИмяТЧЗапасы="Запасы", ИмяТЧСерииНоменклатуры="СерииНоменклатуры") Экспорт
	...
	ЗаполнитьКлючиСвязи(ДокОбъект, ИмяТЧЗапасы);
...
// Устарела. См. СерииНоменклатурыУНФ.ПроверитьЗаполнениеСерийНоменклатуры
...
//
Процедура ПроверкаЗаполненияСерийНоменклатуры(Отказ, Знач Запасы, Знач СерииНоменклатуры, СтруктурнаяЕдиница, 
	Знач ВхФорма, ИмяПоляКлючСвязи="КлючСвязи", ИмяПоляНоменклатура = "Номенклатура", 
	ИмяПоляСтруктурнаяЕдиница = "СтруктурнаяЕдиница") Экспорт
	
	Если ТипЗнч(ВхФорма) = Тип("Структура") Тогда 
		ДополнительныеПараметры = ОбщегоНазначения.СкопироватьРекурсивно(ВхФорма);
	Иначе
		ДополнительныеПараметры = Новый Структура;
		Если ВхФорма <> Неопределено Тогда
			ДополнительныеПараметры.Вставить("Объект", ВхФорма);
		КонецЕсли;
	КонецЕсли;
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		ДополнительныеПараметры.Вставить("ИмяПоляКлючСвязи", ИмяПоляКлючСвязи);
		ДополнительныеПараметры.Вставить("ИмяПоляНоменклатура", ИмяПоляНоменклатура);
		ДополнительныеПараметры.Вставить("ИмяПоляСтруктурнаяЕдиница", ИмяПоляСтруктурнаяЕдиница);
	КонецЕсли;
	ПроверитьЗаполнениеСерийНоменклатуры(Отказ, Запасы, СерииНоменклатуры, ДополнительныеПараметры);
...
// 
// Параметры:
//  Отказ - Булево - Признак удавшейся проверки
//  Запасы - ТабличнаяЧасть - Табличная часть "Запасы"
//  СерииНоменклатуры - ТабличнаяЧасть - Список серий
//  ДополнительныеПараметры - Структура - Дополнительные параметры проверки:
//		*Объект - ДокументОбъект - Объект проверяемого документа
//		*СтруктурнаяЕдиница - СправочникСсылка.СтруктурныеЕдиницы - Структурная единица
//		*ИмяПоляКлючСвязи - Строка - Имя поля ключа связи
//		*ИмяПоляНоменклатура - Строка - Имя поля с номенклатурой
//		*ИмяПоляСтруктурнаяЕдиница - Строка - Имя поля с структурной единицей
//		*ПоРезерву - Булево - Признак проверки количества серий по резерву
Процедура ПроверитьЗаполнениеСерийНоменклатуры(Отказ, Знач Запасы, Знач СерииНоменклатуры, 
	Знач ДополнительныеПараметры) Экспорт
	
	ЭтоОрдер = ?(ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры, "ЭтоОрдер"), Истина, Ложь);
	
	СтруктурнаяЕдиница = Неопределено;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры, "СтруктурнаяЕдиница") Тогда
		СтруктурнаяЕдиница = ДополнительныеПараметры.СтруктурнаяЕдиница;
	КонецЕсли; 
	ИмяПоляКлючСвязи = "КлючСвязи";
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры, "ИмяПоляКлючСвязи") 
		И НЕ ПустаяСтрока(ДополнительныеПараметры.ИмяПоляКлючСвязи) Тогда
		ИмяПоляКлючСвязи = ДополнительныеПараметры.ИмяПоляКлючСвязи;
	КонецЕсли;
	ИмяПоляНоменклатура = "Номенклатура";
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры, "ИмяПоляНоменклатура") 
		И НЕ ПустаяСтрока(ДополнительныеПараметры.ИмяПоляНоменклатура) Тогда
		ИмяПоляНоменклатура = ДополнительныеПараметры.ИмяПоляНоменклатура;
	КонецЕсли;
	ИмяПоляСтруктурнаяЕдиница = "СтруктурнаяЕдиница";
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры, "ИмяПоляСтруктурнаяЕдиница") 
		И НЕ ПустаяСтрока(ДополнительныеПараметры.ИмяПоляСтруктурнаяЕдиница) Тогда
		ИмяПоляСтруктурнаяЕдиница = ДополнительныеПараметры.ИмяПоляСтруктурнаяЕдиница;
	КонецЕсли;
	ПоРезерву = Ложь;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры, "ПоРезерву") 
		И ТипЗнч(ДополнительныеПараметры.ПоРезерву) = Тип("Булево") Тогда
		ПоРезерву = ДополнительныеПараметры.ПоРезерву;
	КонецЕсли;
	...
	Если ИспользованиеСерийНоменклатуры = Истина Тогда
		...
		РеквизитыНоменклатурыСоответствие = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(
			Запасы.ВыгрузитьКолонку(ИмяПоляНоменклатура), 
			"ИспользоватьСерииНоменклатуры, ВидМаркировки, ИспользоватьУникальныеСерии");
		...
		Если Не СтруктурнаяЕдиница = Неопределено И ПолучитьФункциональнуюОпцию("ИспользоватьОрдерныйСклад") Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры, "Объект") Тогда
				ЕстьСкладыВТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				ДополнительныеПараметры.Объект, "ПоложениеСклада");
				
			Если ЕстьСкладыВТабличнойЧасти 
				И ДополнительныеПараметры.Объект.ПоложениеСклада = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
				РеквизитыСкладовСоответствие = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
				Запасы.ВыгрузитьКолонку(ИмяПоляСтруктурнаяЕдиница), "ОрдерныйСклад");
			Иначе
			...
			Иначе
				ЕстьСкладыВТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				ДополнительныеПараметры, "ПоложениеСклада");
				
				Если ЕстьСкладыВТабличнойЧасти 
					И ДополнительныеПараметры.ПоложениеСклада = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
					РеквизитыСкладовСоответствие = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
					Запасы.ВыгрузитьКолонку(ИмяПоляСтруктурнаяЕдиница), "ОрдерныйСклад");
				Иначе
					ОрдерныйСклад = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктурнаяЕдиница, "ОрдерныйСклад");
		...
		МассивКонтроляПоСериям = МассивКонтроляПоСериям(Запасы, ДополнительныеПараметры, ИмяПоляНоменклатура, 
			СтруктурнаяЕдиница);
		...
		Для Каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если НЕ ЭтоОрдер И ОрдерныйСклад = Истина 
			...
			Если ИспользоватьСерииНоменклатуры Тогда
				...
				Если ПоРезерву И СтрокаЗапасы.Резерв <> 0 Тогда
					СтрокаЗапасыКоличество = СтрокаЗапасы.Резерв * Коэффициент;
				Иначе
					СтрокаЗапасыКоличество = СтрокаЗапасы.Количество * Коэффициент;
				КонецЕсли;
				
				Если СтрокаЗапасыКоличество < 0 Тогда
					СтрокаЗапасыКоличество = -СтрокаЗапасыКоличество;
				КонецЕсли;
				...
				Если ИспользоватьУникальныеСерии Тогда
					...
					Если ОтборСерииНоменклатуры.Количество() <> Макс(СтрокаЗапасыКоличество, -СтрокаЗапасыКоличество) Тогда
						...
						Если ПоРезерву И СтрокаЗапасы.Резерв <> 0 Тогда
							ТекстСообщения = НСтр("ru = '[%ИмяТЧ%][Управление остатками серий]%Склад% Число серий номенклатуры отличается от резервируемого количества в строке %Номер%.'");
						Иначе
				...
				Иначе
					...
					Если Не СтрокаЗапасыКоличество = ЧислоНомеровВСтроке Тогда
						...
						ИмяТЧ = Сред(Строка(Запасы), СтрНайти(Строка(Запасы), ".", НаправлениеПоиска.СКонца) + 1);
						Если ПоРезерву И СтрокаЗапасы.Резерв <> 0 Тогда
							ТекстСообщения = НСтр("ru = '[%ИмяТЧ%][Управление остатками серий]%Склад% Число серий номенклатуры в строке %Номер% не равно резерву.'");
						Иначе
	...
	Если ИспользованиеСерийНоменклатуры <> Неопределено Тогда
		...
		ВыполнитьКонтрольДублей(Отказ, Запасы, СерииНоменклатуры, ДополнительныеПараметры, ИмяПоляКлючСвязи);
...
Процедура ЗаполнитьПризнакиИспользованияСерий(Объект, ДополнительныеСвойства = Неопределено) Экспорт
	...
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
		...
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект,, ДополнительныеСвойства);
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, "Материалы", 
			ДополнительныеСвойства, "СерииНоменклатурыМатериалы");
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.СборкаЗапасов") Тогда
		...
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект,, ДополнительныеСвойства);
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, "Продукция", 
			ДополнительныеСвойства, "СерииНоменклатурыПродукция");
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЧекККМ") Тогда
		...
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект,, ДополнительныеСвойства);
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, "Подарки", ДополнительныеСвойства, 
			"СерииПодарков")
		
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда 
		
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект,, ДополнительныеСвойства);
		ОбновитьСтатусыСерийНоменклатурыВТабличнойЧасти(Объект, "ЗапасыВозвраты", ДополнительныеСвойства, 
			"СерииНоменклатурыВозвраты")
