...
Функция КадровыеДанныеОсновногоСотрудникаФизическогоЛица(Организация, ФизическоеЛицо, ИменаПолей, ДатаПолучения = '00010101', ТолькоРазрешенные = Истина, ТекстОшибки = "") Экспорт
	...
	СоздатьВТСотрудникиОрганизации(МенеджерВременныхТаблиц, ТолькоРазрешенные, ПараметрыПолучения);
...
Процедура ЗаполнитьДокументыФизическихЛицПоКлассификаторуМВД() Экспорт
	...
	ОписатьДокумент(, НСтр("ru='Паспорт иностранного гражданина'"),												"10", НСтр("ru='ИНПАСПОРТ'"));		// -
	// -
...
Процедура ЗаполнитьНаименованиеФИОПол(ФизическоеЛицо, Знач ФИО, Знач ПериодДанных = Неопределено)
	...
	ФизическоеЛицо.ФИО = ФамилияИмяОтчество.Фамилия + " " + ФамилияИмяОтчество.Имя + " "
	...
	Если Не ЗначениеЗаполнено(ФизическоеЛицо.Пол) Тогда
		ФизическоеЛицо.Пол = СотрудникиКлиентСервер.ОпределитьПолПоОтчеству(ФамилияИмяОтчество.Отчество);
	КонецЕсли;
