...
//   ПолучатьРеквизитыРасчетов - Булево - Признак необходимости получения дополнительных реквизитов движений для
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПоставщиками(МенеджерВременныхТаблиц, СЗачетомАванса, НомерЗапроса, ПолучатьРеквизитыРасчетов = Ложь) Экспорт
	...
	Иначе
		...
		|ГДЕ
		|	(ТаблицаДокумента.СуммаКурсовойРазницы <> 0
		|			ИЛИ ТаблицаДокумента.СуммаКурсовойРазницыРег <> 0)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок,
		|	НомерСтроки
...
//   ПолучатьРеквизитыРасчетов - Булево - Признак необходимости получения дополнительных реквизитов движений для
//                                        расчетов с контрагентами
//   РеквизитыРасчетовСоответствие - Соответствие - Временное соответствие дополнительных реквизитов движений для
//                                                  расчетов с контрагентами
//
// Возвращаемое значение:
//   Строка - текст запроса с временными таблицами курсовых разниц
...
// Возвращает текст запроса для получения курсовых разниц при расчетах с покупателями при возвратах.
//
// Параметры:
//   МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - Менеджер временных таблиц запроса инициализации данных документа
//   СЗачетомАванса - Булево - Признак необходимости отражения зачета авансов
...
//   Строка - текст запроса с временными таблицами курсовых разниц
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПокупателямиВозвраты(МенеджерВременныхТаблиц, СЗачетомАванса, НомерЗапроса, ПолучатьРеквизитыРасчетов = Ложь, РеквизитыРасчетовСоответствие = Неопределено) Экспорт 
	
	РассчитыватьКурсовыеРазницы = ПолучитьНеобходимостьРасчетаКурсовыхРазниц(МенеджерВременныхТаблиц, "ВременнаяТаблицаРасчетыСПокупателямиВозвраты");
	...
	Если НЕ РассчитыватьКурсовыеРазницы Тогда
		...
		|	ТаблицаРасчеты.Валюта КАК Валюта,
		|	ТаблицаРасчеты.СчетУчета КАК СчетУчета
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницРасчетыСПокупателямиВозвраты
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПокупателямиВозвраты КАК ТаблицаРасчеты
		|ГДЕ
		|	ЛОЖЬ
		|;
		|
		...
		|	&РеквизитыРасчетов
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПокупателямиВозвраты КАК ТаблицаДокумента
		|
		|УПОРЯДОЧИТЬ ПО
		|	ТаблицаДокумента.СодержаниеПроводки,
		|	Документ,
	...
	ИначеЕсли СЗачетомАванса Тогда
		...
		|		КОНЕЦ КАК СуммаРегОстаток
		|	ИЗ
		|		ВременнаяТаблицаРасчетыСПокупателямиВозвраты КАК ВременнаяТаблица
		|	ГДЕ
		|	ВременнаяТаблица.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|				(Организация, Контрагент, Договор, Документ, Заказ, ТипРасчетов) В
		|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты.Организация,
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты.Контрагент,
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты.Договор,
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты.Документ,
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты.Заказ,
		|						ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг)
		|					ИЗ
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты)) КАК ТаблицаОстатки
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ТаблицаРасчеты.Валюта КАК Валюта,
		|	ТаблицаРасчеты.СчетУчета КАК СчетУчета
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницРасчетыСПокупателямиВозвратыПредварительная
		|ИЗ
		|	ВременнаяТаблицаРасчетыСПокупателямиВозвраты КАК ТаблицаРасчеты
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаОстаткиПослеПроведения КАК ТаблицаОстатки
		|		ПО ТаблицаРасчеты.Организация = ТаблицаОстатки.Организация
		...
		|				Валюта В
		|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты.Договор.ВалютаРасчетов
		|					ИЗ
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты)) КАК КурсыВалютРасчетовСрезПоследних
		|		ПО ТаблицаРасчеты.Валюта = КурсыВалютРасчетовСрезПоследних.Валюта
		|ГДЕ
		|	(ТаблицаОстатки.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг)
		|			ИЛИ ЕСТЬNULL(ТаблицаОстатки.СуммаВалОстаток, 0) = 0)
		...
		|		ВЫРАЗИТЬ(&КурсоваяРазница КАК СТРОКА(100)) КАК СодержаниеПроводки
		|	ИЗ
		|		ВременнаяТаблицаКурсовыхРазницРасчетыСПокупателямиВозвратыПредварительная КАК ТаблицаДокумента
		|	ГДЕ
		|		ТаблицаДокумента.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВременнаяТаблицаКурсовыхРазницРасчетыСПокупателямиВозвратыПредварительная";
	...
	Иначе
		...
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты.Документ,
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты.Заказ,
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты.ТипРасчетов
		|					ИЗ
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты)) КАК ТаблицаОстатки
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|				Валюта В
		|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты.Валюта
		|					ИЗ
		|						ВременнаяТаблицаРасчетыСПокупателямиВозвраты)) КАК КурсыВалютРасчетовСрезПоследних
		|		ПО ТаблицаРасчеты.Валюта = КурсыВалютРасчетовСрезПоследних.Валюта
		|ГДЕ
		...
		|	&РеквизитыРасчетовНеопределено
		|ИЗ
		|	ВременнаяТаблицаКурсовыхРазницРасчетыСПокупателямиВозвраты КАК ТаблицаДокумента
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок,
		|	НомерСтроки
...
Функция ПолучитьТекстЗапросаКурсовыеРазницыДенежныеСредстваВКассахККМ(МенеджерВременныхТаблиц, НомерЗапроса) Экспорт
	...
	Если РассчитыватьКурсовыеРазницы Тогда
		...
		|	&ДвижениеЧекаВОткрытойСмене КАК ДвижениеЧекаВОткрытойСмене,
		|	ТаблицаДокумента.Сумма КАК Сумма,
		|	ТаблицаДокумента.СуммаВал КАК СуммаВал,
		|	ТаблицаДокумента.СодержаниеПроводки КАК СодержаниеПроводки
		|ИЗ
...
//
// Параметры:
//   ТаблицаДвижений - ТаблицаЗначений - Таблица в которую необходимо добавить дополнительные поля для отражения
//                                       передачи товаров
...
// Помещает движения по курсовой разницы из таблицы ТаблицаДенежныеСредства в таблицу ТаблицаДвиженияДенежныхСредств.
//
// Параметры:
//   СтруктураДополнительныеСвойства - Структура - Данные проведения документа
//
Процедура ДополнитьДвиженияДенежныхСредствКурсовойРазницей(СтруктураДополнительныеСвойства) Экспорт
	
	Если НЕ Константы.ФункциональнаяУчетВалютныхОпераций.Получить() Тогда
		Возврат;
	КонецЕсли;
	
	Если Константы.ЧастотаРасчетаКурсовыхРазниц.Получить() <> Перечисления.ЧастотаРасчетаКурсовыхРазниц.ВМоментВыполненияОпераций Тогда
		Возврат;
	КонецЕсли;
	
	Для каждого СтрокаДенежныхСредств Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаДенежныеСредства Цикл
		
		Если (СтрокаДенежныхСредств.Статья = Справочники.СтатьиДвиженияДенежныхСредств.ОтрицательнаяКурсоваяРазница
			Или СтрокаДенежныхСредств.Статья = Справочники.СтатьиДвиженияДенежныхСредств.ПоложительнаяКурсоваяРазница)
			И (СтрокаДенежныхСредств.Аналитика = ПланыСчетов.Управленческий.ПрочиеРасходы
			Или СтрокаДенежныхСредств.Аналитика = ПланыСчетов.Управленческий.ПрочиеДоходы) Тогда
			
			НоваяСтрока = СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаДвиженияДенежныхСредств.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаДенежныхСредств);
			
			НоваяСтрока.СценарийПланирования = Справочники.СценарииПланирования.Фактический;
			
			Если СтрокаДенежныхСредств.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
				НоваяСтрока.СуммаПриход = СтрокаДенежныхСредств.Сумма;
				НоваяСтрока.СуммаПриходВал = СтрокаДенежныхСредств.СуммаВал;
			Иначе
				НоваяСтрока.СуммаРасход = СтрокаДенежныхСредств.Сумма;
				НоваяСтрока.СуммаРасходВал = СтрокаДенежныхСредств.СуммаВал;
			КонецЕсли;
