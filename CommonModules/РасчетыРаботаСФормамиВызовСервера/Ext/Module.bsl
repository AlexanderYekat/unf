...
// Функция возвращает параметры диалога включения учета по нескольким валютам
// 
// Возвращаемое значение:
//  Структура - Параметры диалога
//
Функция ПараметрыДиалогаВключенияУчетаПоВалютам() Экспорт
	
	ПараметрыДиалога = Новый Структура;
	
	Если Пользователи.РолиДоступны("ПолныеПрава") Тогда
		
		ТекстСообщения = НСтр("ru='Для выбора валюты необходимо включить опцию учета по нескольким валютам.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(Истина, НСтр("ru='Перейти в настройки'"));
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='В программе отключена возможность использования нескольких валют. 
	...
	ПараметрыДиалога.Вставить("ТекстСообщения", ТекстСообщения);
	ПараметрыДиалога.Вставить("Кнопки", Кнопки);
	ПараметрыДиалога.Вставить("ЭтоВопрос", ЭтоВопрос);
	
	Возврат ПараметрыДиалога;
	
КонецФункции

// Функция возвращает способ разнесения платежа для договора
//
// Параметры:
//  Договор				 - СправочникСсылка.ДоговорыКонтрагентов	 - Договор по которому требуется получить способ разнесения
//  Контрагент			 - СправочникСсылка.Контрагенты	 - Владелец договора
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.СпособыЗачетаИРаспределенияПлатежей - Способ разнесения платежа по договору
//
Функция СпособРазнесенияОплатыДляДоговора(Договор, Контрагент) Экспорт
	
	Способ = Перечисления.СпособыЗачетаИРаспределенияПлатежей.ПустаяСсылка();
	
	Если ЗначениеЗаполнено(Контрагент) И ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
		
		ВестиУчетПоДоговорам = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ВестиРасчетыПоДоговорам");
		
		Если ВестиУчетПоДоговорам И ЗначениеЗаполнено(Договор) Тогда
			СпособИзДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "СпособРазнесенияОплатыПоУмолчанию");
			Если ЗначениеЗаполнено(СпособИзДоговора) Тогда
				Способ = СпособИзДоговора;
			Иначе
				Способ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "СпособРазнесенияОплатыПоУмолчанию");
			КонецЕсли;
		Иначе
			Способ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "СпособРазнесенияОплатыПоУмолчанию");
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Способ) Тогда
		Способ = ПолучитьСпособРазнесенияОплатыПоУмолчанию();
	КонецЕсли;
...
// Функция возвращает способ разнесения оплаты для контрагента
//
// Параметры:
//  Контрагент	 - СправочникСсылка.Контрагенты	 - Контрагент для получения данных
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.СпособыЗачетаИРаспределенияПлатежей - Способ разнесения оплаты по контрагенту
//
Функция СпособРазнесенияОплатыДляКонтрагента(Контрагент) Экспорт
...
// Функция определяет признак доступности элементов формы, связанных с печатью чека
//
// Параметры:
//  пФорма	 					 - Форма  - форма, видимость элементов которой необходимо определить
//  пНапечататьЧекВидимость 	 - Булево - признак видимости элементов формы, связанных с выполнением пробития и печати чека
//  пВидОперацииПодразумеваетЧек - Булево - Признак видимости реквизитов формы, связанных с данными чека
// 
Процедура УстановитьВидимостьЭлементовПечатиЧека(пФорма, пНапечататьЧекВидимость, пВидОперацииПодразумеваетЧек = Истина) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ГруппаФискальныйЧек", "Видимость", пВидОперацииПодразумеваетЧек);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "НомерЧекаККМ", "Видимость", пВидОперацииПодразумеваетЧек);
...
Процедура УстановитьЗаголовкиКолонокТабличныхЧастей(пФорма, пВалютаСсылка) Экспорт
	...
	пФорма.Элементы.РасшифровкаПлатежаСуммаПлатежа.Заголовок = СтрШаблон(НСтр("ru = 'Сумма %1'"), ПредставлениеВалюты);
...
Процедура УстановитьСвязьПараметровВыбораПоОрганизации(пФорма) Экспорт
	...
	Для каждого ЭлементФормы Из Элементы Цикл
		...
		Если ЭлементФормы.ПутьКДанным = "Объект.РасшифровкаПлатежа.ДокументПланирования"
			...
			ИЛИ ЭлементФормы.ПутьКДанным = "Объект.РасшифровкаПлатежа.Договор"
			Тогда
...
Функция ПолучитьДанныеРасшифровкаПлатежаДоговорПриИзменении(
	...
	Если Не Договор.Пустая() И ТипЗнч(Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		СтруктураДанные.Вставить("СпособЗачета", СпособРазнесенияОплатыДляДоговора(Договор, Договор.Владелец));
	Иначе
...
Функция ПолучитьДанныеОбОплатеПоДокументуОтгрузки(СсылкаНаДокумент, ФормироватьПоДвижениям, ЭтоРасчетыСПоставщиком) Экспорт
	...
	// Сценарии для 5 и 6
	// 1. Заказ покупателя - Оплата с указанием заказа - Зачет оплаты в накладной на основании заказа. Отработает запрос 3,
	// 5 (сторно 3) и 6. 2. Заказ покупателя - Оплата БЕЗ указанием заказа - Зачет оплаты в накладной на основании заказа.
	// Отработает запрос 6.
	
	Запрос = Новый Запрос;
	ЗапросТекстФрагментНачало = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	1 КАК НомерЗапроса,
		|	РасчетыСПокупателями.Период КАК Период,
		|	РасчетыСПокупателями.Регистратор КАК Регистратор,
		...
		|	ТИПЗНАЧЕНИЯ(РасчетыСПокупателями.Документ) = ТИП(Документ.ЧекККМ) КАК ЭтоЧек,
		|	ЛОЖЬ КАК ЭтоВозврат
		|ПОМЕСТИТЬ ВременнаяТаблицаОбъединение
		|ИЗ
		|	РегистрНакопления.РасчетыСПокупателями КАК РасчетыСПокупателями
		...
		|	ТИПЗНАЧЕНИЯ(РасчетыСПокупателями.Регистратор) = ТИП(Документ.ЧекККМ),
		|	ЛОЖЬ
		|ИЗ
		|	РегистрНакопления.РасчетыСПокупателями КАК РасчетыСПокупателями
		|ГДЕ
	...
	ЗапросТекстФрагментВозвраты = "
		...
		|	РасчетыСПоставщиками.СпособЗачета,
		|	РасчетыСПоставщиками.СуммаПлатежа,
		|	РасчетыСПоставщиками.Договор.ВалютаРасчетов,
		|	РасчетыСПоставщиками.ИндексСтрокиДокумента,
		|	РасчетыСПоставщиками.СуммаНДС,
		|	РасчетыСПоставщиками.СтавкаНДС,
		|	ЛОЖЬ,
		|	РасчетыСПоставщиками.Курс,
		|	ЛОЖЬ,
		|	РасчетыСПоставщиками.Кратность,
		|	РасчетыСПоставщиками.ВалютаПлатежа,
		|	ЛОЖЬ,
		|	ИСТИНА
		|ИЗ
		|	РегистрНакопления.РасчетыСПоставщиками КАК РасчетыСПоставщиками
		|ГДЕ
		|	РасчетыСПоставщиками.Регистратор = &СсылкаНаДокумент
		|	И ТИПЗНАЧЕНИЯ(РасчетыСПоставщиками.Регистратор) = ТИП(Документ.РасходнаяНакладная)
	...
	ЗапросТекстФрагментОкончание = 
		...
		|	ВременнаяТаблицаОбъединение.ЭтоВозврат КАК ЭтоВозврат
		|ИЗ
		|	ВременнаяТаблицаОбъединение КАК ВременнаяТаблицаОбъединение
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ВременнаяТаблицаОбъединение.ЭтоВозврат
		|
		|ИМЕЮЩИЕ
		|	СУММА(ВременнаяТаблицаОбъединение.СуммаРасчетов) > 0
		|
	...
	Запрос.УстановитьПараметр("СсылкаНаДокумент", СсылкаНаДокумент);
	Запрос.УстановитьПараметр("ФормироватьПоДвижениям", ФормироватьПоДвижениям);
	Запрос.УстановитьПараметр("Возврат", Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику);
	
	Если ЭтоРасчетыСПоставщиком Тогда
		ЗапросТекстФрагментНачало = СтрЗаменить(ЗапросТекстФрагментНачало, "РасчетыСПокупателями",
			"РасчетыСПоставщиками");
		ЗапросТекстФрагментОкончание = СтрЗаменить(ЗапросТекстФрагментОкончание, "РасчетыСПокупателями",
			"РасчетыСПоставщиками");
		ЗапросТекстФрагментВозвраты = СтрЗаменить(ЗапросТекстФрагментВозвраты, "РасчетыСПоставщиками",
			"РасчетыСПокупателями");
		ЗапросТекстФрагментВозвраты = СтрЗаменить(ЗапросТекстФрагментВозвраты, "РасходнаяНакладная",
			"ПриходнаяНакладная");
		Запрос.УстановитьПараметр("Возврат", Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя);
	КонецЕсли;
	
	Запрос.Текст = ЗапросТекстФрагментНачало + ЗапросТекстФрагментВозвраты + ЗапросТекстФрагментОкончание;
	...
	Если Объект.Контрагент.ВестиУчетОплатыПоСчетам Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.Заказ) И 
			...
			Если ТекущиеДанные.СуммаРасчетов = 0 Тогда
				...
				Если ВалютаДенежныхСредств = ВыборкаОплата.ВалютаЗаказа Тогда
				...
				ИначеЕсли ВалютаДенежныхСредств = ВыборкаОплата.ВалютаЗаказа Тогда
					ТекущиеДанные.СуммаПлатежа = ВыборкаОплата.ОплаченоВалДок;
				...
				РассчитатьСуммуНДСНаСервере(ТекущиеДанные);
...
Функция УстановитьВидимостьРеквизитовРасчетовФрагмент(пФорма) Экспорт
	...
	Если НЕ СтруктураРеквизитыРасчетов.ВестиРасчетыПоДоговорам Тогда
...
Процедура ЗаполнитьРеквизитыРасчетов(пФорма, пСтруктураПараметровРасчетов, пВариантЗаполненияРасшифровки, ЭтоЗачетПредоплаты = Ложь) Экспорт
	...
	Если НЕ ЭтоЗачетПредоплаты Тогда
		...
		Иначе
			...
			пВариантЗаполненияРасшифровки = ПолучитьВариантЗаполненияРасшифровкиДляДокументаОплаты(
...
Процедура УстановитьУсловноеОформлениеРасчетов(пФорма, пВидОперацииВозврат) Экспорт
	...
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", НСтр("ru = '<определяется автоматически>'"));
	...
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
		"Объект.РасшифровкаПлатежа.СчетНаОплату", Ложь, ВидСравненияКомпоновкиДанных.НеЗаполнено);
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "РасшифровкаПлатежаСчетНаОплату");
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Доступность", Ложь);
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", НСтр("ru = '<указывается только для способа Вручную>'"));
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветТекста", ЦветаСтиля.ТекстВторостепеннойНадписи);
	
	НовоеУсловноеОформление = пФорма.УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
		"Объект.РасшифровкаПлатежа.СпособЗачета",
		Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто,
		ВидСравненияКомпоновкиДанных.Равно);
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
		"Объект.РасшифровкаПлатежа.СчетНаОплату", Ложь, ВидСравненияКомпоновкиДанных.Заполнено);
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "РасшифровкаПлатежаСчетНаОплату");
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Доступность", Ложь);
	...
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "РасшифровкаПлатежаДокумент");
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", НСтр("ru = '<предоплата>'"));
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветТекста", ЦветаСтиля.ТекстВторостепеннойНадписи);
	...
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "РасшифровкаПлатежаДокумент");
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", НСтр("ru = '<предоплата с зачетом>'"));
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветТекста", ЦветаСтиля.ТекстВторостепеннойНадписи);
	
	УстановитьУсловноеОформлениеРасчетовПоДаннымКонтрагента(пФорма, пВидОперацииВозврат);
	УстановитьУсловноеОформлениеРасчетовПоЧеку(пФорма);
...
Процедура УстановитьУсловноеОформлениеРасчетовПоДаннымКонтрагента(пФорма, пВидОперацииВозврат) Экспорт
	...
	Если пФорма.Объект.Контрагент.ВестиРасчетыПоДокументам
		...
		УстановитьАвтоОтметкуНезаполненногоДляПлатежногоДокумента(пФорма);
	Иначе
...
Процедура УстановитьУсловноеОформлениеРасчетовПоЧеку(пФорма) Экспорт
	Если НЕ типЗнч(пФорма.Объект.Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
		
		НовоеУсловноеОформление = пФорма.УсловноеОформление.Элементы.Добавить();

		НовоеУсловноеОформление = пФорма.УсловноеОформление.Элементы.Добавить();
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.Заказ", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.Документ", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.ПризнакАванса", Истина, ВидСравненияКомпоновкиДанных.Равно);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.ПризнакСпособаРасчета", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
		РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "РасшифровкаПлатежаПризнакСпособаРасчета");
		РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", НСтр("ru = '<аванс>'"));
		РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветТекста", ЦветаСтиля.ТекстВторостепеннойНадписи);
		
		НовоеУсловноеОформление = пФорма.УсловноеОформление.Элементы.Добавить();
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.Заказ", , ВидСравненияКомпоновкиДанных.Заполнено);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.Документ", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.ПризнакАванса", Истина, ВидСравненияКомпоновкиДанных.Равно);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.ПризнакСпособаРасчета", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
		РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "РасшифровкаПлатежаПризнакСпособаРасчета");
		...
		НовоеУсловноеОформление = пФорма.УсловноеОформление.Элементы.Добавить();
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.Заказ", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.Документ", , ВидСравненияКомпоновкиДанных.Заполнено);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.ПризнакАванса", Истина, ВидСравненияКомпоновкиДанных.Равно);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.ПризнакСпособаРасчета", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
		РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "РасшифровкаПлатежаПризнакСпособаРасчета");
		РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", НСтр("ru = '<предоплата>'"));
		...
		НовоеУсловноеОформление = пФорма.УсловноеОформление.Элементы.Добавить();
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.ПризнакАванса", ложь, ВидСравненияКомпоновкиДанных.Равно);
		РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор, "Объект.РасшифровкаПлатежа.ПризнакСпособаРасчета", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
		РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, "РасшифровкаПлатежаПризнакСпособаРасчета");
		РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Текст", НСтр("ru = '<оплата кредита>'"));
		РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветТекста", ЦветаСтиля.ТекстВторостепеннойНадписи);
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуПросмотраНаСервере(пФорма, пПоДвижениям = Ложь, ИмяТЧ = "Предоплата", ИменаЭлементов = Неопределено) Экспорт
	...
	ПоДвижениям = ФормироватьТаблицуПросмотраПоДвижениямВопрос(
...
Процедура РаспределитьСуммуПлатежаПослеРаспределенияЗавершение(пФорма, ИмяТЧ = "Предоплата", ИменаЭлементов = Неопределено) Экспорт
	...
	Если пФорма.Элементы[ИменаЭлементов.СтраницыРасчетовСКонтрагентом].ТекущаяСтраница <> пФорма.Элементы[ИменаЭлементов.СтраницаПросмотр] Тогда
		...
		ВариантЗаполненияРасшифровкиПриИзмененииНаСервере(пФорма, Ложь, ИмяТЧ, ИменаЭлементов);
	КонецЕсли;
...
Процедура ВзаиморасчетыДеньгиПриСозданииНаСервере(пФорма) Экспорт
	...
	ЛожьЭтоЗачет = Ложь;
	пФорма.СпособЗачетаПоУмолчаниюДляКонтрагента = ПолучитьСпособЗачетаДляКонтрагента(пФорма.Объект.Контрагент, ЛожьЭтоЗачет);
	
	пФорма.ДатаПриСозданииНаСервере = пФорма.Объект.Дата;
	ЗаполнитьРеквизитыРасчетов(пФорма, пФорма.СтруктураПараметровРасчетов, пФорма.ВариантЗаполненияРасшифровки);
	
	пФорма.ИсходнаяСуммаРавнаНулю = (пФорма.Объект.СуммаДокумента = 0);
	
	ОбработатьОбщиеРеквизитыРасчетов(пФорма);
	пФорма.Элементы.ДекорацияНадписьДляРежимаАвто.Заголовок = ПолучитьЗаголовокДляРежимаАвто();
...
Процедура НастройкаИтоговЗавершениеНаСервере(Форма, ПоказыватьПанельИтогов = Истина) Экспорт
	...
	ИначеЕсли Форма.ВариантОтображенияИтогов = ПредопределенноеЗначение("Перечисление.ВариантыОтображенияИтоговБанкИКасса.ВедомостьЗаПериод") Тогда
		...
		Элементы.ДекорацияВедомостьПоВалюте.Заголовок = НСтр("ru = 'Ведомость по валюте: %1'",
			Форма.ОтборВалютаВедомости);
		...
		Если Форма.ИмяФормы = "Обработка.БанкИКасса.Форма.ФормаСписка" Тогда
			ИмяОтборПериод = "ОтборПериодБанковскиеВыписки";
		Иначе
			ИмяОтборПериод = "ОтборПериод";
		КонецЕсли;
			
		Если НЕ ЗначениеЗаполнено(Форма[ИмяОтборПериод].ДатаНачала) И НЕ ЗначениеЗаполнено(Форма[ИмяОтборПериод].ДатаОкончания) Тогда
			Форма.ТаблицаИтогов[0].Показатель = НСтр("ru = 'За все время'");
			Форма.ТаблицаИтогов[3].Показатель = НСтр("ru = 'Остаток'");
		Иначе
			Форма.ТаблицаИтогов[0].Показатель = СтрШаблон(НСтр("ru = 'С %1'"), Формат(Форма[ИмяОтборПериод].ДатаНачала,
				"ДЛФ=D;"));
			Форма.ТаблицаИтогов[3].Показатель = СтрШаблон(НСтр("ru = 'По %1'"), Формат(Форма[ИмяОтборПериод].ДатаОкончания,
				"ДЛФ=D;"));
		КонецЕсли;
	...
	ИначеЕсли Форма.ВариантОтображенияИтогов = ПредопределенноеЗначение("Перечисление.ВариантыОтображенияИтоговБанкИКасса.ОстаткиПересчитанныеВВалюту") Тогда
		...
		Элементы.ОтборКурс.Заголовок = СтрШаблон(НСтр("ru = 'Курс на %1'"), Формат(ТекущаяДатаСеанса(), "ДЛФ=D;"));
		Форма.ОтборКурс = Строка(СтруктураДанных.Курс);
		Форма.ИнформацияСуммаВалКонечныйОстаток = СтруктураДанных.Остаток;
		Элементы.ДекорацияОстатокВВалюте.Заголовок = СтрШаблон(НСтр("ru = 'Остатки в валюте: %1'"),
			Форма.ОтборВалютаОстатков);
...
Функция ПолучитьДанныеПоСчетуКассе(Период, СчетКасса, Валюта = Неопределено) Экспорт
	...
	Результат.Вставить("ИнформацияСуммаВалНачальныйОстаток", 0);
	Результат.Вставить("ИнформацияСуммаВалКонечныйОстаток", 0);
	Результат.Вставить("ИнформацияСуммаВалПриход", 0);
	Результат.Вставить("ИнформацияСуммаВалРасход", 0);
	Результат.Вставить("УчетВалютныхОпераций", ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций"));
	Результат.Вставить("НомерБанковскогоСчета", НомерБанковскогоСчета);
...
Функция ПолучитьДанныеПоПланируемымПлатежам(СсылкаНаДокумент) Экспорт
	...
	Если ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ЗакрытиеПланируемыхОплат") Тогда
		Валюта = Неопределено;
	Иначе
	...
	Результат = Новый Структура;
	Результат.Вставить("СуммаПриходЗаДень", 0);
	Результат.Вставить("СуммаРасходЗаДень", 0);
	Результат.Вставить("СуммаПеремещенияЗаДень", 0);
	Результат.Вставить("ПредставлениеВалюты", ПредставлениеВалюты);
...
// Проверяет необходимость выводить подсказку на формах документов Корректировка поступления и 
// Корректировка реализации для релиза 3.0.2.
// 
// Возвращаемое значение:
//  Булево - Необходимость выводить подсказку на формах документов
//
Функция ПроверитьНеобходимостьВыводитьПодсказкуДляРелиза3_0_2() Экспорт

	ДатаВыходаРелиза3_0_2 = '20221101';
	
	УстановитьПривилегированныйРежим(Истина);
	ДатаСозданияБазы = Константы.ДатаПервогоВходаВСистему.Получить();
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат ДатаСозданияБазы < ДатаВыходаРелиза3_0_2;
	
КонецФункции

// Проверяет возможность использования старого Клиент-банка УНФ.
//  
// Возвращаемое значение:
//  Булево - Возможность использования старого Клиент-банка УНФ
//
Функция ПроверитьВозможностьИспользованияСтарогоКлиентБанкаУНФ() Экспорт

	УстановитьПривилегированныйРежим(Истина);
	Результат = Не Константы.ИспользоватьТолькоНовыйКБ.Получить();
	УстановитьПривилегированныйРежим(Ложь);
...
// Проверяет возможность использования нового Клиент-банка.
//  
// Возвращаемое значение:
//  Булево - Использование Клиент-банка БП
//
Функция ПроверитьИспользованиеКлиентБанкаБП() Экспорт

	Если ПроверитьВозможностьИспользованияСтарогоКлиентБанкаУНФ() Тогда
		УстановитьПривилегированныйРежим(Истина);
		Результат = Не Константы.ИспользоватьСтаруюВерсиюКлиентБанкаУНФ.Получить();
		УстановитьПривилегированныйРежим(Ложь);
...
// Выполняет контроль превышения суммы оплаты над суммой документа в строке ТЧ
// 
// Параметры:
// 	пФорма - УправляемаяФорма - Форма документа
// 	ИдентификаторСтроки - Число - Идентификатор текущей строки таблицы Предоплата
// 	СуммаДокумента - Число - Сумма документа
// 	ИмяТЧ - Строка - Имя табличной части
//
Процедура ВыполнитьКонтрольСуммыОплатыВСтроке(пФорма, ИдентификаторСтроки, СуммаДокумента,
	ИмяТЧ = "Предоплата") Экспорт
	...
	СтрокаТабличнойЧасти = Объект[ИмяТЧ].НайтиПоИдентификатору(ИдентификаторСтроки);
	Если СтрокаТабличнойЧасти = Неопределено Тогда
		Возврат;	
	КонецЕсли;
	
	Если СтрокаТабличнойЧасти.Свойство("ОплатаБонусами") Тогда
		ОплатыБезБонусов = Объект.Предоплата.НайтиСтроки(Новый Структура("ОплатаБонусами", Ложь));
	Иначе
		ОплатыБезБонусов = Неопределено;
	КонецЕсли;	
	
	ТЗПредоплата = Объект[ИмяТЧ].Выгрузить(ОплатыБезБонусов, "СуммаРасчетов");
	ОбщаяСуммаРасчетов = ТЗПредоплата.Итог("СуммаРасчетов");
	ПревышениеОплатыНадСуммойДокумента = ОбщаяСуммаРасчетов - СуммаДокумента; 
	
	Если ПревышениеОплатыНадСуммойДокумента > 0 Тогда
		Если ПревышениеОплатыНадСуммойДокумента >= СтрокаТабличнойЧасти.СуммаРасчетов Тогда
			Объект[ИмяТЧ].Удалить(СтрокаТабличнойЧасти);
		Иначе
			СтрокаТабличнойЧасти.СуммаРасчетов	= СтрокаТабличнойЧасти.СуммаРасчетов - ПревышениеОплатыНадСуммойДокумента;
			ПересчитатьСуммуПлатежа(пФорма, СтрокаТабличнойЧасти);
		КонецЕсли;
		СообщитьОПревышенииСуммыПлатежа();
	КонецЕсли;
	
КонецПроцедуры

// Выполняет контроль превышения суммы оплаты над суммой документа при добавлении строки в ТЧ
// 
// Параметры:
...
//
Процедура ВыполнитьКонтрольСуммыОплатыДобавлениеСтроки(СуммаОплаты, СуммаДокумента, Отказ) Экспорт
...
// Выполняет контроль превышения суммы оплаты над суммой документа в таблице оплат
// 
// Параметры:
// 	пФорма - УправляемаяФорма - Форма документа
// 	СуммаДокумента - Число - Сумма документа
// 	ТаблицаОплат - ТаблицаЗначений - Таблица оплат для загрузки в ТЧ Предоплата
//
Процедура ВыполнитьКонтрольСуммыОплатыВТаблице(пФорма, СуммаДокумента, ТаблицаОплат) Экспорт
	
	Если ПроведениеДокументовУНФ.КонтрольОстатковВыключен() Тогда
		Возврат;	
	КонецЕсли;
	
	Если ТаблицаОплат.Колонки.Найти("ОплатаБонусами") = Неопределено Тогда
		ОбщаяСуммаРасчетов = ТаблицаОплат.Итог("СуммаРасчетов");
	Иначе
		ОплатыБезБонусов = ТаблицаОплат.НайтиСтроки(Новый Структура("ОплатаБонусами", Ложь));
		ОбщаяСуммаРасчетов = ТаблицаОплат.Скопировать(ОплатыБезБонусов, "СуммаРасчетов").Итог("СуммаРасчетов");
	КонецЕсли;
	
	ПревышениеОплатыНадСуммойДокумента = ОбщаяСуммаРасчетов - СуммаДокумента; 
	
	Если ПревышениеОплатыНадСуммойДокумента > 0 Тогда
		Для Итератор = 1 - ТаблицаОплат.Количество() По 0 Цикл
			Индекс = - Итератор;
			СтрокаТабличнойЧасти = ТаблицаОплат[Индекс];
			Если ПревышениеОплатыНадСуммойДокумента >= СтрокаТабличнойЧасти.СуммаРасчетов Тогда
				ПревышениеОплатыНадСуммойДокумента = ПревышениеОплатыНадСуммойДокумента - СтрокаТабличнойЧасти.СуммаРасчетов;
				ТаблицаОплат.Удалить(Индекс);
				Продолжить;
			КонецЕсли;
...
// Сообщает о превышения суммы оплаты над суммой документа
// 
// Параметры:
// 	Отказ - Булево - Признак отказа
//
Процедура СообщитьОПревышенииСуммыПлатежаОтказ(Отказ = Ложь) Экспорт
...
Процедура ЗаполнитьИнформациюОбИтогахПоВалюте(Форма)
	...
	Если НЕ Форма.ОтборВалютаВедомости.Пустая() Тогда
		...
		Если Форма.ИмяФормы = "Обработка.БанкИКасса.Форма.ФормаСписка" Тогда
			СтруктураДанные = ПолучитьДанныеПоВалюте(Форма.ОтборПериодБанковскиеВыписки, Форма.ОтборВалютаВедомости);
		Иначе
...
Функция ПолучитьДанныеПоВалюте(Период, Валюта)
	...
	Результат.Вставить("ИнформацияСуммаВалРасход", 0);
	Результат.Вставить("УчетВалютныхОпераций", ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций"));
	Результат.Вставить("НомерБанковскогоСчета");
...
Процедура УстановитьАвтоОтметкуНезаполненногоДляПлатежногоДокумента(пФорма)
	
	Если пФорма.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика
		ИЛИ пФорма.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПоставщика Тогда
		пФорма.Элементы.РасшифровкаПлатежаДокумент.АвтоОтметкаНезаполненного = Истина;
	КонецЕсли;
	
КонецПроцедуры

Процедура ПересчитатьСуммуПлатежа(пФорма, СтрокаТабличнойЧасти)
	
	Объект = пФорма.Объект;
	СтрокаТабличнойЧасти.СуммаПлатежа = ВалютыУНФКлиентСервер.Пересчитать(
		СтрокаТабличнойЧасти.СуммаРасчетов,
		СтрокаТабличнойЧасти.Курс,
		?(Объект.ВалютаДокумента = пФорма.НациональнаяВалюта, пФорма.КурсНациональнаяВалюта, Объект.Курс),
		СтрокаТабличнойЧасти.Кратность,
		?(Объект.ВалютаДокумента = пФорма.НациональнаяВалюта, пФорма.КратностьНациональнаяВалюта, Объект.Кратность));

КонецПроцедуры

Процедура СообщитьОПревышенииСуммыПлатежа()
	
	СообщитьОПревышенииСуммыПлатежаОтказ();
	ТекстСообщения = НСтр("ru = 'Сумма оплаты откорректирована.'");
	ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
