...
Процедура ЗаполнитьКоличествоНоменклатурыПоКоличествуВЕТИС(ТекущаяСтрока, Суффикс = "") Экспорт
	...
	ПараметрыЗаполнения = ИнтеграцияВЕТИСУНФКлиентСервер.ПараметрыПересчетаКоличества();
	ЗаполнитьЗначенияСвойств(ПараметрыЗаполнения, ТекущаяСтрока);
	ПараметрыЗаполнения.ПересчитатьКоличествоЕдиницПоВЕТИС = Истина;
	ПараметрыЗаполнения.Суффикс = Суффикс;
	
	ВыполнитьОперацииПриИзмененииРеквизитовФормы(, ТекущаяСтрока, Неопределено, ПараметрыЗаполнения);
...
Функция КоэффициентЕдиницыИзмеренияПоВЕТИС(ЕдиницаИзмеренияВЕТИС, Номенклатура) Экспорт
	...
	Результат = ИнтеграцияВЕТИСУНФКлиентСервер.ШаблонДанныхКоэффициентаЕдиницыИзмеренияВЕТИС();
	
	КоэффициентВЕТИС = 1;
	
	Если Не (ЗначениеЗаполнено(ЕдиницаИзмеренияВЕТИС)
	...
	РеквизитыЕдиницыИзмеренияВЕТИС = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		...
		"ЕдиницаИзмерения, БазоваяЕдиницаИзмерения, Коэффициент");
	
	ЕдиницаИзмерения = РеквизитыЕдиницыИзмеренияВЕТИС.ЕдиницаИзмерения;
	
	Если НЕ ЗначениеЗаполнено(ЕдиницаИзмерения)
		И ЗначениеЗаполнено(РеквизитыЕдиницыИзмеренияВЕТИС.БазоваяЕдиницаИзмерения)
		И НЕ ЕдиницаИзмерения = РеквизитыЕдиницыИзмеренияВЕТИС.БазоваяЕдиницаИзмерения Тогда
		РеквизитыБазовойЕдиницыИзмеренияВЕТИС = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			РеквизитыЕдиницыИзмеренияВЕТИС.БазоваяЕдиницаИзмерения,
			"ЕдиницаИзмерения, БазоваяЕдиницаИзмерения, Коэффициент");
		
		ЕдиницаИзмерения = РеквизитыБазовойЕдиницыИзмеренияВЕТИС.ЕдиницаИзмерения;
		КоэффициентВЕТИС = РеквизитыЕдиницыИзмеренияВЕТИС.Коэффициент;
	КонецЕсли;
	...
	РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура,
		"ЕдиницаИзмерения, НаборЕдиницИзмерения, ИспользоватьНаборыЕдиницИзмерения, Вес");
	...
	Если ЕдиницаИзмерения = ЕдиницаИзмеренияНоменклатуры Тогда
	...
	Если ЗначениеЗаполнено(КоэффициентВЕТИС) Тогда
		Результат.Коэффициент = Результат.Коэффициент * КоэффициентВЕТИС;
	КонецЕсли;
	
	ВладелецЕдиницИзмерения = ?(РеквизитыНоменклатуры.ИспользоватьНаборыЕдиницИзмерения,
		РеквизитыНоменклатуры.НаборЕдиницИзмерения,
		Номенклатура);
	
	ДанныеЕдиницыИзмерения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЕдиницаИзмерения, "Код, ТипИзмеряемойВеличины");
	ЭтоВесовойТовар = ДанныеЕдиницыИзмерения.ТипИзмеряемойВеличины = Перечисления.ТипыИзмеряемыхВеличин.Вес;
	
	Результат.ТипИзмеряемойВеличины = ДанныеЕдиницыИзмерения.ТипИзмеряемойВеличины;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ВладелецЕдиницИзмерения", ВладелецЕдиницИзмерения);
	Запрос.УстановитьПараметр("ЕдиницаИзмерения", ЕдиницаИзмерения);
	Запрос.УстановитьПараметр("ЭтоВесовойТовар", ЭтоВесовойТовар);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕдиницыИзмерения.Ссылка КАК Ссылка,
	|	ЕдиницыИзмерения.Коэффициент КАК Коэффициент,
	|	ЕСТЬNULL(ВесИОбъемЕдиницТоваров.Вес, 0) КАК Вес,
	|	ЕдиницыИзмерения.ЕдиницаИзмеренияПоКлассификатору = &ЕдиницаИзмерения КАК ЕдиницыИзмеренияСовпадают
	|ИЗ
	|	Справочник.ЕдиницыИзмерения КАК ЕдиницыИзмерения
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВесИОбъемЕдиницТоваров КАК ВесИОбъемЕдиницТоваров
	|		ПО (ВесИОбъемЕдиницТоваров.ЕдиницаИзмерения = ЕдиницыИзмерения.Ссылка)
	|ГДЕ
	|	ЕдиницыИзмерения.Владелец = &ВладелецЕдиницИзмерения
	|	И (ЕдиницыИзмерения.ЕдиницаИзмеренияПоКлассификатору = &ЕдиницаИзмерения
	|			ИЛИ &ЭтоВесовойТовар)
	|	И НЕ ЕдиницыИзмерения.ПометкаУдаления
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЕдиницыИзмеренияСовпадают УБЫВ,
	|	Вес УБЫВ";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	ВесНоменклатуры = РеквизитыНоменклатуры.Вес;
	Если Выборка.Следующий() Тогда
		Если Выборка.ЕдиницыИзмеренияСовпадают Тогда
			Результат.Коэффициент = Результат.Коэффициент * Выборка.Коэффициент;
			Результат.КэшироватьДанные = Истина;
			
			Возврат Результат;
		ИначеЕсли НЕ ЗначениеЗаполнено(ВесНоменклатуры) Тогда
			ВесНоменклатуры = Выборка.Вес / Выборка.Коэффициент;
		КонецЕсли;
	КонецЕсли;
	
	Если ЭтоВесовойТовар Тогда
		// Это весовой товар.
		ДанныеКлассификатора = Справочники.КлассификаторЕдиницИзмерения.ПолучитьДанныеКлассификатора();
		
		Если НЕ ЗначениеЗаполнено(ВесНоменклатуры) Тогда
			ДанныеЕдиницыИзмеренияНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЕдиницаИзмеренияНоменклатуры,
				"Код, ТипИзмеряемойВеличины");
			Если ДанныеЕдиницыИзмеренияНоменклатуры.ТипИзмеряемойВеличины = Перечисления.ТипыИзмеряемыхВеличин.Вес Тогда
				СтрокаКлассификатораНоменклатуры = ДанныеКлассификатора.Строки.Найти(ДанныеЕдиницыИзмеренияНоменклатуры.Код,
					"КодЧисловой", Истина);
				Если НЕ СтрокаКлассификатораНоменклатуры = Неопределено
					И ЗначениеЗаполнено(СтрокаКлассификатораНоменклатуры.Числитель) И ЗначениеЗаполнено(СтрокаКлассификатораНоменклатуры.Знаменатель) Тогда
					ВесНоменклатуры = СтрокаКлассификатораНоменклатуры.Числитель / СтрокаКлассификатораНоменклатуры.Знаменатель;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		ВесЕдиницыВЕТИС = 0;
		СтрокаКлассификатора = ДанныеКлассификатора.Строки.Найти(ДанныеЕдиницыИзмерения.Код, "КодЧисловой", Истина);
		Если НЕ СтрокаКлассификатора = Неопределено
			И ЗначениеЗаполнено(СтрокаКлассификатора.Числитель) И ЗначениеЗаполнено(СтрокаКлассификатора.Знаменатель) Тогда
			ВесЕдиницыВЕТИС = СтрокаКлассификатора.Числитель / СтрокаКлассификатора.Знаменатель;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ВесНоменклатуры) И ЗначениеЗаполнено(ВесЕдиницыВЕТИС) Тогда
			Результат.Коэффициент = Результат.Коэффициент / ВесНоменклатуры * ВесЕдиницыВЕТИС;
			Результат.КэшироватьДанные = Истина;
...
Процедура ЗаполнитьИнвентаризациюПродукцииВЕТИСНаОснованииРаспределенияЗатрат(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|ПОМЕСТИТЬ ВТ_Серии
	|ИЗ
	|	Документ.РаспределениеЗатрат.СерииНоменклатуры КАК Серии
	|ГДЕ
	|	Серии.Ссылка = &ДокументОснование
	|;
	|
	...
	|		КОГДА ВТ_Серии.Серия ЕСТЬ NULL
	|			ТОГДА ВЫБОР
	|					КОГДА ВТ_Запасы.Партия = ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
	|						ТОГДА НЕОПРЕДЕЛЕНО
	|					ИНАЧЕ ВТ_Запасы.Партия
	|				КОНЕЦ
	|		ИНАЧЕ ВТ_Серии.Серия
	|	КОНЕЦ КАК Серия,
	|	-СУММА(ВЫБОР
...
Процедура ЗаполнитьИнвентаризациюПродукцииВЕТИСНаОснованииПересортицыЗапасов(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|ВЫБРАТЬ
	|	Запасы.Ссылка КАК Ссылка,
	|	Запасы.НоменклатураОприходование КАК Номенклатура,
	|	Запасы.ХарактеристикаОприходование КАК Характеристика,
	|	Запасы.ЕдиницаИзмеренияОприходование КАК ЕдиницаИзмерения,
	|	Запасы.Количество КАК Количество,
	|	Запасы.КлючСвязи КАК КлючСвязи,
	|	Запасы.Партия КАК Партия
	|ПОМЕСТИТЬ ВТ_ЗапасыОприходование
	|ИЗ
	|	Документ.ПересортицаЗапасов.Запасы КАК Запасы
	|ГДЕ
	|	Запасы.Ссылка = &ДокументОснование
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПересортицаЗапасовСерииНоменклатурыОприходование.Ссылка КАК Ссылка,
	...
	|	ПересортицаЗапасовСерииНоменклатурыОприходование.КлючСвязи КАК КлючСвязи
	|ПОМЕСТИТЬ ВТ_СерииОприходование
	|ИЗ
	|	Документ.ПересортицаЗапасов.СерииНоменклатурыОприходование КАК ПересортицаЗапасовСерииНоменклатурыОприходование
	|ГДЕ
	|	ПересортицаЗапасовСерииНоменклатурыОприходование.Ссылка = &ДокументОснование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|
	|ВЫБРАТЬ
	|	ВТ_ЗапасыОприходование.Ссылка,
	|	ВТ_ЗапасыОприходование.Номенклатура,
	|	ВТ_ЗапасыОприходование.Характеристика,
	|	ВЫБОР
	|		КОГДА ВТ_СерииОприходование.Серия ЕСТЬ NULL
	|			ТОГДА ВЫБОР
	|					КОГДА ВТ_ЗапасыОприходование.Партия = ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
	|						ТОГДА НЕОПРЕДЕЛЕНО
	|					ИНАЧЕ ВТ_ЗапасыОприходование.Партия
	|				КОНЕЦ
	|		ИНАЧЕ ВТ_СерииОприходование.Серия
	|	КОНЕЦ,
	|	СУММА(ВЫБОР
	|			КОГДА ВТ_СерииОприходование.Серия ЕСТЬ NULL
	|				ТОГДА ВТ_ЗапасыОприходование.Количество
	|			ИНАЧЕ ВТ_СерииОприходование.Количество
	|		КОНЕЦ * ВЫБОР
	|			КОГДА ВТ_ЗапасыОприходование.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|				ТОГДА ВТ_ЗапасыОприходование.ЕдиницаИзмерения.Коэффициент
	|			ИНАЧЕ 1
	|		КОНЕЦ)
	|ИЗ
	|	ВТ_ЗапасыОприходование КАК ВТ_ЗапасыОприходование
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СерииОприходование КАК ВТ_СерииОприходование
	|		ПО ВТ_ЗапасыОприходование.КлючСвязи = ВТ_СерииОприходование.КлючСвязи
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ЗапасыОприходование.Номенклатура,
	|	ВТ_ЗапасыОприходование.Характеристика,
...
Процедура ЗаполнитьИсходящуюТранспортнуюОперациюВЕТИСНаОснованииРасходнойНакладной(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	Основание.Автор КАК Ответственный,
...
Процедура ЗаполнитьИсходящуюТранспортнуюОперациюВЕТИСНаОснованииОтчетаОПереработке(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|ПОМЕСТИТЬ ВТ_Запасы
	|ИЗ
	|	Документ.ОтчетОПереработке.Продукция КАК Запасы
	|ГДЕ
	|	Запасы.Ссылка = &ДокументОснование
	|	И Запасы.Номенклатура.ПодконтрольнаяПродукцияВЕТИС
	|;
...
Функция СтруктураЗапросаДанныеДокументаСборкаЗапасов()
	...
	|	КОНЕЦ * ВЫБОР
	|		КОГДА ТабличнаяЧасть.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|			ТОГДА ТабличнаяЧасть.ЕдиницаИзмерения.Коэффициент
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК Количество
	...
	|		ПО ТабличнаяЧасть.Ссылка = СборкаЗапасовСерииНоменклатурыПродукция.Ссылка
	|			И ТабличнаяЧасть.КлючСвязи = СборкаЗапасовСерииНоменклатурыПродукция.КлючСвязи
	|ГДЕ
	...
	|		ПО ТабличнаяЧасть.Ссылка = СборкаЗапасовСерииНоменклатуры.Ссылка
	|			И ТабличнаяЧасть.КлючСвязи = СборкаЗапасовСерииНоменклатуры.КлючСвязи
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &ДокументОснование
	|	И СправочникНоменклатура.ПодконтрольнаяПродукцияВЕТИС
	|	И &Отбор2";
