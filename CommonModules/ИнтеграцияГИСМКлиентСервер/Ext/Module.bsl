...
Функция СертификатКриптографииИзКонвертаSOAP(КонвертSOAP) Экспорт
	...
	ДвоичныеДанныеСертификата = Base64Значение(СокрЛП(НайтиВXML(КонвертSOAP, "wsse:BinarySecurityToken", Ложь, 1)));
...
Процедура ДобавитьТекстОшибкиУказанНеКорректныйКППОрганизации(СообщениеXML, КПП, Организация) Экспорт
	...
	ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Указан не корректный КПП организации %1.'"), Организация));
...
Процедура ДобавитьТекстОшибкиНеЗаполненоПолноеНаименованиеКонтрагента(СообщениеXML, Контрагент) Экспорт
	...
	ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Не заполнено полное наименование контрагента %1.'"), Контрагент));
...
Процедура ДобавитьТекстОшибкиНеЗаполненGLNПартнера(СообщениеXML, GLN, Партнер) Экспорт
	...
	Если Не ЗначениеЗаполнено(GLN) Тогда
		ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Не заполнен GLN партнера %1.'"), Партнер));
	Иначе
		ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Указан не корректный GLN партнера %1.'"), Партнер));
	КонецЕсли;
...
Функция РазложитьПоТипамШтрихкодов(Штрихкоды) Экспорт
	...
	Для Каждого СтрМас Из Штрихкоды Цикл
		...
		Если ЭтоНомерКиЗ(СтрМас.Штрихкод) Тогда
