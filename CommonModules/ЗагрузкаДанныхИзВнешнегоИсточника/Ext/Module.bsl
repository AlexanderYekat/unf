...
Процедура ДобавитьОписаниеПрочитанногоФайла(РезультатЧтения, ДанныеДокумента, ПолноеИмяФайла)
	...
	Если ДанныеДокумента.ВидДокумента = ВидДокументаОтчетOzon() Тогда
		...
		АдресТаблицыТоваров = ДанныеДокумента.ПараметрыШапки.АдресТаблицыТоваров;
		ТаблицаТоваров = ПолучитьИзВременногоХранилища(АдресТаблицыТоваров);
		
		СуммаПродаж = ТаблицаТоваров.Итог("Сумма") + ТаблицаТоваров.Итог("Доплата");			
		СуммаВозвратов = ТаблицаТоваров.Итог("СуммаВозвращено") + ТаблицаТоваров.Итог("ДоплатаВозвращено");
		ДатаДок = ДанныеДокумента.ПараметрыШапки.ДатаВходящегоДокумента;
		
		Описание = СтрШаблон(НСтр("ru='Отчет о продажах Ozon от %1
			|Продажи: %2 Возвраты: %3'"), Формат(ДатаДок, "ДЛФ=dd.MM.yyyy"), 
		...
		Результат.Вставить("Дата"			, ДатаДок);
	...
	Иначе
		...
		Описание = СтрШаблон(НСтр("ru='Документ не определен как файл Ozon'"));
		Результат.Вставить("Описание", Описание);
...
Функция ПолучитьДанныеДокумента(ТаблицаДанных, ИмяТаблицы = "", ТекстПрогресса = "", ДелительПрогресса = 1)
	...
	Если Не ПустаяСтрока(ВидДокумента) Тогда
		...
		ДанныеДокумента = ПодготовитьТабличныйДокументОтчетOzon(ТаблицаДанных, Истина, Истина);
...
Процедура СоздатьДокументыИзСтруктуры(СоздаваемыеДокументы, АдресХранилища) Экспорт
	...
	Для Каждого Документ Из СоздаваемыеДокументы Цикл
		...
		Попытка    
			...
			СозданДокумент = Документы.ОтчетКомиссионера.СоздатьДокументОтчетКомиссионераИзСтруктуры(Документ, 
				ТекстПрогресса, НастройкиЗагрузкиДанных, НастройкиОбновленияСвойств);	    
...
Функция ПодготовитьТабличныйДокументОтчетOzon(Знач ИсходныеДанные, ЗагружатьПродажи = Истина,
	...
	Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
		...
		Иначе
			...
			Если НЕ НайденаСтрокаЗаголовкаТаблицы Тогда
				...
				ТаблицаЗаголовков = ПодготовитьТаблицуЗаголовковТабличнойЧасти(СтрокаДанных, МаксИндексКолонок, НомерТаблицы);
			...
			ИначеЕсли Не НайденаВтораяСтрокаЗаголовкаТаблицы Тогда	
				...
				НайтиВторуюСтрокуЗаголовкаТабличнойЧасти(СтрокаДанных, ТаблицаЗаголовков, МаксИндексКолонок);
				Если ТаблицаЗаголовков <> Неопределено Тогда	
    ...
    Результат.ВидДокумента 		= ВидДокументаОтчетOzon();
...
Функция ПодготовитьТаблицуЗаголовковТабличнойЧасти(СтрокаДанных, МаксИндексКолонок, НомерТаблицы)
...
Процедура НайтиВторуюСтрокуЗаголовкаТабличнойЧасти(СтрокаДанных, ТаблицаЗаголовков, МаксИндексКолонок)
	...
	Для Н = 0 По МаксИндексКолонок Цикл
		...
		Если ПодготовленноеЗначение = "код" 
			...
			ИЛИ СтрНайти(ПодготовленноеЗначение, "ценареализации") > 0 Тогда  	// АПК:1036 проверка орфографии не требуется.
