...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	ИначеЕсли Источники.Строки.Найти(Метаданные.Справочники.МашиночитаемыеДоверенностиКЭДО, "Метаданные") <> Неопределено Тогда
		
		Команда = Команды.Добавить();
		Команда.ТипПараметра = Новый ОписаниеТипов("СправочникСсылка.МашиночитаемыеДоверенностиКЭДО");
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.27.161";
	Обработчик.Процедура = "Справочники.ШаблоныДокументов.НачальноеЗаполнение";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("c5334758-a5d0-458e-8485-fe09ba7d8707");
	Обработчик.Комментарий = НСтр("ru = 'Заполняет предопределенные шаблоны документов.'");
	Обработчик.НачальноеЗаполнение = Истина;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.27.164";
...
Процедура ЗарегистрироватьПубликациюДокументаКЭДО(ДокументКЭДО, ДатаПубликации, ОтключитьБизнесЛогику = Ложь) Экспорт
...
Процедура СохранитьСертификатПодписи(ПодписанныйОбъект, ПорядковыйНомер, Отпечаток, АдресСертификата) Экспорт
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		Запись.Сертификат = Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(АдресСертификата));
		НаборЗаписей.Записать();
	КонецЕсли;
...
Функция РезультатЗапросаТребуетсяСогласиеНаПрисоединениеККЭДО(ТаблицаФизическихЛиц) Экспорт
	...
	Запрос.Текст =
		...
		|		ПО ТаблицаФизическихЛицГоловныхОрганизаций.ФизическоеЛицо = Сотрудники.ФизическоеЛицо
		|			И ТаблицаФизическихЛицГоловныхОрганизаций.Организация = Сотрудники.ГоловнаяОрганизация
		|ГДЕ
