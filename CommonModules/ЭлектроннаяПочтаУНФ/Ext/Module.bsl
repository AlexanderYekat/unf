...
//
Функция НомерКартинкиУчастникаПоТипуКонтакта(Контакт) Экспорт
	...
	Если ТипКонтакта = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		Если ВидКонтрагента = Перечисления.ВидыКонтрагентов.ФизическоеЛицо
...
//
Функция ВажностьОтображенияКонтакта(Контакт) Экспорт
	...
	ИначеЕсли ТипКонтакта = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		Возврат 3;
	ИначеЕсли НЕ ЗначениеЗаполнено(Контакт)
...
Функция РеквизитыОбъектаДляПредпросмотра(Ссылка)
	...
	Пока ВыборкаУчастников.Следующий() Цикл
		...
		ТипПолучателя = ВыборкаУчастников.ТипПолучателяЭлектронногоПисьма;
		...
		Участник.Вставить("ТипПолучателяЭлектронногоПисьма", ТипПолучателя);
...
Процедура УбратьДублиКонтактов(Контакты)
	...
	Пока Счетчик1 < Контакты.Количество() Цикл
		...
		Пока Счетчик2 < Контакты.Количество() Цикл
			...
			Важность1 = ВажностьОтображенияКонтакта(Контакт1.Ссылка);
			Важность2 = ВажностьОтображенияКонтакта(Контакт2.Ссылка);
...
Функция РазметкаДляОкнаВыбораТипаКонтакта()
	...
	Возврат СтрШаблон(Шаблон,
		ПутьBase64Картинки("КоллекцияКонтрагентыКомпания"),
		ПутьBase64Картинки("ИконкаКонтактногоЛица"),
		ПутьBase64Картинки("КонтактнаяИнформацияЭлПочта"),
...
Функция РазметкаТемы(Тема, Важность)
	...
	Если ВысокаяВажность Тогда
		КартинкаВажности = СтрШаблон(
			"<img src=""%1"" alt=""%2"" />",
			ПутьBase64Картинки("ВосклицательныйЗнакКрасный"),
...
Функция ПутьBase64Картинки(НазваниеКартинки)
	
	Base64Картинки = Base64Строка(БиблиотекаКартинок[НазваниеКартинки].ПолучитьДвоичныеДанные());
...
Функция СтруктураКонтактаДляОтображенияВСписке(ДанныеКонтакта)
	...
	Результат.Вставить("ИндексКартинки", НомерКартинкиУчастникаПоТипуКонтакта(ДанныеКонтакта.Ссылка));
...
Функция ЭлементСсылкиКонтакта(Контакт, Постфикс = "")
	...
	НазваниеКартинкиКонтакта = НазваниеКартинкиКонтакта(Контакт.Ссылка);
	...
	Возврат СтрШаблон("
		...
		|</div>",
		ПутьBase64Картинки(НазваниеКартинкиКонтакта),
