...
Функция ОтказСнятиеУчетПоНесколькимСкладам() Экспорт
	...
	|ГДЕ
	|	НЕ СтруктурныеЕдиницы.ТипСтруктурнойЕдиницы В (&ИсключаемыеТипыСтруктурныхЕдиниц)
	...
	ИсключаемыеТипыСтруктурныхЕдиниц = Новый Массив;
	ИсключаемыеТипыСтруктурныхЕдиниц.Добавить(Перечисления.ТипыСтруктурныхЕдиниц.Подразделение);
	ИсключаемыеТипыСтруктурныхЕдиниц.Добавить(Перечисления.ТипыСтруктурныхЕдиниц.МагазинГруппаСкладов);
	
	Запрос.УстановитьПараметр("ИсключаемыеТипыСтруктурныхЕдиниц", ИсключаемыеТипыСтруктурныхЕдиниц);
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуЗарплата() Экспорт
	...
	Запрос.УстановитьПараметр("РазделУчета", Перечисления.РазделыУчета.РасчетыСПерсоналом);
...
Функция ОтказИзменитьНастройкуИспользоватьПредоплатуВЧекахККМ() Экспорт

	ТекстОшибки = "";

	Если ЕстьЧекиПредоплаты() Тогда
		
		ТекстОшибки = НСтр(
			"ru = 'В базе присутствуют чеки без передачи товара (предоплата или рассрочка). 
			|Отключение флажка запрещено.'");
...
Функция ОтказИзменитьНастройкуРассрочкуПлатежаВЧекахККМ() Экспорт
...
// Проверка на возможность отключения опции СуммовойУчетВРознице.
//
// Возвращаемое значение:
// 	Строка - текст сообщения если изменение настройки невозможно.
Функция ОтказСнятьФункциональнаяОпцияСуммовойУчетВРознице() Экспорт
	...
	|		Справочник.СтруктурныеЕдиницы КАК Справочник
	|	ГДЕ
	|		Справочник.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет)) КАК РегистрыНакопления";
	...
	Если ВыборкаЗапроса.Следующий() И ВыборкаЗапроса.КоличествоРегистраторов > 0 Тогда
		...
		ТекстОшибки = НСтр(
			"ru = 'В базе есть движения или объекты, относящиеся к суммовому учету розничных продаж.
			|Отключение флажка запрещено.'");
...
// Проверка на возможность отключения опции ИспользоватьРегулярныеПлатежи.
//
// Возвращаемое значение:
// 	Строка - текст сообщения если изменение настройки невозможно.
Функция ОтказСнятьФункциональнаяОпцияИспользоватьРегулярныеПлатежи() Экспорт

	КоличествоЗаписей =
		Справочники.ЗадачиАссистентаПоРегулярнымПлатежам.ЗадачаПоРегулярнымПлатежамОсновная.
			ПараметрыРегулярныхПлатежей.Количество();

	Если КоличествоЗаписей > 0 Тогда
		ТекстСообщения = НСтр(
			"ru = 'В базе присутствуют активные регулярные платежи.
			|Изменение настройки невозможно.'");
	КонецЕсли;

	Возврат ТекстСообщения;

КонецФункции // ОтказСнятьФункциональнаяОпцияИспользоватьРегулярныеПлатежи()
...
Функция ЕстьЧекиПредоплаты()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Константа.КонтрагентДляПодарочныхСертификатов КАК КонтрагентДляПодарочныхСертификатов
	|		ПО РасчетыСПокупателями.Контрагент = КонтрагентДляПодарочныхСертификатов.Значение
	|ГДЕ
	|	РасчетыСПокупателями.Документ ССЫЛКА Документ.ЧекККМ
	|	И КонтрагентДляПодарочныхСертификатов.Значение ЕСТЬ NULL";
	Если Не Запрос.Выполнить().Пустой() Тогда
