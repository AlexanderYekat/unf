...
Процедура ДобавитьИзменитьКодМаркировкиКегаНаОборудованииРозлива(Форма, Элемент, ДополнительныеПараметры, ИдентификаторСтроки)
	...
	РезультатОбработкиКега = ШтрихкодированиеИСМП.ДобавитьИзменитьКодМаркировкиКегаНаОборудованииРозлива(
		ПараметрыСканирования,
		Запасы,
		Форма.Объект[ПараметрыСканирования.ИмяТабличнойЧастиШтрихкодыУпаковок]);
	
	Если РезультатОбработкиКега.ЕстьИзменения Тогда
		ДополнительныеПараметры.Вставить("ТребуетсяЗаполнениеКэша", Истина);
	КонецЕсли;
	
	Для Каждого ДобавленныйКодМаркировки Из РезультатОбработкиКега.ДобавленныеКодыМаркировки Цикл
		ШтрихкодУпаковки = Форма.Объект[ПараметрыСканирования.ИмяТабличнойЧастиШтрихкодыУпаковок].Добавить();
		ЗаполнитьЗначенияСвойств(ШтрихкодУпаковки, ДобавленныйКодМаркировки.Значение);
		ШтрихкодУпаковки[ПараметрыСканирования.ИмяКолонкиШтрихкодУпаковки] = ДобавленныйКодМаркировки.Значение.ШтрихкодУпаковки;
	КонецЦикла;
	
	ПараметрыПоиска = Новый Структура(ПараметрыСканирования.ИмяКолонкиШтрихкодУпаковки);
	Для Каждого ИзмененныйКодМаркировки Из РезультатОбработкиКега.ИзмененныеКодыМаркировки Цикл
		ПараметрыПоиска[ПараметрыСканирования.ИмяКолонкиШтрихкодУпаковки] = ИзмененныйКодМаркировки.Значение.ШтрихкодУпаковки;
		ШтрихкодыУпаковок = Форма.Объект[ПараметрыСканирования.ИмяТабличнойЧастиШтрихкодыУпаковок].НайтиСтроки(ПараметрыПоиска);
		Если ШтрихкодыУпаковок.Количество() = 1 Тогда
			ШтрихкодУпаковки = ШтрихкодыУпаковок[0];
			ЗаполнитьЗначенияСвойств(ШтрихкодУпаковки, ИзмененныйКодМаркировки.Значение);
		КонецЕсли;
	КонецЦикла;
