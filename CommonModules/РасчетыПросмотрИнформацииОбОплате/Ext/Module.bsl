...
Функция ПолучитьПредставлениеДоговора(ТекущаяСтрока, ФормироватьПоДвижениям)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.Договор) Тогда
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(""+ТекущаяСтрока.Договор));
	Иначе
...
Функция ПолучитьПредставлениеЗаказа(ТекущаяСтрока, ФормироватьПоДвижениям, Шрифт, ВыводитьЗаголовок = Ложь, пЗаказ = Неопределено)
	...
	Если ТекущаяСтрока.СпособЗачета <> Перечисления.СпособыЗачетаИРаспределенияПлатежей.Авто ИЛИ ФормироватьПоДвижениям Тогда
		...
		Если ЗначениеЗаполнено(ТекущийЗаказ) Тогда
			...
			КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(СтрокаНомерДата));
		Иначе
			Если ВыводитьЗаголовок Тогда
		...
		Если ЭтоИзменениеДокумента ИЛИ НЕ ЭтоОплата ИЛИ ТекущаяСтрока.ЭтоЗачет Или НЕ ТекущаяСтрока.ПризнакАванса Тогда
			...
			Если ЗначениеЗаполнено(ТекущаяСтрока.Документ) Тогда
				...
				Если ТипЗнч(ТекДокумент) = Тип("СправочникСсылка.Номенклатура") Тогда // Подарочный сертификат
					КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(СокрЛП(ТекДокумент.Наименование)));
					Если ЗначениеЗаполнено(ТекущаяСтрока.НомерСертификата) Тогда
						ТекстДокумент = Символы.ПС
							+ СокрЛП(ТекущаяСтрока.НомерСертификата);
						КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(ТекстДокумент,
							Новый Шрифт(ШрифтыСтиля.ОбычныйШрифтТекста,,,Истина)));
					КонецЕсли;
				ИначеЕсли ТипЗнч(ТекДокумент) = Тип("СправочникСсылка.ДисконтныеКарты") Тогда // оплата бонусными баллами
					КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(Строка(ТекДокумент)));
				ИначеЕсли ТипЗнч(ТекДокумент) = Тип("ДокументСсылка.КорректировкаРегистров") Тогда
					КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(Строка(ТекДокумент)));
				Иначе
					Если ТипЗнч(ТекДокумент) = Тип("ДокументСсылка.ЗаказПокупателя")
							И ТекДокумент.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
						КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Заказ-наряд'")));
					Иначе
						КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(ТекДокумент.Метаданные().Представление()));
					КонецЕсли;
...
Функция ПолучитьСуммуПлатежаСтроки(ДокументОбъект, ТекущаяСтрока, ЭтоИзменениеДокумента)
	...
	Если НЕ ЭтоИзменениеДокумента И ТекущаяСтрока.ЭтоЗачет Тогда
		...
		ИначеЕсли ТекущаяСтрока.СуммаРег <> 0 И ТекущаяСтрока.СуммаРасчетов <> 0 Тогда
			...
			Иначе
				Если ДокументОбъект.Ссылка.Метаданные().Реквизиты.Найти("Курс") <> Неопределено Тогда
				...
				Иначе
					ТекущаяСуммаПлатежа = ТекущаяСтрока.Сумма;
				КонецЕсли;
			КонецЕсли;
...
Процедура ВывестиСтрокуРасшифровкиПлатежаДляПросмотра(ЭтоОплата, ТабличныйДокументКарточка, ТекущаяСтрока, ФормироватьПоДвижениям,
	...
	Если НЕ ЭтоИзменениеДокумента И НЕ ТекущаяСтрока.ЭтоЗачет Тогда
		Если ДокументОбъект.НалогообложениеНДС <> Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
			...
			СтруктураОбластей.ОбластьДопСтрока3Значения.Параметры.Значение3 = "" + ДокументОбъект.ВалютаДенежныхСредств.СимвольноеПредставление;
			Если НЕ ЭтоИзменениеДокумента Тогда
...
Процедура ВывестиСтрокуСИнформациейОбОплате(СтруктураВалютыКурсыИКратности, ЭтоОплата, ТабличныйДокументКарточка, ТекущаяСтрока, ФормироватьПоДвижениям,
	...
	ИначеЕсли ТекущаяСтрока.ЭтоВозврат Тогда	
		СтруктураОбластей.ОбластьСтрокаКолонкиСлева.Параметры.ТипОплаты = ТекущаяСтрока.ТипРасчетов;
	Иначе
	...
	Если ЭтоОплата И НЕ ТекущаяСтрока.ПризнакАванса Тогда // Оплата долга денежным документом, а не зачёт оплаты в документе отгрузки.
		...
		ИначеЕсли Не ТекущаяСтрока.ЭтоЧек И Не ТекущаяСтрока.ЭтоВозврат Тогда
...
Функция ПолучитьТабличныйДокументОплатаПоДокументуОтгрузки(Документ, пФормироватьПоДвижениям, Модифицированность, ТаблицаДокументовДляИзменения = Неопределено,
	...
	ЭтоВозврат = Документ.Свойство("ВидОперации")
		И (Документ.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВозвратОтПокупателя
		Или Документ.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ВыкупленныеКомиссионеромТовары
		Или Документ.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику);	
	...
	ПредставлениеРасшифровкиПлатежа.Колонки.Добавить("ЭтоВозврат", Новый ОписаниеТипов("Булево"));
	...
	СуммаДокументаВалРасчетов = Формат(Объект.СуммаДокумента / ?(Объект.Курс = 0, 1, Объект.Курс) * ?(Объект.Кратность
		= 0, 1, Объект.Кратность), "ЧДЦ=2; ЧН=0");
	...
	Если Объект.ВалютаДокумента <> ВалютаРасчетов И ЗначениеЗаполнено(ВалютаРасчетов) Тогда
		...
		СоставСтроки.Добавить(Формат(Объект.СуммаДокумента, "ЧДЦ=2; ЧН=0"));
		СоставСтроки.Добавить(ВалютаДокументаСимвольноеПредставление);
		Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
			ПредставлениеСуммаВалРасчетов = СтрШаблон(НСтр("ru = '(%1 %2 по курсу %3)'"), СуммаДокументаВалРасчетов,
				ВалютаРасчетов.СимвольноеПредставление, Формат(Объект.Курс, "ЧДЦ=4"));
			СоставСтроки.Добавить(ПредставлениеСуммаВалРасчетов);
		КонецЕсли;
		ОбластьСуммаДокумента.Параметры.СуммаДокумента = СтрСоединить(СоставСтроки);
	...
	Если ЭтоВозврат Тогда
		ОбластьВсегоОплачено.Параметры.ВсегоОплаченоТекст = НСтр("ru = 'Всего учтено'");
	Иначе
		ОбластьВсегоОплачено.Параметры.ВсегоОплаченоТекст = НСтр("ru = 'Всего оплачено'");
	КонецЕсли;
		...
		Для Каждого ТекущаяСтрока Из ТаблицаЗаписейДляПредставленияРасшифровкиПлатежа Цикл
			...
			Если НЕ ВыводилиЗаголовок Тогда
				...
				Если Не ЭтоВозврат Тогда
					...
					ОбластьЗачтено.Параметры.Разнесено = ?(ЭтоЗаказ, НСтр("ru = 'Оплачен документами:'"),
						НСтр("ru = 'Документы постоплаты:'"));
