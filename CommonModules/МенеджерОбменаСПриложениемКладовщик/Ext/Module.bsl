////////////////////////////////////////////////////////////////////////////////
// Менеджер обмена через универсальный формат (1С:Кладовщик от 20.05.2024 9:30:27)
//
////////////////////////////////////////////////////////////////////////////////
...
Функция Подключаемый_ИдентификаторМодуля() Экспорт
	Возврат "ced97c9b-5350-439c-8763-d7f2af8e08cb";
КонецФункции
#Область ПОД
#Область Документы
...
Процедура ПКО_Документ_ПоступлениеТоваровРаспоряжение_Получение_ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена)
	Если ПолученныеДанные.ДополнительныеСвойства.Свойство("ЗагрузкаИзЗаказаПоставщику")
		И ЗначениеЗаполнено(ПолученныеДанные.Заказ) Тогда  
			...
			ПолученныеДанные.СтруктурнаяЕдиница = ПолученныеДанные.Заказ.СтруктурнаяЕдиница;
	КонецЕсли; 
...
Процедура ПКО_Справочник_Файлы_ЗаказПоставщику_Получение_ПриКонвертацииДанныхXDTO(ДанныеXDTO, ПолученныеДанные, КомпонентыОбмена)
	...
	Если ДокументПоступление.Дата = Дата(1,1,1) Тогда
		ПолученныеДанные.ДополнительныеСвойства.Вставить("ВладелецФайла", Новый Структура("Значение, ИмяПКО",
		 ДокументЗаказПоставщику, "Документ_ЗаказПоставщику_Получение"));   
	Иначе
		ПолученныеДанные.ДополнительныеСвойства.Вставить("ВладелецФайла", Новый Структура("Значение, ИмяПКО",
		 ДокументПоступление, "Документ_ПоступлениеТоваровРаспоряжение_Получение"));   
	КонецЕсли;
