...
Процедура ДополнительноОбработатьНовостьПослеПолученияПередЗаписью(ОбъектНовости, НовостьХДТО) Экспорт
	...
	ОбработкаНовостейУНФ.ДополнительноОбработатьНовостьПослеПолученияПередЗаписью(ОбъектНовости, НовостьХДТО);
...
Процедура ПереопределитьСбросСостоянияНовостиПриЗаписи(Знач ОбъектНовость, СброситьСостояниеНовости) Экспорт
	...
	ОбработкаНовостейУНФ.ПереопределитьСбросСостоянияНовостиПриЗаписи(ОбъектНовость, СброситьСостояниеНовости);
...
Процедура ПереопределитьДействияКогдаНайденаНеактивнаяЛентаНовостей(ЛентаНовостей, НовостьОбъект, Отказ) Экспорт
	...
	ОбработкаНовостейУНФ.ПереопределитьДействияКогдаНайденаНеактивнаяЛентаНовостей(ЛентаНовостей, НовостьОбъект, Отказ);
...
Процедура ДополнительноОбработатьФормуПросмотраНовостейДляРабочегоСтолаПриСозданииНаСервере(
	...
	ОбработкаНовостейУНФ.ДополнительноОбработатьФормуПросмотраНовостейДляРабочегоСтолаПриСозданииНаСервере(
			Форма,
...
Процедура ДополнительноОбработатьФормуНовостиПриСозданииНаСервере(
	...
	ОбработкаНовостейУНФ.ДополнительноОбработатьФормуНовостиПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка);
...
Процедура ОбработатьЗаписьПользователя(
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='ОбработкаНовостей.ОбработатьЗаписьПользователя.Начало'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если ТипЗнч(ПользовательСсылка) = Тип("СправочникСсылка.Пользователи") Тогда
		...
		Если НЕ ПользовательСсылка.Пустая() Тогда
			...
			// Проверка на точное соответствие, включая номер сборки
			ВерсияПродукта = ВерсииПродукта[ВерсииПродукта.Количество()-1].Значение;
			...
			СтруктураОтборов.Вставить("Продукт", 			Продукт);
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='ОбработкаНовостей.ОбработатьЗаписьПользователя.Конец'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,, ТекстСообщения, РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
...
Процедура ЗначениеПредопределеннойКатегории(Категория, Значение) Экспорт
	...
	ОбработкаНовостейУНФ.ЗначениеПредопределеннойКатегории(Категория, Значение);
...
Процедура ЗаполнитьКодыКатегорийДляАвтоматическогоРасчетаОтборовНовостей(
	...
	ОбработкаНовостейУНФ.ЗаполнитьКодыКатегорийДляАвтоматическогоРасчетаОтборовНовостей(КодыКатегорийНовостей,
		ОбластьВидимости,
		КонтекстВыполнения);
...
Процедура РассчитатьОтборПоКатегории(
	...
	ОбработкаНовостейУНФ.РассчитатьОтборПоКатегории(
			КатегорияНовостей,
			КодКатегорииНовостей,
			ЗначениеКатегорииНовостей,
			РезультатРасчета,
