...
Процедура ПередДобавлениемКомандСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры, СтандартнаяОбработка) Экспорт  
	...
	КомпонентыИмениФормы = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Параметры.ИмяФормы, ".");

	Если КомпонентыИмениФормы.Количество() <> 4 Тогда
		Возврат;
	КонецЕсли;   
	
	СтандартнаяОбработка = Ложь;   
	
	ИмяМетаданных = КомпонентыИмениФормы[0];
	ИмяДокумента = КомпонентыИмениФормы[1];
	ИмяФормы = КомпонентыИмениФормы[3];

	Если ИмяМетаданных = "Документ" И ИмяДокумента = "ЗаказПокупателя" Тогда
		Если ИмяФормы = "ФормаДокумента"
			ИЛИ ИмяФормы = "ФормаСписка"
			ИЛИ ИмяФормы = "ФормаЗаказНаряда"
			ИЛИ ИмяФормы = "ФормаСпискаЗаказНаряда" Тогда
