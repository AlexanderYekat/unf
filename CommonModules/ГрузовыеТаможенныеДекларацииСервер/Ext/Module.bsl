...
Процедура ПодготовитьТаблицуЗапасовИзТаблицыФормы(ПараметрыПодбора, ТаблицаФормы) Экспорт
	...
	СтруктураПроверкиПрослеживаемости = Новый Структура("ПрослеживаемыйТовар");
	...
	Для каждого СтрокаТаблицыФормы Из ТаблицаФормы Цикл
		...
		СтруктураПроверкиПрослеживаемости.ПрослеживаемыйТовар = Ложь;
		ЗаполнитьЗначенияСвойств(СтруктураПроверкиПрослеживаемости, СтрокаТаблицыФормы);
		
		Если СтруктураПроверкиПрослеживаемости.ПрослеживаемыйТовар = Истина Тогда
			Продолжить
		КонецЕсли;
		//--
...
Процедура ПодобратьНомераГТДПоПредыдущимПоступлениям(ПараметрыПодбора) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ПередачаТоваровМеждуОрганизациями") 
		...
		ОтборОрганизации = ИнтеркампаниСервер.ОрганизацииОстатков(
...
Процедура СформироватьОстаткиПереданныеНомеровГТД(ПараметрыПодбора) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТекущийДокумент.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			ТОГДА ТекущийДокумент.Количество
	|		ИНАЧЕ -ТекущийДокумент.Количество
	|	КОНЕЦ,
	|	ТекущийДокумент.Номенклатура,
	|	ТекущийДокумент.Характеристика,
	|	ТекущийДокумент.Партия,
...
Функция ПеренестиОстаткиНомеровГТДВТаблицуФормы(ТаблицаФормы, ПараметрыПодбора) Экспорт
	...
	Если РассчитыватьОбъемИВес(ТаблицаЗапасы) Тогда
...
Функция ИменаТЧИПолейДляТаблицыЗапасы_БазовыеПоляСПрослеживаемостью(пИменаТЧИПолей = Неопределено, пИмяТЧ = "Запасы") Экспорт
	...
	Если пИмяТЧ = "ЗНМатериалы" Тогда
		
		пИмяТЧМатериалы = "Материалы";
		
		ИменаТЧСтруктура.Вставить("ИмяПоляПроверки", "Объект."+пИмяТЧМатериалы+".СтранаПроисхождения");
		ИменаТЧСтруктура.Вставить("ИмяПоляПрослеживаемость", "Объект."+пИмяТЧМатериалы+".ПрослеживаемыйТовар");
		ИменаТЧСтруктура.Вставить("ИмяПоляРНПТ", "Объект."+пИмяТЧМатериалы+".РНПТ");
...
Процедура ПриСозданииНаСервереДляДокументаСПрослеживаемостью(ЭтаФорма, ИменаТЧ, КэшЗначений, ИмяТабличнойЧасти = "Запасы") Экспорт
	...
	Если ИмяТабличнойЧасти = "ЗНМатериалы" Тогда
		...
		ИмяТабличнойЧастиНаФорме = "ЗНМатериалы";
	Иначе
		ИмяТабличнойЧастиНаФорме = ИмяТабличнойЧасти;
	КонецЕсли;
	...
	Если КэшЗначений.Свойство("РеализацияВЕАЭС") Тогда
		...
		РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление, ИмяТабличнойЧастиНаФорме + "КодТНВЭД");
	...
	Если Не КэшЗначений.ВестиУчетПрослеживаемыхТоваров Тогда
		Если СтруктураПараметров.Свойство("ИмяПоляНомерГТД") Тогда
			...
			КонецЦикла;
			ПриСозданииНаСервере(ЭтаФорма, ИменаТЧ, КэшЗначений);
		КонецЕсли;
		Возврат;
	Иначе
		ИменаТЧ = Новый Массив;
		
		СтруктураИмен = Новый Структура("ИмяПоляПроверки, ИмяПоляОформления", "Объект." + ИмяТабличнойЧасти + ".СтранаПроисхождения", ИмяТабличнойЧастиНаФорме + "НомерГТДПриРНПТ");
		
		ИменаТЧ.Добавить(СтруктураИмен);
		ПриСозданииНаСервере(ЭтаФорма, ИменаТЧ, КэшЗначений);
		Возврат;
	КонецЕсли;
...
Функция РассчитыватьОбъемИВес(ТаблицаФормы)
	
	Если Не ТипЗнч(ТаблицаФормы) = Тип("ТаблицаЗначений") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если Не ТаблицаФормы.Колонки.Найти("Вес") = Неопределено
		И Не ТаблицаФормы.Колонки.Найти("Объем") = Неопределено
		И Не ТаблицаФормы.Колонки.Найти("ВесЕдиницыТовара") = Неопределено
		И Не ТаблицаФормы.Колонки.Найти("ОбъемЕдиницыТовара") = Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
