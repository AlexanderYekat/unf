...
Функция СреднийЗаработокФСС(ПараметрыРасчета) Экспорт
	Если ПараметрыРасчета.ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаФСС.Постановление2010") Тогда
		Возврат СреднийЗаработокФСС2010(ПараметрыРасчета);
	Иначе
		Возврат СреднийЗаработокФСС2011(ПараметрыРасчета);
	КонецЕсли;
КонецФункции

Функция СреднийЗаработокФСС2010(ПараметрыРасчета)
...
Функция СреднийЗаработокФСС2011(ПараметрыРасчета)
	...
	Если ПараметрыРасчета.ПрименяетсяФЗ_421_2014 И ПрименятьПредельнуюВеличину Тогда
		МаксимальныйСреднедневнойЗаработок = МаксимальныйСреднедневнойЗаработокДляФЗ_421_2014(ПараметрыРасчета.ДатаНачалаСобытия);
	...
	ИначеЕсли ПрименятьМаксимальныйСреднедневнойБезУчетаИсключаемыхПериодов(ПараметрыРасчета) Тогда
		МаксимальныйСреднедневнойЗаработок = МаксимальныйСреднедневнойЗаработокДляОплатыПособияБезУчетаИсключаемыхПериодов(ПараметрыРасчета);
		СреднедневнойЗаработок = Мин(МаксимальныйСреднедневнойЗаработок, СреднедневнойЗаработок);
	КонецЕсли;
...
Функция УчитываемыйЗаработокФСС(ПараметрыРасчета, ДанныеРасчетаСреднегоЗаработка) Экспорт
	...
	Для Каждого Элемент Из ДанныеРасчетаСреднегоЗаработка Цикл
		Если ПрименятьПредельнуюВеличину Тогда
			Элемент.УчтенныйЗаработок = Мин(Элемент.ПредельнаяВеличина, Элемент.Заработок);
		Иначе
...
Функция МаксимальныйСреднедневнойЗаработокДляОплатыПособияБезУчетаИсключаемыхПериодов(ПараметрыРасчета)
	Итог = 0;
