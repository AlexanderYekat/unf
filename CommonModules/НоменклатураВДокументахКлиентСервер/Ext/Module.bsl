...
// Возвращает значение настрорек формы выбора при открытии из документов установленных для поля Номенклатура
//
// Параметры:
//  ПараметрыВыбораНоменклатуры  - Параметры выбора поля формы
// 
// Возвращаемое значение:
//   - Структура
//
Функция НастройкиФормыИзПараметровВыбораНоменклатуры(ПараметрыВыбораНоменклатуры) Экспорт
	
	Для Каждого ПараметрВыбораНоменклатуры Из ПараметрыВыбораНоменклатуры Цикл
		Если ПараметрВыбораНоменклатуры.Имя = "Дополнительно.НастройкиФормы" Тогда
			Возврат ПараметрВыбораНоменклатуры.Значение;
		КонецЕсли;
	КонецЦикла;
