...
//   * ГруппаСКомандамиПереключения - ГруппаФормы - гурппа с командами для переключения [КомандаHTML] и [КомандаПростойТекст]
...
Функция ОткрытьНавигационнуюСсылкуСПроверками(НавигационнаяСсылка, Элемент, ПоказатьВопросПередПереходом = Ложь)
	...
	Если Не Убратьe1cВНачале(Ссылка) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция Убратьe1cВНачале(Ссылка)
