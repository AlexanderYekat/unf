...
// Выполняет запуск проверки подключения к АТС в фоновом режиме при запуске приложения.
//
Процедура ПередЗапускомПрограммы() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОблачнуюТелефонию") Тогда
		НаименованиеЗадания = НСтр("ru = 'Телефония. Проверка подключения.'");
		...
		ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполненияВФоне, "ТелефонияСервер.ПроверкаПодключения");
	КонецЕсли;
	
КонецПроцедуры

// Выполняет проверку подключения к АТС.
//
Процедура ПроверкаПодключения() Экспорт
	
	АТС = Константы.ИспользуемаяАТС.Получить();
	МодульИнтеграции(АТС).ПроверкаПодключения();
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	НастройкаТелефонии = Настройки.Добавить();
	НастройкаТелефонии.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ОбновлениеПодпискиНаСобытияВАТС;
	НастройкаТелефонии.РаботаетСВнешнимиРесурсами = Истина;
	НастройкаТелефонии.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ИспользоватьРегламентноеОбновлениеПодпискиАТС;
...
Функция НовыйДанныеЗвонка() Экспорт
	...
	ДанныеЗвонка.Вставить("НеобрабатываемыйНомер", Ложь);
	...
	ДанныеЗвонка.Пользователь.Вставить("ВнутреннийИдентификаторВАТС", "");
...
Функция ОбработатьВходящийЗвонок(ДанныеЗвонка, Знач РегистрироватьВходящийДляПользователя) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеЗвонка.НомерКонтакта) Тогда
		ДанныеЗвонка.НеобрабатываемыйНомер = НеобрабатываемыйНомер(ДанныеЗвонка.НомерКонтакта);
	КонецЕсли;
	
	НайтиСоздатьСобытиеПоДаннымЗвонка(ДанныеЗвонка);
	
	Если ДанныеЗвонка.НеобрабатываемыйНомер Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если РегистрироватьВходящийДляПользователя Тогда
		ЗарегистрироватьНовыйТекущийЗвонокДляПользователя(ДанныеЗвонка);
	КонецЕсли;
...
Функция ОбработатьИзменениеЗвонка(ДанныеЗвонка) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеЗвонка.Событие) ИЛИ ДанныеЗвонка.НеобрабатываемыйНомер Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ЗагрузитьДанныеПоАбонентам() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("КодСостояния", "");
	Результат.Вставить("ЗаголовокОшибки", "");
	Результат.Вставить("ТекстОшибки", "");
	Результат.Вставить("СписокПользователей", Неопределено);
	
	URL = КорневойАдресАТС();
	Если URL = Неопределено Тогда
		Результат.ЗаголовокОшибки = "Не заполнен адрес АТС";
		Возврат Результат;
	КонецЕсли;
	...
	Если Не НастройкиИнтеграцииЗаполнены(АТС, НастройкиТелефонии) Тогда
		Результат.ЗаголовокОшибки = "Не заполнены настройки интеграции";
		Возврат Результат;
	КонецЕсли;
	
	ПараметрыОтправки = МодульИнтеграции(АТС).ЗапросСпискаПользователей(НастройкиТелефонии);
	...
	МодульИнтеграции(АТС).ОбработатьОтветЗапросаСпискаПользователей(HTTPОтвет, НастройкиТелефонии, Результат);
...
//
Процедура ПроверитьАдресПубликацииСервисаТелефонии(HTTPОтвет = Неопределено) Экспорт
...
Процедура ОбновитьДанныеЗвонкаВРегистре(ДанныеЗвонка) Экспорт
...
//   * ПоддерживаетсяНастройкаМаршрутизации - Булево - Если истина, то на стороне УНФ происходит перенаправление вызова
//                                                     на ответственного.
Функция НовыйНастройкиИнтеграции()
	...
	НастройкиИнтеграции.Вставить("Проверки", Новый Соответствие);
	НастройкиИнтеграции.Проверки.Вставить("ПроверкаАбонентовАТС", Ложь);
	НастройкиИнтеграции.Проверки.Вставить("ПроверкаСистемыВзаимодействия", Ложь);
	НастройкиИнтеграции.Проверки.Вставить("ПроверкаПодключенияАТС", Ложь);
	НастройкиИнтеграции.Проверки.Вставить("ПроверкаПубликуемогоСервиса", Ложь);
	НастройкиИнтеграции.Проверки.Вставить("ПроверкаСлужебногоПользователя", Ложь);
	НастройкиИнтеграции.Проверки.Вставить("ПроверкаПолучениеПодпискиНаСобытияАТС", Ложь);
	НастройкиИнтеграции.Проверки.Вставить("ПроверкаОбязательныхПолей", Ложь);
...
Процедура ПривестиНомерТелефонаКФормату(ДанныеЗвонка)
	...
	ДанныеЗвонка.НомерКонтакта = СтрЗаменить(ДанныеЗвонка.НомерКонтакта, "+", "");
	ДанныеЗвонка.НомерКонтакта = СтрЗаменить(ДанныеЗвонка.НомерКонтакта, "(", "");
	ДанныеЗвонка.НомерКонтакта = СтрЗаменить(ДанныеЗвонка.НомерКонтакта, ")", "");
	ДанныеЗвонка.НомерКонтакта = СтрЗаменить(ДанныеЗвонка.НомерКонтакта, " ", "");
	ДанныеЗвонка.НомерКонтакта = СтрЗаменить(ДанныеЗвонка.НомерКонтакта, "-", "");
...
Процедура ЗаписатьДанныеЗвонкаВРегистр(ДанныеЗвонка)
	...
	Попытка
		...
		Если НаборЗаписей.Количество() > 0 И НЕ ДанныеЗвонка.НеобрабатываемыйНомер Тогда
...
Процедура ДополнитьСтрокуРегистраДаннымиЗвонка(ЗаписьРегистра, ДанныеЗвонка)
	...
	ЗаписьРегистра.НеобрабатываемыйНомер = ДанныеЗвонка.НеобрабатываемыйНомер;
	...
	Если ДанныеЗвонка.ВызовЗавершен Тогда
		...
		ОбновитьДанныеЗвонкаПоНастройкамТелефонии(ЗаписьРегистра);
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбновитьДанныеЗвонкаПоНастройкамТелефонии(ЗаписьРегистра)

	Если ЗаписьРегистра.ВходящееИсходящее = Перечисления.ВходящееИсходящееСобытие.Входящее
			И ЗаписьРегистра.Неотвеченный И НЕ ЗаписьРегистра.НеобрабатываемыйНомер Тогда
			
			НастройкиТелефонии = ТелефонияСервер.ПолучитьНастройкиТелефонии();
			Если НастройкиТелефонии.ДействиеПропущенногоЗвонка = "НазначитьСотрудника" Тогда
				ЗаписьРегистра.Сотрудник = НастройкиТелефонии.СлужебныйПользовательДляПропущенногоЗвонка;
			КонецЕсли;
...
Функция НайтиСоздатьСобытиеПоДаннымЗвонка(ДанныеЗвонка)
	...
	Попытка
		...
		Если Не ДанныеЗвонка.НеобрабатываемыйНомер И ДанныеЗвонка.ИдентификаторЗвонкаВАТС <> Неопределено Тогда
			НайтиСобытиеПоДаннымЗвонка(ДанныеЗвонка);
		КонецЕсли;
		
		НужноСоздатьСобытие = Не ДанныеЗвонка.НеобрабатываемыйНомер И Не ЗначениеЗаполнено(ДанныеЗвонка.Событие);
...
Процедура ДобавитьСообщениеВСистемуВзаимодействия(ДанныеЗвонка, ДанныеСообщения) Экспорт
...
Процедура ЗарегистрироватьНовыйТекущийЗвонокДляПользователя(ДанныеЗвонка)
...
Функция НастройкиТелефонииПоУмолчанию()
	...
	Результат.Вставить("ДействиеИсходящегоЗвонка", "Вызов");
	
	Результат.Вставить("ДействиеПропущенногоЗвонка", "ПоследнегоОтклонившегоЗвонок");
	Результат.Вставить("СлужебныйПользовательДляПропущенногоЗвонка", Справочники.Сотрудники.ПустаяСсылка());
	...
	// Билайн
	Результат.Вставить("АдресОблачнойАТСБилайн", "");
	Результат.Вставить("КлючДляАвторизацииВАТСБилайн", "");
...
Функция НеобрабатываемыйНомер(НомерТелефонаАбонента)
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ	
	|	НеобрабатываемыеТелефонныеНомера.НомерТелефонаАбонента КАК НомерТелефонаАбонента
	|ИЗ
	|	РегистрСведений.НеобрабатываемыеТелефонныеНомера КАК НеобрабатываемыеТелефонныеНомера
	|ГДЕ
	|	НеобрабатываемыеТелефонныеНомера.НомерТелефонаАбонента = &НомерТелефонаАбонента");
	Запрос.Параметры.Вставить("НомерТелефонаАбонента",НомерТелефонаАбонента);
