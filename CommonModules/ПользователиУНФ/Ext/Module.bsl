...
Процедура ПриПолученииДанныхНаСервере(СтрокиСписка) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаГруппыДоступаПользователей();
	Запрос.УстановитьПараметр("Пользователи", СтрокиСписка.ПолучитьКлючи());
...
// Для вызова из события формы ПриСозданииНаСервере формы ФормаЭлемента справочника Пользователи.
//
// Параметры:
...
Процедура ФормаЭлементаПриСозданииНаСервере(Форма) Экспорт
	...
	ПолеВводаФизическоеЛицо = Форма.Элементы.Найти("ФизическоеЛицо");
	Если ПолеВводаФизическоеЛицо = Неопределено Тогда
		Возврат;
	КонецЕсли;

	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтчетность") Тогда
		ПолеВводаФизическоеЛицо.Видимость = Истина;
		ПолеВводаФизическоеЛицо.Подсказка = НСтр("ru = 'Владелец электронной подписи для сдачи отчетности'");
		ПолеВводаФизическоеЛицо.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
	Иначе
		ПолеВводаФизическоеЛицо.Видимость = Ложь;
	КонецЕсли;
...
// Заполняет персональные настройки новых пользователей, инициализирует установку пользовательских настроек подбора
//
Процедура УстановитьНастройкиНовомуПользователю(Источник, Отказ) Экспорт
	...
	ПодборНоменклатурыВДокументах.УстановитьСтандартныеНастройкиПодбора(СсылкаПользователя);
	РегистрыСведений.НастройкиПользователей.Установить(Истина, "ПоказыватьКолонкуСодержаниеВАктеВыполненныхРабот",
		СсылкаПользователя);
...
// Записывает статистику по количеству значений настроек пользователей
//
Процедура СобратьСтатистикуПоНастройкамПользователей() Экспорт

	ЗаписатьСтатистикуПоНастройке("ПоказыватьПредпросмотрЗаказовПокупателей");
	ЗаписатьСтатистикуПоНастройке("ПоказыватьПредпросмотрЗаказНарядов");
...
Функция ТекстЗапросаГруппыДоступаПользователей()
...
// Параметры:
// 	НазваниеНастройки - Строка
// Возвращаемое значение:
//	Неопределено - если настройка пользователя не найдена
//	Соответствие:
//		* Ключ - Булево,СправочникСсылка
//		* Значение - Число - количество настроек с таким значением				  
// 
Функция КоличествоЗначенийНастройки(НазваниеНастройки)
	
	Настройка = ПланыВидовХарактеристик.НастройкиПользователей.НайтиПоНаименованию(НазваниеНастройки);
	...
	Пока Выборка.Следующий Цикл
		Результат.Вставить(Выборка.Значение, Выборка.Количество);
	КонецЦикла;
...
Процедура ЗаписатьСтатистикуПоНастройке(НазваниеНастройки)
	
	ЗначенияНастройки = КоличествоЗначенийНастройки(НазваниеНастройки);
	
	Если ЗначенияНастройки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого Элемент Из ЗначенияНастройки Цикл
