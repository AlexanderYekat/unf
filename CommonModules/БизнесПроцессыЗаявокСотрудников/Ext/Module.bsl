...
Функция РолиИсполнителейЗадач() Экспорт
	...
	Если КабинетСотрудника.ИспользуетсяВерсияФормата("2.2") Тогда
...
Процедура ПеренестиСогласователейЗаявкиВРегистр(ИмяТаблицы, ПараметрыОбновления) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.РуководительСотрудника) Тогда
			...
			Запись.РольСогласующего = Перечисления.РолиПодписантовКЭДО.НепосредственныйРуководитель;
...
Процедура ОбновитьИсполнителейЗадач(РольИсполнителя, Подразделение)
	...
	ИначеЕсли НаборЗаписей.Количество() = 0 Тогда
		...
		Если ЗначениеЗаполнено(Подразделение.Владелец) Тогда
			ВладелецРодитель = Подразделение.Владелец;
		ИначеЕсли ЗначениеЗаполнено(Подразделение.Родитель) Тогда
			ВладелецРодитель = Подразделение.Родитель;
		КонецЕсли;
...
Процедура ОбновитьРольОтветственныйЗаОбращенияСотрудников() Экспорт
	...
	|	Справочник.РолиИсполнителей КАК РолиИсполнителей
	|ГДЕ
	|	РолиИсполнителей.Ссылка = ЗНАЧЕНИЕ(Справочник.РолиИсполнителей.ОтветственныйЗаОбращенияСотрудников)
