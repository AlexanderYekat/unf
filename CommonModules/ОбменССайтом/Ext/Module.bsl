...
Процедура ЗаданиеВыполнитьОбмен(КодУзлаОбмена) Экспорт
	...
	Метаданные.РегламентныеЗадания.ОбменССайтом);
...
Процедура ДобавитьСвойстваНоменклатурыВКлассификаторXDTO(КлассификаторXDTO, ПакетCML, ВыборкаСвойствНоменклатуры, Параметры) Экспорт
	...
	Пока ВыборкаСвойствНоменклатуры.Следующий() Цикл
		...
		Если ВыборкаСвойствНоменклатуры.Свойство.НаборСвойств.Родитель = НаборСвойств_Справочник_ХарактеристикиНоменклатуры Тогда
			СвойствоXDTO.ДляПредложений = Истина;
		ИначеЕсли ВыборкаСвойствНоменклатуры.Свойство.НаборСвойств.Родитель = УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Номенклатура")
			ИЛИ ВыборкаСвойствНоменклатуры.Свойство.НаборСвойств.Родитель = УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Номенклатура_Общие") Тогда
...
Процедура ПрочитатьПодчиненныеДокументыОплаты(ДокументXDTO, ЗаказСсылка, СвойстваЗаказа, Параметры, СтруктураСтатистики, ОписаниеОшибки)
	...
	Для каждого ПодчиненныйДокументXDTO Из МассивПодчиненныхДокументов Цикл
		...
		Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПодчиненныйДокументXDTO, "Сумма") Тогда
...
Функция ОбновитьСоздатьДополнительныеСведенияОперацияПоПлатежнымКартам(ОперацияПоПлатежнымКартам, СвойстваЗаказа, ОписаниеОшибки)
	...
	ЗапросНаборы.УстановитьПараметр("НаборСвойств_Документ_ОперацияПоПлатежнымКартам",
				...
				ИначеЕсли ТипЗнч(ДокументОбъект[ИмяРеквизита]) = Тип("Число") Тогда
					...
					Исключение
						ТекстСообщенияОбОшибке = СтрШаблон(
							НСтр("ru='Не удалось преобразовать значение свойства в число: %1:%2'"),
							ИмяРеквизита,
							элСоответствие.Значение);
						
						ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекстСообщенияОбОшибке);
					КонецПопытки; 
...
Функция СоздатьЗаполнитьКонтактноеЛицоКонтрагента(КонтрагентОбъект, КонтрагентXDTO, НаименованиеКонтрагента = "", КонтактноеЛицоСсылка = Неопределено)
	...
	Иначе
		...
		ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КонтрагентXDTO, "Наименование")
			И СвойствоXDTOЗаполнено(КонтрагентXDTO.Наименование) Тогда
...
Процедура ОбновитьКонтактнуюИнформациюКонтрагента(КонтрагентОбъект, КонтрагентXDTO, Замещать = Ложь)
	...
	Если НЕ АдресXDTO = Неопределено 
		...
		Если НЕ ПроверитьДублированиеКИ(КонтрагентОбъект, АдресXDTO.Представление, ВидКИ, ТипКИ) Тогда
			ЗаписатьКИ(КонтрагентОбъект, АдресXDTO.Представление, ВидКИ, ТипКИ, ЗамещатьАдрес);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьДанныеССервера(Соединение, ПараметрыЗапроса = "", Заголовки = "", ОписаниеОшибки)
	...
	Исключение
		...
		Если СтрНайти(ПодробноеПредставлениеОшибки, "Transferred a partial file") <> 0 Тогда
...
Функция ПреобразоватьЗначениеJSON(Свойство, Значение, ДополнительныеПараметры, Отказ) Экспорт
	...
	Результат.Вставить("Номер", ОбщегоНазначения.ВычислитьВБезопасномРежиме("Значение.Номер", Значение));
	Результат.Вставить("id",
	ОбщегоНазначения.ВычислитьВБезопасномРежиме("Строка(Значение.УникальныйИдентификатор())", Значение));
...
Процедура ДобавитьНастройкиУзлаВПараметры(УзелОбмена, Параметры)
	...
	|	ОбменУправлениеНебольшойФирмойСайт.ИспользоватьОптимизированныйОбменКартинок КАК ИспользоватьОптимизированныйОбменКартинок
	|ИЗ
