...
// Данные подключения к оборудованию розлива.
// 
// Возвращаемое значение:
...
Функция ДанныеПокдлюченияКегаКОборудованиюРозлива() Экспорт
	
	ДанныеПодключения = Новый Структура();
	
	ДанныеПодключения.Вставить("КодМаркировки",           Справочники.ШтрихкодыУпаковокТоваров.ПустаяСсылка());
	ДанныеПодключения.Вставить("Организация",             ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("Организация"));
	ДанныеПодключения.Вставить("Подразделение",           ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("Подразделение"));
	ДанныеПодключения.Вставить("Склад",                   ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("Склад"));
	ДанныеПодключения.Вставить("ДатаПодключения",         Дата(1, 1, 1));
	ДанныеПодключения.Вставить("СрокРеализации",          Дата(1, 1, 1));
	ДанныеПодключения.Вставить("АдресПодключения",        Неопределено);
	ДанныеПодключения.Вставить("АдресПодключенияСтрокой", Неопределено);
	ДанныеПодключения.Вставить("КодФИАС",                 Неопределено);
	ДанныеПодключения.Вставить("Ответственный",           Неопределено);
...
// 
// Параметры:
//  ДанныеПодключения - см. ДанныеПокдлюченияКегаКОборудованиюРозлива
// 
// Возвращаемое значение:
...
Функция ПодключитьКегКОборудованиюРозлива(ДанныеПодключения) Экспорт
	...
	ДанныеПодключенияКега = РегистрыСведений.КегиНаОборудованииРозливаИСМП.ИнициализироватьДанныеПодключенияКОборудованию();
	ЗаполнитьЗначенияСвойств(ДанныеПодключенияКега, ДанныеПодключения);
	ДанныеПодключенияКега.Статус = Перечисления.СтатусыПодключенияКОборудованиюРозливаИСМП.ПодключеноВоВнешнейСистеме;
	
	РезультатПодключения = ПодключитьНовыйКегКОборудованиюРозлива(
		ДанныеПодключенияКега);
	
	ЗаполнитьЗначенияСвойств(ВозвращаемоеЗначение, РезультатПодключения);
...
Процедура ЗаполнитьДоступныеВидыДокументовСертификации(Элемент, ВидПродукции) Экспорт
	...
	Если ВидПродукции <> ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БАДы") Тогда
		Элемент.СписокВыбора.Добавить(
...
// * Документ - ДокументСсылка.ПодключениеКегаКОборудованиюРозливаИСМП - документ подключения кега
// * ТекстОшибки - Строка, Неопределено                                - текст ошибки подключения при наличии
Функция ПодключитьНовыйКегКОборудованиюРозлива(ДанныеПодключения, ДанныеОтключения = Неопределено) Экспорт
	...
	Попытка
		...
		РегистрыСведений.КегиНаОборудованииРозливаИСМП.УстановитьПодключениеКОборудованию(ДанныеПодключения);
		
		Если ДанныеОтключения <> Неопределено Тогда
			
			ТаблицаДанных = РегистрыСведений.КегиНаОборудованииРозливаИСМП.НоваяТаблицаОтключенияКегОтОборудованияРозлива();
			НоваяСтрока = ТаблицаДанных.Добавить();
			НоваяСтрока.КодМаркировки = ДанныеОтключения.КодМаркировки;
			РегистрыСведений.КегиНаОборудованииРозливаИСМП.ОтключитьКегиОтОборудованияРозлива(ТаблицаДанных);
