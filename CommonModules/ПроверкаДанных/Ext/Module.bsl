...
//
Функция ВыполнитьПроверкуПоСобытию(Организация, СобытиеКалендаря, ОграниченияПараметровКонтроля = Неопределено,
	СохранятьПротокол = Истина) Экспорт
	
	ПараметрыСобытия = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СобытиеКалендаря, "ДатаДокументаОбработкиСобытия,Задача");
	
	СтруктураПараметровКонтроля = ПроверкаДанныхКлиентСервер.ПолучитьОписаниеСтруктурПараметровПроверкиДанных(
		Организация).ПараметрыРаботы.ПараметрыКонтроля;
	...
	Для Каждого ЭлементКонтроля Из СтруктураПараметровКонтроля Цикл
		...
		Иначе
			...
			СтруктураПараметровКонтроля[ЭлементКонтроля.Ключ] = (ОграниченияПараметровКонтроля.Найти(
				ЭлементКонтроля.Ключ) <> Неопределено);
	...
	ИначеЕсли ПараметрыСобытия.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогиСотрудников 
		ИЛИ ПараметрыСобытия.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыЗаСотрудников Тогда
	...
	ИначеЕсли ПараметрыСобытия.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СведенияОЗастрахованныхЛицах
		ИЛИ ПараметрыСобытия.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ПерсСведенияОФизлицах Тогда
		...
		ОграниченияПроверки.СведенияПоСотрудникам.Добавить("ИНН");
		ОграниченияПроверки.СведенияПоСотрудникам.Добавить("СтраховойНомерПФР");
	ИначеЕсли ПараметрыСобытия.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СЗВСтаж Тогда
		...
		ОграниченияПроверки.СведенияПоСотрудникам.Добавить("ДатаРождения");
		ОграниченияПроверки.СведенияПоСотрудникам.Добавить("СтраховойНомерПФР");
	ИначеЕсли ПараметрыСобытия.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Декларация12
		ИЛИ ПараметрыСобытия.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Декларация8
		ИЛИ ПараметрыСобытия.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогАУСН Тогда
		ОграниченияПроверки.СведенияПоОрганизации.Добавить("Наименование");
		ОграниченияПроверки.СведенияПоОрганизации.Добавить("НаименованиеПолное");
		ОграниченияПроверки.СведенияПоОрганизации.Добавить("ИНН");
		ОграниченияПроверки.СведенияПоОрганизации.Добавить("КПП");
...
Функция ВыполнитьПроизвольнуюПроверку(Организация, ПараметрыКонтроля, ДатаНачала, ДатаОкончания, ОграниченияПроверки = Неопределено, СохранятьПротокол = Истина, ПолныйРезультат = Ложь) Экспорт
	...
	Попытка 
		...
		Если ПараметрыКонтроля.СведенияПоОрганизации Тогда
			ВыполнитьПроверкуСведенийПоОрганизации(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
				ОграниченияПроверки.СведенияПоОрганизации);
		КонецЕсли;
		
		Если ПараметрыКонтроля.ПараметрыУчета Тогда
			ВыполнитьПроверкуПараметровУчета(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
				ОграниченияПроверки.ПараметрыУчета);
		КонецЕсли;
		
		Если ПараметрыКонтроля.БанковскиеСчета Тогда
			ВыполнитьПроверкуБанковскихСчетов(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
				ОграниченияПроверки.БанковскиеСчета);
		КонецЕсли;
		
		Если ПараметрыКонтроля.НачальныеОстатки Тогда
			ВыполнитьПроверкуНачальныхОстатков(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
				ОграниченияПроверки.НачальныеОстатки);
		КонецЕсли;
		
		Если ПараметрыКонтроля.НаличиеПоказателейЕНВД Тогда
			ВыполнитьПроверкуНаличияПоказателейЕНВД(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
				ОграниченияПроверки.ВзаиморасчетыССотрудниками);
		КонецЕсли;
		
		Если ПараметрыКонтроля.СведенияПоСотрудникам Тогда
			ВыполнитьПроверкуСведенийПоСотрудникам(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
				ОграниченияПроверки.СведенияПоСотрудникам);
		КонецЕсли;
		
		Если ПараметрыКонтроля.ТарифыСтраховыхВзносов Тогда
			ВыполнитьПроверкуТарифаСтраховыхВзносов(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
				ОграниченияПроверки.ПараметрыУчета);
		КонецЕсли;
		
		Если ПараметрыКонтроля.СведенияПоТорговымТочкам Тогда
			ВыполнитьПроверкуТорговыхТочек(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
				ОграниченияПроверки.ПараметрыУчета);
		КонецЕсли;
		
		Если ПараметрыКонтроля.ТорговыеТочкиРозничнойПродажиАлкоголя Тогда
			ВыполнитьПроверкуТорговыхТочекРозничнойПродажиАлкоголя(ДеревоРезультата, Организация, ДатаНачала,
				ДатаОкончания, ОграниченияПроверки.ПараметрыУчета);
		КонецЕсли;
		
		// обработка дерева результата
		
		Результат = ПолучитьСтруктуруРезультатаРаботы(ДеревоРезультата);
		
		Если СохранятьПротокол Тогда
			
			Результат.КлючПротоколаПроверки = СохранитьРезультатПроверки(
				Организация, Неопределено, ДатаНачала, ДатаОкончания, Результат);
	...
	Исключение
		...
		Результат = НовыйРезультатРаботы();
		Результат.ЕстьКритическаяОшибка = Истина;
...
//
Функция ВыполнитьПовторнуюПроверкуПараметраКонтроля(ПараметрКонтроля, Организация, СобытиеКалендаря = Неопределено,
	ДатаНачала, ДатаОкончания, ОргиниальноеДеревоРезультата) Экспорт
	...
	Попытка
		...
		Иначе
			...
			Результат = ВыполнитьПроизвольнуюПроверку(
				Организация, Новый Структура(ПараметрКонтроля, Истина), ДатаНачала, ДатаОкончания, , Ложь, Истина);
		...
		Результат.КлючПротоколаПроверки = СохранитьРезультатПроверки(Организация, СобытиеКалендаря, ДатаНачала,
			ДатаОкончания, Результат);
				
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Проверка данных.Ошибка повторной проверки данных'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru= 'При выполнении повторной проверки данных для области данных «%1» по организации «%2» произошла ошибка:
					 |%3'"), РаботаВМоделиСервиса.ЗначениеРазделителяСеанса(), Организация, ОписаниеОшибки()),
			РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
...
//
Процедура ВыполнитьПроверкуСведенийПоОрганизации(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
	ОграниченияПроверки = Неопределено) Экспорт
	
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Реквизиты ИП'");
	...
	Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		...
		ФИОФизическихЛиц = РегистрыСведений.ФИОФизическихЛиц.СрезПоследних(ДатаОкончания, Новый Структура("ФизическоеЛицо", ФизЛицо));
	...
	Если Не ЗначениеЗаполнено(Организация.ТарифНаТравматизм) И (ОграниченияПроверки.Найти("ТарифНаТравматизм")<>Неопределено) 
		И НЕ СистемаНалогообложения.ПрименяетсяАУСН Тогда
	...
	ИначеЕсли (СтрДлина(СокрЛП(Организация.РегистрационныйНомерФСС))<> 10 И СтрДлина(СокрЛП(Организация.РегистрационныйНомерФСС))<> 15)
		И (ОграниченияПроверки.Найти("РегистрационныйНомерФСС")<>Неопределено) Тогда
	...
	Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		...
		Если (ОграниченияПроверки.Найти("Руководитель")<>Неопределено) Тогда
			...
			Если ЗначениеЗаполнено(Организация.ПодписьРуководителя) Тогда
				...
				ДанныеФЛ = РегистрыСведений.ФИОФизическихЛиц.СрезПоследних(ДатаОкончания, Новый Структура("ФизическоеЛицо", Организация.ПодписьРуководителя.ФизическоеЛицо));
			...
			Иначе
				...
				ОшибочныеПоля = Новый Соответствие;
				ОшибочныеПоля.Вставить("Объект.ПодписьРуководителя", НСтр("ru='Не заполнена подпись руководителя'"));
				Строка.Расшифровка.ПараметрыРасшифровки = ОшибочныеПоля;
		...
		Если ЗначениеЗаполнено(Организация.ПодписьГлавногоБухгалтера) Тогда
				...
				ДанныеФЛ = РегистрыСведений.ФИОФизическихЛиц.СрезПоследних(ДатаОкончания, Новый Структура("ФизическоеЛицо", Организация.ПодписьГлавногоБухгалтера.ФизическоеЛицо));
				Если ДанныеФЛ.Количество() > 0 Тогда
					ФИО = ДанныеФЛ[0];
					Если ПустаяСтрока(ФИО.Фамилия) ИЛИ ПустаяСтрока(ФИО.Имя) Тогда
						Строка = КорневаяСтрока.Строки.Добавить();
			...
			Иначе
				...
				ОшибочныеПоля = Новый Соответствие;
				ОшибочныеПоля.Вставить("Объект.ПодписьГлавногоБухгалтера", НСтр("ru='Не заполнена подпись главного бухгалтера'"));
				Строка.Расшифровка.ПараметрыРасшифровки = ОшибочныеПоля;
...
Процедура ВыполнитьПроверкуНаличияПоказателейЕНВД(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	|	И (ВидыДеятельностиЕНВД.Актуально ИЛИ ВидыДеятельностиЕНВД.ДатаОкончания МЕЖДУ &ДатаНачала И &ДатаОкончания)
...
Процедура ВыполнитьПроверкуСведенийПоСотрудникам(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	|		ПО (ФИОФизическихЛиц.ФизическоеЛицо = Сотрудники.Физлицо)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Сотрудники.СотрудникПредставление КАК СотрудникПредставление,
	...
	|	ПаспортныеДанныеФизЛицСрезПоследних.Серия КАК Серия,
	|	ПаспортныеДанныеФизЛицСрезПоследних.Номер КАК Номер,
	...
	Пока ВыборкаСотрудники.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ФизЛицо) Тогда
			...
			Если ПустаяСтрока(ВыборкаСотрудники.Отчество) И (ОграниченияПроверки.Найти("Отчество")<>Неопределено) 
				И ВыборкаСотрудники.Гражданство = Справочники.СтраныМира.Россия Тогда
			...
			Если (ОграниченияПроверки.Найти("АдресПоПрописке")<>Неопределено) Тогда
				...
				Строки = ФизЛицо.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Вид", Справочники.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица));
				Если Строки.Количество() = 0 Тогда
			...
			Если (ОграниченияПроверки.Найти("Индекс")<>Неопределено) Тогда
				...
				Иначе
					СтруктураАдреса = УчетСтраховыхВзносовУНФ.СтруктураПолейАдресаИзСпискаПолейКонтактнойИнформации(Строки[0].ЗначенияПолей, Строки[0].Представление);
			...
			Если (ОграниченияПроверки.Найти("УдостоверениеЛичности")<>Неопределено) Тогда
				...
				Иначе
					...
					Если ПустаяСтрока(ДокументУдЛичности.Серия) И (ДокументУдЛичности.ВидДокумента = Справочники.ВидыДокументовФизическихЛиц.ПаспортРФ 
						ИЛИ ДокументУдЛичности.Физлицо.Гражданство = Справочники.СтраныМира.Россия) Тогда
...
// если она есть, или текущую дату
Процедура ВыполнитьПроверкуТарифаСтраховыхВзносов(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
	ОграниченияПроверки = Неопределено) Экспорт
	...
	Если НЕ РегламентированнаяОтчетностьУСН.ТарифыСтраховыхВзносовЗаполнены(Организация, ДатаПроверки) Тогда
		...
		Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
		Строка.Расшифровка.ТипРасшифровки = "НоваяЗаписьРегистраСведений";
		Строка.Расшифровка.ЗначениеРасшифровки = "РегистрСведений.ПрименяемыеТарифыСтраховыхВзносовУНФ.ФормаЗаписи";
		Строка.Расшифровка.ПараметрыРасшифровки = Новый Структура(
			"ЕстьСобственныеДанныеЗаполнения, СобственныеДанныеЗаполнения",
			Истина,
			Новый Структура("ГоловнаяОрганизация,Период",
				Организация,
				НачалоГода(ДатаПроверки)-1));
...
// Процедура выполняет контроль наличия объектов ЕНВД
//
Процедура ВыполнитьПроверкуТорговыхТочекРозничнойПродажиАлкоголя(ДеревоРезультата, Организация, ДатаНачала,
	ДатаОкончания, ОграниченияПроверки = Неопределено) Экспорт
...
Процедура ВывестиСообщенияОбОшибкахЗаполнения(ПутьКДанным, ПереченьОшибок) Экспорт
	Если Не (ТипЗнч(ПереченьОшибок) = Тип("Соответствие") Или ТипЗнч(ПереченьОшибок) = Тип(
		"ФиксированноеСоответствие")) Тогда
		Возврат;
	КонецЕсли;
...
Функция ПолучитьСтруктуруРезультатаРаботы(ДеревоРезультата)
	...
	Для Каждого СтрокаРезультата Из ДеревоРезультата.Строки Цикл
		...
		Если Результат.ЕстьОшибки И Результат.ЕстьПредупреждения Тогда
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Результат.ДеревоРезультата = ДеревоРезультата;
	
	Возврат Результат;
	
КонецФункции

Функция НовыйРезультатРаботы()
	
	Результат = Новый Структура;
	Результат.Вставить("ЕстьКритическаяОшибка", Ложь);
	Результат.Вставить("ЕстьОшибки", Ложь);
	Результат.Вставить("ЕстьПредупреждения", Ложь);
	Результат.Вставить("ДеревоРезультата");
	Результат.Вставить("КлючПротоколаПроверки");
...
Процедура ОпределитьОграниченияПараметровКонтроляПоЗадаче(Задача, ОграниченияПараметровКонтроля)
	...
	ИначеЕсли Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогиСотрудников 
		ИЛИ Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыЗаСотрудников Тогда
	...
	ИначеЕсли Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СЗВСтаж
		ИЛИ Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ПерсСведенияОФизлицах Тогда
		
		ОграниченияПараметровКонтроля.Добавить("СведенияПоОрганизации");
		ОграниченияПараметровКонтроля.Добавить("СведенияПоСотрудникам");
		
	ИначеЕсли Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Декларация12 
		ИЛИ Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Декларация8 Тогда
		ОграниченияПараметровКонтроля.Добавить("СведенияПоОрганизации");
