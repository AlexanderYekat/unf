...
// Процедура добавляет первую строку расшифровки платежа
//
// Параметры:
//   Объект - ДокументОбъект - Документ в расшифровку которого необходимо добавить строку
//
Процедура ДобавитьПервуюСтрокуРасшифровки(Объект) Экспорт
...
Функция ПолучитьСтруктурыОперацийДенежныеСредства(ВидДокумента) Экспорт
	...
	Если ВидДокумента = "РасходСоСчета" Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ВестиУчетРасчетовСБанкомЭквайрером") Тогда
			Представление = НСтр("ru = 'Отчет эквайера'");
			ДобавитьОписаниеОперации(Операции, "РасходГруппаПрочееБанк",
				Перечисления.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты, Представление); // Эквайринг
		КонецЕсли;
	...
	ИначеЕсли ВидДокумента = "ПоступлениеНаСчет" Тогда
		...
		Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
			ДобавитьОписаниеОперации(Операции, "ПоступлениеГруппаПокупкаВалютыБанк",
				Перечисления.ВидыОперацийПоступлениеНаСчет.ПокупкаВалюты);
			ДобавитьОписаниеОперации(Операции, "ПоступлениеГруппаПродажаВалютыБанк",
				Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОтПродажиВалюты);
		КонецЕсли;
	...
	ИначеЕсли ВидДокумента = "ПоступлениеВКассу" Тогда
		...
		Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
			ДобавитьОписаниеОперации(Операции, "ПоступлениеГруппаПокупкаВалютыКасса",
				Перечисления.ВидыОперацийПоступлениеВКассу.ПокупкаВалюты);
			ДобавитьОписаниеОперации(Операции, "ПоступлениеГруппаПродажаВалютыКасса",
				Перечисления.ВидыОперацийПоступлениеВКассу.ПоступлениеОтПродажиВалюты);
		КонецЕсли;
		...
		Если ПолучитьФункциональнуюОпцию("УчетРозничныхПродаж") Тогда
			...
			Если ПолучитьФункциональнуюОпцию("СуммовойУчетВРознице") Тогда
