...
Функция СведенияОбОбъектеИнформационнойБазы(Тип)
	...
	ИначеЕсли Тип = Тип("СправочникСсылка.Контрагенты") Тогда
		НаборСведений.Вставить("ЮридическоеФизическоеЛицо", Новый ОписаниеТипов("ПеречислениеСсылка.ЮридическоеФизическоеЛицо"));
		НаборСведений.Вставить("ВидКонтрагента",      		Новый ОписаниеТипов("ПеречислениеСсылка.ВидыКонтрагентов"));
	...
	ИначеЕсли Тип = Тип("СправочникСсылка.БанковскиеСчета") Тогда
		...
		НаборСведений.Вставить("СчетБанк",                  Новый ОписаниеТипов("ПланСчетовСсылка.Управленческий"));
	ИначеЕсли Тип = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		НаборСведений.Вставить("ВидДоговора",               Новый ОписаниеТипов("ПеречислениеСсылка.ВидыДоговоров"));
		...
		НаборСведений.Вставить("ВидОплаты",                 Новый ОписаниеТипов("СправочникСсылка.СпособыОплаты"));
	КонецЕсли;
...
Функция РеквизитыНовыхОбъектовИнформационнойБазы(Тип)
	...
	ИначеЕсли Тип = Тип("СправочникСсылка.БанковскиеСчета") Тогда
		...
		НаборРеквизитов.Вставить("СВИФТБИК",           МетаданныеБанки.Реквизиты.СВИФТБИК.Тип);
...
Функция ИсточникиКлючевыхРеквизитовНовыхОбъектовИнформационнойБазы(Тип, Схема)
	...
	ИначеЕсли Схема = "BBAN" Тогда
		Если Тип = Тип("СправочникСсылка.БанковскиеСчета") Тогда
			...
			ИсточникиРеквизитов.Вставить("СВИФТБИК",             "SWIFT");
		КонецЕсли;
...
Функция ОписаниеИдентификатораПоСхеме(СведенияОбОбъектеИнформационнойБазы, УчастникиОпераций, Схема)
	...
	ИначеЕсли Схема = "IBAN" Тогда
		Идентификатор = ИдентификаторПоМеждународномуНомеруБанковскогоСчета(
			УчастникиОпераций,
			СведенияОбОбъектеИнформационнойБазы.НомерБанковскогоСчета,
			СведенияОбОбъектеИнформационнойБазы.СВИФТБИК);
	ИначеЕсли Схема = "BKNM" Тогда
...
Функция ИдентификаторПоНомеруБанковскогоСчета(УчастникиОпераций, НомерБанковскогоСчета, БИК, ИспользуетсяБанкДляРасчетов = Ложь) Экспорт
	...
	ИначеЕсли Не ИспользуетсяБанкДляРасчетов И ЗначениеЗаполнено(БИК) Тогда
		...
		Иначе
			НомерСчетаСоответствуетПравиламЦБР = БанковскиеПравила.ПроверитьКонтрольныйКлючВНомереБанковскогоСчета(
				НомерБанковскогоСчета, БИК, Истина);
		КонецЕсли;
	Иначе
		НомерСчетаСоответствуетПравиламЦБР = Истина;
	КонецЕсли;
	...
	Если Не НомерСчетаСоответствуетПравиламЦБР Тогда
		...
		Если БанковскиеПравила.СвойстваКодаБанка(БИК).ЭтоSWIFT Тогда
			Состав.Вставить("SWIFT", БИК);
		КонецЕсли;
	Иначе
		...
		ИначеЕсли БанковскиеПравила.ЭтоСчетЮридическогоЛица(БалансовыйСчет)
			Или БанковскиеПравила.ЭтоКазначейскийСчет(НомерБанковскогоСчета) Тогда
	...
	Если ТипЗнч(БИК) = Тип("Строка") И Не ПустаяСтрока(БИК) Тогда
		...
		БазовыйИдентификатор = ИдентификаторПоНомеруБанковскогоСчета(
			УчастникиОпераций, НомерБанковскогоСчета, "", НомерСчетаСоответствуетПравиламЦБР);
...
Функция ИдентификаторПоМеждународномуНомеруБанковскогоСчета(УчастникиОпераций, НомерБанковскогоСчета, SWIFT) Экспорт
...
Функция ИдентификаторПоИмениФизическогоЛица(УчастникиОпераций, ИмяФизическогоЛица) Экспорт
	...
	НаименованиеСлужебное = ИмяФизическогоЛица;
	...
	Состав.Вставить("Наименование",     		 НаименованиеСлужебное);
	Состав.Вставить("ЮридическоеФизическоеЛицо", Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо);
...
Функция СвойстваСоставаИдентификатора(Схема)
	...
	Если Схема = "BBAN" Тогда
		...
		ОписаниеСвойства = ИменаСвойств.Добавить();
		ОписаниеСвойства.Имя           = "СВИФТБИК";
		ОписаниеСвойства.Представление = НСтр("ru = 'SWIFT'");
...
Процедура НайтиВИнформационнойБазе(УчастникиОпераций, СведенияОВладельцеБанковскогоСчетаИзКонтекста) Экспорт
	...
	КоличествоБанковскихСчетовКонтекста =
		СведенияОВладельцеБанковскогоСчетаИзКонтекста.БанковскийСчетОрганизации.Количество();
...
Функция ОписаниеМетодаПоискаBBAN()
	...
	|	ЕСТЬNULL(БанковскиеСчета.Владелец.БанковскийСчетПоУмолчанию = БанковскиеСчета.Ссылка, ЛОЖЬ) КАК УстановленОсновным,
	|	БанковскиеСчета.СчетУчета КАК СчетУчета,
	//|	БанковскиеСчета.СчетКорпоративныхРасчетов КАК СчетКорпоративныхРасчетов
	|	ЛОЖЬ КАК СчетКорпоративныхРасчетов
	|ИЗ
	...
	|	"""",
	|	БанковскиеСчета.БанкРасчетов.Код,
	|	БанковскиеСчета.БанкРасчетов.СВИФТБИК,
	|	ВЫБОР
	|		КОГДА БанковскиеСчета.Владелец ССЫЛКА Справочник.ФизическиеЛица
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо)
	|		ИНАЧЕ ЕСТЬNULL(БанковскиеСчета.Владелец.ЮридическоеФизическоеЛицо, ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо))
	...
	|	ЕСТЬNULL(БанковскиеСчета.Владелец.БанковскийСчетПоУмолчанию = БанковскиеСчета.Ссылка, ЛОЖЬ),
	|	NULL,
	|	ЛОЖЬ
	|ИЗ
	|	Справочник.БанковскиеСчета КАК БанковскиеСчета
	...
	|		ПО БанковскиеСчета.БанкРасчетов = Банки.Ссылка
	|ИТОГИ ПО
	|	Идентификатор,
	|	ТипВладельца,
	|	Тип";
...
Функция ОписаниеМетодаПоискаTXID()
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВидыКонтрагентов.ЮридическоеЛицо) КАК ВидКонтрагента,
...
Функция ЗапросМетодаПоискаBKNM(ЗначенияПоискаЗапросом)
	...
	|	Справочник.Контрагенты КАК Контрагенты
	|ГДЕ
	|	Контрагенты.ВидКонтрагента = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо)
	|	И НЕ Контрагенты.ЭтоГруппа
...
Функция ОписаниеМетодаПоискаPNNM()
	...
	"ВЫБРАТЬ
	|	ЗначенияПоискаЗапросом.Идентификатор КАК Идентификатор,
	|	ЗначенияПоискаЗапросом.Наименование КАК Наименование
	|ПОМЕСТИТЬ ЗначенияПоискаЗапросом
	|ИЗ
	|	&ЗначенияПоискаЗапросом КАК ЗначенияПоискаЗапросом
	|
	...
	|	Справочник.ФизическиеЛица КАК ФизическиеЛица
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗначенияПоискаЗапросом КАК ПараметрыПоиска
	|		ПО ФизическиеЛица.Наименование = ПараметрыПоиска.Наименование
	|ГДЕ
	|	НЕ ФизическиеЛица.ЭтоГруппа
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ТИПЗНАЧЕНИЯ(Контрагенты.Ссылка),
	|	Контрагенты.Ссылка,
	|	ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо),
...
Процедура НайтиДоговорыКонтрагентовВИнформационнойБазе(УчастникиОпераций, ОтборКонтрагент = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(СпособыОплаты.Ссылка, НЕОПРЕДЕЛЕНО) КАК ВидОплаты
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СпособыОплаты КАК СпособыОплаты
	|		ПО ДоговорыКонтрагентов.Организация = СпособыОплаты.Организация
	|			И ДоговорыКонтрагентов.Владелец = СпособыОплаты.Эквайрер
	|			И ДоговорыКонтрагентов.Ссылка = СпособыОплаты.Договор
	|ГДЕ
...
Функция ВыбратьСсылку(НайденныеСсылки, УчастникиОпераций, ПриоритетнаяСсылка = Неопределено, ПротоколВыбора = Неопределено, КешИдентификаторов = Неопределено) Экспорт
	...
	// 
	// Приоритет идентификаторов рассчитываем так, чтобы ссылка с большим количеством более надежных идентификаторов
	// получила больший приоритет. Основание для расчета приоритета - максимальное количество идентификаторов во всех
	// ссылках, увеличенное на единицу. Такое основание выбрано для того, чтобы примененные схемы сильнее влияли на выбор,
...
Функция СсылкаСоответствуетОтбору(ОписаниеСсылки, Отбор, СведенияОбОбъектеИнформационнойБазы)
	...
	Для Каждого ЭлементОтбора Из СведенияОбОбъектеИнформационнойБазы Цикл
		...
		Если Отбор[ЭлементОтбора.Ключ] = Неопределено Тогда
			// Если конкретное значение не установлено в отборе, ищем по любому не пустому значению.
			Если Не ЗначениеЗаполнено(ОписаниеСсылки[ЭлементОтбора.Ключ]) Тогда
				Возврат Ложь;
			КонецЕсли;
		Иначе
...
Функция СсылкаНового(Идентификаторы, ЗначенияЗаполнения, Тип, УчастникиОпераций, КешИдентификаторов = Неопределено) Экспорт
	...
	// Сгруппируем идентификаторы по схемам, чтобы надежнее определить ЮридическоеФизическоеЛицо (учесть разницу в
	// надежности схем для конкретного типа)
...
Процедура УстановитьПохожиеИдентификаторыСсылкеНового(ОписаниеСсылки, ОписаниеИдентификатора, УчастникиОпераций)
	...
	// Один и тот же объект, еще не записанный в информационную базу, может быть идентифицирован разными идентификаторами:
	// точным и не точным. При этом, если сейчас объект создан по неточному идентификатору, то он не сможет быть найден по
	// точному и наоборот. Поэтому, дополним идентификаторы объекта похожими.
	//
...
Функция СоздатьЭлемент(ОписаниеНовогоОбъекта, ОписаниеСсылки, ШаблонДанныхЗаполнения, Ошибки, МенеджерОбъекта, ТипСсылки, КешОбработкиЗаполнения)
	...
	Если ТипСсылки = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		...
		Иначе
			...
			Попытка
				...
				ШаблонТекстаОшибки =
					НСтр("ru = 'При перечислении %1 элемент справочника Сотрудники ""%2"" должен быть создан заранее'");
				ВидыОпераций = НСтр("ru = 'зарплаты или подотчетнику'");
				ТекстОшибки = СтрШаблон(ШаблонТекстаОшибки, ВидыОпераций, ОписаниеНовогоОбъекта.Наименование);
	...
	// Отключим создание договора при создании контрагента
	Если МенеджерОбъекта = Справочники.Контрагенты Тогда
		НовыйОбъект.ДополнительныеСвойства.Вставить("ПропуститьСозданиеДоговора", Истина);	
	КонецЕсли;	
	
	Если МенеджерОбъекта = Справочники.ДоговорыКонтрагентов Тогда
		НовыйОбъект.Наименование = ШаблоныНаименований.НаименованиеДоговораПоУмолчанию(НовыйОбъект);
	КонецЕсли;	
	...
	Если МенеджерОбъекта = Справочники.ДоговорыКонтрагентов Тогда
		Справочники.ДоговорыКонтрагентов.ЗаписатьДоговорПоУмолчанию(НовыйОбъект);
	КонецЕсли;	
...
// Создаваемые элементы справочников могут содержать ссылки на подчиненных: БанковскийСчетПоУмолчанию.
...
Функция МетаданныеСсылкиНаПодчиненных()
	...
	РеквизитыКонтрагента.Вставить("БанковскийСчетПоУмолчанию",     Метаданные.Справочники.Контрагенты.Реквизиты.БанковскийСчетПоУмолчанию.Тип);
	...
	РеквизитыФизическогоЛица.Вставить("БанковскийСчетПоУмолчанию", Метаданные.Справочники.ФизическиеЛица.Реквизиты.БанковскийСчетПоУмолчанию.Тип);
