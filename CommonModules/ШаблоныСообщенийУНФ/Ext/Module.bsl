...
Процедура ПриОпределенииНастроек(Настройки) Экспорт
	...
	ОтключаемыеПредметыПоФО.Вставить(Метаданные.Документы.ЗаказНаПроизводство.ПолноеИмя(),
		"ИспользоватьПодсистемуПроизводство");
	ОтключаемыеПредметыПоФО.Вставить(Метаданные.Документы.ПриемИПередачаВРемонт.ПолноеИмя(), "ИспользоватьРемонты");
	ОтключаемыеПредметыПоФО.Вставить(Метаданные.Справочники.РабочиеМеста.ПолноеИмя(),
	...
	Предмет = Настройки.ПредметыШаблонов.Добавить();
	Предмет.Имя = "ПодтверждениеПоSMS";
	Предмет.Представление = НСтр("ru = 'Подтверждение по SMS'");
	Предмет.Макет = "";
...
Процедура ПриПодготовкеШаблонаСообщения(Реквизиты, Вложения, НазначениеШаблона, ДополнительныеПараметры) Экспорт
	...
	Если НазначениеШаблона = "ДоговорыКонтрагентов" Тогда
		...
		ШаблоныСообщений.СформироватьСписокРеквизитовПоСКД(Реквизиты, Справочники.ДоговорыКонтрагентов.ПолучитьМакет(
			"СКД_ДанныеШаблонаСообщений"));
	...
	Если НазначениеШаблона = "НачислениеСписаниеБонусов" Тогда
		ШаблоныСообщений.СформироватьСписокРеквизитовПоСКД(Реквизиты, Справочники.ДисконтныеКарты.ПолучитьМакет(
			"СКД_ДанныеШаблонаСообщений"));
	КонецЕсли;
	...
	Справочники.ЗадачиАссистентаПоНапоминаниямОЗаписи.ПриПодготовкеШаблонаСообщения(Реквизиты, Вложения,
		НазначениеШаблона, ДополнительныеПараметры);
	ДополнительныеПараметры.Вставить("ФорматыРеквизитов", Новый Соответствие);
	
	Если НазначениеШаблона = "ПодтверждениеПоSMS" Тогда
		
		Реквизит = Реквизиты.Добавить();
		Реквизит.Имя = "КодПодтверждения";
		Реквизит.Подсказка = "";
		Реквизит.ПолноеПредставление = "";
		Реквизит.Представление = НСтр("ru = 'Код подтверждения'");
		Реквизит.ПроизвольныйПараметр = Ложь;
		Реквизит.Тип = Новый ОписаниеТипов("Строка");
		Реквизит.Формат = "";
		
	КонецЕсли;
	
	ДобавитьПараметрыСКД(ДополнительныеПараметры);
...
Процедура ПриФормированииСообщения(Сообщение, НазначениеШаблона, ПредметСообщения, ПараметрыШаблона) Экспорт
	...
	Справочники.ЗадачиАссистентаПоНапоминаниямОЗаписи.ПриФормированииСообщения(Сообщение, НазначениеШаблона,
		ПредметСообщения, ПараметрыШаблона);
	...
	Если НазначениеШаблона = "ПодтверждениеПоSMS" Тогда
		...
		ПараметрыСообщения = Сообщение.ДополнительныеПараметры.ПараметрыСообщения;
		Сообщение.ЗначенияРеквизитов.Вставить("КодПодтверждения", ПараметрыСообщения.КодПодтверждения);
...
//    * ПроизвольныеПараметры - Соответствие из Произвольный - заполненный список произвольных параметров;
...
Процедура ПриЗаполненииТелефоновПолучателейВСообщении(ПолучателиSMS, НазначениеШаблона, ПредметСообщения) Экспорт
	...
	Если ЗначениеЗаполнено(ВыбранныйПолучатель) Тогда
		ЗаполнитьВыбранногоПолучателя(ПолучателиSMS, Предмет, ВыбранныйПолучатель, 
			Перечисления.ТипыКонтактнойИнформации.Телефон, 
			ВыбранныйВидКИ);
	Иначе
		ЗаполнитьПолучателейSMSПоУмолчанию(ПолучателиSMS, Предмет);
	КонецЕсли;
...
Процедура ПриЗаполненииПочтыПолучателейВСообщении(ПолучателиПисьма, НазначениеШаблона, ПредметСообщения) Экспорт
	...
	Если ТипЗнч(ПредметСообщения) = Тип("Структура") Тогда
		...
		Если НЕ ПредметСообщения.Свойство("ПараметрыСообщения") 
			ИЛИ ТипЗнч(ПредметСообщения.ПараметрыСообщения) <> Тип("Структура") Тогда
			ПредметСообщения.Вставить("ПараметрыСообщения", Новый Структура);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыбранныйПолучатель) Тогда
		ЗаполнитьВыбранногоПолучателя(ПолучателиПисьма, Предмет, ВыбранныйПолучатель,
			Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты, ВыбранныйВидКИ);
	Иначе
...
Процедура ЗаполнитьВыбранногоПолучателя(ПолучателиПисьма, ПредметСообщения, ИмяРеквизита, ТипКонтактнойИнформации,
	...
	Если ТипЗнч(ПолучательИзОбъекта) = Тип("СправочникСсылка.Сотрудники") Тогда
		ПолучательИзОбъекта = РегистрыСведений.СотрудникиПользователя.ПолучитьПользователяПоСотруднику(
			ПолучательИзОбъекта);
	КонецЕсли;
	...
	КонтактнойИнформация = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(ОбъектыКонтактнойИнформации,
		ТипКонтактнойИнформации, ВидКонтактнойИнформации, ТекущаяДатаСеанса());
...
// Заполняет получателей по умолчанию
//
// Параметры:
...
// Заполняет получателей SMS по-умолчанию
//
// Параметры:
//  ПолучателиSMS - ТаблицаЗначений - список получателей сообщения SMS
//  ПредметСообщения - Произвольный - родитель, у которого есть реквизиты, содержащие контактную информацию
//
Процедура ЗаполнитьПолучателейSMSПоУмолчанию(ПолучателиSMS, ПредметСообщения) Экспорт
	
	МассивПолучателей = ЭлектроннаяПочтаУНФ.ПодготовленныеНомераТелефоновПолучателей(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПредметСообщения)
	);
	
	Для Каждого Получатель Из МассивПолучателей Цикл
		ПолучательПисьма = ПолучателиSMS.Добавить();
		ПолучательПисьма.НомерТелефона = Получатель.Телефон;
		ПолучательПисьма.Представление = Строка(Получатель.Представление);
		ПолучательПисьма.Контакт       = Получатель.ИсточникКонтактнойИнформации;
	КонецЦикла;
	
КонецПроцедуры

// Есть доступные шаблоны
//
// Параметры:
//  ДляПисем - Булево
//  Предмет  - Произвольный - объект, у которого есть реквизиты, содержащие контактную информацию
// 
// Возвращаемое значение:
//  Булево - признак наличия доступных шаблонов
//
Функция ЕстьДоступныеШаблоны(ДляПисем, Предмет = Неопределено) Экспорт
	...
	Если Не ДляПисем Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ШаблоныСообщений.ПредназначенДляЭлектронныхПисем",
			"ШаблоныСообщений.ПредназначенДляSMS");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Предмет) Тогда
		Запрос.Текст = Запрос.Текст
			+ " И (ШаблоныСообщений.ПредназначенДляВводаНаОсновании = ЛОЖЬ ИЛИ ШаблоныСообщений.ПолноеИмяТипаПараметраВводаНаОсновании = &ПолноеИмяТипаПредмета)";
...
// Возвращает уникальный идентификатор вложения HTML в виде строки
// 
// Возвращаемое значение:
//  Строка - строковое представление уникального идентификатора вложения HTML
//
...
//  КомандыПечати		 - Массив из Строка - Уникальные идентификаторы выбранных печатных форм
//  Вложения			 - Массив из Структура
// 
// Возвращаемое значение:
//   СправочникСсылка.ШаблоныСообщений
//
Функция СоздатьШаблонEmail(НаименованиеШаблона, ПараметрыШаблона, КомандыПечати = Неопределено,
...
Процедура ДобавитьПараметрыСКД(ДополнительныеПараметры)
	ДополнительныеПараметры.ПараметрыСКД.Вставить("Обращение", НСтр("ru = 'Уважаемый (ая)'"));
	ДополнительныеПараметры.ПараметрыСКД.Вставить("ОбращениеПолЖенский", НСтр("ru = 'Уважаемая'"));
	ДополнительныеПараметры.ПараметрыСКД.Вставить("ОбращениеПолМужской", НСтр("ru = 'Уважаемый'"));
КонецПроцедуры
...
Функция НомерДокументаНаПечать(ДокументСсылка)
	...
	ДанныеДокументаОрганизация = ДанныеДокумента.Организация; // СправочникСсылка.Организации
	ПрефиксОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеДокументаОрганизация, "Префикс");
