...
Процедура СохранитьНовыеПравилаВыгрузки(Приложение, НовыеПравила) Экспорт
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		ИнтеграцияКабинетСотрудника.ПроверитьОбновитьИсключенияИзПравил();
	КонецЕсли;
...
Функция ОбъектыДляРегистрацииПриЗаписиНовыхПравил(Приложение, ДанныеДляРегистрации)
	...
	Для каждого ТипОбъекта Из ТипыОбъектов Цикл
		...
		// Добавляем уже зарегистрированные изменения, кроме новых.
		ТекущиеИзменения = ТекущиеИзмененияОбъектов.Скопировать(Отбор).ВыгрузитьКолонку("Ссылка");
		СтарыеИзменения = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ТекущиеИзменения, ВыгружаемыеОбъектыПоТипу);
...
Функция БудущиеСобытияПриложения(Приложение, Сотрудники, БудущиеСобытияСотрудников, Событие) Экспорт
	...
	|	И БудущиеСобытия.Сотрудник В(&Сотрудники)";
	СотрудникиДляУдаления = ОбщегоНазначенияБЗККлиентСервер.МассивВСоответствие(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Сотрудник"));
	...
	Для каждого Сотрудник Из Сотрудники Цикл
		Если СотрудникиДляУдаления[Сотрудник] <> Неопределено Тогда
			НоваяСтрока = БудущиеСобытия.Добавить();
			НоваяСтрока.Приложение 	= Приложение;
			НоваяСтрока.Событие 	= Событие;
...
Функция ДанныеФизическихЛиц(ФизическиеЛица) Экспорт
...
Функция ДанныеСотрудников(Сотрудники) Экспорт
	...
	Возврат ИнтеграцияУправлениеПерсоналомВнутренний.ДанныеСотрудников(Сотрудники);
...
Функция ДанныеОбОтпусках(Сотрудники, СотрудникиФизическихЛиц) Экспорт
	...
	Возврат ИнтеграцияУправлениеПерсоналомСлужебный.ДанныеОбОтпусках(Сотрудники, СотрудникиФизическихЛиц);
...
Функция ОшибкиЗаполненияФизическихЛиц(ФизическиеЛица, Приложения)
	...
	Данные = ДанныеФизическихЛиц(ФизическиеЛица);
...
Функция ОшибкиЗаполненияСотрудников(Сотрудники, ОшибкиФизическихЛиц, ОшибкиПозицийШР, Приложения)
	...
	ДанныеСотрудников = ДанныеСотрудников(Сотрудники);
...
Функция ФизическиеЛицаИсключения(Приложение)
	
	ФизическиеЛицаИсключения = Новый Соответствие;
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		ФизическиеЛицаИсключения = ИнтеграцияКабинетСотрудника.ФизическиеЛицаИсключения();
	КонецЕсли;
...
Функция ТипОбъектаВыгружаетсяПоПравилам(ТипОбъекта)
	...
	Если ТипОбъекта = ПеречислениеМенеджер.ФизическоеЛицо
		...
		Или ТипОбъекта = ПеречислениеМенеджер.МашиночитаемаяДоверенностьКЭДО
...
Функция ТипОбъектаТипыЗначений()
	...
	ТипОбъектаТипЗначения.Вставить(ПеречислениеМенеджер.МашиночитаемаяДоверенностьКЭДО,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.МашиночитаемыеДоверенностиКЭДО")));
...
Функция ОчиститьДанныеПриОтключенииПриложения(Приложение) Экспорт
...
Функция ОбновлятьРуководителейПриСменеРуководителяОрганизации() Экспорт
...
Функция НастройкиПодключения(Приложение) Экспорт
	...
	НастройкиПодключения = Новый Структура("АдресПриложения,ВерсияDTO,ВерсияAPI");
...
Функция ТипОбъектаТипЗначенияДляПубличныхИдентификаторов() Экспорт
	...
	ТипОбъектаТипЗначения.Вставить(ПеречислениеМенеджер.МашиночитаемаяДоверенностьКЭДО, 	Тип("СправочникСсылка.МашиночитаемыеДоверенностиКЭДО"));
...
#Область ПроверкаАктивныхФоновыхЗаданий

// Выполняет проверку наличия активных фоновых заданий,
// регламентное задание или обмен, запущенный пользователем интерактивно.
//
Функция ЕстьАктивныеФоновыеЗаданияОбмен(Приложение) Экспорт
	
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом Тогда
		Возврат Интеграция1СПерсонал.ЕстьАктивныеФоновыеЗаданияОбмен();
	Иначе
		Возврат ИнтеграцияКабинетСотрудника.ЕстьАктивныеФоновыеЗаданияОбмен();
	КонецЕсли;
